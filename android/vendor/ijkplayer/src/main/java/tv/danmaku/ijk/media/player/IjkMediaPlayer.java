...
public final class IjkMediaPlayer extends AbstractMediaPlayer {
    ...
    private static final int MEDIA_EXIT_READ = 1000;
    private static final int MEDIA_BUFFERING_UPDATE = 3;
    private static final int MEDIA_SEEK_COMPLETE = 4;
    private static final int MEDIA_SET_VIDEO_SIZE = 5;
    private static final int MEDIA_PLAY_PROGRESS = 6;
    private static final int MEDIA_PLAY_STREAM_UNIX_TIME = 7;
    ...
    public static final int FFP_PROP_INT64_VIDEO_BITRATE                    = 20101;
    public static final int FFP_PROP_INT64_AUDIO_BITRATE                    = 20102;

    private IMediaPlayerPresentBufferListener mPresentBufferListener = null;
    //----------------------------------------
    ...
    public void setMediaPlayerPresentBufferListener(IMediaPlayerPresentBufferListener listener) {
        mPresentBufferListener = listener;
    }

    public void enableMediaDecoder(boolean enable) {
        mUseMediaCodec = enable;
    }
    ...
    /*
     * @param multiUrl play multipul video source, must has emmul prefix and url will segprate by '\n'
    */
    public void setMultiDataSource(String multiUrl)
        throws IOException, IllegalArgumentException, SecurityException, IllegalStateException {
        _setMultiDataSource(multiUrl, null, null);
    }
    ...
    private native void _setMultiDataSource(String path, String[] keys, String[] values)
            throws IOException, IllegalArgumentException, SecurityException, IllegalStateException;
    ...
    public void prepareAsync() throws IllegalStateException {
        if (mUseMediaCodec) {
            setOption(OPT_CATEGORY_PLAYER,"mediacodec-all-videos", 1);
        }else {
            setOption(OPT_CATEGORY_PLAYER,"mediacodec-all-videos", 0);
        }
        _prepareAsync();
    }
    ...
    public void standby() throws IllegalStateException {
        stayAwake(false);
        _standby();
    }
    ...
    private native void _standby() throws IllegalStateException;

    public native void _setMute(int mute);
    ...
    public long getVideoBitrate() {
        return _getPropertyLong(FFP_PROP_INT64_VIDEO_BITRATE, 0);
    }

    public long getAudioBitrate() {
        return _getPropertyLong(FFP_PROP_INT64_AUDIO_BITRATE, 0);
    }
    ...
    public native int _changeVideoSource(String path, int playType);

    public native int _changeMultiVideoSource(String path, int playType);

    public native int _prepareNewVideoSource(String path, int playType);

    public native int _changeVideoSourceWithPreparedIndex(int index);

    public native int _deletePreparedVideoSource(int index);
    ...
    private native void _set_record_status(int record_on);

    @Override
    public void setRecordStatus(boolean recordOn){
        _set_record_status(recordOn ? 1 : 0);
    }
    ...
    private static class EventHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case MEDIA_SET_VIDEO_SIZE:
                ...
                return;
            case MEDIA_PLAY_PROGRESS:
                player.onPlayProgress(msg.arg1, msg.arg2);
                return;
            case MEDIA_PLAY_STREAM_UNIX_TIME:
                long time = msg.arg1 < 0 ?  (Integer.MAX_VALUE + (long)msg.arg1 - Integer.MIN_VALUE) : msg.arg1;
                time <<= 32;
                long time1 = msg.arg2 < 0 ?  (Integer.MAX_VALUE + (long)msg.arg2 - Integer.MIN_VALUE) : msg.arg2;
                time += time1;
                player.onPlayStreamUnixTime(time);
                break;
            case MEDIA_ERROR:
            ...
            case MEDIA_NOP: // interface test message - ignore
                break;
            case MEDIA_EXIT_READ:
                player.notifyOnInfo(IMediaPlayer.MEDIA_INFO_EXIT_READ, 0);
                break;
    ...
    @CalledByNative
    private static void postPresentBufferFromNative(Object weakThiz, int colorFormat,
                                             byte[] frame, int width, int height)
    {
        ...
        if (mp.mPresentBufferListener != null) {
            mp.mPresentBufferListener.onDrawFrameBuffer(colorFormat, frame, width, height);
        }
    }

    @CalledByNative
    private  static void postPresentAudioBufferFromNative(Object weakThiz, byte[] frame, int length, int mspts, int sr, int channels){
        if(weakThiz == null){
            return;
        }
        ...
        if(mp.mPresentBufferListener != null){
            mp.mPresentBufferListener.onPostAudioFrameBuffer(frame, length, mspts, sr, channels);
        }
    }
    ...
    private static int ipA2N(String ip) {
        String[] fract = ip.split("\\.");
        if (fract.length != 4) {
            Log.e(TAG, "invalid ip address："+fract.length);
            return 0;
        }
        int ret = 0;
        ret |= ((Integer.parseInt(fract[0]) << 24) & 0xFF000000);
        ret |= ((Integer.parseInt(fract[1]) << 16) & 0xFF0000);
        ret |= ((Integer.parseInt(fract[2]) << 8) & 0xFF00);
        ret |= (Integer.parseInt(fract[3]) & 0xFF);
        return ret;
    }
    ...
    private static boolean onNativeInvoke(Object weakThiz, int what, Bundle args) {
        ...
        switch (what) {
            ...
            case OnNativeInvokeListener.CTRL_DID_TCP_OPEN:
            case OnNativeInvokeListener.CTRL_WILL_TCP_OPEN: {
                String ip = args.getString(OnNativeInvokeListener.ARG_IP);
                int ipN = ipA2N(ip);
                player.notifyOnInfo(IMediaPlayer.MEDIA_INFO_SERVER_IP_CHANGED, ipN);
            }
            default:
                return false;
        }
    ...
    private native void _setPlayType(int playType);

    public void setPlayType(int playType) {
        _setPlayType(playType);
    }
    ...
    public native void _setPlayAudioChannelMode(int mode);
