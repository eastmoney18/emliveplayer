...
void ijkmeta_set_int64_l(IjkMediaMeta *meta, const char *name, int64_t value)
{
    ...
    av_em_dict_set_int(&meta->dict, name, value, 0);
}
...
void ijkmeta_set_string_l(IjkMediaMeta *meta, const char *name, const char *value)
{
    ...
    av_em_dict_set(&meta->dict, name, value, 0);
}

static int64_t get_bit_rate(AVEMCodecParameters *codecpar)
{
    int64_t bit_rate;
    int bits_per_sample;
    ...
    switch (codecpar->codec_type) {
        ...
        case AVMEDIA_TYPE_AUDIO:
            bits_per_sample = av_em_get_bits_per_sample(codecpar->codec_id);
...
void ijkmeta_set_avformat_context_l(IjkMediaMeta *meta, AVEMFormatContext *ic)
{
    if (!meta || !ic)
        return;
    ...
    for (int i = 0; i < ic->nb_streams; i++) {
        ...
        const char *codec_name = avcodec_em_get_name(codecpar->codec_id);
        if (codec_name)
            ijkmeta_set_string_l(stream_meta, IJKM_KEY_CODEC_NAME, codec_name);
        if (codecpar->profile != FF_PROFILE_UNKNOWN) {
            const AVEMCodec *codec = avcodec_em_find_decoder(codecpar->codec_id);
            if (codec) {
                ijkmeta_set_int64_l(stream_meta, IJKM_KEY_CODEC_PROFILE_ID, codecpar->profile);
                const char *profile = av_em_get_profile_name(codec, codecpar->profile);
                if (profile)
                ...
                if (codecpar->format != AV_PIX_FMT_NONE)
                    ijkmeta_set_string_l(stream_meta, IJKM_KEY_CODEC_PIXEL_FORMAT, av_em_get_pix_fmt_name(codecpar->format));
            }
        }
...
const char *ijkmeta_get_string_l(IjkMediaMeta *meta, const char *name)
{
    ...
    AVEMDictionaryEntry *entry = av_em_dict_get(meta->dict, name, NULL, 0);
    if (!entry)
        return NULL;
