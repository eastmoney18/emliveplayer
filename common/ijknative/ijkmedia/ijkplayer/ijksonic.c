...
float emsonicGetSpeed(
    emsonicStream stream)
{
    return stream->speed;
}

/* Set the speed of the stream. */
void emsonicSetSpeed(
    emsonicStream stream,
    float speed)
{
    stream->speed = speed;
...
/* Create a sonic stream.  Return NULL only if we are out of memory and cannot
   allocate the stream. */
emsonicStream emsonicCreateStream(
    int sampleRate,
    int numChannels)
{
    emsonicStream stream = (emsonicStream)calloc(1, sizeof(struct emsonicStreamStruct));
...
int emsonicReadFloatFromStream(
    emsonicStream stream,
    float *samples,
    int maxSamples)
{
...
/* Read short data out of the stream.  Sometimes no data will be available, and zero
   is returned, which is not an error condition. */
int emsonicReadShortFromStream(
    emsonicStream stream,
    short *samples,
    int maxSamples)
{
...
/* Read unsigned char data out of the stream.  Sometimes no data will be available, and zero
   is returned, which is not an error condition. */
int emsonicReadUnsignedCharFromStream(
    emsonicStream stream,
    unsigned char *samples,
    int maxSamples)
{
    ...
    emsonicStream stream)
{
    ...
    if(!emsonicWriteShortToStream(stream, NULL, 0)) {
        return 0;
    }
...
/* Simple wrapper around emsonicWriteFloatToStream that does the short to float
   conversion for you. */
int emsonicWriteShortToStream(
    emsonicStream stream,
    short *samples,
    int numSamples)
{
...
/* Simple wrapper around emsonicWriteFloatToStream that does the unsigned char to float
   conversion for you. */
int emsonicWriteUnsignedCharToStream(
    emsonicStream stream,
    unsigned char *samples,
    int numSamples)
{
...
int emsonicChangeFloatSpeed(
    ...
    int numChannels)
{
    emsonicStream stream = emsonicCreateStream(sampleRate, numChannels);

    emsonicSetSpeed(stream, speed);
    emsonicSetPitch(stream, pitch);
    emsonicSetRate(stream, rate);
    emsonicSetVolume(stream, volume);
    emsonicSetChordPitch(stream, useChordPitch);
    emsonicWriteFloatToStream(stream, samples, numSamples);
    emsonicFlushStream(stream);
    numSamples = emsonicSamplesAvailable(stream);
    emsonicReadFloatFromStream(stream, samples, numSamples);
    emsonicDestroyStream(stream);
    return numSamples;
}

/* This is a non-stream oriented interface to just change the speed of a sound sample */
int emsonicChangeShortSpeed(
    short *samples,
    int numSamples,
    float speed,
    float pitch,
    ...
    int numChannels)
{
    ...
    emsonicSetVolume(stream, volume);
    emsonicSetChordPitch(stream, useChordPitch);
    emsonicWriteShortToStream(stream, samples, numSamples);
    emsonicFlushStream(stream);
    numSamples = emsonicSamplesAvailable(stream);
    emsonicReadShortFromStream(stream, samples, numSamples);
    emsonicDestroyStream(stream);
    return numSamples;
}
