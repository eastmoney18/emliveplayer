...
typedef struct IJKFF_Pipeline_Opaque {
    ...
    bool         (*mediacodec_select_callback)(void *opaque, emmp_mediacodecinfo_context *mcc);
...
static IJKFF_Pipenode *func_open_video_decoder(IJKFF_Pipeline *pipeline, FFPlayer *ffp)
{
    ...
    ALOGI("medacodec-avc:%d", ffp->mediacodec_avc);
    if (ffp->mediacodec_all_videos || ffp->mediacodec_avc || ffp->mediacodec_hevc || ffp->mediacodec_mpeg2) {
        ALOGI("create video decoder from android mediacodec");
        node = ffpipenode_create_video_decoder_from_android_mediacodec(ffp, pipeline, opaque->weak_vout);
    }
    if (!node) {
        ALOGI("create video decoder from ffplay");
...
void ffpipeline_set_mediacodec_select_callback(IJKFF_Pipeline* pipeline, bool (*callback)(void *opaque, emmp_mediacodecinfo_context *mcc), void *opaque)
{
    ALOGD("%s\n", __func__);
    if (!check_ffpipeline(pipeline, __func__))
        return;
...
bool ffpipeline_select_mediacodec_l(IJKFF_Pipeline* pipeline, emmp_mediacodecinfo_context *mcc)
{
    ALOGD("%s\n", __func__);
    if (!check_ffpipeline(pipeline, __func__))
        return false;
