...
typedef struct IJKFF_Pipenode_Opaque {
    ...
    emmp_mediacodecinfo_context mcc;
    ...
    AVEMCodecContext           *avctx; // not own
...
static SDL_AMediaCodec *create_codec_l(JNIEnv *env, IJKFF_Pipenode *node)
{
    ...
    emmp_mediacodecinfo_context *mcc      = &opaque->mcc;
    SDL_AMediaCodec              *acodec   = NULL;

    if (opaque->jsurface == NULL) {
        // we don't need real codec if we don't have a surface
        ALOGI("no surface to render, create mediacodec dummy");
        acodec = SDL_AMediaCodecDummy_create();
    } else {
        ALOGI("create mediacodec, surface addr:%p, codec name:%s", opaque->jsurface, mcc->codec_name);
...
static int recreate_format_l(JNIEnv *env, IJKFF_Pipenode *node)
{
    ...
    if (opaque->codecpar->extradata && opaque->codecpar->extradata_size > 0) {
        if ((opaque->codecpar->codec_id == AV_CODEC_ID_H264 && opaque->codecpar->extradata[0] == 1)
            ...
            opaque->orig_extradata = (uint8_t*) av_em_mallocz(opaque->codecpar->extradata_size + FF_INPUT_BUFFER_PADDING_SIZE);
...
static int reconfigure_codec_l(JNIEnv *env, IJKFF_Pipenode *node, jobject new_surface)
{
    ...
    if (opaque->weak_vout && opaque->weak_vout->opaque){
        SDL_VoutAndroid_decreaseReference(opaque->weak_vout);
    }
    ...
    ALOGI("%s:new acodec: %p, surface: %p\n", __func__, opaque->acodec, opaque->jsurface);
    ...
    ALOGI("%s:new acodec: %p, surface: %p,testtesttest\n", __func__, opaque->acodec, opaque->jsurface);
    ...
    SDL_AMediaCodecBufferInfo *buffer_info)
{
    ...
    frame->pts    = av_em_rescale_q(buffer_info->presentationTimeUs, AV_TIME_BASE_Q, is->video_st->time_base);
    if (frame->pts < 0)
    ...
    if (!d->packet_pending || d->queue->serial != d->pkt_serial) {
        ...
        if (opaque->ffp->mediacodec_handle_resolution_change &&
            ...
            if (size_data && size_data_size >= 7) {
                ...
                const AVEMCodec  *codec      = opaque->decoder->avctx->codec;
                AVEMCodecContext *new_avctx  = avcodec_em_alloc_context3(codec);
                if (!new_avctx)
                    return AVERROR(ENOMEM);

                avcodec_em_parameters_to_context(new_avctx, opaque->codecpar);
                ...
                ret = avcodec_em_open2(new_avctx, codec, &codec_opts);
                ...
                if (got_picture) {
                    if (opaque->codecpar->width  != new_avctx->width &&
                        ...
                        avcodec_em_parameters_from_context(opaque->codecpar, new_avctx);
        ...
        if (opaque->codecpar->codec_id == AV_CODEC_ID_H264 || opaque->codecpar->codec_id == AV_CODEC_ID_HEVC) {
            ...
            if (time_stamp > 0) {
                time_stamp = av_em_rescale_q(time_stamp, is->video_st->time_base, AV_TIME_BASE_Q);
            } else {
                time_stamp = 0;
            }
        }
...
static double pts_from_buffer_info(IJKFF_Pipenode *node, SDL_AMediaCodecBufferInfo *buffer_info)
{
    ...
    int64_t amc_pts = av_em_rescale_q(buffer_info->presentationTimeUs, AV_TIME_BASE_Q, is->video_st->time_base);
    double pts = amc_pts < 0 ? NAN : amc_pts * av_em_q2d(tb);
...
static void func_destroy(IJKFF_Pipenode *node)
{
...
#if AMC_USE_AVBITSTREAM_FILTER
    av_em_freep(&opaque->orig_extradata);
    ...
    avcodec_em_parameters_free(&opaque->codecpar);

    JNIEnv *env = NULL;
    if (JNI_OK == SDL_JNI_SetupThreadEnv(&env)) {
        if (opaque->jsurface != NULL) {
            SDL_JNI_DeleteGlobalRefP(env, &opaque->jsurface);
            ALOGE("%s: SDL_JNI_DeleteGlobalRefP opaque->jsurface \n", __func__);
        }
    }
}
...
static int func_run_sync(IJKFF_Pipenode *node)
{
    ...
    while (!q->abort_request) {
        ...
        if (got_frame) {
            ...
            ret = ffp_queue_picture(ffp, frame, pts, duration, av_em_frame_get_pkt_pos(frame), is->viddec.pkt_serial);
            if (ret) {
                if (frame->opaque)
                    SDL_VoutAndroid_releaseBufferProxyP(opaque->weak_vout, (SDL_AMediaCodecBufferProxy **)&frame->opaque, false);
            }
...
IJKFF_Pipenode *ffpipenode_create_video_decoder_from_android_mediacodec(FFPlayer *ffp, IJKFF_Pipeline *pipeline, SDL_Vout *vout)
{
    ...
    opaque->codecpar = avcodec_em_parameters_alloc();
    if (!opaque->codecpar)
        goto fail;

    ret = avcodec_em_parameters_from_context(opaque->codecpar, opaque->decoder->avctx);
    if (ret)
        goto fail;
