...
struct emsonicStreamStruct;
typedef struct emsonicStreamStruct *emsonicStream;
...
emsonicStream emsonicCreateStream(int sampleRate, int numChannels);
/* Destroy the sonic stream. */
void emsonicDestroyStream(emsonicStream stream);
...
int emsonicWriteFloatToStream(emsonicStream stream, float *samples, int numSamples);
/* Use this to write 16-bit data to be speed up or down into the stream.
   Return 0 if memory realloc failed, otherwise 1 */
int emsonicWriteShortToStream(emsonicStream stream, short *samples, int numSamples);
/* Use this to write 8-bit unsigned data to be speed up or down into the stream.
   Return 0 if memory realloc failed, otherwise 1 */
int emsonicWriteUnsignedCharToStream(emsonicStream stream, unsigned char *samples, int numSamples);
...
int emsonicReadFloatFromStream(emsonicStream stream, float *samples, int maxSamples);
/* Use this to read 16-bit data out of the stream.  Sometimes no data will
   be available, and zero is returned, which is not an error condition. */
int emsonicReadShortFromStream(emsonicStream stream, short *samples, int maxSamples);
/* Use this to read 8-bit unsigned data out of the stream.  Sometimes no data will
   be available, and zero is returned, which is not an error condition. */
int emsonicReadUnsignedCharFromStream(emsonicStream stream, unsigned char *samples, int maxSamples);
/* Force the sonic stream to generate output using whatever data it currently
   has.  No extra delay will be added to the output, but flushing in the middle of
   words could introduce distortion. */
int emsonicFlushStream(emsonicStream stream);
/* Return the number of samples in the output buffer */
int emsonicSamplesAvailable(emsonicStream stream);
/* Get the speed of the stream. */
float emsonicGetSpeed(emsonicStream stream);
/* Set the speed of the stream. */
void emsonicSetSpeed(emsonicStream stream, float speed);
/* Get the pitch of the stream. */
float emsonicGetPitch(emsonicStream stream);
/* Set the pitch of the stream. */
void emsonicSetPitch(emsonicStream stream, float pitch);
/* Get the rate of the stream. */
float emsonicGetRate(emsonicStream stream);
/* Set the rate of the stream. */
void emsonicSetRate(emsonicStream stream, float rate);
/* Get the scaling factor of the stream. */
float emsonicGetVolume(emsonicStream stream);
/* Set the scaling factor of the stream. */
void emsonicSetVolume(emsonicStream stream, float volume);
/* Get the chord pitch setting. */
int emsonicGetChordPitch(emsonicStream stream);
/* Set chord pitch mode on or off.  Default is off.  See the documentation
   page for a description of this feature. */
void emsonicSetChordPitch(emsonicStream stream, int useChordPitch);
/* Get the quality setting. */
int emsonicGetQuality(emsonicStream stream);
/* Set the "quality".  Default 0 is virtually as good as 1, but very much faster. */
void emsonicSetQuality(emsonicStream stream, int quality);
/* Get the sample rate of the stream. */
int emsonicGetSampleRate(emsonicStream stream);
/* Set the sample rate of the stream.  This will drop any samples that have not been read. */
void emsonicSetSampleRate(emsonicStream stream, int sampleRate);
/* Get the number of channels. */
int emsonicGetNumChannels(emsonicStream stream);
/* Set the number of channels.  This will drop any samples that have not been read. */
void emsonicSetNumChannels(emsonicStream stream, int numChannels);
...
int emsonicChangeFloatSpeed(float *samples, int numSamples, float speed, float pitch,
    float rate, float volume, int useChordPitch, int sampleRate, int numChannels);
/* This is a non-stream oriented interface to just change the speed of a sound
   sample.  It works in-place on the sample array, so there must be at least
   speed*numSamples available space in the array. Returns the new number of samples. */
int emsonicChangeShortSpeed(short *samples, int numSamples, float speed, float pitch,
    float rate, float volume, int useChordPitch, int sampleRate, int numChannels);
