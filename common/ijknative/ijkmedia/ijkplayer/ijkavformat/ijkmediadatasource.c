...
static int ijkmds_open(EMURLContext *h, const char *arg, int flags, AVEMDictionary **options)
{
    ...
    av_em_strstart(arg, "ijkmediadatasource:", &arg);
    ...
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        av_em_log(h, AV_LOG_ERROR, "%s: SDL_JNI_SetupThreadEnv: failed", __func__);
        return AVERROR(EINVAL);
    }
...
static int ijkmds_close(EMURLContext *h)
{
    Context *c = h->priv_data;
    JNIEnv *env = NULL;
...
static jobject jbuffer_grow(JNIEnv *env, EMURLContext *h, int new_capacity) {
    Context *c = h->priv_data;
...
static int ijkmds_read(EMURLContext *h, unsigned char *buf, int size)
{
    Context    *c = h->priv_data;
    JNIEnv     *env = NULL;
...
static int64_t ijkmds_seek(EMURLContext *h, int64_t pos, int whence)
{
    Context *c = h->priv_data;
    int64_t  ret;
    ...
    if (whence == AVSEEK_SIZE) {
        av_em_log(h, AV_LOG_TRACE, "%s: AVSEEK_SIZE: %"PRId64"\n", __func__, (int64_t)c->logical_size);
        return c->logical_size;
    } else if (whence == SEEK_CUR) {
        av_em_log(h, AV_LOG_TRACE, "%s: %"PRId64"\n", __func__, pos);
...
static const AVEMClass ijkmediadatasource_context_class = {
...
EMURLProtocol ijkem_ijkmediadatasource_protocol = {
