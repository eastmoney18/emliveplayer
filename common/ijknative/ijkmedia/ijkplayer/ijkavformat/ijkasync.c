...
static int ring_init(RingBuffer *ring, int64_t capacity, int64_t read_back_capacity)
{
    ...
    ring->fifo = av_em_fifo_alloc((unsigned int)(capacity + read_back_capacity));
    if (!ring->fifo)
...
{
    av_em_fifo_freep(&ring->fifo);
}

static void ring_reset(RingBuffer *ring)
{
    av_em_fifo_reset(ring->fifo);
    ring->read_pos = 0;
}

static int ring_size(RingBuffer *ring)
{
    return av_em_fifo_size(ring->fifo) - ring->read_pos;
}

static int ring_space(RingBuffer *ring)
{
    return av_em_fifo_space(ring->fifo);
}
...
static int ring_generic_read(RingBuffer *ring, void *dest, int buf_size, void (*func)(void*, void*, int))
{
    ...
    ret = av_em_fifo_generic_peek_at(ring->fifo, dest, ring->read_pos, buf_size, func);
    ring->read_pos += buf_size;

    if (ring->read_pos > ring->read_back_capacity) {
        av_em_fifo_drain(ring->fifo, ring->read_pos - ring->read_back_capacity);
...
static int ring_generic_write(RingBuffer *ring, void *src, int size, int (*func)(void*, void*, int))
{
    ...
    return av_em_fifo_generic_write(ring->fifo, src, size, func);
}
...
static int wrapped_url_read(void *src, void *dst, int size)
{
    ...
    ret = ffurl_em_read(c->inner, dst, size);
    c->inner_io_error = ret < 0 ? ret : 0;

    return ret;
}

static void call_inject_statistic(EMURLContext *h)
{
    Context *c = h->priv_data;
    ...
    if (c->app_ctx) {
        ...
        av_em_application_on_async_statistic(c->app_ctx, &statistic);
    }
}

static void call_inject_async_fill_speed(EMURLContext *h, int is_full_speed, int64_t bytes, int64_t elapsed_micro)
{
    Context *c = h->priv_data;
    ...
    if (c->app_ctx && bytes > 0 && elapsed_milli > 0) {
        ...
        av_em_application_on_async_read_speed(c->app_ctx, &speed);
    }
}
...
static void *async_buffer_task(void *arg)
{
    ...
    int64_t       count_start_time_micro = av_em_gettime_relative();
    ...
    while (1) {
        ...
        if (c->seek_request) {
            seek_ret = ffurl_em_seek(c->inner, c->seek_pos, c->seek_whence);
            if (seek_ret < 0) {
                c->io_eof_reached = 1;
        ...
        if (ret > 0) {
            ...
            if (count_bytes > FFMIN((1 * 1024 * 1024), c->forwards_capacity)) {
                int64_t now = av_em_gettime_relative();
...
static int async_open(EMURLContext *h, const char *arg, int flags, AVEMDictionary **options)
{
    Context         *c = h->priv_data;
    int              ret;
    AVEMIOInterruptCB  interrupt_callback = {.callback = async_check_interrupt, .opaque = h};

    av_em_strstart(arg, "async:", &arg);
    ...
    if (c->app_ctx_intptr) {
        ...
        av_em_dict_set_int(options, "ijkapplication", c->app_ctx_intptr, 0);
    }
    /* wrap interrupt callback */
    c->interrupt_callback = h->interrupt_callback;
    ret = ffurl_em_open_whitelist(&c->inner, arg, flags, &interrupt_callback, options, h->protocol_whitelist, h->protocol_blacklist, h);
    if (ret != 0) {
        av_em_log(h, AV_LOG_ERROR, "ffurl_open_whitelist failed : %s, %s\n", av_err2str(ret), arg);
        goto url_fail;
    }

    c->logical_size = ffurl_em_size(c->inner);
...
static int async_close(EMURLContext *h)
{
    Context *c = h->priv_data;
    int      ret;
...
static int async_read_internal(EMURLContext *h, void *dest, int size, int read_complete,
...
static int async_read(EMURLContext *h, unsigned char *buf, int size)
{
...
static int64_t async_seek(EMURLContext *h, int64_t pos, int whence)
{
    Context      *c    = h->priv_data;
    RingBuffer   *ring = &c->ring;
    int64_t       ret;
    ...
    if (whence == AVSEEK_SIZE) {
        av_em_log(h, AV_LOG_TRACE, "async_seek: AVSEEK_SIZE: %"PRId64"\n", (int64_t)c->logical_size);
        return c->logical_size;
    } else if (whence == SEEK_CUR) {
        av_em_log(h, AV_LOG_TRACE, "async_seek: %"PRId64"\n", pos);
    ...
    if (new_logical_pos == c->logical_pos) {
        ...
        /* fast seek */
        av_em_log(h, AV_LOG_TRACE, "async_seek: fask_seek %"PRId64" from %d dist:%d/%d\n",
...
static const AVEMClass async_context_class = {
    .class_name = "Async",
...
const EMURLProtocol ijkem_async_protocol = {
    .name                = "async",
...
static int async_test_open(EMURLContext *h, const char *arg, int flags, AVEMDictionary **options)
{
...
static int async_test_close(EMURLContext *h)
{
    return 0;
}

static int async_test_read(EMURLContext *h, unsigned char *buf, int size)
{
    TestContext *c = h->priv_data;
    int          i;
    int          read_len = 0;
...
static int64_t async_test_seek(EMURLContext *h, int64_t pos, int whence)
{
    TestContext *c = h->priv_data;
    int64_t      new_logical_pos;
...
static const AVEMClass async_test_context_class = {
...
int main(void)
{
     ...
     */
    ffurl_close(h);
    av_em_dict_set_int(&opts, "async-test-read-error", -10000, 0);
