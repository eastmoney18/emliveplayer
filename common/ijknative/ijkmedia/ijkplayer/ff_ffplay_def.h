...
#define AV_SYNC_ONCE_WAIT_MAX_DELAY 3
...
#define FFP_PLAY_MODE_RTMP 0
#define FFP_PLAY_MODE_FLV_LIVE 1
#define FFP_PLAY_MODE_VOD_FLV 2
#define FFP_PLAY_MODE_VOD_HLS 3
#define FFP_PLAY_MODE_VOD_MP4  4
#define FFP_PLAY_MODE_LOCAL_VIDEO 5

#define FFP_PLAY_CHANNEL_MODE_LEFT 1
#define FFP_PLAY_CHANNEL_MODE_RIGHT 2
#define FFP_PLAY_CHANNEL_MODE_STEREO 0

#define FFP_PLAY_MAX_PREPARED_VIDEO_SOURCE 20
...
typedef struct Decoder {
    ...
    int64_t start_pts;
    AVEMRational start_pts_tb;
    int64_t next_pts;
    AVEMRational next_pts_tb;
...
typedef struct VideoState {
    ...
    int64_t is_last_pts;
    int64_t is_last_dts;
    int64_t is_seek_find_next_frame;
    int64_t seek_rel;
#ifdef FFP_MERGE
    int read_pause_return;
#endif
    AVEMFormatContext *ic;
    int64_t ic_start_time;
    int realtime;
    ...
    uint8_t *audio_buf;
    uint8_t *audio_buf1;
    short *audio_new_buffer; //sonic buffer
    //uint8_t *audio_buf2;
    unsigned int audio_buf_size; /* in bytes */
    unsigned int audio_buf1_size;
    unsigned int audio_new_buffer_size;
    ...
    volatile int network_disconnect;
    volatile int64_t network_disconnect_time;
    int reconnect_interval;
    int reconnect_count;
    int reconnect_retry_count;
    
    int prepared_source; //current video source is prepared.
} VideoState;
...
typedef struct FFStatistic
{
    ...
    SDL_SpeedSampler3 video_bitrate_sampler;
    SDL_SpeedSampler3 audio_bitrate_sampler;
    int64_t latest_seek_load_duration;
} FFStatistic;

#define FFP_TCP_READ_SAMPLE_RANGE 2000
#define FFP_BITRATE_SAMPLE_RANGE   2000
inline static void ffp_reset_statistic(FFStatistic *dcc)
{
    ...
    SDL_SpeedSampler3Reset(&dcc->video_bitrate_sampler, FFP_BITRATE_SAMPLE_RANGE);
    SDL_SpeedSampler3Reset(&dcc->audio_bitrate_sampler, FFP_BITRATE_SAMPLE_RANGE);
}
...
typedef struct FFPlayer {
    ...
    AVEMDictionary *format_opts;
    AVEMDictionary *codec_opts;
    AVEMDictionary *sws_dict;
    AVEMDictionary *player_opts;
    AVEMDictionary *swr_opts;
    AVEMDictionary *swr_preset_opts;
    ...
    int reconnect_count;
    int reconnect_interval; //in seconds

    int is_recording;

    int64_t prepared_timems;
    int b_first_display_time;

    int new_video_type;
    char new_video_path[1024];
    int b_change_source;
    SDL_mutex *change_source_lock;

    SDL_Thread *prepare_thread;
    SDL_Thread _prepare_thread;
    SDL_cond *prepare_cond;
    int prepare_abort;
    ffplay_format_t prepared_source[FFP_PLAY_MAX_PREPARED_VIDEO_SOURCE];
    int prepared_source_count;
    SDL_mutex *prepared_lock;

    ffplay_format_t *cur_format;
    SDL_mutex *reconfigure_mutex;
    SDL_cond *reconfigure_cond;

    int64_t cur_stream_unix_time;
    int64_t last_send_unix_time;
    
    ijk_present_video_frame_callback video_present_callback;
    ijk_present_audio_frame_callback audio_present_callback;
    int play_channel_mode;
    int video_clock_error;
    
    /*add by ccl time 2018-10-10*/
    int enable_sonic_handle;
    emsonicStream sonic_handle;
    
    int probesize;
    int dns_timeout;
    int dns_cache_count;

    // add by ccl time 2020-10-21
    int prepare_source_abort;
    int audio_stream_type;
    
} FFPlayer;

#define fftime_to_milliseconds(ts) (av_em_rescale(ts, 1000, AV_TIME_BASE))
#define milliseconds_to_fftime(ms) (av_em_rescale(ms, AV_TIME_BASE, 1000))
...
inline static void ffp_reset_internal(FFPlayer *ffp)
{
    ...
    /* format/codec options */
    av_em_dict_free(&ffp->format_opts);
    av_em_dict_free(&ffp->codec_opts);
    av_em_dict_free(&ffp->sws_dict);
    av_em_dict_free(&ffp->player_opts);
    av_em_dict_free(&ffp->swr_opts);
    av_em_dict_free(&ffp->swr_preset_opts);
    ...
    av_em_freep(&ffp->audio_codec_name);
    av_em_freep(&ffp->video_codec_name);
    ffp->rdftspeed              = 0.02;
#if CONFIG_AVFILTER
    av_em_freep(&ffp->vfilters_list);
    ffp->nb_vfilters            = 0;
    ffp->afilters               = NULL;
    ffp->vfilter0               = NULL;
#endif
    ...
    ffp->reconnect_count = 3;
    ffp->reconnect_interval = 1;

    ffp->is_recording = 0;
    ffp->b_first_display_time = 0;
    ffp->prepared_timems = 0;

    ffp->prepare_abort = 0;
    ffp->cur_stream_unix_time = 0;
    ffp->last_send_unix_time = 0;
    
    ffp->video_present_callback = NULL;
    ffp->audio_present_callback = NULL;
    
    ffp->video_clock_error = 0;
    ffp->play_channel_mode = FFP_PLAY_CHANNEL_MODE_STEREO;
    
    /*add by ccl time 2018-10-10*/
    ffp->enable_sonic_handle = 0;
    ffp->sonic_handle = NULL;

    //add by ccl time 2020-10-21
    ffp->prepare_source_abort = 0;
    ffp->audio_stream_type = 3;
}
...
#define FFP_PREPARE_VIDEO_SOURCE_FLAG ((void *)0xAAAAAAAA)

#define FFP_PREPARING_VIDEO_SOURCE_FLAG ((void *)0xBBBBBBBB)
