...
int SDL_Android_UpdateTexImage_FastImage(SDL_Vout *vout, SDL_VoutOverlay *overlay)
{
#if 0
    ALOGE("dump overlay .........");
    ALOGE("format:%c%c%c%c, planes:%d, width:%d, height:%d\n",\
     (overlay->format&0xFF), (overlay->format>>8) &0xFF, (overlay->format>>16) & 0xFF, (overlay->format >> 24) & 0xFF\
    , overlay->planes, overlay->w, overlay->h);
    for (int i = 0; i < overlay->planes; i++) {
        ALOGE("plane:%d linesize:%d\n", i,overlay->pitches[i]);
    }
    ALOGE("dump end...:%p, %p\n", vout->callerOpaque, vout->present_overlayBuffer);
#endif
    if (vout->opaque && vout->callerOpaque && vout->present_overlayBuffer) {
        JNIEnv *env = SDL_JNI_GetThreadEnv();
        if (!env) {
            SDL_JNI_SetupThreadEnv(&env);
        }
        vout->present_overlayBuffer(env, vout->callerOpaque, overlay);
    }
}

static int func_display_overlay_l1(SDL_Vout *vout, SDL_VoutOverlay *overlay)
{
    SDL_Vout_Opaque *opaque = vout->opaque;
    if (!overlay) {
    ...
    if (overlay->format == SDL_FCC__AMC) {
        ...
        return SDL_Android_UpdateTexImage_FastImage(vout, overlay);
    }
}
...
static int func_display_overlay_l(SDL_Vout *vout, SDL_VoutOverlay *overlay)
{
    ...
    if (!native_window && (overlay && overlay->format != SDL_FCC__AMC || !overlay)) {
...
{
    SDL_LockMutex(vout->mutex);
    int retval = func_display_overlay_l1(vout, overlay);
    SDL_UnlockMutex(vout->mutex);
    return retval;
}
...
void SDL_VoutAndroid_decreaseReference(SDL_Vout *vout)
{
    AMCTRACE("%s(%p)\n", __func__, acodec);
    SDL_Vout_Opaque *opaque = vout->opaque;
