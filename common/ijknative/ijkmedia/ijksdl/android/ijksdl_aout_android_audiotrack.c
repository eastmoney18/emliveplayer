...
static void func_present_audio_pcm_data(SDL_Aout *aout, uint8_t *buff, int size, int sr, int channels, int ptsms)
{
    JNIEnv *env = SDL_JNI_GetThreadEnv();
    if (env && aout && aout->caller_opaque) {
        EMMediaPlayer *mp = (EMMediaPlayer *)aout->caller_opaque;
        if (size > mp->audio_pb_length) {
            if (mp->audio_pb) {
                (*env)->DeleteGlobalRef(env, (jbyteArray)mp->audio_pb);
            }
            jbyteArray arr = (*env)->NewByteArray(env, size);
            if (arr) {
                mp->audio_pb = (*env)->NewGlobalRef(env, arr);
                (*env)->DeleteLocalRef(env, arr);
                mp->audio_pb_length = size;
            } else {
                mp->audio_pb = NULL;
                mp->audio_pb_length = 0;
            }
        }
        if (mp->audio_pb) {
            (*env)->SetByteArrayRegion(env, mp->audio_pb, 0, size, (const jbyte *)buff);
            J4AC_IjkMediaPlayer__postPresentAudioBufferFromNative(env, mp->weak_thiz, mp->audio_pb, size, ptsms, sr, channels);
        }
    }
}
...
SDL_Aout *SDL_AoutAndroid_CreateForAudioTrack()
{
    ...
    aout->func_present_audio_pcm = func_present_audio_pcm_data;
    return aout;
}
