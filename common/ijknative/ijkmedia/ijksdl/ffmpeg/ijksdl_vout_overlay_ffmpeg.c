...
{
    AVFrame *managed_frame = av_em_frame_alloc();
    if (!managed_frame) {
        return NULL;
    }

    AVFrame *linked_frame = av_em_frame_alloc();
    if (!linked_frame) {
        av_em_frame_free(&managed_frame);
        return NULL;
    }
    ...
    AVEMBufferRef *frame_buffer_ref = av_buffer_alloc(frame_bytes);
    if (!frame_buffer_ref)
        return NULL;
    ...
    av_em_image_fill_arrays(managed_frame->data, managed_frame->linesize ,NULL,
...
static AVFrame *opaque_obtain_managed_frame_buffer(SDL_VoutOverlay_Opaque* opaque)
{
    ...
    int frame_bytes = av_em_image_get_buffer_size(managed_frame->format, managed_frame->width, managed_frame->height, 1);
    AVEMBufferRef *frame_buffer_ref = av_em_buffer_alloc(frame_bytes);
    if (!frame_buffer_ref)
        return NULL;

    av_em_image_fill_arrays(managed_frame->data, managed_frame->linesize,
...
static void func_free_l(SDL_VoutOverlay *overlay)
{
    ...
    em_sws_freeContext(opaque->img_convert_ctx);

    if (opaque->managed_frame)
        av_em_frame_free(&opaque->managed_frame);

    if (opaque->linked_frame) {
        av_em_frame_unref(opaque->linked_frame);
        av_em_frame_free(&opaque->linked_frame);
    }

    if (opaque->frame_buffer)
        av_em_buffer_unref(&opaque->frame_buffer);
...
static int func_fill_frame(SDL_VoutOverlay *overlay, const AVFrame *frame)
{
    ...
    if (use_linked_frame) {
        // linked frame
        av_em_frame_ref(opaque->linked_frame, frame);
    ...
    if (use_linked_frame) {
        ...
        opaque->img_convert_ctx = em_sws_getCachedContext(opaque->img_convert_ctx,
        ...
        em_sws_scale(opaque->img_convert_ctx, (const uint8_t**) frame->data, frame->linesize,
                  0, frame->height, swscale_dst_pic.data, swscale_dst_pic.linesize);

        if (!opaque->no_neon_warned) {
            opaque->no_neon_warned = 1;
            ALOGE("non-neon image convert %s -> %s\n", av_em_get_pix_fmt_name(frame->format), av_em_get_pix_fmt_name(dst_format));
        }
    }
