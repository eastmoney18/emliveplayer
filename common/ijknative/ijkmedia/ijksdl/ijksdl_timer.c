...
void  SDL_SpeedSampler3Reset(SDL_SpeedSampler3 *sampler, int sample_range)
{
    memset(sampler, 0, sizeof(SDL_SpeedSampler3));
    sampler->sample_range      = sample_range;
}

int64_t SDL_SpeedSampler3Add(SDL_SpeedSampler3 *sampler, int64_t cur, int quantity)
{
    if (quantity < 0)
        return 0;
...
int64_t SDL_SpeedSampler3GetSpeed(SDL_SpeedSampler3 *sampler)
{
    return sampler->last_profile_speed;
}
