...
int SDL_VoutClear(SDL_Vout *vout)
{
    if (vout && vout->display_overlay) {
        return vout->display_overlay(vout, NULL);
    }
    return -1;
}
...
void SDL_VoutPresentYUVOverlayBuffer(SDL_Vout *vout, SDL_VoutOverlay *overlay, void *opaque)
{
    if (!overlay || !vout || !vout->present_overlayBuffer) {
        return;
    }
    return vout->present_overlayBuffer(NULL, opaque, overlay);
}
