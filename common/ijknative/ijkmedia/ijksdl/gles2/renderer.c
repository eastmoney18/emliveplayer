...
const GLfloat IJK_NormalVertices[8] = {-1.0f, -1.0f, 1.0f, -1.0f, -1.0f, 1.0f, 1.0f, 1.0f};

const GLfloat IJK_RotateRightVertices[8] = {-1.0f, 1.0f, -1.0f, -1.0f, 1.0f, 1.0f, 1.0f, -1.0f};

const GLfloat IJK_RotateLeftVertices[8] = {1.0f, -1.0f, 1.0f, 1.0f, -1.0f, -1.0f, -1.0f, 1.0f};

const GLfloat IJK_RotateTopBottomVertices[8] = {1.0f, 1.0f, -1.0f, 1.0f, 1.0f, -1.0f, -1.0f, -1.0f};

const GLfloat IJK_ReversePortraitVertices[8] = {-1.0f, 1.0f, 1.0f, 1.0f, -1.0f, -1.0f, 1.0f, -1.0f};

const GLfloat IJK_ReverseLandscapeVertices[8] = {1.0f, -1.0f, -1.0f, -1.0f, 1.0f, 1.0f, -1.0f, 1.0f};
...
{
    for (int i = 0; i < 8; i++) {
        renderer->vertices[i] = IJK_NormalVertices[i];
    }
    renderer->rotate_mode = IJK_ROTATE_NORMAL;
}
...
static void IJK_GLES2_Renderer_Vertices_apply(IJK_GLES2_Renderer *renderer)
{
    ...
    GLsizei layer_width = renderer->layer_width;
    GLsizei layer_height = renderer->layer_height;
    
    int rotation = 0;
    if (renderer->rotate_mode == IJK_ROTATE_LEFT || renderer->rotate_mode == IJK_ROTATE_RIGHT)
    {
        layer_height = renderer->layer_width;
        layer_width = renderer->layer_height;
        rotation = 1;
    }

    const float dW  = (float)layer_width	/ width;
    const float dH  = (float)layer_height / height;
    float dd        = 1.0f;
    float nW        = 1.0f;
    float nH        = 1.0f;
    ...
    if (rotation) {
        nH = (width  * dd / (float)layer_width);
        nW = (height * dd / (float)layer_height);
    }else {
        nW = (width  * dd / (float)layer_width);
        nH = (height * dd / (float)layer_height);
    }
    const GLfloat *vertices;
    
    switch (renderer->rotate_mode) {
        case IJK_ROTATE_NORMAL:
            vertices = IJK_NormalVertices;
            break;
        case IJK_ROTATE_LEFT:
            vertices = IJK_RotateLeftVertices;
            break;
        case IJK_ROTATE_RIGHT:
            vertices = IJK_RotateRightVertices;
            break;
        case IJK_ROTATE_TOP_BOTTOM:
            vertices = IJK_RotateTopBottomVertices;
            break;
        case IJK_REVERSE_LANDSCAPE:
            vertices = IJK_ReverseLandscapeVertices;
            break;
        case IJK_REVERSE_PORTRAIT:
            vertices = IJK_ReversePortraitVertices;
            break;
        default:
            break;
    }
    ...
    for (int i = 0; i < 8; i++) {
        renderer->vertices[i] = vertices[i] * (i%2 ? nH : nW);
    }
...
GLboolean IJK_GLES2_Renderer_setRotate(IJK_GLES2_Renderer *renderer, IJK_Rotate_Mode rotate_mode)
{
    if (rotate_mode < IJK_ROTATE_NORMAL || rotate_mode > IJK_REVERSE_LANDSCAPE) {
        ALOGE("set rotate mode invalid\n");
        return GL_FALSE;
    }
    renderer->rotate_mode = rotate_mode;
    renderer->vertices_changed = 1;
    return GL_TRUE;
}
...
GLboolean IJK_GLES2_Renderer_renderOverlay(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay)
{
    ...
    if (overlay) {
        ...
        renderer->vertices_changed = 1;
        // fixed:NULL overlay means clear screen
