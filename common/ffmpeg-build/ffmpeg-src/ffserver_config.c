...
static int resolve_host(struct in_addr *sin_addr, const char *hostname)
{
    ...
    if (!em_inet_aton(hostname, sin_addr)) {
#if HAVE_GETADDRINFO
...
static void add_codec(FFServerStream *stream, AVEMCodecContext *av,
                      FFServerConfig *config)
{
    AVEMStream *st;
    AVEMDictionary **opts, *recommended = NULL;
    char *enc_config;
    ...
    st = av_mallocz(sizeof(AVEMStream));
    if (!st)
        return;
    av_dict_get_string(recommended, &enc_config, '=', ',');
    av_dict_free(&recommended);
    av_em_stream_set_recommended_encoder_configuration(st, enc_config);
    st->codec = av;
    stream->streams[stream->nb_streams++] = st;
}

static int ffserver_set_codec(AVEMCodecContext *ctx, const char *codec_name,
                              FFServerConfig *config)
{
    int ret;
    AVEMCodec *codec = avcodec_find_encoder_by_name(codec_name);
...
static AVEMOutputFormat *ffserver_guess_format(const char *short_name,
                                             const char *filename,
                                             const char *mime_type)
{
    AVEMOutputFormat *fmt = av_em_guess_format(short_name, filename, mime_type);

    if (fmt) {
        AVEMOutputFormat *stream_fmt;
        char stream_format_name[64];

        snprintf(stream_format_name, sizeof(stream_format_name), "%s_stream",
                fmt->name);
        stream_fmt = av_em_guess_format(stream_format_name, NULL, NULL);
                                  ...
                                  FFServerConfig *config)
{
    ...
    char buff[1024];
    AVEMCodecContext *ctx;
    AVEMDictionary **dict;
    enum AVEMCodecID guessed_codec_id;
                                      ...
                                      const char **p, FFServerStream **pfeed)
{
    ...
    if (!av_strcasecmp(cmd, "<Feed")) {
        ...
        feed->fmt = av_em_guess_format("ffm", NULL, NULL);
        /* default feed file */
                                        ...
                                        FFServerStream **pstream)
{
    ...
    if (!av_strcasecmp(cmd, "Feed")) {
        ...
        ffserver_get_arg(arg, sizeof(arg), p);
        stream->ifmt = av_em_find_input_format(arg);
        if (!stream->ifmt)
        ...
        if (stream->feed && stream->fmt && strcmp(stream->fmt->name, "ffm")) {
            ...
            if (!config->no_audio &&
                ...
                AVEMCodecContext *audio_enc = avcodec_alloc_context3(avcodec_find_encoder(config->dummy_actx->codec_id));
            ...
            if (!config->no_video &&
                ...
                AVEMCodecContext *video_enc = avcodec_alloc_context3(avcodec_find_encoder(config->dummy_vctx->codec_id));
