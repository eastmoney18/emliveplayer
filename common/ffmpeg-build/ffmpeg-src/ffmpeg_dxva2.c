...
static void dxva2_uninit(AVEMCodecContext *s)
{
...
static int dxva2_get_buffer(AVEMCodecContext *s, AVFrame *frame, int flags)
{
    InputStream  *ist = s->opaque;
    DXVA2Context *ctx = ist->hwaccel_ctx;
...
static int dxva2_retrieve_data(AVEMCodecContext *s, AVFrame *frame)
{
    InputStream        *ist = s->opaque;
    DXVA2Context       *ctx = ist->hwaccel_ctx;
    int                ret;
...
static int dxva2_alloc(AVEMCodecContext *s)
{
    InputStream  *ist = s->opaque;
...
static int dxva2_get_decoder_configuration(AVEMCodecContext *s, const GUID *device_guid,
...
static int dxva2_create_decoder(AVEMCodecContext *s)
{
    InputStream  *ist = s->opaque;
    int loglevel = (ist->hwaccel_id == HWACCEL_AUTO) ? AV_LOG_VERBOSE : AV_LOG_ERROR;
    DXVA2Context *ctx = ist->hwaccel_ctx;
