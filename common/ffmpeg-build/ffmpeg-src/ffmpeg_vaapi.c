...
static int vaapi_get_buffer(AVEMCodecContext *avctx, AVFrame *frame, int flags)
{
...
static int vaapi_retrieve_data(AVEMCodecContext *avctx, AVFrame *input)
{
    InputStream *ist = avctx->opaque;
    VAAPIDecoderContext *ctx = ist->hwaccel_ctx;
    AVFrame *output = 0;
...
static void vaapi_decode_uninit(AVEMCodecContext *avctx)
{
    InputStream *ist = avctx->opaque;
    VAAPIDecoderContext *ctx = ist->hwaccel_ctx;
...
int vaapi_decode_init(AVEMCodecContext *avctx)
{
    ...
    // We would like to set this on the AVEMCodecContext for use by whoever gets
    // the frames from the decoder, but unfortunately the AVEMCodecContext we
...
static AVEMClass *vaapi_log = &vaapi_class;
