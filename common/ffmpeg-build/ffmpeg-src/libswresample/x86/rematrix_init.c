...
av_cold int em_swri_rematrix_init_x86(struct SwrContext *s){
#if HAVE_YASM
    int mm_flags = av_em_get_cpu_flags();
    ...
    if (s->midbuf.fmt == AV_SAMPLE_FMT_S16P){
        ...
        s->native_simd_matrix = av_em_mallocz_array(num,  2 * sizeof(int16_t));
        s->native_simd_one    = av_em_mallocz(2 * sizeof(int16_t));
        ...
        s->native_simd_matrix = av_em_mallocz_array(num, sizeof(float));
        s->native_simd_one = av_em_mallocz(sizeof(float));
        if (!s->native_simd_matrix || !s->native_simd_one)
            return AVERROR(ENOMEM);
