...
static int build_filter(ResampleContext *c, void *filter, double factor, int tap_count, int alloc, int phase_count, int scale,
    ...
    double x, y, w, t, s;
    double *tab = av_em_malloc_array(tap_count+1,  sizeof(*tab));
    double *sin_lut = av_em_malloc_array(ph_nb, sizeof(*sin_lut));
                                    ...
                                    double precision, int cheby, int exact_rational)
{
    ...
    if (exact_rational) {
        ...
        av_em_reduce(&phase_count_exact, &phase_count_exact_den, out_rate, in_rate, INT_MAX);
    ...
    if (!c || c->phase_count != phase_count || c->linear!=linear || c->factor != factor
        ...
        c->felem_size= av_em_get_bytes_per_sample(c->format);
        ...
        case AV_SAMPLE_FMT_DBLP:
            ...
            break;
        default:
            av_em_log(NULL, AV_LOG_ERROR, "Unsupported sample format\n");
            av_assert0(0);
        }

        if (filter_size/factor > INT32_MAX/256) {
            av_em_log(NULL, AV_LOG_ERROR, "Filter length too large\n");
            goto error;
        }
        ...
        c->filter_bank   = av_em_calloc(c->filter_alloc, (phase_count+1)*c->felem_size);
...
static int rebuild_filter_bank_with_compensation(ResampleContext *c)
{
    ...
    new_filter_bank = av_em_calloc(c->filter_alloc, (phase_count + 1) * c->felem_size);
    if (!new_filter_bank)
    ...
    if (!av_em_reduce(&new_src_incr, &new_dst_incr, c->src_incr,
...
static int64_t get_delay(struct SwrContext *s, int64_t base){
    ...
    num *= c->src_incr;
    num -= c->frac;
    return av_em_rescale(num, base, s->in_sample_rate*(int64_t)c->src_incr * c->phase_count);
}
...
static int64_t get_out_samples(struct SwrContext *s, int in_samples) {
    ...
    num *= c->phase_count;
    num -= c->index;
    num = av_em_rescale_rnd(num, s->out_sample_rate, ((int64_t)s->in_sample_rate) * c->phase_count, AV_ROUND_UP) + 2;
...
static int resample_flush(struct SwrContext *s) {
    ...
    int i, j, ret;
    if((ret = em_swri_realloc_audio(a, s->in_buffer_index + 2*s->in_buffer_count)) < 0)
        return ret;
    av_assert0(a->planar);
                                 ...
                                 int in_count, int *out_idx, int *out_sz)
{
    ...
    if ((res = em_swri_realloc_audio(dst, c->filter_length * 2 + 1)) < 0)
        return res;
