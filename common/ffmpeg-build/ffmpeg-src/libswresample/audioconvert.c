...
AudioConvert *em_em_swri_audio_convert_alloc(enum AVSampleFormat out_fmt,
                                       enum AVSampleFormat in_fmt,
                                       int channels, const int *ch_map,
                                       int flags)
{
    AudioConvert *ctx;
    conv_func_type *f = fmt_pair_to_conv_functions[av_em_get_packed_sample_fmt(out_fmt) + AV_SAMPLE_FMT_NB*av_em_get_packed_sample_fmt(in_fmt)];
    ...
    if(channels == 1){
         in_fmt = av_em_get_planar_sample_fmt( in_fmt);
        out_fmt = av_em_get_planar_sample_fmt(out_fmt);
    }
    ...
    if(out_fmt == in_fmt && !ch_map) {
        switch(av_em_get_bytes_per_sample(in_fmt)){
    ...
    if(HAVE_YASM && HAVE_MMX) em_swri_audio_convert_init_x86(ctx, out_fmt, in_fmt, channels);
    if(ARCH_ARM)              em_swri_audio_convert_init_arm(ctx, out_fmt, in_fmt, channels);
    if(ARCH_AARCH64)          em_swri_audio_convert_init_aarch64(ctx, out_fmt, in_fmt, channels);

    return ctx;
}

void em_em_swri_audio_convert_free(AudioConvert **ctx)
{
    av_em_freep(ctx);
}

int em_swri_audio_convert(AudioConvert *ctx, AudioData *out, AudioData *in, int len)
{
    int ch;
    int off=0;
