...
int em_swr_set_matrix(struct SwrContext *s, const double *matrix, int stride)
{
    ...
    nb_in  = av_em_get_channel_layout_nb_channels(s->user_in_ch_layout);
    nb_out = av_em_get_channel_layout_nb_channels(s->user_out_ch_layout);
...
static int clean_layout(SwrContext *s, int64_t layout){
    if(layout && layout != AV_CH_FRONT_CENTER && !(layout&(layout-1))) {
        char buf[128];
        av_em_get_channel_layout_string(buf, sizeof(buf), -1, layout);
        av_em_log(s, AV_LOG_VERBOSE, "Treating %s as mono\n", buf);
...
static int sane_layout(int64_t layout){
    ...
    if(!even(layout & (AV_CH_FRONT_LEFT_OF_CENTER | AV_CH_FRONT_RIGHT_OF_CENTER)))
        return 0;
    if(av_em_get_channel_layout_nb_channels(layout) >= SWR_CH_MAX)
        return 0;
...
av_cold static int auto_matrix(SwrContext *s)
{
    ...
    if(!sane_layout(in_ch_layout)){
        av_em_get_channel_layout_string(buf, sizeof(buf), -1, s->in_ch_layout);
        av_em_log(s, AV_LOG_ERROR, "Input channel layout '%s' is not supported\n", buf);
        return AVERROR(EINVAL);
    }

    if(!sane_layout(out_ch_layout)){
        av_em_get_channel_layout_string(buf, sizeof(buf), -1, s->out_ch_layout);
        av_em_log(s, AV_LOG_ERROR, "Output channel layout '%s' is not supported\n", buf);
        return AVERROR(EINVAL);
    }
...
//FIXME implement full ac3
    ...
    } else if (   av_em_get_packed_sample_fmt(s->out_sample_fmt) < AV_SAMPLE_FMT_FLT
               || av_em_get_packed_sample_fmt(s->int_sample_fmt) < AV_SAMPLE_FMT_FLT) {
        maxval = 1.0;
    } else
        maxval = INT_MAX;
    ...
    av_em_log(s, AV_LOG_DEBUG, "Matrix coefficients:\n");
    for(i=0; i<av_em_get_channel_layout_nb_channels(out_ch_layout); i++){
        const char *c =
            av_em_get_channel_name(av_em_channel_layout_extract_channel(out_ch_layout, i));
        av_em_log(s, AV_LOG_DEBUG, "%s: ", c ? c : "?");
        for(j=0; j<av_em_get_channel_layout_nb_channels(in_ch_layout); j++){
            c = av_em_get_channel_name(av_em_channel_layout_extract_channel(in_ch_layout, j));
            av_em_log(s, AV_LOG_DEBUG, "%s:%f ", c ? c : "?", s->matrix[i][j]);
        }
...
av_cold int em_swri_rematrix_init(SwrContext *s){
    int i, j;
    int nb_in  = av_em_get_channel_layout_nb_channels(s->in_ch_layout);
    int nb_out = av_em_get_channel_layout_nb_channels(s->out_ch_layout);
    ...
    if (s->midbuf.fmt == AV_SAMPLE_FMT_S16P){
        int maxsum = 0;
        s->native_matrix = av_em_calloc(nb_in * nb_out, sizeof(int));
        s->native_one    = av_em_mallocz(sizeof(int));
        ...
        *((int*)s->native_one) = 32768;
        ...
        s->native_matrix = av_em_calloc(nb_in * nb_out, sizeof(float));
        s->native_one    = av_em_mallocz(sizeof(float));
        if (!s->native_matrix || !s->native_one)
            return AVERROR(ENOMEM);
        for (i = 0; i < nb_out; i++)
            for (j = 0; j < nb_in; j++)
        ...
        *((float*)s->native_one) = 1.0;
        ...
        s->native_matrix = av_em_calloc(nb_in * nb_out, sizeof(double));
        s->native_one    = av_em_mallocz(sizeof(double));
        if (!s->native_matrix || !s->native_one)
            return AVERROR(ENOMEM);
        for (i = 0; i < nb_out; i++)
            for (j = 0; j < nb_in; j++)
...
//                 ((double*)s->native_matrix)[i * nb_in + j] = s->matrix[i][j];
    ...
    if(HAVE_YASM && HAVE_MMX)
        return em_swri_rematrix_init_x86(s);

    return 0;
}

av_cold void em_swri_rematrix_free(SwrContext *s){
    av_em_freep(&s->native_matrix);
    av_em_freep(&s->native_one);
    av_em_freep(&s->native_simd_matrix);
    av_em_freep(&s->native_simd_one);
}

int em_swri_rematrix(SwrContext *s, AudioData *out, AudioData *in, int len, int mustcopy){
    int out_i, in_i, i, j;
    int len1 = 0;
    int off = 0;
    ...
    av_assert0(!s->out_ch_layout || out->ch_count == av_em_get_channel_layout_nb_channels(s->out_ch_layout));
    av_assert0(!s-> in_ch_layout || in ->ch_count == av_em_get_channel_layout_nb_channels(s-> in_ch_layout));

    for(out_i=0; out_i<out->ch_count; out_i++){
        switch(s->matrix_ch[out_i][0]){
        case 0:
            if(mustcopy)
                memset(out->ch[out_i], 0, len * av_em_get_bytes_per_sample(s->int_sample_fmt));
            break;
        case 1:
