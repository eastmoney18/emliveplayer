...
#define TMP_EXTRA 2
    double *tmp = av_em_malloc_array(len + TMP_EXTRA, sizeof(double));
    int i;
...
av_cold int em_swri_dither_init(SwrContext *s, enum AVSampleFormat out_fmt, enum AVSampleFormat in_fmt)
{
    int i;
    double scale = 0;
    ...
    out_fmt = av_em_get_packed_sample_fmt(out_fmt);
    in_fmt  = av_em_get_packed_sample_fmt( in_fmt);
    ...
    for (i=0; filters[i].coefs; i++) {
        ...
        if (llabs(s->out_sample_rate - f->rate)*20 <= f->rate && f->name == s->dither.method) {
            ...
            s->dither.ns_scale_1 *= 1 - exp(f->gain_cB * M_LN10 * 0.005) * 2 / (1<<(8*av_em_get_bytes_per_sample(out_fmt)));
            break;
        }
    }
    if (!filters[i].coefs && s->dither.method > SWR_DITHER_NS) {
        av_em_log(s, AV_LOG_WARNING, "Requested noise shaping dither not available at this sampling rate, using triangular hp dither\n");
