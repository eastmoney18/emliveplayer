...
int em_swr_config_frame(SwrContext *s, const AVFrame *out, const AVFrame *in)
{
    em_swr_close(s);

    if (in) {
        if (av_em_opt_set_int(s, "icl", in->channel_layout, 0) < 0)
            goto fail;
        if (av_em_opt_set_int(s, "isf", in->format, 0) < 0)
            goto fail;
        if (av_em_opt_set_int(s, "isr", in->sample_rate, 0) < 0)
            goto fail;
    }

    if (out) {
        if (av_em_opt_set_int(s, "ocl", out->channel_layout, 0) < 0)
            goto fail;
        if (av_em_opt_set_int(s, "osf", out->format,  0) < 0)
            goto fail;
        if (av_em_opt_set_int(s, "osr", out->sample_rate, 0) < 0)
            goto fail;
    }
                                ...
                                AVFrame *out, const AVFrame *in)
{
    ...
    ret = em_swr_convert(s, out_data, out_nb_samples, in_data, in_nb_samples);
...
int em_swr_convert_frame(SwrContext *s,
                      AVFrame *out, const AVFrame *in)
{
    int ret, setup = 0;

    if (!em_swr_is_initialized(s)) {
        if ((ret = em_swr_config_frame(s, out, in)) < 0)
            return ret;
        if ((ret = em_swr_init(s)) < 0)
            return ret;
        setup = 1;
    } else {
    ...
    if (out) {
        if (!out->linesize[0]) {
            out->nb_samples =   em_swr_get_delay(s, s->out_sample_rate)
