...
static double get(uint8_t *a[], int ch, int index, int ch_count, enum AVSampleFormat f){
    const uint8_t *p;
    if(av_em_sample_fmt_is_planar(f)){
        f= av_em_get_alt_sample_fmt(f, 0);
        p= a[ch];
    }else{
        p= a[0];
...
static void shift(uint8_t *a[], int index, int ch_count, enum AVSampleFormat f){
    ...
    if(av_em_sample_fmt_is_planar(f)){
        ...
        for(ch= 0; ch<ch_count; ch++)
            a[ch] += index*av_em_get_bytes_per_sample(f);
    }else{
        a[0] += index*ch_count*av_em_get_bytes_per_sample(f);
    }
}
...
static void setup_array(uint8_t *out[SWR_CH_MAX], uint8_t *in, enum AVSampleFormat format, int samples){
    if(av_em_sample_fmt_is_planar(format)){
        int i;
        int plane_size= av_em_get_bytes_per_sample(format&0xFF)*samples;
        format&=0xFF;
...
int main(int argc, char **argv){
    ...
    if (argc > 1) {
        if (!strcmp(argv[1], "-h") || !strcmp(argv[1], "--help")) {
            av_em_log(NULL, AV_LOG_INFO, "Usage: swresample-test [<num_tests>[ <test>]]  \n"
    ...
    for(test=0; test<num_tests; test++){
        ...
        in_ch_count= av_em_get_channel_layout_nb_channels(in_ch_layout);
        out_ch_count= av_em_get_channel_layout_nb_channels(out_ch_layout);
        av_em_get_channel_layout_string( in_layout_string, sizeof( in_layout_string),  in_ch_count,  in_ch_layout);
        av_em_get_channel_layout_string(out_layout_string, sizeof(out_layout_string), out_ch_count, out_ch_layout);
        fprintf(stderr, "TEST: %s->%s, rate:%5d->%5d, fmt:%s->%s\n",
                in_layout_string, out_layout_string,
                in_sample_rate, out_sample_rate,
                av_em_get_sample_fmt_name(in_sample_fmt), av_em_get_sample_fmt_name(out_sample_fmt));
        forw_ctx  = em_swr_alloc_set_opts(forw_ctx, out_ch_layout, out_sample_fmt,  out_sample_rate,
                                                    in_ch_layout,  in_sample_fmt,  in_sample_rate,
                                        0, 0);
        backw_ctx = em_swr_alloc_set_opts(backw_ctx, in_ch_layout,  in_sample_fmt,             in_sample_rate,
        ...
        if (uint_rand(rand_seed) % 3 == 0)
            av_em_opt_set_int(forw_ctx, "ich", 0, 0);
        if (uint_rand(rand_seed) % 3 == 0)
            av_em_opt_set_int(forw_ctx, "och", 0, 0);

        if(em_swr_init( forw_ctx) < 0)
            fprintf(stderr, "em_swr_init(->) failed\n");
        if(em_swr_init(backw_ctx) < 0)
            fprintf(stderr, "em_swr_init(<-) failed\n");
                //FIXME test planar
        ...
        if(mode==0 /*|| out_sample_rate == in_sample_rate*/) {
            mid_count= em_swr_convert(forw_ctx, amid, 3*SAMPLES, (const uint8_t **)ain, SAMPLES);
        } else if(mode==1){
            mid_count= em_swr_convert(forw_ctx, amid,         0, (const uint8_t **)ain, SAMPLES);
            mid_count+=em_swr_convert(forw_ctx, amid, 3*SAMPLES, (const uint8_t **)ain,       0);
        } else {
            int tmp_count;
            mid_count= em_swr_convert(forw_ctx, amid,         0, (const uint8_t **)ain,       1);
            ...
            mid_count+=em_swr_convert(forw_ctx, amid,         2, (const uint8_t **)ain,       2);
            shift(amid,  mid_count-tmp_count, out_ch_count, out_sample_fmt); tmp_count = mid_count;
            shift(ain,  2, in_ch_count, in_sample_fmt);
            mid_count+=em_swr_convert(forw_ctx, amid,         1, (const uint8_t **)ain, SAMPLES-3);
            shift(amid,  mid_count-tmp_count, out_ch_count, out_sample_fmt); tmp_count = mid_count;
            shift(ain, -3, in_ch_count, in_sample_fmt);
            mid_count+=em_swr_convert(forw_ctx, amid, 3*SAMPLES, (const uint8_t **)ain,       0);
            shift(amid,  -tmp_count, out_ch_count, out_sample_fmt);
        }
        out_count= em_swr_convert(backw_ctx,aout, SAMPLES, (const uint8_t **)amid, mid_count);
        ...
        flush_i++;
        flush_i%=21;
        flush_count = em_swr_convert(backw_ctx,aout, flush_i, 0, 0);
        shift(aout,  flush_i, in_ch_count, in_sample_fmt);
        flush_count+= em_swr_convert(backw_ctx,aout, SAMPLES-flush_i, 0, 0);
        shift(aout, -flush_i, in_ch_count, in_sample_fmt);
        if(flush_count){
            for(ch=0; ch<in_ch_count; ch++){
                double sse, maxdiff=0;
