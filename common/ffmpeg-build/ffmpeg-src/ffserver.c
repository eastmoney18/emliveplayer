...
typedef struct HTTPContext {
    ...
    int switch_pending;
    AVEMFormatContext fmt_ctx; /* instance of FFServerStream for one user */
    ...
    AVEMFormatContext *rtp_ctx[FFSERVER_MAX_STREAMS];

    /* RTP/UDP specific */
    EMURLContext *rtp_handles[FFSERVER_MAX_STREAMS];
...
static inline void print_stream_params(AVEMIOContext *pb, FFServerStream *stream);
...
static inline int check_codec_match(AVEMCodecContext *ccf, AVEMCodecContext *ccs,
                                    int stream);
...
static void ffm_set_write_index(AVEMFormatContext *s, int64_t pos,
                                int64_t file_size)
{
...
{
    static int print_prefix = 1;
    AVEMClass *avc = ptr ? *(AVEMClass**)ptr : NULL;
...
static int socket_open_listen(struct sockaddr_in *my_addr)
{
    ...
    if (em_socket_nonblock(server_fd, 1) < 0)
        av_log(NULL, AV_LOG_WARNING, "em_socket_nonblock failed\n");
...
static void close_connection(HTTPContext *c)
{
    ...
    if (c->fmt_in) {
        ...
        avformat_em_close_input(&c->fmt_in);
    }
    ...
    for(i=0;i<nb_streams;i++) {
        ctx = c->rtp_ctx[i];
        if (ctx) {
            av_em_write_trailer(ctx);
            av_dict_free(&ctx->metadata);
            av_freep(&ctx->streams[0]);
            av_freep(&ctx);
        }
        ffurl_em_close(c->rtp_handles[i]);
    }
    ...
    if (!c->last_packet_sent && c->state == HTTPSTATE_SEND_DATA_TRAILER) {
        /* prepare header */
        if (ctx->oformat && avio_em_open_dyn_buf(&ctx->pb) >= 0) {
            av_em_write_trailer(ctx);
            av_freep(&c->pb_buffer);
            avio_em_close_dyn_buf(ctx->pb, &c->pb_buffer);
        }
    }
...
static int find_stream_in_feed(FFServerStream *feed, AVEMCodecContext *codec,
                               int bit_rate)
{
    int i;
    ...
    for (i = 0; i < feed->nb_streams; i++) {
        AVEMCodecContext *feed_codec = feed->streams[i]->codec;
...
static int modify_current_stream(HTTPContext *c, char *rates)
{
    ...
    for (i = 0; i < req->nb_streams; i++) {
        AVEMCodecContext *codec = req->streams[i]->codec;
...
static int http_parse_request(HTTPContext *c)
{
    ...
    if (av_em_match_ext(filename, "asx")) {
        redir_type = REDIR_ASX;
        filename[strlen(filename)-1] = 'f';
    } else if (av_em_match_ext(filename, "asf") &&
        (!useragent || av_strncasecmp(useragent, "NSPlayer", 8))) {
        /* if this isn't WMP or lookalike, return the redirector file */
        redir_type = REDIR_ASF;
    } else if (av_em_match_ext(filename, "rpm,ram")) {
        redir_type = REDIR_RAM;
        strcpy(filename + strlen(filename)-2, "m");
    } else if (av_em_match_ext(filename, "rtsp")) {
        redir_type = REDIR_RTSP;
        compute_real_filename(filename, sizeof(filename) - 1);
    } else if (av_em_match_ext(filename, "sdp")) {
...
static void fmt_bytecount(AVEMIOContext *pb, int64_t count)
{
    ...
    avio_em_printf(pb, "%"PRId64"%c", count, *s);
}
...
static inline void print_stream_params(AVEMIOContext *pb, FFServerStream *stream)
{
    ...
    avio_em_printf(pb, "<table cellspacing=0 cellpadding=4><tr><th>Stream<th>"
    ...
    for (i = 0; i < stream_no; i++) {
        ...
        avio_em_printf(pb, "<tr><td align=right>%d<td>%s<td align=right>%"PRId64
                        "<td>%s<td>%s\n",
                    i, type, (int64_t)st->codec->bit_rate/1000,
                    codec ? codec->name : "", parameters);
     }

     avio_em_printf(pb, "</table>\n");
}
...
static void compute_status(HTTPContext *c)
{
    ...
    avio_em_printf(pb, "HTTP/1.0 200 OK\r\n");
    avio_em_printf(pb, "Content-type: text/html\r\n");
    avio_em_printf(pb, "Pragma: no-cache\r\n");
    avio_em_printf(pb, "\r\n");

    avio_em_printf(pb, "<!DOCTYPE html>\n");
    avio_em_printf(pb, "<html><head><title>%s Status</title>\n", program_name);
    if (c->stream->feed_filename[0])
        avio_em_printf(pb, "<link rel=\"shortcut icon\" href=\"%s\">\n",
                    c->stream->feed_filename);
    avio_em_printf(pb, "</head>\n<body>");
    avio_em_printf(pb, "<h1>%s Status</h1>\n", program_name);
    /* format status */
    avio_em_printf(pb, "<h2>Available Streams</h2>\n");
    avio_em_printf(pb, "<table cellspacing=0 cellpadding=4>\n");
    avio_em_printf(pb, "<tr><th valign=top>Path<th align=left>Served<br>Conns<th><br>bytes<th valign=top>Format<th>Bit rate<br>kbit/s<th align=left>Video<br>kbit/s<th><br>Codec<th align=left>Audio<br>kbit/s<th><br>Codec<th align=left valign=top>Feed\n");
    stream = config.first_stream;
    while (stream) {
        char sfilename[1024];
        char *eosf;
        ...
        avio_em_printf(pb, "<tr><td><a href=\"/%s\">%s</a> ",
                    sfilename, stream->filename);
        avio_em_printf(pb, "<td align=right> %d <td align=right> ",
                    stream->conns_served);
        ...
        case STREAM_TYPE_LIVE: {
            ...
            for(i=0;i<stream->nb_streams;i++) {
                AVEMStream *st = stream->streams[i];
                AVEMCodec *codec = avcodec_find_encoder(st->codec->codec_id);
            ...
            avio_em_printf(pb, "<td align=center> %s <td align=right> %d "
            ...
            if (stream->feed)
                avio_em_printf(pb, "<td>%s", stream->feed->filename);
            else
                avio_em_printf(pb, "<td>%s", stream->feed_filename);
            avio_em_printf(pb, "\n");
        }
            break;
        default:
            avio_em_printf(pb, "<td align=center> - <td align=right> - "
    ...
    while (stream) {
        ...
        avio_em_printf(pb, "<h2>Feed %s</h2>", stream->filename);
        if (stream->pid) {
            avio_em_printf(pb, "Running as pid %"PRId64".\n", (int64_t) stream->pid);
                ...
                snprintf(ps_cmd, sizeof(ps_cmd),
                 ...
                 if (pid_stat) {
                     ...
                     if (fscanf(pid_stat, "%9s %63s", cpuperc, cpuused) == 2) {
                         avio_em_printf(pb, "Currently using %s%% of the cpu. "
                                         "Total time used %s.\n",
                                     cpuperc, cpuused);
                     }
                     fclose(pid_stat);
    ...
    /* connection status */
    avio_em_printf(pb, "<h2>Connection Status</h2>\n");

    avio_em_printf(pb, "Number of connections: %d / %d<br>\n",
                nb_connections, config.nb_max_connections);

    avio_em_printf(pb, "Bandwidth in use: %"PRIu64"k / %"PRIu64"k<br>\n",
                current_bandwidth, config.max_bandwidth);

    avio_em_printf(pb, "<table>\n");
    avio_em_printf(pb, "<tr><th>#<th>File<th>IP<th>Proto<th>State<th>Target "
    ...
    ti = time(NULL);
    p = ctime(&ti);
    avio_em_printf(pb, "<hr size=1 noshade>Generated at %s", p);
    avio_em_printf(pb, "</body>\n</html>\n");

    len = avio_em_close_dyn_buf(pb, &c->pb_buffer);
...
static int open_input_stream(HTTPContext *c, const char *info)
{
    ...
    /* open stream */
    ret = avformat_em_open_input(&s, input_filename, c->stream->ifmt,
                              &c->stream->in_opts);
    if (ret < 0) {
    ...
    /* set buffer size */
    if (buf_size > 0) {
        ret = emio_set_buf_size(s->pb, buf_size);
        if (ret < 0) {
    ...
    if (strcmp(s->iformat->name, "ffm") &&
        (ret = avformat_em_find_stream_info(c->fmt_in, NULL)) < 0) {
        http_log("Could not find stream info for input '%s'\n", input_filename);
        avformat_em_close_input(&s);
        return ret;
    }
    ...
    if (c->fmt_in->iformat->read_seek)
        av_em_seek_frame(c->fmt_in, -1, stream_pos, 0);
...
static int http_prepare_data(HTTPContext *c)
{
    ...
    case HTTPSTATE_SEND_DATA_HEADER:
        ...
        for(i=0;i<c->stream->nb_streams;i++) {
            AVEMStream *src;
            c->fmt_ctx.streams[i] = av_mallocz(sizeof(AVEMStream));
            ...
            *(c->fmt_ctx.streams[i]) = *src;
            ...
            /* XXX: should be done in AVEMStream, not in codec */
        ...
        if (avio_em_open_dyn_buf(&c->fmt_ctx.pb) < 0) {
            /* XXX: potential leak */
            return -1;
        }
        ...
        if ((ret = avformat_em_write_header(&c->fmt_ctx, NULL)) < 0) {
        ...
        len = avio_em_close_dyn_buf(c->fmt_ctx.pb, &c->pb_buffer);
        c->buffer_ptr = c->pb_buffer;
        c->buffer_end = c->pb_buffer + len;
    ...
    case HTTPSTATE_SEND_DATA:
        ...
        if (c->stream->max_time &&
            ...
            AVEMPacket pkt;
        redo:
            ret = av_em_read_frame(c->fmt_in, &pkt);
            if (ret < 0) {
                if (c->stream->feed) {
                ...
                if (c->stream->feed) {
                    ...
                    for(i=0;i<c->stream->nb_streams;i++) {
                        if (c->stream->feed_streams[i] == pkt.stream_index) {
                            AVEMStream *st = c->fmt_in->streams[source_index];
                            pkt.stream_index = i;
                            if (pkt.flags & AV_PKT_FLAG_KEY &&
                ...
                send_it:
                    ...
                    if (c->is_packetized) {
                        ...
                        ret = emio_open_dyn_packet_buf(&ctx->pb,
                                                       max_packet_size);
                    } else
                        ret = avio_em_open_dyn_buf(&ctx->pb);
                    ...
                    pkt.duration = av_rescale_q(pkt.duration, ist->time_base,
                                                ost->time_base);
                    if ((ret = av_em_write_frame(ctx, &pkt)) < 0) {
                    ...
                    av_freep(&c->pb_buffer);
                    len = avio_em_close_dyn_buf(ctx->pb, &c->pb_buffer);
                    ctx->pb = NULL;
    ...
    case HTTPSTATE_SEND_DATA_TRAILER:
        ...
        ctx = &c->fmt_ctx;
        /* prepare header */
        if (avio_em_open_dyn_buf(&ctx->pb) < 0) {
            /* XXX: potential leak */
            return -1;
        }
        c->fmt_ctx.pb->seekable = 0;
        ...
        if (c->buffer_ptr >= c->buffer_end) {
            ...
            if (c->is_packetized) {
                ...
                if (c->rtp_protocol == RTSP_LOWER_TRANSPORT_TCP) {
                    ...
                    header[2] = len >> 8;
                    header[3] = len;
                    avio_em_write(pb, header, 4);
                    /* write RTP packet data */
                    c->buffer_ptr += 4;
                    avio_em_write(pb, c->buffer_ptr, len);
                    size = avio_em_close_dyn_buf(pb, &c->packet_buffer);
                    ...
                    c->buffer_ptr += 4;
                    ffurl_em_write(c->rtp_handles[c->packet_stream_index],
                                c->buffer_ptr, len);
                    c->buffer_ptr += len;
...
static int http_receive_data(HTTPContext *c)
{
    ...
    if (c->buffer_ptr >= c->buffer_end) {
        ...
        if (c->data_count > FFM_PACKET_SIZE) {
            ...
            fmt_in = av_em_find_input_format(feed->fmt->name);
            if (!fmt_in)
                goto fail;

            pb = avio_em_alloc_context(c->buffer, c->buffer_end - c->buffer,
            ...
            s->pb = pb;
            if (avformat_em_open_input(&s, c->stream->feed_filename, fmt_in, NULL) < 0) {
                av_freep(&pb);
                goto fail;
            }
...
static void rtsp_reply_header(HTTPContext *c, enum RTSPStatusCode error_number)
{
    ...
    avio_em_printf(c->pb, "RTSP/1.0 %d %s\r\n", error_number, str);
    avio_em_printf(c->pb, "CSeq: %d\r\n", c->seq);

    /* output GMT time */
    ti = time(NULL);
    tm = gmtime(&ti);
    strftime(buf2, sizeof(buf2), "%a, %d %b %Y %H:%M:%S", tm);
    avio_em_printf(c->pb, "Date: %s GMT\r\n", buf2);
}
...
static int rtsp_parse_request(HTTPContext *c)
{
    ...
    if (avio_em_open_dyn_buf(&c->pb) < 0) {
        /* XXX: cannot do more */
        c->pb = NULL; /* safety */
        return -1;
    }
 ...
 the_end:
    len = avio_em_close_dyn_buf(c->pb, &c->pb_buffer);
    c->pb = NULL; /* safety */
    if (len < 0)
        /* XXX: cannot do more */
        return -1;
...
static int prepare_sdp_description(FFServerStream *stream, uint8_t **pbuffer,
                                   struct in_addr my_ip)
{
    AVEMFormatContext *avc;
    AVEMStream *avs = NULL;
    AVEMOutputFormat *rtp_format = av_em_guess_format("rtp", NULL, NULL);
    AVEMDictionaryEntry *entry = av_dict_get(stream->metadata, "title", NULL, 0);
    int i;

    *pbuffer = NULL;

    avc =  avformat_em_alloc_context();
    if (!avc || !rtp_format)
        return -1;
    ...
    *pbuffer = av_mallocz(2048);
    if (!*pbuffer)
        goto sdp_done;
    av_em_sdp_create(&avc, 1, *pbuffer, 2048);
...
static void rtsp_cmd_describe(HTTPContext *c, const char *url)
{
    ...
    av_em_url_split(NULL, 0, NULL, 0, NULL, 0, NULL, path1, sizeof(path1), url);
    path = path1;
    if (*path == '/')
        path++;
 ...
 found:
    ...
    rtsp_reply_header(c, RTSP_STATUS_OK);
    avio_em_printf(c->pb, "Content-Base: %s/\r\n", url);
    avio_em_printf(c->pb, "Content-Type: application/sdp\r\n");
    avio_em_printf(c->pb, "Content-Length: %d\r\n", content_length);
    avio_em_printf(c->pb, "\r\n");
    avio_em_write(c->pb, content, content_length);
    av_free(content);
}
                           ...
                           RTSPMessageHeader *h)
{
 ...
 found:
    ...
    rtsp_reply_header(c, RTSP_STATUS_OK);
    /* session ID */
    avio_em_printf(c->pb, "Session: %s\r\n", rtp_c->session_id);

    switch(rtp_c->rtp_protocol) {
    case RTSP_LOWER_TRANSPORT_UDP:
        rtp_port = ff_rtp_get_local_rtp_port(rtp_c->rtp_handles[stream_index]);
        rtcp_port = ff_rtp_get_local_rtcp_port(rtp_c->rtp_handles[stream_index]);
        avio_em_printf(c->pb, "Transport: RTP/AVP/UDP;unicast;"
                    "client_port=%d-%d;server_port=%d-%d",
                    th->client_port_min, th->client_port_max,
                    rtp_port, rtcp_port);
        break;
    case RTSP_LOWER_TRANSPORT_TCP:
        avio_em_printf(c->pb, "Transport: RTP/AVP/TCP;interleaved=%d-%d",
    ...
    if (setup.transport_option[0] != '\0')
        avio_em_printf(c->pb, ";%s", setup.transport_option);
    avio_em_printf(c->pb, "\r\n");
                             ...
                             HTTPContext *rtsp_c)
{
    ...
    if (!ctx)
        return -1;
    ctx->oformat = av_em_guess_format("rtp", NULL, NULL);

    st = avformat_em_new_stream(ctx, NULL);
    if (!st)
        goto fail;
    ...
    if (!c->stream->feed ||
        c->stream->feed == c->stream)
        memcpy(st, c->stream->streams[stream_index], sizeof(AVEMStream));
    else
        memcpy(st,
    ...
    case RTSP_LOWER_TRANSPORT_UDP_MULTICAST:
        ...
        if (ffurl_em_open(&h, ctx->filename, AVIO_FLAG_WRITE, NULL, NULL) < 0)
            goto fail;
     ...
     * be checked */
    if (emio_open_dyn_packet_buf(&ctx->pb, max_packet_size) < 0)
        /* XXX: close stream */
        goto fail;

    if (avformat_em_write_header(ctx, NULL) < 0) {
    fail:
        if (h)
            ffurl_em_close(h);
        av_free(st);
    ...
    avio_em_close_dyn_buf(ctx->pb, &dummy_buf);
    ctx->pb = NULL;
    av_free(dummy_buf);
...
/* FIXME: This code should use avformat_em_new_stream() */
static AVEMStream *add_av_stream1(FFServerStream *stream,
                                AVEMCodecContext *codec, int copy)
{
    AVEMStream *fst;
    ...
    fst = av_mallocz(sizeof(AVEMStream));
    if (!fst)
        return NULL;
    if (copy) {
    ...
    avpriv_em_set_pts_info(fst, 33, 1, 90000);
...
static int add_av_stream(FFServerStream *feed, AVEMStream *st)
{
    AVEMStream *fst;
    AVEMCodecContext *av, *av1;
    int i;
    ...
    if (!fst)
        return -1;
    if (av_em_stream_get_recommended_encoder_configuration(st))
        av_em_stream_set_recommended_encoder_configuration(fst,
            av_strdup(av_em_stream_get_recommended_encoder_configuration(st)));
...
static void extract_mpeg4_header(AVEMFormatContext *infile)
{
    ...
    printf("MPEG4 without extra data: trying to find header in %s\n",
           infile->filename);
    while (mpeg4_count > 0) {
        if (av_em_read_frame(infile, &pkt) < 0)
            break;
...
/* compute the needed AVEMStream for each file */
static void build_file_streams(void)
{
    FFServerStream *stream;
    AVEMFormatContext *infile;
    int i, ret;
    ...
    for(stream = config.first_stream; stream; stream = stream->next) {
        ...
        ret = avformat_em_open_input(&infile, stream->feed_filename,
        ...
        fail:
            ...
            /* find all the AVEMStreams inside and reference them in
             * 'stream' */
            if (avformat_em_find_stream_info(infile, NULL) < 0) {
                http_log("Could not find codec parameters from '%s'\n",
                         stream->feed_filename);
                avformat_em_close_input(&infile);
                goto fail;
            }
...
static inline
int check_codec_match(AVEMCodecContext *ccf, AVEMCodecContext *ccs, int stream)
{
    int matches = 1;
...
/* compute the needed AVEMStream for each feed */
static int build_feed_streams(void)
{
    ...
    for(feed = config.first_feed; feed; feed = feed->next_feed) {
        ...
        if (avio_em_check(feed->feed_filename, AVIO_FLAG_READ) > 0) {
            AVEMFormatContext *s = NULL;
            int matches = 0;

            /* See if it matches */

            if (avformat_em_open_input(&s, feed->feed_filename, NULL, NULL) < 0) {
            ...
            /* set buffer size */
            if (emio_set_buf_size(s->pb, FFM_PACKET_SIZE) < 0) {
                http_log("Failed to set buffer size\n");
                avformat_em_close_input(&s);
                goto bail;
            }
...
drop:
        ...
        if (avio_em_check(feed->feed_filename, AVIO_FLAG_WRITE) <= 0) {
            AVEMFormatContext *s = avformat_em_alloc_context();
            ...
            if (avio_em_open(&s->pb, feed->feed_filename, AVIO_FLAG_WRITE) < 0) {
            ...
            if (avformat_em_write_header(s, NULL) < 0) {
