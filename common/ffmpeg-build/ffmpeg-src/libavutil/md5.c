...
struct AVMD5 *av_em_md5_alloc(void)
{
    return av_em_mallocz(sizeof(struct AVMD5));
}
...
void av_em_md5_update(AVMD5 *ctx, const uint8_t *src, int len)
{
    const uint8_t *end;
    int j;
...
void av_em_md5_final(AVMD5 *ctx, uint8_t *dst)
{
    int i;
    uint64_t finalcount = av_le2ne64(ctx->len << 3);

    av_em_md5_update(ctx, "\200", 1);
    while ((ctx->len & 63) != 56)
        av_em_md5_update(ctx, "", 1);

    av_em_md5_update(ctx, (uint8_t *) &finalcount, 8);

    for (i = 0; i < 4; i++)
        AV_WL32(dst + 4 * i, ctx->ABCD[3 - i]);
}

void av_em_md5_sum(uint8_t *dst, const uint8_t *src, const int len)
{
    AVMD5 ctx;

    av_em_md5_init(&ctx);
    av_em_md5_update(&ctx, src, len);
    av_em_md5_final(&ctx, dst);
}
