...
struct AVTEA *av_em_tea_alloc(void)
{
    return av_em_mallocz(sizeof(struct AVTEA));
}

const int av_em_tea_size = sizeof(AVTEA);

void av_em_tea_init(AVTEA *ctx, const uint8_t key[16], int rounds)
{
    int i;
...
void av_em_tea_crypt(AVTEA *ctx, uint8_t *dst, const uint8_t *src, int count,
