...
const int av_em_camellia_size = sizeof(AVCAMELLIA);
...
struct AVCAMELLIA *av_em_camellia_alloc(void)
{
    return av_em_mallocz(sizeof(struct AVCAMELLIA));
}

av_cold int av_em_camellia_init(AVCAMELLIA *cs, const uint8_t *key, int key_bits)
{
...
void av_em_camellia_crypt(AVCAMELLIA *cs, uint8_t *dst, const uint8_t *src, int count, uint8_t *iv, int decrypt)
{
    int i;
    while (count--) {
        if (decrypt) {
