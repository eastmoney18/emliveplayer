...
const int av_em_ripemd_size = sizeof(AVRIPEMD);

struct AVRIPEMD *av_em_ripemd_alloc(void)
{
    return av_em_mallocz(sizeof(struct AVRIPEMD));
}
...
av_cold int av_em_ripemd_init(AVRIPEMD *ctx, int bits)
{
...
void av_em_ripemd_update(AVRIPEMD* ctx, const uint8_t* data, unsigned int len)
{
    unsigned int i, j;
...
void av_em_ripemd_final(AVRIPEMD* ctx, uint8_t *digest)
{
    int i;
    uint64_t finalcount = av_le2ne64(ctx->count << 3);

    av_em_ripemd_update(ctx, "\200", 1);
    while ((ctx->count & 63) != 56)
        av_em_ripemd_update(ctx, "", 1);
    av_em_ripemd_update(ctx, (uint8_t *)&finalcount, 8); /* Should cause a transform() */
