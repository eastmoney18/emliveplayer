...
const char *av_em_default_item_name(void *ptr)
{
    return (*(AVEMClass **) ptr)->class_name;
}

AVEMClassCategory av_em_default_get_category(void *ptr)
{
    return (*(AVEMClass **) ptr)->category;
}
...
static void format_line(void *avcl, int level, const char *fmt, va_list vl,
                        AVEMBPrint part[4], int *print_prefix, int type[2])
{
    AVEMClass* avc = avcl ? *(AVEMClass **) avcl : NULL;
    av_em_bprint_init(part+0, 0, 1);
    av_em_bprint_init(part+1, 0, 1);
    av_em_bprint_init(part+2, 0, 1);
    av_em_bprint_init(part+3, 0, 65536);

    if(type) type[0] = type[1] = AV_CLASS_CATEGORY_NA + 16;
    if (*print_prefix && avc) {
        if (avc->parent_log_context_offset) {
            AVEMClass** parent = *(AVEMClass ***) (((uint8_t *) avcl) +
                                   avc->parent_log_context_offset);
            if (parent && *parent) {
                av_em_bprintf(part+0, "[%s @ %p] ",
                         (*parent)->item_name(parent), parent);
                if(type) type[0] = get_category(parent);
            }
        }
        av_em_bprintf(part+1, "[%s @ %p] ",
                 avc->item_name(avcl), avcl);
        if(type) type[1] = get_category(avcl);

        if (flags & AV_LOG_PRINT_LEVEL)
            av_em_bprintf(part+2, "[%s] ", get_level_str(level));
    }

    av_em_vbprintf(part+3, fmt, vl);
...
{
    av_em_log_format_line2(ptr, level, fmt, vl, line, line_size, print_prefix);
}
                        ...
                        char *line, int line_size, int *print_prefix)
{
    ...
    av_em_bprint_finalize(part+3, NULL);
    return ret;
}

void av_em_log_default_callback(void* ptr, int level, const char* fmt, va_list vl)
{
...
static void (*av_log_callback)(void*, int, const char*, va_list) =
    av_em_log_default_callback;

void av_em_log(void* avcl, int level, const char *fmt, ...)
{
    AVEMClass* avc = avcl ? *(AVEMClass **) avcl : NULL;
    va_list vl;
    va_start(vl, fmt);
                                   ...
                                   va_list argument_list)
{
    ...
    av_em_log(avc, AV_LOG_WARNING, " is not implemented. Update your FFmpeg "
...
void avpriv_em_request_sample(void *avc, const char *msg, ...)
{
    va_list argument_list;
...
void avpriv_em_report_missing_feature(void *avc, const char *msg, ...)
{
    va_list argument_list;
