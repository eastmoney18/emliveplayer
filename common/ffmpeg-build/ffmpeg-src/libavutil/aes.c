...
const int av_em_aes_size= sizeof(AVAES);

struct AVAES *av_em_aes_alloc(void)
{
    return av_em_mallocz(sizeof(struct AVAES));
}
...
void av_em_aes_crypt(AVAES *a, uint8_t *dst, const uint8_t *src,
...
int av_em_aes_init(AVAES *a, const uint8_t *key, int key_bits, int decrypt)
{
