...
const int av_em_cast5_size = sizeof(AVCAST5);
...
{
    return av_em_mallocz(sizeof(struct AVCAST5));
}

av_cold int av_em_cast5_init(AVCAST5* cs, const uint8_t *key, int key_bits)
{
    uint8_t newKey[16];
    int i;
    uint32_t p[4], q[4];
...
void av_em_cast5_crypt2(AVCAST5* cs, uint8_t* dst, const uint8_t* src, int count, uint8_t *iv, int decrypt)
{
    int i;
    while (count--) {
        if (decrypt) {
...
void av_em_cast5_crypt(AVCAST5* cs, uint8_t* dst, const uint8_t* src, int count, int decrypt)
{
    while (count--) {
        if (decrypt){
