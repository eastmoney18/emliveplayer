...
const char av_em_util_ffversion[] = "FFmpeg version " FFMPEG_VERSION;

const char *av_em_version_info(void)
{
    return FFMPEG_VERSION;
}

unsigned avutil_em_version(void)
{
    ...
    if (av_sat_dadd32(1, 2) != 5) {
        av_em_log(NULL, AV_LOG_FATAL, "Libavutil has been built with a broken binutils, please upgrade binutils and rebuild\n");
        abort();
    }

    if (llrint(1LL<<60) != 1LL<<60) {
        av_em_log(NULL, AV_LOG_ERROR, "Libavutil has been linked to a broken llrint()\n");
    }
...
const char *avutil_em_configuration(void)
{
    return FFMPEG_CONFIGURATION;
}

const char *avutil_em_license(void)
{
#define LICENSE_PREFIX "libavutil license: "
    return LICENSE_PREFIX FFMPEG_LICENSE + sizeof(LICENSE_PREFIX) - 1;
}

const char *av_em_get_media_type_string(enum AVEMMediaType media_type)
{
    switch (media_type) {
...
char av_em_get_picture_type_char(enum AVPictureType pict_type)
{
    switch (pict_type) {
...
unsigned av_em_int_list_length_for_size(unsigned elsize,
...
AVEMRational av_em_get_time_base_q(void)
{
    return (AVEMRational){1, AV_TIME_BASE};
}
