...
PCA *em_pca_init(int n){
    PCA *pca;
    if(n<=0)
        return NULL;

    pca= av_em_mallocz(sizeof(*pca));
    if (!pca)
        return NULL;

    pca->n= n;
    pca->z = av_em_malloc_array(n, sizeof(*pca->z));
    pca->count=0;
    pca->covariance= av_em_calloc(n*n, sizeof(double));
    pca->mean= av_em_calloc(n, sizeof(double));
...
void em_pca_free(PCA *pca){
    av_em_freep(&pca->covariance);
    av_em_freep(&pca->mean);
    av_em_freep(&pca->z);
    av_em_free(pca);
}
