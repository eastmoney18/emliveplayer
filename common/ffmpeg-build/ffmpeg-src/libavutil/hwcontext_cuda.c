...
static AVEMBufferRef *cuda_pool_alloc(void *opaque, int size)
{
    AVHWFramesContext     *ctx = opaque;
    AVCUDADeviceContext *hwctx = ctx->device_ctx->hwctx;
    ...
    if (err != CUDA_SUCCESS) {
        av_em_log(ctx, AV_LOG_ERROR, "Error setting current CUDA context\n");
        return NULL;
    }
    ...
    ret = av_em_buffer_create((uint8_t*)data, size, cuda_buffer_free, ctx, 0);
    if (!ret) {
        cuMemFree(data);
        goto fail;
    }
...
static int cuda_frames_init(AVHWFramesContext *ctx)
{
    ...
    if (i == FF_ARRAY_ELEMS(supported_formats)) {
        av_em_log(ctx, AV_LOG_ERROR, "Pixel format '%s' is not supported\n",
               av_em_get_pix_fmt_name(ctx->sw_format));
        return AVERROR(ENOSYS);
    }

    av_em_pix_fmt_get_chroma_sub_sample(ctx->sw_format, &priv->shift_width, &priv->shift_height);
    ...
    if (!ctx->pool) {
        ...
        ctx->internal->pool_internal = av_em_buffer_pool_init2(size, ctx, cuda_pool_alloc, NULL);
                                   ...
                                   const AVFrame *src)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(src->data) && src->data[i]; i++) {
        ...
        err = cuMemcpy2D(&cpy);
        if (err != CUDA_SUCCESS) {
            av_em_log(ctx, AV_LOG_ERROR, "Error transferring the data from the CUDA frame\n");
            return AVERROR_UNKNOWN;
        }
    }
                              ...
                              AVEMDictionary *opts, int flags)
{
    ...
    err = cuInit(0);
    if (err != CUDA_SUCCESS) {
        av_em_log(ctx, AV_LOG_ERROR, "Could not initialize the CUDA driver API\n");
        return AVERROR_UNKNOWN;
    }

    err = cuDeviceGet(&cu_device, device_idx);
    if (err != CUDA_SUCCESS) {
        av_em_log(ctx, AV_LOG_ERROR, "Could not get the device number %d\n", device_idx);
        return AVERROR_UNKNOWN;
    }

    err = cuCtxCreate(&hwctx->cuda_ctx, 0, cu_device);
    if (err != CUDA_SUCCESS) {
        av_em_log(ctx, AV_LOG_ERROR, "Error creating a CUDA context\n");
        return AVERROR_UNKNOWN;
    }
...
const HWContextType em_hwcontext_type_cuda = {
