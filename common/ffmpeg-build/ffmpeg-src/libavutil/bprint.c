...
static int av_bprint_alloc(AVEMBPrint *buf, unsigned room)
{
    ...
    new_str = av_em_realloc(old_str, new_size);
    if (!new_str)
...
static void av_bprint_grow(AVEMBPrint *buf, unsigned extra_len)
{
...
void av_em_bprint_init(AVEMBPrint *buf, unsigned size_init, unsigned size_max)
{
...
void av_em_bprint_init_for_buffer(AVEMBPrint *buf, char *buffer, unsigned size)
{
    buf->str      = buffer;
    buf->len      = 0;
    buf->size     = size;
...
void av_em_bprintf(AVEMBPrint *buf, const char *fmt, ...)
{
    unsigned room;
    char *dst;
    va_list vl;
...
void av_em_vbprintf(AVEMBPrint *buf, const char *fmt, va_list vl_arg)
{
    unsigned room;
    char *dst;
    int extra_len;
...
void av_em_bprint_chars(AVEMBPrint *buf, char c, unsigned n)
{
...
void av_em_bprint_append_data(AVEMBPrint *buf, const char *data, unsigned size)
{
    unsigned room, real_n;
...
void av_em_bprint_strftime(AVEMBPrint *buf, const char *fmt, const struct tm *tm)
{
    unsigned room;
    size_t l;
...
void av_em_bprint_get_buffer(AVEMBPrint *buf, unsigned size,
...
void av_em_bprint_clear(AVEMBPrint *buf)
{
    if (buf->len) {
        *buf->str = 0;
        buf->len  = 0;
...
int av_em_bprint_finalize(AVEMBPrint *buf, char **ret_str)
{
    ...
    if (ret_str) {
        if (av_bprint_is_allocated(buf)) {
            str = av_em_realloc(buf->str, real_size);
            if (!str)
                str = buf->str;
            buf->str = NULL;
        } else {
            str = av_em_alloc(real_size);
            if (str)
...
void av_em_bprint_escape(AVEMBPrint *dstbuf, const char *src, const char *special_chars,
                      enum AVEscapeMode mode, int flags)
{
    const char *src0 = src;

    if (mode == av_em_escape_MODE_AUTO)
        mode = av_em_escape_MODE_BACKSLASH; /* TODO: implement a heuristic */

    switch (mode) {
    case av_em_escape_MODE_QUOTE:
        /* enclose the string between '' */
        av_em_bprint_chars(dstbuf, '\'', 1);
        for (; *src; src++) {
            if (*src == '\'')
                av_em_bprintf(dstbuf, "'\\''");
            else
                av_em_bprint_chars(dstbuf, *src, 1);
        }
        av_em_bprint_chars(dstbuf, '\'', 1);
        break;

    /* case av_em_escape_MODE_BACKSLASH or unknown mode */
    default:
        /* \-escape characters */
        for (; *src; src++) {
            ...
            int is_special          =
                ...
                (is_ws && (flags & av_em_escape_FLAG_WHITESPACE));

            if (is_strictly_special ||
                (!(flags & av_em_escape_FLAG_STRICT) &&
