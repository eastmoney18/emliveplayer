...
static void dxva2_frames_uninit(AVHWFramesContext *ctx)
{
    ...
    av_em_freep(&s->surfaces_internal);
...
static AVEMBufferRef *dxva2_pool_alloc(void *opaque, int size)
{
    ...
    if (s->nb_surfaces_used < hwctx->nb_surfaces) {
        s->nb_surfaces_used++;
        return av_em_buffer_create((uint8_t*)s->surfaces_internal[s->nb_surfaces_used - 1],
...
static int dxva2_init_pool(AVHWFramesContext *ctx)
{
    ...
    if (FAILED(hr)) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to open device handle\n");
    ...
    hr = IDirect3DDeviceManager9_GetVideoService(device_hwctx->devmgr,
                                                 ...
                                                 (void **)&s->service);
    if (FAILED(hr)) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to create the video service\n");
        return AVERROR_UNKNOWN;
    }
    ...
    if (i == FF_ARRAY_ELEMS(supported_formats)) {
        av_em_log(ctx, AV_LOG_ERROR, "Unsupported pixel format: %s\n",
               av_em_get_pix_fmt_name(ctx->sw_format));
        return AVERROR(EINVAL);
    }

    s->surfaces_internal = av_em_mallocz_array(ctx->initial_pool_size,
    ...
    if (FAILED(hr)) {
        av_em_log(ctx, AV_LOG_ERROR, "Could not create the surfaces\n");
        return AVERROR_UNKNOWN;
    }

    ctx->internal->pool_internal = av_em_buffer_pool_init2(sizeof(*s->surfaces_internal),
...
static int dxva2_frames_init(AVHWFramesContext *ctx)
{
    ...
    if (hwctx->surface_type != DXVA2_VideoDecoderRenderTarget &&
        ...
        av_em_log(ctx, AV_LOG_ERROR, "Unknown surface type: %lu\n",
               hwctx->surface_type);
        return AVERROR(EINVAL);
    }
    ...
    if (!ctx->pool) {
        ...
        if (ret < 0) {
            av_em_log(ctx, AV_LOG_ERROR, "Error creating an internal frame pool\n");
            return ret;
        }
    }
...
{
    frame->buf[0] = av_em_buffer_pool_get(ctx->pool);
    if (!frame->buf[0])
        return AVERROR(ENOMEM);
                                      ...
                                      enum AVPixelFormat **formats)
{
    ...
    fmts = av_em_malloc_array(2, sizeof(*fmts));
    if (!fmts)
        return AVERROR(ENOMEM);
                               ...
                               const AVFrame *src)
{
    ...
    if (FAILED(hr)) {
        av_em_log(ctx, AV_LOG_ERROR, "Error getting a surface description\n");
        return AVERROR_UNKNOWN;
    }
    ...
    if (FAILED(hr)) {
        av_em_log(ctx, AV_LOG_ERROR, "Unable to lock DXVA2 surface\n");
        return AVERROR_UNKNOWN;
    }

    if (download) {
        av_em_image_copy_plane(dst->data[0], dst->linesize[0],
                            (uint8_t*)LockedRect.pBits, LockedRect.Pitch,
                            src->width * bytes_per_component, src->height);
        av_em_image_copy_plane(dst->data[1], dst->linesize[1],
                            (uint8_t*)LockedRect.pBits + LockedRect.Pitch * surfaceDesc.Height,
                            LockedRect.Pitch, src->width * bytes_per_component, src->height / 2);
    } else {
        av_em_image_copy_plane((uint8_t*)LockedRect.pBits, LockedRect.Pitch,
                            dst->data[0], dst->linesize[0],
                            src->width * bytes_per_component, src->height);
        av_em_image_copy_plane((uint8_t*)LockedRect.pBits + LockedRect.Pitch * surfaceDesc.Height,
                               ...
                               AVEMDictionary *opts, int flags)
{
    ...
    if (!priv->d3dlib) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to load D3D9 library\n");
        return AVERROR_UNKNOWN;
    }
    priv->dxva2lib = LoadLibrary("dxva2.dll");
    if (!priv->dxva2lib) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to load DXVA2 library\n");
        return AVERROR_UNKNOWN;
    }

    createD3D = (pDirect3DCreate9 *)GetProcAddress(priv->d3dlib, "Direct3DCreate9");
    if (!createD3D) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to locate Direct3DCreate9\n");
        return AVERROR_UNKNOWN;
    }
    createDeviceManager = (pCreateDeviceManager9 *)GetProcAddress(priv->dxva2lib,
                                                                  "DXVA2CreateDirect3DDeviceManager9");
    if (!createDeviceManager) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to locate DXVA2CreateDirect3DDeviceManager9\n");
        return AVERROR_UNKNOWN;
    }

    priv->d3d9 = createD3D(D3D_SDK_VERSION);
    if (!priv->d3d9) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to create IDirect3D object\n");
        return AVERROR_UNKNOWN;
    }
    ...
    if (FAILED(hr)) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to create Direct3D device\n");
        return AVERROR_UNKNOWN;
    }

    hr = createDeviceManager(&resetToken, &hwctx->devmgr);
    if (FAILED(hr)) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to create Direct3D device manager\n");
        return AVERROR_UNKNOWN;
    }

    hr = IDirect3DDeviceManager9_ResetDevice(hwctx->devmgr, priv->d3d9device, resetToken);
    if (FAILED(hr)) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to bind Direct3D device to device manager\n");
        return AVERROR_UNKNOWN;
    }
...
const HWContextType em_hwcontext_type_dxva2 = {
