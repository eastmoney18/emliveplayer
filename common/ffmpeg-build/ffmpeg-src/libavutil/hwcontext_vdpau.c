...
static int vdpau_init_pixmfts(AVHWDeviceContext *ctx)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(priv->pix_fmts); i++) {
        ...
        priv->pix_fmts[i] = av_em_malloc_array(nb_pix_fmts + 1, sizeof(*priv->pix_fmts[i]));
        if (!priv->pix_fmts[i])
...
do {                                                                            \
    ...
    if (err != VDP_STATUS_OK) {                                                 \
        av_em_log(ctx, AV_LOG_ERROR, "Error getting the " #id " callback.\n");     \
    ...
    if (ret < 0) {
        av_em_log(ctx, AV_LOG_ERROR, "Error querying the supported pixel formats\n");
        return ret;
    }
...
static AVEMBufferRef *vdpau_pool_alloc(void *opaque, int size)
{
    AVHWFramesContext             *ctx = opaque;
    ...
    if (err != VDP_STATUS_OK) {
        av_em_log(ctx, AV_LOG_ERROR, "Error allocating a VDPAU video surface\n");
        return NULL;
    }

    ret = av_em_buffer_create((uint8_t*)(uintptr_t)surf, sizeof(surf),
...
static int vdpau_frames_init(AVHWFramesContext *ctx)
{
    ...
    default:
        av_em_log(ctx, AV_LOG_ERROR, "Unsupported data layout: %s\n",
    ...
    if (!priv->pix_fmts) {
        av_em_log(ctx, AV_LOG_ERROR, "Unsupported chroma type: %d\n", priv->chroma_type);
        return AVERROR(ENOSYS);
    }

    if (!ctx->pool) {
        ctx->internal->pool_internal = av_em_buffer_pool_init2(sizeof(VdpVideoSurface), ctx,
                                      ...
                                      enum AVPixelFormat **formats)
{
    ...
    fmts = av_em_malloc_array(priv->nb_pix_fmts, sizeof(*fmts));
    if (!fmts)
        return AVERROR(ENOMEM);
                                    ...
                                    const AVFrame *src)
{
    ...
    if (map[i].pix_fmt == AV_PIX_FMT_NONE) {
        av_em_log(ctx, AV_LOG_ERROR,
               ...
               av_em_get_pix_fmt_name(dst->format));
    ...
    if (err != VDP_STATUS_OK) {
        av_em_log(ctx, AV_LOG_ERROR, "Error retrieving the data from a VDPAU surface\n");
        return AVERROR_UNKNOWN;
    }
                                  ...
                                  const AVFrame *src)
{
    ...
    if (map[i].pix_fmt == AV_PIX_FMT_NONE) {
        av_em_log(ctx, AV_LOG_ERROR,
               ...
               av_em_get_pix_fmt_name(src->format));
        return AVERROR(EINVAL);
    }
    ...
    if (err != VDP_STATUS_OK) {
        av_em_log(ctx, AV_LOG_ERROR, "Error uploading the data to a VDPAU surface\n");
        return AVERROR_UNKNOWN;
    }
                               ...
                               AVEMDictionary *opts, int flags)
{
    ...
    if (!priv->dpy) {
        av_em_log(ctx, AV_LOG_ERROR, "Cannot open the X11 display %s.\n",
               XDisplayName(device));
        return AVERROR_UNKNOWN;
    }
    ...
    if (err != VDP_STATUS_OK) {
        av_em_log(ctx, AV_LOG_ERROR, "VDPAU device creation on X11 display %s failed.\n",
               display);
        return AVERROR_UNKNOWN;
    }
...
do {                                                                            \
    ...
    av_em_log(ctx, AV_LOG_VERBOSE, "Successfully created a VDPAU device (%s) on "
           "X11 display %s\n", vendor, display);
...
const HWContextType em_hwcontext_type_vdpau = {
