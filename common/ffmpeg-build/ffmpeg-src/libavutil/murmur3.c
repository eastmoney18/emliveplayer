...
AVMurMur3 *av_em_murmur3_alloc(void)
{
    return av_em_mallocz(sizeof(AVMurMur3));
}

void av_em_murmur3_init_seeded(AVMurMur3 *c, uint64_t seed)
{
    memset(c, 0, sizeof(*c));
    c->h1 = c->h2 = seed;
}

void av_em_murmur3_init(AVMurMur3 *c)
{
    // arbitrary random number as seed
    av_em_murmur3_init_seeded(c, 0x725acc55daddca55);
}
...
void av_em_murmur3_update(AVMurMur3 *c, const uint8_t *src, int len)
{
    const uint8_t *end;
...
void av_em_murmur3_final(AVMurMur3 *c, uint8_t dst[16])
{
    uint64_t h1 = c->h1, h2 = c->h2;
