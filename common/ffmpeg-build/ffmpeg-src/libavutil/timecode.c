...
int av_em_timecode_adjust_ntsc_framenum2(int framenum, int fps)
{
...
uint32_t av_em_timecode_get_smpte_from_framenum(const AVTimecode *tc, int framenum)
{
    unsigned fps = tc->fps;
    ...
    framenum += tc->start;
    if (drop)
        framenum = av_em_timecode_adjust_ntsc_framenum2(framenum, tc->fps);
    ff = framenum % fps;
    ss = framenum / fps      % 60;
...
char *av_em_timecode_make_string(const AVTimecode *tc, char *buf, int framenum)
{
    int fps = tc->fps;
    int drop = tc->flags & AV_TIMECODE_FLAG_DROPFRAME;
    ...
    framenum += tc->start;
    if (drop)
        framenum = av_em_timecode_adjust_ntsc_framenum2(framenum, fps);
    if (framenum < 0) {
        framenum = -framenum;
...
char *av_em_timecode_make_smpte_tc_string(char *buf, uint32_t tcsmpte, int prevent_df)
{
...
char *av_em_timecode_make_mpeg_tc_string(char *buf, uint32_t tc25bit)
{
    snprintf(buf, AV_TIMECODE_STR_SIZE, "%02u:%02u:%02u%c%02u",
...
{
    if ((int)tc->fps <= 0) {
        av_em_log(log_ctx, AV_LOG_ERROR, "Timecode frame rate must be specified\n");
        return AVERROR(EINVAL);
    }
    if ((tc->flags & AV_TIMECODE_FLAG_DROPFRAME) && tc->fps != 30 && tc->fps != 60) {
        av_em_log(log_ctx, AV_LOG_ERROR, "Drop frame is only allowed with 30000/1001 or 60000/1001 FPS\n");
        return AVERROR(EINVAL);
    }
    if (check_fps(tc->fps) < 0) {
        av_em_log(log_ctx, AV_LOG_WARNING, "Using non-standard frame rate %d/%d\n",
               tc->rate.num, tc->rate.den);
    }
    return 0;
}

static int fps_from_frame_rate(AVEMRational rate)
{
...
int av_em_timecode_check_frame_rate(AVEMRational rate)
{
    return check_fps(fps_from_frame_rate(rate));
}

int av_em_timecode_init(AVTimecode *tc, AVEMRational rate, int flags, int frame_start, void *log_ctx)
{
...
int av_em_timecode_init_from_string(AVTimecode *tc, AVEMRational rate, const char *str, void *log_ctx)
{
    char c;
    int hh, mm, ss, ff, ret;

    if (sscanf(str, "%d:%d:%d%c%d", &hh, &mm, &ss, &c, &ff) != 5) {
        av_em_log(log_ctx, AV_LOG_ERROR, "Unable to parse timecode, "
                                      "syntax: hh:mm:ss[:;.]ff\n");
