...
struct AVAESCTR *av_em_aes_ctr_alloc(void)
{
    return av_em_mallocz(sizeof(struct AVAESCTR));
}

void av_em_aes_ctr_set_iv(struct AVAESCTR *a, const uint8_t* iv)
{
...
const uint8_t* av_em_aes_ctr_get_iv(struct AVAESCTR *a)
{
    return a->counter;
}

void av_em_aes_ctr_set_random_iv(struct AVAESCTR *a)
{
    uint32_t iv[2];

    iv[0] = av_em_get_random_seed();
    iv[1] = av_em_get_random_seed();

    av_em_aes_ctr_set_iv(a, (uint8_t*)iv);
}

int av_em_aes_ctr_init(struct AVAESCTR *a, const uint8_t *key)
{
    a->aes = av_em_aes_alloc();
    if (!a->aes) {
        return AVERROR(ENOMEM);
    }

    av_em_aes_init(a->aes, key, 128, 0);
...
void av_em_aes_ctr_free(struct AVAESCTR *a)
{
    if (a) {
        av_em_freep(&a->aes);
        av_em_free(a);
...
void av_em_aes_ctr_increment_iv(struct AVAESCTR *a)
{
...
void av_em_aes_ctr_crypt(struct AVAESCTR *a, uint8_t *dst, const uint8_t *src, int count)
{
    ...
    while (src < src_end) {
        if (a->block_offset == 0) {
            av_em_aes_crypt(a->aes, a->encrypted_counter, a->counter, 1, NULL, 0);
