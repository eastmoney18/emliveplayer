...
void av_em_force_cpu_flags(int arg){
    if (   (arg & ( AV_CPU_FLAG_3DNOW    |
                    ...
                    AV_CPU_FLAG_AVX2     ))
        && !(arg & AV_CPU_FLAG_MMX)) {
        av_em_log(NULL, AV_LOG_WARNING, "MMX implied by specified flags\n");
        arg |= AV_CPU_FLAG_MMX;
    }
...
int av_em_get_cpu_flags(void)
{
    if (checked)
        return flags;

    if (ARCH_AARCH64)
        flags = em_get_cpu_flags_aarch64();
    if (ARCH_ARM)
        flags = em_get_cpu_flags_arm();
    if (ARCH_PPC)
        flags = em_get_cpu_flags_ppc();
    if (ARCH_X86)
        flags = em_get_cpu_flags_x86();

    checked = 1;
    return flags;
}

void av_em_set_cpu_flags_mask(int mask)
{
    checked       = 0;
    flags         = av_em_get_cpu_flags() & mask;
    checked       = 1;
}

int av_em_parse_cpu_flags(const char *s)
{
...
#define CPUFLAG_AESNI    (AV_CPU_FLAG_AESNI    | CPUFLAG_SSE42)
    ...
    if ((ret = av_em_opt_eval_flags(&pclass, &cpuflags_opts[0], s, &flags)) < 0)
        return ret;

    return flags & INT_MAX;
}

int av_em_parse_cpu_caps(unsigned *flags, const char *s)
{
        static const AVOption cpuflags_opts[] = {
        { "flags"   , NULL, 0, AV_OPT_TYPE_FLAGS, { .i64 = 0 }, INT64_MIN, INT64_MAX, .unit = "flags" },
#if   ARCH_PPC
...
#define CPU_FLAG_K8  CPU_FLAG_ATHLONXP | AV_CPU_FLAG_SSE2
    ...
    return av_em_opt_eval_flags(&pclass, &cpuflags_opts[0], s, flags);
}

int av_em_cpu_count(void)
{
    ...
    if (!printed) {
        av_em_log(NULL, AV_LOG_DEBUG, "detected %d logical cores\n", nb_cpus);
        printed = 1;
    }
