...
static const AVEMClass eval_class = { "Eval", av_em_default_item_name, NULL, LIBAVUTIL_VERSION_INT, offsetof(Parser,log_offset), offsetof(Parser,log_ctx) };
...
double av_em_strtod(const char *numstr, char **tail)
{
    double d;
    char *next;
...
static double eval_expr(Parser *p, AVExpr *e)
{
    switch (e->type) {
        ...
        case e_root: {
            ...
            for(i=-1; i<1024; i++) {
                if(i<255) {
                    p->var[0] = em_reverse[i&255]*x_max/255;
                } else {
        ...
        default: {
            ...
            switch (e->type) {
                ...
                case e_gcd: return e->value * av_em_gcd(d,d2);
...
void av_em_expr_free(AVExpr *e)
{
    if (!e) return;
    av_em_expr_free(e->param[0]);
    av_em_expr_free(e->param[1]);
    av_em_expr_free(e->param[2]);
    av_em_freep(&e->var);
    av_em_freep(&e);
}
...
{
    AVExpr *d = av_em_mallocz(sizeof(AVExpr));
    ...
    p->s= strchr(p->s, '(');
    if (!p->s) {
        av_em_log(p, AV_LOG_ERROR, "Undefined constant or missing '(' in '%s'\n", s0);
        p->s= next;
        av_em_expr_free(d);
    ...
    if (*next == '(') { // special case do-nothing
        ...
        if ((ret = parse_expr(&d, p)) < 0)
            return ret;
        if (p->s[0] != ')') {
            av_em_log(p, AV_LOG_ERROR, "Missing ')' in '%s'\n", s0);
            av_em_expr_free(d);
            return AVERROR(EINVAL);
        }
        p->s++; // ")"
    ...
    if (p->s[0] != ')') {
        av_em_log(p, AV_LOG_ERROR, "Missing ')' or too many args in '%s'\n", s0);
        av_em_expr_free(d);
        return AVERROR(EINVAL);
    }
    p->s++; // ")"
    ...
    d->type = e_func0;
        ...
        av_em_log(p, AV_LOG_ERROR, "Unknown function in '%s'\n", s0);
        av_em_expr_free(d);
        return AVERROR(EINVAL);
    }
...
{
    AVExpr *e = av_em_mallocz(sizeof(AVExpr));
    if (!e)
        return NULL;
    e->type     =type   ;
    e->value    =value  ;
...
int av_em_expr_parse(AVExpr **expr, const char *s,
                  ...
                  int log_offset, void *log_ctx)
{
    ...
    *wp++ = 0;
    ...
    if ((ret = parse_expr(&e, &p)) < 0)
        goto end;
    if (*p.s) {
        av_em_log(&p, AV_LOG_ERROR, "Invalid chars '%s' at the end of expression '%s'\n", p.s, s0);
        ret = AVERROR(EINVAL);
        goto end;
    }
    if (!verify_expr(e)) {
    ...
    e->var= av_em_mallocz(sizeof(double) *VARS);
    if (!e->var) {
        ret = AVERROR(ENOMEM);
        goto end;
    }
...
double av_em_expr_eval(AVExpr *e, const double *const_values, void *opaque)
{
    Parser p = { 0 };
    p.var= e->var;
...
int av_em_expr_parse_and_eval(double *d, const char *s,
...
{
    AVExpr *e = NULL;
    int ret = av_em_expr_parse(&e, s, const_names, func1_names, funcs1, func2_names, funcs2, log_offset, log_ctx);

    if (ret < 0) {
        *d = NAN;
        return ret;
    }
    *d = av_em_expr_eval(e, const_values, opaque);
    av_em_expr_free(e);
