...
{
    return av_em_mallocz(sizeof(struct AVDES));
}

int av_em_des_init(AVDES *d, const uint8_t *key, int key_bits, av_unused int decrypt) {
...
static void av_em_des_crypt_mac(AVDES *d, uint8_t *dst, const uint8_t *src,
...
void av_em_des_crypt(AVDES *d, uint8_t *dst, const uint8_t *src,
                  int count, uint8_t *iv, int decrypt)
{
    av_em_des_crypt_mac(d, dst, src, count, iv, decrypt, 0);
}

void av_em_des_mac(AVDES *d, uint8_t *dst, const uint8_t *src, int count)
{
    av_em_des_crypt_mac(d, dst, src, count, (uint8_t[8]) { 0 }, 0, 1);
}
