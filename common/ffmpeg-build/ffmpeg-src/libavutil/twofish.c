...
{
    return av_em_mallocz(sizeof(struct AVTWOFISH));
}

const int av_em_twofish_size = sizeof(AVTWOFISH);
...
av_cold int av_em_twofish_init(AVTWOFISH *cs, const uint8_t *key, int key_bits)
{
    int i;
    uint8_t keypad[32];
...
void av_em_twofish_crypt(AVTWOFISH *cs, uint8_t *dst, const uint8_t *src, int count, uint8_t *iv, int decrypt)
{
    int i;
    while (count--) {
        if (decrypt) {
