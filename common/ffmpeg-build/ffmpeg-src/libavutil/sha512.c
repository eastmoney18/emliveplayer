...
const int av_em_sha512_size = sizeof(AVSHA512);

struct AVSHA512 *av_em_sha512_alloc(void)
{
    return av_em_mallocz(sizeof(struct AVSHA512));
}
...
av_cold int av_em_sha512_init(AVSHA512 *ctx, int bits)
{
...
void av_em_sha512_update(AVSHA512* ctx, const uint8_t* data, unsigned int len)
{
    unsigned int i, j;
...
void av_em_sha512_final(AVSHA512* ctx, uint8_t *digest)
{
    uint64_t i = 0;
    uint64_t finalcount = av_be2ne64(ctx->count << 3);

    av_em_sha512_update(ctx, "\200", 1);
    while ((ctx->count & 127) != 112)
        av_em_sha512_update(ctx, "", 1);
    av_em_sha512_update(ctx, (uint8_t *)&i, 8);
    av_em_sha512_update(ctx, (uint8_t *)&finalcount, 8); /* Should cause a transform() */
