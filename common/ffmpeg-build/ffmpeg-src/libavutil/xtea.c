...
AVXTEA *av_em_xtea_alloc(void)
{
    return av_em_mallocz(sizeof(struct AVXTEA));
}

void av_em_xtea_init(AVXTEA *ctx, const uint8_t key[16])
{
    int i;
...
void av_em_xtea_le_init(AVXTEA *ctx, const uint8_t key[16])
{
    int i;
...
void av_em_xtea_crypt(AVXTEA *ctx, uint8_t *dst, const uint8_t *src, int count,
                   uint8_t *iv, int decrypt)
{
    xtea_crypt(ctx, dst, src, count, iv, decrypt, xtea_crypt_ecb);
}

void av_em_xtea_le_crypt(AVXTEA *ctx, uint8_t *dst, const uint8_t *src, int count,
                      uint8_t *iv, int decrypt)
{
