...
static const AVEMClass openclutils_class = {
    .class_name                = "opencl",
...
const char *av_em_opencl_errstr(cl_int status)
{
    int i;
...
static void free_device_list(AVOpenCLDeviceList *device_list)
{
    ...
    for (i = 0; i < device_list->platform_num; i++) {
        ...
        for (j = 0; j < device_list->platform_node[i]->device_num; j++) {
            av_em_freep(&(device_list->platform_node[i]->device_node[j]->device_name));
            av_em_freep(&(device_list->platform_node[i]->device_node[j]));
        }
        av_em_freep(&device_list->platform_node[i]->device_node);
        av_em_freep(&(device_list->platform_node[i]->platform_name));
        av_em_freep(&device_list->platform_node[i]);
    }
    av_em_freep(&device_list->platform_node);
...
static int get_device_list(AVOpenCLDeviceList *device_list)
{
    ...
    if (status != CL_SUCCESS) {
        av_em_log(&opencl_ctx, AV_LOG_ERROR,
               "Could not get OpenCL platform ids: %s\n", av_em_opencl_errstr(status));
        return AVERROR_EXTERNAL;
    }
    platform_ids = av_em_mallocz_array(device_list->platform_num, sizeof(cl_platform_id));
    if (!platform_ids)
    ...
    device_list->platform_node = av_em_mallocz_array(device_list->platform_num, sizeof(AVOpenCLPlatformNode *));
    if (!device_list->platform_node) {
        ret = AVERROR(ENOMEM);
        goto end;
    }
    devices_num = av_em_mallocz(sizeof(int) * FF_ARRAY_ELEMS(device_type));
    if (!devices_num) {
        ret = AVERROR(ENOMEM);
        goto end;
    }
    for (i = 0; i < device_list->platform_num; i++) {
        device_list->platform_node[i] = av_em_mallocz(sizeof(AVOpenCLPlatformNode));
        if (!device_list->platform_node[i]) {
            ret = AVERROR(ENOMEM);
            goto end;
        }
        ...
        if (status != CL_SUCCESS) {
            av_em_log(&opencl_ctx, AV_LOG_WARNING,
                    "Could not get size of platform name: %s\n", av_em_opencl_errstr(status));
        } else {
            device_list->platform_node[i]->platform_name = av_em_alloc(platform_name_size * sizeof(char));
        ...
        device_list->platform_node[i]->device_node = av_em_mallocz_array(total_devices_num, sizeof(AVOpenCLDeviceNode *));
        ...
        for (j = 0; j < FF_ARRAY_ELEMS(device_type); j++) {
            if (devices_num[j]) {
                device_ids = av_em_mallocz_array(devices_num[j], sizeof(cl_device_id));
                if (!device_ids) {
                    ret = AVERROR(ENOMEM);
                    goto end;
                }
                ...
                if (status != CL_SUCCESS) {
                    ...
                    av_em_freep(&device_ids);
                    continue;
                }
                for (k = 0; k < devices_num[j]; k++) {
                    device_num = device_list->platform_node[i]->device_num;
                    device_list->platform_node[i]->device_node[device_num] = av_em_mallocz(sizeof(AVOpenCLDeviceNode));
                    ...
                    device_node->device_name = av_em_alloc(device_name_size * sizeof(char));
    ...
    av_em_freep(&platform_ids);
    av_em_freep(&devices_num);
    av_em_freep(&device_ids);
    if (ret < 0)
...
int av_em_opencl_get_device_list(AVOpenCLDeviceList **device_list)
{
    int ret = 0;
    *device_list = av_em_mallocz(sizeof(AVOpenCLDeviceList));
    if (!(*device_list)) {
        av_em_log(&opencl_ctx, AV_LOG_ERROR, "Could not allocate opencl device list\n");
        return AVERROR(ENOMEM);
    }
    ret = get_device_list(*device_list);
    if (ret < 0) {
        av_em_log(&opencl_ctx, AV_LOG_ERROR, "Could not get device list from environment\n");
        free_device_list(*device_list);
        av_em_freep(device_list);
        return ret;
    }
    return ret;
}

void av_em_opencl_free_device_list(AVOpenCLDeviceList **device_list)
{
    free_device_list(*device_list);
    av_em_freep(device_list);
}
...
static inline int init_opencl_mtx(void)
{
    ...
    if (!atomic_opencl_lock) {
        ...
        if (avpriv_em_atomic_ptr_cas((void * volatile *)&atomic_opencl_lock, NULL, tmp)) {
...
int av_em_opencl_set_option(const char *key, const char *val)
{
    ...
    if (!opencl_ctx.opt_init_flag) {
        av_em_opt_set_defaults(&opencl_ctx);
        opencl_ctx.opt_init_flag = 1;
    }
    ret = av_em_opt_set(&opencl_ctx, key, val, 0);
    UNLOCK_OPENCL;
    return ret;
}

int av_em_opencl_get_option(const char *key, uint8_t **out_val)
{
    int ret = 0;
    LOCK_OPENCL;
    ret = av_em_opt_get(&opencl_ctx, key, 0, out_val);
    UNLOCK_OPENCL;
    return ret;
}

void av_em_opencl_free_option(void)
{
    /*FIXME: free openclutils context*/
    LOCK_OPENCL;
    av_em_opt_free(&opencl_ctx);
    UNLOCK_OPENCL;
}

AVOpenCLExternalEnv *av_em_opencl_alloc_external_env(void)
{
    AVOpenCLExternalEnv *ext = av_em_mallocz(sizeof(AVOpenCLExternalEnv));
    if (!ext) {
        av_em_log(&opencl_ctx, AV_LOG_ERROR,
...
void av_em_opencl_free_external_env(AVOpenCLExternalEnv **ext_opencl_env)
{
    av_em_freep(ext_opencl_env);
}

int av_em_opencl_register_kernel_code(const char *kernel_code)
{
    ...
    for (i = 0; i < opencl_ctx.kernel_code_count; i++) {
        if (opencl_ctx.kernel_code[i].kernel_string == kernel_code) {
            av_em_log(&opencl_ctx, AV_LOG_WARNING, "Same kernel code has been registered\n");
            goto end;
        }
    }
...
cl_program av_em_opencl_compile(const char *program_name, const char *build_opts)
{
    int i;
    ...
    for (i = 0; i < opencl_ctx.kernel_code_count; i++) {
        ...
        ptr = av_em_stristr(opencl_ctx.kernel_code[i].kernel_string, program_name);
    ...
    if(status != CL_SUCCESS) {
        av_em_log(&opencl_ctx, AV_LOG_ERROR,
               "Unable to create OpenCL program '%s': %s\n", program_name, av_em_opencl_errstr(status));
        program = NULL;
        goto end;
    }
        ...
        if (log) {
            ...
            if (status != CL_SUCCESS) {
                ...
                int level = build_status == CL_SUCCESS ? AV_LOG_DEBUG :
                                                         AV_LOG_ERROR;
                av_em_log(&opencl_ctx, level, "Compilation log:\n%s\n", log);
            }
        }
        av_em_freep(&log);
    }
    if (build_status != CL_SUCCESS) {
        av_em_log(&opencl_ctx, AV_LOG_ERROR,
               ...
               program_name, av_em_opencl_errstr(build_status));
        program = NULL;
        goto end;
    }
...
cl_command_queue av_em_opencl_get_command_queue(void)
{
        ...
        if (!opencl_ctx->is_user_created) {
            ...
            if (opencl_ctx->platform_idx >= 0) {
                if (opencl_ctx->device_list.platform_num < opencl_ctx->platform_idx + 1) {
                    av_em_log(opencl_ctx, AV_LOG_ERROR, "User set platform index not exist\n");
                    return AVERROR(EINVAL);
                }
                if (!opencl_ctx->device_list.platform_node[opencl_ctx->platform_idx]->device_num) {
                    av_em_log(opencl_ctx, AV_LOG_ERROR, "No devices in user specific platform with index %d\n",
            ...
            if (!opencl_ctx->platform_id) {
                av_em_log(opencl_ctx, AV_LOG_ERROR, "Could not get OpenCL platforms\n");
                return AVERROR_EXTERNAL;
            }
            /* get a usable device*/
             ...
             */
            av_em_log(opencl_ctx, AV_LOG_VERBOSE, "Platform Name: %s, Device Name: %s\n",
...
int av_em_opencl_init(AVOpenCLExternalEnv *ext_opencl_env)
{
    int ret = init_opencl_mtx( );
    if (ret < 0)
        return ret;
...
void av_em_opencl_uninit(void)
{
    ...
    if (opencl_ctx.init_count <= 0)
        av_em_opt_free(&opencl_ctx); //FIXME: free openclutils context
    UNLOCK_OPENCL;
}

int av_em_opencl_buffer_create(cl_mem *cl_buf, size_t cl_buf_size, int flags, void *host_ptr)
{
    cl_int status;
    *cl_buf = clCreateBuffer(opencl_ctx.context, flags, cl_buf_size, host_ptr, &status);
    if (status != CL_SUCCESS) {
        av_em_log(&opencl_ctx, AV_LOG_ERROR, "Could not create OpenCL buffer: %s\n", av_em_opencl_errstr(status));
        return AVERROR_EXTERNAL;
    }
    return 0;
}

void av_em_opencl_buffer_release(cl_mem *cl_buf)
{
    cl_int status = 0;
    if (!cl_buf)
        return;
...
int av_em_opencl_buffer_write(cl_mem dst_cl_buf, uint8_t *src_buf, size_t buf_size)
{
    cl_int status;
...
int av_em_opencl_buffer_read(uint8_t *dst_buf, cl_mem src_cl_buf, size_t buf_size)
{
    cl_int status;
...
int av_em_opencl_buffer_write_image(cl_mem dst_cl_buf, size_t cl_buffer_size, int dst_cl_offset,
...
int av_em_opencl_buffer_read_image(uint8_t **dst_data, int *plane_size, int plane_num,
...
int64_t av_em_opencl_benchmark(AVOpenCLDeviceNode *device_node, cl_platform_id platform,
                            int64_t (*benchmark)(AVOpenCLExternalEnv *ext_opencl_env))
{
    ...
    ext_opencl_env = av_em_opencl_alloc_external_env();
    ext_opencl_env->device_id = device_node->device_id;
    ext_opencl_env->device_type = device_node->device_type;
    av_em_log(&opencl_ctx, AV_LOG_VERBOSE, "Performing test on OpenCL device %s\n",
           device_node->device_name);
    ...
    if (ret < 0)
        av_em_log(&opencl_ctx, AV_LOG_ERROR, "Benchmark failed with OpenCL device %s\n",
               device_node->device_name);
end:
    ...
    av_em_opencl_free_external_env(&ext_opencl_env);
    return ret;
}
