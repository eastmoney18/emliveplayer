                          ...
                          float max_diff)
{
    ...
    for (i = 0; i < len; i++) {
        if (fabsf(a[i] - b[i]) > max_diff) {
            av_em_log(NULL, AV_LOG_ERROR, "%d: %- .12f - %- .12f = % .12g\n",
                   i, a[i], b[i], a[i] - b[i]);
            return -1;
        }
    }
                            ...
                            const float *v1, const float *v2)
{
    ...
    if (ret = compare_floats(cdst, odst, LEN, FLT_EPSILON))
        av_em_log(NULL, AV_LOG_ERROR, "vector_fmul failed\n");
                                   ...
                                   const float *v1, const float *src0, float scale)
{
    ...
    if (ret = compare_floats(cdst, odst, LEN, ARBITRARY_FMAC_SCALAR_CONST))
        av_em_log(NULL, AV_LOG_ERROR, "vector_fmac_scalar failed\n");
                                   ...
                                   const float *v1, float scale)
{
    ...
    if (ret = compare_floats(cdst, odst, LEN, FLT_EPSILON))
        av_em_log(NULL, AV_LOG_ERROR, "vector_fmul_scalar failed\n");
                                   ...
                                   const double *v1, double scale)
{
    ...
    if (ret = compare_doubles(cdst, odst, LEN, DBL_EPSILON))
        av_em_log(NULL, AV_LOG_ERROR, "vector_dmul_scalar failed\n");
                                   ...
                                   const float *v1, const float *v2, const float *v3)
{
    ...
    if (ret = compare_floats(cdst, odst, LEN, ARBITRARY_FMUL_WINDOW_CONST))
        av_em_log(NULL, AV_LOG_ERROR, "vector_fmul_window failed\n");
                                ...
                                const float *v1, const float *v2, const float *v3)
{
    ...
    if (ret = compare_floats(cdst, odst, LEN, ARBITRARY_FMUL_ADD_CONST))
        av_em_log(NULL, AV_LOG_ERROR, "vector_fmul_add failed\n");
                                    ...
                                    const float *v1, const float *v2)
{
    ...
    if (ret = compare_floats(cdst, odst, LEN, FLT_EPSILON))
        av_em_log(NULL, AV_LOG_ERROR, "vector_fmul_reverse failed\n");
                                  ...
                                  const float *v1, const float *v2)
{
    ...
    if ((ret = compare_floats(cv1, ov1, LEN, FLT_EPSILON)) ||
        ...
        av_em_log(NULL, AV_LOG_ERROR, "butterflies_float failed\n");
                                    ...
                                    const float *v1, const float *v2)
{
    ...
    if (ret = compare_floats(&cprod, &oprod, 1, ARBITRARY_SCALARPRODUCT_CONST))
        av_em_log(NULL, AV_LOG_ERROR, "scalarproduct_float failed\n");
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        case 'c':
        {
            int cpuflags = av_em_get_cpu_flags();
    ...
    if (!seeded)
        seed = av_em_get_random_seed();

    av_em_log(NULL, AV_LOG_INFO, "float_dsp-test: %s %u\n", seeded ? "seed" : "random seed", seed);

    fdsp = avpriv_em_float_dsp_alloc(1);
    av_em_force_cpu_flags(0);
    cdsp = avpriv_em_float_dsp_alloc(1);
