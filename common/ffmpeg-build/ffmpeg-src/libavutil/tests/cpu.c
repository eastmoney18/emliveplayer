...
{
    int cpu_flags_raw = av_em_get_cpu_flags();
    int cpu_flags_eff;
    int cpu_count = av_em_cpu_count();
    char threads[5] = "auto";
    int i;

    for(i = 0; cpu_flag_tab[i].flag; i++) {
        unsigned tmp = 0;
        if (av_em_parse_cpu_caps(&tmp, cpu_flag_tab[i].name) < 0) {
    ...
    for (;;) {
        ...
        case 'c':
        {
            unsigned flags = av_em_get_cpu_flags();
            if (av_em_parse_cpu_caps(&flags, optarg) < 0)
                return 2;

            av_em_force_cpu_flags(flags);
            break;
        }
        case 't':
        {
            int len = av_em_strlcpy(threads, optarg, sizeof(threads));
    ...
    cpu_flags_eff = av_em_get_cpu_flags();
