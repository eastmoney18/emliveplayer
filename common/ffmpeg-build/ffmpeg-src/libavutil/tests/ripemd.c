...
int main(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        for (k = 0; k < 3; k++) {
            av_em_ripemd_init(ctx, lengths[j]);
            if (k == 0)
                av_em_ripemd_update(ctx, "abc", 3);
            else if (k == 1)
                av_em_ripemd_update(ctx, "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq", 56);
            else
                for (i = 0; i < 1000*1000; i++)
                    av_em_ripemd_update(ctx, "a", 1);
            av_em_ripemd_final(ctx, digest);
