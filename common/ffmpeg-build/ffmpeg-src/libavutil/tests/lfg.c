...
int main(void)
{
    ...
    av_em_lfg_init(&state, 0xdeadbeef);
    for (j = 0; j < 10000; j++) {
        START_TIMER
        for (i = 0; i < 624; i++) {
            //av_em_log(NULL, AV_LOG_ERROR, "%X\n", av_lfg_get(&state));
            x += av_lfg_get(&state);
        }
        STOP_TIMER("624 calls of av_lfg_get");
    }
    av_em_log(NULL, AV_LOG_ERROR, "final value:%X\n", x);
    ...
    /* BMG usage example */
    {
        ...
        av_em_lfg_init(&state, 42);

        for (i = 0; i < 1000; i += 2) {
            double bmg_out[2];
            av_em_bmg_get(&state, bmg_out);
        ...
        av_em_log(NULL, AV_LOG_INFO, "sample mean  : %f\n"
                                  "true mean    : %f\n"
                                  "sample stddev: %f\n"
                                  "true stddev  : %f\n",
