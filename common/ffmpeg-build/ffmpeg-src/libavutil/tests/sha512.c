...
int main(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        for (k = 0; k < 3; k++) {
            av_em_sha512_init(ctx, lengths[j]);
            if (k == 0)
                av_em_sha512_update(ctx, "abc", 3);
            else if (k == 1)
                av_em_sha512_update(ctx, "abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmn"
                                       "hijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu", 112);
            else
                for (i = 0; i < 1000*1000; i++)
                    av_em_sha512_update(ctx, "a", 1);
            av_em_sha512_final(ctx, digest);
