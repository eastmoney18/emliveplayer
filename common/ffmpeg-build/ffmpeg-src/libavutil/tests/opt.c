...
typedef struct TestContext {
    ...
    char *string;
    int flags;
    AVEMRational rational;
    AVEMRational video_rate;
    int w, h;
...
int main(void)
{
    ...
    av_em_log_set_callback(log_callback_help);

    printf("Testing default values\n");
    {
        TestContext test_ctx = { 0 };
        test_ctx.class = &test_class;
        av_em_opt_set_defaults(&test_ctx);
        ...
        printf("pix_fmt=%s\n", av_em_get_pix_fmt_name(test_ctx.pix_fmt));
        printf("sample_fmt=%s\n", av_em_get_sample_fmt_name(test_ctx.sample_fmt));
        ...
        av_em_opt_show2(&test_ctx, NULL, -1, 0);

        av_em_opt_free(&test_ctx);
    }

    printf("\nTesting av_em_opt_is_set_to_default()\n");
    {
        int ret;
        TestContext test_ctx = { 0 };
        ...
        av_em_log_set_level(AV_LOG_QUIET);

        while (o = av_em_opt_next(&test_ctx, o)) {
            ret = av_em_opt_is_set_to_default_by_name(&test_ctx, o->name, 0);
    ...
    printf("\nTest av_em_opt_serialize()\n");
    {
        ...
        av_em_opt_set_defaults(&test_ctx);
        if (av_em_opt_serialize(&test_ctx, 0, 0, &buf, '=', ',') >= 0) {
            printf("%s\n", buf);
            av_em_opt_free(&test_ctx);
            memset(&test_ctx, 0, sizeof(test_ctx));
            test_ctx.class = &test_class;
            av_em_set_options_string(&test_ctx, buf, "=", ",");
            av_em_free(buf);
            if (av_em_opt_serialize(&test_ctx, 0, 0, &buf, '=', ',') >= 0) {
                printf("%s\n", buf);
                av_em_free(buf);
    ...
    printf("\nTesting av_em_set_options_string()\n");
    {
        TestContext test_ctx = { 0 };
        ...
        for (i=0; i < FF_ARRAY_ELEMS(options); i++) {
            ...
            av_em_log(&test_ctx, AV_LOG_DEBUG, "Setting options string '%s'\n", options[i]);
            if (silence_log)
                av_em_log_set_callback(NULL);
            if (av_em_set_options_string(&test_ctx, options[i], "=", ":") < 0)
                printf("Error '%s'\n", options[i]);
            else
                printf("OK    '%s'\n", options[i]);
            av_em_log_set_callback(log_callback_help);
    ...
    printf("\nTesting av_em_opt_set_from_string()\n");
    {
        TestContext test_ctx = { 0 };
        static const char * const options[] = {
            "",
        ...
        for (i=0; i < FF_ARRAY_ELEMS(options); i++) {
            av_em_log(&test_ctx, AV_LOG_DEBUG, "Setting options string '%s'\n", options[i]);
            if (av_em_opt_set_from_string(&test_ctx, options[i], shorthand, "=", ":") < 0)
                printf("Error '%s'\n", options[i]);
            else
                printf("OK    '%s'\n", options[i]);
        }
