...
int main(int argc, char *argv[]) {
    ...
    uint8_t *orig = av_em_alloc(MAXSZ + 16);
    uint8_t *comp = av_em_alloc(2*MAXSZ + 16);
    uint8_t *decomp = av_em_alloc(MAXSZ + 16);
...
#else
        if (av_em_lzo1x_decode(decomp, &outlen, comp, &inlen))
#endif
            av_em_log(NULL, AV_LOG_ERROR, "decompression error\n");
STOP_TIMER("lzod")
    }
    if (memcmp(orig, decomp, s))
        av_em_log(NULL, AV_LOG_ERROR, "decompression incorrect\n");
    else
        av_em_log(NULL, AV_LOG_ERROR, "decompression OK\n");
    fclose(in);
    av_em_free(orig);
    av_em_free(comp);
    av_em_free(decomp);
