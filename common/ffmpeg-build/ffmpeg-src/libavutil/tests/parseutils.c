...
static void test_av_em_parse_video_rate(void)
{
    int i;
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(rates); i++) {
        int ret;
        AVEMRational q = { 0, 0 };
        ret = av_em_parse_video_rate(&q, rates[i]);
        printf("'%s' -> %d/%d %s\n",
               rates[i], q.num, q.den, ret ? "ERROR" : "OK");
    }
}

static void test_av_em_parse_color(void)
{
    int i;
    uint8_t rgba[4];
    ...
    for (i = 0;  i < FF_ARRAY_ELEMS(color_names); i++) {
        if (av_em_parse_color(rgba, color_names[i], -1, NULL) >= 0)
...
static void test_av_em_small_strptime(void)
{
    int i;
    struct tm tm = { 0 };
    ...
    for (i = 0;  i < FF_ARRAY_ELEMS(fmt_timespec_entries); i++) {
        ...
        p = av_em_small_strptime(e->timespec, e->fmt, &tm);
        if (p) {
...
static void test_av_em_parse_time(void)
{
    int i;
    int64_t tv;
    time_t tvi;
    ...
    for (i = 0;  i < FF_ARRAY_ELEMS(time_string); i++) {
        ...
        if (av_em_parse_time(&tv, time_string[i], 0)) {
            printf("error\n");
        } else {
            tvi = tv / 1000000;
            tm = gmtime(&tvi);
    ...
    for (i = 0;  i < FF_ARRAY_ELEMS(duration_string); i++) {
        ...
        if (av_em_parse_time(&tv, duration_string[i], 1)) {
            printf("error\n");
        } else {
            printf("%+21"PRIi64"\n", tv);
        }
...
static void test_av_em_get_known_color_name(void)
{
    int i;
    const uint8_t *rgba;
    const char *color;
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(color_table); ++i) {
        color = av_em_get_known_color_name(i, &rgba);
        if (color)
            printf("%s -> R(%d) G(%d) B(%d) A(%d)\n",
...
static void test_av_em_find_info_tag(void)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(tags); ++i) {
        if (av_em_find_info_tag(buff, sizeof(buff), tags[i], args))
            printf("%d. %s found: %s\n", i, tags[i], buff);
        else
...
int main(void)
{
    printf("Testing av_em_parse_video_rate()\n");
    test_av_em_parse_video_rate();

    printf("\nTesting av_em_parse_color()\n");
    test_av_em_parse_color();

    printf("\nTesting av_em_small_strptime()\n");
    test_av_em_small_strptime();

    printf("\nTesting av_em_parse_time()\n");
    test_av_em_parse_time();

    printf("\nTesting av_em_get_known_color_name()\n");
    test_av_em_get_known_color_name();

    printf("\nTesting av_em_find_info_tag()\n");
    test_av_em_find_info_tag();
    return 0;
}
