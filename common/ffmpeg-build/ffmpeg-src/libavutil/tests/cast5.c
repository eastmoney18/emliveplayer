...
int main(int argc, char** argv)
{
    ...
    uint8_t temp[8];
    struct AVCAST5 *cs;
    cs = av_em_cast5_alloc();
    if (!cs)
        return 1;
    for (j = 0; j < 3; j++){

        av_em_cast5_init(cs, Key[j], key_bits[j]);
        av_em_cast5_crypt(cs, temp, rpt, 1, 0);
        for (i = 0;i < 8; i++){
            if (rct[j][i] != temp[i]){
        ...
        av_em_cast5_crypt(cs, temp, rct[j], 1, 1);
        for (i =0; i < 8; i++) {
            if (rpt[i] != temp[i]) {
                av_em_log(NULL, AV_LOG_ERROR, "%d %02x %02x\n", i, rpt[i], temp[i]);
                err = 1;
    ...
    for (i = 0; i < 1000000; i++){
        av_em_cast5_init(cs, rpt2[1], 128);
        av_em_cast5_crypt(cs, rpt2[0], rpt2[0], 2, 0);
        av_em_cast5_init(cs, rpt2[0], 128);
        av_em_cast5_crypt(cs, rpt2[1], rpt2[1], 2, 0);
    }
    for (j = 0; j < 2; j++) {
        for (i = 0; i < 16; i++) {
    ...
    for (j = 0; j < 3; j++) {
        ...
        av_em_cast5_init(cs, Key[j], key_bits[j]);
        memcpy(temp, iv, 8);
        av_em_cast5_crypt2(cs, rpt2[0], rct2[0], 2, temp, 0);
        memcpy(temp, iv, 8);
        av_em_cast5_crypt2(cs, rpt2[0], rpt2[0], 2, temp, 1);
        for (i = 0; i < 16; i++) {
