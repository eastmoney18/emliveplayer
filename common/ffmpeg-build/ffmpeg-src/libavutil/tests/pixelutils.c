                           ...
                           int align, int n)
{
    ...
    av_pixelutils_sad_fn f_out = av_em_pixelutils_get_sad_fn(n, n, align, NULL);
...
{
    int i, align, ret;
    uint8_t *buf1 = av_em_alloc(W1*H1);
    uint8_t *buf2 = av_em_alloc(W2*H2);
    uint32_t state = 0;
...
#define RANDOM_INIT(buf, size) do {             \
    ...
    for (i = 1; i <= 4; i++) {
        for (align = 0; align < 3; align++) {
            ...
            buf1 = av_em_alloc(size1);
            buf2 = av_em_alloc(size2);
            if (!buf1 || !buf2) {
                fprintf(stderr, "malloc failure\n");
                ret = 1;
                goto end;
