...
static void test_blowfish(AVBlowfish *ctx, uint8_t *dst, const uint8_t *src,
                          ...
                          const char *test)
{
    av_em_blowfish_crypt(ctx, dst, src, len, iv, dir);
...
int main(void)
{
    ...
    uint8_t tmp[16], iv[8];
    int i;
    AVBlowfish *ctx = av_em_blowfish_alloc();
    if (!ctx)
        return 1;

    av_em_blowfish_init(ctx, "abcdefghijklmnopqrstuvwxyz", 26);
    ...
    for (i = 0; i < NUM_VARIABLE_KEY_TESTS; i++) {
        av_em_blowfish_init(ctx, variable_key[i], 8);

        av_em_blowfish_crypt_ecb(ctx, &tmptext_l[i], &tmptext_r[i], 0);
        if (tmptext_l[i] != ciphertext_l[i] || tmptext_r[i] != ciphertext_r[i]) {
            printf("Test encryption failed.\n");
            return 2;
        }

        av_em_blowfish_crypt_ecb(ctx, &tmptext_l[i], &tmptext_r[i], 1);
