...
static void bprint_pascal(AVEMBPrint *b, unsigned size)
{
    unsigned i, j;
    unsigned p[42];
...
int main(void)
{
    ...
    av_em_bprint_init(&b, 0, -1);
    bprint_pascal(&b, 5);
    printf("Short text in unlimited buffer: %u/%u\n", (unsigned)strlen(b.str), b.len);
    printf("%s\n", b.str);
    av_em_bprint_finalize(&b, NULL);
    ...
    av_em_bprint_init(&b, 0, 0);
    bprint_pascal(&b, 25);
    printf("Long text count only buffer: %u/%u\n", (unsigned)strlen(b.str), b.len);

    av_em_bprint_init_for_buffer(&b, buf, sizeof(buf));
    bprint_pascal(&b, 25);
    printf("Long text count only buffer: %u/%u\n", (unsigned)strlen(buf), b.len);

    av_em_bprint_init(&b, 0, -1);
    av_em_bprint_strftime(&b, "%Y-%m-%d", &testtime);
    printf("strftime full: %u/%u \"%s\"\n", (unsigned)strlen(buf), b.len, b.str);
    av_em_bprint_finalize(&b, NULL);

    av_em_bprint_init(&b, 0, 8);
    av_em_bprint_strftime(&b, "%Y-%m-%d", &testtime);
