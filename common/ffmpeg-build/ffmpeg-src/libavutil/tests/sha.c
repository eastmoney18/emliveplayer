...
int main(void)
{
    ...
    for (j = 0; j < 3; j++) {
        ...
        for (k = 0; k < 3; k++) {
            av_em_sha_init(ctx, lengths[j]);
            if (k == 0)
                av_em_sha_update(ctx, "abc", 3);
            else if (k == 1)
                av_em_sha_update(ctx, "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq", 56);
            else
                for (i = 0; i < 1000*1000; i++)
                    av_em_sha_update(ctx, "a", 1);
            av_em_sha_final(ctx, digest);
