...
static int run_test(int cbc, int decrypt)
{
    ...
    av_em_des_init(&d, cbc_key, 192, decrypt);
    av_em_des_crypt(&d, large_buffer[delay], large_buffer[0], 10000, cbc ? tmp : NULL, decrypt);
...
int main(void)
{
    ...
    av_em_des_init(&d, test_key, 64, 0);
    av_em_des_crypt(&d, tmp, plain, 1, NULL, 0);
    ...
    for (i = 0; i < 1000; i++) {
        ...
        key[2] = rand64();
        data   = rand64();
        av_em_des_init(&d, (uint8_t *) key, 192, 0);
        av_em_des_crypt(&d, (uint8_t *) &ct, (uint8_t *) &data, 1, NULL, 0);
        av_em_des_init(&d, (uint8_t *) key, 192, 1);
        av_em_des_crypt(&d, (uint8_t *) &ct, (uint8_t *) &ct, 1, NULL, 1);
        if (ct != data) {
