...
static int call_log_format_line2(const char *fmt, char *buffer, int buffer_size, ...)
{
    ...
    ret = av_em_log_format_line2(NULL, AV_LOG_INFO, fmt, args, buffer, buffer_size, &print_prefix);
    va_end(args);
    return ret;
}
...
int main(int argc, char **argv)
{
    ...
    for (use_color=0; use_color<=256; use_color = 255*use_color+1) {
        av_em_log(NULL, AV_LOG_FATAL, "use_color: %d\n", use_color);
        for (i = AV_LOG_DEBUG; i>=AV_LOG_QUIET; i-=8) {
            av_em_log(NULL, i, " %d", i);
            av_em_log(NULL, AV_LOG_INFO, "e ");
            av_em_log(NULL, i + 256*123, "C%d", i);
            av_em_log(NULL, AV_LOG_INFO, "e");
        }
        av_em_log(NULL, AV_LOG_PANIC, "\n");
    }
    {
        int result;
        char buffer[4];
