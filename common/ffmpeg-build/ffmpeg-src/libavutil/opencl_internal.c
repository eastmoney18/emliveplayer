...
int avpriv_em_opencl_set_parameter(FFOpenclParam *opencl_param, ...)
{
    int ret = 0;
    va_list arg_ptr;
    void *param;
    ...
    if (!opencl_param->kernel) {
        av_em_log(opencl_param->ctx, AV_LOG_ERROR, "OpenCL kernel must be set\n");
    ...
    do {
        ...
        if (!param_size) {
            av_em_log(opencl_param->ctx, AV_LOG_ERROR, "Parameter size must not be 0\n");
            ret = AVERROR(EINVAL);
            goto end;
        }
        status = clSetKernelArg(opencl_param->kernel, opencl_param->param_num, param_size, param);
        if (status != CL_SUCCESS) {
            av_em_log(opencl_param->ctx, AV_LOG_ERROR, "Cannot set kernel argument: %s\n", av_em_opencl_errstr(status));
