...
const int av_em_sha_size = sizeof(AVSHA);

struct AVSHA *av_em_sha_alloc(void)
{
    return av_em_mallocz(sizeof(struct AVSHA));
}
...
av_cold int av_em_sha_init(AVSHA *ctx, int bits)
{
...
void av_em_sha_update(AVSHA* ctx, const uint8_t* data, unsigned int len)
{
    unsigned int i, j;
...
void av_em_sha_final(AVSHA* ctx, uint8_t *digest)
{
    int i;
    uint64_t finalcount = av_be2ne64(ctx->count << 3);

    av_em_sha_update(ctx, "\200", 1);
    while ((ctx->count & 63) != 56)
        av_em_sha_update(ctx, "", 1);
    av_em_sha_update(ctx, (uint8_t *)&finalcount, 8); /* Should cause a transform() */
