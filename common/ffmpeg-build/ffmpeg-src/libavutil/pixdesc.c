...
void av_em_read_image_line(uint16_t *dst,
...
void av_em_write_image_line(const uint16_t *src,
    ...
    for (pix_fmt = 0; pix_fmt < AV_PIX_FMT_NB; pix_fmt++)
        if (av_pix_fmt_descriptors[pix_fmt].name &&
            (!strcmp(av_pix_fmt_descriptors[pix_fmt].name, name) ||
             av_em_match_name(name, av_pix_fmt_descriptors[pix_fmt].alias)))
            return pix_fmt;

    return AV_PIX_FMT_NONE;
}

const char *av_em_get_pix_fmt_name(enum AVPixelFormat pix_fmt)
{
...
enum AVPixelFormat av_em_get_pix_fmt(const char *name)
{
    enum AVPixelFormat pix_fmt;
...
int av_em_get_bits_per_pixel(const AVPixFmtDescriptor *pixdesc)
{
    int c, bits = 0;
...
int av_em_get_padded_bits_per_pixel(const AVPixFmtDescriptor *pixdesc)
{
    int c, bits = 0;
    int log2_pixels = pixdesc->log2_chroma_w + pixdesc->log2_chroma_h;
    int steps[4] = {0};
...
char *av_em_get_pix_fmt_string(char *buf, int buf_size,
                            enum AVPixelFormat pix_fmt)
{
    /* print header */
    if (pix_fmt < 0) {
       snprintf (buf, buf_size, "name" " nb_components" " nb_bits");
        ...
        snprintf(buf, buf_size, "%-11s %7d %10d", pixdesc->name,
                 pixdesc->nb_components, av_em_get_bits_per_pixel(pixdesc));
    }

    return buf;
}

const AVPixFmtDescriptor *av_em_pix_fmt_desc_get(enum AVPixelFormat pix_fmt)
{
...
const AVPixFmtDescriptor *av_em_pix_fmt_desc_next(const AVPixFmtDescriptor *prev)
{
    if (!prev)
...
enum AVPixelFormat av_em_pix_fmt_desc_get_id(const AVPixFmtDescriptor *desc)
{
...
int av_em_pix_fmt_get_chroma_sub_sample(enum AVPixelFormat pix_fmt,
...
int av_em_pix_fmt_count_planes(enum AVPixelFormat pix_fmt)
{
    const AVPixFmtDescriptor *desc = av_em_pix_fmt_desc_get(pix_fmt);
...
void em_check_pixfmt_descriptors(void){
    int i, j;
...
//         av_em_log(NULL, AV_LOG_DEBUG, "Checking: %s\n", d->name);
        ...
        for (j=0; j<FF_ARRAY_ELEMS(d->comp); j++) {
            ...
            if (!strncmp(d->name, "bayer_", 6))
                continue;
            av_em_read_image_line(tmp, (void*)data, linesize, d, 0, 0, j, 2, 0);
            av_assert0(tmp[0] == 0 && tmp[1] == 0);
            tmp[0] = tmp[1] = (1<<c->depth) - 1;
            av_em_write_image_line(tmp, data, linesize, d, 0, 0, j, 2);
        }
    }
}


enum AVPixelFormat av_em_pix_fmt_swap_endianness(enum AVPixelFormat pix_fmt)
{
    const AVPixFmtDescriptor *desc = av_em_pix_fmt_desc_get(pix_fmt);
    char name[16];
    int i;
    ...
    if (!desc || strlen(desc->name) < 2)
        return AV_PIX_FMT_NONE;
    av_em_strlcpy(name, desc->name, sizeof(name));
    i = strlen(name) - 2;
...
{
    const AVPixFmtDescriptor *src_desc = av_em_pix_fmt_desc_get(src_pix_fmt);
    const AVPixFmtDescriptor *dst_desc = av_em_pix_fmt_desc_get(dst_pix_fmt);
...
int av_em_get_pix_fmt_loss(enum AVPixelFormat dst_pix_fmt,
                            enum AVPixelFormat src_pix_fmt,
                            int has_alpha)
{
    int loss;
...
enum AVPixelFormat av_em_find_best_pix_fmt_of_2(enum AVPixelFormat dst_pix_fmt1, enum AVPixelFormat dst_pix_fmt2,
                                             enum AVPixelFormat src_pix_fmt, int has_alpha, int *loss_ptr)
{
    enum AVPixelFormat dst_pix_fmt;
    int loss1, loss2, loss_mask;
    const AVPixFmtDescriptor *desc1 = av_em_pix_fmt_desc_get(dst_pix_fmt1);
    const AVPixFmtDescriptor *desc2 = av_em_pix_fmt_desc_get(dst_pix_fmt2);
    int score1, score2;
    ...
    if (score1 == score2) {
        if(av_em_get_padded_bits_per_pixel(desc2) != av_em_get_padded_bits_per_pixel(desc1)) {
            dst_pix_fmt = av_em_get_padded_bits_per_pixel(desc2) < av_em_get_padded_bits_per_pixel(desc1) ? dst_pix_fmt2 : dst_pix_fmt1;
        } else {
    ...
    if (loss_ptr)
        *loss_ptr = av_em_get_pix_fmt_loss(dst_pix_fmt, src_pix_fmt, has_alpha);
    return dst_pix_fmt;
}

const char *av_em_color_range_name(enum AVColorRange range)
{
    return (unsigned) range < AVCOL_RANGE_NB ?
        color_range_names[range] : NULL;
}

const char *av_em_color_primaries_name(enum AVColorPrimaries primaries)
{
    return (unsigned) primaries < AVCOL_PRI_NB ?
        color_primaries_names[primaries] : NULL;
}

const char *av_em_color_transfer_name(enum AVColorTransferCharacteristic transfer)
{
    return (unsigned) transfer < AVCOL_TRC_NB ?
        color_transfer_names[transfer] : NULL;
}

const char *av_em_color_space_name(enum AVColorSpace space)
{
    return (unsigned) space < AVCOL_SPC_NB ?
        color_space_names[space] : NULL;
}

const char *av_em_chroma_location_name(enum AVChromaLocation location)
{
