...
static int doTest(uint8_t *ref[4], int refStride[4], int w, int h,
                  ...
                  struct Results *r)
{
    const AVPixFmtDescriptor *desc_yuva420p = av_em_pix_fmt_desc_get(AV_PIX_FMT_YUVA420P);
    const AVPixFmtDescriptor *desc_src      = av_em_pix_fmt_desc_get(srcFormat);
    const AVPixFmtDescriptor *desc_dst      = av_em_pix_fmt_desc_get(dstFormat);
    ...
    if (cur_srcFormat != srcFormat || cur_srcW != srcW || cur_srcH != srcH) {
        ...
        res = av_em_image_fill_linesizes(srcStride, srcFormat, srcW);
        if (res < 0) {
            fprintf(stderr, "av_em_image_fill_linesizes failed\n");
            goto end;
        }
        for (p = 0; p < 4; p++) {
            srcStride[p] = FFALIGN(srcStride[p], 16);
            if (srcStride[p])
                src[p] = av_em_mallocz(srcStride[p] * srcH + 16);
        ...
        srcContext = em_sws_getContext(w, h, AV_PIX_FMT_YUVA420P, srcW, srcH,
        ...
        em_sws_scale(srcContext, (const uint8_t * const*)ref, refStride, 0, h, src, srcStride);
        em_sws_freeContext(srcContext);
    ...
    res = av_em_image_fill_linesizes(dstStride, dstFormat, dstW);
    if (res < 0) {
        fprintf(stderr, "av_em_image_fill_linesizes failed\n");
        goto end;
    }
    ...
    for (i = 0; i < 4; i++) {
         ...
         * they're freed appropriately (such as using av_em_free for buffers
         * allocated with av_malloc). */
        /* An extra 16 bytes is being allocated because some scalers may write
         * out of bounds. */
        dstStride[i] = FFALIGN(dstStride[i], 16);
        if (dstStride[i])
            dst[i] = av_em_mallocz(dstStride[i] * dstH + 16);
    ...
    dstContext = em_sws_getContext(srcW, srcH, srcFormat, dstW, dstH, dstFormat,
    ...
    em_sws_scale(dstContext, (const uint8_t * const*)src, srcStride, 0, srcH, dst, dstStride);

    for (i = 0; i < 4 && dstStride[i]; i++)
        crc = av_em_crc(av_em_crc_get_table(AV_CRC_32_IEEE), crc, dst[i],
                     dstStride[i] * dstH);
    ...
    if (r && crc == r->crc) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            if (refStride[i])
                out[i] = av_em_mallocz(refStride[i] * h);
        ...
        outContext = em_sws_getContext(dstW, dstH, dstFormat, w, h,
        ...
        em_sws_scale(outContext, (const uint8_t * const*)dst, dstStride, 0, dstH, out, refStride);
                     ...
                     enum AVPixelFormat dstFormat_in)
{
    ...
    for (srcFormat = srcFormat_in != AV_PIX_FMT_NONE ? srcFormat_in : 0;
        ...
        if (!em_sws_isSupportedInput(srcFormat) ||
            !em_sws_isSupportedOutput(srcFormat))
            continue;
        ...
        for (dstFormat = dstFormat_in != AV_PIX_FMT_NONE ? dstFormat_in : 0;
            ...
            if (!em_sws_isSupportedInput(dstFormat) ||
                !em_sws_isSupportedOutput(dstFormat))
                continue;
                    ...
                    enum AVPixelFormat dstFormat_in)
{
    ...
    while (fgets(buf, sizeof(buf), fp)) {
        ...
        srcFormat = av_em_get_pix_fmt(srcStr);
        dstFormat = av_em_get_pix_fmt(dstStr);
...
int main(int argc, char **argv)
{
    ...
    uint8_t *rgb_data   = av_em_alloc(W * H * 4);
    const uint8_t * const rgb_src[4] = { rgb_data, NULL, NULL, NULL };
    int rgb_stride[4]   = { 4 * W, 0, 0, 0 };
    uint8_t *data       = av_em_alloc(4 * W * H);
    ...
    for (i = 1; i < argc; i += 2) {
        ...
        if (!strcmp(argv[i], "-ref")) {
            ...
            srcFormat = av_em_get_pix_fmt(argv[i + 1]);
            if (srcFormat == AV_PIX_FMT_NONE) {
                fprintf(stderr, "invalid pixel format %s\n", argv[i + 1]);
                return -1;
            }
        } else if (!strcmp(argv[i], "-dst")) {
            dstFormat = av_em_get_pix_fmt(argv[i + 1]);
...
bad_option:
    ...
    sws = em_sws_getContext(W / 12, H / 12, AV_PIX_FMT_RGB32, W, H,
                         AV_PIX_FMT_YUVA420P, SWS_BILINEAR, NULL, NULL, NULL);

    av_em_lfg_init(&rand, 1);

    for (y = 0; y < H; y++)
        for (x = 0; x < W * 4; x++)
            rgb_data[ x + y * 4 * W] = av_lfg_get(&rand);
    em_sws_scale(sws, rgb_src, rgb_stride, 0, H / 12, src, stride);
    em_sws_freeContext(sws);
    av_em_free(rgb_data);
