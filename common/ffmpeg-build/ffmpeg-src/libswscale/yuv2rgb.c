 ...
 */
const int32_t em_yuv2rgb_coeffs[11][4] = {
...
const int *em_sws_getCoefficients(int colorspace)
{
    ...
    return em_yuv2rgb_coeffs[colorspace];
}
...
YUV2RGBFUNC(yuv2rgb_c_16_ordered_dither, uint16_t, 0)
    const uint8_t *d16 = em_dither_2x2_8[y & 1];
    const uint8_t *e16 = em_dither_2x2_4[y & 1];
    const uint8_t *f16 = em_dither_2x2_8[(y & 1)^1];
...
YUV2RGBFUNC(yuv2rgb_c_15_ordered_dither, uint16_t, 0)
    const uint8_t *d16 = em_dither_2x2_8[y & 1];
    const uint8_t *e16 = em_dither_2x2_8[(y & 1)^1];
...
YUV2RGBFUNC(yuv2rgb_c_12_ordered_dither, uint16_t, 0)
    const uint8_t *d16 = em_dither_4x4_16[y & 3];
...
YUV2RGBFUNC(yuv2rgb_c_8_ordered_dither, uint8_t, 0)
    const uint8_t *d32 = em_dither_8x8_32[y & 7];
    const uint8_t *d64 = em_dither_8x8_73[y & 7];
...
YUV2RGBFUNC(yuv2rgb_c_4_ordered_dither, uint8_t, 0)
    const uint8_t * d64 = em_dither_8x8_73[y & 7];
    const uint8_t *d128 = em_dither_8x8_220[y & 7];
    int acc;
...
SwsFunc em_yuv2rgb_get_func_ptr(SwsContext *c)
{
    ...
    if (ARCH_PPC)
        t = em_yuv2rgb_init_ppc(c);
    if (ARCH_X86)
        t = em_yuv2rgb_init_x86(c);
    ...
    av_em_log(c, AV_LOG_WARNING,
           ...
           av_em_get_pix_fmt_name(c->srcFormat), av_em_get_pix_fmt_name(c->dstFormat));
...
av_cold int em_yuv2rgb_c_init_tables(SwsContext *c, const int inv_table[4],
...
#define ALLOC_YUV_TABLE(x)          \
    ...
    default:
        if(!isPlanar(c->dstFormat) || bpp <= 24)
            av_em_log(c, AV_LOG_ERROR, "%ibpp not supported by yuv2rgb\n", bpp);
        return -1;
    }
    return 0;
}
