...
DECLARE_ALIGNED(8, const uint8_t, em_dither_8x8_128)[9][8] = {
...
{
    const AVPixFmtDescriptor *desc = av_em_pix_fmt_desc_get(c->srcFormat);
    int i;
...
#define DEBUG_BUFFERS(...)                      \
    if (DEBUG_SWSCALE_BUFFERS)                  \
        av_em_log(c, AV_LOG_DEBUG, __VA_ARGS__)
                   ...
                   int srcSliceH, uint8_t *dst[], int dstStride[])
{
    ...
    if (   (uintptr_t)dst[0]&15 || (uintptr_t)dst[1]&15 || (uintptr_t)dst[2]&15
        ...
        if (HAVE_MMXEXT && (cpu_flags & AV_CPU_FLAG_SSE2) && !warnedAlready){
            av_em_log(c, AV_LOG_WARNING, "Warning: data is not aligned! This can lead to a speedloss\n");
            warnedAlready=1;
        }
    }
    ...
    em_init_vscale_pfn(c, yuv2plane1, yuv2planeX, yuv2nv12cX,
                   yuv2packed1, yuv2packed2, yuv2packedX, yuv2anyX, c->use_mmx_vfilter);

    em_init_slice_from_src(src_slice, (uint8_t**)src, srcStride, c->srcW,
            srcSliceY, srcSliceH, chrSrcSliceY, chrSrcSliceH, 1);

    em_init_slice_from_src(vout_slice, (uint8_t**)dst, dstStride, c->dstW,
    ...
    for (; dstY < dstH; dstY++) {
        ...
        em_rotate_slice(hout_slice, lastPosY, lastCPosY);
...
#if HAVE_MMX_INLINE
        em_updateMMXDitherTables(c, dstY, lumBufIndex, chrBufIndex,
                              lastInLumBuf, lastInChrBuf);
#endif
        if (should_dither) {
            c->chrDither8 = em_dither_8x8_128[chrDstY & 7];
            c->lumDither8 = em_dither_8x8_128[dstY    & 7];
        }
        if (dstY >= dstH - 2) {
            /* hmm looks like we can't use MMX here without overwriting
             * this array's tail */
            em_sws_init_output_funcs(c, &yuv2plane1, &yuv2planeX, &yuv2nv12cX,
    ...
    if (isPlanar(dstFormat) && isALPHA(dstFormat) && !needAlpha) {
        ...
        if (is16BPS(dstFormat) || isNBPS(dstFormat)) {
            const AVPixFmtDescriptor *desc = av_em_pix_fmt_desc_get(dstFormat);
    ...
    if (av_em_get_cpu_flags() & AV_CPU_FLAG_MMXEXT)
...
static av_cold void sws_init_swscale(SwsContext *c)
{
    ...
    em_sws_init_output_funcs(c, &c->yuv2plane1, &c->yuv2planeX,
                             &c->yuv2nv12cX, &c->yuv2packed1,
                             &c->yuv2packed2, &c->yuv2packedX, &c->yuv2anyX);

    em_sws_init_input_funcs(c);
    ...
    if (c->srcBpc == 8) {
        if (c->dstBpc <= 14) {
            ...
            if (c->flags & SWS_FAST_BILINEAR) {
                c->hyscale_fast = em_hyscale_fast_c;
                c->hcscale_fast = em_hcscale_fast_c;
            }
        } else {
...
SwsFunc em_getSwsFunc(SwsContext *c)
{
    ...
    if (ARCH_PPC)
        em_sws_init_swscale_ppc(c);
    if (ARCH_X86)
        em_sws_init_swscale_x86(c);
    if (ARCH_AARCH64)
        em_sws_init_swscale_aarch64(c);
    if (ARCH_ARM)
        em_sws_init_swscale_arm(c);
...
{
    int xp,yp;
    const AVPixFmtDescriptor *desc = av_em_pix_fmt_desc_get(c->dstFormat);
 ...
 */
int attribute_align_arg em_sws_scale(struct SwsContext *c,
                                  ...
                                  const int dstStride[])
{
    ...
    if (!srcStride || !dstStride || !dst || !srcSlice) {
        av_em_log(c, AV_LOG_ERROR, "One of the input parameters to em_sws_scale() is NULL, please check the calling code\n");
        return 0;
    }
    ...
    if ((srcSliceY & (macro_height-1)) ||
        ...
        av_em_log(c, AV_LOG_ERROR, "Slice parameters %d, %d are invalid\n", srcSliceY, srcSliceH);
    ...
    if (c->gamma_flag && c->cascaded_context[0]) {
        ...
        ret = em_sws_scale(c->cascaded_context[0],
        ...
        if (c->cascaded_context[2])
            ret = em_sws_scale(c->cascaded_context[1], (const uint8_t * const *)c->cascaded_tmp, c->cascaded_tmpStride, srcSliceY, srcSliceH, c->cascaded1_tmp, c->cascaded1_tmpStride);
        else
            ret = em_sws_scale(c->cascaded_context[1], (const uint8_t * const *)c->cascaded_tmp, c->cascaded_tmpStride, srcSliceY, srcSliceH, dst, dstStride);

        if (ret < 0)
            return ret;

        if (c->cascaded_context[2]) {
            ret = em_sws_scale(c->cascaded_context[2],
    ...
    if (c->cascaded_context[0] && srcSliceY == 0 && srcSliceH == c->cascaded_context[0]->srcH) {
        ...
        if (ret < 0)
            return ret;
        ret = em_sws_scale(c->cascaded_context[1],
    ...
    if (!check_image_pointers(srcSlice, c->srcFormat, srcStride)) {
        av_em_log(c, AV_LOG_ERROR, "bad src image pointers\n");
        return 0;
    }
    if (!check_image_pointers((const uint8_t* const*)dst, c->dstFormat, dstStride)) {
        av_em_log(c, AV_LOG_ERROR, "bad dst image pointers\n");
        return 0;
    }

    if (c->sliceDir == 0 && srcSliceY != 0 && srcSliceY + srcSliceH != c->srcH) {
        av_em_log(c, AV_LOG_ERROR, "Slices start in the middle!\n");
        return 0;
    }
    if (c->sliceDir == 0) {
...
#define RU (-(int) (0.169 * 224 / 255 * (1 << RGB2YUV_SHIFT) + 0.5))
    ...
    if (c->src0Alpha && !c->dst0Alpha && isALPHA(c->dstFormat)) {
        uint8_t *base;
        int x,y;
        rgb0_tmp = av_em_alloc(FFABS(srcStride[0]) * srcSliceH + 32);
        if (!rgb0_tmp)
