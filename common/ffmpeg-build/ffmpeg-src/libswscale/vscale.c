    ...
    if (c->yuv2packed1 && lum_fsize == 1 && chr_fsize == 1) { // unscaled RGB
        ...
        if ((c->yuv2packed1 && lum_fsize == 1 && chr_fsize == 2) ||
            ...
            if (!c->warned_unuseable_bilinear)
                av_em_log(c, AV_LOG_INFO, "Optimized 2 tap filter code cannot be used\n");
...
int em_init_vscale(SwsContext *c, SwsFilterDescriptor *desc, SwsSlice *src, SwsSlice *dst)
{
    ...
    if (isPlanarYUV(c->dstFormat) || (isGray(c->dstFormat) && !isALPHA(c->dstFormat))) {
        lumCtx = av_em_mallocz(sizeof(VScalerContext));
        if (!lumCtx)
        ...
        if (!isGray(c->dstFormat)) {
            chrCtx = av_em_mallocz(sizeof(VScalerContext));
            if (!chrCtx)
                return AVERROR(ENOMEM);
        ...
        lumCtx = av_em_mallocz_array(sizeof(VScalerContext), 2);
        if (!lumCtx)
            return AVERROR(ENOMEM);
        chrCtx = &lumCtx[1];
    ...
    em_init_vscale_pfn(c, c->yuv2plane1, c->yuv2planeX, c->yuv2nv12cX,
        c->yuv2packed1, c->yuv2packed2, c->yuv2packedX, c->yuv2anyX, c->use_mmx_vfilter);
    return 0;
}

void em_init_vscale_pfn(SwsContext *c,
