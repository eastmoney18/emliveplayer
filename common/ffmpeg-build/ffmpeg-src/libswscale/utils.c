...
static SwsVector *em_sws_getIdentityVec(void);
static void em_sws_addVec(SwsVector *a, SwsVector *b);
static void em_sws_shiftVec(SwsVector *a, int shift);
static void em_sws_printVec2(SwsVector *a, AVEMClass *log_ctx, int log_level);
#endif

static void handle_formats(SwsContext *c);

unsigned em_swscale_version(void)
{
    av_assert0(LIBSWSCALE_VERSION_MICRO >= 100);
    return LIBSWSCALE_VERSION_INT;
}

const char *em_swscale_configuration(void)
{
    return FFMPEG_CONFIGURATION;
}

const char *em_swscale_license(void)
{
...
int em_sws_isSupportedInput(enum AVPixelFormat pix_fmt)
{
    return (unsigned)pix_fmt < AV_PIX_FMT_NB ?
           format_entries[pix_fmt].is_supported_in : 0;
}

int em_sws_isSupportedOutput(enum AVPixelFormat pix_fmt)
{
    return (unsigned)pix_fmt < AV_PIX_FMT_NB ?
           format_entries[pix_fmt].is_supported_out : 0;
}

int em_sws_isSupportedEndiannessConversion(enum AVPixelFormat pix_fmt)
{
    return (unsigned)pix_fmt < AV_PIX_FMT_NB ?
                              ...
                              double param[2], int srcPos, int dstPos)
{
    ...
    av_assert0(filterSize > 0);
    filter = av_em_malloc_array(dstW, filterSize * sizeof(*filter));
    if (!filter)
        goto fail;
    ...
    *outFilterSize = filterSize;
    ...
    for (i = 0; i < dstW; i++) {
        ...
        sum = (sum + one / 2) / one;
        if (!sum) {
            av_em_log(NULL, AV_LOG_WARNING, "SwScaler: zero vector in scaling\n");
            sum = 1;
        }
...
fail:
    if(ret < 0)
        av_em_log(NULL, ret == RETCODE_USE_CASCADE ? AV_LOG_DEBUG : AV_LOG_ERROR, "sws: initFilter failed\n");
    av_em_free(filter);
    av_em_free(filter2);
    return ret;
}
...
static void fill_rgb2yuv_table(SwsContext *c, const int table[4], int dstRange)
{
    ...
    if(/*!dstRange && */!memcmp(table, em_yuv2rgb_coeffs[SWS_CS_DEFAULT], sizeof(em_yuv2rgb_coeffs[SWS_CS_DEFAULT]))) {
...
int em_sws_setColorspaceDetails(struct SwsContext *c, const int inv_table[4],
                             ...
                             int brightness, int contrast, int saturation)
{
    ...
    handle_formats(c);
    desc_dst = av_em_pix_fmt_desc_get(c->dstFormat);
    desc_src = av_em_pix_fmt_desc_get(c->srcFormat);
    ...
    if (need_reinit && (c->srcBpc == 8 || !isYUV(c->srcFormat)))
        em_sws_init_range_convert(c);

    c->dstFormatBpp = av_em_get_bits_per_pixel(desc_dst);
    c->srcFormatBpp = av_em_get_bits_per_pixel(desc_src);

    if (c->cascaded_context[c->cascaded_mainindex])
        return em_sws_setColorspaceDetails(c->cascaded_context[c->cascaded_mainindex],inv_table, srcRange,table, dstRange, brightness,  contrast, saturation);
    ...
    if ((isYUV(c->dstFormat) || isGray(c->dstFormat)) && (isYUV(c->srcFormat) || isGray(c->srcFormat))) {
        if (!c->cascaded_context[0] &&
            ...
            int dstH = c->dstH;
            int ret;
            av_em_log(c, AV_LOG_VERBOSE, "YUV color matrix differs for YUV->YUV, using intermediate RGB to convert\n");
            ...
            ret = av_em_image_alloc(c->cascaded_tmp, c->cascaded_tmpStride,
                                tmp_width, tmp_height, tmp_format, 64);
            if (ret < 0)
                return ret;

            c->cascaded_context[0] = em_sws_alloc_set_opts(srcW, srcH, c->srcFormat,
            ...
            ret = em_sws_init_context(c->cascaded_context[0], NULL , NULL);
            if (ret < 0)
                return ret;
            //we set both src and dst depending on that the RGB side will be ignored
            em_sws_setColorspaceDetails(c->cascaded_context[0], inv_table,
                                     srcRange, table, dstRange,
                                     brightness, contrast, saturation);

            c->cascaded_context[1] = em_sws_getContext(tmp_width, tmp_height, tmp_format,
                                                    dstW, dstH, c->dstFormat,
                                                    c->flags, NULL, NULL, c->param);
            if (!c->cascaded_context[1])
                return -1;
            em_sws_setColorspaceDetails(c->cascaded_context[1], inv_table,
                                     srcRange, table, dstRange,
                                     0, 1 << 16, 1 << 16);
            return 0;
        }
    ...
    if (!isYUV(c->dstFormat) && !isGray(c->dstFormat)) {
        em_yuv2rgb_c_init_tables(c, inv_table, srcRange, brightness,
                                 contrast, saturation);
        // FIXME factorize

        if (ARCH_PPC)
            em_yuv2rgb_init_tables_ppc(c, inv_table, brightness,
                                       contrast, saturation);
    }
...
int em_sws_getColorspaceDetails(struct SwsContext *c, int **inv_table,
...
SwsContext *em_sws_alloc_context(void)
{
    SwsContext *c = av_em_mallocz(sizeof(SwsContext));

    av_assert0(offsetof(SwsContext, redDither) + DITHER32_INT == offsetof(SwsContext, dither32));

    if (c) {
        c->av_class = &em_sws_context_class;
        av_em_opt_set_defaults(c);
    }
...
static uint16_t * alloc_gamma_tbl(double e)
{
    int i = 0;
    uint16_t * tbl;
    tbl = (uint16_t*)av_em_alloc(sizeof(uint16_t) * 1 << 16);
    if (!tbl)
        return NULL;
...
av_cold int em_sws_init_context(SwsContext *c, SwsFilter *srcFilter,
                             SwsFilter *dstFilter)
{
    int i;
    ...
    if(srcFormat!=c->srcFormat || dstFormat!=c->dstFormat)
        av_em_log(c, AV_LOG_WARNING, "deprecated pixel format used, make sure you did set range correctly\n");

    if (!c->contrast && !c->saturation && !c->dstFormatBpp)
        em_sws_setColorspaceDetails(c, em_yuv2rgb_coeffs[SWS_CS_DEFAULT], c->srcRange,
                                 em_yuv2rgb_coeffs[SWS_CS_DEFAULT],
                                 c->dstRange, 0, 1 << 16, 1 << 16);

    handle_formats(c);
    srcFormat = c->srcFormat;
    dstFormat = c->dstFormat;
    desc_src = av_em_pix_fmt_desc_get(srcFormat);
    desc_dst = av_em_pix_fmt_desc_get(dstFormat);

    // If the source has no alpha then disable alpha blendaway
    if (c->src0Alpha)
        c->alphablend = SWS_ALPHA_BLEND_NONE;

    if (!(unscaled && em_sws_isSupportedEndiannessConversion(srcFormat) &&
          av_em_pix_fmt_swap_endianness(srcFormat) == dstFormat)) {
    if (!em_sws_isSupportedInput(srcFormat)) {
        av_em_log(c, AV_LOG_ERROR, "%s is not supported as input pixel format\n",
               av_em_get_pix_fmt_name(srcFormat));
        return AVERROR(EINVAL);
    }
    if (!em_sws_isSupportedOutput(dstFormat)) {
        av_em_log(c, AV_LOG_ERROR, "%s is not supported as output pixel format\n",
               av_em_get_pix_fmt_name(dstFormat));
        return AVERROR(EINVAL);
    }
    }
    ...
    if (srcW < 1 || srcH < 1 || dstW < 1 || dstH < 1) {
        ...
        av_em_log(c, AV_LOG_ERROR, "%dx%d -> %dx%d is invalid scaling dimension\n",
               srcW, srcH, dstW, dstH);
        return AVERROR(EINVAL);
    }
    ...
    av_em_pix_fmt_get_chroma_sub_sample(srcFormat, &c->chrSrcHSubSample, &c->chrSrcVSubSample);
    av_em_pix_fmt_get_chroma_sub_sample(dstFormat, &c->chrDstHSubSample, &c->chrDstVSubSample);

    if (isAnyRGB(dstFormat) && !(flags&SWS_FULL_CHR_H_INT)) {
        if (dstW&1) {
            av_em_log(c, AV_LOG_DEBUG, "Forcing full internal H chroma due to odd output size\n");
        ...
        if (   c->chrSrcHSubSample == 0
            ...
            av_em_log(c, AV_LOG_DEBUG, "Forcing full internal H chroma due to input having non subsampled chroma\n");
            flags |= SWS_FULL_CHR_H_INT;
            c->flags = flags;
        }
    }
    ...
    if (!unscaled && c->gamma_flag && (srcFormat != tmpFmt || dstFormat != tmpFmt)) {
        ...
        c->cascaded_context[0] = em_sws_getContext(srcW, srcH, srcFormat,
                                                srcW, srcH, tmpFmt,
        ...
        c->cascaded_context[1] = em_sws_getContext(srcW, srcH, tmpFmt,
                                                dstW, dstH, tmpFmt,
        ...
        em_free_filters(c2);
        if (em_init_filters(c2) < 0) {
            em_sws_freeContext(c2);
            return -1;
        }

        c->cascaded_context[2] = NULL;
        if (dstFormat != tmpFmt) {
            ret = av_em_image_alloc(c->cascaded1_tmp, c->cascaded1_tmpStride,
                                dstW, dstH, tmpFmt, 64);
            if (ret < 0)
                return ret;

            c->cascaded_context[2] = em_sws_getContext(dstW, dstH, tmpFmt,
                                                dstW, dstH, dstFormat,
                                                flags, NULL, NULL, c->param);
    ...
    if (isBayer(srcFormat)) {
        if (!unscaled ||
            ...
            c->cascaded_context[1] = em_sws_getContext(srcW, srcH, tmpFormat,
                                                    dstW, dstH, dstFormat,
    ...
    if (CONFIG_SWSCALE_ALPHA && isALPHA(srcFormat) && !isALPHA(dstFormat)) {
        ...
        if (!unscaled ||
            ...
            c->cascaded_context[0] = em_sws_alloc_set_opts(srcW, srcH, srcFormat,
                                                        srcW, srcH, tmpFormat,
                                                        flags, c->param);
            if (!c->cascaded_context[0])
                return -1;
            ...
            c->cascaded_context[1] = em_sws_alloc_set_opts(srcW, srcH, tmpFormat,
                                                        dstW, dstH, dstFormat,
                                                        flags, c->param);
            if (!c->cascaded_context[1])
                return -1;
            ...
            ret = em_sws_init_context(c->cascaded_context[1], srcFilter , dstFilter);
            if (ret < 0)
                return ret;
...
// can't downscale !!!
        if (c->canMMXEXTBeUsed && (flags & SWS_FAST_BILINEAR)) {
            c->lumMmxextFilterCodeSize = em_init_hscaler_mmxext(dstW, c->lumXInc, NULL,
                                                             NULL, NULL, 8);
            c->chrMmxextFilterCodeSize = em_init_hscaler_mmxext(c->chrDstW, c->chrXInc,
                                                             NULL, NULL, NULL, 4);
            ...
            c->chrMmxextFilterCode = VirtualAlloc(NULL,
                                                  ...
                                                  PAGE_EXECUTE_READWRITE);
#else
            c->lumMmxextFilterCode = av_em_alloc(c->lumMmxextFilterCodeSize);
            c->chrMmxextFilterCode = av_em_alloc(c->chrMmxextFilterCodeSize);
#endif
...
#endif
            {
                av_em_log(c, AV_LOG_ERROR, "Failed to allocate MMX2FilterCode\n");
                return AVERROR(ENOMEM);
            }
            ...
            em_init_hscaler_mmxext(      dstW, c->lumXInc, c->lumMmxextFilterCode,
                                c->hLumFilter, (uint32_t*)c->hLumFilterPos, 8);
            em_init_hscaler_mmxext(c->chrDstW, c->chrXInc, c->chrMmxextFilterCode,
            ...
            if (   mprotect(c->lumMmxextFilterCode, c->lumMmxextFilterCodeSize, PROT_EXEC | PROT_READ) == -1
                ...
                av_em_log(c, AV_LOG_ERROR, "mprotect failed, cannot use fast bilinear scaler\n");
                goto fail;
            }
#endif
        } else
    ...
    if (flags & SWS_PRINT_INFO) {
        ...
        av_em_log(c, AV_LOG_INFO, "%s scaler, from %s to %s%s ",
               scaler,
               av_em_get_pix_fmt_name(srcFormat),
#ifdef DITHER1XBPP
        ...
        av_em_log(c, AV_LOG_INFO, "using %s\n", cpucaps);

        av_em_log(c, AV_LOG_VERBOSE, "%dx%d -> %dx%d\n", srcW, srcH, dstW, dstH);
        av_em_log(c, AV_LOG_DEBUG,
    ...
    if (unscaled && !usesHFilter && !usesVFilter &&
        ...
        c->swscale = em_sws_alphablendaway;

        if (flags & SWS_PRINT_INFO)
            av_em_log(c, AV_LOG_INFO,
                    "using alpha blendaway %s -> %s special converter\n",
                    av_em_get_pix_fmt_name(srcFormat), av_em_get_pix_fmt_name(dstFormat));
        return 0;
    }
    ...
    c->swscale = em_getSwsFunc(c);
    return em_init_filters(c);
fail: // FIXME replace things by appropriate error codes
    if (ret == RETCODE_USE_CASCADE)  {
        ...
        c->cascaded_context[1] = em_sws_getContext(tmpW, tmpH, tmpFormat,
                                                dstW, dstH, dstFormat,
                                                flags, NULL, dstFilter, c->param);
        if (!c->cascaded_context[1])
            return -1;
...
SwsContext *em_sws_alloc_set_opts(int srcW, int srcH, enum AVPixelFormat srcFormat,
                               int dstW, int dstH, enum AVPixelFormat dstFormat,
                               int flags, const double *param)
{
    SwsContext *c;

    if (!(c = em_sws_alloc_context()))
        return NULL;
...
SwsContext *em_sws_getContext(int srcW, int srcH, enum AVPixelFormat srcFormat,
                           int dstW, int dstH, enum AVPixelFormat dstFormat,
                           ...
                           SwsFilter *dstFilter, const double *param)
{
    ...
    c = em_sws_alloc_set_opts(srcW, srcH, srcFormat,
                           dstW, dstH, dstFormat,
                           flags, param);
    if (!c)
        return NULL;

    if (em_sws_init_context(c, srcFilter, dstFilter) < 0) {
        em_sws_freeContext(c);
        return NULL;
    }
...
SwsFilter *em_sws_getDefaultFilter(float lumaGBlur, float chromaGBlur,
                                float lumaSharpen, float chromaSharpen,
                                float chromaHShift, float chromaVShift,
                                int verbose)
{
    SwsFilter *filter = av_em_alloc(sizeof(SwsFilter));
    if (!filter)
        return NULL;

    if (lumaGBlur != 0.0) {
        filter->lumH = em_sws_getGaussianVec(lumaGBlur, 3.0);
        filter->lumV = em_sws_getGaussianVec(lumaGBlur, 3.0);
    } else {
        filter->lumH = em_sws_getIdentityVec();
        filter->lumV = em_sws_getIdentityVec();
    }

    if (chromaGBlur != 0.0) {
        filter->chrH = em_sws_getGaussianVec(chromaGBlur, 3.0);
        filter->chrV = em_sws_getGaussianVec(chromaGBlur, 3.0);
    } else {
        filter->chrH = em_sws_getIdentityVec();
        filter->chrV = em_sws_getIdentityVec();
    }
    ...
    if (chromaSharpen != 0.0) {
        SwsVector *id = em_sws_getIdentityVec();
        if (!id)
            goto fail;
        em_sws_scaleVec(filter->chrH, -chromaSharpen);
        em_sws_scaleVec(filter->chrV, -chromaSharpen);
        em_sws_addVec(filter->chrH, id);
        em_sws_addVec(filter->chrV, id);
        em_sws_freeVec(id);
    }
    ...
    if (lumaSharpen != 0.0) {
        ...
        if (!id)
            goto fail;
        em_sws_scaleVec(filter->lumH, -lumaSharpen);
        em_sws_scaleVec(filter->lumV, -lumaSharpen);
        em_sws_addVec(filter->lumH, id);
        em_sws_addVec(filter->lumV, id);
        em_sws_freeVec(id);
    }

    if (chromaHShift != 0.0)
        em_sws_shiftVec(filter->chrH, (int)(chromaHShift + 0.5));

    if (chromaVShift != 0.0)
        em_sws_shiftVec(filter->chrV, (int)(chromaVShift + 0.5));

    em_sws_normalizeVec(filter->chrH, 1.0);
    em_sws_normalizeVec(filter->chrV, 1.0);
    em_sws_normalizeVec(filter->lumH, 1.0);
    em_sws_normalizeVec(filter->lumV, 1.0);
    ...
    if (verbose)
        em_sws_printVec2(filter->chrH, NULL, AV_LOG_DEBUG);
    if (verbose)
        em_sws_printVec2(filter->lumH, NULL, AV_LOG_DEBUG);

    return filter;

fail:
    em_sws_freeVec(filter->lumH);
    em_sws_freeVec(filter->lumV);
    em_sws_freeVec(filter->chrH);
    em_sws_freeVec(filter->chrV);
    av_em_freep(&filter);
    return NULL;
}

SwsVector *em_sws_allocVec(int length)
{
    SwsVector *vec;
    ...
    vec = av_em_alloc(sizeof(SwsVector));
    if (!vec)
        return NULL;
    vec->length = length;
    vec->coeff  = av_em_alloc(sizeof(double) * length);
    if (!vec->coeff)
        av_em_freep(&vec);
    return vec;
}

SwsVector *em_sws_getGaussianVec(double variance, double quality)
{
...
static
#endif
SwsVector *em_sws_getConstVec(double c, int length)
{
    int i;
    SwsVector *vec = em_sws_allocVec(length);
...
static
#endif
SwsVector *em_sws_getIdentityVec(void)
{
    return em_sws_getConstVec(1.0, 1);
}
...
void em_sws_scaleVec(SwsVector *a, double scalar)
{
    int i;
...
void em_sws_normalizeVec(SwsVector *a, double height)
{
    em_sws_scaleVec(a, height / sws_dcVec(a));
}
...
static SwsVector *sws_getConvVec(SwsVector *a, SwsVector *b)
{
    ...
    int i, j;
    SwsVector *vec = em_sws_getConstVec(0.0, length);
...
static
#endif
void em_sws_shiftVec(SwsVector *a, int shift)
{
...
static
#endif
void em_sws_addVec(SwsVector *a, SwsVector *b)
{
...
#if FF_API_SWS_VECTOR
void em_sws_subVec(SwsVector *a, SwsVector *b)
{
...
void em_sws_convVec(SwsVector *a, SwsVector *b)
{
...
SwsVector *em_sws_cloneVec(SwsVector *a)
{
    SwsVector *vec = em_sws_allocVec(a->length);
...
/**
 * Print with av_em_log() a textual representation of the vector a
 * if log_level <= av_log_level.
 */
#if !FF_API_SWS_VECTOR
static
#endif
void em_sws_printVec2(SwsVector *a, AVEMClass *log_ctx, int log_level)
{
    int i;
    double max = 0;
    double min = 0;
    ...
    for (i = 0; i < a->length; i++) {
        ...
        av_em_log(log_ctx, log_level, "%1.3f ", a->coeff[i]);
        for (; x > 0; x--)
            av_em_log(log_ctx, log_level, " ");
        av_em_log(log_ctx, log_level, "|\n");
    }
}

void em_sws_freeVec(SwsVector *a)
{
    if (!a)
        return;
    av_em_freep(&a->coeff);
...
void em_sws_freeFilter(SwsFilter *filter)
{
    if (!filter)
        return;
...
void em_sws_freeContext(SwsContext *c)
{
    int i;
    if (!c)
        return;
    ...
    for (i = 0; i < 4; i++)
        av_em_freep(&c->dither_error[i]);

    av_em_freep(&c->vLumFilter);
    av_em_freep(&c->vChrFilter);
    av_em_freep(&c->hLumFilter);
    av_em_freep(&c->hChrFilter);
#if HAVE_ALTIVEC
    av_em_freep(&c->vYCoeffsBank);
    av_em_freep(&c->vCCoeffsBank);
#endif

    av_em_freep(&c->vLumFilterPos);
    av_em_freep(&c->vChrFilterPos);
    av_em_freep(&c->hLumFilterPos);
    av_em_freep(&c->hChrFilterPos);
...
#else
    av_em_free(c->lumMmxextFilterCode);
    av_em_free(c->chrMmxextFilterCode);
#endif
    c->lumMmxextFilterCode = NULL;
    c->chrMmxextFilterCode = NULL;
#endif /* HAVE_MMX_INLINE */

    av_em_freep(&c->yuvTable);
    av_em_freep(&c->formatConvBuffer);

    em_sws_freeContext(c->cascaded_context[0]);
    em_sws_freeContext(c->cascaded_context[1]);
    em_sws_freeContext(c->cascaded_context[2]);
    memset(c->cascaded_context, 0, sizeof(c->cascaded_context));
    av_em_freep(&c->cascaded_tmp[0]);
    av_em_freep(&c->cascaded1_tmp[0]);

    av_em_freep(&c->gamma);
    av_em_freep(&c->inv_gamma);
...
struct SwsContext *em_sws_getCachedContext(struct SwsContext *context, int srcW,
                                        ...
                                        const double *param)
{
    ...
    if (context &&
        ...
        av_em_opt_get_int(context, "src_h_chr_pos", 0, &src_h_chr_pos);
        av_em_opt_get_int(context, "src_v_chr_pos", 0, &src_v_chr_pos);
        av_em_opt_get_int(context, "dst_h_chr_pos", 0, &dst_h_chr_pos);
        av_em_opt_get_int(context, "dst_v_chr_pos", 0, &dst_v_chr_pos);
        em_sws_freeContext(context);
        context = NULL;
    }

    if (!context) {
        if (!(context = em_sws_alloc_context()))
            return NULL;
        context->srcW      = srcW;
        context->srcH      = srcH;
        context->srcFormat = srcFormat;
        ...
        av_em_opt_set_int(context, "src_h_chr_pos", src_h_chr_pos, 0);
        av_em_opt_set_int(context, "src_v_chr_pos", src_v_chr_pos, 0);
        av_em_opt_set_int(context, "dst_h_chr_pos", dst_h_chr_pos, 0);
        av_em_opt_set_int(context, "dst_v_chr_pos", dst_v_chr_pos, 0);

        if (em_sws_init_context(context, srcFilter, dstFilter) < 0) {
            em_sws_freeContext(context);
            return NULL;
        }
    }
