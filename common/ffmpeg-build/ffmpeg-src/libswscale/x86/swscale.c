...
const DECLARE_ALIGNED(8, uint64_t, em_dither4)[2] = {
    0x0103010301030103LL,
    0x0200020002000200LL,};

const DECLARE_ALIGNED(8, uint64_t, em_dither8)[2] = {
                           ...
                           int lastInLumBuf, int lastInChrBuf)
{
    ...
    c->blueDither= em_dither8[dstY&1];
    if (c->dstFormat == AV_PIX_FMT_RGB555 || c->dstFormat == AV_PIX_FMT_BGR555)
        c->greenDither= em_dither8[dstY&1];
    else
        c->greenDither= em_dither4[dstY&1];
    c->redDither= em_dither8[(dstY+1)&1];
    if (dstY < dstH - 2) {
...
void em_hscale ## from_bpc ## to ## to_bpc ## _ ## filter_n ## _ ## opt( \
...
av_cold void em_sws_init_swscale_x86(SwsContext *c)
{
...
#define ASSIGN_SCALE_FUNC2(hscalefn, filtersize, opt1, opt2) do { \
    if (c->srcBpc == 8) { \
        hscalefn = c->dstBpc <= 14 ? em_hscale8to15_ ## filtersize ## _ ## opt2 : \
                                     em_hscale8to19_ ## filtersize ## _ ## opt1; \
    } else if (c->srcBpc == 9) { \
        hscalefn = c->dstBpc <= 14 ? em_hscale9to15_ ## filtersize ## _ ## opt2 : \
                                     em_hscale9to19_ ## filtersize ## _ ## opt1; \
    } else if (c->srcBpc == 10) { \
        hscalefn = c->dstBpc <= 14 ? em_hscale10to15_ ## filtersize ## _ ## opt2 : \
                                     em_hscale10to19_ ## filtersize ## _ ## opt1; \
    } else if (c->srcBpc == 12) { \
        hscalefn = c->dstBpc <= 14 ? em_hscale12to15_ ## filtersize ## _ ## opt2 : \
                                     em_hscale12to19_ ## filtersize ## _ ## opt1; \
    } else if (c->srcBpc == 14 || ((c->srcFormat==AV_PIX_FMT_PAL8||isAnyRGB(c->srcFormat)) && av_em_pix_fmt_desc_get(c->srcFormat)->comp[0].depth<16)) { \
        hscalefn = c->dstBpc <= 14 ? em_hscale14to15_ ## filtersize ## _ ## opt2 : \
                                     em_hscale14to19_ ## filtersize ## _ ## opt1; \
    } else { /* c->srcBpc == 16 */ \
        av_assert0(c->srcBpc == 16);\
        hscalefn = c->dstBpc <= 14 ? em_hscale16to15_ ## filtersize ## _ ## opt2 : \
                                     em_hscale16to19_ ## filtersize ## _ ## opt1; \
    } \
} while (0)
