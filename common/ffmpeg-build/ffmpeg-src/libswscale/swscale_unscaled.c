                           ...
                           int dstStride[])
{
    ...
    if (srcFormat == AV_PIX_FMT_YA8) {
        ...
        switch (dstFormat) {
        case AV_PIX_FMT_RGB32  : conv = em_sws_convertPalette8ToPacked32; break;
        case AV_PIX_FMT_BGR32  : conv = em_sws_convertPalette8ToPacked32; break;
        case AV_PIX_FMT_BGR32_1: conv = em_sws_convertPalette8ToPacked32; break;
        case AV_PIX_FMT_RGB32_1: conv = em_sws_convertPalette8ToPacked32; break;
        case AV_PIX_FMT_RGB24  : conv = em_sws_convertPalette8ToPacked24; break;
        case AV_PIX_FMT_BGR24  : conv = em_sws_convertPalette8ToPacked24; break;
        }
    }

    if (!conv)
        av_em_log(c, AV_LOG_ERROR, "internal error %s -> %s converter\n",
                                     ...
                                     uint8_t *dst[], int dstStride[])
{
    ...
    const AVPixFmtDescriptor *src_format = av_em_pix_fmt_desc_get(c->srcFormat);
    const AVPixFmtDescriptor *dst_format = av_em_pix_fmt_desc_get(c->dstFormat);
    ...
    if ((dst_format->flags & (AV_PIX_FMT_FLAG_PLANAR | AV_PIX_FMT_FLAG_RGB)) !=
        ...
        av_em_log(c, AV_LOG_ERROR, "unsupported conversion to planar RGB %s -> %s\n",
                                     ...
                                     uint8_t *dst[], int dstStride[])
{
    ...
    if ((src_format->flags & (AV_PIX_FMT_FLAG_PLANAR | AV_PIX_FMT_FLAG_RGB)) !=
        ...
        bits_per_sample <= 8) {
        av_em_log(c, AV_LOG_ERROR, "unsupported planar RGB conversion %s -> %s\n",
               src_format->name, dst_format->name);
        return srcSliceH;
    }
    switch (c->dstFormat) {
                                 ...
                                 uint8_t *dst[], int dstStride[])
{
    ...
    if (c->srcFormat != AV_PIX_FMT_GBRP) {
        av_em_log(c, AV_LOG_ERROR, "unsupported planar RGB conversion %s -> %s\n",
               av_em_get_pix_fmt_name(c->srcFormat),
               av_em_get_pix_fmt_name(c->dstFormat));
        return srcSliceH;
    }
...
#define CONV_IS(src, dst) (srcFormat == AV_PIX_FMT_##src && dstFormat == AV_PIX_FMT_##dst)
    ...
    if ((isBGRinInt(srcFormat) && isBGRinInt(dstFormat)) ||
        ...
        case 0x0018000F: conv = em_rgb15to24; break;
        case 0x00180010: conv = em_rgb16to24; break;
        case 0x00180020: conv = em_rgb32to24; break;
        case 0x0020000F: conv = rgb15to32; break;
        case 0x00200010: conv = rgb16to32; break;
        case 0x00200018: conv = em_rgb24to32; break;
        }
    } else if ((isBGRinInt(srcFormat) && isRGBinInt(dstFormat)) ||
               (isRGBinInt(srcFormat) && isBGRinInt(dstFormat))) {
        switch (srcId | (dstId << 16)) {
        case 0x000C000C: conv = em_rgb12tobgr12; break;
        case 0x000F000F: conv = em_rgb15tobgr15; break;
        case 0x000F0010: conv = em_rgb16tobgr15; break;
        case 0x000F0018: conv = rgb24tobgr15; break;
        case 0x000F0020: conv = rgb32tobgr15; break;
        case 0x0010000F: conv = em_rgb15tobgr16; break;
        case 0x00100010: conv = em_rgb16tobgr16; break;
        ...
        case 0x0020000F: conv = em_rgb15tobgr32; break;
        case 0x00200010: conv = em_rgb16tobgr32; break;
...

{
    const enum AVPixelFormat srcFormat = c->srcFormat;
    const enum AVPixelFormat dstFormat = c->dstFormat;
    const AVPixFmtDescriptor *desc_src = av_em_pix_fmt_desc_get(c->srcFormat);
    const AVPixFmtDescriptor *desc_dst = av_em_pix_fmt_desc_get(c->dstFormat);
...
void em_get_unscaled_swscale(SwsContext *c)
{
    ...
    if ((srcFormat == AV_PIX_FMT_YUV420P || srcFormat == AV_PIX_FMT_YUV422P ||
        ...
        c->swscale = em_yuv2rgb_get_func_ptr(c);
    }
...
#define isByteRGB(f) (             \
    ...
    if (av_em_pix_fmt_desc_get(srcFormat)->comp[0].depth == 8 &&
    ...
    if (isBayer(srcFormat)) {
        if (dstFormat == AV_PIX_FMT_RGB24)
            ...
            av_em_log(c, AV_LOG_ERROR, "unsupported bayer conversion\n");
            av_assert0(0);
        }
    }
...
#define isPlanarGray(x) (isGray(x) && (x) != AV_PIX_FMT_YA8 && (x) != AV_PIX_FMT_YA16LE && (x) != AV_PIX_FMT_YA16BE)
    ...
    if (ARCH_PPC)
        em_get_unscaled_swscale_ppc(c);
     if (ARCH_ARM)
         em_get_unscaled_swscale_arm(c);
    if (ARCH_AARCH64)
        em_get_unscaled_swscale_aarch64(c);
}

/* Convert the palette to the same packed 32-bit format as the palette */
void em_sws_convertPalette8ToPacked32(const uint8_t *src, uint8_t *dst,
...
void em_sws_convertPalette8ToPacked24(const uint8_t *src, uint8_t *dst,
                                   int num_pixels, const uint8_t *palette)
{
    int i;
