...
static int qsv_decode_init(AVEMCodecContext *avctx, QSVContext *q, AVEMPacket *avpkt)
{
    ...
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported codec_id %08x\n", avctx->codec_id);
        return ret;
    }
    ...
    if (MFX_ERR_MORE_DATA==ret) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Decode header error %d\n", ret);
     ...
     */
    if (!q->async_fifo) {
        q->async_fifo = av_em_fifo_alloc((1 + 16) *
    ...
    if (!q->input_fifo) {
        q->input_fifo = av_em_fifo_alloc(1024*16);
        if (!q->input_fifo)
            return AVERROR(ENOMEM);
    }

    if (!q->pkt_fifo) {
        q->pkt_fifo = av_em_fifo_alloc( sizeof(AVEMPacket) * (1 + 16) );
        if (!q->pkt_fifo)
            return AVERROR(ENOMEM);
    }
    q->engine_ready = 1;
...
static int alloc_frame(AVEMCodecContext *avctx, QSVFrame *frame)
{
    int ret;

    ret = em_get_buffer(avctx, frame->frame, AV_GET_BUFFER_FLAG_REF);
    if (ret < 0)
        return ret;
...
static int get_surface(AVEMCodecContext *avctx, QSVContext *q, mfxFrameSurface1 **surf)
{
    QSVFrame *frame, **last;
    int ret;
...
static void qsv_fifo_relocate(AVFifoBuffer *f, int bytes_to_free)
{
    ...
    av_em_fifo_drain(f, bytes_to_free);

    data_size = av_em_fifo_size(f);
    if (data_size > 0) {
        if (f->buffer!=f->rptr) {
...
static void close_decoder(QSVContext *q)
{
    ...
    while (q->async_fifo && av_em_fifo_size(q->async_fifo)) {
        ...
        av_em_fifo_generic_read(q->async_fifo, &out_frame, sizeof(out_frame), NULL);
...
static int do_qsv_decode(AVEMCodecContext *avctx, QSVContext *q,
                  ...
                  AVEMPacket *avpkt)
{
    ...
    if (!flush) {
        if (av_em_fifo_size(q->input_fifo)) {
            /* we have got rest of previous packet into buffer */
            if (av_em_fifo_space(q->input_fifo) < avpkt->size) {
                ret = av_em_fifo_grow(q->input_fifo, avpkt->size);
                if (ret < 0)
                    return ret;
            }
            av_em_fifo_generic_write(q->input_fifo, avpkt->data, avpkt->size, NULL);
            bs.Data       = q->input_fifo->rptr;
            bs.DataLength = av_em_fifo_size(q->input_fifo);
            buffered = 1;
        } else {
            bs.Data       = avpkt->data;
            bs.DataLength = avpkt->size;
    ...
    while (1) {
        ...
        if (*sync) {
            ...
            out_frame->queued = 1;
            av_em_fifo_generic_write(q->async_fifo, &out_frame, sizeof(out_frame), NULL);
    ...
    if (!*sync && !bs.DataOffset && !flush) {
        av_em_log(avctx, AV_LOG_WARNING, "A decode call did not consume any data\n");
    ...
    if (buffered) {
        ...
        av_em_fifo_generic_write(q->input_fifo, avpkt->data+bs.DataOffset,
                              avpkt->size - bs.DataOffset, NULL);
    }

    if (MFX_ERR_MORE_DATA!=ret && ret < 0) {
        av_em_freep(&sync);
        av_em_log(avctx, AV_LOG_ERROR, "Error %d during QSV decoding.\n", ret);
        return ff_qsv_error(ret);
    }
    n_out_frames = av_em_fifo_size(q->async_fifo) / (sizeof(out_frame)+sizeof(sync));
    ...
    if (n_out_frames > q->async_depth || (flush && n_out_frames) ) {
        ...
        ret = av_em_frame_ref(frame, src_frame);
        if (ret < 0)
            return ret;
...
*/
static void qsv_packet_push_front(QSVContext *q, AVEMPacket *avpkt)
{
    int fifo_size = av_em_fifo_size(q->pkt_fifo);
    if (!fifo_size) {
    /* easy case fifo is empty */
        av_em_fifo_generic_write(q->pkt_fifo, avpkt, sizeof(*avpkt), NULL);
    } else {
    /* realloc necessary */
        AVEMPacket pkt;
        AVFifoBuffer *fifo = av_em_fifo_alloc(fifo_size+av_em_fifo_space(q->pkt_fifo));

        av_em_fifo_generic_write(fifo, avpkt, sizeof(*avpkt), NULL);

        while (av_em_fifo_size(q->pkt_fifo)) {
            av_em_fifo_generic_read(q->pkt_fifo, &pkt, sizeof(pkt), NULL);
            av_em_fifo_generic_write(fifo,       &pkt, sizeof(pkt), NULL);
        }
        av_em_fifo_free(q->pkt_fifo);
        q->pkt_fifo = fifo;
    }
}
int ff_qsv_decode(AVEMCodecContext *avctx, QSVContext *q,
                  AVFrame *frame, int *got_frame,
                  AVEMPacket *avpkt)
{
    AVEMPacket pkt_ref = { 0 };
    int ret = 0;

    if (q->pkt_fifo && av_em_fifo_size(q->pkt_fifo) >= sizeof(AVEMPacket)) {
        /* we already have got some buffered packets. so add new to tail */
        ret = av_em_packet_ref(&pkt_ref, avpkt);
        if (ret < 0)
            return ret;
        av_em_fifo_generic_write(q->pkt_fifo, &pkt_ref, sizeof(pkt_ref), NULL);
    }
    if (q->reinit_pending) {
    ...
    if (!q->reinit_pending) {
        if (q->pkt_fifo && av_em_fifo_size(q->pkt_fifo) >= sizeof(AVEMPacket)) {
            ...
            while (!*got_frame && av_em_fifo_size(q->pkt_fifo) >= sizeof(AVEMPacket)) {
                av_em_fifo_generic_read(q->pkt_fifo, &pkt_ref, sizeof(pkt_ref), NULL);
...
*/
void ff_qsv_decode_reset(AVEMCodecContext *avctx, QSVContext *q)
{
    QSVFrame *cur;
    AVEMPacket pkt;
    int ret = 0;
    ...
    if (q->reinit_pending) {
        ...
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "MFX decode get param error %d\n", ret);
        }

        ret = MFXVideoDECODE_Reset(q->session, &param);
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "MFX decode reset error %d\n", ret);
        }
