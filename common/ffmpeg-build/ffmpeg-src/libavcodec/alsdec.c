...
static av_cold int read_specific_config(ALSDecContext *ctx)
{
    ...
    if (sconf->chan_sort && avctx->channels > 1) {
        ...
        if (!(sconf->chan_pos = av_em_malloc_array(avctx->channels, sizeof(*sconf->chan_pos))))
        ...
        for (i = 0; i < avctx->channels; i++) {
            ...
            if (idx >= avctx->channels || sconf->chan_pos[idx] != -1) {
                av_em_log(avctx, AV_LOG_WARNING, "Invalid channel reordering.\n");
                ctx->cs_switch = 0;
                break;
            }
    ...
    if (sconf->crc_enabled) {
        ...
        if (avctx->err_recognition & (AV_EF_CRCCHECK|AV_EF_CAREFUL)) {
            ctx->crc_table = av_em_crc_get_table(AV_CRC_32_IEEE_LE);
            ctx->crc       = 0xFFFFFFFF;
...
static int read_var_block_data(ALSDecContext *ctx, ALSBlockData *bd)
{
    ...
    *bd->opt_order  = 1;
    ...
    for (k = 1; k < sub_blocks; k++)
        if (s[k] > 32) {
            av_em_log(avctx, AV_LOG_ERROR, "k invalid for rice code.\n");
            return AVERROR_INVALIDDATA;
        }
            ...
            *bd->opt_order       = get_bits(gb, opt_order_length);
            if (*bd->opt_order > sconf->max_order) {
                *bd->opt_order = sconf->max_order;
                av_em_log(avctx, AV_LOG_ERROR, "Predictor order too large.\n");
                return AVERROR_INVALIDDATA;
            }
        } else {
            *bd->opt_order = sconf->max_order;
    ...
    if (sconf->long_term_prediction) {
        *bd->use_ltp = get_bits1(gb);
        ...
        if (*bd->use_ltp) {
            ...
            c                 = get_bits(gb, 2);
            if (r >= 4) {
                av_em_log(avctx, AV_LOG_ERROR, "r overflow\n");
                return AVERROR_INVALIDDATA;
            }
                         ...
                         unsigned int *js_blocks)
{
    ...
    for (b = 0; b < ctx->num_blocks; b++) {
        ...
        if (bd[0].js_blocks) {
            if (bd[1].js_blocks)
                av_em_log(ctx->avctx, AV_LOG_WARNING, "Invalid channel pair.\n");
...
static int read_channel_data(ALSDecContext *ctx, ALSChannelData *cd, int c)
{
    ...
    while (entries < channels && !(current->stop_flag = get_bits1(gb))) {
        ...
        if (current->master_channel >= channels) {
            av_em_log(ctx->avctx, AV_LOG_ERROR, "Invalid master channel.\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    if (entries == channels) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Damaged channel data.\n");
        return AVERROR_INVALIDDATA;
    }
                                       ...
                                       unsigned int offset, int c)
{
    ...
    if (dep == channels) {
        av_em_log(ctx->avctx, AV_LOG_WARNING, "Invalid channel correlation.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    for (dep = 0; !ch[dep].stop_flag; dep++) {
        ...
        if (ch[dep].time_diff_flag) {
            ...
            if (ch[dep].time_diff_sign) {
                t      = -t;
                if (begin < t) {
                    av_em_log(ctx->avctx, AV_LOG_ERROR, "begin %"PTRDIFF_SPECIFIER" smaller than time diff index %d.\n", begin, t);
                    return AVERROR_INVALIDDATA;
                }
                begin -= t;
            } else {
                if (end < t) {
                    av_em_log(ctx->avctx, AV_LOG_ERROR, "end %"PTRDIFF_SPECIFIER" smaller than time diff index %d.\n", end, t);
                    return AVERROR_INVALIDDATA;
                }
                end   -= t;
            }
...
static int read_frame_data(ALSDecContext *ctx, unsigned int ra_frame)
{
    ...
    if (!sconf->mc_coding || ctx->js_switch) {
        ...
        for (c = 0; c < avctx->channels; c++)
            if (ctx->chan_data[c] < ctx->chan_data_buffer) {
                av_em_log(ctx->avctx, AV_LOG_ERROR, "Invalid channel data.\n");
                return AVERROR_INVALIDDATA;
            }
    ...
    if (get_bits_left(gb) < 0) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Overread %d\n", -get_bits_left(gb));
        return AVERROR_INVALIDDATA;
    }
    ...
    if (sconf->crc_enabled && (avctx->err_recognition & (AV_EF_CRCCHECK|AV_EF_CAREFUL))) {
        ...
        if (ctx->avctx->bits_per_raw_sample == 24) {
            ...
            for (sample = 0;
                ...
                ctx->crc = av_em_crc(ctx->crc_table, ctx->crc, (uint8_t*)(&v), 3);
            }
        } else {
            uint8_t *crc_source;
            ...
            ctx->crc = av_em_crc(ctx->crc_table, ctx->crc, crc_source,
                              ctx->cur_frame_length * avctx->channels *
...
static av_cold int decode_end(AVEMCodecContext *avctx)
{
    ...
    av_em_freep(&ctx->sconf.chan_pos);

    ff_bgmc_end(&ctx->bgmc_lut, &ctx->bgmc_lut_status);

    av_em_freep(&ctx->const_block);
    av_em_freep(&ctx->shift_lsbs);
    av_em_freep(&ctx->opt_order);
    av_em_freep(&ctx->store_prev_samples);
    av_em_freep(&ctx->use_ltp);
    av_em_freep(&ctx->ltp_lag);
    av_em_freep(&ctx->ltp_gain);
    av_em_freep(&ctx->ltp_gain_buffer);
    av_em_freep(&ctx->quant_cof);
    av_em_freep(&ctx->lpc_cof);
    av_em_freep(&ctx->quant_cof_buffer);
    av_em_freep(&ctx->lpc_cof_buffer);
    av_em_freep(&ctx->lpc_cof_reversed_buffer);
    av_em_freep(&ctx->prev_raw_samples);
    av_em_freep(&ctx->raw_samples);
    av_em_freep(&ctx->raw_buffer);
    av_em_freep(&ctx->chan_data);
    av_em_freep(&ctx->chan_data_buffer);
    av_em_freep(&ctx->reverted_channels);
    av_em_freep(&ctx->crc_buffer);
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (!avctx->extradata) {
        av_em_log(avctx, AV_LOG_ERROR, "Missing required ALS extradata.\n");
        return AVERROR_INVALIDDATA;
    }

    if ((ret = read_specific_config(ctx)) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Reading ALSSpecificConfig failed.\n");
        goto fail;
    }
    ...
    if (sconf->floating) {
        ...
        if (avctx->bits_per_raw_sample > 32) {
            av_em_log(avctx, AV_LOG_ERROR, "Bits per raw sample %d larger than 32.\n",
    ...
    ctx->quant_cof        = av_em_malloc_array(num_buffers, sizeof(*ctx->quant_cof));
    ctx->lpc_cof          = av_em_malloc_array(num_buffers, sizeof(*ctx->lpc_cof));
    ctx->quant_cof_buffer = av_em_malloc_array(num_buffers * sconf->max_order,
                                            sizeof(*ctx->quant_cof_buffer));
    ctx->lpc_cof_buffer   = av_em_malloc_array(num_buffers * sconf->max_order,
                                            sizeof(*ctx->lpc_cof_buffer));
    ctx->lpc_cof_reversed_buffer = av_em_malloc_array(sconf->max_order,
                                                   sizeof(*ctx->lpc_cof_buffer));

    if (!ctx->quant_cof              || !ctx->lpc_cof        ||
        !ctx->quant_cof_buffer       || !ctx->lpc_cof_buffer ||
        !ctx->lpc_cof_reversed_buffer) {
        av_em_log(avctx, AV_LOG_ERROR, "Allocating buffer memory failed.\n");
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    ctx->const_block     = av_em_malloc_array(num_buffers, sizeof(*ctx->const_block));
    ctx->shift_lsbs      = av_em_malloc_array(num_buffers, sizeof(*ctx->shift_lsbs));
    ctx->opt_order       = av_em_malloc_array(num_buffers, sizeof(*ctx->opt_order));
    ctx->store_prev_samples = av_em_malloc_array(num_buffers, sizeof(*ctx->store_prev_samples));
    ctx->use_ltp         = av_em_mallocz_array(num_buffers, sizeof(*ctx->use_ltp));
    ctx->ltp_lag         = av_em_malloc_array(num_buffers, sizeof(*ctx->ltp_lag));
    ctx->ltp_gain        = av_em_malloc_array(num_buffers, sizeof(*ctx->ltp_gain));
    ctx->ltp_gain_buffer = av_em_malloc_array(num_buffers * 5, sizeof(*ctx->ltp_gain_buffer));
    ...
    if (sconf->mc_coding) {
        ctx->chan_data_buffer  = av_em_mallocz_array(num_buffers * num_buffers,
                                                 sizeof(*ctx->chan_data_buffer));
        ctx->chan_data         = av_em_mallocz_array(num_buffers,
                                                 sizeof(*ctx->chan_data));
        ctx->reverted_channels = av_em_malloc_array(num_buffers,
    ...
    ctx->prev_raw_samples = av_em_malloc_array(sconf->max_order, sizeof(*ctx->prev_raw_samples));
    ctx->raw_buffer       = av_em_mallocz_array(avctx->channels * channel_size, sizeof(*ctx->raw_buffer));
    ctx->raw_samples      = av_em_malloc_array(avctx->channels, sizeof(*ctx->raw_samples));
    ...
    if (HAVE_BIGENDIAN != sconf->msb_first && sconf->crc_enabled &&
        ...
        ctx->crc_buffer = av_em_malloc_array(ctx->cur_frame_length *
                                          avctx->channels *
                                          av_em_get_bytes_per_sample(avctx->sample_fmt),
