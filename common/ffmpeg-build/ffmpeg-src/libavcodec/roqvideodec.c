...
static void roqvideo_decode_frame(RoqContext *ri)
{
    ...
    if (chunk_size > bytestream2_get_bytes_left(&ri->gb)) {
        av_em_log(ri->avctx, AV_LOG_ERROR, "Chunk does not fit in input buffer\n");
        ...
        for (yp = ypos; yp < ypos + 16; yp += 8)
            for (xp = xpos; xp < xpos + 16; xp += 8) {
                if (bytestream2_tell(&ri->gb) >= chunk_start + chunk_size) {
                    av_em_log(ri->avctx, AV_LOG_VERBOSE, "Chunk is too short\n");
                    return;
                }
                if (vqflg_pos < 0) {
...
static av_cold int roq_decode_init(AVEMCodecContext *avctx)
{
    ...
    s->current_frame = av_em_frame_alloc();
    if (!s->current_frame || !s->last_frame) {
        av_em_frame_free(&s->current_frame);
...
static int roq_decode_frame(AVEMCodecContext *avctx,
                            ...
                            AVEMPacket *avpkt)
{
    ...
    if ((ret = em_reget_buffer(avctx, s->current_frame)) < 0)
        return ret;

    if (copy) {
        ret = av_em_frame_copy(s->current_frame, s->last_frame);
        if (ret < 0)
            return ret;
    }
    ...
    if ((ret = av_em_frame_ref(data, s->current_frame)) < 0)
        return ret;
    *got_frame      = 1;
...
static av_cold int roq_decode_end(AVEMCodecContext *avctx)
{
    RoqContext *s = avctx->priv_data;
