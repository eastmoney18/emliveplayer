    ...
    while (size > 0) {
        ...
        switch (section_type) {
            case HAP_ST_COMPRESSOR_TABLE:
                ret = em_hap_set_chunk_count(ctx, section_size, is_first_table);
                if (ret != 0)
                    return ret;
            ...
            case HAP_ST_SIZE_TABLE:
                ret = em_hap_set_chunk_count(ctx, section_size / 4, is_first_table);
                if (ret != 0)
                    return ret;
...
static int hap_parse_frame_header(AVEMCodecContext *avctx)
{
    ...
    switch (section_type & 0xF0) {
        ...
        case HAP_COMP_SNAPPY:
            ret = em_hap_set_chunk_count(ctx, 1, 1);
            if (ret == 0) {
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "%s compressor\n", compressorstr);

    return ret;
}

static int decompress_chunks_thread(AVEMCodecContext *avctx, void *arg,
                                    int chunk_nb, int thread_nb)
{
    ...
    if (chunk->compressor == HAP_COMP_SNAPPY) {
        ...
        if (ret < 0) {
             av_em_log(avctx, AV_LOG_ERROR, "Snappy uncompress error\n");
             return ret;
        }
    } else if (chunk->compressor == HAP_COMP_NONE) {
...
static int hap_decode(AVEMCodecContext *avctx, void *data,
                      int *got_frame, AVEMPacket *avpkt)
{
    ...
    tframe.f = data;
    ret = em_thread_get_buffer(avctx, &tframe, 0);
    if (ret < 0)
        return ret;
    ...
    if (hap_can_use_tex_in_place(ctx)) {
        ...
        ret = av_em_reallocp(&ctx->tex_buf, ctx->tex_size);
        if (ret < 0)
            return ret;
...
static av_cold int hap_init(AVEMCodecContext *avctx)
{
    HapContext *ctx = avctx->priv_data;
    ...
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid video size %dx%d.\n",
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "%s texture\n", texture_name);
...
static av_cold int hap_close(AVEMCodecContext *avctx)
{
    HapContext *ctx = avctx->priv_data;

    em_hap_free_context(ctx);

    return 0;
}

AVEMCodec em_hap_decoder = {
    .name           = "hap",
