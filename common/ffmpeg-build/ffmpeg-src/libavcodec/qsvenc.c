...
static void dump_video_param(AVEMCodecContext *avctx, QSVEncContext *q,
                             mfxExtBuffer **coding_opts)
{
    ...
    av_em_log(avctx, AV_LOG_VERBOSE, "profile: %s; level: %"PRIu16"\n",
           print_profile(info->CodecProfile), info->CodecLevel);

    av_em_log(avctx, AV_LOG_VERBOSE, "GopPicSize: %"PRIu16"; GopRefDist: %"PRIu16"; GopOptFlag: ",
           info->GopPicSize, info->GopRefDist);
    if (info->GopOptFlag & MFX_GOP_CLOSED)
        av_em_log(avctx, AV_LOG_VERBOSE, "closed ");
    if (info->GopOptFlag & MFX_GOP_STRICT)
        av_em_log(avctx, AV_LOG_VERBOSE, "strict ");
    av_em_log(avctx, AV_LOG_VERBOSE, "; IdrInterval: %"PRIu16"\n", info->IdrInterval);

    av_em_log(avctx, AV_LOG_VERBOSE, "TargetUsage: %"PRIu16"; RateControlMethod: %s\n",
    ...
    if (info->RateControlMethod == MFX_RATECONTROL_CBR ||
        ...
        av_em_log(avctx, AV_LOG_VERBOSE, "QPI: %"PRIu16"; QPP: %"PRIu16"; QPB: %"PRIu16"\n",
        ...
        av_em_log(avctx, AV_LOG_VERBOSE, "ICQQuality: %"PRIu16"\n", info->ICQQuality);
    } else if (info->RateControlMethod == MFX_RATECONTROL_LA_ICQ) {
        av_em_log(avctx, AV_LOG_VERBOSE, "ICQQuality: %"PRIu16"; LookAheadDepth: %"PRIu16"\n",
               info->ICQQuality, co2->LookAheadDepth);
    }
#endif
#if QSV_HAVE_QVBR
    else if (info->RateControlMethod == MFX_RATECONTROL_QVBR) {
        av_em_log(avctx, AV_LOG_VERBOSE, "QVBRQuality: %"PRIu16"\n",
               co3->QVBRQuality);
    }
#endif

    av_em_log(avctx, AV_LOG_VERBOSE, "NumSlice: %"PRIu16"; NumRefFrame: %"PRIu16"\n",
           info->NumSlice, info->NumRefFrame);
    av_em_log(avctx, AV_LOG_VERBOSE, "RateDistortionOpt: %s\n",
    ...
    av_em_log(avctx, AV_LOG_VERBOSE, "MaxFrameSize: %"PRIu16"; ", co2->MaxFrameSize);
#if QSV_HAVE_MAX_SLICE_SIZE
    av_em_log(avctx, AV_LOG_VERBOSE, "MaxSliceSize: %"PRIu16"; ", co2->MaxSliceSize);
#endif
    av_em_log(avctx, AV_LOG_VERBOSE, "\n");
...
#if QSV_HAVE_TRELLIS
    av_em_log(avctx, AV_LOG_VERBOSE, "Trellis: ");
    if (co2->Trellis & MFX_TRELLIS_OFF) {
        av_em_log(avctx, AV_LOG_VERBOSE, "off");
    } else if (!co2->Trellis) {
        av_em_log(avctx, AV_LOG_VERBOSE, "auto");
    } else {
        if (co2->Trellis & MFX_TRELLIS_I) av_em_log(avctx, AV_LOG_VERBOSE, "I");
        if (co2->Trellis & MFX_TRELLIS_P) av_em_log(avctx, AV_LOG_VERBOSE, "P");
        if (co2->Trellis & MFX_TRELLIS_B) av_em_log(avctx, AV_LOG_VERBOSE, "B");
    }
    av_em_log(avctx, AV_LOG_VERBOSE, "\n");
#endif
    ...
    case MFX_LOOKAHEAD_DS_OFF: av_em_log(avctx, AV_LOG_VERBOSE, "off");     break;
    case MFX_LOOKAHEAD_DS_2x:  av_em_log(avctx, AV_LOG_VERBOSE, "2x");      break;
    case MFX_LOOKAHEAD_DS_4x:  av_em_log(avctx, AV_LOG_VERBOSE, "4x");      break;
    default:                   av_em_log(avctx, AV_LOG_VERBOSE, "unknown"); break;
    }
    av_em_log(avctx, AV_LOG_VERBOSE, "\n");

    av_em_log(avctx, AV_LOG_VERBOSE, "AdaptiveI: %s; AdaptiveB: %s; BRefType: ",
           print_threestate(co2->AdaptiveI), print_threestate(co2->AdaptiveB));
    switch (co2->BRefType) {
    case MFX_B_REF_OFF:     av_em_log(avctx, AV_LOG_VERBOSE, "off");       break;
    case MFX_B_REF_PYRAMID: av_em_log(avctx, AV_LOG_VERBOSE, "pyramid");   break;
    default:                av_em_log(avctx, AV_LOG_VERBOSE, "auto");      break;
    }
    av_em_log(avctx, AV_LOG_VERBOSE, "\n");
#endif
    ...
    if (avctx->codec_id == AV_CODEC_ID_H264) {
        av_em_log(avctx, AV_LOG_VERBOSE, "Entropy coding: %s; MaxDecFrameBuffering: %"PRIu16"\n",
...
static int select_rc_mode(AVEMCodecContext *avctx, QSVEncContext *q)
{
    const char *rc_desc;
    mfxU16      rc_mode;
    ...
    av_em_log(avctx, AV_LOG_VERBOSE, "Using the %s ratecontrol method\n", rc_desc);
...
static int init_video_param(AVEMCodecContext *avctx, QSVEncContext *q)
{
    float quant;
    int ret;
...
static int qsv_retrieve_enc_params(AVEMCodecContext *avctx, QSVEncContext *q)
{
    ...
    if (!extradata.SPSBufSize || (need_pps && !extradata.PPSBufSize)) {
        av_em_log(avctx, AV_LOG_ERROR, "No extradata returned from libmfx.\n");
        return AVERROR_UNKNOWN;
    }

    avctx->extradata = av_em_alloc(extradata.SPSBufSize + need_pps * extradata.PPSBufSize +
...
static int qsv_init_opaque_alloc(AVEMCodecContext *avctx, QSVEncContext *q)
{
    ...
    q->opaque_alloc_buf = av_em_buffer_allocz(sizeof(*surfaces) * nb_surfaces);
    if (!q->opaque_alloc_buf)
        return AVERROR(ENOMEM);

    q->opaque_surfaces = av_em_malloc_array(nb_surfaces, sizeof(*q->opaque_surfaces));
...
int ff_qsv_enc_init(AVEMCodecContext *avctx, QSVEncContext *q)
{
    int opaque_alloc = 0;
    int ret;
    ...
    q->async_fifo = av_em_fifo_alloc((1 + q->async_depth) *
                                  (sizeof(AVEMPacket) + sizeof(mfxSyncPoint*) + sizeof(mfxBitstream*)));
    if (!q->async_fifo)
        return AVERROR(ENOMEM);
    ...
    if (MFX_WRN_PARTIAL_ACCELERATION==ret) {
        av_em_log(avctx, AV_LOG_WARNING, "Encoder will work with partial HW acceleration\n");
    } else if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error %d querying encoder params\n", ret);
        return ff_qsv_error(ret);
    }

    ret = MFXVideoENCODE_QueryIOSurf(q->session, &q->param, &q->req);
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error querying the encoding parameters\n");
        return ff_qsv_error(ret);
    }
    ...
    if (avctx->hwaccel_context) {
        ...
        q->extparam = av_em_mallocz_array(qsv->nb_ext_buffers + q->nb_extparam_internal,
                                       sizeof(*q->extparam));
        if (!q->extparam)
            return AVERROR(ENOMEM);
    ...
    if (MFX_WRN_PARTIAL_ACCELERATION==ret) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Error initializing the encoder\n");
        return ff_qsv_error(ret);
    }

    ret = qsv_retrieve_enc_params(avctx, q);
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error retrieving encoding parameters.\n");
        return ret;
    }
...
static int get_free_frame(QSVEncContext *q, QSVFrame **f)
{
    ...
    frame = av_em_mallocz(sizeof(*frame));
    if (!frame)
        return AVERROR(ENOMEM);
    frame->frame = av_em_frame_alloc();
    if (!frame->frame) {
        av_em_freep(&frame);
        return AVERROR(ENOMEM);
    }
    frame->enc_ctrl.Payload = av_em_mallocz(sizeof(mfxPayload*) * QSV_MAX_ENC_PAYLOAD);
                        ...
                        QSVFrame **new_frame)
{
    ...
    if (frame->format == AV_PIX_FMT_QSV) {
        ret = av_em_frame_ref(qf->frame, frame);
        if (ret < 0)
            return ret;
        ...
        if (     frame->height & (q->height_align - 1) ||
            ...
            ret = em_get_buffer(q->avctx, qf->frame, AV_GET_BUFFER_FLAG_REF);
            if (ret < 0)
                return ret;

            qf->frame->height = frame->height;
            qf->frame->width  = frame->width;
            ret = av_em_frame_copy(qf->frame, frame);
            if (ret < 0) {
                av_em_frame_unref(qf->frame);
                return ret;
            }
        } else {
            ret = av_em_frame_ref(qf->frame, frame);
    ...
    qf->surface->Data.TimeStamp = av_em_rescale_q(frame->pts, q->avctx->time_base, (AVEMRational){1, 90000});
...
static void print_interlace_msg(AVEMCodecContext *avctx, QSVEncContext *q)
{
...
static int encode_frame(AVEMCodecContext *avctx, QSVEncContext *q,
                        const AVFrame *frame)
{
    AVEMPacket new_pkt = { 0 };
    mfxBitstream *bs;
    ...
    if (frame) {
        ...
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Error submitting the frame for encoding.\n");
            return ret;
        }
    }
    if (qsv_frame) {
    ...
    ret = av_em_new_packet(&new_pkt, q->packet_size);
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error allocating the output packet\n");
        return ret;
    }

    bs = av_em_mallocz(sizeof(*bs));
    if (!bs) {
        av_em_packet_unref(&new_pkt);
        return AVERROR(ENOMEM);
    }
    ...
    sync = av_em_mallocz(sizeof(*sync));
    if (!sync) {
        av_em_freep(&bs);
        av_em_packet_unref(&new_pkt);
        return AVERROR(ENOMEM);
    ...
    if (ret < 0) {
        ...
        if (ret == MFX_ERR_MORE_DATA)
            return 0;
        av_em_log(avctx, AV_LOG_ERROR, "EncodeFrameAsync returned %d\n", ret);
        return ff_qsv_error(ret);
    }
    ...
    if (sync) {
        av_em_fifo_generic_write(q->async_fifo, &new_pkt, sizeof(new_pkt), NULL);
        av_em_fifo_generic_write(q->async_fifo, &sync,    sizeof(sync),    NULL);
        av_em_fifo_generic_write(q->async_fifo, &bs,      sizeof(bs),    NULL);
    } else {
        av_em_freep(&sync);
        av_em_packet_unref(&new_pkt);
        av_em_freep(&bs);
...
int ff_qsv_encode(AVEMCodecContext *avctx, QSVEncContext *q,
                  AVEMPacket *pkt, const AVFrame *frame, int *got_packet)
{
    int ret;
    ...
    if (!av_em_fifo_space(q->async_fifo) ||
        (!frame && av_em_fifo_size(q->async_fifo))) {
        AVEMPacket new_pkt;
        mfxBitstream *bs;
        mfxSyncPoint *sync;

        av_em_fifo_generic_read(q->async_fifo, &new_pkt, sizeof(new_pkt), NULL);
        av_em_fifo_generic_read(q->async_fifo, &sync,    sizeof(sync),    NULL);
        av_em_fifo_generic_read(q->async_fifo, &bs,      sizeof(bs),      NULL);

        do {
            ret = MFXVideoCORE_SyncOperation(q->session, *sync, 1000);
        } while (ret == MFX_WRN_IN_EXECUTION);

        new_pkt.dts  = av_em_rescale_q(bs->DecodeTimeStamp, (AVEMRational){1, 90000}, avctx->time_base);
        new_pkt.pts  = av_em_rescale_q(bs->TimeStamp,       (AVEMRational){1, 90000}, avctx->time_base);
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (pkt->data) {
            if (pkt->size < new_pkt.size) {
                av_em_log(avctx, AV_LOG_ERROR, "Submitted buffer not large enough: %d < %d\n",
            ...
            ret = av_em_packet_copy_props(pkt, &new_pkt);
            av_em_packet_unref(&new_pkt);
            if (ret < 0)
                return ret;
        } else
...
int ff_qsv_enc_close(AVEMCodecContext *avctx, QSVEncContext *q)
{
    QSVFrame *cur;
    ...
    while (cur) {
        ...
        av_em_frame_free(&cur->frame);
        av_em_free(cur->enc_ctrl.Payload);
        av_em_freep(&cur);
        cur = q->work_frames;
    }

    while (q->async_fifo && av_em_fifo_size(q->async_fifo)) {
        AVEMPacket pkt;
        mfxSyncPoint *sync;
        mfxBitstream *bs;

        av_em_fifo_generic_read(q->async_fifo, &pkt,  sizeof(pkt),  NULL);
        av_em_fifo_generic_read(q->async_fifo, &sync, sizeof(sync), NULL);
        av_em_fifo_generic_read(q->async_fifo, &bs,   sizeof(bs),   NULL);
    ...
    av_em_fifo_free(q->async_fifo);
    q->async_fifo = NULL;

    av_em_freep(&q->opaque_surfaces);
    av_em_buffer_unref(&q->opaque_alloc_buf);

    av_em_freep(&q->extparam);
