 ...
 * the extradata chunk in AVEMCodecContext. The chunk should be 128 bytes
...
static av_cold int flic_decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->extradata_size != 0 &&
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Unexpected extradata size %d\n", avctx->extradata_size);
        return AVERROR_INVALIDDATA;
    }
    ...
    switch (depth) {
        ...
        case 24 : avctx->pix_fmt = AV_PIX_FMT_BGR24; /* Supposedly BGR, but no files to test with */
                  avpriv_em_request_sample(avctx, "24Bpp FLC/FLX");
                  return AVERROR_PATCHWELCOME;
        default :
                  av_em_log(avctx, AV_LOG_ERROR, "Unknown FLC/FLX depth of %d Bpp is unsupported.\n",depth);
                  return AVERROR_INVALIDDATA;
    }
...
static int flic_decode_frame_8BPP(AVEMCodecContext *avctx,
                                  ...
                                  const uint8_t *buf, int buf_size)
{
    ...
    while ((frame_size >= 6) && (num_chunks > 0) &&
        ...
        case FLI_DELTA:
            ...
            while (compressed_lines > 0) {
                ...
                if ((line_packets & 0xC000) == 0xC000) {
                    ...
                    av_em_log(avctx, AV_LOG_ERROR, "Undefined opcode (%x) in DELTA_FLI\n", line_packets);
        ...
        case FLI_BRUN:
            ...
            for (lines = 0; lines < s->avctx->height; lines++) {
                ...
                while (pixel_countdown > 0) {
                    ...
                    byte_run = sign_extend(bytestream2_get_byte(&g2), 8);
                    if (!byte_run) {
                        av_em_log(avctx, AV_LOG_ERROR, "Invalid byte run value.\n");
                        return AVERROR_INVALIDDATA;
                    }
                    ...
                    if (byte_run > 0) {
                        ...
                        for (j = 0; j < byte_run; j++) {
                            ...
                            if (pixel_countdown < 0)
                                av_em_log(avctx, AV_LOG_ERROR, "pixel_countdown < 0 (%d) at line %d\n",
        ...
        case FLI_COPY:
            ...
            if (chunk_size - 6 != FFALIGN(s->avctx->width, 4) * s->avctx->height) {
                av_em_log(avctx, AV_LOG_ERROR, "In chunk FLI_COPY : source data (%d bytes) " \
        ...
        default:
            av_em_log(avctx, AV_LOG_ERROR, "Unrecognized chunk type: %d\n", chunk_type);
            break;
        }
    ...
    if (bytestream2_get_bytes_left(&g2) > 2)
        av_em_log(avctx, AV_LOG_ERROR, "Processed FLI chunk where chunk size = %d " \
...
static int flic_decode_frame_15_16BPP(AVEMCodecContext *avctx,
                                      void *data, int *got_frame,
                                      const uint8_t *buf, int buf_size)
{
    ...
    while ((frame_size > 0) && (num_chunks > 0) &&
        ...
        case FLI_LC:
            av_em_log(avctx, AV_LOG_ERROR, "Unexpected FLI_LC chunk in non-palettized FLC\n");
            bytestream2_skip(&g2, chunk_size - 6);
            break;
                ...
                while (pixel_countdown > 0) {
                    ...
                    if (byte_run > 0) {
                        ...
                        for (j = 0; j < byte_run; j++) {
                            ...
                            pixel_countdown--;
                            if (pixel_countdown < 0)
                                av_em_log(avctx, AV_LOG_ERROR, "pixel_countdown < 0 (%d) (linea%d)\n",
                                       pixel_countdown, lines);
                        }
                    } else {  /* copy bytes if byte_run < 0 */
                        byte_run = -byte_run;
                    ...
                    if (byte_run > 0) {
                        ...
                        for (j = 0; j < byte_run; j++) {
                            *((signed short*)(&pixels[pixel_ptr])) = pixel;
                            ...
                            pixel_countdown--;
                            if (pixel_countdown < 0)
                                av_em_log(avctx, AV_LOG_ERROR, "pixel_countdown < 0 (%d)\n",
                                       pixel_countdown);
                        }
...
static int flic_decode_frame_24BPP(AVEMCodecContext *avctx,
                                   void *data, int *got_frame,
                                   const uint8_t *buf, int buf_size)
{
  av_em_log(avctx, AV_LOG_ERROR, "24Bpp FLC Unsupported due to lack of test files.\n");
  return AVERROR_PATCHWELCOME;
}

static int flic_decode_frame(AVEMCodecContext *avctx,
                             void *data, int *got_frame,
                             AVEMPacket *avpkt)
{
    ...
    av_em_log(avctx, AV_LOG_ERROR, "Unknown FLC format, my science cannot explain how this happened.\n");
    return AVERROR_BUG;
}


static av_cold int flic_decode_end(AVEMCodecContext *avctx)
{
    FlicDecodeContext *s = avctx->priv_data;
