...
static int vaapi_hevc_start_frame(AVEMCodecContext          *avctx,
...
static int vaapi_hevc_end_frame(AVEMCodecContext *avctx)
{
    FFVAContext * const vactx = ff_vaapi_get_context(avctx);
    HEVCContext * const h = avctx->priv_data;
    vaapi_hevc_frame_data *frame_data = h->ref->hwaccel_picture_private;
...
static int vaapi_hevc_decode_slice(AVEMCodecContext *avctx,
...
AVHWAccel em_hevc_vaapi_hwaccel = {
    .name                 = "hevc_vaapi",
