                        ...
                        int channel, int *last)
{
    ...
    *last = 0;
    ...
    if (!c->error_limit) {
        if (add >= 0x2000000U) {
            av_em_log(ctx->avctx, AV_LOG_ERROR, "k %d is too large\n", add);
            goto error;
        }
...
error:
    ...
    if (ret <= 0) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Too few bits (%d) left\n", ret);
    }
    *last = 1;
    return 0;
}
...
{
    if (crc != s->CRC) {
        av_em_log(s->avctx, AV_LOG_ERROR, "CRC error\n");
        return AVERROR_INVALIDDATA;
    }
    if (s->got_extra_bits && crc_extra_bits != s->crc_extra_bits) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Extra bits CRC error\n");
        return AVERROR_INVALIDDATA;
    }
                                   ...
                                   void *dst_l, void *dst_r, const int type)
{
    ...
    do {
        ...
        if (type == AV_SAMPLE_FMT_S16P) {
            if (FFABS(L) + FFABS(R) > (1<<19)) {
                av_em_log(s->avctx, AV_LOG_ERROR, "sample %d %d too large\n", L, R);
                return AVERROR_INVALIDDATA;
            }
        }
    ...
    if (last && count < s->samples) {
        int size = av_em_get_bytes_per_sample(type);
...
static av_cold int wv_alloc_frame_context(WavpackContext *c)
{
    ...
    c->fdec[c->fdec_num] = av_em_mallocz(sizeof(**c->fdec));
...
static av_cold int wavpack_decode_init(AVEMCodecContext *avctx)
{
    WavpackContext *s = avctx->priv_data;
...
static av_cold int wavpack_decode_end(AVEMCodecContext *avctx)
{
    WavpackContext *s = avctx->priv_data;
    int i;

    for (i = 0; i < s->fdec_num; i++)
        av_em_freep(&s->fdec[i]);
    s->fdec_num = 0;

    return 0;
}

static int wavpack_decode_block(AVEMCodecContext *avctx, int block_no,
                                AVFrame *frame, const uint8_t *buf, int buf_size)
{
    ...
    if (block_no >= wc->fdec_num && wv_alloc_frame_context(wc) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error creating frame decode context\n");
        return AVERROR_INVALIDDATA;
    }

    s = wc->fdec[block_no];
    if (!s) {
        av_em_log(avctx, AV_LOG_ERROR, "Context for block %d is not present\n",
               block_no);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->samples != wc->samples) {
        av_em_log(avctx, AV_LOG_ERROR, "Mismatching number of samples in "
               "a sequence: %d and %d\n", wc->samples, s->samples);
        return AVERROR_INVALIDDATA;
    }
    s->frame_flags = bytestream2_get_le32(&gb);
    bpp            = av_em_get_bytes_per_sample(avctx->sample_fmt);
    ...
    while (bytestream2_get_bytes_left(&gb)) {
        ...
        case WP_ID_DECTERMS:
            if (size > MAX_TERMS) {
                av_em_log(avctx, AV_LOG_ERROR, "Too many decorrelation terms\n");
                s->terms = 0;
        ...
        case WP_ID_DECWEIGHTS:
            if (!got_terms) {
                av_em_log(avctx, AV_LOG_ERROR, "No decorrelation terms met\n");
                continue;
            }
            weights = size >> s->stereo_in;
            if (weights > MAX_TERMS || weights > s->terms) {
                av_em_log(avctx, AV_LOG_ERROR, "Too many decorrelation weights\n");
                bytestream2_skip(&gb, ssize);
                continue;
            }
            for (i = 0; i < weights; i++) {
        ...
        case WP_ID_EXTRABITS:
            if (size <= 4) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid EXTRABITS, size = %i\n",
                       size);
                bytestream2_skip(&gb, size);
                continue;
            }
        ...
        case WP_ID_CHANINFO:
            ...
            case 5:
                ...
                if (avctx->channels != size)
                    av_em_log(avctx, AV_LOG_WARNING, "%i channels signalled"
                           " instead of %i.\n", size, avctx->channels);
                chan  |= (bytestream2_get_byte(&gb) & 0xF) << 8;
                chmask = bytestream2_get_le16(&gb);
                break;
            default:
                av_em_log(avctx, AV_LOG_ERROR, "Invalid channel info size %d\n",
                       size);
                chan   = avctx->channels;
        ...
        case WP_ID_SAMPLE_RATE:
            if (size != 3) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid custom sample rate.\n");
                return AVERROR_INVALIDDATA;
            }
    ...
    if (!got_terms) {
        av_em_log(avctx, AV_LOG_ERROR, "No block with decorrelation terms\n");
        return AVERROR_INVALIDDATA;
    }
    if (!got_weights) {
        av_em_log(avctx, AV_LOG_ERROR, "No block with decorrelation weights\n");
        return AVERROR_INVALIDDATA;
    }
    if (!got_samples) {
        av_em_log(avctx, AV_LOG_ERROR, "No block with decorrelation samples\n");
        return AVERROR_INVALIDDATA;
    }
    if (!got_entropy) {
        av_em_log(avctx, AV_LOG_ERROR, "No block with entropy info\n");
        return AVERROR_INVALIDDATA;
    }
    if (s->hybrid && !got_hybrid) {
        av_em_log(avctx, AV_LOG_ERROR, "Hybrid config not found\n");
        return AVERROR_INVALIDDATA;
    }
    if (!got_bs) {
        av_em_log(avctx, AV_LOG_ERROR, "Packed samples not found\n");
        return AVERROR_INVALIDDATA;
    }
    if (!got_float && avctx->sample_fmt == AV_SAMPLE_FMT_FLTP) {
        av_em_log(avctx, AV_LOG_ERROR, "Float information not found\n");
        return AVERROR_INVALIDDATA;
    }
    if (s->got_extra_bits && avctx->sample_fmt != AV_SAMPLE_FMT_FLTP) {
        ...
        if (size < wanted) {
            av_em_log(avctx, AV_LOG_ERROR, "Too small EXTRABITS\n");
            s->got_extra_bits = 0;
        }
    }
    ...
    if (!wc->ch_offset) {
        ...
        if (sr == 0xf) {
            if (!sample_rate) {
                av_em_log(avctx, AV_LOG_ERROR, "Custom sample rate missing.\n");
                return AVERROR_INVALIDDATA;
            }
    ...
    if (wc->ch_offset + s->stereo >= avctx->channels) {
        av_em_log(avctx, AV_LOG_WARNING, "Too many channels coded in a packet.\n");
...
static void wavpack_decode_flush(AVEMCodecContext *avctx)
{
    WavpackContext *s = avctx->priv_data;
    int i;
...
static int wavpack_decode_frame(AVEMCodecContext *avctx, void *data,
                                int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (s->samples <= 0 || s->samples > WV_MAX_SAMPLES) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid number of samples: %d\n",
               s->samples);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->ch_offset != avctx->channels) {
        av_em_log(avctx, AV_LOG_ERROR, "Not enough channels coded in a packet.\n");
        return AVERROR_INVALIDDATA;
    }
