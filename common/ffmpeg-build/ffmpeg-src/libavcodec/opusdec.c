...
{
    int celt_size = av_em_audio_fifo_size(s->celt_delay);
    int ret, i;
    ret = em_swr_convert(s->swr,
                      (uint8_t**)s->out, nb_samples,
                      NULL, 0);
    if (ret < 0)
        return ret;
    else if (ret != nb_samples) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Wrong number of flushed samples: %d\n",
               ret);
        return AVERROR_BUG;
    }
    ...
    if (celt_size) {
        if (celt_size != nb_samples) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Wrong number of CELT delay samples.\n");
            return AVERROR_BUG;
        }
        av_em_audio_fifo_read(s->celt_delay, (void**)s->celt_output, nb_samples);
...
static int opus_init_resample(OpusStreamContext *s)
{
    ...
    av_em_opt_set_int(s->swr, "in_sample_rate", s->silk_samplerate, 0);
    ret = em_swr_init(s->swr);
    if (ret < 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Error opening the resampler.\n");
        return ret;
    }
...
static int opus_decode_redundancy(OpusStreamContext *s, const uint8_t *data, int size)
{
    ...
    return 0;
fail:
    av_em_log(s->avctx, AV_LOG_ERROR, "Error decoding the redundancy frame.\n");
    return ret;
}
...
static int opus_decode_frame(OpusStreamContext *s, const uint8_t *data, int size)
{
    ...
    if (s->packet.mode == OPUS_MODE_SILK || s->packet.mode == OPUS_MODE_HYBRID) {
        if (!em_swr_is_initialized(s->swr)) {
        ...
        if (samples < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Error decoding a SILK frame.\n");
            return samples;
        }
        samples = em_swr_convert(s->swr,
                              (uint8_t**)s->out, s->packet.frame_duration,
                              (const uint8_t**)s->silk_output, samples);
        if (samples < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Error resampling SILK data.\n");
            return samples;
        }
    ...
    if (redundancy) {
        ...
        if (size < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid redundancy frame size.\n");
    ...
    if (s->packet.mode == OPUS_MODE_CELT || s->packet.mode == OPUS_MODE_HYBRID) {
        ...
        int delay_samples = av_em_audio_fifo_size(s->celt_delay);

        if (delay_samples) {
            if (s->packet.mode == OPUS_MODE_HYBRID) {
                av_em_audio_fifo_read(s->celt_delay, (void**)s->celt_output, delay_samples);
                ...
                av_em_audio_fifo_drain(s->celt_delay, delay_samples);
        ...
        if (s->packet.mode == OPUS_MODE_HYBRID) {
            ...
            ret = av_em_audio_fifo_write(s->celt_delay, delaybuf, celt_delay);
            if (ret < 0)
                return ret;
        }
    } else
                                 ...
                                 int nb_samples)
{
    ...
    if (em_swr_is_initialized(s->swr)) {
        if (buf) {
            ...
            av_em_opt_get_int(s->swr, "in_sample_rate", 0, &cur_samplerate);
    ...
    if (!s->out[0] ||
        ...
        av_em_fast_malloc(&s->out_dummy, &s->out_dummy_allocated_size, s->out_size);
        if (!s->out_dummy)
    ...
    if (flush_needed) {
        ...
        if (ret < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Error flushing the resampler.\n");
            return ret;
        }
        em_swr_close(s->swr);
    ...
    for (i = 0; i < s->packet.frame_count; i++) {
        ...
        if (samples < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Error decoding an Opus frame.\n");
            if (s->avctx->err_recognition & AV_EF_EXPLODE)
                return samples;
...
static int opus_decode_packet(AVEMCodecContext *avctx, void *data,
                              int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    for (i = 0; i < c->nb_streams; i++) {
        ...
        delayed_samples = FFMAX(delayed_samples,
                                s->delayed_samples + av_em_audio_fifo_size(c->sync_buffers[i]));
    }
    ...
    if (buf) {
        ...
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Error parsing the packet header.\n");
            return ret;
        }
    ...
    for (i = 0; i < c->nb_streams; i++) {
        float          **out = c->out + 2 * i;
        int sync_size = av_em_audio_fifo_size(c->sync_buffers[i]);
        ...
        ret = av_em_audio_fifo_read(c->sync_buffers[i], (void**)out, sync_size);
        if (ret < 0)
            return ret;
    ...
    for (i = 0; i < c->nb_streams; i++) {
        ...
        if (buffer_samples) {
            ...
            ret = av_em_audio_fifo_write(c->sync_buffers[i], (void**)buf, buffer_samples);
            if (ret < 0)
                return ret;
        }
    }
...
static av_cold void opus_decode_flush(AVEMCodecContext *ctx)
{
    ...
    for (i = 0; i < c->nb_streams; i++) {
        ...
        if (s->celt_delay)
            av_em_audio_fifo_drain(s->celt_delay, av_em_audio_fifo_size(s->celt_delay));
        em_swr_close(s->swr);

        av_em_audio_fifo_drain(c->sync_buffers[i], av_em_audio_fifo_size(c->sync_buffers[i]));
...
static av_cold int opus_decode_close(AVEMCodecContext *avctx)
{
    OpusContext *c = avctx->priv_data;
    int i;
    ...
    for (i = 0; i < c->nb_streams; i++) {
        ...
        av_em_freep(&s->out_dummy);
        s->out_dummy_allocated_size = 0;

        av_em_audio_fifo_free(s->celt_delay);
        em_swr_free(&s->swr);
    }
    ...
    if (c->sync_buffers) {
        for (i = 0; i < c->nb_streams; i++)
            av_em_audio_fifo_free(c->sync_buffers[i]);
    }
    av_em_freep(&c->sync_buffers);
    av_em_freep(&c->decoded_samples);
    av_em_freep(&c->out);
    av_em_freep(&c->out_size);

    c->nb_streams = 0;

    av_em_freep(&c->channel_maps);
    av_em_freep(&c->fdsp);

    return 0;
}

static av_cold int opus_decode_init(AVEMCodecContext *avctx)
{
    OpusContext *c = avctx->priv_data;
    int ret, i, j;
    ...
    c->fdsp = avpriv_em_float_dsp_alloc(0);
    if (!c->fdsp)
        return AVERROR(ENOMEM);
    ...
    c->streams = av_em_mallocz_array(c->nb_streams, sizeof(*c->streams));
    c->out             = av_em_mallocz_array(c->nb_streams, 2 * sizeof(*c->out));
    c->out_size        = av_em_mallocz_array(c->nb_streams, sizeof(*c->out_size));
    c->sync_buffers    = av_em_mallocz_array(c->nb_streams, sizeof(*c->sync_buffers));
    c->decoded_samples = av_em_mallocz_array(c->nb_streams, sizeof(*c->decoded_samples));
    ...
    for (i = 0; i < c->nb_streams; i++) {
        ...
        av_em_opt_set_int(s->swr, "in_sample_fmt",      avctx->sample_fmt,  0);
        av_em_opt_set_int(s->swr, "out_sample_fmt",     avctx->sample_fmt,  0);
        av_em_opt_set_int(s->swr, "in_channel_layout",  layout,             0);
        av_em_opt_set_int(s->swr, "out_channel_layout", layout,             0);
        av_em_opt_set_int(s->swr, "out_sample_rate",    avctx->sample_rate, 0);
        av_em_opt_set_int(s->swr, "filter_size",        16,                 0);
        ...
        s->celt_delay = av_em_audio_fifo_alloc(avctx->sample_fmt,
        ...
        c->sync_buffers[i] = av_em_audio_fifo_alloc(avctx->sample_fmt,
