                         ...
                         float *out_data)
{
    ...
    for (i = 0; i < subframe_count; i++) {
        ...
        avg_energy = (0.01 + avpriv_em_scalarproduct_float_c(fixed_vector,
                                                          fixed_vector,
                                                          SUBFR_SIZE)) /
                     SUBFR_SIZE;
    ...
    if (ctx->mode == MODE_5k0) {
        for (i = 0; i < subframe_count; i++) {
            float energy = avpriv_em_scalarproduct_float_c(ctx->postfilter_syn5k0 + LP_FILTER_ORDER + i * SUBFR_SIZE,
...
static av_cold int sipr_decoder_init(AVEMCodecContext * avctx)
{
...
static int sipr_decode_frame(AVEMCodecContext *avctx, void *data,
