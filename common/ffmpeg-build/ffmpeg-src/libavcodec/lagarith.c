...
static int lag_read_prob_header(lag_rac *rac, GetBitContext *gb)
{
    ...
    for (i = 1; i < 257; i++) {
        if (lag_decode_prob(gb, &rac->prob[i]) < 0) {
            av_em_log(rac->avctx, AV_LOG_ERROR, "Invalid probability encountered.\n");
            return -1;
        }
        if ((uint64_t)cumul_prob + rac->prob[i] > UINT_MAX) {
            av_em_log(rac->avctx, AV_LOG_ERROR, "Integer overflow encountered in cumulative probability calculation.\n");
            return -1;
        }
        cumul_prob += rac->prob[i];
        if (!rac->prob[i]) {
            if (lag_decode_prob(gb, &prob)) {
                av_em_log(rac->avctx, AV_LOG_ERROR, "Invalid probability run encountered.\n");
                return -1;
            }
            if (prob > 256 - i)
                prob = 256 - i;
    ...
    if (!cumul_prob) {
        av_em_log(rac->avctx, AV_LOG_ERROR, "All probabilities are 0!\n");
        return -1;
    }
    ...
    if (cumul_prob & (cumul_prob - 1)) {
        ...
        if (scaled_cumul_prob <= 0) {
            av_em_log(rac->avctx, AV_LOG_ERROR, "Scaled probabilities invalid\n");
        ...
        if (scaled_cumul_prob > cumulative_target) {
            av_em_log(rac->avctx, AV_LOG_ERROR,
                                    ...
                                    int width, int esc_count)
{
    ...
    avpriv_em_request_sample(l->avctx, "zero_run_line");
...
output_zeros:
    if (l->zeros_rem) {
        ...
        if (end - dst < count) {
            av_em_log(l->avctx, AV_LOG_ERROR, "Too many zeros remaining.\n");
            return AVERROR_INVALIDDATA;
        }
                                  ...
                                  const uint8_t *src, int src_size)
{
    ...
    if (esc_count < 4) {
        ...
        if (read > length)
            av_em_log(l->avctx, AV_LOG_WARNING,
        ...
        av_em_log(l->avctx, AV_LOG_ERROR,
 ...
 */
static int lag_decode_frame(AVEMCodecContext *avctx,
                            void *data, int *got_frame, AVEMPacket *avpkt)
{
    ...
    case FRAME_U_RGB24:
        ...
        av_em_fast_malloc(&l->rgb_planes, &l->rgb_planes_allocated,
...
static av_cold int lag_decode_init(AVEMCodecContext *avctx)
{
    LagarithContext *l = avctx->priv_data;
    l->avctx = avctx;

    em_huffyuvdsp_init(&l->hdsp);

    return 0;
}

static av_cold int lag_decode_end(AVEMCodecContext *avctx)
{
    LagarithContext *l = avctx->priv_data;

    av_em_freep(&l->rgb_planes);

    return 0;
}

AVEMCodec ff_lagarith_decoder = {
    .name           = "lagarith",
