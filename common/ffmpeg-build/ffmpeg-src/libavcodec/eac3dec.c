        ...
        for (bin = s->start_freq[ch]; bin < s->end_freq[ch]; bin++) {
            if (s->bap[ch][bin] > 7 && s->bap[ch][bin] < 17) {
                if (gc++ == 2) {
                    ...
                    if (group_code > 26) {
                        av_em_log(s->avctx, AV_LOG_WARNING, "GAQ gain group code out-of-range\n");
                        group_code = 26;
                    }
...
static int ff_eac3_parse_header(AC3DecodeContext *s)
{
    ...
    if (s->frame_type == EAC3_FRAME_TYPE_DEPENDENT) {
        if (!s->eac3_frame_dependent_found) {
            ...
            avpriv_em_request_sample(s->avctx, "Dependent substream decoding");
        }
        return AAC_AC3_PARSE_ERROR_FRAME_TYPE;
    } else if (s->frame_type == EAC3_FRAME_TYPE_RESERVED) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Reserved frame type\n");
        return AAC_AC3_PARSE_ERROR_FRAME_TYPE;
    }
    ...
    if (s->substreamid) {
        ...
        if (!s->eac3_subsbtreamid_found) {
            ...
            avpriv_em_request_sample(s->avctx, "Additional substreams");
        }
        return AAC_AC3_PARSE_ERROR_FRAME_TYPE;
    }
    ...
    if (s->bit_alloc_params.sr_code == EAC3_SR_CODE_REDUCED) {
        ...
        avpriv_em_request_sample(s->avctx, "Reduced sampling rate");
    ...
    if (s->num_blocks > 1 && get_bits1(gbc)) {
        ...
        avpriv_em_request_sample(s->avctx, "Block start info");
    }
