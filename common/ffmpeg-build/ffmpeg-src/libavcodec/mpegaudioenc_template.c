...
static av_cold int MPA_encode_init(AVEMCodecContext *avctx)
{
    ...
    if (channels <= 0 || channels > 2){
        av_em_log(avctx, AV_LOG_ERROR, "encoding %d channel(s) is not allowed in mp2\n", channels);
    ...
    if (i == 3){
        av_em_log(avctx, AV_LOG_ERROR, "Sampling rate %d is not allowed in mp2\n", freq);
        return AVERROR(EINVAL);
    }
    s->freq_index = i;
    ...
    if (i == 15){
        av_em_log(avctx, AV_LOG_ERROR, "bitrate %d is not allowed in mp2\n", bitrate);
        return AVERROR(EINVAL);
    }
    s->bitrate_index = i;
...
static int MPA_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
...
static const AVEMCodecDefault mp2_defaults[] = {
    { "b", "0" },
    { NULL },
};
