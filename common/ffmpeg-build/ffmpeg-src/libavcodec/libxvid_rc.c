...
av_cold int em_xvid_rate_control_init(MpegEncContext *s)
{
    char *tmp_name;
    int fd, i;
    ...
    fd = avpriv_em_tempfile("xvidrc.", &tmp_name, 0, s->avctx);
    if (fd < 0) {
        av_em_log(NULL, AV_LOG_ERROR, "Can't create temporary pass2 file.\n");
        return fd;
    }
    ...
    for (i = 0; i < s->rc_context.num_entries; i++) {
        ...
        if (write(fd, tmp, strlen(tmp)) < 0) {
            ...
            av_em_log(NULL, AV_LOG_ERROR, "Error %s writing 2pass logfile\n", av_err2str(ret));
            av_em_free(tmp_name);
            close(fd);
            return ret;
        }
    ...
    if (xvid_plugin_2pass2(NULL, XVID_PLG_CREATE, &xvid_plg_create,
        ...
        av_em_log(NULL, AV_LOG_ERROR, "xvid_plugin_2pass2 failed\n");
        return -1;
    }
    return 0;
}

float em_xvid_rate_estimate_qscale(MpegEncContext *s, int dry_run)
{
...
av_cold void em_xvid_rate_control_uninit(MpegEncContext *s)
{
