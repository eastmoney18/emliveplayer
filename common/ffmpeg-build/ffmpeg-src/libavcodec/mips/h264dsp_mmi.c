...
void em_h264_add_pixels4_8_mmi(uint8_t *dst, int16_t *src, int stride)
{
    double ftmp[9];
    uint64_t low32;
    ...
    for(i=0; i<16; i++){
        ...
        if(nnz){
            if(nnz==1 && ((int16_t*)block)[i*16])
                em_h264_idct_dc_add_8_mmi(dst + block_offset[i], block + i*16,
                        stride);
            else
                em_h264_idct_add_8_mmi(dst + block_offset[i], block + i*16,
                        stride);
        }
    }
}
        ...
        int16_t *block, int stride, const uint8_t nnzc[15*8])
{
    ...
    for(i=0; i<16; i++){
        if(nnzc[ scan8[i] ])
            em_h264_idct_add_8_mmi(dst + block_offset[i], block + i*16, stride);
        ...
        int16_t *block, int stride, const uint8_t nnzc[15*8])
{
    ...
    for(i=0; i<16; i+=4){
        ...
        if(nnz){
            if(nnz==1 && ((int16_t*)block)[i*16])
                em_h264_idct8_dc_add_8_mmi(dst + block_offset[i],
                        block + i*16, stride);
            else
                em_h264_idct8_add_8_mmi(dst + block_offset[i], block + i*16,
                        stride);
        }
    }
}
        ...
        int16_t *block, int stride, const uint8_t nnzc[15*8])
{
    ...
    for(j=1; j<3; j++){
        for(i=j*16; i<j*16+4; i++){
            if(nnzc[ scan8[i] ])
                em_h264_idct_add_8_mmi(dest[j-1] + block_offset[i],
                        block + i*16, stride);
            else if(((int16_t*)block)[i*16])
                em_h264_idct_dc_add_8_mmi(dest[j-1] + block_offset[i],
                        block + i*16, stride);
        }
    }
}
        ...
        int16_t *block, int stride, const uint8_t nnzc[15*8])
{
    ...
    for(j=1; j<3; j++){
        for(i=j*16+4; i<j*16+8; i++){
            if(nnzc[ scan8[i+4] ])
                em_h264_idct_add_8_mmi(dest[j-1] + block_offset[i+4],
                        block + i*16, stride);
            else if(((int16_t*)block)[i*16])
                em_h264_idct_dc_add_8_mmi(dest[j-1] + block_offset[i+4],
                        block + i*16, stride);
        }
    }
}
...
void em_h264_biweight_pixels16_8_mmi(uint8_t *dst, uint8_t *src, int stride,
...
void em_h264_biweight_pixels8_8_mmi(uint8_t *dst, uint8_t *src, int stride,
        int height, int log2_denom, int weightd, int weights, int offset)
{
    int y;
    double ftmp[9];
...
void em_h264_biweight_pixels4_8_mmi(uint8_t *dst, uint8_t *src, int stride,
        int height, int log2_denom, int weightd, int weights, int offset)
{
    int y;
    double ftmp[7];
