                                       ...
                                       const int chroma_format_idc)
{
    if (8 == bit_depth) {
        if (chroma_format_idc == 1) {
            h->pred8x8[VERT_PRED8x8] = em_h264_intra_pred_vert_8x8_msa;
            h->pred8x8[HOR_PRED8x8] = em_h264_intra_pred_horiz_8x8_msa;
        }

        if (codec_id != AV_CODEC_ID_VP7 && codec_id != AV_CODEC_ID_VP8) {
            if (chroma_format_idc == 1) {
                h->pred8x8[PLANE_PRED8x8] = em_h264_intra_predict_plane_8x8_msa;
            }
        }
        if (codec_id != AV_CODEC_ID_RV40 && codec_id != AV_CODEC_ID_VP7
            && codec_id != AV_CODEC_ID_VP8) {
            if (chroma_format_idc == 1) {
                h->pred8x8[DC_PRED8x8] = em_h264_intra_predict_dc_4blk_8x8_msa;
                h->pred8x8[LEFT_DC_PRED8x8] =
                    em_h264_intra_predict_hor_dc_8x8_msa;
                h->pred8x8[TOP_DC_PRED8x8] =
                    em_h264_intra_predict_vert_dc_8x8_msa;
                h->pred8x8[ALZHEIMER_DC_L0T_PRED8x8] =
                    em_h264_intra_predict_mad_cow_dc_l0t_8x8_msa;
                h->pred8x8[ALZHEIMER_DC_0LT_PRED8x8] =
                    em_h264_intra_predict_mad_cow_dc_0lt_8x8_msa;
                h->pred8x8[ALZHEIMER_DC_L00_PRED8x8] =
                    em_h264_intra_predict_mad_cow_dc_l00_8x8_msa;
                h->pred8x8[ALZHEIMER_DC_0L0_PRED8x8] =
                    em_h264_intra_predict_mad_cow_dc_0l0_8x8_msa;
            }
        } else {
            if (codec_id == AV_CODEC_ID_VP7 || codec_id == AV_CODEC_ID_VP8) {
        ...
        if (chroma_format_idc == 1) {
            h->pred8x8[DC_128_PRED8x8] = em_h264_intra_pred_dc_128_8x8_msa;
        }

        h->pred16x16[DC_PRED8x8] = em_h264_intra_pred_dc_16x16_msa;
        h->pred16x16[VERT_PRED8x8] = em_h264_intra_pred_vert_16x16_msa;
        h->pred16x16[HOR_PRED8x8] = em_h264_intra_pred_horiz_16x16_msa;
        ...
        default:
            h->pred16x16[PLANE_PRED8x8] =
                em_h264_intra_predict_plane_16x16_msa;
            break;
        }

        h->pred16x16[LEFT_DC_PRED8x8] = em_h264_intra_pred_dc_left_16x16_msa;
        h->pred16x16[TOP_DC_PRED8x8] = em_h264_intra_pred_dc_top_16x16_msa;
        h->pred16x16[DC_128_PRED8x8] = em_h264_intra_pred_dc_128_16x16_msa;
    }
}
#endif  // #if HAVE_MSA
...
av_cold void em_h264_pred_init_mips(H264PredContext *h, int codec_id,
                                    int bit_depth,
                                    const int chroma_format_idc)
{
#if HAVE_MSA
