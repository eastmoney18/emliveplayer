...
void em_xvid_idct_mmi(int16_t *block)
{
    __asm__ volatile (
        //# Process each row
...
void em_xvid_idct_put_mmi(uint8_t *dest, int32_t line_size, int16_t *block)
{
    em_xvid_idct_mmi(block);
    ff_put_pixels_clamped_mmi(block, dest, line_size);
}

void em_xvid_idct_add_mmi(uint8_t *dest, int32_t line_size, int16_t *block)
{
    em_xvid_idct_mmi(block);
