...
static const AVEMClass audioresample_context_class = {
                                        ...
                                        int linear, double cutoff)
{
    ...
    if (!(supported_resampling[input_channels-1] & (1<<(output_channels-1)))) {
        int i;
        av_em_log(NULL, AV_LOG_ERROR, "Unsupported audio resampling. Allowed "
               "output channels for %d input channel%s", input_channels,
               input_channels > 1 ? "s:" : ":");
        for (i = 0; i < MAX_CHANNELS; i++)
            if (supported_resampling[input_channels-1] & (1<<i))
                av_em_log(NULL, AV_LOG_ERROR, " %d", i + 1);
        av_em_log(NULL, AV_LOG_ERROR, "\n");
        return NULL;
    }

    s = av_em_mallocz(sizeof(ReSampleContext));
    if (!s) {
        av_em_log(NULL, AV_LOG_ERROR, "Can't allocate memory for resample context.\n");
        return NULL;
    }
    ...
    s->sample_size[0] = av_em_get_bytes_per_sample(s->sample_fmt[0]);
    s->sample_size[1] = av_em_get_bytes_per_sample(s->sample_fmt[1]);
    ...
    if (s->sample_fmt[0] != AV_SAMPLE_FMT_S16) {
        if (!(s->convert_ctx[0] = av_audio_convert_alloc(AV_SAMPLE_FMT_S16, 1,
            ...
            av_em_log(s, AV_LOG_ERROR,
                   ...
                   av_em_get_sample_fmt_name(s->sample_fmt[0]));
            av_em_free(s);
            return NULL;
        }
    }
    ...
    if (s->sample_fmt[1] != AV_SAMPLE_FMT_S16) {
        if (!(s->convert_ctx[1] = av_audio_convert_alloc(s->sample_fmt[1], 1,
            ...
            av_em_log(s, AV_LOG_ERROR,
                   ...
                   av_em_get_sample_fmt_name(s->sample_fmt[1]));
    ...
    s->resample_context = av_em_resample_init(output_rate, input_rate,
                                           filter_length, log2_phase_count,
                                           linear, cutoff);

    *(const AVEMClass**)s->resample_context = &audioresample_context_class;
...
int em_audio_resample(ReSampleContext *s, short *output, short *input, int nb_samples)
{
    ...
    if (s->sample_fmt[0] != AV_SAMPLE_FMT_S16) {
        ...
        if (!s->buffer_size[0] || s->buffer_size[0] < input_size) {
            av_em_free(s->buffer[0]);
            s->buffer_size[0] = input_size;
            s->buffer[0] = av_em_alloc(s->buffer_size[0]);
            if (!s->buffer[0]) {
                av_em_log(s->resample_context, AV_LOG_ERROR, "Could not allocate buffer\n");
                return 0;
            }
        }
        ...
        if (av_audio_convert(s->convert_ctx[0], obuf, ostride,
            ...
            av_em_log(s->resample_context, AV_LOG_ERROR,
    ...
    if (s->sample_fmt[1] != AV_SAMPLE_FMT_S16) {
        int out_size = lenout * av_em_get_bytes_per_sample(s->sample_fmt[1]) *
                       s->output_channels;
        output_bak = output;

        if (!s->buffer_size[1] || s->buffer_size[1] < out_size) {
            av_em_free(s->buffer[1]);
            s->buffer_size[1] = out_size;
            s->buffer[1] = av_em_alloc(s->buffer_size[1]);
            if (!s->buffer[1]) {
                av_em_log(s->resample_context, AV_LOG_ERROR, "Could not allocate buffer\n");
                return 0;
            }
    ...
    for (i = 0; i < s->filter_channels; i++) {
        bufin[i] = av_em_malloc_array((nb_samples + s->temp_len), sizeof(short));
        bufout[i] = av_em_malloc_array(lenout, sizeof(short));
    ...
    for (i = 0; i < s->filter_channels; i++) {
        ...
        nb_samples1 = av_em_resample(s->resample_context, buftmp3[i], bufin[i],
                                  &consumed, nb_samples, lenout, is_last);
        s->temp_len = nb_samples - consumed;
        s->temp[i] = av_em_realloc_array(s->temp[i], s->temp_len, sizeof(short));
...
{
    int i;
    av_em_resample_close(s->resample_context);
    for (i = 0; i < s->filter_channels; i++)
        av_em_freep(&s->temp[i]);
    av_em_freep(&s->buffer[0]);
    av_em_freep(&s->buffer[1]);
    av_audio_convert_free(s->convert_ctx[0]);
