...
static const AVEMClass ac3enc_class = {
...
av_cold int ff_ac3_float_mdct_init(AC3EncodeContext *s)
{
    ...
    window = av_em_malloc_array(n, sizeof(*window));
    if (!window) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Cannot allocate memory.\n");
...
av_cold int ff_ac3_float_encode_init(AVEMCodecContext *avctx)
{
