...
static void fill_picture_parameters(const AVEMCodecContext *avctx, AVDXVAContext *ctx, const HEVCContext *h,
                                    DXVA_PicParams_HEVC *pp)
{
...
static void fill_scaling_lists(AVDXVAContext *ctx, const HEVCContext *h, DXVA_Qmatrix_HEVC *qm)
{
    ...
    for (i = 0; i < 6; i++) {
        for (j = 0; j < 16; j++) {
            pos = 4 * em_hevc_diag_scan4x4_y[j] + em_hevc_diag_scan4x4_x[j];
            qm->ucScalingLists0[i][j] = sl->sl[0][i][pos];
        }

        for (j = 0; j < 64; j++) {
            pos = 8 * em_hevc_diag_scan8x8_y[j] + em_hevc_diag_scan8x8_x[j];
...
static int dxva2_hevc_start_frame(AVEMCodecContext *avctx,
...
static int dxva2_hevc_decode_slice(AVEMCodecContext *avctx,
...
static int dxva2_hevc_end_frame(AVEMCodecContext *avctx)
{
...
AVHWAccel em_hevc_dxva2_hwaccel = {
    .name           = "hevc_dxva2",
...
AVHWAccel em_hevc_d3d11va_hwaccel = {
    .name           = "hevc_d3d11va",
    .type           = AVMEDIA_TYPE_VIDEO,
    .id             = AV_CODEC_ID_HEVC,
