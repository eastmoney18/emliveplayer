...
static av_cold int roq_dpcm_encode_close(AVEMCodecContext *avctx)
{
    ROQDPCMContext *context = avctx->priv_data;

    av_em_freep(&context->frame_buffer);

    return 0;
}

static av_cold int roq_dpcm_encode_init(AVEMCodecContext *avctx)
{
    ROQDPCMContext *context = avctx->priv_data;
    int ret;

    if (avctx->channels > 2) {
        av_em_log(avctx, AV_LOG_ERROR, "Audio must be mono or stereo\n");
        return AVERROR(EINVAL);
    }
    if (avctx->sample_rate != 22050) {
        av_em_log(avctx, AV_LOG_ERROR, "Audio must be 22050 Hz\n");
        return AVERROR(EINVAL);
    }
    ...
    context->frame_buffer = av_em_alloc(8 * ROQ_FRAME_SIZE * avctx->channels *
...
static int roq_dpcm_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                                 const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, ROQ_HEADER_SIZE + data_size, 0)) < 0)
        return ret;
    out = avpkt->data;
