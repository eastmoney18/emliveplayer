...
static av_cold void h261_decode_init_vlc(H261Context *h)
{
    ...
    if (!done) {
        ...
        INIT_VLC_STATIC(&h261_mba_vlc, H261_MBA_VLC_BITS, 35,
                        em_h261_mba_bits, 1, 1,
                        em_h261_mba_code, 1, 1, 662);
        INIT_VLC_STATIC(&h261_mtype_vlc, H261_MTYPE_VLC_BITS, 10,
                        em_h261_mtype_bits, 1, 1,
                        em_h261_mtype_code, 1, 1, 80);
        INIT_VLC_STATIC(&h261_mv_vlc, H261_MV_VLC_BITS, 17,
                        &em_h261_mv_tab[0][1], 2, 1,
                        &em_h261_mv_tab[0][0], 2, 1, 144);
        INIT_VLC_STATIC(&h261_cbp_vlc, H261_CBP_VLC_BITS, 63,
                        &em_h261_cbp_tab[0][1], 2, 1,
                        &em_h261_cbp_tab[0][0], 2, 1, 512);
        INIT_VLC_RL(em_h261_rl_tcoeff, 552);
    }
}

static av_cold int h261_decode_init(AVEMCodecContext *avctx)
{
...
static int h261_decode_gob_header(H261Context *h)
{
    ...
    if (s->qscale == 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "qscale has forbidden 0 value\n");
...
static int h261_decode_block(H261Context *h, int16_t *block, int n, int coded)
{
    ...
    for (;;) {
        ...
        if (i >= 64) {
            CLOSE_READER(re, &s->gb);
            av_em_log(s->avctx, AV_LOG_ERROR, "run overflow at %dx%d\n",
                   s->mb_x, s->mb_y);
            return -1;
        }
        j        = scan_table[i];
...
static int h261_decode_mb(H261Context *h)
{
    ...
    if (h->mba_diff < 0) {
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "illegal mba at %d %d\n", s->mb_x, s->mb_y);
        return SLICE_ERROR;
    }
    ...
    if (h->mtype < 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid mtype index %d\n",
               h->mtype);
        return SLICE_ERROR;
    }
    av_assert0(h->mtype < FF_ARRAY_ELEMS(em_h261_mtype_map));
    h->mtype = em_h261_mtype_map[h->mtype];
...
static int h261_decode_frame(AVEMCodecContext *avctx, void *data,
...
static av_cold int h261_decode_end(AVEMCodecContext *avctx)
{
    H261Context *h    = avctx->priv_data;
    MpegEncContext *s = &h->s;
