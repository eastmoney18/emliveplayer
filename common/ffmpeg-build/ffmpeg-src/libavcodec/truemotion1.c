...
static int truemotion1_decode_header(TrueMotion1Context *s)
{
    ...
    if (s->buf[0] < 0x10)
    {
        av_em_log(s->avctx, AV_LOG_ERROR, "invalid header size (%d)\n", s->buf[0]);
        return AVERROR_INVALIDDATA;
    }

    if (header.header_size + 1 > s->size) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Input packet too small.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (header.version >= 2)
    {
        ...
        {
            av_em_log(s->avctx, AV_LOG_ERROR, "invalid header type (%d)\n", header.header_type);
            return AVERROR_INVALIDDATA;
    ...
    if (s->flags & FLAG_SPRITE) {
        avpriv_em_request_sample(s->avctx, "Frame with sprite");
        ...
        if (header.header_type < 2) {
            if ((s->w < 213) && (s->h >= 176))
            {
                ...
                avpriv_em_request_sample(s->avctx, "Interpolated frame");
            }
        }
    }

    if (header.compression >= 17) {
        av_em_log(s->avctx, AV_LOG_ERROR, "invalid compression type (%d)\n", header.compression);
        return AVERROR_INVALIDDATA;
    }
    ...
    if ((header.compression & 1) && header.header_type)
        ...
        if (header.vectable > 0 && header.vectable < 4)
            ...
            av_em_log(s->avctx, AV_LOG_ERROR, "invalid vector table id (%d)\n", header.vectable);
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    s->w >>= width_shift;
    if (s->w & 1) {
        avpriv_em_request_sample(s->avctx, "Frame with odd width");
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (s->w != s->avctx->width || s->h != s->avctx->height ||
        ...
        s->avctx->sample_aspect_ratio = (AVEMRational){ 1 << width_shift, 1 };
        s->avctx->pix_fmt = new_pix_fmt;

        if ((ret = em_set_dimensions(s->avctx, s->w, s->h)) < 0)
            return ret;

        em_set_sar(s->avctx, s->avctx->sample_aspect_ratio);

        av_em_fast_malloc(&s->vert_pred, &s->vert_pred_size, s->avctx->width * sizeof(unsigned int));
        if (!s->vert_pred)
    ...
    if (s->avctx->debug & FF_DEBUG_PICT_INFO)
        av_em_log(s->avctx, AV_LOG_INFO, "tables: %d / %d c:%d %dx%d t:%d %s%s%s%s\n",
...
static av_cold int truemotion1_decode_init(AVEMCodecContext *avctx)
{
...
{\
    if (index_stream_index >= s->index_stream_size) { \
        av_em_log(s->avctx, AV_LOG_INFO, " help! truemotion1 decoder went out of bounds\n"); \
        return; \
    } \
...
do {                                                                \
    if (index >= 1023) {                                            \
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid index value.\n");   \
        return;                                                     \
    }                                                               \
    index++;                                                        \
} while (0)
...
static int truemotion1_decode_frame(AVEMCodecContext *avctx,
...
static av_cold int truemotion1_decode_end(AVEMCodecContext *avctx)
{
    TrueMotion1Context *s = avctx->priv_data;

    av_em_frame_free(&s->frame);
    av_em_freep(&s->vert_pred);

    return 0;
}

AVEMCodec ff_truemotion1_decoder = {
    .name           = "truemotion1",
