...
static av_cold void vaapi_encode_mjpeg_init_tables(AVEMCodecContext *avctx)
{
...
static int vaapi_encode_mjpeg_write_image_header(AVEMCodecContext *avctx,
...
static int vaapi_encode_mjpeg_write_extra_buffer(AVEMCodecContext *avctx,
                                                 VAAPIEncodePicture *pic,
                                                 int index, int *type,
                                                 char *data, size_t *data_len)
{
...
static int vaapi_encode_mjpeg_init_picture_params(AVEMCodecContext *avctx,
                                                  VAAPIEncodePicture *pic)
{
    VAAPIEncodeContext               *ctx = avctx->priv_data;
    VAEncPictureParameterBufferJPEG *vpic = pic->codec_picture_params;
...
static int vaapi_encode_mjpeg_init_slice_params(AVEMCodecContext *avctx,
                                                VAAPIEncodePicture *pic,
                                                VAAPIEncodeSlice *slice)
{
    VAEncPictureParameterBufferJPEG *vpic = pic->codec_picture_params;
...
static av_cold int vaapi_encode_mjpeg_init_internal(AVEMCodecContext *avctx)
{
    ...
    if (priv->quality < 1 || priv->quality > 100) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid quality value %d "
               "(must be 1-100).\n", priv->quality);
        return AVERROR(EINVAL);
    }
...
static av_cold int vaapi_encode_mjpeg_init(AVEMCodecContext *avctx)
{
    return ff_vaapi_encode_init(avctx, &vaapi_encode_type_mjpeg);
}

static const AVEMCodecDefault vaapi_encode_mjpeg_defaults[] = {
    { "global_quality", "80" },
    { NULL },
};

static const AVEMClass vaapi_encode_mjpeg_class = {
    .class_name = "mjpeg_vaapi",
    .item_name  = av_em_default_item_name,
    .version    = LIBAVUTIL_VERSION_INT,
};

AVEMCodec ff_mjpeg_vaapi_encoder = {
    .name           = "mjpeg_vaapi",
