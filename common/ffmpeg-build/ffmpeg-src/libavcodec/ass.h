...
/**
 * Generate a suitable AVEMCodecContext.subtitle_header for SUBTITLE_ASS.
 *
 ...
 */
int ff_ass_subtitle_header(AVEMCodecContext *avctx,
 ...
 * @return >= 0 on success otherwise an error code <0
 */
int ff_ass_subtitle_header_default(AVEMCodecContext *avctx);
 ...
 * FFASSDecoderContext.
 */
void ff_ass_decoder_flush(AVEMCodecContext *avctx);

/**
 * Escape a text subtitle using ASS syntax into an AVEMBPrint buffer.
 * Newline characters will be escaped to \N.
 *
 * @param buf pointer to an initialized AVEMBPrint buffer
 * @param p source text
 ...
 */
void ff_ass_bprint_text_event(AVEMBPrint *buf, const char *p, int size,
