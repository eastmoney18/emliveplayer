...
static int xwd_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                            const AVFrame *pict, int *got_packet)
{
    ...
    pixdepth = av_em_get_bits_per_pixel(desc);
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, out_size, 0)) < 0)
        return ret;
    buf = pkt->data;
