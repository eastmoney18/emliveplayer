...
{
    int i;
    AVEMRational bestq = (AVEMRational) {0, 0};
    AVEMRational ext;
    AVEMRational target = av_em_inv_q(s->avctx->time_base);
    ...
    for (i = 1; i < 14; i++) {
        ...
        for (ext.num=1; ext.num <= 4; ext.num++) {
            for (ext.den=1; ext.den <= 32; ext.den++) {
                AVEMRational q = av_em_mul_q(ext, ff_mpeg12_frame_rate_tab[i]);

                if (s->codec_id != AV_CODEC_ID_MPEG2VIDEO && (ext.den!=1 || ext.num!=1))
                    continue;
                if (av_em_gcd(ext.den, ext.num) != 1)
                    continue;

                if (    bestq.num==0
                    || av_em_nearer_q(target, bestq, q) < 0
                    || ext.num==1 && ext.den==1 && av_em_nearer_q(target, bestq, q) == 0) {
                    bestq               = q;
...
static av_cold int encode_init(AVEMCodecContext *avctx)
{
    ...
    if (find_frame_rate_index(s) < 0) {
        if (s->strict_std_compliance > FF_COMPLIANCE_EXPERIMENTAL) {
            av_em_log(avctx, AV_LOG_ERROR, "MPEG-1/2 does not support %d/%d fps\n",
    ...
    if (avctx->profile == FF_PROFILE_UNKNOWN) {
        if (avctx->level != FF_LEVEL_UNKNOWN) {
            av_em_log(avctx, AV_LOG_ERROR, "Set profile and level\n");
            return -1;
        }
        /* Main or 4:2:2 */
    ...
    if ((avctx->width & 0xFFF) == 0 && (avctx->height & 0xFFF) == 1) {
        av_em_log(avctx, AV_LOG_ERROR, "Width / Height is invalid for MPEG2\n");
        return AVERROR(EINVAL);
    }

    if (s->strict_std_compliance > FF_COMPLIANCE_UNOFFICIAL) {
        if ((avctx->width & 0xFFF) == 0 || (avctx->height & 0xFFF) == 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Width or Height are not allowed to be multiples of 4096\n"
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (s->tc_opt_str) {
        AVEMRational rate = ff_mpeg12_frame_rate_tab[s->frame_rate_index];
        int ret = av_em_timecode_init_from_string(&s->tc, rate, s->tc_opt_str, s);
        if (ret < 0)
            return ret;
...
static void mpeg1_encode_sequence_header(MpegEncContext *s)
{
    ...
    AVEMRational aspect_ratio = s->avctx->sample_aspect_ratio;

    if (aspect_ratio.num == 0 || aspect_ratio.den == 0)
        aspect_ratio = (AVEMRational){1,1};             // pixel aspect 1.1 (VGA)

    if (s->current_picture.f->key_frame) {
        AVEMRational framerate = ff_mpeg12_frame_rate_tab[s->frame_rate_index];
        ...
        if (s->codec_id == AV_CODEC_ID_MPEG2VIDEO) {
            ...
            side_data = av_em_frame_get_side_data(s->current_picture_ptr->f, AV_FRAME_DATA_PANSCAN);
            if (side_data) {
                AVEMPanScan *pan_scan = (AVEMPanScan *)side_data->data;
        ...
        if (s->drop_frame_timecode)
            time_code = av_em_timecode_adjust_ntsc_framenum2(time_code, fps);
...
void ff_mpeg1_encode_picture_header(MpegEncContext *s, int picture_number)
{
    ...
    side_data = av_em_frame_get_side_data(s->current_picture_ptr->f,
...
static const AVEMClass mpeg ## x ## _class = {            \
...
AVEMCodec ff_mpeg1video_encoder = {
    .name                 = "mpeg1video",
...
AVEMCodec ff_mpeg2video_encoder = {
    .name                 = "mpeg2video",
