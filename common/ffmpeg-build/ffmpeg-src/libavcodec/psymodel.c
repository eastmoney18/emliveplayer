...
extern const FFPsyModel em_aac_psy_model;

av_cold int ff_psy_init(FFPsyContext *ctx, AVEMCodecContext *avctx, int num_lens,
                        ...
                        int num_groups, const uint8_t *group_map)
{
    ...
    ctx->avctx = avctx;
    ctx->ch        = av_em_mallocz_array(sizeof(ctx->ch[0]), avctx->channels * 2);
    ctx->group     = av_em_mallocz_array(sizeof(ctx->group[0]), num_groups);
    ctx->bands     = av_em_malloc_array (sizeof(ctx->bands[0]),      num_lens);
    ctx->num_bands = av_em_malloc_array (sizeof(ctx->num_bands[0]),  num_lens);
...
av_cold void ff_psy_end(FFPsyContext *ctx)
{
    if (ctx->model && ctx->model->end)
        ctx->model->end(ctx);
    av_em_freep(&ctx->bands);
    av_em_freep(&ctx->num_bands);
    av_em_freep(&ctx->group);
    av_em_freep(&ctx->ch);
}
...
av_cold struct FFPsyPreprocessContext* ff_psy_preprocess_init(AVEMCodecContext *avctx)
{
    FFPsyPreprocessContext *ctx;
    int i;
    float cutoff_coeff = 0;
    ctx        = av_em_mallocz(sizeof(FFPsyPreprocessContext));
    if (!ctx)
        return NULL;
    ctx->avctx = avctx;
    ...
    if (avctx->codec_id != AV_CODEC_ID_AAC) {
        ...
        if (ctx->fcoeffs) {
            ctx->fstate = av_em_mallocz_array(sizeof(ctx->fstate[0]), avctx->channels);
            if (!ctx->fstate) {
                av_em_free(ctx->fcoeffs);
                av_em_free(ctx);
                return NULL;
            }
