...
static av_cold int dfa_decode_init(AVEMCodecContext *avctx)
{
    ...
    av_assert0(av_em_image_check_size(avctx->width, avctx->height, 0, avctx) >= 0);

    s->frame_buf = av_em_mallocz(avctx->width * avctx->height);
    if (!s->frame_buf)
    ...
    while (segments--) {
        ...
        if (bitbuf & mask) {
            ...
            if (frame - frame_start < offset || frame_end - frame < count)
                return AVERROR_INVALIDDATA;
            av_em_memcpy_backptr(frame, offset, count);
            frame += count;
        } else {
...
static int dfa_decode_frame(AVEMCodecContext *avctx,
    ...
    while (bytestream2_get_bytes_left(&gb) > 0) {
        ...
        if (chunk_type == 1) {
            ...
            if (decoder[chunk_type - 2](&gb, s->frame_buf, avctx->width, avctx->height)) {
                av_em_log(avctx, AV_LOG_ERROR, "Error decoding %s chunk\n",
...
static av_cold int dfa_decode_end(AVEMCodecContext *avctx)
{
    DfaContext *s = avctx->priv_data;
