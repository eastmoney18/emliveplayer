...
static int txd_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                            AVEMPacket *avpkt) {
    GetByteContext gb;
    ...
    if (version < 8 || version > 9) {
        av_em_log(avctx, AV_LOG_ERROR, "texture data version %i is unsupported\n",
                                                                    version);
    ...
    if (depth == 8) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "depth of %i is unsupported\n", depth);
        return AVERROR_PATCHWELCOME;
    }
...
unsupported:
    av_em_log(avctx, AV_LOG_ERROR, "unsupported d3d format (%08x)\n", d3d_format);
    return AVERROR_PATCHWELCOME;
}

AVEMCodec ff_txd_decoder = {
    .name           = "txd",
