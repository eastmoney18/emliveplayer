...
static void compress_texture(AVEMCodecContext *avctx, const AVFrame *f)
{
...
static int hap_compress_frame(AVEMCodecContext *avctx, uint8_t *dst)
{
    HapContext *ctx = avctx->priv_data;
    int i, final_size = 0;
    ...
    for (i = 0; i < ctx->chunk_count; i++) {
        ...
        if (ret != SNAPPY_OK) {
            av_em_log(avctx, AV_LOG_ERROR, "Snappy compress error.\n");
            return AVERROR_BUG;
        }
...
static int hap_encode(AVEMCodecContext *avctx, AVEMPacket *pkt,
                      const AVFrame *frame, int *got_packet)
{
    ...
    ret = em_alloc_packet2(avctx, pkt, pktsize, header_length);
    if (ret < 0)
        return ret;
    ...
    av_em_shrink_packet(pkt, final_data_size + header_length);
...
static av_cold int hap_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->width % 4 || avctx->height % 4) {
        av_em_log(avctx, AV_LOG_ERROR, "Video size %dx%d is not multiple of 4.\n",
               avctx->width, avctx->height);
    ...
    case HAP_FMT_YCOCGDXT5:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Invalid format %02X\n", ctx->opt_tex_fmt);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (corrected_chunk_count != ctx->opt_chunk_count) {
        av_em_log(avctx, AV_LOG_INFO, "%d chunks requested but %d used.\n",
                                    ctx->opt_chunk_count, corrected_chunk_count);
    }
    ret = em_hap_set_chunk_count(ctx, corrected_chunk_count, 1);
    if (ret != 0)
        return ret;

    ctx->max_snappy = snappy_max_compressed_length(ctx->tex_size / corrected_chunk_count);

    ctx->tex_buf  = av_em_alloc(ctx->tex_size);
    if (!ctx->tex_buf)
...
static const AVEMClass hapenc_class = {
