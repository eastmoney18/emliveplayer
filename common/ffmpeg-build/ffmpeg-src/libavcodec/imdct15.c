...
av_cold void ff_imdct15_uninit(IMDCT15Context **ps)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(s->exptab); i++)
        av_em_freep(&s->exptab[i]);

    av_em_freep(&s->twiddle_exptab);
...
av_cold int ff_imdct15_init(IMDCT15Context **ps, int N)
{
    ...
    s->tmp  = av_em_malloc_array(len, 2 * sizeof(*s->tmp));
    if (!s->tmp)
        goto fail;

    s->twiddle_exptab  = av_em_malloc_array(s->len4, sizeof(*s->twiddle_exptab));
    if (!s->twiddle_exptab)
        goto fail;
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(s->exptab); i++) {
        int N = 15 * (1 << i);
        s->exptab[i] = av_em_alloc(sizeof(*s->exptab[i]) * FFMAX(N, 19));
        if (!s->exptab[i])
            goto fail;
