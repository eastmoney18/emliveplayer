...
static av_cold int qtrle_encode_end(AVEMCodecContext *avctx)
{
    QtrleEncContext *s = avctx->priv_data;

    av_em_frame_free(&s->previous_frame);
    av_em_free(s->rlecode_table);
    av_em_free(s->length_table);
    av_em_free(s->skip_table);
    return 0;
}

static av_cold int qtrle_encode_init(AVEMCodecContext *avctx)
{
    QtrleEncContext *s = avctx->priv_data;
    ...
    case AV_PIX_FMT_GRAY8:
        if (avctx->width % 4) {
            av_em_log(avctx, AV_LOG_ERROR, "Width not being a multiple of 4 is not supported\n");
            return AVERROR(EINVAL);
        }
    ...
    case AV_PIX_FMT_ARGB:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported colorspace.\n");
        break;
    }
    avctx->bits_per_coded_sample = avctx->pix_fmt == AV_PIX_FMT_GRAY8 ? 40 : s->pixel_size*8;

    s->rlecode_table = av_em_mallocz(s->logical_width);
    s->skip_table    = av_em_mallocz(s->logical_width);
    s->length_table  = av_em_mallocz_array(s->logical_width + 1, sizeof(int));
    if (!s->skip_table || !s->length_table || !s->rlecode_table) {
        av_em_log(avctx, AV_LOG_ERROR, "Error allocating memory.\n");
        return AVERROR(ENOMEM);
    }
    s->previous_frame = av_em_frame_alloc();
    if (!s->previous_frame) {
        av_em_log(avctx, AV_LOG_ERROR, "Error allocating picture\n");
        return AVERROR(ENOMEM);
    }
...
static int qtrle_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                              const AVFrame *pict, int *got_packet)
{
    QtrleEncContext * const s = avctx->priv_data;
    int ret;

    if ((ret = em_alloc_packet2(avctx, pkt, s->max_buf_size, 0)) < 0)
        return ret;
    ...
    av_em_frame_unref(s->previous_frame);
    ret = av_em_frame_ref(s->previous_frame, pict);
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "cannot add reference\n");
        return ret;
    }
