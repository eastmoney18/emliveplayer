...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    s->block   = av_em_calloc(s->block_len, sizeof(int));
    s->wrapbuf = av_em_calloc(s->wrapbuf_len, sizeof(int));
    s->ampbuf  = av_em_calloc(0x10000, sizeof(int));
    s->bitstream = av_em_calloc(s->max_framesize, sizeof(*s->bitstream));
...
static av_cold int decode_close(AVEMCodecContext *avctx)
{
    ...
    av_em_freep(&s->block);
    av_em_freep(&s->wrapbuf);
    av_em_freep(&s->ampbuf);
...
AVEMCodec ff_interplay_acm_decoder = {
    .name           = "interplayacm",
