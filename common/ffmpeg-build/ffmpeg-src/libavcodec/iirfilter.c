                                           ...
                                           float stopband)
{
    ...
    if (filt_mode != FF_FILTER_MODE_LOWPASS) {
        av_em_log(avc, AV_LOG_ERROR, "Butterworth filter currently only supports "
                                      ...
                                      float cutoff_ratio, float stopband)
{
    ...
    if (filt_mode != FF_FILTER_MODE_HIGHPASS &&
        ...
        av_em_log(avc, AV_LOG_ERROR, "Biquad filter currently only supports "
                                  "high-pass and low-pass filter modes\n");
        return -1;
    }
    if (order != 2) {
        av_em_log(avc, AV_LOG_ERROR, "Biquad filter must have order of 2\n");
        return -1;
    }
                                                            ...
                                                            float stopband, float ripple)
{
    ...
    case FF_FILTER_TYPE_BIQUAD:
        ...
        break;
    default:
        av_em_log(avc, AV_LOG_ERROR, "filter type is not currently implemented\n");
        goto init_fail;
    }
...
{
    FFIIRFilterState *s = av_em_mallocz(sizeof(FFIIRFilterState) + sizeof(s->x[0]) * (order - 1));
    return s;
}
...
av_cold void ff_iir_filter_free_coeffsp(struct FFIIRFilterCoeffs **coeffsp)
{
    ...
    if (coeffs) {
        av_em_freep(&coeffs->cx);
        av_em_freep(&coeffs->cy);
    }
    av_em_freep(coeffsp);
}
