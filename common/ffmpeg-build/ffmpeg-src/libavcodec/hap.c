...
int em_hap_set_chunk_count(HapContext *ctx, int count, int first_in_frame)
{
    int ret = 0;
    if (first_in_frame == 1 && ctx->chunk_count != count) {
        int ret = av_em_reallocp_array(&ctx->chunks, count, sizeof(HapChunk));
        if (ret == 0)
            ret = av_em_reallocp_array(&ctx->chunk_results, count, sizeof(int));
        if (ret < 0) {
            ctx->chunk_count = 0;
        } else {
...
av_cold void em_hap_free_context(HapContext *ctx)
{
    av_em_freep(&ctx->tex_buf);
    av_em_freep(&ctx->chunks);
    av_em_freep(&ctx->chunk_results);
}
