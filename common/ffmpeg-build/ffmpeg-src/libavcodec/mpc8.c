...
static av_cold int mpc8_decode_init(AVEMCodecContext * avctx)
{
    int i;
    ...
    if (c->maxbands >= BANDS) {
        av_em_log(avctx,AV_LOG_ERROR, "maxbands %d too high\n", c->maxbands);
        return AVERROR_INVALIDDATA;
    }
    channels = get_bits(&gb, 4) + 1;
    if (channels > 2) {
        avpriv_em_request_sample(avctx, "Multichannel MPC SV8");
...
static int mpc8_decode_frame(AVEMCodecContext * avctx, void *data,
                             int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if(maxband > c->maxbands + 1) {
        av_em_log(avctx, AV_LOG_ERROR, "maxband %d too large\n",maxband);
        return AVERROR_INVALIDDATA;
    }
...
static av_cold void mpc8_decode_flush(AVEMCodecContext *avctx)
{
    MPCContext *c = avctx->priv_data;
    c->cur_frame = 0;
}

AVEMCodec ff_mpc8_decoder = {
    .name           = "mpc8",
