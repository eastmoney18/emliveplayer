...
static int gif_image_write_image(AVEMCodecContext *avctx,
                                 ...
                                 AVEMPacket *pkt)
{
    ...
    if ((s->flags & GF_OFFSETTING) && s->last_frame && !palette) {
        ...
        av_em_log(avctx, AV_LOG_DEBUG,"%dx%d image at pos (%d;%d) [area:%dx%d]\n",
    ...
    if (honor_transparency && trans < 0) {
        ...
        if (trans < 0) { // TODO, patch welcome
            av_em_log(avctx, AV_LOG_DEBUG, "No available color, can not use transparency\n");
        } else {
            uint8_t *pal_exdata = s->pal_exdata;
            if (!pal_exdata)
                pal_exdata = av_em_packet_new_side_data(pkt, AV_PKT_DATA_PALETTE, AVPALETTE_SIZE);
            if (!pal_exdata)
...
static av_cold int gif_encode_init(AVEMCodecContext *avctx)
{
    GIFContext *s = avctx->priv_data;

    if (avctx->width > 65535 || avctx->height > 65535) {
        av_em_log(avctx, AV_LOG_ERROR, "GIF does not support resolutions above 65535x65535\n");
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    s->lzw = av_em_mallocz(ff_lzw_encode_state_size);
    s->buf_size = avctx->width*avctx->height*2 + 1000;
    s->buf = av_em_alloc(s->buf_size);
    s->tmpl = av_em_alloc(avctx->width);
    if (!s->tmpl || !s->buf || !s->lzw)
        return AVERROR(ENOMEM);

    if (avpriv_em_set_systematic_pal2(s->palette, avctx->pix_fmt) < 0)
...
static int gif_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                            const AVFrame *pict, int *got_packet)
{
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, avctx->width*avctx->height*7/5 + AV_INPUT_BUFFER_MIN_SIZE, 0)) < 0)
        return ret;
    outbuf_ptr = pkt->data;
    end        = pkt->data + pkt->size;

    if (avctx->pix_fmt == AV_PIX_FMT_PAL8) {
        uint8_t *pal_exdata = av_em_packet_new_side_data(pkt, AV_PKT_DATA_PALETTE, AVPALETTE_SIZE);
        if (!pal_exdata)
            return AVERROR(ENOMEM);
...
static int gif_encode_close(AVEMCodecContext *avctx)
{
    GIFContext *s = avctx->priv_data;
...
static const AVEMClass gif_class = {
