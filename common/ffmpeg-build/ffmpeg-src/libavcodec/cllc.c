...
{
    AVEMCodecContext *avctx = ctx->avctx;
    uint8_t *dst;
    int pred[4];
    int ret;
    int i, j;
...
static int decode_yuv_frame(CLLCContext *ctx, GetBitContext *gb, AVFrame *pic)
{
    ...
    block = get_bits(gb, 8);
    if (block) {
        avpriv_em_request_sample(ctx->avctx, "Blocked YUV");
...
static int cllc_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_picture_ptr, AVEMPacket *avpkt)
{
    ...
    av_em_fast_padded_malloc(&ctx->swapped_buf,
                          &ctx->swapped_buf_size, data_size);
    if (!ctx->swapped_buf) {
        av_em_log(avctx, AV_LOG_ERROR, "Could not allocate swapped buffer.\n");
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Frame coding type: %d\n", coding_type);
    ...
    case 3:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unknown coding type: %d.\n", coding_type);
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int cllc_decode_close(AVEMCodecContext *avctx)
{
    CLLCContext *ctx = avctx->priv_data;

    av_em_freep(&ctx->swapped_buf);

    return 0;
}

static av_cold int cllc_decode_init(AVEMCodecContext *avctx)
{
    CLLCContext *ctx = avctx->priv_data;
    ...
    em_bswapdsp_init(&ctx->bdsp);

    return 0;
}

AVEMCodec ff_cllc_decoder = {
    .name           = "cllc",
