...
{
    AVEMCodecContext *avctx = r->s.avctx;
    int mb_bits;
    ...
    if(rpr){
        if (rpr > r->max_rpr) {
            av_em_log(avctx, AV_LOG_ERROR, "rpr too large\n");
...
static int rv30_decode_intra_types(RV34DecContext *r, GetBitContext *gb, int8_t *dst)
{
    ...
    for(i = 0; i < 4; i++, dst += r->intra_types_stride - 4){
        for(j = 0; j < 4; j+= 2){
            ...
            if (code > 80U*2U) {
                av_em_log(r->s.avctx, AV_LOG_ERROR, "Incorrect intra prediction code\n");
                return -1;
            }
            for(k = 0; k < 2; k++){
                ...
                *dst++ = rv30_itype_from_context[A * 90 + B * 9 + rv30_itype_code[code + k]];
                if(dst[-1] == 9){
                    av_em_log(r->s.avctx, AV_LOG_ERROR, "Incorrect intra prediction mode\n");
                    return -1;
                }
            }
        }
...
static int rv30_decode_mb_info(RV34DecContext *r)
{
    ...
    if (code > 11) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Incorrect MB type code\n");
        return -1;
    }
    if(code > 5){
        av_em_log(s->avctx, AV_LOG_ERROR, "dquant needed\n");
        code -= 6;
    }
 ...
 * Initialize decoder.
 */
static av_cold int rv30_decode_init(AVEMCodecContext *avctx)
{
    ...
    if(avctx->extradata_size < 2*r->max_rpr + 8){
        av_em_log(avctx, AV_LOG_WARNING, "Insufficient extradata - need at least %d bytes, got %d\n",
