...
static void ffat_update_ctx(AVEMCodecContext *avctx)
{
...
static int get_ilbc_mode(AVEMCodecContext *avctx)
{
...
static av_cold int ffat_init_encoder(AVEMCodecContext *avctx)
{
    ATDecodeContext *at = avctx->priv_data;
    OSStatus status;
    ...
    AudioStreamBasicDescription in_format = {
        ...
        .mBytesPerPacket = av_em_get_bytes_per_sample(avctx->sample_fmt) * avctx->channels,
        .mFramesPerPacket = 1,
        .mBytesPerFrame = av_em_get_bytes_per_sample(avctx->sample_fmt) * avctx->channels,
        .mChannelsPerFrame = avctx->channels,
        .mBitsPerChannel = av_em_get_bytes_per_sample(avctx->sample_fmt) * 8,
    };
    AudioStreamBasicDescription out_format = {
        .mSampleRate = avctx->sample_rate,
    ...
    AudioChannelLayout *channel_layout = av_em_alloc(layout_size);
    ...
    if (AudioConverterSetProperty(at->converter, kAudioConverterInputChannelLayout,
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported input channel layout\n");
        av_em_free(channel_layout);
        return AVERROR(EINVAL);
    }
    ...
    if (AudioConverterSetProperty(at->converter, kAudioConverterOutputChannelLayout,
                                  layout_size, channel_layout)) {
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported output channel layout\n");
        av_em_free(channel_layout);
        return AVERROR(EINVAL);
    }
    av_em_free(channel_layout);
    ...
    if (avctx->bit_rate > 0) {
        ...
        if (!status && size) {
            ...
            int count;
            int i;
            AudioValueRange *ranges = av_em_alloc(size);
            if (!ranges)
                return AVERROR(ENOMEM);
            AudioConverterGetProperty(at->converter,
                                      kAudioConverterApplicableEncodeBitRates,
    ...
    if (!AudioConverterGetPropertyInfo(at->converter, kAudioConverterCompressionMagicCookie,
        ...
        uint8_t *extradata;
        if (!(avctx->extradata = av_em_mallocz(avctx->extradata_size + AV_INPUT_BUFFER_PADDING_SIZE)))
            return AVERROR(ENOMEM);
        ...
        if (status != 0) {
            av_em_log(avctx, AV_LOG_ERROR, "AudioToolbox cookie error: %i\n", (int)status);
            return AVERROR_UNKNOWN;
...
static int ffat_encode(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                       const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if (frame) {
        ...
        if (ff_bufqueue_is_full(&at->frame_queue)) {
             ...
             */
            av_em_log(avctx, AV_LOG_ERROR, "Bug: frame queue is too small.\n");
            return AVERROR_BUG;
        }
        ...
        in_frame = av_em_frame_clone(frame);
        if (!in_frame)
            return AVERROR(ENOMEM);
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, at->pkt_size, 0)) < 0)
        return ret;
    ...
    *got_packet_ptr = avctx->frame_size / at->frame_size;
    ...
    if ((!ret || ret == 1) && *got_packet_ptr) {
        ...
        av_em_log(avctx, AV_LOG_WARNING, "Encode error: %i\n", ret);
    }

    return 0;
}

static av_cold void ffat_encode_flush(AVEMCodecContext *avctx)
{
    ATDecodeContext *at = avctx->priv_data;
...
static av_cold int ffat_close_encoder(AVEMCodecContext *avctx)
{
    ATDecodeContext *at = avctx->priv_data;
...
static const AVEMProfile aac_profiles[] = {
...
#define FFAT_ENC_CLASS(NAME) \
    static const AVEMClass ffat_##NAME##_enc_class = { \
...
#define FFAT_ENC(NAME, ID, PROFILES, ...) \
    FFAT_ENC_CLASS(NAME) \
    AVEMCodec ff_##NAME##_at_encoder = { \
        .name           = #NAME "_at", \
