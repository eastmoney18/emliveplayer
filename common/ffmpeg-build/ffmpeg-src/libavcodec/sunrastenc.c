...
static void sunrast_image_write_header(AVEMCodecContext *avctx)
{
...
static void sunrast_image_write_image(AVEMCodecContext *avctx,
...
static av_cold int sunrast_encode_init(AVEMCodecContext *avctx)
{
    SUNRASTContext *s = avctx->priv_data;
...
FF_DISABLE_DEPRECATION_WARNINGS
    ...
    case FF_CODER_TYPE_RAW:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "invalid coder_type\n");
...
static int sunrast_encode_frame(AVEMCodecContext *avctx,  AVEMPacket *avpkt,
                                const AVFrame *frame, int *got_packet_ptr)
{
    SUNRASTContext *s = avctx->priv_data;
    int ret;

    if ((ret = em_alloc_packet2(avctx, avpkt, s->size, 0)) < 0)
        return ret;
...
static const AVEMClass sunrast_class = {
    .class_name = "sunrast",
...
#if FF_API_CODER_TYPE
static const AVEMCodecDefault sunrast_defaults[] = {
     { "coder", "rle" },
     { NULL },
};
#endif

AVEMCodec ff_sunrast_encoder = {
    .name           = "sunrast",
