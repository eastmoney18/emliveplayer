...
static inline void ls_encode_run(JLSState *state, PutBitContext *pb, int run,
                                 int comp, int trail)
{
    while (run >= (1 << em_log2_run[state->run_index[comp]])) {
        put_bits(pb, 1, 1);
        run -= 1 << em_log2_run[state->run_index[comp]];
    ...
    if (!trail && run) {
        ...
        put_bits(pb, 1, 0);
        if (em_log2_run[state->run_index[comp]])
            put_bits(pb, em_log2_run[state->run_index[comp]], run);
    }
}
...
static int encode_picture_ls(AVEMCodecContext *avctx, AVEMPacket *pkt,
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, avctx->width  *avctx->height * comps * 4 +
                                AV_INPUT_BUFFER_MIN_SIZE, 0)) < 0)
        return ret;

    buf2 = av_em_alloc(pkt->size);
    if (!buf2)
        goto memfail;
    ...
    zero = last = av_em_mallocz(FFABS(p->linesize[0]));
    if (!zero)
        goto memfail;
...
static av_cold int encode_init_ls(AVEMCodecContext *ctx)
{
...
static const AVEMClass jpegls_class = {
    .class_name = "jpegls",
