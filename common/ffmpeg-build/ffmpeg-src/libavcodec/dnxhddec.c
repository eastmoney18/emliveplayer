...
static av_cold int dnxhd_decode_init(AVEMCodecContext *avctx)
{
    ...
    ctx->rows = av_em_mallocz_array(avctx->thread_count, sizeof(RowContext));
    if (!ctx->rows)
...
static int dnxhd_init_vlc(DNXHDContext *ctx, uint32_t cid, int bitdepth)
{
    if (cid != ctx->cid) {
        ...
        if ((index = ff_dnxhd_get_cid_table(cid)) < 0) {
            av_em_log(ctx->avctx, AV_LOG_ERROR, "unsupported cid %d\n", cid);
            return AVERROR(ENOSYS);
        }
        if (ff_dnxhd_cid_table[index].bit_depth != bitdepth &&
            ff_dnxhd_cid_table[index].bit_depth != DNXHD_VARIABLE) {
            av_em_log(ctx->avctx, AV_LOG_ERROR, "bit depth mismatches %d %d\n", ff_dnxhd_cid_table[index].bit_depth, bitdepth);
            return AVERROR_INVALIDDATA;
        }
        if (bitdepth > 10) {
            avpriv_em_request_sample(ctx->avctx, "DNXHR 12-bit");
            if (ctx->avctx->strict_std_compliance > FF_COMPLIANCE_EXPERIMENTAL)
                return AVERROR_PATCHWELCOME;
        }
        ctx->cid_table = &ff_dnxhd_cid_table[index];
        av_em_log(ctx->avctx, AV_LOG_VERBOSE, "Profile cid %d.\n", cid);
...
static av_cold int dnxhd_decode_init_thread_copy(AVEMCodecContext *avctx)
{
    DNXHDContext *ctx = avctx->priv_data;
                               ...
                               int first_field)
{
    ...
    if (buf[5] & 2) { /* interlaced */
        ...
        av_em_log(ctx->avctx, AV_LOG_DEBUG,
    ...
    if (ctx->mbaff && ctx->cid_table->cid != 1260)
        av_em_log(ctx->avctx, AV_LOG_WARNING,
    ...
    if (ctx->is_444) {
        if (bitdepth == 8) {
            avpriv_em_request_sample(ctx->avctx, "4:4:4 8 bits\n");
            return AVERROR_INVALIDDATA;
        } else if (bitdepth == 10) {
    ...
    if (ctx->bit_depth != old_bit_depth) {
        em_blockdsp_init(&ctx->bdsp, ctx->avctx);
    ...
    if (ctx->width != ctx->cid_table->width &&
        ...
        av_em_reduce(&ctx->avctx->sample_aspect_ratio.num,
    ...
    if (buf_size < ctx->cid_table->coding_unit_size) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "incorrect frame size (%d < %d).\n",
    ...
    av_em_log(ctx->avctx, AV_LOG_VERBOSE, "%dx%d, 4:%s %d bits, MBAFF=%d ACT=%d\n",
                                                   ...
                                                   int dc_shift)
{
    ...
    while (index1 != eob_index) {
        ...
        if (++i > 63) {
            av_em_log(ctx->avctx, AV_LOG_ERROR, "ac tex damaged %d, %d\n", n, i);
            ret = -1;
            break;
        }
...
static int dnxhd_decode_row(AVEMCodecContext *avctx, void *data,
...
static int dnxhd_decode_frame(AVEMCodecContext *avctx, void *data,
...
decode_coding_unit:
    ...
    if ((avctx->width || avctx->height) &&
        ...
        av_em_log(avctx, AV_LOG_WARNING, "frame size changed: %dx%d -> %dx%d\n",
               avctx->width, avctx->height, ctx->width, ctx->height);
        first_field = 1;
    }
    if (avctx->pix_fmt != AV_PIX_FMT_NONE && avctx->pix_fmt != ctx->pix_fmt) {
        av_em_log(avctx, AV_LOG_WARNING, "pix_fmt changed: %s -> %s\n",
               av_em_get_pix_fmt_name(avctx->pix_fmt), av_em_get_pix_fmt_name(ctx->pix_fmt));
        first_field = 1;
    }

    avctx->pix_fmt = ctx->pix_fmt;
    ret = em_set_dimensions(avctx, ctx->width, ctx->height);
    if (ret < 0)
        return ret;
    ...
    avctx->pix_fmt = ctx->pix_fmt;
    if (ret) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "%d lines with errors\n", ret);
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int dnxhd_decode_close(AVEMCodecContext *avctx)
{
    DNXHDContext *ctx = avctx->priv_data;
    ...
    av_em_freep(&ctx->rows);

    return 0;
}

AVEMCodec ff_dnxhd_decoder = {
    .name           = "dnxhd",
