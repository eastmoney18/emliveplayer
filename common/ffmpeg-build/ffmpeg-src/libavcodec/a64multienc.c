...
static void to_meta_with_crop(AVEMCodecContext *avctx,
...
static void render_charset(AVEMCodecContext *avctx, uint8_t *charset,
                           uint8_t *colrammap)
{
...
static av_cold int a64multi_close_encoder(AVEMCodecContext *avctx)
{
    A64Context *c = avctx->priv_data;
    av_em_freep(&c->mc_meta_charset);
    av_em_freep(&c->mc_best_cb);
    av_em_freep(&c->mc_charset);
    av_em_freep(&c->mc_charmap);
    av_em_freep(&c->mc_colram);
    return 0;
}

static av_cold int a64multi_encode_init(AVEMCodecContext *avctx)
{
    A64Context *c = avctx->priv_data;
    int a;
    av_em_lfg_init(&c->randctx, 1);
    ...
    av_em_log(avctx, AV_LOG_INFO, "charset lifetime set to %d frame(s)\n", c->mc_lifetime);
    ...
    if (!(c->mc_meta_charset = av_em_mallocz_array(c->mc_lifetime, 32000 * sizeof(int))) ||
       !(c->mc_best_cb       = av_em_alloc(CHARSET_CHARS * 32 * sizeof(int)))     ||
       !(c->mc_charmap       = av_em_mallocz_array(c->mc_lifetime, 1000 * sizeof(int))) ||
       !(c->mc_colram        = av_em_mallocz(CHARSET_CHARS * sizeof(uint8_t)))     ||
       !(c->mc_charset       = av_em_alloc(0x800 * (INTERLACED+1) * sizeof(uint8_t)))) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to allocate buffer memory.\n");
        return AVERROR(ENOMEM);
    }

    /* set up extradata */
    if (!(avctx->extradata = av_em_mallocz(8 * 4 + AV_INPUT_BUFFER_PADDING_SIZE))) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to allocate memory for extradata.\n");
        return AVERROR(ENOMEM);
    }
...
static int a64multi_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                                 const AVFrame *p, int *got_packet)
{
    ...
    if (c->mc_frame_counter == c->mc_lifetime) {
        ...
        if (c->mc_lifetime) {
            ...
            if ((ret = em_alloc_packet2(avctx, pkt, alloc_size, 0)) < 0)
                return ret;
            buf = pkt->data;
...
AVEMCodec ff_a64multi5_encoder = {
    .name           = "a64multi5",
