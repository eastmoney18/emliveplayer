...
static int parse_palette(AVEMCodecContext *avctx, GetByteContext *gbc,
...
static int decode_rle(AVEMCodecContext *avctx, AVFrame *p, GetByteContext *gbc,
                      int step)
{
    int i, j;
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (ver == 1) {
        avpriv_em_request_sample(avctx, "QuickDraw version 1");
        return AVERROR_PATCHWELCOME;
    } else if (ver != 2) {
        avpriv_em_request_sample(avctx, "QuickDraw version unknown (%X)", bytestream2_get_be32(&gbc));
        return AVERROR_PATCHWELCOME;
    }
    ...
    while (bytestream2_get_bytes_left(&gbc) >= 4) {
        ...
        case PACKBITSRGN:
            ...
            if (bytestream2_get_bytes_left(&gbc) < (colors + 1) * 8) {
                av_em_log(avctx, AV_LOG_ERROR, "Palette is too small %d\n",
                       bytestream2_get_bytes_left(&gbc));
                return AVERROR_INVALIDDATA;
            }
            ...
            if (opcode == PACKBITSRGN) {
                ...
                avpriv_em_report_missing_feature(avctx, "Packbit mask region");
            }
        ...
        case DIRECTBITSRECT:
        case DIRECTBITSRGN:
            av_em_log(avctx, AV_LOG_DEBUG, "Parsing Directbit opcode\n");

            bytestream2_skip(&gbc, 4);
            rowbytes = bytestream2_get_be16(&gbc) & 0x3FFF;
            if (rowbytes <= 250) {
                avpriv_em_report_missing_feature(avctx, "Short rowbytes");
                return AVERROR_PATCHWELCOME;
            }
            ...
            if (pack_type != 3 && pack_type != 4) {
                avpriv_em_request_sample(avctx, "Pack type %d", pack_type);
                return AVERROR_PATCHWELCOME;
            }
            if ((ret = em_get_buffer(avctx, p, 0)) < 0)
                return ret;
            ...
            if (opcode == DIRECTBITSRGN) {
                bytestream2_skip(&gbc, 2 + 8); /* size + rect */
                avpriv_em_report_missing_feature(avctx, "DirectBit mask region");
            }
            ...
            *got_frame = 1;
            break;
        default:
            av_em_log(avctx, AV_LOG_TRACE, "Unknown 0x%04X opcode\n", opcode);
            break;
        }
        ...
        if (*got_frame) {
            ...
            if (trail)
                av_em_log(avctx, AV_LOG_WARNING, "Got %d trailing bytes\n", trail);
            break;
        }
    }
    ...
    if (*got_frame) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Frame contained no usable data\n");

        return AVERROR_INVALIDDATA;
    }
}

AVEMCodec ff_qdraw_decoder = {
    .name           = "qdraw",
