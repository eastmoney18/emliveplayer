...
static av_cold int dvvideo_encode_init(AVEMCodecContext *avctx)
{
    ...
    s->sys = av_em_dv_codec_profile2(avctx->width, avctx->height, avctx->pix_fmt, avctx->time_base);
    if (!s->sys) {
        av_em_log(avctx, AV_LOG_ERROR, "Found no DV profile for %ix%i %s video. "
                                    "Valid DV profiles are:\n",
               avctx->width, avctx->height, av_em_get_pix_fmt_name(avctx->pix_fmt));
        ff_dv_print_profiles(avctx, AV_LOG_ERROR);
        return AVERROR(EINVAL);
    }
    if (avctx->height > 576) {
        av_em_log(avctx, AV_LOG_ERROR, "DVCPRO HD encoding is not supported.\n");
        return AVERROR_PATCHWELCOME;
    }
    ret = ff_dv_init_dynamic_tables(s, s->sys);
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error initializing work tables.\n");
        return ret;
    }
...
static int dv_encode_video_segment(AVEMCodecContext *avctx, void *arg)
{
    DVVideoContext *s = avctx->priv_data;
    ...
    for (j = 0; j < 5 * s->sys->bpm; j++) {
        ...
        if (enc_blks[j].partial_bit_count)
            av_em_log(avctx, AV_LOG_ERROR, "ac bitstream overflow\n");
    }
                                ...
                                uint8_t *buf)
{
    ...
    uint8_t aspect = 0;
    if ((int) (av_em_q2d(c->avctx->sample_aspect_ratio) *
...
static int dvvideo_encode_frame(AVEMCodecContext *c, AVEMPacket *pkt,
                                const AVFrame *frame, int *got_packet)
{
    DVVideoContext *s = c->priv_data;
    int ret;

    if ((ret = em_alloc_packet2(c, pkt, s->sys->frame_size, 0)) < 0)
        return ret;
