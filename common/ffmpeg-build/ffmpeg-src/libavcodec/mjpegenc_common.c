...
static void jpeg_table_header(AVEMCodecContext *avctx, PutBitContext *p,
...
static void jpeg_put_comments(AVEMCodecContext *avctx, PutBitContext *p)
{
    int size;
    uint8_t *ptr;

    if (avctx->sample_aspect_ratio.num > 0 && avctx->sample_aspect_ratio.den > 0) {
        AVEMRational sar = avctx->sample_aspect_ratio;

        if (sar.num > 65535 || sar.den > 65535) {
            if (!av_em_reduce(&sar.num, &sar.den, avctx->sample_aspect_ratio.num, avctx->sample_aspect_ratio.den, 65535))
...
void ff_mjpeg_init_hvsample(AVEMCodecContext *avctx, int hsample[4], int vsample[4])
{
    int chroma_h_shift, chroma_v_shift;

    av_em_pix_fmt_get_chroma_sub_sample(avctx->pix_fmt, &chroma_h_shift,
                                     &chroma_v_shift);
...
void ff_mjpeg_encode_picture_header(AVEMCodecContext *avctx, PutBitContext *pb,
...
int ff_mjpeg_encode_stuffing(MpegEncContext *s)
{
    ...
    if (ret < 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Buffer reallocation failed\n");
        goto fail;
    }
