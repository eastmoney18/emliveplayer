...
static inline BC_MEDIA_SUBTYPE id2subtype(CHDContext *priv, enum AVEMCodecID id)
{
    switch (id) {
...
{
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tYBuffSz: %u\n", output->YbuffSz);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tYBuffDoneSz: %u\n",
           output->YBuffDoneSz);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tUVBuffDoneSz: %u\n",
           output->UVBuffDoneSz);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tTimestamp: %"PRIu64"\n",
           output->PicInfo.timeStamp);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tPicture Number: %u\n",
           output->PicInfo.picture_number);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tWidth: %u\n",
           output->PicInfo.width);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tHeight: %u\n",
           output->PicInfo.height);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tChroma: 0x%03x\n",
           output->PicInfo.chroma_format);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tPulldown: %u\n",
           output->PicInfo.pulldown);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tFlags: 0x%08x\n",
           output->PicInfo.flags);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tFrame Rate/Res: %u\n",
           output->PicInfo.frame_rate);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tAspect Ratio: %u\n",
           output->PicInfo.aspect_ratio);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tColor Primaries: %u\n",
           output->PicInfo.colour_primaries);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tMetaData: %u\n",
           output->PicInfo.picture_meta_payload);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tSession Number: %u\n",
           output->PicInfo.sess_num);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tycom: %u\n",
           output->PicInfo.ycom);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tCustom Aspect: %u\n",
           output->PicInfo.custom_aspect_ratio_width_height);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tFrames to Drop: %u\n",
           output->PicInfo.n_drop);
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "\tH264 Valid Fields: 0x%08x\n",
...
static uint64_t opaque_list_push(CHDContext *priv, uint64_t reordered_opaque,
                                 uint8_t pic_type)
{
    OpaqueList *newNode = av_em_mallocz(sizeof (OpaqueList));
    if (!newNode) {
        av_em_log(priv->avctx, AV_LOG_ERROR,
...
static OpaqueList *opaque_list_pop(CHDContext *priv, uint64_t fake_timestamp)
{
    ...
    av_em_log(priv->avctx, AV_LOG_VERBOSE,
...
static void flush(AVEMCodecContext *avctx)
{
...
static av_cold int uninit(AVEMCodecContext *avctx)
{
    CHDContext *priv = avctx->priv_data;
    HANDLE device;
    ...
    av_em_parser_close(priv->parser);
    if (priv->bsfc) {
        av_em_bitstream_filter_close(priv->bsfc);
    }

    av_em_freep(&priv->sps_pps_buf);

    av_em_frame_free (&priv->pic);
...
static av_cold int init(AVEMCodecContext *avctx)
{
    CHDContext* priv;
    BC_STATUS ret;
    ...
    av_em_log(avctx, AV_LOG_VERBOSE, "CrystalHD Init for %s\n",
           avctx->codec->name);
    ...
    case BC_MSUBTYPE_AVC1:
        {
            ...
            priv->orig_extradata = av_em_alloc(avctx->extradata_size + AV_INPUT_BUFFER_PADDING_SIZE);
            if (!priv->orig_extradata) {
            ...
            priv->bsfc = av_em_bitstream_filter_init("h264_mp4toannexb");
            if (!priv->bsfc) {
                av_em_log(avctx, AV_LOG_ERROR,
                       "Cannot open the h264_mp4toannexb BSF!\n");
                return AVERROR_BSF_NOT_FOUND;
            }
            av_em_bitstream_filter_filter(priv->bsfc, avctx, NULL, &dummy_p,
                                       &dummy_int, NULL, 0, 0);
        }
    ...
    case BC_MSUBTYPE_DIVX311:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "CrystalHD: Unknown codec name\n");
    ...
    av_em_log(avctx, AV_LOG_VERBOSE, "CrystalHD: starting up\n");
    // Initialize the Link and Decoder devices
    ret = DtsDeviceOpen(&priv->dev, mode);
    if (ret != BC_STS_SUCCESS) {
        av_em_log(avctx, AV_LOG_VERBOSE, "CrystalHD: DtsDeviceOpen failed\n");
        goto fail;
    }
    ...
    if (ret != BC_STS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "CrystalHD: SetInputFormat failed\n");
        goto fail;
    }

    ret = DtsOpenDecoder(priv->dev, BC_STREAM_TYPE_ES);
    if (ret != BC_STS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "CrystalHD: DtsOpenDecoder failed\n");
        goto fail;
    }

    ret = DtsSetColorSpace(priv->dev, OUTPUT_MODE422_YUY2);
    if (ret != BC_STS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "CrystalHD: DtsSetColorSpace failed\n");
        goto fail;
    }
    ret = DtsStartDecoder(priv->dev);
    if (ret != BC_STS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "CrystalHD: DtsStartDecoder failed\n");
        goto fail;
    }
    ret = DtsStartCapture(priv->dev);
    if (ret != BC_STS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "CrystalHD: DtsStartCapture failed\n");
        goto fail;
    }

    if (avctx->codec->id == AV_CODEC_ID_H264) {
        priv->parser = av_em_parser_init(avctx->codec->id);
        if (!priv->parser)
    ...
    av_em_log(avctx, AV_LOG_VERBOSE, "CrystalHD: Init complete.\n");
...
static inline CopyRet copy_frame(AVEMCodecContext *avctx,
                                 ...
                                 void *data, int *got_frame)
{
    ...
    if (output->PicInfo.timeStamp != 0) {
        ...
        av_em_log(avctx, AV_LOG_VERBOSE, "output \"pts\": %"PRIu64"\n",
               output->PicInfo.timeStamp);
        av_em_log(avctx, AV_LOG_VERBOSE, "output picture type %d\n",
               pic_type);
    }
    ...
    av_em_log(avctx, AV_LOG_VERBOSE, "Interlaced state: %d | trust_interlaced %d\n",
    ...
    if (!priv->pic->data[0]) {
        if (em_get_buffer(avctx, priv->pic, AV_GET_BUFFER_FLAG_REF) < 0)
            return RET_ERROR;
    }

    bwidth = av_em_image_get_linesize(avctx->pix_fmt, width, 0);
    if (priv->is_70012) {
        int pStride;
        ...
        sStride = av_em_image_get_linesize(avctx->pix_fmt, pStride, 0);
    } else {
        sStride = bwidth;
    }
    ...
    av_em_log(priv->avctx, AV_LOG_VERBOSE, "CrystalHD: Copying out frame\n");
    ...
    if (interlaced) {
        ...
        height /= 2;
        if (bottom_field) {
            av_em_log(priv->avctx, AV_LOG_VERBOSE, "Interlaced: bottom field\n");
            dY = 1;
        } else {
            av_em_log(priv->avctx, AV_LOG_VERBOSE, "Interlaced: top field\n");
            dY = 0;
        }
        ...
        av_em_image_copy_plane(dst, dStride, src, sStride, bwidth, height);
    }
    ...
    if (!priv->need_second_field) {
        *got_frame       = 1;
        if ((ret = av_em_frame_ref(data, priv->pic)) < 0) {
            return ret;
        }
    }
    ...
    if (!interlaced && (output->PicInfo.flags & VDEC_FLAG_UNKNOWN_SRC) &&
        ...
        av_em_log(priv->avctx, AV_LOG_VERBOSE, "Fieldpair from two packets.\n");
...
static inline CopyRet receive_frame(AVEMCodecContext *avctx,
                                    void *data, int *got_frame)
{
    BC_STATUS ret;
    BC_DTS_PROC_OUT output = {
    ...
    *got_frame = 0;
    ...
    if (ret == BC_STS_FMT_CHANGE) {
        av_em_log(avctx, AV_LOG_VERBOSE, "CrystalHD: Initial format change\n");
        avctx->width  = output.PicInfo.width;
        avctx->height = output.PicInfo.height;
        switch ( output.PicInfo.aspect_ratio ) {
        case vdecAspectRatioSquare:
            avctx->sample_aspect_ratio = (AVEMRational) {  1,  1};
            break;
        case vdecAspectRatio12_11:
            avctx->sample_aspect_ratio = (AVEMRational) { 12, 11};
            break;
        case vdecAspectRatio10_11:
            avctx->sample_aspect_ratio = (AVEMRational) { 10, 11};
            break;
        case vdecAspectRatio16_11:
            avctx->sample_aspect_ratio = (AVEMRational) { 16, 11};
            break;
        case vdecAspectRatio40_33:
            avctx->sample_aspect_ratio = (AVEMRational) { 40, 33};
            break;
        case vdecAspectRatio24_11:
            avctx->sample_aspect_ratio = (AVEMRational) { 24, 11};
            break;
        case vdecAspectRatio20_11:
            avctx->sample_aspect_ratio = (AVEMRational) { 20, 11};
            break;
        case vdecAspectRatio32_11:
            avctx->sample_aspect_ratio = (AVEMRational) { 32, 11};
            break;
        case vdecAspectRatio80_33:
            avctx->sample_aspect_ratio = (AVEMRational) { 80, 33};
            break;
        case vdecAspectRatio18_11:
            avctx->sample_aspect_ratio = (AVEMRational) { 18, 11};
            break;
        case vdecAspectRatio15_11:
            avctx->sample_aspect_ratio = (AVEMRational) { 15, 11};
            break;
        case vdecAspectRatio64_33:
            avctx->sample_aspect_ratio = (AVEMRational) { 64, 33};
            break;
        case vdecAspectRatio160_99:
            avctx->sample_aspect_ratio = (AVEMRational) {160, 99};
            break;
        case vdecAspectRatio4_3:
            avctx->sample_aspect_ratio = (AVEMRational) {  4,  3};
            break;
        case vdecAspectRatio16_9:
            avctx->sample_aspect_ratio = (AVEMRational) { 16,  9};
            break;
        case vdecAspectRatio221_1:
            avctx->sample_aspect_ratio = (AVEMRational) {221,  1};
            break;
        }
        ...
        if (output.PoutFlags & BC_POUT_FLAGS_PIB_VALID) {
            ...
            if (*got_frame > 0) {
                avctx->has_b_frames--;
                priv->last_picture++;
                av_em_log(avctx, AV_LOG_VERBOSE, "CrystalHD: Pipeline length: %u\n",
                       avctx->has_b_frames);
            }
        } else {
            /*
             ...
             */
            av_em_log(avctx, AV_LOG_ERROR, "CrystalHD: ProcOutput succeeded with "
                                        "invalid PIB\n");
            avctx->has_b_frames--;
            copy_ret = RET_OK;
        }
        ...
        av_em_log(avctx, AV_LOG_ERROR, "CrystalHD: ProcOutput failed %d\n", ret);
        return RET_ERROR;
    }
}


static int decode(AVEMCodecContext *avctx, void *data, int *got_frame, AVEMPacket *avpkt)
{
    BC_STATUS ret;
    BC_DTS_STATUS decoder_status = { 0, };
    CopyRet rec_ret;
    ...
    av_em_log(avctx, AV_LOG_VERBOSE, "CrystalHD: decode_frame\n");
    ...
    if (len) {
        ...
        if (priv->parser) {
            ...
            if (priv->bsfc) {
                ret = av_em_bitstream_filter_filter(priv->bsfc, avctx, NULL,
                                                 &in_data, &len,
                                                 avpkt->data, len, 0);
            }
            free_data = ret > 0;
            ...
            if (ret >= 0) {
                ...
                int psize;
                int index;
                H264EMContext *h = priv->parser->priv_data;

                index = av_em_parser_parse2(priv->parser, avctx, &pout, &psize,
        ...
        if (len < tx_free - 1024) {
            ...
            av_em_log(avctx, AV_LOG_WARNING, "CrystalHD: Input buffer full\n");
            len = 0; // We didn't consume any bytes.
        }
    } else {
        av_em_log(avctx, AV_LOG_INFO, "CrystalHD: No more input data\n");
    }

    if (priv->skip_next_output) {
        av_em_log(avctx, AV_LOG_VERBOSE, "CrystalHD: Skipping next output.\n");
        priv->skip_next_output = 0;
        avctx->has_b_frames--;
        return len;
    }
    ...
    if (ret != BC_STS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "CrystalHD: GetDriverStatus failed\n");
        return -1;
    }
    ...
    if (priv->output_ready < 2) {
        if (decoder_status.ReadyListCount != 0)
            priv->output_ready++;
        usleep(BASE_WAIT);
        av_em_log(avctx, AV_LOG_INFO, "CrystalHD: Filling pipeline.\n");
        return len;
        ...
        av_em_log(avctx, AV_LOG_INFO, "CrystalHD: No frames ready. Returning\n");
        return len;
    }
    ...
    do {
        ...
        if (rec_ret == RET_OK && *got_frame == 0) {
             ...
             */
            av_em_log(avctx, AV_LOG_VERBOSE, "Returning after first field.\n");
            avctx->has_b_frames--;
             ...
             */
            av_em_log(avctx, AV_LOG_VERBOSE, "Trying to get second field.\n");
            while (1) {
            ...
            av_em_log(avctx, AV_LOG_VERBOSE, "CrystalHD: Got second field.\n");
...
AVEMCodec em_h264_crystalhd_decoder = {
    .name           = "h264_crystalhd",
...
AVEMCodec ff_mpeg2_crystalhd_decoder = {
...
AVEMCodec em_mpeg4_crystalhd_decoder = {
...
static AVEMClass msmpeg4_class = {
    "msmpeg4_crystalhd",
    av_em_default_item_name,
    options,
    LIBAVUTIL_VERSION_INT,
...
AVEMCodec ff_msmpeg4_crystalhd_decoder = {
...
AVEMCodec ff_vc1_crystalhd_decoder = {
    .name           = "vc1_crystalhd",
...
AVEMCodec ff_wmv3_crystalhd_decoder = {
    .name           = "wmv3_crystalhd",
