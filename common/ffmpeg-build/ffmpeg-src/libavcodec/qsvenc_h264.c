...
static int qsv_h264_set_encode_ctrl(AVEMCodecContext *avctx,
                                    const AVFrame *frame, mfxEncodeCtrl* enc_ctrl)
{
    ...
    if (q->a53_cc && frame) {
        ...
        res = em_alloc_a53_sei(frame, sizeof(mfxPayload) + 2, (void**)&payload, &sei_size);
        if (res < 0)
            return res;
        ...
        // SEI data filled in by em_alloc_a53_sei
