...
static void delete_regions(DVBSubContext *ctx)
{
    while (ctx->region_list) {
        ...
        av_em_freep(&region->pbuf);
        av_em_freep(&region);
    }
}

static av_cold int dvbsub_init_decoder(AVEMCodecContext *avctx)
{
    int i, r, g, b, a = 0;
    ...
    if (ctx->substream < 0) {
        ...
        av_em_log(avctx, AV_LOG_WARNING, "Invalid DVB subtitles stream extradata!\n");
        ctx->composition_id = -1;
        ctx->ancillary_id   = -1;
    } else {
        if (avctx->extradata_size > 5*ctx->substream + 2) {
            ...
            av_em_log(avctx, AV_LOG_WARNING, "Selected DVB subtitles sub-stream %d is not available\n", ctx->substream);
...
static av_cold int dvbsub_close_decoder(AVEMCodecContext *avctx)
{
    DVBSubContext *ctx = avctx->priv_data;
    ...
    av_em_freep(&ctx->display_definition);
...
static int dvbsub_read_2bit_string(AVEMCodecContext *avctx,
                                   ...
                                   int non_mod, uint8_t *map_table, int x_pos)
{
    ...
    if (get_bits(&gb, 6))
        av_em_log(avctx, AV_LOG_ERROR, "line overflow\n");
...
static int dvbsub_read_4bit_string(AVEMCodecContext *avctx, uint8_t *destbuf, int dbuf_len,
                                   const uint8_t **srcbuf, int buf_size,
                                   int non_mod, uint8_t *map_table, int x_pos)
{
    GetBitContext gb;
...
static int dvbsub_read_8bit_string(AVEMCodecContext *avctx,
                                   uint8_t *destbuf, int dbuf_len,
                                    const uint8_t **srcbuf, int buf_size,
                                    int non_mod, uint8_t *map_table, int x_pos)
{
...
static int save_subtitle_set(AVEMCodecContext *avctx, AVSubtitle *sub, int *got_output)
{
    DVBSubContext *ctx = avctx->priv_data;
    DVBSubRegionDisplay *display;
    ...
    if(sub->num_rects) {
        avpriv_em_request_sample(ctx, "Different Version of Segment asked Twice");
    ...
    if(ctx->compute_edt == 0) {
        ...
        *got_output = 1;
        ...
        sub->end_display_time = av_em_rescale_q((sub->pts - ctx->prev_start ), AV_TIME_BASE_Q, (AVEMRational){ 1, 1000 }) - 1;
        *got_output = 1;
    }
    if (sub->num_rects > 0) {

        sub->rects = av_em_mallocz_array(sizeof(*sub->rects), sub->num_rects);
        if (!sub->rects) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        ...
        for(i=0; i<sub->num_rects; i++)
            sub->rects[i] = av_em_mallocz(sizeof(*sub->rects[i]));
        ...
        for (display = ctx->display_list; display; display = display->next) {
            ...
            rect->data[1] = av_em_mallocz(AVPALETTE_SIZE);
            if (!rect->data[1]) {
                ret = AVERROR(ENOMEM);
                goto fail;
            }
            ...
            rect->data[0] = av_em_alloc(region->buf_size);
            if (!rect->data[0]) {
                ret = AVERROR(ENOMEM);
                goto fail;
            }
...
fail:
    if (sub->rects) {
        for(i=0; i<sub->num_rects; i++) {
            rect = sub->rects[i];
            if (rect) {
                av_em_freep(&rect->data[0]);
                av_em_freep(&rect->data[1]);
            }
...
static void dvbsub_parse_pixel_data_block(AVEMCodecContext *avctx, DVBSubObjectDisplay *display,
                                          const uint8_t *buf, int buf_size, int top_bottom, int non_mod)
{
    ...
    while (buf < buf_end) {
        if ((*buf!=0xf0 && x_pos >= region->width) || y_pos >= region->height) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid object location! %d-%d %d-%d %02x\n", x_pos, region->width, y_pos, region->height, *buf);
            return;
        }
        ...
        case 0x11:
            if (region->depth < 4) {
                av_em_log(avctx, AV_LOG_ERROR, "4-bit pixel string in %d-bit region!\n", region->depth);
                return;
            }
        ...
        case 0x12:
            if (region->depth < 8) {
                av_em_log(avctx, AV_LOG_ERROR, "8-bit pixel string in %d-bit region!\n", region->depth);
                return;
            }
        ...
        case 0xf0:
            ...
            break;
        default:
            av_em_log(avctx, AV_LOG_INFO, "Unknown/unsupported pixel block 0x%x\n", *(buf-1));
        }
    }

}

static int dvbsub_parse_object_segment(AVEMCodecContext *avctx,
                                       const uint8_t *buf, int buf_size)
{
    ...
    if (coding_method == 0) {
        ...
        if (buf + top_field_len + bottom_field_len > buf_end) {
            av_em_log(avctx, AV_LOG_ERROR, "Field data size %d+%d too large\n", top_field_len, bottom_field_len);
            return AVERROR_INVALIDDATA;
        }
...
/*  } else if (coding_method == 1) {*/
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Unknown object coding %d\n", coding_method);
    }

    return 0;
}

static int dvbsub_parse_clut_segment(AVEMCodecContext *avctx,
                                     const uint8_t *buf, int buf_size)
{
    DVBSubContext *ctx = avctx->priv_data;
    ...
    if (!clut) {
        clut = av_em_alloc(sizeof(DVBSubCLUT));
        if (!clut)
    ...
    while (buf + 4 < buf_end) {
        ...
        if (depth == 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid clut depth 0x%x!\n", *buf);
        }
...
static int dvbsub_parse_region_segment(AVEMCodecContext *avctx,
                                       const uint8_t *buf, int buf_size)
{
    DVBSubContext *ctx = avctx->priv_data;
    ...
    if (!region) {
        region = av_em_mallocz(sizeof(DVBSubRegion));
        if (!region)
            return AVERROR(ENOMEM);
    ...
    if (region->width * region->height != region->buf_size) {
        av_em_free(region->pbuf);

        region->buf_size = region->width * region->height;

        region->pbuf = av_em_alloc(region->buf_size);
        if (!region->pbuf) {
            region->buf_size =
            region->width =
            region->height = 0;
    ...
    if(region->depth<2 || region->depth>8){
        av_em_log(avctx, AV_LOG_ERROR, "region depth %d is invalid\n", region->depth);
        region->depth= 4;
    }
    region->clut = *buf++;
    ...
    while (buf + 5 < buf_end) {
        ...
        if (!object) {
            object = av_em_mallocz(sizeof(DVBSubObject));
            if (!object)
                return AVERROR(ENOMEM);
        ...
        display = av_em_mallocz(sizeof(DVBSubObjectDisplay));
        if (!display)
            return AVERROR(ENOMEM);
...
static int dvbsub_parse_page_segment(AVEMCodecContext *avctx,
                                     const uint8_t *buf, int buf_size, AVSubtitle *sub, int *got_output)
{
    ...
    while (buf + 5 < buf_end) {
        ...
        if (!display) {
            display = av_em_mallocz(sizeof(DVBSubRegionDisplay));
            if (!display)
                return AVERROR(ENOMEM);
        }
...
static int save_display_set(DVBSubContext *ctx)
{
    ...
    if (x_pos >= 0) {
        ...
        pbuf = av_em_alloc(width * height * 4);
        if (!pbuf)
            return -1;
...
static int dvbsub_parse_display_definition_segment(AVEMCodecContext *avctx,
                                                   const uint8_t *buf,
                                                   int buf_size)
{
    DVBSubContext *ctx = avctx->priv_data;
    ...
    if (!display_def) {
        display_def             = av_em_mallocz(sizeof(*display_def));
        if (!display_def)
            return AVERROR(ENOMEM);
...
static int dvbsub_display_end_segment(AVEMCodecContext *avctx, const uint8_t *buf,
...
static int dvbsub_decode(AVEMCodecContext *avctx,
                         ...
                         AVEMPacket *avpkt)
{
    ...
    while (p_end - p >= 6 && *p == 0x0f) {
        ...
        if (avctx->debug & FF_DEBUG_STARTCODE) {
            av_em_log(avctx, AV_LOG_DEBUG, "segment_type:%d page_id:%d segment_length:%d\n", segment_type, page_id, segment_length);
        }
    ...
    if (got_segment == 15) {
        av_em_log(avctx, AV_LOG_DEBUG, "Missing display_end_segment, emulating\n");
...
static const AVEMClass dvbsubdec_class = {
