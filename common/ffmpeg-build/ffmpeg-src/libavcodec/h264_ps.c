...
static inline int decode_hrd_parameters(GetBitContext *gb, AVEMCodecContext *avctx,
                                        SPS *sps)
{
    int cpb_count, i;
    ...
    if (cpb_count > 32U) {
        av_em_log(avctx, AV_LOG_ERROR, "cpb_count %d invalid\n", cpb_count);
...
static inline int decode_vui_parameters(GetBitContext *gb, AVEMCodecContext *avctx,
                                        SPS *sps)
{
    ...
    if (aspect_ratio_info_present_flag) {
        ...
        } else if (aspect_ratio_idc < FF_ARRAY_ELEMS(em_h264_pixel_aspect)) {
            sps->sar = em_h264_pixel_aspect[aspect_ratio_idc];
        } else {
            av_em_log(avctx, AV_LOG_ERROR, "illegal aspect ratio\n");
            return AVERROR_INVALIDDATA;
        }
    } else {
        sps->sar.num =
    ...
    if (show_bits1(gb) && get_bits_left(gb) < 10) {
        av_em_log(avctx, AV_LOG_WARNING, "Truncated VUI\n");
        return 0;
    }
...
void em_h264_ps_uninit(H264ParamSets *ps)
{
    ...
    for (i = 0; i < MAX_SPS_COUNT; i++)
        av_em_buffer_unref(&ps->sps_list[i]);

    for (i = 0; i < MAX_PPS_COUNT; i++)
        av_em_buffer_unref(&ps->pps_list[i]);

    av_em_buffer_unref(&ps->sps_ref);
    av_em_buffer_unref(&ps->pps_ref);
...
{
    AVEMBufferRef *sps_buf;
    int profile_idc, level_idc, constraint_set_flags = 0;
    unsigned int sps_id;
    int i, log2_max_frame_num_minus4;
    SPS *sps;

    sps_buf = av_em_buffer_allocz(sizeof(*sps));
    if (!sps_buf)
    ...
    if (sps->data_size > sizeof(sps->data)) {
        av_em_log(avctx, AV_LOG_WARNING, "Truncating likely oversized SPS\n");
    ...
    if (sps_id >= MAX_SPS_COUNT) {
        av_em_log(avctx, AV_LOG_ERROR, "sps_id %u out of range\n", sps_id);
        goto fail;
    }
    ...
    if (sps->profile_idc == 100 ||  // High profile
        ...
        if (sps->chroma_format_idc > 3U) {
            avpriv_em_request_sample(avctx, "chroma_format_idc %u",
                                  sps->chroma_format_idc);
            goto fail;
        } else if (sps->chroma_format_idc == 3) {
            sps->residual_color_transform_flag = get_bits1(gb);
            if (sps->residual_color_transform_flag) {
                av_em_log(avctx, AV_LOG_ERROR, "separate color planes are not supported\n");
                goto fail;
            }
        }
        ...
        if (sps->bit_depth_luma   < 8 || sps->bit_depth_luma   > 14 ||
            ...
            av_em_log(avctx, AV_LOG_ERROR, "illegal bit depth value (%d, %d)\n",
    ...
    if (sps->poc_type == 0) { // FIXME #define
        ...
        if (t>12) {
            av_em_log(avctx, AV_LOG_ERROR, "log2_max_poc_lsb (%d) is out of range\n", t);
            goto fail;
        }
        ...
        av_em_log(avctx, AV_LOG_ERROR, "illegal POC type %d\n", sps->poc_type);
        goto fail;
    }
    ...
    if ((unsigned)sps->mb_width  >= INT_MAX / 16 ||
        ...
        av_em_image_check_size(16 * sps->mb_width,
                            16 * sps->mb_height, 0, avctx)) {
        av_em_log(avctx, AV_LOG_ERROR, "mb_width/height overflow\n");
        goto fail;
    }
    ...
    if (sps->crop) {
        ...
        if (avctx->flags2 & AV_CODEC_FLAG2_IGNORE_CROP) {
            av_em_log(avctx, AV_LOG_DEBUG, "discarding sps cropping, original "
            ...
            if (crop_left  > (unsigned)INT_MAX / 4 / step_x ||
                ...
                av_em_log(avctx, AV_LOG_ERROR, "crop values invalid %d %d %d %d / %d %d\n", crop_left, crop_right, crop_top, crop_bottom, width, height);
                goto fail;
            }
    ...
    if (get_bits_left(gb) < 0) {
        av_em_log(avctx, ignore_truncation ? AV_LOG_WARNING : AV_LOG_ERROR,
...
static void init_dequant8_coeff_table(PPS *pps, const SPS *sps)
{
    ...
    for (i = 0; i < 6; i++) {
        ...
        for (q = 0; q < max_qp + 1; q++) {
            int shift = em_h264_quant_div6[q];
            int idx   = em_h264_quant_rem6[q];
            for (x = 0; x < 64; x++)
                pps->dequant8_coeff[i][q][(x >> 3) | ((x & 7) << 3)] =
                    ((uint32_t)em_h264_dequant8_coeff_init[idx][em_h264_dequant8_coeff_init_scan[((x >> 1) & 12) | (x & 3)]] *
        ...
        for (q = 0; q < max_qp + 1; q++) {
            ...
            for (x = 0; x < 16; x++)
                pps->dequant4_coeff[i][q][(x >> 2) | ((x << 2) & 0xF)] =
                    ((uint32_t)em_h264_dequant4_coeff_init[idx][(x & 1) + ((x >> 2) & 1)] *
...
static void build_qp_table(PPS *pps, int t, int index, const int depth)
{
    ...
    for (i = 0; i < max_qp + 1; i++)
        pps->chroma_qp_table[t][i] =
            em_h264_chroma_qp[depth - 8][av_clip(i + index, 0, max_qp)];
}
...
static int more_rbsp_data_in_pps(const SPS *sps, void *logctx)
{
    ...
    if ((profile_idc == 66 || profile_idc == 77 ||
        ...
        av_em_log(logctx, AV_LOG_VERBOSE,
                                         ...
                                         H264ParamSets *ps, int bit_length)
{
    ...
    if (pps_id >= MAX_PPS_COUNT) {
        av_em_log(avctx, AV_LOG_ERROR, "pps_id %u out of range\n", pps_id);
        return AVERROR_INVALIDDATA;
    }

    pps_buf = av_em_buffer_allocz(sizeof(*pps));
    if (!pps_buf)
        return AVERROR(ENOMEM);
    ...
    if (pps->data_size > sizeof(pps->data)) {
        av_em_log(avctx, AV_LOG_WARNING, "Truncating likely oversized PPS\n");
    ...
    if (pps->slice_group_count > 1) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "FMO not supported\n");
    ...
    if (pps->ref_count[0] - 1 > 32 - 1 || pps->ref_count[1] - 1 > 32 - 1) {
        av_em_log(avctx, AV_LOG_ERROR, "reference overflow (pps)\n");
        ret = AVERROR_INVALIDDATA;
        goto fail;
    }
