...
{
    if (avctx->width & 1) {
        av_em_log(avctx, AV_LOG_ERROR, "frwu needs even width\n");
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (bytestream_get_le32(&buf) != MKTAG('F', 'R', 'W', '1')) {
        av_em_log(avctx, AV_LOG_ERROR, "incorrect marker\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    for (field = 0; field < 2; field++) {
        ...
        if (field_size < min_field_size) {
            av_em_log(avctx, AV_LOG_ERROR, "Field size %i is too small (required %i)\n", field_size, min_field_size);
            return AVERROR_INVALIDDATA;
        }
        if (buf_end - buf < field_size) {
            av_em_log(avctx, AV_LOG_ERROR, "Packet is too small, need %i, have %i\n", field_size, (int)(buf_end - buf));
            return AVERROR_INVALIDDATA;
        }
...
static const AVEMClass frwu_class = {
