...
static av_cold int libshine_encode_init(AVEMCodecContext *avctx)
{
    SHINEContext *s = avctx->priv_data;

    if (avctx->channels <= 0 || avctx->channels > 2){
        av_em_log(avctx, AV_LOG_ERROR, "only mono or stereo is supported\n");
    ...
    if (shine_check_config(s->config.wave.samplerate, s->config.mpeg.bitr) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid configuration\n");
        return AVERROR(EINVAL);
    }
...
static int libshine_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                                 const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if (written > 0) {
        if (s->buffer_index + written > BUFFER_SIZE) {
            av_em_log(avctx, AV_LOG_ERROR, "internal buffer too small\n");
            return AVERROR_BUG;
        }
    ...
    if (len <= s->buffer_index) {
        if ((ret = em_alloc_packet2(avctx, avpkt, len, 0)))
            return ret;
...
static av_cold int libshine_encode_close(AVEMCodecContext *avctx)
{
    SHINEContext *s = avctx->priv_data;
