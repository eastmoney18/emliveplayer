...
static inline int wnv1_get_code(WNV1Context *w, int base_value)
{
    ...
    if (v == 15)
        return em_reverse[get_bits(&w->gb, 8 - w->shift)];
    else
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (buf_size <= 8) {
        av_em_log(avctx, AV_LOG_ERROR, "Packet size %d is too small\n", buf_size);
        return AVERROR_INVALIDDATA;
    }

    rbuf = av_em_alloc(buf_size + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!rbuf) {
