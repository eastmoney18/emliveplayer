...
void em_h264_sei_uninit(H264SEIContext *h)
{
    ...
    av_em_freep(&h->a53_caption.a53_caption);
}
                                 ...
                                 const H264ParamSets *ps, void *logctx)
{
    ...
    if (!sps) {
        av_em_log(logctx, AV_LOG_ERROR, "SPS unavailable in decode_picture_timing\n");
    ...
    if (sps->pic_struct_present_flag) {
        ...
        av_em_log(logctx, AV_LOG_DEBUG, "ct_type:%X pic_struct:%d\n",
    ...
    if (user_data_type_code == 0x3) {
        ...
        if (flag) {
            ...
            if (cc_count && size >= cc_count * 3) {
                ...
                ret = av_em_reallocp(&h->a53_caption, new_size);
                if (ret < 0)
                    return ret;
                                         ...
                                         void *logctx, int size)
{
    ...
    user_data = av_em_alloc(16 + size + 1);
    if (!user_data)
    ...
    if (strlen(user_data + 16) > 0)
        av_em_log(logctx, AV_LOG_DEBUG, "user data:\"%s\"\n", user_data + 16);

    av_em_free(user_data);
    return 0;
}
...
int em_h264_sei_decode(H264SEIContext *h, GetBitContext *gb,
                       const H264ParamSets *ps, void *logctx)
{
    int master_ret = 0;
    ...
    while (get_bits_left(gb) > 16 && show_bits(gb, 16)) {
        ...
        if (size > get_bits_left(gb) / 8) {
            av_em_log(logctx, AV_LOG_ERROR, "SEI type %d size %d truncated at %d\n",
        ...
        case SEI_TYPE_GREEN_METADATA:
            ...
            break;
        default:
            av_em_log(logctx, AV_LOG_DEBUG, "unknown SEI type %d\n", type);
        }
...
const char *em_h264_sei_stereo_mode(const H264SEIFramePacking *h)
{
