...
{
    av_em_log(data, AV_LOG_ERROR, "%s\n", msg);
}

static void warning_callback(const char *msg, void *data)
{
    av_em_log(data, AV_LOG_WARNING, "%s\n", msg);
}

static void info_callback(const char *msg, void *data)
{
    av_em_log(data, AV_LOG_DEBUG, "%s\n", msg);
}
...
static OPJ_SIZE_T stream_write(void *out_buffer, OPJ_SIZE_T nb_bytes, void *user_data)
{
    ...
    AVEMPacket *packet = writer->packet;
    ...
    if (nb_bytes > remaining) {
        ...
        if (av_em_grow_packet(packet, (int)needed)) {
            return (OPJ_SIZE_T)-1;
        }
    }
...
static OPJ_BOOL stream_seek(OPJ_OFF_T nb_bytes, void *user_data)
{
    ...
    if (nb_bytes > packet->size) {
        if (nb_bytes > INT_MAX - AV_INPUT_BUFFER_PADDING_SIZE ||
            av_em_grow_packet(packet, (int)nb_bytes - packet->size)) {
            return OPJ_FALSE;
        }
    }
    writer->pos = (int)nb_bytes;
...
static opj_image_t *mj2_create_image(AVEMCodecContext *avctx, opj_cparameters_t *parameters)
{
...
static av_cold int libopenjpeg_encode_init(AVEMCodecContext *avctx)
{
    ...
    if (!ctx->image) {
        av_em_log(avctx, AV_LOG_ERROR, "Error creating the mj2 image\n");
        err = AVERROR(EINVAL);
        goto fail;
    }
...
static int libopenjpeg_copy_packed8(AVEMCodecContext *avctx, const AVFrame *frame, opj_image_t *image)
{
    int compno;
    int x;
    int y;
    ...
    for (compno = 0; compno < numcomps; ++compno) {
        if (image->comps[compno].w > frame->linesize[0] / numcomps) {
            av_em_log(avctx, AV_LOG_ERROR, "Error: frame's linesize is too small for the image\n");
            return 0;
        }
    }
...
// for XYZ 12 bit
static int libopenjpeg_copy_packed12(AVEMCodecContext *avctx, const AVFrame *frame, opj_image_t *image)
{
    int compno;
    int x, y;
    int *image_line;
...
static int libopenjpeg_copy_packed16(AVEMCodecContext *avctx, const AVFrame *frame, opj_image_t *image)
{
    int compno;
    int x;
    int y;
...
static int libopenjpeg_copy_unpacked8(AVEMCodecContext *avctx, const AVFrame *frame, opj_image_t *image)
{
    int compno;
    int x;
    int y;
...
static int libopenjpeg_copy_unpacked16(AVEMCodecContext *avctx, const AVFrame *frame, opj_image_t *image)
{
    int compno;
    int x;
    int y;
...
static int libopenjpeg_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                                    const AVFrame *frame, int *got_packet)
{
    ...
    case AV_PIX_FMT_GBRP14:
    case AV_PIX_FMT_GBRP16:
        gbrframe = av_em_frame_clone(frame);
        if (!gbrframe)
...
#if OPENJPEG_MAJOR_VERSION == 2
    if ((ret = em_alloc_packet2(avctx, pkt, 1024, 0)) < 0) {
        return ret;
    }
#endif // OPENJPEG_MAJOR_VERSION == 2
    ...
    if (!compress) {
        av_em_log(avctx, AV_LOG_ERROR, "Error creating the compressor\n");
        ret = AVERROR(ENOMEM);
        goto done;
    }
    ...
    if (!opj_set_error_handler(compress, error_callback, avctx) ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Error setting the compressor handlers\n");
        ret = AVERROR_EXTERNAL;
        goto done;
    }

    if (!opj_setup_encoder(compress, &ctx->enc_params, image)) {
        av_em_log(avctx, AV_LOG_ERROR, "Error setting up the compressor\n");
        ret = AVERROR_EXTERNAL;
        goto done;
    }
    ...
    if (!stream) {
        av_em_log(avctx, AV_LOG_ERROR, "Error creating the cio stream\n");
        ret = AVERROR(ENOMEM);
        goto done;
    }
#if OPENJPEG_MAJOR_VERSION == 1
    ...
    if (!opj_encode(compress, stream, image, NULL)) {
        av_em_log(avctx, AV_LOG_ERROR, "Error during the opj encode\n");
        ret = AVERROR_EXTERNAL;
        goto done;
    }

    len = cio_tell(stream);
    if ((ret = em_alloc_packet2(avctx, pkt, len, 0)) < 0) {
        goto done;
    }
...
#error Missing call to opj_stream_set_user_data
    ...
    av_em_shrink_packet(pkt, writer.pos);
#endif // OPENJPEG_MAJOR_VERSION == 1
...
static av_cold int libopenjpeg_encode_close(AVEMCodecContext *avctx)
{
    LibOpenJPEGContext *ctx = avctx->priv_data;
...
static const AVEMClass openjpeg_class = {
...
AVEMCodec ff_libopenjpeg_encoder = {
    .name           = "libopenjpeg",
