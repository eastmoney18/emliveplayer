                ...
                if (b < 128)
                    ...
                    while (b-- > 130) {
                        if (repeat >= (INT_MAX >> 8) - 1) {
                            av_em_log(NULL, AV_LOG_ERROR, "repeat overflow\n");
                            return AVERROR_INVALIDDATA;
                        }
...
static int decode_wmv9(AVEMCodecContext *avctx, const uint8_t *buf, int buf_size,
                       int x, int y, int w, int h, int wmv9_mask)
{
    ...
    if (ff_vc1_parse_frame_header(v, &s->gb) < 0) {
        av_em_log(v->s.avctx, AV_LOG_ERROR, "header error\n");
        return AVERROR_INVALIDDATA;
    }

    if (s->pict_type != AV_PICTURE_TYPE_I) {
        av_em_log(v->s.avctx, AV_LOG_ERROR, "expected I-frame\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if ((ret = ff_mpv_frame_start(s, avctx)) < 0) {
        av_em_log(v->s.avctx, AV_LOG_ERROR, "ff_mpv_frame_start error\n");
    ...
    if (v->respic == 3) {
        ...
        avpriv_em_request_sample(v->s.avctx,
...
static int mss2_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                             AVEMPacket *avpkt)
{
    ...
    if (ctx->last_pic->format != avctx->pix_fmt)
        av_em_frame_unref(ctx->last_pic);
    ...
    if (has_wmv9) {
        ...
        if (implicit_rect && used_rects) {
            av_em_log(avctx, AV_LOG_ERROR, "implicit_rect && used_rects > 0\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        for (i = 0; i < used_rects; i++) {
            if (!implicit_rect && arith2_get_bit(&acoder)) {
                av_em_log(avctx, AV_LOG_ERROR, "Unexpected grandchildren\n");
                return AVERROR_INVALIDDATA;
            }
            if (!i) {
    ...
    if (c->mvX < 0 || c->mvY < 0) {
        ...
        if (ctx->last_pic->data[0]) {
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Missing keyframe\n");
            return AVERROR_INVALIDDATA;
        }
    } else {
        if ((ret = em_reget_buffer(avctx, ctx->last_pic)) < 0)
            return ret;
        if ((ret = av_em_frame_ref(frame, ctx->last_pic)) < 0)
            return ret;
    ...
    if (buf_size)
        av_em_log(avctx, AV_LOG_WARNING, "buffer not fully consumed\n");

    if (c->mvX < 0 || c->mvY < 0) {
        av_em_frame_unref(ctx->last_pic);
        ret = av_em_frame_ref(ctx->last_pic, frame);
        if (ret < 0)
            return ret;
    }
...
static av_cold int wmv9_init(AVEMCodecContext *avctx)
{
    VC1Context *v = avctx->priv_data;
    int ret;
...
static av_cold int mss2_decode_end(AVEMCodecContext *avctx)
{
    MSS2Context *const ctx = avctx->priv_data;

    av_em_frame_free(&ctx->last_pic);

    ff_mss12_decode_end(&ctx->c);
    av_em_freep(&ctx->c.pal_pic);
    av_em_freep(&ctx->c.last_pal_pic);
    ff_vc1_decode_end(avctx);

    return 0;
}

static av_cold int mss2_decode_init(AVEMCodecContext *avctx)
{
    MSS2Context * const ctx = avctx->priv_data;
    MSS12Context *c = &ctx->c;
    int ret;
    ...
    if (ret = ff_mss12_decode_init(c, 1, &ctx->sc[0], &ctx->sc[1]))
        return ret;
    ctx->last_pic   = av_em_frame_alloc();
    c->pal_stride   = c->mask_stride;
    c->pal_pic      = av_em_mallocz(c->pal_stride * avctx->height);
    c->last_pal_pic = av_em_mallocz(c->pal_stride * avctx->height);
