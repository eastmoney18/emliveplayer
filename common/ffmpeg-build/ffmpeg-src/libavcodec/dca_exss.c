...
static int parse_descriptor(DCAExssParser *s, DCAExssAsset *asset)
{
    ...
    if (s->static_fields_present) {
        ...
        if (asset->one_to_one_map_ch_to_spkr = get_bits1(&s->gb)) {
            ...
            if ((spkr_remap_nsets = get_bits(&s->gb, 3)) && !spkr_mask_nbits) {
                if (s->avctx)
                    av_em_log(s->avctx, AV_LOG_ERROR, "Speaker mask disabled yet there are remapping sets\n");
                return AVERROR_INVALIDDATA;
            }
    ...
    if (s->mix_metadata_enabled && get_bits1(&s->gb)) {
        ...
        for (i = 0; i < s->nmixoutconfigs; i++) {
            if (!s->nmixoutchs[i]) {
                if (s->avctx)
                    av_em_log(s->avctx, AV_LOG_ERROR, "Invalid speaker layout mask for mixing configuration\n");
                return AVERROR_INVALIDDATA;
            }
    ...
    if (ff_dca_seek_bits(&s->gb, descr_pos + descr_size * 8)) {
        if (s->avctx)
            av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of EXSS asset descriptor\n");
        return AVERROR_INVALIDDATA;
    }
...
int ff_dca_exss_parse(DCAExssParser *s, const uint8_t *data, int size)
{
    ...
    if (s->avctx && ff_dca_check_crc(s->avctx, &s->gb, 32 + 8, header_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid EXSS header checksum\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->exss_size > size) {
        if (s->avctx)
            av_em_log(s->avctx, AV_LOG_ERROR, "Packet too short for EXSS frame\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->static_fields_present = get_bits1(&s->gb)) {
        ...
        if (s->npresents > 1) {
            if (s->avctx)
                avpriv_em_request_sample(s->avctx, "%d audio presentations", s->npresents);
        ...
        if (s->nassets > 1) {
            if (s->avctx)
                avpriv_em_request_sample(s->avctx, "%d audio assets", s->nassets);
            return AVERROR_PATCHWELCOME;
        }
    ...
    for (i = 0; i < s->nassets; i++) {
        ...
        if (offset > s->exss_size) {
            if (s->avctx)
                av_em_log(s->avctx, AV_LOG_ERROR, "EXSS asset out of bounds\n");
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    for (i = 0; i < s->nassets; i++) {
        ...
        if ((ret = set_exss_offsets(&s->assets[i])) < 0) {
            if (s->avctx)
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid extension size in EXSS asset descriptor\n");
            return ret;
        }
    }
    ...
    if (ff_dca_seek_bits(&s->gb, header_size * 8)) {
        if (s->avctx)
            av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of EXSS header\n");
        return AVERROR_INVALIDDATA;
    }
