...
static void copy_frame(AVFrame *f, const uint8_t *src, int width, int height)
{
    uint8_t *src_data[4];
    int src_linesize[4];
    av_em_image_fill_arrays(src_data, src_linesize, src,
                         f->format, width, height, 1);
    av_em_image_copy(f->data, f->linesize, (const uint8_t **)src_data, src_linesize,
 ...
 */
static int get_quant(AVEMCodecContext *avctx, NuvContext *c, const uint8_t *buf,
                     int size)
{
    int i;
    if (size < 2 * 64 * 4) {
        av_em_log(avctx, AV_LOG_ERROR, "insufficient rtjpeg quant data\n");
...
static int codec_reinit(AVEMCodecContext *avctx, int width, int height,
                        int quality)
{
    ...
    if (width != c->width || height != c->height) {
        ...
        if (buf_size > INT_MAX/8)
            return -1;
        if ((ret = av_em_image_check_size(height, width, 0, avctx)) < 0)
            return ret;
        avctx->width  = c->width  = width;
        avctx->height = c->height = height;
        av_em_fast_malloc(&c->decomp_buf, &c->decomp_size,
                       buf_size);
        if (!c->decomp_buf) {
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (buf_size < 12) {
        av_em_log(avctx, AV_LOG_ERROR, "coded frame too small\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (buf_size < 12 || buf[0] != 'V') {
        av_em_log(avctx, AV_LOG_ERROR, "not a nuv video frame\n");
        return AVERROR_INVALIDDATA;
    }
    comptype = buf[1];
    switch (comptype) {
...
retry:
    ...
    if (comptype == NUV_RTJPEG_IN_LZO || comptype == NUV_LZO) {
        ...
        int inlen  = buf_size;
        if (av_em_lzo1x_decode(c->decomp_buf, &outlen, buf, &inlen)) {
            av_em_log(avctx, AV_LOG_ERROR, "error during lzo decompression\n");
            return AVERROR_INVALIDDATA;
        }
        buf      = c->decomp_buf;
    ...
    if (c->codec_frameheader) {
        ...
        if (buf_size < RTJPEG_HEADER_SIZE) {
            av_em_log(avctx, AV_LOG_ERROR, "Too small NUV video frame\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        if (buf[0] != 'V' && AV_RL16(&buf[4]) != 0x000c) {
            av_em_log(avctx, AV_LOG_ERROR, "Unknown secondary frame header (wrong codec_tag?)\n");
            return AVERROR_INVALIDDATA;
        }
        w = AV_RL16(&buf[6]);
        h = AV_RL16(&buf[8]);
    ...
    if ((result = em_reget_buffer(avctx, c->pic)) < 0)
        return result;
    if (init_frame) {
    ...
    case NUV_UNCOMPRESSED: {
        ...
        if (buf_size < c->width * height * 3 / 2) {
            av_em_log(avctx, AV_LOG_ERROR, "uncompressed frame too short\n");
    ...
    case NUV_COPY_LAST:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "unknown compression\n");
        return AVERROR_INVALIDDATA;
    }

    if ((result = av_em_frame_ref(picture, c->pic)) < 0)
        return result;
...
static av_cold int decode_end(AVEMCodecContext *avctx)
{
    ...
    av_em_frame_free(&c->pic);

    return 0;
}

AVEMCodec ff_nuv_decoder = {
    .name           = "nuv",
