...
static av_cold int mvc_decode_init(AVEMCodecContext *avctx)
{
...
static int decode_mvc1(AVEMCodecContext *avctx, GetByteContext *gb,
        ...
        for (x = 0; x < width; x += 4) {
            ...
            if ((v[0] & 0x8000)) {
                if (bytestream2_get_bytes_left(gb) < 12) {
                    av_em_log(avctx, AV_LOG_WARNING, "buffer overflow\n");
...
static int decode_mvc2(AVEMCodecContext *avctx, GetByteContext *gb,
                       ...
                       int linesize, int vflip)
{
    ...
    if ((w & ~3) != width || (h & ~3) != height)
        av_em_log(avctx, AV_LOG_WARNING, "dimension mismatch\n");

    if (bytestream2_get_byteu(gb)) {
        avpriv_em_request_sample(avctx, "bitmap feature");
...
static int mvc_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                            AVEMPacket *avpkt)
{
    MvcContext *s = avctx->priv_data;
    AVFrame *frame = data;
