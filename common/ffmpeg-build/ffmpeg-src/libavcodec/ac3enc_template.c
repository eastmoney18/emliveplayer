...
int AC3_NAME(encode_frame)(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                           const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if (ret) {
        av_em_log(avctx, AV_LOG_ERROR, "Bit allocation failed. Try increasing the bitrate.\n");
        return ret;
    }
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, s->frame_size, 0)) < 0)
        return ret;
