...
static int mismatches_ref(const H264EMContext *h, const H264Picture *pic)
{
...
static void h264_initialise_ref_list(H264EMContext *h, H264SliceContext *sl)
{
    int i, len;
    int j;
        ...
        for (i = 0; i < sl->ref_count[j]; i++) {
            if (sl->ref_list[j][i].parent) {
                if (mismatches_ref(h, sl->ref_list[j][i].parent)) {
                    av_em_log(h->avctx, AV_LOG_ERROR, "Discarding mismatching reference\n");
 ...
 */
static void print_short_term(const H264EMContext *h)
{
    uint32_t i;
    if (h->avctx->debug & FF_DEBUG_MMCO) {
        av_em_log(h->avctx, AV_LOG_DEBUG, "short term list:\n");
        for (i = 0; i < h->short_ref_count; i++) {
            H264Picture *pic = h->short_ref[i];
            av_em_log(h->avctx, AV_LOG_DEBUG, "%"PRIu32" fn:%d poc:%d %p\n",
 ...
 */
static void print_long_term(const H264EMContext *h)
{
    uint32_t i;
    if (h->avctx->debug & FF_DEBUG_MMCO) {
        av_em_log(h->avctx, AV_LOG_DEBUG, "long term list:\n");
        for (i = 0; i < 16; i++) {
 ...
 */
static int pic_num_extract(const H264EMContext *h, int pic_num, int *structure)
{
        ...
        if (get_bits1(&sl->gb)) {    // ref_pic_list_modification_flag_l[01]
            ...
            for (index = 0; ; index++) {
                ...
                if (index >= sl->ref_count[list]) {
                    av_em_log(h->avctx, AV_LOG_ERROR, "reference count overflow\n");
                    return -1;
                }
    ...
    for (list = 0; list < sl->list_count; list++) {
        for (index = 0; index < sl->ref_count[list]; index++) {
            if (   !sl->ref_list[list][index].parent
                ...
                int i;
                av_em_log(h->avctx, AV_LOG_ERROR, "Missing reference picture, default is %d\n", h->default_ref[list].poc);
            ...
            av_assert0(av_em_buffer_get_ref_count(sl->ref_list[list][index].parent->f->buf[0]) > 0);
        }
    }
 ...
 *         reference
 */
static inline int unreference_pic(H264EMContext *h, H264Picture *pic, int refmask)
{
    int i;
 ...
 */
static H264Picture *find_short(H264EMContext *h, int frame_num, int *idx)
{
    int i;
    ...
    for (i = 0; i < h->short_ref_count; i++) {
        H264Picture *pic = h->short_ref[i];
        if (h->avctx->debug & FF_DEBUG_MMCO)
            av_em_log(h->avctx, AV_LOG_DEBUG, "%d %d %p\n", i, pic->frame_num, pic);
 ...
 */
static void remove_short_at_index(H264EMContext *h, int i)
{
 ...
 */
static H264Picture *remove_short(H264EMContext *h, int frame_num, int ref_mask)
{
    H264Picture *pic;
    int i;

    if (h->avctx->debug & FF_DEBUG_MMCO)
        av_em_log(h->avctx, AV_LOG_DEBUG, "remove short %d count %d\n", frame_num, h->short_ref_count);
 ...
 * @return the removed picture or NULL if an error occurs
 */
static H264Picture *remove_long(H264EMContext *h, int i, int ref_mask)
{
    H264Picture *pic;
...
void em_h264_remove_all_refs(H264EMContext *h)
{
    ...
    if (h->short_ref_count && !h->last_pic_for_ec.f->data[0]) {
        ...
        if (h->short_ref[0]->f->buf[0])
            em_h264_ref_picture(h, &h->last_pic_for_ec, h->short_ref[0]);
    }
...
static int check_opcodes(MMCO *mmco1, MMCO *mmco2, int n_mmcos)
{
    ...
    for (i = 0; i < n_mmcos; i++) {
        if (mmco1[i].opcode != mmco2[i].opcode) {
            av_em_log(NULL, AV_LOG_ERROR, "MMCO opcode [%d, %d] at %d mismatches between slices\n",
...
int em_h264_execute_ref_pic_marking(H264EMContext *h, MMCO *mmco, int mmco_count)
{
    ...
    if ((h->avctx->debug & FF_DEBUG_MMCO) && mmco_count == 0)
        av_em_log(h->avctx, AV_LOG_DEBUG, "no mmco here\n");

    for (i = 0; i < mmco_count; i++) {
        int av_uninit(structure), av_uninit(frame_num);
        if (h->avctx->debug & FF_DEBUG_MMCO)
            av_em_log(h->avctx, AV_LOG_DEBUG, "mmco:%d %d %d\n", h->mmco[i].opcode,
        ...
        if (mmco[i].opcode == MMCO_SHORT2UNUSED ||
            ...
            if (!pic) {
                if (mmco[i].opcode != MMCO_SHORT2LONG ||
                    ...
                    av_em_log(h->avctx, h->short_ref_count ? AV_LOG_ERROR : AV_LOG_DEBUG, "mmco: unref short failure\n");
        ...
        case MMCO_SHORT2UNUSED:
            if (h->avctx->debug & FF_DEBUG_MMCO)
                av_em_log(h->avctx, AV_LOG_DEBUG, "mmco: unref short %d count %d\n",
        ...
        case MMCO_LONG2UNUSED:
            ...
            if (pic) {
                ...
                av_em_log(h->avctx, AV_LOG_DEBUG, "mmco: unref long failure\n");
            break;
        case MMCO_LONG:
            ...
            if (h->short_ref[0] == h->cur_pic_ptr) {
                av_em_log(h->avctx, AV_LOG_ERROR, "mmco: cannot assign current picture to short and long at the same time\n");
                ...
                for (j = 0; j < FF_ARRAY_ELEMS(h->long_ref); j++) {
                    if (h->long_ref[j] == h->cur_pic_ptr) {
                        if (j != mmco[i].long_arg)
                            av_em_log(h->avctx, AV_LOG_ERROR, "mmco: cannot assign current picture to 2 long term references\n");
                        remove_long(h, j, 0);
                    }
                }
            }
    ...
    if (!current_ref_assigned) {
        ...
        if (h->short_ref_count && h->short_ref[0] == h->cur_pic_ptr) {
            ...
            av_em_log(h->avctx, AV_LOG_ERROR, "illegal short term reference "
            ...
            if (pic) {
                av_em_log(h->avctx, AV_LOG_ERROR, "illegal short term buffer state detected\n");
                err = AVERROR_INVALIDDATA;
            }
                                   ...
                                   int first_slice)
{
    ...
    if (h->nal_unit_type == NAL_IDR_SLICE) { // FIXME fields
        ...
        if (get_bits1(gb)) { // adaptive_ref_pic_marking_mode_flag
            ...
            if (first_slice) {
                ret = em_generate_sliding_window_mmcos(h, first_slice);
