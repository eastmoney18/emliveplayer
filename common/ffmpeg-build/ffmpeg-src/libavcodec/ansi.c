...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    /* defaults */
    s->font        = avpriv_em_vga16_font;
    s->font_height = 16;
    s->fg          = DEFAULT_FG_COLOR;
    s->bg          = DEFAULT_BG_COLOR;

    if (!avctx->width || !avctx->height) {
        int ret = em_set_dimensions(avctx, 80 << 3, 25 << 4);
        if (ret < 0)
            return ret;
    }
    return 0;
...
static void hscroll(AVEMCodecContext *avctx)
{
    AnsiContext *s = avctx->priv_data;
    int i;
...
static void erase_line(AVEMCodecContext * avctx, int xoffset, int xlength)
{
    AnsiContext *s = avctx->priv_data;
    int i;
...
static void erase_screen(AVEMCodecContext *avctx)
{
    AnsiContext *s = avctx->priv_data;
    int i;
 ...
 */
static void draw_char(AVEMCodecContext *avctx, int c)
{
    AnsiContext *s = avctx->priv_data;
    int fg = s->fg;
    int bg = s->bg;
 ...
 */
static int execute_code(AVEMCodecContext * avctx, int c)
{
    AnsiContext *s = avctx->priv_data;
    int ret, i;
    ...
    case 'l': //reset screen mode
        ...
        case 17: case 18: //640x480 (60 rows)
            ...
            break;
        default:
            avpriv_em_request_sample(avctx, "Unsupported screen mode");
        }
        ...
        if (width != avctx->width || height != avctx->height) {
            ...
            if (ret < 0)
                return ret;
            if ((ret = em_get_buffer(avctx, s->frame,
    ...
    case 'm': //Select Graphics Rendition
        ...
        for (i = 0; i < FFMIN(s->nb_args, MAX_NB_ARGS); i++) {
            ...
            if (m == 0) {
                ...
                avpriv_em_request_sample(avctx, "Unsupported rendition parameter");
            }
        }
        break;
    ...
    case 'u': //Restore Cursor Position
        ...
        break;
    default:
        avpriv_em_request_sample(avctx, "Unknown escape code");
        break;
    }
                            ...
                            AVEMPacket *avpkt)
{
    ...
    while(buf < buf_end) {
        ...
        case STATE_CODE:
            ...
            default:
                if (s->nb_args > MAX_NB_ARGS)
                    av_em_log(avctx, AV_LOG_WARNING, "args overflow (%i)\n", s->nb_args);
