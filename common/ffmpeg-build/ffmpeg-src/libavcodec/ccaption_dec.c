...
static const AVEMRational ms_tb = {1, 1000};
...
static av_cold int init_decoder(AVEMCodecContext *avctx)
{
    ...
    av_em_bprint_init(&ctx->buffer, 0, AV_BPRINT_SIZE_UNLIMITED);
    ...
    /* allocate pkt buffer */
    ctx->pktbuf = av_em_buffer_alloc(128);
    if (!ctx->pktbuf) {
        ret = AVERROR(ENOMEM);
    }
    return ret;
...
{
    CCaptionSubContext *ctx = avctx->priv_data;
    av_em_bprint_finalize(&ctx->buffer, NULL);
    av_em_buffer_unref(&ctx->pktbuf);
    return 0;
}

static void flush_decoder(AVEMCodecContext *avctx)
{
    CCaptionSubContext *ctx = avctx->priv_data;
...
static int write_char(CCaptionSubContext *ctx, struct Screen *screen, char ch)
{
    ...
    /* We have extra space at end only for null character */
        ...
        av_em_log(ctx, AV_LOG_WARNING, "Data Ignored since exceeding screen width\n");
    ...
    for (i = 0; screen->row_used && i < SCREEN_ROWS; i++)
    {
        if (CHECK_FLAG(screen->row_used, i)) {
            ...
            for (; j < SCREEN_COLUMNS; j++) {
                ...
                if (override) {
                    av_em_bprintf(&ctx->buffer, "%s%s%s", e_tag, s_tag, override);
                } else {
                    av_em_bprintf(&ctx->buffer, "%s%s%c", e_tag, s_tag, row[j]);
                }
            }
            av_em_bprintf(&ctx->buffer, "\\N");
        }
    }
    if (!av_em_bprint_is_complete(&ctx->buffer))
...
static void handle_pac(CCaptionSubContext *ctx, uint8_t hi, uint8_t lo)
{
    ...
    if (row_map[index] <= 0) {
        av_em_log(ctx, AV_LOG_DEBUG, "Invalid pac index encountered\n");
        return;
    }
...
static int decode(AVEMCodecContext *avctx, void *data, int *got_sub, AVEMPacket *avpkt)
{
    CCaptionSubContext *ctx = avctx->priv_data;
    AVSubtitle *sub = data;
    ...
    if (ctx->pktbuf->size < len) {
        ret = av_em_buffer_realloc(&ctx->pktbuf, len);
         if (ret < 0) {
            av_em_log(ctx, AV_LOG_WARNING, "Insufficient Memory of %d truncated to %d\n", len, ctx->pktbuf->size);
            len = ctx->pktbuf->size;
            ret = 0;
        }
    }
    ...
    for (i  = 0; i < len; i += 3) {
        ...
        if (*ctx->buffer.str || ctx->real_time)
        {
            ...
            if (!ctx->real_time)
                sub->end_display_time = av_em_rescale_q(ctx->end_time - ctx->start_time,
    ...
    if (ctx->real_time && ctx->screen_touched &&
        sub->pts > ctx->last_real_time + av_em_rescale_q(200, ms_tb, AV_TIME_BASE_Q)) {
        ctx->last_real_time = sub->pts;
        ctx->screen_touched = 0;
...
static const AVEMClass ccaption_dec_class = {
