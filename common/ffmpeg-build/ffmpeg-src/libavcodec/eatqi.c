...
static av_cold int tqi_decode_init(AVEMCodecContext *avctx)
{
    TqiContext *t = avctx->priv_data;

    em_blockdsp_init(&t->bdsp, avctx);
    em_bswapdsp_init(&t->bsdsp);
...
static int tqi_decode_mb(TqiContext *t, int16_t (*block)[64])
{
    ...
    for (n = 0; n < 6; n++) {
        ...
        if (ret < 0) {
            av_em_log(t->avctx, AV_LOG_ERROR, "ac-tex damaged at %d %d\n",
                   t->mb_x, t->mb_y);
            return -1;
        }
    }
...
static inline void tqi_idct_put(AVEMCodecContext *avctx, AVFrame *frame,
                                int16_t (*block)[64])
{
    TqiContext *t = avctx->priv_data;
...
static int tqi_decode_frame(AVEMCodecContext *avctx,
                            ...
                            AVEMPacket *avpkt)
{
    ...
    av_em_fast_padded_malloc(&t->bitstream_buf, &t->bitstream_buf_size,
                          buf_end - buf);
    if (!t->bitstream_buf)
...
static av_cold int tqi_decode_end(AVEMCodecContext *avctx)
{
    TqiContext *t = avctx->priv_data;
