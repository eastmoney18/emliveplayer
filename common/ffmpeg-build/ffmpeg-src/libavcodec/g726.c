...
static av_cold int g726_encode_init(AVEMCodecContext *avctx)
{
    G726Context* c = avctx->priv_data;

    if (avctx->strict_std_compliance > FF_COMPLIANCE_UNOFFICIAL &&
        avctx->sample_rate != 8000) {
        av_em_log(avctx, AV_LOG_ERROR, "Sample rates other than 8kHz are not "
               "allowed when the compliance level is higher than unofficial. "
               "Resample or reduce the compliance level.\n");
        return AVERROR(EINVAL);
    }
    if (avctx->sample_rate <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid sample rate %d\n",
               avctx->sample_rate);
        return AVERROR(EINVAL);
    }

    if(avctx->channels != 1){
        av_em_log(avctx, AV_LOG_ERROR, "Only mono is supported\n");
        return AVERROR(EINVAL);
    }
...
static int g726_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                             const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, out_size, 0)) < 0)
        return ret;
...
static const AVEMClass g726_class = {
    .class_name = "g726",
...
AVEMCodec ff_adpcm_g726_encoder = {
    .name           = "g726",
...
static av_cold int g726_decode_init(AVEMCodecContext *avctx)
{
    G726Context* c = avctx->priv_data;

    if(avctx->channels > 1){
        avpriv_em_request_sample(avctx, "Decoding more than one channel");
    ...
    if (c->code_size < 2 || c->code_size > 5) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid number of bits %d\n", c->code_size);
        return AVERROR(EINVAL);
    }
    g726_reset(c);
...
static int g726_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (get_bits_left(&gb) > 0)
        av_em_log(avctx, AV_LOG_ERROR, "Frame invalidly split, missing parser?\n");
...
static void g726_decode_flush(AVEMCodecContext *avctx)
{
    G726Context *c = avctx->priv_data;
    g726_reset(c);
}
...
AVEMCodec ff_adpcm_g726_decoder = {
    .name           = "g726",
    .long_name      = NULL_IF_CONFIG_SMALL("G.726 ADPCM"),
    .type           = AVMEDIA_TYPE_AUDIO,
    .id             = AV_CODEC_ID_ADPCM_G726,
...
AVEMCodec ff_adpcm_g726le_decoder = {
    .name           = "g726le",
    .type           = AVMEDIA_TYPE_AUDIO,
