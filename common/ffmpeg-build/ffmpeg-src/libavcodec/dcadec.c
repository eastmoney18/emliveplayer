...
int ff_dca_set_channel_layout(AVEMCodecContext *avctx, int *ch_remap, int dca_mask)
{
...
static int dcadec_decode_frame(AVEMCodecContext *avctx, void *data,
                               int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (input_size < MIN_PACKET_SIZE || input_size > MAX_PACKET_SIZE) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid packet size\n");
    ...
    if (mrk != DCA_SYNCWORD_CORE_BE && mrk != DCA_SYNCWORD_SUBSTREAM) {
        av_em_fast_padded_malloc(&s->buffer, &s->buffer_size, input_size);
        if (!s->buffer)
        ...
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Not a valid DCA frame\n");
            return ret;
        }
    ...
    if (s->packet & DCA_PACKET_LBR) {
        ...
        if (s->packet & DCA_PACKET_CORE) {
            ...
            if (!(prev_packet & DCA_PACKET_RESIDUAL) && s->xll.nreschsets > 0
                && s->xll.nchsets > 1) {
                av_em_log(avctx, AV_LOG_VERBOSE, "Forcing XLL recovery mode\n");
        ...
        av_em_log(avctx, AV_LOG_ERROR, "No valid DCA sub-stream found\n");
        if (s->core_only)
            av_em_log(avctx, AV_LOG_WARNING, "Consider disabling 'core_only' option\n");
        return AVERROR_INVALIDDATA;
    }
...
static av_cold void dcadec_flush(AVEMCodecContext *avctx)
{
    DCAContext *s = avctx->priv_data;
...
static av_cold int dcadec_close(AVEMCodecContext *avctx)
{
    DCAContext *s = avctx->priv_data;
...
static av_cold int dcadec_init(AVEMCodecContext *avctx)
{
    DCAContext *s = avctx->priv_data;
    ...
    s->crctab = av_em_crc_get_table(AV_CRC_16_CCITT);
...
static const AVEMClass dcadec_class = {
