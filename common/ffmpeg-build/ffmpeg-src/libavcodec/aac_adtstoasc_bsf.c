 ...
 */
static int aac_adtstoasc_filter(AVBSFContext *bsfc, AVEMPacket *out)
{
    ...
    if (avpriv_em_aac_parse_header(&gb, &hdr) < 0) {
        av_em_log(bsfc, AV_LOG_ERROR, "Error parsing ADTS frame header!\n");
        ret = AVERROR_INVALIDDATA;
        goto fail;
    }

    if (!hdr.crc_absent && hdr.num_aac_frames > 1) {
        avpriv_em_report_missing_feature(bsfc,
    ...
    if (!ctx->first_frame_done) {
        ...
        extradata = av_em_mallocz(2 + pce_size + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!extradata) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
...
packet_too_small:
    av_em_log(bsfc, AV_LOG_ERROR, "Input packet too small\n");
    ret = AVERROR_INVALIDDATA;
fail:
    av_em_packet_free(&in);
    return ret;
...
const AVBitStreamFilter em_aac_adtstoasc_bsf = {
    .name           = "aac_adtstoasc",
