...
static av_cold void destroy_buffers(CinVideoContext *cin)
{
    ...
    for (i = 0; i < 3; ++i)
        av_em_freep(&cin->bitmap_table[i]);
}
...
static av_cold int allocate_buffers(CinVideoContext *cin)
{
    ...
    for (i = 0; i < 3; ++i) {
        cin->bitmap_table[i] = av_em_mallocz(cin->bitmap_size);
        if (!cin->bitmap_table[i]) {
            av_em_log(cin->avctx, AV_LOG_ERROR, "Can't allocate bitmap buffers.\n");
            destroy_buffers(cin);
...
static av_cold int cinvideo_decode_init(AVEMCodecContext *avctx)
{
                           ...
                           unsigned char *dst, int dst_size)
{
    ...
    while (src + 1 < src_end && dst < dst_end) {
        ...
        if (code & 0x80) {
            ...
            len = code + 1;
            if (len > src_end-src) {
                av_em_log(NULL, AV_LOG_ERROR, "RLE overread\n");
                return AVERROR_INVALIDDATA;
            }
...
static int cinvideo_decode_frame(AVEMCodecContext *avctx,
                                 ...
                                 AVEMPacket *avpkt)
{
    ...
    if ((res = em_reget_buffer(avctx, cin->frame)) < 0)
        return res;
    ...
    if ((res = av_em_frame_ref(data, cin->frame)) < 0)
        return res;
...
static av_cold int cinvideo_decode_end(AVEMCodecContext *avctx)
{
    CinVideoContext *cin = avctx->priv_data;

    av_em_frame_free(&cin->frame);
...
AVEMCodec ff_dsicinvideo_decoder = {
    .name           = "dsicinvideo",
