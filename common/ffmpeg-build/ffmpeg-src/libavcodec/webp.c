...
typedef struct WebPContext {
    ...
    AVEMCodecContext *avctx;              /* parent AVEMCodecContext */
...
static av_always_inline int webp_get_vlc(GetBitContext *gb, VLC_TYPE (*table)[2])
{
    ...
    if (n < 0) {
        ...
        index = (em_reverse[index] >> (8 - nb_bits)) + code;
        code  = table[index][0];
        n     = table[index][1];
    }
    SKIP_BITS(re, gb, n);
                                       ...
                                       int alphabet_size)
{
    ...
    codes = av_em_malloc_array(alphabet_size, sizeof(*codes));
    if (!codes)
                                    ...
                                    int alphabet_size)
{
    ...
    code_lengths = av_em_mallocz_array(alphabet_size, sizeof(*code_lengths));
    if (!code_lengths) {
        ret = AVERROR(ENOMEM);
        goto finish;
    }
    ...
    if (get_bits1(&s->gb)) {
        ...
        if (max_symbol > alphabet_size) {
            av_em_log(s->avctx, AV_LOG_ERROR, "max symbol %d > alphabet size %d\n",
                                      ...
                                      int w, int h)
{
    ...
    if (role == IMAGE_ROLE_ARGB && !img->is_alpha_primary) {
        ...
        ret = em_thread_get_buffer(s->avctx, &pt, 0);
    } else
        ret = av_em_frame_get_buffer(img->frame, 1);
    if (ret < 0)
        return ret;
    ...
    if (get_bits1(&s->gb)) {
        ...
        if (img->color_cache_bits < 1 || img->color_cache_bits > 11) {
            av_em_log(s->avctx, AV_LOG_ERROR, "invalid color cache bits: %d\n",
                   img->color_cache_bits);
            return AVERROR_INVALIDDATA;
        }
        img->color_cache = av_em_mallocz_array(1 << img->color_cache_bits,
    ...
    img->huffman_groups = av_em_mallocz_array(img->nb_huffman_groups *
    ...
    while (y < img->frame->height) {
        ...
        if (v < NUM_LITERAL_CODES) {
            ...
            if (!img->color_cache_bits) {
                av_em_log(s->avctx, AV_LOG_ERROR, "color cache not found\n");
                return AVERROR_INVALIDDATA;
            }
...
static int apply_color_indexing_transform(WebPContext *s)
{
    ...
    if (pal->size_reduction > 0) {
        ...
        line = av_em_alloc(img->frame->linesize[0]);
        if (!line)
            return AVERROR(ENOMEM);
...
static int vp8_lossless_decode_frame(AVEMCodecContext *avctx, AVFrame *p,
                                     ...
                                     unsigned int data_size, int is_alpha_chunk)
{
    ...
    if (!is_alpha_chunk) {
        if (get_bits(&s->gb, 8) != 0x2F) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid WebP Lossless signature\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        if (s->width && s->width != w) {
            av_em_log(avctx, AV_LOG_WARNING, "Width mismatch. %d != %d\n",
                   s->width, w);
        }
        s->width = w;
        if (s->height && s->height != h) {
            av_em_log(avctx, AV_LOG_WARNING, "Height mismatch. %d != %d\n",
                   s->width, w);
        }
        s->height = h;
        ...
        if (get_bits(&s->gb, 3) != 0x0) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid WebP Lossless version\n");
            return AVERROR_INVALIDDATA;
        }
    } else {
        if (!s->width || !s->height)
    ...
    while (get_bits1(&s->gb)) {
        ...
        if (used & (1 << transform)) {
            av_em_log(avctx, AV_LOG_ERROR, "Transform %d used more than once\n",
                   transform);
            ret = AVERROR_INVALIDDATA;
...
static int vp8_lossy_decode_alpha(AVEMCodecContext *avctx, AVFrame *p,
                                  uint8_t *data_start,
                                  unsigned int data_size)
{
    ...
    if (s->alpha_compression == ALPHA_COMPRESSION_NONE) {
        ...
        s->alpha_frame = av_em_frame_alloc();
        if (!s->alpha_frame)
            return AVERROR(ENOMEM);
        ...
        if (ret < 0) {
            av_em_frame_free(&s->alpha_frame);
            return ret;
        }
        if (!alpha_got_frame) {
            av_em_frame_free(&s->alpha_frame);
...
static int vp8_lossy_decode_frame(AVEMCodecContext *avctx, AVFrame *p,
                                  int *got_frame, uint8_t *data_start,
                                  unsigned int data_size)
{
    WebPContext *s = avctx->priv_data;
    ...
    if (data_size > INT_MAX) {
        av_em_log(avctx, AV_LOG_ERROR, "unsupported chunk size\n");
...
static int webp_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                             AVEMPacket *avpkt)
{
    ...
    *got_frame   = 0;
    ...
    if (bytestream2_get_le32(&gb) != MKTAG('R', 'I', 'F', 'F')) {
        av_em_log(avctx, AV_LOG_ERROR, "missing RIFF tag\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (bytestream2_get_le32(&gb) != MKTAG('W', 'E', 'B', 'P')) {
        av_em_log(avctx, AV_LOG_ERROR, "missing WEBP tag\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    while (bytestream2_get_bytes_left(&gb) > 8) {
        ...
        case MKTAG('V', 'P', '8', 'X'):
            ...
            ret = av_em_image_check_size(s->width, s->height, 0, avctx);
            if (ret < 0)
                return ret;
            break;
        case MKTAG('A', 'L', 'P', 'H'): {
            ...
            if (chunk_size == 0) {
                av_em_log(avctx, AV_LOG_ERROR, "invalid ALPHA chunk size\n");
                return AVERROR_INVALIDDATA;
            }
        ...
        case MKTAG('E', 'X', 'I', 'F'): {
            ...
            AVEMDictionary *exif_metadata = NULL;
            GetByteContext exif_gb;

            if (s->has_exif) {
                av_em_log(avctx, AV_LOG_VERBOSE, "Ignoring extra EXIF chunk\n");
                goto exif_end;
            }
            ...
            if (avpriv_exif_decode_ifd(avctx, &exif_gb, le, 0, &exif_metadata) < 0) {
                av_em_log(avctx, AV_LOG_ERROR, "error decoding Exif data\n");
                goto exif_end;
            }

            av_em_dict_copy(avempriv_frame_get_metadatap(data), exif_metadata, 0);

exif_end:
            av_em_dict_free(&exif_metadata);
            bytestream2_skip(&gb, chunk_size);
            break;
        }
        case MKTAG('I', 'C', 'C', 'P'):
        ...
        case MKTAG('X', 'M', 'P', ' '):
            ...
            av_em_log(avctx, AV_LOG_WARNING, "skipping unsupported chunk: %s\n",
                   chunk_str);
            bytestream2_skip(&gb, chunk_size);
            break;
        default:
            AV_WL32(chunk_str, chunk_type);
            av_em_log(avctx, AV_LOG_VERBOSE, "skipping unknown chunk: %s\n",
                   chunk_str);
            bytestream2_skip(&gb, chunk_size);
            break;
        }
    ...
    if (!*got_frame) {
        av_em_log(avctx, AV_LOG_ERROR, "image data not found\n");
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int webp_decode_close(AVEMCodecContext *avctx)
{
    WebPContext *s = avctx->priv_data;
