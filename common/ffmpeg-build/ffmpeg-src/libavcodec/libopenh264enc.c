...
static const AVEMClass class = {
    "libopenh264enc", av_em_default_item_name, options, LIBAVUTIL_VERSION_INT
};
...
static void libopenh264_trace_callback(void *ctx, int level, const char *msg)
{
    ...
    av_em_log(ctx, equiv_ffmpeg_log_level, "%s\n", msg);
}

static av_cold int svc_encode_close(AVEMCodecContext *avctx)
{
    ...
    if (s->skipped > 0)
        av_em_log(avctx, AV_LOG_WARNING, "%d frames skipped\n", s->skipped);
    return 0;
}

static av_cold int svc_encode_init(AVEMCodecContext *avctx)
{
    SVCContext *s = avctx->priv_data;
    ...
    if (memcmp(&libver, &g_stCodecVersion, sizeof(libver))) {
        av_em_log(avctx, AV_LOG_ERROR, "Incorrect library version loaded\n");
        return AVERROR(EINVAL);
    }
#endif

    if (WelsCreateSVCEncoder(&s->encoder)) {
        av_em_log(avctx, AV_LOG_ERROR, "Unable to create encoder\n");
    ...
    // Set the logging callback function to one that uses av_em_log() (see implementation above).
    ...
    // Set the AVEMCodecContext as the libopenh264 callback context so that it can be passed to av_em_log().
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    param.fMaxFrameRate              = 1/av_em_q2d(avctx->time_base);
    ...
    if ((avctx->slices > 1) && (s->max_nal_size)){
        av_em_log(avctx,AV_LOG_ERROR,"Invalid combination -slices %d and -max_nal_size %d.\n",avctx->slices,s->max_nal_size);
        goto fail;
    }
    ...
    if (s->slice_mode == SM_DYN_SLICE) {
        if (s->max_nal_size){
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Invalid -max_nal_size, "
    ...
    if ((*s->encoder)->InitializeExt(s->encoder, &param) != cmResultSuccess) {
        av_em_log(avctx, AV_LOG_ERROR, "Initialize failed\n");
        goto fail;
    }
    ...
    if (avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER) {
        ...
        avctx->extradata = av_em_mallocz(size + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!avctx->extradata) {
            err = AVERROR(ENOMEM);
            goto fail;
        }
    ...
    props = em_add_cpb_side_data(avctx);
    if (!props) {
        err = AVERROR(ENOMEM);
        goto fail;
    }
...
static int svc_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                            const AVFrame *frame, int *got_packet)
{
    ...
    if (encoded != cmResultSuccess) {
        av_em_log(avctx, AV_LOG_ERROR, "EncodeFrame failed\n");
        return AVERROR_UNKNOWN;
    }
    if (fbi.eFrameType == videoFrameTypeSkip) {
        s->skipped++;
        av_em_log(avctx, AV_LOG_DEBUG, "frame skipped\n");
        return 0;
    }
    first_layer = 0;
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "%d slices\n", fbi.sLayerInfo[fbi.iLayerNum - 1].iNalCount);

    if ((ret = em_alloc_packet2(avctx, avpkt, size, size))) {
...
AVEMCodec ff_libopenh264_encoder = {
    .name           = "libopenh264",
