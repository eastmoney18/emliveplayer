                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (bytestream2_get_bytes_left(&gb) < ALIAS_HEADER_SIZE) {
        av_em_log(avctx, AV_LOG_ERROR, "Header too small %d.\n", avpkt->size);
    ...
    while (bytestream2_get_bytes_left(&gb) > 0) {
        ...
        if (!count || x + count > avctx->width) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid run length %d.\n", count);
            return AVERROR_INVALIDDATA;
        }
    ...
    if (x != width || y != height) {
        av_em_log(avctx, AV_LOG_ERROR, "Picture stopped at %d,%d.\n", x, y);
        return AVERROR_INVALIDDATA;
    }
...
AVEMCodec ff_alias_pix_decoder = {
    .name         = "alias_pix",
