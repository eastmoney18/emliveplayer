...
static int twinvq_read_bitstream(AVEMCodecContext *avctx, TwinVQContext *tctx,
                                 const uint8_t *buf, int buf_size)
{
    ...
    if (bits->window_type > 8) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid window type, broken sample?\n");
...
static av_cold int twinvq_decode_init(AVEMCodecContext *avctx)
{
    int isampf, ibps;
    ...
    if (isampf < 8 || isampf > 44) {
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported sample rate\n");
        return AVERROR_INVALIDDATA;
    }
    switch (isampf) {
    case 44:
    ...
    if (avctx->channels <= 0 || avctx->channels > TWINVQ_CHANNELS_MAX) {
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported number of channels: %i\n",
               avctx->channels);
        return -1;
    }
    ...
    if (ibps < 8 || ibps > 48) {
        av_em_log(avctx, AV_LOG_ERROR, "Bad bitrate per channel value %d\n", ibps);
        return AVERROR_INVALIDDATA;
    }
