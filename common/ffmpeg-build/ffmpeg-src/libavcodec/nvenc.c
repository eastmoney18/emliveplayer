                                     ...
                                     const char *error_string)
{
    ...
    av_em_log(log_ctx, AV_LOG_ERROR, "%s: %s (%d)\n", error_string, desc, err);
    return ret;
}

static av_cold int nvenc_load_libraries(AVEMCodecContext *avctx)
{
    ...
    av_em_log(avctx, AV_LOG_VERBOSE, "Nvenc initialized successfully\n");

    return 0;
}

static av_cold int nvenc_open_session(AVEMCodecContext *avctx)
{
...
static int nvenc_check_codec_support(AVEMCodecContext *avctx)
{
    NvencContext *ctx = avctx->priv_data;
    NV_ENCODE_API_FUNCTION_LIST *p_nvenc = &ctx->nvenc_dload_funcs.nvenc_funcs;
    int i, ret, count = 0;
    ...
    guids = av_em_alloc(count * sizeof(GUID));
    if (!guids)
...
static int nvenc_check_cap(AVEMCodecContext *avctx, NV_ENC_CAPS cap)
{
    NvencContext *ctx = avctx->priv_data;
    NV_ENCODE_API_FUNCTION_LIST *p_nvenc = &ctx->nvenc_dload_funcs.nvenc_funcs;
...
static int nvenc_check_capabilities(AVEMCodecContext *avctx)
{
    NvencContext *ctx = avctx->priv_data;
    int ret;
    ...
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_VERBOSE, "Codec not supported\n");
        return ret;
    }

    ret = nvenc_check_cap(avctx, NV_ENC_CAPS_SUPPORT_YUV444_ENCODE);
    if (ctx->data_pix_fmt == AV_PIX_FMT_YUV444P && ret <= 0) {
        av_em_log(avctx, AV_LOG_VERBOSE, "YUV444P not supported\n");
        return AVERROR(ENOSYS);
    }

    ret = nvenc_check_cap(avctx, NV_ENC_CAPS_SUPPORT_LOSSLESS_ENCODE);
    if (ctx->preset >= PRESET_LOSSLESS_DEFAULT && ret <= 0) {
        av_em_log(avctx, AV_LOG_VERBOSE, "Lossless encoding not supported\n");
        return AVERROR(ENOSYS);
    }

    ret = nvenc_check_cap(avctx, NV_ENC_CAPS_WIDTH_MAX);
    if (ret < avctx->width) {
        av_em_log(avctx, AV_LOG_VERBOSE, "Width %d exceeds %d\n",
               avctx->width, ret);
        return AVERROR(ENOSYS);
    }
    ...
    if (ret < avctx->height) {
        av_em_log(avctx, AV_LOG_VERBOSE, "Height %d exceeds %d\n",
               avctx->height, ret);
        return AVERROR(ENOSYS);
    }
    ...
    if (ret < avctx->max_b_frames) {
        av_em_log(avctx, AV_LOG_VERBOSE, "Max B-frames %d exceed %d\n",
    ...
    if (ret < 1 && avctx->flags & AV_CODEC_FLAG_INTERLACED_DCT) {
        av_em_log(avctx, AV_LOG_VERBOSE,
...
static av_cold int nvenc_check_device(AVEMCodecContext *avctx, int idx)
{
    NvencContext *ctx = avctx->priv_data;
    NvencDynLoadFunctions *dl_fn = &ctx->nvenc_dload_funcs;
    ...
    av_em_log(avctx, loglevel, "[ GPU #%d - < %s > has Compute SM %d.%d ]\n", idx, name, major, minor);
    if (((major << 4) | minor) < NVENC_CAP) {
        av_em_log(avctx, loglevel, "does not support NVENC\n");
        goto fail;
    }

    cu_res = dl_fn->cu_ctx_create(&ctx->cu_context_internal, 0, cu_device);
    if (cu_res != CUDA_SUCCESS) {
        av_em_log(avctx, AV_LOG_FATAL, "Failed creating CUDA context for NVENC: 0x%x\n", (int)cu_res);
        goto fail;
    }
    ...
    if (cu_res != CUDA_SUCCESS) {
        av_em_log(avctx, AV_LOG_FATAL, "Failed popping CUDA context: 0x%x\n", (int)cu_res);
        goto fail2;
    }
    ...
    av_em_log(avctx, loglevel, "supports NVENC\n");
...
static av_cold int nvenc_setup_device(AVEMCodecContext *avctx)
{
    NvencContext *ctx = avctx->priv_data;
    NvencDynLoadFunctions *dl_fn = &ctx->nvenc_dload_funcs;
    ...
    if (avctx->pix_fmt == AV_PIX_FMT_CUDA) {
        ...
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_FATAL, "Provided device doesn't support required NVENC features\n");
            return ret;
        }
#else
        return AVERROR_BUG;
        ...
        if (!nb_devices) {
            av_em_log(avctx, AV_LOG_FATAL, "No CUDA capable devices found\n");
                return AVERROR_EXTERNAL;
        }

        av_em_log(avctx, AV_LOG_VERBOSE, "%d CUDA capable devices found\n", nb_devices);
        ...
        if (!dl_fn->nvenc_device_count) {
            av_em_log(avctx, AV_LOG_FATAL, "No NVENC capable devices found\n");
            return AVERROR_EXTERNAL;
        }

        av_em_log(avctx, AV_LOG_FATAL, "Requested GPU %d, but only %d GPUs are available!\n", ctx->device, dl_fn->nvenc_device_count);
        return AVERROR(EINVAL);
    }
...
static av_cold void set_constqp(AVEMCodecContext *avctx)
{
    NvencContext *ctx = avctx->priv_data;
...
static av_cold void set_vbr(AVEMCodecContext *avctx)
{
    NvencContext *ctx = avctx->priv_data;
    NV_ENC_RC_PARAMS *rc = &ctx->encode_config.rcParams;
    int qp_inter_p;
...
static av_cold void set_lossless(AVEMCodecContext *avctx)
{
    NvencContext *ctx = avctx->priv_data;
    NV_ENC_RC_PARAMS *rc = &ctx->encode_config.rcParams;
...
static void nvenc_override_rate_control(AVEMCodecContext *avctx)
{
    NvencContext *ctx    = avctx->priv_data;
    NV_ENC_RC_PARAMS *rc = &ctx->encode_config.rcParams;
...
static av_cold void nvenc_setup_rate_control(AVEMCodecContext *avctx)
{
    NvencContext *ctx = avctx->priv_data;
...
static av_cold int nvenc_setup_h264_config(AVEMCodecContext *avctx)
{
    NvencContext *ctx                      = avctx->priv_data;
...
static av_cold int nvenc_setup_hevc_config(AVEMCodecContext *avctx)
{
    NvencContext *ctx                      = avctx->priv_data;
    NV_ENC_CONFIG *cc                      = &ctx->encode_config;
...
static av_cold int nvenc_setup_codec_config(AVEMCodecContext *avctx)
{
    switch (avctx->codec->id) {
    case AV_CODEC_ID_H264:
...
static av_cold int nvenc_setup_encoder(AVEMCodecContext *avctx)
{
    NvencContext *ctx = avctx->priv_data;
    NvencDynLoadFunctions *dl_fn = &ctx->nvenc_dload_funcs;
    NV_ENCODE_API_FUNCTION_LIST *p_nvenc = &dl_fn->nvenc_funcs;
...
static av_cold int nvenc_alloc_surface(AVEMCodecContext *avctx, int idx)
{
    NvencContext *ctx = avctx->priv_data;
    NvencDynLoadFunctions *dl_fn = &ctx->nvenc_dload_funcs;
    NV_ENCODE_API_FUNCTION_LIST *p_nvenc = &dl_fn->nvenc_funcs;
    ...
    default:
        av_em_log(avctx, AV_LOG_FATAL, "Invalid input pixel format\n");
        return AVERROR(EINVAL);
    }

    if (avctx->pix_fmt == AV_PIX_FMT_CUDA) {
        ctx->surfaces[idx].in_ref = av_em_frame_alloc();
    ...
    if (nv_status != NV_ENC_SUCCESS) {
        ...
        av_em_frame_free(&ctx->surfaces[idx].in_ref);
        return err;
    }
...
static av_cold int nvenc_setup_surfaces(AVEMCodecContext *avctx)
{
    NvencContext *ctx = avctx->priv_data;
    int i, res;
    ...
    ctx->surfaces = av_em_mallocz_array(ctx->nb_surfaces, sizeof(*ctx->surfaces));
    if (!ctx->surfaces)
        return AVERROR(ENOMEM);

    ctx->timestamp_list = av_em_fifo_alloc(ctx->nb_surfaces * sizeof(int64_t));
    if (!ctx->timestamp_list)
        return AVERROR(ENOMEM);
    ctx->output_surface_queue = av_em_fifo_alloc(ctx->nb_surfaces * sizeof(NvencSurface*));
    if (!ctx->output_surface_queue)
        return AVERROR(ENOMEM);
    ctx->output_surface_ready_queue = av_em_fifo_alloc(ctx->nb_surfaces * sizeof(NvencSurface*));
...
static av_cold int nvenc_setup_extradata(AVEMCodecContext *avctx)
{
    NvencContext *ctx = avctx->priv_data;
    NvencDynLoadFunctions *dl_fn = &ctx->nvenc_dload_funcs;
    NV_ENCODE_API_FUNCTION_LIST *p_nvenc = &dl_fn->nvenc_funcs;
    ...
    avctx->extradata = av_em_mallocz(outSize + AV_INPUT_BUFFER_PADDING_SIZE);
...
av_cold int ff_nvenc_encode_close(AVEMCodecContext *avctx)
{
    NvencContext *ctx               = avctx->priv_data;
    NvencDynLoadFunctions *dl_fn = &ctx->nvenc_dload_funcs;
    NV_ENCODE_API_FUNCTION_LIST *p_nvenc = &dl_fn->nvenc_funcs;
    ...
    av_em_fifo_freep(&ctx->timestamp_list);
    av_em_fifo_freep(&ctx->output_surface_ready_queue);
    av_em_fifo_freep(&ctx->output_surface_queue);
    ...
    av_em_log(avctx, AV_LOG_VERBOSE, "Nvenc unloaded\n");

    return 0;
}

av_cold int ff_nvenc_encode_init(AVEMCodecContext *avctx)
{
    NvencContext *ctx = avctx->priv_data;
    int ret;
...
static int nvenc_copy_frame(AVEMCodecContext *avctx, NvencSurface *inSurf,
            NV_ENC_LOCK_INPUT_BUFFER *lockBufferParams, const AVFrame *frame)
{
    ...
    if (frame->format == AV_PIX_FMT_YUV420P) {
        av_em_image_copy_plane(buf, lockBufferParams->pitch,
            frame->data[0], frame->linesize[0],
            avctx->width, avctx->height);

        buf += off;

        av_em_image_copy_plane(buf, lockBufferParams->pitch >> 1,
        ...
        av_em_log(avctx, AV_LOG_FATAL, "Invalid pixel format!\n");
        return AVERROR(EINVAL);
    }
...
static int nvenc_find_free_reg_resource(AVEMCodecContext *avctx)
{
    NvencContext *ctx = avctx->priv_data;
    NvencDynLoadFunctions *dl_fn = &ctx->nvenc_dload_funcs;
    NV_ENCODE_API_FUNCTION_LIST *p_nvenc = &dl_fn->nvenc_funcs;
    ...
    av_em_log(avctx, AV_LOG_ERROR, "Too many registered CUDA frames\n");
    return AVERROR(ENOMEM);
}

static int nvenc_register_frame(AVEMCodecContext *avctx, const AVFrame *frame)
{
    NvencContext *ctx = avctx->priv_data;
    NvencDynLoadFunctions *dl_fn = &ctx->nvenc_dload_funcs;
    NV_ENCODE_API_FUNCTION_LIST *p_nvenc = &dl_fn->nvenc_funcs;
...
static int nvenc_upload_frame(AVEMCodecContext *avctx, const AVFrame *frame,
                                      NvencSurface *nvenc_frame)
{
    ...
    if (avctx->pix_fmt == AV_PIX_FMT_CUDA) {
        ...
        if (reg_idx < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Could not register an input CUDA frame\n");
            return reg_idx;
        }

        res = av_em_frame_ref(nvenc_frame->in_ref, frame);
        if (res < 0)
            return res;
        ...
        if (nv_status != NV_ENC_SUCCESS) {
            av_em_frame_unref(nvenc_frame->in_ref);
...
static void nvenc_codec_specific_pic_params(AVEMCodecContext *avctx,
...
{
    av_em_fifo_generic_write(queue, &timestamp, sizeof(timestamp), NULL);
}

static inline int64_t timestamp_queue_dequeue(AVFifoBuffer* queue)
{
    int64_t timestamp = AV_NOPTS_VALUE;
    if (av_em_fifo_size(queue) > 0)
        av_em_fifo_generic_read(queue, &timestamp, sizeof(timestamp), NULL);

    return timestamp;
}

static int nvenc_set_timestamp(AVEMCodecContext *avctx,
...
static int process_output_surface(AVEMCodecContext *avctx, AVEMPacket *pkt, NvencSurface *tmpoutsurf)
{
    NvencContext *ctx = avctx->priv_data;
    NvencDynLoadFunctions *dl_fn = &ctx->nvenc_dload_funcs;
    NV_ENCODE_API_FUNCTION_LIST *p_nvenc = &dl_fn->nvenc_funcs;
    ...
    case AV_CODEC_ID_H265:
      ...
      break;
    default:
      av_em_log(avctx, AV_LOG_ERROR, "Unknown codec name\n");
      res = AVERROR(EINVAL);
      goto error;
    }
    slice_offsets = av_em_mallocz(slice_mode_data * sizeof(*slice_offsets));
    ...
    if (res = em_alloc_packet2(avctx, pkt, lock_params.bitstreamSizeInBytes,0)) {
    ...
    if (avctx->pix_fmt == AV_PIX_FMT_CUDA) {
        ...
        av_em_frame_unref(tmpoutsurf->in_ref);
    ...
    case NV_ENC_PIC_TYPE_BI:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unknown picture type encountered, expect the output to be broken.\n");
        av_em_log(avctx, AV_LOG_ERROR, "Please report this error and include as much information on how to reproduce it as possible.\n");
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    em_side_data_set_encoder_stats(pkt,
...
static int output_ready(AVEMCodecContext *avctx, int flush)
{
    NvencContext *ctx = avctx->priv_data;
    ...
    nb_ready   = av_em_fifo_size(ctx->output_surface_ready_queue)   / sizeof(NvencSurface*);
    nb_pending = av_em_fifo_size(ctx->output_surface_queue)         / sizeof(NvencSurface*);
    if (flush)
        return nb_ready > 0;
    return (nb_ready > 0) && (nb_ready + nb_pending >= ctx->async_depth);
}

int ff_nvenc_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                          const AVFrame *frame, int *got_packet)
{
    ...
    if (frame) {
        ...
        if (!inSurf) {
            av_em_log(avctx, AV_LOG_ERROR, "No free surfaces\n");
            return AVERROR_BUG;
        }
    ...
    if (frame) {
        av_em_fifo_generic_write(ctx->output_surface_queue, &inSurf, sizeof(inSurf), NULL);
    ...
    if (nv_status == NV_ENC_SUCCESS) {
        while (av_em_fifo_size(ctx->output_surface_queue) > 0) {
            av_em_fifo_generic_read(ctx->output_surface_queue, &tmpoutsurf, sizeof(tmpoutsurf), NULL);
            av_em_fifo_generic_write(ctx->output_surface_ready_queue, &tmpoutsurf, sizeof(tmpoutsurf), NULL);
        }
    }

    if (output_ready(avctx, !frame)) {
        av_em_fifo_generic_read(ctx->output_surface_ready_queue, &tmpoutsurf, sizeof(tmpoutsurf), NULL);
