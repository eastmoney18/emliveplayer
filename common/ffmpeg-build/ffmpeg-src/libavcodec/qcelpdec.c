 ...
 */
static av_cold int qcelp_decode_init(AVEMCodecContext *avctx)
{
...
static void apply_gain_ctrl(float *v_out, const float *v_ref, const float *v_in)
{
    ...
    for (i = 0; i < 160; i += 40) {
        float res = avpriv_em_scalarproduct_float_c(v_ref + i, v_ref + i, 40);
 ...
 */
static qcelp_packet_rate determine_bitrate(AVEMCodecContext *avctx,
                                           const int buf_size,
                                           const uint8_t **buf)
{
    qcelp_packet_rate bitrate;
    ...
    if (bitrate == SILENCE) {
        ...
        avpriv_em_request_sample(avctx, "Blank frame handling");
    }
    return bitrate;
}

static void warn_insufficient_frame_quality(AVEMCodecContext *avctx,
                                            const char *message)
{
    av_em_log(avctx, AV_LOG_WARNING, "Frame #%d, IFQ: %s\n",
...
static void postfilter(QCELPContext *q, float *samples, float *lpc)
{
    ...
    ff_adaptive_gain_control(samples, pole_out + 10,
                             avpriv_em_scalarproduct_float_c(q->formant_mem + 10,
                                                          q->formant_mem + 10,
                                                          160),
                             160, 0.9375, &q->postfilter_agc_mem);
}

static int qcelp_decode_frame(AVEMCodecContext *avctx, void *data,
