...
static int ir2_decode_frame(AVEMCodecContext *avctx,
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (start >= buf_size) {
        av_em_log(s->avctx, AV_LOG_ERROR, "input buffer size too small (%d)\n", buf_size);
        return AVERROR_INVALIDDATA;
    }
    ...
    if ((ret = av_em_frame_ref(picture, p)) < 0)
        return ret;
...
static av_cold int ir2_decode_init(AVEMCodecContext *avctx)
{
...
static av_cold int ir2_decode_end(AVEMCodecContext *avctx)
{
    Ir2Context * const ic = avctx->priv_data;

    av_em_frame_free(&ic->picture);

    return 0;
}

AVEMCodec ff_indeo2_decoder = {
    .name           = "indeo2",
