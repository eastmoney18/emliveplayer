...
typedef struct TiffEncoderContext {
    AVEMClass *class;                         ///< for private options
...
static inline int check_size(TiffEncoderContext *s, uint64_t need)
{
    if (s->buf_size < *s->buf - s->buf_start + need) {
        *s->buf = s->buf_start + s->buf_size + 1;
        av_em_log(s->avctx, AV_LOG_ERROR, "Buffer is too small\n");
        return 1;
    }
    return 0;
}
                        ...
                        uint8_t *dst, int n, int compr)
{
    ...
    case TIFF_ADOBE_DEFLATE:
    {
        ...
        if (compress(dst, &zlen, src, n) != Z_OK) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Compressing failed\n");
    ...
    default:
        av_em_log(s->avctx, AV_LOG_ERROR, "Unsupported compression method: %d\n",
               compr);
        return AVERROR(EINVAL);
    }
}
                        ...
                        const AVFrame *pict, int *got_packet)
{
    ...
    s->bpp          = av_em_get_bits_per_pixel(desc);
    ...
    case AV_PIX_FMT_YUV411P:
        av_em_pix_fmt_get_chroma_sub_sample(avctx->pix_fmt, &shift_h, &shift_v);
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, packet_size, 0)) < 0)
        return ret;
    ptr          = pkt->data;
    ...
    av_em_fast_padded_mallocz(&s->strip_sizes  , &s->strip_sizes_size  , sizeof(s->strip_sizes  [0]) * strips);
    av_em_fast_padded_mallocz(&s->strip_offsets, &s->strip_offsets_size, sizeof(s->strip_offsets[0]) * strips);
    ...
    if (s->compr == TIFF_DEFLATE || s->compr == TIFF_ADOBE_DEFLATE) {
        ...
        av_em_free(zbuf);
        if (ret < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Encode strip failed\n");
            goto fail;
        }
        ptr           += ret;
    ...
    {
    if (s->compr == TIFF_LZW) {
        s->lzws = av_em_alloc(ff_lzw_encode_state_size);
        if (!s->lzws) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
    ...
    if (avctx->sample_aspect_ratio.num > 0 &&
        ...
        AVEMRational y = av_em_mul_q(av_em_make_q(s->dpi, 1),
...
static av_cold int encode_close(AVEMCodecContext *avctx)
{
    ...
    av_em_freep(&s->strip_sizes);
    av_em_freep(&s->strip_offsets);
    av_em_freep(&s->yuv_line);
...
static const AVEMClass tiffenc_class = {
