...
static int generate_fake_vps(QSVEncContext *q, AVEMCodecContext *avctx)
{
    GetByteContext gbc;
    PutByteContext pbc;
    ...
    /* parse the SPS */
    ret = em_h2645_extract_rbsp(avctx->extradata + 4, avctx->extradata_size - 4, &sps_nal);
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error unescaping the SPS buffer\n");
        return ret;
    }

    ret = init_get_bits8(&gb, sps_nal.data, sps_nal.size);
    if (ret < 0) {
        av_em_freep(&sps_nal.rbsp_buffer);
        return ret;
    }
    ...
    type = get_bits(&gb, 6);
    if (type != NAL_SPS) {
        av_em_log(avctx, AV_LOG_ERROR, "Unexpected NAL type in the extradata: %d\n",
               type);
        av_em_freep(&sps_nal.rbsp_buffer);
    ...
    ret = em_hevc_parse_sps(&sps, &gb, &sps_id, 0, NULL, avctx);
    av_em_freep(&sps_nal.rbsp_buffer);
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error parsing the SPS\n");
        return ret;
    }
    ...
    ret = em_hevc_encode_nal_vps(&vps, sps.vps_id, vps_rbsp_buf, sizeof(vps_rbsp_buf));
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error writing the VPS\n");
        return ret;
    }
    ...
    new_extradata = av_em_mallocz(vps_size + avctx->extradata_size + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!new_extradata)
...
static av_cold int qsv_enc_init(AVEMCodecContext *avctx)
{
    ...
    if (q->load_plugin != LOAD_PLUGIN_NONE) {
        ...
        if (q->qsv.load_plugins[0]) {
            ...
            av_em_freep(&q->qsv.load_plugins);

            if (q->load_plugin == LOAD_PLUGIN_HEVC_SW)
                q->qsv.load_plugins = av_em_strdup(uid_hevcenc_sw);
            else
                q->qsv.load_plugins = av_em_strdup(uid_hevcenc_hw);
...
static int qsv_enc_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
...
static av_cold int qsv_enc_close(AVEMCodecContext *avctx)
{
    QSVHEVCEncContext *q = avctx->priv_data;
...
static const AVEMCodecDefault qsv_enc_defaults[] = {
    { "b",         "1M"    },
    { "refs",      "0"     },
