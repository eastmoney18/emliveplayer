...
static int merge_superframe(const struct CachedBuf *in, int n_in, AVEMPacket *out)
{
...
static int vp9_superframe_filter(AVBSFContext *ctx, AVEMPacket *out)
{
    GetBitContext gb;
    ...
    if (invisible && !uses_superframe_syntax) {
        s->cache[s->n_cache].data = av_em_alloc(in->size);
    ...
    for (n = 0; n < s->n_cache; n++)
        av_em_freep(&s->cache[n].data);
    s->n_cache = 0;

    res = av_em_packet_copy_props(out, in);
    if (res < 0)
        goto done;
...
const AVBitStreamFilter em_vp9_superframe_bsf = {
    .name           = "vp9_superframe",
