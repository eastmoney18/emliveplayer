...
typedef struct FLACParseContext {
    AVEMCodecParserContext *pc;      /**< parent context                        */
    AVEMCodecContext *avctx;         /**< codec context pointer for logging     */
...
static int frame_header_is_valid(AVEMCodecContext *avctx, const uint8_t *buf,
                                 FLACFrameInfo *fi)
{
    GetBitContext gb;
 ...
 * This function is based on av_em_fifo_generic_read, which is why there is a comment
                               ...
                               uint8_t** wrap_buf, int* allocated_size)
{
    ...
    tmp_buf = av_em_fast_realloc(*wrap_buf, allocated_size, len);

    if (!tmp_buf) {
        av_em_log(fpc->avctx, AV_LOG_ERROR,
...
static int find_headers_search_validate(FLACParseContext *fpc, int offset)
{
    ...
    if (frame_header_is_valid(fpc->avctx, header_buf, &fi)) {
        ...
        *end_handle = av_em_mallocz(sizeof(**end_handle));
        if (!*end_handle) {
            av_em_log(fpc->avctx, AV_LOG_ERROR,
        ...
        (*end_handle)->link_penalty = av_em_alloc(sizeof(int) *
                                            FLAC_MAX_SEQUENTIAL_HEADERS);
        if (!(*end_handle)->link_penalty) {
            av_em_freep(end_handle);
            av_em_log(fpc->avctx, AV_LOG_ERROR,
...
static int find_new_headers(FLACParseContext *fpc, int search_start)
{
    ...
    search_end = av_em_fifo_size(fpc->fifo_buf) - (MAX_FRAME_HEADER_SIZE - 1);
                                    ...
                                    int                log_level_offset)
{
    ...
    if (child_fi->samplerate != header_fi->samplerate) {
        ...
        av_em_log(fpc->avctx, AV_LOG_WARNING + log_level_offset,
                                 ...
                                 int                log_level_offset)
{
    ...
    if (deduction && !deduction_expected) {
        ...
        if (header->link_penalty[i] < FLAC_HEADER_CRC_FAIL_PENALTY ||
            ...
            crc      = av_em_crc(av_em_crc_get_table(AV_CRC_16_ANSI), 0, buf, read_len);
            read_len = (end->offset - start->offset) - read_len;

            if (read_len) {
                buf = flac_fifo_read(fpc, end->offset - read_len, &read_len);
                crc = av_em_crc(av_em_crc_get_table(AV_CRC_16_ANSI), crc, buf, read_len);
            }
        }
                           ...
                           int *poutbuf_size)
{
    ...
    if (!child) {
        *poutbuf_size = av_em_fifo_size(fpc->fifo_buf) - header->offset;
    } else {
    ...
    *poutbuf = flac_fifo_read_wrap(fpc, header->offset, *poutbuf_size,
    ...
    if (child)
        return child->offset - av_em_fifo_size(fpc->fifo_buf);
    return 0;
}

static int flac_parse(AVEMCodecParserContext *s, AVEMCodecContext *avctx,
                      ...
                      const uint8_t *buf, int buf_size)
{
    ...
    if (fpc->best_header && fpc->best_header->best_child) {
        ...
        for (curr = fpc->headers; curr != best_child; curr = temp) {
            ...
            temp = curr->next;
            av_em_freep(&curr->link_penalty);
            av_em_free(curr);
            fpc->nb_headers_buffered--;
        }
        /* Release returned data from ring buffer. */
        av_em_fifo_drain(fpc->fifo_buf, best_child->offset);
        ...
        av_em_freep(&fpc->best_header->link_penalty);
        av_em_freep(&fpc->best_header);
    }
    ...
    while ((buf && buf_size && read_end < buf + buf_size &&
        ...
        if (!av_em_fifo_space(fpc->fifo_buf) &&
            av_em_fifo_size(fpc->fifo_buf) / FLAC_AVG_FRAME_SIZE >
        ...
        /* Fill the buffer. */
        if (   av_em_fifo_space(fpc->fifo_buf) < read_end - read_start
            && av_em_fifo_realloc2(fpc->fifo_buf, (read_end - read_start) + 2*av_em_fifo_size(fpc->fifo_buf)) < 0) {
            av_em_log(avctx, AV_LOG_ERROR,
                   "couldn't reallocate buffer of size %"PTRDIFF_SPECIFIER"\n",
                   (read_end - read_start) + av_em_fifo_size(fpc->fifo_buf));
            goto handle_error;
        }

        if (buf && buf_size) {
            av_em_fifo_generic_write(fpc->fifo_buf, (void*) read_start,
                                  read_end - read_start, NULL);
        } else {
            int8_t pad[MAX_FRAME_HEADER_SIZE] = { 0 };
            av_em_fifo_generic_write(fpc->fifo_buf, (void*) pad, sizeof(pad), NULL);
        }

        /* Tag headers and update sequences. */
        start_offset = av_em_fifo_size(fpc->fifo_buf) -
    ...
    if (fpc->best_header) {
        ...
        if (fpc->best_header->offset > 0) {
            /* Output a junk frame. */
            av_em_log(avctx, AV_LOG_DEBUG, "Junk frame till offset %i\n",
...
static av_cold int flac_parse_init(AVEMCodecParserContext *c)
{
    FLACParseContext *fpc = c->priv_data;
    fpc->pc = c;
    /* There will generally be FLAC_MIN_HEADERS buffered in the fifo before
       it drains.  This is allocated early to avoid slow reallocation. */
    fpc->fifo_buf = av_em_fifo_alloc_array(FLAC_MIN_HEADERS + 3, FLAC_AVG_FRAME_SIZE);
    if (!fpc->fifo_buf) {
        av_em_log(fpc->avctx, AV_LOG_ERROR,
...
static void flac_parse_close(AVEMCodecParserContext *c)
{
    FLACParseContext *fpc = c->priv_data;
    ...
    av_em_fifo_freep(&fpc->fifo_buf);
    av_em_freep(&fpc->wrap_buf);
}

AVEMCodecParser ff_flac_parser = {
