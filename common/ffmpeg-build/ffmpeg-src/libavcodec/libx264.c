...
static void X264_log(void *p, int level, const char *fmt, va_list args)
{
    ...
    av_em_vlog(p, level_map[level], fmt, args);
}


static int encode_nals(AVEMCodecContext *ctx, AVEMPacket *pkt,
                       const x264_nal_t *nals, int nnal)
{
    ...
    if ((ret = em_alloc_packet2(ctx, pkt, size, 0)) < 0)
        return ret;
    ...
    if (x4->sei_size > 0 && nnal > 0) {
        if (x4->sei_size > size) {
            av_em_log(ctx, AV_LOG_ERROR, "Error: nal buffer is too small\n");
            return -1;
        }
...
static void reconfig_encoder(AVEMCodecContext *ctx, const AVFrame *frame)
{
    X264Context *x4 = ctx->priv_data;
  ...
  if (x4->avcintra_class < 0) {
    ...
    side_data = av_em_frame_get_side_data(frame, AV_FRAME_DATA_STEREO3D);
    if (side_data) {
...
static int X264_frame(AVEMCodecContext *ctx, AVEMPacket *pkt, const AVFrame *frame,
                      int *got_packet)
{
    X264Context *x4 = ctx->priv_data;
    x264_nal_t *nal;
    ...
    if (frame) {
        ...
        if (x4->a53_cc) {
            ...
            ret = em_alloc_a53_sei(frame, 0, &sei_data, &sei_size);
            if (ret < 0) {
                av_em_log(ctx, AV_LOG_ERROR, "Not enough memory for closed captions, skipping\n");
            } else if (sei_data) {
                x4->pic.extra_sei.payloads = av_em_mallocz(sizeof(x4->pic.extra_sei.payloads[0]));
                if (x4->pic.extra_sei.payloads == NULL) {
                    av_em_log(ctx, AV_LOG_ERROR, "Not enough memory for closed captions, skipping\n");
                    av_em_free(sei_data);
                } else {
                    x4->pic.extra_sei.sei_free = av_em_free;
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (ret) {
        em_side_data_set_encoder_stats(pkt, (pic_out.i_qpplus1 - 1) * FF_QP2LAMBDA, NULL, 0, pict_type);
...
static av_cold int X264_close(AVEMCodecContext *avctx)
{
...
#define PARSE_X264_OPT(name, var)\
    if (x4->var && x264_param_parse(&x4->params, name, x4->var) < 0) {\
        av_em_log(avctx, AV_LOG_ERROR, "Error parsing option '%s' with value '%s'.\n", name, x4->var);\
        return AVERROR(EINVAL);\
    }

static av_cold int X264_init(AVEMCodecContext *avctx)
{
    X264Context *x4 = avctx->priv_data;
    ...
    if (avctx->global_quality > 0)
        av_em_log(avctx, AV_LOG_WARNING, "-qscale is ignored, -crf is recommended.\n");
    ...
    if (x4->preset || x4->tune)
        if (x264_param_default_preset(&x4->params, x4->preset, x4->tune) < 0) {
            int i;
            av_em_log(avctx, AV_LOG_ERROR, "Error setting preset/tune %s/%s.\n", x4->preset, x4->tune);
            av_em_log(avctx, AV_LOG_INFO, "Possible presets:");
            for (i = 0; x264_preset_names[i]; i++)
                av_em_log(avctx, AV_LOG_INFO, " %s", x264_preset_names[i]);
            av_em_log(avctx, AV_LOG_INFO, "\n");
            av_em_log(avctx, AV_LOG_INFO, "Possible tunes:");
            for (i = 0; x264_tune_names[i]; i++)
                av_em_log(avctx, AV_LOG_INFO, " %s", x264_tune_names[i]);
            av_em_log(avctx, AV_LOG_INFO, "\n");
            return AVERROR(EINVAL);
        }
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (avctx->refs >= 0)
        ...
        if (!strcmp(x4->level, "1b")) {
            ...
            level_id = av_em_strtod(x4->level, &tail) * 10 + 0.5;
            if (*tail)
                level_id = -1;
        }
        if (level_id <= 0)
            av_em_log(avctx, AV_LOG_WARNING, "Failed to parse level\n");
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    /* Allow specifying the x264 profile through AVEMCodecContext. */
    if (!x4->profile)
        switch (avctx->profile) {
        case FF_PROFILE_H264_BASELINE:
            x4->profile = av_em_strdup("baseline");
            break;
        case FF_PROFILE_H264_HIGH:
            x4->profile = av_em_strdup("high");
            break;
        case FF_PROFILE_H264_HIGH_10:
            x4->profile = av_em_strdup("high10");
            break;
        case FF_PROFILE_H264_HIGH_422:
            x4->profile = av_em_strdup("high422");
            break;
        case FF_PROFILE_H264_HIGH_444:
            x4->profile = av_em_strdup("high444");
            break;
        case FF_PROFILE_H264_MAIN:
            x4->profile = av_em_strdup("main");
            break;
        default:
            break;
        }
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (x4->profile)
        if (x264_param_apply_profile(&x4->params, x4->profile) < 0) {
            int i;
            av_em_log(avctx, AV_LOG_ERROR, "Error setting profile %s.\n", x4->profile);
            av_em_log(avctx, AV_LOG_INFO, "Possible profiles:");
            for (i = 0; x264_profile_names[i]; i++)
                av_em_log(avctx, AV_LOG_INFO, " %s", x264_profile_names[i]);
            av_em_log(avctx, AV_LOG_INFO, "\n");
            return AVERROR(EINVAL);
        }
    ...
    av_em_reduce(&sw, &sh, avctx->sample_aspect_ratio.num, avctx->sample_aspect_ratio.den, 4096);
    ...
    if (x4->x264_params) {
        ...
        AVEMDictionaryEntry *en = NULL;

        if (!av_em_dict_parse_string(&dict, x4->x264_params, "=", ":", 0)) {
            while ((en = av_em_dict_get(dict, "", en, AV_DICT_IGNORE_SUFFIX))) {
    ...
    // update AVEMCodecContext with x264 parameters
    ...
    if (avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER) {
        ...
        avctx->extradata = p = av_em_mallocz(s + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!p)
        ...
        for (i = 0; i < nnal; i++) {
            ...
            if (nal[i].i_type == NAL_SEI) {
                ...
                x4->sei      = av_em_alloc(x4->sei_size);
                if (!x4->sei)
                    return AVERROR(ENOMEM);
...
static av_cold void X264_init_static(AVEMCodec *codec)
{
    if (x264_bit_depth == 8)
...
static const AVEMCodecDefault x264_defaults[] = {
    { "b",                "0" },
    { "bf",               "-1" },
    { "flags2",           "0" },
    { "g",                "-1" },
...
static const AVEMClass x264_class = {
    .class_name = "libx264",
...
#if CONFIG_LIBX264RGB_ENCODER
static const AVEMClass rgbclass = {
...
AVEMCodec ff_libx264rgb_encoder = {
    .name           = "libx264rgb",
...
#if CONFIG_LIBX262_ENCODER
static const AVEMClass X262_class = {
    .class_name = "libx262",
    .item_name  = av_em_default_item_name,
    .option     = options,
    .version    = LIBAVUTIL_VERSION_INT,
