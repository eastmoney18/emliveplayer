...
static av_cold int avui_encode_init(AVEMCodecContext *avctx)
{
    if (avctx->width != 720 || avctx->height != 486 && avctx->height != 576) {
        av_em_log(avctx, AV_LOG_ERROR, "Only 720x486 and 720x576 are supported.\n");
        return AVERROR(EINVAL);
    }
    if (!(avctx->extradata = av_em_mallocz(144 + AV_INPUT_BUFFER_PADDING_SIZE)))
...
static int avui_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                             const AVFrame *pic, int *got_packet)
{
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, size, size)) < 0)
        return ret;
    dst = pkt->data;
    if (!interlaced) {
