...
{
    uint8_t rgba[4];
    if (av_em_parse_color(rgba, str, strcspn(str, "\" >"), log_ctx) < 0)
        return -1;
...
static void rstrip_spaces_buf(AVEMBPrint *buf)
{
    while (buf->len > 0 && buf->str[buf->len - 1] == ' ')
        buf->str[--buf->len] = 0;
}

void em_htmlmarkup_to_ass(void *log_ctx, AVEMBPrint *dst, const char *in)
{
            ...
            if (sscanf(in+tag_close+1, "%127[^>]>%n", buffer, &len) >= 1 && len > 0) {
                ...
                if ((!tag_close && sptr < FF_ARRAY_ELEMS(stack)) ||
                        ...
                        if (tag_close) {
                                ...
                                if (stack[sptr-1].param[i][0])
                                    for (j=sptr-2; j>=0; j--)
                                        if (stack[j].param[i][0]) {
                                            av_em_bprintf(dst, "%s", stack[j].param[i]);
                                            break;
                                        }
                        } else {
                            while (param) {
                            ...
                            for (i=0; i<PARAM_NUMBER; i++)
                                if (stack[sptr].param[i][0])
                                    av_em_bprintf(dst, "%s", stack[sptr].param[i]);
                        }
                    } else if (!tagname[1] && strspn(tagname, "bisu") == 1) {
                        av_em_bprintf(dst, "{\\%c%d}", tagname[0], !tag_close);
                    } else {
                        unknown = 1;
                    ...
                    if (tag_close) {
                        ...
                        av_em_strlcpy(stack[sptr++].tag, tagname,
                                   sizeof(stack[0].tag));
                    break;
                }
            }
