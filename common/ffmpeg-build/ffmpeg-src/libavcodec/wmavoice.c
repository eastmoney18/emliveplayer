...
static av_cold void wmavoice_init_static_data(AVEMCodec *codec)
{
 ...
 */
static av_cold int wmavoice_decode_init(AVEMCodecContext *ctx)
{
    ...
    if (decode_vbmtree(&s->gb, s->vbm_tree) < 0) {
        av_em_log(ctx, AV_LOG_ERROR, "Invalid VBM tree; broken extradata?\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (pitch_range <= 0) {
        av_em_log(ctx, AV_LOG_ERROR, "Invalid pitch range; broken extradata?\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->block_delta_pitch_hrange <= 0) {
        av_em_log(ctx, AV_LOG_ERROR, "Invalid delta pitch hrange; broken extradata?\n");
        return AVERROR_INVALIDDATA;
    }
                           ...
                           const float *in, float *out, int size)
{
    ...
    do {
        dot = avpriv_em_scalarproduct_float_c(in, ptr, size);
        if (dot > optimal_gain) {
            optimal_gain  = dot;
            best_hist_ptr = ptr;
        }
    ...
    if (optimal_gain <= 0)
        return -1;
    dot = avpriv_em_scalarproduct_float_c(best_hist_ptr, best_hist_ptr, size);
...
static float tilt_factor(const float *lpcs, int n_lpcs)
{
    ...
    rh0 = 1.0     + avpriv_em_scalarproduct_float_c(lpcs,  lpcs,    n_lpcs);
    rh1 = lpcs[0] + avpriv_em_scalarproduct_float_c(lpcs, &lpcs[1], n_lpcs - 1);
...
#undef log_range
    ...
    sq = (1.0 / 64.0) * sqrtf(1 / avpriv_em_scalarproduct_float_c(coeffs, coeffs,
                                                               remainder));
                                ...
                                float *excitation)
{
    ...
    idx = get_bits(gb, 7);
    fcb_gain = expf(avpriv_em_scalarproduct_float_c(s->gain_pred_err,
                                                 gain_coeff, 6) -
    ...
    if (frame_desc->acb_type == ACB_TYPE_ASYMMETRIC) {
        ...
        if (idx) {
            ...
            av_em_memcpy_backptr((uint8_t *) excitation, sizeof(float) * block_pitch,
                              sizeof(float) * size);
    }
 ...
 * @return 0 on success, <0 on error.
 */
static int synth_frame(AVEMCodecContext *ctx, GetBitContext *gb, int frame_idx,
                       float *samples,
                       const double *lsps, const double *prev_lsps,
                       float *excitation, float *synth)
{
 ...
 */
static int synth_superframe(AVEMCodecContext *ctx, AVFrame *frame,
                            int *got_frame_ptr)
{
    WMAVoiceContext *s = ctx->priv_data;
     ...
     * the wild yet. */
    if (!get_bits1(gb)) {
        avpriv_em_request_sample(ctx, "WMAPro-in-WMAVoice");
    ...
    if ((res = em_get_buffer(ctx, frame, 0)) < 0)
        return res;
 ...
 */
static int wmavoice_decode_packet(AVEMCodecContext *ctx, void *data,
...
static av_cold int wmavoice_decode_end(AVEMCodecContext *ctx)
{
    WMAVoiceContext *s = ctx->priv_data;
...
static av_cold void wmavoice_flush(AVEMCodecContext *ctx)
{
    WMAVoiceContext *s = ctx->priv_data;
    int n;
