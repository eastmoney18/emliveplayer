...
static av_cold int bfi_decode_init(AVEMCodecContext *avctx)
{
    BFIContext *bfi = avctx->priv_data;
    avctx->pix_fmt  = AV_PIX_FMT_PAL8;
    bfi->dst        = av_em_mallocz(avctx->width * avctx->height);
    if (!bfi->dst)
        return AVERROR(ENOMEM);
    return 0;
}

static int bfi_decode_frame(AVEMCodecContext *avctx, void *data,
                            int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (!avctx->frame_number) {
        ...
        if (avctx->extradata_size > 768) {
            av_em_log(NULL, AV_LOG_ERROR, "Palette is too large.\n");
    ...
    while (dst != frame_end) {
        ...
        case 0:                // normal chain
            if (length >= bytestream2_get_bytes_left(&g)) {
                av_em_log(avctx, AV_LOG_ERROR, "Frame larger than buffer.\n");
                return AVERROR_INVALIDDATA;
            }
...
static av_cold int bfi_decode_close(AVEMCodecContext *avctx)
{
    BFIContext *bfi = avctx->priv_data;
    av_em_freep(&bfi->dst);
    return 0;
