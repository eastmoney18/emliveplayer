...
static enum AVPixelFormat h263_get_format(AVEMCodecContext *avctx)
{
    ...
    return avctx->pix_fmt = em_get_format(avctx, avctx->codec->pix_fmts);
}

av_cold int em_h263_decode_init(AVEMCodecContext *avctx)
{
    ...
    case AV_CODEC_ID_FLV1:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported codec %d\n",
               avctx->codec->id);
...
av_cold int em_h263_decode_end(AVEMCodecContext *avctx)
{
    MpegEncContext *s = avctx->priv_data;
...
static int decode_slice(MpegEncContext *s)
{
    ...
    if (s->partitioned_frame) {
        ...
        if (CONFIG_MPEG4_DECODER && s->codec_id == AV_CODEC_ID_MPEG4)
            if ((ret = em_mpeg4_decode_partitions(s->avctx->priv_data)) < 0)
                return ret;
    ...
    for (; s->mb_y < s->mb_height; s->mb_y++) {
        ...
        for (; s->mb_x < s->mb_width; s->mb_x++) {
            ...
            if (ret < 0) {
                ...
                av_em_log(s->avctx, AV_LOG_ERROR, "Error at MB: %d\n", xy);
                ff_em_er_add_slice(&s->er, s->resync_mb_x, s->resync_mb_y,
    ...
    if (s->msmpeg4_version || (s->workaround_bugs & FF_BUG_NO_PADDING)) { // FIXME perhaps solve this more cleanly
        ...
        if (left > max_extra)
            ...
            av_em_log(s->avctx, AV_LOG_ERROR, "overreading %d bits\n", -left);
        else
            ff_em_er_add_slice(&s->er, s->resync_mb_x, s->resync_mb_y,
                            s->mb_x - 1, s->mb_y, ER_MB_END);
    ...
    ff_em_er_add_slice(&s->er, s->resync_mb_x, s->resync_mb_y, s->mb_x, s->mb_y,
                    ER_MB_END & part_mask);

    return AVERROR_INVALIDDATA;
}

int em_h263_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                         AVEMPacket *avpkt)
{
    ...
    if (s->avctx->flags & AV_CODEC_FLAG_TRUNCATED) {
        ...
        if (CONFIG_MPEG4_DECODER && s->codec_id == AV_CODEC_ID_MPEG4) {
            next = em_mpeg4_find_frame_end(&s->parse_context, buf, buf_size);
        } else if (CONFIG_H263_DECODER && s->codec_id == AV_CODEC_ID_H263) {
            next = em_h263_find_frame_end(&s->parse_context, buf, buf_size);
        ...
        if (em_combine_frame(&s->parse_context, next, (const uint8_t **)&buf,
                             &buf_size) < 0)
            return buf_size;
    }
...
retry:
    if (s->divx_packed && s->bitstream_buffer_size) {
        ...
        for(i=0; i < buf_size-3; i++) {
            if (buf[i]==0 && buf[i+1]==0 && buf[i+2]==1) {
                if (buf[i+3]==0xB0) {
                    av_em_log(s->avctx, AV_LOG_WARNING, "Discarding excessive bitstream in packed xvid\n");
    ...
    if (CONFIG_WMV2_DECODER && s->msmpeg4_version == 5) {
        ...
        if (s->avctx->extradata_size && s->picture_number == 0) {
            ...
            if (init_get_bits8(&gb, s->avctx->extradata, s->avctx->extradata_size) >= 0 )
                em_mpeg4_decode_picture_header(avctx->priv_data, &gb);
        }
        ret = em_mpeg4_decode_picture_header(avctx->priv_data, &s->gb);
    } else if (CONFIG_H263I_DECODER && s->codec_id == AV_CODEC_ID_H263I) {
        ret = ff_intel_h263_decode_picture_header(s);
    } else if (CONFIG_FLV_DECODER && s->h263_flv) {
        ret = em_flv_decode_picture_header(s);
    } else {
        ret = em_h263_decode_picture_header(s);
    }

    if (ret < 0 || ret == FRAME_SKIPPED) {
        if (   s->width  != avctx->coded_width
            || s->height != avctx->coded_height) {
                av_em_log(s->avctx, AV_LOG_WARNING, "Reverting picture dimensions change due to header decoding failure\n");
    ...
    if (ret < 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "header damaged\n");
        return ret;
    }
    ...
    if (CONFIG_MPEG4_DECODER && avctx->codec_id == AV_CODEC_ID_MPEG4) {
        if (em_mpeg4_workaround_bugs(avctx) == 1)
            goto retry;
    }
    ...
    if (s->width  != avctx->coded_width  ||
        ...
        if (avctx->pix_fmt != h263_get_format(avctx)) {
            av_em_log(avctx, AV_LOG_ERROR, "format change not supported\n");
...
frame_end:
    ...
    if (CONFIG_MPEG4_DECODER && avctx->codec_id == AV_CODEC_ID_MPEG4)
        em_mpeg4_frame_end(avctx, buf, buf_size);
...
const enum AVPixelFormat em_h263_hwaccel_pixfmt_list_420[] = {
...
AVEMCodec em_h263_decoder = {
    .name           = "h263",
    .long_name      = NULL_IF_CONFIG_SMALL("H.263 / H.263-1996, H.263+ / H.263-1998 / H.263 version 2"),
    .type           = AVMEDIA_TYPE_VIDEO,
    .id             = AV_CODEC_ID_H263,
    .priv_data_size = sizeof(MpegEncContext),
    .init           = em_h263_decode_init,
