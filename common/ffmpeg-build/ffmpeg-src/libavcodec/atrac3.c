...
static int decode_bytes(const uint8_t *input, uint8_t *out, int bytes)
{
    ...
    if (off)
        avpriv_em_request_sample(NULL, "Offset of %d", off);
...
static av_cold int atrac3_decode_close(AVEMCodecContext *avctx)
{
                                     ...
                                     int channel_num, int coding_mode)
{
    ...
    if (coding_mode == JOINT_STEREO && channel_num == 1) {
        if (get_bits(gb, 2) != 3) {
            av_em_log(NULL,AV_LOG_ERROR,"JS mono Sound Unit id != 3.\n");
            return AVERROR_INVALIDDATA;
        }
    } else {
        if (get_bits(gb, 6) != 0x28) {
            av_em_log(NULL,AV_LOG_ERROR,"Sound Unit id != 0x28.\n");
            return AVERROR_INVALIDDATA;
        }
    }
...
static int decode_frame(AVEMCodecContext *avctx, const uint8_t *databuf,
                        float **out_samples)
{
    ATRAC3Context *q = avctx->priv_data;
    int ret, i;
...
static int atrac3_decode_frame(AVEMCodecContext *avctx, void *data,
                               int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (ret) {
        av_em_log(avctx, AV_LOG_ERROR, "Frame decoding error!\n");
        return ret;
    }
...
static av_cold int atrac3_decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->channels <= 0 || avctx->channels > 2) {
        av_em_log(avctx, AV_LOG_ERROR, "Channel configuration error!\n");
    ...
    if (avctx->extradata_size == 14) {
        ...
        if (avctx->block_align !=  96 * avctx->channels * frame_factor &&
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Unknown frame/channel/frame_factor "
                   "configuration %d/%d/%d\n", avctx->block_align,
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Unknown extradata size %d.\n",
    ...
    if (version != 4) {
        av_em_log(avctx, AV_LOG_ERROR, "Version %d != 4.\n", version);
        return AVERROR_INVALIDDATA;
    }

    if (samples_per_frame != SAMPLES_PER_FRAME &&
        samples_per_frame != SAMPLES_PER_FRAME * 2) {
        av_em_log(avctx, AV_LOG_ERROR, "Unknown amount of samples per frame %d.\n",
               samples_per_frame);
        return AVERROR_INVALIDDATA;
    }

    if (delay != 0x88E) {
        av_em_log(avctx, AV_LOG_ERROR, "Unknown amount of delay %x != 0x88E.\n",
               delay);
        return AVERROR_INVALIDDATA;
    }

    if (q->coding_mode == STEREO)
        av_em_log(avctx, AV_LOG_DEBUG, "Normal stereo detected.\n");
    else if (q->coding_mode == JOINT_STEREO) {
        if (avctx->channels != 2) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid coding mode\n");
            return AVERROR_INVALIDDATA;
        }
        av_em_log(avctx, AV_LOG_DEBUG, "Joint stereo detected.\n");
    } else {
        av_em_log(avctx, AV_LOG_ERROR, "Unknown channel coding mode %x!\n",
               q->coding_mode);
        return AVERROR_INVALIDDATA;
    }
    ...
    q->decoded_bytes_buffer = av_em_mallocz(FFALIGN(avctx->block_align, 4) +
    ...
    if ((ret = ff_mdct_init(&q->mdct_ctx, 9, 1, 1.0 / 32768)) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error initializing MDCT\n");
        av_em_freep(&q->decoded_bytes_buffer);
        return ret;
    }
    ...
    q->fdsp = avpriv_em_float_dsp_alloc(avctx->flags & AV_CODEC_FLAG_BITEXACT);

    q->units = av_em_mallocz_array(avctx->channels, sizeof(*q->units));
    if (!q->units || !q->fdsp) {
