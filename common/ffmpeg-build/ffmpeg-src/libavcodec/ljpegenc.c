...
static int ljpeg_encode_bgr(AVEMCodecContext *avctx, PutBitContext *pb,
                            const AVFrame *frame)
{
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    for (y = 0; y < height; y++) {
        ...
        if (pb->buf_end - pb->buf - (put_bits_count(pb) >> 3) < width * 4 * 4) {
            av_em_log(avctx, AV_LOG_ERROR, "encoded frame too large\n");
            return -1;
        }
...
static int ljpeg_encode_yuv(AVEMCodecContext *avctx, PutBitContext *pb,
                            const AVFrame *frame)
{
    LJpegEncContext *s  = avctx->priv_data;
...
static int ljpeg_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                              const AVFrame *pict, int *got_packet)
{
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, max_pkt_size, 0)) < 0)
        return ret;
...
static av_cold int ljpeg_encode_close(AVEMCodecContext *avctx)
{
    LJpegEncContext *s = avctx->priv_data;

    av_em_freep(&s->scratch);

    return 0;
}

static av_cold int ljpeg_encode_init(AVEMCodecContext *avctx)
{
    LJpegEncContext *s = avctx->priv_data;
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    s->scratch = av_em_malloc_array(avctx->width + 1, sizeof(*s->scratch));
    if (!s->scratch)
        goto fail;
...
static const AVEMClass ljpeg_class = {
    .class_name = "ljpeg",
