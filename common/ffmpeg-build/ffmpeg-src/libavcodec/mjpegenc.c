...
av_cold int ff_mjpeg_encode_init(MpegEncContext *s)
{
    ...
    if (s->width > 65500 || s->height > 65500) {
        av_em_log(s, AV_LOG_ERROR, "JPEG does not support resolutions above 65500x65500\n");
        return AVERROR(EINVAL);
    }

    m = av_em_alloc(sizeof(MJpegContext));
    if (!m)
...
#define V_MAX 2
static int amv_encode_picture(AVEMCodecContext *avctx, AVEMPacket *pkt,
...

{
    ...
    av_em_pix_fmt_get_chroma_sub_sample(avctx->pix_fmt, &chroma_h_shift, &chroma_v_shift);
    ...
    if ((avctx->height & 15) && avctx->strict_std_compliance > FF_COMPLIANCE_UNOFFICIAL) {
        ...
        av_em_log(avctx, AV_LOG_WARNING, "If you have a device that plays AMV videos, please test if videos "
               "with such heights work with it and report your findings to ffmpeg-devel@ffmpeg.org\n");
        return AVERROR_EXPERIMENTAL;
    }

    pic = av_em_frame_clone(pic_arg);
    if (!pic)
        return AVERROR(ENOMEM);
...
static const AVEMClass mjpeg_class = {
...
static const AVEMClass amv_class = {
