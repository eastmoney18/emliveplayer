...
static void unpack_intraframe(AVEMCodecContext *avctx, uint8_t *src,
                              uint32_t size)
{
    ...
    while (bytestream2_tell(&gb) < size) {
        if (getbit(&gb, &bitbuf, &bits)) {
            ...
            if (offset < 0 || offset + count >= hnm->width * hnm->height) {
                av_em_log(avctx, AV_LOG_ERROR, "Attempting to read out of bounds\n");
                break;
...
static void postprocess_current_frame(AVEMCodecContext *avctx)
{
    Hnm4VideoContext *hnm = avctx->priv_data;
...
static void copy_processed_frame(AVEMCodecContext *avctx, AVFrame *frame)
{
    Hnm4VideoContext *hnm = avctx->priv_data;
...
static void decode_interframe_v4(AVEMCodecContext *avctx, uint8_t *src, uint32_t size)
{
    Hnm4VideoContext *hnm = avctx->priv_data;
    GetByteContext gb;
        ...
        if (count == 0) {
            ...
            if (tag == 0) {
                if (writeoffset + 2 > hnm->width * hnm->height) {
                    av_em_log(avctx, AV_LOG_ERROR, "writeoffset out of bounds\n");
                    break;
                }
                hnm->current[writeoffset++] = bytestream2_get_byte(&gb);
                hnm->current[writeoffset++] = bytestream2_get_byte(&gb);
...
static void decode_interframe_v4a(AVEMCodecContext *avctx, uint8_t *src,
                                  uint32_t size)
{
    Hnm4VideoContext *hnm = avctx->priv_data;
    GetByteContext gb;
    ...
    while (bytestream2_tell(&gb) < size) {
        ...
        if (count == 0) {
            ...
            if (offset + hnm->width + count >= hnm->width * hnm->height) {
                ...
                av_em_log(avctx, AV_LOG_ERROR, "Attempting to write out of bounds\n");
                break;
            }
...
static void hnm_update_palette(AVEMCodecContext *avctx, uint8_t *src,
                               uint32_t size)
{
    Hnm4VideoContext *hnm = avctx->priv_data;
    GetByteContext gb;
...
static int hnm_decode_frame(AVEMCodecContext *avctx, void *data,
                            int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (avpkt->size < 8) {
        av_em_log(avctx, AV_LOG_ERROR, "packet too small\n");
    ...
    if (chunk_id == HNM4_CHUNK_ID_PL) {
        ...
        *got_frame = 1;
        ...
        av_em_log(avctx, AV_LOG_ERROR, "invalid chunk id: %d\n", chunk_id);
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int hnm_decode_init(AVEMCodecContext *avctx)
{
    Hnm4VideoContext *hnm = avctx->priv_data;
    int ret;
    ...
    hnm->buffer1   = av_em_mallocz(avctx->width * avctx->height);
    hnm->buffer2   = av_em_mallocz(avctx->width * avctx->height);
    hnm->processed = av_em_mallocz(avctx->width * avctx->height);

    if (   !hnm->buffer1 || !hnm->buffer2 || !hnm->processed
        || avctx->width * avctx->height == 0
        || avctx->height % 2) {
        av_em_log(avctx, AV_LOG_ERROR, "av_em_mallocz() failed\n");
        av_em_freep(&hnm->buffer1);
        av_em_freep(&hnm->buffer2);
        av_em_freep(&hnm->processed);
...
static av_cold int hnm_decode_end(AVEMCodecContext *avctx)
{
    Hnm4VideoContext *hnm = avctx->priv_data;
...
AVEMCodec em_hnm4_video_decoder = {
    .name           = "hnm4video",
