...
static int hevc_extradata_to_annexb(AVBSFContext *ctx)
{
    ...
    for (i = 0; i < num_arrays; i++) {
        ...
        if (!(type == NAL_VPS || type == NAL_SPS || type == NAL_PPS ||
            ...
            av_em_log(ctx, AV_LOG_ERROR, "Invalid NAL unit type in extradata: %d\n",
                   type);
        ...
        for (j = 0; j < cnt; j++) {
            ...
            ret = av_em_reallocp(&new_extradata, new_extradata_size + nalu_len + 4 + AV_INPUT_BUFFER_PADDING_SIZE);
            if (ret < 0)
                goto fail;
    ...
    if (!new_extradata_size)
        av_em_log(ctx, AV_LOG_WARNING, "No parameter sets in the extradata\n");

    return length_size;
fail:
    av_em_freep(&new_extradata);
    return ret;
}
...
static int hevc_mp4toannexb_filter(AVBSFContext *ctx, AVEMPacket *out)
{
    HEVCBSFContext *s = ctx->priv_data;
    AVEMPacket *in;
    GetByteContext gb;
    ...
    while (bytestream2_get_bytes_left(&gb)) {
        ...
        ret = av_em_grow_packet(out, 4 + nalu_size + extra_size);
        if (ret < 0)
            goto fail;
...
const AVBitStreamFilter em_hevc_mp4toannexb_bsf = {
