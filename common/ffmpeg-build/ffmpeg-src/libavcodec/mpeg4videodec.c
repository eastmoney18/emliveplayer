 ...
 */
void em_mpeg4_pred_ac(MpegEncContext *s, int16_t *block, int n, int dir)
{
    int i;
...
static inline int mpeg4_is_resync(Mpeg4DecContext *ctx)
{
    ...
    if (bits_count + 8 >= s->gb.size_in_bits) {
        ...
        if (v == em_mpeg4_resync_prefix[bits_count & 7]) {
            int len, mb_num;
            ...
            if (len >= em_mpeg4_get_video_packet_prefix_length(s))
                return mb_num;
        }
    }
    return 0;
 ...
 */
int em_mpeg4_decode_video_packet_header(Mpeg4DecContext *ctx)
{
    MpegEncContext *s = &ctx->m;
    ...
    if (len != em_mpeg4_get_video_packet_prefix_length(s)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "marker does not match f_code\n");
        return -1;
    }
        ...
        if (ctx->shape != BIN_ONLY_SHAPE) {
            ...
            if (s->pict_type == AV_PICTURE_TYPE_S &&
                ...
                if (mpeg4_decode_sprite_trajectory(ctx, &s->gb) < 0)
                    return AVERROR_INVALIDDATA;
                av_em_log(s->avctx, AV_LOG_ERROR, "untested\n");
            }
...
static inline int mpeg4_decode_dc(MpegEncContext *s, int n, int *dir_ptr)
{
    ...
    if (code < 0 || code > 9 /* && s->nbit < 9 */) {
        av_em_log(s->avctx, AV_LOG_ERROR, "illegal dc vlc\n");
        return -1;
    }
        ...
        if (code > 8) {
            if (get_bits1(&s->gb) == 0) { /* marker */
                if (s->avctx->err_recognition & (AV_EF_BITSTREAM|AV_EF_COMPLIANT)) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "dc marker bit missing\n");
                    return -1;
                }
            }
        }
    ...
    return em_mpeg4_pred_dc(s, n, level, dir_ptr, 0);
}
        ...
        for (; s->mb_x < s->mb_width; s->mb_x++) {
            ...
            if (s->pict_type == AV_PICTURE_TYPE_I) {
                ...
                do {
                    ...
                    cbpc = get_vlc2(&s->gb, em_h263_intra_MCBPC_vlc.table, INTRA_MCBPC_VLC_BITS, 2);
                    if (cbpc < 0) {
                        av_em_log(s->avctx, AV_LOG_ERROR,
...
try_again:
                ...
                cbpc = get_vlc2(&s->gb, em_h263_inter_MCBPC_vlc.table, INTER_MCBPC_VLC_BITS, 2);
                if (cbpc < 0) {
                    av_em_log(s->avctx, AV_LOG_ERROR,
                           "mcbpc corrupted at %d %d\n", s->mb_x, s->mb_y);
                    return -1;
                ...
                if (s->mb_intra) {
                    ...
                    if ((cbpc & 16) == 0) {
                        ...
                        em_h263_pred_motion(s, 0, 0, &pred_x, &pred_y);
                        if (!s->mcsel) {
                            mx = em_h263_decode_motion(s, pred_x, s->f_code);
                            if (mx >= 0xffff)
                                return -1;

                            my = em_h263_decode_motion(s, pred_y, s->f_code);
                            if (my >= 0xffff)
                                return -1;
                        ...
                        s->current_picture.mb_type[xy] = MB_TYPE_8x8 |
                                                         MB_TYPE_L0;
                        for (i = 0; i < 4; i++) {
                            int16_t *mot_val = em_h263_pred_motion(s, i, 0, &pred_x, &pred_y);
                            mx = em_h263_decode_motion(s, pred_x, s->f_code);
                            if (mx >= 0xffff)
                                return -1;
    ...
    for (s->mb_y = s->resync_mb_y; mb_num < mb_count; s->mb_y++) {
        ...
        for (; mb_num < mb_count && s->mb_x < s->mb_width; s->mb_x++) {
            ...
            if (s->pict_type == AV_PICTURE_TYPE_I) {
                ...
                int cbpy    = get_vlc2(&s->gb, em_h263_cbpy_vlc.table, CBPY_VLC_BITS, 1);
                if (cbpy < 0) {
                    av_em_log(s->avctx, AV_LOG_ERROR,
 ...
 */
int em_mpeg4_decode_partitions(Mpeg4DecContext *ctx)
{
    MpegEncContext *s = &ctx->m;
    int mb_num;
    ...
    if (mb_num <= 0) {
        ff_em_er_add_slice(&s->er, s->resync_mb_x, s->resync_mb_y,
                        s->mb_x, s->mb_y, part_a_error);
        return -1;
    }

    if (s->resync_mb_x + s->resync_mb_y * s->mb_width + mb_num > s->mb_num) {
        av_em_log(s->avctx, AV_LOG_ERROR, "slice below monitor ...\n");
        ff_em_er_add_slice(&s->er, s->resync_mb_x, s->resync_mb_y,
                        s->mb_x, s->mb_y, part_a_error);
        return -1;
    }
                                     ...
                                     int n, int coded, int intra, int rvlc)
{
    ...
    if (intra) {
        if (ctx->use_intra_dc_vlc) {
            ...
            i = -1;
            em_mpeg4_pred_dc(s, n, 0, &dc_pred_dir, 0);
        }
        if (!coded)
            goto not_coded;
        ...
        if (rvlc) {
            rl     = &em_rvlc_rl_intra;
            rl_vlc = em_rvlc_rl_intra.rl_vlc[0];
        } else {
            rl     = &em_mpeg4_rl_intra;
            rl_vlc = em_mpeg4_rl_intra.rl_vlc[0];
        }
        if (s->ac_pred) {
            if (dc_pred_dir == 0)
        ...
        if (s->mpeg_quant) {
            ...
            qadd = 0;
            if (rvlc)
                rl_vlc = em_rvlc_rl_inter.rl_vlc[0];
            else
                rl_vlc = em_h263_rl_inter.rl_vlc[0];
        } else {
            qmul = s->qscale << 1;
            qadd = (s->qscale - 1) | 1;
            if (rvlc)
                rl_vlc = em_rvlc_rl_inter.rl_vlc[s->qscale];
            else
                rl_vlc = em_h263_rl_inter.rl_vlc[s->qscale];
        }
    }
    {
        OPEN_READER(re, &s->gb);
            ...
            if (level == 0) {
                ...
                if (rvlc) {
                    ...
                    if (SHOW_UBITS(re, &s->gb, 5) != 0x10) {
                        av_em_log(s->avctx, AV_LOG_ERROR, "reverse esc missing\n");
                        return -1;
                    }
                            ...
                            if (s->error_recognition >= FF_ER_COMPLIANT) {
                                ...
                                if (abs_level<=MAX_LEVEL && run<=MAX_RUN) {
                                    ...
                                    if (abs_level <= rl->max_level[last][run]) {
                                        av_em_log(s->avctx, AV_LOG_ERROR, "illegal 3. esc, vlc encoding possible\n");
                                        return -1;
                                    }
                                    if (s->error_recognition > FF_ER_COMPLIANT) {
                                        if (abs_level <= rl->max_level[last][run]*2) {
                                            av_em_log(s->avctx, AV_LOG_ERROR, "illegal 3. esc, esc 1 encoding possible\n");
                                            return -1;
                                        }
                                        if (run1 >= 0 && abs_level <= rl->max_level[last][run1]) {
                                            av_em_log(s->avctx, AV_LOG_ERROR, "illegal 3. esc, esc 2 encoding possible\n");
                                            return -1;
                                        }
                                    }
                                }
...
not_coded:
    if (intra) {
        if (!ctx->use_intra_dc_vlc) {
            block[0] = em_mpeg4_pred_dc(s, n, block[0], &dc_pred_dir, 0);

            i -= i >> 31;  // if (i == -1) i = 0;
        }

        em_mpeg4_pred_ac(s, block, n, dc_pred_dir);
        if (s->ac_pred)
...
static int mpeg4_decode_mb(MpegEncContext *s, int16_t block[6][64])
{
    ...
    if (s->pict_type == AV_PICTURE_TYPE_P ||
        ...
        if (s->pict_type == AV_PICTURE_TYPE_S &&
            ...
            s->mcsel = 0;
        cbpy = get_vlc2(&s->gb, em_h263_cbpy_vlc.table, CBPY_VLC_BITS, 1) ^ 0x0F;
        if (cbpy < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR,
        ...
        if ((cbpc & 16) == 0) {
            if (s->mcsel) {
                ...
                for (i = 0; i < 2; i++) {
                    ...
                    my = em_h263_decode_motion(s, pred_y / 2, s->f_code);
                    if (my >= 0xffff)
                        return -1;
            ...
            s->mv_type                     = MV_TYPE_8X8;
            for (i = 0; i < 4; i++) {
                mot_val = em_h263_pred_motion(s, i, 0, &pred_x, &pred_y);
                mx      = em_h263_decode_motion(s, pred_x, s->f_code);
                if (mx >= 0xffff)
                    return -1;
        ...
        if (s->mb_x == 0) {
            ...
            em_thread_await_progress(&s->next_picture_ptr->tf, s->mb_y, 0);
        }
        ...
        if (modb1) {
            ...
            if (mb_type < 0) {
                av_em_log(s->avctx, AV_LOG_ERROR, "illegal MB_type\n");
                return -1;
            }
            ...
            if ((mb_type & (MB_TYPE_DIRECT2 | MB_TYPE_INTERLACED)) == 0) {
                ...
                if (USES_LIST(mb_type, 0)) {
                    ...
                    mx = em_h263_decode_motion(s, s->last_mv[0][0][0], s->f_code);
                    my = em_h263_decode_motion(s, s->last_mv[0][0][1], s->f_code);
                    s->last_mv[0][1][0] =
                    s->last_mv[0][0][0] =
                    s->mv[0][0][0]      = mx;
                    s->last_mv[0][1][1] =
                ...
                if (USES_LIST(mb_type, 1)) {
                    ...
                    mx = em_h263_decode_motion(s, s->last_mv[1][0][0], s->b_code);
                    my = em_h263_decode_motion(s, s->last_mv[1][0][1], s->b_code);
                    s->last_mv[1][1][0] =
                    s->last_mv[1][0][0] =
                    s->mv[1][0][0]      = mx;
                    s->last_mv[1][1][1] =
                ...
                if (USES_LIST(mb_type, 0)) {
                    ...
                    for (i = 0; i < 2; i++) {
                        mx = em_h263_decode_motion(s, s->last_mv[0][i][0], s->f_code);
                        my = em_h263_decode_motion(s, s->last_mv[0][i][1] / 2, s->f_code);
                        s->last_mv[0][i][0] =
                        s->mv[0][i][0]      = mx;
                ...
                if (USES_LIST(mb_type, 1)) {
                    ...
                    for (i = 0; i < 2; i++) {
                        mx = em_h263_decode_motion(s, s->last_mv[1][i][0], s->b_code);
                        my = em_h263_decode_motion(s, s->last_mv[1][i][1] / 2, s->b_code);
                        s->last_mv[1][i][0] =
                        s->mv[1][i][0]      = mx;
        ...
        if (IS_DIRECT(mb_type)) {
            if (IS_SKIP(mb_type)) {
                ...
                mx = em_h263_decode_motion(s, 0, 1);
                my = em_h263_decode_motion(s, 0, 1);
            }

            s->mv_dir = MV_DIR_FORWARD | MV_DIR_BACKWARD | MV_DIRECT;
            mb_type  |= em_mpeg4_set_direct_mv(s, mx, my);
        }
...
intra:
        ...
        cbpy = get_vlc2(&s->gb, em_h263_cbpy_vlc.table, CBPY_VLC_BITS, 1);
        if (cbpy < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR,
    ...
    if (s->codec_id == AV_CODEC_ID_MPEG4) {
        ...
        if (next) {
            ...
            if (s->pict_type == AV_PICTURE_TYPE_B) {
                const int delta= s->mb_x + 1 == s->mb_width ? 2 : 1;
                em_thread_await_progress(&s->next_picture_ptr->tf,
...
static int mpeg4_decode_gop_header(MpegEncContext *s, GetBitContext *gb)
{
    ...
    if (!show_bits(gb, 23)) {
        av_em_log(s->avctx, AV_LOG_WARNING, "GOP header invalid\n");
        return -1;
    }
...
static int decode_vol_header(Mpeg4DecContext *ctx, GetBitContext *gb)
{
    ...
    if (s->aspect_ratio_info == FF_ASPECT_EXTENDED) {
        ...
        s->avctx->sample_aspect_ratio = em_h263_pixel_aspect[s->aspect_ratio_info];
    }

    if ((ctx->vol_control_parameters = get_bits1(gb))) { /* vol control parameter */
        int chroma_format = get_bits(gb, 2);
        if (chroma_format != CHROMA_420)
            av_em_log(s->avctx, AV_LOG_ERROR, "illegal chroma format\n");
    ...
    if (ctx->shape != RECT_SHAPE)
        av_em_log(s->avctx, AV_LOG_ERROR, "only rectangular vol supported\n");
    if (ctx->shape == GRAY_SHAPE && vo_ver_id != 1) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Gray shape not supported\n");
    ...
    if (!s->avctx->framerate.num) {
        av_em_log(s->avctx, AV_LOG_ERROR, "framerate==0\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    s->avctx->time_base = av_em_inv_q(av_em_mul_q(s->avctx->framerate, (AVEMRational){s->avctx->ticks_per_frame, 1}));
    ...
    if (ctx->shape != BIN_ONLY_SHAPE) {
        ...
        if (!get_bits1(gb) && (s->avctx->debug & FF_DEBUG_PICT_INFO))
            av_em_log(s->avctx, AV_LOG_INFO,           /* OBMC Disable */
        ...
        if (ctx->vol_sprite_usage == STATIC_SPRITE)
            av_em_log(s->avctx, AV_LOG_ERROR, "Static Sprites not supported\n");
        if (ctx->vol_sprite_usage == STATIC_SPRITE ||
            ctx->vol_sprite_usage == GMC_SPRITE) {
            if (ctx->vol_sprite_usage == STATIC_SPRITE) {
        ...
        if (get_bits1(gb) == 1) {                   /* not_8_bit */
            ...
            if (get_bits(gb, 4) != 8)               /* bits_per_pixel */
                av_em_log(s->avctx, AV_LOG_ERROR, "N-bit not supported\n");
        ...
        if ((s->mpeg_quant = get_bits1(gb))) { /* vol_quant_type */
            ...
            for (i = 0; i < 64; i++) {
                ...
                v = em_mpeg4_default_intra_matrix[i];
                s->intra_matrix[j]        = v;
                s->chroma_intra_matrix[j] = v;

                v = em_mpeg4_default_non_intra_matrix[i];
                s->inter_matrix[j]        = v;
            ...
            if (get_bits1(gb)) {
                ...
                for (i = 0; i < 64; i++) {
                    ...
                    if (get_bits_left(gb) < 8) {
                        av_em_log(s->avctx, AV_LOG_ERROR, "insufficient data for custom matrix\n");
                        return AVERROR_INVALIDDATA;
                    }
                    v = get_bits(gb, 8);
                    if (v == 0)
        ...
        if (get_bits_left(gb) < 4) {
            av_em_log(s->avctx, AV_LOG_ERROR, "VOL Header truncated\n");
            return AVERROR_INVALIDDATA;
        }
...
no_cplx_est:
        ...
        if (vo_ver_id != 1) {
            ...
            if (ctx->new_pred) {
                av_em_log(s->avctx, AV_LOG_ERROR, "new pred not supported\n");
        ...
        if (ctx->scalability) {
            ...
            if (h_sampling_factor_n == 0 || h_sampling_factor_m == 0 ||
                ...
                *gb            = bak;
                ...
                av_em_log(s->avctx, AV_LOG_ERROR, "scalability not supported\n");
    ...
    if (s->avctx->debug&FF_DEBUG_PICT_INFO) {
        av_em_log(s->avctx, AV_LOG_DEBUG, "tb %d/%d, tincrbits:%d, qp_prec:%d, ps:%d, low_delay:%d  %s%s%s%s\n",
...
int em_mpeg4_workaround_bugs(AVEMCodecContext *avctx)
{
...
static int decode_vop_header(Mpeg4DecContext *ctx, GetBitContext *gb)
{
    ...
    if (s->pict_type == AV_PICTURE_TYPE_B && s->low_delay &&
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "low_delay flag set incorrectly, clearing it\n");
        s->low_delay = 0;
    }
    ...
    if (get_bits1(gb) != 1) {
        if (s->avctx->debug & FF_DEBUG_PICT_INFO)
            av_em_log(s->avctx, AV_LOG_ERROR, "vop not coded\n");
        return FRAME_SKIPPED;
    }
    if (ctx->new_pred)
    ...
    if (ctx->shape != BIN_ONLY_SHAPE) {
        ...
        if (get_bits_left(gb) < 3) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Header truncated\n");
            return AVERROR_INVALIDDATA;
        }
        ctx->intra_dc_threshold = em_mpeg4_dc_threshold[get_bits(gb, 3)];
        if (!s->progressive_sequence) {
    ...
    if (s->pict_type == AV_PICTURE_TYPE_S &&
        ...
        if (ctx->vol_sprite_usage == STATIC_SPRITE)
            av_em_log(s->avctx, AV_LOG_ERROR, "static sprite not supported\n");
    }
    ...
    s->y_dc_scale_table = em_mpeg4_y_dc_scale_table;
    s->c_dc_scale_table = em_mpeg4_c_dc_scale_table;
 ...
 *         0 if a VOP is found
 */
int em_mpeg4_decode_picture_header(Mpeg4DecContext *ctx, GetBitContext *gb)
{
    MpegEncContext *s = &ctx->m;
    ...
    for (;;) {
        if (get_bits_count(gb) >= gb->size_in_bits) {
            if (gb->size_in_bits == 8 &&
                ...
                av_em_log(s->avctx, AV_LOG_VERBOSE, "frame skip %d\n", gb->size_in_bits);
        ...
        if (s->avctx->debug & FF_DEBUG_STARTCODE) {
            av_em_log(s->avctx, AV_LOG_DEBUG, "startcode: %3X ", startcode);
            if (startcode <= 0x11F)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Video Object Start");
            else if (startcode <= 0x12F)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Video Object Layer Start");
            else if (startcode <= 0x13F)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Reserved");
            else if (startcode <= 0x15F)
                av_em_log(s->avctx, AV_LOG_DEBUG, "FGS bp start");
            else if (startcode <= 0x1AF)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Reserved");
            else if (startcode == 0x1B0)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Visual Object Seq Start");
            else if (startcode == 0x1B1)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Visual Object Seq End");
            else if (startcode == 0x1B2)
                av_em_log(s->avctx, AV_LOG_DEBUG, "User Data");
            else if (startcode == 0x1B3)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Group of VOP start");
            else if (startcode == 0x1B4)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Video Session Error");
            else if (startcode == 0x1B5)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Visual Object Start");
            else if (startcode == 0x1B6)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Video Object Plane start");
            else if (startcode == 0x1B7)
                av_em_log(s->avctx, AV_LOG_DEBUG, "slice start");
            else if (startcode == 0x1B8)
                av_em_log(s->avctx, AV_LOG_DEBUG, "extension start");
            else if (startcode == 0x1B9)
                av_em_log(s->avctx, AV_LOG_DEBUG, "fgs start");
            else if (startcode == 0x1BA)
                av_em_log(s->avctx, AV_LOG_DEBUG, "FBA Object start");
            else if (startcode == 0x1BB)
                av_em_log(s->avctx, AV_LOG_DEBUG, "FBA Object Plane start");
            else if (startcode == 0x1BC)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Mesh Object start");
            else if (startcode == 0x1BD)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Mesh Object Plane start");
            else if (startcode == 0x1BE)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Still Texture Object start");
            else if (startcode == 0x1BF)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Texture Spatial Layer start");
            else if (startcode == 0x1C0)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Texture SNR Layer start");
            else if (startcode == 0x1C1)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Texture Tile start");
            else if (startcode == 0x1C2)
                av_em_log(s->avctx, AV_LOG_DEBUG, "Texture Shape Layer start");
            else if (startcode == 0x1C3)
                av_em_log(s->avctx, AV_LOG_DEBUG, "stuffing start");
            else if (startcode <= 0x1C5)
                av_em_log(s->avctx, AV_LOG_DEBUG, "reserved");
            else if (startcode <= 0x1FF)
                av_em_log(s->avctx, AV_LOG_DEBUG, "System start");
            av_em_log(s->avctx, AV_LOG_DEBUG, " at %d\n", get_bits_count(gb));
        }
...
av_cold void em_mpeg4videodec_static_init(void) {
    static int done = 0;

    if (!done) {
        ff_rl_init(&em_mpeg4_rl_intra, em_mpeg4_static_rl_table_store[0]);
        ff_rl_init(&em_rvlc_rl_inter, em_mpeg4_static_rl_table_store[1]);
        ff_rl_init(&em_rvlc_rl_intra, em_mpeg4_static_rl_table_store[2]);
        INIT_VLC_RL(em_mpeg4_rl_intra, 554);
        INIT_VLC_RL(em_rvlc_rl_inter, 1072);
        INIT_VLC_RL(em_rvlc_rl_intra, 1072);
        INIT_VLC_STATIC(&dc_lum, DC_VLC_BITS, 10 /* 13 */,
                        &em_mpeg4_DCtab_lum[0][1], 2, 1,
                        &em_mpeg4_DCtab_lum[0][0], 2, 1, 512);
        INIT_VLC_STATIC(&dc_chrom, DC_VLC_BITS, 10 /* 13 */,
                        &em_mpeg4_DCtab_chrom[0][1], 2, 1,
                        &em_mpeg4_DCtab_chrom[0][0], 2, 1, 512);
        INIT_VLC_STATIC(&sprite_trajectory, SPRITE_TRAJ_VLC_BITS, 15,
                        &em_sprite_trajectory_tab[0][1], 4, 2,
                        &em_sprite_trajectory_tab[0][0], 4, 2, 128);
        INIT_VLC_STATIC(&mb_type_b_vlc, MB_TYPE_B_VLC_BITS, 4,
                        &em_mb_type_b_tab[0][1], 2, 1,
                        &em_mb_type_b_tab[0][0], 2, 1, 16);
        done = 1;
    }
}

int em_mpeg4_frame_end(AVEMCodecContext *avctx, const uint8_t *buf, int buf_size)
{
    Mpeg4DecContext *ctx = avctx->priv_data;
    MpegEncContext    *s = &ctx->m;
...
#if HAVE_THREADS
static int mpeg4_update_thread_context(AVEMCodecContext *dst,
                                       const AVEMCodecContext *src)
{
    ...
    if (CONFIG_MPEG4_DECODER && !init && s1->xvid_build >= 0)
        em_xvid_idct_init(&s->m.idsp, dst);

    return 0;
}
#endif

static av_cold int decode_init(AVEMCodecContext *avctx)
{
    Mpeg4DecContext *ctx = avctx->priv_data;
    MpegEncContext *s = &ctx->m;
    int ret;
    ...
    if ((ret = em_h263_decode_init(avctx)) < 0)
        return ret;

    em_mpeg4videodec_static_init();
...
static const AVEMClass mpeg4_class = {
    "MPEG4 Video Decoder",
    av_em_default_item_name,
    mpeg4_options,
    LIBAVUTIL_VERSION_INT,
};

AVEMCodec em_mpeg4_decoder = {
    .name                  = "mpeg4",
    ...
    .init                  = decode_init,
    .close                 = em_h263_decode_end,
    .decode                = em_h263_decode_frame,
    .capabilities          = AV_CODEC_CAP_DRAW_HORIZ_BAND | AV_CODEC_CAP_DR1 |
                             AV_CODEC_CAP_TRUNCATED | AV_CODEC_CAP_DELAY |
                             AV_CODEC_CAP_FRAME_THREADS,
    .flush                 = ff_mpeg_flush,
    .max_lowres            = 3,
    .pix_fmts              = em_h263_hwaccel_pixfmt_list_420,
    .profiles              = NULL_IF_CONFIG_SMALL(em_mpeg4_video_profiles),
...
static const AVEMClass mpeg4_vdpau_class = {
...
AVEMCodec em_mpeg4_vdpau_decoder = {
    .name           = "mpeg4_vdpau",
