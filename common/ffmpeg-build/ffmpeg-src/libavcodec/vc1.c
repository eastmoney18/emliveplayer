        ...
        if (!(height % 3) && (width % 3)) { // use 2x3 decoding
            for (y = 0; y < height; y += 3) {
                for (x = width & 1; x < width; x += 2) {
                    ...
                    if (code < 0) {
                        av_em_log(v->s.avctx, AV_LOG_DEBUG, "invalid NORM-6 VLC\n");
                        return -1;
                    }
                    planep[x + 0]              = (code >> 0) & 1;
                    planep[x + 1]              = (code >> 1) & 1;
 ...
 * @return Status
 */
int ff_vc1_decode_sequence_header(AVEMCodecContext *avctx, VC1Context *v, GetBitContext *gb)
{
    av_em_log(avctx, AV_LOG_DEBUG, "Header: %0X\n", show_bits_long(gb, 32));
    v->profile = get_bits(gb, 2);
    if (v->profile == PROFILE_COMPLEX) {
        av_em_log(avctx, AV_LOG_WARNING, "WMV3 Complex Profile is not fully supported\n");
    }
    ...
    if (v->res_sprite) {
        ...
        int ret = em_set_dimensions(v->s.avctx, w, h);
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Failed to set dimensions %d %d\n", w, h);
            return ret;
        }
        skip_bits(gb, 5); //frame rate
        v->res_x8 = get_bits1(gb);
        if (get_bits1(gb)) { // something to do with DC VLC selection
            av_em_log(avctx, AV_LOG_ERROR, "Unsupported sprite feature\n");
            return -1;
        }
...
static int decode_sequence_header_adv(VC1Context *v, GetBitContext *gb)
{
    ...
    if (v->level >= 5) {
        av_em_log(v->s.avctx, AV_LOG_ERROR, "Reserved LEVEL %i\n",v->level);
    }
    v->chromaformat = get_bits(gb, 2);
    if (v->chromaformat != 1) {
        av_em_log(v->s.avctx, AV_LOG_ERROR,
    ...
    av_em_log(v->s.avctx, AV_LOG_DEBUG,
    ...
    v->psf = get_bits1(gb);
    if (v->psf) { //PsF, 6.1.13
        av_em_log(v->s.avctx, AV_LOG_ERROR, "Progressive Segmented Frame mode: not supported (yet)\n");
        return -1;
    }
    v->s.max_b_frames = v->s.avctx->max_b_frames = 7;
    if (get_bits1(gb)) { //Display Info - decoding is not affected by it
        int w, h, ar = 0;
        av_em_log(v->s.avctx, AV_LOG_DEBUG, "Display extended info:\n");
        w = get_bits(gb, 14) + 1;
        h = get_bits(gb, 14) + 1;
        av_em_log(v->s.avctx, AV_LOG_DEBUG, "Display dimensions: %ix%i\n", w, h);
        if (get_bits1(gb))
            ar = get_bits(gb, 4);
        if (ar && ar < 14) {
            ...
            w = get_bits(gb, 8) + 1;
            h = get_bits(gb, 8) + 1;
            v->s.avctx->sample_aspect_ratio = (AVEMRational){w, h};
        } else {
            av_em_reduce(&v->s.avctx->sample_aspect_ratio.num,
                      &v->s.avctx->sample_aspect_ratio.den,
                      v->s.avctx->height * w,
                      v->s.avctx->width * h,
                      1 << 30);
        }
        em_set_sar(v->s.avctx, v->s.avctx->sample_aspect_ratio);
        av_em_log(v->s.avctx, AV_LOG_DEBUG, "Aspect: %i:%i\n",
...
int ff_vc1_decode_entry_point(AVEMCodecContext *avctx, VC1Context *v, GetBitContext *gb)
{
    int i;
    int w,h;
    int ret;

    av_em_log(avctx, AV_LOG_DEBUG, "Entry point: %08X\n", show_bits_long(gb, 32));
    ...
    if ((ret = em_set_dimensions(avctx, w, h)) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to set dimensions %d %d\n", w, h);
        return ret;
    }
    ...
    if ((v->range_mapy_flag = get_bits1(gb))) {
        av_em_log(avctx, AV_LOG_ERROR, "Luma scaling is not supported, expect wrong picture\n");
        v->range_mapy = get_bits(gb, 3);
    }
    if ((v->range_mapuv_flag = get_bits1(gb))) {
        av_em_log(avctx, AV_LOG_ERROR, "Chroma scaling is not supported, expect wrong picture\n");
        v->range_mapuv = get_bits(gb, 3);
    }

    av_em_log(avctx, AV_LOG_DEBUG, "Entry point info:\n"
...
static int read_bfraction(VC1Context *v, GetBitContext* gb) {
    ...
    if (bfraction_lut_index == 21 || bfraction_lut_index < 0) {
        av_em_log(v->s.avctx, AV_LOG_ERROR, "bfraction invalid\n");
...
int ff_vc1_parse_frame_header(VC1Context *v, GetBitContext* gb)
{
    ...
    case AV_PICTURE_TYPE_P:
        ...
        if ((v->mv_mode  == MV_PMODE_INTENSITY_COMP &&
            ...
            if (status < 0)
                return -1;
            av_em_log(v->s.avctx, AV_LOG_DEBUG, "MB MV Type plane encoding: "
                   "Imode: %i, Invert: %i\n", status>>1, status&1);
        } else {
            v->mv_type_is_raw = 0;
        ...
        if (status < 0)
            return -1;
        av_em_log(v->s.avctx, AV_LOG_DEBUG, "MB Skip plane encoding: "
               "Imode: %i, Invert: %i\n", status>>1, status&1);
        ...
        if (v->dquant) {
            av_em_log(v->s.avctx, AV_LOG_DEBUG, "VOP DQuant info\n");
    ...
    case AV_PICTURE_TYPE_B:
        ...
        if (status < 0)
            return -1;
        av_em_log(v->s.avctx, AV_LOG_DEBUG, "MB Direct Type plane encoding: "
               "Imode: %i, Invert: %i\n", status>>1, status&1);
        status = bitplane_decoding(v->s.mbskip_table, &v->skip_is_raw, v);
        if (status < 0)
            return -1;
...
int ff_vc1_parse_frame_header_adv(VC1Context *v, GetBitContext* gb)
{
    ...
    if (v->panscanflag) {
        avpriv_em_report_missing_feature(v->s.avctx, "Pan-scan");
        //...
    }
    ...
    case AV_PICTURE_TYPE_BI:
        if (v->fcm == ILACE_FRAME) { //interlace frame picture
            ...
            if (status < 0)
                return -1;
            av_em_log(v->s.avctx, AV_LOG_DEBUG, "FIELDTX plane encoding: "
                   "Imode: %i, Invert: %i\n", status>>1, status&1);
        }
        status = bitplane_decoding(v->acpred_plane, &v->acpred_is_raw, v);
        if (status < 0)
            return -1;
        av_em_log(v->s.avctx, AV_LOG_DEBUG, "ACPRED plane encoding: "
               "Imode: %i, Invert: %i\n", status>>1, status&1);
        ...
        if (v->overlap && v->pq <= 8) {
            ...
            if (v->condover == CONDOVER_SELECT) {
                ...
                if (status < 0)
                    return -1;
                av_em_log(v->s.avctx, AV_LOG_DEBUG, "CONDOVER plane encoding: "
                       "Imode: %i, Invert: %i\n", status>>1, status&1);
            }
        }
        break;
    case AV_PICTURE_TYPE_P:
        ...
        if (v->interlace) {
            ...
            if (v->fcm == ILACE_FRAME) { // interlaced frame picture
                ...
                status = bitplane_decoding(v->s.mbskip_table, &v->skip_is_raw, v);
                av_em_log(v->s.avctx, AV_LOG_DEBUG, "SKIPMB plane encoding: "
                       "Imode: %i, Invert: %i\n", status>>1, status&1);
    ...
    case AV_PICTURE_TYPE_B:
        ...
        if (v->field_mode) {
            int mvmode;
            av_em_log(v->s.avctx, AV_LOG_DEBUG, "B Fields\n");
            if (v->extended_dmv)
                v->dmvrange = get_unary(gb, 0, 3);
            ...
            if (status < 0)
                return -1;
            av_em_log(v->s.avctx, AV_LOG_DEBUG, "MB Forward Type plane encoding: "
                   "Imode: %i, Invert: %i\n", status>>1, status&1);
            mbmodetab = get_bits(gb, 3);
            ...
            if (get_bits1(gb)) /* intcomp - present but shall always be 0 */
                av_em_log(v->s.avctx, AV_LOG_WARNING, "Intensity compensation set for B picture\n");
            v->intcomp          = 0;
