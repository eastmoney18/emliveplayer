...
static Jpeg2000TgtNode *ff_jpeg2000_tag_tree_init(int w, int h)
{
    ...
    t = res = av_em_mallocz_array(tt_size, sizeof(*t));
    if (!res)
        return NULL;
...
static void init_band_stepsize(AVEMCodecContext *avctx,
                               Jpeg2000Band *band,
...
//         break;
    ...
    default:
        band->f_stepsize = 0;
        av_em_log(avctx, AV_LOG_ERROR, "Unknown quantization format\n");
        break;
    }
                     ...
                     int log2_band_prec_height)
{
    ...
    prec->cblk = av_em_mallocz_array(nb_codeblocks, sizeof(*prec->cblk));
    if (!prec->cblk)
        return AVERROR(ENOMEM);
...
static int init_band(AVEMCodecContext *avctx,
                     Jpeg2000ResLevel *reslevel,
                     Jpeg2000Component *comp,
                     Jpeg2000CodingStyle *codsty,
                     Jpeg2000QuantStyle *qntsty,
                     ...
                     int cbps, int dx, int dy)
{
    ...
    band->prec = av_em_mallocz_array(nb_precincts, sizeof(*band->prec));
    if (!band->prec)
        return AVERROR(ENOMEM);
                               ...
                               AVEMCodecContext *avctx)
{
    ...
    if (codsty->nreslevels2decode <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "nreslevels2decode %d invalid or uninitialized\n", codsty->nreslevels2decode);
    ...
    if (av_em_image_check_size(comp->coord[0][1] - comp->coord[0][0],
    ...
    if (comp->coord[0][1] - comp->coord[0][0] > 32768 ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "component size too large\n");
    ...
    if (codsty->transform == FF_DWT97) {
        ...
        comp->i_data = NULL;
        comp->f_data = av_em_mallocz_array(csize, sizeof(*comp->f_data));
        if (!comp->f_data)
            return AVERROR(ENOMEM);
    } else {
        csize += AV_INPUT_BUFFER_PADDING_SIZE / sizeof(*comp->i_data);
        comp->f_data = NULL;
        comp->i_data = av_em_mallocz_array(csize, sizeof(*comp->i_data));
        if (!comp->i_data)
            return AVERROR(ENOMEM);
    }
    comp->reslevel = av_em_mallocz_array(codsty->nreslevels, sizeof(*comp->reslevel));
    if (!comp->reslevel)
        return AVERROR(ENOMEM);
    ...
    for (reslevelno = 0; reslevelno < codsty->nreslevels; reslevelno++) {
        ...
        reslevel->band = av_em_mallocz_array(reslevel->nbands, sizeof(*reslevel->band));
        if (!reslevel->band)
            return AVERROR(ENOMEM);
    ...
    for (reslevelno = 0;
        ...
        for (bandno = 0; bandno < reslevel->nbands; bandno++) {
            ...
            for (precno = 0; precno < reslevel->num_precincts_x * reslevel->num_precincts_y; precno++) {
                if (band->prec) {
                    Jpeg2000Prec *prec = band->prec + precno;
                    av_em_freep(&prec->zerobits);
                    av_em_freep(&prec->cblkincl);
                    av_em_freep(&prec->cblk);
                }
            }

            av_em_freep(&band->prec);
        }
        av_em_freep(&reslevel->band);
    }

    ff_dwt_destroy(&comp->dwt);
    av_em_freep(&comp->reslevel);
    av_em_freep(&comp->i_data);
    av_em_freep(&comp->f_data);
}
