                               ...
                               const int data_size, AVEMCodecContext *avctx)
{
    ...
    if (hdr_size > data_size) {
        av_em_log(avctx, AV_LOG_ERROR, "frame data too small\n");
    ...
    if (flags & 2) {
        if (ptr - buf > hdr_size - 64) {
            av_em_log(avctx, AV_LOG_ERROR, "header data too small\n");
            return AVERROR_INVALIDDATA;
        }
                                 ...
                                 const int data_size, AVEMCodecContext *avctx)
{
    ...
    if (hdr_size < 8 || hdr_size > data_size) {
        av_em_log(avctx, AV_LOG_ERROR, "picture header too small\n");
        return AVERROR_INVALIDDATA;
    }

    pic_data_size = AV_RB32(buf + 1);
    if (pic_data_size > data_size) {
        av_em_log(avctx, AV_LOG_ERROR, "picture data too small\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (num_slices != AV_RB16(buf + 5)) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid number of slices\n");
        return AVERROR_INVALIDDATA;
    }

    if (ctx->total_slices != num_slices) {
        av_em_freep(&ctx->slice_data);
        ctx->slice_data = av_em_malloc_array(num_slices + 1, sizeof(ctx->slice_data[0]));
        if (!ctx->slice_data)
    ...
    if (hdr_size + num_slices * 2 > data_size) {
        av_em_log(avctx, AV_LOG_ERROR, "slice table too small\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (data_ptr > buf + data_size) {
        av_em_log(avctx, AV_LOG_ERROR, "out of slice data\n");
        return -1;
    }
...
static int decode_slice(AVEMCodecContext *avctx, void *tdata)
{
    ...
    if (slice_data_size < 6) {
        av_em_log(avctx, AV_LOG_ERROR, "slice data too small\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (v_data_size < 0 || a_data_size < 0 || hdr_size < 6) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid data size\n");
        return AVERROR_INVALIDDATA;
    }
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (buf_size < 28 || buf_size < AV_RB32(buf) ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "invalid frame\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (em_get_buffer(avctx, ctx->frame, 0) < 0)
        return -1;
...
AVEMCodec ff_prores_lgpl_decoder = {
    .name           = "prores_lgpl",
