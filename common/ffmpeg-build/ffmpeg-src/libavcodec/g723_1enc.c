...
static av_cold int g723_1_encode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->bit_rate == 6300) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Bitrate not supported yet, use 6300\n");
        return AVERROR_PATCHWELCOME;
    } else {
        av_em_log(avctx, AV_LOG_ERROR, "Bitrate not supported, use 6300\n");
        return AVERROR(EINVAL);
    }
 ...
 */
static int pack_bitstream(G723_1_Context *p, AVEMPacket *avpkt)
{
    PutBitContext pb;
    int info_bits = 0;
    int i, temp;
...
static int g723_1_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                               const AVFrame *frame, int *got_packet_ptr)
{
    ...
    /* duplicate input */
    start = in = av_em_alloc(frame->nb_samples * sizeof(int16_t));
    if (!in)
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, 24, 0)) < 0)
        return ret;
