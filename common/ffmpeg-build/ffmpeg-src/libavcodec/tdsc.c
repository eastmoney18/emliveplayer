...
typedef struct TDSCContext {
    AVEMCodecContext *jpeg_avctx;   // wrapper context for MJPEG
...
static av_cold int tdsc_close(AVEMCodecContext *avctx)
{
    TDSCContext *ctx = avctx->priv_data;

    av_em_frame_free(&ctx->refframe);
    av_em_frame_free(&ctx->jpgframe);
    av_em_freep(&ctx->deflatebuffer);
    av_em_freep(&ctx->tilebuffer);
    av_em_freep(&ctx->cursor);
    avcodec_em_free_context(&ctx->jpeg_avctx);

    return 0;
}

static av_cold int tdsc_init(AVEMCodecContext *avctx)
{
    TDSCContext *ctx = avctx->priv_data;
    ...
    if (!(avctx->width && avctx->height)) {
        av_em_log(avctx, AV_LOG_ERROR, "Video size not set.\n");
        return AVERROR_INVALIDDATA;
    }

    /* This value should be large enough for a RAW-only frame plus headers */
    ctx->deflatelen = avctx->width * avctx->height * (3 + 1);
    ret = av_em_reallocp(&ctx->deflatebuffer, ctx->deflatelen);
    if (ret < 0)
        return ret;

    /* Allocate reference and JPEG frame */
    ctx->refframe = av_em_frame_alloc();
    ctx->jpgframe = av_em_frame_alloc();
    if (!ctx->refframe || !ctx->jpgframe)
        return AVERROR(ENOMEM);

    /* Prepare everything needed for JPEG decoding */
    codec = avcodec_em_find_decoder(AV_CODEC_ID_MJPEG);
    if (!codec)
        return AVERROR_BUG;
    ctx->jpeg_avctx = avcodec_em_alloc_context3(codec);
    if (!ctx->jpeg_avctx)
        return AVERROR(ENOMEM);
    ...
    ret = em_codec_open2_recursive(ctx->jpeg_avctx, codec, NULL);
    if (ret < 0)
        return ret;
...
static void tdsc_paint_cursor(AVEMCodecContext *avctx, uint8_t *dst, int stride)
{
    TDSCContext *ctx = avctx->priv_data;
...
static int tdsc_load_cursor(AVEMCodecContext *avctx)
{
    TDSCContext *ctx  = avctx->priv_data;
    ...
    if (ctx->cursor_hot_x > ctx->cursor_w ||
        ...
        av_em_log(avctx, AV_LOG_WARNING, "Invalid hotspot position %d.%d.\n",
    ...
    ret = av_em_reallocp(&ctx->cursor, ctx->cursor_stride * ctx->cursor_h);
    if (ret < 0) {
    ...
    case CUR_FMT_RGBA:
        ...
        break;
    default:
        avpriv_em_request_sample(avctx, "Cursor format %08x", cursor_fmt);
...
static int tdsc_decode_jpeg_tile(AVEMCodecContext *avctx, int tile_size,
                                 int x, int y, int w, int h)
{
    ...
    av_em_init_packet(&jpkt);
    jpkt.data = ctx->tilebuffer;
    jpkt.size = tile_size;

    ret = avcodec_em_decode_video2(ctx->jpeg_avctx, ctx->jpgframe,
                                &got_frame, &jpkt);
    ...
    av_em_frame_unref(ctx->jpgframe);

    return 0;
}

/* Parse frame and either copy data or decode JPEG. */
static int tdsc_decode_tiles(AVEMCodecContext *avctx, int number_tiles)
{
    TDSCContext *ctx = avctx->priv_data;
    int i;
    ...
    for (i = 0; i < number_tiles; i++) {
        ...
        if (bytestream2_get_bytes_left(&ctx->gbc) < 4 ||
            ...
            av_em_log(avctx, AV_LOG_ERROR, "TDSB tag is too small.\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        ret = av_em_reallocp(&ctx->tilebuffer, tile_size);
        if (!ctx->tilebuffer)
            return ret;
        ...
        if (tile_mode == MKTAG('G','E','P','J')) {
            ...
            av_em_image_copy_plane(ctx->refframe->data[0] + x * 3 +
                                ctx->refframe->linesize[0] * y,
                                ctx->refframe->linesize[0], ctx->tilebuffer,
                                w * 3, w * 3, h);
        } else {
            av_em_log(avctx, AV_LOG_ERROR, "Unknown tile type %08x.\n", tile_mode);
            return AVERROR_INVALIDDATA;
        }
        av_em_log(avctx, AV_LOG_DEBUG, "Tile %d, %dx%d (%d.%d)\n", i, w, h, x, y);
    }

    return 0;
}

static int tdsc_parse_tdsf(AVEMCodecContext *avctx, int number_tiles)
{
    TDSCContext *ctx = avctx->priv_data;
    ...
    if (avctx->width != w || avctx->height != h) {
        av_em_log(avctx, AV_LOG_DEBUG, "Size update %dx%d -> %d%d.\n",
               avctx->width, avctx->height, ctx->width, ctx->height);
        ret = em_set_dimensions(avctx, w, h);
        if (ret < 0)
            return ret;
        init_refframe = 1;
    }
    ...
    if (init_refframe) {
        ret = av_em_frame_get_buffer(ctx->refframe, 32);
        if (ret < 0)
            return ret;
    }
...
static int tdsc_parse_dtsm(AVEMCodecContext *avctx)
{
    TDSCContext *ctx = avctx->priv_data;
    int ret;
    ...
    if (action == 2 || action == 3) {
        ...
        avpriv_em_request_sample(avctx, "Cursor action %d", action);
    }

    return 0;
}

static int tdsc_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (ret) {
        av_em_log(avctx, AV_LOG_ERROR, "Deflate error %d.\n", ret);
    ...
    if (bytestream2_get_bytes_left(&ctx->gbc) < 4 + 4) {
        av_em_log(avctx, AV_LOG_ERROR, "Frame is too small.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (tag_header == MKTAG('T','D','S','F')) {
        ...
        if (bytestream2_get_bytes_left(&ctx->gbc) < TDSF_HEADER_SIZE) {
            av_em_log(avctx, AV_LOG_ERROR, "TDSF tag is too small.\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    if (tag_header == MKTAG('D','T','S','M')) {
        ...
        if (bytestream2_get_bytes_left(&ctx->gbc) < tag_size) {
            av_em_log(avctx, AV_LOG_ERROR, "DTSM tag is too small.\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    ret = av_em_frame_copy(frame, ctx->refframe);
    if (ret < 0)
        return ret;
