...
static int set_bps_params(AVEMCodecContext *avctx)
{
    ...
    case 24:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "invalid/unsupported bits per sample: %d\n",
...
static void set_sample_rate_params(AVEMCodecContext *avctx)
{
...
static av_cold int tak_decode_init(AVEMCodecContext *avctx)
{
    TAKDecContext *s = avctx->priv_data;
...
static int tak_decode_frame(AVEMCodecContext *avctx, void *data,
                            int *got_frame_ptr, AVEMPacket *pkt)
{
    ...
    if (avctx->err_recognition & (AV_EF_CRCCHECK|AV_EF_COMPLIANT)) {
        if (ff_tak_check_crc(pkt->data, hsize)) {
            av_em_log(avctx, AV_LOG_ERROR, "CRC error\n");
    ...
    if (s->ti.frame_samples <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "unsupported/invalid number of samples\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (avctx->bits_per_raw_sample <= 16) {
        int buf_size = av_em_samples_get_buffer_size(NULL, avctx->channels,
                                                  s->nb_samples,
                                                  AV_SAMPLE_FMT_S32P, 0);
        if (buf_size < 0)
            return buf_size;
        av_em_fast_malloc(&s->decode_buffer, &s->decode_buffer_size, buf_size);
        if (!s->decode_buffer)
    ...
    if (get_bits_left(gb) < 0)
        av_em_log(avctx, AV_LOG_DEBUG, "overread\n");
    else if (get_bits_left(gb) > 0)
        av_em_log(avctx, AV_LOG_DEBUG, "underread\n");
...
static int update_thread_context(AVEMCodecContext *dst,
...
static av_cold int tak_decode_close(AVEMCodecContext *avctx)
{
    TAKDecContext *s = avctx->priv_data;
