...
static av_cold int cdg_decode_init(AVEMCodecContext *avctx)
{
    CDGraphicsContext *cc = avctx->priv_data;

    cc->frame = av_em_frame_alloc();
    if (!cc->frame)
...
static int cdg_decode_frame(AVEMCodecContext *avctx,
                            void *data, int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (buf_size < CDG_MINIMUM_PKT_SIZE) {
        av_em_log(avctx, AV_LOG_ERROR, "buffer too small for decoder\n");
        return AVERROR(EINVAL);
    }
    if (buf_size > CDG_HEADER_SIZE + CDG_DATA_SIZE) {
        av_em_log(avctx, AV_LOG_ERROR, "buffer too big for decoder\n");
        return AVERROR(EINVAL);
    }

    bytestream2_init(&gb, avpkt->data, avpkt->size);

    if ((ret = em_reget_buffer(avctx, cc->frame)) < 0)
        return ret;
    ...
    if ((command & CDG_MASK) == CDG_COMMAND) {
        ...
        case CDG_INST_LOAD_PAL_HIGH:
            if (buf_size - CDG_HEADER_SIZE < CDG_DATA_SIZE) {
                av_em_log(avctx, AV_LOG_ERROR, "buffer too small for loading palette\n");
                return AVERROR(EINVAL);
            }
        ...
        case CDG_INST_TILE_BLOCK:
            if (buf_size - CDG_HEADER_SIZE < CDG_DATA_SIZE) {
                av_em_log(avctx, AV_LOG_ERROR, "buffer too small for drawing tile\n");
                return AVERROR(EINVAL);
            }

            ret = cdg_tile_block(cc, cdg_data, inst == CDG_INST_TILE_BLOCK_XOR);
            if (ret) {
                av_em_log(avctx, AV_LOG_ERROR, "tile is out of range\n");
                return ret;
            }
            break;
        ...
        case CDG_INST_SCROLL_COPY:
            if (buf_size - CDG_HEADER_SIZE < CDG_MINIMUM_SCROLL_SIZE) {
                av_em_log(avctx, AV_LOG_ERROR, "buffer too small for scrolling\n");
                return AVERROR(EINVAL);
            }
            ...
            av_em_frame_unref(cc->frame);
            ret = av_em_frame_ref(cc->frame, frame);
            if (ret < 0)
                return ret;
            break;
        ...
        if (!frame->data[0]) {
            ret = av_em_frame_ref(frame, cc->frame);
            if (ret < 0)
                return ret;
        }
        *got_frame = 1;
...
static av_cold int cdg_decode_end(AVEMCodecContext *avctx)
{
    CDGraphicsContext *cc = avctx->priv_data;

    av_em_frame_free(&cc->frame);

    return 0;
}

AVEMCodec ff_cdgraphics_decoder = {
    .name           = "cdgraphics",
