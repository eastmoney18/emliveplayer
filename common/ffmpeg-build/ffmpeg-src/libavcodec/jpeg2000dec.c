                           ...
                           int threshold)
{
    ...
    if (!node) {
        av_em_log(s->avctx, AV_LOG_ERROR, "missing node\n");
...
static int get_siz(Jpeg2000DecoderContext *s)
{
    ...
    if (bytestream2_get_bytes_left(&s->g) < 36) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Insufficient space for SIZ\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->image_offset_x || s->image_offset_y) {
        avpriv_em_request_sample(s->avctx, "Support for image offsets");
        return AVERROR_PATCHWELCOME;
    }
    if (av_em_image_check_size(s->width, s->height, 0, s->avctx)) {
        avpriv_em_request_sample(s->avctx, "Large Dimensions");
        return AVERROR_PATCHWELCOME;
    }

    if (ncomponents <= 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid number of components: %d\n",
               s->ncomponents);
        return AVERROR_INVALIDDATA;
    }

    if (ncomponents > 4) {
        avpriv_em_request_sample(s->avctx, "Support for %d components",
                              ncomponents);
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (s->tile_width <= 0 || s->tile_height <= 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid tile dimension %dx%d.\n",
               s->tile_width, s->tile_height);
        return AVERROR_INVALIDDATA;
    }

    if (bytestream2_get_bytes_left(&s->g) < 3 * s->ncomponents) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Insufficient space for %d components in SIZ\n", s->ncomponents);
        return AVERROR_INVALIDDATA;
    }
    ...
    for (i = 0; i < s->ncomponents; i++) { // Ssiz_i XRsiz_i, YRsiz_i
        ...
        if (   !s->cdx[i] || s->cdx[i] == 3 || s->cdx[i] > 4
            ...
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid sample separation %d/%d\n", s->cdx[i], s->cdy[i]);
            return AVERROR_INVALIDDATA;
        }
    ...
    s->tile = av_em_mallocz_array(s->numXtiles * s->numYtiles, sizeof(*s->tile));
    if (!s->tile) {
        s->numXtiles = s->numYtiles = 0;
    ...
    for (i = 0; i < s->numXtiles * s->numYtiles; i++) {
        ...
        tile->comp = av_em_mallocz(s->ncomponents * sizeof(*tile->comp));
        if (!tile->comp)
            return AVERROR(ENOMEM);
    }
...
static int get_cox(Jpeg2000DecoderContext *s, Jpeg2000CodingStyle *c)
{
    ...
    if (bytestream2_get_bytes_left(&s->g) < 5) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Insufficient space for COX\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (c->nreslevels >= JPEG2000_MAX_RESLEVELS) {
        av_em_log(s->avctx, AV_LOG_ERROR, "nreslevels %d is invalid\n", c->nreslevels);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (c->nreslevels <= s->reduction_factor) {
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "reduction_factor too large for this bitstream, max is %d\n", c->nreslevels - 1);
    ...
    if (c->log2_cblk_width > 10 || c->log2_cblk_height > 10 ||
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "cblk size invalid\n");
        return AVERROR_INVALIDDATA;
    }

    c->cblk_style = bytestream2_get_byteu(&s->g);
    if (c->cblk_style != 0) { // cblk style
        av_em_log(s->avctx, AV_LOG_WARNING, "extra cblk styles %X\n", c->cblk_style);
        if (c->cblk_style & JPEG2000_CBLK_BYPASS)
            av_em_log(s->avctx, AV_LOG_WARNING, "Selective arithmetic coding bypass\n");
    }
        ...
        for (i = 0; i < c->nreslevels; i++) {
            ...
            if (i)
                if (c->log2_prec_widths[i] == 0 || c->log2_prec_heights[i] == 0) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "PPx %d PPy %d invalid\n",
                   ...
                   uint8_t *properties)
{
    ...
    if (bytestream2_get_bytes_left(&s->g) < 5) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Insufficient space for COD\n");
        return AVERROR_INVALIDDATA;
    }
                   ...
                   uint8_t *properties)
{
    ...
    if (bytestream2_get_bytes_left(&s->g) < 2) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Insufficient space for COC\n");
        return AVERROR_INVALIDDATA;
    }
...
static int get_poc(Jpeg2000DecoderContext *s, int size, Jpeg2000POC *p)
{
    ...
    if (bytestream2_get_bytes_left(&s->g) < 5 || size < 2 + elem_size) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Insufficient space for POC\n");
        return AVERROR_INVALIDDATA;
    }

    if (elem_size > 7) {
        avpriv_em_request_sample(s->avctx, "Fat POC not supported");
        return AVERROR_PATCHWELCOME;
    }

    tmp.nb_poc = (size - 2) / elem_size;
    if (tmp.nb_poc > MAX_POCS) {
        avpriv_em_request_sample(s->avctx, "Too many POCs (%d)", tmp.nb_poc);
        return AVERROR_PATCHWELCOME;
    }
    ...
    for (i = 0; i<tmp.nb_poc; i++) {
        ...
        if (   e->RSpoc >= e->REpoc || e->REpoc > 33
            ...
            || !e->LYEpoc) {
            av_em_log(s->avctx, AV_LOG_ERROR, "POC Entry %d is invalid (%d, %d, %d, %d, %d, %d)\n", i,
...
static int get_sot(Jpeg2000DecoderContext *s, int n)
{
    ...
    if (Psot > bytestream2_get_bytes_left(&s->g) - 2 + n + 2) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Psot %"PRIu32" too big\n", Psot);
        return AVERROR_INVALIDDATA;
    }
                                  ...
                                  int layno, uint8_t *expn, int numgbits)
{
    ...
    for (bandno = 0; bandno < rlevel->nbands; bandno++) {
        ...
        for (cblkno = 0; cblkno < nb_code_blocks; cblkno++) {
            ...
            if (cblk->npasses + newpasses >= JPEG2000_MAX_PASSES) {
                avpriv_em_request_sample(s->avctx, "Too many passes");
                return AVERROR_PATCHWELCOME;
            }
            if ((llen = getlblockinc(s)) < 0)
                return llen;
            if (cblk->lblock + llen + av_log2(newpasses) > 16) {
                avpriv_em_request_sample(s->avctx,
    ...
    if (codsty->csty & JPEG2000_CSTY_EPH) {
        if (bytestream2_peek_be16(&s->g) == JPEG2000_EPH)
            ...
            av_em_log(s->avctx, AV_LOG_ERROR, "EPH marker not found. instead %X\n", bytestream2_peek_be32(&s->g));
    }
                                             ...
                                             int Ppoc, int *tp_index)
{
    ...
    case JPEG2000_PGOD_RLCP:
        av_em_log(s->avctx, AV_LOG_DEBUG, "Progression order RLCP\n");
        ok_reslevel = 1;
    ...
    case JPEG2000_PGOD_LRCP:
        av_em_log(s->avctx, AV_LOG_DEBUG, "Progression order LRCP\n");
        for (layno = 0; layno < LYEpoc; layno++) {
            ok_reslevel = 1;
            for (reslevelno = RSpoc; ok_reslevel && reslevelno < REpoc; reslevelno++) {
                ok_reslevel = 0;
    ...
    case JPEG2000_PGOD_CPRL:
        av_em_log(s->avctx, AV_LOG_DEBUG, "Progression order CPRL\n");
        for (compno = CSpoc; compno < CEpoc; compno++) {
            Jpeg2000Component *comp     = tile->comp + compno;
            Jpeg2000CodingStyle *codsty = tile->codsty + compno;
            Jpeg2000QuantStyle *qntsty  = tile->qntsty + compno;
                ...
                for (x = tile->coord[0][0]; x < tile->coord[0][1]; x = (x/step_x + 1)*step_x) {
                    for (reslevelno = RSpoc; reslevelno < FFMIN(codsty->nreslevels, REpoc); reslevelno++) {
                        ...
                        if (prcx >= rlevel->num_precincts_x || prcy >= rlevel->num_precincts_y) {
                            av_em_log(s->avctx, AV_LOG_WARNING, "prc %d %d outside limits %d %d\n",
    ...
    case JPEG2000_PGOD_RPCL:
        av_em_log(s->avctx, AV_LOG_WARNING, "Progression order RPCL\n");
        ok_reslevel = 1;
        for (reslevelno = RSpoc; ok_reslevel && reslevelno < REpoc; reslevelno++) {
            ok_reslevel = 0;
            step_x = 30;
    ...
    case JPEG2000_PGOD_PCRL:
        av_em_log(s->avctx, AV_LOG_WARNING, "Progression order PCRL\n");
        step_x = 32;
        step_y = 32;
        for (compno = CSpoc; compno < CEpoc; compno++) {
            Jpeg2000Component *comp     = tile->comp + compno;
        ...
        if (step_x >= 31 || step_y >= 31){
            avpriv_em_request_sample(s->avctx, "PCRL with large step");
            return AVERROR_PATCHWELCOME;
        }
        step_x = 1<<step_x;
        step_y = 1<<step_y;
                       ...
                       int width, int height, int bandpos)
{
    ...
    while (passno--) {
        if (bpno < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "bpno became negative\n");
            return AVERROR_INVALIDDATA;
        }
        switch(pass_t) {
        case 0:
        ...
        if (passno && (coder_type = needs_termination(codsty->cblk_style, pass_cnt))) {
            if (term_cnt >= cblk->nb_terminations) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Missing needed termination \n");
                return AVERROR_INVALIDDATA;
            }
            if (FFABS(cblk->data + cblk->data_start[term_cnt + 1] - 2 - t1->mqc.bp) > 0) {
                av_em_log(s->avctx, AV_LOG_WARNING, "Mid mismatch %"PTRDIFF_SPECIFIER" in pass %d of %d\n",
    ...
    if (cblk->data + cblk->length - 2*(term_cnt < cblk->nb_terminations) != t1->mqc.bp) {
        av_em_log(s->avctx, AV_LOG_WARNING, "End mismatch %"PTRDIFF_SPECIFIER"\n",
...
static inline void mct_decode(Jpeg2000DecoderContext *s, Jpeg2000Tile *tile)
{
    ...
    for (i = 1; i < 3; i++) {
        if (tile->codsty[0].transform != tile->codsty[i].transform) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Transforms mismatch, MCT not supported\n");
            return;
        }
        if (memcmp(tile->comp[0].coord, tile->comp[i].coord, sizeof(tile->comp[0].coord))) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Coords mismatch, MCT not supported\n");
            return;
        }
    }
...
#define WRITE_FRAME(D, PIXEL)                                                                     \
    ...
    {                                                                                             \
        const AVPixFmtDescriptor *pixdesc = av_em_pix_fmt_desc_get(s->avctx->pix_fmt);               \
...
static int jpeg2000_decode_tile(AVEMCodecContext *avctx, void *td,
...
static void jpeg2000_dec_cleanup(Jpeg2000DecoderContext *s)
{
    ...
    for (tileno = 0; tileno < s->numXtiles * s->numYtiles; tileno++) {
        if (s->tile[tileno].comp) {
            ...
            av_em_freep(&s->tile[tileno].comp);
        }
    }
    av_em_freep(&s->tile);
...
static int jpeg2000_read_main_headers(Jpeg2000DecoderContext *s)
{
    ...
    for (;;) {
        ...
        if (bytestream2_get_bytes_left(&s->g) < 2) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Missing EOC\n");
            break;
        }
        ...
        if (marker == JPEG2000_SOD) {
            ...
            if (!s->tile) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Missing SIZ\n");
                return AVERROR_INVALIDDATA;
            }
            if (s->curtileno < 0) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Missing SOT\n");
                return AVERROR_INVALIDDATA;
            }
            ...
            if (tp->tp_end < s->g.buffer) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid tpend\n");
                return AVERROR_INVALIDDATA;
            }
        ...
        if (len < 2 || bytestream2_get_bytes_left(&s->g) < len - 2) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid len %d left=%d\n", len, bytestream2_get_bytes_left(&s->g));
            return AVERROR_INVALIDDATA;
        }
        ...
        case JPEG2000_SIZ:
            if (s->ncomponents) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Duplicate SIZ\n");
                return AVERROR_INVALIDDATA;
            }
            ret = get_siz(s);
            if (!s->tile)
            ...
            do {
                ...
                if (atom2 == JP2_CODESTREAM) {
                    ...
                    if (colour_count > 256   ||
                        ...
                        colour_depth[2] > 16 ||
                        atom2_size < size) {
                        avpriv_em_request_sample(s->avctx, "Unknown palette");
...
static av_cold int jpeg2000_decode_init(AVEMCodecContext *avctx)
{
    Jpeg2000DecoderContext *s = avctx->priv_data;
...
static int jpeg2000_decode_frame(AVEMCodecContext *avctx, void *data,
                                 int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (bytestream2_get_be16u(&s->g) != JPEG2000_SOC) {
        av_em_log(avctx, AV_LOG_ERROR, "SOC marker not present\n");
        ret = AVERROR_INVALIDDATA;
        goto end;
    }
...
static av_cold void jpeg2000_init_static_data(AVEMCodec *codec)
{
...
static const AVEMClass jpeg2000_class = {
    .class_name = "jpeg2000",
