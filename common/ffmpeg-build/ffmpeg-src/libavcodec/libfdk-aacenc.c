...
static const AVEMClass aac_enc_class = {
    "libfdk_aac", av_em_default_item_name, aac_enc_options, LIBAVUTIL_VERSION_INT
};
...
static int aac_encode_close(AVEMCodecContext *avctx)
{
...
static av_cold int aac_encode_init(AVEMCodecContext *avctx)
{
    ...
    if ((err = aacEncOpen(&s->handle, 0, avctx->channels)) != AACENC_OK) {
        av_em_log(avctx, AV_LOG_ERROR, "Unable to open the encoder: %s\n",
               aac_get_error(err));
        goto error;
    }
    ...
    if ((err = aacEncoder_SetParam(s->handle, AACENC_AOT, aot)) != AACENC_OK) {
        av_em_log(avctx, AV_LOG_ERROR, "Unable to set the AOT %d: %s\n",
    ...
    if (aot == FF_PROFILE_AAC_ELD + 1 && s->eld_sbr) {
        if ((err = aacEncoder_SetParam(s->handle, AACENC_SBR_MODE,
                                       1)) != AACENC_OK) {
            av_em_log(avctx, AV_LOG_ERROR, "Unable to enable SBR for ELD: %s\n",
                   aac_get_error(err));
            goto error;
        }
    }
    ...
    if ((err = aacEncoder_SetParam(s->handle, AACENC_SAMPLERATE,
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Unable to set the sample rate %d: %s\n",
   ...
   should suffice. */
    ...
    if (avctx->flags & AV_CODEC_FLAG_QSCALE || s->vbr) {
        ...
        if ((err = aacEncoder_SetParam(s->handle, AACENC_BITRATEMODE,
                                       mode)) != AACENC_OK) {
            av_em_log(avctx, AV_LOG_ERROR, "Unable to set the VBR bitrate mode %d: %s\n",
                   mode, aac_get_error(err));
            goto error;
        }
    } else {
        ...
        if ((err = aacEncoder_SetParam(s->handle, AACENC_BITRATE,
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Unable to set the bitrate %"PRId64": %s\n",
    ...
    if ((err = aacEncoder_SetParam(s->handle, AACENC_TRANSMUX,
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Unable to set the transmux format: %s\n",
               aac_get_error(err));
        goto error;
    }
    ...
    if (s->latm && s->header_period) {
        if ((err = aacEncoder_SetParam(s->handle, AACENC_HEADER_PERIOD,
             ...
             av_em_log(avctx, AV_LOG_ERROR, "Unable to set header period: %s\n",
                    aac_get_error(err));
             goto error;
        }
    }
    ...
    if ((err = aacEncoder_SetParam(s->handle, AACENC_SIGNALING_MODE,
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Unable to set signaling mode %d: %s\n",
    ...
    if ((err = aacEncoder_SetParam(s->handle, AACENC_AFTERBURNER,
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Unable to set afterburner to %d: %s\n",
    ...
    if (avctx->cutoff > 0) {
        if (avctx->cutoff < (avctx->sample_rate + 255) >> 8 || avctx->cutoff > 20000) {
            av_em_log(avctx, AV_LOG_ERROR, "cutoff valid range is %d-20000\n",
                   (avctx->sample_rate + 255) >> 8);
            goto error;
        }
        if ((err = aacEncoder_SetParam(s->handle, AACENC_BANDWIDTH,
                                       avctx->cutoff)) != AACENC_OK) {
            av_em_log(avctx, AV_LOG_ERROR, "Unable to set the encoder bandwidth to %d: %s\n",
    ...
    if ((err = aacEncEncode(s->handle, NULL, NULL, NULL, NULL)) != AACENC_OK) {
        av_em_log(avctx, AV_LOG_ERROR, "Unable to initialize the encoder: %s\n",
               aac_get_error(err));
        return AVERROR(EINVAL);
    }

    if ((err = aacEncInfo(s->handle, &info)) != AACENC_OK) {
        av_em_log(avctx, AV_LOG_ERROR, "Unable to get encoder info: %s\n",
               aac_get_error(err));
        goto error;
    }
                            ...
                            const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, FFMAX(8192, 768 * avctx->channels), 0)) < 0)
        return ret;
    ...
    if ((err = aacEncEncode(s->handle, &in_buf, &out_buf, &in_args,
        ...
        if (!frame && err == AACENC_ENCODE_EOF)
            return 0;
        av_em_log(avctx, AV_LOG_ERROR, "Unable to encode frame: %s\n",
               aac_get_error(err));
        return AVERROR(EINVAL);
    }
...
static const AVEMProfile profiles[] = {
...
AVEMCodec ff_libfdk_aac_encoder = {
    .name                  = "libfdk_aac",
