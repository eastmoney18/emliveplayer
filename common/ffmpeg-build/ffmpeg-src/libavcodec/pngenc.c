...
{
    const AVCRC *crc_table = av_em_crc_get_table(AV_CRC_32_IEEE_LE);
    uint32_t crc = ~0U;
    uint8_t tagbuf[4];

    bytestream_put_be32(f, length);
    AV_WL32(tagbuf, tag);
    crc = av_em_crc(crc_table, crc, tagbuf, 4);
    bytestream_put_be32(f, av_bswap32(tag));
    if (length > 0) {
        crc = av_em_crc(crc_table, crc, buf, length);
        memcpy(*f, buf, length);
        *f += length;
    }
...
static void png_write_image_data(AVEMCodecContext *avctx,
                                 const uint8_t *buf, int length)
{
    ...
    crc = av_em_crc(crc_table, crc, s->bytestream - 8, 8);
...
/* XXX: do filtering */
static int png_write_row(AVEMCodecContext *avctx, const uint8_t *data, int size)
{
    PNGEncContext *s = avctx->priv_data;
    int ret;
...
{
    double gamma = avpriv_em_get_gamma_from_trc(trc);
    if (gamma <= 1e-6)
        return 0;
...
static int encode_headers(AVEMCodecContext *avctx, const AVFrame *pict)
{
    ...
    side_data = av_em_frame_get_side_data(pict, AV_FRAME_DATA_STEREO3D);
    if (side_data) {
        ...
        switch (stereo3d->type) {
            ...
            case AV_STEREO3D_2D:
                break;
            default:
                av_em_log(avctx, AV_LOG_WARNING, "Only side-by-side stereo3d flag can be defined within sTER chunk\n");
                break;
        }
    }
...
static int encode_frame(AVEMCodecContext *avctx, const AVFrame *pict)
{
    PNGEncContext *s       = avctx->priv_data;
    ...
    crow_base = av_em_alloc((row_size + 32) << (s->filter_type == PNG_FILTER_VALUE_MIXED));
    if (!crow_base) {
        ret = AVERROR(ENOMEM);
        goto the_end;
    }
    ...
    if (s->is_progressive) {
        progressive_buf = av_em_alloc(row_size + 1);
        top_buf = av_em_alloc(row_size + 1);
...
the_end:
    av_em_freep(&crow_base);
    av_em_freep(&progressive_buf);
    av_em_freep(&top_buf);
    deflateReset(&s->zstream);
    return ret;
}

static int encode_png(AVEMCodecContext *avctx, AVEMPacket *pkt,
                      const AVFrame *pict, int *got_packet)
{
    ...
    ret = em_alloc_packet2(avctx, pkt, max_packet_size, 0);
    if (ret < 0)
        return ret;
...
static int apng_encode_frame(AVEMCodecContext *avctx, const AVFrame *pict,
                             APNGFctlChunk *best_fctl_chunk, APNGFctlChunk *best_last_fctl_chunk)
{
    ...
    if ((ret = av_em_frame_get_buffer(diffFrame, 32)) < 0)
        goto fail;

    original_bytestream = s->bytestream;
    original_bytestream_end = s->bytestream_end;

    temp_bytestream = av_em_alloc(original_bytestream_end - original_bytestream);
    ...
    for (last_fctl_chunk.dispose_op = 0; last_fctl_chunk.dispose_op < 3; ++last_fctl_chunk.dispose_op) {
        ...
        for (fctl_chunk.blend_op = 0; fctl_chunk.blend_op < 2; ++fctl_chunk.blend_op) {
            ...
            if (last_fctl_chunk.dispose_op != APNG_DISPOSE_OP_PREVIOUS) {
                diffFrame->width = pict->width;
                diffFrame->height = pict->height;
                ret = av_em_frame_copy(diffFrame, s->last_frame);
                if (ret < 0)
                    goto fail;
                ...
                diffFrame->width = pict->width;
                diffFrame->height = pict->height;
                ret = av_em_frame_copy(diffFrame, s->prev_frame);
                if (ret < 0)
                    goto fail;
            }
...
fail:
    av_em_freep(&temp_bytestream);
    av_em_frame_free(&diffFrame);
    return ret;
}

static int encode_apng(AVEMCodecContext *avctx, AVEMPacket *pkt,
                       const AVFrame *pict, int *got_packet)
{
    PNGEncContext *s = avctx->priv_data;
    int ret;
    ...
    if (pict && avctx->codec_id == AV_CODEC_ID_APNG && s->color_type == PNG_COLOR_TYPE_PALETTE) {
        uint32_t checksum = ~av_em_crc(av_em_crc_get_table(AV_CRC_32_IEEE_LE), ~0U, pict->data[1], 256 * sizeof(uint32_t));
    ...
    if (avctx->frame_number == 0) {
        ...
        s->bytestream = avctx->extradata = av_em_alloc(FF_MIN_BUFFER_SIZE);
        if (!avctx->extradata)
            return AVERROR(ENOMEM);
        ...
        s->last_frame_packet = av_em_alloc(max_packet_size);
    ...
    if (pict) {
        if (!s->last_frame) {
            ...
            if (!s->prev_frame) {
                ...
                if ((ret = av_em_frame_get_buffer(s->prev_frame, 32)) < 0)
                    return ret;
            }

            // Do disposal, but not blending
            av_em_frame_copy(s->prev_frame, s->last_frame);
        ...
        av_em_frame_unref(s->last_frame);
        ret = av_em_frame_ref(s->last_frame, (AVFrame*)pict);
        if (ret < 0)
            return ret;
...
static av_cold int png_enc_init(AVEMCodecContext *avctx)
{
    PNGEncContext *s = avctx->priv_data;
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (s->dpi && s->dpm) {
      av_em_log(avctx, AV_LOG_ERROR, "Only one of 'dpi' or 'dpm' options should be set\n");
      return AVERROR(EINVAL);
    } else if (s->dpi) {
...
static av_cold int png_enc_close(AVEMCodecContext *avctx)
{
    PNGEncContext *s = avctx->priv_data;

    deflateEnd(&s->zstream);
    av_em_frame_free(&s->last_frame);
    av_em_frame_free(&s->prev_frame);
    av_em_freep(&s->last_frame_packet);
    return 0;
}
...
static const AVEMClass pngenc_class = {
...
static const AVEMClass apngenc_class = {
