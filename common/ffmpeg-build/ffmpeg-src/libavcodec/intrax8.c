...
#undef init_or_vlc
    ...
    if (offset != sizeof(table) / sizeof(VLC_TYPE) / 2) {
        av_em_log(NULL, AV_LOG_ERROR, "table size %zd does not match needed %i\n",
...
av_cold int ff_intrax8_common_init(AVEMCodecContext *avctx,
                                   ...
                                   int mb_width, int mb_height)
{
    ...
    w->prediction_table = av_em_mallocz(w->mb_width * 2 * 2);
...
{
    av_em_freep(&w->prediction_table);
}
