...
static int rv40_parse_slice_header(RV34DecContext *r, GetBitContext *gb, SliceInfo *si)
{
    ...
    if ((ret = av_em_image_check_size(w, h, 0, r->s.avctx)) < 0)
        return ret;
    si->width  = w;
    si->height = h;
...
static int rv40_decode_mb_info(RV34DecContext *r)
{
    ...
    if(s->pict_type == AV_PICTURE_TYPE_P){
        ...
        if(q < PBTYPE_ESCAPE)
            return q;
        q = get_vlc2(gb, ptype_vlc[prev_type].table, PTYPE_VLC_BITS, 1);
        av_em_log(s->avctx, AV_LOG_ERROR, "Dquant for P-frame\n");
    }else{
        ...
        if(q < PBTYPE_ESCAPE)
            return q;
        q = get_vlc2(gb, btype_vlc[prev_type].table, BTYPE_VLC_BITS, 1);
        av_em_log(s->avctx, AV_LOG_ERROR, "Dquant for B-frame\n");
    }
    return 0;
}
 ...
 * Initialize decoder.
 */
static av_cold int rv40_decode_init(AVEMCodecContext *avctx)
{
