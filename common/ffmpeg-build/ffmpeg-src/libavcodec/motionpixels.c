...
static av_cold int mp_decode_end(AVEMCodecContext *avctx)
{
    MotionPixelsContext *mp = avctx->priv_data;

    av_em_freep(&mp->changes_map);
    av_em_freep(&mp->vpt);
    av_em_freep(&mp->hpt);
    av_em_freep(&mp->bswapbuf);
    av_em_frame_free(&mp->frame);

    return 0;
}

static av_cold int mp_decode_init(AVEMCodecContext *avctx)
{
    MotionPixelsContext *mp = avctx->priv_data;
    ...
    if(avctx->extradata_size < 2){
        av_em_log(avctx, AV_LOG_ERROR, "extradata too small\n");
        return AVERROR_INVALIDDATA;
    }

    motionpixels_tableinit();
    mp->avctx = avctx;
    em_bswapdsp_init(&mp->bdsp);
    mp->changes_map = av_em_mallocz_array(avctx->width, h4);
    mp->offset_bits_len = av_log2(avctx->width * avctx->height) + 1;
    mp->vpt = av_em_mallocz_array(avctx->height, sizeof(YuvPixel));
    mp->hpt = av_em_mallocz_array(h4 / 4, w4 / 4 * sizeof(YuvPixel));
...
static int mp_get_code(MotionPixelsContext *mp, GetBitContext *gb, int size, int code)
{
    while (get_bits1(gb)) {
        ...
        if (size > mp->max_codes_bits) {
            av_em_log(mp->avctx, AV_LOG_ERROR, "invalid code size %d/%d\n", size, mp->max_codes_bits);
            return AVERROR_INVALIDDATA;
        }
        code <<= 1;
    ...
    if (mp->current_codes_count >= MAX_HUFF_CODES) {
        av_em_log(mp->avctx, AV_LOG_ERROR, "too many codes\n");
        return AVERROR_INVALIDDATA;
    }
...
static int mp_read_codes_table(MotionPixelsContext *mp, GetBitContext *gb)
{
    if (mp->codes_count == 1) {
        ...
        if (mp->current_codes_count < mp->codes_count) {
            av_em_log(mp->avctx, AV_LOG_ERROR, "too few codes\n");
            return AVERROR_INVALIDDATA;
        }
   }
   return 0;
...
static int mp_decode_frame(AVEMCodecContext *avctx,
                                 ...
                                 AVEMPacket *avpkt)
{
    ...
    if ((ret = em_reget_buffer(avctx, mp->frame)) < 0)
        return ret;

    /* le32 bitstream msb first */
    av_em_fast_padded_malloc(&mp->bswapbuf, &mp->bswapbuf_size, buf_size);
    if (!mp->bswapbuf)
        return AVERROR(ENOMEM);
    ...
    if ((ret = av_em_frame_ref(data, mp->frame)) < 0)
        return ret;
    *got_frame       = 1;
    return buf_size;
}

AVEMCodec ff_motionpixels_decoder = {
    .name           = "motionpixels",
