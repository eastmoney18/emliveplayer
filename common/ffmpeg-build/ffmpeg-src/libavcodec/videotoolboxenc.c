...
static int vtenc_populate_extradata(AVEMCodecContext   *avctx,
...
{
    BufNode *info = av_em_alloc(sizeof(BufNode));
    if (!info) {
...
static CMVideoCodecType get_cm_codec_type(enum AVEMCodecID id)
{
    switch (id) {
    ...
    size_t                      *size)
{
    ...
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Error getting parameter set sizes: %d\n", status);
    ...
    size_t                      dst_size)
{
    ...
    for (i = 0; i < ps_count || is_count_bad; i++) {
        ...
        if (dst_size < next_offset) {
            av_em_log(avctx, AV_LOG_ERROR, "Error: buffer too small for parameter sets.\n");
    ...
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Error getting parameter set data: %d\n", status);
        return AVERROR_EXTERNAL;
    }
...
static int set_extradata(AVEMCodecContext *avctx, CMSampleBufferRef sample_buffer)
{
    CMVideoFormatDescriptionRef vid_fmt;
    size_t total_size;
    int status;
    ...
    if (!vid_fmt) {
        av_em_log(avctx, AV_LOG_ERROR, "No video format.\n");
        return AVERROR_EXTERNAL;
    }

    status = get_params_size(avctx, vid_fmt, &total_size);
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Could not get parameter sets.\n");
        return status;
    }

    avctx->extradata = av_em_mallocz(total_size + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!avctx->extradata) {
    ...
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Could not copy param sets.\n");
        return status;
    }
    ...
    CMSampleBufferRef sample_buffer)
{
    ...
    if (status || !sample_buffer) {
        av_em_log(avctx, AV_LOG_ERROR, "Error encoding frame: %d\n", (int)status);
    ...
    size_t            *size)
{
    ...
    vid_fmt = CMSampleBufferGetFormatDescription(sample_buffer);
    if (!vid_fmt) {
        av_em_log(avctx, AV_LOG_ERROR, "Error getting buffer format description.\n");
        return AVERROR_EXTERNAL;
    }
    ...
    status = CMVideoFormatDescriptionGetH264ParameterSetAtIndex(vid_fmt,
                                                                ...
                                                                &isize);
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Error getting length code size: %d\n", status);
        return AVERROR_EXTERNAL;
    }
 ...
 */
static bool get_vt_profile_level(AVEMCodecContext *avctx,
                                 CFStringRef    *profile_level_val)
{
    ...
    *profile_level_val = NULL;
    ...
    if (!*profile_level_val) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid Profile/Level.\n");
        return false;
    }
...
static int get_cv_pixel_format(AVEMCodecContext* avctx,
...
static void add_color_attr(AVEMCodecContext *avctx, CFMutableDictionaryRef dict) {
    VTEncContext *vtctx = avctx->priv_data;
...
static int create_cv_pixel_buffer_info(AVEMCodecContext* avctx,
...
static int get_cv_color_primaries(AVEMCodecContext *avctx,
                                  CFStringRef *primaries)
{
    ...
    switch (pri) {
        ...
        default:
            av_em_log(avctx, AV_LOG_ERROR, "Color primaries %s is not supported.\n", av_em_color_primaries_name(pri));
            *primaries = NULL;
            return -1;
    }
...
static int get_cv_transfer_function(AVEMCodecContext *avctx,
                                    ...
                                    CFNumberRef *gamma_level)
{
    ...
    *gamma_level = NULL;
    ...
    switch (trc) {
        ...
        default:
            av_em_log(avctx, AV_LOG_ERROR, "Transfer function %s is not supported.\n", av_em_color_transfer_name(trc));
            return -1;
    }
...
static int get_cv_ycbcr_matrix(AVEMCodecContext *avctx, CFStringRef *matrix) {
    switch(avctx->colorspace) {
        ...
        default:
            av_em_log(avctx, AV_LOG_ERROR, "Color space %s is not supported.\n", av_em_color_space_name(avctx->colorspace));
            return -1;
    }
...
static int vtenc_create_encoder(AVEMCodecContext   *avctx,
                                CMVideoCodecType codec_type,
                                CFStringRef      profile_level,
                                CFNumberRef      gamma_level,
                                CFDictionaryRef  enc_info,
                                ...
                                VTCompressionSessionRef *session)
{
    ...
    if (status || !vtctx->session) {
        av_em_log(avctx, AV_LOG_ERROR, "Error: cannot create compression session: %d\n", status);

#if !TARGET_OS_IPHONE
        if (!vtctx->allow_sw) {
            av_em_log(avctx, AV_LOG_ERROR, "Try -allow_sw 1. The hardware encoder may be busy, or not supported.\n");
        }
#endif
    ...
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Error setting bitrate property: %d\n", status);
        return AVERROR_EXTERNAL;
    }
    ...
    if (profile_level) {
        status = VTSessionSetProperty(vtctx->session,
                                      ...
                                      profile_level);
        if (status) {
            av_em_log(avctx, AV_LOG_ERROR, "Error setting profile/level property: %d\n", status);
            return AVERROR_EXTERNAL;
        }
    }
    ...
    if (avctx->gop_size > 0) {
        ...
        if (status) {
            av_em_log(avctx, AV_LOG_ERROR, "Error setting 'max key-frame interval' property: %d\n", status);
            return AVERROR_EXTERNAL;
        }
    }
    ...
    if (vtctx->frames_before) {
        ...
        if (status == kVTPropertyNotSupportedErr) {
            av_em_log(avctx, AV_LOG_WARNING, "frames_before property is not supported on this device. Ignoring.\n");
        } else if (status) {
            av_em_log(avctx, AV_LOG_ERROR, "Error setting frames_before property: %d\n", status);
        }
    }
    ...
    if (vtctx->frames_after) {
        ...
        if (status == kVTPropertyNotSupportedErr) {
            av_em_log(avctx, AV_LOG_WARNING, "frames_after property is not supported on this device. Ignoring.\n");
        } else if (status) {
            av_em_log(avctx, AV_LOG_ERROR, "Error setting frames_after property: %d\n", status);
        }
    }
    ...
    if (avctx->sample_aspect_ratio.num != 0) {
        ...
        AVEMRational *avpar = &avctx->sample_aspect_ratio;

        av_em_reduce(&avpar->num, &avpar->den,
                   avpar->num,  avpar->den,
                  0xFFFFFFFF);
    ...
    if (vtctx->transfer_function) {
        ...
        if (status) {
            av_em_log(avctx, AV_LOG_WARNING, "Could not set transfer function: %d\n", status);
        }
    }
    ...
    if (vtctx->ycbcr_matrix) {
        ...
        if (status) {
            av_em_log(avctx, AV_LOG_WARNING, "Could not set ycbcr matrix: %d\n", status);
        }
    }
    ...
    if (vtctx->color_primaries) {
        ...
        if (status) {
            av_em_log(avctx, AV_LOG_WARNING, "Could not set color primaries: %d\n", status);
        }
    }
    ...
    if (gamma_level) {
        ...
        if (status) {
            av_em_log(avctx, AV_LOG_WARNING, "Could not set gamma level: %d\n", status);
        }
    }
    ...
    if (!vtctx->has_b_frames) {
        ...
        if (status) {
            av_em_log(avctx, AV_LOG_ERROR, "Error setting 'allow frame reordering' property: %d\n", status);
            return AVERROR_EXTERNAL;
        }
    }
    ...
    if (vtctx->entropy != VT_ENTROPY_NOT_SET) {
        ...
        if (status) {
            av_em_log(avctx, AV_LOG_ERROR, "Error setting entropy property: %d\n", status);
            return AVERROR_EXTERNAL;
        }
    }
    ...
    if (vtctx->realtime) {
        ...
        if (status) {
            av_em_log(avctx, AV_LOG_ERROR, "Error setting realtime property: %d\n", status);
        }
    }

    status = VTCompressionSessionPrepareToEncodeFrames(vtctx->session);
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Error: cannot prepare encoder: %d\n", status);
        return AVERROR_EXTERNAL;
    }
...
static av_cold int vtenc_init(AVEMCodecContext *avctx)
{
    ...
    if (!codec_type) {
        av_em_log(avctx, AV_LOG_ERROR, "Error: no mapping for AVEMCodecID %d\n", avctx->codec_id);
        return AVERROR(EINVAL);
    }

    vtctx->has_b_frames = avctx->max_b_frames > 0;
    if(vtctx->has_b_frames && vtctx->profile == H264_PROF_BASELINE){
        av_em_log(avctx, AV_LOG_WARNING, "Cannot use B-frames with baseline profile. Output will not contain B-frames.\n");
        vtctx->has_b_frames = false;
    }

    if (vtctx->entropy == VT_CABAC && vtctx->profile == H264_PROF_BASELINE) {
        av_em_log(avctx, AV_LOG_WARNING, "CABAC entropy requires 'main' or 'high' profile, but baseline was requested. Encode will not use CABAC entropy.\n");
    ...
    size_t        dst_size)
{
    ...
    while (remaining_src_size > 0) {
        ...
        status = CMBlockBufferCopyDataBytes(block,
                                            ...
                                            size_buf);
        if (status) {
            av_em_log(avctx, AV_LOG_ERROR, "Cannot copy length: %d\n", status);
            return AVERROR_EXTERNAL;
        }
        ...
        if (status) {
            av_em_log(avctx, AV_LOG_ERROR, "Cannot copy data: %d\n", status);
            return AVERROR_EXTERNAL;
        }
    ...
    AVEMPacket          *pkt)
{
    ...
    if (add_header) {
        vid_fmt = CMSampleBufferGetFormatDescription(sample_buffer);
        if (!vid_fmt) {
            av_em_log(avctx, AV_LOG_ERROR, "Cannot get format description.\n");
            return AVERROR_EXTERNAL;
        }
    ...
    status = em_alloc_packet2(avctx, pkt, out_buf_size, out_buf_size);
    if (status < 0)
        return status;
    ...
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Error copying packet data: %d", status);
        return status;
    }
    ...
    if (CMTIME_IS_INVALID(dts)) {
        if (!vtctx->has_b_frames) {
            ...
            av_em_log(avctx, AV_LOG_ERROR, "DTS is invalid.\n");
            return AVERROR_EXTERNAL;
        }
    }
    ...
    size_t         *contiguous_buf_size)
{
    ...
    int av_color_range  = av_em_frame_get_color_range(frame);
    int i;
    int range_guessed;
    int status;
    ...
    if (status) {
        av_em_log(avctx,
            ...
            av_em_get_pix_fmt_name(av_format),
            av_color_range > AVCOL_RANGE_UNSPECIFIED &&
            av_color_range < AVCOL_RANGE_NB ?
               av_em_color_range_name(av_color_range) :
               "Unknown");
...
static int copy_avframe_to_pixel_buffer(AVEMCodecContext   *avctx,
                                        const AVFrame    *frame,
                                        ...
                                        const size_t     *plane_rows)
{
    ...
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Error: Could not unlock CVPixelBuffer base address: %d.\n", status);
        return AVERROR_EXTERNAL;
    }
...
static int create_cv_pixel_buffer(AVEMCodecContext   *avctx,
                                  const AVFrame    *frame,
                                  CVPixelBufferRef *cv_img)
{
    int plane_count;
    ...
    if (status) {
        av_em_log(
            ...
            frame->format,
            av_em_frame_get_color_range(frame),
            status
        );
    ...
    if (!pix_buf_pool) {
        av_em_log(avctx, AV_LOG_ERROR, "Could not get pixel buffer pool.\n");
        return AVERROR_EXTERNAL;
    }
    ...
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Could not create pixel buffer from pool: %d.\n", status);
        return AVERROR_EXTERNAL;
    }
...
#else
    AVFrame *enc_frame = av_em_frame_alloc();
    if (!enc_frame) return AVERROR(ENOMEM);

    status = av_em_frame_ref(enc_frame, frame);
    if (status) {
        av_em_frame_free(&enc_frame);
        return status;
    }
    ...
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Error: Could not create CVPixelBuffer: %d\n", status);
        return AVERROR_EXTERNAL;
    }
#endif
...
static int vtenc_send_frame(AVEMCodecContext *avctx,
                            VTEncContext   *vtctx,
                            const AVFrame  *frame)
{
    CMTime time;
    ...
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Error: cannot encode frame: %d\n", status);
        return AVERROR_EXTERNAL;
    }
    ...
    int            *got_packet)
{
    ...
    if (frame) {
        ...
        if (status) {
            av_em_log(avctx, AV_LOG_ERROR, "Error flushing frames: %d\n", status);
            status = AVERROR_EXTERNAL;
            goto end_nopkt;
        }
    }
                                    ...
                                    CFDictionaryRef  pixel_buffer_info)
{
    ...
    frame->buf[0] = av_em_buffer_alloc(y_size + 2 * chroma_size);
    ...
    status = vtenc_send_frame(avctx, vtctx, frame);
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Error sending frame: %d\n", status);
        goto pe_cleanup;
    }
    ...
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "popping: %d\n", status);
        goto pe_cleanup;
    }
...
static av_cold int vtenc_close(AVEMCodecContext *avctx)
{
    VTEncContext *vtctx = avctx->priv_data;
...
static const AVEMClass h264_videotoolbox_class = {
...
AVEMCodec em_h264_videotoolbox_encoder = {
