...
static int vmd_decode(VmdVideoContext *s, AVFrame *frame)
{
    ...
    if (s->buf[15] & 0x02) {
        ...
        if (bytestream2_get_bytes_left(&gb) >= PALETTE_COUNT * 3) {
            ...
            av_em_log(s->avctx, AV_LOG_ERROR, "Incomplete palette\n");
            return AVERROR_INVALIDDATA;
        }
    }
...
static av_cold int vmdvideo_decode_end(AVEMCodecContext *avctx)
{
    VmdVideoContext *s = avctx->priv_data;

    av_em_frame_free(&s->prev_frame);
    av_em_freep(&s->unpack_buffer);
    s->unpack_buffer_size = 0;

    return 0;
}

static av_cold int vmdvideo_decode_init(AVEMCodecContext *avctx)
{
    VmdVideoContext *s = avctx->priv_data;
    int i;
    unsigned int *palette32;
    ...
    if (s->unpack_buffer_size) {
        s->unpack_buffer = av_em_alloc(s->unpack_buffer_size);
        if (!s->unpack_buffer)
...
static int vmdvideo_decode_frame(AVEMCodecContext *avctx,
                                 ...
                                 AVEMPacket *avpkt)
{
    ...
    /* shuffle frames */
    av_em_frame_unref(s->prev_frame);
    if ((ret = av_em_frame_ref(s->prev_frame, frame)) < 0)
        return ret;
