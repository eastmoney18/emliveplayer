...
static av_cold int screenpresso_close(AVEMCodecContext *avctx)
{
    ScreenpressoContext *ctx = avctx->priv_data;

    av_em_frame_free(&ctx->current);
    av_em_freep(&ctx->inflated_buf);

    return 0;
}

static av_cold int screenpresso_init(AVEMCodecContext *avctx)
{
    ScreenpressoContext *ctx = avctx->priv_data;
...
static int screenpresso_decode_frame(AVEMCodecContext *avctx, void *data,
                                     int *got_frame, AVEMPacket *avpkt)
{
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Compression level %d\n", avpkt->data[0] >> 4);
    ...
    ret = em_reget_buffer(avctx, ctx->current);
    if (ret < 0)
        return ret;
    ...
    if (keyframe)
        av_em_image_copy_plane(ctx->current->data[0] +
    ...
    ret = av_em_frame_ref(frame, ctx->current);
    if (ret < 0)
        return ret;
...
AVEMCodec ff_screenpresso_decoder = {
    .name           = "screenpresso",
