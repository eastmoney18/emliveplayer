...
int ff_jpegls_decode_lse(MJpegDecodeContext *s)
{
    ...
    case 1:
        ...
        if(s->avctx->debug & FF_DEBUG_PICT_INFO) {
            av_em_log(s->avctx, AV_LOG_DEBUG, "Coding parameters maxval:%d T1:%d T2:%d T3:%d reset:%d\n",
...
//        ff_jpegls_reset_coding_parameters(s, 0);
    ...
    case 3:
        ...
        if (wt < 1 || wt > MAX_COMPONENTS) {
            avpriv_em_request_sample(s->avctx, "wt %d", wt);
        ...
        if(s->avctx->debug & FF_DEBUG_PICT_INFO) {
            av_em_log(s->avctx, AV_LOG_DEBUG, "LSE palette %d tid:%d wt:%d maxtab:%d\n", id, tid, wt, maxtab);
        }
        if (maxtab >= 256) {
            avpriv_em_request_sample(s->avctx, ">8bit palette");
            return AVERROR_PATCHWELCOME;
        }
        ...
        break;
    case 4:
        avpriv_em_request_sample(s->avctx, "oversize image");
        return AVERROR(ENOSYS);
    default:
        av_em_log(s->avctx, AV_LOG_ERROR, "invalid id %d\n", id);
        return AVERROR_INVALIDDATA;
    }
    ...
    while (x < w) {
        ...
        if ((FFABS(D0) <= state->near) &&
            ...
            while (get_bits1(&s->gb)) {
                int r;
                r = 1 << em_log2_run[state->run_index[comp]];
                if (x + r * stride > w)
                    r = (w - x) / stride;
                for (i = 0; i < r; i++) {
                    W(dst, x, Ra);
                ...
                if (r != 1 << em_log2_run[state->run_index[comp]])
                    return;
            ...
            /* decode aborted run */
            r = em_log2_run[state->run_index[comp]];
            if (r)
            ...
            if (x >= w) {
                av_em_log(NULL, AV_LOG_ERROR, "run overflow\n");
                av_assert0(x <= w);
                return;
            }
            ...
            err    = ls_get_code_runterm(&s->gb, state, RItype,
                                         em_log2_run[state->run_index[comp]]);
                             ...
                             int point_transform, int ilv)
{
    ...
    zero = av_em_mallocz(s->picture_ptr->linesize[0]);
    if (!zero)
        return AVERROR(ENOMEM);
    last = zero;
    cur  = s->picture_ptr->data[0];

    state = av_em_mallocz(sizeof(JLSState));
    if (!state) {
        av_em_free(zero);
        return AVERROR(ENOMEM);
    }
    ...
    if (s->avctx->debug & FF_DEBUG_PICT_INFO) {
        ...
        av_em_log(s->avctx, AV_LOG_DEBUG, "JPEG params: ILV=%i Pt=%i BPP=%i, scan = %i\n",
    ...
    if (ilv == 0) { /* separate planes */
        ...
        avpriv_em_report_missing_feature(s->avctx, "Sample interleaved images");
    ...
    if (s->xfrm && s->nb_components == 3) {
        ...
        if (s->bits <= 8) {
            ...
            avpriv_em_report_missing_feature(s->avctx, "16bit xfrm");
    }
