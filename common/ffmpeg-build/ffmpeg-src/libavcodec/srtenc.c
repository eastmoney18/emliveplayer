...
static void srt_print(SRTContext *s, const char *str, ...)
{
    va_list vargs;
    va_start(vargs, str);
    av_em_vbprintf(&s->buffer, str, vargs);
    va_end(vargs);
}
...
static void srt_stack_push_pop(SRTContext *s, const char c, int close)
{
    if (close) {
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "tag stack overflow\n");
}
...
static av_cold int srt_encode_init(AVEMCodecContext *avctx)
{
                        ...
                        int t1, int t2)
{
    ...
    // encoding API passing the AVEMPacket is available.
}
...
static int encode_frame(AVEMCodecContext *avctx,
...
static int srt_encode_frame(AVEMCodecContext *avctx,
                               unsigned char *buf, int bufsize, const AVSubtitle *sub)
{
    return encode_frame(avctx, buf, bufsize, sub, &srt_callbacks);
}

static int text_encode_frame(AVEMCodecContext *avctx,
                             unsigned char *buf, int bufsize, const AVSubtitle *sub)
{
    return encode_frame(avctx, buf, bufsize, sub, &text_callbacks);
}

static int srt_encode_close(AVEMCodecContext *avctx)
{
    SRTContext *s = avctx->priv_data;
