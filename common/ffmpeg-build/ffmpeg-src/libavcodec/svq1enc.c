...
static void svq1_write_header(SVQ1EncContext *s, int frame_type)
{
    ...
    if (frame_type == AV_PICTURE_TYPE_I) {
        ...
        i = em_match_2uint16((void*)ff_svq1_frame_size_table,
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (!s->motion_val8[plane]) {
            s->motion_val8[plane]  = av_em_mallocz((s->m.b8_stride *
                                                 block_height * 2 + 2) *
                                                2 * sizeof(int16_t));
            s->motion_val16[plane] = av_em_mallocz((s->m.mb_stride *
                                                 (block_height + 2) + 1) *
                                                2 * sizeof(int16_t));
    ...
    for (y = 0; y < block_height; y++) {
        ...
        for (x = 0; x < block_width; x++) {
            ...
            if (s->pict_type == AV_PICTURE_TYPE_P) {
                ...
                motion_ptr = em_h263_pred_motion(&s->m, 0, 0, &pred_x, &pred_y);
                if (s->m.mb_type[x + y * s->m.mb_stride] &
                    ...
                    em_h263_encode_motion(&s->m.pb, mx - pred_x, 1);
                    em_h263_encode_motion(&s->m.pb, my - pred_y, 1);
...
static av_cold int svq1_encode_end(AVEMCodecContext *avctx)
{
    SVQ1EncContext *const s = avctx->priv_data;
    int i;

    av_em_log(avctx, AV_LOG_DEBUG, "RD: %f\n",
    ...
    av_em_freep(&s->m.me.scratchpad);
    av_em_freep(&s->m.me.map);
    av_em_freep(&s->m.me.score_map);
    av_em_freep(&s->mb_type);
    av_em_freep(&s->dummy);
    av_em_freep(&s->scratchbuf);

    for (i = 0; i < 3; i++) {
        av_em_freep(&s->motion_val8[i]);
        av_em_freep(&s->motion_val16[i]);
    }

    av_em_frame_free(&s->current_picture);
    av_em_frame_free(&s->last_picture);

    return 0;
}

static av_cold int svq1_encode_init(AVEMCodecContext *avctx)
{
    SVQ1EncContext *const s = avctx->priv_data;
    int ret;

    if (avctx->width >= 4096 || avctx->height >= 4096) {
        av_em_log(avctx, AV_LOG_ERROR, "Dimensions too large, maximum is 4095x4095\n");
    ...
    s->current_picture = av_em_frame_alloc();
    s->last_picture    = av_em_frame_alloc();
    ...
    s->m.me.temp           =
    s->m.me.scratchpad     = av_em_mallocz((avctx->width + 64) *
                                        2 * 16 * 2 * sizeof(uint8_t));
    s->m.me.map            = av_em_mallocz(ME_MAP_SIZE * sizeof(uint32_t));
    s->m.me.score_map      = av_em_mallocz(ME_MAP_SIZE * sizeof(uint32_t));
    s->mb_type             = av_em_mallocz((s->y_block_width + 1) *
                                        s->y_block_height * sizeof(int16_t));
    s->dummy               = av_em_mallocz((s->y_block_width + 1) *
    ...
    em_h263_encode_init(&s->m); // mv_penalty

    return 0;
}

static int svq1_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                             const AVFrame *pict, int *got_packet)
{
    SVQ1EncContext *const s = avctx->priv_data;
    int i, ret;

    if ((ret = em_alloc_packet2(avctx, pkt, s->y_block_width * s->y_block_height *
    ...
    if (!s->current_picture->data[0]) {
        if ((ret = em_get_buffer(avctx, s->current_picture, 0)) < 0) {
            return ret;
        }
    }
    if (!s->last_picture->data[0]) {
        ret = em_get_buffer(avctx, s->last_picture, 0);
        if (ret < 0)
            return ret;
    }
    if (!s->scratchbuf) {
        s->scratchbuf = av_em_malloc_array(s->current_picture->linesize[0], 16 * 3);
        if (!s->scratchbuf)
            return AVERROR(ENOMEM);
    }
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    em_side_data_set_encoder_stats(pkt, pict->quality, NULL, 0, s->pict_type);
    ...
    for (i = 0; i < 3; i++)
        if (svq1_encode_plane(s, i,
            ...
            int j;
            for (j = 0; j < i; j++) {
                av_em_freep(&s->motion_val8[j]);
                av_em_freep(&s->motion_val16[j]);
            }
            av_em_freep(&s->scratchbuf);
            return -1;
        }
...
static const AVEMClass svq1enc_class = {
    .class_name = "svq1enc",
