...
static av_cold int pcm_encode_init(AVEMCodecContext *avctx)
{
    avctx->frame_size = 0;
    ...
    avctx->bits_per_coded_sample = av_em_get_bits_per_sample(avctx->codec->id);
...
static int pcm_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                            const AVFrame *frame, int *got_packet_ptr)
{
    ...
    sample_size = av_em_get_bits_per_sample(avctx->codec->id) / 8;
    n           = frame->nb_samples * avctx->channels;
    samples     = (const short *)frame->data[0];

    if ((ret = em_alloc_packet2(avctx, avpkt, n * sample_size, n * sample_size)) < 0)
        return ret;
    dst = avpkt->data;
    ...
    case AV_CODEC_ID_PCM_S24DAUD:
        for (; n > 0; n--) {
            uint32_t tmp = em_reverse[(*samples >> 8) & 0xff] +
                           (em_reverse[*samples & 0xff] << 8);
...
static av_cold int pcm_decode_init(AVEMCodecContext *avctx)
{
    PCMDecode *s = avctx->priv_data;
    int i;

    if (avctx->channels <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "PCM channels out of bounds\n");
    ...
    if (avctx->sample_fmt == AV_SAMPLE_FMT_S32)
        avctx->bits_per_raw_sample = av_em_get_bits_per_sample(avctx->codec_id);
...
static int pcm_decode_frame(AVEMCodecContext *avctx, void *data,
                            int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    sample_size = av_em_get_bits_per_sample(avctx->codec_id) / 8;

    /* av_em_get_bits_per_sample returns 0 for AV_CODEC_ID_PCM_DVD */
    samples_per_block = 1;
    ...
    if (sample_size == 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid sample_size\n");
        return AVERROR(EINVAL);
    }
    ...
    if (avctx->codec_id != avctx->codec->id) {
        av_em_log(avctx, AV_LOG_ERROR, "codec ids mismatch\n");
        return AVERROR(EINVAL);
    }
    ...
    case AV_CODEC_ID_PCM_S24DAUD:
        for (; n > 0; n--) {
            ...
            AV_WN16A(samples, em_reverse[(v >> 8) & 0xff] +
                             (em_reverse[v        & 0xff] << 8));
            samples += 2;
        }
        break;
