...
static void vp9_unref_frame(AVEMCodecContext *ctx, VP9Frame *f)
{
    em_thread_release_buffer(ctx, &f->tf);
    av_em_buffer_unref(&f->extradata);
    av_em_buffer_unref(&f->hwaccel_priv_buf);
    f->segmentation_map = NULL;
    f->hwaccel_picture_private = NULL;
}

static int vp9_alloc_frame(AVEMCodecContext *ctx, VP9Frame *f)
{
    VP9Context *s = ctx->priv_data;
    int ret, sz;

    if ((ret = em_thread_get_buffer(ctx, &f->tf, AV_GET_BUFFER_FLAG_REF)) < 0)
        return ret;
    sz = 64 * s->sb_cols * s->sb_rows;
    if (!(f->extradata = av_em_buffer_allocz(sz * (1 + sizeof(struct VP9mvrefPair))))) {
        goto fail;
    }
    ...
    if (ctx->hwaccel) {
        ...
        if (hwaccel->frame_priv_data_size) {
            f->hwaccel_priv_buf = av_em_buffer_allocz(hwaccel->frame_priv_data_size);
...
static int vp9_ref_frame(AVEMCodecContext *ctx, VP9Frame *dst, VP9Frame *src)
{
    int res;

    if ((res = em_thread_ref_frame(&dst->tf, &src->tf)) < 0) {
        return res;
    } else if (!(dst->extradata = av_em_buffer_ref(src->extradata))) {
        goto fail;
    }
...
static int update_size(AVEMCodecContext *ctx, int w, int h)
{
#define HWACCEL_MAX (CONFIG_VP9_DXVA2_HWACCEL + CONFIG_VP9_D3D11VA_HWACCEL + CONFIG_VP9_VAAPI_HWACCEL)
    ...
    if (!(s->pix_fmt == s->gf_fmt && w == s->w && h == s->h)) {
        if ((res = em_set_dimensions(ctx, w, h)) < 0)
            return res;
        ...
        *fmtp = AV_PIX_FMT_NONE;
        ...
        res = em_thread_get_format(ctx, pix_fmts);
        if (res < 0)
            return res;
...
#undef assign
    ...
    av_em_freep(&s->b_base);
    av_em_freep(&s->block_base);
...
static int update_block_buffers(AVEMCodecContext *ctx)
{
    VP9Context *s = ctx->priv_data;
    ...
    av_em_free(s->b_base);
    av_em_free(s->block_base);
    chroma_blocks = 64 * 64 >> (s->ss_h + s->ss_v);
    chroma_eobs   = 16 * 16 >> (s->ss_h + s->ss_v);
    if (s->s.frames[CUR_FRAME].uses_2pass) {
        int sbs = s->sb_cols * s->sb_rows;

        s->b_base = av_em_malloc_array(s->cols * s->rows, sizeof(VP9Block));
        s->block_base = av_em_mallocz(((64 * 64 + 2 * chroma_blocks) * bytesperpixel * sizeof(int16_t) +
        ...
        s->b_base = av_em_alloc(sizeof(VP9Block));
        s->block_base = av_em_mallocz((64 * 64 + 2 * chroma_blocks) * bytesperpixel * sizeof(int16_t) +
                                   16 * 16 + 2 * chroma_eobs);
        if (!s->b_base || !s->block_base)
            return AVERROR(ENOMEM);
...
static int read_colorspace_details(AVEMCodecContext *ctx)
{
    ...
    if (ctx->colorspace == AVCOL_SPC_RGB) { // RGB = profile 1
        ...
        if (ctx->profile & 1) {
            if (get_bits1(&s->gb)) {
                av_em_log(ctx, AV_LOG_ERROR, "Reserved bit set in RGB\n");
                return AVERROR_INVALIDDATA;
            }
        } else {
            av_em_log(ctx, AV_LOG_ERROR, "RGB not supported in profile %d\n",
                   ctx->profile);
            return AVERROR_INVALIDDATA;
        }
    } else {
        ...
        if (ctx->profile & 1) {
            ...
            if (s->pix_fmt == AV_PIX_FMT_YUV420P) {
                av_em_log(ctx, AV_LOG_ERROR, "YUV 4:2:0 not supported in profile %d\n",
                       ctx->profile);
                return AVERROR_INVALIDDATA;
            } else if (get_bits1(&s->gb)) {
                av_em_log(ctx, AV_LOG_ERROR, "Profile %d color details reserved bit set\n",
                       ctx->profile);
                return AVERROR_INVALIDDATA;
            }
        } else {
...
static int decode_frame_header(AVEMCodecContext *ctx,
                               const uint8_t *data, int size, int *ref)
{
    ...
    if ((res = init_get_bits8(&s->gb, data, size)) < 0) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to initialize bitstream reader\n");
        return res;
    }
    if (get_bits(&s->gb, 2) != 0x2) { // frame marker
        av_em_log(ctx, AV_LOG_ERROR, "Invalid frame marker\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ctx->profile > 3) {
        av_em_log(ctx, AV_LOG_ERROR, "Profile %d is not yet supported\n", ctx->profile);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->s.h.keyframe) {
        if (get_bits_long(&s->gb, 24) != VP9_SYNCCODE) { // synccode
            av_em_log(ctx, AV_LOG_ERROR, "Invalid sync code\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        if (s->s.h.intraonly) {
            ...
            if (!s->s.refs[s->s.h.refidx[0]].f->buf[0] ||
                ...
                av_em_log(ctx, AV_LOG_ERROR, "Not all references are available\n");
                return AVERROR_INVALIDDATA;
            }
            if (get_bits1(&s->gb)) {
    ...
    if ((res = update_size(ctx, w, h)) < 0) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to initialize decoder for %dx%d @ %d\n",
               w, h, s->pix_fmt);
        return res;
    }
    ...
    if (s->s.h.tiling.tile_cols != (1 << s->s.h.tiling.log2_tile_cols)) {
        ...
        s->c_b = av_em_fast_realloc(s->c_b, &s->c_b_size,
                                 sizeof(VP56RangeCoder) * s->s.h.tiling.tile_cols);
        if (!s->c_b) {
            av_em_log(ctx, AV_LOG_ERROR, "Ran out of memory during range coder init\n");
            return AVERROR(ENOMEM);
        }
    }
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (ref->format != ctx->pix_fmt) {
                av_em_log(ctx, AV_LOG_ERROR,
                       ...
                       av_em_get_pix_fmt_name(ref->format),
                       av_em_get_pix_fmt_name(ctx->pix_fmt));
                return AVERROR_INVALIDDATA;
    ...
    if (size2 > size - (data2 - data)) {
        av_em_log(ctx, AV_LOG_ERROR, "Invalid compressed header size\n");
        return AVERROR_INVALIDDATA;
    }
    ff_vp56_init_range_decoder(&s->c, data2, size2);
    if (vp56_rac_get_prob_branchy(&s->c, 128)) { // marker bit
        av_em_log(ctx, AV_LOG_ERROR, "Marker bit was set\n");
        return AVERROR_INVALIDDATA;
    }
...
#define RETURN_MV(mv) \
    ...
    if (s->s.h.use_last_frame_mvs) {
        ...
        if (!s->s.frames[REF_FRAME_MVPAIR].uses_2pass)
            em_thread_await_progress(&s->s.frames[REF_FRAME_MVPAIR].tf, row >> 3, 0);
        if (mv->ref[0] == ref) {
            RETURN_MV(mv->mv[0]);
        } else if (mv->ref[1] == ref) {
            RETURN_MV(mv->mv[1]);
...
static void decode_mode(AVEMCodecContext *ctx)
{
    ...
    if (!s->s.h.segmentation.enabled) {
        ...
        if (!s->s.h.errorres && s->s.frames[REF_FRAME_SEGMAP].segmentation_map) {
            ...
            if (!s->s.frames[REF_FRAME_SEGMAP].uses_2pass)
                em_thread_await_progress(&s->s.frames[REF_FRAME_SEGMAP].tf, row >> 3, 0);
            for (y = 0; y < h4; y++) {
...
static av_always_inline int decode_coeffs(AVEMCodecContext *ctx, int is8bitsperpixel)
{
    VP9Context *s = ctx->priv_data;
    VP9Block *b = s->b;
...
static int decode_coeffs_8bpp(AVEMCodecContext *ctx)
{
    return decode_coeffs(ctx, 1);
}

static int decode_coeffs_16bpp(AVEMCodecContext *ctx)
{
...
static av_always_inline void intra_recon(AVEMCodecContext *ctx, ptrdiff_t y_off,
...
static void intra_recon_8bpp(AVEMCodecContext *ctx, ptrdiff_t y_off, ptrdiff_t uv_off)
{
    intra_recon(ctx, y_off, uv_off, 1);
}

static void intra_recon_16bpp(AVEMCodecContext *ctx, ptrdiff_t y_off, ptrdiff_t uv_off)
{
                                              ...
                                              int bw, int bh, int w, int h, int bytesperpixel)
{
    ...
    th = (y + bh + 4 * !!my + 7) >> 6;
    em_thread_await_progress(ref_frame, FFMAX(th, 0), 0);
    if (x < !!mx * 3 || y < !!my * 3 ||
...
static av_always_inline void inter_recon(AVEMCodecContext *ctx, int bytesperpixel)
{
    VP9Context *s = ctx->priv_data;
    VP9Block *b = s->b;
    int row = s->row, col = s->col;
...
static void inter_recon_8bpp(AVEMCodecContext *ctx)
{
    inter_recon(ctx, 1);
}

static void inter_recon_16bpp(AVEMCodecContext *ctx)
{
    inter_recon(ctx, 2);
}
...
static void decode_b(AVEMCodecContext *ctx, int row, int col,
...
static void decode_sb(AVEMCodecContext *ctx, int row, int col, struct VP9Filter *lflvl,
...
static void decode_sb_mem(AVEMCodecContext *ctx, int row, int col, struct VP9Filter *lflvl,
                          ptrdiff_t yoff, ptrdiff_t uvoff, enum BlockLevel bl)
{
    VP9Context *s = ctx->priv_data;
    VP9Block *b = s->b;
...
static void loopfilter_sb(AVEMCodecContext *ctx, struct VP9Filter *lflvl,
...
static av_cold int vp9_decode_free(AVEMCodecContext *ctx)
{
    VP9Context *s = ctx->priv_data;
    int i;
    ...
    for (i = 0; i < 3; i++) {
        ...
        av_em_frame_free(&s->s.frames[i].tf.f);
    }
    for (i = 0; i < 8; i++) {
        if (s->s.refs[i].f->buf[0])
            em_thread_release_buffer(ctx, &s->s.refs[i]);
        av_em_frame_free(&s->s.refs[i].f);
        if (s->next_refs[i].f->buf[0])
            em_thread_release_buffer(ctx, &s->next_refs[i]);
        av_em_frame_free(&s->next_refs[i].f);
    }
    free_buffers(s);
    av_em_freep(&s->c_b);
    s->c_b_size = 0;
...
static int vp9_decode_frame(AVEMCodecContext *ctx, void *frame,
                            int *got_frame, AVEMPacket *pkt)
{
    ...
    if ((res = decode_frame_header(ctx, data, size, &ref)) < 0) {
        ...
        if (!s->s.refs[ref].f->buf[0]) {
            av_em_log(ctx, AV_LOG_ERROR, "Requested reference %d not available\n", ref);
            return AVERROR_INVALIDDATA;
        }
        if ((res = av_em_frame_ref(frame, s->s.refs[ref].f)) < 0)
            return res;
        ...
        for (i = 0; i < 8; i++) {
            if (s->next_refs[i].f->buf[0])
                em_thread_release_buffer(ctx, &s->next_refs[i]);
            if (s->s.refs[i].f->buf[0] &&
                (res = em_thread_ref_frame(&s->next_refs[i], &s->s.refs[i])) < 0)
                return res;
        }
        *got_frame = 1;
        return pkt->size;
    ...
    for (i = 0; i < 8; i++) {
        ...
        if (s->s.h.refreshrefmask & (1 << i)) {
            res = em_thread_ref_frame(&s->next_refs[i], &s->s.frames[CUR_FRAME].tf);
        } else if (s->s.refs[i].f->buf[0]) {
            res = em_thread_ref_frame(&s->next_refs[i], &s->s.refs[i]);
        }
        if (res < 0)
            return res;
    }
        ...
        for (tile_row = 0; tile_row < s->s.h.tiling.tile_rows; tile_row++) {
            ...
            if (s->pass != 2) {
                for (tile_col = 0; tile_col < s->s.h.tiling.tile_cols; tile_col++) {
                    ...
                    if (tile_size > size) {
                        em_thread_report_progress(&s->s.frames[CUR_FRAME].tf, INT_MAX, 0);
                        return AVERROR_INVALIDDATA;
                    }
            ...
            for (row = s->tile_row_start; row < s->tile_row_end;
                ...
                em_thread_report_progress(&s->s.frames[CUR_FRAME].tf, row >> 3, 0);
            }
        }
...
finish:
    ...
    for (i = 0; i < 8; i++) {
        if (s->s.refs[i].f->buf[0])
            em_thread_release_buffer(ctx, &s->s.refs[i]);
        if (s->next_refs[i].f->buf[0] &&
            (res = em_thread_ref_frame(&s->s.refs[i], &s->next_refs[i])) < 0)
            return res;
    }

    if (!s->s.h.invisible) {
        if ((res = av_em_frame_ref(frame, s->s.frames[CUR_FRAME].tf.f)) < 0)
            return res;
        *got_frame = 1;
    }
...
static void vp9_decode_flush(AVEMCodecContext *ctx)
{
    VP9Context *s = ctx->priv_data;
    int i;
...
static int init_frames(AVEMCodecContext *ctx)
{
    VP9Context *s = ctx->priv_data;
    int i;

    for (i = 0; i < 3; i++) {
        s->s.frames[i].tf.f = av_em_frame_alloc();
        if (!s->s.frames[i].tf.f) {
            vp9_decode_free(ctx);
            av_em_log(ctx, AV_LOG_ERROR, "Failed to allocate frame buffer %d\n", i);
            return AVERROR(ENOMEM);
        }
    }
    for (i = 0; i < 8; i++) {
        s->s.refs[i].f = av_em_frame_alloc();
        s->next_refs[i].f = av_em_frame_alloc();
...
static av_cold int vp9_decode_init(AVEMCodecContext *ctx)
{
    VP9Context *s = ctx->priv_data;
...
#if HAVE_THREADS
static av_cold int vp9_decode_init_thread_copy(AVEMCodecContext *avctx)
{
    return init_frames(avctx);
}

static int vp9_decode_update_thread_context(AVEMCodecContext *dst, const AVEMCodecContext *src)
{
    int i, res;
    ...
    for (i = 0; i < 8; i++) {
        if (s->s.refs[i].f->buf[0])
            em_thread_release_buffer(dst, &s->s.refs[i]);
        if (ssrc->next_refs[i].f->buf[0]) {
            if ((res = em_thread_ref_frame(&s->s.refs[i], &ssrc->next_refs[i])) < 0)
                return res;
        }
    }
