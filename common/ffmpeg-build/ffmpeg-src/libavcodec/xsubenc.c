...
static int xsub_encode(AVEMCodecContext *avctx, unsigned char *buf,
                       int bufsize, const AVSubtitle *h)
{
    ...
    if (bufsize < 27 + 7*2 + 4*3) {
        av_em_log(avctx, AV_LOG_ERROR, "Buffer too small for XSUB header.\n");
        return -1;
    }

    // TODO: support multiple rects
    if (h->num_rects != 1)
        av_em_log(avctx, AV_LOG_WARNING, "Only single rects supported (%d in subtitle.)\n", h->num_rects);
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (!h->rects[0]->data[0] || !h->rects[0]->data[1]) {
        av_em_log(avctx, AV_LOG_WARNING, "No subtitle bitmap available.\n");
        return -1;
    }

    // TODO: color reduction, similar to dvdsub encoder
    if (h->rects[0]->nb_colors > 4)
        av_em_log(avctx, AV_LOG_WARNING, "No more than 4 subtitle colors supported (%d found.)\n", h->rects[0]->nb_colors);

    // TODO: Palette swapping if color zero is not transparent
    if (((uint32_t *)h->rects[0]->data[1])[0] & 0xff000000)
        av_em_log(avctx, AV_LOG_WARNING, "Color index 0 is not transparent. Transparency will be messed up.\n");

    if (make_tc(startTime, start_tc) || make_tc(endTime, end_tc)) {
        av_em_log(avctx, AV_LOG_WARNING, "Time code >= 100 hours.\n");
        return -1;
    }
...
static av_cold int xsub_encoder_init(AVEMCodecContext *avctx)
{
    if (!avctx->codec_tag)
