...
typedef struct Mpeg1Context {
    ...
    AVEMPanScan pan_scan;         /* some temporary storage for the panscan */
    AVStereo3D stereo3d;
    int has_stereo3d;
    uint8_t *a53_caption;
    ...
    int has_afd;
    int slice_count;
    AVEMRational save_aspect;
    int save_width, save_height, save_progressive_seq;
    AVEMRational frame_rate_ext;  /* MPEG-2 specific framerate modificator */
...
#define check_scantable_index(ctx, x)                                         \
    do {                                                                      \
        if ((x) > MAX_INDEX) {                                                \
            av_em_log(ctx->avctx, AV_LOG_ERROR, "ac-tex damaged at %d %d\n",     \
                   ctx->mb_x, ctx->mb_y);                                     \
...
static int mpeg_decode_mb(MpegEncContext *s, int16_t block[12][64])
{
    ...
    if (s->mb_skip_run-- != 0) {
        if (s->pict_type == AV_PICTURE_TYPE_P) {
            ...
            if (IS_INTRA(mb_type)) {
                av_em_log(s->avctx, AV_LOG_ERROR, "skip with previntra\n");
                return AVERROR_INVALIDDATA;
            }
            s->current_picture.mb_type[s->mb_x + s->mb_y * s->mb_stride] =
...
//    motion_type = 0; /* avoid warning */
    if (IS_INTRA(mb_type)) {
        ...
        if ((CONFIG_MPEG1_XVMC_HWACCEL || CONFIG_MPEG2_XVMC_HWACCEL) && s->pack_pblocks)
            em_xvmc_pack_pblocks(s, -1); // inter are always full blocks
        ...
        if (s->codec_id == AV_CODEC_ID_MPEG2VIDEO) {
            ...
            for (i = 0; i < 6; i++) {
                ret = ff_mpeg1_decode_block_intra(&s->gb,
                                                  ...
                                                  i, s->qscale);
                if (ret < 0) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "ac-tex damaged at %d %d\n",
                           s->mb_x, s->mb_y);
                    return ret;
                }
        ...
        if (mb_type & MB_TYPE_ZERO_MV) {
            ...
            case MT_DMV:
                if (s->progressive_sequence){
                    av_em_log(s->avctx, AV_LOG_ERROR, "MT_DMV in progressive_sequence\n");
                    return AVERROR_INVALIDDATA;
                }
...
static av_cold int mpeg_decode_init(AVEMCodecContext *avctx)
{
...
#if HAVE_THREADS
static int mpeg_decode_update_thread_context(AVEMCodecContext *avctx,
                                             const AVEMCodecContext *avctx_from)
{
...
#if FF_API_VDPAU
static inline int uses_vdpau(AVEMCodecContext *avctx) {
    return avctx->pix_fmt == AV_PIX_FMT_VDPAU_MPEG1 || avctx->pix_fmt == AV_PIX_FMT_VDPAU_MPEG2;
}
#endif

static enum AVPixelFormat mpeg_get_pixelformat(AVEMCodecContext *avctx)
{
    ...
    return em_thread_get_format(avctx, pix_fmts);
}

static void setup_hwaccel_for_pixfmt(AVEMCodecContext *avctx)
{
...
static int mpeg_decode_postinit(AVEMCodecContext *avctx)
{
    Mpeg1Context *s1  = avctx->priv_data;
    MpegEncContext *s = &s1->mpeg_enc_ctx;
    ...
    if (avctx->codec_id == AV_CODEC_ID_MPEG1VIDEO) {
        // MPEG-1 aspect
        avctx->sample_aspect_ratio = av_em_d2q(1.0 / ff_mpeg1_aspect[s->aspect_ratio_info], 255);
    } else { // MPEG-2
        // MPEG-2 aspect
        if (s->aspect_ratio_info > 1) {
            AVEMRational dar =
                av_em_mul_q(av_em_div_q(ff_mpeg2_aspect[s->aspect_ratio_info],
                                  (AVEMRational) { s1->pan_scan.width,
                                                 s1->pan_scan.height }),
                         (AVEMRational) { s->width, s->height });
            ...
            if ((s1->pan_scan.width == 0) || (s1->pan_scan.height == 0) ||
                (av_em_cmp_q(dar, (AVEMRational) { 4, 3 }) &&
                 av_em_cmp_q(dar, (AVEMRational) { 16, 9 }))) {
                s->avctx->sample_aspect_ratio =
                    av_em_div_q(ff_mpeg2_aspect[s->aspect_ratio_info],
                             (AVEMRational) { s->width, s->height });
            } else {
                s->avctx->sample_aspect_ratio =
                    av_em_div_q(ff_mpeg2_aspect[s->aspect_ratio_info],
                             (AVEMRational) { s1->pan_scan.width, s1->pan_scan.height });
// issue1613 4/3 16/9 -> 16/9
// res_change_ffmpeg_aspect.ts 4/3 225/44 ->4/3
// widescreen-issue562.mpg 4/3 16/9 -> 16/9
//                s->avctx->sample_aspect_ratio = av_em_mul_q(s->avctx->sample_aspect_ratio, (AVEMRational) {s->width, s->height});
    ...
    if (av_em_image_check_sar(s->width, s->height,
    ...
    if ((s1->mpeg_enc_ctx_allocated == 0)                   ||
        ...
        av_em_cmp_q(s1->save_aspect, s->avctx->sample_aspect_ratio) ||
        ...
        ret = em_set_dimensions(avctx, s->width, s->height);
        if (ret < 0)
            return ret;
        ...
        if (avctx->codec_id == AV_CODEC_ID_MPEG1VIDEO) {
            ...
            // MPEG-2 fps
            av_em_reduce(&s->avctx->framerate.num,
...
static int mpeg1_decode_picture(AVEMCodecContext *avctx, const uint8_t *buf,
                                int buf_size)
{
    Mpeg1Context *s1  = avctx->priv_data;
    MpegEncContext *s = &s1->mpeg_enc_ctx;
...
static void mpeg_decode_sequence_extension(Mpeg1Context *s1)
{
    ...
    if (!s->chroma_format) {
        s->chroma_format = 1;
        av_em_log(s->avctx, AV_LOG_WARNING, "Chroma format invalid\n");
    }
...
static void mpeg_decode_sequence_display_extension(Mpeg1Context *s1)
{
    ...
    if (s->avctx->debug & FF_DEBUG_PICT_INFO)
        av_em_log(s->avctx, AV_LOG_DEBUG, "sde w:%d, h:%d\n", w, h);
}
                       ...
                       uint16_t matrix1[64], int intra)
{
    ...
    for (i = 0; i < 64; i++) {
        ...
        if (v == 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "matrix damaged\n");
            return AVERROR_INVALIDDATA;
        }
        if (intra && i == 0 && v != 8) {
            av_em_log(s->avctx, AV_LOG_DEBUG, "intra matrix specifies invalid DC quantizer %d, ignoring\n", v);
...
static int mpeg_field_start(MpegEncContext *s, const uint8_t *buf, int buf_size)
{
    ...
    if (s->first_field || s->picture_structure == PICT_FRAME) {
        ...
        pan_scan = av_em_frame_new_side_data(s->current_picture_ptr->f,
        ...
        if (s1->a53_caption) {
            AVFrameSideData *sd = av_em_frame_new_side_data(
                s->current_picture_ptr->f, AV_FRAME_DATA_A53_CC,
                s1->a53_caption_size);
            if (sd)
                memcpy(sd->data, s1->a53_caption, s1->a53_caption_size);
            av_em_freep(&s1->a53_caption);
            avctx->properties |= FF_CODEC_PROPERTY_CLOSED_CAPTIONS;
        }

        if (s1->has_stereo3d) {
            AVStereo3D *stereo = av_em_stereo3d_create_side_data(s->current_picture_ptr->f);
            if (!stereo)
                return AVERROR(ENOMEM);
        ...
        if (s1->has_afd) {
            AVFrameSideData *sd =
                av_em_frame_new_side_data(s->current_picture_ptr->f,
                                       AV_FRAME_DATA_AFD, 1);
            if (!sd)
                return AVERROR(ENOMEM);
        ...
        if (HAVE_THREADS && (avctx->active_thread_type & FF_THREAD_FRAME))
            em_thread_finish_setup(avctx);
    } else { // second field
        int i;

        if (!s->current_picture_ptr) {
            av_em_log(s->avctx, AV_LOG_ERROR, "first field missing\n");
            return AVERROR_INVALIDDATA;
        }
                             ...
                             const uint8_t **buf, int buf_size)
{
    ...
    if (s->qscale == 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "qscale == 0\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (mb_y == 0 && s->codec_tag == AV_RL32("SLIF")) {
        ...
        while (get_bits_left(&s->gb) > 0) {
            int code = get_vlc2(&s->gb, ff_mbincr_vlc.table,
                                MBINCR_VLC_BITS, 2);
            if (code < 0) {
                av_em_log(s->avctx, AV_LOG_ERROR, "first mb_incr damaged\n");
                return AVERROR_INVALIDDATA;
            }
            if (code >= 33) {
                if (code == 33)
    ...
    if (s->mb_x >= (unsigned) s->mb_width) {
        av_em_log(s->avctx, AV_LOG_ERROR, "initial skip overflow\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (avctx->hwaccel && avctx->hwaccel->decode_slice) {
        ...
        int start_code = -1;
        buf_end = avpriv_em_find_start_code(buf_start + 2, *buf + buf_size, &start_code);
    ...
    for (;;) {
        // If 1, we memcpy blocks in xvmcvideo.
        if ((CONFIG_MPEG1_XVMC_HWACCEL || CONFIG_MPEG2_XVMC_HWACCEL) && s->pack_pblocks)
            em_xvmc_init_block(s); // set s->block
            ...
            if (s->mb_y >= s->mb_height) {
                ...
                if (left >= 32 && !is_d10) {
                    ...
                    if (show_bits(&gb, 24) == 0x060E2B) {
                        av_em_log(avctx, AV_LOG_DEBUG, "Invalid MXF data found in video stream\n");
                        is_d10 = 1;
                    }
                    if (left > 32 && show_bits_long(&gb, 32) == 0x201) {
                        av_em_log(avctx, AV_LOG_DEBUG, "skipping m704 alpha (unsupported)\n");
                        goto eos;
                    }
                }
                ...
                if (left < 0 ||
                    ...
                    av_em_log(avctx, AV_LOG_ERROR, "end mismatch left=%d %0X at %d %d\n",
        ...
        if (s->mb_skip_run == -1) {
            ...
            for (;;) {
                int code = get_vlc2(&s->gb, ff_mbincr_vlc.table,
                                    MBINCR_VLC_BITS, 2);
                if (code < 0) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "mb incr damaged\n");
                    return AVERROR_INVALIDDATA;
                }
                if (code >= 33) {
                    if (code == 33) {
                        ...
                        if (s->mb_skip_run != 0 || show_bits(&s->gb, 15) != 0) {
                            av_em_log(s->avctx, AV_LOG_ERROR, "slice mismatch\n");
                            return AVERROR_INVALIDDATA;
                        }
...
eos: // end of slice
    if (get_bits_left(&s->gb) < 0) {
        av_em_log(s, AV_LOG_ERROR, "overread %d\n", -get_bits_left(&s->gb));
        return AVERROR_INVALIDDATA;
    }
...
static int slice_decode_thread(AVEMCodecContext *c, void *arg)
{
    ...
    for (;;) {
        ...
        start_code = -1;
        buf        = avpriv_em_find_start_code(buf, s->gb.buffer_end, &start_code);
 ...
 */
static int slice_end(AVEMCodecContext *avctx, AVFrame *pict)
{
    Mpeg1Context *s1  = avctx->priv_data;
    MpegEncContext *s = &s1->mpeg_enc_ctx;
    ...
    if (/* s->mb_y << field_pic == s->mb_height && */ !s->first_field && !s1->first_slice) {
        ...
        ff_em_er_frame_end(&s->er);

        ff_mpv_frame_end(s);

        if (s->pict_type == AV_PICTURE_TYPE_B || s->low_delay) {
            int ret = av_em_frame_ref(pict, s->current_picture_ptr->f);
            if (ret < 0)
                return ret;
            ...
            if (s->last_picture_ptr) {
                int ret = av_em_frame_ref(pict, s->last_picture_ptr->f);
                if (ret < 0)
                    return ret;
...
static int mpeg1_decode_sequence(AVEMCodecContext *avctx,
                                 const uint8_t *buf, int buf_size)
{
    Mpeg1Context *s1  = avctx->priv_data;
    MpegEncContext *s = &s1->mpeg_enc_ctx;
    ...
    if (s->aspect_ratio_info == 0) {
        av_em_log(avctx, AV_LOG_ERROR, "aspect ratio has forbidden 0 value\n");
        if (avctx->err_recognition & (AV_EF_BITSTREAM | AV_EF_COMPLIANT))
            return AVERROR_INVALIDDATA;
    }
    ...
    if (show_bits(&s->gb, 23) != 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "sequence header damaged\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->avctx->debug & FF_DEBUG_PICT_INFO)
        av_em_log(s->avctx, AV_LOG_DEBUG, "vbv buffer: %d, bitrate:%"PRId64", aspect_ratio_info: %d \n",
               s->avctx->rc_buffer_size, s->bit_rate, s->aspect_ratio_info);

    return 0;
}

static int vcr2_init_sequence(AVEMCodecContext *avctx)
{
    Mpeg1Context *s1  = avctx->priv_data;
    MpegEncContext *s = &s1->mpeg_enc_ctx;
    int i, v, ret;
...
static int mpeg_decode_a53_cc(AVEMCodecContext *avctx,
                              const uint8_t *p, int buf_size)
{
    ...
    if (buf_size >= 6 &&
        ...
        if (cc_count > 0 && buf_size >= 7 + cc_count * 3) {
            ...
            s1->a53_caption      = av_em_alloc(s1->a53_caption_size);
            if (s1->a53_caption)
...
static void mpeg_decode_user_data(AVEMCodecContext *avctx,
                                  const uint8_t *p, int buf_size)
{
    Mpeg1Context *s = avctx->priv_data;
    ...
    for(i=0; !(!p[i-2] && !p[i-1] && p[i]==1) && i<buf_size; i++){
        av_em_log(avctx, AV_LOG_ERROR, "%c", p[i]);
    }
    av_em_log(avctx, AV_LOG_ERROR, "\n");
#endif
...
static void mpeg_decode_gop(AVEMCodecContext *avctx,
                            const uint8_t *buf, int buf_size)
{
    Mpeg1Context *s1  = avctx->priv_data;
    MpegEncContext *s = &s1->mpeg_enc_ctx;
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (s->avctx->debug & FF_DEBUG_PICT_INFO) {
        ...
        av_em_timecode_make_mpeg_tc_string(tcbuf, tc);
        av_em_log(s->avctx, AV_LOG_DEBUG,
...
static int decode_chunks(AVEMCodecContext *avctx, AVFrame *picture,
                         int *got_output, const uint8_t *buf, int buf_size)
{
    ...
    for (;;) {
        ...
        buf_ptr = avpriv_em_find_start_code(buf_ptr, buf_end, &start_code);
        if (start_code > 0x1ff) {
            if (!skip_frame) {
                if (HAVE_THREADS &&
        ...
        if (avctx->debug & FF_DEBUG_STARTCODE)
            av_em_log(avctx, AV_LOG_DEBUG, "%3"PRIX32" at %"PTRDIFF_SPECIFIER" left %d\n",
        ...
        case PICTURE_START_CODE:
            if (picture_start_code_seen && s2->picture_structure == PICT_FRAME) {
               ...
               av_em_log(avctx, AV_LOG_WARNING, "ignoring extra picture following a frame-picture\n");
               break;
            }
            picture_start_code_seen = 1;

            if (s2->width <= 0 || s2->height <= 0) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid frame dimensions %dx%d.\n",
                       s2->width, s2->height);
                return AVERROR_INVALIDDATA;
            }
        ...
        default:
            if (start_code >= SLICE_MIN_START_CODE &&
                ...
                if (s2->progressive_sequence && !s2->progressive_frame) {
                    ...
                    av_em_log(s2->avctx, AV_LOG_ERROR,
                ...
                if (s2->progressive_sequence && !s2->frame_pred_frame_dct)
                    av_em_log(s2->avctx, AV_LOG_WARNING, "invalid frame_pred_frame_dct\n");
            ...
            if (start_code >= SLICE_MIN_START_CODE &&
                ...
                if (buf_end - buf_ptr < 2) {
                    av_em_log(s2->avctx, AV_LOG_ERROR, "slice too small\n");
                    return AVERROR_INVALIDDATA;
                }
                ...
                if (!s2->pict_type) {
                    av_em_log(avctx, AV_LOG_ERROR, "Missing picture start code\n");
                    if (avctx->err_recognition & AV_EF_EXPLODE)
                        return AVERROR_INVALIDDATA;
                    break;
                }
                ...
                if (HAVE_THREADS &&
                    ...
                    if (ret < 0) {
                        ...
                        if (s2->resync_mb_x >= 0 && s2->resync_mb_y >= 0)
                            ff_em_er_add_slice(&s2->er, s2->resync_mb_x,
...
static int mpeg_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_output, AVEMPacket *avpkt)
{
    ...
    if (buf_size == 0 || (buf_size == 4 && AV_RB32(buf) == SEQ_END_CODE)) {
        ...
        if (s2->low_delay == 0 && s2->next_picture_ptr) {
            int ret = av_em_frame_ref(picture, s2->next_picture_ptr->f);
            if (ret < 0)
                return ret;
    ...
    if (s2->avctx->flags & AV_CODEC_FLAG_TRUNCATED) {
        ...
        if (em_combine_frame(&s2->parse_context, next,
                             (const uint8_t **) &buf, &buf_size) < 0)
            return buf_size;
    }

    s2->codec_tag = avpriv_em_toupper4(avctx->codec_tag);
    ...
    if (avctx->extradata && !s->extradata_decoded) {
        ...
        if (*got_output) {
            av_em_log(avctx, AV_LOG_ERROR, "picture in extradata\n");
            *got_output = 0;
        }
    ...
    if (ret<0 || *got_output) {
        ...
        if (s2->timecode_frame_start != -1 && *got_output) {
            AVFrameSideData *tcside = av_em_frame_new_side_data(picture,
...
static av_cold int mpeg_decode_end(AVEMCodecContext *avctx)
{
    Mpeg1Context *s = avctx->priv_data;
...
AVEMCodec ff_mpeg1video_decoder = {
    .name                  = "mpeg1video",
...
AVEMCodec ff_mpeg2video_decoder = {
    .name           = "mpeg2video",
...
//legacy decoder
AVEMCodec ff_mpegvideo_decoder = {
    .name           = "mpegvideo",
    .long_name      = NULL_IF_CONFIG_SMALL("MPEG-1 video"),
    .type           = AVMEDIA_TYPE_VIDEO,
    .id             = AV_CODEC_ID_MPEG2VIDEO,
...
FF_DISABLE_DEPRECATION_WARNINGS
static av_cold int mpeg_mc_decode_init(AVEMCodecContext *avctx)
{
...
AVEMCodec ff_mpeg_xvmc_decoder = {
    .name           = "mpegvideo_xvmc",
...
#if CONFIG_MPEG_VDPAU_DECODER && FF_API_VDPAU
AVEMCodec ff_mpeg_vdpau_decoder = {
...
#if CONFIG_MPEG1_VDPAU_DECODER && FF_API_VDPAU
AVEMCodec ff_mpeg1_vdpau_decoder = {
