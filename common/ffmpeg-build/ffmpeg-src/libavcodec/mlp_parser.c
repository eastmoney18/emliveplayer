...
int ff_mlp_read_major_sync(void *log, MLPHeaderInfo *mh, GetBitContext *gb)
{
    ...
    if (header_size < 0 || gb->size_in_bits < header_size << 3) {
        av_em_log(log, AV_LOG_ERROR, "packet too short, unable to read major sync\n");
        return -1;
    }

    checksum = ff_mlp_checksum16(gb->buffer, header_size - 2);
    if (checksum != AV_RL16(gb->buffer+header_size-2)) {
        av_em_log(log, AV_LOG_ERROR, "major sync info header checksum error\n");
...
static av_cold int mlp_init(AVEMCodecParserContext *s)
{
    ff_mlp_init_crc();
    return 0;
}

static int mlp_parse(AVEMCodecParserContext *s,
                     ...
                     const uint8_t *buf, int buf_size)
{
    ...
    *poutbuf_size = 0;
    ...
    if (!mp->in_sync) {
        ...
        if (!mp->in_sync) {
            if (em_combine_frame(&mp->pc, END_NOT_FOUND, &buf, &buf_size) != -1)
                av_em_log(avctx, AV_LOG_WARNING, "em_combine_frame failed\n");
            return buf_size;
        }

        if ((ret = em_combine_frame(&mp->pc, i - 7, &buf, &buf_size)) < 0) {
            av_em_log(avctx, AV_LOG_WARNING, "em_combine_frame failed\n");
            return ret;
        }
    ...
    if (em_combine_frame(&mp->pc, next, &buf, &buf_size) < 0) {
    ...
    if (!sync_present) {
        ...
        if ((((parity_bits >> 4) ^ parity_bits) & 0xF) != 0xF) {
            av_em_log(avctx, AV_LOG_INFO, "mlpparse: Parity check failed.\n");
            goto lost_sync;
        }
    } else {
        GetBitContext gb;
