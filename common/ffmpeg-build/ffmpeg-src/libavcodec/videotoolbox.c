                                    ...
                                    uint32_t size)
{
    ...
    tmp = av_em_fast_realloc(vtctx->bitstream,
...
int ff_videotoolbox_alloc_frame(AVEMCodecContext *avctx, AVFrame *frame)
{
    frame->width  = avctx->width;
    frame->height = avctx->height;
    frame->format = avctx->pix_fmt;
    frame->buf[0] = av_em_buffer_alloc(1);
...
CFDataRef ff_videotoolbox_avcc_extradata_create(AVEMCodecContext *avctx)
{
    H264EMContext *h     = avctx->priv_data;
    CFDataRef data = NULL;
    uint8_t *p;
    int vt_extradata_size = 6 + 2 + h->ps.sps->data_size + 3 + h->ps.pps->data_size;
    uint8_t *vt_extradata = av_em_alloc(vt_extradata_size);
    if (!vt_extradata)
        return NULL;
    ...
    av_em_free(vt_extradata);
    return data;
}

int ff_videotoolbox_buffer_create(VTContext *vtctx, AVFrame *frame)
{
    av_em_buffer_unref(&frame->buf[0]);

    frame->buf[0] = av_em_buffer_create((uint8_t*)vtctx->frame,
                                     sizeof(vtctx->frame),
...
int ff_videotoolbox_h264_start_frame(AVEMCodecContext *avctx,
                                     const uint8_t *buffer,
                                     uint32_t size)
{
...
int ff_videotoolbox_h264_decode_slice(AVEMCodecContext *avctx,
                                      const uint8_t *buffer,
                                      uint32_t size)
{
    VTContext *vtctx = avctx->internal->hwaccel_priv_data;
...
int ff_videotoolbox_uninit(AVEMCodecContext *avctx)
{
    VTContext *vtctx = avctx->internal->hwaccel_priv_data;
    if (vtctx) {
        av_em_freep(&vtctx->bitstream);
        if (vtctx->frame)
...
static CFDataRef videotoolbox_esds_extradata_create(AVEMCodecContext *avctx)
{
    CFDataRef data;
    ...
    if (!(rw_extradata = av_em_mallocz(full_size + VIDEOTOOLBOX_ESDS_EXTRADATA_PADDING)))
        return NULL;
                                          ...
                                          CMTime duration)
{
    ...
    if (!image_buffer) {
        av_em_log(NULL, AV_LOG_DEBUG, "vt decoder cb: output image buffer is null\n");
        return;
    }
...
static OSStatus videotoolbox_session_decode_frame(AVEMCodecContext *avctx)
{
    OSStatus status;
    CMSampleBufferRef sample_buf;
...
static int videotoolbox_common_end_frame(AVEMCodecContext *avctx, AVFrame *frame)
{
    int status;
    AVVideotoolboxContext *videotoolbox = avctx->hwaccel_context;
    VTContext *vtctx = avctx->internal->hwaccel_priv_data;
    ...
    if (status) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to decode frame (%d)\n", status);
...
static int videotoolbox_h264_end_frame(AVEMCodecContext *avctx)
{
    H264EMContext *h = avctx->priv_data;
...
static int videotoolbox_mpeg_start_frame(AVEMCodecContext *avctx,
                                         const uint8_t *buffer,
                                         uint32_t size)
{
    VTContext *vtctx = avctx->internal->hwaccel_priv_data;
...
static int videotoolbox_mpeg_decode_slice(AVEMCodecContext *avctx,
                                          const uint8_t *buffer,
                                          uint32_t size)
{
    return 0;
...
static int videotoolbox_mpeg_end_frame(AVEMCodecContext *avctx)
{
...
static int videotoolbox_default_init(AVEMCodecContext *avctx)
{
    AVVideotoolboxContext *videotoolbox = avctx->hwaccel_context;
    OSStatus status;
    ...
    if (!videotoolbox) {
        av_em_log(avctx, AV_LOG_ERROR, "hwaccel context is not set\n");
        return -1;
    }
    ...
    if (!videotoolbox->cm_fmt_desc) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "format description creation failed\n");
        return -1;
    }
...
static void videotoolbox_default_free(AVEMCodecContext *avctx)
{
    AVVideotoolboxContext *videotoolbox = avctx->hwaccel_context;
...
AVHWAccel em_h263_videotoolbox_hwaccel = {
...
AVHWAccel em_h264_videotoolbox_hwaccel = {
...
AVHWAccel em_mpeg4_videotoolbox_hwaccel = {
...
AVVideotoolboxContext *av_em_videotoolbox_alloc_context(void)
{
    AVVideotoolboxContext *ret = av_em_mallocz(sizeof(*ret));
...
int av_em_videotoolbox_default_init(AVEMCodecContext *avctx)
{
    return av_em_videotoolbox_default_init2(avctx, NULL);
}

int av_em_videotoolbox_default_init2(AVEMCodecContext *avctx, AVVideotoolboxContext *vtctx)
{
    avctx->hwaccel_context = vtctx ?: av_em_videotoolbox_alloc_context();
    if (!avctx->hwaccel_context)
        return AVERROR(ENOMEM);
    return videotoolbox_default_init(avctx);
}

void av_em_videotoolbox_default_free(AVEMCodecContext *avctx)
{

    videotoolbox_default_free(avctx);
    av_em_freep(&avctx->hwaccel_context);
}
