...
static int tta_check_crc(TTAContext *s, const uint8_t *buf, int buf_size)
{
    ...
    crc = av_em_crc(s->crc_table, 0xFFFFFFFFU, buf, buf_size);
...
static int allocate_buffers(AVEMCodecContext *avctx)
{
    TTAContext *s = avctx->priv_data;

    if (s->bps < 3) {
        s->decode_buffer = av_em_mallocz_array(sizeof(int32_t)*s->frame_length, s->channels);
        if (!s->decode_buffer)
            return AVERROR(ENOMEM);
    } else
        s->decode_buffer = NULL;
    s->ch_ctx = av_em_malloc_array(avctx->channels, sizeof(*s->ch_ctx));
    if (!s->ch_ctx) {
...
static av_cold int tta_decode_init(AVEMCodecContext * avctx)
{
    TTAContext *s = avctx->priv_data;
    GetBitContext gb;
    int total_frames;
    ...
    s->crc_table = av_em_crc_get_table(AV_CRC_32_IEEE_LE);
    ...
    if (show_bits_long(&gb, 32) == AV_RL32("TTA1")) {
        ...
        if (s->format > 2) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid format\n");
            return AVERROR_INVALIDDATA;
        }
        if (s->format == FORMAT_ENCRYPTED) {
            if (!s->pass) {
                av_em_log(avctx, AV_LOG_ERROR, "Missing password for encrypted stream. Please use the -password option\n");
        ...
        if (s->channels == 0) {
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Invalid samplerate\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        default:
            av_em_log(avctx, AV_LOG_ERROR, "Invalid/unsupported sample format.\n");
            return AVERROR_INVALIDDATA;
        }

        // prevent overflow
        if (avctx->sample_rate > 0x7FFFFFu) {
            av_em_log(avctx, AV_LOG_ERROR, "sample_rate too large\n");
            return AVERROR(EINVAL);
        }
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "format: %d chans: %d bps: %d rate: %d block: %d\n",
            s->format, avctx->channels, avctx->bits_per_coded_sample, avctx->sample_rate,
            avctx->block_align);
        av_em_log(avctx, AV_LOG_DEBUG, "data_length: %d frame_length: %d last: %d total: %d\n",
            s->data_length, s->frame_length, s->last_frame_length, total_frames);

        if(s->frame_length >= UINT_MAX / (s->channels * sizeof(int32_t))){
            av_em_log(avctx, AV_LOG_ERROR, "frame_length too large\n");
            return AVERROR_INVALIDDATA;
        }
    } else {
        av_em_log(avctx, AV_LOG_ERROR, "Wrong extradata present\n");
        return AVERROR_INVALIDDATA;
    }
...
static int tta_decode_frame(AVEMCodecContext *avctx, void *data,
...
static av_cold int tta_decode_close(AVEMCodecContext *avctx) {
    TTAContext *s = avctx->priv_data;

    if (s->bps < 3)
        av_em_freep(&s->decode_buffer);
    s->decode_buffer = NULL;
    av_em_freep(&s->ch_ctx);
...
static const AVEMClass tta_decoder_class = {
