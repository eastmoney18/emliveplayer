...
av_const int em_h263_aspect_to_info(AVEMRational aspect){
    ...
    if(aspect.num==0 || aspect.den==0) aspect= (AVEMRational){1,1};

    for(i=1; i<6; i++){
        if(av_em_cmp_q(em_h263_pixel_aspect[i], aspect) == 0){
            return i;
        }
    }
...
void em_h263_encode_picture_header(MpegEncContext * s, int picture_number)
{
    ...
    format = em_match_2uint16(em_h263_format, FF_ARRAY_ELEMS(em_h263_format), s->width, s->height);
    if (!s->h263_plus) {
        /* H.263v1 */
            ...
            if (code == rl->n) {
              if(!CONFIG_FLV_ENCODER || s->h263_flv <= 1){
                ...
                if(level < 128)
                    ...
                    em_flv2_encode_ac_esc(&s->pb, slevel, level, run, last);
              }
            } else {
                       ...
                       int motion_x, int motion_y)
{
    ...
    if (!s->mb_intra) {
        ...
        if(s->mv_type==MV_TYPE_16X16){
            ...
            put_bits(&s->pb, em_h263_cbpy_tab[cbpy][1], em_h263_cbpy_tab[cbpy][0]);
            if(s->dquant)
            ...
            if (!s->umvplus) {
                em_h263_encode_motion_vector(s, motion_x - pred_x,
                                                motion_y - pred_y, 1);
            }
            else {
        ...
        if (s->h263_aic) {
            ...
            for(i=0; i<6; i++) {
                ...
                pred_dc = em_h263_pred_dc(s, i, &dc_ptr[i]);
                level -= pred_dc;
                /* Quant */
                if (level >= 0)
...
av_cold void em_h263_encode_init(MpegEncContext *s)
{
    static int done = 0;
    ...
    if (!done) {
        ...
        init_uni_h263_rl_tab(&em_h263_rl_inter    , NULL, uni_h263_inter_rl_len);
