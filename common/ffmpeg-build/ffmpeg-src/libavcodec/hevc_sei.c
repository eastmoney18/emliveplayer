...
static int decode_pic_timing(HEVCContext *s)
{
    ...
    if (sps->vui.frame_field_info_present_flag) {
        ...
        if (pic_struct == 2) {
            av_em_log(s->avctx, AV_LOG_DEBUG, "BOTTOM Field\n");
            s->picture_struct = AV_PICTURE_STRUCTURE_BOTTOM_FIELD;
        } else if (pic_struct == 1) {
            av_em_log(s->avctx, AV_LOG_DEBUG, "TOP Field\n");
    ...
    if (user_data_type_code == 0x3) {
        ...
        if (flag) {
            ...
            if (cc_count && size >= cc_count * 3) {
                ...
                ret = av_em_reallocp(&s->a53_caption, new_size);
                if (ret < 0)
                    return ret;
...
static int active_parameter_sets(HEVCContext *s)
{
    ...
    if (num_sps_ids_minus1 < 0 || num_sps_ids_minus1 > 15) {
        av_em_log(s->avctx, AV_LOG_ERROR, "num_sps_ids_minus1 %d invalid\n", num_sps_ids_minus1);
        return AVERROR_INVALIDDATA;
    }

    active_seq_parameter_set_id = get_ue_golomb_long(gb);
    if (active_seq_parameter_set_id >= MAX_SPS_COUNT) {
        av_em_log(s->avctx, AV_LOG_ERROR, "active_parameter_set_id %d invalid\n", active_seq_parameter_set_id);
        return AVERROR_INVALIDDATA;
    }
    ...
    case SEI_TYPE_PICTURE_TIMING:
        {
            ...
            av_em_log(s->avctx, AV_LOG_DEBUG, "Skipped PREFIX SEI %d\n", type);
            skip_bits(gb, 8 * size);
            return ret;
        }
...
static int decode_nal_sei_suffix(HEVCContext *s, int type, int size)
{
    ...
    case SEI_TYPE_DECODED_PICTURE_HASH:
        return decode_nal_sei_decoded_picture_hash(s);
    default:
        av_em_log(s->avctx, AV_LOG_DEBUG, "Skipped SUFFIX SEI %d\n", type);
        skip_bits_long(gb, 8 * size);
        return 0;
    }
}
...
static int decode_nal_sei_message(HEVCContext *s)
{
    ...
    int payload_size = 0;
    int byte = 0xFF;
    av_em_log(s->avctx, AV_LOG_DEBUG, "Decoding SEI\n");
