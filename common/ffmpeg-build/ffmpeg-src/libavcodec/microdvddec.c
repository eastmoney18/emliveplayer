...
static void microdvd_open_tags(AVEMBPrint *new_line, struct microdvd_tag *tags)
{
    int i, sidx;
        ...
        case 'y':
            for (sidx = 0; sidx < sizeof(MICRODVD_STYLES) - 1; sidx++)
                if (tags[i].data1 & (1 << sidx))
                    av_em_bprintf(new_line, "{\\%c1}", MICRODVD_STYLES[sidx]);
            break;

        case 'c':
            av_em_bprintf(new_line, "{\\c&H%06X&}", tags[i].data1);
            break;

        case 'f':
            av_em_bprintf(new_line, "{\\fn%.*s}",
                       tags[i].data_string_len, tags[i].data_string);
            break;

        case 's':
            av_em_bprintf(new_line, "{\\fs%d}", tags[i].data1);
            break;

        case 'p':
            if (tags[i].data1 == 0)
                av_em_bprintf(new_line, "{\\an8}");
            break;

        case 'o':
            av_em_bprintf(new_line, "{\\pos(%d,%d)}",
...
static void microdvd_close_no_persistent_tags(AVEMBPrint *new_line,
    ...
    for (i = sizeof(MICRODVD_TAGS) - 2; i >= 0; i--) {
        ...
        case 'y':
            for (sidx = sizeof(MICRODVD_STYLES) - 2; sidx >= 0; sidx--)
                if (tags[i].data1 & (1 << sidx))
                    av_em_bprintf(new_line, "{\\%c0}", MICRODVD_STYLES[sidx]);
            break;

        case 'c':
            av_em_bprintf(new_line, "{\\c}");
            break;

        case 'f':
            av_em_bprintf(new_line, "{\\fn}");
            break;

        case 's':
            av_em_bprintf(new_line, "{\\fs}");
            break;
        }
        tags[i].key = 0;
    }
...
static int microdvd_decode_frame(AVEMCodecContext *avctx,
                                 void *data, int *got_sub_ptr, AVEMPacket *avpkt)
{
    ...
    av_em_bprint_init(&new_line, 0, 2048);
    ...
    while (line < end && *line) {
        ...
        while (line < end && *line && *line != '|') {
            av_em_bprint_chars(&new_line, *line, 1);
            line++;
        }
...
static int microdvd_init(AVEMCodecContext *avctx)
{
    int i, sidx;
    AVEMBPrint font_buf;
    ...
    av_em_bprint_init(&font_buf, 0, AV_BPRINT_SIZE_AUTOMATIC);
    av_em_bprintf(&font_buf, "%s", ASS_DEFAULT_FONT);
    ...
    if (avctx->extradata) {
        ...
        for (i = 0; i < sizeof(MICRODVD_TAGS) - 1; i++) {
            ...
            case 'f':
                av_em_bprint_clear(&font_buf);
                av_em_bprintf(&font_buf, "%.*s",
                           tags[i].data_string_len, tags[i].data_string);
                break;
            }
        }
