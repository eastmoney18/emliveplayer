...
static av_cold int ass_encode_init(AVEMCodecContext *avctx)
{
    avctx->extradata = av_em_alloc(avctx->subtitle_header_size + 1);
    if (!avctx->extradata)
...
static int ass_encode_frame(AVEMCodecContext *avctx,
                            ...
                            const AVSubtitle *sub)
{
    ...
    for (i=0; i<sub->num_rects; i++) {
        ...
        if (!strncmp(ass, "Dialogue: ", 10)) {
            if (i > 0) {
                av_em_log(avctx, AV_LOG_ERROR, "ASS encoder supports only one "
                       "ASS rectangle field.\n");
...
#define SKIP_ENTRY(ptr) do {        \
    ...
    if (sep)                        \
        ...
        len = av_em_strlcpy(buf+total_len, ass, bufsize-total_len);
