...
void ff_dv_print_profiles(void *logctx, int loglevel)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(dv_profiles); i++) {
        ...
        av_em_log(logctx, loglevel,
               "Frame size: %dx%d; pixel format: %s, framerate: %d/%d\n",
               p->width, p->height, av_em_get_pix_fmt_name(p->pix_fmt),
...
const AVDVProfile* ff_dv_frame_profile(AVEMCodecContext* codec, const AVDVProfile *sys,
...
const AVDVProfile *av_em_dv_frame_profile(const AVDVProfile *sys,
                                       const uint8_t *frame, unsigned buf_size)
{
    return ff_dv_frame_profile(NULL, sys, frame, buf_size);
}

const AVDVProfile *av_em_dv_codec_profile(int width, int height,
                                       enum AVPixelFormat pix_fmt)
{
#if CONFIG_DVPROFILE
    return av_em_dv_codec_profile2(width, height, pix_fmt, (AVEMRational){0, 0});
#endif

    return NULL;
}

const AVDVProfile *av_em_dv_codec_profile2(int width, int height,
                                       enum AVPixelFormat pix_fmt,
                                       AVEMRational frame_rate)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(dv_profiles); i++)
        ...
        {
            if( invalid_framerate || av_em_div_q(dv_profiles[i].time_base, frame_rate).num == 1 )
                return &dv_profiles[i];
