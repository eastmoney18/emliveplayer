...
static int vp56_size_changed(VP56Context *s)
{
    ...
    if (s->mb_width > 1000 || s->mb_height > 1000) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "picture too big\n");
        return AVERROR_INVALIDDATA;
    }

    av_em_reallocp_array(&s->above_blocks, 4*s->mb_width+6,
                      sizeof(*s->above_blocks));
    av_em_reallocp_array(&s->macroblocks, s->mb_width*s->mb_height,
                      sizeof(*s->macroblocks));
    av_em_free(s->edge_emu_buffer_alloc);
    s->edge_emu_buffer_alloc = av_em_alloc(16*stride);
...
static int ff_vp56_decode_mbs(AVEMCodecContext *avctx, void *, int, int);

int ff_vp56_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                         AVEMPacket *avpkt)
{
    ...
    if (res == VP56_SIZE_CHANGE) {
        for (i = 0; i < 4; i++) {
            av_em_frame_unref(s->frames[i]);
            if (s->alpha_context)
                av_em_frame_unref(s->alpha_context->frames[i]);
        }
    }

    ret = em_get_buffer(avctx, p, AV_GET_BUFFER_FLAG_REF);
    if (ret < 0)
        return ret;

    if (avctx->pix_fmt == AV_PIX_FMT_YUVA420P) {
        av_em_frame_unref(s->alpha_context->frames[VP56_FRAME_CURRENT]);
        if ((ret = av_em_frame_ref(s->alpha_context->frames[VP56_FRAME_CURRENT], p)) < 0) {
            av_em_frame_unref(p);
            return ret;
        }
    }
    ...
    if (avctx->pix_fmt == AV_PIX_FMT_YUVA420P) {
        ...
        if (res != 0) {
            if(res==VP56_SIZE_CHANGE) {
                av_em_log(avctx, AV_LOG_ERROR, "Alpha reconfiguration\n");
                avctx->width  = bak_w;
                avctx->height = bak_h;
    ...
    if ((res = av_em_frame_ref(data, p)) < 0)
        return res;
    *got_frame = 1;
...
static int ff_vp56_decode_mbs(AVEMCodecContext *avctx, void *data,
...
next:
    if (p->key_frame || s->golden_frame) {
        av_em_frame_unref(s->frames[VP56_FRAME_GOLDEN]);
        if ((res = av_em_frame_ref(s->frames[VP56_FRAME_GOLDEN], p)) < 0)
            return res;
    }

    av_em_frame_unref(s->frames[VP56_FRAME_PREVIOUS]);
    FFSWAP(AVFrame *, s->frames[VP56_FRAME_CURRENT],
                      s->frames[VP56_FRAME_PREVIOUS]);
    return 0;
}

av_cold int ff_vp56_init(AVEMCodecContext *avctx, int flip, int has_alpha)
{
    VP56Context *s = avctx->priv_data;
    return ff_vp56_init_context(avctx, s, flip, has_alpha);
}

av_cold int ff_vp56_init_context(AVEMCodecContext *avctx, VP56Context *s,
                                  int flip, int has_alpha)
{
    ...
    em_h264chroma_init(&s->h264chroma, 8);
...
av_cold int ff_vp56_free(AVEMCodecContext *avctx)
{
    VP56Context *s = avctx->priv_data;
...
av_cold int ff_vp56_free_context(VP56Context *s)
{
    ...
    av_em_freep(&s->above_blocks);
    av_em_freep(&s->macroblocks);
    av_em_freep(&s->edge_emu_buffer_alloc);

    for (i = 0; i < FF_ARRAY_ELEMS(s->frames); i++)
        av_em_frame_free(&s->frames[i]);
