...
static av_cold int decoder_init(AVEMCodecContext * avctx)
{
    G729Context* ctx = avctx->priv_data;
    int i,k;

    if (avctx->channels != 1) {
        av_em_log(avctx, AV_LOG_ERROR, "Only mono sound is supported (requested channels: %d).\n", avctx->channels);
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (buf_size % 10 == 0) {
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Packet type: %s\n", "G.729 @ 8kbit/s");
    } else if (buf_size == 8) {
        packet_type = FORMAT_G729D_6K4;
        format = &format_g729d_6k4;
        av_em_log(avctx, AV_LOG_DEBUG, "Packet type: %s\n", "G.729D @ 6.4kbit/s");
    } else {
        av_em_log(avctx, AV_LOG_ERROR, "Packet size %d is unknown.\n", buf_size);
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (pitch_delay_int[i] > PITCH_DELAY_MAX) {
            av_em_log(avctx, AV_LOG_WARNING, "pitch_delay_int %d is too large\n", pitch_delay_int[i]);
