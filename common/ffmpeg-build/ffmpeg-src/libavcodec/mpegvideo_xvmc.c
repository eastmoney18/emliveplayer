 ...
 */
void em_xvmc_init_block(MpegEncContext *s)
{
 ...
 */
void em_xvmc_pack_pblocks(MpegEncContext *s, int cbp)
{
    int i, j = 0;
 ...
 */
static int em_xvmc_field_start(AVEMCodecContext *avctx, const uint8_t *buf, uint32_t buf_size)
{
 ...
 * It should be safe to call the function a few times for the same field.
 */
static int em_xvmc_field_end(AVEMCodecContext *avctx)
{
    struct MpegEncContext *s = avctx->priv_data;
    struct xvmc_pix_fmt *render = (struct xvmc_pix_fmt*)s->current_picture.f->data[2];
    assert(render);
 ...
 */
static void em_xvmc_decode_mb(struct MpegEncContext *s)
{
    ...
    if (s->encoding) {
        av_em_log(s->avctx, AV_LOG_ERROR, "XVMC doesn't support encoding!!!\n");
        return;
    }
...
AVHWAccel ff_mpeg1_xvmc_hwaccel = {
    ...
    .start_frame    = em_xvmc_field_start,
    .end_frame      = em_xvmc_field_end,
    .decode_slice   = NULL,
    .decode_mb      = em_xvmc_decode_mb,
    .priv_data_size = 0,
};
#endif
