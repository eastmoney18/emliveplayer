...
static av_cold int qsv_decode_close(AVEMCodecContext *avctx)
{
    QSVH2645Context *s = avctx->priv_data;

    ff_qsv_decode_close(&s->qsv);

    av_em_bitstream_filter_close(s->bsf);

    return 0;
}

static av_cold int qsv_decode_init(AVEMCodecContext *avctx)
{
    QSVH2645Context *s = avctx->priv_data;
    int ret;
    ...
    if (avctx->codec_id == AV_CODEC_ID_HEVC && s->load_plugin != LOAD_PLUGIN_NONE) {
        ...
        if (s->qsv.load_plugins[0]) {
            ...
            av_em_freep(&s->qsv.load_plugins);
            s->qsv.load_plugins = av_em_strdup(uid_hevcenc_sw);
    ...
    if (avctx->codec_id == AV_CODEC_ID_H264) {
        s->bsf = av_em_bitstream_filter_init("h264_mp4toannexb");
        //regarding ticks_per_frame description, should be 2 for h.264:
        avctx->ticks_per_frame = 2;
    } else
        s->bsf = av_em_bitstream_filter_init("hevc_mp4toannexb");
    if (!s->bsf) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
...
static int qsv_decode_frame(AVEMCodecContext *avctx, void *data,
                            int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (avpkt->size) {
        if (avpkt->size > 3 && !avpkt->data[0] &&
            ...
            ret = av_em_bitstream_filter_filter(s->bsf, avctx, "private_spspps_buf",
...
static void qsv_decode_flush(AVEMCodecContext *avctx)
{
    QSVH2645Context *s = avctx->priv_data;
...
static const AVEMClass hevc_class = {
    .class_name = "hevc_qsv",
