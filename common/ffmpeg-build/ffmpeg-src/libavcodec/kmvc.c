    ...
    for (by = 0; by < h; by += 8)
        for (bx = 0; bx < w; bx += 8) {
            if (!bytestream2_get_bytes_left(&ctx->g)) {
                av_em_log(ctx->avctx, AV_LOG_ERROR, "Data overrun\n");
                    ...
                    if (!res) {
                        ...
                        if (!res) {     // fill whole 4x4 block
                            ...
                            if ((l0x-mx) + 320*(l0y-my) < 0 || (l0x-mx) + 320*(l0y-my) > 320*197 - 4) {
                                av_em_log(ctx->avctx, AV_LOG_ERROR, "Invalid MV\n");
                                return AVERROR_INVALIDDATA;
                            }
                            for (j = 0; j < 16; j++)
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (blocksize != 8 && blocksize != 127) {
        av_em_log(avctx, AV_LOG_ERROR, "Block size = %i\n", blocksize);
        return AVERROR_INVALIDDATA;
    }
    ...
    case 4:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unknown compression method %i\n", header & KMVC_METHOD);
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int decode_init(AVEMCodecContext * avctx)
{
    ...
    if (avctx->width > 320 || avctx->height > 200) {
        av_em_log(avctx, AV_LOG_ERROR, "KMVC supports frames <= 320x200\n");
    ...
    if (avctx->extradata_size < 12) {
        ...
        if (c->palsize >= (unsigned)MAX_PALSIZE) {
            c->palsize = 127;
            av_em_log(avctx, AV_LOG_ERROR, "KMVC palette too large\n");
            return AVERROR_INVALIDDATA;
        }
    }
