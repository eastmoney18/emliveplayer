 ...
 * to the decoder using the extradata[_size] fields in AVEMCodecContext. There
...
static av_cold int wma_decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->codec->id == AV_CODEC_ID_WMAV2 && avctx->extradata_size >= 8){
        if (AV_RL16(extradata+4)==0xd && s->use_variable_block_len){
            av_em_log(avctx, AV_LOG_WARNING, "Disabling use_variable_block_len, if this fails contact the ffmpeg developers and send us the file\n");
    ...
    if (s->use_exp_vlc)
        init_vlc(&s->exp_vlc, EXPVLCBITS, sizeof(em_aac_scalefactor_bits), // FIXME move out of context
                 em_aac_scalefactor_bits, 1, 1,
                 em_aac_scalefactor_code, 4, 4, 0);
    else
...
static int decode_exp_vlc(WMACodecContext *s, int ch)
{
    ...
    while (q < q_end) {
        ...
        if (code < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Exponent vlc invalid\n");
            return -1;
        }
        /* NOTE: this offset is the same as MPEG-4 AAC! */
        last_exp += code - 60;
        if ((unsigned) last_exp + 60 >= FF_ARRAY_ELEMS(pow_tab)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Exponent out of range: %d\n",
                   last_exp);
            return -1;
        }
        v  = ptab[last_exp];
...
static int wma_decode_block(WMACodecContext *s)
{
    ...
    if (s->frame_len_bits - s->block_len_bits >= s->nb_block_sizes){
        av_em_log(s->avctx, AV_LOG_ERROR, "block_len_bits not initialized to a valid value\n");
        return -1;
    }
    ...
    if ((s->block_pos + s->block_len) > s->frame_len) {
        av_em_log(s->avctx, AV_LOG_ERROR, "frame_len overflow\n");
        return -1;
    }
    ...
    for (;;) {
        if (get_bits_left(&s->gb) < 7) {
            av_em_log(s->avctx, AV_LOG_ERROR, "total_gain overread\n");
...
static int wma_decode_superframe(AVEMCodecContext *avctx, void *data,
                                 int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (s->use_bit_reservoir) {
        ...
        if (nb_frames <= 0) {
            ...
            av_em_log(avctx, is_error ? AV_LOG_ERROR : AV_LOG_WARNING,
...
//             s->reset_block_lengths = 1; //XXX is this needed ?
    ...
    if (s->use_bit_reservoir) {
        ...
        if (len > MAX_CODED_SUPERFRAME_SIZE || len < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "len %d invalid\n", len);
            goto fail;
        }
...
static av_cold void flush(AVEMCodecContext *avctx)
{
    WMACodecContext *s = avctx->priv_data;
