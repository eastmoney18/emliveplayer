...
static int pcx_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                            AVEMPacket *avpkt)
{
    GetByteContext gb;
    ...
    if (bytestream2_get_byteu(&gb) != 0x0a || bytestream2_get_byteu(&gb) > 5) {
        av_em_log(avctx, AV_LOG_ERROR, "this is not PCX encoded data\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (xmax < xmin || ymax < ymin) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid image dimensions\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (bytes_per_scanline < (w * bits_per_pixel * nplanes + 7) / 8 ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "PCX data is corrupted\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    case 0x0201:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "invalid PCX file\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    scanline = av_em_alloc(bytes_per_scanline + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!scanline)
    ...
    if (nplanes == 3 && bits_per_pixel == 8) {
        ...
        if (avpkt->size < 769) {
            av_em_log(avctx, AV_LOG_ERROR, "File is too short\n");
        ...
        if (bytestream2_tell(&gb) != palstart) {
            av_em_log(avctx, AV_LOG_WARNING, "image data possibly corrupted\n");
            bytestream2_seek(&gb, palstart, SEEK_SET);
        }
        if (bytestream2_get_byte(&gb) != 12) {
            av_em_log(avctx, AV_LOG_ERROR, "expected palette after image data\n");
            ret = avctx->err_recognition & AV_EF_EXPLODE ?
                  AVERROR_INVALIDDATA : avpkt->size;
            goto end;
        }
