 ...
 */
static int decode_gop_header(IVI45DecContext *ctx, AVEMCodecContext *avctx)
{
    ...
    if (tile_size > 256) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid tile size: %d\n", tile_size);
    ...
    if (ctx->gop_flags & 2) {
        avpriv_em_report_missing_feature(avctx, "YV12 picture format");
    ...
    for (p = 0; p <= 1; p++) {
        for (i = 0; i < (!p ? pic_conf.luma_bands : pic_conf.chroma_bands); i++) {
            ...
            if (p==0 && blk_size==4) {
                av_em_log(avctx, AV_LOG_ERROR, "4x4 luma blocks are unsupported!\n");
                return AVERROR_PATCHWELCOME;
            }
            ...
            if (get_bits1(&ctx->gb)) {
                avpriv_em_report_missing_feature(avctx, "Extended transform info");
                return AVERROR_PATCHWELCOME;
            }
            ...
            if (band->blk_size == 8) {
                if(quant_mat >= 5){
                    av_em_log(avctx, AV_LOG_ERROR, "quant_mat %d too large!\n", quant_mat);
                    return -1;
                }
            ...
            if (get_bits(&ctx->gb, 2)) {
                av_em_log(avctx, AV_LOG_ERROR, "End marker missing!\n");
                return AVERROR_INVALIDDATA;
            }
        }
    }
    ...
    if (ctx->gop_flags & 8) {
        if (get_bits(&ctx->gb, 3)) {
            av_em_log(avctx, AV_LOG_ERROR, "Alignment bits are not zero!\n");
            return AVERROR_INVALIDDATA;
        }
...
static int decode_pic_hdr(IVI45DecContext *ctx, AVEMCodecContext *avctx)
{
    ...
    if (ctx->frame_type == FRAMETYPE_INTRA) {
        if ((ret = decode_gop_header(ctx, avctx)) < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid GOP header, skipping frames.\n");
            ctx->gop_invalid = 1;
            return ret;
        }
        ctx->gop_invalid = 0;
    ...
    if (ctx->frame_type == FRAMETYPE_INTER_SCAL && !ctx->is_scalable) {
        av_em_log(avctx, AV_LOG_ERROR, "Scalable inter frame in non scalable stream\n");
                          ...
                          IVITile *tile, AVEMCodecContext *avctx)
{
    ...
    if (tile->num_MBs != IVI_MBs_PER_TILE(tile->width, tile->height, band->mb_size)) {
        av_em_log(avctx, AV_LOG_ERROR, "Allocated tile size %d mismatches parameters %d\n",
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (result) {
        av_em_log(avctx, AV_LOG_ERROR, "Couldn't allocate color planes!\n");
        return AVERROR_INVALIDDATA;
    }
