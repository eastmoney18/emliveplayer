...
static av_cold int vqa_decode_init(AVEMCodecContext *avctx)
{
    ...
    if (s->avctx->extradata_size != VQA_HEADER_SIZE) {
        av_em_log(s->avctx, AV_LOG_ERROR, "expected extradata size of %d\n", VQA_HEADER_SIZE);
    ...
    case 2:
        break;
    case 3:
        avpriv_em_report_missing_feature(avctx, "VQA Version %d", s->vqa_version);
        return AVERROR_PATCHWELCOME;
    default:
        avpriv_em_request_sample(avctx, "VQA Version %i", s->vqa_version);
        return AVERROR_PATCHWELCOME;
    }
    s->width = AV_RL16(&s->avctx->extradata[6]);
    s->height = AV_RL16(&s->avctx->extradata[8]);
    if ((ret = av_em_image_check_size(s->width, s->height, 0, avctx)) < 0) {
        s->width= s->height= 0;
        return ret;
    }
    ...
    if (s->width % s->vector_width || s->height % s->vector_height) {
        av_em_log(avctx, AV_LOG_ERROR, "Image size not multiple of block size\n");
        return AVERROR_INVALIDDATA;
    }

    /* allocate codebooks */
    s->codebook_size = MAX_CODEBOOK_SIZE;
    s->codebook = av_em_alloc(s->codebook_size);
    if (!s->codebook)
        goto fail;
    s->next_codebook_buffer = av_em_alloc(s->codebook_size);
    ...
    s->decode_buffer = av_em_mallocz(s->decode_buffer_size);
    if (!s->decode_buffer)
        goto fail;
    ...
    return 0;
fail:
    av_em_freep(&s->codebook);
    av_em_freep(&s->next_codebook_buffer);
    av_em_freep(&s->decode_buffer);
    return AVERROR(ENOMEM);
}

#define CHECK_COUNT() \
    if (dest_index + count > dest_size) { \
        av_em_log(s->avctx, AV_LOG_ERROR, "decode_format80 problem: next op would overflow dest_index\n"); \
        av_em_log(s->avctx, AV_LOG_ERROR, "current dest_index = %d, count = %d, dest_size = %d\n", \
...
#define CHECK_COPY(idx) \
    if (idx < 0 || idx + count > dest_size) { \
        av_em_log(s->avctx, AV_LOG_ERROR, "decode_format80 problem: next op would overflow dest_index\n"); \
        av_em_log(s->avctx, AV_LOG_ERROR, "current src_pos = %d, count = %d, dest_size = %d\n", \
...
static int decode_format80(VqaContext *s, int src_size,
    ...
    if (src_size < 0 || src_size > bytestream2_get_bytes_left(&s->gb)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Chunk size %d is out of range\n",
               src_size);
        return AVERROR_INVALIDDATA;
    }
    ...
    while (bytestream2_tell(&s->gb) - start < src_size) {
        ...
        if (dest_index >= dest_size) {
            av_em_log(s->avctx, AV_LOG_ERROR, "decode_format80 problem: dest_index (%d) exceeded dest_size (%d)\n",
    ...
    if (check_size)
        if (dest_index < dest_size) {
            av_em_log(s->avctx, AV_LOG_ERROR, "decode_format80 problem: decode finished with dest_index (%d) < dest_size (%d)\n",
                dest_index, dest_size);
...
static int vqa_decode_chunk(VqaContext *s, AVFrame *frame)
{
    ...
    while (bytestream2_get_bytes_left(&s->gb) >= 8) {
        ...
        default:
            av_em_log(s->avctx, AV_LOG_ERROR, "Found unknown chunk type: %c%c%c%c (%08X)\n",
            (chunk_type >> 24) & 0xFF,
            (chunk_type >> 16) & 0xFF,
            (chunk_type >>  8) & 0xFF,
            (chunk_type >>  0) & 0xFF,
    ...
    if ((cpl0_chunk != -1) && (cplz_chunk != -1)) {
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "problem: found both CPL0 and CPLZ chunks\n");
        return AVERROR_INVALIDDATA;
    }
...
/* yet to be handled */
    ...
    if (cpl0_chunk != -1) {
        ...
        if (chunk_size / 3 > 256 || chunk_size > bytestream2_get_bytes_left(&s->gb)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "problem: found a palette chunk with %d colors\n",
                chunk_size / 3);
            return AVERROR_INVALIDDATA;
        }
    ...
    if ((cbf0_chunk != -1) && (cbfz_chunk != -1)) {
        ...
        /* a chunk should not have both chunk types */
        av_em_log(s->avctx, AV_LOG_ERROR, "problem: found both CBF0 and CBFZ chunks\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (cbf0_chunk != -1) {
        ...
        if (chunk_size > MAX_CODEBOOK_SIZE) {
            av_em_log(s->avctx, AV_LOG_ERROR, "problem: CBF0 chunk too large (0x%X bytes)\n",
                chunk_size);
            return AVERROR_INVALIDDATA;
        }
    ...
    if (vptz_chunk == -1) {
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "problem: no VPTZ chunk found\n");
        return AVERROR_INVALIDDATA;
    }
...
/* not implemented yet */
    ...
    if ((cbp0_chunk != -1) && (cbpz_chunk != -1)) {
        /* a chunk should not have both chunk types */
        av_em_log(s->avctx, AV_LOG_ERROR, "problem: found both CBP0 and CBPZ chunks\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (cbp0_chunk != -1) {
        ...
        if (chunk_size > MAX_CODEBOOK_SIZE - s->next_codebook_buffer_index) {
            av_em_log(s->avctx, AV_LOG_ERROR, "cbp0 chunk too large (%u bytes)\n",
                   chunk_size);
            return AVERROR_INVALIDDATA;
        }
    ...
    if (cbpz_chunk != -1) {
        ...
        if (chunk_size > MAX_CODEBOOK_SIZE - s->next_codebook_buffer_index) {
            av_em_log(s->avctx, AV_LOG_ERROR, "cbpz chunk too large (%u bytes)\n",
                   chunk_size);
            return AVERROR_INVALIDDATA;
        }
...
static int vqa_decode_frame(AVEMCodecContext *avctx,
...
static av_cold int vqa_decode_end(AVEMCodecContext *avctx)
{
    VqaContext *s = avctx->priv_data;
