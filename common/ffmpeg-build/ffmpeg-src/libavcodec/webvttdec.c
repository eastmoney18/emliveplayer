...
static int webvtt_event_to_ass(AVEMBPrint *buf, const char *p)
{
    ...
    while (*p) {
        ...
        for (i = 0; i < FF_ARRAY_ELEMS(webvtt_tag_replace); i++) {
            ...
            if (!strncmp(p, from, len)) {
                av_em_bprintf(buf, "%s", webvtt_tag_replace[i].to);
                p += len;
                again = 1;
                break;
            }
...
static int webvtt_decode_frame(AVEMCodecContext *avctx,
