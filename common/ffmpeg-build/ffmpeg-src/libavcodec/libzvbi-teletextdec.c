...
{
    av_em_freep(&(*sub_rect)->data[0]);
    av_em_freep(&(*sub_rect)->data[1]);
    av_em_freep(&(*sub_rect)->ass);
    av_em_freep(sub_rect);
}
...
static int gen_sub_text(TeletextContext *ctx, AVSubtitleRect *sub_rect, vbi_page *page, int chop_top)
{
    const char *in;
    AVEMBPrint buf;
    char *vbi_text = av_em_alloc(TEXT_MAXSZ);
    int sz;
    ...
    if (sz <= 0) {
        av_em_log(ctx, AV_LOG_ERROR, "vbi_print error\n");
        av_em_free(vbi_text);
        return AVERROR_EXTERNAL;
    }
    vbi_text[sz] = '\0';
    in  = vbi_text;
    av_em_bprint_init(&buf, 0, TEXT_MAXSZ);
    ...
    if (ctx->chop_spaces) {
        for (;;) {
            ...
            av_em_bprint_append_data(&buf, in, sz);
            av_em_bprintf(&buf, "\n");
            in += nl;
        }
    } else {
    ...
    if (buf.len) {
        ...
        av_em_log(ctx, AV_LOG_DEBUG, "subtext:%s:txetbus\n", sub_rect->ass);
    } else {
...
static int gen_sub_bitmap(TeletextContext *ctx, AVSubtitleRect *sub_rect, vbi_page *page, int chop_top)
{
    ...
    if (vc >= vcend) {
        av_em_log(ctx, AV_LOG_DEBUG, "dropping empty page %3x\n", page->pgno);
        sub_rect->type = SUBTITLE_NONE;
        return 0;
    }

    sub_rect->data[0] = av_em_mallocz(resx * resy);
    ...
    sub_rect->data[1] = av_em_mallocz(AVPALETTE_SIZE);
    if (!sub_rect->data[1]) {
        av_em_freep(&sub_rect->data[0]);
        return AVERROR(ENOMEM);
    }
...
static void handler(vbi_event *ev, void *user_data)
{
    ...
    av_em_log(ctx, AV_LOG_DEBUG, "decoded page %s.%02x\n",
    ...
    av_em_log(ctx, AV_LOG_DEBUG, "%d x %d page chop:%d\n",
           page.columns, page.rows, chop_top);

    if (ctx->nb_pages < MAX_BUFFERED_PAGES) {
        if ((new_pages = av_em_realloc_array(ctx->pages, ctx->nb_pages + 1, sizeof(TeletextPage)))) {
            TeletextPage *cur_page = new_pages + ctx->nb_pages;
            ctx->pages = new_pages;
            cur_page->sub_rect = av_em_mallocz(sizeof(*cur_page->sub_rect));
        ...
        av_em_log(ctx, AV_LOG_ERROR, "Buffered too many pages, dropping page %s.\n", pgno_str);
...
static int slice_to_vbi_lines(TeletextContext *ctx, uint8_t* buf, int size)
{
    ...
    if (size)
        av_em_log(ctx, AV_LOG_WARNING, "%d bytes remained after slicing data\n", size);
    return lines;
}

static int teletext_decode_frame(AVEMCodecContext *avctx, void *data, int *data_size, AVEMPacket *pkt)
{
    ...
    if (avctx->pkt_timebase.num && pkt->pts != AV_NOPTS_VALUE)
        ctx->pts = av_em_rescale_q(pkt->pts, avctx->pkt_timebase, AV_TIME_BASE_Q);
...
#ifdef DEBUG
                int i;
                av_em_log(avctx, AV_LOG_DEBUG, "line numbers:");
                for(i = 0; i < lines; i++)
                    av_em_log(avctx, AV_LOG_DEBUG, " %d", ctx->sliced[i].line);
    ...
    if (ctx->nb_pages) {
        ...
        if (ctx->pages->sub_rect->type != SUBTITLE_NONE) {
            sub->rects = av_em_alloc(sizeof(*sub->rects));
            if (sub->rects) {
                sub->num_rects = 1;
...
FF_ENABLE_DEPRECATION_WARNINGS
            ...
            av_em_log(avctx, AV_LOG_DEBUG, "sending empty sub\n");
            sub->rects = NULL;
        }
...
static int teletext_init_decoder(AVEMCodecContext *avctx)
{
    TeletextContext *ctx = avctx->priv_data;
    ...
    if (!(maj > 0 || min > 2 || min == 2 && rev >= 26)) {
        av_em_log(avctx, AV_LOG_ERROR, "decoder needs zvbi version >= 0.2.26.\n");
        return AVERROR_EXTERNAL;
    }
...
#endif
    av_em_log(avctx, AV_LOG_VERBOSE, "page filter: %s\n", ctx->pgno);
    return (ctx->format_id == 1) ? ff_ass_subtitle_header_default(avctx) : 0;
}

static int teletext_close_decoder(AVEMCodecContext *avctx)
{
    TeletextContext *ctx = avctx->priv_data;
...
static void teletext_flush(AVEMCodecContext *avctx)
{
...
static const AVEMClass teletext_class = {
...
AVEMCodec ff_libzvbi_teletext_decoder = {
