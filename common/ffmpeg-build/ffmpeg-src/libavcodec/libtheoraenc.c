                              ...
                              const ogg_packet* packet)
{
    ...
    if (packet->bytes < 0) {
        ...
        if ((err = av_em_reallocp(&avc_context->extradata, newsize)) < 0) {
            avc_context->extradata_size = 0;
            message = "av_em_realloc failed";
        }
    }
    if (message) {
        av_em_log(avc_context, AV_LOG_ERROR, "concatenate_packet failed: %s\n", message);
        return err;
    }
...
static int get_stats(AVEMCodecContext *avctx, int eos)
{
    ...
    if (bytes < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error getting first pass stats\n");
        return AVERROR_EXTERNAL;
    }
    if (!eos) {
        void *tmp = av_em_fast_realloc(h->stats, &h->stats_size,
        ...
        if (!avctx->stats_out)
            return AVERROR(ENOMEM);
        av_em_base64_encode(avctx->stats_out, b64_size, h->stats, h->stats_offset);
    }
    return 0;
#else
    av_em_log(avctx, AV_LOG_ERROR, "libtheora too old to support 2pass\n");
...
static int submit_stats(AVEMCodecContext *avctx)
{
    ...
    if (!h->stats) {
        if (!avctx->stats_in) {
            av_em_log(avctx, AV_LOG_ERROR, "No statsfile for second pass\n");
            return AVERROR(EINVAL);
        }
        h->stats_size = strlen(avctx->stats_in) * 3/4;
        h->stats      = av_em_alloc(h->stats_size);
        if (!h->stats) {
            h->stats_size = 0;
            return AVERROR(ENOMEM);
        }
        h->stats_size = av_em_base64_decode(h->stats, avctx->stats_in, h->stats_size);
    }
    while (h->stats_size - h->stats_offset > 0) {
        ...
        if (bytes < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Error submitting stats\n");
            return AVERROR_EXTERNAL;
        }
        if (!bytes)
            return 0;
...
static av_cold int encode_init(AVEMCodecContext* avc_context)
{
    th_info t_info;
    ...
    t_info.pic_x        = 0;
    t_info.pic_y        = 0;
    /* Swap numerator and denominator as time_base in AVEMCodecContext gives the
    ...
    if (avc_context->pix_fmt == AV_PIX_FMT_YUV420P)
        ...
        av_em_log(avc_context, AV_LOG_ERROR, "Unsupported pix_fmt\n");
        return AVERROR(EINVAL);
    }
    avcodec_em_get_chroma_sub_sample(avc_context->pix_fmt, &h->uv_hshift, &h->uv_vshift);
    ...
    if (!h->t_state) {
        av_em_log(avc_context, AV_LOG_ERROR, "theora_encode_init failed\n");
        return AVERROR_EXTERNAL;
    }
    ...
    if (th_encode_ctl(h->t_state, TH_ENCCTL_SET_KEYFRAME_FREQUENCY_FORCE,
        ...
        av_em_log(avc_context, AV_LOG_ERROR, "Error setting GOP size\n");
        return AVERROR_EXTERNAL;
    }
...
static int encode_frame(AVEMCodecContext* avc_context, AVEMPacket *pkt,
                        const AVFrame *frame, int *got_packet)
{
    ...
    if (result) {
        ...
        av_em_log(avc_context, AV_LOG_ERROR, "theora_encode_YUVin failed (%s) [%d]\n", message, result);
        return AVERROR_EXTERNAL;
    }
    ...
    case 1:
        ...
        break;
    default:
        av_em_log(avc_context, AV_LOG_ERROR, "theora_encode_packetout failed [%d]\n", result);
        return AVERROR_EXTERNAL;
    }

    /* Copy ogg_packet content out to buffer */
    if ((ret = em_alloc_packet2(avc_context, pkt, o_packet.bytes, 0)) < 0)
        return ret;
...
static av_cold int encode_close(AVEMCodecContext* avc_context)
{
    TheoraContext *h = avc_context->priv_data;

    th_encode_free(h->t_state);
    av_em_freep(&h->stats);
    av_em_freep(&avc_context->stats_out);
    av_em_freep(&avc_context->extradata);
    avc_context->extradata_size = 0;

    return 0;
}

/** AVEMCodec struct exposed to libavcodec */
AVEMCodec ff_libtheora_encoder = {
    .name           = "libtheora",
