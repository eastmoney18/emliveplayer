...
av_cold int ff_libwebp_encode_init_common(AVEMCodecContext *avctx)
{
    ...
    if (avctx->compression_level < 0 || avctx->compression_level > 6) {
        av_em_log(avctx, AV_LOG_WARNING, "invalid compression level: %d\n",
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "%s - quality=%.1f method=%d\n",
...
int ff_libwebp_get_frame(AVEMCodecContext *avctx, LibWebPContextCommon *s,
    ...
    if (avctx->width > WEBP_MAX_DIMENSION || avctx->height > WEBP_MAX_DIMENSION) {
        av_em_log(avctx, AV_LOG_ERROR, "Picture size is too large. Max is %dx%d.\n",
               WEBP_MAX_DIMENSION, WEBP_MAX_DIMENSION);
        return AVERROR(EINVAL);
    }

    *pic_ptr = av_em_alloc(sizeof(*pic));
    pic = *pic_ptr;
    if (!pic)
        return AVERROR(ENOMEM);
    ...
    if (avctx->pix_fmt == AV_PIX_FMT_RGB32) {
        ...
        if (frame->linesize[1] != frame->linesize[2] || s->cr_threshold) {
            ...
            *alt_frame_ptr = av_em_frame_alloc();
            ...
            ret = av_em_frame_get_buffer(alt_frame, 32);
            if (ret < 0)
                goto end;
            alt_frame->format = frame->format;
            av_em_frame_copy(alt_frame, frame);
            frame = alt_frame;
            if (s->cr_threshold) {
                int x,y, x2, y2, p;
                int bs = s->cr_size;
