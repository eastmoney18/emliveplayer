...
static int parse_frame_header(DCACoreDecoder *s)
{
    ...
    if (get_bits(&s->gb, 5) != DCA_PCMBLOCK_SAMPLES - 1) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Deficit samples are not supported\n");
    ...
    if (s->npcmblocks & (DCA_SUBBAND_SAMPLES - 1)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Unsupported number of PCM sample blocks (%d)\n", s->npcmblocks);
    ...
    if (s->frame_size < 96) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid core frame size (%d bytes)\n", s->frame_size);
    ...
    if (s->audio_mode >= AMODE_COUNT) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Unsupported audio channel arrangement (%d)\n", s->audio_mode);
    ...
    if (!s->sample_rate) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid core audio sampling frequency\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->lfe_present == LFE_FLAG_INVALID) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid low frequency effects flag\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (!s->source_pcm_res) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid source PCM resolution\n");
        return AVERROR_INVALIDDATA;
    }
...
static int parse_coding_header(DCACoreDecoder *s, enum HeaderType header, int xch_base)
{
    ...
    case HEADER_CORE:
        ...
        if (s->nchannels != ff_dca_channels[s->audio_mode]) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid number of primary audio channels (%d) for audio channel arrangement (%d)\n", s->nchannels, s->audio_mode);
            return AVERROR_INVALIDDATA;
        }
    ...
    case HEADER_XXCH:
        ...
        if (s->xxch_crc_present
            ...
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XXCH channel set header checksum\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        if (nchannels > DCA_XXCH_CHANNELS_MAX) {
            avpriv_em_request_sample(s->avctx, "%d XXCH channels", nchannels);
            return AVERROR_PATCHWELCOME;
        }
        ...
        if (av_popcount(s->xxch_spkr_mask) != nchannels) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XXCH speaker layout mask (%#x)\n", s->xxch_spkr_mask);
            return AVERROR_INVALIDDATA;
        }

        if (s->xxch_core_mask & s->xxch_spkr_mask) {
            av_em_log(s->avctx, AV_LOG_ERROR, "XXCH speaker layout mask (%#x) overlaps with core (%#x)\n", s->xxch_spkr_mask, s->xxch_core_mask);
            return AVERROR_INVALIDDATA;
        }
        ...
        if (get_bits1(&s->gb)) {
            ...
            if (index >= FF_DCA_INV_DMIXTABLE_SIZE) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XXCH downmix scale index (%d)\n", index);
                return AVERROR_INVALIDDATA;
            }
            ...
            for (ch = 0; ch < nchannels; ch++) {
                ...
                if ((mask & s->xxch_core_mask) != mask) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XXCH downmix channel mapping mask (%#x)\n", mask);
                    return AVERROR_INVALIDDATA;
                }
                    ...
                    if (s->xxch_dmix_mask[ch] & (1U << n)) {
                        ...
                        if (code &= 63) {
                            ...
                            if (index >= FF_DCA_DMIXTABLE_SIZE) {
                                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XXCH downmix coefficient index (%d)\n", index);
                                return AVERROR_INVALIDDATA;
                            }
    ...
    for (ch = xch_base; ch < s->nchannels; ch++) {
        ...
        if (s->nsubbands[ch] > DCA_SUBBANDS) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid subband activity count\n");
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    for (ch = xch_base; ch < s->nchannels; ch++) {
        if ((n = get_bits(&s->gb, 3)) && header == HEADER_XXCH)
            n += xch_base - 1;
        if (n > s->nchannels) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid joint intensity coding index\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    for (ch = xch_base; ch < s->nchannels; ch++) {
        ...
        if (s->scale_factor_sel[ch] == 7) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid scale factor code book\n");
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    for (ch = xch_base; ch < s->nchannels; ch++) {
        ...
        if (s->bit_allocation_sel[ch] == 7) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid bit allocation quantizer select\n");
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    if (header == HEADER_XXCH) {
        ...
        if (ff_dca_seek_bits(&s->gb, header_pos + header_size * 8)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of XXCH channel set header\n");
            return AVERROR_INVALIDDATA;
        }
    } else {
...
static inline int parse_scale(DCACoreDecoder *s, int *scale_index, int sel)
{
    ...
    if ((unsigned int)*scale_index >= scale_size) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid scale factor index\n");
        return AVERROR_INVALIDDATA;
    }
...
static inline int parse_joint_scale(DCACoreDecoder *s, int sel)
{
    ...
    if ((unsigned int)scale_index >= FF_ARRAY_ELEMS(ff_dca_joint_scale_factors)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid joint scale factor index\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    for (ch = xch_base; ch < s->nchannels; ch++) {
        ...
        for (band = 0; band < s->subband_vq_start[ch]; band++) {
            ...
            if (abits > DCA_ABITS_MAX) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid bit allocation index\n");
                return AVERROR_INVALIDDATA;
            }
    ...
    for (ch = xch_base; ch < s->nchannels; ch++) {
        if (s->joint_intensity_index[ch]) {
            ...
            if (s->joint_scale_sel[ch] == 7) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid joint scale factor code book\n");
                return AVERROR_INVALIDDATA;
            }
        }
    }
...
static inline int parse_block_codes(DCACoreDecoder *s, int32_t *audio, int abits)
{
    ...
    if (decode_blockcodes(code1, code2, levels, audio)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Failed to decode block code(s)\n");
        return AVERROR_INVALIDDATA;
    }
                                ...
                                int xch_base, int *sub_pos, int *lfe_pos)
{
    ...
    if (*sub_pos + nsamples > s->npcmblocks) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Subband sample buffer overflow\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->lfe_present && header == HEADER_CORE) {
        ...
        if (index >= FF_ARRAY_ELEMS(ff_dca_scale_factor_quant7)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid LFE scale factor index\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    for (ssf = 0, ofs = *sub_pos; ssf < s->nsubsubframes[sf]; ssf++) {
        ...
        if ((ssf == s->nsubsubframes[sf] - 1 || s->sync_ssf) && get_bits(&s->gb, 16) != 0xffff) {
            av_em_log(s->avctx, AV_LOG_ERROR, "DSYNC check failed\n");
            return AVERROR_INVALIDDATA;
        }
...
static int alloc_sample_buffer(DCACoreDecoder *s)
{
    ...
    av_em_fast_mallocz(&s->subband_buffer, &s->subband_size,
...
static int parse_xch_frame(DCACoreDecoder *s)
{
    ...
    if (s->ch_mask & DCA_SPEAKER_MASK_Cs) {
        av_em_log(s->avctx, AV_LOG_ERROR, "XCH with Cs speaker already present\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ff_dca_seek_bits(&s->gb, s->frame_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of XCH frame\n");
        return AVERROR_INVALIDDATA;
    }
...
static int parse_xxch_frame(DCACoreDecoder *s)
{
    ...
    if (get_bits_long(&s->gb, 32) != DCA_SYNCWORD_XXCH) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XXCH sync word\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ff_dca_check_crc(s->avctx, &s->gb, header_pos + 32, header_pos + header_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XXCH frame header checksum\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->xxch_mask_nbits <= DCA_SPEAKER_Cs) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid number of bits for XXCH speaker mask (%d)\n", s->xxch_mask_nbits);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (xxch_nchsets > 1) {
        avpriv_em_request_sample(s->avctx, "%d XXCH channel sets", xxch_nchsets);
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (mask != s->xxch_core_mask) {
        av_em_log(s->avctx, AV_LOG_ERROR, "XXCH core speaker activity mask (%#x) disagrees with core (%#x)\n", s->xxch_core_mask, mask);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ff_dca_seek_bits(&s->gb, header_pos + header_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of XXCH frame header\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ff_dca_seek_bits(&s->gb, header_pos + header_size * 8 + xxch_frame_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of XXCH channel set\n");
        return AVERROR_INVALIDDATA;
    }
                              ...
                              int *xbr_nsubbands, int xbr_transition_mode, int sf, int *sub_pos)
{
    ...
    for (ch = xbr_base_ch; ch < xbr_nchannels; ch++) {
        for (band = 0; band < xbr_nsubbands[ch]; band++) {
            ...
            if (xbr_bit_allocation[ch][band] > DCA_ABITS_MAX) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XBR bit allocation index\n");
                return AVERROR_INVALIDDATA;
            }
        }
    }
    ...
    for (ch = xbr_base_ch; ch < xbr_nchannels; ch++) {
        ...
        if (!xbr_scale_nbits[ch]) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid number of bits for XBR scale factor index\n");
            return AVERROR_INVALIDDATA;
        }
    }
        ...
        for (band = 0; band < xbr_nsubbands[ch]; band++) {
            if (xbr_bit_allocation[ch][band]) {
                ...
                if (scale_index >= scale_size) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XBR scale factor index\n");
                    return AVERROR_INVALIDDATA;
                }
    ...
    for (ssf = 0, ofs = *sub_pos; ssf < s->nsubsubframes[sf]; ssf++) {
        ...
        // DSYNC
        if ((ssf == s->nsubsubframes[sf] - 1 || s->sync_ssf) && get_bits(&s->gb, 16) != 0xffff) {
            av_em_log(s->avctx, AV_LOG_ERROR, "XBR-DSYNC check failed\n");
            return AVERROR_INVALIDDATA;
        }
...
static int parse_xbr_frame(DCACoreDecoder *s)
{
    ...
    if (get_bits_long(&s->gb, 32) != DCA_SYNCWORD_XBR) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XBR sync word\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ff_dca_check_crc(s->avctx, &s->gb, header_pos + 32, header_pos + header_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XBR frame header checksum\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    for (i = 0, ch2 = 0; i < xbr_nchsets; i++) {
        ...
        for (ch1 = 0; ch1 < xbr_nchannels[i]; ch1++, ch2++) {
            ...
            if (xbr_nsubbands[ch2] > DCA_SUBBANDS) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid number of active XBR subbands (%d)\n", xbr_nsubbands[ch2]);
                return AVERROR_INVALIDDATA;
            }
        }
    }
    ...
    if (ff_dca_seek_bits(&s->gb, header_pos + header_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of XBR frame header\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    for (i = 0, xbr_base_ch = 0; i < xbr_nchsets; i++) {
        ...
        if (ff_dca_seek_bits(&s->gb, header_pos + xbr_frame_size[i] * 8)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of XBR channel set\n");
            return AVERROR_INVALIDDATA;
        }
    }
...
static int parse_x96_subframe_audio(DCACoreDecoder *s, int sf, int xch_base, int *sub_pos)
{
    ...
    for (ssf = 0, ofs = *sub_pos; ssf < s->nsubsubframes[sf]; ssf++) {
        ...
        // DSYNC
        if ((ssf == s->nsubsubframes[sf] - 1 || s->sync_ssf) && get_bits(&s->gb, 16) != 0xffff) {
            av_em_log(s->avctx, AV_LOG_ERROR, "X96-DSYNC check failed\n");
            return AVERROR_INVALIDDATA;
        }
...
static int alloc_x96_sample_buffer(DCACoreDecoder *s)
{
    ...
    // Reallocate subband sample buffer
    av_em_fast_mallocz(&s->x96_subband_buffer, &s->x96_subband_size,
...
static int parse_x96_subframe_header(DCACoreDecoder *s, int xch_base)
{
    ...
    for (ch = xch_base; ch < s->x96_nchannels; ch++) {
        ...
        for (band = s->x96_subband_start; band < s->nsubbands[ch]; band++) {
            ...
            if (abits < 0 || abits > 7 + 8 * s->x96_high_res) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid X96 bit allocation index\n");
                return AVERROR_INVALIDDATA;
            }
    ...
    for (ch = xch_base; ch < s->x96_nchannels; ch++) {
        if (s->joint_intensity_index[ch]) {
            s->joint_scale_sel[ch] = get_bits(&s->gb, 3);
            if (s->joint_scale_sel[ch] == 7) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid X96 joint scale factor code book\n");
                return AVERROR_INVALIDDATA;
            }
        }
    }
...
static int parse_x96_coding_header(DCACoreDecoder *s, int exss, int xch_base)
{
    ...
    if (exss) {
        ...
        if (s->x96_crc_present
            && ff_dca_check_crc(s->avctx, &s->gb, header_pos, header_pos + header_size * 8)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid X96 channel set header checksum\n");
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    if (s->x96_rev_no < 8) {
        ...
        if (s->x96_subband_start > 27) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid X96 subband start index (%d)\n", s->x96_subband_start);
            return AVERROR_INVALIDDATA;
        }
    } else {
    ...
    for (ch = xch_base; ch < s->x96_nchannels; ch++) {
        ...
        if (s->nsubbands[ch] < DCA_SUBBANDS) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid X96 subband activity count (%d)\n", s->nsubbands[ch]);
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    for (ch = xch_base; ch < s->x96_nchannels; ch++) {
        ...
        if (n > s->x96_nchannels) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid X96 joint intensity coding index\n");
            return AVERROR_INVALIDDATA;
        }
        s->joint_intensity_index[ch] = n;
    }
    ...
    for (ch = xch_base; ch < s->x96_nchannels; ch++) {
        ...
        if (s->scale_factor_sel[ch] >= 6) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid X96 scale factor code book\n");
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    if (exss) {
        ...
        // CRC16 of channel set header
        if (ff_dca_seek_bits(&s->gb, header_pos + header_size * 8)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of X96 channel set header\n");
            return AVERROR_INVALIDDATA;
        }
    } else {
        if (s->crc_present)
...
static int parse_x96_frame(DCACoreDecoder *s)
{
    ...
    if (s->x96_rev_no < 1 || s->x96_rev_no > 8) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid X96 revision (%d)\n", s->x96_rev_no);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ff_dca_seek_bits(&s->gb, s->frame_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of X96 frame\n");
        return AVERROR_INVALIDDATA;
    }
...
static int parse_x96_frame_exss(DCACoreDecoder *s)
{
    ...
    if (get_bits_long(&s->gb, 32) != DCA_SYNCWORD_X96) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid X96 sync word\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ff_dca_check_crc(s->avctx, &s->gb, header_pos + 32, header_pos + header_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid X96 frame header checksum\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ff_dca_seek_bits(&s->gb, header_pos + header_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of X96 frame header\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    for (i = 0, x96_base_ch = 0; i < x96_nchsets; i++) {
        ...
        if (ff_dca_seek_bits(&s->gb, header_pos + x96_frame_size[i] * 8)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of X96 channel set\n");
            return AVERROR_INVALIDDATA;
        }
    }
...
static int parse_aux_data(DCACoreDecoder *s)
{
    ...
    if (get_bits_long(&s->gb, 32) != DCA_SYNCWORD_REV1AUX) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid auxiliary data sync word\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->prim_dmix_embedded = get_bits1(&s->gb)) {
        ...
        if (s->prim_dmix_type >= DCA_DMIX_TYPE_COUNT) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid primary channel set downmix type\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        for (i = 0; i < m * n; i++) {
            ...
            if (index >= FF_DCA_DMIXTABLE_SIZE) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid downmix coefficient index\n");
                return AVERROR_INVALIDDATA;
            }
    ...
    if (ff_dca_check_crc(s->avctx, &s->gb, aux_pos, get_bits_count(&s->gb))) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid auxiliary data checksum\n");
        return AVERROR_INVALIDDATA;
    }
...
static int parse_optional_info(DCACoreDecoder *s)
{
    ...
    if (s->ext_audio_present && !dca->core_only) {
        ...
        case EXT_AUDIO_XCH:
            ...
            if (!s->xch_pos) {
                av_em_log(s->avctx, AV_LOG_ERROR, "XCH sync word not found\n");
        ...
        case EXT_AUDIO_X96:
            ...
            if (!s->x96_pos) {
                av_em_log(s->avctx, AV_LOG_ERROR, "X96 sync word not found\n");
                if (s->avctx->err_recognition & AV_EF_EXPLODE)
                    return AVERROR_INVALIDDATA;
            }
            break;
        ...
        case EXT_AUDIO_XXCH:
            ...
            for (; sync_pos >= last_pos; sync_pos--) {
                if (AV_RB32(s->gb.buffer + sync_pos * 4) == DCA_SYNCWORD_XXCH) {
                    ...
                    if (size >= 11 && size <= dist &&
                        !av_em_crc(dca->crctab, 0xffff, s->gb.buffer +
                                (sync_pos + 1) * 4, size - 4)) {
            ...
            if (!s->xxch_pos) {
                av_em_log(s->avctx, AV_LOG_ERROR, "XXCH sync word not found\n");
                if (s->avctx->err_recognition & AV_EF_EXPLODE)
                    return AVERROR_INVALIDDATA;
            }
            break;
...
int ff_dca_core_parse(DCACoreDecoder *s, uint8_t *data, int size)
{
    ...
    if (ff_dca_seek_bits(&s->gb, s->frame_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of core frame\n");
        if (s->avctx->err_recognition & AV_EF_EXPLODE)
            return AVERROR_INVALIDDATA;
    }
...
int ff_dca_core_filter_fixed(DCACoreDecoder *s, int x96_synth)
{
    ...
    av_em_fast_malloc(&s->output_buffer, &s->output_size,
    ...
    if (s->lfe_present) {
        ...
        if (s->lfe_present == LFE_FLAG_128) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Fixed point mode doesn't support LFF=1\n");
            return AVERROR(EINVAL);
        }
...
int ff_dca_core_filter_frame(DCACoreDecoder *s, AVFrame *frame)
{
    ...
    if ((ret = em_side_data_update_matrix_encoding(frame, matrix_encoding)) < 0)
        return ret;
...
{
    if (!(s->float_dsp = avpriv_em_float_dsp_alloc(0)))
        return -1;
    if (!(s->fixed_dsp = avpriv_em_alloc_fixed_dsp(0)))
        return -1;
...
{
    av_em_freep(&s->float_dsp);
    av_em_freep(&s->fixed_dsp);

    ff_mdct_end(&s->imdct[0]);
    ff_mdct_end(&s->imdct[1]);

    av_em_freep(&s->subband_buffer);
    s->subband_size = 0;

    av_em_freep(&s->x96_subband_buffer);
    s->x96_subband_size = 0;

    av_em_freep(&s->output_buffer);
    s->output_size = 0;
}
