                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (bytestream2_get_bytes_left(&gb) < 2) {
        av_em_log(avctx, AV_LOG_ERROR, "Input data too small\n");
        return AVERROR_INVALIDDATA;
    }

    if (width < 16 || height < 16) {
        av_em_log(avctx, AV_LOG_ERROR, "Dimensions too small\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (inter && !s->last_frame->data[0]) {
        av_em_log(avctx, AV_LOG_WARNING, "Missing reference frame.\n");
        ret = em_get_buffer(avctx, s->last_frame, AV_GET_BUFFER_FLAG_REF);
        if (ret < 0)
            return ret;
    ...
    av_em_fast_padded_malloc(&s->bitstream_buf, &s->bitstream_buf_size,
...
static av_cold int decode_end(AVEMCodecContext *avctx)
{
    ...
    av_em_frame_free(&t->last_frame);
    av_em_freep(&t->bitstream_buf);
    return 0;
}

AVEMCodec ff_eamad_decoder = {
    .name           = "eamad",
