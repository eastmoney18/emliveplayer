...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if(deflate(&c->zstream, Z_SYNC_FLUSH) != Z_OK){
        av_em_log(avctx, AV_LOG_ERROR, "Error compressing data\n");
        return -1;
    }
...
static av_cold int encode_end(AVEMCodecContext *avctx)
{
    ...
    av_em_freep(&c->comp_buf);
    av_em_freep(&c->work_buf);
...
static av_cold int encode_init(AVEMCodecContext *avctx)
{
    ...
    if(lvl < 0 || lvl > 9){
        av_em_log(avctx, AV_LOG_ERROR, "Compression level should be 0-9, not %i\n", lvl);
    ...
    if (!(c->work_buf = av_em_alloc(c->comp_size))) {
        av_em_log(avctx, AV_LOG_ERROR, "Can't allocate work buffer.\n");
    ...
    if (!(c->comp_buf = av_em_alloc(c->comp_size))) {
        av_em_log(avctx, AV_LOG_ERROR, "Can't allocate compression buffer.\n");
        return AVERROR(ENOMEM);
    }
    c->pstride = FFALIGN(avctx->width, 16);
    if (!(c->prev = av_em_alloc(c->pstride * avctx->height))) {
        av_em_log(avctx, AV_LOG_ERROR, "Can't allocate picture.\n");
        return AVERROR(ENOMEM);
    }
