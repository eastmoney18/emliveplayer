    ...
    for (j = 0; j < h; j += 16) {
        ...
        for (i = 0; i < w; i += 16, dst2 += 16 * bpp) {
            if (bytestream2_get_bytes_left(gb) <= 0) {
                av_em_log(c->avctx, AV_LOG_ERROR, "Premature end of data!\n");
            ...
            if (flags & HT_RAW) {
                ...
                for (k = 0; k < rects; k++) {
                    ...
                    if (   (xy >> 4) + (wh >> 4) + 1 > w - i
                        || (xy & 0xF) + (wh & 0xF)+1 > h - j) {
                        av_em_log(c->avctx, AV_LOG_ERROR, "Rectangle outside picture\n");
                        return AVERROR_INVALIDDATA;
                    }
...
{
    av_em_freep(&c->curbits);
    av_em_freep(&c->curmask);
    av_em_freep(&c->screendta);
    c->cur_w = c->cur_h = 0;
    c->cur_hx = c->cur_hy = 0;
                        ...
                        AVEMPacket *avpkt)
{
    ...
    while (chunks--) {
        if (bytestream2_get_bytes_left(gb) < 12) {
            av_em_log(avctx, AV_LOG_ERROR, "Premature end of data!\n");
            return -1;
        }
        ...
        case MAGIC_WMVd: // cursor
            if (w*(int64_t)h*c->bpp2 > INT_MAX/2 - 2) {
                av_em_log(avctx, AV_LOG_ERROR, "dimensions too large\n");
                return AVERROR_INVALIDDATA;
            }
            ...
            if (c->cur_w * c->cur_h >= INT_MAX / c->bpp2) {
                ...
                if ((ret = av_em_reallocp(&c->curbits, screen_size)) < 0 ||
                    (ret = av_em_reallocp(&c->curmask, screen_size)) < 0 ||
                    (ret = av_em_reallocp(&c->screendta, screen_size)) < 0) {
                    reset_buffers(c);
                    return ret;
                }
            }
        ...
        case 0x00000005: // HexTile encoded rectangle
            ...
            break;
        default:
            av_em_log(avctx, AV_LOG_ERROR, "Unsupported block type 0x%08X\n", enc);
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    case 32:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported bitdepth %i\n", c->bpp);
        return AVERROR_INVALIDDATA;
    }
    c->bpp2 = c->bpp / 8;
