...
av_cold void ff_af_queue_init(AVEMCodecContext *avctx, AudioFrameQueue *afq)
{
    afq->avctx = avctx;
...
{
    if(afq->frame_count)
        av_em_log(afq->avctx, AV_LOG_WARNING, "%d frames left in the queue on closing\n", afq->frame_count);
    av_em_freep(&afq->frames);
    memset(afq, 0, sizeof(*afq));
}

int ff_af_queue_add(AudioFrameQueue *afq, const AVFrame *f)
{
    AudioFrame *new = av_em_fast_realloc(afq->frames, &afq->frame_alloc, sizeof(*afq->frames)*(afq->frame_count+1));
    if(!new)
    ...
    if (f->pts != AV_NOPTS_VALUE) {
        new->pts = av_em_rescale_q(f->pts,
                                      afq->avctx->time_base,
                                      (AVEMRational){ 1, afq->avctx->sample_rate });
        new->pts -= afq->remaining_delay;
        if(afq->frame_count && new[-1].pts >= new->pts)
            av_em_log(afq->avctx, AV_LOG_WARNING, "Queue input is backward in time\n");
    } else {
                        ...
                        int64_t *duration)
{
    ...
    if(!afq->frame_count)
        av_em_log(afq->avctx, AV_LOG_WARNING, "Trying to remove %d samples, but the queue is empty\n", nb_samples);
    if (pts)
        *pts = em_samples_to_time_base(afq->avctx, out_pts);
    ...
    if(nb_samples){
        ...
        av_em_log(afq->avctx, AV_LOG_DEBUG, "Trying to remove %d more samples than there are in the queue\n", nb_samples);
    }
    if (duration)
        *duration = em_samples_to_time_base(afq->avctx, removed_samples);
}
