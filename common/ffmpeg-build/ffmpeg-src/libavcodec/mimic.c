...
static av_cold int mimic_decode_end(AVEMCodecContext *avctx)
{
    MimicContext *ctx = avctx->priv_data;
    int i;

    av_em_freep(&ctx->swap_buf);
    ctx->swap_buf_size = 0;

    for (i = 0; i < FF_ARRAY_ELEMS(ctx->frames); i++) {
        if (ctx->frames[i].f)
            em_thread_release_buffer(avctx, &ctx->frames[i]);
        av_em_frame_free(&ctx->frames[i].f);
    }
...
static av_cold int mimic_decode_init(AVEMCodecContext *avctx)
{
    MimicContext *ctx = avctx->priv_data;
    int ret, i;
...
#if HAVE_THREADS
static int mimic_decode_update_thread_context(AVEMCodecContext *avctx, const AVEMCodecContext *avctx_from)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(dst->frames); i++) {
        em_thread_release_buffer(avctx, &dst->frames[i]);
        if (i != src->next_cur_index && src->frames[i].f->data[0]) {
            ret = em_thread_ref_frame(&dst->frames[i], &src->frames[i]);
            if (ret < 0)
                return ret;
        }
    }
                  ...
                  int is_iframe)
{
    ...
    for (plane = 0; plane < 3; plane++) {
        ...
        for (y = 0; y < ctx->num_vblocks[plane]; y++) {
                ...
                if (is_iframe || get_bits1(&ctx->gb) == is_chroma) {
                    ...
                    if (is_chroma || is_iframe || !get_bits1(&ctx->gb)) {
                        if ((ret = vlc_decode_block(ctx, num_coeffs,
                                                    qscale)) < 0) {
                            av_em_log(ctx->avctx, AV_LOG_ERROR, "Error decoding "
                                   "block.\n");
                            return ret;
                        }
                        ...
                        if (index != ctx->cur_index && p) {
                            em_thread_await_progress(&ctx->frames[index],
                                                     cur_row, 0);
                            p += src -
                    ...
                    em_thread_await_progress(&ctx->frames[ctx->prev_index],
                                             cur_row, 0);
            ...
            em_thread_report_progress(&ctx->frames[ctx->cur_index],
                                      cur_row++, 0);
        }
    }
...
static int mimic_decode_frame(AVEMCodecContext *avctx, void *data,
                              int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (buf_size <= MIMIC_HEADER_SIZE) {
        av_em_log(avctx, AV_LOG_ERROR, "insufficient data\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (!ctx->avctx) {
        ...
        if (!(width == 160 && height == 120) &&
            ...
            av_em_log(avctx, AV_LOG_ERROR, "invalid width/height!\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        avpriv_em_request_sample(avctx, "Resolution changing");
        return AVERROR_PATCHWELCOME;
    }

    if (is_pframe && !ctx->frames[ctx->prev_index].f->data[0]) {
        av_em_log(avctx, AV_LOG_ERROR, "decoding must start with keyframe\n");
        return AVERROR_INVALIDDATA;
    }

    em_thread_release_buffer(avctx, &ctx->frames[ctx->cur_index]);
    ctx->frames[ctx->cur_index].f->pict_type = is_pframe ? AV_PICTURE_TYPE_P :
                                                           AV_PICTURE_TYPE_I;
    if ((res = em_thread_get_buffer(avctx, &ctx->frames[ctx->cur_index],
    ...
    av_em_fast_padded_malloc(&ctx->swap_buf, &ctx->swap_buf_size, swap_buf_size);
    if (!ctx->swap_buf)
        return AVERROR(ENOMEM);
    ...
    em_thread_report_progress(&ctx->frames[ctx->cur_index], INT_MAX, 0);
    if (res < 0) {
    ...
    if ((res = av_em_frame_ref(data, ctx->frames[ctx->cur_index].f)) < 0)
        return res;
    *got_frame      = 1;
...
#if HAVE_THREADS
static av_cold int mimic_init_thread_copy(AVEMCodecContext *avctx)
{
    MimicContext *ctx = avctx->priv_data;
    int i;
