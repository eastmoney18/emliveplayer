...
{
    av_em_log(data, AV_LOG_ERROR, "%s", msg);
}

static void warning_callback(const char *msg, void *data)
{
    av_em_log(data, AV_LOG_WARNING, "%s", msg);
}

static void info_callback(const char *msg, void *data)
{
    av_em_log(data, AV_LOG_DEBUG, "%s", msg);
}
...
static inline void libopenjpeg_copy_to_packed16(AVFrame *picture, opj_image_t *image) {
    uint16_t *img_ptr;
    const AVPixFmtDescriptor *desc = av_em_pix_fmt_desc_get(picture->format);
    int index, x, y, c;
    int adjust[4];
...
static av_cold int libopenjpeg_decode_init(AVEMCodecContext *avctx)
{
...
static int libopenjpeg_decode_frame(AVEMCodecContext *avctx,
                                    ...
                                    AVEMPacket *avpkt)
{
    ...
    *got_frame = 0;
    ...
    if ((AV_RB32(buf) == 12) &&
        ...
        /* If the AVEMPacket contains a jp2c box, then skip to
    ...
    if (!dec) {
        av_em_log(avctx, AV_LOG_ERROR, "Error initializing decoder.\n");
    ...
    if (!opj_set_error_handler(dec, error_callback, avctx) ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Error setting decoder handlers.\n");
        ret = AVERROR_EXTERNAL;
        goto done;
    }
...
#error Missing call to opj_stream_set_user_data
    ...
    if (ret) {
        av_em_log(avctx, AV_LOG_ERROR, "Error decoding codestream header.\n");
        ret = AVERROR_EXTERNAL;
        goto done;
    }
    ...
    ret = em_set_dimensions(avctx, width, height);
    if (ret < 0)
        goto done;
    ...
    if (avctx->pix_fmt == AV_PIX_FMT_NONE) {
        av_em_log(avctx, AV_LOG_ERROR, "Unable to determine pixel format.\n");
        ret = AVERROR_UNKNOWN;
        goto done;
    }
    ...
    if (ret) {
        av_em_log(avctx, AV_LOG_ERROR, "Error decoding codestream.\n");
        ret = AVERROR_EXTERNAL;
        goto done;
    }
    ...
    case 8:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "unsupported pixel size %d\n", pixel_size);
...
static av_cold void libopenjpeg_static_init(AVEMCodec *codec)
{
...
AVEMCodec ff_libopenjpeg_decoder = {
    .name           = "libopenjpeg",
