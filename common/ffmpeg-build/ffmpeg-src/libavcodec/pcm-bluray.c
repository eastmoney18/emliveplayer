 ...
 */
static int pcm_bluray_parse_header(AVEMCodecContext *avctx,
                                   const uint8_t *header)
{
    ...
    if (!(avctx->bits_per_coded_sample == 16 || avctx->bits_per_coded_sample == 24)) {
        av_em_log(avctx, AV_LOG_ERROR, "unsupported sample depth (%d)\n", avctx->bits_per_coded_sample);
    ...
    default:
        avctx->sample_rate = 0;
        av_em_log(avctx, AV_LOG_ERROR, "reserved sample rate (%d)\n",
               header[2] & 0x0f);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (!avctx->channels) {
        av_em_log(avctx, AV_LOG_ERROR, "reserved channel configuration (%d)\n",
               channel_layout);
        return AVERROR_INVALIDDATA;
    }
...
static int pcm_bluray_decode_frame(AVEMCodecContext *avctx, void *data,
...
AVEMCodec ff_pcm_bluray_decoder = {
    .name           = "pcm_bluray",
