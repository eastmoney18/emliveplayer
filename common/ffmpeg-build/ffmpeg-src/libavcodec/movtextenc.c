...
#define av_bprint_append_any(buf, data, size)   av_em_bprint_append_data(buf, ((const char*)data), size)
...
static void mov_text_cleanup(MovTextContext *s)
{
    ...
    if (s->box_flags & STYL_BOX) {
        for (j = 0; j < s->count; j++) {
            av_em_freep(&s->style_attributes[j]);
        }
        av_em_freep(&s->style_attributes);
    }
}
...
static av_cold int mov_text_encode_init(AVEMCodecContext *avctx)
{
    /*
    ...
    avctx->extradata = av_em_mallocz(avctx->extradata_size + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!avctx->extradata)
        return AVERROR(ENOMEM);

    av_em_bprint_init(&s->buffer, 0, AV_BPRINT_SIZE_UNLIMITED);
...
static void mov_text_style_cb(void *priv, const char style, int close)
{
    ...
    if (!close) {
        if (!(s->box_flags & STYL_BOX)) {   //first style entry
            ...
            s->style_attributes_temp = av_em_alloc(sizeof(*s->style_attributes_temp));

            if (!s->style_attributes_temp) {
                av_em_bprint_clear(&s->buffer);
            ...
            if (s->style_attributes_temp->style_flag) { //break the style record here and start a new one
                ...
                av_em_dynarray_add(&s->style_attributes, &s->count, s->style_attributes_temp);
                s->style_attributes_temp = av_em_alloc(sizeof(*s->style_attributes_temp));
                if (!s->style_attributes_temp) {
                    mov_text_cleanup(s);
                    av_em_bprint_clear(&s->buffer);
...
{
    MovTextContext *s = priv;
    av_em_bprint_append_data(&s->buffer, text, len);
    s->text_pos += len;
}
...
{
    MovTextContext *s = priv;
    av_em_bprint_append_data(&s->buffer, "\n", 1);
    s->text_pos += 1;
}
...
static int mov_text_encode_frame(AVEMCodecContext *avctx, unsigned char *buf,
                                 int bufsize, const AVSubtitle *sub)
{
    ...
    for (i = 0; i < sub->num_rects; i++) {
        ...
        if (sub->rects[i]->type != SUBTITLE_ASS) {
            av_em_log(avctx, AV_LOG_ERROR, "Only SUBTITLE_ASS type supported.\n");
    ...
    if (!av_em_bprint_is_complete(&s->buffer)) {
    ...
    if (s->buffer.len > bufsize - 3) {
        av_em_log(avctx, AV_LOG_ERROR, "Buffer too small for ASS event.\n");
...
static int mov_text_encode_close(AVEMCodecContext *avctx)
{
    MovTextContext *s = avctx->priv_data;
    ff_ass_split_free(s->ass_ctx);
    av_em_bprint_finalize(&s->buffer, NULL);
    return 0;
}

AVEMCodec ff_movtext_encoder = {
    .name           = "mov_text",
