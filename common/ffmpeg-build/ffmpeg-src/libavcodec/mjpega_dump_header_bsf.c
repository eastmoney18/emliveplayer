...
static int mjpega_dump_header(AVBSFContext *ctx, AVEMPacket *out)
{
    AVEMPacket *in;
    uint8_t *out_buf;
    ...
    ret = av_em_new_packet(out, in->size + 44);
    if (ret < 0)
        goto fail;
        ...
        if (in->data[i] == 0xff) {
            ...
            case APP1:
                if (i + 8 < in->size && AV_RL32(in->data + i + 8) == AV_RL32("mjpg")) {
                    av_em_log(ctx, AV_LOG_ERROR, "bitstream already formatted\n");
    ...
    av_em_log(ctx, AV_LOG_ERROR, "could not find SOS marker in bitstream\n");
fail:
    av_em_packet_unref(out);
    av_em_packet_free(&in);
...
const AVBitStreamFilter em_mjpega_dump_header_bsf = {
    .name      = "mjpegadump",
