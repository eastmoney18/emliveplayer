...
av_cold int ff_opus_parse_extradata(AVEMCodecContext *avctx,
                                    OpusContext *s)
{
    ...
    if (extradata_size < 19) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid extradata size: %d\n",
               extradata_size);
        return AVERROR_INVALIDDATA;
    }
    ...
    version = extradata[8];
    if (version > 15) {
        avpriv_em_request_sample(avctx, "Extradata version %d", version);
    ...
    if (!channels) {
        av_em_log(avctx, AV_LOG_ERROR, "Zero channel count specified in the extadata\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (!map_type) {
        ...
        avpriv_em_request_sample(avctx, "Mapping type %d", map_type);
        return AVERROR_PATCHWELCOME;
    }

    s->channel_maps = av_em_mallocz_array(channels, sizeof(*s->channel_maps));
    if (!s->channel_maps)
