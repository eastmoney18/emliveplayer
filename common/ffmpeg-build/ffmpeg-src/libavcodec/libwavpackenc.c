...
static int wavpack_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                                const AVFrame *frame, int *got_output)
{
    ...
    if (frame) {
        ...
        if (!ret) {
            av_em_log(avctx, AV_LOG_ERROR, "Error encoding a frame: %s\n",
    ...
    if (!s->got_output &&
        ...
        if (!ret) {
            av_em_log(avctx, AV_LOG_ERROR, "Error flushing the encoder: %s\n",
                   WavpackGetErrorMessage(s->wv));
            return AVERROR_UNKNOWN;
        }
    }
...
{
    AVEMCodecContext *avctx = id;
    LibWavpackContext *s  = avctx->priv_data;
    int ret, offset = s->pkt->size;

    if (s->user_size) {
        if (s->user_size - count < s->pkt->size) {
            av_em_log(avctx, AV_LOG_ERROR, "Provided packet too small.\n");
            return 0;
        }
        s->pkt->size += count;
    } else {
        ret = av_em_grow_packet(s->pkt, count);
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Error allocating output packet.\n");
            return 0;
        }
    }
...
static av_cold int wavpack_encode_init(AVEMCodecContext *avctx)
{
    ...
    if (!s->wv) {
        av_em_log(avctx, AV_LOG_ERROR, "Error allocating the encoder.\n");
...
fail:
    av_em_log(avctx, AV_LOG_ERROR, "Error configuring the encoder: %s.\n",
           WavpackGetErrorMessage(s->wv));
...
AVEMCodec ff_libwavpack_encoder = {
    .name           = "libwavpack",
