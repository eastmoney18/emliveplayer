...
static av_cold int cdxl_decode_init(AVEMCodecContext *avctx)
{
...
static int cdxl_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_frame, AVEMPacket *pkt)
{
    ...
    if (c->format != BIT_PLANAR && c->format != BIT_LINE && c->format != CHUNKY) {
        avpriv_em_request_sample(avctx, "Pixel format 0x%0x", c->format);
    ...
    if (!encoding && c->palette_size && c->bpp <= 8) {
        ...
        avpriv_em_request_sample(avctx, "Encoding %d, bpp %d and format 0x%x",
    ...
    if (encoding) {
        av_em_fast_padded_malloc(&c->new_video, &c->new_video_size,
...
static av_cold int cdxl_decode_end(AVEMCodecContext *avctx)
{
    CDXLVideoContext *c = avctx->priv_data;

    av_em_freep(&c->new_video);

    return 0;
}

AVEMCodec ff_cdxl_decoder = {
    .name           = "cdxl",
