...
static int tscc2_decode_frame(AVEMCodecContext *avctx, void *data,
                              int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (frame_type > 1) {
        av_em_log(avctx, AV_LOG_ERROR, "Incorrect frame type %"PRIu32"\n",
               frame_type);
        return AVERROR_INVALIDDATA;
    }

    if ((ret = em_reget_buffer(avctx, c->pic)) < 0) {
        return ret;
    }

    if (frame_type == 0) {
        *got_frame      = 1;
        if ((ret = av_em_frame_ref(data, c->pic)) < 0)
            return ret;
    ...
    if (bytestream2_get_bytes_left(&gb) < 4) {
        av_em_log(avctx, AV_LOG_ERROR, "Frame is too short\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (c->quant[0] < 2 || c->quant[0] > NUM_VLC_SETS + 1 ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Invalid quantisers %d / %d\n",
               c->quant[0], c->quant[1]);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (size > bytestream2_get_bytes_left(&gb)) {
        av_em_log(avctx, AV_LOG_ERROR, "Slice properties chunk is too large\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    for (i = 0; i < size; i++) {
        ...
        val >>= 6;
        if (pos + len > num_mb) {
            av_em_log(avctx, AV_LOG_ERROR, "Too many slice properties\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    if (pos < num_mb) {
        av_em_log(avctx, AV_LOG_ERROR, "Too few slice properties (%d / %d)\n",
               pos, num_mb);
        return AVERROR_INVALIDDATA;
    }
    ...
    for (i = 0; i < c->mb_height; i++) {
        ...
        if (!size) {
            ...
            if (!skip_row) {
                av_em_log(avctx, AV_LOG_ERROR, "Non-skip row with zero size\n");
                return AVERROR_INVALIDDATA;
            }
        }
        if (bytestream2_get_bytes_left(&gb) < size) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid slice size (%"PRIu32"/%u)\n",
                   size, bytestream2_get_bytes_left(&gb));
            return AVERROR_INVALIDDATA;
        }
        ret = tscc2_decode_slice(c, i, buf + bytestream2_tell(&gb), size);
        if (ret) {
            av_em_log(avctx, AV_LOG_ERROR, "Error decoding slice %d\n", i);
            return ret;
        }
...
static av_cold int tscc2_decode_end(AVEMCodecContext *avctx)
{
    TSCC2Context * const c = avctx->priv_data;

    av_em_frame_free(&c->pic);
    av_em_freep(&c->slice_quants);
    free_vlcs(c);

    return 0;
}

static av_cold int tscc2_decode_init(AVEMCodecContext *avctx)
{
    TSCC2Context * const c = avctx->priv_data;
    int ret;
    ...
    c->slice_quants = av_em_alloc(c->mb_width * c->mb_height);
    if (!c->slice_quants) {
        av_em_log(avctx, AV_LOG_ERROR, "Cannot allocate slice information\n");
        free_vlcs(c);
