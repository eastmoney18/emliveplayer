        ...
        if ((history < 128) && (i + 1 < nb_samples)) {
            ...
            if (block_size > 0) {
                if (block_size >= nb_samples - i) {
                    av_em_log(alac->avctx, AV_LOG_ERROR,
...
static int decode_element(AVEMCodecContext *avctx, AVFrame *frame, int ch_index,
                          int channels)
{
    ...
    if (bps > 32U) {
        av_em_log(avctx, AV_LOG_ERROR, "bps is unsupported: %d\n", bps);
    ...
    if (!output_samples || output_samples > alac->max_samples_per_frame) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid samples per frame: %"PRIu32"\n",
               output_samples);
    ...
    if (!alac->nb_samples) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "sample count mismatch: %"PRIu32" != %d\n",
    ...
    if (is_compressed) {
        ...
        if (!alac->rice_limit) {
            avpriv_em_request_sample(alac->avctx,
        ...
        for (ch = 0; ch < channels; ch++) {
            ...
            if (prediction_type[ch] == 15) {
                ...
                av_em_log(avctx, AV_LOG_WARNING, "unknown prediction type: %i\n",
                       prediction_type[ch]);
            }
            lpc_prediction(alac->predict_error_buffer[ch],
...
static int alac_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    while (get_bits_left(&alac->gb) >= 3) {
        ...
        if (element > TYPE_CPE && element != TYPE_LFE) {
            av_em_log(avctx, AV_LOG_ERROR, "syntax element unsupported: %d\n", element);
            return AVERROR_PATCHWELCOME;
        }
        ...
        if (ch + channels > alac->channels ||
            ...
            av_em_log(avctx, AV_LOG_ERROR, "invalid element channel count\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    if (!got_end) {
        av_em_log(avctx, AV_LOG_ERROR, "no end tag found. incomplete packet.\n");
        return AVERROR_INVALIDDATA;
    }

    if (avpkt->size * 8 - get_bits_count(&alac->gb) > 8) {
        av_em_log(avctx, AV_LOG_ERROR, "Error : %d bits left\n",
    ...
    if (alac->channels == ch)
        *got_frame_ptr = 1;
        ...
        av_em_log(avctx, AV_LOG_WARNING, "Failed to decode all channels\n");

    return avpkt->size;
}

static av_cold int alac_decode_close(AVEMCodecContext *avctx)
{
    ALACContext *alac = avctx->priv_data;

    int ch;
    for (ch = 0; ch < FFMIN(alac->channels, 2); ch++) {
        av_em_freep(&alac->predict_error_buffer[ch]);
        if (!alac->direct_output)
            av_em_freep(&alac->output_samples_buffer[ch]);
        av_em_freep(&alac->extra_bits_buffer[ch]);
    }
...
static av_cold int alac_decode_init(AVEMCodecContext * avctx)
{
    int ret;
    ALACContext *alac = avctx->priv_data;
    alac->avctx = avctx;
    ...
    if (alac_set_info(alac)) {
        av_em_log(avctx, AV_LOG_ERROR, "set_info failed\n");
        return -1;
    }
    ...
    case 32: avctx->sample_fmt = AV_SAMPLE_FMT_S32P;
             break;
    default: avpriv_em_request_sample(avctx, "Sample depth %d", alac->sample_size);
             return AVERROR_PATCHWELCOME;
    }
    avctx->bits_per_raw_sample = alac->sample_size;

    if (alac->channels < 1) {
        av_em_log(avctx, AV_LOG_WARNING, "Invalid channel count\n");
    ...
    if (avctx->channels > ALAC_MAX_CHANNELS || avctx->channels <= 0 ) {
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported channel count: %d\n",
               avctx->channels);
        return AVERROR_PATCHWELCOME;
    }
...
static const AVEMClass alac_class = {
    .class_name = "alac",
