...
static const AVEMClass vorbis_parser_class = {
                           ...
                           const uint8_t *buf, int buf_size)
{
    ...
    if (buf_size < 30) {
        av_em_log(s, AV_LOG_ERROR, "Id header is too short\n");
        return AVERROR_INVALIDDATA;
    }

    /* make sure this is the Id header */
    if (buf[0] != 1) {
        av_em_log(s, AV_LOG_ERROR, "Wrong packet type in Id header\n");
        return AVERROR_INVALIDDATA;
    }

    /* check for header signature */
    if (memcmp(&buf[1], "vorbis", 6)) {
        av_em_log(s, AV_LOG_ERROR, "Invalid packet signature in Id header\n");
        return AVERROR_INVALIDDATA;
    }

    if (!(buf[29] & 0x1)) {
        av_em_log(s, AV_LOG_ERROR, "Invalid framing bit in Id header\n");
        return AVERROR_INVALIDDATA;
    }
                              ...
                              const uint8_t *buf, int buf_size)
{
    ...
    /* avoid overread */
    if (buf_size < 7) {
        av_em_log(s, AV_LOG_ERROR, "Setup header is too short\n");
        return AVERROR_INVALIDDATA;
    }

    /* make sure this is the Setup header */
    if (buf[0] != 5) {
        av_em_log(s, AV_LOG_ERROR, "Wrong packet type in Setup header\n");
        return AVERROR_INVALIDDATA;
    }

    /* check for header signature */
    if (memcmp(&buf[1], "vorbis", 6)) {
        av_em_log(s, AV_LOG_ERROR, "Invalid packet signature in Setup header\n");
        return AVERROR_INVALIDDATA;
    }

    /* reverse bytes so we can easily read backwards with get_bits() */
    if (!(rev_buf = av_em_alloc(buf_size))) {
    ...
    if (!got_framing_bit) {
        av_em_log(s, AV_LOG_ERROR, "Invalid Setup header\n");
    ...
    if (last_mode_count > 63) {
        av_em_log(s, AV_LOG_ERROR, "Unsupported mode count: %d\n",
               last_mode_count);
        ret = AVERROR_INVALIDDATA;
        goto bad_header;
    }
...
int av_em_vorbis_parse_frame_flags(AVVorbisParseContext *s, const uint8_t *buf,
...
bad_packet:
            av_em_log(s, AV_LOG_ERROR, "Invalid packet\n");
            return AVERROR_INVALIDDATA;
        }
        if (s->mode_count == 1)
            mode = 0;
        ...
        if (mode >= s->mode_count) {
            av_em_log(s, AV_LOG_ERROR, "Invalid mode in packet\n");
            return AVERROR_INVALIDDATA;
        }
...
int av_em_vorbis_parse_frame(AVVorbisParseContext *s, const uint8_t *buf,
                          int buf_size)
{
    return av_em_vorbis_parse_frame_flags(s, buf, buf_size, NULL);
}

void av_em_vorbis_parse_reset(AVVorbisParseContext *s)
{
    if (s->valid_extradata)
        s->previous_blocksize = s->blocksize[0];
}

void av_em_vorbis_parse_free(AVVorbisParseContext **s)
{
    av_em_freep(s);
}

AVVorbisParseContext *av_em_vorbis_parse_init(const uint8_t *extradata,
                                           int extradata_size)
{
    AVVorbisParseContext *s = av_em_mallocz(sizeof(*s));
    int ret;
...
static int vorbis_parse(AVEMCodecParserContext *s1, AVEMCodecContext *avctx,
                        ...
                        const uint8_t *buf, int buf_size)
{
    ...
    if ((duration = av_em_vorbis_parse_frame(s->vp, buf, buf_size)) >= 0)
...
static void vorbis_parser_close(AVEMCodecParserContext *ctx)
{
    VorbisParseContext *s = ctx->priv_data;
    av_em_vorbis_parse_free(&s->vp);
}

AVEMCodecParser ff_vorbis_parser = {
