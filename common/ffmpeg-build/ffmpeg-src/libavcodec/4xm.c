                          ...
                          int log2w, int log2h, int stride)
{
    ...
    if (code == 1) {
        ...
        if (bytestream2_get_bytes_left(&f->g2) < 4) {
            av_em_log(f->avctx, AV_LOG_ERROR, "wordstream overread\n");
    ...
    if ((code&3)==0 && bytestream2_get_bytes_left(&f->g) < 1) {
        av_em_log(f->avctx, AV_LOG_ERROR, "bytestream overread\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (start > src || src > end) {
        av_em_log(f->avctx, AV_LOG_ERROR, "mv out of pic\n");
        return AVERROR_INVALIDDATA;
    }
...
static int decode_p_frame(FourXContext *f, const uint8_t *buf, int length)
{
    ...
    if (bitstream_size > length || bitstream_size >= INT_MAX/8 ||
        ...
        av_em_log(f->avctx, AV_LOG_ERROR, "lengths %d %d %d %d\n", bitstream_size, bytestream_size, wordstream_size,
        bitstream_size+ bytestream_size+ wordstream_size - length);
        return AVERROR_INVALIDDATA;
    }

    av_em_fast_padded_malloc(&f->bitstream_buffer, &f->bitstream_buffer_size,
                          bitstream_size);
...
static int decode_i_block(FourXContext *f, int16_t *block)
{
    ...
    if (get_bits_left(&f->gb) < 2){
        av_em_log(f->avctx, AV_LOG_ERROR, "%d bits left before decode_i_block()\n", get_bits_left(&f->gb));
        return -1;
    }
    ...
    if (val >> 4) {
        av_em_log(f->avctx, AV_LOG_ERROR, "error dc run != 0\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    for (;;) {
        ...
        if (code == 0xf0) {
            ...
            if (code & 0xf) {
                ...
                av_em_log(f->avctx, AV_LOG_ERROR, "0 coeff\n");
                return AVERROR_INVALIDDATA;
            }
            i    += code >> 4;
            if (i >= 64) {
                av_em_log(f->avctx, AV_LOG_ERROR, "run %d oveflow\n", i);
                return 0;
            }
                                          ...
                                          int buf_size)
{
    ...
    for (;;) {
        ...
        if (ptr_end - ptr < FFMAX(end - start + 1, 0) + 1) {
            av_em_log(f->avctx, AV_LOG_ERROR, "invalid data in read_huffman_tables\n");
            return NULL;
        }
    ...
    if (ptr > ptr_end) {
        av_em_log(f->avctx, AV_LOG_ERROR, "ptr overflow in read_huffman_tables\n");
        return NULL;
    }
...
static int decode_i2_frame(FourXContext *f, const uint8_t *buf, int length)
{
    ...
    if (length < mbs * 8) {
        av_em_log(f->avctx, AV_LOG_ERROR, "packet size too small\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    for (y = 0; y < height; y += 16) {
        for (x = 0; x < width; x += 16) {
            ...
            if (color[0] & 0x8000)
                av_em_log(f->avctx, AV_LOG_ERROR, "unk bit 1\n");
            if (color[1] & 0x8000)
                av_em_log(f->avctx, AV_LOG_ERROR, "unk bit 2\n");
...
static int decode_i_frame(FourXContext *f, const uint8_t *buf, int length)
{
    ...
    if (prestream_size + bitstream_size + 12 != length
        || prestream_size > (1 << 26)) {
        av_em_log(f->avctx, AV_LOG_ERROR, "size mismatch %d %d %d\n",
    ...
    if (!prestream) {
        av_em_log(f->avctx, AV_LOG_ERROR, "Error reading Huffman tables.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (get_vlc2(&f->pre_gb, f->pre_vlc.table, ACDC_VLC_BITS, 3) != 256)
        av_em_log(f->avctx, AV_LOG_ERROR, "end mismatch\n");
                        ...
                        int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (buf_size < AV_RL32(buf + 4) + 8) {
        av_em_log(f->avctx, AV_LOG_ERROR, "size mismatch %d %"PRIu32"\n",
    ...
    if (frame_4cc == AV_RL32("cfrm")) {
        ...
        if (f->version <= 1) {
            av_em_log(f->avctx, AV_LOG_ERROR, "cfrm in version %d\n", f->version);
            return AVERROR_INVALIDDATA;
        }
        ...
        if (data_size < 0 || whole_size < 0) {
            av_em_log(f->avctx, AV_LOG_ERROR, "sizes invalid\n");
            return AVERROR_INVALIDDATA;
        }

        for (i = 0; i < CFRAME_BUFFER_COUNT; i++)
            if (f->cfrm[i].id && f->cfrm[i].id < avctx->frame_number)
                av_em_log(f->avctx, AV_LOG_ERROR, "lost c frame %d\n",
                       f->cfrm[i].id);
        ...
        cfrm->data = av_em_fast_realloc(cfrm->data, &cfrm->allocated_size,
                                     cfrm->size + data_size + AV_INPUT_BUFFER_PADDING_SIZE);
        // explicit check needed as memcpy below might not catch a NULL
        if (!cfrm->data) {
            av_em_log(f->avctx, AV_LOG_ERROR, "realloc failure\n");
            return AVERROR(ENOMEM);
        }
        ...
        if (cfrm->size >= whole_size) {
            ...
            if (id != avctx->frame_number)
                av_em_log(f->avctx, AV_LOG_ERROR, "cframe id mismatch %d %d\n",
    ...
    if ((ret = em_get_buffer(avctx, picture, 0)) < 0)
        return ret;

    if (frame_4cc == AV_RL32("ifr2")) {
        picture->pict_type = AV_PICTURE_TYPE_I;
        if ((ret = decode_i2_frame(f, buf - 4, frame_size + 4)) < 0) {
            av_em_log(f->avctx, AV_LOG_ERROR, "decode i2 frame failed\n");
            return ret;
        }
    } else if (frame_4cc == AV_RL32("ifrm")) {
        picture->pict_type = AV_PICTURE_TYPE_I;
        if ((ret = decode_i_frame(f, buf, frame_size)) < 0) {
            av_em_log(f->avctx, AV_LOG_ERROR, "decode i frame failed\n");
            return ret;
        }
    } else if (frame_4cc == AV_RL32("pfrm") || frame_4cc == AV_RL32("pfr2")) {
        picture->pict_type = AV_PICTURE_TYPE_P;
        if ((ret = decode_p_frame(f, buf, frame_size)) < 0) {
            av_em_log(f->avctx, AV_LOG_ERROR, "decode p frame failed\n");
            return ret;
        }
    } else if (frame_4cc == AV_RL32("snd_")) {
        av_em_log(avctx, AV_LOG_ERROR, "ignoring snd_ chunk length:%d\n",
               buf_size);
    } else {
        av_em_log(avctx, AV_LOG_ERROR, "ignoring unknown chunk length:%d\n",
               buf_size);
    }

    picture->key_frame = picture->pict_type == AV_PICTURE_TYPE_I;

    av_em_image_copy_plane(picture->data[0], picture->linesize[0],
...
static av_cold int decode_end(AVEMCodecContext *avctx)
{
    ...
    av_em_freep(&f->frame_buffer);
    av_em_freep(&f->last_frame_buffer);
    av_em_freep(&f->bitstream_buffer);
    f->bitstream_buffer_size = 0;
    for (i = 0; i < CFRAME_BUFFER_COUNT; i++) {
        av_em_freep(&f->cfrm[i].data);
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->extradata_size != 4 || !avctx->extradata) {
        av_em_log(avctx, AV_LOG_ERROR, "extradata wrong or missing\n");
        return AVERROR_INVALIDDATA;
    }
    if((avctx->width % 16) || (avctx->height % 16)) {
        av_em_log(avctx, AV_LOG_ERROR, "unsupported width/height\n");
        return AVERROR_INVALIDDATA;
    }

    ret = av_em_image_check_size(avctx->width, avctx->height, 0, avctx);
    if (ret < 0)
        return ret;

    f->frame_buffer      = av_em_mallocz(avctx->width * avctx->height * 2);
    f->last_frame_buffer = av_em_mallocz(avctx->width * avctx->height * 2);
    ...
    em_blockdsp_init(&f->bdsp, avctx);
    em_bswapdsp_init(&f->bbdsp);
    f->avctx = avctx;
    init_vlcs(f);
