                                ...
                                const char *libname2)
{
    if (libname2) {
        ...
        if (!s->lib2) {
            av_em_log(logctx, AV_LOG_WARNING, "%s not found\n", libname);
            return AVERROR_ENCODER_NOT_FOUND;
        }
        s->host_init = dlsym(s->lib2, "bcm_host_init");
        if (!s->host_init) {
            av_em_log(logctx, AV_LOG_WARNING, "bcm_host_init not found\n");
            dlclose(s->lib2);
            s->lib2 = NULL;
            return AVERROR_ENCODER_NOT_FOUND;
        }
    ...
    if (!s->ptr_Init || !s->ptr_Deinit || !s->ptr_ComponentNameEnum ||
        ...
        av_em_log(logctx, AV_LOG_WARNING, "Not all functions found in %s\n", libname);
        dlclose(s->lib);
        s->lib = NULL;
        if (s->lib2)
            dlclose(s->lib2);
...
static av_cold OMXContext *omx_init(void *logctx, const char *libname, const char *prefix)
{
    ...
    omx_context = av_em_mallocz(sizeof(*omx_context));
    if (!omx_context)
        return NULL;
    if (libname) {
        ret = omx_try_load(omx_context, logctx, libname, prefix, NULL);
        if (ret < 0) {
            av_em_free(omx_context);
            return NULL;
        }
    } else {
                                   ...
                                   OMX_U32 data1, OMX_U32 data2, OMX_PTR event_data)
{
    ...
    case OMX_EventError:
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "OMX error %"PRIx32"\n", (uint32_t) data1);
        s->error = data1;
    ...
    case OMX_EventCmdComplete:
        if (data1 == OMX_CommandStateSet) {
            pthread_mutex_lock(&s->state_mutex);
            s->state = data2;
            av_em_log(s->avctx, AV_LOG_VERBOSE, "OMX state changed to %"PRIu32"\n", (uint32_t) data2);
            pthread_cond_broadcast(&s->state_cond);
            pthread_mutex_unlock(&s->state_mutex);
        } else if (data1 == OMX_CommandPortDisable) {
            av_em_log(s->avctx, AV_LOG_VERBOSE, "OMX port %"PRIu32" disabled\n", (uint32_t) data2);
        } else if (data1 == OMX_CommandPortEnable) {
            av_em_log(s->avctx, AV_LOG_VERBOSE, "OMX port %"PRIu32" enabled\n", (uint32_t) data2);
        } else {
            av_em_log(s->avctx, AV_LOG_VERBOSE, "OMX command complete, command %"PRIu32", value %"PRIu32"\n",
                                             (uint32_t) data1, (uint32_t) data2);
        }
        break;
    case OMX_EventPortSettingsChanged:
        av_em_log(s->avctx, AV_LOG_VERBOSE, "OMX port %"PRIu32" settings changed\n", (uint32_t) data1);
        break;
    default:
        av_em_log(s->avctx, AV_LOG_VERBOSE, "OMX event %d %"PRIx32" %"PRIx32"\n",
    ...
    if (s->input_zerocopy) {
        if (buffer->pAppPrivate) {
            if (buffer->pOutputPortPrivate)
                av_em_free(buffer->pAppPrivate);
            else
                av_em_frame_free((AVFrame**)&buffer->pAppPrivate);
                                  ...
                                  const char *role, char *str, int str_size)
{
...
#if CONFIG_OMX_RPI
    if (av_em_strstart(role, "video_encoder.", NULL)) {
        av_em_strlcpy(str, "OMX.broadcom.video_encode", str_size);
        return 0;
    }
#endif
    omx_context->ptr_GetComponentsOfRole((OMX_STRING) role, &num, NULL);
    if (!num) {
        av_em_log(logctx, AV_LOG_WARNING, "No component for role %s found\n", role);
        return AVERROR_ENCODER_NOT_FOUND;
    }
    components = av_em_mallocz(sizeof(char*) * num);
    if (!components)
        return AVERROR(ENOMEM);
    for (i = 0; i < num; i++) {
        components[i] = av_em_mallocz(OMX_MAX_STRINGNAME_SIZE);
        if (!components) {
            ret = AVERROR(ENOMEM);
            goto end;
        }
    ...
    av_em_strlcpy(str, components[0], str_size);
end:
    for (i = 0; i < num; i++)
        av_em_free(components[i]);
    av_em_free(components);
    return ret;
}
...
static av_cold int omx_component_init(AVEMCodecContext *avctx, const char *role)
{
    ...
    if (err != OMX_ErrorNone) {
        av_em_log(avctx, AV_LOG_ERROR, "OMX_GetHandle(%s) failed: %x\n", s->component_name, err);
        return AVERROR_UNKNOWN;
    }

    // This one crashes the mediaserver on qcom, if used over IOMX
    INIT_STRUCT(role_params);
    av_em_strlcpy(role_params.cRole, role, sizeof(role_params.cRole));
    ...
    for (i = 0; i < video_port_params.nPorts; i++) {
        ...
        if (err != OMX_ErrorNone) {
            av_em_log(avctx, AV_LOG_WARNING, "port %d error %x\n", port, err);
            break;
        }
    ...
    if (s->in_port < 0 || s->out_port < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "No in or out port found (in %d out %d)\n", s->in_port, s->out_port);
        return AVERROR_UNKNOWN;
    }
    ...
    if (s->color_format == 0) {
        av_em_log(avctx, AV_LOG_ERROR, "No supported pixel formats (%d formats available)\n", i);
        return AVERROR_UNKNOWN;
    }
    ...
    if (err != OMX_ErrorNone)
        av_em_log(avctx, AV_LOG_WARNING, "Unable to set video bitrate parameter\n");
    ...
    s->in_buffer_headers  = av_em_mallocz(sizeof(OMX_BUFFERHEADERTYPE*) * s->num_in_buffers);
    s->free_in_buffers    = av_em_mallocz(sizeof(OMX_BUFFERHEADERTYPE*) * s->num_in_buffers);
    s->out_buffer_headers = av_em_mallocz(sizeof(OMX_BUFFERHEADERTYPE*) * s->num_out_buffers);
    s->done_out_buffers   = av_em_mallocz(sizeof(OMX_BUFFERHEADERTYPE*) * s->num_out_buffers);
    ...
    if (wait_for_state(s, OMX_StateIdle) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Didn't get OMX_StateIdle\n");
        return AVERROR_UNKNOWN;
    }
    err = OMX_SendCommand(s->handle, OMX_CommandStateSet, OMX_StateExecuting, NULL);
    CHECK(err);
    if (wait_for_state(s, OMX_StateExecuting) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Didn't get OMX_StateExecuting\n");
        return AVERROR_UNKNOWN;
    }
...
static av_cold void cleanup(OMXCodecContext *s)
{
    ...
    av_em_freep(&s->in_buffer_headers);
    av_em_freep(&s->out_buffer_headers);
    av_em_freep(&s->free_in_buffers);
    av_em_freep(&s->done_out_buffers);
    av_em_freep(&s->output_buf);
}

static av_cold int omx_encode_init(AVEMCodecContext *avctx)
{
    OMXCodecContext *s = avctx->priv_data;
    int ret = AVERROR_ENCODER_NOT_FOUND;
    const char *role;
    ...
    av_em_log(avctx, AV_LOG_INFO, "Using %s\n", s->component_name);
    ...
    if (avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER) {
        while (1) {
            ...
            if (buffer->nFlags & OMX_BUFFERFLAG_CODECCONFIG) {
                if ((ret = av_em_reallocp(&avctx->extradata, avctx->extradata_size + buffer->nFilledLen + AV_INPUT_BUFFER_PADDING_SIZE)) < 0) {
            ...
            if (err != OMX_ErrorNone) {
                append_buffer(&s->output_mutex, &s->output_cond,
                              &s->num_done_out_buffers, s->done_out_buffers, buffer);
                av_em_log(avctx, AV_LOG_ERROR, "OMX_FillThisBuffer failed: %x\n", err);
                ret = AVERROR_UNKNOWN;
                goto fail;
            }
            if (avctx->codec->id == AV_CODEC_ID_H264) {
...
static int omx_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                            const AVFrame *frame, int *got_packet)
{
    ...
    if (frame) {
        ...
        buffer->nFilledLen = av_em_image_fill_arrays(dst, linesize, buffer->pBuffer, avctx->pix_fmt, s->stride, s->plane_size, 1);

        if (s->input_zerocopy) {
            uint8_t *src[4] = { NULL };
            int src_linesize[4];
            av_em_image_fill_arrays(src, src_linesize, frame->data[0], avctx->pix_fmt, s->stride, s->plane_size, 1);
            if (frame->linesize[0] == src_linesize[0] &&
                ...
                AVFrame *local = av_em_frame_clone(frame);
                if (!local) {
                ...
                uint8_t *buf = av_em_alloc(av_em_image_get_buffer_size(avctx->pix_fmt, s->stride, s->plane_size, 1));
                if (!buf) {
                    // Return the buffer to the queue so it's not lost
                    append_buffer(&s->input_mutex, &s->input_cond, &s->num_free_in_buffers, s->free_in_buffers, buffer);
                    return AVERROR(ENOMEM);
        ...
        if (need_copy)
            av_em_image_copy(dst, linesize, (const uint8_t**) frame->data, frame->linesize, avctx->pix_fmt, avctx->width, avctx->height);
        buffer->nFlags = OMX_BUFFERFLAG_ENDOFFRAME;
        buffer->nOffset = 0;
        // Convert the timestamps to microseconds; some encoders can ignore
        // the framerate and do VFR bit allocation based on timestamps.
        buffer->nTimeStamp = to_omx_ticks(av_em_rescale_q(frame->pts, avctx->time_base, AV_TIME_BASE_Q));
        err = OMX_EmptyThisBuffer(s->handle, buffer);
        if (err != OMX_ErrorNone) {
            append_buffer(&s->input_mutex, &s->input_cond, &s->num_free_in_buffers, s->free_in_buffers, buffer);
            av_em_log(avctx, AV_LOG_ERROR, "OMX_EmptyThisBuffer failed: %x\n", err);
            return AVERROR_UNKNOWN;
        }
        s->num_in_frames++;
    }
    ...
    while (!*got_packet && ret == 0) {
        ...
        if (buffer->nFlags & OMX_BUFFERFLAG_CODECCONFIG && avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER) {
            ...
            if (!(buffer->nFlags & OMX_BUFFERFLAG_ENDOFFRAME) || !pkt->data) {
                ...
                if ((ret = av_em_reallocp(&s->output_buf, newsize)) < 0) {
                    s->output_buf_size = 0;
                    goto end;
                }
                ...
                if (buffer->nFlags & OMX_BUFFERFLAG_ENDOFFRAME) {
                    if ((ret = av_em_packet_from_data(pkt, s->output_buf, s->output_buf_size)) < 0) {
                        av_em_freep(&s->output_buf);
                        s->output_buf_size = 0;
                        goto end;
                    }
                ...
                if ((ret = em_alloc_packet2(avctx, pkt, s->output_buf_size + buffer->nFilledLen, 0)) < 0) {
                    av_em_log(avctx, AV_LOG_ERROR, "Error getting output packet of size %d.\n",
            ...
            if (buffer->nFlags & OMX_BUFFERFLAG_ENDOFFRAME) {
                pkt->pts = av_em_rescale_q(from_omx_ticks(buffer->nTimeStamp), AV_TIME_BASE_Q, avctx->time_base);
...
static av_cold int omx_encode_end(AVEMCodecContext *avctx)
{
    OMXCodecContext *s = avctx->priv_data;
...
static const AVEMClass omx_mpeg4enc_class = {
    .class_name = "mpeg4_omx",
    .item_name  = av_em_default_item_name,
    .option     = options,
    .version    = LIBAVUTIL_VERSION_INT,
};
AVEMCodec em_mpeg4_omx_encoder = {
    .name             = "mpeg4_omx",
...
static const AVEMClass omx_h264enc_class = {
    .class_name = "h264_omx",
    .item_name  = av_em_default_item_name,
    .option     = options,
    .version    = LIBAVUTIL_VERSION_INT,
};
AVEMCodec em_h264_omx_encoder = {
    .name             = "h264_omx",
