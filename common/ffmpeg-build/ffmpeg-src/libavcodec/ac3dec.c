...
/**
 * AVEMCodec initialization
 */
static av_cold int ac3_decode_init(AVEMCodecContext *avctx)
{
...
#if (USE_FIXED)
    s->fdsp = avpriv_em_alloc_fixed_dsp(avctx->flags & AV_CODEC_FLAG_BITEXACT);
#else
    s->fdsp = avpriv_em_float_dsp_alloc(avctx->flags & AV_CODEC_FLAG_BITEXACT);
    ff_fmt_convert_init(&s->fmt_conv, avctx);
#endif

    ff_ac3dsp_init(&s->ac3dsp, avctx->flags & AV_CODEC_FLAG_BITEXACT);
    av_em_lfg_init(&s->dith_state, 0);
...
static int parse_frame_header(AC3DecodeContext *s)
{
    ...
    if (s->bitstream_id <= 10) {
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "E-AC-3 support not compiled in\n");
                            ...
                            uint8_t absexp, int8_t *dexps)
{
    ...
    for (i = 0, j = 0; i < ngrps * 3; i++) {
        prevexp += dexp[i] - 2;
        if (prevexp > 24U) {
            av_em_log(s->avctx, AV_LOG_ERROR, "exponent %d is out-of-range\n", prevexp);
            return -1;
        }
        switch (group_size) {
...
static void ac3_decode_transform_coeffs_ch(AC3DecodeContext *s, int ch_index, mant_groups *m)
{
    ...
    for (freq = start_freq; freq < end_freq; freq++) {
        ...
        default: /* 6 to 15 */
            ...
            if (bap > 15) {
                av_em_log(s->avctx, AV_LOG_ERROR, "bap %d is invalid in plain AC-3\n", bap);
                bap = 15;
            }
...
static int decode_audio_block(AC3DecodeContext *s, int blk)
{
    ...
    if (s->eac3 && (!blk || get_bits1(gbc))) {
        ...
        if (s->spx_in_use) {
            ...
            if (start_subband >= end_subband) {
                av_em_log(s->avctx, AV_LOG_ERROR, "invalid spectral extension "
                       "range (%d >= %d)\n", start_subband, end_subband);
    ...
    if (s->eac3 ? s->cpl_strategy_exists[blk] : get_bits1(gbc)) {
        ...
        if (s->cpl_in_use[blk]) {
            ...
            if (channel_mode < AC3_CHMODE_STEREO) {
                av_em_log(s->avctx, AV_LOG_ERROR, "coupling not allowed in mono or dual-mono\n");
                return AVERROR_INVALIDDATA;
            }
            ...
            if (s->eac3 && get_bits1(gbc)) {
                ...
                avpriv_em_request_sample(s->avctx, "Enhanced coupling");
            ...
            if (cpl_start_subband >= cpl_end_subband) {
                av_em_log(s->avctx, AV_LOG_ERROR, "invalid coupling range (%d >= %d)\n",
        ...
        if (!blk) {
            av_em_log(s->avctx, AV_LOG_ERROR, "new coupling strategy must "
        ...
        for (ch = 1; ch <= fbw_channels; ch++) {
            if (s->channel_in_cpl[ch]) {
                if ((s->eac3 && s->first_cpl_coords[ch]) || get_bits1(gbc)) {
                    ...
                    av_em_log(s->avctx, AV_LOG_ERROR, "new coupling coordinates must "
                           "be present in block 0\n");
                    return AVERROR_INVALIDDATA;
                }
            } else {
    ...
    if (channel_mode == AC3_CHMODE_STEREO) {
        if ((s->eac3 && !blk) || get_bits1(gbc)) {
            ...
            av_em_log(s->avctx, AV_LOG_WARNING, "Warning: "
        ...
        if (s->exp_strategy[blk][ch] != EXP_REUSE) {
            ...
            if (s->channel_in_cpl[ch])
                ...
                if (bandwidth_code > 60) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "bandwidth code = %d > 60\n", bandwidth_code);
                    return AVERROR_INVALIDDATA;
                }
    ...
    if (s->bit_allocation_syntax) {
        if (get_bits1(gbc)) {
            ...
            av_em_log(s->avctx, AV_LOG_ERROR, "new bit allocation info must "
                   "be present in block 0\n");
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    if (!s->eac3 || !blk) {
        if (s->snr_offset_strategy && get_bits1(gbc)) {
            ...
            av_em_log(s->avctx, AV_LOG_ERROR, "new snr offsets must be present in block 0\n");
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    if (cpl_in_use) {
        if (s->first_cpl_leak || get_bits1(gbc)) {
            ...
            av_em_log(s->avctx, AV_LOG_ERROR, "new coupling leak info must "
                   "be present in block 0\n");
            return AVERROR_INVALIDDATA;
        }
        s->first_cpl_leak = 0;
    ...
    if (s->dba_syntax && get_bits1(gbc)) {
        ...
        for (ch = !cpl_in_use; ch <= fbw_channels; ch++) {
            ...
            if (s->dba_mode[ch] == DBA_RESERVED) {
                av_em_log(s->avctx, AV_LOG_ERROR, "delta bit allocation strategy reserved\n");
                return AVERROR_INVALIDDATA;
            }
            bit_alloc_stages[ch] = FFMAX(bit_alloc_stages[ch], 2);
        }
    ...
    for (ch = !cpl_in_use; ch <= s->channels; ch++) {
        ...
        if (bit_alloc_stages[ch] > 1) {
            ...
            if (ff_ac3_bit_alloc_calc_mask(&s->bit_alloc_params, s->band_psd[ch],
                ...
                av_em_log(s->avctx, AV_LOG_ERROR, "error in bit allocation\n");
                return AVERROR_INVALIDDATA;
            }
        }
 ...
 */
static int ac3_decode_frame(AVEMCodecContext * avctx, void *data,
                            int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (err) {
        ...
        case AAC_AC3_PARSE_ERROR_SYNC:
            av_em_log(avctx, AV_LOG_ERROR, "frame sync error\n");
            return AVERROR_INVALIDDATA;
        case AAC_AC3_PARSE_ERROR_BSID:
            av_em_log(avctx, AV_LOG_ERROR, "invalid bitstream id\n");
            break;
        case AAC_AC3_PARSE_ERROR_SAMPLE_RATE:
            av_em_log(avctx, AV_LOG_ERROR, "invalid sample rate\n");
            break;
        case AAC_AC3_PARSE_ERROR_FRAME_SIZE:
            av_em_log(avctx, AV_LOG_ERROR, "invalid frame size\n");
            break;
        case AAC_AC3_PARSE_ERROR_FRAME_TYPE:
            ...
            if (s->frame_type == EAC3_FRAME_TYPE_DEPENDENT || s->substreamid) {
                ...
                *got_frame_ptr = 0;
                ...
                av_em_log(avctx, AV_LOG_ERROR, "invalid frame type\n");
            }
            break;
        ...
        if (s->frame_size > buf_size) {
            av_em_log(avctx, AV_LOG_ERROR, "incomplete frame\n");
            err = AAC_AC3_PARSE_ERROR_FRAME_SIZE;
        } else if (avctx->err_recognition & (AV_EF_CRCCHECK|AV_EF_CAREFUL)) {
            /* check for crc mismatch */
            if (av_em_crc(av_em_crc_get_table(AV_CRC_16_ANSI), 0, &buf[2],
                       s->frame_size - 2)) {
                av_em_log(avctx, AV_LOG_ERROR, "frame CRC mismatch\n");
    ...
    if (!err || (s->channels && s->out_channels != s->channels)) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "unable to determine channel mode\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    for (blk = 0; blk < s->num_blocks; blk++) {
        if (!err && decode_audio_block(s, blk)) {
            av_em_log(avctx, AV_LOG_ERROR, "error decoding the audio block\n");
            err = 1;
        }
        if (err)
    ...
    av_em_frame_set_decode_error_flags(frame, err ? FF_DECODE_ERROR_INVALID_BITSTREAM : 0);
    ...
    /* AVDownmixInfo */
    if ((downmix_info = av_em_downmix_info_update_side_data(frame))) {
 ...
 */
static av_cold int ac3_decode_end(AVEMCodecContext *avctx)
{
    AC3DecodeContext *s = avctx->priv_data;
