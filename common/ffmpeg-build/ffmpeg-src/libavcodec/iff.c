 ...
 */
static int cmap_read_palette(AVEMCodecContext *avctx, uint32_t *pal)
{
    ...
    if (avctx->bits_per_coded_sample > 8) {
        av_em_log(avctx, AV_LOG_ERROR, "bits_per_coded_sample > 8 not supported\n");
 ...
 * decoder structures.
 *
 * @param avctx the AVEMCodecContext where to extract extra context to
 * @param avpkt the AVEMPacket to extract extra context from or NULL to use avctx
 * @return >= 0 in case of success, a negative error code otherwise
 */
static int extract_header(AVEMCodecContext *const avctx,
                          const AVEMPacket *const avpkt)
{
    IffContext *s = avctx->priv_data;
    const uint8_t *buf;
    unsigned buf_size = 0;
    ...
    if (avctx->extradata_size < 2) {
        av_em_log(avctx, AV_LOG_ERROR, "not enough extradata\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (buf_size >= 41) {
        ...
        if (s->masking == MASK_HAS_MASK) {
            if (s->bpp >= 8 && !s->ham) {
                ...
                av_em_freep(&s->mask_buf);
                av_em_freep(&s->mask_palbuf);
                s->mask_buf = av_em_alloc((s->planesize * 32) + AV_INPUT_BUFFER_PADDING_SIZE);
                if (!s->mask_buf)
                    return AVERROR(ENOMEM);
                if (s->bpp > 16) {
                    av_em_log(avctx, AV_LOG_ERROR, "bpp %d too large for palette\n", s->bpp);
                    av_em_freep(&s->mask_buf);
                    return AVERROR(ENOMEM);
                }
                s->mask_palbuf = av_em_alloc((2 << s->bpp) * sizeof(uint32_t) + AV_INPUT_BUFFER_PADDING_SIZE);
                if (!s->mask_palbuf) {
                    av_em_freep(&s->mask_buf);
                    return AVERROR(ENOMEM);
                }
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Masking not supported\n");
            return AVERROR_PATCHWELCOME;
        }
        if (!s->bpp || s->bpp > 32) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid number of bitplanes: %u\n", s->bpp);
            return AVERROR_INVALIDDATA;
        } else if (s->ham >= 8) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid number of hold bits for HAM: %u\n", s->ham);
            return AVERROR_INVALIDDATA;
        }

        av_em_freep(&s->ham_buf);
        av_em_freep(&s->ham_palbuf);
        ...
        if (s->ham) {
            ...
            s->ham_buf = av_em_alloc((s->planesize * 8) + AV_INPUT_BUFFER_PADDING_SIZE);
            if (!s->ham_buf)
                return AVERROR(ENOMEM);

            ham_count = 8 * (1 << s->ham);
            s->ham_palbuf = av_em_alloc((ham_count << !!(s->masking == MASK_HAS_MASK)) * sizeof (uint32_t) + AV_INPUT_BUFFER_PADDING_SIZE);
            if (!s->ham_palbuf) {
                av_em_freep(&s->ham_buf);
                return AVERROR(ENOMEM);
            }
...
{
    IffContext *s = avctx->priv_data;
    av_em_freep(&s->planebuf);
    av_em_freep(&s->ham_buf);
    av_em_freep(&s->ham_palbuf);
    av_em_freep(&s->video[0]);
    av_em_freep(&s->video[1]);
    av_em_freep(&s->pal[0]);
    av_em_freep(&s->pal[1]);
    return 0;
}
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->bits_per_coded_sample <= 8) {
        ...
        if (avctx->codec_tag == MKTAG('R', 'G', 'B', '8')) {
            ...
            if (avctx->bits_per_coded_sample == 24) {
                ...
                avpriv_em_request_sample(avctx, "unknown bits_per_coded_sample");
                return AVERROR_PATCHWELCOME;
            }
        }
    } else {
    ...
    if ((err = av_em_image_check_size(avctx->width, avctx->height, 0, avctx)))
        return err;
    s->planesize = FFALIGN(avctx->width, 16) >> 3; // Align plane size in bits to word-boundary
    s->planebuf  = av_em_alloc(s->planesize + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!s->planebuf)
        return AVERROR(ENOMEM);
    ...
    if (avctx->codec_tag == MKTAG('A', 'N', 'I', 'M')) {
        ...
        s->video[0] = av_em_calloc(FFALIGN(avctx->width, 2) * avctx->height, s->bpp);
        s->video[1] = av_em_calloc(FFALIGN(avctx->width, 2) * avctx->height, s->bpp);
        s->pal[0] = av_em_calloc(256, sizeof(*s->pal[0]));
        s->pal[1] = av_em_calloc(256, sizeof(*s->pal[1]));
...
static void decodeplane8(uint8_t *dst, const uint8_t *buf, int buf_size, int plane)
{
    ...
    if (plane >= 8) {
        av_em_log(NULL, AV_LOG_WARNING, "Ignoring extra planes beyond 8\n");
        return;
    }
    do {
                          ...
                          GetByteContext *gb)
{
    ...
    if (x < dst_size) {
        av_em_log(NULL, AV_LOG_WARNING, "decode_byterun ended before plane size\n");
...
static int unsupported(AVEMCodecContext *avctx)
{
    IffContext *s = avctx->priv_data;
    avpriv_em_request_sample(avctx, "bitmap (compression 0x%0x, bpp %i, ham %i, interlaced %i)", s->compression, s->bpp, s->ham, s->is_interlaced);
    return AVERROR_INVALIDDATA;
}
                        ...
                        AVEMPacket *avpkt)
{
    ...
    desc = av_em_pix_fmt_desc_get(avctx->pix_fmt);
    ...
    case 0x0:
        if (avctx->codec_tag == MKTAG('A', 'C', 'B', 'M')) {
            ...
            int raw_width = avctx->width * (av_em_get_bits_per_pixel(desc) >> 3);
            int x;
            for (y = 0; y < avctx->height && buf < buf_end; y++) {
                uint8_t *row = &frame->data[0][y * frame->linesize[0]];
    ...
    case 0x1:
        if (avctx->codec_tag == MKTAG('I', 'L', 'B', 'M') || // interleaved
            ...
            if (av_em_get_bits_per_pixel(desc) == 32)
