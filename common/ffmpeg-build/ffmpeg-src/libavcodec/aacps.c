 ...
 */ \
static int read_ ## PAR ## _data(AVEMCodecContext *avctx, GetBitContext *gb, PSContext *ps, \
...
{ \
    ...
    return 0; \
err: \
    av_em_log(avctx, AV_LOG_ERROR, "illegal "#PAR"\n"); \
    return -1; \
}
...
int AAC_RENAME(em_ps_read_data)(AVEMCodecContext *avctx, GetBitContext *gb_host, PSContext *ps, int bits_left)
{
    int e;
    ...
    if (header) {     //enable_ps_header
        ...
        if (ps->enable_iid) {
            ...
            if (iid_mode > 5) {
                av_em_log(avctx, AV_LOG_ERROR, "iid_mode %d is reserved.\n",
                       iid_mode);
                goto err;
            }
        ...
        if (ps->enable_icc) {
            ...
            if (ps->icc_mode > 5) {
                av_em_log(avctx, AV_LOG_ERROR, "icc_mode %d is reserved.\n",
                       ps->icc_mode);
                goto err;
            }
    ...
    if (ps->enable_ext) {
        ...
        if (cnt < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "ps extension overflow %d\n", cnt);
            goto err;
        }
        skip_bits(gb, cnt);
    }
        ...
        if (ps->enable_iid){
            for (b = 0; b < ps->nr_iid_par; b++) {
                if (FFABS(ps->iid_par[ps->num_env][b]) > 7 + 8 * ps->iid_quant) {
                    av_em_log(avctx, AV_LOG_ERROR, "iid_par invalid\n");
                    goto err;
                }
            }
        }
        if (ps->enable_icc){
            for (b = 0; b < ps->nr_iid_par; b++) {
                if (ps->icc_par[ps->num_env][b] > 7U) {
                    av_em_log(avctx, AV_LOG_ERROR, "icc_par invalid\n");
                    goto err;
                }
            }
        }
    ...
    av_em_log(avctx, AV_LOG_ERROR, "Expected to read %d PS bits actually read %d.\n", bits_left, bits_consumed);
err:
    ps->start = 0;
...
int AAC_RENAME(em_ps_apply)(AVEMCodecContext *avctx, PSContext *ps, INTFLOAT L[2][38][64], INTFLOAT R[2][38][64], int top)
{
...
av_cold void AAC_RENAME(em_ps_init)(void) {
...
av_cold void AAC_RENAME(em_ps_ctx_init)(PSContext *ps)
{
    AAC_RENAME(em_psdsp_init)(&ps->dsp);
}
