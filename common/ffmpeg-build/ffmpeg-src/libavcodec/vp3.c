...
static int theora_decode_header(AVEMCodecContext *avctx, GetBitContext *gb);
static int theora_decode_tables(AVEMCodecContext *avctx, GetBitContext *gb);
...
static av_cold void free_tables(AVEMCodecContext *avctx)
{
    Vp3DecodeContext *s = avctx->priv_data;

    av_em_freep(&s->superblock_coding);
    av_em_freep(&s->all_fragments);
    av_em_freep(&s->coded_fragment_list[0]);
    av_em_freep(&s->dct_tokens_base);
    av_em_freep(&s->superblock_fragments);
    av_em_freep(&s->macroblock_coding);
    av_em_freep(&s->motion_val[0]);
    av_em_freep(&s->motion_val[1]);
}

static void vp3_decode_flush(AVEMCodecContext *avctx)
{
    Vp3DecodeContext *s = avctx->priv_data;

    if (s->golden_frame.f)
        em_thread_release_buffer(avctx, &s->golden_frame);
    if (s->last_frame.f)
        em_thread_release_buffer(avctx, &s->last_frame);
    if (s->current_frame.f)
        em_thread_release_buffer(avctx, &s->current_frame);
}

static av_cold int vp3_decode_end(AVEMCodecContext *avctx)
{
    Vp3DecodeContext *s = avctx->priv_data;
    int i;

    free_tables(avctx);
    av_em_freep(&s->edge_emu_buffer);

    s->theora_tables = 0;

    /* release all frames */
    vp3_decode_flush(avctx);
    av_em_frame_free(&s->current_frame.f);
    av_em_frame_free(&s->last_frame.f);
    av_em_frame_free(&s->golden_frame.f);
                       ...
                       int eob_run)
{
    ...
    while (coeff_i < num_coeffs && get_bits_left(gb) > 0) {
        ...
        if ((unsigned) token <= 6U) {
            ...
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid token %d\n", token);
            return -1;
        }
    }

    if (blocks_ended > s->num_coded_frags[plane][coeff_index])
        av_em_log(s->avctx, AV_LOG_ERROR, "More blocks ended than coded!\n");
                              ...
                              int plane, int inter, int16_t block[64])
{
    ...
    do {
        ...
        case 1: // zero run
            ...
            i += (token >> 2) & 0x7f;
            if (i > 63) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Coefficient index overflow\n");
                return i;
            }
...
static void vp3_draw_horiz_band(Vp3DecodeContext *s, int y)
{
    ...
    if (HAVE_THREADS && s->avctx->active_thread_type & FF_THREAD_FRAME) {
         ...
         * the frame. This makes the other threads' em_thread_await_progress()
         * calls cheaper, because they don't have to clip their values. */
        em_thread_report_progress(&s->current_frame,
                                ...
                                int motion_y, int y)
{
    ...
    em_thread_await_progress(ref_frame, ref_row, 0);
}
...
static av_cold int allocate_tables(AVEMCodecContext *avctx)
{
    Vp3DecodeContext *s = avctx->priv_data;
    ...
    s->superblock_coding = av_em_mallocz(s->superblock_count);
    s->all_fragments     = av_em_mallocz_array(s->fragment_count, sizeof(Vp3Fragment));

    s->coded_fragment_list[0] = av_em_mallocz_array(s->fragment_count, sizeof(int));

    s->dct_tokens_base = av_em_mallocz_array(s->fragment_count,
                                          64 * sizeof(*s->dct_tokens_base));
    s->motion_val[0] = av_em_mallocz_array(y_fragment_count, sizeof(*s->motion_val[0]));
    s->motion_val[1] = av_em_mallocz_array(c_fragment_count, sizeof(*s->motion_val[1]));

    /* work out the block mapping tables */
    s->superblock_fragments = av_em_mallocz_array(s->superblock_count, 16 * sizeof(int));
    s->macroblock_coding    = av_em_mallocz(s->macroblock_count + 1);
...
{
    s->current_frame.f = av_em_frame_alloc();
    s->last_frame.f    = av_em_frame_alloc();
    s->golden_frame.f  = av_em_frame_alloc();
...
static av_cold int vp3_decode_init(AVEMCodecContext *avctx)
{
    Vp3DecodeContext *s = avctx->priv_data;
    ...
    em_hpeldsp_init(&s->hdsp, avctx->flags | AV_CODEC_FLAG_BITEXACT);
...
#undef TRANSPOSE
    ...
    avcodec_em_get_chroma_sub_sample(avctx->pix_fmt, &s->chroma_x_shift, &s->chroma_y_shift);
...
vlc_fail:
    av_em_log(avctx, AV_LOG_FATAL, "Invalid huffman table\n");
    return -1;
}

/// Release and shuffle frames after decode finishes
static int update_frames(AVEMCodecContext *avctx)
{
    Vp3DecodeContext *s = avctx->priv_data;
    int ret = 0;
    ...
    em_thread_release_buffer(avctx, &s->last_frame);
    ret = em_thread_ref_frame(&s->last_frame, &s->current_frame);
    if (ret < 0)
        goto fail;

    if (s->keyframe) {
        em_thread_release_buffer(avctx, &s->golden_frame);
        ret = em_thread_ref_frame(&s->golden_frame, &s->current_frame);
    }
...
{
    em_thread_release_buffer(s->avctx, dst);
    if (src->f->data[0])
        return em_thread_ref_frame(dst, src);
    return 0;
}
...
#if HAVE_THREADS
static int vp3_update_thread_context(AVEMCodecContext *dst, const AVEMCodecContext *src)
{
...
static int vp3_decode_frame(AVEMCodecContext *avctx,
                            ...
                            AVEMPacket *avpkt)
{
    ...
    if (s->theora && get_bits1(&gb)) {
        ...
        if (s->avctx->active_thread_type&FF_THREAD_FRAME) {
            av_em_log(avctx, AV_LOG_ERROR, "midstream reconfiguration with multithreading is unsupported, try -threads 1\n");
    ...
    if (!s->all_fragments) {
        av_em_log(avctx, AV_LOG_ERROR, "Data packet without prior valid headers\n");
        return -1;
    }
    if (!s->theora)
        skip_bits(&gb, 1);
    ...
    if (s->avctx->debug & FF_DEBUG_PICT_INFO)
        av_em_log(s->avctx, AV_LOG_INFO, " VP3 %sframe #%d: Q index = %d\n",
    ...
    if (em_thread_get_buffer(avctx, &s->current_frame, AV_GET_BUFFER_FLAG_REF) < 0)
        goto error;

    if (!s->edge_emu_buffer)
        s->edge_emu_buffer = av_em_alloc(9 * FFABS(s->current_frame.f->linesize[0]));
    ...
    if (s->keyframe) {
        ...
        if (!s->golden_frame.f->data[0]) {
            ...
            if (em_thread_get_buffer(avctx, &s->golden_frame,
                                     AV_GET_BUFFER_FLAG_REF) < 0)
                goto error;
            em_thread_release_buffer(avctx, &s->last_frame);
            if ((ret = em_thread_ref_frame(&s->last_frame,
                                           &s->golden_frame)) < 0)
                goto error;
            em_thread_report_progress(&s->last_frame, INT_MAX, 0);
        }
    }
    ...
    if (unpack_superblocks(s, &gb)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "error in unpack_superblocks\n");
        goto error;
    }
    if (unpack_modes(s, &gb)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "error in unpack_modes\n");
        goto error;
    }
    if (unpack_vectors(s, &gb)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "error in unpack_vectors\n");
        goto error;
    }
    if (unpack_block_qpis(s, &gb)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "error in unpack_block_qpis\n");
        goto error;
    }
    if (unpack_dct_coeffs(s, &gb)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "error in unpack_dct_coeffs\n");
        goto error;
    }
    ...
    if ((ret = av_em_frame_ref(data, s->current_frame.f)) < 0)
        return ret;
    for (i = 0; i < 3; i++) {
        AVFrame *dst = data;
...
error:
    em_thread_report_progress(&s->current_frame, INT_MAX, 0);

    if (!HAVE_THREADS || !(s->avctx->active_thread_type & FF_THREAD_FRAME))
        av_em_frame_unref(s->current_frame.f);

    return -1;
}

static int read_huffman_tree(AVEMCodecContext *avctx, GetBitContext *gb)
{
    Vp3DecodeContext *s = avctx->priv_data;
    ...
    if (get_bits1(gb)) {
        ...
        if (s->entries >= 32) { /* overflow */
            av_em_log(avctx, AV_LOG_ERROR, "huffman tree overflow\n");
            return -1;
        }
        token = get_bits(gb, 5);
...
#if HAVE_THREADS
static int vp3_init_thread_copy(AVEMCodecContext *avctx)
{
    Vp3DecodeContext *s = avctx->priv_data;
...
static int theora_decode_header(AVEMCodecContext *avctx, GetBitContext *gb)
{
    ...
    int ret;
    AVEMRational fps, aspect;

    s->theora_header = 0;
    s->theora = get_bits_long(gb, 24);
    av_em_log(avctx, AV_LOG_DEBUG, "Theora bitstream version %X\n", s->theora);
    ...
    /* sanity check */
    if (av_em_image_check_size(visible_width, visible_height, 0, avctx) < 0 ||
    ...
    if (fps.num && fps.den) {
        if (fps.num < 0 || fps.den < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid framerate\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    if (aspect.num && aspect.den) {
        av_em_reduce(&avctx->sample_aspect_ratio.num,
                  &avctx->sample_aspect_ratio.den,
                  aspect.num, aspect.den, 1 << 30);
        em_set_sar(avctx, avctx->sample_aspect_ratio);
    }
    ...
    if (s->theora >= 0x030200) {
        ...
        if (avctx->pix_fmt == AV_PIX_FMT_NONE) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid pixel format\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    if (!(avctx->flags2 & AV_CODEC_FLAG2_IGNORE_CROP)) {
        ...
        if ((s->offset_x & 0x1F) && !(avctx->flags & AV_CODEC_FLAG_UNALIGNED)) {
            ...
            if (!s->offset_x_warned) {
                s->offset_x_warned = 1;
                av_em_log(avctx, AV_LOG_WARNING, "Reducing offset_x from %d to %d"
...
static int theora_decode_tables(AVEMCodecContext *avctx, GetBitContext *gb)
{
    ...
    if (matrices > 384) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid number of base matrixes\n");
        return -1;
    }
    ...
    for (inter = 0; inter <= 1; inter++) {
        for (plane = 0; plane <= 2; plane++) {
            ...
            if (!newqr) {
                ...
                if (qi > 63) {
                    av_em_log(avctx, AV_LOG_ERROR, "invalid qi %d > 63\n", qi);
                    return -1;
                }
...
static av_cold int theora_decode_init(AVEMCodecContext *avctx)
{
    Vp3DecodeContext *s = avctx->priv_data;
    GetBitContext gb;
    int ptype;
    ...
    if (!avctx->extradata_size) {
        av_em_log(avctx, AV_LOG_ERROR, "Missing extradata!\n");
        return -1;
    }

    if (avpriv_split_xiph_headers(avctx->extradata, avctx->extradata_size,
                                  42, header_start, header_len) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Corrupt extradata\n");
        return -1;
    }
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (!(ptype & 0x80)) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid extradata!\n");
//          return -1;
        }
