...
static void vb_decode_palette(VBDecContext *c, int data_size)
{
    ...
    if (start + size > 255) {
        av_em_log(c->avctx, AV_LOG_ERROR, "Palette change runs beyond entry 256\n");
        return;
    }
    if (size*3+2 > data_size) {
        av_em_log(c->avctx, AV_LOG_ERROR, "Palette data runs beyond chunk size\n");
        return;
    }
    ...
    for (blk = 0; blk < blocks; blk++) {
        ...
        case 0x40:
            ...
            if (!t) { //raw block
                if (bytestream2_get_bytes_left(&g) < 16) {
                    av_em_log(c->avctx, AV_LOG_ERROR, "Insufficient data\n");
        ...
        case 0xC0: // pattern fill
            ...
            case 2:
                ...
                break;
            case 3:
                av_em_log(c->avctx, AV_LOG_ERROR, "Invalid opcode seen @%d\n", blk);
                return AVERROR_INVALIDDATA;
            }
            break;
        }
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (flags & VB_HAS_VIDEO) {
        ...
        if(size > bytestream2_get_bytes_left(&c->stream)+4 || size<4){
            av_em_log(avctx, AV_LOG_ERROR, "Frame size invalid\n");
            return -1;
        }
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    c->frame      = av_em_mallocz(avctx->width * avctx->height);
    c->prev_frame = av_em_mallocz(avctx->width * avctx->height);

    if (!c->frame || !c->prev_frame) {
        av_em_freep(&c->frame);
        av_em_freep(&c->prev_frame);
