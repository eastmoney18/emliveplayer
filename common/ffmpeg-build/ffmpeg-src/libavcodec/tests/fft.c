...
static int fft_ref_init(int nbits, int inverse)
{
    ...
    exptab = av_em_malloc_array((n / 2), sizeof(*exptab));
    if (!exptab)
...
static int check_diff(FFTSample *tab1, FFTSample *tab2, int n, double scale)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (e >= 1e-3) {
            av_em_log(NULL, AV_LOG_ERROR, "ERROR %5d: "FMT" "FMT"\n",
                   i, tab1[i], tab2[i]);
            err = 1;
        }
        error += e * e;
    ...
    av_em_log(NULL, AV_LOG_INFO, "max:%f e:%g\n", max, sqrt(error / n));
    return err;
}
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        case 'c':
        {
            unsigned cpuflags = av_em_get_cpu_flags();

            if (av_em_parse_cpu_caps(&cpuflags, optarg) < 0)
                return 1;

            av_em_force_cpu_flags(cpuflags);
            break;
        }
        }
    }
    ...
    tab      = av_em_malloc_array(fft_size, sizeof(FFTComplex));
    tab1     = av_em_malloc_array(fft_size, sizeof(FFTComplex));
    tab_ref  = av_em_malloc_array(fft_size, sizeof(FFTComplex));
    tab2     = av_em_malloc_array(fft_size, sizeof(FFTSample));
...
#if CONFIG_MDCT
    case TRANSFORM_MDCT:
        av_em_log(NULL, AV_LOG_INFO, "Scale factor is set to %f\n", scale);
        if (do_inverse)
            av_em_log(NULL, AV_LOG_INFO, "IMDCT");
        else
            av_em_log(NULL, AV_LOG_INFO, "MDCT");
        ff_mdct_init(&m, fft_nbits, do_inverse, scale);
        break;
#endif /* CONFIG_MDCT */
    case TRANSFORM_FFT:
        if (do_inverse)
            av_em_log(NULL, AV_LOG_INFO, "IFFT");
        else
            av_em_log(NULL, AV_LOG_INFO, "FFT");
...
#    if CONFIG_RDFT
    case TRANSFORM_RDFT:
        if (do_inverse)
            av_em_log(NULL, AV_LOG_INFO, "IDFT_C2R");
        else
            av_em_log(NULL, AV_LOG_INFO, "DFT_R2C");
...
#    if CONFIG_DCT
    case TRANSFORM_DCT:
        if (do_inverse)
            av_em_log(NULL, AV_LOG_INFO, "DCT_III");
        else
            av_em_log(NULL, AV_LOG_INFO, "DCT_II");
        ff_dct_init(&d, fft_nbits, do_inverse ? DCT_III : DCT_II);
        break;
#    endif /* CONFIG_DCT */
#endif /* FFT_FLOAT */
    default:
        av_em_log(NULL, AV_LOG_ERROR, "Requested transform not supported\n");
        goto cleanup;
    }
    av_em_log(NULL, AV_LOG_INFO, " %d test\n", fft_size);
    ...
    /* checking result */
    av_em_log(NULL, AV_LOG_INFO, "Checking...\n");
    ...
    if (do_speed) {
        ...
        av_em_log(NULL, AV_LOG_INFO, "Speed test...\n");
        /* we measure during about 1 seconds */
        nb_its = 1;
        for (;;) {
            time_start = av_em_gettime_relative();
            ...
            duration = av_em_gettime_relative() - time_start;
            if (duration >= 1000000)
                break;
            nb_its *= 2;
        }
