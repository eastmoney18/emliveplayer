                          ...
                          int use_pred)
{
    ...
    if ((ret = build_huff10(huff, &vlc, &fsym)) < 0) {
        av_em_log(c->avctx, AV_LOG_ERROR, "Cannot build Huffman codes\n");
        return ret;
    }
    ...
    for (slice = 0; slice < c->slices; slice++) {
        ...
        if (!slice_size) {
            av_em_log(c->avctx, AV_LOG_ERROR, "Plane has more than one symbol "
        ...
        for (j = sstart; j < send; j++) {
            for (i = 0; i < width * step; i += step) {
                if (get_bits_left(&gb) <= 0) {
                    av_em_log(c->avctx, AV_LOG_ERROR,
                           "Slice decoding ran out of bits\n");
                    goto fail;
                }
                pix = get_vlc2(&gb, vlc.table, vlc.bits, 3);
                if (pix < 0) {
                    av_em_log(c->avctx, AV_LOG_ERROR, "Decoding error\n");
                    goto fail;
                }
                if (use_pred) {
                    prev += pix;
        ...
        if (get_bits_left(&gb) > 32)
            av_em_log(c->avctx, AV_LOG_WARNING,
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (c->pro) {
        if (bytestream2_get_bytes_left(&gb) < c->frame_info_size) {
            av_em_log(avctx, AV_LOG_ERROR, "Not enough data for frame information\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        for (i = 0; i < c->planes; i++) {
            ...
            if (bytestream2_get_bytes_left(&gb) < 1024 + 4 * c->slices) {
                av_em_log(avctx, AV_LOG_ERROR, "Insufficient data for a plane\n");
                return AVERROR_INVALIDDATA;
            }
            slice_start = 0;
            slice_end   = 0;
            for (j = 0; j < c->slices; j++) {
                ...
                if (slice_end < 0 || slice_end < slice_start ||
                    ...
                    av_em_log(avctx, AV_LOG_ERROR, "Incorrect slice size\n");
                    return AVERROR_INVALIDDATA;
                }
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "frame information flags %"PRIX32"\n",
           c->frame_info);

    c->frame_pred = (c->frame_info >> 8) & 3;

    if (c->frame_pred == PRED_GRADIENT) {
        avpriv_em_request_sample(avctx, "Frame with gradient prediction");
        return AVERROR_PATCHWELCOME;
    }

    av_em_fast_malloc(&c->slice_bits, &c->slice_bits_size,
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->extradata_size >= 16) {
        av_em_log(avctx, AV_LOG_DEBUG, "Encoder version %d.%d.%d.%d\n",
               avctx->extradata[3], avctx->extradata[2],
               avctx->extradata[1], avctx->extradata[0]);
        av_em_log(avctx, AV_LOG_DEBUG, "Original format %"PRIX32"\n",
               AV_RB32(avctx->extradata + 4));
        c->frame_info_size = AV_RL32(avctx->extradata + 8);
        c->flags           = AV_RL32(avctx->extradata + 12);

        if (c->frame_info_size != 4)
            avpriv_em_request_sample(avctx, "Frame info not 4 bytes");
        av_em_log(avctx, AV_LOG_DEBUG, "Encoding parameters %08"PRIX32"\n", c->flags);
    ...
    case MKTAG('U', 'L', 'H', '2'):
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unknown Ut Video FOURCC provided (%08X)\n",
               avctx->codec_tag);
        return AVERROR_INVALIDDATA;
    }
