...
static int xvid_encode_close(AVEMCodecContext *avctx);
static int xvid_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
 ...
 * compliance.
 *
 * @param avctx AVEMCodecContext pointer to context
 ...
 */
static int xvid_strip_vol_header(AVEMCodecContext *avctx, AVEMPacket *pkt,
                                 ...
                                 unsigned int frame_len)
{
    ...
    if (vo_len > 0) {
        ...
        if (!avctx->extradata) {
            avctx->extradata = av_em_alloc(vo_len);
            if (!avctx->extradata)
 ...
 */
static void xvid_correct_framerate(AVEMCodecContext *avctx)
{
    int frate, fbase;
    ...
    gcd = av_em_gcd(est_frate, est_fbase);
    if (gcd > 1) {
        est_frate /= gcd;
        est_fbase /= gcd;
    }
...
static av_cold int xvid_encode_init(AVEMCodecContext *avctx)
{
    int xerr, i, ret = -1;
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (xvid_flags & AV_CODEC_FLAG_PASS1) {
        ...
        rc2pass1.context     = x;
        x->twopassbuffer     = av_em_alloc(BUFFER_SIZE);
        x->old_twopassbuffer = av_em_alloc(BUFFER_SIZE);
        ...
        fd = avpriv_em_tempfile("xvidff.", &x->twopassfile, 0, avctx);
        if (fd < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Xvid: Cannot write 2-pass pipe\n");
            return fd;
        }
        x->twopassfd = fd;
        ...
        if (ret == -1)
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Xvid: Cannot write to 2-pass pipe\n");
            ret = AVERROR(EIO);
        }
        if (ret < 0)
            return ret;
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if ((avctx->intra_matrix || avctx->inter_matrix)) {
        ...
        if (avctx->intra_matrix) {
            ...
            x->intra_matrix = av_em_alloc(sizeof(unsigned char) * 64);
            if (!x->intra_matrix)
                return AVERROR(ENOMEM);
        } else
            intra = NULL;
        if (avctx->inter_matrix) {
            ...
            x->inter_matrix = av_em_alloc(sizeof(unsigned char) * 64);
            if (!x->inter_matrix)
                return AVERROR(ENOMEM);
        } else
            inter = NULL;
    ...
    if (x->quicktime_format) {
        ...
        av_em_init_packet(&packet);

        picture = av_em_frame_alloc();
        if (!picture)
            return AVERROR(ENOMEM);

        xerr = xvid_encore(NULL, XVID_ENC_CREATE, &xvid_enc_create, NULL);
        if( xerr ) {
            av_em_frame_free(&picture);
            av_em_log(avctx, AV_LOG_ERROR, "Xvid: Could not create encoder reference\n");
            return AVERROR_EXTERNAL;
        }
        x->encoder_handle = xvid_enc_create.handle;
        size = ((avctx->width + 1) & ~1) * ((avctx->height + 1) & ~1);
        picture->data[0] = av_em_alloc(size + size / 2);
        if (!picture->data[0]) {
            av_em_frame_free(&picture);
            return AVERROR(ENOMEM);
        }
        ...
        if (!ret && got_packet)
            av_em_packet_unref(&packet);
        av_em_free(picture->data[0]);
        av_em_frame_free(&picture);
                             ...
                             const AVFrame *picture, int *got_packet)
{
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, mb_width*(int64_t)mb_height*MAX_MB_BYTES + AV_INPUT_BUFFER_MIN_SIZE, 0)) < 0)
        return ret;
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        em_side_data_set_encoder_stats(pkt, xvid_enc_stats.quant * FF_QP2LAMBDA, NULL, 0, pict_type);
...
static av_cold int xvid_encode_close(AVEMCodecContext *avctx)
{
    struct xvid_context *x = avctx->priv_data;
    ...
    if (x->twopassbuffer) {
        av_em_freep(&x->twopassbuffer);
        av_em_freep(&x->old_twopassbuffer);
        avctx->stats_out = NULL;
    }
    if (x->twopassfd>=0) {
    ...
    av_em_freep(&x->twopassfile);
    av_em_freep(&x->intra_matrix);
    av_em_freep(&x->inter_matrix);
...
static const AVEMClass xvid_class = {
    .class_name = "libxvid",
