...
static av_cold int seqvideo_decode_init(AVEMCodecContext *avctx)
{
...
static int seqvideo_decode_frame(AVEMCodecContext *avctx,
                                 ...
                                 AVEMPacket *avpkt)
{
    ...
    if ((ret = em_reget_buffer(avctx, seq->frame)) < 0)
        return ret;

    if (seqvideo_decode(seq, buf, buf_size))
        return AVERROR_INVALIDDATA;

    if ((ret = av_em_frame_ref(data, seq->frame)) < 0)
        return ret;
    *got_frame       = 1;
...
static av_cold int seqvideo_decode_end(AVEMCodecContext *avctx)
{
    SeqVideoContext *seq = avctx->priv_data;

    av_em_frame_free(&seq->frame);

    return 0;
}

AVEMCodec ff_tiertexseqvideo_decoder = {
