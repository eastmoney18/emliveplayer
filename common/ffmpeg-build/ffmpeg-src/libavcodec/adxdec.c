...
static av_cold int adx_decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->extradata_size >= 24) {
        if ((ret = ff_adx_decode_header(avctx, avctx->extradata,
                                        ...
                                        c->coeff)) < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "error parsing ADX header\n");
...
static int adx_decode_frame(AVEMCodecContext *avctx, void *data,
...
static void adx_decode_flush(AVEMCodecContext *avctx)
{
    ADXContext *c = avctx->priv_data;
...
AVEMCodec ff_adpcm_adx_decoder = {
    .name           = "adpcm_adx",
