...
static double get_fps(AVEMCodecContext *avctx)
{
    return 1.0 / av_em_q2d(avctx->time_base) / FFMAX(avctx->ticks_per_frame, 1);
}

static inline double qp2bits(RateControlEntry *rce, double qp)
{
    if (qp <= 0.0) {
        av_em_log(NULL, AV_LOG_ERROR, "qp<=0.0\n");
    }
...
{
    if (bits < 0.9) {
        av_em_log(NULL, AV_LOG_ERROR, "bits<0.9\n");
    }
...
av_cold int ff_rate_control_init(MpegEncContext *s)
{
    ...
    res = av_em_expr_parse(&rcc->rc_eq_eval,
                        s->rc_eq ? s->rc_eq : "tex^qComp",
                        const_names, func1_names, func1,
                        NULL, NULL, 0, s->avctx);
    if (res < 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Error parsing rc_eq \"%s\"\n", s->rc_eq);
        return res;
    }
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (s->avctx->flags & AV_CODEC_FLAG_PASS2) {
        ...
        if (i <= 0 || i >= INT_MAX / sizeof(RateControlEntry))
            return -1;
        rcc->entry       = av_em_mallocz(i * sizeof(RateControlEntry));
        if (!rcc->entry)
...
#if CONFIG_LIBXVID
            return em_xvid_rate_control_init(s);
#else
            av_em_log(s->avctx, AV_LOG_ERROR,
    ...
    if (!(s->avctx->flags & AV_CODEC_FLAG_PASS2)) {
        ...
        if (s->avctx->qblur > 1.0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "qblur too large\n");
            return -1;
        }
...
av_cold void ff_rate_control_uninit(MpegEncContext *s)
{
    ...
    av_em_expr_free(rcc->rc_eq_eval);
    av_em_freep(&rcc->entry);

#if CONFIG_LIBXVID
    if ((s->avctx->flags & AV_CODEC_FLAG_PASS2) && s->rc_strategy == MPV_RC_STRATEGY_XVID)
        em_xvid_rate_control_uninit(s);
#endif
}
...
int ff_vbv_update(MpegEncContext *s, int frame_size)
{
    ...
    if (buffer_size) {
        ...
        if (rcc->buffer_index < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "rc buffer underflow\n");
            if (frame_size > max_rate && s->qscale == s->avctx->qmax) {
                av_em_log(s->avctx, AV_LOG_ERROR, "max bitrate possibly too small or try trellis with large lmax or increase qmax\n");
            }
            rcc->buffer_index = 0;
        }
        ...
        if (rcc->buffer_index > buffer_size) {
            ...
            if (s->avctx->debug & FF_DEBUG_RC)
                av_em_log(s->avctx, AV_LOG_DEBUG, "stuffing %d bytes\n", stuffing);
                         ...
                         double rate_factor, int frame_num)
{
    ...
    bits = av_em_expr_eval(rcc->rc_eq_eval, const_values, rce);
    if (isnan(bits)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Error evaluating rc_eq \"%s\"\n", s->rc_eq);
        return -1;
    }
    ...
    for (i = 0; i < s->avctx->rc_override_count; i++) {
        EMRcOverride *rco = s->avctx->rc_override;
...
float ff_rate_estimate_qscale(MpegEncContext *s, int dry_run)
{
...
#if CONFIG_LIBXVID
    if ((s->avctx->flags & AV_CODEC_FLAG_PASS2) && s->rc_strategy == MPV_RC_STRATEGY_XVID)
        return em_xvid_rate_estimate_qscale(s, dry_run);
#endif
    ...
    if (s->avctx->flags & AV_CODEC_FLAG_PASS2) {
        ...
        if (picture_number >= rcc->num_entries) {
            av_em_log(s, AV_LOG_ERROR, "Input is longer than 2-pass log file\n");
            return -1;
        }
        rce         = &rcc->entry[picture_number];
    ...
    if (s->avctx->debug & FF_DEBUG_RC) {
        av_em_log(s->avctx, AV_LOG_DEBUG,
               ...
               av_em_get_picture_type_char(pict_type),
...
static int init_pass2(MpegEncContext *s)
{
    ...
    if (all_available_bits < all_const_bits) {
        av_em_log(s->avctx, AV_LOG_ERROR, "requested bitrate is too low\n");
        return -1;
    }

    qscale         = av_em_malloc_array(rcc->num_entries, sizeof(double));
    blurred_qscale = av_em_malloc_array(rcc->num_entries, sizeof(double));
    if (!qscale || !blurred_qscale) {
        av_em_free(qscale);
        av_em_free(blurred_qscale);
        return AVERROR(ENOMEM);
    }
    toobig = 0;
