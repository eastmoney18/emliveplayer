 ...
 */
static void set_mv_strides(EREMContext *s, int *mv_step, int *stride)
{
 ...
 */
static void put_dc(EREMContext *s, uint8_t *dest_y, uint8_t *dest_cb,
 ...
 */
static void guess_dc(EREMContext *s, int16_t *dc, int w,
                     int h, int stride, int is_luma)
{
    int b_x, b_y;
    int16_t  (*col )[4] = av_em_malloc_array(stride, h*sizeof( int16_t)*4);
    uint32_t (*dist)[4] = av_em_malloc_array(stride, h*sizeof(uint32_t)*4);

    if(!col || !dist) {
        av_em_log(s->avctx, AV_LOG_ERROR, "guess_dc() is out of memory\n");
        goto fail;
    }
 ...
 * @param h     height in 8 pixel blocks
 */
static void h_block_filter(EREMContext *s, uint8_t *dst, int w,
                           int h, int stride, int is_luma)
{
 ...
 * @param h     height in 8 pixel blocks
 */
static void v_block_filter(EREMContext *s, uint8_t *dst, int w, int h,
...
static void guess_mv(EREMContext *s)
{
...
#define MV_UNCHANGED 1
    ...
    if (s->last_pic.motion_val[0])
        em_thread_await_progress(s->last_pic.tf, mb_height-1, 0);
...
static int is_intra_more_likely(EREMContext *s)
{
        ...
        for (mb_x = 0; mb_x < s->mb_width; mb_x++) {
            ...
            if (s->cur_pic.f->pict_type == AV_PICTURE_TYPE_I) {
                ...
                if (s->avctx->codec_id == AV_CODEC_ID_H264) {
                    ...
                    em_thread_await_progress(s->last_pic.tf, mb_y, 0);
                }
...
//      av_em_log(NULL, AV_LOG_ERROR, "is_intra_likely: %d type:%d\n", is_intra_likely, s->pict_type);
    return is_intra_likely > 0;
}

void ff_em_er_frame_start(EREMContext *s)
{
    if (!s->avctx->error_concealment)
        return;

    if (!s->mecc_inited) {
        ff_me_em_cmp_init(&s->mecc, s->avctx);
        s->mecc_inited = 1;
    }
...
static int er_supported(EREMContext *s)
{
 ...
 */
void ff_em_er_add_slice(EREMContext *s, int startx, int starty,
                     int endx, int endy, int status)
{
    ...
    if (status & (ER_AC_ERROR | ER_AC_END)) {
        ...
        avpriv_em_atomic_int_add_and_fetch(&s->error_count, start_i - end_i - 1);
    }
    ...
    if (status & ER_MB_ERROR) {
        s->error_occurred = 1;
        avpriv_em_atomic_int_set(&s->error_count, INT_MAX);
    }
...
void ff_em_er_frame_end(EREMContext *s)
{
    int *linesize = NULL;
    ...
    if (   mb_x == s->mb_width
        ...
        av_em_log(s->avctx, AV_LOG_DEBUG, "ignoring last missing slice\n");
        return;
    }
    ...
    if (s->last_pic.f) {
        if (s->last_pic.f->width  != s->cur_pic.f->width  ||
            ...
            av_em_log(s->avctx, AV_LOG_WARNING, "Cannot use previous picture in error concealment\n");
    ...
    if (s->next_pic.f) {
        if (s->next_pic.f->width  != s->cur_pic.f->width  ||
            ...
            av_em_log(s->avctx, AV_LOG_WARNING, "Cannot use next picture in error concealment\n");
            memset(&s->next_pic, 0, sizeof(s->next_pic));
        }
    }

    if (!s->cur_pic.motion_val[0] || !s->cur_pic.ref_index[0]) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Warning MVs not available\n");

        for (i = 0; i < 2; i++) {
            s->ref_index_buf[i]  = av_em_buffer_allocz(s->mb_stride * s->mb_height * 4 * sizeof(uint8_t));
            s->motion_val_buf[i] = av_em_buffer_allocz((size + 4) * 2 * sizeof(uint16_t));
        ...
        if (i < 2) {
            for (i = 0; i < 2; i++) {
                av_em_buffer_unref(&s->ref_index_buf[i]);
                av_em_buffer_unref(&s->motion_val_buf[i]);
    ...
    if (s->avctx->debug & FF_DEBUG_ER) {
        for (mb_y = 0; mb_y < s->mb_height; mb_y++) {
            for (mb_x = 0; mb_x < s->mb_width; mb_x++) {
                ...
                av_em_log(s->avctx, AV_LOG_DEBUG, "%2X ", status);
            }
    ...
    av_em_log(s->avctx, AV_LOG_INFO, "concealing %d DC, %d AC, %d MV errors in %c frame\n",
           dc_error, ac_error, mv_error, av_em_get_picture_type_char(s->cur_pic.f->pict_type));
        ...
        for (mb_y = 0; mb_y < s->mb_height; mb_y++) {
            for (mb_x = 0; mb_x < s->mb_width; mb_x++) {
                ...
                if (s->pp_time) {
                    ...
                    em_thread_await_progress(s->next_pic.tf, mb_y, 0);
...
ec_clean:
    ...
    memset(&s->cur_pic, 0, sizeof(EREMPicture));
    memset(&s->last_pic, 0, sizeof(EREMPicture));
    memset(&s->next_pic, 0, sizeof(EREMPicture));
}
