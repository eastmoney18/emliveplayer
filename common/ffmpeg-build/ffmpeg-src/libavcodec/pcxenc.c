...
static av_cold int pcx_encode_init(AVEMCodecContext *avctx)
{
...
static int pcx_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                            const AVFrame *frame, int *got_packet)
{
    ...
    if (avctx->width > 65535 || avctx->height > 65535) {
        av_em_log(avctx, AV_LOG_ERROR, "image dimensions do not fit in 16 bits\n");
        return AVERROR(EINVAL);
    }
    ...
    case AV_PIX_FMT_GRAY8:
        bpp = 8;
        nplanes = 1;
        avpriv_em_set_systematic_pal2(palette256, avctx->pix_fmt);
        pal = palette256;
        break;
    ...
    case AV_PIX_FMT_MONOBLACK:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "unsupported pixfmt\n");
        return AVERROR(EINVAL);
    }
    ...
    if (sw > 0xFFFFu || sh > 0xFFFFu)
        av_em_reduce(&sw, &sh, sw, sh, 0xFFFFu);
