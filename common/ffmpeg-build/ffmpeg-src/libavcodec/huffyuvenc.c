...
static int store_huffman_tables(HYuvContext *s, uint8_t *buf)
{
    ...
    for (i = 0; i < count; i++) {
        if ((ret = em_huff_gen_len_table(s->len[i], s->stats[i], s->vlc_n, 0)) < 0)
            return ret;

        if (em_huffyuv_generate_bits_table(s->bits[i], s->len[i], s->vlc_n) < 0) {
            return -1;
        }
...
static av_cold int encode_init(AVEMCodecContext *avctx)
{
    ...
    em_huffyuv_common_init(avctx);
    em_huffyuvencdsp_init(&s->hencdsp);

    avctx->extradata = av_em_mallocz(3*MAX_N + 4);
    if (s->flags&AV_CODEC_FLAG_PASS1) {
#define STATS_OUT_SIZE 21*MAX_N*3 + 4
        avctx->stats_out = av_em_mallocz(STATS_OUT_SIZE); // 21*256*3(%llu ) + 3(\n) + 1(0) = 16132
        if (!avctx->stats_out)
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    av_em_pix_fmt_get_chroma_sub_sample(avctx->pix_fmt,
                                     &s->chroma_h_shift,
                                     &s->chroma_v_shift);
    ...
    case AV_PIX_FMT_YUV422P:
        if (s->width & 1) {
            av_em_log(avctx, AV_LOG_ERROR, "Width must be even for this colorspace.\n");
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (s->version > 3 && avctx->strict_std_compliance > FF_COMPLIANCE_EXPERIMENTAL) {
        av_em_log(avctx, AV_LOG_ERROR, "Ver > 3 is under development, files encoded with it may not be decodable with future versions!!!\n"
    ...
    if (em_huffyuv_alloc_temp(s)) {
        em_huffyuv_common_end(s);
        return AVERROR(ENOMEM);
    }
                        ...
                        const AVFrame *pict, int *got_packet)
{
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, width * height * 3 * 4 + AV_INPUT_BUFFER_MIN_SIZE, 0)) < 0)
        return ret;
...
static const AVEMClass normal_class = {
    .class_name = "huffyuv",
...
static const AVEMClass ff_class = {
    .class_name = "ffvhuff",
    .item_name  = av_em_default_item_name,
    .option     = ff_options,
    .version    = LIBAVUTIL_VERSION_INT,
