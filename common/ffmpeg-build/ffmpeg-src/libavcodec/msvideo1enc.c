                               ...
                               const AVFrame *pict, int *got_packet)
{
    ...
    if(!c->prev)
        c->prev = av_em_alloc(avctx->width * 3 * (avctx->height + 3));
...
static av_cold int encode_init(AVEMCodecContext *avctx)
{
    ...
    if((avctx->width&3) || (avctx->height&3)){
        av_em_log(avctx, AV_LOG_ERROR, "width and height must be multiples of 4\n");
        return -1;
    }
