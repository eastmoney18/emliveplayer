...
static av_cold int decode_end(AVEMCodecContext *avctx)
{
    ...
    av_em_frame_free(&c93->pictures[0]);
    av_em_frame_free(&c93->pictures[1]);
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    s->pictures[0] = av_em_frame_alloc();
    s->pictures[1] = av_em_frame_alloc();
...
static inline int copy_block(AVEMCodecContext *avctx, uint8_t *to,
        uint8_t *from, int offset, int height, int stride)
{
    ...
    if (from_y + height > HEIGHT) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid offset %d during C93 decoding\n",
               offset);
                        ...
                        int *got_frame, AVEMPacket *avpkt)
{
    ...
    if ((ret = em_reget_buffer(avctx, newpic)) < 0)
        return ret;
    ...
    for (y = 0; y < HEIGHT; y += 8) {
        ...
        for (x = 0; x < WIDTH; x += 8) {
                ...
                for (j = 0; j < 8; j += 4) {
                    for (i = 0; i < 8; i += 4) {
                        ...
                        if (block_type == C93_4X4_FROM_CURR && from_y == y+j &&
                            ...
                            avpriv_em_request_sample(avctx, "block overlap %d %d %d %d", from_x, x+i, from_y, y+j);
                            return AVERROR_INVALIDDATA;
                        }
            ...
            default:
                av_em_log(avctx, AV_LOG_ERROR, "unexpected type %x at %dx%d\n",
                       block_type, x, y);
                return AVERROR_INVALIDDATA;
            }
            bt >>= 4;
    ...
    if ((ret = av_em_frame_ref(data, newpic)) < 0)
        return ret;
    *got_frame = 1;
