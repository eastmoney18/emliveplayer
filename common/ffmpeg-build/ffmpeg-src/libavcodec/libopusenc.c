...
static void libopus_write_header(AVEMCodecContext *avctx, int stream_count,
...
static int libopus_configure_encoder(AVEMCodecContext *avctx, OpusMSEncoder *enc,
                                     LibopusEncOpts *opts)
{
    int ret;
...
static av_cold int libopus_encode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->bit_rate < 500 || avctx->bit_rate > 256000 * avctx->channels) {
        av_em_log(avctx, AV_LOG_ERROR, "The bit rate %"PRId64" bps is unsupported. "
    ...
    case 2880:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Invalid frame duration: %g.\n"
    ...
    if (!avctx->extradata) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to allocate extradata.\n");
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    avctx->extradata_size = header_size;

    opus->samples = av_em_mallocz_array(frame_size, avctx->channels *
                               av_em_get_bytes_per_sample(avctx->sample_fmt));
    if (!opus->samples) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to allocate samples buffer.\n");
        ret = AVERROR(ENOMEM);
        goto fail;
    }
...
static int libopus_encode(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                          const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, (1275 * 3 + 7) * opus->stream_count, 0)) < 0)
        return ret;
    ...
    if (discard_padding > 0) {
        uint8_t* side_data = av_em_packet_new_side_data(avpkt,
...
static av_cold int libopus_encode_close(AVEMCodecContext *avctx)
{
    LibopusEncContext *opus = avctx->priv_data;
...
static const AVEMClass libopus_class = {
    .class_name = "libopus",
...
static const AVEMCodecDefault libopus_defaults[] = {
    { "b",                 "0" },
