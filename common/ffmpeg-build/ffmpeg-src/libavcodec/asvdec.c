...
{
    return em_reverse[get_bits(gb, n) << (8 - n)];
}
    ...
    for (i = 0; i < 11; i++) {
        ...
        if (ccp) {
            if (ccp == 16)
                break;
            if (ccp < 0 || i >= 10) {
                av_em_log(a->avctx, AV_LOG_ERROR, "coded coeff pattern damaged\n");
                        ...
                        AVEMPacket *avpkt)
{
    ...
    av_em_fast_padded_malloc(&a->bitstream_buffer, &a->bitstream_buffer_size,
                          buf_size);
    ...
    if (avctx->codec_id == AV_CODEC_ID_ASV1) {
        ...
        for (i = 0; i < buf_size; i++)
            a->bitstream_buffer[i] = em_reverse[buf[i]];
    }
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->extradata_size < 1) {
        av_em_log(avctx, AV_LOG_WARNING, "No extradata provided\n");
    }

    ff_asv_common_init(avctx);
    em_blockdsp_init(&a->bdsp, avctx);
    ...
    if (avctx->extradata_size < 1 || (a->inv_qscale = avctx->extradata[0]) == 0) {
        av_em_log(avctx, AV_LOG_ERROR, "illegal qscale 0\n");
        if (avctx->codec_id == AV_CODEC_ID_ASV1)
            a->inv_qscale = 6;
        else
            a->inv_qscale = 10;
...
static av_cold int decode_end(AVEMCodecContext *avctx)
{
    ...
    av_em_freep(&a->bitstream_buffer);
