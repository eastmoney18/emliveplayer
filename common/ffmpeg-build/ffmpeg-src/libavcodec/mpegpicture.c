...
do {\
    if (pic->table &&\
       (ret = av_em_buffer_make_writable(&pic->table)) < 0)\
    return ret;\
} while (0)
...
int ff_mpeg_framesize_alloc(AVEMCodecContext *avctx, MotionEstContext *me,
                            ScratchpadContext *sc, int linesize)
{
    ...
    if (linesize < 24) {
        av_em_log(avctx, AV_LOG_ERROR, "Image too small, temporary buffers cannot function\n");
    ...
    return 0;
fail:
    av_em_freep(&sc->edge_emu_buffer);
 ...
 */
static int alloc_frame_buffer(AVEMCodecContext *avctx,  Picture *pic,
                              MotionEstContext *me, ScratchpadContext *sc,
                              int chroma_x_shift, int chroma_y_shift,
                              int linesize, int uvlinesize)
{
    int edges_needed = av_em_codec_is_encoder(avctx->codec);
    int r, ret;
    ...
    if (avctx->codec_id != AV_CODEC_ID_WMV3IMAGE &&
        ...
        r = em_thread_get_buffer(avctx, &pic->tf,
                                 pic->reference ? AV_GET_BUFFER_FLAG_REF : 0);
    } else {
        pic->f->width  = avctx->width;
        pic->f->height = avctx->height;
        pic->f->format = avctx->pix_fmt;
        r = avcodec_em_default_get_buffer2(avctx, pic->f, 0);
    }

    if (r < 0 || !pic->f->buf[0]) {
        av_em_log(avctx, AV_LOG_ERROR, "get_buffer() failed (%d %p)\n",
               r, pic->f->data[0]);
        return -1;
    }
    ...
    if (avctx->hwaccel) {
        ...
        if (avctx->hwaccel->frame_priv_data_size) {
            pic->hwaccel_priv_buf = av_em_buffer_allocz(avctx->hwaccel->frame_priv_data_size);
            if (!pic->hwaccel_priv_buf) {
                av_em_log(avctx, AV_LOG_ERROR, "alloc_frame_buffer() failed (hwaccel private data allocation)\n");
                return -1;
            }
...
static int alloc_picture_tables(AVEMCodecContext *avctx, Picture *pic, int encoding, int out_format,
                                int mb_stride, int mb_width, int mb_height, int b8_stride)
{
    ...
    pic->mbskip_table_buf = av_em_buffer_allocz(mb_array_size + 2);
    pic->qscale_table_buf = av_em_buffer_allocz(big_mb_num + mb_stride);
    pic->mb_type_buf      = av_em_buffer_allocz((big_mb_num + mb_stride) *
                                             sizeof(uint32_t));
    if (!pic->mbskip_table_buf || !pic->qscale_table_buf || !pic->mb_type_buf)
        return AVERROR(ENOMEM);

    if (encoding) {
        pic->mb_var_buf    = av_em_buffer_allocz(mb_array_size * sizeof(int16_t));
        pic->mc_mb_var_buf = av_em_buffer_allocz(mb_array_size * sizeof(int16_t));
        pic->mb_mean_buf   = av_em_buffer_allocz(mb_array_size);
    ...
    if (out_format == FMT_H263 || encoding || avctx->debug_mv ||
        ...
        for (i = 0; mv_size && i < 2; i++) {
            pic->motion_val_buf[i] = av_em_buffer_allocz(mv_size);
            pic->ref_index_buf[i]  = av_em_buffer_allocz(ref_index_size);
 ...
 */
int em_alloc_picture(AVEMCodecContext *avctx, Picture *pic, MotionEstContext *me,
                     ...
                     ptrdiff_t *linesize, ptrdiff_t *uvlinesize)
{
    ...
    return 0;
fail:
    av_em_log(avctx, AV_LOG_ERROR, "Error allocating a picture.\n");
    ff_mpeg_unref_picture(avctx, pic);
    ff_free_picture_tables(pic);
    return AVERROR(ENOMEM);
}
 ...
 */
void ff_mpeg_unref_picture(AVEMCodecContext *avctx, Picture *pic)
{
    ...
    if (avctx->codec_id != AV_CODEC_ID_WMV3IMAGE &&
        avctx->codec_id != AV_CODEC_ID_VC1IMAGE  &&
        avctx->codec_id != AV_CODEC_ID_MSS2)
        em_thread_release_buffer(avctx, &pic->tf);
    else if (pic->f)
        av_em_frame_unref(pic->f);

    av_em_buffer_unref(&pic->hwaccel_priv_buf);
...
do {                                                                          \
    if (src->table &&                                                         \
        ...
        av_em_buffer_unref(&dst->table);                                         \
        dst->table = av_em_buffer_ref(src->table);                               \
        if (!dst->table) {                                                    \
...
int ff_mpeg_ref_picture(AVEMCodecContext *avctx, Picture *dst, Picture *src)
{
    int ret;
...
static int find_unused_picture(AVEMCodecContext *avctx, Picture *picture, int shared)
{
    int i;
...
int ff_find_unused_picture(AVEMCodecContext *avctx, Picture *picture, int shared)
{
...
void ff_free_picture_tables(Picture *pic)
{
    ...
    av_em_buffer_unref(&pic->mb_var_buf);
    av_em_buffer_unref(&pic->mc_mb_var_buf);
    av_em_buffer_unref(&pic->mb_mean_buf);
    av_em_buffer_unref(&pic->mbskip_table_buf);
    av_em_buffer_unref(&pic->qscale_table_buf);
