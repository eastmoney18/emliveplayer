...
static int gif_read_image(GifState *s, AVFrame *frame)
{
    ...
    if (has_local_palette) {
        ...
        if (!s->has_global_palette) {
            av_em_log(s->avctx, AV_LOG_ERROR, "picture doesn't have either global or local palette.\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    if (!width || width > s->screen_width || left >= s->screen_width) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid image width.\n");
        return AVERROR_INVALIDDATA;
    }
    if (!height || height > s->screen_height || top >= s->screen_height) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid image height.\n");
        return AVERROR_INVALIDDATA;
    }
    if (left + width > s->screen_width) {
        /* width must be kept around to avoid lzw vs line desync */
        pw = s->screen_width - left;
        av_em_log(s->avctx, AV_LOG_WARNING, "Image too wide by %d, truncating.\n",
    ...
    if (top + height > s->screen_height) {
        ...
        av_em_log(s->avctx, AV_LOG_WARNING, "Image too high by %d, truncating.\n",
    ...
    if (s->gce_disposal != GCE_DISPOSAL_NONE) {
        ...
        if (s->gce_disposal == GCE_DISPOSAL_BACKGROUND) {
            ...
            av_em_fast_malloc(&s->stored_img, &s->stored_img_size, frame->linesize[0] * frame->height);
            if (!s->stored_img)
    ...
    if ((ret = ff_lzw_decode_init(s->lzw, code_size, s->gb.buffer,
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "LZW init failed\n");
        return ret;
    }
    ...
    for (y = 0; y < height; y++) {
        ...
        if (count != width) {
            if (count)
                av_em_log(s->avctx, AV_LOG_ERROR, "LZW decode failed\n");
            goto decode_tail;
        }
...
static int gif_parse_next_image(GifState *s, AVFrame *frame)
{
    while (bytestream2_get_bytes_left(&s->gb) > 0) {
        ...
        av_em_log(s->avctx, AV_LOG_DEBUG, "code=%02x '%c'\n", code, code);
...
static av_cold int gif_decode_init(AVEMCodecContext *avctx)
{
...
static int gif_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame, AVEMPacket *avpkt)
{
    GifState *s = avctx->priv_data;
    int ret;
    ...
    av_em_frame_set_pkt_duration(s->frame, avpkt->duration);
    ...
    if (s->keyframe) {
        ...
        if ((ret = em_set_dimensions(avctx, s->screen_width, s->screen_height)) < 0)
            return ret;

        av_em_frame_unref(s->frame);
        if ((ret = em_get_buffer(avctx, s->frame, 0)) < 0)
            return ret;

        av_em_fast_malloc(&s->idx_line, &s->idx_line_size, s->screen_width);
        if (!s->idx_line)
            return AVERROR(ENOMEM);
        ...
        if (!s->keyframe_ok) {
            av_em_log(avctx, AV_LOG_ERROR, "cannot decode frame without keyframe\n");
            return AVERROR_INVALIDDATA;
        }
...
static av_cold int gif_decode_close(AVEMCodecContext *avctx)
{
    GifState *s = avctx->priv_data;

    ff_lzw_decode_close(&s->lzw);
    av_em_frame_free(&s->frame);
    av_em_freep(&s->idx_line);
    av_em_freep(&s->stored_img);
...
static const AVEMClass decoder_class = {
