...
static av_cold int encode_init(AVEMCodecContext *avctx)
{
    ...
    case AV_PIX_FMT_RGB48BE:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_INFO, "unsupported pixel format\n");
        return -1;
    }
...
static void encode_rgb48_10bit(AVEMCodecContext *avctx, const AVFrame *pic,
                               uint8_t *dst)
{
    DPXContext *s = avctx->priv_data;
...
static void encode_gbrp10(AVEMCodecContext *avctx, const AVFrame *pic, uint8_t *dst)
{
    DPXContext *s = avctx->priv_data;
...
static void encode_gbrp12(AVEMCodecContext *avctx, const AVFrame *pic, uint16_t *dst)
{
    DPXContext *s = avctx->priv_data;
...
#define HEADER_SIZE 1664  /* DPX Generic header */
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, size + HEADER_SIZE, 0)) < 0)
        return ret;
    buf = pkt->data;
    ...
    case 16:
        if (need_align) {
            ...
            size = av_em_image_copy_to_buffer(buf + HEADER_SIZE, pkt->size - HEADER_SIZE,
