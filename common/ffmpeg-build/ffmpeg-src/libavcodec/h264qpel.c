...
av_cold void em_h264qpel_init(H264QpelContext *c, int bit_depth)
{
#undef FUNCC
...
#define SET_QPEL(depth)                         \
    ...
    if (ARCH_AARCH64)
        em_h264qpel_init_aarch64(c, bit_depth);
    if (ARCH_ARM)
        em_h264qpel_init_arm(c, bit_depth);
    if (ARCH_PPC)
        em_h264qpel_init_ppc(c, bit_depth);
    if (ARCH_X86)
        em_h264qpel_init_x86(c, bit_depth);
    if (ARCH_MIPS)
        em_h264qpel_init_mips(c, bit_depth);
}
