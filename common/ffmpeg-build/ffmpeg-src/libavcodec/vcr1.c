...
static av_cold int vcr1_decode_init(AVEMCodecContext *avctx)
{
    avctx->pix_fmt = AV_PIX_FMT_YUV410P;

    if (avctx->width % 8 || avctx->height%4) {
        avpriv_em_request_sample(avctx, "odd dimensions (%d x %d) support", avctx->width, avctx->height);
...
static int vcr1_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_frame, AVEMPacket *avpkt)
{
    ...
    if(avpkt->size < 32 + avctx->height + avctx->width*avctx->height*5/8){
        av_em_log(avctx, AV_LOG_ERROR, "Insufficient input data. %d < %d\n", avpkt->size ,  32 + avctx->height + avctx->width*avctx->height*5/8);
