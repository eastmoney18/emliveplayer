...
static int dirac_combine_frame(AVEMCodecParserContext *s, AVEMCodecContext *avctx,
                               int next, const uint8_t **buf, int *buf_size)
{
    ...
    if (next == -1) {
        ...
        void *new_buffer =
            av_em_fast_realloc(pc->buffer, &pc->buffer_size,
...
static int dirac_parse(AVEMCodecParserContext *s, AVEMCodecContext *avctx,
...
static void dirac_parse_close(AVEMCodecParserContext *s)
{
    DiracParseContext *pc = s->priv_data;
...
AVEMCodecParser ff_dirac_parser = {
