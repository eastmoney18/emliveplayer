...
typedef struct DCAContext {
    const AVEMClass   *class;       ///< class for AVOptions
...
static inline int ff_dca_check_crc(AVEMCodecContext *avctx, GetBitContext *s,
                                   int p1, int p2)
{
    ...
    if (((p1 | p2) & 7) || p1 < 0 || p2 > s->size_in_bits || p2 - p1 < 16)
        return -1;
    if (av_em_crc(dca->crctab, 0xffff, s->buffer + p1 / 8, (p2 - p1) / 8))
        return -1;
    return 0;
}
