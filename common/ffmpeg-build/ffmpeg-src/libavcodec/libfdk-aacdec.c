...
static const AVEMClass fdk_aac_dec_class = {
    "libfdk-aac decoder", av_em_default_item_name, fdk_aac_dec_options, LIBAVUTIL_VERSION_INT
};

static int get_stream_info(AVEMCodecContext *avctx)
{
    ...
    if (!info) {
        av_em_log(avctx, AV_LOG_ERROR, "Unable to get stream info\n");
        return AVERROR_UNKNOWN;
    }

    if (info->sampleRate <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Stream info not initialized\n");
        return AVERROR_UNKNOWN;
    }
    ...
    for (i = 0; i < info->numChannels; i++) {
        ...
        if (ctype <= ACT_NONE || ctype >= FF_ARRAY_ELEMS(channel_counts)) {
            av_em_log(avctx, AV_LOG_WARNING, "unknown channel type\n");
            break;
        }
    ...
    if (!ch_error &&
        av_em_get_channel_layout_nb_channels(ch_layout) != info->numChannels) {
        av_em_log(avctx, AV_LOG_WARNING, "unsupported channel configuration\n");
        ch_error = 1;
    }
    if (ch_error)
...
static av_cold int fdk_aac_decode_close(AVEMCodecContext *avctx)
{
    FDKAACDecContext *s = avctx->priv_data;

    if (s->handle)
        aacDecoder_Close(s->handle);
    av_em_freep(&s->decoder_buffer);
    av_em_freep(&s->anc_buffer);

    return 0;
}

static av_cold int fdk_aac_decode_init(AVEMCodecContext *avctx)
{
    FDKAACDecContext *s = avctx->priv_data;
    ...
    if (!s->handle) {
        av_em_log(avctx, AV_LOG_ERROR, "Error opening decoder\n");
        return AVERROR_UNKNOWN;
    }
    ...
    if (avctx->extradata_size) {
        if ((err = aacDecoder_ConfigRaw(s->handle, &avctx->extradata,
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Unable to set extradata\n");
    ...
    if ((err = aacDecoder_SetParam(s->handle, AAC_CONCEAL_METHOD,
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Unable to set error concealment method\n");
        return AVERROR_UNKNOWN;
    }
    ...
    if (avctx->request_channel_layout > 0 &&
        ...
        case AV_CH_LAYOUT_MONO:
            ...
            break;
        default:
            av_em_log(avctx, AV_LOG_WARNING, "Invalid request_channel_layout\n");
            break;
        }
        ...
        if (downmix_channels != -1) {
            if (aacDecoder_SetParam(s->handle, AAC_PCM_MAX_OUTPUT_CHANNELS,
               ...
               av_em_log(avctx, AV_LOG_WARNING, "Unable to set output channels in the decoder\n");
            } else {
               s->anc_buffer = av_em_alloc(DMX_ANC_BUFFSIZE);
               if (!s->anc_buffer) {
                   av_em_log(avctx, AV_LOG_ERROR, "Unable to allocate ancillary buffer for the decoder\n");
                   return AVERROR(ENOMEM);
               }
               if (aacDecoder_AncDataInit(s->handle, s->anc_buffer, DMX_ANC_BUFFSIZE)) {
                   av_em_log(avctx, AV_LOG_ERROR, "Unable to register downmix ancillary buffer in the decoder\n");
                   return AVERROR_UNKNOWN;
               }
            }
        }
    ...
    if (s->drc_boost != -1) {
        if (aacDecoder_SetParam(s->handle, AAC_DRC_BOOST_FACTOR, s->drc_boost) != AAC_DEC_OK) {
            av_em_log(avctx, AV_LOG_ERROR, "Unable to set DRC boost factor in the decoder\n");
            return AVERROR_UNKNOWN;
        }
    }
    ...
    if (s->drc_cut != -1) {
        if (aacDecoder_SetParam(s->handle, AAC_DRC_ATTENUATION_FACTOR, s->drc_cut) != AAC_DEC_OK) {
            av_em_log(avctx, AV_LOG_ERROR, "Unable to set DRC attenuation factor in the decoder\n");
            return AVERROR_UNKNOWN;
        }
    }
    ...
    if (s->drc_level != -1) {
        if (aacDecoder_SetParam(s->handle, AAC_DRC_REFERENCE_LEVEL, s->drc_level) != AAC_DEC_OK) {
            av_em_log(avctx, AV_LOG_ERROR, "Unable to set DRC reference level in the decoder\n");
            return AVERROR_UNKNOWN;
        }
    }
    ...
    if (s->drc_heavy != -1) {
        if (aacDecoder_SetParam(s->handle, AAC_DRC_HEAVY_COMPRESSION, s->drc_heavy) != AAC_DEC_OK) {
            av_em_log(avctx, AV_LOG_ERROR, "Unable to set DRC heavy compression in the decoder\n");
            return AVERROR_UNKNOWN;
        }
    }
    ...
    if (aacDecoder_SetParam(s->handle, AAC_PCM_LIMITER_ENABLE, s->level_limit) != AAC_DEC_OK) {
        av_em_log(avctx, AV_LOG_ERROR, "Unable to set in signal level limiting in the decoder\n");
        return AVERROR_UNKNOWN;
    }
#endif
    ...
    s->decoder_buffer = av_em_alloc(s->decoder_buffer_size);
    if (!s->decoder_buffer)
        return AVERROR(ENOMEM);
...
static int fdk_aac_decode_frame(AVEMCodecContext *avctx, void *data,
                                int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (err != AAC_DEC_OK) {
        av_em_log(avctx, AV_LOG_ERROR, "aacDecoder_Fill() failed: %x\n", err);
        return AVERROR_INVALIDDATA;
    }
    ...
    memcpy(frame->extended_data[0], s->decoder_buffer,
           ...
           av_em_get_bytes_per_sample(avctx->sample_fmt));
...
static av_cold void fdk_aac_decode_flush(AVEMCodecContext *avctx)
{
    FDKAACDecContext *s = avctx->priv_data;
    AAC_DECODER_ERROR err;
    ...
    if ((err = aacDecoder_SetParam(s->handle,
        ...
        av_em_log(avctx, AV_LOG_WARNING, "failed to clear buffer when flushing\n");
}

AVEMCodec ff_libfdk_aac_decoder = {
    .name           = "libfdk_aac",
