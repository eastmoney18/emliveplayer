...
typedef struct APEContext {
    AVEMClass *class;                          ///< class for AVOptions
...
static av_cold int ape_decode_close(AVEMCodecContext *avctx)
{
    APEContext *s = avctx->priv_data;
    int i;

    for (i = 0; i < APE_FILTER_LEVELS; i++)
        av_em_freep(&s->filterbuf[i]);
...
static av_cold int ape_decode_init(AVEMCodecContext *avctx)
{
    APEContext *s = avctx->priv_data;
    int i;

    if (avctx->extradata_size != 6) {
        av_em_log(avctx, AV_LOG_ERROR, "Incorrect extradata\n");
        return AVERROR(EINVAL);
    }
    if (avctx->channels > 2) {
        av_em_log(avctx, AV_LOG_ERROR, "Only mono and stereo is supported\n");
        return AVERROR(EINVAL);
    }
    ...
    av_em_log(avctx, AV_LOG_VERBOSE, "Compression Level: %d - Flags: %d\n",
           s->compression_level, s->flags);
    if (s->compression_level % 1000 || s->compression_level > COMPRESSION_LEVEL_INSANE ||
        !s->compression_level ||
        (s->fileversion < 3930 && s->compression_level == COMPRESSION_LEVEL_INSANE)) {
        av_em_log(avctx, AV_LOG_ERROR, "Incorrect compression level %d\n",
               s->compression_level);
                                        ...
                                        APERice *rice)
{
    ...
    if (!rice->k)
        ...
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Too many bits: %d\n", rice->k);
        return AVERROR_INVALIDDATA;
    }
...
static void ape_unpack_mono(APEContext *ctx, int count)
{
    if (ctx->frameflags & APE_FRAMECODE_STEREO_SILENCE) {
        ...
        av_em_log(ctx->avctx, AV_LOG_DEBUG, "pure silence mono\n");
        return;
    }
...
static void ape_unpack_stereo(APEContext *ctx, int count)
{
    ...
    if ((ctx->frameflags & APE_FRAMECODE_STEREO_SILENCE) == APE_FRAMECODE_STEREO_SILENCE) {
        /* We are pure silence, so we're done. */
        av_em_log(ctx->avctx, AV_LOG_DEBUG, "pure silence stereo\n");
        return;
    }
...
static int ape_decode_frame(AVEMCodecContext *avctx, void *data,
                            int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if(!s->samples){
        ...
        if (buf_size != avpkt->size) {
            av_em_log(avctx, AV_LOG_WARNING, "packet size is not a multiple of 4. "
                   "extra bytes at the end will be skipped.\n");
        }
        if (s->fileversion < 3950) // previous versions overread two bytes
            buf_size += 2;
        av_em_fast_padded_malloc(&s->data, &s->data_size, buf_size);
        if (!s->data)
            return AVERROR(ENOMEM);
        ...
        if (s->fileversion >= 3900) {
            if (offset > 3) {
                av_em_log(avctx, AV_LOG_ERROR, "Incorrect offset passed\n");
                s->data = NULL;
                return AVERROR_INVALIDDATA;
            }
        ...
        if (!nblocks || nblocks > INT_MAX) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid sample count: %"PRIu32".\n",
                   nblocks);
            return AVERROR_INVALIDDATA;
        }
        ...
        if (init_frame_decoder(s) < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Error reading frame header\n");
            return AVERROR_INVALIDDATA;
        }
        s->samples = nblocks;
    }
    ...
    av_em_fast_malloc(&s->decoded_buffer, &s->decoded_size,
    ...
    if (s->error) {
        s->samples=0;
        av_em_log(avctx, AV_LOG_ERROR, "Error decoding frame\n");
        return AVERROR_INVALIDDATA;
    }
...
static void ape_flush(AVEMCodecContext *avctx)
{
    APEContext *s = avctx->priv_data;
    s->samples= 0;
}
...
static const AVEMClass ape_decoder_class = {
