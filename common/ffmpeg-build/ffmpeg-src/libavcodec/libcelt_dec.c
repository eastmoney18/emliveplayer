...
static av_cold int libcelt_dec_init(AVEMCodecContext *c)
{
...
static av_cold int libcelt_dec_close(AVEMCodecContext *c)
{
    struct libcelt_context *celt = c->priv_data;
...
static int libcelt_dec_decode(AVEMCodecContext *c, void *data,
                              int *got_frame_ptr, AVEMPacket *pkt)
{
    ...
    if ((err = em_get_buffer(c, frame, 0)) < 0)
        return err;
