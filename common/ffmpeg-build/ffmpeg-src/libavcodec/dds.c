...
static int parse_pixel_format(AVEMCodecContext *avctx)
{
    ...
    if (size != 32) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid pixel format header %d.\n", size);
    ...
    av_em_log(avctx, AV_LOG_VERBOSE, "fourcc %s bpp %d "
           "r 0x%x g 0x%x b 0x%x a 0x%x\n", buf, bpp, r, g, b, a);
    if (gimp_tag) {
        av_em_get_codec_tag_string(buf, sizeof(buf), gimp_tag);
        av_em_log(avctx, AV_LOG_VERBOSE, "and GIMP-DDS tag %s\n", buf);
    }
    ...
    if (ctx->compressed) {
        ...
        case MKTAG('D', 'X', '1', '0'):
            ...
            av_em_log(avctx, AV_LOG_VERBOSE, "DXGI format %d.\n", dxgi);
            switch (dxgi) {
            /* RGB types. */
            ...
            break;
        default:
            av_em_log(avctx, AV_LOG_ERROR, "Unsupported %s fourcc.\n", buf);
            return AVERROR_INVALIDDATA;
        }
        ...
        if (bpp == 8) {
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Unsupported palette bpp %d.\n", bpp);
            return AVERROR_INVALIDDATA;
        }
    } else {
        /*  8 bpp */
        ...
        /* give up */
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Unknown pixel format "
...
static int decompress_texture_thread(AVEMCodecContext *avctx, void *arg,
...
static void run_postproc(AVEMCodecContext *avctx, AVFrame *frame)
{
    DDSContext *ctx = avctx->priv_data;
    int i, x_off;
    ...
    case DDS_ALPHA_EXP:
         ...
         * alpha channel. */
        av_em_log(avctx, AV_LOG_DEBUG, "Post-processing alpha exponent.\n");
    ...
    case DDS_NORMAL_MAP:
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Post-processing normal map.\n");
    ...
    case DDS_RAW_YCOCG:
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Post-processing raw YCoCg.\n");
    ...
    case DDS_SWAP_ALPHA:
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Post-processing swapped Luma/Alpha.\n");
    ...
    case DDS_SWIZZLE_A2XY:
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Post-processing A2XY swizzle.\n");
        do_swizzle(frame, 0, 1);
        break;
    case DDS_SWIZZLE_RBXG:
        /* Swap G and A, then B and new A (G). */
        av_em_log(avctx, AV_LOG_DEBUG, "Post-processing RBXG swizzle.\n");
        do_swizzle(frame, 1, 3);
        do_swizzle(frame, 2, 3);
        break;
    case DDS_SWIZZLE_RGXB:
        /* Swap B and A. */
        av_em_log(avctx, AV_LOG_DEBUG, "Post-processing RGXB swizzle.\n");
        do_swizzle(frame, 2, 3);
        break;
    case DDS_SWIZZLE_RXBG:
        /* Swap G and A. */
        av_em_log(avctx, AV_LOG_DEBUG, "Post-processing RXBG swizzle.\n");
        do_swizzle(frame, 1, 3);
        break;
    case DDS_SWIZZLE_RXGB:
        /* Swap R and A (misleading name). */
        av_em_log(avctx, AV_LOG_DEBUG, "Post-processing RXGB swizzle.\n");
        do_swizzle(frame, 0, 3);
        break;
    case DDS_SWIZZLE_XGBR:
        /* Swap B and A, then R and new A (B). */
        av_em_log(avctx, AV_LOG_DEBUG, "Post-processing XGBR swizzle.\n");
        do_swizzle(frame, 2, 3);
        do_swizzle(frame, 0, 3);
        break;
    case DDS_SWIZZLE_XGXR:
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Post-processing XGXR swizzle.\n");
        do_swizzle(frame, 1, 3);
        do_swizzle(frame, 0, 3);
        do_swizzle(frame, 0, 1);
        break;
    case DDS_SWIZZLE_XRBG:
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Post-processing XRBG swizzle.\n");
        do_swizzle(frame, 1, 3);
        do_swizzle(frame, 0, 3);
        break;
    }
...
static int dds_decode(AVEMCodecContext *avctx, void *data,
                      int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (bytestream2_get_bytes_left(gbc) < 128) {
        av_em_log(avctx, AV_LOG_ERROR, "Frame is too small (%d).\n",
    ...
    if (bytestream2_get_le32(gbc) != MKTAG('D', 'D', 'S', ' ') ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Invalid DDS header.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid image size %dx%d.\n",
    ...
    if (mipmap != 0)
        av_em_log(avctx, AV_LOG_VERBOSE, "Found %d mipmaps (ignored).\n", mipmap);
    ...
    if (ctx->compressed) {
        ...
        int linesize = av_em_image_get_linesize(avctx->pix_fmt, frame->width, 0);
        ...
        if (bytestream2_get_bytes_left(gbc) < frame->height * linesize) {
            av_em_log(avctx, AV_LOG_ERROR, "Buffer is too small (%d < %d).\n",
                   bytestream2_get_bytes_left(gbc), frame->height * linesize);
            return AVERROR_INVALIDDATA;
        }

        av_em_image_copy_plane(frame->data[0], frame->linesize[0],
                            gbc->buffer, linesize,
