...
typedef struct MagicYUVContext {
    ...
    uint8_t             *buf;           // pointer to AVEMPacket->data
    int                 hshift[4];
    int                 vshift[4];
...
static int decode_slice(AVEMCodecContext *avctx, void *tdata,
                        int j, int threadnr)
{
    MagicYUVContext *s = avctx->priv_data;
    ...
    for (i = 0; i < s->planes; i++) {
        ...
        if (pred == LEFT) {
            ...
            avpriv_em_request_sample(avctx, "unknown prediction: %d", pred);
        }
    }
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (version != 7) {
        avpriv_em_request_sample(avctx, "unsupported version: %d", version);
    ...
    case 0x6b:
        ...
        break;
    default:
        avpriv_em_request_sample(avctx, "unsupported format: 0x%X", format);
        return AVERROR_PATCHWELCOME;
    }
    ...
    if ((ret = em_set_dimensions(avctx, width, height)) < 0)
        return ret;

    slice_width = bytestream2_get_le32(&gb);
    if (slice_width != avctx->coded_width) {
        avpriv_em_request_sample(avctx, "unsupported slice width: %d", slice_width);
        return AVERROR_PATCHWELCOME;
    }
    s->slice_height = bytestream2_get_le32(&gb);
    if ((s->slice_height <= 0) || (s->slice_height > INT_MAX - avctx->coded_height)) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid slice height: %d\n", s->slice_height);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->nb_slices > INT_MAX / sizeof(Slice)) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid number of slices: %d\n", s->nb_slices);
        return AVERROR_INVALIDDATA;
    }

    for (i = 0; i < s->planes; i++) {
        av_em_fast_malloc(&s->slices[i], &s->slices_size[i], s->nb_slices * sizeof(Slice));
        if (!s->slices[i])
    ...
    while (get_bits_left(&b) >= 8) {
        ...
        if (j == 256) {
            ...
            if (build_huff(&s->vlc[i], s->len[i])) {
                av_em_log(avctx, AV_LOG_ERROR, "Cannot build Huffman codes\n");
                return AVERROR_INVALIDDATA;
            }
            i++;
            if (i == s->planes) {
    ...
    if (i != s->planes) {
        av_em_log(avctx, AV_LOG_ERROR, "Huffman tables too short\n");
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int decode_end(AVEMCodecContext *avctx)
{
    ...
    av_em_freep(&s->slices[0]);
    av_em_freep(&s->slices[1]);
    av_em_freep(&s->slices[2]);
    av_em_freep(&s->slices[3]);
    s->slices_size[0] = 0;
    s->slices_size[1] = 0;
    s->slices_size[2] = 0;
    s->slices_size[3] = 0;
