...
void em_h264_direct_dist_scale_factor(const H264EMContext *const h,
                                      H264SliceContext *sl)
{
...
static void fill_colmap(const H264EMContext *h, H264SliceContext *sl,
...
void em_h264_direct_ref_list_init(const H264EMContext *const h, H264SliceContext *sl)
{
    ...
    if (h->picture_structure == PICT_FRAME) {
        ...
        if (col_poc[0] == INT_MAX && col_poc[1] == INT_MAX) {
            av_em_log(h->avctx, AV_LOG_ERROR, "co located POCs unavailable\n");
            sl->col_parity = 1;
        } else
...
static void await_reference_mb_row(const H264EMContext *const h, H264Ref *ref,
                                   int mb_y)
{
    ...
    em_thread_await_progress(&ref->parent->tf,
                             FFMIN(16 * mb_y >> ref_field_picture,
                                   ref_height - 1),
                             ref_field_picture && ref_field);
}

static void pred_spatial_direct_motion(const H264EMContext *const h, H264SliceContext *sl,
                                       int *mb_type)
{
    int b8_stride = 2;
...
static void pred_temp_direct_motion(const H264EMContext *const h, H264SliceContext *sl,
                                    int *mb_type)
{
    int b8_stride = 2;
    int b4_stride = h->b_stride;
