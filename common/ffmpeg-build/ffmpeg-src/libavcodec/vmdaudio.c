...
static av_cold int vmdaudio_decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->block_align < 1 || avctx->block_align % avctx->channels) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid block align\n");
        return AVERROR(EINVAL);
    }
    ...
    s->out_bps = av_em_get_bytes_per_sample(avctx->sample_fmt);

    s->chunk_size = avctx->block_align + avctx->channels * (s->out_bps == 2);

    av_em_log(avctx, AV_LOG_DEBUG, "%d channels, %d bits/sample, "
...
static int vmdaudio_decode_frame(AVEMCodecContext *avctx, void *data,
                                 int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (buf_size < 16) {
        av_em_log(avctx, AV_LOG_WARNING, "skipping small junk packet\n");
        *got_frame_ptr = 0;
        return buf_size;
    }
    ...
    if (block_type < BLOCK_TYPE_AUDIO || block_type > BLOCK_TYPE_SILENCE) {
        av_em_log(avctx, AV_LOG_ERROR, "unknown block type: %d\n", block_type);
        return AVERROR(EINVAL);
    }
    buf      += 16;
    buf_size -= 16;
