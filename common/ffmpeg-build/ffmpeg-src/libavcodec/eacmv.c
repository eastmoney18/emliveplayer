...
static av_cold int cmv_decode_init(AVEMCodecContext *avctx){
    ...
    s->last2_frame = av_em_frame_alloc();
    if (!s->last_frame || !s->last2_frame) {
        av_em_frame_free(&s->last_frame);
        av_em_frame_free(&s->last2_frame);
...
static int cmv_process_header(CmvContext *s, const uint8_t *buf, const uint8_t *buf_end)
{
    ...
    if(buf_end - buf < 16) {
        av_em_log(s->avctx, AV_LOG_WARNING, "truncated header\n");
    ...
    if (s->width  != s->avctx->width ||
        ...
        av_em_frame_unref(s->last2_frame);
    }
    ...
    fps = AV_RL16(&buf[10]);
    if (fps > 0)
        s->avctx->framerate = (AVEMRational){ fps, 1 };
...
static int cmv_decode_frame(AVEMCodecContext *avctx,
                            ...
                            AVEMPacket *avpkt)
{
    ...
    av_em_frame_unref(s->last2_frame);
    av_em_frame_move_ref(s->last2_frame, s->last_frame);
...
static av_cold int cmv_decode_end(AVEMCodecContext *avctx){
    CmvContext *s = avctx->priv_data;
