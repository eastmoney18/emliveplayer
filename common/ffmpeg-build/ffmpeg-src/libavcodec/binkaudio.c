...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    s->bands = av_em_alloc((s->num_bands + 1) * sizeof(*s->bands));
    if (!s->bands)
                        ...
                        int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (!get_bits_left(gb)) {
        ...
        buf = av_em_realloc(s->packet_buffer, avpkt->size + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!buf)
            return AVERROR(ENOMEM);
    ...
    if (decode_block(s, (float **)frame->extended_data,
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Incomplete packet\n");
        return AVERROR_INVALIDDATA;
    }
    get_bits_align32(gb);
...
AVEMCodec ff_binkaudio_rdft_decoder = {
    .name           = "binkaudio_rdft",
...
AVEMCodec ff_binkaudio_dct_decoder = {
    .name           = "binkaudio_dct",
