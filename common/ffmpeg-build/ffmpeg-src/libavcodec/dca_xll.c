    ...
    for (i = 0; i < m; i++) {
        ...
        if (!c->primary_chset) {
            ...
            if (index >= FF_DCA_INV_DMIXTABLE_SIZE) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XLL downmix scale index\n");
        ...
        for (j = 0; j < c->nchannels; j++) {
            ...
            if (index >= FF_DCA_DMIXTABLE_SIZE) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XLL downmix coefficient index\n");
                return AVERROR_INVALIDDATA;
            }
...
static int chs_parse_header(DCAXllDecoder *s, DCAXllChSet *c, DCAExssAsset *asset)
{
    ...
    if (ff_dca_check_crc(s->avctx, &s->gb, header_pos, header_pos + header_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XLL sub-header checksum\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (c->nchannels > DCA_XLL_CHANNELS_MAX) {
        avpriv_em_request_sample(s->avctx, "%d XLL channels", c->nchannels);
    ...
    if (c->storage_bit_res != 16 && c->storage_bit_res != 24) {
        avpriv_em_request_sample(s->avctx, "%d-bit XLL storage resolution", c->storage_bit_res);
        return AVERROR_PATCHWELCOME;
    }

    if (c->pcm_bit_res > c->storage_bit_res) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid PCM bit resolution for XLL channel set (%d > %d)\n", c->pcm_bit_res, c->storage_bit_res);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (c->freq > 192000) {
        avpriv_em_request_sample(s->avctx, "%d Hz XLL sampling frequency", c->freq);
        return AVERROR_PATCHWELCOME;
    }

    // Sampling frequency modifier
    if (get_bits(&s->gb, 2)) {
        avpriv_em_request_sample(s->avctx, "XLL sampling frequency modifier");
        return AVERROR_PATCHWELCOME;
    }

    // Which replacement set this channel set is member of
    if (get_bits(&s->gb, 2)) {
        avpriv_em_request_sample(s->avctx, "XLL replacement set");
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (asset->one_to_one_map_ch_to_spkr) {
        ...
        if (c->primary_chset != (c == p)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "The first (and only) XLL channel set must be primary\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        if (c->dmix_coeffs_present && c->primary_chset) {
            ...
            if (c->dmix_type >= DCA_DMIX_TYPE_COUNT) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XLL primary channel set downmix type\n");
                return AVERROR_INVALIDDATA;
            }
        }
        ...
        if (!c->hier_chset && s->nchsets != 1) {
            avpriv_em_request_sample(s->avctx, "XLL channel set outside of hierarchy");
            return AVERROR_PATCHWELCOME;
        }
        ...
        if (!get_bits1(&s->gb)) {
            avpriv_em_request_sample(s->avctx, "Disabled XLL channel mask");
            return AVERROR_PATCHWELCOME;
        }
        ...
        if (av_popcount(c->ch_mask) != c->nchannels) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XLL channel mask\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        if (c->nchannels != 2 || s->nchsets != 1 || get_bits1(&s->gb)) {
            avpriv_em_request_sample(s->avctx, "Custom XLL channel to speaker mapping");
            return AVERROR_PATCHWELCOME;
        }
    ...
    if (c->freq > 96000) {
        ...
        if (get_bits1(&s->gb)) {
            avpriv_em_request_sample(s->avctx, "Extra XLL frequency bands");
            return AVERROR_PATCHWELCOME;
        }
        c->nfreqbands = 2;
    } else {
    ...
    if (c != p && (c->nfreqbands != p->nfreqbands || c->freq != p->freq
        ...
        avpriv_em_request_sample(s->avctx, "Different XLL audio characteristics");
        return AVERROR_PATCHWELCOME;
    }
    ...
    for (band = 0, b = c->bands; band < c->nfreqbands; band++, b++) {
        ...
        if ((b->decor_enabled = get_bits1(&s->gb)) && c->nchannels > 1) {
            ...
            for (i = 0; i < c->nchannels; i++) {
                ...
                if (b->orig_order[i] >= c->nchannels) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XLL original channel order\n");
                    return AVERROR_INVALIDDATA;
                }
            }
        ...
        if (b->highest_pred_order > s->nsegsamples) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XLL adaptive predicition order\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        for (i = 0; i < c->nchannels; i++) {
            for (j = 0; j < b->adapt_pred_order[i]; j++) {
                k = get_linear(&s->gb, 8);
                if (k == -128) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XLL reflection coefficient index\n");
                    return AVERROR_INVALIDDATA;
                }
                if (k < 0)
        ...
        if ((band == 0 && s->scalable_lsbs) || (band != 0 && get_bits1(&s->gb))) {
            ...
            if (b->lsb_section_size < 0 || b->lsb_section_size > s->frame_size) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid LSB section size\n");
                return AVERROR_INVALIDDATA;
            }
            ...
            for (i = 0; i < c->nchannels; i++) {
                ...
                if (b->nscalablelsbs[i] && !b->lsb_section_size) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "LSB section missing with non-zero LSB width\n");
                    return AVERROR_INVALIDDATA;
                }
            }
        } else {
    ...
    if (ff_dca_seek_bits(&s->gb, header_pos + header_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of XLL sub-header\n");
        return AVERROR_INVALIDDATA;
    }
...
static int chs_alloc_msb_band_data(DCAXllDecoder *s, DCAXllChSet *c)
{
    ...
    av_em_fast_malloc(&c->sample_buffer[0], &c->sample_size[0], nsamples * sizeof(int32_t));
...
static int chs_alloc_lsb_band_data(DCAXllDecoder *s, DCAXllChSet *c)
{
    ...
    av_em_fast_malloc(&c->sample_buffer[1], &c->sample_size[1], nsamples * sizeof(int32_t));
...
static int chs_parse_band_data(DCAXllDecoder *s, DCAXllChSet *c, int band, int seg, int band_data_end)
{
            ...
            if (c->bitalloc_hybrid_linear[k]) {
                ...
                for (j = 0; j < nisosamples; j++) {
                    ...
                    if (loc >= nsamples_part_b) {
                        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid isolated sample location\n");
                        return AVERROR_INVALIDDATA;
                    }
                    part_b[loc] = -1;
                }
    ...
    if (b->lsb_section_size) {
        ...
        if (ff_dca_seek_bits(&s->gb, band_data_end - b->lsb_section_size * 8)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of XLL band data\n");
            return AVERROR_INVALIDDATA;
        }
...
static int chs_assemble_freq_bands(DCAXllDecoder *s, DCAXllChSet *c)
{
    ...
    av_em_fast_malloc(&c->sample_buffer[2], &c->sample_size[2],
...
static int parse_common_header(DCAXllDecoder *s)
{
    ...
    if (get_bits_long(&s->gb, 32) != DCA_SYNCWORD_XLL) {
        av_em_log(s->avctx, AV_LOG_VERBOSE, "Invalid XLL sync word\n");
    ...
    if (stream_ver > 1) {
        avpriv_em_request_sample(s->avctx, "XLL stream version %d", stream_ver);
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (ff_dca_check_crc(s->avctx, &s->gb, 32, header_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XLL common header checksum\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->frame_size < 0 || s->frame_size >= DCA_XLL_PBR_BUFFER_MAX) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid XLL frame size (%d bytes)\n", s->frame_size);
        return AVERROR_INVALIDDATA;
    }
    s->frame_size++;
    ...
    if (s->nchsets > DCA_XLL_CHSETS_MAX) {
        avpriv_em_request_sample(s->avctx, "%d XLL channel sets", s->nchsets);
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (s->nframesegs > 1024) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Too many segments per XLL frame\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (!s->nsegsamples_log2) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Too few samples per XLL segment\n");
        return AVERROR_INVALIDDATA;
    }
    s->nsegsamples = 1 << s->nsegsamples_log2;
    if (s->nsegsamples > 512) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Too many samples per XLL segment\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->nframesamples > 65536) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Too many samples per XLL frame\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ff_dca_seek_bits(&s->gb, header_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of XLL common header\n");
        return AVERROR_INVALIDDATA;
    }
...
static int parse_navi_table(DCAXllDecoder *s)
{
    ...
    if (navi_nb > 1024) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Too many NAVI entries (%d)\n", navi_nb);
        return AVERROR_INVALIDDATA;
    }

    // Reallocate NAVI table
    av_em_fast_malloc(&s->navi, &s->navi_size, navi_nb * sizeof(*s->navi));
    if (!s->navi)
        return AVERROR(ENOMEM);
            ...
            for (chs = 0, c = s->chset; chs < s->nchsets; chs++, c++) {
                ...
                if (c->nfreqbands > band) {
                    ...
                    if (size < 0 || size >= s->frame_size) {
                        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid NAVI segment size (%d bytes)\n", size);
                        return AVERROR_INVALIDDATA;
                    }
                    size++;
                }
    ...
    if (ff_dca_check_crc(s->avctx, &s->gb, navi_pos, get_bits_count(&s->gb))) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid NAVI checksum\n");
        return AVERROR_INVALIDDATA;
    }
            ...
            for (chs = 0, c = s->chset; chs < s->nchsets; chs++, c++) {
                if (c->nfreqbands > band) {
                    ...
                    if (navi_pos > s->gb.size_in_bits) {
                        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid NAVI position\n");
                        return AVERROR_INVALIDDATA;
                    }
...
static int parse_frame(DCAXllDecoder *s, uint8_t *data, int size, DCAExssAsset *asset)
{
    ...
    if (ff_dca_seek_bits(&s->gb, s->frame_size * 8)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Read past end of XLL frame\n");
        return AVERROR_INVALIDDATA;
    }
    return ret;
}
...
static int copy_to_pbr(DCAXllDecoder *s, uint8_t *data, int size, int delay)
{
    ...
    if (!s->pbr_buffer && !(s->pbr_buffer = av_em_alloc(DCA_XLL_PBR_BUFFER_MAX + AV_INPUT_BUFFER_PADDING_SIZE)))
        return AVERROR(ENOMEM);
...
static int combine_residual_frame(DCAXllDecoder *s, DCAXllChSet *c)
{
    ...
    if (!(dca->packet & DCA_PACKET_CORE)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Residual encoded channels are present without core\n");
        return AVERROR(EINVAL);
    }

    if (c->freq != dca->core.output_rate) {
        av_em_log(s->avctx, AV_LOG_WARNING, "Sample rate mismatch between core (%d Hz) and XLL (%d Hz)\n", dca->core.output_rate, c->freq);
        return AVERROR_INVALIDDATA;
    }

    if (nsamples != dca->core.npcmsamples) {
        av_em_log(s->avctx, AV_LOG_WARNING, "Number of samples per frame mismatch between core (%d) and XLL (%d)\n", dca->core.npcmsamples, nsamples);
        return AVERROR_INVALIDDATA;
    }
    ...
    for (ch = 0; ch < c->nchannels; ch++) {
        ...
        if (spkr < 0) {
            av_em_log(s->avctx, AV_LOG_WARNING, "Residual encoded channel (%d) references unavailable core channel\n", c->ch_remap[ch]);
            return AVERROR_INVALIDDATA;
        }
        ...
        if (shift > 24) {
            av_em_log(s->avctx, AV_LOG_WARNING, "Invalid core shift (%d bits)\n", shift);
            return AVERROR_INVALIDDATA;
        }
