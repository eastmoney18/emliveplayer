...
static av_cold int flashsv_encode_end(AVEMCodecContext *avctx)
{
    ...
    av_em_freep(&s->previous_frame);
    av_em_freep(&s->tmpblock);

    return 0;
}

static av_cold int flashsv_encode_init(AVEMCodecContext *avctx)
{
    FlashSVContext *s = avctx->priv_data;
    ...
    s->tmpblock  = av_em_mallocz(3 * 256 * 256);
    s->encbuffer = av_em_mallocz(s->image_width * s->image_height * 3);
...
static int flashsv_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                                const AVFrame *pict, int *got_packet)
{
    ...
    if (avctx->frame_number == 0) {
        s->previous_frame = av_em_mallocz(FFABS(p->linesize[0]) * s->image_height);
        if (!s->previous_frame) {
            av_em_log(avctx, AV_LOG_ERROR, "Memory allocation failed.\n");
            return AVERROR(ENOMEM);
        }
    ...
    if ((res = em_alloc_packet2(avctx, pkt, s->image_width * s->image_height * 3, 0)) < 0)
        return res;
