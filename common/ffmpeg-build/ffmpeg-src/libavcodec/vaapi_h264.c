 ...
 */
static void fill_vaapi_plain_pred_weight_table(H264EMContext   *h,
                                               int            list,
...
static int vaapi_h264_start_frame(AVEMCodecContext          *avctx,
...
static int vaapi_h264_end_frame(AVEMCodecContext *avctx)
{
    FFVAContext * const vactx = ff_vaapi_get_context(avctx);
    H264EMContext * const h = avctx->priv_data;
    H264SliceContext *sl = &h->slice_ctx[0];
...
static int vaapi_h264_decode_slice(AVEMCodecContext *avctx,
                                   ...
                                   uint32_t        size)
{
    ...
    slice_param->slice_type                     = em_h264_get_slice_type(sl);
...
AVHWAccel em_h264_vaapi_hwaccel = {
    .name           = "h264_vaapi",
