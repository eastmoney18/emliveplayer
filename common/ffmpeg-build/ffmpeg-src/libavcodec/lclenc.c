                        ...
                        const AVFrame *p, int *got_packet)
{
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, max_size, 0)) < 0)
        return ret;
    ...
    if (zret != Z_OK) {
        av_em_log(avctx, AV_LOG_ERROR, "Deflate reset error: %d\n", zret);
        return -1;
    }
...
static av_cold int encode_init(AVEMCodecContext *avctx)
{
    ...
    avctx->extradata = av_em_mallocz(8 + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!avctx->extradata)
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (zret != Z_OK) {
        av_em_log(avctx, AV_LOG_ERROR, "Deflate init error: %d\n", zret);
