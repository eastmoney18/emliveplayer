...
//    mb_addr= pc->mb_addr - 1;
    ...
    if (p->is_avc && !p->nal_length_size)
        av_em_log(logctx, AV_LOG_ERROR, "AVC-parser: nal length size invalid\n");
    ...
    for (i = 0; i < buf_size; i++) {
        if (i >= next_avc) {
            ...
            if (nalsize <= 0 || nalsize > buf_size - i) {
                av_em_log(logctx, AV_LOG_ERROR, "AVC-parser: nal size %d remaining %d\n", nalsize, buf_size - i);
                return buf_size;
            }
            next_avc = i + nalsize;
            state    = 5;
...
static int scan_mmco_reset(AVEMCodecParserContext *s, GetBitContext *gb,
                           void *logctx)
{
    ...
    if (em_h264_parse_ref_count(&list_count, ref_count, gb, p->ps.pps,
    ...
    if ((p->ps.pps->weighted_pred && slice_type_nos == AV_PICTURE_TYPE_P) ||
        ...
        em_h264_pred_weight_table(gb, p->ps.sps, ref_count, slice_type_nos,
                                  &pwt, logctx);
 ...
 */
static inline int parse_nal_units(AVEMCodecParserContext *s,
                                  ...
                                  const uint8_t * const buf, int buf_size)
{
    ...
    for (;;) {
        ...
        consumed = em_h2645_extract_rbsp(buf + buf_index, src_length, &nal);
        if (consumed < 0)
            break;
        ...
        switch (nal.type) {
        case NAL_SPS:
            em_h264_decode_seq_parameter_set(&nal.gb, avctx, &p->ps, 0);
            break;
        case NAL_PPS:
            em_h264_decode_picture_parameter_set(&nal.gb, avctx, &p->ps,
                                                 nal.size_bits);
            break;
        case NAL_SEI:
            em_h264_sei_decode(&p->sei, &nal.gb, &p->ps, avctx);
            break;
        case NAL_IDR_SLICE:
            s->key_frame = 1;
        ...
        case NAL_SLICE:
            ...
            s->pict_type = em_h264_golomb_to_pict_type[slice_type % 5];
            ...
            avctx->profile = em_h264_get_profile(sps);
            ...
            em_h264_init_poc(field_poc, &s->output_picture_number, sps,
    ...
    av_em_log(avctx, AV_LOG_ERROR, "missing picture in access unit with size %d\n", buf_size);
fail:
    av_em_freep(&nal.rbsp_buffer);
    return -1;
}

static int h264_parse(AVEMCodecParserContext *s,
                      AVEMCodecContext *avctx,
                      ...
                      const uint8_t *buf, int buf_size)
{
    ...
    if (!p->got_first) {
        ...
        if (avctx->extradata_size) {
            em_h264_decode_extradata(avctx->extradata, avctx->extradata_size,
...
static int h264_split(AVEMCodecContext *avctx,
                      const uint8_t *buf, int buf_size)
{
    uint32_t state = -1;
    int has_sps    = 0;
...
static void h264_close(AVEMCodecParserContext *s)
{
    H264ParseContext *p = s->priv_data;
    ParseContext *pc = &p->pc;

    av_em_freep(&pc->buffer);

    em_h264_sei_uninit(&p->sei);
    em_h264_ps_uninit(&p->ps);
}

static av_cold int init(AVEMCodecParserContext *s)
{
    H264ParseContext *p = s->priv_data;

    em_h264dsp_init(&p->h264dsp, 8, 1);
    return 0;
}

AVEMCodecParser em_h264_parser = {
