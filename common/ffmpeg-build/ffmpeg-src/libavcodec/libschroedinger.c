...
static unsigned int get_video_format_idx(AVEMCodecContext *avctx)
{
...
{
    FFSchroQueueElement *p_new = av_em_mallocz(sizeof(FFSchroQueueElement));
...
SchroVideoFormatEnum ff_get_schro_video_format_preset(AVEMCodecContext *avctx)
{
...
{
    AVFrame *p_pic = priv;
    av_em_frame_free(&p_pic);
}

SchroFrame *ff_create_schro_frame(AVEMCodecContext *avctx,
                                  SchroFrameFormat schro_frame_fmt)
{
    AVFrame *p_pic;
    SchroFrame *p_frame;
    ...
    p_pic = av_em_frame_alloc();
    if (!p_pic)
        return NULL;

    if (em_get_buffer(avctx, p_pic, AV_GET_BUFFER_FLAG_REF) < 0) {
        av_em_frame_free(&p_pic);
        return NULL;
    }
