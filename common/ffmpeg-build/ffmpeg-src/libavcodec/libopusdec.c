...
static av_cold int libopus_decode_init(AVEMCodecContext *avc)
{
    ...
    if (avc->extradata_size >= OPUS_HEAD_SIZE + 2 + avc->channels) {
        ...
        if (nb_streams + nb_coupled != avc->channels)
            av_em_log(avc, AV_LOG_WARNING, "Inconsistent channel mapping.\n");
        mapping = avc->extradata + OPUS_HEAD_SIZE + 2;
    } else {
        if (avc->channels > 2 || channel_map) {
            av_em_log(avc, AV_LOG_ERROR,
    ...
    opus->dec = opus_multistream_decoder_create(avc->sample_rate, avc->channels,
                                                ...
                                                mapping, &ret);
    if (!opus->dec) {
        av_em_log(avc, AV_LOG_ERROR, "Unable to create decoder: %s\n",
               opus_strerror(ret));
    ...
    if (ret != OPUS_OK)
        av_em_log(avc, AV_LOG_WARNING, "Failed to set gain: %s\n",
               opus_strerror(ret));
#else
    {
...
static av_cold int libopus_decode_close(AVEMCodecContext *avc)
{
    struct libopus_context *opus = avc->priv_data;
...
static int libopus_decode(AVEMCodecContext *avc, void *data,
                          int *got_frame_ptr, AVEMPacket *pkt)
{
    ...
    if ((ret = em_get_buffer(avc, frame, 0)) < 0)
        return ret;
    ...
    if (nb_samples < 0) {
        av_em_log(avc, AV_LOG_ERROR, "Decoding error: %s\n",
...
static void libopus_flush(AVEMCodecContext *avc)
{
    struct libopus_context *opus = avc->priv_data;
