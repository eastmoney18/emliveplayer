...
{
    put_bits(pb, n, em_reverse[v << (8 - n)]);
}
...
static inline void asv2_put_level(ASV1Context *a, PutBitContext *pb, int level)
{
    ...
    if (index <= 62) {
        ...
        if (level < -128 || level > 127) {
            av_em_log(a->avctx, AV_LOG_WARNING, "Cliping level %d, increase qscale\n", level);
...
static inline int encode_mb(ASV1Context *a, int16_t block[6][64])
{
    ...
    if (a->pb.buf_end - a->pb.buf - (put_bits_count(&a->pb) >> 3) < MAX_MB_SIZE) {
        av_em_log(a->avctx, AV_LOG_ERROR, "encoded frame too large\n");
        return -1;
    }
                        ...
                        const AVFrame *pict, int *got_packet)
{
    ...
    if (pict->width % 16 || pict->height % 16) {
        AVFrame *clone = av_em_frame_alloc();
        int i;
        ...
        ret = av_em_frame_get_buffer(clone, 32);
        if (ret < 0) {
            av_em_frame_free(&clone);
            return ret;
        }

        ret = av_em_frame_copy(clone, pict);
        if (ret < 0) {
            av_em_frame_free(&clone);
            return ret;
        }
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, a->mb_height * a->mb_width * MAX_MB_SIZE +
    ...
    if (avctx->codec_id == AV_CODEC_ID_ASV1) {
        ...
        int i;
        for (i = 0; i < 4 * size; i++)
            pkt->data[i] = em_reverse[pkt->data[i]];
    }
...
static av_cold int encode_init(AVEMCodecContext *avctx)
{
    ...
    avctx->extradata                   = av_em_mallocz(8);
    if (!avctx->extradata)
        return AVERROR(ENOMEM);
