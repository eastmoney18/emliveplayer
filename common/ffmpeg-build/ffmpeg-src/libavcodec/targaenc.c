...
static int targa_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                              const AVFrame *p, int *got_packet)
{
    ...
    if(avctx->width > 0xffff || avctx->height > 0xffff) {
        av_em_log(avctx, AV_LOG_ERROR, "image dimensions too large\n");
        return AVERROR(EINVAL);
    }
    picsize = av_em_image_get_buffer_size(avctx->pix_fmt,
                                       avctx->width, avctx->height, 1);
    if ((ret = em_alloc_packet2(avctx, pkt, picsize + 45, 0)) < 0)
        return ret;
    ...
    avctx->bits_per_coded_sample = av_em_get_bits_per_pixel(av_em_pix_fmt_desc_get(avctx->pix_fmt));
    ...
    case AV_PIX_FMT_BGRA:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Pixel format '%s' not supported.\n",
...
static av_cold int targa_encode_init(AVEMCodecContext *avctx)
{
...
static const AVEMClass targa_class = {
    .class_name = "targa",
