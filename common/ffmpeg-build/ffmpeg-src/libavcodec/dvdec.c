...
static av_cold int dvvideo_decode_init(AVEMCodecContext *avctx)
{
...
static int dv_decode_video_segment(AVEMCodecContext *avctx, void *arg)
{
    DVVideoContext *s = avctx->priv_data;
...
retry:
    ...
    if (vs_bit_buffer_damaged && !retried) {
        av_em_log(avctx, AV_LOG_ERROR, "Concealing bitstream errors\n");
        retried = 1;
        goto retry;
    }
...
static int dvvideo_decode_frame(AVEMCodecContext *avctx, void *data,
                                int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (!sys || buf_size < sys->frame_size) {
        av_em_log(avctx, AV_LOG_ERROR, "could not find dv frame profile\n");
    ...
    if (sys != s->sys) {
        ...
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Error initializing the work tables.\n");
            return ret;
        }
    ...
    avctx->framerate    = av_em_inv_q(s->sys->time_base);

    ret = em_set_dimensions(avctx, s->sys->width, s->sys->height);
    if (ret < 0)
        return ret;
    ...
    if (*vsc_pack == dv_video_control) {
        ...
        em_set_sar(avctx, s->sys->sar[is16_9]);
    }
