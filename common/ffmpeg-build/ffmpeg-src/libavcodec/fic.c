...
static int fic_decode_slice(AVEMCodecContext *avctx, void *tdata)
{
...
static void fic_draw_cursor(AVEMCodecContext *avctx, int cur_x, int cur_y)
{
    FICContext *ctx = avctx->priv_data;
...
static int fic_decode_frame(AVEMCodecContext *avctx, void *data,
                            int *got_frame, AVEMPacket *avpkt)
{
    ...
    if ((ret = em_reget_buffer(avctx, ctx->frame)) < 0)
        return ret;

    /* Header + at least one slice (4) */
    if (avpkt->size < FIC_HEADER_SIZE + 4) {
        av_em_log(avctx, AV_LOG_ERROR, "Frame data is too small.\n");
        return AVERROR_INVALIDDATA;
    }

    /* Check for header. */
    if (memcmp(src, fic_header, 7))
        av_em_log(avctx, AV_LOG_WARNING, "Invalid FIC Header.\n");

    /* Is it a skip frame? */
    if (src[17]) {
        if (!ctx->final_frame) {
            av_em_log(avctx, AV_LOG_WARNING, "Initial frame is skipped\n");
            return AVERROR_INVALIDDATA;
        }
        goto skip;
    }
    ...
    nslices = src[13];
    if (!nslices) {
        av_em_log(avctx, AV_LOG_ERROR, "Zero slices found.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (msize <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Not enough frame data to decode.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    /* Allocate slice data. */
    av_em_fast_malloc(&ctx->slice_data, &ctx->slice_data_size,
                   nslices * sizeof(ctx->slice_data[0]));
    if (!ctx->slice_data_size) {
        av_em_log(avctx, AV_LOG_ERROR, "Could not allocate slice data.\n");
    ...
    av_em_frame_free(&ctx->final_frame);
    ctx->final_frame = av_em_frame_clone(ctx->frame);
    if (!ctx->final_frame) {
        av_em_log(avctx, AV_LOG_ERROR, "Could not clone frame buffer.\n");
        return AVERROR(ENOMEM);
    }

    /* Make sure we use a user-supplied buffer. */
    if ((ret = em_reget_buffer(avctx, ctx->final_frame)) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Could not make frame writable.\n");
        return ret;
    }
...
skip:
    *got_frame = 1;
    if ((ret = av_em_frame_ref(data, ctx->final_frame)) < 0)
        return ret;

    return avpkt->size;
}

static av_cold int fic_decode_close(AVEMCodecContext *avctx)
{
    FICContext *ctx = avctx->priv_data;

    av_em_freep(&ctx->slice_data);
    av_em_frame_free(&ctx->final_frame);
    av_em_frame_free(&ctx->frame);

    return 0;
}

static av_cold int fic_decode_init(AVEMCodecContext *avctx)
{
    FICContext *ctx = avctx->priv_data;
...
static const AVEMClass fic_decoder_class = {
