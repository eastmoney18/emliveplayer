...
static av_cold int libx265_encode_close(AVEMCodecContext *avctx)
{
...
static av_cold int libx265_encode_init(AVEMCodecContext *avctx)
{
    libx265Context *ctx = avctx->priv_data;

    ctx->api = x265_api_get(av_em_pix_fmt_desc_get(avctx->pix_fmt)->comp[0].depth);
    if (!ctx->api)
        ctx->api = x265_api_get(0);

    ctx->params = ctx->api->param_alloc();
    if (!ctx->params) {
        av_em_log(avctx, AV_LOG_ERROR, "Could not allocate x265 param structure.\n");
    ...
    if (ctx->api->param_default_preset(ctx->params, ctx->preset, ctx->tune) < 0) {
        ...
        for (i = 0; x265_preset_names[i]; i++)
            av_em_log(avctx, AV_LOG_INFO, " %s", x265_preset_names[i]);

        av_em_log(avctx, AV_LOG_INFO, "\n");
        av_em_log(avctx, AV_LOG_INFO, "Possible tunes:");
        for (i = 0; x265_tune_names[i]; i++)
            av_em_log(avctx, AV_LOG_INFO, " %s", x265_tune_names[i]);
    ...
    if (ctx->crf >= 0) {
        ...
        if (ctx->api->param_parse(ctx->params, "crf", crf) == X265_PARAM_BAD_VALUE) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid crf: %2.2f.\n", ctx->crf);
            return AVERROR(EINVAL);
        }
    ...
    if (ctx->x265_opts) {
        ...
        if (!av_em_dict_parse_string(&dict, ctx->x265_opts, "=", ":", 0)) {
    ...
    if (!ctx->encoder) {
        av_em_log(avctx, AV_LOG_ERROR, "Cannot open libx265 encoder.\n");
    ...
    if (avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER) {
        ...
        if (avctx->extradata_size <= 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Cannot encode headers.\n");
            libx265_encode_close(avctx);
            return AVERROR_INVALIDDATA;
        }

        avctx->extradata = av_em_alloc(avctx->extradata_size + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!avctx->extradata) {
...
static int libx265_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                                const AVFrame *pic, int *got_packet)
{
    ...
    if (pic) {
        ...
        x265pic.pts      = pic->pts;
        x265pic.bitDepth = av_em_pix_fmt_desc_get(avctx->pix_fmt)->comp[0].depth;
    ...
    ret = em_alloc_packet(pkt, payload);
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error getting output packet.\n");
        return ret;
    }
    dst = pkt->data;
...
static av_cold void libx265_encode_init_csp(AVEMCodec *codec)
{
    if (x265_api_get(12))
...
static const AVEMCodecDefault x265_defaults[] = {
    { "b", "0" },
    { NULL },
};

AVEMCodec ff_libx265_encoder = {
    .name             = "libx265",
