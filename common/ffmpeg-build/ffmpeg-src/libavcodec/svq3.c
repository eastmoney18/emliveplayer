 ...
 * via the AVEMCodecContext's extradata[_size] field:
 *
 * AVEMCodecContext.extradata = pointer to ImageDescription, first characters
 * are expected to be 'S', 'V', 'Q', and '3', NOT the 4-byte atom length
 * AVEMCodecContext.extradata_size = size of ImageDescription atom memory
...
static int svq3_decode_end(AVEMCodecContext *avctx);
                                    ...
                                    int index, const int type)
{
    static const uint8_t *const scan_patterns[4] = {
        luma_dc_zigzag_scan, ff_zigzag_scan, svq3_scan, em_h264_chroma_dc_scan
    };
        ...
        for (j = 0; j < 16; j += part_width) {
            ...
            if (mode == PREDICT_MODE) {
                ...
                if (dx == INVALID_VLC || dy == INVALID_VLC) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "invalid MV vlc\n");
                    return -1;
                }
            }
            ...
            for (i = j * 16; i < j * 16 + 4; i++)
                if (s->non_zero_count_cache[scan8[i]] || s->mb[i * 16]) {
                    ...
                    svq3_add_idct_c(ptr, s->mb + i * 16,
                                    uvlinesize, em_h264_chroma_qp[0][s->qscale + 12] - 12, 2);
                }
        }
    }
}
...
static int svq3_decode_mb(SVQ3Context *s, unsigned int mb_type)
{
    ...
    if (mb_type == 0) {           /* SKIP */
        ...
        if (mb_type == 8) {
            ...
            for (i = 0; i < 16; i += 2) {
                ...
                if (left[1] == -1 || left[2] == -1) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "weird prediction\n");
                    return -1;
                }
            }
        ...
        if (mb_type == 8) {
            em_h264_check_intra4x4_pred_mode(s->intra4x4_pred_mode_cache,
        ...
        dir = em_h264_i_mb_type_info[mb_type - 8].pred_mode;
        dir = (dir >> 1) ^ 3 * (dir & 1) ^ 1;

        if ((s->intra16x16_pred_mode = em_h264_check_intra_pred_mode(s->avctx, s->top_samples_available,
                                                                     s->left_samples_available, dir, 0)) < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "em_h264_check_intra_pred_mode < 0\n");
            return s->intra16x16_pred_mode;
        }

        cbp     = em_h264_i_mb_type_info[mb_type - 8].cbp;
    ...
    if (!IS_INTRA16x16(mb_type) &&
        ...
        if ((vlc = get_interleaved_ue_golomb(&s->gb_slice)) >= 48U){
            av_em_log(s->avctx, AV_LOG_ERROR, "cbp_vlc=%"PRIu32"\n", vlc);
            return -1;
        }

        cbp = IS_INTRA(mb_type) ? em_h264_golomb_to_intra4x4_cbp[vlc]
                                : em_h264_golomb_to_inter_cbp[vlc];
    }
    if (IS_INTRA16x16(mb_type) ||
        ...
        if (s->qscale > 31u) {
            av_em_log(s->avctx, AV_LOG_ERROR, "qscale:%d\n", s->qscale);
            return -1;
        }
    }
    if (IS_INTRA16x16(mb_type)) {
    ...
    if (IS_INTRA(mb_type))
        s->chroma_pred_mode = em_h264_check_intra_pred_mode(s->avctx, s->top_samples_available,
                                                            s->left_samples_available, DC_PRED8x8, 1);

    return 0;
}

static int svq3_decode_slice_header(AVEMCodecContext *avctx)
{
    ...
    if (((header & 0x9F) != 1 && (header & 0x9F) != 2) || (header & 0x60) == 0) {
        /* TODO: what? */
        av_em_log(avctx, AV_LOG_ERROR, "unsupported slice header (%02X)\n", header);
        return -1;
    } else {
        ...
        if (slice_bytes > get_bits_left(&s->gb)) {
            av_em_log(avctx, AV_LOG_ERROR, "slice after bitstream end\n");
            return -1;
        }

        skip_bits(&s->gb, 8);

        av_em_fast_malloc(&s->slice_buf, &s->slice_size, slice_bytes + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!s->slice_buf)
    ...
    if ((slice_id = get_interleaved_ue_golomb(&s->gb_slice)) >= 3) {
        av_em_log(s->avctx, AV_LOG_ERROR, "illegal slice type %u \n", slice_id);
        return -1;
    }

    s->pict_type = em_h264_golomb_to_pict_type[slice_id];
...
static void init_dequant4_coeff_table(SVQ3Context *s)
{
    ...
    for (q = 0; q < max_qp + 1; q++) {
        int shift = em_h264_quant_div6[q] + 2;
        int idx   = em_h264_quant_rem6[q];
        for (x = 0; x < 16; x++)
            s->dequant4_coeff[q][(x >> 2) | ((x << 2) & 0xF)] =
                ((uint32_t)em_h264_dequant4_coeff_init[idx][(x & 1) + ((x >> 2) & 1)] * 16) << shift;
    }
}

static av_cold int svq3_decode_init(AVEMCodecContext *avctx)
{
    SVQ3Context *s = avctx->priv_data;
    int m, x, y;
    ...
    s->cur_pic  = av_em_mallocz(sizeof(*s->cur_pic));
    s->last_pic = av_em_mallocz(sizeof(*s->last_pic));
    s->next_pic = av_em_mallocz(sizeof(*s->next_pic));
    if (!s->next_pic || !s->last_pic || !s->cur_pic) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }

    s->cur_pic->f  = av_em_frame_alloc();
    s->last_pic->f = av_em_frame_alloc();
    s->next_pic->f = av_em_frame_alloc();
    if (!s->cur_pic->f || !s->last_pic->f || !s->next_pic->f)
        return AVERROR(ENOMEM);

    em_h264dsp_init(&s->h264dsp, 8, 1);
    em_h264_pred_init(&s->hpc, AV_CODEC_ID_SVQ3, 8, 1);
    ...
    if (marker_found) {
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Unknown fields %d %d %d %d %d\n",
        ...
        if (s->has_watermark) {
            ...
            buf = av_em_alloc(buf_len);
            if (!buf) {
                ret = AVERROR(ENOMEM);
                goto fail;
            }
            av_em_log(avctx, AV_LOG_DEBUG, "watermark size: %ux%u\n",
    ...
    s->intra4x4_pred_mode = av_em_mallocz(s->mb_stride * 2 * 8);
    if (!s->intra4x4_pred_mode)
        return AVERROR(ENOMEM);

    s->mb2br_xy = av_em_mallocz(s->mb_stride * (s->mb_height + 1) *
                             sizeof(*s->mb2br_xy));
    if (!s->mb2br_xy)
        return AVERROR(ENOMEM);
...
static void free_picture(AVEMCodecContext *avctx, H264Picture *pic)
{
    int i;
    for (i = 0; i < 2; i++) {
        av_em_buffer_unref(&pic->motion_val_buf[i]);
        av_em_buffer_unref(&pic->ref_index_buf[i]);
    }
    av_em_buffer_unref(&pic->mb_type_buf);

    av_em_frame_unref(pic->f);
}

static int get_buffer(AVEMCodecContext *avctx, H264Picture *pic)
{
    SVQ3Context *s = avctx->priv_data;
    ...
    if (!pic->motion_val_buf[0]) {
        ...
        pic->mb_type_buf = av_em_buffer_allocz((big_mb_num + s->mb_stride) * sizeof(uint32_t));
        if (!pic->mb_type_buf)
            return AVERROR(ENOMEM);
        pic->mb_type = (uint32_t*)pic->mb_type_buf->data + 2 * s->mb_stride + 1;

        for (i = 0; i < 2; i++) {
            pic->motion_val_buf[i] = av_em_buffer_allocz(2 * (b4_array_size + 4) * sizeof(int16_t));
            pic->ref_index_buf[i]  = av_em_buffer_allocz(4 * mb_array_size);
    ...
    ret = em_get_buffer(avctx, pic->f,
                        pic->reference ? AV_GET_BUFFER_FLAG_REF : 0);
    if (ret < 0)
        goto fail;

    if (!s->edge_emu_buffer) {
        s->edge_emu_buffer = av_em_mallocz_array(pic->f->linesize[0], 17);
        if (!s->edge_emu_buffer)
            return AVERROR(ENOMEM);
    }
...
static int svq3_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (buf_size == 0) {
        if (s->next_pic->f->data[0] && !s->low_delay && !s->last_frame_output) {
            ret = av_em_frame_ref(data, s->next_pic->f);
            if (ret < 0)
                return ret;
    ...
    if (s->watermark_key) {
        av_em_fast_padded_malloc(&s->buf, &s->buf_size, buf_size);
        if (!s->buf)
            return AVERROR(ENOMEM);
    ...
    if (s->pict_type != AV_PICTURE_TYPE_I) {
        if (!s->last_pic->f->data[0]) {
            av_em_log(avctx, AV_LOG_ERROR, "Missing reference frame.\n");
            av_em_frame_unref(s->last_pic->f);
        ...
        if (s->pict_type == AV_PICTURE_TYPE_B && !s->next_pic->f->data[0]) {
            av_em_log(avctx, AV_LOG_ERROR, "Missing reference frame.\n");
            av_em_frame_unref(s->next_pic->f);
    ...
    if (s->pict_type == AV_PICTURE_TYPE_B) {
        ...
        if (s->frame_num_offset == 0 ||
            ...
            av_em_log(s->avctx, AV_LOG_ERROR, "error in B-frame picture id\n");
            return -1;
        }
    } else {
    ...
    if (s->mb_y != s->mb_height || s->mb_x != s->mb_width) {
        av_em_log(avctx, AV_LOG_INFO, "frame num %d incomplete pic x %d y %d left %d\n", avctx->frame_number, s->mb_y, s->mb_x, left);
        //av_em_hex_dump(stderr, buf+buf_size-8, 8);
    }

    if (left < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "frame num %d left %d\n", avctx->frame_number, left);
        return -1;
    }

    if (s->pict_type == AV_PICTURE_TYPE_B || s->low_delay)
        ret = av_em_frame_ref(data, s->cur_pic->f);
    else if (s->last_pic->f->data[0])
        ret = av_em_frame_ref(data, s->last_pic->f);
    if (ret < 0)
        return ret;
...
static av_cold int svq3_decode_end(AVEMCodecContext *avctx)
{
    SVQ3Context *s = avctx->priv_data;
    ...
    av_em_frame_free(&s->cur_pic->f);
    av_em_frame_free(&s->next_pic->f);
    av_em_frame_free(&s->last_pic->f);
    av_em_freep(&s->cur_pic);
    av_em_freep(&s->next_pic);
    av_em_freep(&s->last_pic);
    av_em_freep(&s->slice_buf);
    av_em_freep(&s->intra4x4_pred_mode);
    av_em_freep(&s->edge_emu_buffer);
    av_em_freep(&s->mb2br_xy);
