...
static av_cold int idcin_decode_init(AVEMCodecContext *avctx)
{
    ...
    if (s->avctx->extradata_size != HUFFMAN_TABLE_SIZE) {
        av_em_log(s->avctx, AV_LOG_ERROR, "  id CIN video: expected extradata size of %d\n", HUFFMAN_TABLE_SIZE);
        return -1;
    }
    ...
    for (y = 0; y < (frame->linesize[0] * s->avctx->height);
        ...
        for (x = y; x < y + s->avctx->width; x++) {
            ...
            while(node_num >= HUF_TOKENS) {
                if(!bit_pos) {
                    if(dat_pos >= s->size) {
                        av_em_log(s->avctx, AV_LOG_ERROR, "Huffman decode error.\n");
                        return -1;
                    }
                    bit_pos = 8;
                    v = s->buf[dat_pos++];
...
static int idcin_decode_frame(AVEMCodecContext *avctx,
