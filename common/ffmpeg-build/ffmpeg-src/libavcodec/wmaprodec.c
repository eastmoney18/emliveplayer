...
typedef struct WMAProDecodeCtx {
    ...
    AVEMCodecContext*  avctx;                         ///< codec context for av_log
...
{
#define PRINT(a, b)     av_em_log(s->avctx, AV_LOG_DEBUG, " %s = %d\n", a, b);
#define PRINT_HEX(a, b) av_em_log(s->avctx, AV_LOG_DEBUG, " %s = %"PRIx32"\n", a, b);
...
static av_cold int get_rate(AVEMCodecContext *avctx)
{
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (!avctx->block_align) {
        av_em_log(avctx, AV_LOG_ERROR, "block_align is not set\n");
    ...
    if (avctx->codec_id == AV_CODEC_ID_XMA2 && avctx->extradata_size >= 34) {
        ...
        if (s->bits_per_sample > 32 || s->bits_per_sample < 1) {
            avpriv_em_request_sample(avctx, "bits per sample is %d", s->bits_per_sample);
        ...
        avpriv_em_request_sample(avctx, "Unknown extradata size");
        return AVERROR_PATCHWELCOME;
    }

    if (avctx->codec_id != AV_CODEC_ID_WMAPRO && avctx->channels > 2) {
        avpriv_em_report_missing_feature(avctx, ">2 channels support");
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (s->log2_frame_size > 25) {
        avpriv_em_request_sample(avctx, "Large block align");
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (avctx->codec_id == AV_CODEC_ID_WMAPRO) {
        ...
        if (bits > WMAPRO_BLOCK_MAX_BITS) {
            avpriv_em_request_sample(avctx, "14-bit block sizes");
            return AVERROR_PATCHWELCOME;
        }
    ...
    if (s->max_num_subframes > MAX_SUBFRAMES) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid number of subframes %"PRId8"\n",
               s->max_num_subframes);
        return AVERROR_INVALIDDATA;
    }

    if (s->min_samples_per_subframe < WMAPRO_BLOCK_MIN_SIZE) {
        av_em_log(avctx, AV_LOG_ERROR, "min_samples_per_subframe of %d too small\n",
    ...
    if (avctx->channels < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid number of channels %d\n",
               avctx->channels);
        return AVERROR_INVALIDDATA;
    ...
    for (i = 0; i < num_possible_block_sizes; i++) {
        ...
        s->num_sfb[i]               = band - 1;
        if (s->num_sfb[i] <= 0) {
            av_em_log(avctx, AV_LOG_ERROR, "num_sfb invalid\n");
            return AVERROR_INVALIDDATA;
        }
    }
...
static int decode_subframe_length(WMAProDecodeCtx *s, int offset)
{
    ...
    if (subframe_len < s->min_samples_per_subframe ||
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "broken frame: subframe_len %i\n",
               subframe_len);
        return AVERROR_INVALIDDATA;
    }
    return subframe_len;
        ...
        for (c = 0; c < s->avctx->channels; c++) {
            ...
            if (contains_subframe[c]) {
                ...
                if (num_samples[c] > s->samples_per_frame) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "broken frame: "
...
static int decode_subframe(WMAProDecodeCtx *s)
{
    ...
    */
    ...
    if (get_bits1(&s->gb)) {
        ...
        if (num_fill_bits >= 0) {
            if (get_bits_count(&s->gb) + num_fill_bits > s->num_saved_bits) {
                av_em_log(s->avctx, AV_LOG_ERROR, "invalid number of fill bits\n");
                return AVERROR_INVALIDDATA;
            }
    ...
    if (get_bits1(&s->gb)) {
        avpriv_em_request_sample(s->avctx, "Reserved bit");
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (transmit_coeffs) {
        ...
        if ((s->transmit_num_vec_coeffs = get_bits1(&s->gb))) {
            ...
            for (i = 0; i < s->channels_for_cur_subframe; i++) {
                ...
                if (num_vec_coeffs > s->subframe_len) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "num_vec_coeffs %d is too large\n", num_vec_coeffs);
                    return AVERROR_INVALIDDATA;
                }
        ...
        if (quant_step < 0) {
            av_em_log(s->avctx, AV_LOG_DEBUG, "negative quant step\n");
        }
    ...
    for (i = 0; i < s->channels_for_cur_subframe; i++) {
        ...
        if (s->channel[c].cur_subframe >= s->channel[c].num_subframes) {
            av_em_log(s->avctx, AV_LOG_ERROR, "broken subframe\n");
            return AVERROR_INVALIDDATA;
        }
                      ...
                      int append)
{
    ...
    if (len <= 0 || buflen > MAX_FRAMESIZE) {
        avpriv_em_request_sample(s->avctx, "Too small input buffer");
        s->packet_loss = 1;
        return;
    }
 ...
 */
static int decode_packet(AVEMCodecContext *avctx, void *data,
    ...
    *got_frame_ptr = 0;
    ...
    if (s->packet_done || s->packet_loss) {
        ...
        if (avctx->codec_id == AV_CODEC_ID_WMAPRO && buf_size < avctx->block_align) {
            av_em_log(avctx, AV_LOG_ERROR, "Input packet too small (%d < %d)\n",
    ...
    if (remaining_bits(s, gb) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Overread %d\n", -remaining_bits(s, gb));
        s->packet_loss = 1;
    }
