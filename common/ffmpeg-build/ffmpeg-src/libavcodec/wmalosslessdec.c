...
typedef struct WmallDecodeCtx {
    ...
    int8_t          num_channels;                   ///< number of channels in the stream (same as AVEMCodecContext.num_channels)
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    s->frame_data = av_em_mallocz(s->max_frame_size + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!s->frame_data)
    ...
    if (avctx->extradata_size >= 18) {
        ...
        if (s->bits_per_sample == 16)
            ...
            av_em_log(avctx, AV_LOG_WARNING, "Decoding audio at 24 bit-depth\n");
            avctx->sample_fmt = AV_SAMPLE_FMT_S32P;
            avctx->bits_per_raw_sample = 24;
        } else {
            av_em_log(avctx, AV_LOG_ERROR, "Unknown bit-depth: %"PRIu8"\n",
                   s->bits_per_sample);
        ...
        avpriv_em_request_sample(avctx, "Unsupported extradata size");
    ...
    if (s->max_num_subframes > MAX_SUBFRAMES) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid number of subframes %"PRIu8"\n",
               s->max_num_subframes);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->num_channels < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid number of channels %"PRId8"\n",
               s->num_channels);
        return AVERROR_INVALIDDATA;
    ...
    for (c = 0; c < s->num_channels; c++) {
        ...
        for (i = 0; i < s->cdlms_ttl[c]; i++) {
            ...
            if(s->cdlms[c][i].order & 8 && s->bits_per_sample == 16) {
                static int warned;
                if(!warned)
                    avpriv_em_request_sample(s->avctx, "CDLMS of order %d",
                                          s->cdlms[c][i].order);
                warned = 1;
            }
        }
...
static int decode_subframe(WmallDecodeCtx *s)
{
    ...
    if (s->seekable_tile) {
        ...
        if (s->do_arith_coding) {
            avpriv_em_request_sample(s->avctx, "Arithmetic coding");
            return AVERROR_PATCHWELCOME;
        }
    ...
    if (!rawpcm_tile) {
        ...
        if (s->bV3RTM) {
            ...
            if (s->do_lpc) {
                decode_lpc(s);
                avpriv_em_request_sample(s->avctx, "Expect wrong output since "
                                      "inverse LPC filter");
            }
        } else
            s->do_lpc = 0;
...
static int decode_packet(AVEMCodecContext *avctx, void *data, int *got_frame_ptr,
                         AVEMPacket* avpkt)
{
    WmallDecodeCtx *s = avctx->priv_data;
    ...
    if (!buf_size && s->num_saved_bits > get_bits_count(&s->gb)) {
        ...
        if (spliced_packet)
            avpriv_em_request_sample(avctx, "Bitstream splicing");
    ...
    *got_frame_ptr   = s->frame->nb_samples > 0;
    av_em_frame_move_ref(data, s->frame);
...
static av_cold int decode_close(AVEMCodecContext *avctx)
{
    ...
    av_em_freep(&s->frame_data);

    return 0;
}

AVEMCodec ff_wmalossless_decoder = {
    .name           = "wmalossless",
