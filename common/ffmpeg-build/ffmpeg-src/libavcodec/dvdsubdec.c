...
static void reset_rects(AVSubtitle *sub_header)
{
    ...
    if (sub_header->rects) {
        for (i = 0; i < sub_header->num_rects; i++) {
            av_em_freep(&sub_header->rects[i]->data[0]);
            av_em_freep(&sub_header->rects[i]->data[1]);
            av_em_freep(&sub_header->rects[i]);
        }
        av_em_freep(&sub_header->rects);
                                ...
                                const uint8_t *buf, int buf_size)
{
    ...
    if (cmd_pos < 0 || cmd_pos > buf_size - 2 - offset_size) {
        if (cmd_pos > size) {
            av_em_log(ctx, AV_LOG_ERROR, "Discarding invalid packet\n");
            return 0;
        }
    ...
    the_end:
        ...
        if (offset1 >= 0 && offset2 >= 0) {
            ...
            if (w > 0 && h > 0) {
                ...
                sub_header->rects = av_em_mallocz(sizeof(*sub_header->rects));
                if (!sub_header->rects)
                    goto fail;
                sub_header->rects[0] = av_em_mallocz(sizeof(AVSubtitleRect));
                if (!sub_header->rects[0])
                    goto fail;
                sub_header->num_rects = 1;
                bitmap = sub_header->rects[0]->data[0] = av_em_alloc(w * h);
                if (!bitmap)
                    goto fail;
                ...
                if (decode_rle(bitmap + w, w * 2, w, h / 2,
                    ...
                    goto fail;
                sub_header->rects[0]->data[1] = av_em_mallocz(AVPALETTE_SIZE);
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (next_cmd_pos < cmd_pos) {
            av_em_log(ctx, AV_LOG_ERROR, "Invalid command offset\n");
            break;
        }
        if (next_cmd_pos == cmd_pos)
            break;
...
static int append_to_cached_buf(AVEMCodecContext *avctx,
                                const uint8_t *buf, int buf_size)
{
    DVDSubContext *ctx = avctx->priv_data;

    if (ctx->buf_size >= sizeof(ctx->buf) - buf_size) {
        av_em_log(avctx, AV_LOG_WARNING, "Attempt to reconstruct "
...
static int dvdsub_decode(AVEMCodecContext *avctx,
...
static int parse_ifo_palette(DVDSubContext *ctx, char *p)
{
    ...
    if ((ifo = fopen(p, "r")) == NULL) {
        av_em_log(ctx, AV_LOG_WARNING, "Unable to open IFO file \"%s\": %s\n", p, av_err2str(AVERROR(errno)));
        return AVERROR_EOF;
    }
    if (fread(ifostr, 12, 1, ifo) != 1 || memcmp(ifostr, "DVDVIDEO-VTS", 12)) {
        av_em_log(ctx, AV_LOG_WARNING, "\"%s\" is not a proper IFO file\n", p);
    ...
    if (ctx->has_palette == 0) {
        av_em_log(ctx, AV_LOG_WARNING, "Failed to read palette from IFO file \"%s\"\n", p);
        ret = AVERROR_INVALIDDATA;
    }
end:
    fclose(ifo);
...
static int dvdsub_parse_extradata(AVEMCodecContext *avctx)
{
    ...
    dataorig = data = av_em_alloc(avctx->extradata_size+1);
    if (!data)
...
static av_cold int dvdsub_init(AVEMCodecContext *avctx)
{
    DVDSubContext *ctx = avctx->priv_data;
    int ret;
    ...
    if (ctx->has_palette) {
        int i;
        av_em_log(avctx, AV_LOG_DEBUG, "palette:");
        for(i=0;i<16;i++)
            av_em_log(avctx, AV_LOG_DEBUG, " 0x%06x", ctx->palette[i]);
...
static void dvdsub_flush(AVEMCodecContext *avctx)
{
    DVDSubContext *ctx = avctx->priv_data;
    ctx->buf_size = 0;
}

static av_cold int dvdsub_close(AVEMCodecContext *avctx)
{
    dvdsub_flush(avctx);
    return 0;
}
...
static const AVEMClass dvdsub_class = {
