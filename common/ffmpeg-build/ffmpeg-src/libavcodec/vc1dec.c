...
{
    AVEMCodecContext *avctx = v->s.avctx;
    int sprite, i;

    for (sprite = 0; sprite <= v->two_sprites; sprite++) {
        vc1_sprite_parse_transform(gb, sd->coefs[sprite]);
        if (sd->coefs[sprite][1] || sd->coefs[sprite][3])
            avpriv_em_request_sample(avctx, "Non-zero rotation coefficients");
        av_em_log(avctx, AV_LOG_DEBUG, sprite ? "S2:" : "S1:");
        for (i = 0; i < 7; i++)
            av_em_log(avctx, AV_LOG_DEBUG, " %d.%.3d",
    ...
    if (sd->effect_type = get_bits_long(gb, 30)) {
        ...
        if (sd->effect_type != 13 || sd->effect_params1[0] != sd->coefs[0][6]) {
            ...
            av_em_log(avctx, AV_LOG_DEBUG, "Effect: %d; params: ", sd->effect_type);
            for (i = 0; i < sd->effect_pcount1; i++)
                av_em_log(avctx, AV_LOG_DEBUG, " %d.%.2d",
        ...
        if (sd->effect_pcount2 > 10) {
            av_em_log(avctx, AV_LOG_ERROR, "Too many effect parameters\n");
            return AVERROR_INVALIDDATA;
        } else if (sd->effect_pcount2) {
            i = -1;
            av_em_log(avctx, AV_LOG_DEBUG, "Effect params 2: ");
    ...
    if (sd->effect_flag = get_bits1(gb))
        av_em_log(avctx, AV_LOG_DEBUG, "Effect flag set\n");

    if (get_bits_count(gb) >= gb->size_in_bits +
       (avctx->codec_id == AV_CODEC_ID_WMV3IMAGE ? 64 : 0)) {
        av_em_log(avctx, AV_LOG_ERROR, "Buffer overrun\n");
        return AVERROR_INVALIDDATA;
    }
    if (get_bits_count(gb) < gb->size_in_bits - 8)
        av_em_log(avctx, AV_LOG_WARNING, "Buffer not fully read\n");
...
static int vc1_decode_sprites(VC1Context *v, GetBitContext* gb)
{
    ...
    if (!s->current_picture.f || !s->current_picture.f->data[0]) {
        av_em_log(avctx, AV_LOG_ERROR, "Got no sprites\n");
        return AVERROR_UNKNOWN;
    }

    if (v->two_sprites && (!s->last_picture_ptr || !s->last_picture.f->data[0])) {
        av_em_log(avctx, AV_LOG_WARNING, "Need two sprites, only got one\n");
        v->two_sprites = 0;
    }

    av_em_frame_unref(v->sprite_output_frame);
    if ((ret = em_get_buffer(avctx, v->sprite_output_frame, 0)) < 0)
        return ret;
...
static void vc1_sprite_flush(AVEMCodecContext *avctx)
{
...
av_cold int ff_vc1_decode_init_alloc_tables(VC1Context *v)
{
    ...
    v->fieldtx_plane    = av_em_mallocz(s->mb_stride * mb_height);
    ...
    v->block            = av_em_alloc(sizeof(*v->block) * v->n_allocated_blks);
    v->cbp_base         = av_em_alloc(sizeof(v->cbp_base[0]) * 2 * s->mb_stride);
    if (!v->block || !v->cbp_base)
        goto error;
    v->cbp              = v->cbp_base + s->mb_stride;
    v->ttblk_base       = av_em_alloc(sizeof(v->ttblk_base[0]) * 2 * s->mb_stride);
    if (!v->ttblk_base)
        goto error;
    v->ttblk            = v->ttblk_base + s->mb_stride;
    v->is_intra_base    = av_em_mallocz(sizeof(v->is_intra_base[0]) * 2 * s->mb_stride);
    if (!v->is_intra_base)
        goto error;
    v->is_intra         = v->is_intra_base + s->mb_stride;
    v->luma_mv_base     = av_em_mallocz(sizeof(v->luma_mv_base[0]) * 2 * s->mb_stride);
    if (!v->luma_mv_base)
        goto error;
    v->luma_mv          = v->luma_mv_base + s->mb_stride;

    /* allocate block type info in that way so it could be used with s->block_index[] */
    v->mb_type_base = av_em_alloc(s->b8_stride * (mb_height * 2 + 1) + s->mb_stride * (mb_height + 1) * 2);
    if (!v->mb_type_base)
        goto error;
    ...
    v->blk_mv_type_base = av_em_mallocz(     s->b8_stride * (mb_height * 2 + 1) + s->mb_stride * (mb_height + 1) * 2);
    if (!v->blk_mv_type_base)
        goto error;
    v->blk_mv_type      = v->blk_mv_type_base + s->b8_stride + 1;
    v->mv_f_base        = av_em_mallocz(2 * (s->b8_stride * (mb_height * 2 + 1) + s->mb_stride * (mb_height + 1) * 2));
    if (!v->mv_f_base)
        goto error;
    v->mv_f[0]          = v->mv_f_base + s->b8_stride + 1;
    v->mv_f[1]          = v->mv_f[0] + (s->b8_stride * (mb_height * 2 + 1) + s->mb_stride * (mb_height + 1) * 2);
    v->mv_f_next_base   = av_em_mallocz(2 * (s->b8_stride * (mb_height * 2 + 1) + s->mb_stride * (mb_height + 1) * 2));
    if (!v->mv_f_next_base)
        goto error;
    ...
    if (s->avctx->codec_id == AV_CODEC_ID_WMV3IMAGE || s->avctx->codec_id == AV_CODEC_ID_VC1IMAGE) {
        for (i = 0; i < 4; i++)
            if (!(v->sr_rows[i >> 1][i & 1] = av_em_alloc(v->output_width)))
 ...
 */
static av_cold int vc1_decode_init(AVEMCodecContext *avctx)
{
    VC1Context *v = avctx->priv_data;
    MpegEncContext *s = &v->s;
    GetBitContext gb;
    ...
    if (avctx->codec_id == AV_CODEC_ID_WMV3 || avctx->codec_id == AV_CODEC_ID_WMV3IMAGE) {
        ...
        if (count > 0) {
            av_em_log(avctx, AV_LOG_INFO, "Extra data: %i bits left, value: %X\n",
                   count, get_bits_long(&gb, FFMIN(count, 32)));
        } else if (count < 0) {
            av_em_log(avctx, AV_LOG_INFO, "Read %i bits in overflow\n", -count);
        }
    } else { // VC1/WVC1/WVP2
        ...
        if (avctx->extradata_size < 16) {
            av_em_log(avctx, AV_LOG_ERROR, "Extradata size too small: %i\n", avctx->extradata_size);
            return -1;
        }

        buf2  = av_em_mallocz(avctx->extradata_size + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!buf2)
            return AVERROR(ENOMEM);
        ...
        if (!seq_initialized || !ep_initialized) {
            av_em_log(avctx, AV_LOG_ERROR, "Incomplete extradata\n");
            return -1;
        }
    ...
    if (!CONFIG_GRAY || !(avctx->flags & AV_CODEC_FLAG_GRAY))
        avctx->pix_fmt = em_get_format(avctx, avctx->codec->pix_fmts);
    else {
    ...
    if (avctx->codec_id == AV_CODEC_ID_WMV3IMAGE || avctx->codec_id == AV_CODEC_ID_VC1IMAGE) {
        ...
        if ((v->sprite_width&1) || (v->sprite_height&1)) {
            avpriv_em_request_sample(avctx, "odd sprites support");
 ...
 */
av_cold int ff_vc1_decode_end(AVEMCodecContext *avctx)
{
    VC1Context *v = avctx->priv_data;
    int i;

    av_em_frame_free(&v->sprite_output_frame);

    for (i = 0; i < 4; i++)
        av_em_freep(&v->sr_rows[i >> 1][i & 1]);
    av_em_freep(&v->hrd_rate);
    av_em_freep(&v->hrd_buffer);
    ff_mpv_common_end(&v->s);
    av_em_freep(&v->mv_type_mb_plane);
    av_em_freep(&v->direct_mb_plane);
    av_em_freep(&v->forward_mb_plane);
    av_em_freep(&v->fieldtx_plane);
    av_em_freep(&v->acpred_plane);
    av_em_freep(&v->over_flags_plane);
    av_em_freep(&v->mb_type_base);
    av_em_freep(&v->blk_mv_type_base);
    av_em_freep(&v->mv_f_base);
    av_em_freep(&v->mv_f_next_base);
    av_em_freep(&v->block);
    av_em_freep(&v->cbp_base);
    av_em_freep(&v->ttblk_base);
    av_em_freep(&v->is_intra_base); // FIXME use v->mb_type[]
    av_em_freep(&v->luma_mv_base);
 ...
 * @todo TODO: Handle VC-1 IDUs (Transport level?)
 */
static int vc1_decode_frame(AVEMCodecContext *avctx, void *data,
                            int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (buf_size == 0 || (buf_size == 4 && AV_RB32(buf) == VC1_CODE_ENDOFSEQ)) {
        ...
        if (s->low_delay == 0 && s->next_picture_ptr) {
            if ((ret = av_em_frame_ref(pict, s->next_picture_ptr->f)) < 0)
                return ret;
    ...
    if (avctx->codec_id == AV_CODEC_ID_VC1 || avctx->codec_id == AV_CODEC_ID_VC1IMAGE) {
        int buf_size2 = 0;
        buf2 = av_em_mallocz(buf_size + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!buf2)
            return AVERROR(ENOMEM);
        ...
        if (IS_MARKER(AV_RB32(buf))) { /* frame starts with marker and needs to be parsed */
            ...
            for (start = buf, end = buf + buf_size; next < end; start = next) {
                ...
                case VC1_CODE_FIELD: {
                    ...
                    tmp = av_em_realloc_array(slices, sizeof(*slices), (n_slices+1));
                    if (!tmp) {
                        ret = AVERROR(ENOMEM);
                        goto err;
                    }
                    slices = tmp;
                    slices[n_slices].buf = av_em_mallocz(buf_size + AV_INPUT_BUFFER_PADDING_SIZE);
            ...
            if ((divider == (buf + buf_size)) || AV_RB32(divider) != VC1_CODE_FIELD) {
                av_em_log(avctx, AV_LOG_ERROR, "Error in WVC1 interlaced frame\n");
    ...
    if (v->res_sprite) {
        ...
        if (avctx->codec_id == AV_CODEC_ID_WMV3IMAGE || avctx->codec_id == AV_CODEC_ID_VC1IMAGE) {
            if (v->new_sprite) {
                // switch AVEMCodecContext parameters to those of the sprites
    ...
    if (avctx->debug & FF_DEBUG_PICT_INFO)
        av_em_log(v->s.avctx, AV_LOG_DEBUG, "pict_type: %c\n", av_em_get_picture_type_char(s->pict_type));

    if ((avctx->codec_id == AV_CODEC_ID_WMV3IMAGE || avctx->codec_id == AV_CODEC_ID_VC1IMAGE)
        && s->pict_type != AV_PICTURE_TYPE_I) {
        av_em_log(v->s.avctx, AV_LOG_ERROR, "Sprite decoder: expected I-frame\n");
        ret = AVERROR_INVALIDDATA;
        goto err;
    }

    if ((s->mb_height >> v->field_mode) == 0) {
        av_em_log(v->s.avctx, AV_LOG_ERROR, "image too short\n");
        ret = AVERROR_INVALIDDATA;
        goto err;
    }
    ...
    if (!s->last_picture_ptr && (s->pict_type == AV_PICTURE_TYPE_B || s->droppable)) {
        av_em_log(v->s.avctx, AV_LOG_DEBUG, "Skipping B frame without reference frames\n");
        goto end;
    }
    ...
    if (avctx->hwaccel) {
        if (v->field_mode && buf_start_second_field) {
            ...
            if (ff_vc1_parse_frame_header_adv(v, &s->gb) < 0) {
                av_em_log(avctx, AV_LOG_ERROR, "parsing header for second field failed");
                ret = AVERROR_INVALIDDATA;
                goto err;
            }
        ...
        for (i = 0; i <= n_slices; i++) {
            if (i > 0 &&  slices[i - 1].mby_start >= mb_height) {
                if (v->field_mode <= 0) {
                    av_em_log(v->s.avctx, AV_LOG_ERROR, "Slice %d starts beyond "
            ...
            if (i) {
                ...
                if (v->field_mode && i == n_slices1 + 2) {
                    if ((header_ret = ff_vc1_parse_frame_header_adv(v, &s->gb)) < 0) {
                        av_em_log(v->s.avctx, AV_LOG_ERROR, "Field header damaged\n");
                        ret = AVERROR_INVALIDDATA;
                    ...
                    v->pic_header_flag = 1;
                    if ((header_ret = ff_vc1_parse_frame_header_adv(v, &s->gb)) < 0) {
                        av_em_log(v->s.avctx, AV_LOG_ERROR, "Slice header damaged\n");
                        ret = AVERROR_INVALIDDATA;
                        if (avctx->err_recognition & AV_EF_EXPLODE)
                            goto err;
                        continue;
            ...
            if (!v->field_mode || v->second_field)
                ...
                if (i >= n_slices) {
                    av_em_log(v->s.avctx, AV_LOG_ERROR, "first field slice count too large\n");
                    continue;
                }
                s->end_mb_y = (i <= n_slices1 + 1) ? mb_height : FFMIN(mb_height, slices[i].mby_start % mb_height);
            }
            if (s->end_mb_y <= s->start_mb_y) {
                av_em_log(v->s.avctx, AV_LOG_ERROR, "end mb y %d %d invalid\n", s->end_mb_y, s->start_mb_y);
                continue;
            }
            if (!v->p_frame_skipped && s->pict_type != AV_PICTURE_TYPE_I && !v->cbpcy_vlc) {
                av_em_log(v->s.avctx, AV_LOG_ERROR, "missing cbpcy_vlc\n");
                continue;
            }
...
image:
        ...
        if ((ret = vc1_decode_sprites(v, &s->gb)) < 0)
            goto err;
#endif
        if ((ret = av_em_frame_ref(pict, v->sprite_output_frame)) < 0)
            goto err;
        *got_frame = 1;
    } else {
        if (s->pict_type == AV_PICTURE_TYPE_B || s->low_delay) {
            if ((ret = av_em_frame_ref(pict, s->current_picture_ptr->f)) < 0)
                goto err;
            ff_print_debug_info(s, s->current_picture_ptr, pict);
            *got_frame = 1;
        } else if (s->last_picture_ptr) {
            if ((ret = av_em_frame_ref(pict, s->last_picture_ptr->f)) < 0)
                goto err;
...
AVEMCodec ff_wmv3_vdpau_decoder = {
    .name           = "wmv3_vdpau",
...
AVEMCodec ff_vc1_vdpau_decoder = {
    .name           = "vc1_vdpau",
...
AVEMCodec ff_wmv3image_decoder = {
    .name           = "wmv3image",
