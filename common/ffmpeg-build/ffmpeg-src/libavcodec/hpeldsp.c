...
av_cold void em_hpeldsp_init(HpelDSPContext *c, int flags)
{
#define hpel_funcs(prefix, idx, num) \
    ...
    if (ARCH_AARCH64)
        em_hpeldsp_init_aarch64(c, flags);
    if (ARCH_ALPHA)
        em_hpeldsp_init_alpha(c, flags);
    if (ARCH_ARM)
        em_hpeldsp_init_arm(c, flags);
    if (ARCH_PPC)
        em_hpeldsp_init_ppc(c, flags);
    if (ARCH_X86)
        em_hpeldsp_init_x86(c, flags);
    if (ARCH_MIPS)
        em_hpeldsp_init_mips(c, flags);
}
