...
static void fill_picture_parameters(const AVEMCodecContext *avctx, AVDXVAContext *ctx, const H264EMContext *h,
                                    DXVA_PicParams_H264 *pp)
{
...
static void fill_scaling_lists(const AVEMCodecContext *avctx, AVDXVAContext *ctx, const H264EMContext *h, DXVA_Qmatrix_H264 *qm)
{
    const PPS *pps = h->ps.pps;
    unsigned i, j;
...
static int is_slice_short(const AVEMCodecContext *avctx, AVDXVAContext *ctx)
{
...
static void fill_slice_long(AVEMCodecContext *avctx, DXVA_Slice_H264_Long *slice,
                            const DXVA_PicParams_H264 *pp, unsigned position, unsigned size)
{
    const H264EMContext *h = avctx->priv_data;
    ...
    slice->slice_type            = em_h264_get_slice_type(sl);
...
static int commit_bitstream_and_slice_buffer(AVEMCodecContext *avctx,
                                             ...
                                             DECODER_BUFFER_DESC *sc)
{
    ...
    for (i = 0; i < ctx_pic->slice_count; i++) {
        ...
        if (start_code_size + size > end - current) {
            av_em_log(avctx, AV_LOG_ERROR, "Failed to build bitstream");
            break;
        }
...
static int dxva2_h264_start_frame(AVEMCodecContext *avctx,
...
static int dxva2_h264_decode_slice(AVEMCodecContext *avctx,
...
static int dxva2_h264_end_frame(AVEMCodecContext *avctx)
{
    ...
    if (!ret)
        em_h264_draw_horiz_band(h, sl, 0, h->avctx->height);
    return ret;
}

#if CONFIG_H264_DXVA2_HWACCEL
AVHWAccel em_h264_dxva2_hwaccel = {
    .name           = "h264_dxva2",
...
AVHWAccel em_h264_d3d11va_hwaccel = {
    .name           = "h264_d3d11va",
    .type           = AVMEDIA_TYPE_VIDEO,
    .id             = AV_CODEC_ID_H264,
