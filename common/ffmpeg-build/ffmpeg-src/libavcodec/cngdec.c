...
static av_cold int cng_decode_close(AVEMCodecContext *avctx)
{
    CNGContext *p = avctx->priv_data;
    av_em_freep(&p->refl_coef);
    av_em_freep(&p->target_refl_coef);
    av_em_freep(&p->lpc_coef);
    av_em_freep(&p->filter_out);
    av_em_freep(&p->excitation);
    return 0;
}

static av_cold int cng_decode_init(AVEMCodecContext *avctx)
{
    CNGContext *p = avctx->priv_data;
    ...
    p->refl_coef        = av_em_mallocz_array(p->order, sizeof(*p->refl_coef));
    p->target_refl_coef = av_em_mallocz_array(p->order, sizeof(*p->target_refl_coef));
    p->lpc_coef         = av_em_mallocz_array(p->order, sizeof(*p->lpc_coef));
    p->filter_out       = av_em_mallocz_array(avctx->frame_size + p->order,
                                     sizeof(*p->filter_out));
    p->excitation       = av_em_mallocz_array(avctx->frame_size, sizeof(*p->excitation));
...
static void cng_decode_flush(AVEMCodecContext *avctx)
{
    CNGContext *p = avctx->priv_data;
    p->inited = 0;
}

static int cng_decode_frame(AVEMCodecContext *avctx, void *data,
...
AVEMCodec ff_comfortnoise_decoder = {
    .name           = "comfortnoise",
