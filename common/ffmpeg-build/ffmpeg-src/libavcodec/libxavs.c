...
static int XAVS_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                      const AVFrame *frame, int *got_packet)
{
    ...
    if (!ret) {
        if (!frame && !(x4->end_of_stream)) {
            if ((ret = em_alloc_packet2(avctx, pkt, 4, 0)) < 0)
                return ret;
...
static av_cold int XAVS_close(AVEMCodecContext *avctx)
{
    XavsContext *x4 = avctx->priv_data;

    av_em_freep(&avctx->extradata);
    av_em_freep(&x4->sei);
    av_em_freep(&x4->pts_buffer);
...
static av_cold int XAVS_init(AVEMCodecContext *avctx)
{
    XavsContext *x4 = avctx->priv_data;
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (!(x4->pts_buffer = av_em_mallocz_array((avctx->max_b_frames+1), sizeof(*x4->pts_buffer))))
    ...
    if (avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER && 0) {
        ...
        avctx->extradata = p = av_em_alloc(s);
        for (i = 0; i < nnal; i++) {
            /* Don't put the SEI in extradata. */
            if (nal[i].i_type == NAL_SEI) {
                x4->sei = av_em_alloc( 5 + nal[i].i_payload * 4 / 3 );
...
static const AVEMClass xavs_class = {
    .class_name = "libxavs",
...
static const AVEMCodecDefault xavs_defaults[] = {
    { "b",                "0" },
    { NULL },
};

AVEMCodec ff_libxavs_encoder = {
    .name           = "libxavs",
