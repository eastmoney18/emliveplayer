...
static void vc1_extract_header(AVEMCodecParserContext *s, AVEMCodecContext *avctx,
...
static int vc1_parse(AVEMCodecParserContext *s,
                           ...
                           const uint8_t *buf, int buf_size)
{
    ...
    if (s->flags & PARSER_FLAG_COMPLETE_FRAMES) {
        ...
        if (em_combine_frame(&vpc->pc, next, &buf, &buf_size) < 0) {
            vpc->bytes_to_skip = 0;
            *poutbuf = NULL;
            *poutbuf_size = 0;
            return buf_size;
...
static int vc1_split(AVEMCodecContext *avctx,
                           const uint8_t *buf, int buf_size)
{
    uint32_t state = -1;
    int charged = 0;
...
static av_cold int vc1_parse_init(AVEMCodecParserContext *s)
{
    VC1ParseContext *vpc = s->priv_data;
