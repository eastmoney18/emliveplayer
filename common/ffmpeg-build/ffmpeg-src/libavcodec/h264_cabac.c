...
static int decode_cabac_field_decoding_flag(const H264EMContext *h, H264SliceContext *sl)
{
...
static int decode_cabac_mb_skip(const H264EMContext *h, H264SliceContext *sl,
                                int mb_x, int mb_y)
{
    int mba_xy, mbb_xy;
    int ctx = 0;
...
static int decode_cabac_mb_chroma_pre_mode(const H264EMContext *h, H264SliceContext *sl)
{
...
//    if(!get_cabac(&sl->cabac, &sl->cabac_state[ctxbase+(amvd>2)+(amvd>32)])){
    ...
    if( mvd >= 9 ) {
        ...
        while( get_cabac_bypass( &sl->cabac ) ) {
            ...
            k++;
            if(k>24){
                av_em_log(sl->h264->avctx, AV_LOG_ERROR, "overflow in decode_cabac_mb_mvd\n");
                return INT_MIN;
            }
        }
        while( k-- ) {
...
decode_cabac_residual_internal(const H264EMContext *h, H264SliceContext *sl,
                               int16_t *block,
...
#else
        DECODE_SIGNIFICANCE( 63, sig_off[last], em_h264_last_coeff_flag_offset_8x8[last] );
    } else {
        if (is_dc && chroma422) { // dc 422
            DECODE_SIGNIFICANCE(7, sig_coeff_offset_dc[last], sig_coeff_offset_dc[last]);
        } else {
...
static av_noinline void decode_cabac_residual_dc_internal(const H264EMContext *h,
                                                          H264SliceContext *sl,
                                                          int16_t *block,
                                                          int cat, int n,
...
static av_noinline void decode_cabac_residual_dc_internal_422(const H264EMContext *h,
                                                              H264SliceContext *sl,
                                                              int16_t *block,
                                                              int cat, int n,
                                                              const uint8_t *scantable,
...
static av_noinline void decode_cabac_residual_nondc_internal(const H264EMContext *h,
                                                             H264SliceContext *sl,
                                                             int16_t *block,
                                                             int cat, int n,
                                                             const uint8_t *scantable,
...
static av_always_inline void decode_cabac_residual_dc(const H264EMContext *h,
                                                      H264SliceContext *sl,
                                                      int16_t *block,
                                                      int cat, int n,
                                                      const uint8_t *scantable,
...
static av_always_inline void
decode_cabac_residual_dc_422(const H264EMContext *h, H264SliceContext *sl,
                             int16_t *block,
                             int cat, int n, const uint8_t *scantable,
                             int max_coeff)
{
...
static av_always_inline void decode_cabac_residual_nondc(const H264EMContext *h,
                                                         H264SliceContext *sl,
                                                         int16_t *block,
                                                         int cat, int n,
                                                         const uint8_t *scantable,
...
static av_always_inline void decode_cabac_luma_residual(const H264EMContext *h, H264SliceContext *sl,
...
decode_intra_mb:
    ...
    if(IS_INTRA_PCM(mb_type)) {
        const int mb_size = em_h264_mb_sizes[sps->chroma_format_idc] *
        ...
        ret = em_init_cabac_decoder(&sl->cabac, ptr, sl->cabac.bytestream_end - ptr);
        if (ret < 0)
            return ret;
                ...
                for( i = 0; i < 4; i++ ) {
                    ...
                    if(IS_DIR(sl->sub_mb_type[i], 0, list)){
                        ...
                        if (rc > 1) {
                            ...
                            if (ref[list][i] >= rc) {
                                av_em_log(h->avctx, AV_LOG_ERROR, "Reference %d >= %d\n", ref[list][i], rc);
                                return -1;
                            }
                        }else
                            ref[list][i] = 0;
        ...
        if(get_cabac_noinline( &sl->cabac, &sl->cabac_state[60 + (sl->last_qscale_diff != 0)])){
            ...
            while( get_cabac_noinline( &sl->cabac, &sl->cabac_state[60 + ctx] ) ) {
                ...
                if(val > 2*max_qp){ //prevent infinite loop
                    av_em_log(h->avctx, AV_LOG_ERROR, "cabac decode of qscale diff failed at %d %d\n", sl->mb_x, sl->mb_y);
                    return -1;
                }
            }
            ...
            if( cbp&0x30 ){
                ...
                for (c = 0; c < 2; c++)
                    decode_cabac_residual_dc(h, sl, sl->mb + ((256 + 16 * 16 * c) << pixel_shift),
                                             3, CHROMA_DC_BLOCK_INDEX + c, em_h264_chroma_dc_scan, 4);
            }
