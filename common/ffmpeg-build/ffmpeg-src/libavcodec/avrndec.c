...
static av_cold int init(AVEMCodecContext *avctx)
{
    ...
    if(!a->is_mjpeg && avctx->lowres) {
        av_em_log(avctx, AV_LOG_ERROR, "lowres is not possible with rawvideo\n");
        return AVERROR(EINVAL);
    }

    if(a->is_mjpeg) {
        AVEMCodec *codec = avcodec_em_find_decoder(AV_CODEC_ID_MJPEG);
        ...
        a->mjpeg_avctx = avcodec_em_alloc_context3(codec);

        av_em_dict_set(&thread_opt, "threads", "1", 0); // Is this needed ?
        ...
        if ((ret = em_codec_open2_recursive(a->mjpeg_avctx, codec, &thread_opt)) < 0) {
...
static av_cold int end(AVEMCodecContext *avctx)
{
    AVRnContext *a = avctx->priv_data;

    avcodec_em_close(a->mjpeg_avctx);
    av_em_freep(&a->mjpeg_avctx);
                        ...
                        int *got_frame, AVEMPacket *avpkt)
{
    ...
    if(a->is_mjpeg) {
        ret = avcodec_em_decode_video2(a->mjpeg_avctx, data, got_frame, avpkt);

        if (ret >= 0 && *got_frame && avctx->width <= p->width && avctx->height <= p->height) {
            int shift = p->height - avctx->height;
            int subsample_h, subsample_v;

            av_em_pix_fmt_get_chroma_sub_sample(p->format, &subsample_h, &subsample_v);
