...
static void vp6_parse_coeem_huffman(VP56Context *s);
...
static int vp6_parse_header(VP56Context *s, const uint8_t *buf, int buf_size)
{
    ...
    if (s->frames[VP56_FRAME_CURRENT]->key_frame) {
        ...
        if (buf[1] & 1) {
            avpriv_em_report_missing_feature(s->avctx, "Interlacing");
        ...
        if (!rows || !cols) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid size %dx%d\n", cols << 4, rows << 4);
            return AVERROR_INVALIDDATA;
        }
        ...
        if (!s->macroblocks || /* first frame */
            ...
            if (s->avctx->extradata_size == 0 &&
                ...
                int ret = em_set_dimensions(s->avctx, 16 * cols, 16 * rows);
                if (ret < 0)
                    return ret;
    ...
    if (coeff_offset) {
        ...
        if (buf_size < 0) {
            if (s->frames[VP56_FRAME_CURRENT]->key_frame)
                em_set_dimensions(s->avctx, 0, 0);
            return AVERROR_INVALIDDATA;
        }
        if (s->use_huffman) {
            s->parse_coeff = vp6_parse_coeem_huffman;
                               ...
                               const uint8_t *map, unsigned size, VLC *vlc)
{
    ...
    return em_huff_build_tree(s->avctx, vlc, size, FF_HUFFMAN_BITS,
                              nodes, vp6_huff_cmp,
...
static av_cold int vp6_decode_init(AVEMCodecContext *avctx)
{
    ...
    if (s->has_alpha) {
        s->alpha_context = av_em_mallocz(sizeof(VP56Context));
...
static av_cold int vp6_decode_free(AVEMCodecContext *avctx)
{
    VP56Context *s = avctx->priv_data;
