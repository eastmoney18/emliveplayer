...
{
    avctx->subtitle_header = av_em_asprintf(
             "[Script Info]\r\n"
             ...
             !(avctx->flags & AV_CODEC_FLAG_BITEXACT) ? AV_STRINGIFY(LIBavcodec_em_version) : "",
...
{
    return av_em_asprintf("%d,%d,%s,%s,0,0,0,,%s",
                    ...
                    const char *speaker)
{
    ...
    rects = av_em_realloc_array(sub->rects, (sub->num_rects+1), sizeof(*sub->rects));
    if (!rects)
        return AVERROR(ENOMEM);
    sub->rects = rects;
    rects[sub->num_rects]       = av_em_mallocz(sizeof(*rects[0]));
