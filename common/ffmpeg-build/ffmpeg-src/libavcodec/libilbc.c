...
static int get_mode(AVEMCodecContext *avctx)
{
...
static const AVEMClass ilbc_dec_class = {
    .class_name = "libilbc",
...
static av_cold int ilbc_decode_init(AVEMCodecContext *avctx)
{
    ILBCDecContext *s  = avctx->priv_data;
    int mode;

    if ((mode = get_mode(avctx)) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "iLBC frame mode not indicated\n");
...
static int ilbc_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (s->decoder.no_of_bytes > buf_size) {
        av_em_log(avctx, AV_LOG_ERROR, "iLBC frame too short (%u, should be %u)\n",
...
static const AVEMClass ilbc_enc_class = {
    .class_name = "libilbc",
    .item_name  = av_em_default_item_name,
...
static av_cold int ilbc_encode_init(AVEMCodecContext *avctx)
{
...
static int ilbc_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                             const AVFrame *frame, int *got_packet_ptr)
{
    ILBCEncContext *s = avctx->priv_data;
    int ret;

    if ((ret = em_alloc_packet2(avctx, avpkt, 50, 0)) < 0)
        return ret;
...
static const AVEMCodecDefault ilbc_encode_defaults[] = {
    { "b", "0" },
    { NULL }
};

AVEMCodec ff_libilbc_encoder = {
    .name           = "libilbc",
    .long_name      = NULL_IF_CONFIG_SMALL("iLBC (Internet Low Bitrate Codec)"),
    .type           = AVMEDIA_TYPE_AUDIO,
    .id             = AV_CODEC_ID_ILBC,
