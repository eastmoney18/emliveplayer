...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if(s->pred == DWT_97
       ...
       && avctx->global_quality == 0){
        av_em_log(avctx, AV_LOG_ERROR, "The 9/7 wavelet is incompatible with lossless mode.\n");
        return -1;
    }
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    s->m.me.temp      =
    s->m.me.scratchpad= av_em_mallocz_array((avctx->width+64), 2*16*2*sizeof(uint8_t));
    s->m.me.map       = av_em_mallocz(ME_MAP_SIZE*sizeof(uint32_t));
    s->m.me.score_map = av_em_mallocz(ME_MAP_SIZE*sizeof(uint32_t));
    s->m.sc.obmc_scratchpad= av_em_mallocz(MB_SIZE*MB_SIZE*12*sizeof(uint32_t));
    ...
    if(avctx->flags&AV_CODEC_FLAG_PASS1){
        if(!avctx->stats_out)
            avctx->stats_out = av_em_mallocz(256);
...
/*    case AV_PIX_FMT_RGB32:
        ...
        break;*/
    default:
        av_em_log(avctx, AV_LOG_ERROR, "pixel format not supported\n");
        return -1;
    }
    ...
    s->input_picture = av_em_frame_alloc();
    if (!s->input_picture)
        return AVERROR(ENOMEM);
    ...
    if(s->motion_est == FF_ME_ITER){
        ...
        for(i=0; i<s->max_ref_frames; i++){
            s->ref_mvs[i]= av_em_mallocz_array(size, sizeof(int16_t[2]));
            s->ref_scores[i]= av_em_mallocz_array(size, sizeof(uint32_t));
...
static void iterative_me(SnowContext *s){
    ...
    for(pass=0; pass<25; pass++){
        ...
        av_em_log(s->avctx, AV_LOG_DEBUG, "pass:%d changed:%d\n", pass, change);
        if(!change)
            break;
    }
    ...
    if(s->block_max_depth == 1){
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "pass:4mv changed:%d\n", change*4);
    }
}
                        ...
                        const AVFrame *pict, int *got_packet)
{
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, s->b_width*s->b_height*MB_SIZE*MB_SIZE*3 + AV_INPUT_BUFFER_MIN_SIZE, 0)) < 0)
        return ret;
    ...
    av_em_frame_unref(avctx->coded_frame);
    ret = av_em_frame_ref(avctx->coded_frame, s->current_picture);
    if (ret < 0)
        return ret;
...
redo_frame:
    ...
    if (s->spatial_decomposition_count <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Resolution too low\n");
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    em_side_data_set_encoder_stats(pkt, s->current_picture->quality,
                                   s->encoding_error,
...
static av_cold int encode_end(AVEMCodecContext *avctx)
{
    SnowContext *s = avctx->priv_data;

    ff_snow_common_end(s);
    ff_rate_control_uninit(&s->m);
    av_em_frame_free(&s->input_picture);
...
static const AVEMClass snowenc_class = {
