...
static int targa_decode_rle(AVEMCodecContext *avctx, TargaContext *s,
                            ...
                            int bpp, int interleave)
{
    ...
    if (count) {
        av_em_log(avctx, AV_LOG_ERROR, "Packet went out of bounds\n");
        return AVERROR_INVALIDDATA;
    }
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (!pal && (first_clr || colors || csize)) {
        av_em_log(avctx, AV_LOG_WARNING, "File without colormap has colormap information set.\n");
    ...
    case 32:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Bit depth %i is not supported\n", bpp);
        return AVERROR_INVALIDDATA;
    }

    if (colors && (colors + first_clr) > 256) {
        av_em_log(avctx, AV_LOG_ERROR, "Incorrect palette: %i colors with offset %i\n", colors, first_clr);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (colors) {
        ...
        default:
            av_em_log(avctx, AV_LOG_ERROR, "Palette entry size %i bits is not supported\n", csize);
            return AVERROR_INVALIDDATA;
        }
