...
static int rate_control(AVEMCodecContext *avctx, void *arg)
{
...
static int encode_hq_slice(AVEMCodecContext *avctx, void *arg)
{
    SliceArgs *slice_dat = arg;
    VC2EncContext *s = slice_dat->ctx;
 ...
 */
static int dwt_plane(AVEMCodecContext *avctx, void *arg)
{
...
static int encode_frame(VC2EncContext *s, AVEMPacket *avpkt, const AVFrame *frame,
                        const char *aux_data, const int header_size, int field)
{
    ...
    if (field < 2) {
        ret = em_alloc_packet2(s->avctx, avpkt,
                               max_frame_bytes << s->interlaced,
                               max_frame_bytes << s->interlaced);
        if (ret) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Error getting output packet.\n");
            return ret;
        }
...
static av_cold int vc2_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                                      const AVFrame *frame, int *got_packet)
{
    ...
    /* Rate control */
    max_frame_bytes = (av_em_rescale(r_bitrate, s->avctx->time_base.num,
                                  s->avctx->time_base.den) >> 3) - header_size;
    s->slice_max_bytes = av_em_rescale(max_frame_bytes, 1, s->num_x*s->num_y);
...
static av_cold int vc2_encode_end(AVEMCodecContext *avctx)
{
    int i;
    VC2EncContext *s = avctx->priv_data;

    av_em_log(avctx, AV_LOG_INFO, "Qavg: %i\n", s->q_avg);

    for (i = 0; i < 3; i++) {
        ff_vc2enc_free_transforms(&s->transform_args[i].t);
        av_em_freep(&s->plane[i].coef_buf);
    }

    av_em_freep(&s->slice_args);
    av_em_freep(&s->coef_lut_len);
    av_em_freep(&s->coef_lut_val);

    return 0;
}

static av_cold int vc2_encode_init(AVEMCodecContext *avctx)
{
    Plane *p;
    SubBand *b;
    int i, j, level, o, shift;
    const AVPixFmtDescriptor *fmt = av_em_pix_fmt_desc_get(avctx->pix_fmt);
    ...
    if (s->interlaced)
        av_em_log(avctx, AV_LOG_WARNING, "Interlacing enabled!\n");

    if ((s->slice_width  & (s->slice_width  - 1)) ||
        (s->slice_height & (s->slice_height - 1))) {
        av_em_log(avctx, AV_LOG_ERROR, "Slice size is not a power of two!\n");
        return AVERROR_UNKNOWN;
    }

    if ((s->slice_width > avctx->width) ||
        (s->slice_height > avctx->height)) {
        av_em_log(avctx, AV_LOG_ERROR, "Slice size is bigger than the image!\n");
        return AVERROR_UNKNOWN;
    }
    ...
    if (s->base_vf <= 0) {
        if (avctx->strict_std_compliance <= FF_COMPLIANCE_UNOFFICIAL) {
            ...
            av_em_log(avctx, AV_LOG_WARNING, "Disabling strict compliance\n");
        } else {
            av_em_log(avctx, AV_LOG_WARNING, "Given format does not strictly comply with "
                   "the specifications, please add a -strict -1 flag to use it\n");
            return AVERROR_UNKNOWN;
        }
    } else {
        av_em_log(avctx, AV_LOG_INFO, "Selected base video format = %i (%s)\n",
    ...
    for (i = 0; i < 3; i++) {
        ...
        p->coef_buf = av_em_alloc(p->coef_stride*p->dwt_height*sizeof(dwtcoef));
        if (!p->coef_buf)
            goto alloc_fail;
        for (level = s->wavelet_depth-1; level >= 0; level--) {
            w = w >> 1;
    ...
    s->slice_args = av_em_calloc(s->num_x*s->num_y, sizeof(SliceArgs));
    if (!s->slice_args)
        goto alloc_fail;

    /* Lookup tables */
    s->coef_lut_len = av_em_alloc(COEF_LUT_TAB*(s->q_ceil+1)*sizeof(*s->coef_lut_len));
    if (!s->coef_lut_len)
        goto alloc_fail;

    s->coef_lut_val = av_em_alloc(COEF_LUT_TAB*(s->q_ceil+1)*sizeof(*s->coef_lut_val));
    if (!s->coef_lut_val)
        goto alloc_fail;
...
alloc_fail:
    ...
    av_em_log(avctx, AV_LOG_ERROR, "Unable to allocate memory!\n");
...
static const AVEMClass vc2enc_class = {
...
static const AVEMCodecDefault vc2enc_defaults[] = {
    { "b",              "600000000"   },
    { NULL },
};
