...
static av_cold int libwebp_encode_init(AVEMCodecContext *avctx)
{
    return ff_libwebp_encode_init_common(avctx);
}

static int libwebp_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                                const AVFrame *frame, int *got_packet)
{
    ...
    if (!ret) {
        av_em_log(avctx, AV_LOG_ERROR, "WebPEncode() failed with error: %d\n",
               pic->error_code);
        ret = ff_libwebp_error_to_averror(pic->error_code);
        goto end;
    }

    ret = em_alloc_packet2(avctx, pkt, mw.size, mw.size);
    if (ret < 0)
        goto end;
...
static int libwebp_encode_close(AVEMCodecContext *avctx)
{
    LibWebPContextCommon *s  = avctx->priv_data;
    av_em_frame_free(&s->ref);
