                                                   ...
                                                   int index)
{
    ...
    if (av_em_get_channel_layout_nb_channels(channel_layout) <= index)
        return 0;
...
static av_cold int mlp_decode_init(AVEMCodecContext *avctx)
{
...
static int read_major_sync(MLPDecodeContext *m, GetBitContext *gb)
{
    ...
    if (mh.group1_bits == 0) {
        av_em_log(m->avctx, AV_LOG_ERROR, "invalid/unknown bits per sample\n");
        return AVERROR_INVALIDDATA;
    }
    if (mh.group2_bits > mh.group1_bits) {
        av_em_log(m->avctx, AV_LOG_ERROR,
    ...
    if (mh.group1_samplerate == 0) {
        av_em_log(m->avctx, AV_LOG_ERROR, "invalid/unknown sampling rate\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (m->avctx->codec_id == AV_CODEC_ID_MLP && mh.num_substreams > 2) {
        av_em_log(m->avctx, AV_LOG_ERROR, "MLP only supports up to 2 substreams.\n");
        return AVERROR_INVALIDDATA;
    }
    if (mh.num_substreams > MAX_SUBSTREAMS) {
        avpriv_em_request_sample(m->avctx,
    ...
    if (m->avctx->codec_id == AV_CODEC_ID_MLP) {
        ...
        if (m->avctx->channels<=2 && m->substream[substr].ch_layout == AV_CH_LAYOUT_MONO && m->max_decoded_substream == 1) {
            av_em_log(m->avctx, AV_LOG_DEBUG, "Mono stream with 2 substreams, ignoring 2nd\n");
                               ...
                               const uint8_t *buf, unsigned int substr)
{
    ...
    if (m->avctx->codec_id == AV_CODEC_ID_MLP && s->noise_type) {
        av_em_log(m->avctx, AV_LOG_ERROR, "MLP must have 0x31ea sync word.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (m->avctx->request_channel_layout && (s->ch_layout & m->avctx->request_channel_layout) ==
        ...
        av_em_log(m->avctx, AV_LOG_DEBUG,
    ...
    if (substr == m->max_decoded_substream
        ...
        if (tmp != lossless_check)
            av_em_log(m->avctx, AV_LOG_WARNING,
    ...
    for (ch = 0; ch <= s->max_matrix_channel; ch++) {
        ...
        if (m->avctx->codec_id == AV_CODEC_ID_TRUEHD) {
            uint64_t channel = thd_channel_layout_extract_channel(s->ch_layout,
                                                                  ch_assign);
            ch_assign = av_em_get_channel_layout_channel_index(s->ch_layout,
                                                            channel);
        }
    ...
    if (checksum != get_bits(gbp, 8))
        av_em_log(m->avctx, AV_LOG_ERROR, "restart header checksum error\n");
                              ...
                              unsigned int filter)
{
    ...
    if (m->filter_changed[channel][filter]++ > 1) {
        av_em_log(m->avctx, AV_LOG_ERROR, "Filters may change only once per access unit.\n");
        return AVERROR_INVALIDDATA;
    }
...
static int read_matrix_params(MLPDecodeContext *m, unsigned int substr, GetBitContext *gbp)
{
    ...
    if (m->matrix_changed++ > 1) {
        av_em_log(m->avctx, AV_LOG_ERROR, "Matrices may change only once per access unit.\n");
        return AVERROR_INVALIDDATA;
    }
                               ...
                               GetBitContext *gbp, unsigned int ch)
{
    ...
    if (fir->order + iir->order > 8) {
        av_em_log(m->avctx, AV_LOG_ERROR, "Total filter orders too high.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (cp->huff_lsbs > 24) {
        av_em_log(m->avctx, AV_LOG_ERROR, "Invalid huff_lsbs.\n");
        cp->huff_lsbs = 0;
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->param_presence_flags & PARAM_BLOCKSIZE)
        if (get_bits1(gbp)) {
            ...
            if (s->blocksize < 8 || s->blocksize > m->access_unit_size) {
                av_em_log(m->avctx, AV_LOG_ERROR, "Invalid blocksize.\n");
                s->blocksize = 0;
                return AVERROR_INVALIDDATA;
            }
        }
                           ...
                           unsigned int substr)
{
    ...
    if (s->blockpos + s->blocksize > m->access_unit_size) {
        av_em_log(m->avctx, AV_LOG_ERROR, "too many audio samples in frame\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->data_check_present) {
        if (get_bits_count(gbp) != expected_stream_pos)
            av_em_log(m->avctx, AV_LOG_ERROR, "block data length mismatch\n");
        skip_bits(gbp, 8);
    }
...
{
    AVEMCodecContext *avctx = m->avctx;
    SubStream *s = &m->substream[substr];
    unsigned int mat;
    ...
    if (m->avctx->channels != s->max_matrix_channel + 1) {
        av_em_log(m->avctx, AV_LOG_ERROR, "channel count mismatch\n");
        return AVERROR_INVALIDDATA;
    }

    if (!s->blockpos) {
        av_em_log(avctx, AV_LOG_ERROR, "No samples to output.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if ((ret = em_side_data_update_matrix_encoding(frame, s->matrix_encoding)) < 0)
        return ret;
...
static int read_access_unit(AVEMCodecContext *avctx, void* data,
                            int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    for (substr = 0; substr < m->num_substreams; substr++) {
        ...
        if (extraword_present) {
            if (m->avctx->codec_id == AV_CODEC_ID_MLP) {
                av_em_log(m->avctx, AV_LOG_ERROR, "There must be no extraword for MLP.\n");
                goto error;
            }
            skip_bits(&gb, 16);
            substr_header_size += 2;
        ...
        if (!(nonrestart_substr ^ m->is_major_sync_unit)) {
            av_em_log(m->avctx, AV_LOG_ERROR, "Invalid nonrestart_substr.\n");
            goto error;
        }
    ...
    if ((((parity_bits >> 4) ^ parity_bits) & 0xF) != 0xF) {
        av_em_log(avctx, AV_LOG_ERROR, "Parity check failed.\n");
        goto error;
    }
    ...
    for (substr = 0; substr <= m->max_decoded_substream; substr++) {
        ...
        if (substream_data_len[substr] * 8 - get_bits_count(&gb) >= 32) {
            ...
            if (substr == m->max_decoded_substream)
                av_em_log(m->avctx, AV_LOG_INFO, "End of stream indicated.\n");
        }
        ...
        if (substream_parity_present[substr]) {
            ...
            if ((get_bits(&gb, 8) ^ parity) != 0xa9    )
                av_em_log(m->avctx, AV_LOG_ERROR, "Substream %d parity check failed.\n", substr);
            if ( get_bits(&gb, 8)           != checksum)
                av_em_log(m->avctx, AV_LOG_ERROR, "Substream %d checksum failed.\n"    , substr);
        }
...
substream_length_mismatch:
    av_em_log(m->avctx, AV_LOG_ERROR, "substream %d length mismatch\n", substr);
    return AVERROR_INVALIDDATA;
