                                ...
                                const float ROUNDING)
{
    ...
    const float Q   = em_aac_pow2sf_tab [q_idx];
    const float Q34 = em_aac_pow34sf_tab[q_idx];
    ...
    for (i = 0; i < size; i += dim) {
        ...
        curbits =  em_aac_spectral_bits[cb-1][curidx];
        vec     = &em_aac_codebook_vectors[cb-1][curidx*dim];
        if (BT_UNSIGNED) {
            for (j = 0; j < dim; j++) {
        ...
        if (cost >= uplim)
            return uplim;
        if (pb) {
            put_bits(pb, em_aac_spectral_bits[cb-1][curidx], em_aac_spectral_codes[cb-1][curidx]);
            if (BT_UNSIGNED)
                for (j = 0; j < dim; j++)
                    if (em_aac_codebook_vectors[cb-1][curidx*dim+j] != 0.0f)
                        put_bits(pb, 1, in[i+j] < 0.0f);
            if (BT_ESC) {
                for (j = 0; j < 2; j++) {
                    if (em_aac_codebook_vectors[cb-1][curidx*2+j] == 64.0f) {
