...
static av_cold void log_encoder_error(AVEMCodecContext *avctx, const char *desc)
{
    VP8Context *ctx = avctx->priv_data;
    const char *error  = vpx_codec_error(&ctx->encoder);
    const char *detail = vpx_codec_error_detail(&ctx->encoder);

    av_em_log(avctx, AV_LOG_ERROR, "%s: %s\n", desc, error);
    if (detail)
        av_em_log(avctx, AV_LOG_ERROR, "  Additional information: %s\n", detail);
}

static av_cold void dump_enc_cfg(AVEMCodecContext *avctx,
                                 const struct vpx_codec_enc_cfg *cfg)
{
    int width = -30;
    int level = AV_LOG_DEBUG;

    av_em_log(avctx, level, "vpx_codec_enc_cfg\n");
    av_em_log(avctx, level, "generic settings\n"
           "  %*s%u\n  %*s%u\n  %*s%u\n  %*s%u\n  %*s%u\n"
    ...
    av_em_log(avctx, level, "rate control settings\n"
           "  %*s%u\n  %*s%u\n  %*s%u\n  %*s%u\n"
           "  %*s%d\n  %*s%p(%"SIZE_SPECIFIER")\n  %*s%u\n",
    ...
    av_em_log(avctx, level, "quantizer settings\n"
           "  %*s%u\n  %*s%u\n",
           width, "rc_min_quantizer:", cfg->rc_min_quantizer,
           width, "rc_max_quantizer:", cfg->rc_max_quantizer);
    av_em_log(avctx, level, "bitrate tolerance\n"
           "  %*s%u\n  %*s%u\n",
           width, "rc_undershoot_pct:", cfg->rc_undershoot_pct,
           width, "rc_overshoot_pct:",  cfg->rc_overshoot_pct);
    av_em_log(avctx, level, "decoder buffer model\n"
            "  %*s%u\n  %*s%u\n  %*s%u\n",
            width, "rc_buf_sz:",         cfg->rc_buf_sz,
            width, "rc_buf_initial_sz:", cfg->rc_buf_initial_sz,
            width, "rc_buf_optimal_sz:", cfg->rc_buf_optimal_sz);
    av_em_log(avctx, level, "2 pass rate control settings\n"
           "  %*s%u\n  %*s%u\n  %*s%u\n",
           width, "rc_2pass_vbr_bias_pct:",       cfg->rc_2pass_vbr_bias_pct,
           width, "rc_2pass_vbr_minsection_pct:", cfg->rc_2pass_vbr_minsection_pct,
           width, "rc_2pass_vbr_maxsection_pct:", cfg->rc_2pass_vbr_maxsection_pct);
    av_em_log(avctx, level, "keyframing settings\n"
           "  %*s%d\n  %*s%u\n  %*s%u\n",
           width, "kf_mode:",     cfg->kf_mode,
           width, "kf_min_dist:", cfg->kf_min_dist,
           width, "kf_max_dist:", cfg->kf_max_dist);
    av_em_log(avctx, level, "\n");
}
...
{
    av_em_freep(&cx_frame->buf);
    if (cx_frame->buf_alpha)
        av_em_freep(&cx_frame->buf_alpha);
    av_em_freep(&cx_frame);
}
...
static av_cold int codecctl_int(AVEMCodecContext *avctx,
                                enum vp8e_enc_control_id id, int val)
{
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "  %*s%d\n", width, buf, val);
...
static av_cold int vp8_free(AVEMCodecContext *avctx)
{
    VP8Context *ctx = avctx->priv_data;
    ...
    av_em_freep(&ctx->twopass_stats.buf);
    av_em_freep(&avctx->stats_out);
    free_frame_list(ctx->coded_frame_list);
    return 0;
}

#if CONFIG_LIBVPX_VP9_ENCODER
static int set_pix_fmt(AVEMCodecContext *avctx, vpx_codec_caps_t codec_caps,
                       ...
                       vpx_img_fmt_t *img_fmt)
{
    ...
    av_em_log(avctx, AV_LOG_ERROR, "Unsupported pixel format.\n");
    return AVERROR_INVALIDDATA;
}

#if VPX_ENCODER_ABI_VERSION > 8
static void set_colorspace(AVEMCodecContext *avctx)
{
    ...
    if (ctx->vpx_cs) {
        ...
        default:
            av_em_log(avctx, AV_LOG_WARNING, "Unsupported colorspace (%d)\n",
                   avctx->colorspace);
            return;
        }
    }
...
#if VPX_ENCODER_ABI_VERSION >= 11
static void set_color_range(AVEMCodecContext *avctx)
{
    ...
    default:
        av_em_log(avctx, AV_LOG_WARNING, "Unsupported color range (%d)\n",
               avctx->color_range);
        return;
    }
...
static av_cold int vpx_init(AVEMCodecContext *avctx,
                            const struct vpx_codec_iface *iface)
{
    ...
    av_em_log(avctx, AV_LOG_INFO, "%s\n", vpx_codec_version_str());
    av_em_log(avctx, AV_LOG_VERBOSE, "%s\n", vpx_codec_build_config());

    if (avctx->pix_fmt == AV_PIX_FMT_YUVA420P)
        ctx->is_alpha = 1;

    if ((res = vpx_codec_enc_config_default(iface, &enccfg, 0)) != VPX_CODEC_OK) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to get config: %s\n",
    ...
    if(!avctx->bit_rate)
        if(avctx->rc_max_rate || avctx->rc_buffer_size || avctx->rc_initial_buffer_occupancy) {
            av_em_log( avctx, AV_LOG_ERROR, "Rate control parameters set without a bitrate\n");
            return AVERROR(EINVAL);
        }
    ...
    if (avctx->bit_rate) {
        enccfg.rc_target_bitrate = av_em_rescale_rnd(avctx->bit_rate, 1, 1000,
                                                  AV_ROUND_NEAR_INF);
#if CONFIG_LIBVPX_VP9_ENCODER
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (avctx->rc_buffer_aggressivity != 1.0) {
        av_em_log(avctx, AV_LOG_WARNING, "The rc_buffer_aggressivity option is "
    ...
    if (enccfg.g_pass == VPX_RC_FIRST_PASS)
        ...
        if (!avctx->stats_in) {
            av_em_log(avctx, AV_LOG_ERROR, "No stats file for second pass\n");
            return AVERROR_INVALIDDATA;
        }

        ctx->twopass_stats.sz  = strlen(avctx->stats_in) * 3 / 4;
        ret = av_em_reallocp(&ctx->twopass_stats.buf, ctx->twopass_stats.sz);
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR,
        ...
        decode_size = av_em_base64_decode(ctx->twopass_stats.buf, avctx->stats_in,
                                       ctx->twopass_stats.sz);
        if (decode_size < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Stat buffer decode failed\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "vpx_codec_control\n");
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (avctx->mb_threshold) {
        av_em_log(avctx, AV_LOG_WARNING, "The mb_threshold option is deprecated, "
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Using deadline: %d\n", ctx->deadline);
 ...
 */
static int storeframe(AVEMCodecContext *avctx, struct FrameListData *cx_frame,
                      AVEMPacket *pkt)
{
    int ret = em_alloc_packet2(avctx, pkt, cx_frame->sz, 0);
    uint8_t *side_data;
    if (ret >= 0) {
        int pict_type;
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        em_side_data_set_encoder_stats(pkt, 0, cx_frame->sse + 1,
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (cx_frame->sz_alpha > 0) {
            side_data = av_em_packet_new_side_data(pkt,
 ...
 */
static int queue_frames(AVEMCodecContext *avctx, AVEMPacket *pkt_out)
{
    VP8Context *ctx = avctx->priv_data;
    ...
    while ((pkt = vpx_codec_get_cx_data(&ctx->encoder, &iter)) &&
        ...
        case VPX_CODEC_CX_FRAME_PKT:
            if (!size) {
                ...
                struct FrameListData *cx_frame =
                    av_em_alloc(sizeof(struct FrameListData));
                ...
                cx_pktcpy(cx_frame, pkt, pkt_alpha, ctx);
                cx_frame->buf = av_em_alloc(cx_frame->sz);
                ...
                if (ctx->is_alpha) {
                    cx_frame->buf_alpha = av_em_alloc(cx_frame->sz_alpha);
                    if (!cx_frame->buf_alpha) {
                        av_em_log(avctx, AV_LOG_ERROR,
                               "Data buffer alloc (%"SIZE_SPECIFIER" bytes) failed\n",
                               cx_frame->sz_alpha);
        ...
        case VPX_CODEC_STATS_PKT: {
            ...
            int err;
            if ((err = av_em_reallocp(&stats->buf,
                                   stats->sz +
                                   pkt->data.twopass_stats.sz)) < 0) {
                stats->sz = 0;
                av_em_log(avctx, AV_LOG_ERROR, "Stat buffer realloc failed\n");
                return err;
            }
...
static int vp8_encode(AVEMCodecContext *avctx, AVEMPacket *pkt,
                      const AVFrame *frame, int *got_packet)
{
    ...
    if (frame) {
        ...
        if (ctx->is_alpha) {
            ...
            u_plane = av_em_alloc(frame->linesize[1] * frame->height);
            v_plane = av_em_alloc(frame->linesize[2] * frame->height);
            if (!u_plane || !v_plane) {
                av_em_free(u_plane);
                av_em_free(v_plane);
                return AVERROR(ENOMEM);
    ...
    if (!frame && avctx->flags & AV_CODEC_FLAG_PASS1) {
        ...
        avctx->stats_out = av_em_alloc(b64_size);
        if (!avctx->stats_out) {
            av_em_log(avctx, AV_LOG_ERROR, "Stat buffer alloc (%d bytes) failed\n",
                   b64_size);
            return AVERROR(ENOMEM);
        }
        av_em_base64_encode(avctx->stats_out, b64_size, ctx->twopass_stats.buf,
                         ctx->twopass_stats.sz);
    }

    if (rawimg_alpha) {
        av_em_freep(&rawimg_alpha->planes[VPX_PLANE_U]);
        av_em_freep(&rawimg_alpha->planes[VPX_PLANE_V]);
    }
...
static const AVEMCodecDefault defaults[] = {
    { "qmin",             "-1" },
    { "qmax",             "-1" },
    { "g",                "-1" },
    { "keyint_min",       "-1" },
...
#if CONFIG_LIBVPX_VP8_ENCODER
static av_cold int vp8_init(AVEMCodecContext *avctx)
{
    return vpx_init(avctx, vpx_codec_vp8_cx());
}

static const AVEMClass class_vp8 = {
...
AVEMCodec ff_libvpx_vp8_encoder = {
    .name           = "libvpx",
...
#if CONFIG_LIBVPX_VP9_ENCODER
static av_cold int vp9_init(AVEMCodecContext *avctx)
{
    return vpx_init(avctx, vpx_codec_vp9_cx());
}

static const AVEMClass class_vp9 = {
...
AVEMCodec ff_libvpx_vp9_encoder = {
    .name           = "libvpx-vp9",
