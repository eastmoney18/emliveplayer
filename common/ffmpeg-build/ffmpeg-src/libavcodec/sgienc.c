...
static av_cold int encode_init(AVEMCodecContext *avctx)
{
    if (avctx->width > 65535 || avctx->height > 65535) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "SGI does not support resolutions above 65535x65535\n");
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, bytes_per_channel * length, 0)) < 0)
        return ret;
    ...
    if (s->rle) {
        ...
        if (!(encode_buf = av_em_alloc(width * bytes_per_channel)))
...
static const AVEMClass sgi_class = {
    .class_name = "sgi",
