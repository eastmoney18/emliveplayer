...
static int mp3_header_decompress(AVBSFContext *ctx, AVEMPacket *out)
{
    AVEMPacket *in;
    uint32_t header;
    ...
    if(ctx->par_in->extradata_size != 15 || strcmp(ctx->par_in->extradata, "FFCMP3 0.0")){
        av_em_log(ctx, AV_LOG_ERROR, "Extradata invalid %d\n", ctx->par_in->extradata_size);
        ret = AVERROR(EINVAL);
        goto fail;
    }
    ...
    if(bitrate_index == 30){
        av_em_log(ctx, AV_LOG_ERROR, "Could not find bitrate_index.\n");
        ret = AVERROR(EINVAL);
        goto fail;
    }
    ...
    ret = av_em_new_packet(out, frame_size);
    if (ret < 0)
        goto fail;
    ret = av_em_packet_copy_props(out, in);
    if (ret < 0) {
        av_em_packet_free(&out);
        goto fail;
    }
...
const AVBitStreamFilter em_mp3_header_decompress_bsf = {
    .name      = "mp3decomp",
