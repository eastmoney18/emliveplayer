...
{
    int ret, i;
    int edges_needed = av_em_codec_is_encoder(s->avctx->codec);
    ...
    if ((ret = em_get_buffer(s->avctx, frame, AV_GET_BUFFER_FLAG_REF)) < 0)
        return ret;
    if (edges_needed) {
...
int ff_snow_alloc_blocks(SnowContext *s){
    ...
    av_em_free(s->block);
    s->block= av_em_mallocz_array(w * h,  sizeof(BlockNode) << (s->block_max_depth*2));
    if (!s->block)
...
av_cold int ff_snow_common_init(AVEMCodecContext *avctx){
...
//    dec += FFMAX(s->chroma_h_shift, s->chroma_v_shift);
    ...
    for(i=0; i<MAX_REF_FRAMES; i++) {
        ...
        s->last_picture[i] = av_em_frame_alloc();
        if (!s->last_picture[i])
            goto fail;
    }

    s->mconly_picture = av_em_frame_alloc();
...
int ff_snow_common_init_after_header(AVEMCodecContext *avctx) {
    SnowContext *s = avctx->priv_data;
    int plane_index, level, orientation;
    int ret, emu_buf_size;

    if(!s->scratchbuf) {
        if ((ret = em_get_buffer(s->avctx, s->mconly_picture,
    ...
    if(s->mconly_picture->format != avctx->pix_fmt) {
        av_em_log(avctx, AV_LOG_ERROR, "pixel format changed\n");
    ...
    for(plane_index=0; plane_index < s->nb_planes; plane_index++){
        ...
        for(level=s->spatial_decomposition_count-1; level>=0; level--){
            for(orientation=level ? 1 : 0; orientation<4; orientation++){
                ...
                av_em_freep(&b->x_coeff);
                b->x_coeff=av_em_mallocz_array(((b->width+1) * b->height+1), sizeof(x_and_coeff));
                if (!b->x_coeff)
                    goto fail;
            }
            w= (w+1)>>1;
...
static int halfpel_interpol(SnowContext *s, uint8_t *halfpel[4][4], AVFrame *frame){
    ...
    for(p=0; p < s->nb_planes; p++){
        ...
        halfpel[1][p] = av_em_malloc_array(ls, (h + 2 * EDGE_WIDTH));
        halfpel[2][p] = av_em_malloc_array(ls, (h + 2 * EDGE_WIDTH));
        halfpel[3][p] = av_em_malloc_array(ls, (h + 2 * EDGE_WIDTH));
        if (!halfpel[1][p] || !halfpel[2][p] || !halfpel[3][p]) {
            av_em_freep(&halfpel[1][p]);
            av_em_freep(&halfpel[2][p]);
            av_em_freep(&halfpel[3][p]);
            return AVERROR(ENOMEM);
        }
...
void ff_snow_release_buffer(AVEMCodecContext *avctx)
{
    SnowContext *s = avctx->priv_data;
    int i;

    if(s->last_picture[s->max_ref_frames-1]->data[0]){
        av_em_frame_unref(s->last_picture[s->max_ref_frames-1]);
        for(i=0; i<9; i++)
            if(s->halfpel_plane[s->max_ref_frames-1][1+i/3][i%3]) {
                av_em_free(s->halfpel_plane[s->max_ref_frames-1][1+i/3][i%3] - EDGE_WIDTH*(1+s->current_picture->linesize[i%3]));
...
int ff_snow_frame_start(SnowContext *s){
   ...
   int i, ret;
    ...
    if(s->keyframe){
        ...
        s->ref_frames= i;
        if(s->ref_frames==0){
            av_em_log(s->avctx,AV_LOG_ERROR, "No reference frames\n");
            return AVERROR_INVALIDDATA;
        }
    }
...
av_cold void ff_snow_common_end(SnowContext *s)
{
    ...
    av_em_freep(&s->spatial_dwt_buffer);
    av_em_freep(&s->temp_dwt_buffer);
    av_em_freep(&s->spatial_idwt_buffer);
    av_em_freep(&s->temp_idwt_buffer);
    av_em_freep(&s->run_buffer);

    s->m.me.temp= NULL;
    av_em_freep(&s->m.me.scratchpad);
    av_em_freep(&s->m.me.map);
    av_em_freep(&s->m.me.score_map);
    av_em_freep(&s->m.sc.obmc_scratchpad);

    av_em_freep(&s->block);
    av_em_freep(&s->scratchbuf);
    av_em_freep(&s->emu_edge_buffer);

    for(i=0; i<MAX_REF_FRAMES; i++){
        av_em_freep(&s->ref_mvs[i]);
        av_em_freep(&s->ref_scores[i]);
        if(s->last_picture[i] && s->last_picture[i]->data[0]) {
            av_assert0(s->last_picture[i]->data[0] != s->current_picture->data[0]);
        }
        av_em_frame_free(&s->last_picture[i]);
    }
    ...
    av_em_frame_free(&s->mconly_picture);
