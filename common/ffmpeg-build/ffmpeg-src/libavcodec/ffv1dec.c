        ...
        if (s->ac != AC_GOLOMB_RICE) {
            ...
            if (run_mode) {
                if (run_count == 0 && run_mode == 1) {
                    if (get_bits1(&s->gb)) {
                        run_count = 1 << em_log2_run[run_index];
                        if (x + run_count <= w)
                            run_index++;
                    } else {
                        if (em_log2_run[run_index])
                            run_count = get_bits(&s->gb, em_log2_run[run_index]);
                        else
                            run_count = 0;
                        if (run_index)
                            run_index--;
...
static int decode_slice_header(FFV1Context *f, FFV1Context *fs)
{
    ...
    for (i = 0; i < f->plane_count; i++) {
        ...
        if (idx >= (unsigned)f->quant_table_count) {
            av_em_log(f->avctx, AV_LOG_ERROR, "quant_table_index out of range\n");
            return -1;
        }
    ...
    if (av_em_image_check_sar(f->width, f->height,
                           f->cur->sample_aspect_ratio) < 0) {
        av_em_log(f->avctx, AV_LOG_WARNING, "ignoring invalid SAR: %u/%u\n",
               f->cur->sample_aspect_ratio.num,
               f->cur->sample_aspect_ratio.den);
        f->cur->sample_aspect_ratio = (AVEMRational){ 0, 1 };
    }
    ...
    if (fs->version > 3) {
        ...
        if (fs->slice_coding_mode != 1) {
            ...
            if ((uint64_t)fs->slice_rct_by_coef + (uint64_t)fs->slice_rct_ry_coef > 4) {
                av_em_log(f->avctx, AV_LOG_ERROR, "slice_rct_y_coef out of range\n");
                return AVERROR_INVALIDDATA;
            }
        }
    }
...
static int decode_slice(AVEMCodecContext *c, void *arg)
{
    FFV1Context *fs   = *(void **)arg;
    FFV1Context *f    = fs->avctx->priv_data;
    int width, height, x, y, ret;
    const int ps      = av_em_pix_fmt_desc_get(c->pix_fmt)->comp[0].step;
    ...
    if(f->fsrc && !p->key_frame)
        em_thread_await_progress(&f->last_picture, si, 0);
    ...
    if(f->fsrc && !p->key_frame) {
        ...
        for (i = 0; i < f->plane_count; i++) {
            ...
            av_em_free(pdst->state);
            av_em_free(pdst->vlc_state);
            memcpy(pdst, psrc, sizeof(*pdst));
            pdst->state = NULL;
            pdst->vlc_state = NULL;

            if (fssrc->ac) {
                pdst->state = av_em_malloc_array(CONTEXT_SIZE,  psrc->context_count);
                memcpy(pdst->state, psrc->state, CONTEXT_SIZE * psrc->context_count);
            } else {
                pdst->vlc_state = av_em_malloc_array(sizeof(*pdst->vlc_state), psrc->context_count);
    ...
    if (fs->ac != AC_GOLOMB_RICE && f->version > 2) {
        ...
        if (v) {
            av_em_log(f->avctx, AV_LOG_ERROR, "bytestream end mismatching by %d\n", v);
            fs->slice_damaged = 1;
        }
    }
    ...
    em_thread_report_progress(&f->picture, si, 0);
...
static int read_extra_header(FFV1Context *f)
{
    ...
    if (f->version < 2) {
        av_em_log(f->avctx, AV_LOG_ERROR, "Invalid version in global header\n");
        return AVERROR_INVALIDDATA;
    }
    if (f->version > 2) {
        c->bytestream_end -= 4;
    ...
    if (f->chroma_h_shift > 4U || f->chroma_v_shift > 4U) {
        av_em_log(f->avctx, AV_LOG_ERROR, "chroma shift parameters %d %d are invalid\n",
    ...
    if (f->num_h_slices > (unsigned)f->width  || !f->num_h_slices ||
        ...
        av_em_log(f->avctx, AV_LOG_ERROR, "slice count invalid\n");
        return AVERROR_INVALIDDATA;
    }

    f->quant_table_count = get_symbol(c, state, 0);
    if (f->quant_table_count > (unsigned)MAX_QUANT_TABLES || !f->quant_table_count) {
        av_em_log(f->avctx, AV_LOG_ERROR, "quant table count %d is invalid\n", f->quant_table_count);
    ...
    for (i = 0; i < f->quant_table_count; i++) {
        ...
        if (f->context_count[i] < 0) {
            av_em_log(f->avctx, AV_LOG_ERROR, "read_quant_table error\n");
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    if (f->version > 2) {
        unsigned v;
        v = av_em_crc(av_em_crc_get_table(AV_CRC_32_IEEE), 0,
                   f->avctx->extradata, f->avctx->extradata_size);
        if (v || f->avctx->extradata_size < 4) {
            av_em_log(f->avctx, AV_LOG_ERROR, "CRC mismatch %X!\n", v);
            return AVERROR_INVALIDDATA;
        }
    ...
    if (f->avctx->debug & FF_DEBUG_PICT_INFO)
        av_em_log(f->avctx, AV_LOG_DEBUG,
...
static int read_header(FFV1Context *f)
{
    ...
    if (f->version < 2) {
        ...
        if (v >= 2) {
            av_em_log(f->avctx, AV_LOG_ERROR, "invalid version %d in ver01 header\n", v);
            return AVERROR_INVALIDDATA;
        }
        f->version = v;
        f->ac = get_symbol(c, state, 0);
        ...
        if (f->plane_count) {
            if (colorspace          != f->colorspace                 ||
                ...
                av_em_log(f->avctx, AV_LOG_ERROR, "Invalid change of global parameters\n");
                return AVERROR_INVALIDDATA;
            }
        }
    ...
    if (f->colorspace == 0) {
        ...
        if (f->chroma_h_shift || f->chroma_v_shift) {
            av_em_log(f->avctx, AV_LOG_ERROR,
        ...
        av_em_log(f->avctx, AV_LOG_ERROR, "colorspace not supported\n");
        return AVERROR(ENOSYS);
    }
    if (f->avctx->pix_fmt == AV_PIX_FMT_NONE) {
        av_em_log(f->avctx, AV_LOG_ERROR, "format not supported\n");
        return AVERROR(ENOSYS);
    }
    ...
    if (f->slice_count > (unsigned)MAX_SLICES || f->slice_count <= 0 || f->slice_count > f->max_slice_count) {
        av_em_log(f->avctx, AV_LOG_ERROR, "slice count %d is invalid (max=%d)\n", f->slice_count, f->max_slice_count);
        return AVERROR_INVALIDDATA;
    }
...
static int decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (f->last_picture.f)
        em_thread_release_buffer(avctx, &f->last_picture);
    ...
    if ((ret = em_thread_get_buffer(avctx, &f->picture, AV_GET_BUFFER_FLAG_REF)) < 0)
        return ret;

    if (avctx->debug & FF_DEBUG_PICT_INFO)
        av_em_log(avctx, AV_LOG_DEBUG, "ver:%d keyframe:%d coder:%d ec:%d slices:%d bps:%d\n",
    ...
    for (i = f->slice_count - 1; i >= 0; i--) {
        ...
        if (buf_p - c->bytestream_start < v) {
            av_em_log(avctx, AV_LOG_ERROR, "Slice pointer chain broken\n");
            em_thread_report_progress(&f->picture, INT_MAX, 0);
            return AVERROR_INVALIDDATA;
        }
        buf_p -= v;

        if (f->ec) {
            unsigned crc = av_em_crc(av_em_crc_get_table(AV_CRC_32_IEEE), 0, buf_p, v);
            if (crc) {
                int64_t ts = avpkt->pts != AV_NOPTS_VALUE ? avpkt->pts : avpkt->dts;
                av_em_log(f->avctx, AV_LOG_ERROR, "CRC mismatch %X!", crc);
                if (ts != AV_NOPTS_VALUE && avctx->pkt_timebase.num) {
                    av_em_log(f->avctx, AV_LOG_ERROR, "at %f seconds\n", ts*av_em_q2d(avctx->pkt_timebase));
                } else if (ts != AV_NOPTS_VALUE) {
                    av_em_log(f->avctx, AV_LOG_ERROR, "at %"PRId64"\n", ts);
                } else {
                    av_em_log(f->avctx, AV_LOG_ERROR, "\n");
                }
                fs->slice_damaged = 1;
            ...
            if (avctx->debug & FF_DEBUG_PICT_INFO) {
                av_em_log(avctx, AV_LOG_DEBUG, "slice %d, CRC: 0x%08X\n", i, AV_RB32(buf_p + v - 4));
            }
        }
    ...
    for (i = f->slice_count - 1; i >= 0; i--) {
        ...
        if (fs->slice_damaged && f->last_picture.f->data[0]) {
            ...
            const uint8_t *src[4];
            uint8_t *dst[4];
            em_thread_await_progress(&f->last_picture, INT_MAX, 0);
            for (j = 0; j < 4; j++) {
            ...
            av_em_image_copy(dst, p->linesize, src,
    ...
    if (f->last_picture.f)
        em_thread_release_buffer(avctx, &f->last_picture);
    f->cur = NULL;
    if ((ret = av_em_frame_ref(data, f->picture.f)) < 0)
        return ret;
...
#if HAVE_THREADS
static int init_thread_copy(AVEMCodecContext *avctx)
{
    FFV1Context *f = avctx->priv_data;
    int i, ret;
    ...
    for (i = 0; i < f->quant_table_count; i++) {
        ...
        f->initial_states[i] = av_em_memdup(f->initial_states[i],
                                         f->context_count[i] * sizeof(*f->initial_states[i]));
    }

    f->picture.f      = av_em_frame_alloc();
    f->last_picture.f = av_em_frame_alloc();
...
static int update_thread_context(AVEMCodecContext *dst, const AVEMCodecContext *src)
{
    ...
    em_thread_release_buffer(dst, &fdst->picture);
    if (fsrc->picture.f->data[0]) {
        if ((ret = em_thread_ref_frame(&fdst->picture, &fsrc->picture)) < 0)
            return ret;
    }
