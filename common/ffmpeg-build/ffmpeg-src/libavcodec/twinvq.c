...
int ff_twinvq_decode_frame(AVEMCodecContext *avctx, void *data,
...
av_cold int ff_twinvq_decode_close(AVEMCodecContext *avctx)
{
    TwinVQContext *tctx = avctx->priv_data;
    int i;
    ...
    for (i = 0; i < 3; i++) {
        ...
        av_em_freep(&tctx->cos_tabs[i]);
    }

    av_em_freep(&tctx->curr_frame);
    av_em_freep(&tctx->spectrum);
    av_em_freep(&tctx->prev_frame);
    av_em_freep(&tctx->tmp_buf);
    av_em_freep(&tctx->fdsp);

    return 0;
}

av_cold int ff_twinvq_decode_init(AVEMCodecContext *avctx)
{
    int ret;
    TwinVQContext *tctx = avctx->priv_data;
    ...
    if (!avctx->block_align) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Block align is %d bits, expected %d\n",
    ...
    if (tctx->frames_per_packet > TWINVQ_MAX_FRAMES_PER_PACKET) {
        av_em_log(avctx, AV_LOG_ERROR, "Too many frames per packet (%d)\n",
               tctx->frames_per_packet);
        return AVERROR_INVALIDDATA;
    }

    tctx->fdsp = avpriv_em_float_dsp_alloc(avctx->flags & AV_CODEC_FLAG_BITEXACT);
    if (!tctx->fdsp) {
