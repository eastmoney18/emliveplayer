...
static const AVEMClass dnxhd_class = {
    .class_name = "dnxhd",
...
static av_cold int dnxhd_encode_init(AVEMCodecContext *avctx)
{
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "cid %d\n", ctx->cid);
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (avctx->thread_count > MAX_THREADS) {
        av_em_log(avctx, AV_LOG_ERROR, "too many threads\n");
        return AVERROR(EINVAL);
    }

    if (avctx->qmax <= 1) {
        av_em_log(avctx, AV_LOG_ERROR, "qmax must be at least 2\n");
        return AVERROR(EINVAL);
    }

    ctx->thread[0] = ctx;
    for (i = 1; i < avctx->thread_count; i++) {
        ctx->thread[i] = av_em_alloc(sizeof(DNXHDEncContext));
...
static int dnxhd_write_header(AVEMCodecContext *avctx, uint8_t *buf)
{
    DNXHDEncContext *ctx = avctx->priv_data;
...
static int dnxhd_calc_bits_thread(AVEMCodecContext *avctx, void *arg,
...
static int dnxhd_encode_thread(AVEMCodecContext *avctx, void *arg,
                               int jobnr, int threadnr)
{
    DNXHDEncContext *ctx = avctx->priv_data;
    int mb_y = jobnr, mb_x;
...
static int dnxhd_mb_var_thread(AVEMCodecContext *avctx, void *arg,
                               int jobnr, int threadnr)
{
    DNXHDEncContext *ctx = avctx->priv_data;
...
static int dnxhd_encode_rdo(AVEMCodecContext *avctx, DNXHDEncContext *ctx)
{
...
{
    int buckets[RADIX_PASSES][NBUCKETS];
    RCCMPEntry *tmp = av_em_malloc_array(size, sizeof(*tmp));
...
static int dnxhd_encode_fast(AVEMCodecContext *avctx, DNXHDEncContext *ctx)
{
    int max_bits = 0;
    int ret, x, y;
...
static int dnxhd_encode_picture(AVEMCodecContext *avctx, AVEMPacket *pkt,
                                const AVFrame *frame, int *got_packet)
{
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, ctx->cid_table->frame_size, 0)) < 0)
        return ret;
    buf = pkt->data;
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    em_side_data_set_encoder_stats(pkt, ctx->qscale * FF_QP2LAMBDA, NULL, 0, AV_PICTURE_TYPE_I);
...
static av_cold int dnxhd_encode_end(AVEMCodecContext *avctx)
{
    DNXHDEncContext *ctx = avctx->priv_data;
    int max_level        = 1 << (ctx->cid_table->bit_depth + 2);
    int i;

    av_em_free(ctx->vlc_codes - max_level * 2);
    av_em_free(ctx->vlc_bits - max_level * 2);
    av_em_freep(&ctx->run_codes);
    av_em_freep(&ctx->run_bits);

    av_em_freep(&ctx->mb_bits);
    av_em_freep(&ctx->mb_qscale);
    av_em_freep(&ctx->mb_rc);
    av_em_freep(&ctx->mb_cmp);
    av_em_freep(&ctx->slice_size);
    av_em_freep(&ctx->slice_offs);

    av_em_freep(&ctx->qmatrix_c);
    av_em_freep(&ctx->qmatrix_l);
    av_em_freep(&ctx->qmatrix_c16);
    av_em_freep(&ctx->qmatrix_l16);

    for (i = 1; i < avctx->thread_count; i++)
        av_em_freep(&ctx->thread[i]);

    return 0;
}

static const AVEMCodecDefault dnxhd_defaults[] = {
    { "qmax", "1024" }, /* Maximum quantization scale factor allowed for VC-3 */
    { NULL },
};

AVEMCodec ff_dnxhd_encoder = {
    .name           = "dnxhd",
