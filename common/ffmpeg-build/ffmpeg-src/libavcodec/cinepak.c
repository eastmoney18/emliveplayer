...
static int cinepak_decode (CinepakContext *s)
{
    ...
    if (s->sega_film_skip_bytes == -1) {
        if (!encoded_buf_size) {
            avpriv_em_request_sample(s->avctx, "encoded_buf_size 0");
...
static av_cold int cinepak_decode_init(AVEMCodecContext *avctx)
{
...
static int cinepak_decode_frame(AVEMCodecContext *avctx,
                                ...
                                AVEMPacket *avpkt)
{
    ...
    if ((ret = cinepak_decode(s)) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "cinepak_decode failed\n");
    }
...
static av_cold int cinepak_decode_end(AVEMCodecContext *avctx)
{
    CinepakContext *s = avctx->priv_data;
