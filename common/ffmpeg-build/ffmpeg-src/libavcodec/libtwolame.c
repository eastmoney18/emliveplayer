...
static av_cold int twolame_encode_close(AVEMCodecContext *avctx)
{
...
static av_cold int twolame_encode_init(AVEMCodecContext *avctx)
{
    TWOLAMEContext *s = avctx->priv_data;
    int ret;
...
static int twolame_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                                const AVFrame *frame, int *got_packet_ptr)
{
    TWOLAMEContext *s = avctx->priv_data;
    int ret;

    if ((ret = em_alloc_packet2(avctx, avpkt, MPA_MAX_CODED_FRAME_SIZE, 0)) < 0)
        return ret;
...
static const AVEMClass twolame_class = {
...
static const AVEMCodecDefault twolame_defaults[] = {
    { "b", "0" },
    { NULL },
};
...
AVEMCodec ff_libtwolame_encoder = {
    .name           = "libtwolame",
