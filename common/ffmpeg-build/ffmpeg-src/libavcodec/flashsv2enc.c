...
{
    av_em_freep(&s->encbuffer);
    av_em_freep(&s->keybuffer);
    av_em_freep(&s->databuffer);
    av_em_freep(&s->blockbuffer);
    av_em_freep(&s->current_frame);
    av_em_freep(&s->key_frame);

    av_em_freep(&s->frame_blocks);
    av_em_freep(&s->key_blocks);
}
...
static av_cold int flashsv2_encode_init(AVEMCodecContext * avctx)
{
    ...
    if (av_em_image_check_size(avctx->width, avctx->height, 0, avctx) < 0)
        return -1;
    ...
    s->encbuffer     = av_em_mallocz(s->frame_size);
    s->keybuffer     = av_em_mallocz(s->frame_size);
    s->databuffer    = av_em_mallocz(s->frame_size * 6);
    s->current_frame = av_em_mallocz(s->frame_size);
    s->key_frame     = av_em_mallocz(s->frame_size);
    s->frame_blocks  = av_em_mallocz(s->blocks_size);
    s->key_blocks    = av_em_mallocz(s->blocks_size);
    ...
    if (!s->encbuffer || !s->keybuffer || !s->databuffer
        ...
        || !s->frame_blocks) {
        av_em_log(avctx, AV_LOG_ERROR, "Memory allocation failed.\n");
        cleanup(s);
        return -1;
    }
...
static int optimum_dist(FlashSV2Context * s)
{
    ...
    av_em_log(s->avctx, AV_LOG_DEBUG, "dist: %d\n", dist);
    return dist;
#else
    return 15;
#endif
                                   ...
                                   int stride)
{
    ...
    if (block_width != s->block_width || block_height != s->block_height) {
        ...
        if (s->rows * s->cols > s->blocks_size / sizeof(Block)) {
            s->frame_blocks = av_em_realloc_array(s->frame_blocks, s->rows, s->cols * sizeof(Block));
            s->key_blocks = av_em_realloc_array(s->key_blocks, s->cols, s->rows * sizeof(Block));
            if (!s->frame_blocks || !s->key_blocks) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Memory allocation failed.\n");
                return -1;
            }
            s->blocks_size = s->rows * s->cols * sizeof(Block);
        }
        ...
        av_em_fast_malloc(&s->blockbuffer, &s->blockbuffer_size, block_width * block_height * 6);
        if (!s->blockbuffer) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Could not allocate block buffer.\n");
    ...
    if (s->use15_7) {
        if ((s->use_custom_palette && s->palette_type != 1) || update_palette) {
            ...
            if (res)
                return res;
            s->palette_type = 1;
            av_em_log(s->avctx, AV_LOG_DEBUG, "Generated optimum palette\n");
        } else if (!s->use_custom_palette && s->palette_type != 0) {
            res = generate_default_palette(&s->palette);
            if (res)
                return res;
            s->palette_type = 0;
            av_em_log(s->avctx, AV_LOG_DEBUG, "Generated default palette\n");
        }
    }
...
static int flashsv2_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                                 const AVFrame *p, int *got_packet)
{
    ...
    if ((res = em_alloc_packet2(avctx, pkt, s->frame_size + AV_INPUT_BUFFER_MIN_SIZE, 0)) < 0)
        return res;
    ...
    if (!keyframe
        ...
        if (keyframe)
            av_em_log(avctx, AV_LOG_DEBUG, "Recommending key frame at frame %d\n", avctx->frame_number);
    }
    ...
    if (keyframe) {
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Inserting key frame at frame %d\n", avctx->frame_number);
    }
...
static av_cold int flashsv2_encode_end(AVEMCodecContext * avctx)
{
    FlashSV2Context *s = avctx->priv_data;
