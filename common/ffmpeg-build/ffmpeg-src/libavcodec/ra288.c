...
static av_cold int ra288_decode_close(AVEMCodecContext *avctx)
{
    RA288Context *ractx = avctx->priv_data;

    av_em_freep(&ractx->fdsp);

    return 0;
}

static av_cold int ra288_decode_init(AVEMCodecContext *avctx)
{
    RA288Context *ractx = avctx->priv_data;
    ...
    ractx->fdsp = avpriv_em_float_dsp_alloc(avctx->flags & AV_CODEC_FLAG_BITEXACT);
    if (!ractx->fdsp)
...
{
    for (; n >= 0; n--)
        tgt[n] = avpriv_em_scalarproduct_float_c(src, src - n, len);
...
static void decode(RA288Context *ractx, float gain, int cb_coef)
{
    ...
    sum = avpriv_em_scalarproduct_float_c(buffer, buffer, 5);
...
static int ra288_decode_frame(AVEMCodecContext * avctx, void *data,
