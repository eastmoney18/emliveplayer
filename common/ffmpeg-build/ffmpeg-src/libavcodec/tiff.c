...
static void free_geotags(TiffContext *const s)
{
    ...
    for (i = 0; i < s->geotag_count; i++) {
        if (s->geotags[i].val)
            av_em_freep(&s->geotags[i].val);
    }
    av_em_freep(&s->geotags);
    s->geotag_count = 0;
}
...
static char *get_geokey_val(int key, int val)
{
    ...
    if (val == TIFF_GEO_KEY_UNDEFINED)
        return av_em_strdup("undefined");
    if (val == TIFF_GEO_KEY_USER_DEFINED)
        return av_em_strdup("User-Defined");

#define RET_GEOKEY_VAL(TYPE, array)\
    if (val >= TIFF_##TYPE##_OFFSET &&\
        val - TIFF_##TYPE##_OFFSET < FF_ARRAY_ELEMS(ff_tiff_##array##_codes))\
        return av_em_strdup(ff_tiff_##array##_codes[val - TIFF_##TYPE##_OFFSET]);
    ...
    case TIFF_PROJECTED_CS_TYPE_GEOKEY:
        ap = av_em_strdup(search_keyval(ff_tiff_proj_cs_type_codes, FF_ARRAY_ELEMS(ff_tiff_proj_cs_type_codes), val));
        if(ap) return ap;
        break;
    case TIFF_PROJECTION_GEOKEY:
        ap = av_em_strdup(search_keyval(ff_tiff_projection_codes, FF_ARRAY_ELEMS(ff_tiff_projection_codes), val));
        if(ap) return ap;
        break;
...
static char *doubles2str(double *dp, int count, const char *sep)
{
    ...
    if (count >= (INT_MAX - 1)/component_len)
        return NULL;
    ap = av_em_alloc(component_len * count + 1);
    if (!ap)
        return NULL;
    ap0   = ap;
    ap[0] = '\0';
...
{
    switch(type) {
    case TIFF_DOUBLE: return ff_tadd_doubles_metadata(count, name, sep, &s->gb, s->le, avempriv_frame_get_metadatap(frame));
    case TIFF_SHORT : return ff_tadd_shorts_metadata(count, name, sep, &s->gb, s->le, 0, avempriv_frame_get_metadatap(frame));
    case TIFF_STRING: return ff_tadd_string_metadata(count, name, &s->gb, s->le, avempriv_frame_get_metadatap(frame));
...
static int deinvert_buffer(TiffContext *s, const uint8_t *src, int size)
{
    ...
    av_em_fast_padded_malloc(&s->deinvert_buf, &s->deinvert_buf_size, size);
    if (!s->deinvert_buf)
        return AVERROR(ENOMEM);
    for (i = 0; i < size; i++)
        s->deinvert_buf[i] = em_reverse[src[i]];
                           ...
                           int size)
{
    ...
    if (zret != Z_OK) {
        av_em_log(NULL, AV_LOG_ERROR, "Inflate init error: %d\n", zret);
        return zret;
    }
                                ...
                                int size)
{
    ...
    if (ret != LZMA_OK) {
        av_em_log(NULL, AV_LOG_ERROR, "LZMA init error: %d\n", ret);
        return ret;
    }
                            ...
                            int strip_start, int is_yuv)
{
    ...
    int ret, line;
    uint8_t *buf = av_em_alloc(outlen);
    if (!buf)
        return AVERROR(ENOMEM);
    if (s->fill_order) {
        if ((ret = deinvert_buffer(s, src, size)) < 0) {
                           ...
                           const uint8_t *src, int size, int width, int lines)
{
    int i, ret = 0;
    int line;
    uint8_t *src2 = av_em_alloc((unsigned)size +
                             ...
                             const uint8_t *src, int size, int strip_start, int lines)
{
    ...
    const AVPixFmtDescriptor *desc = av_em_pix_fmt_desc_get(p->format);
    ...
    if (is_yuv) {
        ...
        av_em_fast_padded_malloc(&s->yuv_line, &s->yuv_line_size, bytes_per_row);
        if (s->yuv_line == NULL) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Not enough memory\n");
            return AVERROR(ENOMEM);
        }
        dst = s->yuv_line;
        stride = 0;
    ...
    if (s->compr == TIFF_LZW) {
        ...
        if (size > 1 && !src[0] && (src[1]&1)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Old style LZW is unsupported\n");
        }
        if ((ret = ff_lzw_decode_init(s->lzw, 8, src, size, FF_LZW_TIFF)) < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Error initializing LZW decoder\n");
            return ret;
        }
        for (line = 0; line < lines; line++) {
            pixels = ff_lzw_decode(s->lzw, dst, width);
            if (pixels < width) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Decoded only %i bytes of %i\n",
                       pixels, width);
                return AVERROR_INVALIDDATA;
            }
            if (s->bpp < 8 && s->avctx->pix_fmt == AV_PIX_FMT_PAL8)
    ...
    for (line = 0; line < lines; line++) {
        if (src - ssrc > size) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Source data overread\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        case TIFF_PACKBITS:
            for (pixels = 0; pixels < width;) {
                if (ssrc + size - src < 2) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "Read went out of bounds\n");
                    return AVERROR_INVALIDDATA;
                }
                code = s->fill_order ? (int8_t) em_reverse[*src++]: (int8_t) *src++;
                if (code >= 0) {
                    code++;
...
static int init_image(TiffContext *s, ThreadFrame *frame)
{
    ...
    case 243:
        if (s->photometric == TIFF_PHOTOMETRIC_YCBCR) {
            if (s->subsampling[0] == 1 && s->subsampling[1] == 1) {
                ...
                av_em_log(s->avctx, AV_LOG_ERROR, "Unsupported YCbCr subsampling\n");
    ...
    if (s->photometric == TIFF_PHOTOMETRIC_YCBCR) {
        ...
        if((desc->flags & AV_PIX_FMT_FLAG_RGB) ||
           !(desc->flags & AV_PIX_FMT_FLAG_PLANAR) ||
           desc->nb_components < 3) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Unsupported YCbCr variant\n");
            return AVERROR_INVALIDDATA;
        }
    }

    if (s->width != s->avctx->width || s->height != s->avctx->height) {
        ret = em_set_dimensions(s->avctx, s->width, s->height);
        if (ret < 0)
            return ret;
    }
    if ((ret = em_thread_get_buffer(s->avctx, frame, 0)) < 0)
        return ret;
...
static void set_sar(TiffContext *s, unsigned tag, unsigned num, unsigned den)
{
    ...
    if (s->res[0] && s->res[1] && s->res[2] && s->res[3])
        av_em_reduce(&s->avctx->sample_aspect_ratio.num, &s->avctx->sample_aspect_ratio.den,
...
static int tiff_decode_tag(TiffContext *s, AVFrame *frame)
{
    ...
    case TIFF_COMPR:
        ...
        case TIFF_ADOBE_DEFLATE:
            ...
            break;
#else
            av_em_log(s->avctx, AV_LOG_ERROR, "Deflate: ZLib not compiled in\n");
            return AVERROR(ENOSYS);
#endif
        case TIFF_JPEG:
        case TIFF_NEWJPEG:
            avpriv_em_report_missing_feature(s->avctx, "JPEG compression");
            return AVERROR_PATCHWELCOME;
        case TIFF_LZMA:
#if CONFIG_LZMA
            break;
#else
            av_em_log(s->avctx, AV_LOG_ERROR, "LZMA not compiled in\n");
            return AVERROR(ENOSYS);
#endif
        default:
            av_em_log(s->avctx, AV_LOG_ERROR, "Unknown compression method %i\n",
                   s->compr);
            return AVERROR_INVALIDDATA;
        }
        break;
    ...
    case TIFF_TILE_WIDTH:
        av_em_log(s->avctx, AV_LOG_ERROR, "Tiled images are not supported\n");
        return AVERROR_PATCHWELCOME;
        break;
    case TIFF_PREDICTOR:
        s->predictor = value;
    ...
    case TIFF_PHOTOMETRIC:
        ...
        case TIFF_PHOTOMETRIC_LINEAR_RAW:
            ...
            return AVERROR_PATCHWELCOME;
        default:
            av_em_log(s->avctx, AV_LOG_ERROR, "PhotometricInterpretation %u is "
                   "unknown\n", value);
            return AVERROR_INVALIDDATA;
        }
        break;
    ...
    case TIFF_YCBCR_SUBSAMPLING:
        if (count != 2) {
            av_em_log(s->avctx, AV_LOG_ERROR, "subsample count invalid\n");
            return AVERROR_INVALIDDATA;
        }
        for (i = 0; i < count; i++) {
            s->subsampling[i] = ff_tget(&s->gb, type, s->le);
            if (s->subsampling[i] <= 0) {
                av_em_log(s->avctx, AV_LOG_ERROR, "subsampling %d is invalid\n", s->subsampling[i]);
                return AVERROR_INVALIDDATA;
            }
        }
        break;
...
#define ADD_METADATA(count, name, sep)\
    if ((ret = add_metadata(count, type, name, sep, s, frame)) < 0) {\
        av_em_log(s->avctx, AV_LOG_ERROR, "Error allocating temporary buffer\n");\
        goto end;\
    }
    ...
    case TIFF_GEO_KEY_DIRECTORY:
        ...
        if (s->geotag_count > count / 4 - 1) {
            ...
            av_em_log(s->avctx, AV_LOG_WARNING, "GeoTIFF key directory buffer shorter than specified\n");
        }
        if (bytestream2_get_bytes_left(&s->gb) < s->geotag_count * sizeof(int16_t) * 4) {
            s->geotag_count = 0;
            return -1;
        }
        s->geotags = av_em_mallocz_array(s->geotag_count, sizeof(TiffGeoTag));
        if (!s->geotags) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Error allocating temporary buffer\n");
            s->geotag_count = 0;
            goto end;
    ...
    case TIFF_GEO_DOUBLE_PARAMS:
        ...
        if (bytestream2_get_bytes_left(&s->gb) < count * sizeof(int64_t))
            return AVERROR_INVALIDDATA;
        dp = av_em_malloc_array(count, sizeof(double));
        if (!dp) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Error allocating temporary buffer\n");
            goto end;
        }
        ...
        for (i = 0; i < s->geotag_count; i++) {
            if (s->geotags[i].type == TIFF_GEO_DOUBLE_PARAMS) {
                if (s->geotags[i].count == 0
                    ...
                    av_em_log(s->avctx, AV_LOG_WARNING, "Invalid GeoTIFF key %d\n", s->geotags[i].key);
                } else {
            ...
            if (s->geotags[i].type == TIFF_GEO_ASCII_PARAMS) {
                if (s->geotags[i].count == 0
                    ...
                    if (bytestream2_get_bytes_left(&s->gb) < s->geotags[i].count)
                        return AVERROR_INVALIDDATA;
                    ap = av_em_alloc(s->geotags[i].count);
                    if (!ap) {
                        av_em_log(s->avctx, AV_LOG_ERROR, "Error allocating temporary buffer\n");
                        return AVERROR(ENOMEM);
                    }
...
static int decode_frame(AVEMCodecContext *avctx,
                        void *data, int *got_frame, AVEMPacket *avpkt)
{
    ...
    if ((ret = ff_tdecode_header(&s->gb, &le, &off))) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid TIFF header\n");
        return ret;
    } else if (off >= UINT_MAX - 14 || avpkt->size < off + 14) {
        av_em_log(avctx, AV_LOG_ERROR, "IFD offset is greater than image size\n");
        return AVERROR_INVALIDDATA;
    }
    s->le          = le;
    ...
    for (i = 0; i<s->geotag_count; i++) {
        ...
        if (!keyname) {
            av_em_log(avctx, AV_LOG_WARNING, "Unknown or unsupported GeoTIFF key %d\n", s->geotags[i].key);
            continue;
        }
        if (get_geokey_type(s->geotags[i].key) != s->geotags[i].type) {
            av_em_log(avctx, AV_LOG_WARNING, "Type of GeoTIFF key %d is wrong\n", s->geotags[i].key);
            continue;
        }
        ret = av_em_dict_set(avempriv_frame_get_metadatap(p), keyname, s->geotags[i].val, 0);
        if (ret<0) {
            av_em_log(avctx, AV_LOG_ERROR, "Writing metadata with key '%s' failed\n", keyname);
            return ret;
        }
    }

    if (!s->strippos && !s->stripoff) {
        av_em_log(avctx, AV_LOG_ERROR, "Image data is missing\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->strips == 1 && !s->stripsize) {
        av_em_log(avctx, AV_LOG_WARNING, "Image data size missing\n");
    ...
    if (s->rps <= 0 || s->rps % s->subsampling[1]) {
        av_em_log(avctx, AV_LOG_ERROR, "rps %d invalid\n", s->rps);
        return AVERROR_INVALIDDATA;
    }
    ...
    for (plane = 0; plane < planes; plane++) {
        ...
        for (i = 0; i < s->height; i += s->rps) {
            ...
            if (soff > avpkt->size || ssize > avpkt->size - soff) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid strip size/offset\n");
                return AVERROR_INVALIDDATA;
            }
        ...
        if (s->predictor == 2) {
            if (s->photometric == TIFF_PHOTOMETRIC_YCBCR) {
                av_em_log(s->avctx, AV_LOG_ERROR, "predictor == 2 with YUV is unsupported");
                return AVERROR_PATCHWELCOME;
            }
            dst   = p->data[plane];
            soff  = s->bpp >> 3;
...
static av_cold int tiff_init(AVEMCodecContext *avctx)
{
...
static av_cold int tiff_end(AVEMCodecContext *avctx)
{
    TiffContext *const s = avctx->priv_data;
    ...
    av_em_freep(&s->deinvert_buf);
    return 0;
}

AVEMCodec ff_tiff_decoder = {
    .name           = "tiff",
