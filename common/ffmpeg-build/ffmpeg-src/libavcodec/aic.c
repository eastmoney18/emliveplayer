...
static int aic_decode_header(AICContext *ctx, const uint8_t *src, int size)
{
    ...
    if (src[0] != 1) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Invalid version %d\n", src[0]);
        return AVERROR_INVALIDDATA;
    }
    if (src[1] != AIC_HDR_SIZE - 2) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Invalid header size %d\n", src[1]);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (frame_size > size) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Frame size should be %"PRIu32" got %d\n",
               frame_size, size);
        return AVERROR_INVALIDDATA;
    }
...
static int aic_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                            AVEMPacket *avpkt)
{
    ...
    if (buf_size < off) {
        av_em_log(avctx, AV_LOG_ERROR, "Too small frame\n");
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int aic_decode_init(AVEMCodecContext *avctx)
{
    AICContext *ctx = avctx->priv_data;
    int i;
    uint8_t scan[64];
    ...
    ctx->slice_data = av_em_malloc_array(ctx->slice_width, AIC_BAND_COEFFS
                                * sizeof(*ctx->slice_data));
    if (!ctx->slice_data) {
        av_em_log(avctx, AV_LOG_ERROR, "Error allocating slice buffer\n");
...
static av_cold int aic_decode_close(AVEMCodecContext *avctx)
{
    AICContext *ctx = avctx->priv_data;
