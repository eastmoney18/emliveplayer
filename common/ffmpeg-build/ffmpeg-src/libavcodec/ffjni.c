...
JNIEnv *ff_jni_attach_env(int *attached, void *log_ctx)
{
    ...
    *attached = 0;
    ...
    if (java_vm == NULL) {
        java_vm = av_em_jni_get_java_vm(log_ctx);
    }
    pthread_mutex_unlock(&lock);

    if (!java_vm) {
        av_em_log(log_ctx, AV_LOG_ERROR, "No Java virtual machine has been registered\n");
        return NULL;
    }
    ...
    case JNI_EDETACHED:
        if ((*java_vm)->AttachCurrentThread(java_vm, &env, NULL) != 0) {
            av_em_log(log_ctx, AV_LOG_ERROR, "Failed to attach the JNI environment to the current thread\n");
            env = NULL;
        } else {
            *attached = 1;
        }
    ...
    case JNI_OK:
        break;
    case JNI_EVERSION:
        av_em_log(log_ctx, AV_LOG_ERROR, "The specified JNI version is not supported\n");
        break;
    default:
        av_em_log(log_ctx, AV_LOG_ERROR, "Failed to get the JNI environment attached to this thread");
        break;
    }
...
char *ff_jni_jstring_to_utf_chars(JNIEnv *env, jstring string, void *log_ctx)
{
    ...
    if ((*env)->ExceptionCheck(env)) {
        ...
        av_em_log(log_ctx, AV_LOG_ERROR, "String.getStringUTFChars() threw an exception\n");
        return NULL;
    }

    ret = av_em_strdup(utf_chars);

    (*env)->ReleaseStringUTFChars(env, string, utf_chars);
    if ((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
        av_em_log(log_ctx, AV_LOG_ERROR, "String.releaseStringUTFChars() threw an exception\n");
        return NULL;
    }
...
jstring ff_jni_utf_chars_to_jstring(JNIEnv *env, const char *utf_chars, void *log_ctx)
{
    ...
    if ((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
        av_em_log(log_ctx, AV_LOG_ERROR, "NewStringUTF() threw an exception\n");
        return NULL;
    }
...
int ff_jni_exception_get_summary(JNIEnv *env, jthrowable exception, char **error, void *log_ctx)
{
    ...
    av_em_bprint_init(&bp, 0, AV_BPRINT_SIZE_AUTOMATIC);

    exception_class = (*env)->GetObjectClass(env, exception);
    if ((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
        av_em_log(log_ctx, AV_LOG_ERROR, "Could not find Throwable class\n");
    ...
    if ((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
        av_em_log(log_ctx, AV_LOG_ERROR, "Could not find Throwable class's class\n");
        ret = AVERROR_EXTERNAL;
        goto done;
    }
    ...
    if ((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
        av_em_log(log_ctx, AV_LOG_ERROR, "Could not find method Class.getName()\n");
        ret = AVERROR_EXTERNAL;
        goto done;
    }
    ...
    if ((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
        av_em_log(log_ctx, AV_LOG_ERROR, "Class.getName() threw an exception\n");
        ret = AVERROR_EXTERNAL;
        goto done;
    }
    ...
    if ((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
        av_em_log(log_ctx, AV_LOG_ERROR, "Could not find method java/lang/Throwable.getMessage()\n");
        ret = AVERROR_EXTERNAL;
        goto done;
    }
    ...
    if ((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
        av_em_log(log_ctx, AV_LOG_ERROR, "Throwable.getMessage() threw an exception\n");
        ret = AVERROR_EXTERNAL;
        goto done;
    }
    ...
    if (name && message) {
        av_em_bprintf(&bp, "%s: %s", name, message);
    } else if (name && !message) {
        av_em_bprintf(&bp, "%s occurred", name);
    } else if (!name && message) {
        av_em_bprintf(&bp, "Exception: %s", message);
    } else {
        av_em_log(log_ctx, AV_LOG_WARNING, "Could not retreive exception name and message\n");
        av_em_bprintf(&bp, "Exception occurred");
    }

    ret = av_em_bprint_finalize(&bp, error);
done:
...
int ff_jni_exception_check(JNIEnv *env, int log, void *log_ctx)
{
    ...
    av_em_log(log_ctx, AV_LOG_ERROR, "%s\n", message);
    av_em_free(message);
...
int ff_jni_init_jfields(JNIEnv *env, void *jfields, const struct FFJniField *jfields_mapping, int global, void *log_ctx)
{
    ...
    for (i = 0; jfields_mapping[i].name; i++) {
        ...
        if (type == FF_JNI_CLASS) {
            ...
            default:
                av_em_log(log_ctx, AV_LOG_ERROR, "Unknown JNI field type\n");
                ret = AVERROR(EINVAL);
                goto done;
            }
        }
