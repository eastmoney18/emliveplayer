...
static int cpia_decode_frame(AVEMCodecContext *avctx,
                             void *data, int *got_frame, AVEMPacket* avpkt)
{
    ...
    if ( avpkt->size < FRAME_HEADER_SIZE
      ...
      || (header[29] != NO_DECIMATION && header[29] != DECIMATION_ENAB)
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Invalid header!\n");
        return AVERROR_INVALIDDATA;
    }

    // currently unsupported properties
    if (header[17] == SUBSAMPLE_422) {
        avpriv_em_report_missing_feature(avctx, "4:2:2 subsampling");
        return AVERROR_PATCHWELCOME;
    }
    if (header[18] == YUVORDER_UYVY) {
        avpriv_em_report_missing_feature(avctx, "YUV byte order UYVY");
        return AVERROR_PATCHWELCOME;
    }
    if (header[29] == DECIMATION_ENAB) {
        avpriv_em_report_missing_feature(avctx, "Decimation");
        return AVERROR_PATCHWELCOME;
    }
    ...
    if ((ret = em_reget_buffer(avctx, frame)) < 0)
        return ret;
    ...
    for ( i = 0;
        ...
        if (src_size < linelength) {
            av_em_frame_set_decode_error_flags(frame, FF_DECODE_ERROR_INVALID_BITSTREAM);
            av_em_log(avctx, AV_LOG_WARNING, "Frame ended unexpectedly!\n");
            break;
        }
        if (src[linelength - 1] != EOL) {
            av_em_frame_set_decode_error_flags(frame, FF_DECODE_ERROR_INVALID_BITSTREAM);
            av_em_log(avctx, AV_LOG_WARNING, "Wrong line length %d or line not terminated properly (found 0x%02x)!\n", linelength, src[linelength - 1]);
            break;
        }
        ...
        if ((i & 1) && header[17] == SUBSAMPLE_420) {
            ...
            for (j = 0; j < linelength - 1; j++) {
                if (y > y_end) {
                    av_em_frame_set_decode_error_flags(frame, FF_DECODE_ERROR_INVALID_BITSTREAM);
                    av_em_log(avctx, AV_LOG_WARNING, "Decoded data exceeded linesize!\n");
                    break;
                }
    ...
    *got_frame = 1;
    if ((ret = av_em_frame_ref(data, cpia->frame)) < 0)
        return ret;

    return avpkt->size;
}

static av_cold int cpia_decode_init(AVEMCodecContext *avctx)
{
...
static av_cold int cpia_decode_end(AVEMCodecContext *avctx)
{
    CpiaContext *s = avctx->priv_data;
