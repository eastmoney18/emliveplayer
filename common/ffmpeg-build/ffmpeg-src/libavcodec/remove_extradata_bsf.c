...
static int remove_extradata(AVBSFContext *ctx, AVEMPacket *out)
{
...
static int remove_extradata_init(AVBSFContext *ctx)
{
    ...
    s->parser = av_em_parser_init(ctx->par_in->codec_id);

    if (s->parser) {
        s->avctx = avcodec_em_alloc_context3(NULL);
        if (!s->avctx)
            return AVERROR(ENOMEM);

        ret = avcodec_em_parameters_to_context(s->avctx, ctx->par_in);
        if (ret < 0)
            return ret;
    }
...
static void remove_extradata_close(AVBSFContext *ctx)
{
    ...
    avcodec_em_free_context(&s->avctx);
    av_em_parser_close(s->parser);
}
...
static const AVEMClass remove_extradata_class = {
...
const AVBitStreamFilter em_remove_extradata_bsf = {
    .name           = "remove_extra",
