...
static int decode_13(AVEMCodecContext *avctx, DxaDecContext *c, uint8_t* dst,
        ...
        for(i = 0; i < avctx->width; i += 4){
            ...
            case 4: // motion compensation
                ...
                if (i < -x || avctx->width  - i - 4 < x ||
                    ...
                    av_em_log(avctx, AV_LOG_ERROR, "MV %d %d out of bounds\n", x,y);
                    return AVERROR_INVALIDDATA;
                }
                tmp2 += x + y*stride;
            case 0: // skip
            ...
            case 34:
                ...
                break;
            default:
                av_em_log(avctx, AV_LOG_ERROR, "Unknown opcode %d\n", type);
                return AVERROR_INVALIDDATA;
            }
        }
        dst += stride * 4;
...
static int decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (compr != 4 && compr != -1) {
        ...
        if (uncompress(c->decomp_buf, &dsize, avpkt->data + bytestream2_tell(&gb),
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Uncompress failed!\n");
    ...
    if (avctx->debug & FF_DEBUG_PICT_INFO)
        av_em_log(avctx, AV_LOG_DEBUG, "compr:%2d, dsize:%d\n", compr, (int)dsize);
    ...
    case 13:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unknown/unsupported compression type %d\n", compr);
        return AVERROR_INVALIDDATA;
    }

    av_em_frame_unref(c->prev);
    if ((ret = av_em_frame_ref(c->prev, frame)) < 0)
        return ret;
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->width%4 || avctx->height%4) {
        avpriv_em_request_sample(avctx, "dimensions are not a multiple of 4");
        return AVERROR_INVALIDDATA;
    }

    c->prev = av_em_frame_alloc();
    if (!c->prev)
    ...
    c->decomp_buf = av_em_alloc(c->dsize + DECOMP_BUF_PADDING);
    if (!c->decomp_buf) {
        av_em_frame_free(&c->prev);
