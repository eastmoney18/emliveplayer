...
static const AVEMClass amediaformat_class = {
    .class_name = "amediaformat",
    .item_name  = av_em_default_item_name,
    .version    = LIBavcodec_em_version_INT,
};
...
static const AVEMClass amediacodec_class = {
...
int ff_AMediaCodecProfile_getProfileFromAVEMCodecContext(AVEMCodecContext *avctx)
{
    int ret = -1;
...
char *ff_AMediaCodecList_getCodecNameByType(const char *mime, int profile, int encoder, void *log_ctx)
{
    ...
    for(i = 0; i < codec_count; i++) {
        ...
        for (j = 0; j < type_count; j++) {
            ...
            if (!av_em_strcasecmp(supported_type, mime)) {
...
FFAMediaFormat *ff_AMediaFormat_new(void)
{
    ...
    format = av_em_mallocz(sizeof(FFAMediaFormat));
    if (!format) {
        return NULL;
    }
...
FFAMediaCodec* ff_AMediaCodec_createCodecByName(const char *name)
{
    ...
    codec = av_em_mallocz(sizeof(FFAMediaCodec));
    if (!codec) {
        return NULL;
    }
