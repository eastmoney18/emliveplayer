...
static av_cold int m101_decode_init(AVEMCodecContext *avctx)
{
    if (avctx->extradata_size < 6*4) {
        avpriv_em_request_sample(avctx, "Missing or too small extradata (size %d)\n", avctx->extradata_size);
    ...
    if (avctx->extradata[2*4] == 10)
        ...
        avpriv_em_request_sample(avctx, "BPS %d\n", avctx->extradata[2*4]);
        return AVERROR_INVALIDDATA;
    }
...
static int m101_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                      AVEMPacket *avpkt)
{
    ...
    if (stride < min_stride || avpkt->size < stride * (uint64_t)avctx->height) {
        av_em_log(avctx, AV_LOG_ERROR, "stride (%d) is invalid for packet sized %d\n",
               stride, avpkt->size);
        return AVERROR_INVALIDDATA;
    }
