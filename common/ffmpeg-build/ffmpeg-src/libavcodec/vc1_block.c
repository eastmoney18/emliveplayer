        ...
        if (idx_mbmode <= 5) { // 1-MV
            ...
            if (bmvtype == BMV_TYPE_DIRECT) {
                ...
                if (!s->next_picture_ptr->field_picture) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "Mixed field/frame direct mode not supported\n");
                    return;
                }
            }
...
static int vc1_decode_b_mb_intfr(VC1Context *v)
{
    ...
    if (direct) {
        if (s->next_picture_ptr->field_picture)
            av_em_log(s->avctx, AV_LOG_WARNING, "Mixed frame/field direct mode not supported\n");
...
static void vc1_decode_i_blocks(VC1Context *v)
{
    ...
    for (s->mb_y = 0; s->mb_y < s->end_mb_y; s->mb_y++) {
        ...
        for (; s->mb_x < v->end_mb_x; s->mb_x++) {
            ...
            if (get_bits_count(&s->gb) > v->bits) {
                ff_em_er_add_slice(&s->er, 0, 0, s->mb_x, s->mb_y, ER_MB_ERROR);
                av_em_log(s->avctx, AV_LOG_ERROR, "Bits overconsumption: %i > %i\n",
    ...
    ff_em_er_add_slice(&s->er, 0, 0, s->mb_width - 1, s->mb_height - 1, ER_MB_END);
}
...
static void vc1_decode_i_blocks_adv(VC1Context *v)
{
    ...
    for (; s->mb_y < s->end_mb_y; s->mb_y++) {
        ...
        for (;s->mb_x < s->mb_width; s->mb_x++) {
            ...
            if (get_bits_count(&s->gb) > v->bits) {
                ...
                ff_em_er_add_slice(&s->er, 0, s->start_mb_y, s->mb_x, s->mb_y, ER_MB_ERROR);
                av_em_log(s->avctx, AV_LOG_ERROR, "Bits overconsumption: %i > %i\n",
                       get_bits_count(&s->gb), v->bits);
                return;
            }
    ...
    if (v->s.loop_filter)
        ff_mpeg_draw_horiz_band(s, (s->end_mb_y - 1) * 16, 16);
    ff_em_er_add_slice(&s->er, 0, s->start_mb_y << v->field_mode, s->mb_width - 1,
    ...
    for (s->mb_y = s->start_mb_y; s->mb_y < s->end_mb_y; s->mb_y++) {
        ...
        for (; s->mb_x < s->mb_width; s->mb_x++) {
            ...
            if (get_bits_count(&s->gb) > v->bits || get_bits_count(&s->gb) < 0) {
                // TODO: may need modification to handle slice coding
                ff_em_er_add_slice(&s->er, 0, s->start_mb_y, s->mb_x, s->mb_y, ER_MB_ERROR);
                av_em_log(s->avctx, AV_LOG_ERROR, "Bits overconsumption: %i > %i at %ix%i\n",
...
static void vc1_decode_skip_blocks(VC1Context *v)
{
    ...
    ff_em_er_add_slice(&s->er, 0, s->start_mb_y, s->mb_width - 1, s->end_mb_y - 1, ER_MB_END);
    s->first_slice_line = 1;
    for (s->mb_y = s->start_mb_y; s->mb_y < s->end_mb_y; s->mb_y++) {
        s->mb_x = 0;
        init_block_index(v);
