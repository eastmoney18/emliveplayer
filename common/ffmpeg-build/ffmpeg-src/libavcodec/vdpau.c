...
AVVDPAUContext *av_em_alloc_vdpaucontext(void)
{
    return av_em_vdpau_alloc_context();
}

MAKE_ACCESSORS(AVVDPAUContext, vdpau_hwaccel, AVVDPAU_Render2, render2)

int av_em_vdpau_get_surface_parameters(AVEMCodecContext *avctx,
                                    VdpChromaType *type,
...
int ff_vdpau_common_init(AVEMCodecContext *avctx, VdpDecoderProfile profile,
                         int level)
{
    ...
    if (!hwctx) {
        ...
        av_em_log(avctx, AV_LOG_WARNING, "hwaccel_context has not been setup by the user application, cannot initialize\n");
        return 0;
    }
    ...
    if (av_em_vdpau_get_surface_parameters(avctx, &type, &width, &height))
        return AVERROR(ENOSYS);
...
int ff_vdpau_common_uninit(AVEMCodecContext *avctx)
{
    VDPAUContext *vdctx = avctx->internal->hwaccel_priv_data;
...
static int ff_vdpau_common_reinit(AVEMCodecContext *avctx)
{
    VDPAUHWContext *hwctx = avctx->hwaccel_context;
    VDPAUContext *vdctx = avctx->internal->hwaccel_priv_data;
...
int ff_vdpau_common_end_frame(AVEMCodecContext *avctx, AVFrame *frame,
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    av_em_freep(&pic_ctx->bitstream_buffers);
...
int ff_vdpau_mpeg_end_frame(AVEMCodecContext *avctx)
{
                        ...
                        const uint8_t *buf, uint32_t size)
{
    ...
    buffers = av_em_fast_realloc(buffers, &pic_ctx->bitstream_buffers_allocated,
...
void ff_vdpau_add_data_chunk(uint8_t *data, const uint8_t *buf, int buf_size)
{
    ...
    render->bitstream_buffers= av_em_fast_realloc(
...
void ff_vdpau_h264_set_reference_frames(H264EMContext *h)
{
...
void ff_vdpau_h264_picture_start(H264EMContext *h)
{
...
void ff_vdpau_h264_picture_complete(H264EMContext *h)
{
    struct vdpau_render_state *render;
    ...
    em_h264_draw_horiz_band(h, &h->slice_ctx[0], 0, h->avctx->height);
...
int av_em_vdpau_get_profile(AVEMCodecContext *avctx, VdpDecoderProfile *profile)
{
#define PROFILE(prof)                      \
do {                                       \
...
AVVDPAUContext *av_em_vdpau_alloc_context(void)
{
    return av_em_mallocz(sizeof(AVVDPAUContext));
}

int av_em_vdpau_bind_context(AVEMCodecContext *avctx, VdpDevice device,
                          VdpGetProcAddress *get_proc, unsigned flags)
{
    ...
    if (av_em_reallocp(&avctx->hwaccel_context, sizeof(*hwctx)))
        return AVERROR(ENOMEM);
