...
void em_hevc_set_qPy(HEVCContext *s, int xBase, int yBase, int log2_cb_size)
{
                                           ...
                                           int log2_trafo_size)
{
    ...
    if (boundary_upper) {
        RefPicList *rpl_top = (lc->boundary_flags & BOUNDARY_UPPER_SLICE) ?
                              em_hevc_get_ref_list(s, s->ref, x0, y0 - 1) :
                              s->ref->refPicList;
    ...
    if (boundary_left) {
        RefPicList *rpl_left = (lc->boundary_flags & BOUNDARY_LEFT_SLICE) ?
                               em_hevc_get_ref_list(s, s->ref, x0 - 1, y0) :
                               s->ref->refPicList;
    ...
    if (s->ps.sps->sao_enabled) {
        ...
        if (y && x_end) {
            ...
            if (s->threads_type & FF_THREAD_FRAME )
                em_thread_report_progress(&s->ref->tf, y, 0);
        }
        if (x_end && y_end) {
            sao_filter_CTB(s, x , y);
            if (s->threads_type & FF_THREAD_FRAME )
                em_thread_report_progress(&s->ref->tf, y + ctb_size, 0);
        }
    } else if (s->threads_type & FF_THREAD_FRAME && x_end)
        em_thread_report_progress(&s->ref->tf, y + ctb_size - 4, 0);
}
...
void em_hevc_hls_filters(HEVCContext *s, int x_ctb, int y_ctb, int ctb_size)
{
    ...
    if (y_ctb && x_ctb)
        em_hevc_hls_filter(s, x_ctb - ctb_size, y_ctb - ctb_size, ctb_size);
    if (y_ctb && x_end)
        em_hevc_hls_filter(s, x_ctb, y_ctb - ctb_size, ctb_size);
    if (x_ctb && y_end)
        em_hevc_hls_filter(s, x_ctb - ctb_size, y_ctb, ctb_size);
}
