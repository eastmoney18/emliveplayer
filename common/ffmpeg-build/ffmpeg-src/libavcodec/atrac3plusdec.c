...
static av_cold int atrac3p_decode_close(AVEMCodecContext *avctx)
{
    ATRAC3PContext *ctx = avctx->priv_data;

    av_em_freep(&ctx->ch_units);
    av_em_freep(&ctx->fdsp);
...
static av_cold int atrac3p_decode_init(AVEMCodecContext *avctx)
{
    ATRAC3PContext *ctx = avctx->priv_data;
    int i, ch, ret;
    ...
    ctx->ch_units = av_em_mallocz_array(ctx->num_channel_blocks, sizeof(*ctx->ch_units));
    ctx->fdsp = avpriv_em_float_dsp_alloc(avctx->flags & AV_CODEC_FLAG_BITEXACT);
...
static void reconstruct_frame(ATRAC3PContext *ctx, Atrac3pChanUnitCtx *ch_unit,
                              int num_channels, AVEMCodecContext *avctx)
{
    int ch, sb;
...
static int atrac3p_decode_frame(AVEMCodecContext *avctx, void *data,
                                int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (get_bits1(&ctx->gb)) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid start bit!\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    while (get_bits_left(&ctx->gb) >= 2 &&
        ...
        if (ch_unit_id == CH_UNIT_EXTENSION) {
            avpriv_em_report_missing_feature(avctx, "Channel unit extension");
