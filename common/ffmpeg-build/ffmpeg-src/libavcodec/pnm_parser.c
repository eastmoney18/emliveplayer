...
static int pnm_parse(AVEMCodecParserContext *s, AVEMCodecContext *avctx,
...
retry:
    ...
    if (ff_pnm_decode_header(avctx, &pnmctx) < 0) {
        ...
        next = pnmctx.bytestream - pnmctx.bytestream_start
               + av_em_image_get_buffer_size(avctx->pix_fmt, avctx->width, avctx->height, 1);
