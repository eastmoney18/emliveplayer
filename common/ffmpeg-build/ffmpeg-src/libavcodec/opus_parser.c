 ...
 */
static int opus_find_frame_end(AVEMCodecParserContext *ctx, AVEMCodecContext *avctx,
                               const uint8_t *buf, int buf_size, int *header_len)
{
    ...
    *header_len = 0;
    ...
    if (avctx->extradata && !s->extradata_parsed) {
        ...
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Error parsing Ogg extradata.\n");
            return AVERROR_INVALIDDATA;
        }
        av_em_freep(&s->ctx.channel_maps);
    ...
    if (payload_len <= buf_size && (!s->ts_framing || start_found)) {
        ...
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Error parsing Opus packet header.\n");
...
static int opus_parse(AVEMCodecParserContext *ctx, AVEMCodecContext *avctx,
                       ...
                       const uint8_t *buf, int buf_size)
{
    ...
    if (s->ts_framing && next != AVERROR_INVALIDDATA &&
        em_combine_frame(pc, next, &buf, &buf_size) < 0) {
        *poutbuf      = NULL;
        *poutbuf_size = 0;
        return buf_size;
    }
...
AVEMCodecParser ff_opus_parser = {
