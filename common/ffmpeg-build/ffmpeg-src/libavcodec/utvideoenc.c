...
static av_cold int utvideo_encode_close(AVEMCodecContext *avctx)
{
    UtvideoContext *c = avctx->priv_data;
    int i;

    av_em_freep(&c->slice_bits);
    for (i = 0; i < 4; i++)
        av_em_freep(&c->slice_buffer[i]);

    return 0;
}

static av_cold int utvideo_encode_init(AVEMCodecContext *avctx)
{
    UtvideoContext *c = avctx->priv_data;
    ...
    case AV_PIX_FMT_YUV422P:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unknown pixel format: %d\n",
               avctx->pix_fmt);
        return AVERROR_INVALIDDATA;
    }

    em_bswapdsp_init(&c->bdsp);
    em_huffyuvencdsp_init(&c->hdsp);
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (c->frame_pred == PRED_GRADIENT) {
        av_em_log(avctx, AV_LOG_ERROR, "Gradient prediction is not supported.\n");
        return AVERROR_OPTION_NOT_FOUND;
    }
    ...
    subsampled_height = avctx->height >> av_em_pix_fmt_desc_get(avctx->pix_fmt)->log2_chroma_h;
    ...
    if (!avctx->extradata) {
        av_em_log(avctx, AV_LOG_ERROR, "Could not allocate extradata.\n");
        utvideo_encode_close(avctx);
        return AVERROR(ENOMEM);
    }

    for (i = 0; i < c->planes; i++) {
        c->slice_buffer[i] = av_em_alloc(c->slice_stride * (avctx->height + 2) +
                                       AV_INPUT_BUFFER_PADDING_SIZE);
        if (!c->slice_buffer[i]) {
            av_em_log(avctx, AV_LOG_ERROR, "Cannot allocate temporary buffer 1.\n");
            utvideo_encode_close(avctx);
            return AVERROR(ENOMEM);
        }
    }
...
static int encode_plane(AVEMCodecContext *avctx, uint8_t *src,
                        ...
                        int width, int height, PutByteContext *pb)
{
    ...
    case PRED_NONE:
        for (i = 0; i < c->slices; i++) {
            ...
            av_em_image_copy_plane(dst + sstart * width, width,
    ...
    case PRED_MEDIAN:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unknown prediction mode: %d\n",
               c->frame_pred);
        return AVERROR_OPTION_NOT_FOUND;
    }
    ...
    if ((ret = em_huff_gen_len_table(lengths, counts, 256, 1)) < 0)
        return ret;
...
static int utvideo_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                                const AVFrame *pic, int *got_packet)
{
    ...
    ret = em_alloc_packet2(avctx, pkt, (256 + 4 * c->slices + width * height) *
                           c->planes + 4, 0);
    ...
    av_em_fast_padded_malloc(&c->slice_bits, &c->slice_bits_size, width * height + 4);

    if (!c->slice_bits) {
        av_em_log(avctx, AV_LOG_ERROR, "Cannot allocate temporary buffer 2.\n");
        return AVERROR(ENOMEM);
    }
    ...
    case AV_PIX_FMT_RGBA:
        for (i = 0; i < c->planes; i++) {
            ...
            if (ret) {
                av_em_log(avctx, AV_LOG_ERROR, "Error encoding plane %d.\n", i);
                return ret;
            }
        }
        break;
...
static const AVEMClass utvideo_class = {
    .class_name = "utvideo",
