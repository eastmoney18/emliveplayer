...
static int parse_lfe_24(DCALbrDecoder *s)
{
    ...
    if (step_i > step_max) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid LFE step size index\n");
        return -1;
    }
...
static int parse_lfe_chunk(DCALbrDecoder *s, LBRChunk *chunk)
{
    ...
    av_em_log(s->avctx, AV_LOG_ERROR, "LFE chunk too short\n");
    return -1;
}
    ...
    for (sf = 0; sf < 1 << group; sf += diff ? 8 : 1) {
        ...
        for (freq = 1;; freq++) {
            if (get_bits_left(&s->gb) < 1) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Tonal group chunk too short\n");
                return -1;
            }

            diff = parse_vlc(&s->gb, &ff_dca_vlc_tnl_grp[group], 2);
            if (diff >= FF_ARRAY_ELEMS(ff_dca_fst_amp)) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid tonal frequency diff\n");
                return -1;
            }
            ...
            if (freq >> (5 - group) > s->nsubbands * 4 - 5) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid spectral line offset\n");
                return -1;
            }
...
static int parse_tonal_chunk(DCALbrDecoder *s, LBRChunk *chunk)
{
    ...
    if (chunk->id == LBR_CHUNK_SCF || chunk->id == LBR_CHUNK_TONAL_SCF) {
        if (get_bits_left(&s->gb) < 36) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Tonal scale factor chunk too short\n");
            return -1;
        }
        for (sb = 0; sb < 6; sb++)
...
static int parse_scale_factors(DCALbrDecoder *s, uint8_t *scf)
{
    ...
    for (sf = 0; sf < 7; sf += dist) {
        ...
        if (dist > 7 - sf) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid scale factor distance\n");
            return -1;
        }
...
static int parse_grid_1_chunk(DCALbrDecoder *s, LBRChunk *chunk, int ch1, int ch2)
{
    ...
    if (get_bits_left(&s->gb) < 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "First grid chunk too short\n");
        return -1;
    }
...
static int alloc_sample_buffer(DCALbrDecoder *s)
{
    ...
    av_em_fast_mallocz(&s->ts_buffer, &s->ts_size, nsamples * sizeof(float));
    if (!s->ts_buffer)
        return -1;
...
static int parse_decoder_init(DCALbrDecoder *s, GetByteContext *gb)
{
    ...
    if (sr_code >= FF_ARRAY_ELEMS(ff_dca_sampling_freqs)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid LBR sample rate\n");
        return AVERROR_INVALIDDATA;
    }
    s->sample_rate = ff_dca_sampling_freqs[sr_code];
    if (s->sample_rate > 48000) {
        avpriv_em_report_missing_feature(s->avctx, "%d Hz LBR sample rate", s->sample_rate);
    ...
    if (!(s->ch_mask & 0x7)) {
        avpriv_em_report_missing_feature(s->avctx, "LBR channel mask %#x", s->ch_mask);
        return AVERROR_PATCHWELCOME;
    }
    ...
    if ((version & 0xff00) != 0x0800) {
        avpriv_em_report_missing_feature(s->avctx, "LBR stream version %#x", version);
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (s->flags & LBR_FLAG_DMIX_MULTI_CH) {
        avpriv_em_report_missing_feature(s->avctx, "LBR multi-channel downmix");
        return AVERROR_PATCHWELCOME;
    }
    if ((s->flags & LBR_FLAG_LFE_PRESENT) && s->sample_rate != 48000) {
        if (!(s->warned & 2)) {
            avpriv_em_report_missing_feature(s->avctx, "%d Hz LFE interpolation", s->sample_rate);
            s->warned |= 2;
        }
    ...
    case LBR_FLAG_BAND_LIMIT_1_4:
        ...
        break;
    default:
        avpriv_em_report_missing_feature(s->avctx, "LBR band limit %#x", s->flags & LBR_FLAG_BAND_LIMIT_MASK);
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (s->limited_range < 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid LBR band limit for frequency range\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->flags & LBR_FLAG_DMIX_STEREO) {
        ...
        if (s->nchannels_total < 3 || s->nchannels_total > DCA_LBR_CHANNELS_TOTAL - 2) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid number of channels for LBR stereo downmix\n");
            return AVERROR_INVALIDDATA;
        }

        // This decoder doesn't support ECS chunk
        if (dca->request_channel_layout != DCA_SPEAKER_LAYOUT_STEREO && !(s->warned & 4)) {
            avpriv_em_report_missing_feature(s->avctx, "Embedded LBR stereo downmix");
            s->warned |= 4;
        }
...
int ff_dca_lbr_parse(DCALbrDecoder *s, uint8_t *data, DCAExssAsset *asset)
{
    ...
    if (bytestream2_get_be32(&gb) != DCA_SYNCWORD_LBR) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid LBR sync word\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    case LBR_HEADER_SYNC_ONLY:
        if (!s->sample_rate) {
            av_em_log(s->avctx, AV_LOG_ERROR, "LBR decoder not initialized\n");
            return AVERROR_INVALIDDATA;
        }
        break;
    case LBR_HEADER_DECODER_INIT:
        ...
        break;
    default:
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid LBR header type\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (chunk_len > bytestream2_get_bytes_left(&gb)) {
        ...
        av_em_log(s->avctx, AV_LOG_WARNING, "LBR frame chunk was truncated\n");
    ...
    case LBR_CHUNK_FRAME:
        if (s->avctx->err_recognition & (AV_EF_CRCCHECK | AV_EF_CAREFUL)) {
            ...
            for (i = 0; i < chunk_len - 2; i++)
                res += gb.buffer[i];
            if (checksum != res) {
                av_em_log(s->avctx, AV_LOG_WARNING, "Invalid LBR checksum\n");
                if (s->avctx->err_recognition & AV_EF_EXPLODE)
                    return AVERROR_INVALIDDATA;
            }
        } else {
    ...
    case LBR_CHUNK_FRAME_NO_CSUM:
        break;
    default:
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid LBR frame chunk ID\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    while (bytestream2_get_bytes_left(&gb) > 0) {
        ...
        if (chunk_len > bytestream2_get_bytes_left(&gb)) {
            chunk_len = bytestream2_get_bytes_left(&gb);
            av_em_log(s->avctx, AV_LOG_WARNING, "LBR chunk %#x was truncated\n", chunk_id);
            if (s->avctx->err_recognition & AV_EF_EXPLODE)
                return AVERROR_INVALIDDATA;
        }
...
int ff_dca_lbr_filter_frame(DCALbrDecoder *s, AVFrame *frame)
{
    ...
    if ((ret = em_side_data_update_matrix_encoding(frame, AV_MATRIX_ENCODING_NONE)) < 0)
        return ret;
...
av_cold int ff_dca_lbr_init(DCALbrDecoder *s)
{
    ...
    if (!(s->fdsp = avpriv_em_float_dsp_alloc(0)))
        return -1;
