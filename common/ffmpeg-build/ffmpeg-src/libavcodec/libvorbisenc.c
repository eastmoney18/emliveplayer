...
typedef struct LibvorbisEncContext {
    AVEMClass *av_class;                  /**< class for AVOptions            */
...
static const AVEMClass vorbis_class = {
...
static av_cold int libvorbis_setup(vorbis_info *vi, AVEMCodecContext *avctx)
{
    ...
    if (avctx->channels == 3 &&
        ...
        if (avctx->channel_layout) {
            char name[32];
            av_em_get_channel_layout_string(name, sizeof(name), avctx->channels,
                                         avctx->channel_layout);
            av_em_log(avctx, AV_LOG_ERROR, "%s not supported by Vorbis: "
...
static av_cold int libvorbis_encode_close(AVEMCodecContext *avctx)
{
    LibvorbisEncContext *s = avctx->priv_data;
    ...
    av_em_fifo_freep(&s->pkt_fifo);
    ff_af_queue_close(&s->afq);
    av_em_freep(&avctx->extradata);

    av_em_vorbis_parse_free(&s->vp);

    return 0;
}

static av_cold int libvorbis_encode_init(AVEMCodecContext *avctx)
{
    LibvorbisEncContext *s = avctx->priv_data;
    ...
    if ((ret = libvorbis_setup(&s->vi, avctx))) {
        av_em_log(avctx, AV_LOG_ERROR, "encoder setup failed\n");
        goto error;
    }
    if ((ret = vorbis_analysis_init(&s->vd, &s->vi))) {
        av_em_log(avctx, AV_LOG_ERROR, "analysis init failed\n");
        ret = vorbis_error_to_averror(ret);
        goto error;
    }
    s->dsp_initialized = 1;
    if ((ret = vorbis_block_init(&s->vd, &s->vb))) {
        av_em_log(avctx, AV_LOG_ERROR, "dsp init failed\n");
        ret = vorbis_error_to_averror(ret);
        goto error;
    }
    ...
    avctx->extradata_size = 1 + xiph_len(header.bytes)      +
                                ...
                                header_code.bytes;
    p = avctx->extradata = av_em_alloc(avctx->extradata_size +
    ...
    p[0]    = 2;
    offset  = 1;
    offset += av_em_xiphlacing(&p[offset], header.bytes);
    offset += av_em_xiphlacing(&p[offset], header_comm.bytes);
    ...
    s->vp = av_em_vorbis_parse_init(avctx->extradata, avctx->extradata_size);
    if (!s->vp) {
    ...
    s->pkt_fifo = av_em_fifo_alloc(BUFFER_SIZE);
    if (!s->pkt_fifo) {
        ret = AVERROR(ENOMEM);
        goto error;
    }
...
static int libvorbis_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                                  const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if (frame) {
        ...
        if ((ret = vorbis_analysis_wrote(&s->vd, samples)) < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "error in vorbis_analysis_wrote()\n");
            return vorbis_error_to_averror(ret);
        }
    ...
    while ((ret = vorbis_analysis_blockout(&s->vd, &s->vb)) == 1) {
        ...
        while ((ret = vorbis_bitrate_flushpacket(&s->vd, &op)) == 1) {
            if (av_em_fifo_space(s->pkt_fifo) < sizeof(ogg_packet) + op.bytes) {
                av_em_log(avctx, AV_LOG_ERROR, "packet buffer is too small\n");
                return AVERROR_BUG;
            }
            av_em_fifo_generic_write(s->pkt_fifo, &op, sizeof(ogg_packet), NULL);
            av_em_fifo_generic_write(s->pkt_fifo, op.packet, op.bytes, NULL);
        }
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "error getting available packets\n");
            break;
        }
    }
    if (ret < 0) {
    ...
    if (av_em_fifo_size(s->pkt_fifo) < sizeof(ogg_packet))
        return 0;

    av_em_fifo_generic_read(s->pkt_fifo, &op, sizeof(ogg_packet), NULL);

    if ((ret = em_alloc_packet2(avctx, avpkt, op.bytes, 0)) < 0)
        return ret;
    av_em_fifo_generic_read(s->pkt_fifo, avpkt->data, op.bytes, NULL);

    avpkt->pts = em_samples_to_time_base(avctx, op.granulepos);

    duration = av_em_vorbis_parse_frame(s->vp, avpkt->data, avpkt->size);
    if (duration > 0) {
...
AVEMCodec ff_libvorbis_encoder = {
    .name           = "libvorbis",
