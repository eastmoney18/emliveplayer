...
static av_cold int xan_decode_end(AVEMCodecContext *avctx)
{
    ...
    av_em_freep(&s->buffer1);
    av_em_freep(&s->buffer2);
    av_em_freep(&s->palettes);
...
static av_cold int xan_decode_init(AVEMCodecContext *avctx)
{
    ...
    s->buffer1 = av_em_alloc(s->buffer1_size);
    if (!s->buffer1)
        return AVERROR(ENOMEM);
    s->buffer2_size = avctx->width * avctx->height;
    s->buffer2 = av_em_alloc(s->buffer2_size + 130);
    if (!s->buffer2) {
        av_em_freep(&s->buffer1);
        return AVERROR(ENOMEM);
    }
                                          ...
                                          int motion_y)
{
    ...
    if (prev_palette_plane == palette_plane && FFABS(curframe_index - prevframe_index) < pixel_count) {
         avpriv_em_request_sample(s->avctx, "Overlapping copy");
         return ;
    }
    ...
    while (total_pixels && opcode_buffer < opcode_buffer_end) {
        ...
        case 19:
            if (bytestream2_get_bytes_left(&size_segment) < 1) {
                av_em_log(s->avctx, AV_LOG_ERROR, "size_segment overread\n");
                return AVERROR_INVALIDDATA;
            }
        ...
        if (opcode < 12) {
            ...
            if (bytestream2_get_bytes_left(&vector_segment) <= 0) {
                av_em_log(s->avctx, AV_LOG_ERROR, "vector_segment overread\n");
                return AVERROR_INVALIDDATA;
            }
                            ...
                            AVEMPacket *avpkt)
{
    ...
    while (bytestream2_get_bytes_left(&ctx) > 8 && tag != VGA__TAG) {
        ...
        if (size < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid tag size %d\n", size);
            return AVERROR_INVALIDDATA;
        }
        ...
        case PALT_TAG:
            ...
            if (s->palettes_count >= PALETTES_MAX)
                return AVERROR_INVALIDDATA;
            tmpptr = av_em_realloc_array(s->palettes,
        ...
        case SHOT_TAG:
            ...
            if (new_pal < s->palettes_count) {
                ...
                av_em_log(avctx, AV_LOG_ERROR, "Invalid palette selected\n");
            break;
        case VGA__TAG:
            break;
        default:
    ...
    if (s->palettes_count <= 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "No palette found\n");
        return AVERROR_INVALIDDATA;
    }
