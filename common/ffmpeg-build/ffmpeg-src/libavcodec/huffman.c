...
int em_huff_gen_len_table(uint8_t *dst, const uint64_t *stats, int stats_size, int skip0)
{
    HeapElem *h  = av_em_malloc_array(sizeof(*h), stats_size);
    int *up      = av_em_malloc_array(sizeof(*up) * 2, stats_size);
    uint8_t *len = av_em_malloc_array(sizeof(*len) * 2, stats_size);
    uint16_t *map= av_em_malloc_array(sizeof(*map), stats_size);
    int offset, i, next;
    int size = 0;
    int ret = 0;
 ...
 */
int em_huff_build_tree(AVEMCodecContext *avctx, VLC *vlc, int nb_codes, int nb_bits,
                       Node *nodes, HuffCmp cmp, int flags)
{
    ...
    if (build_huff_tree(vlc, nodes, nb_codes * 2 - 2, flags, nb_bits) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error building tree\n");
        return -1;
    }
    return 0;
}
