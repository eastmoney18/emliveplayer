...
static int mss1_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                             AVEMPacket *avpkt)
{
    ...
    if ((ret = em_reget_buffer(avctx, ctx->pic)) < 0)
        return ret;
    ...
    if ((ret = av_em_frame_ref(data, ctx->pic)) < 0)
        return ret;
...
static av_cold int mss1_decode_init(AVEMCodecContext *avctx)
{
...
static av_cold int mss1_decode_end(AVEMCodecContext *avctx)
{
    MSS1Context * const ctx = avctx->priv_data;

    av_em_frame_free(&ctx->pic);
    ff_mss12_decode_end(&ctx->ctx);

    return 0;
}

AVEMCodec ff_mss1_decoder = {
    .name           = "mss1",
