...
#ifdef DEBUG
    av_em_log(NULL, AV_LOG_INFO, " %u. of %u code len %d code %d - ", p, num, bits[p], codes[p]);
    init_get_bits(&gb, (uint8_t *)&codes[p], bits[p]);
    for (i = 0; i < bits[p]; ++i)
        av_em_log(NULL, AV_LOG_INFO, "%s", get_bits1(&gb) ? "1" : "0");
...
#ifdef DEBUG
        av_em_log(NULL, AV_LOG_INFO, " %d. code len %d code %d - ", p, bits[p], codes[p]);
        init_get_bits(&gb, (uint8_t *)&codes[p], bits[p]);
        for (i = 0; i < bits[p]; ++i)
            av_em_log(NULL, AV_LOG_INFO, "%s", get_bits1(&gb) ? "1" : "0");
        av_em_log(NULL, AV_LOG_INFO, "\n");
...
int ff_vorbis_ready_floor1_list(AVEMCodecContext *avctx,
