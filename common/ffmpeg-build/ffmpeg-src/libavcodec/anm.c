                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (bytestream2_get_byte(&s->gb) != 0x42) {
        avpriv_em_request_sample(avctx, "Unknown record type");
        return AVERROR_INVALIDDATA;
    }
    if (bytestream2_get_byte(&s->gb)) {
        avpriv_em_request_sample(avctx, "Padding bytes");
        ...
        if (count) {
            ...
            if (!count) {
                if (type == 0)
                    break; // stop
                if (type == 2) {
                    avpriv_em_request_sample(avctx, "Unknown opcode");
                    return AVERROR_PATCHWELCOME;
                }
                continue;
            }
