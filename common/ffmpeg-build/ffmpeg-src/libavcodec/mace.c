...
static av_cold int mace_decode_init(AVEMCodecContext * avctx)
{
...
static int mace_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (buf_size % (avctx->channels << is_mace3)) {
        av_em_log(avctx, AV_LOG_ERROR, "buffer size %d is odd\n", buf_size);
