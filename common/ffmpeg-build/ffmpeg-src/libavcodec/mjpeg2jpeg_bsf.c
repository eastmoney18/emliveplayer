...
static int mjpeg2jpeg_filter(AVBSFContext *ctx, AVEMPacket *out)
{
    AVEMPacket *in;
    int ret = 0;
    ...
    if (in->size < 12) {
        av_em_log(ctx, AV_LOG_ERROR, "input is truncated\n");
        ret = AVERROR_INVALIDDATA;
        goto fail;
    }
    if (AV_RB16(in->data) != 0xffd8) {
        av_em_log(ctx, AV_LOG_ERROR, "input is not MJPEG\n");
        ret = AVERROR_INVALIDDATA;
        goto fail;
    }
    ...
    ret = av_em_new_packet(out, output_size);
    if (ret < 0)
        goto fail;
...
const AVBitStreamFilter em_mjpeg2jpeg_bsf = {
    .name           = "mjpeg2jpeg",
