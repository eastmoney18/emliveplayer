...
static uint8_t *create_new_buffer(const uint8_t *src, int size) {
    uint8_t *dst = av_em_alloc(size + AV_INPUT_BUFFER_PADDING_SIZE);
...
static int mpeg4_unpack_bframes_filter(AVBSFContext *ctx, AVEMPacket *out)
{
    ...
    av_em_log(ctx, AV_LOG_DEBUG, "Found %d VOP startcode(s) in this packet.\n", nb_vop);

    if (pos_vop2 >= 0) {
        if (s->b_frame_buf) {
            av_em_log(ctx, AV_LOG_WARNING,
                   "Missing one N-VOP packet, discarding one B-frame.\n");
            av_em_freep(&s->b_frame_buf);
    ...
    if (nb_vop == 1 && s->b_frame_buf) {
        ...
        av_em_packet_from_data(out, s->b_frame_buf, s->b_frame_buf_size);
        if (in->size <= MAX_NVOP_SIZE) {
            /* N-VOP */
            av_em_log(ctx, AV_LOG_DEBUG, "Skipping N-VOP.\n");
            s->b_frame_buf      = NULL;
            s->b_frame_buf_size = 0;
        } else {
        ...
        av_em_log(ctx, AV_LOG_DEBUG, "Updating DivX userdata (remove trailing 'p').\n");
        av_em_packet_move_ref(out, in);
...
const AVBitStreamFilter em_mpeg4_unpack_bframes_bsf = {
