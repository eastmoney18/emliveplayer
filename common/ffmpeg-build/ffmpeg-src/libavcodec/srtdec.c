...
static void srt_to_ass(AVEMCodecContext *avctx, AVEMBPrint *dst,
                       const char *in, int x1, int y1, int x2, int y2)
{
    if (x1 >= 0 && y1 >= 0) {
        ...
        if (x2 >= 0 && y2 >= 0 && (x2 != x1 || y2 != y1) && x2 >= x1 && y2 >= y1) {
            ...
            av_em_bprintf(dst, "{\\an5}{\\pos(%d,%d)}", scaled_x, scaled_y);
        } else {
            ...
            av_em_bprintf(dst, "{\\an1}{\\pos(%d,%d)}", scaled_x, scaled_y);
        }
    }

    em_htmlmarkup_to_ass(avctx, dst, in);
}

static int srt_decode_frame(AVEMCodecContext *avctx,
                            void *data, int *got_sub_ptr, AVEMPacket *avpkt)
{
    ...
    int size, ret;
    const uint8_t *p = av_em_packet_get_side_data(avpkt, AV_PKT_DATA_SUBTITLE_POSITION, &size);
    ...
    av_em_bprint_init(&buffer, 0, AV_BPRINT_SIZE_UNLIMITED);
