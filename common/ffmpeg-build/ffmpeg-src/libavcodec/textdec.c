...
static int text_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_sub_ptr, AVEMPacket *avpkt)
{
    int ret = 0;
    AVEMBPrint buf;
    AVSubtitle *sub = data;
...
static void text_flush(AVEMCodecContext *avctx)
{
    TextContext *text = avctx->priv_data;
...
#define DECLARE_CLASS(decname) static const AVEMClass decname ## _decoder_class = {   \
...
static int linebreak_init(AVEMCodecContext *avctx)
{
    TextContext *text = avctx->priv_data;
    text->linebreaks = "|";
...
AVEMCodec ff_subviewer1_decoder = {
    .name           = "subviewer1",
