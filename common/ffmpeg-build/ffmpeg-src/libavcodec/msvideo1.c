...
#define CHECK_STREAM_PTR(n) \
  if ((stream_ptr + n) > s->size ) { \
    av_em_log(s->avctx, AV_LOG_ERROR, " MS Video-1 warning: stream_ptr out of bounds (%d >= %d)\n", \
      stream_ptr + n, s->size); \
    return; \
  }
...
static av_cold int msvideo1_decode_init(AVEMCodecContext *avctx)
{
...
static int msvideo1_decode_frame(AVEMCodecContext *avctx,
...
static av_cold int msvideo1_decode_end(AVEMCodecContext *avctx)
{
    Msvideo1Context *s = avctx->priv_data;
