 ...
 */
int ff_mpeg1_find_frame_end(ParseContext *pc, const uint8_t *buf, int buf_size, AVEMCodecParserContext *s)
{
    int i;
...
*/
    ...
    for (i = 0; i < buf_size; i++) {
        ...
        if (pc->frame_start_found & 1) {
            ...
            i = avpriv_em_find_start_code(buf + i, buf + buf_size, &state) - buf - 1;
            ...
            if (pc->frame_start_found == 0 && s && state == PICTURE_START_CODE) {
                em_fetch_timestamp(s, i - 3, 1, i > 3);
            }
        }
    }
    pc->state = state;
