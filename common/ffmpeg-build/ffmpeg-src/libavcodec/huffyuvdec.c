...
static int read_len_table(uint8_t *dst, GetBitContext *gb, int n)
{
    ...
    for (i = 0; i < n;) {
        ...
        if (i + repeat > n || get_bits_left(gb) < 0) {
            av_em_log(NULL, AV_LOG_ERROR, "Error reading huffman table\n");
...
{
    int ret;
    uint16_t *symbols = av_em_mallocz(5 << VLC_BITS);
    uint16_t *bits;
    uint8_t *len;
    if (!symbols)
...
static int read_huffman_tables(HYuvContext *s, const uint8_t *src, int length)
{
    ...
    for (i = 0; i < count; i++) {
        if ((ret = read_len_table(s->len[i], &gb, s->vlc_n)) < 0)
            return ret;
        if ((ret = em_huffyuv_generate_bits_table(s->bits[i], s->len[i], s->vlc_n)) < 0)
            return ret;
        ff_free_vlc(&s->vlc[i]);
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (s->predictor == MEDIAN && avctx->pix_fmt == AV_PIX_FMT_YUV422P &&
        avctx->width % 4) {
        av_em_log(avctx, AV_LOG_ERROR, "width must be a multiple of 4 "
               "for this combination of colorspace and predictor type.\n");
        ret = AVERROR_INVALIDDATA;
        goto error;
    }

    if ((ret = em_huffyuv_alloc_temp(s)) < 0) {
        em_huffyuv_common_end(s);
        goto error;
    }
...
#if HAVE_THREADS
static av_cold int decode_init_thread_copy(AVEMCodecContext *avctx)
{
    HYuvContext *s = avctx->priv_data;
    int i, ret;
