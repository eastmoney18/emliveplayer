...
typedef struct {
    uint32_t type;
    size_t base_size;
    int (*decode)(const uint8_t *tsmb, MovTextContext *m, AVEMPacket *avpkt);
} Box;
...
static void mov_text_cleanup_ftab(MovTextContext *m)
{
    int i;
    if (m->ftab_temp)
        av_em_freep(&m->ftab_temp->font);
    av_em_freep(&m->ftab_temp);
    if (m->ftab) {
        for(i = 0; i < m->count_f; i++) {
            av_em_freep(&m->ftab[i]->font);
            av_em_freep(&m->ftab[i]);
        }
    }
    av_em_freep(&m->ftab);
}

static int mov_text_tx3g(AVEMCodecContext *avctx, MovTextContext *m)
{
    ...
    for (i = 0; i < m->ftab_entries; i++) {
        ...
        m->ftab_temp = av_em_mallocz(sizeof(*m->ftab_temp));
        if (!m->ftab_temp) {
            mov_text_cleanup_ftab(m);
        ...
        m->ftab_temp->font = av_em_alloc(font_length + 1);
        if (!m->ftab_temp->font) {
            mov_text_cleanup_ftab(m);
            return AVERROR(ENOMEM);
        }
        ...
        av_em_dynarray_add(&m->ftab, &m->count_f, m->ftab_temp);
        if (!m->ftab) {
            mov_text_cleanup_ftab(m);
            return AVERROR(ENOMEM);
        }
...
static int decode_twrp(const uint8_t *tsmb, MovTextContext *m, AVEMPacket *avpkt)
{
...
static int decode_hlit(const uint8_t *tsmb, MovTextContext *m, AVEMPacket *avpkt)
{
...
static int decode_hclr(const uint8_t *tsmb, MovTextContext *m, AVEMPacket *avpkt)
{
...
static int decode_styl(const uint8_t *tsmb, MovTextContext *m, AVEMPacket *avpkt)
{
    int i;
    ...
    for(i = 0; i < m->style_entries; i++) {
        m->s_temp = av_em_alloc(sizeof(*m->s_temp));
        if (!m->s_temp) {
            mov_text_cleanup(m);
            return AVERROR(ENOMEM);
        }
        ...
        av_em_dynarray_add(&m->s, &m->count_s, m->s_temp);
        if(!m->s) {
            mov_text_cleanup(m);
            return AVERROR(ENOMEM);
        }
...
static int text_to_ass(AVEMBPrint *buf, const char *text, const char *text_end,
                        MovTextContext *m)
{
    int i = 0;
    int j = 0;
    ...
    if (text < text_end && m->box_flags & TWRP_BOX) {
        if (m->w.wrap_flag == 1) {
            av_em_bprintf(buf, "{\\q1}"); /* End of line wrap */
        } else {
            av_em_bprintf(buf, "{\\q2}"); /* No wrap */
        }
    }
        ...
        if (m->box_flags & HLIT_BOX) {
            if (text_pos == m->h.hlit_start) {
                ...
                if (m->box_flags & HCLR_BOX) {
                    av_em_bprintf(buf, "{\\2c&H%02x%02x%02x&}", m->c.hlit_color[2],
                                m->c.hlit_color[1], m->c.hlit_color[0]);
                } else {
                    av_em_bprintf(buf, "{\\1c&H000000&}{\\2c&HFFFFFF&}");
                }
            }
            if (text_pos == m->h.hlit_end) {
                if (m->box_flags & HCLR_BOX) {
                    av_em_bprintf(buf, "{\\2c&H000000&}");
                } else {
                    av_em_bprintf(buf, "{\\1c&HFFFFFF&}{\\2c&H000000&}");
                }
            }
        }
...
static int mov_text_init(AVEMCodecContext *avctx) {
    /*
...
static int mov_text_decode_frame(AVEMCodecContext *avctx,
...
static int mov_text_decode_close(AVEMCodecContext *avctx)
{
    MovTextContext *m = avctx->priv_data;
    mov_text_cleanup_ftab(m);
    return 0;
...
static void mov_text_flush(AVEMCodecContext *avctx)
{
    MovTextContext *m = avctx->priv_data;
