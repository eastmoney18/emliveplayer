...
typedef struct DiracContext {
    ...
    struct {
        AVEMRational bytes;       /* average bytes per slice                   */
...
static int alloc_sequence_buffers(DiracContext *s)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        s->plane[i].idwt.buf_base = av_em_mallocz_array((w+max_xblen), h * (2 << s->pshift));
        s->plane[i].idwt.tmp      = av_em_malloc_array((w+16), 2 << s->pshift);
    ...
    s->sbsplit  = av_em_malloc_array(sbwidth, sbheight);
    s->blmotion = av_em_malloc_array(sbwidth, sbheight * 16 * sizeof(*s->blmotion));
...
static int alloc_buffers(DiracContext *s, int stride)
{
    ...
    av_em_freep(&s->edge_emu_buffer_base);
    memset(s->edge_emu_buffer, 0, sizeof(s->edge_emu_buffer));
    av_em_freep(&s->mctmp);
    av_em_freep(&s->mcscratch);

    s->edge_emu_buffer_base = av_em_malloc_array(stride, MAX_BLOCKSIZE);

    s->mctmp     = av_em_malloc_array((stride+MAX_BLOCKSIZE), (h+MAX_BLOCKSIZE) * sizeof(*s->mctmp));
    s->mcscratch = av_em_malloc_array(stride, MAX_BLOCKSIZE);
...
static void free_sequence_buffers(DiracContext *s)
{
    ...
    for (i = 0; i < MAX_FRAMES; i++) {
        if (s->all_frames[i].avframe->data[0]) {
            av_em_frame_unref(s->all_frames[i].avframe);
            memset(s->all_frames[i].interpolated, 0, sizeof(s->all_frames[i].interpolated));
        }

        for (j = 0; j < 3; j++)
            for (k = 1; k < 4; k++)
                av_em_freep(&s->all_frames[i].hpel_base[j][k]);
    }
    ...
    for (i = 0; i < 3; i++) {
        av_em_freep(&s->plane[i].idwt.buf_base);
        av_em_freep(&s->plane[i].idwt.tmp);
    }

    s->buffer_stride = 0;
    av_em_freep(&s->sbsplit);
    av_em_freep(&s->blmotion);
    av_em_freep(&s->edge_emu_buffer_base);
...
static av_cold int dirac_decode_init(AVEMCodecContext *avctx)
{
    ...
    for (i = 0; i < MAX_FRAMES; i++) {
        s->all_frames[i].avframe = av_em_frame_alloc();
        if (!s->all_frames[i].avframe) {
            while (i > 0)
                av_em_frame_free(&s->all_frames[--i].avframe);
            return AVERROR(ENOMEM);
        }
    }
...
static void dirac_decode_flush(AVEMCodecContext *avctx)
{
    DiracContext *s = avctx->priv_data;
...
static av_cold int dirac_decode_end(AVEMCodecContext *avctx)
{
    DiracContext *s = avctx->priv_data;
    int i;
                             ...
                             int blockcnt_one, int is_arith)
{
    ...
    if (s->codeblock_mode && !(s->old_delta_quant && blockcnt_one)) {
        ...
        if (quant < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid quant\n");
            return;
        }
        b->quant = quant;
    }
    ...
    if (b->quant > 115) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Unsupported quant %d\n", b->quant);
        b->quant = 0;
        return;
    }
...
static int decode_subband_arith(AVEMCodecContext *avctx, void *b)
{
    DiracContext *s = avctx->priv_data;
...
static int decode_subband_golomb(AVEMCodecContext *avctx, void *arg)
{
    DiracContext *s = avctx->priv_data;
    SubBand **b     = arg;
 ...
 */
static int decode_lowdelay_slice(AVEMCodecContext *avctx, void *arg)
{
    DiracContext *s = avctx->priv_data;
 ...
 */
static int decode_hq_slice(AVEMCodecContext *avctx, void *arg)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (bits_end >= INT_MAX) {
            av_em_log(s->avctx, AV_LOG_ERROR, "end too far away\n");
...
static int decode_lowdelay(DiracContext *s)
{
    ...
    slices = av_em_mallocz_array(s->num_x, s->num_y * sizeof(DiracSlice));
    if (!slices)
        return AVERROR(ENOMEM);
    ...
    if (s->hq_picture) {
        ...
        for (slice_y = 0; bufsize > 0 && slice_y < s->num_y; slice_y++) {
            for (slice_x = 0; bufsize > 0 && slice_x < s->num_x; slice_x++) {
                ...
                if (bytes >= INT_MAX) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "too many bytes\n");
                    av_em_free(slices);
                    return AVERROR_INVALIDDATA;
                }
...
static int dirac_unpack_prediction_parameters(DiracContext *s)
{
    ...
    if (idx > 4) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Block prediction index too high\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (!s->plane[0].xbsep || !s->plane[0].ybsep || s->plane[0].xbsep < s->plane[0].xblen/2 || s->plane[0].ybsep < s->plane[0].yblen/2) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Block separation too small\n");
        return AVERROR_INVALIDDATA;
    }
    if (s->plane[0].xbsep > s->plane[0].xblen || s->plane[0].ybsep > s->plane[0].yblen) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Block separation greater than size\n");
        return AVERROR_INVALIDDATA;
    }
    if (FFMAX(s->plane[0].xblen, s->plane[0].yblen) > MAX_BLOCKSIZE) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Unsupported large block size\n");
    ...
    if (s->mv_precision > 3) {
        av_em_log(s->avctx, AV_LOG_ERROR, "MV precision finer than eighth-pel\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (get_interleaved_ue_golomb(gb)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Unknown picture prediction mode\n");
        return AVERROR_INVALIDDATA;
    }
...
#define CHECKEDREAD(dst, cond, errmsg) \
    ...
    if (cond) { \
        av_em_log(s->avctx, AV_LOG_ERROR, errmsg); \
        return AVERROR_INVALIDDATA; \
    }\
    dst = tmp;
    ...
    if (!s->low_delay) {
        ...
        if (s->ld_picture) {
            ...
            if (s->lowdelay.bytes.den <= 0) {
                av_em_log(s->avctx,AV_LOG_ERROR,"Invalid lowdelay.bytes.den\n");
                return AVERROR_INVALIDDATA;
            }
            ...
            if (s->highquality.prefix_bytes >= INT_MAX / 8) {
                av_em_log(s->avctx,AV_LOG_ERROR,"too many prefix bytes\n");
                return AVERROR_INVALIDDATA;
            }
        }
        ...
        if (get_bits1(gb)) {
            av_em_log(s->avctx,AV_LOG_DEBUG,"Low Delay: Has Custom Quantization Matrix!\n");
            ...
            if (s->wavelet_depth > 4) {
                av_em_log(s->avctx,AV_LOG_ERROR,"Mandatory custom low delay matrix missing for depth %d\n", s->wavelet_depth);
                return AVERROR_INVALIDDATA;
            }
...
static int interpolate_refplane(DiracContext *s, DiracFrame *ref, int plane, int width, int height)
{
    ...
    for (i = 1; i < 4; i++) {
        if (!ref->hpel_base[plane][i])
            ref->hpel_base[plane][i] = av_em_alloc((height+2*edge) * ref->avframe->linesize[plane] + 32);
        if (!ref->hpel_base[plane][i]) {
            return AVERROR(ENOMEM);
        }
...
static int get_buffer_with_edge(AVEMCodecContext *avctx, AVFrame *f, int flags)
{
    int ret, i;
    int chroma_x_shift, chroma_y_shift;
    avcodec_em_get_chroma_sub_sample(avctx->pix_fmt, &chroma_x_shift, &chroma_y_shift);

    f->width  = avctx->width  + 2 * EDGE_WIDTH;
    f->height = avctx->height + 2 * EDGE_WIDTH + 2;
    ret = em_get_buffer(avctx, f, flags);
    if (ret < 0)
        return ret;
...
static int dirac_decode_picture_header(DiracContext *s)
{
    ...
    av_em_log(s->avctx,AV_LOG_DEBUG,"PICTURE_NUM: %d\n",picnum);
    ...
    for (i = 0; i < s->num_refs; i++) {
        ...
        if (!s->ref_pics[i] || refdist)
            av_em_log(s->avctx, AV_LOG_DEBUG, "Reference not found\n");
        ...
        if (!s->ref_pics[i]) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Reference could not be allocated\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    if (s->current_picture->reference) {
        ...
        if (retire != picnum) {
            ...
            if (retire_pic)
                ...
                av_em_log(s->avctx, AV_LOG_DEBUG, "Frame to retire not found\n");
        }

        /* if reference array is full, remove the oldest as per the spec */
        while (add_frame(s->ref_frames, MAX_REFERENCE_FRAMES, s->current_picture)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Reference frame overflow\n");
...
static int get_delayed_pic(DiracContext *s, AVFrame *picture, int *got_frame)
{
    ...
    if (out) {
        ...
        *got_frame = 1;
        if((ret = av_em_frame_ref(picture, out->avframe)) < 0)
            return ret;
    }
...
static int dirac_decode_data_unit(AVEMCodecContext *avctx, const uint8_t *buf, int size)
{
    DiracContext *s   = avctx->priv_data;
    DiracFrame *pic   = NULL;
    ...
    if (parse_code == DIRAC_PCODE_SEQ_HEADER) {
        ...
        ret = av_em_dirac_parse_sequence_header(&dsh, buf + DATA_UNIT_HEADER_SIZE, size - DATA_UNIT_HEADER_SIZE, avctx);
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "error parsing sequence header");
            return ret;
        }

        ret = em_set_dimensions(avctx, dsh->width, dsh->height);
        if (ret < 0) {
            av_em_freep(&dsh);
            return ret;
        }

        em_set_sar(avctx, dsh->sample_aspect_ratio);
        ...
        if (!s->seen_sequence_header) {
            av_em_log(avctx, AV_LOG_DEBUG, "Dropping frame without sequence header\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        for (i = 0; i < MAX_FRAMES; i++)
            if (s->all_frames[i].avframe->data[0] == NULL)
                pic = &s->all_frames[i];
        if (!pic) {
            av_em_log(avctx, AV_LOG_ERROR, "framelist full\n");
            return AVERROR_INVALIDDATA;
        }

        av_em_frame_unref(pic->avframe);

        /* [DIRAC_STD] Defined in 9.6.1 ... */
        tmp            =  parse_code & 0x03;                   /* [DIRAC_STD] num_refs()      */
        if (tmp > 2) {
            av_em_log(avctx, AV_LOG_ERROR, "num_refs of 3\n");
            return AVERROR_INVALIDDATA;
        }
        s->num_refs      = tmp;
        ...
        if (s->low_delay && !(s->ld_picture || s->hq_picture) ) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid low delay flag\n");
            return AVERROR_INVALIDDATA;
        }
...
static int dirac_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame, AVEMPacket *pkt)
{
    DiracContext *s     = avctx->priv_data;
    ...
    *got_frame = 0;
    ...
    for (;;) {
        ...
        if (ret < 0)
        {
            av_em_log(s->avctx, AV_LOG_ERROR,"Error in dirac_decode_data_unit\n");
            return ret;
        }
    ...
    if (s->current_picture->avframe->display_picture_number > s->frame_number) {
        ...
        if (add_frame(s->delay_frames, MAX_DELAY, s->current_picture)) {
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Delay frame overflow\n");
        ...
        if (delayed_frame) {
            ...
            if((ret=av_em_frame_ref(data, delayed_frame->avframe)) < 0)
                return ret;
            *got_frame = 1;
        }
    } else if (s->current_picture->avframe->display_picture_number == s->frame_number) {
        /* The right frame at the right time :-) */
        if((ret=av_em_frame_ref(data, s->current_picture->avframe)) < 0)
            return ret;
        *got_frame = 1;
    }
