...
{
    em_blockdsp_init(&s->bdsp, s->avctx);
    em_h264chroma_init(&s->h264chroma, 8); //for lowres
    em_hpeldsp_init(&s->hdsp, s->avctx->flags);
...
{
    return em_alloc_picture(s->avctx, pic, &s->me, &s->sc, shared, 0,
...
static void free_duplicate_context(MpegEncContext *s)
{
    ...
    av_em_freep(&s->sc.edge_emu_buffer);
    av_em_freep(&s->me.scratchpad);
    s->me.temp =
    s->sc.rd_scratchpad =
    s->sc.b_scratchpad =
    s->sc.obmc_scratchpad = NULL;

    av_em_freep(&s->dct_error_sum);
    av_em_freep(&s->me.map);
    av_em_freep(&s->me.score_map);
    av_em_freep(&s->blocks);
    av_em_freep(&s->ac_val_base);
    s->block = NULL;
}
...
int ff_update_duplicate_context(MpegEncContext *dst, MpegEncContext *src)
{
    ...
    if (!dst->sc.edge_emu_buffer &&
        ...
        av_em_log(dst->avctx, AV_LOG_ERROR, "failed to allocate context "
               "scratch buffers.\n");
        return ret;
    }
...
int ff_mpeg_update_thread_context(AVEMCodecContext *dst,
...
#define REBASE_PICTURE(pic, new_ctx, old_ctx)                                 \
    ...
    if (s1->bitstream_buffer) {
        if (s1->bitstream_buffer_size +
            ...
            av_em_fast_malloc(&s->bitstream_buffer,
    ...
    if (!s->sc.edge_emu_buffer)
        if (s1->linesize) {
            if (ff_mpeg_framesize_alloc(s->avctx, &s->me,
                                        &s->sc, s1->linesize) < 0) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Failed to allocate context "
                       "scratch buffers.\n");
                return AVERROR(ENOMEM);
            }
        } else {
            av_em_log(s->avctx, AV_LOG_ERROR, "Context scratch buffers could not "
...
void ff_mpv_decode_init(MpegEncContext *s, AVEMCodecContext *avctx)
{
    s->avctx           = avctx;
    ...
    s->codec_tag          = avpriv_em_toupper4(avctx->codec_tag);
}
...
av_cold int ff_mpv_common_init(MpegEncContext *s)
{
    ...
    if (nb_slices > MAX_THREADS || (nb_slices > s->mb_height && s->mb_height)) {
        ...
        av_em_log(s->avctx, AV_LOG_WARNING, "too many threads/slices (%d),"
               " reducing to %d\n", nb_slices, max_slices);
        nb_slices = max_slices;
    }

    if ((s->width || s->height) &&
        av_em_image_check_size(s->width, s->height, 0, s->avctx))
        return -1;

    dct_init(s);

    /* set chroma shifts */
    avcodec_em_get_chroma_sub_sample(s->avctx->pix_fmt,
                                  &s->chroma_x_shift,
                                  &s->chroma_y_shift);
    ...
    for (i = 0; i < MAX_PICTURE_COUNT; i++) {
        s->picture[i].f = av_em_frame_alloc();
        if (!s->picture[i].f)
            goto fail;
    }
    s->next_picture.f = av_em_frame_alloc();
    if (!s->next_picture.f)
        goto fail;
    s->last_picture.f = av_em_frame_alloc();
    if (!s->last_picture.f)
        goto fail;
    s->current_picture.f = av_em_frame_alloc();
    if (!s->current_picture.f)
        goto fail;
    s->new_picture.f = av_em_frame_alloc();
    if (!s->new_picture.f)
        goto fail;
...
//     if (s->width && s->height) {
        if (nb_slices > 1) {
            for (i = 0; i < nb_slices; i++) {
                if (i) {
                    s->thread_context[i] = av_em_memdup(s, sizeof(MpegEncContext));
...
static void free_context_frame(MpegEncContext *s)
{
    ...
    av_em_freep(&s->mb_type);
    av_em_freep(&s->p_mv_table_base);
    av_em_freep(&s->b_forw_mv_table_base);
    av_em_freep(&s->b_back_mv_table_base);
    av_em_freep(&s->b_bidir_forw_mv_table_base);
    av_em_freep(&s->b_bidir_back_mv_table_base);
    av_em_freep(&s->b_direct_mv_table_base);
    s->p_mv_table            = NULL;
    s->b_forw_mv_table       = NULL;
    s->b_back_mv_table       = NULL;
    s->b_bidir_forw_mv_table = NULL;
    ...
    for (i = 0; i < 2; i++) {
        for (j = 0; j < 2; j++) {
            for (k = 0; k < 2; k++) {
                av_em_freep(&s->b_field_mv_table_base[i][j][k]);
                s->b_field_mv_table[i][j][k] = NULL;
            }
            av_em_freep(&s->b_field_select_table[i][j]);
            av_em_freep(&s->p_field_mv_table_base[i][j]);
            s->p_field_mv_table[i][j] = NULL;
        }
        av_em_freep(&s->p_field_select_table[i]);
    }

    av_em_freep(&s->dc_val_base);
    av_em_freep(&s->coded_block_base);
    av_em_freep(&s->mbintra_table);
    av_em_freep(&s->cbp_table);
    av_em_freep(&s->pred_dir_table);

    av_em_freep(&s->mbskip_table);

    av_em_freep(&s->er.error_status_table);
    av_em_freep(&s->er.er_temp_buffer);
    av_em_freep(&s->mb_index2xy);
    av_em_freep(&s->lambda_table);

    av_em_freep(&s->cplx_tab);
    av_em_freep(&s->bits_tab);
...
int ff_mpv_common_frame_size_change(MpegEncContext *s)
{
    ...
    if (s->slice_context_count > 1) {
        ...
        for (i = 1; i < s->slice_context_count; i++) {
            av_em_freep(&s->thread_context[i]);
        }
    } else
    ...
    if ((s->width || s->height) &&
        (err = av_em_image_check_size(s->width, s->height, 0, s->avctx)) < 0)
        goto fail;
...
void ff_mpv_common_end(MpegEncContext *s)
{
    ...
    av_em_freep(&s->parse_context.buffer);
    s->parse_context.buffer_size = 0;

    av_em_freep(&s->bitstream_buffer);
    s->allocated_bitstream_buffer_size = 0;
    ...
    if (s->picture) {
        for (i = 0; i < MAX_PICTURE_COUNT; i++) {
            ...
            ff_mpeg_unref_picture(s->avctx, &s->picture[i]);
            av_em_frame_free(&s->picture[i].f);
        }
    }
    av_em_freep(&s->picture);
    ff_free_picture_tables(&s->last_picture);
    ff_mpeg_unref_picture(s->avctx, &s->last_picture);
    av_em_frame_free(&s->last_picture.f);
    ff_free_picture_tables(&s->current_picture);
    ff_mpeg_unref_picture(s->avctx, &s->current_picture);
    av_em_frame_free(&s->current_picture.f);
    ff_free_picture_tables(&s->next_picture);
    ff_mpeg_unref_picture(s->avctx, &s->next_picture);
    av_em_frame_free(&s->next_picture.f);
    ff_free_picture_tables(&s->new_picture);
    ff_mpeg_unref_picture(s->avctx, &s->new_picture);
    av_em_frame_free(&s->new_picture.f);
...
static void gray_frame(AVFrame *frame)
{
    ...
    av_em_pix_fmt_get_chroma_sub_sample(frame->format, &h_chroma_shift, &v_chroma_shift);
 ...
 */
int ff_mpv_frame_start(MpegEncContext *s, AVEMCodecContext *avctx)
{
    int i, ret;
    Picture *pic;
    s->mb_skipped = 0;

    if (!em_thread_can_start_frame(avctx)) {
        av_em_log(avctx, AV_LOG_ERROR, "Attempt to start a frame outside SETUP state\n");
        return -1;
    }
    ...
    if (s->current_picture_ptr && !s->current_picture_ptr->f->buf[0]) {
        ...
        if (i < 0) {
            av_em_log(s->avctx, AV_LOG_ERROR, "no frame buffer available\n");
            return i;
        }
        pic = &s->picture[i];
    }
    ...
    if ((!s->last_picture_ptr || !s->last_picture_ptr->f->buf[0]) &&
        ...
        em_thread_report_progress(&s->last_picture_ptr->tf, INT_MAX, 0);
        em_thread_report_progress(&s->last_picture_ptr->tf, INT_MAX, 1);
    }
    if ((!s->next_picture_ptr || !s->next_picture_ptr->f->buf[0]) &&
        ...
        em_thread_report_progress(&s->next_picture_ptr->tf, INT_MAX, 0);
        em_thread_report_progress(&s->next_picture_ptr->tf, INT_MAX, 1);
    }
...
void ff_mpv_frame_end(MpegEncContext *s)
{
    ...
    if (s->current_picture.reference)
        em_thread_report_progress(&s->current_picture_ptr->tf, INT_MAX, 0);
}
 ...
 */
void ff_print_debug_info2(AVEMCodecContext *avctx, AVFrame *pict, uint8_t *mbskip_table,
                         ...
                         int mb_width, int mb_height, int mb_stride, int quarter_sample)
{
    if ((avctx->flags2 & AV_CODEC_FLAG2_EXPORT_MVS) && mbtype_table && motion_val[0]) {
        ...
        AVMotionVector *mvs = av_em_malloc_array(mb_width * mb_height, 2 * 4 * sizeof(AVMotionVector));
        if (!mvs)
            return;
        ...
        if (mbcount) {
            ...
            av_em_log(avctx, AV_LOG_DEBUG, "Adding %d MVs info to frame %d\n", mbcount, avctx->frame_number);
            sd = av_em_frame_new_side_data(pict, AV_FRAME_DATA_MOTION_VECTORS, mbcount * sizeof(AVMotionVector));
            if (!sd) {
                av_em_freep(&mvs);
                return;
            }
    ...
    if (avctx->debug & (FF_DEBUG_SKIP | FF_DEBUG_QP | FF_DEBUG_MB_TYPE)) {
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "New frame, type: %c\n",
               av_em_get_picture_type_char(pict->pict_type));
        for (y = 0; y < mb_height; y++) {
            for (x = 0; x < mb_width; x++) {
                if (avctx->debug & FF_DEBUG_SKIP) {
                    ...
                    if (count > 9)
                        count = 9;
                    av_em_log(avctx, AV_LOG_DEBUG, "%1d", count);
                }
                if (avctx->debug & FF_DEBUG_QP) {
                    av_em_log(avctx, AV_LOG_DEBUG, "%2d",
                ...
                if (avctx->debug & FF_DEBUG_MB_TYPE) {
                    ...
                    // Type & MV direction
                    if (IS_PCM(mb_type))
                        av_em_log(avctx, AV_LOG_DEBUG, "P");
                    else if (IS_INTRA(mb_type) && IS_ACPRED(mb_type))
                        av_em_log(avctx, AV_LOG_DEBUG, "A");
                    else if (IS_INTRA4x4(mb_type))
                        av_em_log(avctx, AV_LOG_DEBUG, "i");
                    else if (IS_INTRA16x16(mb_type))
                        av_em_log(avctx, AV_LOG_DEBUG, "I");
                    else if (IS_DIRECT(mb_type) && IS_SKIP(mb_type))
                        av_em_log(avctx, AV_LOG_DEBUG, "d");
                    else if (IS_DIRECT(mb_type))
                        av_em_log(avctx, AV_LOG_DEBUG, "D");
                    else if (IS_GMC(mb_type) && IS_SKIP(mb_type))
                        av_em_log(avctx, AV_LOG_DEBUG, "g");
                    else if (IS_GMC(mb_type))
                        av_em_log(avctx, AV_LOG_DEBUG, "G");
                    else if (IS_SKIP(mb_type))
                        av_em_log(avctx, AV_LOG_DEBUG, "S");
                        ...
                        av_em_log(avctx, AV_LOG_DEBUG, "X");
                    }
    ...
    if ((avctx->debug & (FF_DEBUG_VIS_QP | FF_DEBUG_VIS_MB_TYPE)) ||
        ...
        avcodec_em_get_chroma_sub_sample(avctx->pix_fmt, &h_chroma_shift, &v_chroma_shift);

        av_em_frame_make_writable(pict);
...
{
    AVEMBufferRef *ref = av_em_buffer_ref(p->qscale_table_buf);
    ...
    ref->size -= offset;
    ref->data += offset;
    return av_em_frame_set_qp_table(f, ref, s->mb_stride, qp_type);
}
                                            ...
                                            int mx, int my)
{
    ...
    mx = em_h263_round_chroma(mx);
    my = em_h263_round_chroma(my);
                            ...
                            int lowres_flag, int is_mpeg12)
{
    ...
    if(s->avctx->debug&FF_DEBUG_DCT_COEFF) {
       ...
       int i,j;
       av_em_log(s->avctx, AV_LOG_DEBUG, "DCT coeffs of MB at %dx%d:\n", s->mb_x, s->mb_y);
       for(i=0; i<6; i++){
           for(j=0; j<64; j++){
               av_em_log(s->avctx, AV_LOG_DEBUG, "%5d",
            ...
            if(!s->encoding){
                ...
                if(HAVE_THREADS && s->avctx->active_thread_type&FF_THREAD_FRAME) {
                    if (s->mv_dir & MV_DIR_FORWARD) {
                        em_thread_await_progress(&s->last_picture_ptr->tf,
...
void ff_mpeg_flush(AVEMCodecContext *avctx){
    int i;
...
void ff_mpv_report_decode_progress(MpegEncContext *s)
{
    if (s->pict_type != AV_PICTURE_TYPE_B && !s->partitioned_frame && !s->er.error_occurred)
        em_thread_report_progress(&s->current_picture_ptr->tf, s->mb_y, 0);
}
