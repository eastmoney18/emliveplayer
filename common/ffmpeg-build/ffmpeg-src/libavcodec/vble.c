...
static int vble_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                             AVEMPacket *avpkt)
{
    ...
    if (version != 1)
        av_em_log(avctx, AV_LOG_WARNING, "Unsupported VBLE Version: %d\n", version);

    init_get_bits(&gb, src + 4, (avpkt->size - 4) * 8);

    /* Unpack */
    if (vble_unpack(ctx, &gb) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid Code\n");
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int vble_decode_close(AVEMCodecContext *avctx)
{
    VBLEContext *ctx = avctx->priv_data;
    av_em_freep(&ctx->val);
...
static av_cold int vble_decode_init(AVEMCodecContext *avctx)
{
    VBLEContext *ctx = avctx->priv_data;

    /* Stash for later use */
    ctx->avctx = avctx;
    em_huffyuvdsp_init(&ctx->hdsp);

    avctx->pix_fmt = AV_PIX_FMT_YUV420P;
    avctx->bits_per_raw_sample = 8;

    ctx->size = av_em_image_get_buffer_size(avctx->pix_fmt,
                                         avctx->width, avctx->height, 1);

    ctx->val = av_em_malloc_array(ctx->size, sizeof(*ctx->val));

    if (!ctx->val) {
        av_em_log(avctx, AV_LOG_ERROR, "Could not allocate values buffer.\n");
