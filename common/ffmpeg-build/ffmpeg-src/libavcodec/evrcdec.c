 ...
 */
static evrc_packet_rate determine_bitrate(AVEMCodecContext *avctx,
                                          int *buf_size,
                                          const uint8_t **buf)
{
    evrc_packet_rate bitrate;
...
static void warn_insufficient_frame_quality(AVEMCodecContext *avctx,
                                            const char *message)
{
    av_em_log(avctx, AV_LOG_WARNING, "Frame #%d, %s\n",
 ...
 * TIA/IS-127 5.2
 */
static av_cold int evrc_decode_init(AVEMCodecContext *avctx)
{
    EVRCContext *e = avctx->priv_data;
    int i, n, idx = 0;
...
static int evrc_decode_frame(AVEMCodecContext *avctx, void *data,
...
erasure:
        error_flag = 1;
        av_em_log(avctx, AV_LOG_WARNING, "frame erasure\n");
...
static const AVEMClass evrcdec_class = {
    .class_name = "evrc",
