 ...
 * @param destptr must be padded sufficiently for av_em_memcpy_backptr
 */
static unsigned int mszh_decomp(const unsigned char * srcptr, int srclen, unsigned char * destptr, unsigned int destsize)
{
    ...
    while (srcptr < srcptr_end && destptr < destptr_end) {
        if (!(mask & maskbit)) {
            ...
            if (ofs) {
                av_em_memcpy_backptr(destptr, ofs, cnt);
            } else {
 ...
 */
static int zlib_decomp(AVEMCodecContext *avctx, const uint8_t *src, int src_len, int offset, int expected)
{
    LclDecContext *c = avctx->priv_data;
    int zret = inflateReset(&c->zstream);
    if (zret != Z_OK) {
        av_em_log(avctx, AV_LOG_ERROR, "Inflate reset error: %d\n", zret);
    ...
    if (zret != Z_OK && zret != Z_STREAM_END) {
        av_em_log(avctx, AV_LOG_ERROR, "Inflate error: %d\n", zret);
        return AVERROR_UNKNOWN;
    }
    if (expected != (unsigned int)c->zstream.total_out) {
        av_em_log(avctx, AV_LOG_ERROR, "Decoded size differs (%d != %lu)\n",
...
static int decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame, AVEMPacket *avpkt)
{
    ...
    case AV_CODEC_ID_MSZH:
        ...
        case COMP_MSZH:
            if (c->imgtype == IMGTYPE_RGB24 && len == FFALIGN(width * 3, 4) * height ||
                ...
                if (len < 8) {
                    av_em_log(avctx, AV_LOG_ERROR, "len %d is too small\n", len);
                ...
                if (mthread_outlen != mszh_dlen) {
                    av_em_log(avctx, AV_LOG_ERROR, "Mthread1 decoded size differs (%d != %d)\n",
                ...
                if (mthread_outlen != mszh_dlen) {
                    av_em_log(avctx, AV_LOG_ERROR, "Mthread2 decoded size differs (%d != %d)\n",
                           mthread_outlen, mszh_dlen);
                    return AVERROR_INVALIDDATA;
                }
                ...
                if (c->decomp_size != mszh_dlen) {
                    av_em_log(avctx, AV_LOG_ERROR, "Decoded size differs (%d != %d)\n",
        ...
        default:
            av_em_log(avctx, AV_LOG_ERROR, "BUG! Unknown MSZH compression in frame decoder.\n");
            return AVERROR_INVALIDDATA;
        }
        break;
#if CONFIG_ZLIB_DECODER
...
#endif
    default:
        av_em_log(avctx, AV_LOG_ERROR, "BUG! Unknown codec in frame decoder compression switch.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (avctx->codec_id == AV_CODEC_ID_ZLIB && (c->flags & FLAG_PNGFILTER)) {
        ...
        case IMGTYPE_YUV420:
            ...
            break;
        default:
            av_em_log(avctx, AV_LOG_ERROR, "BUG! Unknown imagetype in pngfilter switch.\n");
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    case IMGTYPE_YUV420:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "BUG! Unknown imagetype in image decoder.\n");
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->extradata_size < 8) {
        av_em_log(avctx, AV_LOG_ERROR, "Extradata size too small.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if ((avctx->codec_id == AV_CODEC_ID_MSZH  && avctx->extradata[7] != CODEC_MSZH) ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Codec id and codec type mismatch. This should not happen.\n");
    }
    ...
    case IMGTYPE_YUV111:
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Image type is YUV 1:1:1.\n");
        break;
    case IMGTYPE_YUV422:
        c->decomp_size = basesize * 2;
        max_decomp_size = max_basesize * 2;
        avctx->pix_fmt = AV_PIX_FMT_YUV422P;
        av_em_log(avctx, AV_LOG_DEBUG, "Image type is YUV 4:2:2.\n");
        if (avctx->width % 4) {
            avpriv_em_request_sample(avctx, "Unsupported dimensions");
            return AVERROR_INVALIDDATA;
        }
        break;
    case IMGTYPE_RGB24:
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Image type is RGB 24.\n");
        break;
    case IMGTYPE_YUV411:
        c->decomp_size = basesize / 2 * 3;
        max_decomp_size = max_basesize / 2 * 3;
        avctx->pix_fmt = AV_PIX_FMT_YUV411P;
        av_em_log(avctx, AV_LOG_DEBUG, "Image type is YUV 4:1:1.\n");
        break;
    case IMGTYPE_YUV211:
        c->decomp_size = basesize * 2;
        max_decomp_size = max_basesize * 2;
        avctx->pix_fmt = AV_PIX_FMT_YUV422P;
        av_em_log(avctx, AV_LOG_DEBUG, "Image type is YUV 2:1:1.\n");
        break;
    case IMGTYPE_YUV420:
        c->decomp_size = basesize / 2 * 3;
        max_decomp_size = max_basesize / 2 * 3;
        avctx->pix_fmt = AV_PIX_FMT_YUV420P;
        av_em_log(avctx, AV_LOG_DEBUG, "Image type is YUV 4:2:0.\n");
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported image format %d.\n", c->imgtype);
        return AVERROR_INVALIDDATA;
    }

    av_em_pix_fmt_get_chroma_sub_sample(avctx->pix_fmt, &subsample_h, &subsample_v);
    ...
    case AV_CODEC_ID_MSZH:
        switch (c->compression) {
        case COMP_MSZH:
            av_em_log(avctx, AV_LOG_DEBUG, "Compression enabled.\n");
            break;
        case COMP_MSZH_NOCOMP:
            c->decomp_size = 0;
            av_em_log(avctx, AV_LOG_DEBUG, "No compression.\n");
            break;
        default:
            av_em_log(avctx, AV_LOG_ERROR, "Unsupported compression format for MSZH (%d).\n", c->compression);
            return AVERROR_INVALIDDATA;
        }
        break;
#if CONFIG_ZLIB_DECODER
    case AV_CODEC_ID_ZLIB:
        ...
        case COMP_ZLIB_HISPEED:
            av_em_log(avctx, AV_LOG_DEBUG, "High speed compression.\n");
            break;
        case COMP_ZLIB_HICOMP:
            av_em_log(avctx, AV_LOG_DEBUG, "High compression.\n");
            break;
        case COMP_ZLIB_NORMAL:
            av_em_log(avctx, AV_LOG_DEBUG, "Normal compression.\n");
            break;
        default:
            if (c->compression < Z_NO_COMPRESSION || c->compression > Z_BEST_COMPRESSION) {
                av_em_log(avctx, AV_LOG_ERROR, "Unsupported compression level for ZLIB: (%d).\n", c->compression);
                return AVERROR_INVALIDDATA;
            }
            av_em_log(avctx, AV_LOG_DEBUG, "Compression level for ZLIB: (%d).\n", c->compression);
        }
        break;
#endif
    default:
        av_em_log(avctx, AV_LOG_ERROR, "BUG! Unknown codec in compression switch.\n");
        return AVERROR_INVALIDDATA;
    }

    /* Allocate decompression buffer */
    if (c->decomp_size) {
        if (!(c->decomp_buf = av_em_alloc(max_decomp_size))) {
            av_em_log(avctx, AV_LOG_ERROR, "Can't allocate decompression buffer.\n");
    ...
    if (c->flags & FLAG_MULTITHREAD)
        av_em_log(avctx, AV_LOG_DEBUG, "Multithread encoder flag set.\n");
    if (c->flags & FLAG_NULLFRAME)
        av_em_log(avctx, AV_LOG_DEBUG, "Nullframe insertion flag set.\n");
    if (avctx->codec_id == AV_CODEC_ID_ZLIB && (c->flags & FLAG_PNGFILTER))
        av_em_log(avctx, AV_LOG_DEBUG, "PNG filter flag set.\n");
    if (c->flags & FLAGMASK_UNUSED)
        av_em_log(avctx, AV_LOG_ERROR, "Unknown flag set (%d).\n", c->flags);
    ...
    if (avctx->codec_id == AV_CODEC_ID_ZLIB) {
        ...
        if (zret != Z_OK) {
            av_em_log(avctx, AV_LOG_ERROR, "Inflate init error: %d\n", zret);
            av_em_freep(&c->decomp_buf);
            return AVERROR_UNKNOWN;
        }
    }
#endif
