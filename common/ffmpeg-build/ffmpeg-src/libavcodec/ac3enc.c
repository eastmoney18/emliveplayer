...
{
    const AVCRC *crc_ctx = av_em_crc_get_table(AV_CRC_16_ANSI);
    ...
    if (s->eac3) {
        /* compute crc2 */
        crc2_partial = av_em_crc(crc_ctx, 0, frame + 2, s->frame_size - 5);
    } else {
    /* compute crc1 */
    /* this is not so easy because it is at the beginning of the data... */
    crc1    = av_bswap16(av_em_crc(crc_ctx, 0, frame + 4, frame_size_58 - 4));
    crc_inv = s->crc_inv[s->frame_size > s->frame_size_min];
    crc1    = mul_poly(crc_inv, crc1, CRC16_POLY);
    AV_WB16(frame + 2, crc1);

    /* compute crc2 */
    crc2_partial = av_em_crc(crc_ctx, 0, frame + frame_size_58,
                          s->frame_size - frame_size_58 - 3);
    }
    crc2 = av_em_crc(crc_ctx, crc2_partial, frame + s->frame_size - 3, 1);
...
static void dprint_options(AC3EncodeContext *s)
{
    ...
    case  6:  av_em_strlcpy(strbuf, "AC-3 (alt syntax)",       32); break;
    case  8:  av_em_strlcpy(strbuf, "AC-3 (standard)",         32); break;
    case  9:  av_em_strlcpy(strbuf, "AC-3 (dnet half-rate)",   32); break;
    case 10:  av_em_strlcpy(strbuf, "AC-3 (dnet quater-rate)", 32); break;
    case 16:  av_em_strlcpy(strbuf, "E-AC-3 (enhanced)",       32); break;
    default: snprintf(strbuf, 32, "ERROR");
    }
    ff_dlog(avctx, "bitstream_id: %s (%d)\n", strbuf, s->bitstream_id);
    ff_dlog(avctx, "sample_fmt: %s\n", av_em_get_sample_fmt_name(avctx->sample_fmt));
    av_em_get_channel_layout_string(strbuf, 32, s->channels, avctx->channel_layout);
    ...
    if (opt->audio_production_info) {
        ...
        case AC3ENC_OPT_NOT_INDICATED: av_em_strlcpy(strbuf, "notindicated", 32); break;
        case AC3ENC_OPT_LARGE_ROOM:    av_em_strlcpy(strbuf, "large", 32);        break;
        case AC3ENC_OPT_SMALL_ROOM:    av_em_strlcpy(strbuf, "small", 32);        break;
    ...
    if (s->channel_mode == AC3_CHMODE_STEREO) {
        ...
        case AC3ENC_OPT_NOT_INDICATED: av_em_strlcpy(strbuf, "notindicated", 32); break;
        case AC3ENC_OPT_MODE_ON:       av_em_strlcpy(strbuf, "on", 32);           break;
        case AC3ENC_OPT_MODE_OFF:      av_em_strlcpy(strbuf, "off", 32);          break;
    ...
    if (s->bitstream_id == 6) {
        if (opt->extended_bsi_1) {
            ...
            case AC3ENC_OPT_NOT_INDICATED: av_em_strlcpy(strbuf, "notindicated", 32); break;
            case AC3ENC_OPT_DOWNMIX_LTRT:  av_em_strlcpy(strbuf, "ltrt", 32);         break;
            case AC3ENC_OPT_DOWNMIX_LORO:  av_em_strlcpy(strbuf, "loro", 32);         break;
        ...
        if (opt->extended_bsi_2) {
            ...
            case AC3ENC_OPT_ADCONV_STANDARD: av_em_strlcpy(strbuf, "standard", 32); break;
            case AC3ENC_OPT_ADCONV_HDCD:     av_em_strlcpy(strbuf, "hdcd", 32);     break;
                               ...
                               int *ctx_param)
{
    ...
    if (mixlev < min_value) {
        ...
        if (*opt_param >= 0.0) {
            av_em_log(log_ctx, AV_LOG_WARNING, "requested %s is not valid. using "
...
int ff_ac3_validate_metadata(AC3EncodeContext *s)
{
    ...
    if ((avctx->audio_service_type == AV_AUDIO_SERVICE_TYPE_KARAOKE &&
        ...
        ((avctx->audio_service_type == AV_AUDIO_SERVICE_TYPE_COMMENTARY ||
         ...
         && avctx->channels > 1)) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid audio service type for the "
    ...
    if (opt->audio_production_info) {
        if (opt->mixing_level == AC3ENC_OPT_NONE) {
            av_em_log(avctx, AV_LOG_ERROR, "mixing_level must be set if "
                   "room_type is set\n");
            return AVERROR(EINVAL);
        }
        if (opt->mixing_level < 80) {
            av_em_log(avctx, AV_LOG_ERROR, "invalid mixing level. must be between "
                   "80dB and 111dB\n");
            return AVERROR(EINVAL);
        }
        /* default room type */
    ...
    if (!s->eac3 && (opt->extended_bsi_1 || opt->extended_bsi_2)) {
        if (s->bitstream_id > 8 && s->bitstream_id < 11) {
            ...
            if (warn_once) {
                av_em_log(avctx, AV_LOG_WARNING, "alternate bitstream syntax is "
 ...
 */
av_cold int ff_ac3_encode_close(AVEMCodecContext *avctx)
{
    int blk, ch;
    AC3EncodeContext *s = avctx->priv_data;

    av_em_freep(&s->windowed_samples);
    if (s->planar_samples)
    for (ch = 0; ch < s->channels; ch++)
        av_em_freep(&s->planar_samples[ch]);
    av_em_freep(&s->planar_samples);
    av_em_freep(&s->bap_buffer);
    av_em_freep(&s->bap1_buffer);
    av_em_freep(&s->mdct_coef_buffer);
    av_em_freep(&s->fixed_coef_buffer);
    av_em_freep(&s->exp_buffer);
    av_em_freep(&s->grouped_exp_buffer);
    av_em_freep(&s->psd_buffer);
    av_em_freep(&s->band_psd_buffer);
    av_em_freep(&s->mask_buffer);
    av_em_freep(&s->qmant_buffer);
    av_em_freep(&s->cpl_coord_exp_buffer);
    av_em_freep(&s->cpl_coord_mant_buffer);
    av_em_freep(&s->fdsp);
    for (blk = 0; blk < s->num_blocks; blk++) {
        AC3Block *block = &s->blocks[blk];
        av_em_freep(&block->mdct_coef);
        av_em_freep(&block->fixed_coef);
        av_em_freep(&block->exp);
        av_em_freep(&block->grouped_exp);
        av_em_freep(&block->psd);
        av_em_freep(&block->band_psd);
        av_em_freep(&block->mask);
        av_em_freep(&block->qmant);
        av_em_freep(&block->cpl_coord_exp);
        av_em_freep(&block->cpl_coord_mant);
    }
                                    ...
                                    uint64_t *channel_layout)
{
    ...
    if (!ch_layout)
        ch_layout = av_em_get_default_channel_layout(channels);
...
static av_cold int validate_options(AC3EncodeContext *s)
{
    ...
    if (!avctx->channel_layout) {
        av_em_log(avctx, AV_LOG_WARNING, "No channel layout specified. The "
                                      "encoder will guess the layout, but it "
                                      "might be incorrect.\n");
    }
    ret = set_channel_info(s, avctx->channels, &avctx->channel_layout);
    if (ret) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid channel layout\n");
        return ret;
    }
    ...
    if (s->eac3) {
        ...
        if (avctx->bit_rate < min_br || avctx->bit_rate > max_br) {
            av_em_log(avctx, AV_LOG_ERROR, "invalid bit rate. must be %d to %d "
    ...
    /* validate cutoff */
    if (avctx->cutoff < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid cutoff frequency\n");
        return AVERROR(EINVAL);
    }
...
av_cold int ff_ac3_encode_init(AVEMCodecContext *avctx)
{
    AC3EncodeContext *s = avctx->priv_data;
