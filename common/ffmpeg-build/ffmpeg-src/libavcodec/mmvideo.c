...
static av_cold int mm_decode_init(AVEMCodecContext *avctx)
{
...
static int mm_decode_frame(AVEMCodecContext *avctx,
                            ...
                            AVEMPacket *avpkt)
{
    ...
    if ((res = em_reget_buffer(avctx, s->frame)) < 0)
        return res;
    ...
    if ((res = av_em_frame_ref(data, s->frame)) < 0)
        return res;
...
static av_cold int mm_decode_end(AVEMCodecContext *avctx)
{
    MmContext *s = avctx->priv_data;
