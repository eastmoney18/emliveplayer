...
static int qdm2_get_vlc(GetBitContext *gb, const VLC *vlc, int flag, int depth)
{
    ...
    if (flag) {
        ...
        if (value >= 60) {
            av_em_log(NULL, AV_LOG_ERROR, "value %d in qdm2_get_vlc too large\n", value);
            return 0;
        }
                                          ...
                                          QDM2SubPacket *sub_packet)
{
    ...
    av_em_log(NULL, AV_LOG_DEBUG, "Subpacket: type=%d size=%d start_offs=%x\n",
                                     ...
                                     int cm_table_select)
{
    ...
    if (!superblocktype_2_3) {
        ...
        avpriv_em_request_sample(NULL, "!superblocktype_2_3");
        return;
    ...
    for (sb = sb_min; sb < sb_max; sb++) {
        ...
        if (joined_stereo) {
            ...
            if (fix_coding_method_array(sb, q->nb_channels,
                                            q->coding_method)) {
                av_em_log(NULL, AV_LOG_ERROR, "coding method invalid\n");
                build_sb_samples_from_noise(q, sb);
                continue;
            }
            channels = 1;
                ...
                switch (q->coding_method[ch][sb][j / 2]) {
                    case 8:
                        if (get_bits_left(gb) >= 10) {
                            if (zero_encoding) {
                                ...
                                n = get_bits(gb, 8);
                                if (n >= 243) {
                                    av_em_log(NULL, AV_LOG_ERROR, "Invalid 8bit codeword\n");
                    ...
                    case 24:
                        if (get_bits_left(gb) >= 7) {
                            n = get_bits(gb, 7);
                            if (n >= 125) {
                                av_em_log(NULL, AV_LOG_ERROR, "Invalid 7bit codeword\n");
                                return AVERROR_INVALIDDATA;
                            }
                    ...
                    case 30:
                        if (get_bits_left(gb) >= 4) {
                            ...
                            if (index >= FF_ARRAY_ELEMS(type30_dequant)) {
                                av_em_log(NULL, AV_LOG_ERROR, "index %d out of type30_dequant array\n", index);
                                return AVERROR_INVALIDDATA;
                            }
                        ...
                        if (get_bits_left(gb) >= 7) {
                            if (type34_first) {
                                ...
                                if (index >= FF_ARRAY_ELEMS(type34_delta)) {
                                    av_em_log(NULL, AV_LOG_ERROR, "index %d out of type34_delta array\n", index);
                                    return AVERROR_INVALIDDATA;
                                }
...
static void qdm2_decode_super_block(QDM2Context *q)
{
    ...
    if (header.type < 2 || header.type >= 8) {
        q->has_errors = 1;
        av_em_log(NULL, AV_LOG_ERROR, "bad superblock type\n");
        return;
    }
    ...
    if (header.type == 2 || header.type == 4 || header.type == 5) {
        ...
        if (csum != 0) {
            q->has_errors = 1;
            av_em_log(NULL, AV_LOG_ERROR, "bad packet checksum\n");
            return;
        }
    }
            ...
            while ((n = qdm2_get_vlc(gb, &vlc_tab_fft_tone_offset[local_int_8], 1, 2)) < 2) {
                if (get_bits_left(gb)<0) {
                    if(local_int_4 < q->group_size)
                        av_em_log(NULL, AV_LOG_ERROR, "overread in qdm2_fft_decode_tones()\n");
                    return;
                }
                offset = 1;
                if (n == 0) {
 ...
 */
static av_cold int qdm2_decode_init(AVEMCodecContext *avctx)
{
    ...
    */
    ...
    if (!avctx->extradata || (avctx->extradata_size < 48)) {
        av_em_log(avctx, AV_LOG_ERROR, "extradata missing or truncated\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (extradata_size < 12) {
        av_em_log(avctx, AV_LOG_ERROR, "not enough extradata (%i)\n",
               extradata_size);
        return AVERROR_INVALIDDATA;
    }

    if (memcmp(extradata, "frmaQDM", 7)) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid headers, QDM? not found\n");
        return AVERROR_INVALIDDATA;
    }

    if (extradata[7] == 'C') {
//        s->is_qdmc = 1;
        avpriv_em_report_missing_feature(avctx, "QDMC version 1");
    ...
    if(size > extradata_size){
        av_em_log(avctx, AV_LOG_ERROR, "extradata size too small, %i < %i\n",
               extradata_size, size);
        return AVERROR_INVALIDDATA;
    }

    extradata += 4;
    av_em_log(avctx, AV_LOG_DEBUG, "size: %d\n", size);
    if (AV_RB32(extradata) != MKBETAG('Q','D','C','A')) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid extradata, expecting QDCA\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->checksum_size >= 1U << 28) {
        av_em_log(avctx, AV_LOG_ERROR, "data block size too large (%u)\n", s->checksum_size);
        return AVERROR_INVALIDDATA;
    }
    ...
    if ((s->fft_order < 7) || (s->fft_order > 9)) {
        avpriv_em_request_sample(avctx, "Unknown FFT order %d", s->fft_order);
        return AVERROR_PATCHWELCOME;
    }
    if (s->fft_size != (1 << (s->fft_order - 1))) {
        av_em_log(avctx, AV_LOG_ERROR, "FFT size %d not power of 2.\n", s->fft_size);
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int qdm2_decode_close(AVEMCodecContext *avctx)
{
    QDM2Context *s = avctx->priv_data;
...
static int qdm2_decode(QDM2Context *q, const uint8_t *in, int16_t *out)
{
    ...
    if (q->sub_packet == 0) {
        ...
        av_em_log(NULL,AV_LOG_DEBUG,"Superblock follows\n");
...
static int qdm2_decode_frame(AVEMCodecContext *avctx, void *data,
