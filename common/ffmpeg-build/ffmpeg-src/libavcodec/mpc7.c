...
static av_cold int mpc7_decode_init(AVEMCodecContext * avctx)
{
    int i, j;
    ...
    if (avctx->channels != 2) {
        avpriv_em_request_sample(avctx, "%d channels", avctx->channels);
        return AVERROR_PATCHWELCOME;
    }

    if(avctx->extradata_size < 16){
        av_em_log(avctx, AV_LOG_ERROR, "Too small extradata size (%i)!\n", avctx->extradata_size);
        return -1;
    }
    memset(c->oldDSCF, 0, sizeof(c->oldDSCF));
    av_em_lfg_init(&c->rnd, 0xDEADBEEF);
    ...
    if(c->maxbands >= BANDS){
        av_em_log(avctx, AV_LOG_ERROR, "Too many bands: %i\n", c->maxbands);
        return -1;
    }
    skip_bits_long(&gb, 88);
    c->gapless = get_bits1(&gb);
    c->lastframelen = get_bits(&gb, 11);
    av_em_log(avctx, AV_LOG_DEBUG, "IS: %d, MSS: %d, TG: %d, LFL: %d, bands: %d\n",
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Initing VLC\n");
    ...
    if(init_vlc(&scfi_vlc, MPC7_SCFI_BITS, MPC7_SCFI_SIZE,
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Cannot init SCFI VLC\n");
        return -1;
    }
    ...
    if(init_vlc(&dscf_vlc, MPC7_DSCF_BITS, MPC7_DSCF_SIZE,
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Cannot init DSCF VLC\n");
        return -1;
    }
    ...
    if(init_vlc(&hdr_vlc, MPC7_HDR_BITS, MPC7_HDR_SIZE,
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Cannot init HDR VLC\n");
        return -1;
    }
    for(i = 0; i < MPC7_QUANT_VLC_TABLES; i++){
        for(j = 0; j < 2; j++){
            ...
            if(init_vlc(&quant_vlc[i][j], 9, mpc7_quant_vlc_sizes[i],
                ...
                av_em_log(avctx, AV_LOG_ERROR, "Cannot init QUANT VLC %i,%i\n",i,j);
                return -1;
            }
        }
    }
...
static int mpc7_decode_frame(AVEMCodecContext * avctx, void *data,
                             int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (buf_size <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "packet size is too small (%i bytes)\n",
               avpkt->size);
    ...
    av_em_fast_padded_malloc(&c->bits, &c->buf_size, buf_size);
    if (!c->bits)
    ...
    for(i = 0; i <= c->maxbands; i++){
        for(ch = 0; ch < 2; ch++){
            ...
            if (bands[i].res[ch] < -1 || bands[i].res[ch] > 17) {
                av_em_log(avctx, AV_LOG_ERROR, "subband index invalid\n");
                return AVERROR_INVALIDDATA;
            }
        }
    ...
    if (!last_frame && ((bits_avail < bits_used) || (bits_used + 32 <= bits_avail))) {
        av_em_log(avctx, AV_LOG_ERROR, "Error decoding frame: used %i of %i bits\n", bits_used, bits_avail);
        return -1;
    }
    if(c->frames_to_skip){
        c->frames_to_skip--;
...
static void mpc7_decode_flush(AVEMCodecContext *avctx)
{
    MPCContext *c = avctx->priv_data;
...
static av_cold int mpc7_decode_close(AVEMCodecContext *avctx)
{
    MPCContext *c = avctx->priv_data;
    av_em_freep(&c->bits);
    c->buf_size = 0;
