...
static av_cold int sgirle_decode_init(AVEMCodecContext *avctx)
{
 ...
 * @return <0 on error
 */
static int decode_sgirle8(AVEMCodecContext *avctx, uint8_t *dst,
...
#define INC_XY(n)                                                             \
    ...
    while (src_end - src >= 2) {
        ...
        if (v > 0 && v < 0xC0) {
            ...
            avpriv_em_request_sample(avctx, "opcode %d", v);
...
static int sgirle_decode_frame(AVEMCodecContext *avctx, void *data,
