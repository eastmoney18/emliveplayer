                           ...
                           int qsel, int is_chroma, int is_hqa)
{
    ...
    if (!is_hqa) {
        ...
        q = em_hq_quants[qsel][is_chroma][get_bits(gb, 2)];
    } else {
        q = em_hq_quants[qsel][is_chroma][get_bits(gb, 2)];
        block[0] = get_sbits(gb, 9) << 6;
    }
    ...
    for (;;) {
        ...
        pos += em_hq_ac_skips[val];
        if (pos >= 64)
            break;
        block[ff_zigzag_direct[pos]] = (em_hq_ac_syms[val] * q[pos]) >> 12;
        pos++;
    }
                           ...
                           int prof_num, size_t data_size)
{
    ...
    if ((unsigned)prof_num >= NUM_HQ_PROFILES) {
        profile = &em_hq_profile[0];
        avpriv_em_request_sample(ctx->avctx, "HQ Profile %d", prof_num);
    } else {
        profile = &em_hq_profile[prof_num];
        av_em_log(ctx->avctx, AV_LOG_VERBOSE, "HQ Profile %d\n", prof_num);
    }
    ...
    ret = em_get_buffer(ctx->avctx, pic, 0);
    if (ret < 0)
        return ret;
...
static int hqa_decode_frame(HQContext *ctx, AVFrame *pic, size_t data_size)
{
    ...
    av_em_log(ctx->avctx, AV_LOG_VERBOSE, "HQA Profile\n");
...
static int hq_hqa_decode_frame(AVEMCodecContext *avctx, void *data,
                               int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (info_tag == MKTAG('I', 'N', 'F', 'O')) {
        ...
        if (bytestream2_get_bytes_left(&ctx->gbc) < info_size) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid INFO size (%d).\n", info_size);
            return AVERROR_INVALIDDATA;
        }
    ...
    if ((tag & 0x00FFFFFF) == (MKTAG('U', 'V', 'C', ' ') & 0x00FFFFFF)) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Not a HQ/HQA frame.\n");
        return AVERROR_INVALIDDATA;
    }
    if (ret < 0) {
...
static av_cold int hq_hqa_decode_init(AVEMCodecContext *avctx)
{
    HQContext *ctx = avctx->priv_data;
    ctx->avctx = avctx;

    em_hqdsp_init(&ctx->hqhqadsp);

    return em_hq_init_vlcs(ctx);
}

static av_cold int hq_hqa_decode_close(AVEMCodecContext *avctx)
{
    HQContext *ctx = avctx->priv_data;
