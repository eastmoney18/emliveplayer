...
{
    char *str = av_em_alloc(len + 1);
    if (str) {
        memcpy(str, buf, len);
        str[len] = 0;
        if (*(void **)dest)
            av_em_free(*(void **)dest);
        *(char **)dest = str;
    }
    return !str;
}
...
static uint8_t *realloc_section_array(ASSSplitContext *ctx)
{
    ...
    uint8_t *tmp = av_em_realloc_array(*section_ptr, (*count+1), section->size);
    if (!tmp)
        return NULL;
    *section_ptr = tmp;
...
{
    int i;
    int *order = av_em_malloc_array(FF_ARRAY_ELEMS(section->fields), sizeof(*order));
    ...
    while (buf && *buf) {
        ...
        if (buf[0] == ';' || (buf[0] == '!' && buf[1] == ':')) {
            ...
            while (!is_eol(*buf)) {
                buf = skip_space(buf);
                len = strcspn(buf, ", \r\n");
                if (!(tmp = av_em_realloc_array(order, (*number + 1), sizeof(*order))))
                    return NULL;
                order = tmp;
                order[*number] = -1;
                for (i=0; section->fields[i].name; i++)
...
{
    ASSSplitContext *ctx = av_em_mallocz(sizeof(*ctx));
    if (!ctx)
        return NULL;
    ctx->current_section = -1;
        ...
        for (i=0; i<*count; i++, ptr += section->size)
            for (j=0; section->fields[j].name; j++) {
                ...
                if (field->type == ASS_STR)
                    av_em_freep(ptr + field->offset);
            }
    *count = 0;

    if (section->format_header)
        av_em_freep((uint8_t *)&ctx->ass + section->offset);
}
...
void ff_ass_free_dialog(ASSDialog **dialogp)
{
    ...
    if (!dialog)
        return;
    av_em_freep(&dialog->style);
    av_em_freep(&dialog->name);
    av_em_freep(&dialog->effect);
    av_em_freep(&dialog->text);
    av_em_freep(dialogp);
}
...
ASSDialog *ff_ass_split_dialog2(ASSSplitContext *ctx, const char *buf)
{
    ...
    ASSDialog *dialog = av_em_mallocz(sizeof(*dialog));
    if (!dialog)
        return NULL;
