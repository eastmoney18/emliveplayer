...
static const AVEMRational dirac_preset_aspect_ratios[] = {
    {  1,  1 },
    { 10, 11 },
    { 12, 11 },
    { 40, 33 },
 ...
 */
static const AVEMRational dirac_frame_rate[] = {
    { 15000, 1001 },
    {    25,    2 },
};
                                   ...
                                   void *log_ctx)
{
    ...
    if (dsh->chroma_format > 2U) {
        if (log_ctx)
            av_em_log(log_ctx, AV_LOG_ERROR, "Unknown chroma format %d\n",
                   dsh->chroma_format);
    ...
    avcodec_em_get_chroma_sub_sample(dsh->pix_fmt, &chroma_x_shift, &chroma_y_shift);
    if ((dsh->width % (1<<chroma_x_shift)) || (dsh->height % (1<<chroma_y_shift))) {
        if (log_ctx)
            av_em_log(log_ctx, AV_LOG_ERROR, "Dimensions must be an integer multiple of the chroma subsampling\n");
        return AVERROR_INVALIDDATA;
    }
...
int av_em_dirac_parse_sequence_header(AVDiracSeqHeader **pdsh,
                                   ...
                                   void *log_ctx)
{
    ...
    if (dsh->version.major < 2 && log_ctx)
        av_em_log(log_ctx, AV_LOG_WARNING, "Stream is old and may not work\n");
    else if (dsh->version.major > 2 && log_ctx)
        av_em_log(log_ctx, AV_LOG_WARNING, "Stream may have unhandled features\n");
    ...
    if (picture_coding_mode != 0) {
        if (log_ctx) {
            av_em_log(log_ctx, AV_LOG_ERROR, "Unsupported picture coding mode %d",
                   picture_coding_mode);
        }
        ret = AVERROR_INVALIDDATA;
        goto fail;
