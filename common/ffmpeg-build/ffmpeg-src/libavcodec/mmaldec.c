...
{
    if (ref && avpriv_em_atomic_int_add_and_fetch(&ref->refcount, -1) == 0) {
...
static int ffmmal_set_ref(AVFrame *frame, FFPoolRef *pool,
                          MMAL_BUFFER_HEADER_T *buffer)
{
    FFBufferRef *ref = av_em_mallocz(sizeof(*ref));
    if (!ref)
    ...
    frame->buf[0] = av_em_buffer_create((void *)ref, sizeof(*ref),
    ...
    avpriv_em_atomic_int_add_and_fetch(&ref->pool->refcount, 1);
...
static void ffmmal_stop_decoder(AVEMCodecContext *avctx)
{
    ...
    while (ctx->waiting_buffers) {
        ...
        if (buffer->flags & MMAL_BUFFER_HEADER_FLAG_FRAME_END)
            avpriv_em_atomic_int_add_and_fetch(&ctx->packets_buffered, -1);

        av_em_buffer_unref(&buffer->ref);
        av_em_free(buffer);
    }
    ctx->waiting_buffers_tail = NULL;

    av_assert0(avpriv_em_atomic_int_get(&ctx->packets_buffered) == 0);

    ctx->frames_output = ctx->eos_received = ctx->eos_sent = ctx->packets_sent = ctx->extradata_sent = 0;
}

static av_cold int ffmmal_close_decoder(AVEMCodecContext *avctx)
{
    MMALDecodeContext *ctx = avctx->priv_data;
...
{
    AVEMCodecContext *avctx = (AVEMCodecContext*)port->userdata;
    MMALDecodeContext *ctx = avctx->priv_data;

    if (!buffer->cmd) {
        FFBufferEntry *entry = buffer->user_data;
        av_em_buffer_unref(&entry->ref);
...
static void control_port_cb(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
    ...
    if (buffer->cmd == MMAL_EVENT_ERROR) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "MMAL error %d on control port\n", (int)status);
    } else {
        char s[20];
        av_em_get_codec_tag_string(s, sizeof(s), buffer->cmd);
        av_em_log(avctx, AV_LOG_WARNING, "Unknown MMAL event %s on control port\n", s);
    }
...
static int ffmmal_fill_output_port(AVEMCodecContext *avctx)
{
    MMALDecodeContext *ctx = avctx->priv_data;
    MMAL_BUFFER_HEADER_T *buffer;
    MMAL_STATUS_T status;
    ...
    while ((buffer = mmal_queue_get(ctx->pool_out->pool->queue))) {
        if ((status = mmal_port_send_buffer(ctx->decoder->output[0], buffer))) {
            mmal_buffer_header_release(buffer);
            av_em_log(avctx, AV_LOG_ERROR, "MMAL error %d when sending output buffer.\n", (int)status);
...
static int ffmal_update_format(AVEMCodecContext *avctx)
{
    MMALDecodeContext *ctx = avctx->priv_data;
    MMAL_STATUS_T status;
    int ret = 0;
    ...
    ffmmal_poolref_unref(ctx->pool_out);
    if (!(ctx->pool_out = av_em_mallocz(sizeof(*ctx->pool_out)))) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    if ((ret = em_set_dimensions(avctx, format_out->es->video.crop.x + format_out->es->video.crop.width,
...
static av_cold int ffmmal_init_decoder(AVEMCodecContext *avctx)
{
    MMALDecodeContext *ctx = avctx->priv_data;
    MMAL_STATUS_T status;
    ...
    if (mmal_vc_init()) {
        av_em_log(avctx, AV_LOG_ERROR, "Cannot initialize MMAL VC driver!\n");
        return AVERROR(ENOSYS);
    }

    if ((ret = em_get_format(avctx, avctx->codec->pix_fmts)) < 0)
        return ret;
    ...
    av_em_get_codec_tag_string(tmp, sizeof(tmp), format_in->encoding);
    av_em_log(avctx, AV_LOG_DEBUG, "Using MMAL %s encoding.\n", tmp);
    ...
    if (mmal_port_parameter_set_uint32(decoder->input[0], MMAL_PARAMETER_VIDEO_MAX_NUM_CALLBACKS,
        ...
        av_em_log(avctx, AV_LOG_WARNING, "Could not set input buffering limit.\n");
    }
#endif
...
static void ffmmal_flush(AVEMCodecContext *avctx)
{
    MMALDecodeContext *ctx = avctx->priv_data;
    MMAL_COMPONENT_T *decoder = ctx->decoder;
    MMAL_STATUS_T status;
...
fail:
    av_em_log(avctx, AV_LOG_ERROR, "MMAL flush error: %i\n", (int)status);
}
...
static int ffmmal_add_packet(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                             int is_extradata)
{
    MMALDecodeContext *ctx = avctx->priv_data;
    ...
    if (avpkt->size) {
        if (avpkt->buf) {
            buf = av_em_buffer_ref(avpkt->buf);
            size = avpkt->size;
            data = avpkt->data;
        } else {
            buf = av_em_buffer_alloc(avpkt->size);
            if (buf) {
    ...
    do {
        FFBufferEntry *buffer = av_em_mallocz(sizeof(*buffer));
        if (!buffer) {
            ret = AVERROR(ENOMEM);
            goto done;
        }
        ...
        if (buf) {
            buffer->ref = av_em_buffer_ref(buf);
            if (!buffer->ref) {
                av_em_free(buffer);
                ret = AVERROR(ENOMEM);
                goto done;
    ...
    av_em_buffer_unref(&buf);
    return ret;
}

// Move prepared/split packets from waiting_buffers to the MMAL decoder.
static int ffmmal_fill_input_port(AVEMCodecContext *avctx)
{
    MMALDecodeContext *ctx = avctx->priv_data;
    ...
    while (ctx->waiting_buffers) {
        ...
        if (status) {
            av_em_log(avctx, AV_LOG_ERROR, "MMAL error %d when sending input\n", (int)status);
            return AVERROR_UNKNOWN;
        }
    }
...
static int ffmal_copy_frame(AVEMCodecContext *avctx,  AVFrame *frame,
                            MMAL_BUFFER_HEADER_T *buffer)
{
    MMALDecodeContext *ctx = avctx->priv_data;
    int ret = 0;
    ...
    if (avctx->pix_fmt == AV_PIX_FMT_MMAL) {
        ...
        if ((ret = em_decode_frame_props(avctx, frame)) < 0)
            goto done;
        ...
        av_em_image_fill_arrays(src, linesize,
                             buffer->data + buffer->type->video.offset[0],
                             avctx->pix_fmt, w, h, 1);
        av_em_image_copy(frame->data, frame->linesize, src, linesize,
...
static int ffmmal_read_frame(AVEMCodecContext *avctx, AVFrame *frame, int *got_frame)
{
    MMALDecodeContext *ctx = avctx->priv_data;
    ...
    while (1) {
        ...
        if (avpriv_em_atomic_int_get(&ctx->packets_buffered) > MAX_DELAYED_FRAMES ||
            ...
            if (!buffer) {
                av_em_log(avctx, AV_LOG_ERROR, "Did not get output frame from MMAL.\n");
                ret = AVERROR_UNKNOWN;
                goto done;
            }
        } else {
        ...
        if (buffer->cmd == MMAL_EVENT_FORMAT_CHANGED) {
            ...
            av_em_log(avctx, AV_LOG_INFO, "Changing output format.\n");
            ...
            char s[20];
            av_em_get_codec_tag_string(s, sizeof(s), buffer->cmd);
            av_em_log(avctx, AV_LOG_WARNING, "Unknown MMAL event %s on output port\n", s);
            goto done;
...
static int ffmmal_decode(AVEMCodecContext *avctx, void *data, int *got_frame,
                         AVEMPacket *avpkt)
{
    MMALDecodeContext *ctx = avctx->priv_data;
    AVFrame *frame = data;
...
AVHWAccel em_h264_mmal_hwaccel = {
    .name       = "h264_mmal",
...
AVHWAccel em_mpeg4_mmal_hwaccel = {
    .name       = "mpeg4_mmal",
    .type       = AVMEDIA_TYPE_VIDEO,
...
#define FFMMAL_DEC_CLASS(NAME) \
    static const AVEMClass ffmmal_##NAME##_dec_class = { \
...
#define FFMMAL_DEC(NAME, ID) \
    ...
    AVEMCodec ff_##NAME##_mmal_decoder = { \
        .name           = #NAME "_mmal", \
