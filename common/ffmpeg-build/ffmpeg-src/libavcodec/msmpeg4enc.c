...
static av_cold int init_mv_table(MVTable *tab)
{
    ...
    tab->table_mv_index = av_em_alloc(sizeof(uint16_t) * 4096);
                                  ...
                                  int mx, int my)
{
    ...
    if ((unsigned)mx >= 64 ||
        (unsigned)my >= 64)
        av_em_log(s->avctx, AV_LOG_ERROR, "error mx=%d my=%d\n", mx, my);
#endif
                          ...
                          int motion_x, int motion_y)
{
    ...
    if (!s->mb_intra) {
        ...
        if(s->msmpeg4_version<=2){
            ...
            put_bits(&s->pb,
                     em_h263_cbpy_tab[coded_cbp>>2][1],
                     em_h263_cbpy_tab[coded_cbp>>2][0]);
        ...
        if(s->msmpeg4_version<=2){
            ...
            put_bits(&s->pb,
                     em_h263_cbpy_tab[cbp>>2][1],
                     em_h263_cbpy_tab[cbp>>2][0]);
        }else{
            if (s->pict_type == AV_PICTURE_TYPE_I) {
                put_bits(&s->pb,
