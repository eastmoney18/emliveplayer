...
static int vda_old_h264_start_frame(AVEMCodecContext *avctx,
...
static int vda_old_h264_decode_slice(AVEMCodecContext *avctx,
                                 ...
                                 uint32_t size)
{
    ...
    tmp = av_em_fast_realloc(vda->bitstream,
                          &vda->allocated_size,
...
static int vda_old_h264_end_frame(AVEMCodecContext *avctx)
{
    ...
    context = av_em_mallocz(sizeof(*context));
    buffer = av_em_buffer_create(NULL, 0, vda_h264_release_buffer, context, 0);
    if (!context || !buffer) {
...
int em_vda_create_decoder(struct vda_context *vda_ctx,
                          uint8_t *extradata,
                          int extradata_size)
{
    OSStatus status;
    ...
    if (extradata_size >= 4 && (extradata[4] & 0x03) != 0x03) {
        ...
        if (!(rw_extradata = av_em_alloc(extradata_size)))
            return AVERROR(ENOMEM);
...
int em_vda_destroy_decoder(struct vda_context *vda_ctx)
{
...
AVHWAccel em_h264_vda_old_hwaccel = {
    .name           = "h264_vda",
...
static int vda_h264_end_frame(AVEMCodecContext *avctx)
{
    H264EMContext *h        = avctx->priv_data;
    VTContext *vda        = avctx->internal->hwaccel_priv_data;
...
int ff_vda_default_init(AVEMCodecContext *avctx)
{
    AVVDAContext *vda_ctx = avctx->hwaccel_context;
    OSStatus status = kVDADecoderNoErr;
    CFNumberRef height;
    ...
    if (status != kVDADecoderNoErr) {
        av_em_log(avctx, AV_LOG_ERROR, "Cannot initialize VDA %d\n", status);
    }
