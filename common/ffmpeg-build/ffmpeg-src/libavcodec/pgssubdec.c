...
static void flush_cache(AVEMCodecContext *avctx)
{
    PGSSubContext *ctx = avctx->priv_data;
    int i;

    for (i = 0; i < ctx->objects.count; i++) {
        av_em_freep(&ctx->objects.object[i].rle);
...
static av_cold int init_decoder(AVEMCodecContext *avctx)
{
...
static av_cold int close_decoder(AVEMCodecContext *avctx)
{
    flush_cache(avctx);
 ...
 */
static int decode_rle(AVEMCodecContext *avctx, AVSubtitleRect *rect,
                      const uint8_t *buf, unsigned int buf_size)
{
    ...
    rect->data[0] = av_em_malloc_array(rect->w, rect->h);
    ...
    while (buf < rle_bitmap_end && line_count < rect->h) {
        ...
        if (run > 0 && pixel_count + run <= rect->w * rect->h) {
            ...
            if (pixel_count % rect->w > 0) {
                av_em_log(avctx, AV_LOG_ERROR, "Decoded %d pixels, when line should be %d pixels\n",
    ...
    if (pixel_count < rect->w * rect->h) {
        av_em_log(avctx, AV_LOG_ERROR, "Insufficient RLE data for subtitle\n");
        return AVERROR_INVALIDDATA;
    }
 ...
 */
static int parse_object_segment(AVEMCodecContext *avctx,
                                  const uint8_t *buf, int buf_size)
{
    ...
    if (!object) {
        if (ctx->objects.count >= MAX_EPOCH_OBJECTS) {
            av_em_log(avctx, AV_LOG_ERROR, "Too many objects in epoch\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    if (avctx->width < width || avctx->height < height || !width || !height) {
        av_em_log(avctx, AV_LOG_ERROR, "Bitmap dimensions (%dx%d) invalid.\n", width, height);
        return AVERROR_INVALIDDATA;
    }
    ...
    av_em_fast_padded_malloc(&object->rle, &object->rle_buffer_size, rle_bitmap_len);
 ...
 * @param buf_size size of packet to process
 */
static int parse_palette_segment(AVEMCodecContext *avctx,
                                  const uint8_t *buf, int buf_size)
{
    PGSSubContext *ctx = avctx->priv_data;
    ...
    if (!palette) {
        if (ctx->palettes.count >= MAX_EPOCH_PALETTES) {
            av_em_log(avctx, AV_LOG_ERROR, "Too many palettes in epoch\n");
            return AVERROR_INVALIDDATA;
        }
 ...
 */
static int parse_presentation_segment(AVEMCodecContext *avctx,
                                      const uint8_t *buf, int buf_size,
                                      int64_t pts)
{
    PGSSubContext *ctx = avctx->priv_data;
    ...
    for (i = 0; i < ctx->presentation.object_count; i++)
    {
        ...
        if (buf_end - buf < 8) {
            av_em_log(avctx, AV_LOG_ERROR, "Insufficent space for object\n");
        ...
        if (ctx->presentation.objects[i].x > avctx->width ||
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Subtitle out of video bounds. x = %d, y = %d, video width = %d, video height = %d.\n",
 ...
 * @param buf_size size of packet to process
 */
static int display_end_segment(AVEMCodecContext *avctx, void *data,
                               const uint8_t *buf, int buf_size)
{
    AVSubtitle    *sub = data;
    PGSSubContext *ctx = avctx->priv_data;
    ...
    if (!ctx->presentation.object_count)
        return 1;
    sub->rects = av_em_mallocz_array(ctx->presentation.object_count, sizeof(*sub->rects));
    if (!sub->rects) {
        return AVERROR(ENOMEM);
    }
    ...
    if (!palette) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Invalid palette id %d\n",
    ...
    for (i = 0; i < ctx->presentation.object_count; i++) {
        ...
        sub->rects[i]  = av_em_mallocz(sizeof(*sub->rects[0]));
        if (!sub->rects[i]) {
            avsubtitle_em_free(sub);
            return AVERROR(ENOMEM);
        }
        ...
        if (!object) {
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Invalid object id %d\n",
        ...
        if (object->rle) {
            if (object->rle_remaining_len) {
                av_em_log(avctx, AV_LOG_ERROR, "RLE data length %u is %u bytes shorter than expected\n",
        ...
        sub->rects[i]->data[1] = av_em_mallocz(AVPALETTE_SIZE);
...
static int decode(AVEMCodecContext *avctx, void *data, int *data_size,
                  AVEMPacket *avpkt)
{
    ...
    *data_size = 0;
    ...
    while (buf < buf_end) {
        ...
        case DISPLAY_SEGMENT:
            ...
            break;
        default:
            av_em_log(avctx, AV_LOG_ERROR, "Unknown subtitle segment type 0x%x, length %d\n",
...
static const AVEMClass pgsdec_class = {
