...
int ff_sse16_sse2(MpegEncContext *v, uint8_t *pix1, uint8_t *pix2,
                  ptrdiff_t stride, int h);
int em_hf_noise8_mmx(uint8_t *pix1, ptrdiff_t stride, int h);
int em_hf_noise16_mmx(uint8_t *pix1, ptrdiff_t stride, int h);
...
#define hadamard_func(cpu)                                                    \
    int em_hadamard8_diff_ ## cpu(MpegEncContext *s, uint8_t *src1,           \
                                  uint8_t *src2, ptrdiff_t stride, int h);    \
    int em_hadamard8_diff16_ ## cpu(MpegEncContext *s, uint8_t *src1,         \
                                    uint8_t *src2, ptrdiff_t stride, int h);
                      ...
                      ptrdiff_t stride, int h)
{
    ...
    score2 = em_hf_noise16_mmx(pix1, stride, h) + em_hf_noise8_mmx(pix1+8, stride, h)
           - em_hf_noise16_mmx(pix2, stride, h) - em_hf_noise8_mmx(pix2+8, stride, h);
                     ...
                     ptrdiff_t stride, int h)
{
    ...
    int score2 = em_hf_noise8_mmx(pix1, stride, h) -
                 em_hf_noise8_mmx(pix2, stride, h);
...
av_cold void ff_me_cmp_init_x86(MECmpContext *c, AVEMCodecContext *avctx)
{
    ...
    if (EXTERNAL_MMX(cpu_flags)) {
        c->hadamard8_diff[0] = em_hadamard8_diff16_mmx;
        c->hadamard8_diff[1] = em_hadamard8_diff_mmx;
    ...
    if (EXTERNAL_MMXEXT(cpu_flags)) {
        c->hadamard8_diff[0] = em_hadamard8_diff16_mmxext;
        c->hadamard8_diff[1] = em_hadamard8_diff_mmxext;
    ...
    if (EXTERNAL_SSE2(cpu_flags)) {
        ...
        c->hadamard8_diff[0] = em_hadamard8_diff16_sse2;
        c->hadamard8_diff[1] = em_hadamard8_diff_sse2;
#endif
...
#if HAVE_ALIGNED_STACK
        c->hadamard8_diff[0] = em_hadamard8_diff16_ssse3;
        c->hadamard8_diff[1] = em_hadamard8_diff_ssse3;
#endif
    }
}
