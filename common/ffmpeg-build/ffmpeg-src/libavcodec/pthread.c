 ...
 */
static void validate_thread_parameters(AVEMCodecContext *avctx)
{
...
int em_thread_init(AVEMCodecContext *avctx)
{
    validate_thread_parameters(avctx);

    if (avctx->active_thread_type&FF_THREAD_SLICE)
        return em_slice_thread_init(avctx);
    else if (avctx->active_thread_type&FF_THREAD_FRAME)
        return em_frame_thread_init(avctx);

    return 0;
}

void em_thread_free(AVEMCodecContext *avctx)
{
    if (avctx->active_thread_type&FF_THREAD_FRAME)
        em_frame_thread_free(avctx, avctx->thread_count);
    else
        em_slice_thread_free(avctx);
}
