...
static av_cold void dprint_compression_options(FlacEncodeContext *s)
{
    ...
    av_em_log(avctx, AV_LOG_DEBUG, " compression: %d\n", opt->compression_level);

    switch (opt->lpc_type) {
    case FF_LPC_TYPE_NONE:
        av_em_log(avctx, AV_LOG_DEBUG, " lpc type: None\n");
        break;
    case FF_LPC_TYPE_FIXED:
        av_em_log(avctx, AV_LOG_DEBUG, " lpc type: Fixed pre-defined coefficients\n");
        break;
    case FF_LPC_TYPE_LEVINSON:
        av_em_log(avctx, AV_LOG_DEBUG, " lpc type: Levinson-Durbin recursion with Welch window\n");
        break;
    case FF_LPC_TYPE_CHOLESKY:
        av_em_log(avctx, AV_LOG_DEBUG, " lpc type: Cholesky factorization, %d pass%s\n",
               opt->lpc_passes, opt->lpc_passes == 1 ? "" : "es");
        break;
    }

    av_em_log(avctx, AV_LOG_DEBUG, " prediction order: %d, %d\n",
           opt->min_prediction_order, opt->max_prediction_order);

    switch (opt->prediction_order_method) {
    case ORDER_METHOD_EST:
        av_em_log(avctx, AV_LOG_DEBUG, " order method: %s\n", "estimate");
        break;
    ...
    av_em_log(avctx, AV_LOG_DEBUG, " partition order: %d, %d\n",
           opt->min_partition_order, opt->max_partition_order);

    av_em_log(avctx, AV_LOG_DEBUG, " block size: %d\n", avctx->frame_size);

    av_em_log(avctx, AV_LOG_DEBUG, " lpc precision: %d\n",
           opt->lpc_coeff_precision);
}


static av_cold int flac_encode_init(AVEMCodecContext *avctx)
{
    ...
    case AV_SAMPLE_FMT_S32:
        if (avctx->bits_per_raw_sample != 24)
            av_em_log(avctx, AV_LOG_WARNING, "encoding as 24 bits-per-sample\n");
    ...
    if (channels < 1 || channels > FLAC_MAX_CHANNELS) {
        av_em_log(avctx, AV_LOG_ERROR, "%d channels not supported (max %d)\n",
    ...
    if (i == 12) {
        if (freq % 1000 == 0 && freq < 255000) {
            ...
            av_em_log(avctx, AV_LOG_ERROR, "%d Hz not supported\n", freq);
            return AVERROR(EINVAL);
        }
        s->samplerate = freq;
    }
    ...
    if (level > 12) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid compression level: %d\n",
    ...
    if (s->options.min_partition_order > s->options.max_partition_order) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid partition orders: min=%d max=%d\n",
...
FF_DISABLE_DEPRECATION_WARNINGS
    if (avctx->min_prediction_order >= 0) {
        if (s->options.lpc_type == FF_LPC_TYPE_FIXED) {
            ...
            av_em_log(avctx, AV_LOG_ERROR, "invalid min prediction order: %d\n",
    ...
    if (avctx->max_prediction_order >= 0) {
        if (s->options.lpc_type == FF_LPC_TYPE_FIXED) {
            ...
            av_em_log(avctx, AV_LOG_ERROR, "invalid max prediction order: %d\n",
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (s->options.max_prediction_order < s->options.min_prediction_order) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid prediction orders: min=%d max=%d\n",
    ...
    if (avctx->frame_size > 0) {
        if (avctx->frame_size < FLAC_MIN_BLOCKSIZE ||
            ...
            av_em_log(avctx, AV_LOG_ERROR, "invalid block size: %d\n",
                   avctx->frame_size);
            return AVERROR(EINVAL);
        }
    } else {
    ...
    s->md5ctx = av_em_md5_alloc();
    if (!s->md5ctx)
        return AVERROR(ENOMEM);
    av_em_md5_init(s->md5ctx);

    streaminfo = av_em_alloc(FLAC_STREAMINFO_SIZE);
    if (!streaminfo)
        return AVERROR(ENOMEM);
    ...
    if (channels == 3 &&
        ...
        if (avctx->channel_layout) {
            av_em_log(avctx, AV_LOG_ERROR, "Channel layout not supported by Flac, "
                                             "output stream will have incorrect "
                                             "channel layout.\n");
        } else {
            av_em_log(avctx, AV_LOG_WARNING, "No channel layout specified. The encoder "
...
static void copy_samples(FlacEncodeContext *s, const void *samples)
{
    int i, j, ch;
    FlacFrame *frame;
    int shift = av_em_get_bytes_per_sample(s->avctx->sample_fmt) * 8 -
                s->avctx->bits_per_raw_sample;
...
static void write_frame_header(FlacEncodeContext *s)
{
    ...
    flush_put_bits(&s->pb);
    crc = av_em_crc(av_em_crc_get_table(AV_CRC_8_ATM), 0, s->pb.buf,
...
static void write_frame_footer(FlacEncodeContext *s)
{
    int crc;
    flush_put_bits(&s->pb);
    crc = av_bswap16(av_em_crc(av_em_crc_get_table(AV_CRC_16_ANSI), 0, s->pb.buf,
                            put_bits_count(&s->pb)>>3));
...
static int write_frame(FlacEncodeContext *s, AVEMPacket *avpkt)
{
...
static int update_md5_sum(FlacEncodeContext *s, const void *samples)
{
    ...
    if (s->avctx->bits_per_raw_sample > 16 || HAVE_BIGENDIAN) {
        av_em_fast_malloc(&s->md5_buffer, &s->md5_buffer_size, buf_size);
        if (!s->md5_buffer)
            return AVERROR(ENOMEM);
    }
    ...
    av_em_md5_update(s->md5ctx, buf, buf_size);

    return 0;
}


static int flac_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                             const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if (!frame) {
        ...
        av_em_md5_final(s->md5ctx, s->md5sum);
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (!s->flushed) {
#endif
            uint8_t *side_data = av_em_packet_new_side_data(avpkt, AV_PKT_DATA_NEW_EXTRADATA,
    ...
    if (frame_bytes < 0 || frame_bytes > s->max_framesize) {
        ...
        frame_bytes = encode_frame(s);
        if (frame_bytes < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Bad frame count\n");
            return frame_bytes;
        }
    }

    if ((ret = em_alloc_packet2(avctx, avpkt, frame_bytes, 0)) < 0)
        return ret;
    ...
    if ((ret = update_md5_sum(s, frame->data[0])) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error updating MD5 checksum\n");
        return ret;
    }
    ...
    avpkt->pts      = frame->pts;
    avpkt->duration = em_samples_to_time_base(avctx, frame->nb_samples);
...
static av_cold int flac_encode_close(AVEMCodecContext *avctx)
{
    if (avctx->priv_data) {
        FlacEncodeContext *s = avctx->priv_data;
        av_em_freep(&s->md5ctx);
        av_em_freep(&s->md5_buffer);
...
static const AVEMClass flac_encoder_class = {
