 ...
 */
static void count_colors(AVEMCodecContext *avctx, unsigned hits[33],
...
static void select_palette(AVEMCodecContext *avctx, int out_palette[4],
...
static void build_color_map(AVEMCodecContext *avctx, int cmap[],
...
static int encode_dvd_subtitles(AVEMCodecContext *avctx,
                                ...
                                const AVSubtitle *h)
{
    ...
    for (i = 0; i < rects; i++)
        if (h->rects[i]->type != SUBTITLE_BITMAP) {
            av_em_log(avctx, AV_LOG_ERROR, "Bitmap subtitle required\n");
            return AVERROR(EINVAL);
        }
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (rects > 1) {
        ...
        vrect.w = xmax - xmin;
        vrect.h = ymax - ymin;
        if ((ret = av_em_image_check_size(vrect.w, vrect.h, 0, avctx)) < 0)
            return ret;
    ...
    if (rects > 1) {
        if (!(vrect_data = av_em_calloc(vrect.w, vrect.h)))
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Selected palette:");
    for (i = 0; i < 4; i++)
        av_em_log(avctx, AV_LOG_DEBUG, " 0x%06x@@%02x (0x%x,0x%x)",
    ...
    if ((q - outbuf) + vrect.w * vrect.h / 2 + 17 + 21 > outbuf_size) {
        av_em_log(NULL, AV_LOG_ERROR, "dvd_subtitle too big\n");
    ...
    *q++ = 0xff; // terminating command
    ...
    av_em_log(NULL, AV_LOG_DEBUG, "subtitle_packet size=%"PTRDIFF_SPECIFIER"\n", q - outbuf);
    ret = q - outbuf;

fail:
    av_em_free(vrect_data);
    return ret;
}

static int dvdsub_init(AVEMCodecContext *avctx)
{
    DVDSubtitleContext *dvdc = avctx->priv_data;
    ...
    av_em_bprint_init(&extradata, 0, 1);
    if (avctx->width && avctx->height)
        av_em_bprintf(&extradata, "size: %dx%d\n", avctx->width, avctx->height);
    av_em_bprintf(&extradata, "palette:");
    for (i = 0; i < 16; i++)
        av_em_bprintf(&extradata, " %06"PRIx32"%c",
                   dvdc->global_palette[i] & 0xFFFFFF, i < 15 ? ',' : '\n');

    ret = avpriv_em_bprint_to_extradata(avctx, &extradata);
    if (ret < 0)
        return ret;
...
static int dvdsub_encode(AVEMCodecContext *avctx,
...
static const AVEMClass dvdsubenc_class = {
