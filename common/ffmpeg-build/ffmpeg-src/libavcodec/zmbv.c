...
static int zmbv_decode_xor_8(ZmbvContext *c)
{
    ...
    if (src - c->decomp_buf != c->decomp_len)
        av_em_log(c->avctx, AV_LOG_ERROR, "Used %"PTRDIFF_SPECIFIER" of %i bytes\n",
...
static int zmbv_decode_xor_24(ZmbvContext *c)
{
    ...
    if (src - c->decomp_buf != c->decomp_len)
        av_em_log(c->avctx, AV_LOG_ERROR, "Used %i of %i bytes\n",
               src-c->decomp_buf, c->decomp_len);
    return 0;
}
...
static int decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (c->flags & ZMBV_KEYFRAME) {
        ...
        if (hi_ver != 0 || lo_ver != 1) {
            avpriv_em_request_sample(avctx, "Version %i.%i", hi_ver, lo_ver);
            return AVERROR_PATCHWELCOME;
        }
        if (c->bw == 0 || c->bh == 0) {
            avpriv_em_request_sample(avctx, "Block size %ix%i", c->bw, c->bh);
            return AVERROR_PATCHWELCOME;
        }
        if (c->comp != 0 && c->comp != 1) {
            avpriv_em_request_sample(avctx, "Compression type %i", c->comp);
            return AVERROR_PATCHWELCOME;
        }
        ...
        default:
            c->decode_xor = NULL;
            avpriv_em_request_sample(avctx, "Format %i", c->fmt);
            return AVERROR_PATCHWELCOME;
        }
        ...
        c->cur  = av_em_realloc_f(c->cur, avctx->width * avctx->height,  (c->bpp / 8));
        c->prev = av_em_realloc_f(c->prev, avctx->width * avctx->height,  (c->bpp / 8));
    ...
    if (!c->decode_intra) {
        av_em_log(avctx, AV_LOG_ERROR, "Error! Got no format or no keyframe!\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (c->comp == 0) { // uncompressed data
        if (c->decomp_size < len) {
            av_em_log(avctx, AV_LOG_ERROR, "Buffer too small\n");
            return AVERROR_INVALIDDATA;
        }
...
#endif
        case ZMBV_FMT_32BPP:
            av_em_image_copy_plane(out, frame->linesize[0], src, c->stride,
                                c->stride, c->height);
            break;
        default:
            av_em_log(avctx, AV_LOG_ERROR, "Cannot handle format %i\n", c->fmt);
        }
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (c->decomp_size) {
        if (!(c->decomp_buf = av_em_mallocz(c->decomp_size))) {
