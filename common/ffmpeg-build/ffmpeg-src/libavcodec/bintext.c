...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if ((s->flags & BINTEXT_FONT)) {
        ...
        default:
            av_em_log(avctx, AV_LOG_WARNING, "font height %i not supported\n", s->font_height);
            s->font_height = 8;
        case 8:
...
av_unused static void hscroll(AVEMCodecContext *avctx)
{
    XbinContext *s = avctx->priv_data;
 ...
 */
static void draw_char(AVEMCodecContext *avctx, int c, int a)
{
    XbinContext *s = avctx->priv_data;
    if (s->y > avctx->height - s->font_height)
        return;
