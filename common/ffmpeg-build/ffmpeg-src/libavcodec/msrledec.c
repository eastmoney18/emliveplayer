...
static int msrle_decode_pal4(AVEMCodecContext *avctx, AVFrame *pic,
                             GetByteContext *gb)
{
    ...
    while (line >= 0 && pixel_ptr <= avctx->width) {
        ...
        if (rle_code == 0) {
            ...
            if (stream_byte == 0) {
                ...
                stream_byte = bytestream2_get_byte(gb);
                avpriv_em_request_sample(avctx, "Unused stream byte %X", stream_byte);
            } else {
...
static int msrle_decode_8_16_24_32(AVEMCodecContext *avctx, AVFrame *pic,
                                   int depth, GetByteContext *gb)
{
    ...
    while (bytestream2_get_bytes_left(gb) > 0) {
        ...
        if(p1 == 0) { //Escape code
            ...
            if(p2 == 0) { //End-of-line
                ...
                pos += p1;
                if (line < 0 || pos >= width){
                    av_em_log(avctx, AV_LOG_ERROR, "Skip beyond picture bounds\n");
                    return -1;
                }
            ...
            if (output + p2 * (depth >> 3) > output_end) {
                ...
                av_em_log(avctx, AV_LOG_ERROR, "bytestream overrun\n");
                return AVERROR_INVALIDDATA;
            }
    ...
    av_em_log(avctx, AV_LOG_WARNING, "MS RLE warning: no end-of-picture code\n");
    return 0;
}


int ff_msrle_decode(AVEMCodecContext *avctx, AVFrame *pic,
                    int depth, GetByteContext *gb)
{
    switch(depth){
    case  4:
    ...
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unknown depth %d\n", depth);
        return -1;
    }
}
