                 ...
                 AVLFG *rand_state)
{
    ...
    if (numpoints > 24*numCB) {
        ...
        int *temp_points = av_em_malloc_array(dim, (numpoints/8)*sizeof(int));
        if (!temp_points)
                ...
                AVLFG *rand_state)
{
    ...
    int *dist_cb = av_em_malloc_array(numpoints, sizeof(int));
    int *size_part = av_em_malloc_array(numCB, sizeof(int));
    cell *list_buffer = av_em_malloc_array(numpoints, sizeof(cell));
    cell *free_cells;
    ...
    elbg->cells = av_em_malloc_array(numCB, sizeof(cell *));
    elbg->utility = av_em_malloc_array(numCB, sizeof(int));
    elbg->nearest_cb = closest_cb;
    elbg->points = points;
    elbg->utility_inc = av_em_malloc_array(numCB, sizeof(*elbg->utility_inc));
    elbg->scratchbuf = av_em_malloc_array(5*dim, sizeof(int));
...
out:
    av_em_free(dist_cb);
    av_em_free(size_part);
    av_em_free(elbg->utility);
    av_em_free(list_buffer);
    av_em_free(elbg->cells);
    av_em_free(elbg->utility_inc);
    av_em_free(elbg->scratchbuf);
    return ret;
}
