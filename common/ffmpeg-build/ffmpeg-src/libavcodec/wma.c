                                 ...
                                 const CoefVLCTable *vlc_table)
{
    ...
    run_table    = av_em_malloc_array(n, sizeof(uint16_t));
    level_table  = av_em_malloc_array(n, sizeof(uint16_t));
    flevel_table = av_em_malloc_array(n, sizeof(*flevel_table));
    int_table    = av_em_malloc_array(n, sizeof(uint16_t));
    if (!run_table || !level_table || !flevel_table || !int_table) {
        av_em_freep(&run_table);
        av_em_freep(&level_table);
        av_em_freep(&flevel_table);
        av_em_freep(&int_table);
    ...
    *pint_table   = int_table;
    av_em_free(level_table);

    return 0;
}

av_cold int ff_wma_init(AVEMCodecContext *avctx, int flags2)
{
    ...
    if (s->byte_offset_bits + 3 > MIN_CACHE_BITS) {
        av_em_log(avctx, AV_LOG_ERROR, "byte_offset_bits %d is too large\n", s->byte_offset_bits);
...
int ff_wma_end(AVEMCodecContext *avctx)
{
    WMACodecContext *s = avctx->priv_data;
    int i;
    ...
    for (i = 0; i < 2; i++) {
        ...
        av_em_freep(&s->run_table[i]);
        av_em_freep(&s->level_table[i]);
        av_em_freep(&s->int_table[i]);
    }
    av_em_freep(&s->fdsp);
 ...
 */
int ff_wma_run_level_decode(AVEMCodecContext *avctx, GetBitContext *gb,
