...
static int write_frame(AlacEncodeContext *s, AVEMPacket *avpkt,
...
static av_cold int alac_encode_close(AVEMCodecContext *avctx)
{
...
static av_cold int alac_encode_init(AVEMCodecContext *avctx)
{
    AlacEncodeContext *s = avctx->priv_data;
    int ret;
    ...
    avctx->extradata = av_em_mallocz(ALAC_EXTRADATA_SIZE + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!avctx->extradata) {
        ret = AVERROR(ENOMEM);
        goto error;
    }
...
static int alac_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                             const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, 2 * max_frame_size, 0)) < 0)
        return ret;
...
static const AVEMClass alacenc_class = {
    .class_name = "alacenc",
