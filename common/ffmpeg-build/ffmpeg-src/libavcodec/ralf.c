...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->extradata_size < 24 || memcmp(avctx->extradata, "LSD:", 4)) {
        av_em_log(avctx, AV_LOG_ERROR, "Extradata is not groovy, dude\n");
        return AVERROR_INVALIDDATA;
    }

    ctx->version = AV_RB16(avctx->extradata + 4);
    if (ctx->version != 0x103) {
        avpriv_em_request_sample(avctx, "Unknown version %X", ctx->version);
    ...
    if (avctx->channels < 1 || avctx->channels > 2
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Invalid coding parameters %d Hz %d ch\n",
    ...
    if (ctx->max_frame_size > (1 << 20) || !ctx->max_frame_size) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid frame size %d\n",
               ctx->max_frame_size);
    }
...
static int decode_block(AVEMCodecContext *avctx, GetBitContext *gb,
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (ctx->has_pkt) {
        ...
        if (table_bytes + 3 > avpkt->size || avpkt->size > RALF_MAX_PKT_SIZE) {
            av_em_log(avctx, AV_LOG_ERROR, "Wrong packet's breath smells of wrong data!\n");
            return AVERROR_INVALIDDATA;
        }
        if (memcmp(ctx->pkt, avpkt->data, 2 + table_bytes)) {
            av_em_log(avctx, AV_LOG_ERROR, "Wrong packet tails are wrong!\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    if (src_size < 5) {
        av_em_log(avctx, AV_LOG_ERROR, "too short packets are too short!\n");
        return AVERROR_INVALIDDATA;
    }
    table_size  = AV_RB16(src);
    table_bytes = (table_size + 7) >> 3;
    if (src_size < table_bytes + 3) {
        av_em_log(avctx, AV_LOG_ERROR, "short packets are short!\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    for (i = 0; i < ctx->num_blocks; i++) {
        if (bytes_left < ctx->block_size[i]) {
            av_em_log(avctx, AV_LOG_ERROR, "I'm pedaling backwards\n");
            break;
        }
        ...
        if (decode_block(avctx, &gb, samples0 + ctx->sample_offset,
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Sir, I got carsick in your office. Not decoding the rest of packet.\n");
            break;
        }
