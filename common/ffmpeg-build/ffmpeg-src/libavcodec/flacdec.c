...
static void flac_set_bps(FLACContext *s)
{
    ...
    int want32 = av_em_get_bytes_per_sample(req) > 2;
    int planar = av_em_sample_fmt_is_planar(req);
...
static av_cold int flac_decode_init(AVEMCodecContext *avctx)
{
...
static void dump_headers(AVEMCodecContext *avctx, FLACStreaminfo *s)
{
    av_em_log(avctx, AV_LOG_DEBUG, "  Max Blocksize: %d\n", s->max_blocksize);
    av_em_log(avctx, AV_LOG_DEBUG, "  Max Framesize: %d\n", s->max_framesize);
    av_em_log(avctx, AV_LOG_DEBUG, "  Samplerate: %d\n", s->samplerate);
    av_em_log(avctx, AV_LOG_DEBUG, "  Channels: %d\n", s->channels);
    av_em_log(avctx, AV_LOG_DEBUG, "  Bits: %d\n", s->bps);
}
...
static int allocate_buffers(FLACContext *s)
{
    ...
    buf_size = av_em_samples_get_buffer_size(NULL, s->flac_stream_info.channels,
                                          s->flac_stream_info.max_blocksize,
                                          AV_SAMPLE_FMT_S32P, 0);
    if (buf_size < 0)
        return buf_size;

    av_em_fast_malloc(&s->decoded_buffer, &s->decoded_buffer_size, buf_size);
    if (!s->decoded_buffer)
        return AVERROR(ENOMEM);

    ret = av_em_samples_fill_arrays((uint8_t **)s->decoded, NULL,
                                 s->decoded_buffer,
...
static int decode_residuals(FLACContext *s, int32_t *decoded, int pred_order)
{
    ...
    if (method_type > 1) {
        av_em_log(s->avctx, AV_LOG_ERROR, "illegal residual coding method %d\n",
               method_type);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (samples << rice_order != s->blocksize) {
        av_em_log(s->avctx, AV_LOG_ERROR, "invalid rice order: %i blocksize %i\n",
               rice_order, s->blocksize);
        return AVERROR_INVALIDDATA;
    }

    if (pred_order > samples) {
        av_em_log(s->avctx, AV_LOG_ERROR, "invalid predictor order: %i > %i\n",
               pred_order, samples);
        return AVERROR_INVALIDDATA;
    }
                                 ...
                                 int pred_order, int bps)
{
    ...
    case 4:
        ...
        break;
    default:
        av_em_log(s->avctx, AV_LOG_ERROR, "illegal pred order %d\n", pred_order);
        return AVERROR_INVALIDDATA;
    }
                               ...
                               int bps)
{
    ...
    if (coeff_prec == 16) {
        av_em_log(s->avctx, AV_LOG_ERROR, "invalid coeff precision\n");
        return AVERROR_INVALIDDATA;
    }
    qlevel = get_sbits(&s->gb, 5);
    if (qlevel < 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "qlevel %d not supported, maybe buggy stream\n",
               qlevel);
        return AVERROR_INVALIDDATA;
    }
...
static inline int decode_subframe(FLACContext *s, int channel)
{
    ...
    if (get_bits1(&s->gb)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "invalid subframe padding\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (bps > 32) {
        avpriv_em_report_missing_feature(s->avctx, "Decorrelated bit depth > 32");
...
//FIXME use av_log2 for types
    if (type == 0) {
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "invalid coding type\n");
        return AVERROR_INVALIDDATA;
    }
...
static int decode_frame(FLACContext *s)
{
    ...
    if ((ret = ff_flac_decode_frame_header(s->avctx, gb, &fi, 0)) < 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "invalid frame header\n");
        return ret;
    }
    ...
    if (!s->flac_stream_info.bps && !fi.bps) {
        av_em_log(s->avctx, AV_LOG_ERROR, "bps not found in STREAMINFO or frame header\n");
        return AVERROR_INVALIDDATA;
    }
    if (!fi.bps) {
        fi.bps = s->flac_stream_info.bps;
    } else if (s->flac_stream_info.bps && fi.bps != s->flac_stream_info.bps) {
        av_em_log(s->avctx, AV_LOG_ERROR, "switching bps mid-stream is not "
                                       "supported\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (fi.blocksize > s->flac_stream_info.max_blocksize) {
        av_em_log(s->avctx, AV_LOG_ERROR, "blocksize %d > %d\n", fi.blocksize,
               s->flac_stream_info.max_blocksize);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (!s->flac_stream_info.samplerate && !fi.samplerate) {
        av_em_log(s->avctx, AV_LOG_ERROR, "sample rate not found in STREAMINFO"
                                        " or frame header\n");
        return AVERROR_INVALIDDATA;
    }
    if (fi.samplerate == 0)
...
static int flac_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    *got_frame_ptr = 0;
    ...
    if (buf_size > 5 && !memcmp(buf, "\177FLAC", 5)) {
        av_em_log(s->avctx, AV_LOG_DEBUG, "skipping flac header packet 1\n");
        return buf_size;
    }

    if (buf_size > 0 && (*buf & 0x7F) == FLAC_METADATA_TYPE_VORBIS_COMMENT) {
        av_em_log(s->avctx, AV_LOG_DEBUG, "skipping vorbis comment\n");
        return buf_size;
    }
    ...
    if (AV_RB32(buf) == MKBETAG('f','L','a','C')) {
        if (!s->got_streaminfo && (ret = parse_streaminfo(s, buf, buf_size))) {
            av_em_log(s->avctx, AV_LOG_ERROR, "invalid header\n");
            return ret;
        }
    ...
    if ((ret = decode_frame(s)) < 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "decode_frame() failed\n");
        return ret;
    }
    bytes_read = get_bits_count(&s->gb)/8;

    if ((s->avctx->err_recognition & (AV_EF_CRCCHECK|AV_EF_COMPLIANT)) &&
        av_em_crc(av_em_crc_get_table(AV_CRC_16_ANSI),
               0, buf, bytes_read)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "CRC error at PTS %"PRId64"\n", avpkt->pts);
    ...
    if ((ret = em_thread_get_buffer(avctx, &tframe, 0)) < 0)
        return ret;
    ...
    if (bytes_read < buf_size) {
        av_em_log(s->avctx, AV_LOG_DEBUG, "underread: %d orig size: %d\n",
...
static av_cold int flac_decode_close(AVEMCodecContext *avctx)
{
    FLACContext *s = avctx->priv_data;

    av_em_freep(&s->decoded_buffer);
...
static const AVEMClass flac_decoder_class = {
    "FLAC decoder",
