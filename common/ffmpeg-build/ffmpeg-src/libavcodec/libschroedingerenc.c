...
*/
static int set_chroma_format(AVEMCodecContext *avctx)
{
...
static av_cold int libschroedinger_encode_init(AVEMCodecContext *avctx)
{
    SchroEncoderParams *p_schro_params = avctx->priv_data;
    ...
    p_schro_params->frame_size = av_em_image_get_buffer_size(avctx->pix_fmt,
                                                          avctx->width,
                                                          avctx->height, 1);
...
static SchroFrame *libschroedinger_frame_from_data(AVEMCodecContext *avctx,
                                                   const AVFrame *frame)
{
    SchroEncoderParams *p_schro_params = avctx->priv_data;
    ...
    if (in_frame) {
        ...
        if (av_em_frame_copy(in_frame->priv, frame) < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Failed to copy input data\n");
            return NULL;
        }
    }
...
static void libschroedinger_free_frame(void *data)
{
    ...
    av_em_freep(&enc_frame->p_encbuf);
    av_em_free(enc_frame);
}

static int libschroedinger_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                                        const AVFrame *frame, int *got_packet)
{
    ...
    while (go) {
        ...
        case SCHRO_STATE_END_OF_STREAM:
             ...
             */
            if ((err = av_em_reallocp(&p_schro_params->enc_buf,
            ...
            /* Create output frame. */
            p_frame_output = av_em_mallocz(sizeof(FFSchroEncodedFrame));
            if (!p_frame_output)
                return AVERROR(ENOMEM);
            /* Set output data. */
        ...
        default:
            av_em_log(avctx, AV_LOG_ERROR, "Unknown Schro Encoder state\n");
            return -1;
        }
    }
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (last_frame_in_sequence && p_schro_params->enc_buf_size > 0) {
        ...
        av_em_freep(&p_schro_params->enc_buf);
        p_schro_params->enc_buf_size = 0;
    }
...
static int libschroedinger_encode_close(AVEMCodecContext *avctx)
{
    SchroEncoderParams *p_schro_params = avctx->priv_data;
...
static const AVEMClass libschroedinger_class = {
...
AVEMCodec ff_libschroedinger_encoder = {
