...
static int copy_from(IpvideoContext *s, AVFrame *src, AVFrame *dst, int delta_x, int delta_y)
{
    ...
    if (motion_offset < 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "motion offset < 0 (%d)\n", motion_offset);
        return AVERROR_INVALIDDATA;
    } else if (motion_offset > s->upper_motion_limit_offset) {
        av_em_log(s->avctx, AV_LOG_ERROR, "motion offset above limit (%d >= %d)\n",
            motion_offset, s->upper_motion_limit_offset);
        return AVERROR_INVALIDDATA;
    }
    if (!src->data[0]) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Invalid decode type, corrupted header?\n");
...
static int ipvideo_decode_block_opcode_0x6(IpvideoContext *s, AVFrame *frame)
{
    ...
    av_em_log(s->avctx, AV_LOG_ERROR, "Help! Mystery opcode 0x6 seen\n");
...
static int ipvideo_decode_block_opcode_0x7(IpvideoContext *s, AVFrame *frame)
{
    ...
    if (bytestream2_get_bytes_left(&s->stream_ptr) < 4) {
        av_em_log(s->avctx, AV_LOG_ERROR, "too little data for opcode 0x7\n");
        return AVERROR_INVALIDDATA;
    }
...
static int ipvideo_decode_block_opcode_0x8(IpvideoContext *s, AVFrame *frame)
{
    ...
    if (bytestream2_get_bytes_left(&s->stream_ptr) < 12) {
        av_em_log(s->avctx, AV_LOG_ERROR, "too little data for opcode 0x8\n");
        return AVERROR_INVALIDDATA;
    }
...
static int ipvideo_decode_block_opcode_0x9(IpvideoContext *s, AVFrame *frame)
{
    ...
    if (bytestream2_get_bytes_left(&s->stream_ptr) < 8) {
        av_em_log(s->avctx, AV_LOG_ERROR, "too little data for opcode 0x9\n");
        return AVERROR_INVALIDDATA;
    }
...
static int ipvideo_decode_block_opcode_0xA(IpvideoContext *s, AVFrame *frame)
{
    ...
    if (bytestream2_get_bytes_left(&s->stream_ptr) < 16) {
        av_em_log(s->avctx, AV_LOG_ERROR, "too little data for opcode 0xA\n");
        return AVERROR_INVALIDDATA;
    }
...
static int ipvideo_decode_block_opcode_0xD(IpvideoContext *s, AVFrame *frame)
{
    ...
    if (bytestream2_get_bytes_left(&s->stream_ptr) < 4) {
        av_em_log(s->avctx, AV_LOG_ERROR, "too little data for opcode 0xD\n");
        return AVERROR_INVALIDDATA;
    }
...
static void ipvideo_decode_opcodes(IpvideoContext *s, AVFrame *frame)
{
    ...
    for (y = 0; y < s->avctx->height; y += 8) {
        for (x = 0; x < s->avctx->width; x += 8) {
            ...
            if (ret != 0) {
                av_em_log(s->avctx, AV_LOG_ERROR, "decode problem on frame %d, @ block (%d, %d)\n",
...
static av_cold int ipvideo_decode_init(AVEMCodecContext *avctx)
{
    ...
    s->second_last_frame = av_em_frame_alloc();
    if (!s->last_frame || !s->second_last_frame) {
        av_em_frame_free(&s->last_frame);
        av_em_frame_free(&s->second_last_frame);
...
static int ipvideo_decode_frame(AVEMCodecContext *avctx,
                                ...
                                AVEMPacket *avpkt)
{
    ...
    if (av_em_packet_get_side_data(avpkt, AV_PKT_DATA_PARAM_CHANGE, NULL)) {
        av_em_frame_unref(s->last_frame);
        av_em_frame_unref(s->second_last_frame);
    }
    ...
    if (!s->is_16bpp) {
        const uint8_t *pal = av_em_packet_get_side_data(avpkt, AV_PKT_DATA_PALETTE, NULL);
        if (pal) {
    ...
    *got_frame = 1;
    ...
    if ((ret = av_em_frame_ref(s->last_frame, frame)) < 0)
        return ret;
...
static av_cold int ipvideo_decode_end(AVEMCodecContext *avctx)
{
    IpvideoContext *s = avctx->priv_data;
...
AVEMCodec ff_interplay_video_decoder = {
    .name           = "interplayvideo",
