...
static av_noinline void FUNC(hl_decode_mb)(const H264EMContext *h, H264SliceContext *sl)
{
    ...
    if (!SIMPLE && IS_INTRA_PCM(mb_type)) {
        ...
        if (PIXEL_SHIFT) {
            ...
            init_get_bits(&gb, sl->intra_pcm_ptr,
                          em_h264_mb_sizes[h->ps.sps->chroma_format_idc] * bit_depth);
...
static av_noinline void FUNC(hl_decode_mb_444)(const H264EMContext *h, H264SliceContext *sl)
{
    const int mb_x    = sl->mb_x;
    const int mb_y    = sl->mb_y;
    const int mb_xy   = sl->mb_xy;
