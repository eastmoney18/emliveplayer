...
static av_cold int encode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->sample_rate > 48000) {
        av_em_log(avctx, AV_LOG_ERROR, "sample rate is too high: %d > 48kHz\n",
               avctx->sample_rate);
        return AVERROR(EINVAL);
    }
...
static int apply_window_and_mdct(AVEMCodecContext *avctx, const AVFrame *frame)
{
    WMACodecContext *s = avctx->priv_data;
...
static void encode_exp_vlc(WMACodecContext *s, int ch, const int *exp_param)
{
    ...
    while (q < q_end) {
        ...
        put_bits(&s->pb, em_aac_scalefactor_bits[code],
                 em_aac_scalefactor_code[code]);
        /* XXX: use a table */
        q       += *ptr++;
        last_exp = exp;
    }
...
static int encode_superframe(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                             const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, 2 * MAX_CODED_SUPERFRAME_SIZE, 0)) < 0)
        return ret;
    ...
    if (error > 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid input data or requested bitrate too low, cannot encode\n");
        avpkt->size = 0;
        return AVERROR(EINVAL);
    }
