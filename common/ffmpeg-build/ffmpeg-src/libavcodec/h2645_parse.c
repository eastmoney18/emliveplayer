...
int em_h2645_extract_rbsp(const uint8_t *src, int length,
                          H2645NAL *nal)
{
    int i, si, di;
    uint8_t *dst;
...
#define FIND_FIRST_ZERO                                                 \
    ...
    av_em_fast_malloc(&nal->rbsp_buffer, &nal->rbsp_buffer_size,
            ...
            if (src[si + 2] == 3) { // escape
                ...
                if (nal->skipped_bytes_pos) {
                    ...
                    if (nal->skipped_bytes_pos_size < nal->skipped_bytes) {
                        ...
                        av_em_reallocp_array(&nal->skipped_bytes_pos,
...
int em_h2645_packet_split(H2645Packet *pkt, const uint8_t *buf, int length,
    ...
    while (length >= 4) {
        ...
        if (buf >= next_avc) {
            ...
            if (extract_length > length) {
                av_em_log(logctx, AV_LOG_ERROR, "Invalid NAL unit size.\n");
                return AVERROR_INVALIDDATA;
            }
            ...
            while (buf[0] != 0 || buf[1] != 0 || buf[2] != 1) {
                ...
                if (length < 4) {
                    if (pkt->nb_nals > 0) {
                        ...
                        av_em_log(logctx, AV_LOG_ERROR, "No start code is found.\n");
                        return AVERROR_INVALIDDATA;
                    }
        ...
        if (pkt->nals_allocated < pkt->nb_nals + 1) {
            ...
            void *tmp = av_em_realloc_array(pkt->nals, new_size, sizeof(*pkt->nals));
            ...
            nal->skipped_bytes_pos = av_em_malloc_array(nal->skipped_bytes_pos_size, sizeof(*nal->skipped_bytes_pos));
            if (!nal->skipped_bytes_pos)
                return AVERROR(ENOMEM);
        ...
        consumed = em_h2645_extract_rbsp(buf, extract_length, nal);
        if (consumed < 0)
            return consumed;
        ...
        if (ret <= 0 || nal->size <= 0) {
            if (ret < 0) {
                av_em_log(logctx, AV_LOG_ERROR, "Invalid NAL unit %d, skipping.\n",
                       nal->type);
            }
            pkt->nb_nals--;
        }
...
void em_h2645_packet_uninit(H2645Packet *pkt)
{
    int i;
    for (i = 0; i < pkt->nals_allocated; i++) {
        av_em_freep(&pkt->nals[i].rbsp_buffer);
        av_em_freep(&pkt->nals[i].skipped_bytes_pos);
    }
    av_em_freep(&pkt->nals);
