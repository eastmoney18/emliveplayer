...
static SchroBuffer *find_next_parse_unit(SchroParseUnitContext *parse_ctx)
{
    ...
    in_buf = av_em_alloc(next_pu_offset);
    if (!in_buf) {
        av_em_log(parse_ctx, AV_LOG_ERROR, "Unable to allocate input buffer\n");
        return NULL;
    }
...
static av_cold int libschroedinger_decode_init(AVEMCodecContext *avctx)
{
...
static void libschroedinger_handle_first_access_unit(AVEMCodecContext *avctx)
{
    SchroDecoderParams *p_schro_params = avctx->priv_data;
    ...
    if (av_em_image_check_size(p_schro_params->format->width,
                            p_schro_params->format->height, 0, avctx) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid dimensions (%dx%d)\n",
...
static int libschroedinger_decode_frame(AVEMCodecContext *avctx,
    ...
    *got_frame = 0;
    ...
    do {
        if ((enc_buf = find_next_parse_unit(&parse_ctx))) {
            ...
            enc_buf->tag = schro_tag_new(av_em_alloc(sizeof(int64_t)), av_em_free);
            if (!enc_buf->tag->value) {
                av_em_log(avctx, AV_LOG_ERROR, "Unable to allocate SchroTag\n");
        ...
        while (go) {
            ...
            case SCHRO_DECODER_OK:
                ...
                if (frame) {
                    ...
                    framewithpts = av_em_alloc(sizeof(LibSchroFrameContext));
                    if (!framewithpts) {
                        av_em_log(avctx, AV_LOG_ERROR, "Unable to allocate FrameWithPts\n");
                        return AVERROR(ENOMEM);
                    }
    ...
    if (framewithpts && framewithpts->frame) {
        ...
        if ((ret = em_get_buffer(avctx, avframe, 0)) < 0)
            return ret;
...
static av_cold int libschroedinger_decode_close(AVEMCodecContext *avctx)
{
    SchroDecoderParams *p_schro_params = avctx->priv_data;
    /* Free the decoder. */
    schro_decoder_free(p_schro_params->decoder);
    av_em_freep(&p_schro_params->format);
...
static void libschroedinger_flush(AVEMCodecContext *avctx)
{
...
AVEMCodec ff_libschroedinger_decoder = {
