...
static int atrac1_decode_frame(AVEMCodecContext *avctx, void *data,
                               int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (buf_size < 212 * avctx->channels) {
        av_em_log(avctx, AV_LOG_ERROR, "Not enough data to decode!\n");
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int atrac1_decode_end(AVEMCodecContext * avctx)
{
    AT1Ctx *q = avctx->priv_data;
...
static av_cold int atrac1_decode_init(AVEMCodecContext *avctx)
{
    AT1Ctx *q = avctx->priv_data;
    int ret;
    ...
    if (avctx->channels < 1 || avctx->channels > AT1_MAX_CHANNELS) {
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported number of channels: %d\n",
               avctx->channels);
        return AVERROR(EINVAL);
    }

    if (avctx->block_align <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported block align.");
