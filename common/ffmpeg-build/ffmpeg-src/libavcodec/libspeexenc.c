...
typedef struct LibSpeexEncContext {
    AVEMClass *class;             ///< AVEMClass for private options
...
static av_cold void print_enc_params(AVEMCodecContext *avctx,
                                     LibSpeexEncContext *s)
{
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "mode: %s\n", mode_str);
    if (s->header.vbr) {
        av_em_log(avctx, AV_LOG_DEBUG, "rate control: VBR\n");
        av_em_log(avctx, AV_LOG_DEBUG, "  quality: %f\n", s->vbr_quality);
    } else if (s->abr) {
        av_em_log(avctx, AV_LOG_DEBUG, "rate control: ABR\n");
        av_em_log(avctx, AV_LOG_DEBUG, "  bitrate: %"PRId64" bps\n", (int64_t)avctx->bit_rate);
    } else {
        av_em_log(avctx, AV_LOG_DEBUG, "rate control: CBR\n");
        av_em_log(avctx, AV_LOG_DEBUG, "  bitrate: %"PRId64" bps\n", (int64_t)avctx->bit_rate);
    }
    av_em_log(avctx, AV_LOG_DEBUG, "complexity: %d\n",
           avctx->compression_level);
    av_em_log(avctx, AV_LOG_DEBUG, "frame size: %d samples\n",
           avctx->frame_size);
    av_em_log(avctx, AV_LOG_DEBUG, "frames per packet: %d\n",
           s->frames_per_packet);
    av_em_log(avctx, AV_LOG_DEBUG, "packet size: %d\n",
           avctx->frame_size * s->frames_per_packet);
    av_em_log(avctx, AV_LOG_DEBUG, "voice activity detection: %d\n", s->vad);
    av_em_log(avctx, AV_LOG_DEBUG, "discontinuous transmission: %d\n", s->dtx);
}
...
static av_cold int encode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->channels < 1 || avctx->channels > 2) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid channels (%d). Only stereo and "
               "mono are supported\n", avctx->channels);
    ...
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Sample rate of %d Hz is not supported. "
    ...
    if (!s->enc_state) {
        av_em_log(avctx, AV_LOG_ERROR, "Error initializing libspeex\n");
        return -1;
    }
    ...
    if (s->dtx) {
        ...
        if (!(s->abr || s->vad || s->header.vbr))
            av_em_log(avctx, AV_LOG_WARNING, "DTX is not much of use without ABR, VAD or VBR\n");
    }
    ...
    /* allocate extradata */
    avctx->extradata = av_em_alloc(header_size + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!avctx->extradata) {
        speex_header_free(header_data);
        speex_encoder_destroy(s->enc_state);
        av_em_log(avctx, AV_LOG_ERROR, "memory allocation error\n");
...
static int encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                        const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if (s->pkt_frame_count == s->frames_per_packet) {
        s->pkt_frame_count = 0;
        if ((ret = em_alloc_packet2(avctx, avpkt, speex_bits_nbytes(&s->bits), 0)) < 0)
            return ret;
...
static const AVEMClass speex_class = {
    .class_name = "libspeex",
