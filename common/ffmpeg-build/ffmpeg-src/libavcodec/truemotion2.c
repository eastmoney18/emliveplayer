...
static int tm2_read_tree(TM2Context *ctx, uint32_t prefix, int length, TM2Huff *huff)
{
    ...
    if (length > huff->max_bits) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Tree exceeded its given depth (%i)\n",
               huff->max_bits);
    ...
    if (!get_bits1(&ctx->gb)) { /* literal */
        ...
        if (huff->num >= huff->max_num) {
            av_em_log(ctx->avctx, AV_LOG_DEBUG, "Too many literals\n");
            return AVERROR_INVALIDDATA;
        }
...
static int tm2_build_huff_table(TM2Context *ctx, TM2Codes *code)
{
    ...
    if ((huff.val_bits < 1) || (huff.val_bits > 32) ||
        ...
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Incorrect tree parameters - literal "
               "length: %i, max code length: %i\n", huff.val_bits, huff.max_bits);
        return AVERROR_INVALIDDATA;
    }
    if ((huff.nodes <= 0) || (huff.nodes > 0x10000)) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Incorrect number of Huffman tree "
               "nodes: %i\n", huff.nodes);
        return AVERROR_INVALIDDATA;
    }
    /* one-node tree */
    ...
    huff.nums    = av_em_calloc(huff.max_num, sizeof(int));
    huff.bits    = av_em_calloc(huff.max_num, sizeof(uint32_t));
    huff.lens    = av_em_calloc(huff.max_num, sizeof(int));
    ...
    if (huff.num != huff.max_num) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Got less codes than expected: %i of %i\n",
    ...
    if (res >= 0) {
        ...
        if (res < 0)
            av_em_log(ctx->avctx, AV_LOG_ERROR, "Cannot build VLC table\n");
        else {
            code->bits = huff.max_bits;
            code->length = huff.max_num;
            code->recode = av_em_malloc_array(code->length, sizeof(int));
            if (!code->recode) {
                res = AVERROR(ENOMEM);
                goto out;
            }
...
out:
    ...
    av_em_free(huff.nums);
    av_em_free(huff.bits);
    av_em_free(huff.lens);
...
static inline int tm2_read_header(TM2Context *ctx, const uint8_t *buf)
{
    ...
    case TM2_OLD_HEADER_MAGIC:
        avpriv_em_request_sample(ctx->avctx, "Old TM2 header");
        return 0;
    case TM2_NEW_HEADER_MAGIC:
        return 0;
    default:
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Not a TM2 header: 0x%08"PRIX32"\n",
               magic);
        return AVERROR_INVALIDDATA;
    }
}
...
static int tm2_read_deltas(TM2Context *ctx, int stream_id)
{
    ...
    if ((d < 1) || (d > TM2_DELTAS) || (mb < 1)) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Incorrect delta table: %i deltas x %i bits\n", d, mb);
        return AVERROR_INVALIDDATA;
    }
...
static int tm2_read_stream(TM2Context *ctx, const uint8_t *buf, int stream_id, int buf_size)
{
    ...
    if (buf_size < 4) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "not enough space for len left\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (len >= INT_MAX / 4 - 1 || len < 0 || skip > buf_size) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Error, invalid stream size.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if ((toks < 0) || (toks > 0xFFFFFF)) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Incorrect number of tokens: %i\n", toks);
        ret = AVERROR_INVALIDDATA;
        goto end;
    }
    ret = av_em_reallocp_array(&ctx->tokens[stream_id], toks, sizeof(int));
    if (ret < 0) {
    ...
    if (len > 0) {
        ...
        for (i = 0; i < toks; i++) {
            ...
            if (stream_id <= TM2_MOT && ctx->tokens[stream_id][i] >= TM2_DELTAS || ctx->tokens[stream_id][i]<0) {
                av_em_log(ctx->avctx, AV_LOG_ERROR, "Invalid delta token index %d for type %d, n=%d\n",
...
static inline int GET_TOK(TM2Context *ctx,int type)
{
    if (ctx->tok_ptrs[type] >= ctx->tok_lens[type]) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "Read token from stream %i out of bounds (%i>=%i)\n", type, ctx->tok_ptrs[type], ctx->tok_lens[type]);
        return 0;
    }
    if (type <= TM2_MOT) {
        if (ctx->tokens[type][ctx->tok_ptrs[type]] >= TM2_DELTAS) {
            av_em_log(ctx->avctx, AV_LOG_ERROR, "token %d is too large\n", ctx->tokens[type][ctx->tok_ptrs[type]]);
            return 0;
        }
...
static inline void tm2_motion_block(TM2Context *ctx, AVFrame *pic, int bx, int by)
{
    ...
    if (4*bx+mx<0 || 4*by+my<0 || 4*bx+mx+4 > ctx->avctx->width || 4*by+my+4 > ctx->avctx->height) {
        av_em_log(ctx->avctx, AV_LOG_ERROR, "MV out of picture\n");
        return;
    }
...
static int tm2_decode_blocks(TM2Context *ctx, AVFrame *p)
{
    ...
    if (ctx->tok_lens[TM2_TYPE]<bw*bh) {
        av_em_log(ctx->avctx,AV_LOG_ERROR,"Got %i tokens for %i blocks\n",ctx->tok_lens[TM2_TYPE],bw*bh);
        return AVERROR_INVALIDDATA;
    }
    ...
    for (j = 0; j < bh; j++) {
        ...
        for (i = 0; i < bw; i++) {
            ...
            case TM2_MOTION:
                ...
                break;
            default:
                av_em_log(ctx->avctx, AV_LOG_ERROR, "Skipping unknown block type %i\n", type);
            }
        }
    }
                        ...
                        AVEMPacket *avpkt)
{
    ...
    av_em_fast_padded_malloc(&l->buffer, &l->buffer_size, buf_size);
    if (!l->buffer) {
        av_em_log(avctx, AV_LOG_ERROR, "Cannot allocate temporary buffer\n");
        return AVERROR(ENOMEM);
    }

    if ((ret = em_reget_buffer(avctx, p)) < 0)
        return ret;
    ...
    for (i = 0; i < TM2_NUM_STREAMS; i++) {
        if (offset >= buf_size) {
            av_em_log(avctx, AV_LOG_ERROR, "no space for tm2_read_stream\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    l->cur = !l->cur;
    *got_frame      = 1;
    ret = av_em_frame_ref(data, l->pic);
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if ((avctx->width & 3) || (avctx->height & 3)) {
        av_em_log(avctx, AV_LOG_ERROR, "Width and height must be multiple of 4\n");
    ...
    l->pic = av_em_frame_alloc();
    if (!l->pic)
        return AVERROR(ENOMEM);

    em_bswapdsp_init(&l->bdsp);

    l->last  = av_em_malloc_array(w >> 2, 4 * sizeof(*l->last) );
    l->clast = av_em_malloc_array(w >> 2, 4 * sizeof(*l->clast));
    ...
    w += 8;
    h += 8;
    l->Y1_base = av_em_calloc(w * h, sizeof(*l->Y1_base));
    l->Y2_base = av_em_calloc(w * h, sizeof(*l->Y2_base));
    l->y_stride = w;
    w = (w + 1) >> 1;
    h = (h + 1) >> 1;
    l->U1_base = av_em_calloc(w * h, sizeof(*l->U1_base));
    l->V1_base = av_em_calloc(w * h, sizeof(*l->V1_base));
    l->U2_base = av_em_calloc(w * h, sizeof(*l->U2_base));
    l->V2_base = av_em_calloc(w * h, sizeof(*l->V1_base));
    l->uv_stride = w;
    l->cur = 0;
    if (!l->Y1_base || !l->Y2_base || !l->U1_base ||
        !l->V1_base || !l->U2_base || !l->V2_base ||
        !l->last    || !l->clast) {
        av_em_freep(&l->Y1_base);
        av_em_freep(&l->Y2_base);
        av_em_freep(&l->U1_base);
        av_em_freep(&l->U2_base);
        av_em_freep(&l->V1_base);
        av_em_freep(&l->V2_base);
        av_em_freep(&l->last);
        av_em_freep(&l->clast);
        av_em_frame_free(&l->pic);
        return AVERROR(ENOMEM);
    }
...
AVEMCodec ff_truemotion2_decoder = {
    .name           = "truemotion2",
