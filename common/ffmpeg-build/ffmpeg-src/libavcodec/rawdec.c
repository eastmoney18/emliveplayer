...
static const AVEMClass rawdec_class = {
    .class_name = "rawdec",
    .option     = options,
    .version    = LIBAVUTIL_VERSION_INT,
};

static av_cold int raw_init_decoder(AVEMCodecContext *avctx)
{
    RawVideoContext *context = avctx->priv_data;
    const AVPixFmtDescriptor *desc;

    em_bswapdsp_init(&context->bbdsp);

    if (   avctx->codec_tag == MKTAG('r','a','w',' ')
        || avctx->codec_tag == MKTAG('N','O','1','6'))
        avctx->pix_fmt = avpriv_em_find_pix_fmt(avpriv_em_pix_fmt_bps_mov,
                                      avctx->bits_per_coded_sample);
    else if (avctx->codec_tag == MKTAG('W', 'R', 'A', 'W'))
        avctx->pix_fmt = avpriv_em_find_pix_fmt(avpriv_em_pix_fmt_bps_avi,
                                      avctx->bits_per_coded_sample);
    else if (avctx->codec_tag && (avctx->codec_tag & 0xFFFFFF) != MKTAG('B','I','T', 0))
        avctx->pix_fmt = avpriv_em_find_pix_fmt(em_raw_pix_fmt_tags, avctx->codec_tag);
    ...
    if (desc->flags & (AV_PIX_FMT_FLAG_PAL | AV_PIX_FMT_FLAG_PSEUDOPAL)) {
        context->palette = av_em_buffer_alloc(AVPALETTE_SIZE);
        if (!context->palette)
            return AVERROR(ENOMEM);
        if (desc->flags & AV_PIX_FMT_FLAG_PSEUDOPAL)
            avpriv_em_set_systematic_pal2((uint32_t*)context->palette->data, avctx->pix_fmt);
        else {
...
static void flip(AVEMCodecContext *avctx, AVFrame *frame)
{
...
static void name(AVEMCodecContext *avctx, uint8_t * dst, const uint8_t *buf, int buf_size, int packed) \
{ \
    int i; \
    if (!packed) { \
...
static int raw_decode(AVEMCodecContext *avctx, void *data, int *got_frame,
                      AVEMPacket *avpkt)
{
    const AVPixFmtDescriptor *desc;
    RawVideoContext *context       = avctx->priv_data;
    ...
    if (avctx->width <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "width is not set\n");
        return AVERROR_INVALIDDATA;
    }
    if (avctx->height <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "height is not set\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "PACKET SIZE: %d, STRIDE: %d\n", avpkt->size, stride);

    if (stride == 0 || avpkt->size < stride * avctx->height) {
        av_em_log(avctx, AV_LOG_ERROR, "Packet too small (%d)\n", avpkt->size);
        return AVERROR_INVALIDDATA;
    }
    ...
    if ((avctx->bits_per_coded_sample == 8 || avctx->bits_per_coded_sample == 4
        ...
        if (context->is_mono) {
            ...
            context->frame_size = av_em_image_get_buffer_size(avctx->pix_fmt,
        ...
        context->is_lt_16bpp = av_em_get_bits_per_pixel(desc) == 16 && avctx->bits_per_coded_sample && avctx->bits_per_coded_sample < 16;
        context->frame_size = av_em_image_get_buffer_size(avctx->pix_fmt, avctx->width,
                                                       avctx->height, 1);
    }
    ...
    res = em_decode_frame_props(avctx, frame);
    if (res < 0)
        return res;

    av_em_frame_set_pkt_pos     (frame, avctx->internal->pkt->pos);
    av_em_frame_set_pkt_duration(frame, avctx->internal->pkt->duration);
    ...
    if ((res = av_em_image_check_size(avctx->width, avctx->height, 0, avctx)) < 0)
        return res;

    if (need_copy)
        frame->buf[0] = av_em_buffer_alloc(FFMAX(context->frame_size, buf_size));
    else
        frame->buf[0] = av_em_buffer_ref(avpkt->buf);
    if (!frame->buf[0])
        return AVERROR(ENOMEM);
    ...
    if (context->is_1_2_4_8_bpp) {
        ...
        if (packed && swap) {
            av_em_fast_padded_malloc(&context->bitstream_buf, &context->bitstream_buf_size, buf_size);
    ...
    if (buf_size < len && ((avctx->codec_tag & 0xFFFFFF) != MKTAG('B','I','T', 0) || !need_copy)) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid buffer size, packet size %d < expected frame_size %d\n", buf_size, len);
        av_em_buffer_unref(&frame->buf[0]);
        return AVERROR(EINVAL);
    }

    if ((res = av_em_image_fill_arrays(frame->data, frame->linesize,
                                    buf, avctx->pix_fmt,
    ...
    if (avctx->pix_fmt == AV_PIX_FMT_PAL8) {
        const uint8_t *pal = av_em_packet_get_side_data(avpkt, AV_PKT_DATA_PALETTE,
                                                     NULL);
        if (pal) {
            av_em_buffer_unref(&context->palette);
            context->palette = av_em_buffer_alloc(AVPALETTE_SIZE);
            if (!context->palette) {
                av_em_buffer_unref(&frame->buf[0]);
                return AVERROR(ENOMEM);
    ...
    if ((avctx->pix_fmt == AV_PIX_FMT_PAL8 && buf_size < context->frame_size) ||
        ...
        frame->buf[1]  = av_em_buffer_ref(context->palette);
        if (!frame->buf[1]) {
            av_em_buffer_unref(&frame->buf[0]);
            return AVERROR(ENOMEM);
        }
...
static av_cold int raw_close_decoder(AVEMCodecContext *avctx)
{
    RawVideoContext *context = avctx->priv_data;
