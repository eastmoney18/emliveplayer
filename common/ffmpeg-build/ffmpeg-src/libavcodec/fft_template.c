...
av_cold int ff_fft_init(FFTContext *s, int nbits, int inverse)
{
    ...
    if (nbits <= 16) {
        s->revtab = av_em_alloc(n * sizeof(uint16_t));
        if (!s->revtab)
            goto fail;
    } else {
        s->revtab32 = av_em_alloc(n * sizeof(uint32_t));
        if (!s->revtab32)
            goto fail;
    }
    s->tmp_buf = av_em_alloc(n * sizeof(FFTComplex));
    if (!s->tmp_buf)
        goto fail;
    s->inverse = inverse;
    ...
    return 0;
 fail:
    av_em_freep(&s->revtab);
    av_em_freep(&s->revtab32);
    av_em_freep(&s->tmp_buf);
    return -1;
}
