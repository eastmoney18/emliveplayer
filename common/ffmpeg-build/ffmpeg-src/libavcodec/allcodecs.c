...
#define REGISTER_HWACCEL(X, x)                                          \
    {                                                                   \
        extern AVHWAccel em_##x##_hwaccel;                              \
        if (CONFIG_##X##_HWACCEL)                                       \
            av_em_register_hwaccel(&em_##x##_hwaccel);                     \
    }

#define REGISTER_ENCODER(X, x)                                          \
    {                                                                   \
        extern AVEMCodec em_##x##_encoder;                                \
        if (CONFIG_##X##_ENCODER)                                       \
            avcodec_em_register(&em_##x##_encoder);                        \
    }

#define REGISTER_DECODER(X, x)                                          \
    {                                                                   \
        extern AVEMCodec em_##x##_decoder;                                \
        if (CONFIG_##X##_DECODER)                                       \
            avcodec_em_register(&em_##x##_decoder);                        \
    }
...
#define REGISTER_PARSER(X, x)                                           \
    {                                                                   \
        extern AVEMCodecParser em_##x##_parser;                           \
        if (CONFIG_##X##_PARSER)                                        \
            av_em_register_codec_parser(&em_##x##_parser);                 \
    }
