...
int ff_qsv_codec_id_to_mfx(enum AVEMCodecID codec_id)
{
    switch (codec_id) {
...
static int ff_qsv_set_display_handle(AVEMCodecContext *avctx, QSVSession *qs)
{
 ...
 */
int ff_qsv_init_internal_session(AVEMCodecContext *avctx, QSVSession *qs,
                                 const char *load_plugins)
{
    ...
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Error initializing an internal MFX session\n");
        return ff_qsv_error(ret);
    }
    ...
    if (load_plugins && *load_plugins) {
        while (*load_plugins) {
            ...
            char *plugin = av_em_get_token(&load_plugins, ":");
            if (!plugin)
                return AVERROR(ENOMEM);
            if (strlen(plugin) != 2 * sizeof(uid.Data)) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid plugin UID length\n");
                err = AVERROR(EINVAL);
            ...
            for (i = 0; i < sizeof(uid.Data); i++) {
                ...
                if (err != 1) {
                    av_em_log(avctx, AV_LOG_ERROR, "Invalid plugin UID\n");
                    err = AVERROR(EINVAL);
                    goto load_plugin_fail;
                }
            ...
            if (ret < 0) {
                av_em_log(avctx, AV_LOG_ERROR, "Could not load the requested plugin: %s\n",
                       plugin);
