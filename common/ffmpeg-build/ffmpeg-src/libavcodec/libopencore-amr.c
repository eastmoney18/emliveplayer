...
static int amr_decode_fix_avctx(AVEMCodecContext *avctx)
{
    ...
    if (avctx->channels > 1) {
        avpriv_em_report_missing_feature(avctx, "multi-channel AMR");
...
static av_cold int amr_nb_decode_init(AVEMCodecContext *avctx)
{
    ...
    if (!s->dec_state) {
        av_em_log(avctx, AV_LOG_ERROR, "Decoder_Interface_init error\n");
        return -1;
    }
...
static av_cold int amr_nb_decode_close(AVEMCodecContext *avctx)
{
    AMRContext *s = avctx->priv_data;
...
static int amr_nb_decode_frame(AVEMCodecContext *avctx, void *data,
                               int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (packet_size > buf_size) {
        av_em_log(avctx, AV_LOG_ERROR, "AMR frame too short (%d, should be %d)\n",
...
AVEMCodec ff_libopencore_amrnb_decoder = {
...
static int get_bitrate_mode(int bitrate, void *log_ctx)
{
    ...
    for (i = 0; i < 8; i++)
        av_em_strlcatf(log_buf, sizeof(log_buf), "%.2fk, ", rates[i].rate    / 1000.f);
    av_em_strlcatf(log_buf, sizeof(log_buf), "using %.2fk", rates[best].rate / 1000.f);
    av_em_log(log_ctx, AV_LOG_WARNING, "%s\n", log_buf);
...
static const AVEMClass amrnb_class = {
    "libopencore_amrnb", av_em_default_item_name, options, LIBAVUTIL_VERSION_INT
};

static av_cold int amr_nb_encode_init(AVEMCodecContext *avctx)
{
    AMRContext *s = avctx->priv_data;

    if (avctx->sample_rate != 8000 && avctx->strict_std_compliance > FF_COMPLIANCE_UNOFFICIAL) {
        av_em_log(avctx, AV_LOG_ERROR, "Only 8000Hz sample rate supported\n");
        return AVERROR(ENOSYS);
    }

    if (avctx->channels != 1) {
        av_em_log(avctx, AV_LOG_ERROR, "Only mono supported\n");
        return AVERROR(ENOSYS);
    }
    ...
    if (!s->enc_state) {
        av_em_log(avctx, AV_LOG_ERROR, "Encoder_Interface_init error\n");
        return -1;
    }
...
static av_cold int amr_nb_encode_close(AVEMCodecContext *avctx)
{
    AMRContext *s = avctx->priv_data;
...
static int amr_nb_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                               const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, 32, 0)) < 0)
        return ret;

    if (frame) {
        if (frame->nb_samples < avctx->frame_size) {
            flush_buf = av_em_mallocz_array(avctx->frame_size, sizeof(*flush_buf));
            if (!flush_buf)
...
AVEMCodec ff_libopencore_amrnb_encoder = {
    .name           = "libopencore_amrnb",
    .long_name      = NULL_IF_CONFIG_SMALL("OpenCORE AMR-NB (Adaptive Multi-Rate Narrow-Band)"),
    .type           = AVMEDIA_TYPE_AUDIO,
    .id             = AV_CODEC_ID_AMR_NB,
...
static av_cold int amr_wb_decode_init(AVEMCodecContext *avctx)
{
...
static int amr_wb_decode_frame(AVEMCodecContext *avctx, void *data,
                               int *got_frame_ptr, AVEMPacket *avpkt)
{
    AVFrame *frame     = data;
    const uint8_t *buf = avpkt->data;
    ...
    if (!packet_size) {
        av_em_log(avctx, AV_LOG_ERROR, "amr packet_size invalid\n");
        return AVERROR_INVALIDDATA;
    }
...
static int amr_wb_decode_close(AVEMCodecContext *avctx)
{
    AMRWBContext *s = avctx->priv_data;
...
AVEMCodec ff_libopencore_amrwb_decoder = {
