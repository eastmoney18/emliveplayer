...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    em_blockdsp_init(&ctx->bdsp, avctx);
...
static int decode_frame_header(ProresContext *ctx, const uint8_t *buf,
                               const int data_size, AVEMCodecContext *avctx)
{
    ...
    if (hdr_size > data_size) {
        av_em_log(avctx, AV_LOG_ERROR, "error, wrong header size\n");
    ...
    if (version > 1) {
        av_em_log(avctx, AV_LOG_ERROR, "unsupported version: %d\n", version);
    ...
    if (width != avctx->width || height != avctx->height) {
        av_em_log(avctx, AV_LOG_ERROR, "picture resolution change: %dx%d -> %dx%d\n",
    ...
    if (ctx->alpha_info > 2) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid alpha mode %d\n", ctx->alpha_info);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (flags & 2) {
        if(buf + data_size - ptr < 64) {
            av_em_log(avctx, AV_LOG_ERROR, "Header truncated\n");
            return AVERROR_INVALIDDATA;
        }
...
static int decode_picture_header(AVEMCodecContext *avctx, const uint8_t *buf, const int buf_size)
{
    ProresContext *ctx = avctx->priv_data;
    ...
    if (hdr_size < 8 || hdr_size > buf_size) {
        av_em_log(avctx, AV_LOG_ERROR, "error, wrong picture header size\n");
        return AVERROR_INVALIDDATA;
    }

    pic_data_size = AV_RB32(buf + 1);
    if (pic_data_size > buf_size) {
        av_em_log(avctx, AV_LOG_ERROR, "error, wrong picture data size\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (log2_slice_mb_width > 3 || log2_slice_mb_height) {
        av_em_log(avctx, AV_LOG_ERROR, "unsupported slice resolution: %dx%d\n",
    ...
    if (ctx->slice_count != slice_count || !ctx->slices) {
        av_em_freep(&ctx->slices);
        ctx->slice_count = 0;
        ctx->slices = av_em_mallocz_array(slice_count, sizeof(*ctx->slices));
        if (!ctx->slices)
    ...
    if (hdr_size + slice_count*2 > buf_size) {
        av_em_log(avctx, AV_LOG_ERROR, "error, wrong slice count\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    for (i = 0; i < slice_count; i++) {
        ...
        if (slice->data_size < 6) {
            av_em_log(avctx, AV_LOG_ERROR, "error, wrong slice data size\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        if (data_ptr > buf + buf_size) {
            av_em_log(avctx, AV_LOG_ERROR, "error, slice out of bounds\n");
            return AVERROR_INVALIDDATA;
        }
    }

    if (mb_x || mb_y != ctx->mb_height) {
        av_em_log(avctx, AV_LOG_ERROR, "error wrong mb count y %d h %d\n",
               mb_y, ctx->mb_height);
        return AVERROR_INVALIDDATA;
    }
...
static av_always_inline int decode_ac_coeffs(AVEMCodecContext *avctx, GetBitContext *gb,
                                             int16_t *out, int blocks_per_slice)
{
    ...
    for (pos = block_mask;;) {
        ...
        pos += run + 1;
        if (pos >= max_coeffs) {
            av_em_log(avctx, AV_LOG_ERROR, "ac tex damaged %d, %d\n", pos, max_coeffs);
            return AVERROR_INVALIDDATA;
        }
...
static int decode_slice_luma(AVEMCodecContext *avctx, SliceContext *slice,
...
static int decode_slice_chroma(AVEMCodecContext *avctx, SliceContext *slice,
                               uint16_t *dst, int dst_stride,
                               const uint8_t *buf, unsigned buf_size,
...
static int decode_slice_thread(AVEMCodecContext *avctx, void *arg, int jobnr, int threadnr)
{
    ProresContext *ctx = avctx->priv_data;
    ...
    slice->ret = -1;
    //av_em_log(avctx, AV_LOG_INFO, "slice %d mb width %d mb x %d y %d\n",
    ...
    if (y_data_size < 0 || u_data_size < 0 || v_data_size < 0
        ...
        av_em_log(avctx, AV_LOG_ERROR, "invalid plane data size\n");
        return AVERROR_INVALIDDATA;
    }
...
static int decode_picture(AVEMCodecContext *avctx)
{
    ProresContext *ctx = avctx->priv_data;
    int i;
                        ...
                        AVEMPacket *avpkt)
{
 ...
 decode_picture:
    ...
    if (pic_size < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "error decoding picture header\n");
        return pic_size;
    }

    if ((ret = decode_picture(avctx)) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "error decoding picture\n");
        return ret;
    }
