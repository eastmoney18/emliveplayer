                         ...
                         IDWTELEM *base_buffer)
{
    ...
    buf->line        = av_em_mallocz_array(line_count, sizeof(IDWTELEM *));
    if (!buf->line)
        return AVERROR(ENOMEM);
    buf->data_stack  = av_em_malloc_array(max_allocated_lines, sizeof(IDWTELEM *));
    if (!buf->data_stack) {
        av_em_freep(&buf->line);
        return AVERROR(ENOMEM);
    }

    for (i = 0; i < max_allocated_lines; i++) {
        buf->data_stack[i] = av_em_malloc_array(line_width, sizeof(IDWTELEM));
        if (!buf->data_stack[i]) {
            for (i--; i >=0; i--)
                av_em_freep(&buf->data_stack[i]);
            av_em_freep(&buf->data_stack);
            av_em_freep(&buf->line);
            return AVERROR(ENOMEM);
        }
    }
