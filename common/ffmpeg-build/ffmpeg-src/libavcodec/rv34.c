 ...
 * @todo Maybe replace with em_h263_decode_mba() ?
 */
...
static int rv34_decode_intra_mb_header(RV34DecContext *r, int8_t *intra_types)
{
    ...
    if(r->is16){
        ...
        if(!r->rv30){
            if(!get_bits1(gb))
                av_em_log(s->avctx, AV_LOG_ERROR, "Need DQUANT\n");
        }
                          ...
                          h264_chroma_mc_func (*chroma_mc))
{
    ...
    if (HAVE_THREADS && (s->avctx->active_thread_type & FF_THREAD_FRAME)) {
        ...
        em_thread_await_progress(f, mb_row, 0);
    }
...
static int rv34_decode_mv(RV34DecContext *r, int block_type)
{
    ...
    case RV34_MB_B_DIRECT:
        ...
        if (HAVE_THREADS && (s->avctx->active_thread_type & FF_THREAD_FRAME))
            em_thread_await_progress(&s->next_picture_ptr->tf, FFMAX(0, s->mb_y-1), 0);
...
{
    av_em_freep(&r->intra_types_hist);
    r->intra_types = NULL;
    av_em_freep(&r->tmp_b_block_base);
    av_em_freep(&r->mb_type);
    av_em_freep(&r->cbp_luma);
    av_em_freep(&r->cbp_chroma);
    av_em_freep(&r->deblock_coefs);
}
...
static int rv34_decoder_alloc(RV34DecContext *r)
{
    ...
    r->cbp_chroma       = av_em_mallocz(r->s.mb_stride * r->s.mb_height *
                                    sizeof(*r->cbp_chroma));
    r->cbp_luma         = av_em_mallocz(r->s.mb_stride * r->s.mb_height *
                                    sizeof(*r->cbp_luma));
    r->deblock_coefs    = av_em_mallocz(r->s.mb_stride * r->s.mb_height *
                                    sizeof(*r->deblock_coefs));
    r->intra_types_hist = av_em_alloc(r->intra_types_stride * 4 * 2 *
                                    sizeof(*r->intra_types_hist));
    r->mb_type          = av_em_mallocz(r->s.mb_stride * r->s.mb_height *
                                     sizeof(*r->mb_type));
...
static int rv34_decode_slice(RV34DecContext *r, int end, const uint8_t* buf, int buf_size)
{
    ...
    if(res < 0){
        av_em_log(s->avctx, AV_LOG_ERROR, "Incorrect or unknown slice header\n");
        return -1;
    }

    slice_type = r->si.type ? r->si.type : AV_PICTURE_TYPE_I;
    if (slice_type != s->pict_type) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Slice type mismatch\n");
        return AVERROR_INVALIDDATA;
    }
    if (s->width != r->si.width || s->height != r->si.height) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Size mismatch\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if(r->si.start != mb_pos){
        av_em_log(s->avctx, AV_LOG_ERROR, "Slice indicates MB offset %d, got %d\n", r->si.start, mb_pos);
    ...
    while(!check_slice_end(r, s)) {
        ...
        if(res < 0){
            ff_em_er_add_slice(&s->er, s->resync_mb_x, s->resync_mb_y, s->mb_x-1, s->mb_y, ER_MB_ERROR);
            return -1;
        }
        if (++s->mb_x == s->mb_width) {
            ...
            if (HAVE_THREADS && (s->avctx->active_thread_type & FF_THREAD_FRAME))
                em_thread_report_progress(&s->current_picture_ptr->tf,
                                          s->mb_y - 2, 0);
    ...
    ff_em_er_add_slice(&s->er, s->resync_mb_x, s->resync_mb_y, s->mb_x-1, s->mb_y, ER_MB_END);
 ...
 * Initialize decoder.
 */
av_cold int ff_rv34_decode_init(AVEMCodecContext *avctx)
{
    ...
    em_h264_pred_init(&r->h, AV_CODEC_ID_RV40, 8, 1);
...
int ff_rv34_decode_init_thread_copy(AVEMCodecContext *avctx)
{
    int err;
    RV34DecContext *r = avctx->priv_data;
...
int ff_rv34_decode_update_thread_context(AVEMCodecContext *dst, const AVEMCodecContext *src)
{
...
static int get_slice_offset(AVEMCodecContext *avctx, const uint8_t *buf, int n)
{
    if(avctx->slice_count) return avctx->slice_offset[n];
    else                   return AV_RL32(buf + n*8 - 4) == 1 ? AV_RL32(buf + n*8) :  AV_RB32(buf + n*8);
}

static int finish_frame(AVEMCodecContext *avctx, AVFrame *pict)
{
    RV34DecContext *r = avctx->priv_data;
    MpegEncContext *s = &r->s;
...
static AVEMRational update_sar(int old_w, int old_h, AVEMRational sar, int new_w, int new_h)
{
    // attempt to keep aspect during typical resolution switches
    if (!sar.num)
        sar = (AVEMRational){1, 1};

    sar = av_em_mul_q(sar, (AVEMRational){new_h * old_w, new_w * old_h});
    return sar;
}

int ff_rv34_decode_frame(AVEMCodecContext *avctx,
                            ...
                            AVEMPacket *avpkt)
{
    ...
    if(get_slice_offset(avctx, slices_hdr, 0) < 0 ||
       get_slice_offset(avctx, slices_hdr, 0) > buf_size){
        av_em_log(avctx, AV_LOG_ERROR, "Slice offset is invalid\n");
        return AVERROR_INVALIDDATA;
    }
    init_get_bits(&s->gb, buf+get_slice_offset(avctx, slices_hdr, 0), (buf_size-get_slice_offset(avctx, slices_hdr, 0))*8);
    if(r->parse_slice_header(r, &r->s.gb, &si) < 0 || si.start){
        av_em_log(avctx, AV_LOG_ERROR, "First slice header is incorrect\n");
        return AVERROR_INVALIDDATA;
    }
    if ((!s->last_picture_ptr || !s->last_picture_ptr->f->data[0]) &&
        si.type == AV_PICTURE_TYPE_B) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid decoder state: B-frame without "
               "reference data.\n");
        faulty_b = 1;
    }
    ...
    if (si.start == 0) {
        if (s->mb_num_left > 0 && s->current_picture_ptr) {
            av_em_log(avctx, AV_LOG_ERROR, "New frame but still %d MB left.\n",
                   s->mb_num_left);
            ff_em_er_frame_end(&s->er);
            ff_mpv_frame_end(s);
        }
        ...
        if (s->width != si.width || s->height != si.height) {
            ...
            av_em_log(s->avctx, AV_LOG_WARNING, "Changing dimensions to %dx%d\n",
                   si.width, si.height);

            if (av_em_image_check_size(si.width, si.height, 0, s->avctx))
                return AVERROR_INVALIDDATA;
            ...
            err = em_set_dimensions(s->avctx, s->width, s->height);
            if (err < 0)
                return err;
        ...
        if (!r->tmp_b_block_base) {
            ...
            r->tmp_b_block_base = av_em_alloc(s->linesize * 48);
            for (i = 0; i < 2; i++)
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "Decoder needs full frames in frame "
    ...
    for(i = 0; i < slice_count; i++){
        ...
        if (size < 0 || size > buf_size - offset) {
            av_em_log(avctx, AV_LOG_ERROR, "Slice size is invalid\n");
            break;
        }
    ...
    if (s->current_picture_ptr) {
        if (last) {
            ...
            *got_picture_ptr = ret;
                   ...
                   (s->avctx->active_thread_type & FF_THREAD_FRAME)) {
            av_em_log(avctx, AV_LOG_INFO, "marking unfished frame as finished\n");
...
av_cold int ff_rv34_decode_end(AVEMCodecContext *avctx)
{
    RV34DecContext *r = avctx->priv_data;
