...
DECLARE_ALIGNED(16, const int8_t, em_hevc_epel_filters[7][4]) = {
    { -2, 58, 10, -2},
    { -4, 54, 16, -2},
    { -6, 46, 28, -4},
    { -4, 36, 36, -4},
...
DECLARE_ALIGNED(16, const int8_t, em_hevc_qpel_filters[3][16]) = {
...
void em_hevc_dsp_init(HEVCDSPContext *hevcdsp, int bit_depth)
{
#undef FUNC
...
int i = 0;
    ...
    if (ARCH_X86)
        em_hevc_dsp_init_x86(hevcdsp, bit_depth);
    if (ARCH_ARM)
        em_hevcdsp_init_arm(hevcdsp, bit_depth);
    if (ARCH_MIPS)
        em_hevc_dsp_init_mips(hevcdsp, bit_depth);
}
