...
static av_cold int dvdsub_parse_init(AVEMCodecParserContext *s)
{
    return 0;
}

static int dvdsub_parse(AVEMCodecParserContext *s,
                        ...
                        const uint8_t *buf, int buf_size)
{
    ...
    if (pc->packet_index == 0) {
        if (buf_size < 2 || AV_RB16(buf) && buf_size < 6) {
            if (buf_size)
                av_em_log(avctx, AV_LOG_DEBUG, "Parser input %d too small\n", buf_size);
            return buf_size;
        }
        pc->packet_len = AV_RB16(buf);
        if (pc->packet_len == 0) /* HD-DVD subpicture packet */
            pc->packet_len = AV_RB32(buf+2);
        av_em_freep(&pc->packet);
        pc->packet = av_em_alloc(pc->packet_len);
    }
    if (pc->packet) {
...
static av_cold void dvdsub_parse_close(AVEMCodecParserContext *s)
{
    DVDSubParseContext *pc = s->priv_data;
    av_em_freep(&pc->packet);
}

AVEMCodecParser ff_dvdsub_parser = {
