...
static int pix_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                            AVEMPacket *avpkt)
{
    AVFrame *frame = data;
    ...
    if (magic[0] != 0x12 ||
        ...
        magic[2] != 0x02 ||
        magic[3] != 0x02) {
        av_em_log(avctx, AV_LOG_ERROR, "Not a BRender PIX file.\n");
        return AVERROR_INVALIDDATA;
    }

    chunk_type = bytestream2_get_be32(&gb);
    if (chunk_type != HEADER1_CHUNK && chunk_type != HEADER2_CHUNK) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid chunk type %d.\n", chunk_type);
        return AVERROR_INVALIDDATA;
    }

    ret = pix_decode_header(&hdr, &gb);
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid header length.\n");
        return ret;
    }
    switch (hdr.format) {
    case 3:
    ...
    case 18:
        ...
        break;
    default:
        avpriv_em_request_sample(avctx, "Format %d", hdr.format);
        return AVERROR_PATCHWELCOME;
    }

    if ((ret = em_set_dimensions(avctx, hdr.width, hdr.height)) < 0)
        return ret;
    ...
    if (avctx->pix_fmt == AV_PIX_FMT_PAL8 &&
        ...
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid palette header length.\n");
            return ret;
        }
        if (palhdr.format != 7)
            avpriv_em_request_sample(avctx, "Palette not in RGB format");
        ...
        if (chunk_type != IMAGE_DATA_CHUNK || data_len != 1032 ||
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Invalid palette data.\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    if (chunk_type != IMAGE_DATA_CHUNK || data_len != bytes_left ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Invalid image data.\n");
        return AVERROR_INVALIDDATA;
    }
...
AVEMCodec ff_brender_pix_decoder = {
    .name         = "brender_pix",
