...
static int insert_text(AVEMBPrint *dst, const char *in, const char *arg)
{
    av_em_bprintf(dst, "%s", arg);
    return 0;
}

static int insert_datetime(AVEMBPrint *dst, const char *in, const char *arg)
{
    char buf[16] = {0};
    time_t now = time(0);
    struct tm ltime;
    ...
    if (strftime(buf, sizeof(buf), arg, &ltime))
        av_em_bprintf(dst, "%s", buf);
    return 0;
}

static int insert_color(AVEMBPrint *dst, const char *in, const char *arg)
{
    return 1; // skip id
}

static int insert_font(AVEMBPrint *dst, const char *in, const char *arg)
{
    return 1; // skip id
}
...
static const struct {
    const char *from;
    const char *arg;
    int (*func)(AVEMBPrint *dst, const char *in, const char *arg);
} ass_codes_map[] = {
...
static void jacosub_to_ass(AVEMCodecContext *avctx, AVEMBPrint *dst, const char *src)
{
    ...
    if (valign || halign) {
        ...
        case ALIGN_VB | ALIGN_JL: av_em_bprintf(dst, "{\\an1}"); break; // bottom left
        case ALIGN_VB | ALIGN_JC: av_em_bprintf(dst, "{\\an2}"); break; // bottom center
        case ALIGN_VB | ALIGN_JR: av_em_bprintf(dst, "{\\an3}"); break; // bottom right
        case ALIGN_VM | ALIGN_JL: av_em_bprintf(dst, "{\\an4}"); break; // middle left
        case ALIGN_VM | ALIGN_JC: av_em_bprintf(dst, "{\\an5}"); break; // middle center
        case ALIGN_VM | ALIGN_JR: av_em_bprintf(dst, "{\\an6}"); break; // middle right
        case ALIGN_VT | ALIGN_JL: av_em_bprintf(dst, "{\\an7}"); break; // top left
        case ALIGN_VT | ALIGN_JC: av_em_bprintf(dst, "{\\an8}"); break; // top center
        case ALIGN_VT | ALIGN_JR: av_em_bprintf(dst, "{\\an9}"); break; // top right
        }
    }
    ...
    while (*src && *src != '\n') {
        ...
        if (i == FF_ARRAY_ELEMS(ass_codes_map))
            av_em_bprintf(dst, "%c", *src++);
    }
}

static int jacosub_decode_frame(AVEMCodecContext *avctx,
                                void *data, int *got_sub_ptr, AVEMPacket *avpkt)
{
    ...
    if (*ptr) {
        ...
        av_em_bprint_init(&buffer, JSS_MAX_LINESIZE, JSS_MAX_LINESIZE);
        jacosub_to_ass(avctx, &buffer, ptr);
        ret = ff_ass_add_rect(sub, buffer.str, s->readorder++, 0, NULL, NULL);
        av_em_bprint_finalize(&buffer, NULL);
        if (ret < 0)
            return ret;
    }
