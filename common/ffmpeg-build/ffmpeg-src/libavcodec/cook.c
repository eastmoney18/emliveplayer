...
static av_cold int init_cook_vlc_tables(COOKContext *q)
{
    ...
    av_em_log(q->avctx, AV_LOG_DEBUG, "sqvh VLC init\n");
    for (i = 0; i < 7; i++) {
    ...
    for (i = 0; i < q->num_subpackets; i++) {
        if (q->subpacket[i].joint_stereo == 1) {
            ...
            av_em_log(q->avctx, AV_LOG_DEBUG, "subpacket %i Joint-stereo VLC used.\n", i);
        }
    }

    av_em_log(q->avctx, AV_LOG_DEBUG, "VLC tables initialized.\n");
    return result;
}
...
static av_cold int init_cook_mlt(COOKContext *q)
{
    ...
    if ((q->mlt_window = av_em_malloc_array(mlt_size, sizeof(*q->mlt_window))) == 0)
    ...
    if ((ret = ff_mdct_init(&q->mdct_ctx, av_log2(mlt_size) + 1, 1, 1.0 / 32768.0))) {
        av_em_freep(&q->mlt_window);
        return ret;
    }
    av_em_log(q->avctx, AV_LOG_DEBUG, "MDCT initialized, order = %d.\n",
           av_log2(mlt_size) + 1);
...
static av_cold int cook_decode_close(AVEMCodecContext *avctx)
{
    int i;
    COOKContext *q = avctx->priv_data;
    av_em_log(avctx, AV_LOG_DEBUG, "Deallocating memory.\n");

    /* Free allocated memory buffers. */
    av_em_freep(&q->mlt_window);
    av_em_freep(&q->decoded_bytes_buffer);
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Memory deallocated.\n");
                           ...
                           int *quant_index_table)
{
    ...
    for (i = 1; i < p->total_subbands; i++) {
        ...
        if (quant_index_table[i] > 63 || quant_index_table[i] < -63) {
            av_em_log(q->avctx, AV_LOG_ERROR,
    ...
    if (vlc)
        ...
        for (i = 0; i < length; i++) {
            ...
            if (v == (1<<p->js_vlc_bits)-1) {
                av_em_log(q->avctx, AV_LOG_ERROR, "decouple value too large\n");
                return AVERROR_INVALIDDATA;
            }
...
static int cook_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    for (i = 0; i < q->num_subpackets; i++) {
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "subpacket[%i] %i %i\n",
 ...
 * Cook initialization
 *
 * @param avctx     pointer to the AVEMCodecContext
 */
static av_cold int cook_decode_init(AVEMCodecContext *avctx)
{
    COOKContext *q = avctx->priv_data;
    ...
    if (extradata_size < 8) {
        av_em_log(avctx, AV_LOG_ERROR, "Necessary extradata missing!\n");
        return AVERROR_INVALIDDATA;
    }
    av_em_log(avctx, AV_LOG_DEBUG, "codecdata_length=%d\n", avctx->extradata_size);

    /* Take data from the AVEMCodecContext (RM container). */
    if (!avctx->channels) {
    ...
    /* Initialize RNG. */
    av_em_lfg_init(&q->random_state, 0);
    ...
    while (edata_ptr < edata_ptr_end) {
        ...
        if (extradata_size >= 8) {
            ...
            if (q->subpacket[s].js_subband_start >= 51) {
                av_em_log(avctx, AV_LOG_ERROR, "js_subband_start %d is too large\n", q->subpacket[s].js_subband_start);
                return AVERROR_INVALIDDATA;
            }
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "subpacket[%i].cookversion=%x\n", s,
               q->subpacket[s].cookversion);
        q->subpacket[s].joint_stereo = 0;
        switch (q->subpacket[s].cookversion) {
        case MONO:
            if (avctx->channels != 1) {
                avpriv_em_request_sample(avctx, "Container channels != 1");
                return AVERROR_PATCHWELCOME;
            }
            av_em_log(avctx, AV_LOG_DEBUG, "MONO\n");
            break;
        case STEREO:
            if (avctx->channels != 1) {
            ...
            av_em_log(avctx, AV_LOG_DEBUG, "STEREO\n");
            break;
        case JOINT_STEREO:
            if (avctx->channels != 2) {
                avpriv_em_request_sample(avctx, "Container channels != 2");
                return AVERROR_PATCHWELCOME;
            }
            av_em_log(avctx, AV_LOG_DEBUG, "JOINT_STEREO\n");
            ...
            break;
        case MC_COOK:
            av_em_log(avctx, AV_LOG_DEBUG, "MULTI_CHANNEL\n");
            if (extradata_size >= 4)
                channel_mask |= q->subpacket[s].channel_mask = bytestream_get_be32(&edata_ptr);

            if (av_em_get_channel_layout_nb_channels(q->subpacket[s].channel_mask) > 1) {
                q->subpacket[s].total_subbands = q->subpacket[s].subbands +
                                                 q->subpacket[s].js_subband_start;
                q->subpacket[s].joint_stereo = 1;
                q->subpacket[s].num_channels = 2;
            ...
            break;
        default:
            avpriv_em_request_sample(avctx, "Cook version %d",
                                  q->subpacket[s].cookversion);
            return AVERROR_PATCHWELCOME;
        }

        if (s > 1 && q->subpacket[s].samples_per_channel != q->samples_per_channel) {
            av_em_log(avctx, AV_LOG_ERROR, "different number of samples per channel!\n");
            return AVERROR_INVALIDDATA;
        } else
        ...
        if (q->subpacket[s].total_subbands > 53) {
            avpriv_em_request_sample(avctx, "total_subbands > 53");
            return AVERROR_PATCHWELCOME;
        }

        if ((q->subpacket[s].js_vlc_bits > 6) ||
            (q->subpacket[s].js_vlc_bits < 2 * q->subpacket[s].joint_stereo)) {
            av_em_log(avctx, AV_LOG_ERROR, "js_vlc_bits = %d, only >= %d and <= 6 allowed!\n",
                   q->subpacket[s].js_vlc_bits, 2 * q->subpacket[s].joint_stereo);
            return AVERROR_INVALIDDATA;
        }

        if (q->subpacket[s].subbands > 50) {
            avpriv_em_request_sample(avctx, "subbands > 50");
            return AVERROR_PATCHWELCOME;
        }
        if (q->subpacket[s].subbands == 0) {
            avpriv_em_request_sample(avctx, "subbands = 0");
            return AVERROR_PATCHWELCOME;
        }
        ...
        if (q->num_subpackets + q->subpacket[s].num_channels > q->avctx->channels) {
            av_em_log(avctx, AV_LOG_ERROR, "Too many subpackets %d for channels %d\n", q->num_subpackets, q->avctx->channels);
            return AVERROR_INVALIDDATA;
        }
        ...
        if (s > FFMIN(MAX_SUBPACKETS, avctx->block_align)) {
            avpriv_em_request_sample(avctx, "subpackets > %d", FFMIN(MAX_SUBPACKETS, avctx->block_align));
            return AVERROR_PATCHWELCOME;
        }
    }
    /* Generate tables */
    ...
    q->decoded_bytes_buffer =
        av_em_mallocz(avctx->block_align
    ...
    if (q->samples_per_channel != 256 && q->samples_per_channel != 512 &&
        ...
        avpriv_em_request_sample(avctx, "samples_per_channel = %d",
