...
#define VALIDATE_INDEX(idx, limit) \
    if (idx >= limit) {\
        av_em_log(vc->avctx, AV_LOG_ERROR,\
               idx_err_str,\
...
static void vorbis_free(vorbis_context *vc)
{
    ...
    av_em_freep(&vc->channel_residues);
    av_em_freep(&vc->saved);
    av_em_freep(&vc->fdsp);

    if (vc->residues)
        for (i = 0; i < vc->residue_count; i++)
            av_em_freep(&vc->residues[i].classifs);
    av_em_freep(&vc->residues);
    av_em_freep(&vc->modes);
    ...
    if (vc->codebooks)
        for (i = 0; i < vc->codebook_count; ++i) {
            av_em_freep(&vc->codebooks[i].codevectors);
            ff_free_vlc(&vc->codebooks[i].vlc);
        }
    av_em_freep(&vc->codebooks);

    if (vc->floors)
        for (i = 0; i < vc->floor_count; ++i) {
            if (vc->floors[i].floor_type == 0) {
                av_em_freep(&vc->floors[i].data.t0.map[0]);
                av_em_freep(&vc->floors[i].data.t0.map[1]);
                av_em_freep(&vc->floors[i].data.t0.book_list);
                av_em_freep(&vc->floors[i].data.t0.lsp);
            } else {
                av_em_freep(&vc->floors[i].data.t1.list);
            }
        }
    av_em_freep(&vc->floors);

    if (vc->mappings)
        for (i = 0; i < vc->mapping_count; ++i) {
            av_em_freep(&vc->mappings[i].magnitude);
            av_em_freep(&vc->mappings[i].angle);
            av_em_freep(&vc->mappings[i].mux);
        }
    av_em_freep(&vc->mappings);
}
...
static int vorbis_parse_setup_hdr_codebooks(vorbis_context *vc)
{
    ...
    vc->codebooks = av_em_mallocz(vc->codebook_count * sizeof(*vc->codebooks));
    tmp_vlc_bits  = av_em_mallocz(V_MAX_VLCS * sizeof(*tmp_vlc_bits));
    tmp_vlc_codes = av_em_mallocz(V_MAX_VLCS * sizeof(*tmp_vlc_codes));
    codebook_multiplicands = av_em_alloc(V_MAX_VLCS * sizeof(*codebook_multiplicands));
    if (!vc->codebooks ||
    ...
    for (cb = 0; cb < vc->codebook_count; ++cb) {
        ...
        if (!ordered) {
            ...
            if (current_entry>used_entries) {
                av_em_log(vc->avctx, AV_LOG_ERROR, " More codelengths than codes in codebook. \n");
                ret = AVERROR_INVALIDDATA;
                goto error;
            }
        }
...
// Weed out unused vlcs and build codevector vector
            if (used_entries) {
                codebook_setup->codevectors =
                    av_em_mallocz_array(used_entries, codebook_setup->dimensions *
            ...
            if (j != used_entries) {
                av_em_log(vc->avctx, AV_LOG_ERROR, "Bug in codevector vector building code. \n");
                ret = AVERROR_INVALIDDATA;
                goto error;
            }
            entries = used_entries;
        } else if (codebook_setup->lookup_type >= 2) {
            av_em_log(vc->avctx, AV_LOG_ERROR, "Codebook lookup type not supported. \n");
            ret = AVERROR_INVALIDDATA;
            goto error;
        }
...
// Initialize VLC table
        if (ff_vorbis_len2vlc(tmp_vlc_bits, tmp_vlc_codes, entries)) {
            av_em_log(vc->avctx, AV_LOG_ERROR, " Invalid code lengths while generating vlcs. \n");
            ret = AVERROR_INVALIDDATA;
            goto error;
        }
        ...
        if ((ret = init_vlc(&codebook_setup->vlc, codebook_setup->nb_bits,
                            ...
                            INIT_VLC_LE))) {
            av_em_log(vc->avctx, AV_LOG_ERROR, " Error generating vlc tables. \n");
            goto error;
        }
    }

    av_em_free(tmp_vlc_bits);
    av_em_free(tmp_vlc_codes);
    av_em_free(codebook_multiplicands);
    return 0;
...
static int vorbis_parse_setup_hdr_tdtransforms(vorbis_context *vc)
{
    ...
    for (i = 0; i < vorbis_time_count; ++i) {
        ...
        if (vorbis_tdtransform) {
            av_em_log(vc->avctx, AV_LOG_ERROR, "Vorbis time domain transform data nonzero. \n");
            return AVERROR_INVALIDDATA;
        }
    }
    return 0;
...
static int vorbis_parse_setup_hdr_floors(vorbis_context *vc)
{
    ...
    vc->floors = av_em_mallocz(vc->floor_count * sizeof(*vc->floors));
    if (!vc->floors)
    ...
    for (i = 0; i < vc->floor_count; ++i) {
        ...
        if (floor_setup->floor_type == 1) {
            ...
            floor_setup->data.t1.list = av_em_mallocz_array(floor_setup->data.t1.x_list_dim,
...
// Precalculate order of x coordinates - needed for decode
            ...
            if (!floor_setup->data.t0.order) {
                av_em_log(vc->avctx, AV_LOG_ERROR, "Floor 0 order is 0.\n");
                return AVERROR_INVALIDDATA;
            }
            floor_setup->data.t0.rate           = get_bits(gb, 16);
            if (!floor_setup->data.t0.rate) {
                av_em_log(vc->avctx, AV_LOG_ERROR, "Floor 0 rate is 0.\n");
                return AVERROR_INVALIDDATA;
            }
            ...
            floor_setup->data.t0.book_list =
                av_em_alloc(floor_setup->data.t0.num_books);
            ...
            floor_setup->data.t0.lsp =
                av_em_malloc_array((floor_setup->data.t0.order + 1 + max_codebook_dim),
            ...
            av_em_log(vc->avctx, AV_LOG_ERROR, "Invalid floor type!\n");
            return AVERROR_INVALIDDATA;
        }
    }
    return 0;
...
static int vorbis_parse_setup_hdr_residues(vorbis_context *vc)
{
    ...
    vc->residues      = av_em_mallocz(vc->residue_count * sizeof(*vc->residues));
    if (!vc->residues)
        return AVERROR(ENOMEM);
    ...
    for (i = 0; i < vc->residue_count; ++i) {
        ...
        res_setup->classifs = av_em_malloc_array(res_setup->ptns_to_read,
                                        vc->audio_channels *
...
static int vorbis_parse_setup_hdr_mappings(vorbis_context *vc)
{
    ...
    vc->mappings      = av_em_mallocz(vc->mapping_count * sizeof(*vc->mappings));
    if (!vc->mappings)
        return AVERROR(ENOMEM);
    ...
    for (i = 0; i < vc->mapping_count; ++i) {
        ...
        if (get_bits(gb, 16)) {
            av_em_log(vc->avctx, AV_LOG_ERROR, "Other mappings than type 0 are not compliant with the Vorbis I specification. \n");
            return AVERROR_INVALIDDATA;
        }
        if (get_bits1(gb)) {
        ...
        if (get_bits1(gb)) {
            ...
            mapping_setup->magnitude      = av_em_mallocz(mapping_setup->coupling_steps *
                                                       sizeof(*mapping_setup->magnitude));
            mapping_setup->angle          = av_em_mallocz(mapping_setup->coupling_steps *
        ...
        if (get_bits(gb, 2)) {
            av_em_log(vc->avctx, AV_LOG_ERROR, "%u. mapping setup data invalid.\n", i);
            return AVERROR_INVALIDDATA; // following spec.
        }

        if (mapping_setup->submaps>1) {
            mapping_setup->mux = av_em_mallocz_array(vc->audio_channels,
...
static int create_map(vorbis_context *vc, unsigned floor_number)
{
    ...
    for (blockflag = 0; blockflag < 2; ++blockflag) {
        ...
        floors[floor_number].data.t0.map[blockflag] =
            av_em_malloc_array(n + 1, sizeof(int32_t)); // n + sentinel
...
static int vorbis_parse_setup_hdr_modes(vorbis_context *vc)
{
    ...
    vc->modes      = av_em_mallocz(vc->mode_count * sizeof(*vc->modes));
    if (!vc->modes)
        return AVERROR(ENOMEM);
...
static int vorbis_parse_setup_hdr(vorbis_context *vc)
{
    ...
    if ((get_bits(gb, 8) != 'v') || (get_bits(gb, 8) != 'o') ||
        ...
        av_em_log(vc->avctx, AV_LOG_ERROR, " Vorbis setup header packet corrupt (no vorbis signature). \n");
        return AVERROR_INVALIDDATA;
    }

    if ((ret = vorbis_parse_setup_hdr_codebooks(vc))) {
        av_em_log(vc->avctx, AV_LOG_ERROR, " Vorbis setup header packet corrupt (codebooks). \n");
        return ret;
    }
    if ((ret = vorbis_parse_setup_hdr_tdtransforms(vc))) {
        av_em_log(vc->avctx, AV_LOG_ERROR, " Vorbis setup header packet corrupt (time domain transforms). \n");
        return ret;
    }
    if ((ret = vorbis_parse_setup_hdr_floors(vc))) {
        av_em_log(vc->avctx, AV_LOG_ERROR, " Vorbis setup header packet corrupt (floors). \n");
        return ret;
    }
    if ((ret = vorbis_parse_setup_hdr_residues(vc))) {
        av_em_log(vc->avctx, AV_LOG_ERROR, " Vorbis setup header packet corrupt (residues). \n");
        return ret;
    }
    if ((ret = vorbis_parse_setup_hdr_mappings(vc))) {
        av_em_log(vc->avctx, AV_LOG_ERROR, " Vorbis setup header packet corrupt (mappings). \n");
        return ret;
    }
    if ((ret = vorbis_parse_setup_hdr_modes(vc))) {
        av_em_log(vc->avctx, AV_LOG_ERROR, " Vorbis setup header packet corrupt (modes). \n");
        return ret;
    }
    if (!get_bits1(gb)) {
        av_em_log(vc->avctx, AV_LOG_ERROR, " Vorbis setup header packet corrupt (framing flag). \n");
...
static int vorbis_parse_id_hdr(vorbis_context *vc)
{
    ...
    if ((get_bits(gb, 8) != 'v') || (get_bits(gb, 8) != 'o') ||
        (get_bits(gb, 8) != 'r') || (get_bits(gb, 8) != 'b') ||
        (get_bits(gb, 8) != 'i') || (get_bits(gb, 8) != 's')) {
        av_em_log(vc->avctx, AV_LOG_ERROR, " Vorbis id header packet corrupt (no vorbis signature). \n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (vc->audio_channels <= 0) {
        av_em_log(vc->avctx, AV_LOG_ERROR, "Invalid number of channels\n");
        return AVERROR_INVALIDDATA;
    }
    vc->audio_samplerate = get_bits_long(gb, 32);
    if (vc->audio_samplerate <= 0) {
        av_em_log(vc->avctx, AV_LOG_ERROR, "Invalid samplerate\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (bl0 > 13 || bl0 < 6 || bl1 > 13 || bl1 < 6 || bl1 < bl0) {
        av_em_log(vc->avctx, AV_LOG_ERROR, " Vorbis id header packet corrupt (illegal blocksize). \n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if ((get_bits1(gb)) == 0) {
        av_em_log(vc->avctx, AV_LOG_ERROR, " Vorbis id header packet corrupt (framing flag not set). \n");
        return AVERROR_INVALIDDATA;
    }

    vc->channel_residues =  av_em_malloc_array(vc->blocksize[1]  / 2, vc->audio_channels * sizeof(*vc->channel_residues));
    vc->saved            =  av_em_mallocz_array(vc->blocksize[1] / 4, vc->audio_channels * sizeof(*vc->saved));
    ...
    vc->fdsp = avpriv_em_float_dsp_alloc(vc->avctx->flags & AV_CODEC_FLAG_BITEXACT);
    if (!vc->fdsp)
        return AVERROR(ENOMEM);
...
static av_cold int vorbis_decode_init(AVEMCodecContext *avctx)
{
    ...
    if (!headers_len) {
        av_em_log(avctx, AV_LOG_ERROR, "Extradata missing.\n");
        return AVERROR_INVALIDDATA;
    }

    if ((ret = avpriv_split_xiph_headers(headers, headers_len, 30, header_start, header_len)) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Extradata corrupt.\n");
        return ret;
    }
    ...
    if (hdr_type != 1) {
        av_em_log(avctx, AV_LOG_ERROR, "First header is not the id header.\n");
        return AVERROR_INVALIDDATA;
    }
    if ((ret = vorbis_parse_id_hdr(vc))) {
        av_em_log(avctx, AV_LOG_ERROR, "Id header corrupt.\n");
        vorbis_free(vc);
        return ret;
    }
    ...
    hdr_type = get_bits(gb, 8);
    if (hdr_type != 5) {
        av_em_log(avctx, AV_LOG_ERROR, "Third header is not the setup header.\n");
        vorbis_free(vc);
        return AVERROR_INVALIDDATA;
    }
    if ((ret = vorbis_parse_setup_hdr(vc))) {
        av_em_log(avctx, AV_LOG_ERROR, "Setup header corrupt.\n");
        vorbis_free(vc);
        return ret;
    }
                                ...
                                vorbis_floor_data *vfu, float *vec)
{
    ...
    if (amplitude > 0) {
        ...
        if (book_idx >= vf->num_books) {
            av_em_log(vc->avctx, AV_LOG_ERROR, "floor0 dec: booknumber too high!\n");
            book_idx =  0;
        }
                                                           ...
                                                           int vr_type)
{
    ...
    if (max_output > ch_left * vlen) {
        if (max_output <= ch_left * vlen + vr->partition_size*ch_used/ch) {
            ...
            av_em_log(vc->avctx, AV_LOG_ERROR, "Insufficient output buffer\n");
            return AVERROR_INVALIDDATA;
        }
    }
                                        ...
                                        unsigned ch_left)
{
    if (vr->type == 2)
        ...
        av_em_log(vc->avctx, AV_LOG_ERROR, " Invalid residue type while residue decode?! \n");
        return AVERROR_INVALIDDATA;
    }
}
...
static int vorbis_parse_audio_packet(vorbis_context *vc, float **floor_ptr)
{
    ...
    if (get_bits1(gb)) {
        av_em_log(vc->avctx, AV_LOG_ERROR, "Not a Vorbis I audio packet.\n");
...
// Decode floor
    ...
    for (i = 0; i < vc->audio_channels; ++i) {
        ...
        if (ret < 0) {
            av_em_log(vc->avctx, AV_LOG_ERROR, "Invalid codebook in vorbis_floor_decode.\n");
            return AVERROR_INVALIDDATA;
        }
        no_residue[i] = ret;
    }
...
// Decode residue
    ...
    for (i = 0; i < mapping->submaps; ++i) {
        ...
        if (ch_left < ch) {
            av_em_log(vc->avctx, AV_LOG_ERROR, "Too many channels in vorbis_floor_decode.\n");
            return AVERROR_INVALIDDATA;
        }
        if (ch) {
...
static int vorbis_decode_frame(AVEMCodecContext *avctx, void *data,
                               int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (*buf == 3 && buf_size > 7) {
        av_em_log(avctx, AV_LOG_DEBUG, "Ignoring comment header\n");
        return buf_size;
    }
    ...
    if (!vc->channel_residues || !vc->modes) {
        av_em_log(avctx, AV_LOG_ERROR, "Data packet before valid headers\n");
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int vorbis_decode_close(AVEMCodecContext *avctx)
{
    vorbis_context *vc = avctx->priv_data;
...
static av_cold void vorbis_decode_flush(AVEMCodecContext *avctx)
{
    vorbis_context *vc = avctx->priv_data;
