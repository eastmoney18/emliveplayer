                        ...
                        int16_t block[64], int *last_dc)
{
    ...
    *last_dc += dc;
    ...
    do {
        hqx_get_ac(gb, &em_hqx_ac[ac_idx], &run, &lev);
        pos += run;
        if (pos >= 64)
            break;
...
static int decode_slice_thread(AVEMCodecContext *avctx, void *arg,
                               int slice_no, int threadnr)
{
    ...
    if (slice_off[slice_no] < HQX_HEADER_SIZE ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Invalid slice size %d.\n", ctx->data_size);
        return AVERROR_INVALIDDATA;
    }
...
static int hqx_decode_frame(AVEMCodecContext *avctx, void *data,
                            int *got_picture_ptr, AVEMPacket *avpkt)
{
    ...
    if (src[0] != 'H' || src[1] != 'Q') {
        av_em_log(avctx, AV_LOG_ERROR, "Not an HQX frame.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ctx->dcb == 8) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid DC precision %d.\n", ctx->dcb);
        return AVERROR_INVALIDDATA;
    }
    ret = av_em_image_check_size(ctx->width, ctx->height, 0, avctx);
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid stored dimensions %dx%d.\n",
    ...
    case HQX_444A:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Invalid format: %d.\n", ctx->format);
        return AVERROR_INVALIDDATA;
    }

    ret = em_get_buffer(avctx, ctx->pic, 0);
    if (ret < 0)
        return ret;
...
static av_cold int hqx_decode_close(AVEMCodecContext *avctx)
{
    int i;
    HQXContext *ctx = avctx->priv_data;
...
static av_cold int hqx_decode_init(AVEMCodecContext *avctx)
{
    HQXContext *ctx = avctx->priv_data;

    em_hqxdsp_init(&ctx->hqxdsp);

    return em_hqx_init_vlcs(ctx);
}

AVEMCodec em_hqx_decoder = {
    .name           = "hqx",
