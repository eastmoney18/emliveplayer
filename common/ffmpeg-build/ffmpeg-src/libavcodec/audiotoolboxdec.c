...
static UInt32 ffat_get_format_id(enum AVEMCodecID codec, int profile)
{
    switch (codec) {
...
static int ffat_update_ctx(AVEMCodecContext *avctx)
{
    ...
    if (!AudioConverterGetProperty(at->converter,
        ...
        avctx->channel_layout = av_em_get_default_channel_layout(avctx->channels);
    ...
    if (!AudioConverterGetPropertyInfo(at->converter, kAudioConverterOutputChannelLayout,
                                       &size, NULL) && size) {
        AudioChannelLayout *layout = av_em_alloc(size);
...
static uint8_t* ffat_get_magic_cookie(AVEMCodecContext *avctx, UInt32 *cookie_size)
{
    ATDecodeContext *at = avctx->priv_data;
    if (avctx->codec_id == AV_CODEC_ID_AAC) {
        ...
        *cookie_size = 5 + 3 + 5+13 + 5+at->extradata_size;
        if (!(extradata = av_em_alloc(*cookie_size)))
            return NULL;
...
static av_cold int ffat_usable_extradata(AVEMCodecContext *avctx)
{
    ATDecodeContext *at = avctx->priv_data;
...
static int ffat_set_extradata(AVEMCodecContext *avctx)
{
    ATDecodeContext *at = avctx->priv_data;
    if (ffat_usable_extradata(avctx)) {
        ...
        status = AudioConverterSetProperty(at->converter,
                                           ...
                                           cookie_size, cookie);
        if (status != 0)
            av_em_log(avctx, AV_LOG_WARNING, "AudioToolbox cookie error: %i\n", (int)status);
...
static av_cold int ffat_create_decoder(AVEMCodecContext *avctx, AVEMPacket *pkt)
{
    ATDecodeContext *at = avctx->priv_data;
    OSStatus status;
    int i;
    ...
    AudioStreamBasicDescription out_format = {
        ...
        .mFramesPerPacket = 1,
        .mBitsPerChannel = av_em_get_bytes_per_sample(sample_fmt) * 8,
    };
    ...
    if (ffat_usable_extradata(avctx)) {
        ...
        if (cookie != at->extradata)
            av_em_free(cookie);
        if (status != 0) {
            av_em_log(avctx, AV_LOG_ERROR, "AudioToolbox header-parse error: %i\n", (int)status);
    ...
    if (status != 0) {
        av_em_log(avctx, AV_LOG_ERROR, "AudioToolbox init error: %i\n", (int)status);
        return AVERROR_UNKNOWN;
    }
    ...
    if(!(at->decoded_data = av_em_alloc(av_em_get_bytes_per_sample(avctx->sample_fmt)
...
static av_cold int ffat_init_decoder(AVEMCodecContext *avctx)
{
    ATDecodeContext *at = avctx->priv_data;
                                     ...
                                     void *inctx)
{
    ...
    av_em_packet_unref(&at->in_pkt);
    av_em_packet_move_ref(&at->in_pkt, &at->new_in_pkt);
...
static void ffat_copy_samples(AVEMCodecContext *avctx, AVFrame *frame)
{
    ATDecodeContext *at = avctx->priv_data;
...
static int ffat_decode(AVEMCodecContext *avctx, void *data,
                       int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (avctx->codec_id == AV_CODEC_ID_AAC && avpkt->size > 2 &&
        ...
        AVEMPacket filter_pkt = {0};
        if (!at->bsf) {
            const AVBitStreamFilter *bsf = av_em_bsf_get_by_name("aac_adtstoasc");
            if(!bsf)
                return AVERROR_BSF_NOT_FOUND;
            if ((ret = av_em_bsf_alloc(bsf, &at->bsf)))
                return ret;
            if (((ret = avcodec_em_parameters_from_context(at->bsf->par_in, avctx)) < 0) ||
                ((ret = av_em_bsf_init(at->bsf)) < 0)) {
                av_em_bsf_free(&at->bsf);
                return ret;
            }
        }

        if ((ret = av_em_packet_ref(&filter_pkt, avpkt)) < 0)
            return ret;

        if ((ret = av_em_bsf_send_packet(at->bsf, &filter_pkt)) < 0) {
            av_em_packet_unref(&filter_pkt);
            return ret;
        }

        if ((ret = av_em_bsf_receive_packet(at->bsf, &filtered_packet)) < 0)
            return ret;
        ...
        .mBuffers = {
            {
                ...
                .mDataByteSize = av_em_get_bytes_per_sample(avctx->sample_fmt) * avctx->frame_size
                                 * avctx->channels,
            }
        }
    };

    av_em_packet_unref(&at->new_in_pkt);

    if (avpkt->size) {
        if (filtered_packet.data) {
            at->new_in_pkt = filtered_packet;
        } else if ((ret = av_em_packet_ref(&at->new_in_pkt, avpkt)) < 0) {
            return ret;
        }
    } else {
        at->eof = 1;
    ...
    if ((!ret || ret == 1) && frame->nb_samples) {
        ...
        *got_frame_ptr = 1;
        ...
        av_em_log(avctx, AV_LOG_WARNING, "Decode error: %i\n", ret);
    } else {
        at->last_pts = avpkt->pts;
    }
...
static av_cold void ffat_decode_flush(AVEMCodecContext *avctx)
{
    ATDecodeContext *at = avctx->priv_data;
...
static av_cold int ffat_close_decoder(AVEMCodecContext *avctx)
{
    ATDecodeContext *at = avctx->priv_data;
    ...
    av_em_packet_unref(&at->new_in_pkt);
    av_em_packet_unref(&at->in_pkt);
    av_em_free(at->decoded_data);
    return 0;
}

#define FFAT_DEC_CLASS(NAME) \
    static const AVEMClass ffat_##NAME##_dec_class = { \
...
#define FFAT_DEC(NAME, ID) \
    FFAT_DEC_CLASS(NAME) \
    AVEMCodec ff_##NAME##_at_decoder = { \
        .name           = #NAME "_at", \
