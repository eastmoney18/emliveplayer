...
static void compute_band_indexes(MPADecodeContext *s, GranuleDef *g)
{
    if (g->block_type == 2) {
        if (g->switch_point) {
            if(s->sample_rate_index == 8)
                avpriv_em_request_sample(s->avctx, "switch point in 8khz");
...
#ifdef DEBUG
    if(e < 1)
        av_em_log(NULL, AV_LOG_WARNING, "l3_unscale: e is %d\n", e);
#endif
    if (e > 31)
        return 0;
    m = (m + (1 << (e - 1))) >> e;
                          ...
                          int16_t *exponents, int end_pos2)
{
    ...
    while (s_index <= 572) {
        ...
        if (pos >= end_pos) {
            if (pos > end_pos2 && last_pos) {
                ...
                av_em_log(s->avctx, AV_LOG_INFO, "overread, skip %d enddists: %d %d\n", last_pos - pos, end_pos-pos, end_pos2-pos);
    ...
    if (bits_left < 0 && (s->err_recognition & (AV_EF_BUFFER|AV_EF_COMPLIANT))) {
        av_em_log(s->avctx, AV_LOG_ERROR, "bits_left=%d\n", bits_left);
        s_index=0;
...
static int mp_decode_layer3(MPADecodeContext *s)
{
    ...
    for (gr = 0; gr < nb_granules; gr++) {
        for (ch = 0; ch < s->nb_channels; ch++) {
            ...
            if (g->big_values > 288) {
                av_em_log(s->avctx, AV_LOG_ERROR, "big_values too big\n");
            ...
            if (blocksplit_flag) {
                ...
                if (g->block_type == 0) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "invalid block type\n");
                    return AVERROR_INVALIDDATA;
                }
                           ...
                           const uint8_t *buf, int buf_size)
{
    ...
    default:
        ...
        if (s->in_gb.buffer) {
            ...
            if (i >= 0 && i <= BACKSTEP_SIZE) {
                ...
                av_em_log(s->avctx, AV_LOG_ERROR, "invalid old backstep %d\n", i);
            s->gb           = s->in_gb;
            s->in_gb.buffer = NULL;
            s->extrasize    = 0;
        }
        ...
        if (i < 0 || i > BACKSTEP_SIZE || nb_frames < 0) {
            if (i < 0)
                av_em_log(s->avctx, AV_LOG_ERROR, "invalid new backstep %d\n", i);
    ...
    if (!samples) {
        ...
        if ((ret = em_get_buffer(s->avctx, s->frame, 0)) < 0)
            return ret;
...
static int decode_frame(AVEMCodecContext * avctx, void *data, int *got_frame_ptr,
                        AVEMPacket *avpkt)
{
    ...
    if (header>>8 == AV_RB32("TAG")>>8) {
        av_em_log(avctx, AV_LOG_DEBUG, "discarding ID3 tag\n");
        return buf_size;
    }
    ret = avpriv_mpegaudio_decode_header((MPADecodeHeader *)s, header);
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Header missing\n");
        return AVERROR_INVALIDDATA;
    } else if (ret == 1) {
    ...
    if (s->frame_size <= 0) {
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "incorrect frame size - multiple frames in buffer?\n");
    ...
    if (ret >= 0) {
        ...
        *got_frame_ptr       = 1;
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Error while decoding MPEG audio frame.\n");
...
static int decode_frame_adu(AVEMCodecContext *avctx, void *data,
                            int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    ret = avpriv_mpegaudio_decode_header((MPADecodeHeader *)s, header);
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid frame header\n");
        return ret;
    }
    /* update codec info */
    avctx->sample_rate = s->sample_rate;
...
static av_cold int decode_close_mp3on4(AVEMCodecContext * avctx)
{
    MP3On4DecodeContext *s = avctx->priv_data;
    int i;

    for (i = 0; i < s->frames; i++)
        av_em_freep(&s->mp3decctx[i]);

    return 0;
}


static av_cold int decode_init_mp3on4(AVEMCodecContext * avctx)
{
    MP3On4DecodeContext *s = avctx->priv_data;
    ...
    if ((avctx->extradata_size < 2) || !avctx->extradata) {
        av_em_log(avctx, AV_LOG_ERROR, "Codec extradata missing or too short.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (!cfg.chan_config || cfg.chan_config > 7) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid channel config number.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    s->mp3decctx[0] = av_em_mallocz(sizeof(MPADecodeContext));
    if (!s->mp3decctx[0])
        goto alloc_fail;
    ...
    for (i = 1; i < s->frames; i++) {
        s->mp3decctx[i] = av_em_mallocz(sizeof(MPADecodeContext));
        if (!s->mp3decctx[i])
            goto alloc_fail;
...
static void flush_mp3on4(AVEMCodecContext *avctx)
{
    int i;
    MP3On4DecodeContext *s = avctx->priv_data;
...
static int decode_frame_mp3on4(AVEMCodecContext *avctx, void *data,
                               int *got_frame_ptr, AVEMPacket *avpkt)
{
    AVFrame *frame         = data;
    const uint8_t *buf     = avpkt->data;
    ...
    for (fr = 0; fr < s->frames; fr++) {
        ...
        if (fsize < HEADER_SIZE) {
            av_em_log(avctx, AV_LOG_ERROR, "Frame size smaller than header size\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Bad header, discard block\n");
            return AVERROR_INVALIDDATA;
        }

        if (ch + m->nb_channels > avctx->channels ||
            s->coff[fr] + m->nb_channels > avctx->channels) {
            av_em_log(avctx, AV_LOG_ERROR, "frame channel count exceeds codec "
                                        "channel count\n");
            return AVERROR_INVALIDDATA;
        }
        ch += m->nb_channels;
        ...
        if ((ret = mp_decode_frame(m, outptr, buf, fsize)) < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "failed to decode channel %d\n", ch);
    ...
    if (ch != avctx->channels) {
        av_em_log(avctx, AV_LOG_ERROR, "failed to decode all channels\n");
        return AVERROR_INVALIDDATA;
    }
