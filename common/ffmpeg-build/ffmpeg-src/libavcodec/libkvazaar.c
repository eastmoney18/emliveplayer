...
static av_cold int libkvazaar_init(AVEMCodecContext *avctx)
{
    ...
    if (ctx->kvz_params) {
        ...
        if (!av_em_dict_parse_string(&dict, ctx->kvz_params, "=", ",", 0)) {
            AVEMDictionaryEntry *entry = NULL;
            while ((entry = av_em_dict_get(dict, "", entry, AV_DICT_IGNORE_SUFFIX))) {
                if (!api->config_parse(cfg, entry->key, entry->value)) {
                    av_em_log(avctx, AV_LOG_WARNING, "Invalid option: %s=%s.\n",
    ...
    if (!enc) {
        av_em_log(avctx, AV_LOG_ERROR, "Could not open kvazaar encoder.\n");
        return AVERROR_BUG;
    }
    ...
    if (avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER) {
        ...
        avctx->extradata = p = av_em_mallocz(len_out + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!p) {
...
static av_cold int libkvazaar_close(AVEMCodecContext *avctx)
{
...
static int libkvazaar_encode(AVEMCodecContext *avctx,
                             AVEMPacket *avpkt,
                             const AVFrame *frame,
                             int *got_packet_ptr)
{
    ...
    *got_packet_ptr = 0;
    ...
    if (frame) {
        ...
        if (frame->format != avctx->pix_fmt) {
            av_em_log(avctx, AV_LOG_ERROR,
                   ...
                   av_em_get_pix_fmt_name(frame->format));
            retval = AVERROR_INVALIDDATA;
            goto done;
        }
        ...
        if (!input_pic) {
            av_em_log(avctx, AV_LOG_ERROR, "Failed to allocate picture.\n");
        ...
        // Copy pixels from frame to input_pic.
        {
            ...
            av_em_image_copy(input_pic->data, dst_linesizes,
    ...
    retval = ctx->api->encoder_encode(ctx->encoder,
                                      ...
                                      &frame_info);
    if (!retval) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to encode frame.\n");
        retval = AVERROR_INVALIDDATA;
        goto done;
    }
    else
    ...
    if (data_out) {
        ...
        retval = em_alloc_packet(avpkt, len_out);
        if (retval < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Failed to allocate output packet.\n");
            goto done;
        }
...
AVEMCodec ff_libkvazaar_encoder = {
    .name             = "libkvazaar",
