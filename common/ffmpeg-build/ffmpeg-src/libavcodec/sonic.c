...
static int intlist_write(PutBitContext *pb, int *buf, int entries, int base_2_part)
{
    ...
    copy = av_em_calloc(entries, sizeof(*copy));
    if (!copy)
    ...
    bits = av_em_calloc(entries*max, sizeof(*bits));
    if (!bits)
    {
        av_em_free(copy);
        return AVERROR(ENOMEM);
...
static int intlist_read(GetBitContext *gb, int *buf, int entries, int base_2_part)
{
    ...
    int pos = 0, level = 0;
    int *bits = av_em_calloc(entries, sizeof(*bits));
...
//    av_em_log(NULL, AV_LOG_INFO, "entries: %d, low bits: %d\n", entries, low_bits);
...
//            av_em_log(NULL, AV_LOG_INFO, "actual run: %d\n", actual_run);
...
//    av_em_log(NULL, AV_LOG_INFO, "zeros: %d pos: %d\n", n_zeros, pos);
...
{
    int i;
    int *state = av_em_calloc(window_entries, sizeof(*state));
...
static av_cold int sonic_encode_init(AVEMCodecContext *avctx)
{
    ...
    {
        av_em_log(avctx, AV_LOG_ERROR, "Only mono and stereo streams are supported by now\n");
    ...
    if (s->num_taps < 32 || s->num_taps > 1024 || s->num_taps % 32) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid number of taps\n");
        return AVERROR_INVALIDDATA;
    }

    // generate taps
    s->tap_quant = av_em_calloc(s->num_taps, sizeof(*s->tap_quant));
    if (!s->tap_quant)
        return AVERROR(ENOMEM);
    ...
    s->tail = av_em_calloc(s->tail_size, sizeof(*s->tail));
    if (!s->tail)
        return AVERROR(ENOMEM);

    s->predictor_k = av_em_calloc(s->num_taps, sizeof(*s->predictor_k) );
    if (!s->predictor_k)
        return AVERROR(ENOMEM);

    for (i = 0; i < s->channels; i++)
    {
        s->coded_samples[i] = av_em_calloc(s->block_align, sizeof(**s->coded_samples));
        if (!s->coded_samples[i])
            return AVERROR(ENOMEM);
    }

    s->int_samples = av_em_calloc(s->frame_size, sizeof(*s->int_samples));

    s->window_size = ((2*s->tail_size)+s->frame_size);
    s->window = av_em_calloc(s->window_size, sizeof(*s->window));
    if (!s->window || !s->int_samples)
        return AVERROR(ENOMEM);

    avctx->extradata = av_em_mallocz(16);
    if (!avctx->extradata)
        return AVERROR(ENOMEM);
    ...
    av_em_log(avctx, AV_LOG_INFO, "Sonic: ver: %d.%d ls: %d dr: %d taps: %d block: %d frame: %d downsamp: %d\n",
...
static av_cold int sonic_encode_close(AVEMCodecContext *avctx)
{
    SonicContext *s = avctx->priv_data;
    int i;

    for (i = 0; i < s->channels; i++)
        av_em_freep(&s->coded_samples[i]);

    av_em_freep(&s->predictor_k);
    av_em_freep(&s->tail);
    av_em_freep(&s->tap_quant);
    av_em_freep(&s->window);
    av_em_freep(&s->int_samples);

    return 0;
}

static int sonic_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                              const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, s->frame_size * 5 + 1000, 0)) < 0)
        return ret;
...
//        av_em_log(avctx, AV_LOG_DEBUG, "quant: %d energy: %f / %f\n", quant, energy1, energy2);
...
//    av_em_log(avctx, AV_LOG_DEBUG, "used bytes: %d\n", (put_bits_count(&pb)+7)/8);
...
static av_cold int sonic_decode_init(AVEMCodecContext *avctx)
{
    SonicContext *s = avctx->priv_data;
    GetBitContext gb;
    int i;
    ...
    if (!avctx->extradata)
    {
        av_em_log(avctx, AV_LOG_ERROR, "No mandatory headers present\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->version != 2)
    {
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported Sonic version, please report\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->version >= 1)
    {
        ...
        if (sample_rate_index >= FF_ARRAY_ELEMS(samplerate_table)) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid sample_rate_index %d\n", sample_rate_index);
            return AVERROR_INVALIDDATA;
        }
        s->samplerate = samplerate_table[sample_rate_index];
        av_em_log(avctx, AV_LOG_INFO, "Sonicv2 chans: %d samprate: %d\n",
    ...
    if (s->decorrelation != 3 && s->channels != 2) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid decorrelation %d\n", s->decorrelation);
        return AVERROR_INVALIDDATA;
    }

    s->downsampling = get_bits(&gb, 2);
    if (!s->downsampling) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid downsampling value\n");
        return AVERROR_INVALIDDATA;
    }

    s->num_taps = (get_bits(&gb, 5)+1)<<5;
    if (get_bits1(&gb)) // XXX FIXME
        av_em_log(avctx, AV_LOG_INFO, "Custom quant table\n");
...
//    avctx->frame_size = s->block_align;
    ...
    for (i = 0; i < s->channels; i++)
    {
        s->predictor_state[i] = av_em_calloc(s->num_taps, sizeof(**s->predictor_state));
...
static av_cold int sonic_decode_close(AVEMCodecContext *avctx)
{
    SonicContext *s = avctx->priv_data;
    int i;
    ...
    for (i = 0; i < s->channels; i++)
    {
        av_em_freep(&s->predictor_state[i]);
        av_em_freep(&s->coded_samples[i]);
    }
...
static int sonic_decode_frame(AVEMCodecContext *avctx,
...
//    av_em_log(NULL, AV_LOG_INFO, "buf_size: %d\n", buf_size);
...
//    av_em_log(NULL, AV_LOG_INFO, "quant: %d\n", quant);
