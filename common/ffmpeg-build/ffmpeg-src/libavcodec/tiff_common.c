...
int ff_tadd_rational_metadata(int count, const char *name, const char *sep,
                              GetByteContext *gb, int le, AVEMDictionary **metadata)
{
    AVEMBPrint bp;
    char *ap;
    int32_t nom, denom;
    ...
    av_em_bprint_init(&bp, 10 * count, AV_BPRINT_SIZE_UNLIMITED);

    for (i = 0; i < count; i++) {
        nom   = ff_tget_long(gb, le);
        denom = ff_tget_long(gb, le);
        av_em_bprintf(&bp, "%s%7i:%-7i", auto_sep(count, sep, i, 4), nom, denom);
    }

    if ((i = av_em_bprint_finalize(&bp, &ap))) {
        return i;
    }
    if (!ap) {
    ...
    av_em_dict_set(metadata, name, ap, AV_DICT_DONT_STRDUP_VAL);
                          ...
                          GetByteContext *gb, int le, AVEMDictionary **metadata)
{
    ...
    for (i = 0; i < count; i++) {
        av_em_bprintf(&bp, "%s%7i", auto_sep(count, sep, i, 8), ff_tget_long(gb, le));
    }
                             ...
                             GetByteContext *gb, int le, AVEMDictionary **metadata)
{
    ...
    av_em_bprint_init(&bp, 10 * count, 100 * count);

    for (i = 0; i < count; i++) {
        av_em_bprintf(&bp, "%s%.15g", auto_sep(count, sep, i, 4), ff_tget_double(gb, le));
    }
...
int ff_tadd_shorts_metadata(int count, const char *name, const char *sep,
                            GetByteContext *gb, int le, int is_signed, AVEMDictionary **metadata)
{
    AVEMBPrint bp;
    char *ap;
    int i;
    ...
    for (i = 0; i < count; i++) {
        ...
        av_em_bprintf(&bp, "%s%5i", auto_sep(count, sep, i, 8), v);
    }
                           ...
                           GetByteContext *gb, int le, int is_signed, AVEMDictionary **metadata)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        av_em_bprintf(&bp, "%s%3i", auto_sep(count, sep, i, 16), v);
    }
                            ...
                            GetByteContext *gb, int le, AVEMDictionary **metadata)
{
    ...
    av_em_dict_set(metadata, name, value, AV_DICT_DONT_STRDUP_VAL);
    return 0;
}
