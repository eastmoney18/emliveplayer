...
static int sp5x_decode_frame(AVEMCodecContext *avctx,
                              void *data, int *got_frame,
                              AVEMPacket *avpkt)
{
    const uint8_t *buf = avpkt->data;
    int buf_size = avpkt->size;
    AVEMPacket avpkt_recoded;
    const int qscale = 5;
    uint8_t *recoded;
    int i = 0, j = 0;
    ...
    recoded = av_em_mallocz(buf_size + 1024);
    if (!recoded)
        return -1;
    ...
    av_em_init_packet(&avpkt_recoded);
