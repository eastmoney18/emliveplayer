 ...
 */
static av_cold int escape124_decode_init(AVEMCodecContext *avctx)
{
...
static av_cold int escape124_decode_close(AVEMCodecContext *avctx)
{
    unsigned i;
    Escape124Context *s = avctx->priv_data;

    for (i = 0; i < 3; i++)
        av_em_freep(&s->codebooks[i].blocks);
                                 ...
                                 unsigned size)
{
    ...
    if (size >= INT_MAX / sizeof(MacroBlock))
        return cb;
    cb.blocks = av_em_alloc(size ? size * sizeof(MacroBlock) : 1);
    if (!cb.blocks)
        return cb;
...
static int escape124_decode_frame(AVEMCodecContext *avctx,
                                  ...
                                  AVEMPacket *avpkt)
{
    ...
    if (!(frame_flags & 0x114) || !(frame_flags & 0x7800000)) {
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Skipping frame\n");

        *got_frame = 1;
        if ((ret = av_em_frame_ref(frame, s->frame)) < 0)
            return ret;
    ...
    if ((ret = av_em_frame_ref(s->frame, frame)) < 0)
        return ret;
...
AVEMCodec ff_escape124_decoder = {
    .name           = "escape124",
