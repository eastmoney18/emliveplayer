...
int em_h264_pred_weight_table(GetBitContext *gb, const SPS *sps,
                              ...
                              H264PredWeightTable *pwt, void *logctx)
{
    ...
    if (pwt->luma_log2_weight_denom > 7U) {
        av_em_log(logctx, AV_LOG_ERROR, "luma_log2_weight_denom %d is out of range\n", pwt->luma_log2_weight_denom);
        pwt->luma_log2_weight_denom = 0;
    }
    if (pwt->chroma_log2_weight_denom > 7U) {
        av_em_log(logctx, AV_LOG_ERROR, "chroma_log2_weight_denom %d is out of range\n", pwt->chroma_log2_weight_denom);
 ...
 */
int em_h264_check_intra4x4_pred_mode(int8_t *pred_mode_cache, void *logctx,
 ...
 * change the dc mode so it only uses the available blocks.
 */
int em_h264_check_intra_pred_mode(void *logctx, int top_samples_available,
...
int em_h264_parse_ref_count(int *plist_count, int ref_count[2],
                            ...
                            int slice_type_nos, int picture_structure, void *logctx)
{
    ...
    if (slice_type_nos != AV_PICTURE_TYPE_I) {
        ...
        if (ref_count[0] - 1 > max[0] || ref_count[1] - 1 > max[1]) {
            av_em_log(logctx, AV_LOG_ERROR, "reference overflow %u > %u or %u > %u\n",
...
int em_h264_init_poc(int pic_field_poc[2], int *pic_poc,
                               ...
                               int is_avc, void *logctx)
{
    ...
    ret = em_h2645_packet_split(&pkt, data, size, logctx, is_avc, 2, AV_CODEC_ID_H264);
    if (ret < 0) {
        ret = 0;
        goto fail;
    }
    ...
    for (i = 0; i < pkt.nb_nals; i++) {
        ...
        switch (nal->type) {
        case NAL_SPS:
            ret = em_h264_decode_seq_parameter_set(&nal->gb, logctx, ps, 0);
            if (ret < 0)
                goto fail;
            break;
        case NAL_PPS:
            ret = em_h264_decode_picture_parameter_set(&nal->gb, logctx, ps,
                                                       nal->size_bits);
            if (ret < 0)
                goto fail;
            break;
        default:
            av_em_log(logctx, AV_LOG_VERBOSE, "Ignoring NAL type %d in extradata\n",
                   nal->type);
            break;
        }
    }
                                   ...
                                   int err_recognition, void *logctx)
{
    ...
    if (ret < 0 && !(err_recognition & AV_EF_EXPLODE)) {
        ...
        escaped_buf = av_em_mallocz(escaped_buf_size);
        if (!escaped_buf)
...
int em_h264_decode_extradata(const uint8_t *data, int size, H264ParamSets *ps,
                             ...
                             int err_recognition, void *logctx)
{
    ...
    if (data[0] == 1) {
        ...
        *is_avc = 1;
        ...
        if (size < 7) {
            av_em_log(logctx, AV_LOG_ERROR, "avcC %d too short\n", size);
            return AVERROR_INVALIDDATA;
        }
 ...
 */
int em_h264_get_profile(const SPS *sps)
{
