...
unsigned ff_dxva2_get_surface_index(const AVEMCodecContext *avctx,
...
int ff_dxva2_commit_buffer(AVEMCodecContext *avctx,
                           AVDXVAContext *ctx,
...
#endif
    if (FAILED(hr)) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to get a buffer for %u: 0x%lx\n",
               type, hr);
        return -1;
    }
    if (size <= dxva_size) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Buffer for type %u was too small\n", type);
        result = -1;
    }
...
int ff_dxva2_common_end_frame(AVEMCodecContext *avctx, AVFrame *frame,
                              const void *pp, unsigned pp_size,
                              const void *qm, unsigned qm_size,
                              int (*commit_bs_si)(AVEMCodecContext *,
                                                  ...
                                                  DECODER_BUFFER_DESC *slice))
{
    ...
    if (FAILED(hr)) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to begin frame: 0x%lx\n", hr);
#if CONFIG_D3D11VA
        if (avctx->pix_fmt == AV_PIX_FMT_D3D11VA_VLD)
            if (D3D11VA_CONTEXT(ctx)->context_mutex != INVALID_HANDLE_VALUE)
                ReleaseMutex(D3D11VA_CONTEXT(ctx)->context_mutex);
...
#endif
    if (FAILED(hr)) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to execute: 0x%lx\n", hr);
        result = -1;
    }
...
#endif
    if (FAILED(hr)) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to end frame: 0x%lx\n", hr);
        result = -1;
    }
