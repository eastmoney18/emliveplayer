                ...
                if(delta == 1) {
                    ...
                    if ((me_x + filled < 0) || (me_x + me_w + filled > width) ||
                       ...
                       (filled + me_w > width) || (height - me_h < 0))
                        av_em_log(NULL, AV_LOG_ERROR, "Bogus motion vector (%i,%i), block size %ix%i at %i,%i\n",
                        ...
                        AVEMPacket *avpkt)
{
    ...
    av_em_frame_unref(ref);
    av_em_frame_move_ref(ref, p);

    if ((ret = em_get_buffer(avctx, p, AV_GET_BUFFER_FLAG_REF)) < 0)
        return ret;
    outdata = p->data[0];
    ...
    if ((ret = av_em_frame_ref(data, p)) < 0)
        return ret;
...
static av_cold int decode_end(AVEMCodecContext *avctx)
{
    ...
    av_em_frame_free(&a->pic);
    av_em_frame_free(&a->ref);
...
static av_cold int decode_init(AVEMCodecContext *avctx){
    ...
    a->pic = av_em_frame_alloc();
    a->ref = av_em_frame_alloc();
    if (!a->pic || !a->ref) {
        decode_end(avctx);
