...
static int s302m_parse_frame_header(AVEMCodecContext *avctx, const uint8_t *buf,
                                    int buf_size)
{
    uint32_t h;
    ...
    if (AES3_HEADER_LEN + frame_size != buf_size || bits > 24) {
        av_em_log(avctx, AV_LOG_ERROR, "frame has invalid header\n");
        return AVERROR_INVALIDDATA;
    }
...
static int s302m_decode_frame(AVEMCodecContext *avctx, void *data,
                              int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (avctx->bits_per_raw_sample == 24) {
        ...
        for (; buf_size > 6; buf_size -= 7) {
            *o++ = (em_reverse[buf[2]]        << 24) |
                   (em_reverse[buf[1]]        << 16) |
                   (em_reverse[buf[0]]        <<  8);
            *o++ = (em_reverse[buf[6] & 0xf0] << 28) |
                   (em_reverse[buf[5]]        << 20) |
                   (em_reverse[buf[4]]        << 12) |
                   (em_reverse[buf[3] & 0x0f] <<  4);
            buf += 7;
        }
        ...
        for (; buf_size > 5; buf_size -= 6) {
            *o++ = (em_reverse[buf[2] & 0xf0] << 28) |
                   (em_reverse[buf[1]]        << 20) |
                   (em_reverse[buf[0]]        << 12);
            *o++ = (em_reverse[buf[5] & 0xf0] << 28) |
                   (em_reverse[buf[4]]        << 20) |
                   (em_reverse[buf[3]]        << 12);
            buf += 6;
        }
        ...
        for (; buf_size > 4; buf_size -= 5) {
            *o++ = (em_reverse[buf[1]]        <<  8) |
                    em_reverse[buf[0]];
            *o++ = (em_reverse[buf[4] & 0xf0] << 12) |
                   (em_reverse[buf[3]]        <<  4) |
                   (em_reverse[buf[2]]        >>  4);
            buf += 5;
        }
...
static const AVEMClass s302m_class = {
    "SMPTE 302M Decoder",
