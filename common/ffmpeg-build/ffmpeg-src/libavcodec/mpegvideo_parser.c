...
static void mpegvideo_extract_headers(AVEMCodecParserContext *s,
...
//FIXME replace the crap with get_bits()
    ...
    while (buf < buf_end) {
        start_code= -1;
        buf= avpriv_em_find_start_code(buf, buf_end, &start_code);
        ...
        case SEQ_START_CODE:
            if (bytes_left >= 7) {
                ...
                if(!avctx->width || !avctx->height || !avctx->coded_width || !avctx->coded_height){
                    set_dim_ret = em_set_dimensions(avctx, pc->width, pc->height);
                    did_set_size=1;
                }
 ...
 the_end: ;
    if (set_dim_ret < 0)
        av_em_log(avctx, AV_LOG_ERROR, "Failed to set dimensions\n");
...
static int mpegvideo_parse(AVEMCodecParserContext *s,
                           AVEMCodecContext *avctx,
                           ...
                           const uint8_t *buf, int buf_size)
{
    ...
    ff_dlog(NULL, "pict_type=%d frame_rate=%0.3f repeat_pict=%d\n",
            s->pict_type, av_em_q2d(avctx->framerate), s->repeat_pict);
...
static int mpegvideo_split(AVEMCodecContext *avctx,
                           const uint8_t *buf, int buf_size)
{
    int i;
    uint32_t state= -1;
...
static int mpegvideo_parse_init(AVEMCodecParserContext *s)
{
    s->pict_type = AV_PICTURE_TYPE_NONE; // first frame might be partial
    return 0;
}

AVEMCodecParser ff_mpegvideo_parser = {
