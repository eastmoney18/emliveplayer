...
static int mss3_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                             AVEMPacket *avpkt)
{
    ...
    if (keyframe & ~0x301) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid frame type %X\n", keyframe);
        return AVERROR_INVALIDDATA;
    }
    keyframe   = !(keyframe & 1);
    ...
    if (dec_x + dec_width > avctx->width ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Invalid frame dimensions %dx%d +%d,%d\n",
    ...
    if (quality < 1 || quality > 100) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid quality setting %d\n", quality);
        return AVERROR_INVALIDDATA;
    }
    bytestream2_skip(&gb, 4);

    if (keyframe && !bytestream2_get_bytes_left(&gb)) {
        av_em_log(avctx, AV_LOG_ERROR, "Keyframe without data found\n");
        return AVERROR_INVALIDDATA;
    }
        ...
        for (x = 0; x < mb_width; x++) {
            for (i = 0; i < 3; i++) {
                ...
                if (c->got_error || acoder->got_error) {
                    av_em_log(avctx, AV_LOG_ERROR, "Error decoding block %d,%d\n",
                           x, y);
                    c->got_error = 1;
                    return AVERROR_INVALIDDATA;
                }
...
static av_cold int mss3_decode_end(AVEMCodecContext *avctx)
{
    ...
    for (i = 0; i < 3; i++)
        av_em_freep(&c->dct_coder[i].prev_dc);

    return 0;
}

static av_cold int mss3_decode_init(AVEMCodecContext *avctx)
{
    MSS3Context * const c = avctx->priv_data;
    int i;
    ...
    for (i = 0; i < 3; i++) {
        ...
        c->dct_coder[i].prev_dc = av_em_alloc(sizeof(*c->dct_coder[i].prev_dc) *
                                            b_width * b_height);
        if (!c->dct_coder[i].prev_dc) {
            av_em_log(avctx, AV_LOG_ERROR, "Cannot allocate buffer\n");
            av_em_frame_free(&c->pic);
            while (i >= 0) {
                av_em_freep(&c->dct_coder[i].prev_dc);
                i--;
