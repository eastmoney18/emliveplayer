...
static int vaapi_encode_make_packed_header(AVEMCodecContext *avctx,
                                           ...
                                           int type, char *data, size_t bit_len)
{
    ...
    if (vas != VA_STATUS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to create parameter buffer "
    ...
    if (vas != VA_STATUS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to create data buffer "
               "for packed header (type %d): %d (%s).\n",
               type, vas, vaErrorStr(vas));
        return AVERROR(EIO);
    }
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Packed header buffer (%d) is %#x/%#x "
           "(%zu bits).\n", type, param_buffer, data_buffer, bit_len);
    return 0;
}

static int vaapi_encode_make_param_buffer(AVEMCodecContext *avctx,
                                          VAAPIEncodePicture *pic,
                                          int type, char *data, size_t len)
{
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Param buffer (%d) is %#x.\n",
           type, buffer);
    return 0;
}

static int vaapi_encode_wait(AVEMCodecContext *avctx,
                             VAAPIEncodePicture *pic)
{
    VAAPIEncodeContext *ctx = avctx->priv_data;
    VAStatus vas;
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Sync to pic %"PRId64"/%"PRId64" "
           "(recon surface %#x).\n", pic->display_order,
           pic->encode_order, pic->recon_surface);

    vas = vaSyncSurface(ctx->hwctx->display, pic->recon_surface);
    if (vas != VA_STATUS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to sync to picture completion: "
               "%d (%s).\n", vas, vaErrorStr(vas));
        return AVERROR(EIO);
    }

    // Input is definitely finished with now.
    av_em_frame_free(&pic->input_image);

    pic->encode_complete = 1;
    return 0;
}

static int vaapi_encode_issue(AVEMCodecContext *avctx,
                              VAAPIEncodePicture *pic)
{
    VAAPIEncodeContext *ctx = avctx->priv_data;
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Issuing encode for pic %"PRId64"/%"PRId64" "
           "as type %s.\n", pic->display_order, pic->encode_order,
           picture_type_name[pic->type]);
    if (pic->nb_refs == 0) {
        av_em_log(avctx, AV_LOG_DEBUG, "No reference pictures.\n");
    } else {
        av_em_log(avctx, AV_LOG_DEBUG, "Refers to:");
        for (i = 0; i < pic->nb_refs; i++) {
            av_em_log(avctx, AV_LOG_DEBUG, " %"PRId64"/%"PRId64,
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Input surface is %#x.\n", pic->input_surface);

    pic->recon_image = av_em_frame_alloc();
    if (!pic->recon_image) {
        err = AVERROR(ENOMEM);
        goto fail;
    }

    err = av_em_hwframe_get_buffer(ctx->recon_frames_ref, pic->recon_image, 0);
    if (err < 0) {
        err = AVERROR(ENOMEM);
        goto fail;
    }
    pic->recon_surface = (VASurfaceID)(uintptr_t)pic->recon_image->data[3];
    av_em_log(avctx, AV_LOG_DEBUG, "Recon surface is %#x.\n", pic->recon_surface);
    ...
    vas = vaCreateBuffer(ctx->hwctx->display, ctx->va_context,
                         ...
                         &pic->output_buffer);
    if (vas != VA_STATUS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to create bitstream "
               "output buffer: %d (%s).\n", vas, vaErrorStr(vas));
        err = AVERROR(ENOMEM);
        goto fail;
    }
    av_em_log(avctx, AV_LOG_DEBUG, "Output buffer is %#x.\n",
           pic->output_buffer);

    if (ctx->codec->picture_params_size > 0) {
        pic->codec_picture_params = av_em_alloc(ctx->codec->picture_params_size);
    ...
    if (ctx->codec->init_picture_params) {
        ...
        if (err < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Failed to initialise picture "
                   "parameters: %d.\n", err);
            goto fail;
        }
        err = vaapi_encode_make_param_buffer(avctx, pic,
    ...
    if (pic->type == PICTURE_TYPE_IDR) {
        if (ctx->codec->write_sequence_header) {
            ...
            if (err < 0) {
                av_em_log(avctx, AV_LOG_ERROR, "Failed to write per-sequence "
                       "header: %d.\n", err);
                goto fail;
            }
    ...
    if (ctx->codec->write_picture_header) {
        ...
        if (err < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Failed to write per-picture "
                   "header: %d.\n", err);
            goto fail;
        }
        err = vaapi_encode_make_packed_header(avctx, pic,
    ...
    if (ctx->codec->write_extra_buffer) {
        for (i = 0;; i++) {
            ...
            if (err == AVERROR_EOF)
                break;
            if (err < 0) {
                av_em_log(avctx, AV_LOG_ERROR, "Failed to write extra "
                       "buffer %d: %d.\n", i, err);
                goto fail;
            }
    ...
    for (i = 0; i < pic->nb_slices; i++) {
        slice = av_em_mallocz(sizeof(*slice));
        if (!slice) {
            err = AVERROR(ENOMEM);
            goto fail;
        }
        ...
        if (ctx->codec->slice_params_size > 0) {
            slice->codec_slice_params = av_em_mallocz(ctx->codec->slice_params_size);
        ...
        if (ctx->codec->init_slice_params) {
            ...
            if (err < 0) {
                av_em_log(avctx, AV_LOG_ERROR, "Failed to initalise slice "
                       "parameters: %d.\n", err);
                goto fail;
            }
        }
        ...
        if (ctx->codec->write_slice_header) {
            ...
            err = ctx->codec->write_slice_header(avctx, pic, slice,
                                                 data, &bit_len);
            if (err < 0) {
                av_em_log(avctx, AV_LOG_ERROR, "Failed to write per-slice "
                       "header: %d.\n", err);
                goto fail;
            }
            err = vaapi_encode_make_packed_header(avctx, pic,
    ...
    vas = vaBeginPicture(ctx->hwctx->display, ctx->va_context,
                         pic->input_surface);
    if (vas != VA_STATUS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to begin picture encode issue: "
               "%d (%s).\n", vas, vaErrorStr(vas));
        err = AVERROR(EIO);
    ...
    if (vas != VA_STATUS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to upload encode parameters: "
               "%d (%s).\n", vas, vaErrorStr(vas));
        err = AVERROR(EIO);
        goto fail_with_picture;
    }
    ...
    if (vas != VA_STATUS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to end picture encode issue: "
               "%d (%s).\n", vas, vaErrorStr(vas));
        err = AVERROR(EIO);
        goto fail_at_end;
    }
...
fail_at_end:
    av_em_freep(&pic->codec_picture_params);
    av_em_frame_free(&pic->recon_image);
    return err;
}

static int vaapi_encode_output(AVEMCodecContext *avctx,
                               VAAPIEncodePicture *pic, AVEMPacket *pkt)
{
    VAAPIEncodeContext *ctx = avctx->priv_data;
    ...
    vas = vaMapBuffer(ctx->hwctx->display, pic->output_buffer,
                      (void**)&buf_list);
    if (vas != VA_STATUS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to map output buffers: "
               "%d (%s).\n", vas, vaErrorStr(vas));
        err = AVERROR(EIO);
        goto fail;
    }
    ...
    for (buf = buf_list; buf; buf = buf->next) {
        av_em_log(avctx, AV_LOG_DEBUG, "Output buffer: %u bytes "
               "(status %08x).\n", buf->size, buf->status);

        err = av_em_new_packet(pkt, buf->size);
        if (err < 0)
            goto fail;
    ...
    if (vas != VA_STATUS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to unmap output buffers: "
               "%d (%s).\n", vas, vaErrorStr(vas));
        err = AVERROR(EIO);
        goto fail;
    }
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Output read for pic %"PRId64"/%"PRId64".\n",
           pic->display_order, pic->encode_order);
    return 0;
...
static int vaapi_encode_discard(AVEMCodecContext *avctx,
                                VAAPIEncodePicture *pic)
{
    VAAPIEncodeContext *ctx = avctx->priv_data;
    ...
    if (pic->output_buffer != VA_INVALID_ID) {
        av_em_log(avctx, AV_LOG_DEBUG, "Discard output for pic "
               "%"PRId64"/%"PRId64".\n",
               pic->display_order, pic->encode_order);
...
static int vaapi_encode_free(AVEMCodecContext *avctx,
                             VAAPIEncodePicture *pic)
{
    int i;
    ...
    for (i = 0; i < pic->nb_slices; i++) {
        av_em_freep(&pic->slices[i]->priv_data);
        av_em_freep(&pic->slices[i]->codec_slice_params);
        av_em_freep(&pic->slices[i]);
    }
    av_em_freep(&pic->codec_picture_params);
...
static int vaapi_encode_step(AVEMCodecContext *avctx,
...
static int vaapi_encode_get_next(AVEMCodecContext *avctx,
                                 VAAPIEncodePicture **pic_out)
{
    ...
    *pic_out = start;
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Pictures:");
    for (pic = ctx->pic_start; pic; pic = pic->next) {
        av_em_log(avctx, AV_LOG_DEBUG, " %s (%"PRId64"/%"PRId64")",
               picture_type_name[pic->type],
               pic->display_order, pic->encode_order);
    }
    av_em_log(avctx, AV_LOG_DEBUG, "\n");
...
static int vaapi_encode_mangle_end(AVEMCodecContext *avctx)
{
    VAAPIEncodeContext *ctx = avctx->priv_data;
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Pictures at end of stream:");
    for (pic = ctx->pic_start; pic; pic = pic->next) {
        av_em_log(avctx, AV_LOG_DEBUG, " %s (%"PRId64"/%"PRId64")",
               picture_type_name[pic->type],
               pic->display_order, pic->encode_order);
...
static int vaapi_encode_clear_old(AVEMCodecContext *avctx)
{
    VAAPIEncodeContext *ctx = avctx->priv_data;
...
int ff_vaapi_encode2(AVEMCodecContext *avctx, AVEMPacket *pkt,
                     const AVFrame *input_image, int *got_packet)
{
    ...
    if (input_image) {
        av_em_log(avctx, AV_LOG_DEBUG, "Encode frame: %ux%u (%"PRId64").\n",
               input_image->width, input_image->height, input_image->pts);

        err = vaapi_encode_get_next(avctx, &pic);
        if (err) {
            av_em_log(avctx, AV_LOG_ERROR, "Input setup failed: %d.\n", err);
            return err;
        }

        pic->input_image = av_em_frame_alloc();
        if (!pic->input_image) {
            err = AVERROR(ENOMEM);
            goto fail;
        }
        err = av_em_frame_ref(pic->input_image, input_image);
        if (err < 0)
            goto fail;
    ...
    if (err < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Encode failed: %d.\n", err);
        goto fail;
    }
    ...
    if (!pic) {
        *got_packet = 0;
        ...
        if (err < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Output failed: %d.\n", err);
            goto fail;
        }
    ...
    if (err < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "List clearing failed: %d.\n", err);
        goto fail;
    }
...
static av_cold int vaapi_encode_check_config(AVEMCodecContext *avctx)
{
    VAAPIEncodeContext *ctx = avctx->priv_data;
    VAStatus vas;
    int i, n, err;
    ...
    profiles = av_em_malloc_array(n, sizeof(VAProfile));
    if (!profiles) {
        err = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    if (vas != VA_STATUS_SUCCESS) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to query profiles: %d (%s).\n",
               vas, vaErrorStr(vas));
        err = AVERROR(ENOSYS);
        goto fail;
    }
    ...
    if (i >= n) {
        av_em_log(ctx, AV_LOG_ERROR, "Encoding profile not found (%d).\n",
               ctx->va_profile);
        err = AVERROR(ENOSYS);
        goto fail;
    }
    ...
    entrypoints = av_em_malloc_array(n, sizeof(VAEntrypoint));
    if (!entrypoints) {
        err = AVERROR(ENOMEM);
        goto fail;
    }
    vas = vaQueryConfigEntrypoints(ctx->hwctx->display, ctx->va_profile,
                                   entrypoints, &n);
    if (vas != VA_STATUS_SUCCESS) {
        av_em_log(ctx, AV_LOG_ERROR, "Failed to query entrypoints for "
               "profile %u: %d (%s).\n", ctx->va_profile,
               vas, vaErrorStr(vas));
        err = AVERROR(ENOSYS);
        goto fail;
    ...
    if (i >= n) {
        av_em_log(ctx, AV_LOG_ERROR, "Encoding entrypoint not found "
    ...
    if (vas != VA_STATUS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to fetch config "
               "attributes: %d (%s).\n", vas, vaErrorStr(vas));
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(attr); i++) {
        ...
        case VAConfigAttribRateControl:
            if (!(ctx->va_rc_mode & attr[i].value)) {
                av_em_log(avctx, AV_LOG_ERROR, "Rate control mode is not "
                       "supported: %x\n", attr[i].value);
                err = AVERROR(EINVAL);
                goto fail;
            }
        ...
        case VAConfigAttribEncMaxRefFrames:
        {
            ...
            if (avctx->gop_size > 1 && ref_l0 < 1) {
                av_em_log(avctx, AV_LOG_ERROR, "P frames are not "
                       "supported (%x).\n", attr[i].value);
                err = AVERROR(EINVAL);
                goto fail;
            }
            if (avctx->max_b_frames > 0 && ref_l1 < 1) {
                av_em_log(avctx, AV_LOG_ERROR, "B frames are not "
                       "supported (%x).\n", attr[i].value);
                err = AVERROR(EINVAL);
                goto fail;
            }
    ...
    err = 0;
fail:
    av_em_freep(&profiles);
    av_em_freep(&entrypoints);
    return err;
}

av_cold int ff_vaapi_encode_init(AVEMCodecContext *avctx,
                                 const VAAPIEncodeType *type)
{
    ...
    if (!avctx->hw_frames_ctx) {
        av_em_log(avctx, AV_LOG_ERROR, "A hardware frames reference is "
    ...
    ctx->priv_data = av_em_mallocz(type->priv_data_size);
    if (!ctx->priv_data) {
        err = AVERROR(ENOMEM);
        goto fail;
    }

    ctx->input_frames_ref = av_em_buffer_ref(avctx->hw_frames_ctx);
    ...
    ctx->device_ref = av_em_buffer_ref(ctx->input_frames->device_ref);
    if (!ctx->device_ref) {
        err = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    vas = vaCreateConfig(ctx->hwctx->display,
                         ...
                         &ctx->va_config);
    if (vas != VA_STATUS_SUCCESS) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to create encode pipeline "
               "configuration: %d (%s).\n", vas, vaErrorStr(vas));
        err = AVERROR(EIO);
        goto fail;
    }

    hwconfig = av_em_hwdevice_hwconfig_alloc(ctx->device_ref);
    if (!hwconfig) {
        err = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    constraints = av_em_hwdevice_get_hwframe_constraints(ctx->device_ref,
                                                      hwconfig);
    if (!constraints) {
        err = AVERROR(ENOMEM);
        goto fail;
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Using %s as format of "
           "reconstructed frames.\n", av_em_get_pix_fmt_name(recon_format));
    ...
    if (ctx->aligned_width  < constraints->min_width  ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Hardware does not support encoding at "
    ...
    av_em_freep(&hwconfig);
    av_em_hwframe_constraints_free(&constraints);

    ctx->recon_frames_ref = av_em_hwframe_ctx_alloc(ctx->device_ref);
    ...
    err = av_em_hwframe_ctx_init(ctx->recon_frames_ref);
    if (err < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Failed to initialise reconstructed "
               "frame context: %d.\n", err);
        goto fail;
    }
    ...
    if (ctx->codec->sequence_params_size > 0) {
        ctx->codec_sequence_params =
            av_em_mallocz(ctx->codec->sequence_params_size);
    ...
    if (ctx->codec->picture_params_size > 0) {
        ctx->codec_picture_params =
            av_em_mallocz(ctx->codec->picture_params_size);
    ...
    if (ctx->codec->init_sequence_params) {
        ...
        if (err < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Codec sequence initialisation "
                   "failed: %d.\n", err);
            goto fail;
        }
    }
...
av_cold int ff_vaapi_encode_close(AVEMCodecContext *avctx)
{
    VAAPIEncodeContext *ctx = avctx->priv_data;
    ...
    av_em_freep(&ctx->codec_sequence_params);
    av_em_freep(&ctx->codec_picture_params);

    av_em_buffer_unref(&ctx->recon_frames_ref);
    av_em_buffer_unref(&ctx->input_frames_ref);
    av_em_buffer_unref(&ctx->device_ref);

    av_em_freep(&ctx->priv_data);
