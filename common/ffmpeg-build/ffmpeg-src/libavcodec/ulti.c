...
static av_cold int ulti_decode_init(AVEMCodecContext *avctx)
{
...
static av_cold int ulti_decode_end(AVEMCodecContext *avctx){
    UltimotionDecodeContext *s = avctx->priv_data;
...
static int ulti_decode_frame(AVEMCodecContext *avctx,
    ...
    while(!done) {
        ...
        if((idx & 0xF8) == 0x70) {
            ...
            case 0x70: //change modifier
                ...
                if(modifier>1)
                    av_em_log(avctx, AV_LOG_INFO, "warning: modifier must be 0 or 1, got %i\n", modifier);
                break;
            ...
            case 0x74: //skip some blocks
                ...
                break;
            default:
                av_em_log(avctx, AV_LOG_INFO, "warning: unknown escape 0x%02X\n", idx);
            }
