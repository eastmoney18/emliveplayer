...
static int sunrast_decode_frame(AVEMCodecContext *avctx, void *data,
                                int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (AV_RB32(buf) != RAS_MAGIC) {
        av_em_log(avctx, AV_LOG_ERROR, "this is not sunras encoded data\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (type == RT_EXPERIMENTAL) {
        avpriv_em_request_sample(avctx, "TIFF/IFF/EXPERIMENTAL (compression) type");
        return AVERROR_PATCHWELCOME;
    }
    if (type > RT_FORMAT_IFF) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid (compression) type\n");
        return AVERROR_INVALIDDATA;
    }
    if (maptype == RMT_RAW) {
        avpriv_em_request_sample(avctx, "Unknown colormap type");
        return AVERROR_PATCHWELCOME;
    }
    if (maptype > RMT_RAW) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid colormap type\n");
        return AVERROR_INVALIDDATA;
    }

    if (type == RT_FORMAT_TIFF || type == RT_FORMAT_IFF) {
        av_em_log(avctx, AV_LOG_ERROR, "unsupported (compression) type\n");
        return -1;
    }
    ...
    switch (depth) {
        ...
        case 32:
            ...
            break;
        default:
            av_em_log(avctx, AV_LOG_ERROR, "invalid depth\n");
            return AVERROR_INVALIDDATA;
    }
    ...
    if (depth > 8 && maplength) {
        av_em_log(avctx, AV_LOG_WARNING, "useless colormap found or file is corrupted, trying to recover\n");

    } else if (maplength) {
        unsigned int len = maplength / 3;

        if (maplength % 3 || maplength > 768) {
            av_em_log(avctx, AV_LOG_WARNING, "invalid colormap length\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    if (maplength && depth < 8) {
        ptr = ptr2 = av_em_malloc_array((w + 15), h);
        if (!ptr)
