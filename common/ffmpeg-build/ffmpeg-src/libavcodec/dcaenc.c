...
static int encode_init(AVEMCodecContext *avctx)
{
    ...
    if (!layout) {
        av_em_log(avctx, AV_LOG_WARNING, "No channel layout specified. The "
                                      ...
                                      "might be incorrect.\n");
        layout = av_em_get_default_channel_layout(avctx->channels);
    }
    switch (layout) {
    ...
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported channel layout!\n");
    ...
    if (avctx->bit_rate < 32000 || avctx->bit_rate > 3840000) {
        av_em_log(avctx, AV_LOG_ERROR, "Bit rate %"PRId64" not supported.", (int64_t)avctx->bit_rate);
        return AVERROR(EINVAL);
    }
                        ...
                        const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, c->frame_size, 0)) < 0)
        return ret;
