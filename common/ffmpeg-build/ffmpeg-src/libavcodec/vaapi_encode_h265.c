...
static int vaapi_encode_h265_write_sequence_header(AVEMCodecContext *avctx,
...
static int vaapi_encode_h265_write_slice_header(AVEMCodecContext *avctx,
                                                VAAPIEncodePicture *pic,
                                                VAAPIEncodeSlice *slice,
                                                char *data, size_t *data_len)
{
...
static int vaapi_encode_h265_init_sequence_params(AVEMCodecContext *avctx)
{
    VAAPIEncodeContext                 *ctx = avctx->priv_data;
    VAEncSequenceParameterBufferHEVC  *vseq = ctx->codec_sequence_params;
    VAEncPictureParameterBufferHEVC   *vpic = ctx->codec_picture_params;
...
static int vaapi_encode_h265_init_picture_params(AVEMCodecContext *avctx,
                                                 VAAPIEncodePicture *pic)
{
    VAAPIEncodeContext               *ctx = avctx->priv_data;
    VAEncPictureParameterBufferHEVC *vpic = pic->codec_picture_params;
...
static int vaapi_encode_h265_init_slice_params(AVEMCodecContext *avctx,
                                               VAAPIEncodePicture *pic,
                                               VAAPIEncodeSlice *slice)
{
    VAAPIEncodeContext                *ctx = avctx->priv_data;
...
static av_cold int vaapi_encode_h265_init_constant_bitrate(AVEMCodecContext *avctx)
{
    VAAPIEncodeContext      *ctx = avctx->priv_data;
    VAAPIEncodeH265Context *priv = ctx->priv_data;
    int hrd_buffer_size;
...
static av_cold int vaapi_encode_h265_init_fixed_qp(AVEMCodecContext *avctx)
{
    VAAPIEncodeContext      *ctx = avctx->priv_data;
    VAAPIEncodeH265Context *priv = ctx->priv_data;
...
static av_cold int vaapi_encode_h265_init_internal(AVEMCodecContext *avctx)
{
    ...
    case FF_PROFILE_HEVC_MAIN_10:
        av_em_log(avctx, AV_LOG_ERROR, "H.265 main 10-bit profile "
               "is not supported.\n");
        return AVERROR_PATCHWELCOME;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unknown H.265 profile %d.\n",
               avctx->profile);
        return AVERROR(EINVAL);
    }
    ...
    av_em_log(avctx, AV_LOG_VERBOSE, "Input %ux%u -> Aligned %ux%u -> CTU %ux%u.\n",
...
static av_cold int vaapi_encode_h265_init(AVEMCodecContext *avctx)
{
...
static const AVEMCodecDefault vaapi_encode_h265_defaults[] = {
    { "profile",        "1"   },
    { "level",          "51"  },
    { "b",              "0"   },
    { "bf",             "2"   },
...
static const AVEMClass vaapi_encode_h265_class = {
...
AVEMCodec em_hevc_vaapi_encoder = {
    .name           = "hevc_vaapi",
