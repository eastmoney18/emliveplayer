    ...
    while (band < num_bands) {
        ...
        do {
            ...
            if (run > num_bands - band - run_len) {
                av_em_log(c->avctx, AV_LOG_ERROR, "Invalid band type run\n");
    ...
    for (w = 0; w < c->num_windows; w++) {
        ...
        for (b = 0; b < c->num_bands; b++) {
            ...
            if (scale < 0 || scale > 127) {
                av_em_log(c->avctx, AV_LOG_ERROR, "Invalid scale value %d\n",
                       scale);
                return AVERROR_INVALIDDATA;
            }
...
static inline int get_egolomb(GetBitContext *gb)
{
    ...
    while (get_bits1(gb)) {
        v++;
        if (v > 30) {
            av_em_log(NULL, AV_LOG_WARNING, "Too large golomb code in get_egolomb.\n");
            v = 30;
            break;
        }
    }
...
static int on2avc_decode_frame(AVEMCodecContext * avctx, void *data,
    ...
    if (c->is_av500) {
        ...
        while (bytestream2_get_bytes_left(&gb) > 2) {
            ...
            if (!frame_size || frame_size > bytestream2_get_bytes_left(&gb)) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid subframe size %d\n",
                       frame_size);
                return AVERROR_INVALIDDATA;
            }
            num_frames++;
        ...
        if (!num_frames) {
            av_em_log(avctx, AV_LOG_ERROR, "No subframes present\n");
            return AVERROR_INVALIDDATA;
        }
...
static av_cold int on2avc_decode_init(AVEMCodecContext *avctx)
{
    On2AVCContext *c = avctx->priv_data;
    int i;

    if (avctx->channels > 2U) {
        avpriv_em_request_sample(avctx, "Decoding more than 2 channels");
    ...
    if (c->is_av500 && avctx->channels == 2) {
        av_em_log(avctx, AV_LOG_ERROR, "0x500 version should be mono\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    c->fdsp = avpriv_em_float_dsp_alloc(avctx->flags & AV_CODEC_FLAG_BITEXACT);
    if (!c->fdsp)
    ...
    return 0;
vlc_fail:
    av_em_log(avctx, AV_LOG_ERROR, "Cannot init VLC\n");
    on2avc_free_vlcs(c);
    av_em_freep(&c->fdsp);
    return AVERROR(ENOMEM);
}

static av_cold int on2avc_decode_close(AVEMCodecContext *avctx)
{
    On2AVCContext *c = avctx->priv_data;
