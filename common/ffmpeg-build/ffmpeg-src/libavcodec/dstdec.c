...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->channels > DST_MAX_CHANNELS) {
        avpriv_em_request_sample(avctx, "Channel count %d", avctx->channels);
                        ...
                        int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (!get_bits1(gb)) {
        avpriv_em_request_sample(avctx, "Not Same Segmentation");
        return AVERROR_PATCHWELCOME;
    }

    if (!get_bits1(gb)) {
        avpriv_em_request_sample(avctx, "Not Same Segmentation For All Channels");
        return AVERROR_PATCHWELCOME;
    }

    if (!get_bits1(gb)) {
        avpriv_em_request_sample(avctx, "Not End Of Channel Segmentation");
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (same_map) {
        ...
        avpriv_em_request_sample(avctx, "Not Same Mapping");
