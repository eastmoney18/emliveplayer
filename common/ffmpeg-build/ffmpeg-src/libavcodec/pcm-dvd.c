...
static av_cold int pcm_dvd_decode_init(AVEMCodecContext *avctx)
{
    ...
    if (!(s->extra_samples = av_em_alloc(8 * 3 * 4)))
        return AVERROR(ENOMEM);

    return 0;
}

static av_cold int pcm_dvd_decode_uninit(AVEMCodecContext *avctx)
{
    PCMDVDContext *s = avctx->priv_data;

    av_em_freep(&s->extra_samples);

    return 0;
}

static int pcm_dvd_parse_header(AVEMCodecContext *avctx, const uint8_t *header)
{
    ...
    if (avctx->debug & FF_DEBUG_PICT_INFO)
        av_em_log(avctx, AV_LOG_DEBUG, "pcm_dvd_parse_header: header = %02x%02x%02x\n",
...
static void *pcm_dvd_decode_samples(AVEMCodecContext *avctx, const uint8_t *src,
                                    void *dst, int blocks)
{
    PCMDVDContext *s = avctx->priv_data;
    int16_t *dst16   = dst;
...
static int pcm_dvd_decode_frame(AVEMCodecContext *avctx, void *data,
                                int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (buf_size < 3) {
        av_em_log(avctx, AV_LOG_ERROR, "PCM packet too small\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->last_block_size && s->last_block_size != s->block_size) {
        av_em_log(avctx, AV_LOG_WARNING, "block_size has changed %d != %d\n", s->last_block_size, s->block_size);
        s->extra_sample_count = 0;
    }
    s->last_block_size = s->block_size;
    src      += 3;
    ...
    if ((retval = em_get_buffer(avctx, frame, 0)) < 0)
        return retval;
    dst = frame->data[0];
