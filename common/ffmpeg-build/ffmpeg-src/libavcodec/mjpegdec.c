...
static void parse_avid(MJpegDecodeContext *s, uint8_t *buf, int len)
{
    ...
    if (s->avctx->debug & FF_DEBUG_PICT_INFO)
        av_em_log(s->avctx, AV_LOG_INFO, "AVID: len:%d %d\n", len, len > 14 ? buf[12] : -1);
}

static void init_idct(AVEMCodecContext *avctx)
{
...
av_cold int ff_mjpeg_decode_init(AVEMCodecContext *avctx)
{
    MJpegDecodeContext *s = avctx->priv_data;

    if (!s->picture_ptr) {
        s->picture = av_em_frame_alloc();
        if (!s->picture)
    ...
    s->avctx = avctx;
    em_blockdsp_init(&s->bdsp, avctx);
    em_hpeldsp_init(&s->hdsp, avctx->flags);
    init_idct(avctx);
    s->buffer_size   = 0;
    s->buffer        = NULL;
    s->start_code    = -1;
    ...
    if (s->extern_huff) {
        av_em_log(avctx, AV_LOG_INFO, "using external huffman table\n");
    ...
    if (avctx->field_order == AV_FIELD_BB) { /* quicktime icefloe 019 */
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "bottom field first\n");
...
int ff_mjpeg_decode_dqt(MJpegDecodeContext *s)
{
    ...
    if (8*len > get_bits_left(&s->gb)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "dqt: len %d is too large\n", len);
    ...
    while (len >= 65) {
        ...
        if (pr > 1) {
            av_em_log(s->avctx, AV_LOG_ERROR, "dqt: invalid precision\n");
            return AVERROR_INVALIDDATA;
        }
        index = get_bits(&s->gb, 4);
        if (index >= 4)
            return -1;
        av_em_log(s->avctx, AV_LOG_DEBUG, "index=%d\n", index);
        /* read quant table */
        for (i = 0; i < 64; i++) {
        ...
        av_em_log(s->avctx, AV_LOG_DEBUG, "qscale[%d]: %d\n",
...
int ff_mjpeg_decode_dht(MJpegDecodeContext *s)
{
    ...
    if (8*len > get_bits_left(&s->gb)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "dht: len %d is too large\n", len);
        return AVERROR_INVALIDDATA;
    }
    ...
    while (len > 0) {
        ...
        av_em_log(s->avctx, AV_LOG_DEBUG, "class=%d index=%d nb_codes=%d\n",
...
int ff_mjpeg_decode_sof(MJpegDecodeContext *s)
{
    ...
    if (bits > 16 || bits < 1) {
        av_em_log(s->avctx, AV_LOG_ERROR, "bits %d is invalid\n", bits);
        return AVERROR_INVALIDDATA;
    }

    if (s->avctx->bits_per_raw_sample != bits) {
        av_em_log(s->avctx, s->avctx->bits_per_raw_sample > 0 ? AV_LOG_INFO : AV_LOG_DEBUG, "Changing bps from %d to %d\n", s->avctx->bits_per_raw_sample, bits);
        s->avctx->bits_per_raw_sample = bits;
        init_idct(s->avctx);
    }
    if (s->pegasus_rct)
    ...
    if(s->lossless && s->avctx->lowres){
        av_em_log(s->avctx, AV_LOG_ERROR, "lowres is not possible with lossless jpeg\n");
        return -1;
    }
    ...
    av_em_log(s->avctx, AV_LOG_DEBUG, "sof0: picture: %dx%d\n", width, height);
    if (av_em_image_check_size(width, height, 0, s->avctx))
        return AVERROR_INVALIDDATA;
    ...
    if (s->ls && !(bits <= 8 || nb_components == 1)) {
        avpriv_em_report_missing_feature(s->avctx,
    ...
    for (i = 0; i < nb_components; i++) {
        ...
        if (s->quant_index[i] >= 4) {
            av_em_log(s->avctx, AV_LOG_ERROR, "quant_index is invalid\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        av_em_log(s->avctx, AV_LOG_DEBUG, "component %d %d:%d id: %d quant:%d\n",
               i, h_count[i], v_count[i],
    ...
    if (s->ls && (s->h_max > 1 || s->v_max > 1)) {
        avpriv_em_report_missing_feature(s->avctx, "Subsampling in JPEG-LS");
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (width != s->width || height != s->height || bits != s->bits ||
        ...
        ret = em_set_dimensions(s->avctx, width, height);
        if (ret < 0)
            return ret;
    ...
    if (s->got_picture && s->interlaced && (s->bottom_field == !s->interlace_polarity)) {
        if (s->progressive) {
            avpriv_em_request_sample(s->avctx, "progressively coded interlaced picture");
            return AVERROR_INVALIDDATA;
        }
    } else{
    ...
    av_em_log(s->avctx, AV_LOG_DEBUG, "pix fmt id %x\n", pix_fmt_id);
    ...
    default:
unk_pixfmt:
        av_em_log(s->avctx, AV_LOG_ERROR, "Unhandled pixel format 0x%x bits:%d\n", pix_fmt_id, s->bits);
        memset(s->upscale_h, 0, sizeof(s->upscale_h));
        memset(s->upscale_v, 0, sizeof(s->upscale_v));
        return AVERROR_PATCHWELCOME;
    }
    if ((AV_RB32(s->upscale_h) || AV_RB32(s->upscale_v)) && s->avctx->lowres) {
        av_em_log(s->avctx, AV_LOG_ERROR, "lowres not supported for weird subsampling\n");
        return AVERROR_PATCHWELCOME;
    }
    if (s->ls) {
        memset(s->upscale_h, 0, sizeof(s->upscale_h));
        ...
        if (s->nb_components == 3) {
            ...
            av_em_log(s->avctx, AV_LOG_ERROR, "Unsupported number of components %d\n", s->nb_components);
            return AVERROR_PATCHWELCOME;
    ...
    s->pix_desc = av_em_pix_fmt_desc_get(s->avctx->pix_fmt);
    if (!s->pix_desc) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Could not get a pixel format descriptor.\n");
        return AVERROR_BUG;
    }
    ...
    av_em_frame_unref(s->picture_ptr);
    if (em_get_buffer(s->avctx, s->picture_ptr, AV_GET_BUFFER_FLAG_REF) < 0)
        return -1;
    s->picture_ptr->pict_type = AV_PICTURE_TYPE_I;
    s->picture_ptr->key_frame = 1;
    s->got_picture            = 1;
    ...
    if (len != (8 + (3 * nb_components)))
        av_em_log(s->avctx, AV_LOG_DEBUG, "decode_sof0: error, len(%d) mismatch\n", len);
    }

    if ((s->rgb && !s->lossless && !s->ls) ||
        (!s->rgb && s->ls && s->nb_components > 1)) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Unsupported coding and pixel format combination\n");
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (s->progressive) {
        ...
        for (i = 0; i < s->nb_components; i++) {
            ...
            av_em_freep(&s->blocks[i]);
            av_em_freep(&s->last_nnz[i]);
            s->blocks[i]       = av_em_mallocz_array(size, sizeof(**s->blocks));
            s->last_nnz[i]     = av_em_mallocz_array(size, sizeof(**s->last_nnz));
                        ...
                        int dc_index, int ac_index, int16_t *quant_matrix)
{
    ...
    if (val == 0xfffff) {
        av_em_log(s->avctx, AV_LOG_ERROR, "error dc\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    do {
        ...
        if (code) {
            ...
            if (i > 63) {
                av_em_log(s->avctx, AV_LOG_ERROR, "error count: %d\n", i);
                return AVERROR_INVALIDDATA;
            }
        ...
        for (i = ss; ; i++) {
            ...
            if (code) {
                ...
                if (run == 0xF) {// ZRL - skip 15 coefficients
                    i += 15;
                    if (i >= se) {
                        av_em_log(s->avctx, AV_LOG_ERROR, "ZRL overflow: %d\n", i);
                        return AVERROR_INVALIDDATA;
                    }
                } else {
                    val = (1 << run);
...
static int ljpeg_decode_rgb_scan(MJpegDecodeContext *s, int nb_components, int predictor, int point_transform)
{
    ...
    av_em_fast_malloc(&s->ljpeg_buffer, &s->ljpeg_buffer_size,
                             ...
                             const AVFrame *reference)
{
    ...
    if (mb_bitmask) {
        if (mb_bitmask_size != (s->mb_width * s->mb_height + 7)>>3) {
            av_em_log(s->avctx, AV_LOG_ERROR, "mb_bitmask_size mismatches\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    av_em_pix_fmt_get_chroma_sub_sample(s->avctx->pix_fmt, &chroma_h_shift,
                                     &chroma_v_shift);
    ...
    for (mb_y = 0; mb_y < s->mb_height; mb_y++) {
        for (mb_x = 0; mb_x < s->mb_width; mb_x++) {
            ...
            if (get_bits_left(&s->gb) < 0) {
                av_em_log(s->avctx, AV_LOG_ERROR, "overread %d\n",
                       -get_bits_left(&s->gb));
                return AVERROR_INVALIDDATA;
            }
            for (i = 0; i < nb_components; i++) {
                                            ...
                                            int se, int Ah, int Al)
{
    ...
    if (se < ss || se > 63) {
        av_em_log(s->avctx, AV_LOG_ERROR, "SS/SE %d/%d is invalid\n", ss, se);
        return AVERROR_INVALIDDATA;
    }
...
static void mjpeg_idct_scan_progressive_ac(MJpegDecodeContext *s)
{
    ...
    for (c = 0; c < s->nb_components; c++) {
        ...
        if (~s->coefs_finished[c])
            av_em_log(s->avctx, AV_LOG_WARNING, "component %d is incomplete\n", c);
                        ...
                        int mb_bitmask_size, const AVFrame *reference)
{
    ...
    if (len != 6 + 2 * nb_components) {
        av_em_log(s->avctx, AV_LOG_ERROR, "decode_sos: invalid len (%d)\n", len);
        return AVERROR_INVALIDDATA;
    }
    for (i = 0; i < nb_components; i++) {
        id = get_bits(&s->gb, 8) - 1;
        av_em_log(s->avctx, AV_LOG_DEBUG, "component: %d\n", id);
    ...
    if (s->avctx->debug & FF_DEBUG_PICT_INFO)
        av_em_log(s->avctx, AV_LOG_DEBUG, "%s %s p:%d >>:%d ilv:%d bits:%d skip:%d %s comp:%d\n",
...
//            reset_ls_coding_parameters(s, 0);
    ...
    if (s->interlaced &&
        ...
        if (show_bits(&bak, 16) == 0xFFD1) {
            av_em_log(s->avctx, AV_LOG_DEBUG, "AVRn interlaced picture marker found\n");
            s->gb = bak;
            skip_bits(&s->gb, 16);
            s->bottom_field ^= 1;
    ...
    return 0;
 out_of_range:
    av_em_log(s->avctx, AV_LOG_ERROR, "decode_sos: ac/dc index out of range\n");
    return AVERROR_INVALIDDATA;
}
...
static int mjpeg_decode_dri(MJpegDecodeContext *s)
{
    ...
    s->restart_count    = 0;
    av_em_log(s->avctx, AV_LOG_DEBUG, "restart interval: %d\n",
           s->restart_interval);
...
static int mjpeg_decode_app(MJpegDecodeContext *s)
{
    ...
    if (s->avctx->debug & FF_DEBUG_STARTCODE) {
        char id_str[32];
        av_em_get_codec_tag_string(id_str, sizeof(id_str), av_bswap32(id));
        av_em_log(s->avctx, AV_LOG_DEBUG, "APPx (%s / %8X) len=%d\n", id_str, id, len);
    }
    ...
    if (id == AV_RB32("AVI1")) {
        ...
        */
        ...
        av_em_log(s->avctx, AV_LOG_DEBUG, "polarity %d\n", i);
#if 0
        skip_bits(&s->gb, 8);
        skip_bits(&s->gb, 32);
        skip_bits(&s->gb, 32);
...
//    len -= 2;
    ...
    if (   id == AV_RB32("Adob")
        ...
        if (s->avctx->debug & FF_DEBUG_PICT_INFO)
            av_em_log(s->avctx, AV_LOG_INFO, "mjpeg: Adobe header found, transform=%d\n", s->adobe_transform);
        len -= 7;
        goto out;
    }
    ...
    if (id == AV_RB32("LJIF")) {
        ...
        case 2:
            ...
            break;
        default:
            av_em_log(s->avctx, AV_LOG_ERROR, "unknown colorspace %d\n", i);
        }

        len -= 9;
        if (s->got_picture)
            if (rgb != s->rgb || pegasus_rct != s->pegasus_rct) {
                av_em_log(s->avctx, AV_LOG_WARNING, "Mismatching LJIF tag\n");
                goto out;
            }
    ...
    if (id == AV_RL32("colr") && len > 0) {
        ...
        if (s->avctx->debug & FF_DEBUG_PICT_INFO)
            av_em_log(s->avctx, AV_LOG_INFO, "COLR %d\n", s->colr);
        len --;
        goto out;
    }
    if (id == AV_RL32("xfrm") && len > 0) {
        ...
        if (s->avctx->debug & FF_DEBUG_PICT_INFO)
            av_em_log(s->avctx, AV_LOG_INFO, "XFRM %d\n", s->xfrm);
        len --;
        goto out;
    }
    ...
    if (s->start_code == APP3 && id == AV_RB32("_JPS") && len >= 10) {
        ...
        if (s->avctx->debug & FF_DEBUG_PICT_INFO)
            av_em_log(s->avctx, AV_LOG_INFO, "_JPSJPS_\n");
        ...
        s->stereo3d = av_em_stereo3d_alloc();
        if (!s->stereo3d) {
            goto out;
        }
        if (type == 0) {
    ...
    if (s->start_code == APP1 && id == AV_RB32("Exif") && len >= 2) {
        ...
        if (ret) {
            av_em_log(s->avctx, AV_LOG_ERROR, "mjpeg: invalid TIFF header in EXIF data\n");
        } else {
            ...
            if (ret < 0) {
                av_em_log(s->avctx, AV_LOG_ERROR, "mjpeg: error decoding EXIF data\n");
            }
        }
...
#endif
            if (s->avctx->debug & FF_DEBUG_PICT_INFO)
                av_em_log(s->avctx, AV_LOG_INFO, "mjpeg: Apple MJPEG-A header found\n");
        }
    }
...
static int mjpeg_decode_com(MJpegDecodeContext *s)
{
    ...
    if (len >= 2 && 8 * len - 16 <= get_bits_left(&s->gb)) {
        ...
        if (s->avctx->debug & FF_DEBUG_PICT_INFO)
            av_em_log(s->avctx, AV_LOG_INFO, "comment: '%s'\n", cbuf);
        ...
        if (!strncmp(cbuf, "AVID", 4)) {
            ...
            s->avctx->sample_aspect_ratio = (AVEMRational) { 1, 2 };
            s->multiscope = 2;
        }
                         ...
                         int *unescaped_buf_size)
{
    ...
    av_em_fast_padded_malloc(&s->buffer, &s->buffer_size, buf_end - *buf_ptr);
    if (!s->buffer)
        return AVERROR(ENOMEM);
    ...
    if (start_code == SOS && !s->ls) {
        ...
        *unescaped_buf_size = dst - s->buffer;
        ...
        av_em_log(s->avctx, AV_LOG_DEBUG, "escaping removed %"PTRDIFF_SPECIFIER" bytes\n",
        ...
        while (b < t) {
            ...
            if (x == 0xFF && b < t) {
                x = src[b++];
                if (x & 0x80) {
                    av_em_log(s->avctx, AV_LOG_WARNING, "Invalid escape sequence\n");
                    x &= 0x7f;
                }
                put_bits(&pb, 7, x);
                bit_count--;
...
int ff_mjpeg_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                          AVEMPacket *avpkt)
{
    AVFrame     *frame = data;
    ...
    av_em_dict_free(&s->exif_metadata);
    av_em_freep(&s->stereo3d);
    s->adobe_transform = -1;
    ...
    while (buf_ptr < buf_end) {
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "marker=%x avail_size_in_buf=%"PTRDIFF_SPECIFIER"\n",
               start_code, buf_end - buf_ptr);

        ret = init_get_bits8(&s->gb, unescaped_buf_ptr, unescaped_buf_size);

        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "invalid buffer\n");
            goto fail;
        }

        s->start_code = start_code;
        if (s->avctx->debug & FF_DEBUG_STARTCODE)
            av_em_log(avctx, AV_LOG_DEBUG, "startcode: %X\n", start_code);
        ...
        if (!CONFIG_JPEGLS_DECODER &&
            ...
            av_em_log(avctx, AV_LOG_ERROR, "JPEG-LS support not enabled.\n");
        ...
        case DHT:
            if ((ret = ff_mjpeg_decode_dht(s)) < 0) {
                av_em_log(avctx, AV_LOG_ERROR, "huffman table decode error\n");
                goto fail;
            }
            break;
        case SOF0:
...
eoi_parser:
            ...
            if ((ret = av_em_frame_ref(frame, s->picture_ptr)) < 0)
                return ret;
            *got_frame = 1;
            s->got_picture = 0;
            ...
            if (!s->lossless) {
                ...
                AVEMBufferRef *qp_table_buf = av_em_buffer_alloc(qpw);
                if (qp_table_buf) {
                    memset(qp_table_buf->data, qp, qpw);
                    av_em_frame_set_qp_table(data, qp_table_buf, 0, FF_QSCALE_TYPE_MPEG1);
                }

                if(avctx->debug & FF_DEBUG_QP)
                    av_em_log(avctx, AV_LOG_DEBUG, "QP: %d\n", qp);
            }
...
skip:
    ...
    if (s->got_picture && s->cur_scan) {
        av_em_log(avctx, AV_LOG_WARNING, "EOI missing, emulating\n");
        goto eoi_parser;
    }
    av_em_log(avctx, AV_LOG_FATAL, "No JPEG data found in image\n");
    return AVERROR_INVALIDDATA;
fail:
    s->got_picture = 0;
    return ret;
the_end:
    ...
    is16bit = av_em_pix_fmt_desc_get(s->avctx->pix_fmt)->comp[0].step > 1;
    ...
    if (AV_RB32(s->upscale_h)) {
        ...
        avcodec_em_get_chroma_sub_sample(s->avctx->pix_fmt, &hshift, &vshift);
        av_assert0(s->nb_components == av_em_pix_fmt_count_planes(s->picture_ptr->format));
    ...
    if (s->stereo3d) {
        AVStereo3D *stereo = av_em_stereo3d_create_side_data(data);
        if (stereo) {
    ...
    av_em_dict_copy(avempriv_frame_get_metadatap(data), s->exif_metadata, 0);
    av_em_dict_free(&s->exif_metadata);

the_end_no_picture:
    av_em_log(avctx, AV_LOG_DEBUG, "decode frame unused %"PTRDIFF_SPECIFIER" bytes\n",
           buf_end - buf_ptr);
//  return buf_end - buf_ptr;
    return buf_ptr - buf;
}

av_cold int ff_mjpeg_decode_end(AVEMCodecContext *avctx)
{
    MJpegDecodeContext *s = avctx->priv_data;
    int i, j;

    if (s->interlaced && s->bottom_field == !s->interlace_polarity && s->got_picture && !avctx->frame_number) {
        av_em_log(avctx, AV_LOG_INFO, "Single field\n");
    }

    if (s->picture) {
        av_em_frame_free(&s->picture);
        s->picture_ptr = NULL;
...
static void decode_flush(AVEMCodecContext *avctx)
{
    MJpegDecodeContext *s = avctx->priv_data;
    s->got_picture = 0;
}
...
static const AVEMClass mjpegdec_class = {
