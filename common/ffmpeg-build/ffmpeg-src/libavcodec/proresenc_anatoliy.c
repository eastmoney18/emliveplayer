...
static void encode_ac_coeffs(AVEMCodecContext *avctx, PutBitContext *pb,
...
static int encode_slice_plane(AVEMCodecContext *avctx, int mb_count,
...
static av_always_inline unsigned encode_slice_data(AVEMCodecContext *avctx,
...
static int encode_slice(AVEMCodecContext *avctx, const AVFrame *pic, int mb_x,
...
static int prores_encode_picture(AVEMCodecContext *avctx, const AVFrame *pic,
...
static int prores_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                               const AVFrame *pict, int *got_packet)
{
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, frame_size + AV_INPUT_BUFFER_MIN_SIZE, 0)) < 0)
        return ret;
...
static av_cold int prores_encode_init(AVEMCodecContext *avctx)
{
    int i;
    ProresContext* ctx = avctx->priv_data;

    if (avctx->pix_fmt != AV_PIX_FMT_YUV422P10) {
        av_em_log(avctx, AV_LOG_ERROR, "need YUV422P10\n");
    ...
    if ((avctx->height & 0xf) || (avctx->width & 0xf)) {
        ctx->fill_y = av_em_alloc(4 * (DEFAULT_SLICE_MB_WIDTH << 8));
        if (!ctx->fill_y)
...
static av_cold int prores_encode_close(AVEMCodecContext *avctx)
{
    ProresContext* ctx = avctx->priv_data;
    av_em_freep(&ctx->fill_y);

    return 0;
}

AVEMCodec ff_prores_aw_encoder = {
    .name           = "prores_aw",
