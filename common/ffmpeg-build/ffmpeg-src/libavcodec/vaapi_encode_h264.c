...
static int vaapi_encode_h264_write_sequence_header(AVEMCodecContext *avctx,
...
static int vaapi_encode_h264_write_slice_header(AVEMCodecContext *avctx,
                                                VAAPIEncodePicture *pic,
                                                VAAPIEncodeSlice *slice,
                                                char *data, size_t *data_len)
{
...
static int vaapi_encode_h264_init_sequence_params(AVEMCodecContext *avctx)
{
    VAAPIEncodeContext                 *ctx = avctx->priv_data;
    VAEncSequenceParameterBufferH264  *vseq = ctx->codec_sequence_params;
    VAEncPictureParameterBufferH264   *vpic = ctx->codec_picture_params;
...
static int vaapi_encode_h264_init_picture_params(AVEMCodecContext *avctx,
                                                 VAAPIEncodePicture *pic)
{
    VAAPIEncodeContext                *ctx = avctx->priv_data;
    VAEncSequenceParameterBufferH264 *vseq = ctx->codec_sequence_params;
...
static int vaapi_encode_h264_init_slice_params(AVEMCodecContext *avctx,
                                               VAAPIEncodePicture *pic,
                                               VAAPIEncodeSlice *slice)
{
    VAAPIEncodeContext                 *ctx = avctx->priv_data;
    ...
    slice->priv_data = av_em_mallocz(sizeof(*pslice));
    if (!slice->priv_data)
...
static av_cold int vaapi_encode_h264_init_constant_bitrate(AVEMCodecContext *avctx)
{
    VAAPIEncodeContext      *ctx = avctx->priv_data;
    VAAPIEncodeH264Context *priv = ctx->priv_data;
    int hrd_buffer_size;
    ...
    if (avctx->bit_rate > INT32_MAX) {
        av_em_log(avctx, AV_LOG_ERROR, "Target bitrate of 2^31 bps or "
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Using constant-bitrate = %"PRId64" bps.\n",
           avctx->bit_rate);
    return 0;
}

static av_cold int vaapi_encode_h264_init_fixed_qp(AVEMCodecContext *avctx)
{
    VAAPIEncodeContext      *ctx = avctx->priv_data;
    VAAPIEncodeH264Context *priv = ctx->priv_data;
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Using fixed QP = "
           "%d / %d / %d for IDR- / P- / B-frames.\n",
           priv->fixed_qp_idr, priv->fixed_qp_p, priv->fixed_qp_b);
    return 0;
}

static av_cold int vaapi_encode_h264_init_internal(AVEMCodecContext *avctx)
{
    ...
    case FF_PROFILE_H264_EXTENDED:
        av_em_log(avctx, AV_LOG_ERROR, "H.264 extended profile "
               "is not supported.\n");
    ...
    case FF_PROFILE_H264_HIGH_10_INTRA:
        av_em_log(avctx, AV_LOG_ERROR, "H.264 10-bit profiles "
               "are not supported.\n");
        return AVERROR_PATCHWELCOME;
    ...
    case FF_PROFILE_H264_CAVLC_444:
        av_em_log(avctx, AV_LOG_ERROR, "H.264 non-4:2:0 profiles "
               "are not supported.\n");
        return AVERROR_PATCHWELCOME;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unknown H.264 profile %d.\n",
               avctx->profile);
        return AVERROR(EINVAL);
    }
    if (opt->low_power) {
...
#else
        av_em_log(avctx, AV_LOG_ERROR, "Low-power encoding is not "
...
#else
        av_em_log(avctx, AV_LOG_WARNING, "The encode quality option is not "
               "supported with this VAAPI version.\n");
#endif
    }
...
static av_cold int vaapi_encode_h264_init(AVEMCodecContext *avctx)
{
...
static const AVEMCodecDefault vaapi_encode_h264_defaults[] = {
    { "profile",        "100" },
    { "level",          "51"  },
    { "b",              "0"   },
    { "bf",             "2"   },
...
static const AVEMClass vaapi_encode_h264_class = {
...
AVEMCodec em_h264_vaapi_encoder = {
    .name           = "h264_vaapi",
