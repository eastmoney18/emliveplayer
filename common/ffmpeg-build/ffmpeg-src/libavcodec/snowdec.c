...
static int decode_q_branch(SnowContext *s, int level, int x, int y){
    ...
    if(level==s->block_max_depth || get_rac(&s->c, &s->block_state[4 + s_context])){
        ...
        if(type){
            ...
            if (ref >= s->ref_frames) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Invalid ref\n");
...
#define GET_S(dst, check) \
    ...
    if(!(check)){\
        av_em_log(s->avctx, AV_LOG_ERROR, "Error " #dst " is %d\n", tmp);\
        return AVERROR_INVALIDDATA;\
    }\
    dst= tmp;
    ...
    if(s->keyframe){
        ...
        if (s->colorspace_type == 1) {
            ...
            if(s->chroma_h_shift == 1 && s->chroma_v_shift==1){
                ...
                av_em_log(s, AV_LOG_ERROR, "unsupported color subsample mode %d %d\n", s->chroma_h_shift, s->chroma_v_shift);
            ...
            av_em_log(s, AV_LOG_ERROR, "unsupported color space\n");
            s->chroma_h_shift = s->chroma_v_shift = 1;
            s->avctx->pix_fmt= AV_PIX_FMT_YUV420P;
            return AVERROR_INVALIDDATA;
        }
...
//        s->rate_scalability= get_rac(&s->c, s->header_state);
    ...
    if(s->spatial_decomposition_type > 1U){
        av_em_log(s->avctx, AV_LOG_ERROR, "spatial_decomposition_type %d not supported\n", s->spatial_decomposition_type);
        return AVERROR_INVALIDDATA;
    }
    if(FFMIN(s->avctx-> width>>s->chroma_h_shift,
             s->avctx->height>>s->chroma_v_shift) >> (s->spatial_decomposition_count-1) <= 1){
        av_em_log(s->avctx, AV_LOG_ERROR, "spatial_decomposition_count %d too large for size\n", s->spatial_decomposition_count);
        return AVERROR_INVALIDDATA;
    }
    ...
    if(s->block_max_depth > 1 || s->block_max_depth < 0){
        av_em_log(s->avctx, AV_LOG_ERROR, "block_max_depth= %d is too large\n", s->block_max_depth);
        s->block_max_depth= 0;
        return AVERROR_INVALIDDATA;
    }
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (s->avctx->flags2 & AV_CODEC_FLAG2_EXPORT_MVS) {
        s->avmv = av_em_malloc_array(s->b_width * s->b_height, sizeof(AVMotionVector) << (s->block_max_depth*2));
    }
    s->avmv_index = 0;
    ...
    if(!(s->avctx->debug&2048))
        res = av_em_frame_ref(picture, s->current_picture);
    else
        res = av_em_frame_ref(picture, s->mconly_picture);
    if (res >= 0 && s->avmv_index) {
        AVFrameSideData *sd;

        sd = av_em_frame_new_side_data(picture, AV_FRAME_DATA_MOTION_VECTORS, s->avmv_index * sizeof(AVMotionVector));
        if (!sd)
    ...
    *got_frame = 1;
    ...
    if(bytes_read ==0) av_em_log(s->avctx, AV_LOG_ERROR, "error at end of frame\n"); //FIXME
