...
static av_cold int jpg_init(AVEMCodecContext *avctx, JPGContext *c)
{
    int ret;
...
static ePICPixHashElem *epic_hash_add(ePICPixHash *hash, uint32_t key)
{
    ...
    if (!(hash->bucket_fill[idx] < hash->bucket_size[idx])) {
        ...
        bucket = av_em_realloc(hash->bucket[idx], new_size * sizeof(*bucket));
        if (!bucket)
            return NULL;
...
static int epic_add_pixel_to_cache(ePICPixHash *hash, uint32_t key, uint32_t pix)
{
    ...
    new_elem = av_em_mallocz(sizeof(*new_elem));
    if (!new_elem)
        return AVERROR(ENOMEM);
    ...
    for (i = 0; i < EPIC_HASH_SIZE; i++) {
        for (j = 0; j < hash->bucket_fill[i]; j++) {
            ...
            while (list_elem) {
                ...
                av_em_free(list_elem);
                list_elem = tmp;
            }
        }
        av_em_freep(&hash->bucket[i]);
        hash->bucket_size[i] =
                                       ...
                                       const uint32_t *above_row)
{
    ...
    if (R<0 || G<0 || B<0) {
        av_em_log(NULL, AV_LOG_ERROR, "RGB %d %d %d is out of range\n", R, G, B);
        return 0;
    }
                               ...
                               AVEMCodecContext *avctx)
{
    ...
    if (extrabytes > 3 || src_size < extrabytes) {
        av_em_log(avctx, AV_LOG_ERROR, "ePIC: invalid data size VLI\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (src_size < els_dsize) {
        av_em_log(avctx, AV_LOG_ERROR, "ePIC: data too short, needed %"SIZE_SPECIFIER", got %"SIZE_SPECIFIER"\n",
               els_dsize, src_size);
        return AVERROR_INVALIDDATA;
    }
...
static int g2m_init_buffers(G2MContext *c)
{
    ...
    if (!c->framebuf || c->old_width < c->width || c->old_height < c->height) {
        ...
        av_em_free(c->framebuf);
        c->framebuf = av_em_mallocz_array(c->framebuf_stride, aligned_height);
        if (!c->framebuf)
            return AVERROR(ENOMEM);
    }
    if (!c->synth_tile || !c->jpeg_tile ||
        ...
        av_em_freep(&c->synth_tile);
        av_em_freep(&c->jpeg_tile);
        av_em_freep(&c->kempf_buf);
        av_em_freep(&c->kempf_flags);
        av_em_freep(&c->epic_buf_base);
        c->epic_buf    = NULL;
        c->synth_tile  = av_em_mallocz(c->tile_stride      * aligned_height);
        c->jpeg_tile   = av_em_mallocz(c->tile_stride      * aligned_height);
        c->kempf_buf   = av_em_mallocz((c->tile_width + 1) * aligned_height +
                                    AV_INPUT_BUFFER_PADDING_SIZE);
        c->kempf_flags = av_em_mallocz(c->tile_width       * aligned_height);
        if (!c->synth_tile || !c->jpeg_tile ||
            !c->kempf_buf || !c->kempf_flags)
            return AVERROR(ENOMEM);
        if (c->compression == 2) {
            c->epic_buf_base = av_em_mallocz(c->epic_buf_stride * aligned_height + 4);
            if (!c->epic_buf_base)
                return AVERROR(ENOMEM);
...
static int g2m_load_cursor(AVEMCodecContext *avctx, G2MContext *c,
                           GetByteContext *gb)
{
    int i, j, k;
    uint8_t *dst;
    ...
    if (cursor_w < 1 || cursor_w > 256 ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Invalid cursor dimensions %"PRIu32"x%"PRIu32"\n",
               cursor_w, cursor_h);
        return AVERROR_INVALIDDATA;
    }
    if (cursor_hot_x > cursor_w || cursor_hot_y > cursor_h) {
        av_em_log(avctx, AV_LOG_WARNING, "Invalid hotspot position %"PRIu32",%"PRIu32"\n",
    ...
    if (cur_size - 9 > bytestream2_get_bytes_left(gb) ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Invalid cursor data size %"PRIu32"/%u\n",
               cur_size, bytestream2_get_bytes_left(gb));
        return AVERROR_INVALIDDATA;
    }
    if (cursor_fmt != 1 && cursor_fmt != 32) {
        avpriv_em_report_missing_feature(avctx, "Cursor format %d",
                                      cursor_fmt);
        return AVERROR_PATCHWELCOME;
    }

    if ((err = av_em_reallocp(&c->cursor, cursor_stride * cursor_h)) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Cannot allocate cursor buffer\n");
        return err;
    }
...
static int g2m_decode_frame(AVEMCodecContext *avctx, void *data,
                            int *got_picture_ptr, AVEMPacket *avpkt)
{
    ...
    if ((magic & ~0xF) != MKBETAG('G', '2', 'M', '0') ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Wrong magic %08X\n", magic);
        return AVERROR_INVALIDDATA;
    }
    ...
    while (bytestream2_get_bytes_left(&bc) > 5) {
        ...
        if (chunk_size > bytestream2_get_bytes_left(&bc)) {
            av_em_log(avctx, AV_LOG_ERROR, "Invalid chunk size %"PRIu32" type %02X\n",
        ...
        case DISPLAY_INFO:
            ...
            c->got_header = 0;
            if (chunk_size < 21) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid display info size %"PRIu32"\n",
                       chunk_size);
                break;
            }
            ...
            if (c->width != avctx->width || c->height != avctx->height) {
                ret = em_set_dimensions(avctx, c->width, c->height);
                if (ret < 0)
                    goto header_fail;
            }
            ...
            if (c->bpp == 32) {
                ...
                avpriv_em_request_sample(avctx, "bpp=%d", c->bpp);
                ret = AVERROR_PATCHWELCOME;
                goto header_fail;
            }
        ...
        case TILE_DATA:
            ...
            if (chunk_size < 2) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid tile data size %"PRIu32"\n",
                       chunk_size);
                break;
            }
            ...
            if (ret && c->framebuf)
                av_em_log(avctx, AV_LOG_ERROR, "Error decoding tile %d,%d\n",
                       c->tile_x, c->tile_y);
            break;
        case CURSOR_POS:
            if (chunk_size < 5) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid cursor pos size %"PRIu32"\n",
                       chunk_size);
                break;
            }
        ...
        case CURSOR_SHAPE:
            if (chunk_size < 8) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid cursor data size %"PRIu32"\n",
                       chunk_size);
                break;
            }
        ...
        case CHUNK_CD:
            break;
        default:
            av_em_log(avctx, AV_LOG_WARNING, "Skipping chunk type %02d\n",
                   chunk_type);
        }
    ...
    if (c->width && c->height && c->framebuf) {
        if ((ret = em_get_buffer(avctx, pic, 0)) < 0)
            return ret;
...
static av_cold int g2m_decode_init(AVEMCodecContext *avctx)
{
    G2MContext *const c = avctx->priv_data;
    int ret;

    if ((ret = jpg_init(avctx, &c->jc)) != 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Cannot initialise VLCs\n");
...
static av_cold int g2m_decode_end(AVEMCodecContext *avctx)
{
    G2MContext *const c = avctx->priv_data;
    ...
    av_em_freep(&c->synth_tile);
    av_em_freep(&c->jpeg_tile);
    av_em_freep(&c->cursor);
    av_em_freep(&c->framebuf);

    return 0;
}

AVEMCodec ff_g2m_decoder = {
    .name           = "g2m",
