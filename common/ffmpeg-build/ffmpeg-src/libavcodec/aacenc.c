 ...
 */
static void put_audio_specific_config(AVEMCodecContext *avctx)
{
    PutBitContext pb;
...
void em_quantize_band_cost_cache_init(struct AACEncContext *s)
{
...
{
    const float *lwindow = sce->ics.use_kb_window[0] ? em_aac_kbd_long_1024 : ff_sine_1024;
    const float *pwindow = sce->ics.use_kb_window[1] ? em_aac_kbd_long_1024 : ff_sine_1024;
...
{
    const float *lwindow = sce->ics.use_kb_window[1] ? em_aac_kbd_long_1024 : ff_sine_1024;
    const float *swindow = sce->ics.use_kb_window[0] ? em_aac_kbd_short_128 : ff_sine_128;
    float *out = sce->ret_buf;
...
{
    const float *lwindow = sce->ics.use_kb_window[0] ? em_aac_kbd_long_1024 : ff_sine_1024;
    const float *swindow = sce->ics.use_kb_window[1] ? em_aac_kbd_short_128 : ff_sine_128;
    float *out = sce->ret_buf;
...
{
    const float *swindow = sce->ics.use_kb_window[0] ? em_aac_kbd_short_128 : ff_sine_128;
    const float *pwindow = sce->ics.use_kb_window[1] ? em_aac_kbd_short_128 : ff_sine_128;
 ...
 * Encode scalefactors.
 */
static void encode_scale_factors(AVEMCodecContext *avctx, AACEncContext *s,
                                 SingleChannelElement *sce)
{
    ...
    for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
        for (i = 0; i < sce->ics.max_sfb; i++) {
            if (!sce->zeroes[w*16 + i]) {
                ...
                put_bits(&s->pb, em_aac_scalefactor_bits[diff], em_aac_scalefactor_code[diff]);
            }
        }
    }
}
 ...
 */
static int encode_individual_channel(AVEMCodecContext *avctx, AACEncContext *s,
                                     SingleChannelElement *sce,
                                     int common_window)
{
...
static int aac_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                            const AVFrame *frame, int *got_packet_ptr)
{
        ...
        for (ch = 0; ch < chans; ch++) {
            ...
            for (k = 0; k < 1024; k++) {
                if (!isfinite(cpe->ch[ch].coeffs[k])) {
                    av_em_log(avctx, AV_LOG_ERROR, "Input contains NaN/+-Inf\n");
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, 8192 * s->channels, 0)) < 0)
        return ret;
    frame_bits = its = 0;
    do {
...
static av_cold int aac_encode_end(AVEMCodecContext *avctx)
{
    AACEncContext *s = avctx->priv_data;

    av_em_log(avctx, AV_LOG_INFO, "Qavg: %.3f\n", s->lambda_sum / s->lambda_count);
...
static av_cold int dsp_init(AVEMCodecContext *avctx, AACEncContext *s)
{
    int ret = 0;
    ...
    // window init
    ff_kbd_window_init(em_aac_kbd_long_1024, 4.0, 1024);
    ff_kbd_window_init(em_aac_kbd_short_128, 6.0, 128);
...
static av_cold int alloc_buffers(AVEMCodecContext *avctx, AACEncContext *s)
{
    int ch;
...
static av_cold int aac_encode_init(AVEMCodecContext *avctx)
{
    AACEncContext *s = avctx->priv_data;
    int i, ret = 0;
    ...
    ERROR_IF(s->samplerate_index == 16 ||
             s->samplerate_index >= em_aac_swb_size_1024_len ||
             s->samplerate_index >= em_aac_swb_size_128_len,
    ...
    /* Coder limitations */
    s->coder = &em_aac_coders[s->options.coder];
    ...
    sizes[0]   = em_aac_swb_size_1024[s->samplerate_index];
    sizes[1]   = em_aac_swb_size_128[s->samplerate_index];
    lengths[0] = em_aac_num_swb_1024[s->samplerate_index];
    lengths[1] = em_aac_num_swb_128[s->samplerate_index];
    for (i = 0; i < s->chan_map[0]; i++)
    ...
    av_em_lfg_init(&s->lfg, 0x72adca55);

    if (HAVE_MIPSDSP)
        em_aac_coder_init_mips(s);
...
static const AVEMClass aacenc_class = {
    "AAC encoder",
...
static const AVEMCodecDefault aac_encode_defaults[] = {
    { "b", "0" },
    { NULL }
};

AVEMCodec em_aac_encoder = {
    .name           = "aac",
