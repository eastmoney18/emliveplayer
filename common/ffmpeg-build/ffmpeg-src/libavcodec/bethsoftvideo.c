...
static av_cold int bethsoftvid_decode_init(AVEMCodecContext *avctx)
{
    BethsoftvidContext *vid = avctx->priv_data;
    avctx->pix_fmt = AV_PIX_FMT_PAL8;

    vid->frame = av_em_frame_alloc();
    if (!vid->frame)
...
static int bethsoftvid_decode_frame(AVEMCodecContext *avctx,
                              ...
                              AVEMPacket *avpkt)
{
    ...
    if ((ret = em_reget_buffer(avctx, vid->frame)) < 0)
        return ret;
    ...
    switch(block_type = bytestream2_get_byte(&vid->g)){
        case PALETTE_BLOCK: {
            *got_frame = 0;
            if ((ret = set_palette(vid)) < 0) {
                av_em_log(avctx, AV_LOG_ERROR, "error reading palette\n");
                return ret;
            }
    ...
    if ((ret = av_em_frame_ref(data, vid->frame)) < 0)
        return ret;
...
static av_cold int bethsoftvid_decode_end(AVEMCodecContext *avctx)
{
    BethsoftvidContext * vid = avctx->priv_data;
    av_em_frame_free(&vid->frame);
    return 0;
}

AVEMCodec ff_bethsoftvid_decoder = {
    .name           = "bethsoftvid",
