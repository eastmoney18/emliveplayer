...
{
    if (!crc_init) {
        av_em_crc_init(crc_63, 0,  8,   0x63, sizeof(crc_63));
        av_em_crc_init(crc_1D, 0,  8,   0x1D, sizeof(crc_1D));
        av_em_crc_init(crc_2D, 0, 16, 0x002D, sizeof(crc_2D));
        crc_init = 1;
    }
}
...
uint16_t ff_mlp_checksum16(const uint8_t *buf, unsigned int buf_size)
{
    ...
    crc = av_em_crc(crc_2D, 0, buf, buf_size - 2);
...
{
    uint8_t checksum = av_em_crc(crc_63, 0x3c, buf, buf_size - 1); // crc_63[0xa2] == 0x3c
...
uint8_t ff_mlp_restart_checksum(const uint8_t *buf, unsigned int bit_size)
{
    ...
    crc = av_em_crc(crc_1D, crc, buf + 1, num_bytes - 2);
    crc ^= buf[num_bytes - 1];
