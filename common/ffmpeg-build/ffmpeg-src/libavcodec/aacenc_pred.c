...
void em_aac_apply_main_pred(AACEncContext *s, SingleChannelElement *sce)
{
    int sfb, k;
    const int pmax = FFMIN(sce->ics.max_sfb, em_aac_pred_sfb_max[s->samplerate_index]);
...
void em_aac_adjust_common_pred(AACEncContext *s, ChannelElement *cpe)
{
    int start, w, w2, g, i, count = 0;
    SingleChannelElement *sce0 = &cpe->ch[0];
    SingleChannelElement *sce1 = &cpe->ch[1];
    const int pmax0 = FFMIN(sce0->ics.max_sfb, em_aac_pred_sfb_max[s->samplerate_index]);
    const int pmax1 = FFMIN(sce1->ics.max_sfb, em_aac_pred_sfb_max[s->samplerate_index]);
    ...
    for (w = 0; w < sce0->ics.num_windows; w += sce0->ics.group_len[w]) {
        ...
        for (g = 0; g < sce0->ics.num_swb; g++) {
            ...
            ph_err1 = em_aac_is_encoding_err(s, cpe, start, w, g,
                                             ener0, ener1, ener01, 1, -1);
            ph_err2 = em_aac_is_encoding_err(s, cpe, start, w, g,
                                             ener0, ener1, ener01, 1, +1);
...
void em_aac_search_for_pred(AACEncContext *s, SingleChannelElement *sce)
{
 ...
 */
void em_aac_encode_main_pred(AACEncContext *s, SingleChannelElement *sce)
{
    int sfb;
    IndividualChannelStream *ics = &sce->ics;
    const int pmax = FFMIN(ics->max_sfb, em_aac_pred_sfb_max[s->samplerate_index]);
