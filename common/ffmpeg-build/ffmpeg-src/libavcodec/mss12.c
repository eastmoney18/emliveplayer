...
{
    AVEMCodecContext *avctx = c->avctx;
    int i;

    if (avctx->extradata_size < 52 + 256 * 3) {
        av_em_log(avctx, AV_LOG_ERROR, "Insufficient extradata size %d\n",
    ...
    if (avctx->coded_width > 4096 || avctx->coded_height > 4096) {
        av_em_log(avctx, AV_LOG_ERROR, "Frame dimensions %dx%d too large",
               avctx->coded_width, avctx->coded_height);
        return AVERROR_INVALIDDATA;
    }
    if (avctx->coded_width < 1 || avctx->coded_height < 1) {
        av_em_log(avctx, AV_LOG_ERROR, "Frame dimensions %dx%d too small",
               avctx->coded_width, avctx->coded_height);
        return AVERROR_INVALIDDATA;
    }

    av_em_log(avctx, AV_LOG_DEBUG, "Encoder version %"PRIu32".%"PRIu32"\n",
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "%d free colour(s)\n", c->free_colours);

    av_em_log(avctx, AV_LOG_DEBUG, "Display dimensions %"PRIu32"x%"PRIu32"\n",
           AV_RB32(avctx->extradata + 12), AV_RB32(avctx->extradata + 16));
    av_em_log(avctx, AV_LOG_DEBUG, "Coded dimensions %dx%d\n",
           avctx->coded_width, avctx->coded_height);
    av_em_log(avctx, AV_LOG_DEBUG, "%g frames per second\n",
           av_int2float(AV_RB32(avctx->extradata + 28)));
    av_em_log(avctx, AV_LOG_DEBUG, "Bitrate %"PRIu32" bps\n",
           AV_RB32(avctx->extradata + 32));
    av_em_log(avctx, AV_LOG_DEBUG, "Max. lead time %g ms\n",
           av_int2float(AV_RB32(avctx->extradata + 36)));
    av_em_log(avctx, AV_LOG_DEBUG, "Max. lag time %g ms\n",
           av_int2float(AV_RB32(avctx->extradata + 40)));
    av_em_log(avctx, AV_LOG_DEBUG, "Max. seek time %g ms\n",
    ...
    if (version) {
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Slice split %d\n", c->slice_split);
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Used colours %d\n",
               c->full_model_syms);
    } else {
        c->slice_split     = 0;
    ...
    c->mask        = av_em_malloc_array(c->mask_stride, avctx->height);
    if (!c->mask) {
        av_em_log(avctx, AV_LOG_ERROR, "Cannot allocate mask plane\n");
