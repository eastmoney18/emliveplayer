...
static av_cold int rscc_init(AVEMCodecContext *avctx)
{
    RsccContext *ctx = avctx->priv_data;

    /* These needs to be set to estimate uncompressed buffer */
    int ret = av_em_image_check_size(avctx->width, avctx->height, 0, avctx);
    if (ret < 0) {
    ...
    ctx->reference = av_em_frame_alloc();
    if (!ctx->reference)
    ...
    if (avctx->codec_tag == MKTAG('I', 'S', 'C', 'C')) {
        ...
        case 8:
            avpriv_em_report_missing_feature(avctx, "8 bits per pixel");
        ...
        case 32:
            ...
            break;
        default:
            av_em_log(avctx, AV_LOG_ERROR, "Invalid bits per pixel value (%d)\n",
        ...
        ctx->component_size = 4;
        av_em_log(avctx, AV_LOG_WARNING, "Invalid codec tag\n");
    }
    ...
    ctx->inflated_buf = av_em_alloc(ctx->inflated_size);
    if (!ctx->inflated_buf)
        return AVERROR(ENOMEM);
...
static av_cold int rscc_close(AVEMCodecContext *avctx)
{
    RsccContext *ctx = avctx->priv_data;

    av_em_freep(&ctx->tiles);
    av_em_freep(&ctx->inflated_buf);
    av_em_frame_free(&ctx->reference);

    return 0;
}

static int rscc_decode_frame(AVEMCodecContext *avctx, void *data,
                                     int *got_frame, AVEMPacket *avpkt)
{
    ...
    av_em_fast_malloc(&ctx->tiles, &ctx->tiles_size,
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "Frame with %d tiles.\n", tiles_nb);
    ...
    if (tiles_nb > 5) {
        ...
        if (packed_tiles_size != tiles_nb * TILE_SIZE) {
            ...
            inflated_tiles = av_em_alloc(length);
            if (!inflated_tiles) {
                ret = AVERROR(ENOMEM);
                goto end;
            }
            ...
            if (ret) {
                av_em_log(avctx, AV_LOG_ERROR, "Tile deflate error %d.\n", ret);
                ret = AVERROR_UNKNOWN;
                goto end;
            }
    ...
    if (pixel_size == packed_size) {
        if (bytestream2_get_bytes_left(gbc) < pixel_size) {
            av_em_log(avctx, AV_LOG_ERROR, "Insufficient input for %d\n", pixel_size);
            ret = AVERROR_INVALIDDATA;
            goto end;
        }
        pixels = gbc->buffer;
        ...
        if (ret) {
            av_em_log(avctx, AV_LOG_ERROR, "Pixel deflate error %d.\n", ret);
            ret = AVERROR_UNKNOWN;
            goto end;
        }
    ...
    ret = em_reget_buffer(avctx, ctx->reference);
    if (ret < 0)
        goto end;
    ...
    for (i = 0; i < tiles_nb; i++) {
        ...
        av_em_image_copy_plane(dst, -1 * ctx->reference->linesize[0],
    ...
    ret = av_em_frame_ref(frame, ctx->reference);
    if (ret < 0)
        goto end;
    ...
    *got_frame = 1;
    ...
    av_em_free(inflated_tiles);
    return ret;
}

AVEMCodec ff_rscc_decoder = {
    .name           = "rscc",
