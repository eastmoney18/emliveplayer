...
static av_cold int shorten_decode_init(AVEMCodecContext *avctx)
{
...
static int allocate_buffers(ShortenContext *s)
{
    ...
    for (chan = 0; chan < s->channels; chan++) {
        if (FFMAX(1, s->nmean) >= UINT_MAX / sizeof(int32_t)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "nmean too large\n");
        ...
        if ((err = av_em_reallocp_array(&s->offset[chan],
                               sizeof(int32_t),
                               FFMAX(1, s->nmean))) < 0)
            return err;

        if ((err = av_em_reallocp_array(&s->decoded_base[chan], (s->blocksize + s->nwrap),
    ...
    if ((err = av_em_reallocp_array(&s->coeffs, s->nwrap, sizeof(*s->coeffs))) < 0)
        return err;
...
static int init_offset(ShortenContext *s)
{
    ...
    case TYPE_S16LH:
        ...
        break;
    default:
        av_em_log(s->avctx, AV_LOG_ERROR, "unknown audio type\n");
...
static int decode_aiem_header(AVEMCodecContext *avctx, const uint8_t *header,
                              int header_size)
{
    int len, bps, exp;
    GetByteContext gb;
    ...
    if (bytestream2_get_le32(&gb) != MKTAG('F', 'O', 'R', 'M')) {
        av_em_log(avctx, AV_LOG_ERROR, "missing FORM tag\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (tag != MKTAG('A', 'I', 'F', 'F')) {
        av_em_log(avctx, AV_LOG_ERROR, "missing AIFF tag\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    while (bytestream2_get_le32(&gb) != MKTAG('C', 'O', 'M', 'M')) {
        ...
        if (len < 0 || bytestream2_get_bytes_left(&gb) < 18) {
            av_em_log(avctx, AV_LOG_ERROR, "no COMM chunk found\n");
            return AVERROR_INVALIDDATA;
        }
    }
    len = bytestream2_get_be32(&gb);
    ...
    if (len < 18) {
        av_em_log(avctx, AV_LOG_ERROR, "COMM chunk was too short\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (bps != 16 && bps != 8) {
        av_em_log(avctx, AV_LOG_ERROR, "unsupported number of bits per sample: %d\n", bps);
    ...
    if (exp < -63 || exp > 63) {
        av_em_log(avctx, AV_LOG_ERROR, "exp %d is out of range\n", exp);
        return AVERROR_INVALIDDATA;
    }
    if (exp >= 0)
    ...
    len -= 18;
    if (len > 0)
        av_em_log(avctx, AV_LOG_INFO, "%d header bytes unparsed\n", len);

    return 0;
}

static int decode_wave_header(AVEMCodecContext *avctx, const uint8_t *header,
                              int header_size)
{
    int len, bps;
    short wave_format;
    ...
    if (bytestream2_get_le32(&gb) != MKTAG('W', 'A', 'V', 'E')) {
        av_em_log(avctx, AV_LOG_ERROR, "missing WAVE tag\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    while (bytestream2_get_le32(&gb) != MKTAG('f', 'm', 't', ' ')) {
        ...
        if (len < 0 || bytestream2_get_bytes_left(&gb) < 16) {
            av_em_log(avctx, AV_LOG_ERROR, "no fmt chunk found\n");
            return AVERROR_INVALIDDATA;
        }
    }
    len = bytestream2_get_le32(&gb);
    ...
    if (len < 16) {
        av_em_log(avctx, AV_LOG_ERROR, "fmt chunk was too short\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    case WAVE_FORMAT_PCM:
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "unsupported wave format\n");
        return AVERROR(ENOSYS);
    }
                               ...
                               int residual_size, int32_t coffset)
{
    ...
    if (command == FN_QLPC) {
        ...
        if ((unsigned)pred_order > s->nwrap) {
            av_em_log(s->avctx, AV_LOG_ERROR, "invalid pred_order %d\n",
                   pred_order);
...
static int read_header(ShortenContext *s)
{
    ...
    if (get_bits_long(&s->gb, 32) != AV_RB32("ajkg")) {
        av_em_log(s->avctx, AV_LOG_ERROR, "missing shorten magic 'ajkg'\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (!s->channels) {
        av_em_log(s->avctx, AV_LOG_ERROR, "No channels reported\n");
        return AVERROR_INVALIDDATA;
    }
    if (s->channels > MAX_CHANNELS) {
        av_em_log(s->avctx, AV_LOG_ERROR, "too many channels: %d\n", s->channels);
        s->channels = 0;
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->version > 0) {
        ...
        if ((unsigned)skip_bytes > get_bits_left(&s->gb)/8) {
            av_em_log(s->avctx, AV_LOG_ERROR, "invalid skip_bytes: %d\n", skip_bytes);
            return AVERROR_INVALIDDATA;
        }
    ...
    if (s->header_size >= OUT_BUFFER_SIZE ||
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "header is wrong size: %d\n",
               s->header_size);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (AV_RL32(s->header) == MKTAG('R','I','F','F')) {
        ...
        if ((ret = decode_aiem_header(s->avctx, s->header, s->header_size)) < 0)
            return ret;
    } else {
        avpriv_em_report_missing_feature(s->avctx, "unsupported bit packing %X", AV_RL32(s->header));
        return AVERROR_PATCHWELCOME;
    }
...
static int shorten_decode_frame(AVEMCodecContext *avctx, void *data,
                                int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (s->max_framesize == 0) {
        ...
        tmp_ptr = av_em_fast_realloc(s->bitstream, &s->allocated_bitstream_size,
                                  s->max_framesize + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!tmp_ptr) {
            s->max_framesize = 0;
            av_em_log(avctx, AV_LOG_ERROR, "error allocating bitstream buffer\n");
    ...
    while (s->cur_chan < s->channels) {
        ...
        if (cmd > FN_VERBATIM) {
            av_em_log(avctx, AV_LOG_ERROR, "unknown shorten function %d\n", cmd);
            *got_frame_ptr = 0;
            break;
        }
        ...
        if (!is_audio_command[cmd]) {
            ...
            case FN_BITSHIFT: {
                ...
                if (bitshift > 32) {
                    av_em_log(avctx, AV_LOG_ERROR, "bitshift %d is invalid\n",
                           bitshift);
                    return AVERROR_INVALIDDATA;
                }
                s->bitshift = bitshift;
            ...
            case FN_BLOCKSIZE: {
                ...
                if (!blocksize || blocksize > MAX_BLOCKSIZE) {
                    av_em_log(avctx, AV_LOG_ERROR, "invalid or unsupported "
                                                "block size: %d\n", blocksize);
                    return AVERROR(EINVAL);
                }
                s->blocksize = blocksize;
...
static av_cold int shorten_decode_close(AVEMCodecContext *avctx)
{
    ShortenContext *s = avctx->priv_data;
    int i;
    ...
    for (i = 0; i < s->channels; i++) {
        s->decoded[i] = NULL;
        av_em_freep(&s->decoded_base[i]);
        av_em_freep(&s->offset[i]);
    }
    av_em_freep(&s->bitstream);
    av_em_freep(&s->coeffs);

    return 0;
}

AVEMCodec ff_shorten_decoder = {
    .name           = "shorten",
