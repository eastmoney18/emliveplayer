...
static av_cold int init_bundles(BinkContext *c)
{
    ...
    for (i = 0; i < BINKB_NB_SRC; i++) {
        c->bundle[i].data = av_em_mallocz(blocks * 64);
        if (!c->bundle[i].data)
...
static int read_runs(AVEMCodecContext *avctx, GetBitContext *gb, Bundle *b)
{
    int t, v;
    ...
    if (dec_end > b->data_end) {
        av_em_log(avctx, AV_LOG_ERROR, "Run value went out of bounds\n");
...
static int read_motion_values(AVEMCodecContext *avctx, GetBitContext *gb, Bundle *b)
{
    int t, sign, v;
    const uint8_t *dec_end;
    ...
    dec_end = b->cur_dec + t;
    if (dec_end > b->data_end) {
        av_em_log(avctx, AV_LOG_ERROR, "Too many motion values\n");
        return AVERROR_INVALIDDATA;
    }
    if (get_bits1(gb)) {
        v = get_bits(gb, 4);
...
static int read_block_types(AVEMCodecContext *avctx, GetBitContext *gb, Bundle *b)
{
    int t, v;
    int last = 0;
    const uint8_t *dec_end;
    ...
    dec_end = b->cur_dec + t;
    if (dec_end > b->data_end) {
        av_em_log(avctx, AV_LOG_ERROR, "Too many block type values\n");
        return AVERROR_INVALIDDATA;
    }
    if (get_bits1(gb)) {
        v = get_bits(gb, 4);
...
static int read_patterns(AVEMCodecContext *avctx, GetBitContext *gb, Bundle *b)
{
    int t, v;
    const uint8_t *dec_end;
    ...
    dec_end = b->cur_dec + t;
    if (dec_end > b->data_end) {
        av_em_log(avctx, AV_LOG_ERROR, "Too many pattern values\n");
        return AVERROR_INVALIDDATA;
    }
    while (b->cur_dec < dec_end) {
        v  = GET_HUFF(gb, b->tree);
...
static int read_colors(GetBitContext *gb, Bundle *b, BinkContext *c)
{
    ...
    dec_end = b->cur_dec + t;
    if (dec_end > b->data_end) {
        av_em_log(c->avctx, AV_LOG_ERROR, "Too many color values\n");
        return AVERROR_INVALIDDATA;
    }
    if (get_bits1(gb)) {
...
static int read_dcs(AVEMCodecContext *avctx, GetBitContext *gb, Bundle *b,
    ...
    for (i = 0; i < len; i += 8) {
        ...
        if (bsize) {
            for (j = 0; j < len2; j++) {
                ...
                *dst++ = v;
                if (v < -32768 || v > 32767) {
                    av_em_log(avctx, AV_LOG_ERROR, "DC value went out of bounds: %d\n", v);
                    return AVERROR_INVALIDDATA;
                }
            }
        } else {
                           ...
                           const int32_t quant_matrices[16][64], int q)
{
    ...
    if (q == -1) {
        ...
        quant_idx = q;
        if (quant_idx > 15U) {
            av_em_log(NULL, AV_LOG_ERROR, "quant_index %d out of range\n", quant_idx);
            return AVERROR_INVALIDDATA;
        }
    }
        ...
        for (bx = 0; bx < bw; bx++, dst += 8) {
            ...
            case 1:
                ...
                do {
                    ...
                    i += run;
                    if (i > 64) {
                        av_em_log(c->avctx, AV_LOG_ERROR, "Run went out of bounds\n");
                        return AVERROR_INVALIDDATA;
                    }
                    if (mode) {
            ...
            case 3:
                ...
                if (ref < ref_start || ref + 8*stride > ref_end) {
                    av_em_log(c->avctx, AV_LOG_WARNING, "Reference block is out of bounds\n");
            ...
            case 8:
                ...
                break;
            default:
                av_em_log(c->avctx, AV_LOG_ERROR, "Unknown block type %d\n", blk);
                return AVERROR_INVALIDDATA;
            }
        }
    }
                           ...
                           uint8_t *ref_end)
{
    ...
    if (ref < ref_start || ref > ref_end) {
        av_em_log(c->avctx, AV_LOG_ERROR, "Copy out of bounds @%d, %d\n",
               xoff, yoff);
        return AVERROR_INVALIDDATA;
    }
    c->hdsp.put_pixels_tab[1][0](dst, ref, stride, 8);
        ...
        for (bx = 0; bx < bw; bx++, dst += 8, prev += 8) {
            ...
            case SCALED_BLOCK:
                ...
                case RAW_BLOCK:
                    ...
                    break;
                default:
                    av_em_log(c->avctx, AV_LOG_ERROR, "Incorrect 16x16 block type %d\n", blk);
                    return AVERROR_INVALIDDATA;
                }
                if (blk != FILL_BLOCK)
...
static int decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame, AVEMPacket *pkt)
{
    ...
    if (c->version > 'b') {
        if ((ret = em_get_buffer(avctx, frame, AV_GET_BUFFER_FLAG_REF)) < 0)
            return ret;
    } else {
        if ((ret = em_reget_buffer(avctx, c->last)) < 0)
            return ret;
        if ((ret = av_em_frame_ref(frame, c->last)) < 0)
            return ret;
    }
    ...
    if (c->version > 'b') {
        av_em_frame_unref(c->last);
        if ((ret = av_em_frame_ref(c->last, frame)) < 0)
            return ret;
    }
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->extradata_size < 4) {
        av_em_log(avctx, AV_LOG_ERROR, "Extradata missing or too short\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    em_hpeldsp_init(&c->hdsp, avctx->flags);
    em_binkdsp_init(&c->binkdsp);
