...
static int wavesynth_parse_extradata(AVEMCodecContext *avc)
{
    ...
    if (ws->nb_inter < 0)
        return AVERROR(EINVAL);
    ws->inter = av_em_calloc(ws->nb_inter, sizeof(*ws->inter));
    if (!ws->inter)
...
static av_cold int wavesynth_init(AVEMCodecContext *avc)
{
    struct wavesynth_context *ws = avc->priv_data;
    int i, r;
    ...
    if (r < 0) {
        av_em_log(avc, AV_LOG_ERROR, "Invalid intervals definitions.\n");
        goto fail;
    }
    ws->sin = av_em_alloc(sizeof(*ws->sin) << SIN_BITS);
    if (!ws->sin) {
        r = AVERROR(ENOMEM);
        goto fail;
    }
...
static int wavesynth_decode(AVEMCodecContext *avc, void *rframe, int *rgot_frame,
                            AVEMPacket *packet)
{
    struct wavesynth_context *ws = avc->priv_data;
...
static av_cold int wavesynth_close(AVEMCodecContext *avc)
{
    struct wavesynth_context *ws = avc->priv_data;
...
AVEMCodec ff_ffwavesynth_decoder = {
    .name           = "wavesynth",
