...
static int realloc_buffer(LAMEContext *s)
{
    if (!s->buffer || s->buffer_size - s->buffer_index < BUFFER_SIZE) {
        ...
        ff_dlog(s->avctx, "resizing output buffer: %d -> %d\n", s->buffer_size,
                new_size);
        if ((err = av_em_reallocp(&s->buffer, new_size)) < 0) {
...
static av_cold int mp3lame_encode_close(AVEMCodecContext *avctx)
{
    LAMEContext *s = avctx->priv_data;

    av_em_freep(&s->samples_flt[0]);
    av_em_freep(&s->samples_flt[1]);
...
static av_cold int mp3lame_encode_init(AVEMCodecContext *avctx)
{
    LAMEContext *s = avctx->priv_data;
    int ret;
    ...
    if (avctx->sample_fmt == AV_SAMPLE_FMT_FLTP) {
        ...
        for (ch = 0; ch < avctx->channels; ch++) {
            s->samples_flt[ch] = av_em_malloc_array(avctx->frame_size,
...
static int mp3lame_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                                const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if (frame) {
        ...
        case AV_SAMPLE_FMT_FLTP:
            if (frame->linesize[0] < 4 * FFALIGN(frame->nb_samples, 8)) {
                av_em_log(avctx, AV_LOG_ERROR, "inadequate AVFrame plane padding\n");
    ...
    ret = realloc_buffer(s);
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "error reallocating output buffer\n");
        return ret;
    }
    ...
    if (ret < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid mp3 header at start of buffer\n");
        return AVERROR_BUG;
    } else if (ret) {
        av_em_log(avctx, AV_LOG_ERROR, "free format output not supported\n");
        return -1;
    }
    len = hdr.frame_size;
    ...
    if (len <= s->buffer_index) {
        if ((ret = em_alloc_packet2(avctx, avpkt, len, 0)) < 0)
            return ret;
...
static const AVEMClass libmp3lame_class = {
...
static const AVEMCodecDefault libmp3lame_defaults[] = {
    { "b",          "0" },
    { NULL },
};
...
AVEMCodec ff_libmp3lame_encoder = {
    .name                  = "libmp3lame",
