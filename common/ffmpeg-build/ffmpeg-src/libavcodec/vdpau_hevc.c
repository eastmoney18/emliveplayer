...
static int vdpau_hevc_start_frame(AVEMCodecContext *avctx,
                                  const uint8_t *buffer, uint32_t size)
{
    ...
    for (size_t i = 0; i < 6; i++) {
        for (size_t j = 0; j < 16; j++) {
            ...
            uint8_t pos = 4 * em_hevc_diag_scan4x4_y[j] + em_hevc_diag_scan4x4_x[j];
            info->ScalingList4x4[i][j] = sl->sl[0][i][pos];
        }
        for (size_t j = 0; j < 64; j++) {
            uint8_t pos = 8 * em_hevc_diag_scan8x8_y[j] + em_hevc_diag_scan8x8_x[j];
    ...
    info->NumPocTotalCurr = em_hevc_frame_nb_refs(h);
    ...
    for (ssize_t i = 0, j = 0; i < h->rps[ST_CURR_BEF].nb_refs; i++) {
        ...
        if (frame) {
            ...
            if (!found) {
                av_em_log(avctx, AV_LOG_WARNING, "missing surface: %p\n",
                       (void *)id);
            }
        } else {
            av_em_log(avctx, AV_LOG_WARNING, "missing STR Before frame: %zd\n", i);
        }
    }
    /* Reference Picture Set list, one of the short-term RPS. These
       correspond to positions in the RefPics array. */
    for (ssize_t i = 0, j = 0; i < h->rps[ST_CURR_AFT].nb_refs; i++) {
        ...
        if (frame) {
            ...
            av_em_log(avctx, AV_LOG_WARNING, "missing STR After frame: %zd\n", i);
        }
    }
    ...
    for (ssize_t i = 0, j = 0; i < h->rps[LT_CURR].nb_refs; i++) {
        ...
        if (frame) {
            ...
            av_em_log(avctx, AV_LOG_WARNING, "missing LTR frame: %zd\n", i);
        }
    }
...
static int vdpau_hevc_decode_slice(AVEMCodecContext *avctx,
                                   const uint8_t *buffer, uint32_t size)
{
    HEVCContext *h = avctx->priv_data;
...
static int vdpau_hevc_end_frame(AVEMCodecContext *avctx)
{
    HEVCContext *h = avctx->priv_data;
    struct vdpau_picture_context *pic_ctx = h->ref->hwaccel_picture_private;
    int val;
...
static int vdpau_hevc_init(AVEMCodecContext *avctx)
{
...
AVHWAccel em_hevc_vdpau_hwaccel = {
    .name           = "hevc_vdpau",
