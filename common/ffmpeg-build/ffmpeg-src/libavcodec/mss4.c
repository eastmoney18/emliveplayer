...
static int mss4_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                             AVEMPacket *avpkt)
{
        ...
        for (x = 0; x < mb_width; x++) {
            ...
            case SKIP_BLOCK:
                if (frame_type == INTRA_FRAME) {
                    av_em_log(avctx, AV_LOG_ERROR, "Skip block in intra frame\n");
                    return AVERROR_INVALIDDATA;
                }
                break;
            }
...
static av_cold int mss4_decode_end(AVEMCodecContext *avctx)
{
    ...
    for (i = 0; i < 3; i++)
        av_em_freep(&c->prev_dc[i]);
    mss4_free_vlcs(c);

    return 0;
}

static av_cold int mss4_decode_init(AVEMCodecContext *avctx)
{
    MSS4Context * const c = avctx->priv_data;
    int i;
    ...
    for (i = 0; i < 3; i++) {
        ...
        c->prev_dc[i]   = av_em_malloc_array(c->dc_stride[i], sizeof(**c->prev_dc));
        if (!c->prev_dc[i]) {
