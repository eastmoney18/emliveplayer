...
static av_cold int xan_decode_end(AVEMCodecContext *avctx)
{
    XanContext *s = avctx->priv_data;

    av_em_frame_free(&s->pic);

    av_em_freep(&s->y_buffer);
    av_em_freep(&s->scratch_buffer);

    return 0;
}

static av_cold int xan_decode_init(AVEMCodecContext *avctx)
{
    XanContext *s = avctx->priv_data;
    ...
    if (avctx->height < 8) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid frame height: %d.\n", avctx->height);
        return AVERROR(EINVAL);
    }
    if (avctx->width & 1) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid frame width: %d.\n", avctx->width);
        return AVERROR(EINVAL);
    }

    s->buffer_size = avctx->width * avctx->height;
    s->y_buffer = av_em_alloc(s->buffer_size);
    if (!s->y_buffer)
        return AVERROR(ENOMEM);
    s->scratch_buffer = av_em_alloc(s->buffer_size + 130);
    if (!s->scratch_buffer) {
        xan_decode_end(avctx);
        return AVERROR(ENOMEM);
    }

    s->pic = av_em_frame_alloc();
    if (!s->pic) {
        xan_decode_end(avctx);
        return AVERROR(ENOMEM);
    }
                      ...
                      uint8_t *dest, const int dest_len)
{
    ...
    while (dest < dest_end) {
        ...
        if (opcode < 0xe0) {
            ...
            dest += size;
            av_em_memcpy_backptr(dest, back, size2);
            dest += size2;
        } else {
...
static int xan_decode_chroma(AVEMCodecContext *avctx, unsigned chroma_off)
{
    XanContext *s = avctx->priv_data;
    uint8_t *U, *V;
    int val, uval, vval;
    ...
    if (chroma_off + 4 >= bytestream2_get_bytes_left(&s->gb)) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid chroma block position\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (offset >= bytestream2_get_bytes_left(&s->gb)) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid chroma block offset\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (dec_size < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Chroma unpacking failed\n");
        return dec_size;
    }
...
static int xan_decode_frame_type0(AVEMCodecContext *avctx)
{
    XanContext *s = avctx->priv_data;
    ...
    if (corr_off >= bytestream2_size(&s->gb)) {
        av_em_log(avctx, AV_LOG_WARNING, "Ignoring invalid correction block position\n");
        corr_off = 0;
    }
    bytestream2_seek(&s->gb, 12, SEEK_SET);
    ret = xan_unpack_luma(s, src, s->buffer_size >> 1);
    if (ret) {
        av_em_log(avctx, AV_LOG_ERROR, "Luma decoding failed\n");
        return ret;
    }
...
static int xan_decode_frame_type1(AVEMCodecContext *avctx)
{
    XanContext *s = avctx->priv_data;
...
static int xan_decode_frame(AVEMCodecContext *avctx,
                            ...
                            AVEMPacket *avpkt)
{
    ...
    if ((ret = em_reget_buffer(avctx, s->pic)) < 0)
        return ret;
    ...
    case 1:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unknown frame type %d\n", ftype);
        return AVERROR_INVALIDDATA;
    }
    if (ret)
        return ret;

    if ((ret = av_em_frame_ref(data, s->pic)) < 0)
        return ret;
