...
static int tak_parse(AVEMCodecParserContext *s, AVEMCodecContext *avctx,
                     ...
                     const uint8_t *buf, int buf_size)
{
    ...
    while (buf_size || t->index + needed <= pc->index) {
        if (buf_size && t->index + TAK_MAX_FRAME_HEADER_BYTES > pc->index) {
            ...
            if (em_combine_frame(pc, END_NOT_FOUND, &tmp_buf, &tmp_buf_size) != -1)
