                                 ...
                                 int type, int id, int *channels)
{
    ...
    if (che_pos) {
        if (!ac->che[type][id]) {
            if (!(ac->che[type][id] = av_em_mallocz(sizeof(ChannelElement))))
                return AVERROR(ENOMEM);
            AAC_RENAME(em_aac_sbr_ctx_init)(ac, &ac->che[type][id]->sbr);
        }
        if (type != TYPE_CCE) {
            if (*channels >= MAX_CHANNELS - (type == TYPE_CPE || (type == TYPE_SCE && ac->oc[1].m4ac.ps == 1))) {
                av_em_log(ac->avctx, AV_LOG_ERROR, "Too many channels\n");
                return AVERROR_INVALIDDATA;
            }
        ...
        if (ac->che[type][id])
            AAC_RENAME(em_aac_sbr_ctx_close)(&ac->che[type][id]->sbr);
        av_em_freep(&ac->che[type][id]);
    }
    return 0;
}

static int frame_configure_elements(AVEMCodecContext *avctx)
{
    ...
    /* get output buffer */
    av_em_frame_unref(ac->frame);
    if (!avctx->channels)
        return 1;

    ac->frame->nb_samples = 2048;
    if ((ret = em_get_buffer(avctx, ac->frame, 0)) < 0)
        return ret;
...
static int output_configure(AACContext *ac,
                            ...
                            enum OCStatus oc_type, int get_new_frame)
{
    AVEMCodecContext *avctx = ac->avctx;
    ...
    for (i = 0; i < tags; i++) {
        ...
        if (id_map[type][id] >= MAX_ELEM_ID) {
            avpriv_em_request_sample(ac->avctx, "Remapped id too large\n");
 ...
 * @return  Returns error status. 0 - OK, !0 - error
 */
static int set_default_channel_config(AVEMCodecContext *avctx,
                                      ...
                                      int channel_config)
{
    ...
    *tags = tags_per_config[channel_config];
    ...
    if (channel_config == 7 && avctx->strict_std_compliance < FF_COMPLIANCE_STRICT) {
        av_em_log(avctx, AV_LOG_INFO, "Assuming an incorrectly encoded 7.1 channel layout"
...
static ChannelElement *get_che(AACContext *ac, int type, int elem_id)
{
    ...
    if (!ac->tags_mapped && type == TYPE_CPE &&
        ...
        av_em_log(ac->avctx, AV_LOG_DEBUG, "mono with CPE\n");
    ...
    if (!ac->tags_mapped && type == TYPE_SCE &&
        ...
        av_em_log(ac->avctx, AV_LOG_DEBUG, "stereo with SCE\n");
    ...
    case 6:
        ...
        if (ac->tags_mapped == tags_per_config[ac->oc[1].m4ac.chan_config] - 1 && (type == TYPE_LFE || type == TYPE_SCE)) {
            if (!ac->warned_remapping_once && (type != TYPE_LFE || elem_id != 0)) {
                av_em_log(ac->avctx, AV_LOG_WARNING,
 ...
 * @return  Returns error status. 0 - OK, !0 - error
 */
static int decode_pce(AVEMCodecContext *avctx, MPEG4AudioConfig *m4ac,
                      uint8_t (*layout_map)[3],
                      GetBitContext *gb)
{
    ...
    if (get_bits_left(gb) < 4 * (num_front + num_side + num_back + num_lfe + num_assoc_data + num_cc)) {
        av_em_log(avctx, AV_LOG_ERROR, "decode_pce: " overread_err);
        return -1;
    }
 ...
 * @return  Returns error status. 0 - OK, !0 - error
 */
static int decode_ga_specific_config(AACContext *ac, AVEMCodecContext *avctx,
                                     GetBitContext *gb,
                                     ...
                                     int channel_config)
{
    ...
    if (get_bits1(gb)) { // frameLengthFlag
        avpriv_em_request_sample(avctx, "960/120 MDCT window");
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (extension_flag) {
        ...
        case AOT_ER_AAC_LD:
            ...
            if (res_flags) {
                avpriv_em_report_missing_feature(avctx,
...
static int decode_eld_specific_config(AACContext *ac, AVEMCodecContext *avctx,
                                     GetBitContext *gb,
                                     MPEG4AudioConfig *m4ac,
                                     int channel_config)
{
    ...
    while (get_bits(gb, 4) != ELDEXT_TERM) {
        ...
        if (get_bits_left(gb) < len * 8 + 4) {
            av_em_log(avctx, AV_LOG_ERROR, overread_err);
            return AVERROR_INVALIDDATA;
        }
                                        ...
                                        int sync_extension)
{
    ...
    if (bit_size < 0 || bit_size > INT_MAX) {
        av_em_log(avctx, AV_LOG_ERROR, "Audio specific config size is invalid\n");
        return AVERROR_INVALIDDATA;
    }
...
#define AAC_INIT_VLC_STATIC(num, size)                                     \
    INIT_VLC_STATIC(&vlc_spectral[num], 8, em_aac_spectral_sizes[num],     \
         em_aac_spectral_bits[num], sizeof(em_aac_spectral_bits[num][0]),  \
                                    sizeof(em_aac_spectral_bits[num][0]),  \
        em_aac_spectral_codes[num], sizeof(em_aac_spectral_codes[num][0]), \
                                    sizeof(em_aac_spectral_codes[num][0]), \
        size);
...
static av_cold void aac_static_table_init(void)
{
    ...
    AAC_RENAME(em_aac_sbr_init)();

    em_aac_tableinit();

    INIT_VLC_STATIC(&vlc_scalefactors, 7,
                    FF_ARRAY_ELEMS(em_aac_scalefactor_code),
                    em_aac_scalefactor_bits,
                    sizeof(em_aac_scalefactor_bits[0]),
                    sizeof(em_aac_scalefactor_bits[0]),
                    em_aac_scalefactor_code,
                    sizeof(em_aac_scalefactor_code[0]),
                    sizeof(em_aac_scalefactor_code[0]),
                    352);

    // window initialization
    AAC_RENAME(ff_kbd_window_init)(AAC_RENAME(em_aac_kbd_long_1024), 4.0, 1024);
    AAC_RENAME(ff_kbd_window_init)(AAC_RENAME(em_aac_kbd_short_128), 6.0, 128);
...
static av_cold int aac_decode_init(AVEMCodecContext *avctx)
{
    AACContext *ac = avctx->priv_data;
    int ret;
    ...
    if (avctx->channels > MAX_CHANNELS) {
        av_em_log(avctx, AV_LOG_ERROR, "Too many channels\n");
        return AVERROR_INVALIDDATA;
    }

#if USE_FIXED
    ac->fdsp = avpriv_em_alloc_fixed_dsp(avctx->flags & AV_CODEC_FLAG_BITEXACT);
#else
    ac->fdsp = avpriv_em_float_dsp_alloc(avctx->flags & AV_CODEC_FLAG_BITEXACT);
#endif /* USE_FIXED */
    if (!ac->fdsp) {
        return AVERROR(ENOMEM);
    }
...
static int skip_data_stream_element(AACContext *ac, GetBitContext *gb)
{
    ...
    if (get_bits_left(gb) < 8 * count) {
        av_em_log(ac->avctx, AV_LOG_ERROR, "skip_data_stream_element: "overread_err);
        return AVERROR_INVALIDDATA;
    }
                             ...
                             GetBitContext *gb)
{
    ...
    if (get_bits1(gb)) {
        ...
        if (ics->predictor_reset_group == 0 ||
            ...
            av_em_log(ac->avctx, AV_LOG_ERROR,
                   "Invalid Predictor Reset Group.\n");
            return AVERROR_INVALIDDATA;
        }
    }
    for (sfb = 0; sfb < FFMIN(ics->max_sfb, em_aac_pred_sfb_max[ac->oc[1].m4ac.sampling_index]); sfb++) {
                           ...
                           GetBitContext *gb)
{
    ...
    if (aot != AOT_ER_AAC_ELD) {
        if (get_bits1(gb)) {
            av_em_log(ac->avctx, AV_LOG_ERROR, "Reserved bit set.\n");
    ...
    if (ics->window_sequence[0] == EIGHT_SHORT_SEQUENCE) {
        ...
        ics->num_swb           =   em_aac_num_swb_128[sampling_index];
        ...
        if (aot == AOT_ER_AAC_LD || aot == AOT_ER_AAC_ELD) {
            ...
            ics->num_swb       =   em_aac_num_swb_1024[sampling_index];
    ...
    for (g = 0; g < ics->num_window_groups; g++) {
        ...
        while (k < ics->max_sfb) {
            ...
            if (sect_band_type == 12) {
                av_em_log(ac->avctx, AV_LOG_ERROR, "invalid band type\n");
                return AVERROR_INVALIDDATA;
            }
            do {
                ...
                if (get_bits_left(gb) < 0) {
                    av_em_log(ac->avctx, AV_LOG_ERROR, "decode_band_types: "overread_err);
                    return AVERROR_INVALIDDATA;
                }
            ...
            if (band_type[idx] == ZERO_BT) {
                ...
                for (; i < run_end; i++, idx++) {
                    ...
                    if (offset[2] != clipped_offset) {
                        avpriv_em_request_sample(ac->avctx,
...
#else
                    sf[idx] = em_aac_pow2sf_tab[-clipped_offset + POW_SF2_ZERO];
#endif /* USE_FIXED */
                }
                ...
                for (; i < run_end; i++, idx++) {
                    ...
                    sf[idx] = -offset[0];
#else
                    sf[idx] = -em_aac_pow2sf_tab[offset[0] - 100 + POW_SF2_ZERO];
#endif /* USE_FIXED */
                }
            }
        }
...
#if !USE_FIXED
                const float *vq = em_aac_codebook_vector_vals[cbt_m1];
#endif /* !USE_FIXED */
                const uint16_t *cb_vector_idx = em_aac_codebook_vector_idx[cbt_m1];
                        ...
                        do {
                            ...
                            for (j = 0; j < 2; j++) {
                                if (nzt & 1<<j) {
                                    ...
                                    if (b > 8) {
                                        av_em_log(ac->avctx, AV_LOG_ERROR, "error in spectral data, ESC overflow\n");
                                        return AVERROR_INVALIDDATA;
                                    }
...
static void apply_prediction(AACContext *ac, SingleChannelElement *sce)
{
    ...
    if (sce->ics.window_sequence[0] != EIGHT_SHORT_SEQUENCE) {
        for (sfb = 0;
             sfb < em_aac_pred_sfb_max[ac->oc[1].m4ac.sampling_index];
             sfb++) {
                      ...
                      GetBitContext *gb, int common_window, int scale_flag)
{
    ...
    if (!scale_flag) {
        ...
        if (!eld_syntax && get_bits1(gb)) {
            avpriv_em_request_sample(ac->avctx, "SSR");
            return AVERROR_PATCHWELCOME;
        }
...
static int decode_cpe(AACContext *ac, GetBitContext *gb, ChannelElement *cpe)
{
    ...
    if (common_window) {
        ...
        if (ms_present == 3) {
            av_em_log(ac->avctx, AV_LOG_ERROR, "ms_present = 3 is reserved.\n");
            return AVERROR_INVALIDDATA;
        } else if (ms_present)
...
static int decode_fill(AACContext *ac, GetBitContext *gb, int len) {
    ...
    if (ac->avctx->debug & FF_DEBUG_PICT_INFO)
        av_em_log(ac->avctx, AV_LOG_DEBUG, "FILL:%s\n", buf);
                                    ...
                                    ChannelElement *che, enum RawDataBlockType elem_type)
{
    ...
    if (ac->avctx->debug & FF_DEBUG_STARTCODE)
        av_em_log(ac->avctx, AV_LOG_DEBUG, "extension type: %d len:%d\n", type, cnt);
    ...
    case EXT_SBR_DATA:
        if (!che) {
            av_em_log(ac->avctx, AV_LOG_ERROR, "SBR was found before the first channel element.\n");
            return res;
        } else if (!ac->oc[1].m4ac.sbr) {
            av_em_log(ac->avctx, AV_LOG_ERROR, "SBR signaled to be not-present but was found in the bitstream.\n");
            skip_bits_long(gb, 8 * cnt - 4);
            return res;
        } else if (ac->oc[1].m4ac.sbr == -1 && ac->oc[1].status == OC_LOCKED) {
            av_em_log(ac->avctx, AV_LOG_ERROR, "Implicit SBR was found with a first occurrence after the first frame.\n");
            skip_bits_long(gb, 8 * cnt - 4);
            return res;
...
{
    const INTFLOAT *lwindow      = ics->use_kb_window[0] ? AAC_RENAME(em_aac_kbd_long_1024) : AAC_RENAME(ff_sine_1024);
    const INTFLOAT *swindow      = ics->use_kb_window[0] ? AAC_RENAME(em_aac_kbd_short_128) : AAC_RENAME(ff_sine_128);
    const INTFLOAT *lwindow_prev = ics->use_kb_window[1] ? AAC_RENAME(em_aac_kbd_long_1024) : AAC_RENAME(ff_sine_1024);
    const INTFLOAT *swindow_prev = ics->use_kb_window[1] ? AAC_RENAME(em_aac_kbd_short_128) : AAC_RENAME(ff_sine_128);
...
static void imdct_and_windowing_eld(AACContext *ac, SingleChannelElement *sce)
{
    ...
    const int n2 = n >> 1;
    const int n4 = n >> 2;
    const INTFLOAT *const window = n == 480 ? AAC_RENAME(em_aac_eld_window_480) :
                                           AAC_RENAME(em_aac_eld_window_512);
    ...
    for (type = 3; type >= 0; type--) {
        for (i = 0; i < MAX_ELEM_ID; i++) {
            ...
            if (che && che->present) {
                ...
                av_em_log(ac->avctx, AV_LOG_VERBOSE, "ChannelElement %d.%d missing \n", type, i);
            }
        }
    }
}
...
static int parse_adts_frame_header(AACContext *ac, GetBitContext *gb)
{
    ...
    size = avpriv_em_aac_parse_header(gb, &hdr_info);
    if (size > 0) {
        if (!ac->warned_num_aac_frames && hdr_info.num_aac_frames != 1) {
            // This is 2 for "VLB " audio in NSV files.
            // See samples/nsv/vlb_audio.
            avpriv_em_report_missing_feature(ac->avctx,
...
static int aac_decode_er_frame(AVEMCodecContext *avctx, void *data,
                               int *got_frame_ptr, GetBitContext *gb)
{
    ...
    if (chan_config < 0 || (chan_config >= 8 && chan_config < 11) || chan_config >= 13) {
        avpriv_em_request_sample(avctx, "Unknown ER channel configuration %d",
                              chan_config);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (!ac->frame->data[0] && samples) {
        av_em_log(avctx, AV_LOG_ERROR, "no frame data found\n");
        return AVERROR_INVALIDDATA;
    }
...
static int aac_decode_frame_int(AVEMCodecContext *avctx, void *data,
                                int *got_frame_ptr, GetBitContext *gb, AVEMPacket *avpkt)
{
    AACContext *ac = avctx->priv_data;
    ...
    if (show_bits(gb, 12) == 0xfff) {
        if ((err = parse_adts_frame_header(ac, gb)) < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "Error decoding AAC frame header.\n");
            goto fail;
        }
        if (ac->oc[1].m4ac.sampling_index > 12) {
            av_em_log(ac->avctx, AV_LOG_ERROR, "invalid sampling rate index %d\n", ac->oc[1].m4ac.sampling_index);
    ...
    while ((elem_type = get_bits(gb, 3)) != TYPE_END) {
        ...
        if (avctx->debug & FF_DEBUG_STARTCODE)
            av_em_log(avctx, AV_LOG_DEBUG, "Elem type:%x id:%x\n", elem_type, elem_id);
        ...
        if (elem_type < TYPE_DSE) {
            if (!(che=get_che(ac, elem_type, elem_id))) {
                av_em_log(ac->avctx, AV_LOG_ERROR, "channel element %d.%d is not allocated\n",
                       elem_type, elem_id);
                err = AVERROR_INVALIDDATA;
                goto fail;
            }
        ...
        case TYPE_FIL:
            ...
            if (get_bits_left(gb) < 8 * elem_id) {
                    av_em_log(avctx, AV_LOG_ERROR, "TYPE_FIL: "overread_err);
                    err = AVERROR_INVALIDDATA;
                    goto fail;
            }
            while (elem_id > 0)
    ...
    if (multiplier) {
        int side_size;
        const uint8_t *side = av_em_packet_get_side_data(avpkt, AV_PKT_DATA_SKIP_SAMPLES, &side_size);
        if (side && side_size>=4)
...
static int aac_decode_frame(AVEMCodecContext *avctx, void *data,
                            int *got_frame_ptr, AVEMPacket *avpkt)
{
    AACContext *ac = avctx->priv_data;
    ...
    const uint8_t *new_extradata = av_em_packet_get_side_data(avpkt,
                                       AV_PKT_DATA_NEW_EXTRADATA,
                                       &new_extradata_size);
    int jp_dualmono_size;
    const uint8_t *jp_dualmono   = av_em_packet_get_side_data(avpkt,
                                       AV_PKT_DATA_JP_DUALMONO,
                                       &jp_dualmono_size);

    if (new_extradata && 0) {
        av_em_free(avctx->extradata);
        avctx->extradata = av_em_mallocz(new_extradata_size +
...
static av_cold int aac_decode_close(AVEMCodecContext *avctx)
{
    AACContext *ac = avctx->priv_data;
    int i, type;
    ...
    for (i = 0; i < MAX_ELEM_ID; i++) {
        for (type = 0; type < 4; type++) {
            if (ac->che[type][i])
                AAC_RENAME(em_aac_sbr_ctx_close)(&ac->che[type][i]->sbr);
            av_em_freep(&ac->che[type][i]);
        }
    }
...
static const AVEMClass aac_decoder_class = {
