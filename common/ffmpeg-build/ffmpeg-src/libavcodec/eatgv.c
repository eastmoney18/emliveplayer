...
static av_cold int tgv_decode_init(AVEMCodecContext *avctx)
{
    TgvContext *s = avctx->priv_data;
    s->avctx         = avctx;
    avctx->framerate = (AVEMRational){ 15, 1 };
                  ...
                  uint8_t *dst, int width, int height)
{
    ...
    while (size > 0 && src < src_end) {
        ...
        if (size2 > 0) {
            ...
            av_em_memcpy_backptr(dst, offset, run);
            dst += run;
        }
    }
                            ...
                            const uint8_t *buf, const uint8_t *buf_end)
{
    ...
    if (num_mvs > s->num_mvs) {
        int err = av_em_reallocp_array(&s->mv_codebook, num_mvs, sizeof(*s->mv_codebook));
        if (err < 0) {
            s->num_mvs = 0;
            return err;
        }
    ...
    if (num_blocks_packed > s->num_blocks_packed) {
        int err;
        if ((err = av_em_reallocp(&s->block_codebook, num_blocks_packed * 16)) < 0) {
        ...
        for (x = 0; x < s->avctx->width / 4; x++) {
            ...
            if (vector < num_mvs) {
                ...
                if (mx < 0 || mx + 4 > s->avctx->width ||
                    ...
                    av_em_log(s->avctx, AV_LOG_ERROR, "MV %d %d out of picture\n", mx, my);
                    continue;
                }
...
static int tgv_decode_frame(AVEMCodecContext *avctx,
                            ...
                            AVEMPacket *avpkt)
{
    ...
    if (chunk_type == kVGT_TAG) {
        int pal_count, i;
        if(buf_end - buf < 12) {
            av_em_log(avctx, AV_LOG_WARNING, "truncated header\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        if (s->avctx->width != s->width || s->avctx->height != s->height) {
            av_em_freep(&s->frame_buffer);
            av_em_frame_unref(s->last_frame);
            if ((ret = em_set_dimensions(s->avctx, s->width, s->height)) < 0)
                return ret;
        }
    ...
    if (chunk_type == kVGT_TAG) {
        ...
        if (!s->frame_buffer &&
            !(s->frame_buffer = av_em_mallocz(s->width * s->height)))
            return AVERROR(ENOMEM);

        if (unpack(buf, buf_end, s->frame_buffer, s->avctx->width, s->avctx->height) < 0) {
            av_em_log(avctx, AV_LOG_WARNING, "truncated intra frame\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        if (!s->last_frame->data[0]) {
            av_em_log(avctx, AV_LOG_WARNING, "inter frame without corresponding intra frame\n");
            return buf_size;
        }
        frame->key_frame = 0;
        frame->pict_type = AV_PICTURE_TYPE_P;
        if (tgv_decode_inter(s, frame, buf, buf_end) < 0) {
            av_em_log(avctx, AV_LOG_WARNING, "truncated inter frame\n");
            return AVERROR_INVALIDDATA;
        }
    }
...
static av_cold int tgv_decode_end(AVEMCodecContext *avctx)
{
    TgvContext *s = avctx->priv_data;
    av_em_frame_free(&s->last_frame);
    av_em_freep(&s->frame_buffer);
    av_em_freep(&s->mv_codebook);
    av_em_freep(&s->block_codebook);
    return 0;
}

AVEMCodec ff_eatgv_decoder = {
    .name           = "eatgv",
