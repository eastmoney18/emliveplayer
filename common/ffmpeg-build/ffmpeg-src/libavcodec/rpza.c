...
static int rpza_decode_stream(RpzaContext *s)
{
    ...
    if (bytestream2_peek_byte(&s->gb) != 0xe1)
        av_em_log(s->avctx, AV_LOG_ERROR, "First chunk byte is 0x%02x instead of 0xe1\n",
    ...
    while (bytestream2_get_bytes_left(&s->gb)) {
        ...
        /* Unknown opcode */
        default:
            av_em_log(s->avctx, AV_LOG_ERROR, "Unknown opcode %d in rpza chunk."
...
static av_cold int rpza_decode_init(AVEMCodecContext *avctx)
{
...
static int rpza_decode_frame(AVEMCodecContext *avctx,
...
static av_cold int rpza_decode_end(AVEMCodecContext *avctx)
{
    RpzaContext *s = avctx->priv_data;
