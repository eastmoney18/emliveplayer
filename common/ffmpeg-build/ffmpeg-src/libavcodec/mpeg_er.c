...
static void set_erpic(EREMPicture *dst, Picture *src)
{
    int i;
...
int ff_mpeg_er_init(MpegEncContext *s)
{
    ...
    er->er_temp_buffer     = av_em_alloc(s->mb_height * s->mb_stride);
    er->error_status_table = av_em_mallocz(mb_array_size);
    ...
    return 0;
fail:
    av_em_freep(&er->er_temp_buffer);
    av_em_freep(&er->error_status_table);
