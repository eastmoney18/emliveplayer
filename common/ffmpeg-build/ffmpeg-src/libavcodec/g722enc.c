...
static av_cold int g722_encode_close(AVEMCodecContext *avctx)
{
    G722Context *c = avctx->priv_data;
    int i;
    for (i = 0; i < 2; i++) {
        av_em_freep(&c->paths[i]);
        av_em_freep(&c->node_buf[i]);
        av_em_freep(&c->nodep_buf[i]);
    }
    return 0;
}

static av_cold int g722_encode_init(AVEMCodecContext * avctx)
{
    G722Context *c = avctx->priv_data;
    int ret;

    if (avctx->channels != 1) {
        av_em_log(avctx, AV_LOG_ERROR, "Only mono tracks are allowed.\n");
    ...
    if (avctx->trellis) {
        ...
        int i;
        for (i = 0; i < 2; i++) {
            c->paths[i] = av_em_mallocz_array(max_paths, sizeof(**c->paths));
            c->node_buf[i] = av_em_mallocz_array(frontier, 2 * sizeof(**c->node_buf));
            c->nodep_buf[i] = av_em_mallocz_array(frontier, 2 * sizeof(**c->nodep_buf));
    ...
    if (avctx->frame_size) {
        ...
        if (avctx->frame_size & 1 || avctx->frame_size > MAX_FRAME_SIZE) {
            ...
            av_em_log(avctx, AV_LOG_WARNING, "Requested frame size is not "
    ...
    if (avctx->trellis) {
        ...
        if (avctx->trellis < MIN_TRELLIS || avctx->trellis > MAX_TRELLIS) {
            ...
            av_em_log(avctx, AV_LOG_WARNING, "Requested trellis value is not "
                   "allowed. Using %d instead of %d\n", new_trellis,
                   avctx->trellis);
...
static int g722_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
...
AVEMCodec ff_adpcm_g722_encoder = {
    .name           = "g722",
