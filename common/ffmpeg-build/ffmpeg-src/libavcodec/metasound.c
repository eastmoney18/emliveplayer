...
static int metasound_read_bitstream(AVEMCodecContext *avctx, TwinVQContext *tctx,
...
static av_cold int metasound_decode_init(AVEMCodecContext *avctx)
{
    int isampf, ibps;
    ...
    for (;;) {
        if (!props->tag) {
            av_em_log(avctx, AV_LOG_ERROR, "Could not find tag %08"PRIX32"\n", tag);
            return AVERROR_INVALIDDATA;
        }
        if (props->tag == tag) {
...
AVEMCodec ff_metasound_decoder = {
    .name           = "metasound",
