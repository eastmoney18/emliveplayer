                           ...
                           int len, int pixelstride)
{
    ...
    while (out_buf < out_end) {
        ...
        if (out_end - out_buf <= pixelstride * (count - 1)) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid pixel count.\n");
            return AVERROR_INVALIDDATA;
        }
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if (bytestream2_get_bytes_left(&s->g) < SGI_HEADER_SIZE) {
        av_em_log(avctx, AV_LOG_ERROR, "buf_size too small (%d)\n", avpkt->size);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->bytes_per_channel != 1 && s->bytes_per_channel != 2) {
        av_em_log(avctx, AV_LOG_ERROR, "wrong channel number\n");
        return AVERROR_INVALIDDATA;
    }

    /* Check for supported image dimensions. */
    if (dimension != 2 && dimension != 3) {
        av_em_log(avctx, AV_LOG_ERROR, "wrong dimension number\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (s->depth == SGI_GRAYSCALE) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "wrong picture format\n");
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int sgi_decode_init(AVEMCodecContext *avctx)
{
    SgiState *s = avctx->priv_data;
