...
static av_cold int ws_snd_decode_init(AVEMCodecContext *avctx)
{
    avctx->channels       = 1;
...
static int ws_snd_decode_frame(AVEMCodecContext *avctx, void *data,
                               int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (in_size > buf_size) {
        av_em_log(avctx, AV_LOG_ERROR, "Frame data is larger than input buffer\n");
