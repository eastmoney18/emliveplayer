                               ...
                               int iM, HufDec *hdecod)
{
    for (; im <= iM; im++) {
        ...
        if (l > HUF_DECBITS) {
            ...
            pl->p = av_em_realloc(pl->p, pl->lit * sizeof(int));
            if (!pl->p)
                          ...
                          uint16_t *dst, int dst_size)
{
    ...
    freq = av_em_mallocz_array(HUF_ENCSIZE, sizeof(*freq));
    hdec = av_em_mallocz_array(HUF_DECSIZE, sizeof(*hdec));
    if (!freq || !hdec) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
                          ...
                          int dsize, EXRThreadData *td)
{
    ...
    if (!td->bitmap)
        td->bitmap = av_em_alloc(BITMAP_SIZE);
    if (!td->lut)
        td->lut = av_em_alloc(1 << 17);
    if (!td->bitmap || !td->lut) {
        av_em_freep(&td->bitmap);
        av_em_freep(&td->lut);
        return AVERROR(ENOMEM);
    }
    ...
    for (c = 0; c < s->nb_channels; c++) {
        if (s->channels[c].pixel_type == EXR_HALF) {/* B44 only compress half float data */
            for (iY = 0; iY < nbB44BlockH; iY++) {
                for (iX = 0; iX < nbB44BlockW; iX++) {/* For each B44 block */
                    if (stayToUncompress < 3) {
                        av_em_log(s, AV_LOG_ERROR, "Not enough data for B44A block: %d", stayToUncompress);
                        return AVERROR_INVALIDDATA;
                    }
                    ...
                    if (src[compressed_size - stayToUncompress + 2] == 0xfc) { /* B44A block */
                        ...
                        if (stayToUncompress < 14) {
                            av_em_log(s, AV_LOG_ERROR, "Not enough data for B44 block: %d", stayToUncompress);
                            return AVERROR_INVALIDDATA;
                        }
            ...
            if (stayToUncompress < td->ysize * td->xsize * 4) {
                av_em_log(s, AV_LOG_ERROR, "Not enough data for uncompress channel: %d", stayToUncompress);
                return AVERROR_INVALIDDATA;
            }
...
static int decode_block(AVEMCodecContext *avctx, void *tdata,
                        int jobnr, int threadnr)
{
    ...
    avpriv_trc_function trc_func = avpriv_em_get_trc_function_from_trc(s->apply_trc_type);
    int ret;
    ...
    if (s->is_tile) {
        ...
        if (tileLevelX || tileLevelY) { /* tile level, is not the full res level */
            avpriv_em_report_missing_feature(s->avctx, "Subres tile before full res tile");
    ...
    if (data_size < uncompressed_size || s->is_tile) { /* td->tmp is use for tile reorganization */
        av_em_fast_padded_malloc(&td->tmp, &td->tmp_size, uncompressed_size);
        if (!td->tmp)
            return AVERROR(ENOMEM);
    }

    if (data_size < uncompressed_size) {
        av_em_fast_padded_malloc(&td->uncompressed_data,
        ...
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "decode_block() failed.\n");
            return ret;
        }
...
static int decode_header(EXRContext *s)
{
    ...
    if (bytestream2_get_bytes_left(&s->gb) < 10) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Header too short to parse.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (magic_number != 20000630) {
        ...
        av_em_log(s->avctx, AV_LOG_ERROR, "Wrong magic number %d.\n", magic_number);
        return AVERROR_INVALIDDATA;
    }

    version = bytestream2_get_byte(&s->gb);
    if (version != 2) {
        avpriv_em_report_missing_feature(s->avctx, "Version %d", version);
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (flags == 0x00)
        ...
        avpriv_em_report_missing_feature(s->avctx, "flags %d", flags);
        return AVERROR_PATCHWELCOME;
    }
        ...
        if ((var_size = check_header_variable(s, "channels",
            ...
            while (bytestream2_get_bytes_left(&ch_gb) >= 19) {
                ...
                if (bytestream2_get_bytes_left(&ch_gb) < 4) {
                    av_em_log(s->avctx, AV_LOG_ERROR, "Incomplete header.\n");
                    return AVERROR_INVALIDDATA;
                }

                current_pixel_type = bytestream2_get_le32(&ch_gb);
                if (current_pixel_type >= EXR_UNKNOWN) {
                    avpriv_em_report_missing_feature(s->avctx, "Pixel type %d",
                                                  current_pixel_type);
                    return AVERROR_PATCHWELCOME;
                }
                ...
                s->channels = av_em_realloc(s->channels,
            ...
            if (FFMIN3(s->channel_offsets[0],
                ...
                if (s->channel_offsets[0] < 0)
                    av_em_log(s->avctx, AV_LOG_ERROR, "Missing red channel.\n");
                if (s->channel_offsets[1] < 0)
                    av_em_log(s->avctx, AV_LOG_ERROR, "Missing green channel.\n");
                if (s->channel_offsets[2] < 0)
                    av_em_log(s->avctx, AV_LOG_ERROR, "Missing blue channel.\n");
                return AVERROR_INVALIDDATA;
            }
            ...
            av_em_log(s->avctx, AV_LOG_DEBUG, "line order: %d.\n", line_order);
            if (line_order > 2) {
                av_em_log(s->avctx, AV_LOG_ERROR, "Unknown line order.\n");
                return AVERROR_INVALIDDATA;
            }
            ...
            if (s->tile_attr.level_mode >= EXR_TILE_LEVEL_UNKNOWN){
                avpriv_em_report_missing_feature(s->avctx, "Tile level mode %d",
                                              s->tile_attr.level_mode);
                return AVERROR_PATCHWELCOME;
            }

            if (s->tile_attr.level_round >= EXR_TILE_ROUND_UNKNOWN) {
                avpriv_em_report_missing_feature(s->avctx, "Tile level round %d",
    ...
    em_set_sar(s->avctx, av_em_d2q(av_int2float(sar), 255));

    if (s->compression == EXR_UNKN) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Missing compression attribute.\n");
        return AVERROR_INVALIDDATA;
    }

    if (s->is_tile) {
        if (s->tile_attr.xSize < 1 || s->tile_attr.ySize < 1) {
            av_em_log(s->avctx, AV_LOG_ERROR, "Invalid tile attribute.\n");
            return AVERROR_INVALIDDATA;
        }
    }

    if (bytestream2_get_bytes_left(&s->gb) <= 0) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Incomplete frame.\n");
        return AVERROR_INVALIDDATA;
    }
...
static int decode_frame(AVEMCodecContext *avctx, void *data,
                        int *got_frame, AVEMPacket *avpkt)
{
    ...
    case EXR_HALF:
        ...
        break;
    case EXR_UINT:
        avpriv_em_request_sample(avctx, "32-bit unsigned int");
        return AVERROR_PATCHWELCOME;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Missing channel list.\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    case EXR_B44A:
        ...
        break;
    default:
        avpriv_em_report_missing_feature(avctx, "Compression %d", s->compression);
        return AVERROR_PATCHWELCOME;
    }
    ...
    if (s->xmin > s->xmax                  ||
        ...
        s->xmax >= s->w                    ||
        s->ymax >= s->h) {
        av_em_log(avctx, AV_LOG_ERROR, "Wrong or missing size information.\n");
        return AVERROR_INVALIDDATA;
    }

    if ((ret = em_set_dimensions(avctx, s->w, s->h)) < 0)
        return ret;

    s->desc          = av_em_pix_fmt_desc_get(avctx->pix_fmt);
    if (!s->desc)
        return AVERROR_INVALIDDATA;
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    trc_func = avpriv_em_get_trc_function_from_trc(s->apply_trc_type);
    if (trc_func) {
        for (i = 0; i < 65536; ++i) {
            t = exr_half2float(i);
            t.f = trc_func(t.f);
    ...
    s->thread_data = av_em_mallocz_array(avctx->thread_count, sizeof(EXRThreadData));
    if (!s->thread_data)
        return AVERROR_INVALIDDATA;
...
static av_cold int decode_end(AVEMCodecContext *avctx)
{
    EXRContext *s = avctx->priv_data;
    int i;
    for (i = 0; i < avctx->thread_count; i++) {
        EXRThreadData *td = &s->thread_data[i];
        av_em_freep(&td->uncompressed_data);
        av_em_freep(&td->tmp);
        av_em_freep(&td->bitmap);
        av_em_freep(&td->lut);
    }
...
static const AVEMClass exr_class = {
    .class_name = "EXR",
