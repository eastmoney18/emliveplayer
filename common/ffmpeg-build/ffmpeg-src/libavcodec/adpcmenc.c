...
static av_cold int adpcm_encode_close(AVEMCodecContext *avctx);

static av_cold int adpcm_encode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->channels > 2) {
        av_em_log(avctx, AV_LOG_ERROR, "only stereo or mono is supported\n");
        return AVERROR(EINVAL);
    }

    if (avctx->trellis && (unsigned)avctx->trellis > 16U) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid trellis size\n");
        return AVERROR(EINVAL);
    }
    ...
    case AV_CODEC_ID_ADPCM_MS:
        ...
        avctx->bits_per_coded_sample = 4;
        avctx->block_align    = BLKSIZE;
        if (!(avctx->extradata = av_em_alloc(32 + AV_INPUT_BUFFER_PADDING_SIZE)))
            goto error;
    ...
    case AV_CODEC_ID_ADPCM_SWF:
        if (avctx->sample_rate != 11025 &&
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Sample rate must be 11025, "
                   "22050 or 44100\n");
            ret = AVERROR(EINVAL);
            goto error;
        }
...
static av_cold int adpcm_encode_close(AVEMCodecContext *avctx)
{
    ADPCMEncodeContext *s = avctx->priv_data;
    av_em_freep(&s->paths);
    av_em_freep(&s->node_buf);
    av_em_freep(&s->nodep_buf);
    av_em_freep(&s->trellis_hash);
...
static void adpcm_compress_trellis(AVEMCodecContext *avctx,
...
static int adpcm_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                              const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, pkt_size, 0)) < 0)
        return ret;
    dst = avpkt->data;
