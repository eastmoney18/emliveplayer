...
static av_cold int amrnb_decode_init(AVEMCodecContext *avctx)
{
...
static int amrnb_decode_frame(AVEMCodecContext *avctx, void *data,
                              int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (p->cur_frame_mode == NO_DATA) {
        av_em_log(avctx, AV_LOG_ERROR, "Corrupt bitstream\n");
        return AVERROR_INVALIDDATA;
    }
    if (p->cur_frame_mode == MODE_DTX) {
        avpriv_em_report_missing_feature(avctx, "dtx mode");
        av_em_log(avctx, AV_LOG_INFO, "Note: libopencore_amrnb supports dtx\n");
        return AVERROR_PATCHWELCOME;
    }
    ...
    for (subframe = 0; subframe < 4; subframe++) {
        ...
        if (fixed_sparse.pitch_lag == 0) {
            av_em_log(avctx, AV_LOG_ERROR, "The file is corrupted, pitch_lag = 0 is not allowed\n");
            return AVERROR_INVALIDDATA;
        }
