...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->extradata_size < 12) {
        av_em_log(avctx, AV_LOG_ERROR, "Extradata size must be >= 12 instead of %i\n",
    ...
    default:
        l->lossy = AV_RL32(avctx->extradata + 8);
        avpriv_em_request_sample(avctx, "LOCO codec version %i", version);
    }
    ...
    case LOCO_RGBA:
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_INFO, "Unknown colorspace, index = %i\n", l->mode);
        return AVERROR_INVALIDDATA;
    }
    if (avctx->debug & FF_DEBUG_PICT_INFO)
        av_em_log(avctx, AV_LOG_INFO, "lossy:%i, version:%i, mode: %i\n", l->lossy, version, l->mode);

    return 0;
}

AVEMCodec ff_loco_decoder = {
    .name           = "loco",
