...
static av_cold void free_frame_buffers(Indeo3DecodeContext *ctx)
{
    ...
    for (p = 0; p < 3; p++) {
        av_em_freep(&ctx->planes[p].buffers[0]);
        av_em_freep(&ctx->planes[p].buffers[1]);
...
static av_cold int allocate_frame_buffers(Indeo3DecodeContext *ctx,
                                          AVEMCodecContext *avctx, int luma_width, int luma_height)
{
    ...
    if (luma_width  < 16 || luma_width  > 640 ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Invalid picture dimensions: %d x %d!\n",
    ...
    for (p = 0; p < 3; p++) {
        ...
        ctx->planes[p].buffers[0] = av_em_alloc(!p ? luma_size : chroma_size);
        ctx->planes[p].buffers[1] = av_em_alloc(!p ? luma_size : chroma_size);
...
static int copy_cell(Indeo3DecodeContext *ctx, Plane *plane, Cell *cell)
{
    ...
    if ((cell->ypos << 2) + mv_y < -1 || (cell->xpos << 2) + mv_x < 0 ||
        ...
        av_em_log(ctx->avctx, AV_LOG_ERROR,
 ...
 *
 *  @param ctx      pointer to the decoder context
 *  @param avctx    ptr to the AVEMCodecContext
 *  @param plane    pointer to the plane descriptor
 *  @param cell     pointer to the cell  descriptor
 ...
 */
static int decode_cell(Indeo3DecodeContext *ctx, AVEMCodecContext *avctx,
                       ...
                       const uint8_t *last_ptr)
{
    ...
    if (prim_indx >= 24 || second_indx >= 24) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid VQ table indexes! Primary: %d, secondary: %d!\n",
    ...
    case 4:
        if (mode >= 3 && cell->mv_ptr) {
            av_em_log(avctx, AV_LOG_ERROR, "Attempt to apply Mode 3/4 to an INTER cell!\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    case 11: /*----------------- MODE 11 (4x8 INTER block processing) ------------------*/
        if (mode == 10 && !cell->mv_ptr) { /* MODE 10 INTRA processing */
            ...
            if (mode == 11 && !cell->mv_ptr) {
               av_em_log(avctx, AV_LOG_ERROR, "Attempt to use Mode 11 for an INTRA cell!\n");
               return AVERROR_INVALIDDATA;
            }
        ...
        break;
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported coding mode: %d\n", mode);
        return AVERROR_INVALIDDATA;
    }//switch mode

    switch (error) {
    case IV3_BAD_RLE:
        av_em_log(avctx, AV_LOG_ERROR, "Mode %d: RLE code %X is not allowed at the current line\n",
               mode, data_ptr[-1]);
        return AVERROR_INVALIDDATA;
    case IV3_BAD_DATA:
        av_em_log(avctx, AV_LOG_ERROR, "Mode %d: invalid VQ data\n", mode);
        return AVERROR_INVALIDDATA;
    case IV3_BAD_COUNTER:
        av_em_log(avctx, AV_LOG_ERROR, "Mode %d: RLE-FB invalid counter: %d\n", mode, code);
        return AVERROR_INVALIDDATA;
    case IV3_UNSUPPORTED:
        av_em_log(avctx, AV_LOG_ERROR, "Mode %d: unsupported RLE code: %X\n", mode, data_ptr[-1]);
        return AVERROR_INVALIDDATA;
    case IV3_OUT_OF_DATA:
        av_em_log(avctx, AV_LOG_ERROR, "Mode %d: attempt to read past end of buffer\n", mode);
        return AVERROR_INVALIDDATA;
    }
...
#define CHECK_CELL \
    if (curr_cell.xpos + curr_cell.width > (plane->width >> 2) ||               \
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Invalid cell: x=%d, y=%d, w=%d, h=%d\n",   \
               curr_cell.xpos, curr_cell.ypos, curr_cell.width, curr_cell.height); \
        return AVERROR_INVALIDDATA;                                                              \
    }


static int parse_bintree(Indeo3DecodeContext *ctx, AVEMCodecContext *avctx,
                         ...
                         const int depth, const int strip_width)
{
    ...
    if (depth <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Stack overflow (corrupted binary tree)!\n");
    ...
    while (get_bits_left(&ctx->gb) >= 2) { /* loop until return */
        ...
        case INTRA_NULL:
            if (!curr_cell.tree) { /* MC tree INTRA code */
                ...
                if (code >= 2) {
                    av_em_log(avctx, AV_LOG_ERROR, "Invalid VQ_NULL code: %d\n", code);
                    return AVERROR_INVALIDDATA;
                }
                if (code == 1)
                    av_em_log(avctx, AV_LOG_ERROR, "SkipCell procedure not implemented yet!\n");
        ...
        case INTER_DATA:
            if (!curr_cell.tree) { /* MC tree INTER code */
                ...
                if (ctx->next_cell_data >= ctx->last_byte) {
                    av_em_log(avctx, AV_LOG_ERROR, "motion vector out of array\n");
                    return AVERROR_INVALIDDATA;
                }
                mv_idx = *(ctx->next_cell_data++);
                if (mv_idx >= ctx->num_vectors) {
                    av_em_log(avctx, AV_LOG_ERROR, "motion vector index out of range\n");
                    return AVERROR_INVALIDDATA;
                }
...
static int decode_plane(Indeo3DecodeContext *ctx, AVEMCodecContext *avctx,
...
static int decode_frame_headers(Indeo3DecodeContext *ctx, AVEMCodecContext *avctx,
                                const uint8_t *buf, int buf_size)
{
    ...
    if ((frame_num ^ word2 ^ data_size ^ OS_HDR_ID) != check_sum) {
        av_em_log(avctx, AV_LOG_ERROR, "OS header checksum mismatch!\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (bytestream2_get_le16(&gb) != 32) {
        av_em_log(avctx, AV_LOG_ERROR, "Unsupported codec version!\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (av_em_image_check_size(width, height, 0, avctx))
        return AVERROR_INVALIDDATA;
    ...
    if (width != ctx->width || height != ctx->height) {
        ...
        if ((res = allocate_frame_buffers(ctx, avctx, width, height)) < 0)
             return res;
        if ((res = em_set_dimensions(avctx, width, height)) < 0)
            return res;
    }
    ...
    if (FFMIN3(y_offset, v_offset, u_offset) < 0 ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "One of the y/u/v offsets is invalid\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ctx->data_size == 16) {
        av_em_log(avctx, AV_LOG_DEBUG, "Sync frame encountered!\n");
        return 16;
    }

    if (ctx->frame_flags & BS_8BIT_PEL) {
        avpriv_em_request_sample(avctx, "8-bit pixel format");
        return AVERROR_PATCHWELCOME;
    }

    if (ctx->frame_flags & BS_MV_X_HALF || ctx->frame_flags & BS_MV_Y_HALF) {
        avpriv_em_request_sample(avctx, "Halfpel motion vectors");
        return AVERROR_PATCHWELCOME;
    }
...
static av_cold int decode_init(AVEMCodecContext *avctx)
{
    ...
    em_hpeldsp_init(&ctx->hdsp, avctx->flags);
                        ...
                        AVEMPacket *avpkt)
{
    ...
    if ((res = em_get_buffer(avctx, frame, 0)) < 0)
        return res;
...
static av_cold int decode_close(AVEMCodecContext *avctx)
{
