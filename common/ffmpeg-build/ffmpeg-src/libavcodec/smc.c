...
{ \
    ...
    { \
        av_em_log(s->avctx, AV_LOG_INFO, "warning: block counter just went negative (this should not happen)\n"); \
        return; \
    } \
}
...
static void smc_decode_stream(SmcContext *s)
{
    ...
    if (chunk_size != buf_size)
        av_em_log(s->avctx, AV_LOG_INFO, "warning: MOV chunk size != encoded chunk size (%d != %d); using MOV chunk size\n",
            chunk_size, buf_size);
    ...
    while (total_blocks) {
        ...
        if (row_ptr >= image_size) {
            av_em_log(s->avctx, AV_LOG_INFO, "SMC decoder just went out of bounds (row ptr = %d, height = %d)\n",
                row_ptr, image_size);
            return;
        }
        ...
        case 0x30:
            ...
            if ((row_ptr == 0) && (pixel_ptr == 0)) {
                av_em_log(s->avctx, AV_LOG_INFO, "encountered repeat block opcode (%02X) but no blocks rendered yet\n",
                    opcode & 0xF0);
                return;
            }
        ...
        case 0x50:
            ...
            if ((row_ptr == 0) && (pixel_ptr < 2 * 4)) {
                av_em_log(s->avctx, AV_LOG_INFO, "encountered repeat block opcode (%02X) but not enough blocks rendered yet\n",
                    opcode & 0xF0);
                return;
            }
        ...
        case 0xF0:
            avpriv_em_request_sample(s->avctx, "0xF0 opcode");
            break;
        }
    }
...
static av_cold int smc_decode_init(AVEMCodecContext *avctx)
{
...
static int smc_decode_frame(AVEMCodecContext *avctx,
...
static av_cold int smc_decode_end(AVEMCodecContext *avctx)
{
    SmcContext *s = avctx->priv_data;
