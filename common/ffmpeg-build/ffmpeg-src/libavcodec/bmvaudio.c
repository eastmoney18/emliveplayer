...
static av_cold int bmv_aud_decode_init(AVEMCodecContext *avctx)
{
    avctx->channels       = 2;
...
static int bmv_aud_decode_frame(AVEMCodecContext *avctx, void *data,
                                int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (buf_size < total_blocks * 65 + 1) {
        av_em_log(avctx, AV_LOG_ERROR, "expected %d bytes, got %d\n",
...
AVEMCodec ff_bmv_audio_decoder = {
    .name           = "bmv_audio",
