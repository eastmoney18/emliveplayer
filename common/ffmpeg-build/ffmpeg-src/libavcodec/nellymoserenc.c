...
static av_cold int encode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->channels != 1) {
        av_em_log(avctx, AV_LOG_ERROR, "Nellymoser supports only 1 channel\n");
    ...
    if (avctx->sample_rate != 8000 && avctx->sample_rate != 16000 &&
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Nellymoser works only with 8000, 16000, 11025, 22050 and 44100 sample rate\n");
        return AVERROR(EINVAL);
    }
    ...
    if (s->avctx->trellis) {
        s->opt  = av_em_alloc(NELLY_BANDS * OPT_SIZE * sizeof(float  ));
        s->path = av_em_alloc(NELLY_BANDS * OPT_SIZE * sizeof(uint8_t));
        if (!s->opt || !s->path) {
            ret = AVERROR(ENOMEM);
            goto error;
        }
                        ...
                        const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, NELLY_BLOCK_LEN, 0)) < 0)
        return ret;
...
AVEMCodec ff_nellymoser_encoder = {
    .name           = "nellymoser",
