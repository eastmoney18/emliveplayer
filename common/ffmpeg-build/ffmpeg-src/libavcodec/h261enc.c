...
int em_h261_get_picture_format(int width, int height)
{
    // QCIF
...
void em_h261_encode_picture_header(MpegEncContext *s, int picture_number)
{
    ...
    format = em_h261_get_picture_format(s->width, s->height);
...
void em_h261_reorder_mb_index(MpegEncContext *s)
{
    ...
    if (em_h261_get_picture_format(s->width, s->height) == 1) { // CIF
        s->mb_x  = index % 11;
        index   /= 11;
        s->mb_y  = index % 3;
        index   /= 3;
...
static void h261_encode_motion(H261Context *h, int val)
{
    ...
    if (val == 0) {
        code = 0;
        put_bits(&s->pb, em_h261_mv_tab[code][1], em_h261_mv_tab[code][0]);
    } else {
        if (val > 15)
            val -= 32;
        if (val < -16)
...
void em_h261_encode_mb(MpegEncContext *s, int16_t block[6][64],
                       int motion_x, int motion_y)
{
    ...
    put_bits(&s->pb,
             em_h261_mba_bits[s->mb_skip_run],
             em_h261_mba_code[s->mb_skip_run]);
    s->mb_skip_run = 0;
    ...
    put_bits(&s->pb,
             em_h261_mtype_bits[h->mtype],
             em_h261_mtype_code[h->mtype]);
    ...
    if (HAS_CBP(h->mtype)) {
        av_assert1(cbp > 0);
        put_bits(&s->pb,
                 em_h261_cbp_tab[cbp - 1][1],
                 em_h261_cbp_tab[cbp - 1][0]);
    }
    for (i = 0; i < 6; i++)
        /* encode each block */
...
av_cold void em_h261_encode_init(MpegEncContext *s)
{
    em_h261_common_init();
    ...
    init_uni_h261_rl_tab(&em_h261_rl_tcoeff, NULL, uni_h261_rl_len);
...
static const AVEMClass h261_class = {
