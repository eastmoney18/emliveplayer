...
static av_cold int s302m_encode_init(AVEMCodecContext *avctx)
{
...
static int s302m_encode2_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                               const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if (buf_size - AES3_HEADER_LEN > UINT16_MAX) {
        av_em_log(avctx, AV_LOG_ERROR, "number of samples in frame too big\n");
        return AVERROR(EINVAL);
    }
    ...
    if (avctx->bits_per_raw_sample == 24) {
        ...
        for (c = 0; c < frame->nb_samples; c++) {
            ...
            for (channels = 0; channels < avctx->channels; channels += 2) {
                o[0] = em_reverse[(samples[0] & 0x0000FF00) >> 8];
                o[1] = em_reverse[(samples[0] & 0x00FF0000) >> 16];
                o[2] = em_reverse[(samples[0] & 0xFF000000) >> 24];
                o[3] = em_reverse[(samples[1] & 0x00000F00) >> 4] | vucf;
                o[4] = em_reverse[(samples[1] & 0x000FF000) >> 12];
                o[5] = em_reverse[(samples[1] & 0x0FF00000) >> 20];
                o[6] = em_reverse[(samples[1] & 0xF0000000) >> 28];
                o += 7;
                samples += 2;
            }
        ...
        for (c = 0; c < frame->nb_samples; c++) {
            ...
            for (channels = 0; channels < avctx->channels; channels += 2) {
                o[0] = em_reverse[ (samples[0] & 0x000FF000) >> 12];
                o[1] = em_reverse[ (samples[0] & 0x0FF00000) >> 20];
                o[2] = em_reverse[((samples[0] & 0xF0000000) >> 28) | vucf];
                o[3] = em_reverse[ (samples[1] & 0x000FF000) >> 12];
                o[4] = em_reverse[ (samples[1] & 0x0FF00000) >> 20];
                o[5] = em_reverse[ (samples[1] & 0xF0000000) >> 28];
                o += 6;
                samples += 2;
            }
        ...
        for (c = 0; c < frame->nb_samples; c++) {
            ...
            for (channels = 0; channels < avctx->channels; channels += 2) {
                o[0] = em_reverse[ samples[0] & 0xFF];
                o[1] = em_reverse[(samples[0] & 0xFF00) >>  8];
                o[2] = em_reverse[(samples[1] & 0x0F)   <<  4] | vucf;
                o[3] = em_reverse[(samples[1] & 0x0FF0) >>  4];
                o[4] = em_reverse[(samples[1] & 0xF000) >> 12];
                o += 5;
                samples += 2;
