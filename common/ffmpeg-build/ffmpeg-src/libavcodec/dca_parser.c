...
static av_cold int dca_parse_init(AVEMCodecParserContext *s)
{
...
static int dca_parse(AVEMCodecParserContext *s, AVEMCodecContext *avctx,
                     ...
                     const uint8_t *buf, int buf_size)
{
    ...
    if (!dca_parse_params(pc1, buf, buf_size, &duration, &sample_rate)) {
        ...
        s->duration = av_em_rescale(duration, avctx->sample_rate, sample_rate);
    } else
        s->duration = 0;
