...
static av_cold int amrwb_decode_init(AVEMCodecContext *avctx)
{
...
static int amrwb_decode_frame(AVEMCodecContext *avctx, void *data,
                              int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (!ctx->fr_quality || ctx->fr_cur_mode > MODE_SID)
        av_em_log(avctx, AV_LOG_ERROR, "Encountered a bad or corrupted frame\n");

    if (ctx->fr_cur_mode == MODE_SID) { /* Comfort noise frame */
        avpriv_em_request_sample(avctx, "SID mode");
        return AVERROR_PATCHWELCOME;
    }
