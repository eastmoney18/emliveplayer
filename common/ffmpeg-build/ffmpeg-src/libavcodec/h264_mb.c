...
static inline void get_lowest_part_y(const H264EMContext *h, H264SliceContext *sl,
 ...
 */
static void await_references(const H264EMContext *h, H264SliceContext *sl)
{
        ...
        for (ref = 0; ref < 48 && nrefs[list]; ref++) {
            ...
            if (row >= 0) {
                ...
                if (!FIELD_PICTURE(h) && ref_field_picture) { // frame referencing two fields
                    ...
                    em_thread_await_progress(&ref_pic->parent->tf,
                                             FFMIN((row >> 1) - !(row & 1),
                                                   pic_height - 1),
                                             1);
                    em_thread_await_progress(&ref_pic->parent->tf,
...
static av_always_inline void mc_dir_part(const H264EMContext *h, H264SliceContext *sl,
                                         H264Ref *pic,
...
static av_always_inline void mc_part_std(const H264EMContext *h, H264SliceContext *sl,
                                         int n, int square,
...
static av_always_inline void mc_part_weighted(const H264EMContext *h, H264SliceContext *sl,
                                              int n, int square,
                                              int height, int delta,
                                              uint8_t *dest_y, uint8_t *dest_cb,
                                              uint8_t *dest_cr,
...
static av_always_inline void prefetch_motion(const H264EMContext *h, H264SliceContext *sl,
...
static av_always_inline void xchg_mb_border(const H264EMContext *h, H264SliceContext *sl,
                                            uint8_t *src_y,
...
static av_always_inline void hl_decode_mb_predict_luma(const H264EMContext *h,
                                                       H264SliceContext *sl,
...
static av_always_inline void hl_decode_mb_idct_luma(const H264EMContext *h, H264SliceContext *sl,
                                                    int mb_type, int simple,
                                                    int transform_bypass,
                                                    int pixel_shift,
                                                    const int *block_offset,
