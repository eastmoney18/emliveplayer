...
static int noise(AVBSFContext *ctx, AVEMPacket *out)
{
    ...
    ret = av_em_new_packet(out, in->size);
    if (ret < 0)
        goto fail;
...
static const AVEMClass noise_class = {
    .class_name = "noise",
...
const AVBitStreamFilter em_noise_bsf = {
    .name           = "noise",
