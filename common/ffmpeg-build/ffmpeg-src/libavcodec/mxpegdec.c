...
static av_cold int mxpeg_decode_end(AVEMCodecContext *avctx)
{
    ...
    for (i = 0; i < 2; ++i)
        av_em_frame_free(&s->picture[i]);

    s->bitmask_size = 0;
    av_em_freep(&s->mxm_bitmask);
    av_em_freep(&s->completion_bitmask);

    return 0;
}

static av_cold int mxpeg_decode_init(AVEMCodecContext *avctx)
{
    MXpegDecodeContext *s = avctx->priv_data;
                            ...
                            const uint8_t *buf_ptr, int buf_size)
{
    ...
    if (bitmask_size > buf_size - 12) {
        av_em_log(s->jpg.avctx, AV_LOG_ERROR,
    ...
    if (s->bitmask_size != bitmask_size) {
        s->bitmask_size = 0;
        av_em_freep(&s->mxm_bitmask);
        s->mxm_bitmask = av_em_alloc(bitmask_size);
        if (!s->mxm_bitmask) {
            av_em_log(s->jpg.avctx, AV_LOG_ERROR,
        ...
        av_em_freep(&s->completion_bitmask);
        s->completion_bitmask = av_em_mallocz(bitmask_size);
                                  ...
                                  AVFrame *reference_ptr)
{
    if ((jpg->width + 0x0F)>>4 != s->mb_width ||
        ...
        av_em_log(jpg->avctx, AV_LOG_ERROR,
...
static int mxpeg_decode_frame(AVEMCodecContext *avctx,
            ...
            goto the_end;
        {
            ...
            case SOS:
                ...
                if (!jpg->got_picture) {
                    ...
                    av_em_frame_unref(jpg->picture_ptr);
                    if ((ret = em_get_buffer(avctx, jpg->picture_ptr,
                ...
                if (s->got_mxm_bitmask) {
                    ...
                    if (!reference_ptr->data[0] &&
                        (ret = em_get_buffer(avctx, reference_ptr,
                                             AV_GET_BUFFER_FLAG_REF)) < 0)
                        return ret;
...
the_end:
    if (jpg->got_picture) {
        int ret = av_em_frame_ref(data, jpg->picture_ptr);
        if (ret < 0)
            return ret;
        *got_frame = 1;
