...
static av_cold int smvjpeg_decode_end(AVEMCodecContext *avctx)
{
    ...
    av_em_frame_free(&s->picture[0]);
    av_em_frame_free(&s->picture[1]);
    ret = avcodec_em_close(s->avctx);
    av_em_freep(&s->avctx);
    return ret;
}

static av_cold int smvjpeg_decode_init(AVEMCodecContext *avctx)
{
    SMVJpegDecodeContext *s = avctx->priv_data;
    AVEMCodec *codec;
    ...
    s->picture[0] = av_em_frame_alloc();
    if (!s->picture[0])
        return AVERROR(ENOMEM);

    s->picture[1] = av_em_frame_alloc();
    if (!s->picture[1]) {
        av_em_frame_free(&s->picture[0]);
        return AVERROR(ENOMEM);
    }
    ...
    if (s->frames_per_jpeg <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid number of frames per jpeg.\n");
        ret = AVERROR_INVALIDDATA;
    }

    codec = avcodec_em_find_decoder(AV_CODEC_ID_MJPEG);
    if (!codec) {
        av_em_log(avctx, AV_LOG_ERROR, "MJPEG codec not found\n");
        smvjpeg_decode_end(avctx);
        return AVERROR_DECODER_NOT_FOUND;
    }

    s->avctx = avcodec_em_alloc_context3(codec);

    av_em_dict_set(&thread_opt, "threads", "1", 0);
    s->avctx->refcounted_frames = 1;
    s->avctx->flags = avctx->flags;
    s->avctx->idct_algo = avctx->idct_algo;
    if ((r = em_codec_open2_recursive(s->avctx, codec, &thread_opt)) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "MJPEG codec failed to open\n");
        ret = r;
    }
...
static int smvjpeg_decode_frame(AVEMCodecContext *avctx, void *data, int *data_size,
                            AVEMPacket *avpkt)
{
    ...
    if (!cur_frame) {
        av_em_frame_unref(mjpeg_data);
        ret = avcodec_em_decode_video2(s->avctx, mjpeg_data, &s->mjpeg_data_size, avpkt);
        if (ret < 0) {
            s->mjpeg_data_size = 0;
            return ret;
        }
    ...
    desc = av_em_pix_fmt_desc_get(s->avctx->pix_fmt);
    av_assert0(desc);

    if (mjpeg_data->height % (s->frames_per_jpeg << desc->log2_chroma_h)) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid height\n");
    ...
    *data_size = s->mjpeg_data_size;
    ...
    /* We shouldn't get here if frames_per_jpeg <= 0 because this was rejected
       in init */
    ret = em_set_dimensions(avctx, mjpeg_data->width, mjpeg_data->height / s->frames_per_jpeg);
    if (ret < 0) {
        av_em_log(s, AV_LOG_ERROR, "Failed to set dimensions\n");
        return ret;
    }
    ...
    if (*data_size) {
        ...
        /* em_init_buffer_info(avctx, &s->picture[1]); */
        ...
        ret = av_em_frame_ref(data, s->picture[1]);
    }

    return ret;
}

static const AVEMClass smvjpegdec_class = {
    .class_name = "SMVJPEG decoder",
    .item_name  = av_em_default_item_name,
    .version    = LIBAVUTIL_VERSION_INT,
};

AVEMCodec ff_smvjpeg_decoder = {
    .name           = "smvjpeg",
