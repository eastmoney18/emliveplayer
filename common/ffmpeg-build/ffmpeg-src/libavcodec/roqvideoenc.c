...
static int create_cel_evals(RoqContext *enc, RoqTempdata *tempData)
{
    ...
    tempData->cel_evals = av_em_malloc_array(enc->width*enc->height/64, sizeof(CelEvaluation));
...
static void reconstruct_and_encode_image(RoqContext *enc, RoqTempdata *tempData, int w, int h, int numBlocks)
{
    ...
    av_em_log(enc->avctx, AV_LOG_ERROR, "Expected distortion: %i Actual: %i\n",
           dist,
                             ...
                             int size, int cbsize)
{
    ...
    int *buf;
    int *codebook = av_em_malloc_array(6*c_size, cbsize*sizeof(int));
    int *closest_cb;
    ...
    if (size == 4) {
        closest_cb = av_em_malloc_array(6*c_size, inputCount*sizeof(int));
        if (!closest_cb) {
            ret = AVERROR(ENOMEM);
            goto out;
        }
...
static int generate_new_codebooks(RoqContext *enc, RoqTempdata *tempData)
{
    ...
    uint8_t mb2[3*4];
    roq_cell *results4 = av_em_alloc(sizeof(roq_cell)*MAX_CBS_4x4*4);
    uint8_t *yuvClusters=av_em_malloc_array(max, sizeof(int)*6*4);
    int *points = av_em_malloc_array(max, 6*4*sizeof(int));
    int bias;
    ...
    tempData->closest_cb2 = av_em_malloc_array(max, 4*sizeof(int));
 ...
 retry_encode:
    ...
    if (tempData->mainChunkSize/8 > 65535 && enc->quake3_compat) {
        if (enc->lambda > 100000) {
            av_em_log(enc->avctx, AV_LOG_ERROR, "Cannot encode video in Quake compatible form\n");
            return AVERROR(EINVAL);
        }
        av_em_log(enc->avctx, AV_LOG_ERROR,
    ...
    av_em_freep(&tempData->cel_evals);
    av_em_freep(&tempData->closest_cb2);
...
static av_cold int roq_encode_end(AVEMCodecContext *avctx)
{
    RoqContext *enc = avctx->priv_data;

    av_em_frame_free(&enc->current_frame);
    av_em_frame_free(&enc->last_frame);

    av_em_freep(&enc->tmpData);
    av_em_freep(&enc->this_motion4);
    av_em_freep(&enc->last_motion4);
    av_em_freep(&enc->this_motion8);
    av_em_freep(&enc->last_motion8);

    return 0;
}

static av_cold int roq_encode_init(AVEMCodecContext *avctx)
{
    RoqContext *enc = avctx->priv_data;

    av_em_lfg_init(&enc->randctx, 1);

    enc->avctx = avctx;

    enc->framesSinceKeyframe = 0;
    if ((avctx->width & 0xf) || (avctx->height & 0xf)) {
        av_em_log(avctx, AV_LOG_ERROR, "Dimensions must be divisible by 16\n");
        return AVERROR(EINVAL);
    }

    if (avctx->width > 65535 || avctx->height > 65535) {
        av_em_log(avctx, AV_LOG_ERROR, "Dimensions are max %d\n", enc->quake3_compat ? 32768 : 65535);
        return AVERROR(EINVAL);
    }

    if (((avctx->width)&(avctx->width-1))||((avctx->height)&(avctx->height-1)))
        av_em_log(avctx, AV_LOG_ERROR, "Warning: dimensions not power of two, this is not supported by quake\n");
    ...
    enc->last_frame    = av_em_frame_alloc();
    enc->current_frame = av_em_frame_alloc();
    if (!enc->last_frame || !enc->current_frame) {
        roq_encode_end(avctx);
        return AVERROR(ENOMEM);
    }

    enc->tmpData      = av_em_alloc(sizeof(RoqTempdata));

    enc->this_motion4 =
        av_em_mallocz_array((enc->width*enc->height/16), sizeof(motion_vect));

    enc->last_motion4 =
        av_em_malloc_array ((enc->width*enc->height/16), sizeof(motion_vect));

    enc->this_motion8 =
        av_em_mallocz_array((enc->width*enc->height/64), sizeof(motion_vect));

    enc->last_motion8 =
        av_em_malloc_array ((enc->width*enc->height/64), sizeof(motion_vect));
...
static int roq_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
                            const AVFrame *frame, int *got_packet)
{
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, size, 0)) < 0)
        return ret;
    ...
    if (enc->first_frame) {
        /* Alloc memory for the reconstruction data (we must know the stride
         for that) */
        if ((ret = em_get_buffer(avctx, enc->current_frame, 0)) < 0 ||
            (ret = em_get_buffer(avctx, enc->last_frame,    0)) < 0)
            return ret;
...
static const AVEMClass roq_class = {
    .class_name = "RoQ",
