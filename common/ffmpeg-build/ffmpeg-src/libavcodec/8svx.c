...
/** decode a frame */
static int eightsvx_decode_frame(AVEMCodecContext *avctx, void *data,
                                 int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (!esc->data[0] && avpkt) {
        ...
        if (avpkt->size % avctx->channels) {
            av_em_log(avctx, AV_LOG_WARNING, "Packet with odd size, ignoring last byte\n");
        }
        if (avpkt->size < (hdr_size + 1) * avctx->channels) {
            av_em_log(avctx, AV_LOG_ERROR, "packet size is too small\n");
        ...
        if (!(esc->data[0] = av_em_alloc(chan_size)))
            return AVERROR(ENOMEM);
        if (avctx->channels == 2) {
            if (!(esc->data[1] = av_em_alloc(chan_size))) {
                av_em_freep(&esc->data[0]);
                return AVERROR(ENOMEM);
            }
        }
    ...
    if (!esc->data[0]) {
        av_em_log(avctx, AV_LOG_ERROR, "unexpected empty packet\n");
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int eightsvx_decode_init(AVEMCodecContext *avctx)
{
    EightSvxContext *esc = avctx->priv_data;

    if (avctx->channels < 1 || avctx->channels > 2) {
        av_em_log(avctx, AV_LOG_ERROR, "8SVX does not support more than 2 channels\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    default:
        av_em_log(avctx, AV_LOG_ERROR, "Invalid codec id %d.\n", avctx->codec->id);
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int eightsvx_decode_close(AVEMCodecContext *avctx)
{
    EightSvxContext *esc = avctx->priv_data;

    av_em_freep(&esc->data[0]);
    av_em_freep(&esc->data[1]);
    esc->data_size = 0;
    esc->data_idx = 0;
...
AVEMCodec ff_eightsvx_fib_decoder = {
  .name           = "8svx_fib",
...
AVEMCodec ff_eightsvx_exp_decoder = {
  .name           = "8svx_exp",
