...
static av_cold int dvbsub_parse_init(AVEMCodecParserContext *s)
{
    DVBSubParseContext *pc = s->priv_data;
    pc->packet_buf = av_em_alloc(PARSE_BUF_SIZE);

    return 0;
}

static int dvbsub_parse(AVEMCodecParserContext *s,
...
/* if not currently in a packet, discard data */
    ...
    while (p < p_end)
    {
        if (*p == 0x0f)
        {
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Junk in packet\n");
...
static av_cold void dvbsub_parse_close(AVEMCodecParserContext *s)
{
    DVBSubParseContext *pc = s->priv_data;
    av_em_freep(&pc->packet_buf);
}

AVEMCodecParser ff_dvbsub_parser = {
