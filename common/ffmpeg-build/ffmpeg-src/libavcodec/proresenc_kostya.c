...
static int encode_slice(AVEMCodecContext *avctx, const AVFrame *pic,
                        PutBitContext *pb,
...
static int find_slice_quant(AVEMCodecContext *avctx,
...
static int find_quant_thread(AVEMCodecContext *avctx, void *arg,
                        ...
                        const AVFrame *pic, int *got_packet)
{
    ...
    if ((ret = em_alloc_packet2(avctx, pkt, pkt_size + AV_INPUT_BUFFER_MIN_SIZE, 0)) < 0)
        return ret;
        ...
        for (y = 0; y < ctx->mb_height; y++) {
            ...
            for (x = mb = 0; x < ctx->mb_width; x += mbs_per_slice, mb++) {
                ...
                if (pkt_size <= buf - orig_buf + 2 * max_slice_size) {
                    ...
                    ret = av_em_grow_packet(pkt, delta);
                    if (ret < 0)
                        return ret;
...
static av_cold int encode_close(AVEMCodecContext *avctx)
{
    ...
    if (ctx->tdata) {
        for (i = 0; i < avctx->thread_count; i++)
            av_em_freep(&ctx->tdata[i].nodes);
    }
    av_em_freep(&ctx->tdata);
    av_em_freep(&ctx->slice_q);
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (ctx->profile == PRORES_PROFILE_AUTO) {
        ...
        av_em_log(avctx, AV_LOG_INFO, "Autoselected %s. It can be overridden "
               "through -profile option.\n", ctx->profile == PRORES_PROFILE_4444
               ? "4:4:4:4 profile because of the used input colorspace"
               : "HQ profile to keep best quality");
    }
    if (av_em_pix_fmt_desc_get(avctx->pix_fmt)->flags & AV_PIX_FMT_FLAG_ALPHA) {
        if (ctx->profile != PRORES_PROFILE_4444) {
            // force alpha and warn
            av_em_log(avctx, AV_LOG_WARNING, "Profile selected will not "
                   "encode alpha. Override with -profile if needed.\n");
            ctx->alpha_bits = 0;
        }
        if (ctx->alpha_bits & 7) {
            av_em_log(avctx, AV_LOG_ERROR, "alpha bits should be 0, 8 or 16\n");
            return AVERROR(EINVAL);
        }
    ...
    if (strlen(ctx->vendor) != 4) {
        av_em_log(avctx, AV_LOG_ERROR, "vendor ID should be 4 bytes\n");
    ...
    if (!ctx->force_quant) {
        if (!ctx->bits_per_mb) {
            ...
            av_em_log(avctx, AV_LOG_ERROR, "too few bits per MB, please set at least 128\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        ctx->slice_q = av_em_alloc(ctx->slices_per_picture * sizeof(*ctx->slice_q));
        if (!ctx->slice_q) {
            encode_close(avctx);
            return AVERROR(ENOMEM);
        }

        ctx->tdata = av_em_mallocz(avctx->thread_count * sizeof(*ctx->tdata));
        if (!ctx->tdata) {
            encode_close(avctx);
            return AVERROR(ENOMEM);
        }
        ...
        for (j = 0; j < avctx->thread_count; j++) {
            ctx->tdata[j].nodes = av_em_alloc((ctx->slices_width + 1)
                                            * TRELLIS_WIDTH
        ...
        if (ctx->force_quant > 64) {
            av_em_log(avctx, AV_LOG_ERROR, "too large quantiser, maximum is 64\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    av_em_log(avctx, AV_LOG_DEBUG, "frame size upper bound: %d\n",
...
static const AVEMClass proresenc_class = {
...
AVEMCodec ff_prores_ks_encoder = {
    .name           = "prores_ks",
