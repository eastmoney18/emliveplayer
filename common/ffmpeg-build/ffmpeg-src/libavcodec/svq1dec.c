...
static int svq1_decode_delta_block(AVEMCodecContext *avctx, HpelDSPContext *hdsp,
...
static int svq1_decode_frame_header(AVEMCodecContext *avctx, AVFrame *frame)
{
...
static int svq1_decode_frame(AVEMCodecContext *avctx, void *data,
                             int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (s->frame_code != 0x20) {
        ...
        if (buf_size < 9 * 4) {
            av_em_log(avctx, AV_LOG_ERROR, "Input packet too small\n");
            return AVERROR_INVALIDDATA;
        }

        av_em_fast_padded_malloc(&s->pkt_swapped,
    ...
    result = em_set_dimensions(avctx, s->width, s->height);
    if (result < 0)
        return result;
    ...
    result = em_get_buffer(avctx, cur, s->nonref ? 0 : AV_GET_BUFFER_FLAG_REF);
    if (result < 0)
        return result;

    pmv = av_em_alloc((FFALIGN(s->width, 16) / 8 + 3) * sizeof(*pmv));
    if (!pmv)
        return AVERROR(ENOMEM);
    ...
    if (!s->nonref) {
        av_em_frame_unref(s->prev);
        result = av_em_frame_ref(s->prev, cur);
        if (result < 0)
            goto err;
    }
...
static av_cold int svq1_decode_init(AVEMCodecContext *avctx)
{
    SVQ1Context *s = avctx->priv_data;
    int i;
    int offset = 0;

    s->prev = av_em_frame_alloc();
    if (!s->prev)
        return AVERROR(ENOMEM);
...
static av_cold int svq1_decode_end(AVEMCodecContext *avctx)
{
    SVQ1Context *s = avctx->priv_data;

    av_em_frame_free(&s->prev);
    av_em_freep(&s->pkt_swapped);
    s->pkt_swapped_allocated = 0;

    return 0;
}

static void svq1_flush(AVEMCodecContext *avctx)
{
    SVQ1Context *s = avctx->priv_data;
