 ...
 * @param n block index
 */
static inline int pred_non_zero_count(const H264EMContext *h, H264SliceContext *sl, int n)
{
...
av_cold void em_h264_decode_init_vlc(void){
    static int done = 0;
 ...
 */
static int decode_residual(const H264EMContext *h, H264SliceContext *sl,
                           ...
                           int max_coeff)
{
    ...
    if(total_coeff > (unsigned)max_coeff) {
        av_em_log(h->avctx, AV_LOG_ERROR, "corrupted macroblock %d %d (total_coeff=%d)\n", sl->mb_x, sl->mb_y, total_coeff);
        return -1;
    }
    ...
    if(trailing_ones<total_coeff) {
        ...
        if(level_code >= 100){
            ...
            if(prefix<14){ //FIXME try to build a large unified VLC table for all this
                ...
                if(prefix>=16){
                    if(prefix > 25+3){
                        av_em_log(h->avctx, AV_LOG_ERROR, "Invalid level prefix\n");
                        return -1;
                    }
...
#define STORE_BLOCK(type) \
    ...
    if(zeros_left<0){
        av_em_log(h->avctx, AV_LOG_ERROR, "negative number of zero coeffs at %d %d\n", sl->mb_x, sl->mb_y);
        return -1;
    }
...
int decode_luma_residual(const H264EMContext *h, H264SliceContext *sl,
...
int em_h264_decode_mb_cavlc(const H264EMContext *h, H264SliceContext *sl)
{
    ...
    if (sl->slice_type_nos == AV_PICTURE_TYPE_B) {
        if(mb_type < 23){
            partition_count = em_h264_b_mb_type_info[mb_type].partition_count;
            mb_type         = em_h264_b_mb_type_info[mb_type].type;
        }else{
            mb_type -= 23;
        ...
        if(mb_type < 5){
            partition_count = em_h264_p_mb_type_info[mb_type].partition_count;
            mb_type         = em_h264_p_mb_type_info[mb_type].type;
        }else{
            mb_type -= 5;
            goto decode_intra_mb;
        }
...
decode_intra_mb:
        if(mb_type > 25){
            av_em_log(h->avctx, AV_LOG_ERROR, "mb_type %d in %c slice too large at %d %d\n", mb_type, av_em_get_picture_type_char(sl->slice_type), sl->mb_x, sl->mb_y);
            return -1;
        }
        partition_count=0;
        cbp                      = em_h264_i_mb_type_info[mb_type].cbp;
        sl->intra16x16_pred_mode = em_h264_i_mb_type_info[mb_type].pred_mode;
        mb_type                  = em_h264_i_mb_type_info[mb_type].type;
    }
    ...
    if(IS_INTRA_PCM(mb_type)){
        const int mb_size = em_h264_mb_sizes[h->ps.sps->chroma_format_idc] *
                            h->ps.sps->bit_depth_luma;

        // We assume these blocks are very rare so we do not optimize it.
        sl->intra_pcm_ptr = align_get_bits(&sl->gb);
        if (get_bits_left(&sl->gb) < mb_size) {
            av_em_log(h->avctx, AV_LOG_ERROR, "Not enough data for an intra PCM block.\n");
            return AVERROR_INVALIDDATA;
        }
...
//                fill_intra4x4_pred_table(h);
            ...
            if (em_h264_check_intra4x4_pred_mode(sl->intra4x4_pred_mode_cache, h->avctx,
                                                 sl->top_samples_available, sl->left_samples_available) < 0)
                return -1;
        }else{
            sl->intra16x16_pred_mode = em_h264_check_intra_pred_mode(h->avctx, sl->top_samples_available,
                                                                     sl->left_samples_available, sl->intra16x16_pred_mode, 0);
            if (sl->intra16x16_pred_mode < 0)
                return -1;
        }
        if(decode_chroma){
            pred_mode= em_h264_check_intra_pred_mode(h->avctx, sl->top_samples_available,
        ...
        if (sl->slice_type_nos == AV_PICTURE_TYPE_B) {
            for(i=0; i<4; i++){
                ...
                if(sl->sub_mb_type[i] >=13){
                    av_em_log(h->avctx, AV_LOG_ERROR, "B sub_mb_type %u out of range at %d %d\n", sl->sub_mb_type[i], sl->mb_x, sl->mb_y);
                    return -1;
                }
                sub_partition_count[i] = em_h264_b_sub_mb_type_info[sl->sub_mb_type[i]].partition_count;
                sl->sub_mb_type[i]     = em_h264_b_sub_mb_type_info[sl->sub_mb_type[i]].type;
            }
            if( IS_DIRECT(sl->sub_mb_type[0]|sl->sub_mb_type[1]|sl->sub_mb_type[2]|sl->sub_mb_type[3])) {
                em_h264_pred_direct_motion(h, sl, &mb_type);
            ...
            for(i=0; i<4; i++){
                ...
                if(sl->sub_mb_type[i] >=4){
                    av_em_log(h->avctx, AV_LOG_ERROR, "P sub_mb_type %u out of range at %d %d\n", sl->sub_mb_type[i], sl->mb_x, sl->mb_y);
                    return -1;
                }
                sub_partition_count[i] = em_h264_p_sub_mb_type_info[sl->sub_mb_type[i]].partition_count;
                sl->sub_mb_type[i]     = em_h264_p_sub_mb_type_info[sl->sub_mb_type[i]].type;
            }
        }
            ...
            for(i=0; i<4; i++){
                ...
                if(IS_DIR(sl->sub_mb_type[i], 0, list)){
                    ...
                    if(ref_count == 1){
                        ...
                        if(tmp>=ref_count){
                            av_em_log(h->avctx, AV_LOG_ERROR, "ref %u overflow\n", tmp);
                            return -1;
                        }
                    }
                    ref[list][i]= tmp;
    ...
    if(!IS_INTRA16x16(mb_type)){
        ...
        if(decode_chroma){
            if(cbp > 47){
                av_em_log(h->avctx, AV_LOG_ERROR, "cbp too large (%u) at %d %d\n", cbp, sl->mb_x, sl->mb_y);
                return -1;
            }
            if (IS_INTRA4x4(mb_type))
                cbp = em_h264_golomb_to_intra4x4_cbp[cbp];
            else
                cbp = em_h264_golomb_to_inter_cbp[cbp];
        }else{
            if(cbp > 15){
                av_em_log(h->avctx, AV_LOG_ERROR, "cbp too large (%u) at %d %d\n", cbp, sl->mb_x, sl->mb_y);
                return -1;
        ...
        if (!decode_chroma && cbp>15) {
            av_em_log(h->avctx, AV_LOG_ERROR, "gray chroma\n");
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    if(cbp || IS_INTRA16x16(mb_type)){
        ...
        if (((unsigned)sl->qscale) > max_qp){
            ...
            if (((unsigned)sl->qscale) > max_qp){
                av_em_log(h->avctx, AV_LOG_ERROR, "dquant out of range (%d) at %d %d\n", dquant, sl->mb_x, sl->mb_y);
                return -1;
            }
        }
            ...
            if(cbp&0x30){
                for(chroma_idx=0; chroma_idx<2; chroma_idx++)
                    if (decode_residual(h, sl, gb, sl->mb + ((256 + 16*16*chroma_idx) << pixel_shift),
                                        ...
                                        CHROMA422(h) ? em_h264_chroma422_dc_scan : em_h264_chroma_dc_scan,
                                        NULL, 4 * num_c8x8) < 0) {
                        return -1;
                    }
            }
