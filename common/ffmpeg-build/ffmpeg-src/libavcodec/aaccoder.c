...
static void search_for_quantizers_anmr(AVEMCodecContext *avctx, AACEncContext *s,
                ...
                for (q = minscale; q < maxscale; q++) {
                    ...
                    for (i = 0; i < q1 - q0; i++) {
                        ...
                        cost = paths[idx - 1][i].cost + dist
                               + em_aac_scalefactor_bits[q - i + SCALE_DIFF_ZERO];
...
static void search_for_quantizers_fast(AVEMCodecContext *avctx, AACEncContext *s,
                                       SingleChannelElement *sce,
                                       const float lambda)
{
    int i, w, w2, g;
...
static void search_for_pns(AACEncContext *s, AVEMCodecContext *avctx, SingleChannelElement *sce)
{
    FFPsyBand *band;
    int w, g, w2, i;
    ...
    for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
        ...
        for (g = 0;  g < sce->ics.num_swb; g++) {
            ...
            noise_amp = -em_aac_pow2sf_tab[noise_sfi + POW_SF2_ZERO];    /* Dequantize */
            if (prev != -1000) {
...
static void mark_pns(AACEncContext *s, AVEMCodecContext *avctx, SingleChannelElement *sce)
{
    FFPsyBand *band;
    int w, g, w2;
    int wlen = 1024 / sce->ics.num_windows;
...
AACCoefficientsEncoder em_aac_coders[AAC_CODER_NB] = {
    [AAC_CODER_ANMR] = {
        search_for_quantizers_anmr,
        encode_window_bands_info,
        quantize_and_encode_band,
        em_aac_encode_tns_info,
        em_aac_encode_ltp_info,
        em_aac_encode_main_pred,
        em_aac_adjust_common_pred,
        em_aac_adjust_common_ltp,
        em_aac_apply_main_pred,
        em_aac_apply_tns,
        em_aac_update_ltp,
        em_aac_ltp_insert_new_frame,
        set_special_band_scalefactors,
        search_for_pns,
        mark_pns,
        em_aac_search_for_tns,
        em_aac_search_for_ltp,
        search_for_ms,
        em_aac_search_for_is,
        em_aac_search_for_pred,
    },
    [AAC_CODER_TWOLOOP] = {
