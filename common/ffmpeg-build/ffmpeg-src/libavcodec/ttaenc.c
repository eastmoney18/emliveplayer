...
static av_cold int tta_encode_init(AVEMCodecContext *avctx)
{
...
static int tta_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
...
#define PRED(x, k) (int32_t)((((uint64_t)(x) << (k)) - (x)) >> (k))
    ...
    put_bits32(&pb, av_em_crc(s->crc_table, UINT32_MAX, avpkt->data, out_bytes) ^ UINT32_MAX);
    flush_put_bits(&pb);
...
static av_cold int tta_encode_close(AVEMCodecContext *avctx)
{
    TTAEncContext *s = avctx->priv_data;
