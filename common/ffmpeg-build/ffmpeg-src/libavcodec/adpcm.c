...
static av_cold int adpcm_decode_init(AVEMCodecContext * avctx)
{
...
static inline int16_t adpcm_ms_expand_nibble(ADPCMChannelStatus *c, int nibble)
{
    ...
    if (c->idelta > INT_MAX/768) {
        av_em_log(NULL, AV_LOG_WARNING, "idelta overflow\n");
...
static int xa_decode(AVEMCodecContext *avctx, int16_t *out0, int16_t *out1,
                     ...
                     ADPCMChannelStatus *right, int channels, int sample_offset)
{
    ...
    for(i=0;i<4;i++) {
        ...
        if (filter >= FF_ARRAY_ELEMS(xa_adpcm_table)) {
            avpriv_em_request_sample(avctx, "unknown XA-ADPCM filter %d", filter);
            filter=0;
        }
...
static void adpcm_swf_decode(AVEMCodecContext *avctx, const uint8_t *buf, int buf_size, int16_t *samples)
{
    ADPCMDecodeContext *c = avctx->priv_data;
    GetBitContext gb;
    const int *table;
 ...
 */
static int get_nb_samples(AVEMCodecContext *avctx, GetByteContext *gb,
...
static int adpcm_decode_frame(AVEMCodecContext *avctx, void *data,
                              int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (nb_samples <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid number of samples in packet\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (coded_samples) {
        if (!approx_nb_samples && coded_samples != nb_samples)
            av_em_log(avctx, AV_LOG_WARNING, "mismatch in coded sample count\n");
    ...
    case AV_CODEC_ID_ADPCM_IMA_QT:
        ...
        for (channel = 0; channel < avctx->channels; channel++) {
            ...
            if (cs->step_index > 88u){
                av_em_log(avctx, AV_LOG_ERROR, "ERROR: step_index[%d] = %i\n",
    ...
    case AV_CODEC_ID_ADPCM_MS:
    {
        ...
        if (block_predictor > 6) {
            av_em_log(avctx, AV_LOG_ERROR, "ERROR: block_predictor[0] = %d\n",
                   block_predictor);
            return AVERROR_INVALIDDATA;
        }
        ...
        if (st) {
            block_predictor = bytestream2_get_byteu(&gb);
            if (block_predictor > 6) {
                av_em_log(avctx, AV_LOG_ERROR, "ERROR: block_predictor[1] = %d\n",
                       block_predictor);
                return AVERROR_INVALIDDATA;
            }
    ...
    case AV_CODEC_ID_ADPCM_IMA_DK3:
    {
        ...
        if (c->status[0].step_index > 88u || c->status[1].step_index > 88u){
            av_em_log(avctx, AV_LOG_ERROR, "ERROR: step_index = %i/%i\n",
...
#define DK3_GET_NEXT_NIBBLE() \
    ...
    case AV_CODEC_ID_ADPCM_EA_R3: {
        ...
        for (channel=0; channel<avctx->channels; channel++) {
            ...
            if (!count) {
                ...
                av_em_log(avctx, AV_LOG_WARNING, "per-channel sample count mismatch\n");
    ...
    case AV_CODEC_ID_ADPCM_IMA_AMV:
        ...
        if (c->status[0].step_index > 88u) {
            av_em_log(avctx, AV_LOG_ERROR, "ERROR: step_index = %i\n",
...
#define THP_GET16(g) \
    sign_extend( \
        ...
        if (avctx->extradata) {
            ...
            if (avctx->extradata_size < 32 * avctx->channels) {
                av_em_log(avctx, AV_LOG_ERROR, "Missing coeff table\n");
                return AVERROR_INVALIDDATA;
            }
    ...
    if (avpkt->size && bytestream2_tell(&gb) == 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Nothing consumed\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    *got_frame_ptr = 1;
    ...
    if (avpkt->size < bytestream2_tell(&gb)) {
        av_em_log(avctx, AV_LOG_ERROR, "Overread of %d < %d\n", avpkt->size, bytestream2_tell(&gb));
        return avpkt->size;
    }
...
static void adpcm_flush(AVEMCodecContext *avctx)
{
    ADPCMDecodeContext *c = avctx->priv_data;
    c->has_status = 0;
}
