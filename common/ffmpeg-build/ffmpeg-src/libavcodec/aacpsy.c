 ...
 */
static av_cold void lame_window_init(AacPsyContext *ctx, AVEMCodecContext *avctx)
{
    int i, j;
...
static av_cold int psy_3gpp_init(FFPsyContext *ctx) {
    ...
    ctx->model_priv_data = av_em_mallocz(sizeof(AacPsyContext));
    ...
    pctx->ch = av_em_mallocz_array(ctx->avctx->channels, sizeof(AacPsyChannel));
    if (!pctx->ch) {
        av_em_freep(&ctx->model_priv_data);
        return AVERROR(ENOMEM);
    }
...
static av_cold void psy_3gpp_end(FFPsyContext *apc)
{
    ...
    av_em_freep(&pctx->ch);
    av_em_freep(&apc->model_priv_data);
}
...
const FFPsyModel em_aac_psy_model =
{
