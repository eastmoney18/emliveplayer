...
static av_cold int flashsv_decode_end(AVEMCodecContext *avctx)
{
    FlashSVContext *s = avctx->priv_data;
    inflateEnd(&s->zstream);
    /* release the frame if needed */
    av_em_frame_free(&s->frame);

    /* free the tmpblock */
    av_em_freep(&s->tmpblock);

    return 0;
}

static av_cold int flashsv_decode_init(AVEMCodecContext *avctx)
{
    FlashSVContext *s = avctx->priv_data;
...
static int flashsv2_prime(FlashSVContext *s, uint8_t *src, int size)
{
    ...
    if ((zret = inflateReset(&s->zstream)) != Z_OK) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Inflate reset error: %d\n", zret);
...
static int flashsv_decode_block(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                                ...
                                int blk_idx)
{
    ...
    if (ret == Z_DATA_ERROR) {
        av_em_log(avctx, AV_LOG_ERROR, "Zlib resync occurred\n");
    ...
    if (!s->color_depth) {
        ...
        ret = decode_hybrid(s->tmpblock, s->zstream.next_out,
                      s->frame->data[0],
                      s->image_height - (y_pos + 1 + s->diem_height),
                      x_pos, s->diem_height, width,
                      s->frame->linesize[0], s->pal);
        if (ret < 0) {
            av_em_log(avctx, AV_LOG_ERROR, "decode_hybrid failed\n");
            return ret;
        }
    }
...
static int flashsv_decode_frame(AVEMCodecContext *avctx, void *data,
                                int *got_frame, AVEMPacket *avpkt)
{
    ...
    if (s->ver == 2) {
        skip_bits(&gb, 6);
        if (get_bits1(&gb)) {
            avpriv_em_request_sample(avctx, "iframe");
            return AVERROR_PATCHWELCOME;
        }
        if (get_bits1(&gb)) {
            avpriv_em_request_sample(avctx, "Custom palette");
            return AVERROR_PATCHWELCOME;
        }
    }
    ...
    if (s->block_size < s->block_width * s->block_height) {
        ...
        if ((err = av_em_reallocp(&s->tmpblock, tmpblock_size)) < 0) {
            s->block_size = 0;
        ...
        if (s->ver == 2) {
            ...
            if ((err = av_em_reallocp(&s->deflate_block, s->deflate_block_size)) < 0) {
                s->block_size = 0;
                av_em_log(avctx, AV_LOG_ERROR, "Cannot allocate deflate buffer.\n");
                return err;
            }
        }
    }
    ...
    if (avctx->width == 0 && avctx->height == 0) {
        if ((ret = em_set_dimensions(avctx, s->image_width, s->image_height)) < 0)
            return ret;
    }
    ...
    if (avctx->width != s->image_width || avctx->height != s->image_height) {
        ...
        av_em_log(avctx, AV_LOG_ERROR, "fh = %d, fv %d  vs  ch = %d, cv = %d\n",
    ...
    if (s->is_keyframe) {
        int err;
        if ((err = av_em_reallocp(&s->keyframedata, avpkt->size)) < 0)
            return err;
        memcpy(s->keyframedata, avpkt->data, avpkt->size);
    }
    if(s->ver == 2 && !s->blocks)
        s->blocks = av_em_mallocz((v_blocks + !!v_part) * (h_blocks + !!h_part) *
                               sizeof(s->blocks[0]));
    ...
    if ((ret = em_reget_buffer(avctx, s->frame)) < 0)
        return ret;
    ...
    for (j = 0; j < v_blocks + (v_part ? 1 : 0); j++) {
        ...
        for (i = 0; i < h_blocks + (h_part ? 1 : 0); i++) {
            ...
            s->zlibprime_prev = 0;
            s->diff_start     = 0;
            s->diem_height    = cur_blk_height;

            if (8 * size > get_bits_left(&gb)) {
                av_em_frame_unref(s->frame);
                return AVERROR_INVALIDDATA;
            }
            ...
            if (s->ver == 2 && size) {
                ...
                if (has_diff) {
                    if (size < 3) {
                        av_em_log(avctx, AV_LOG_ERROR, "size too small for diff\n");
                        return AVERROR_INVALIDDATA;
                    }
                    if (!s->keyframe) {
                        av_em_log(avctx, AV_LOG_ERROR,
                    ...
                    s->diem_height = get_bits(&gb, 8);
                    if (s->diff_start + s->diem_height > cur_blk_height) {
                        av_em_log(avctx, AV_LOG_ERROR,
                               "Block parameters invalid: %d + %d > %d\n",
                               s->diff_start, s->diem_height, cur_blk_height);
                        return AVERROR_INVALIDDATA;
                    }
                    av_em_log(avctx, AV_LOG_DEBUG,
                           "%dx%d diff start %d height %d\n",
                           i, j, s->diff_start, s->diem_height);
                    size -= 2;
                }

                if (s->zlibprime_prev)
                    av_em_log(avctx, AV_LOG_DEBUG, "%dx%d zlibprime_prev\n", i, j);

                if (s->zlibprime_curr) {
                    int col = get_bits(&gb, 8);
                    int row = get_bits(&gb, 8);
                    av_em_log(avctx, AV_LOG_DEBUG, "%dx%d zlibprime_curr %dx%d\n",
                           i, j, col, row);
                    if (size < 3) {
                        av_em_log(avctx, AV_LOG_ERROR, "size too small for zlibprime_curr\n");
                        return AVERROR_INVALIDDATA;
                    }
                    size -= 2;
                    avpriv_em_request_sample(avctx, "zlibprime_curr");
                    return AVERROR_PATCHWELCOME;
                }
    ...
    if (s->is_keyframe && s->ver == 2) {
        if (!s->keyframe) {
            s->keyframe = av_em_alloc(s->frame->linesize[0] * avctx->height);
            if (!s->keyframe) {
                av_em_log(avctx, AV_LOG_ERROR, "Cannot allocate image data\n");
                return AVERROR(ENOMEM);
            }
        }
    ...
    if ((ret = av_em_frame_ref(data, s->frame)) < 0)
        return ret;

    *got_frame = 1;

    if ((get_bits_count(&gb) / 8) != buf_size)
        av_em_log(avctx, AV_LOG_ERROR, "buffer not fully consumed (%d != %d)\n",
...
static av_cold int flashsv2_decode_init(AVEMCodecContext *avctx)
{
    FlashSVContext *s = avctx->priv_data;
...
static av_cold int flashsv2_decode_end(AVEMCodecContext *avctx)
{
    FlashSVContext *s = avctx->priv_data;

    av_em_freep(&s->keyframedata);
    av_em_freep(&s->blocks);
    av_em_freep(&s->keyframe);
    av_em_freep(&s->deflate_block);
    flashsv_decode_end(avctx);

    return 0;
}

AVEMCodec ff_flashsv2_decoder = {
    .name           = "flashsv2",
