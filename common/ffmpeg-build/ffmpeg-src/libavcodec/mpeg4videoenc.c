 ...
 */
void em_clean_mpeg4_qscales(MpegEncContext *s)
{
    int i;
    int8_t *const qscale_table = s->current_picture.qscale_table;
...
static inline void mpeg4_encode_dc(PutBitContext *s, int level, int n)
{
    ...
    if (n < 4) {
        /* luminance */
        put_bits(s, em_mpeg4_DCtab_lum[size][1], em_mpeg4_DCtab_lum[size][0]);
    } else {
        /* chrominance */
        put_bits(s, em_mpeg4_DCtab_chrom[size][1], em_mpeg4_DCtab_chrom[size][0]);
    }
...
void em_mpeg4_encode_mb(MpegEncContext *s, int16_t block[6][64],
...
//                s->mb_skipped = 1;
            ...
            if (!mb_type) {
                ...
                em_h263_encode_motion_vector(s, motion_x, motion_y, 1);
                s->b_count++;
                s->f_count++;
            } else {
                ...
                if (s->mv_type != MV_TYPE_FIELD) {
                    if (s->mv_dir & MV_DIR_FORWARD) {
                        em_h263_encode_motion_vector(s,
            ...
            if (s->mv_type == MV_TYPE_16X16) {
                if (s->dquant)
                    cbpc += 8;
                put_bits(&s->pb,
                         em_h263_inter_MCBPC_bits[cbpc],
                         em_h263_inter_MCBPC_code[cbpc]);

                put_bits(pb2, em_h263_cbpy_tab[cbpy][1], em_h263_cbpy_tab[cbpy][0]);
                if (s->dquant)
                ...
                put_bits(&s->pb,
                         em_h263_inter_MCBPC_bits[cbpc + 16],
                         em_h263_inter_MCBPC_code[cbpc + 16]);
                put_bits(pb2, em_h263_cbpy_tab[cbpy][1], em_h263_cbpy_tab[cbpy][0]);
                ...
                for (i = 0; i < 4; i++) {
                    ...
                    em_h263_pred_motion(s, i, 0, &pred_x, &pred_y);
        ...
        for (i = 0; i < 6; i++)
            dc_diff[i] = em_mpeg4_pred_dc(s, i, block[i][0], &dir[i], 1);
        ...
        if (s->pict_type == AV_PICTURE_TYPE_I) {
            if (s->dquant)
                cbpc += 4;
            put_bits(&s->pb,
                     em_h263_intra_MCBPC_bits[cbpc],
                     em_h263_intra_MCBPC_code[cbpc]);
        } else {
            if (s->dquant)
                cbpc += 8;
            put_bits(&s->pb, 1, 0);     /* mb coded */
            put_bits(&s->pb,
                     em_h263_inter_MCBPC_bits[cbpc + 4],
                     em_h263_inter_MCBPC_code[cbpc + 4]);
        }
 ...
 */
void em_mpeg4_stuffing(PutBitContext *pbc)
{
    int length;
    put_bits(pbc, 1, 0);
...
void em_set_mpeg4_time(MpegEncContext *s)
{
    if (s->pict_type == AV_PICTURE_TYPE_B) {
                                    ...
                                    int vol_number)
{
    ...
    s->aspect_ratio_info = em_h263_aspect_to_info(s->avctx->sample_aspect_ratio);
...
int em_mpeg4_encode_picture_header(MpegEncContext *s, int picture_number)
{
    uint64_t time_incr;
    ...
    if (time_incr > 3600) {
        av_em_log(s->avctx, AV_LOG_ERROR, "time_incr %"PRIu64" too large\n", time_incr);
...
static av_cold void init_uni_dc_tab(void)
{
    ...
    for (level = -256; level < 256; level++) {
        ...
        /* luminance */
        uni_code = em_mpeg4_DCtab_lum[size][0];
        uni_len  = em_mpeg4_DCtab_lum[size][1];
        ...
        /* chrominance */
        uni_code = em_mpeg4_DCtab_chrom[size][0];
        uni_len  = em_mpeg4_DCtab_chrom[size][1];
...
static av_cold int encode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->width >= (1<<13) || avctx->height >= (1<<13)) {
        av_em_log(avctx, AV_LOG_ERROR, "dimensions too large for MPEG-4\n");
        return AVERROR(EINVAL);
    }
    ...
    if (!done) {
        ...
        init_uni_mpeg4_rl_tab(&em_mpeg4_rl_intra, uni_mpeg4_intra_rl_bits, uni_mpeg4_intra_rl_len);
        init_uni_mpeg4_rl_tab(&em_h263_rl_inter, uni_mpeg4_inter_rl_bits, uni_mpeg4_inter_rl_len);
    }
    ...
    if (s->avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER) {
        s->avctx->extradata = av_em_alloc(1024);
...
void em_mpeg4_init_partitions(MpegEncContext *s)
{
...
void em_mpeg4_merge_partitions(MpegEncContext *s)
{
...
void em_mpeg4_encode_video_packet_header(MpegEncContext *s)
{
    int mb_num_bits = av_log2(s->mb_num - 1) + 1;

    put_bits(&s->pb, em_mpeg4_get_video_packet_prefix_length(s), 0);
    put_bits(&s->pb, 1, 1);
...
static const AVEMClass mpeg4enc_class = {
