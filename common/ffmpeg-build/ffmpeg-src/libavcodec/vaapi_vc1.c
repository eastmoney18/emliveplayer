...
static int vaapi_vc1_start_frame(AVEMCodecContext *avctx, av_unused const uint8_t *buffer, av_unused uint32_t size)
{
...
static int vaapi_vc1_decode_slice(AVEMCodecContext *avctx, const uint8_t *buffer, uint32_t size)
{
    VC1Context * const v = avctx->priv_data;
    MpegEncContext * const s = &v->s;
    FFVAContext * const vactx = ff_vaapi_get_context(avctx);
