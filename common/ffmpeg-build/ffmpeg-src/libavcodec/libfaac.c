...
static av_cold int Faac_encode_close(AVEMCodecContext *avctx)
{
...
static av_cold int Faac_encode_init(AVEMCodecContext *avctx)
{
    FaacAudioContext *s = avctx->priv_data;
    ...
    if (avctx->channels < 1 || avctx->channels > 6) {
        av_em_log(avctx, AV_LOG_ERROR, "encoding %d channel(s) is not allowed\n", avctx->channels);
        ret = AVERROR(EINVAL);
        goto error;
    }
    ...
    if (!s->faac_handle) {
        av_em_log(avctx, AV_LOG_ERROR, "error in faacEncOpen()\n");
        ret = AVERROR_UNKNOWN;
        goto error;
    }
    ...
    if (faac_cfg->version != FAAC_CFG_VERSION) {
        av_em_log(avctx, AV_LOG_ERROR, "wrong libfaac version (compiled for: %d, using %d)\n", FAAC_CFG_VERSION, faac_cfg->version);
        ret = AVERROR(EINVAL);
        goto error;
    }
    ...
    switch(avctx->profile) {
        ...
        case FF_PROFILE_AAC_LTP:
            ...
            break;
        default:
            av_em_log(avctx, AV_LOG_ERROR, "invalid AAC profile\n");
            ret = AVERROR(EINVAL);
            goto error;
    }
    ...
    if (avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER) {
        ...
        if (!faacEncGetDecoderSpecificInfo(s->faac_handle, &buffer,
            ...
            avctx->extradata = av_em_alloc(decoder_specific_info_size + AV_INPUT_BUFFER_PADDING_SIZE);
            if (!avctx->extradata) {
                ret = AVERROR(ENOMEM);
                goto error;
            }
    ...
    if (!faacEncSetConfiguration(s->faac_handle, faac_cfg)) {
        ...
        if (!i) {
            av_em_log(avctx, AV_LOG_ERROR, "libfaac doesn't support this output format!\n");
            ret = AVERROR(EINVAL);
            goto error;
        } else {
            avctx->bit_rate = 1000*i;
            av_em_log(avctx, AV_LOG_WARNING, "libfaac doesn't support the specified bitrate, using %dkbit/s instead\n", i);
        }
    }
...
static int Faac_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                             const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, (7 + 768) * avctx->channels, 0)) < 0)
        return ret;
    ...
    if (bytes_written < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "faacEncEncode() error\n");
