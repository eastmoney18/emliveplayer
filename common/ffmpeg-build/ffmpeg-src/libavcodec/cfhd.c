...
static av_cold int cfhd_decode_init(AVEMCodecContext *avctx)
{
...
static void free_buffers(AVEMCodecContext *avctx)
{
    CFHDContext *s = avctx->priv_data;
    int i, j;

    for (i = 0; i < 4; i++) {
        av_em_freep(&s->plane[i].idwt_buf);
        av_em_freep(&s->plane[i].idwt_tmp);
...
static int alloc_buffers(AVEMCodecContext *avctx)
{
    CFHDContext *s = avctx->priv_data;
    int i, j, k, ret, planes;

    if ((ret = em_set_dimensions(avctx, s->coded_width, s->coded_height)) < 0)
        return ret;
    avctx->pix_fmt = s->coded_format;

    avcodec_em_get_chroma_sub_sample(avctx->pix_fmt, &s->chroma_x_shift, &s->chroma_y_shift);
    planes = av_em_pix_fmt_count_planes(avctx->pix_fmt);
    ...
    for (i = 0; i < planes; i++) {
        ...
        s->plane[i].idwt_buf = av_em_mallocz_array(height * stride, sizeof(*s->plane[i].idwt_buf));
        s->plane[i].idwt_tmp = av_em_malloc_array(height * stride, sizeof(*s->plane[i].idwt_tmp));
...
static int cfhd_decode(AVEMCodecContext *avctx, void *data, int *got_frame,
                       AVEMPacket *avpkt)
{
    CFHDContext *s = avctx->priv_data;
    GetByteContext gb;
    ...
    planes = av_em_pix_fmt_count_planes(s->coded_format);
    ...
    while (bytestream2_get_bytes_left(&gb) > 4) {
        ...
        if (abs_tag8 >= 0x60 && abs_tag8 <= 0x6f) {
            av_em_log(avctx, AV_LOG_DEBUG, "large len %x\n", ((tagu & 0xff) << 16) | data);
        } else if (tag == 20) {
            av_em_log(avctx, AV_LOG_DEBUG, "Width %"PRIu16"\n", data);
            s->coded_width = data;
        } else if (tag == 21) {
            av_em_log(avctx, AV_LOG_DEBUG, "Height %"PRIu16"\n", data);
            s->coded_height = data;
        } else if (tag == 101) {
            av_em_log(avctx, AV_LOG_DEBUG, "Bits per component: %"PRIu16"\n", data);
            s->bpc = data;
        } else if (tag == 12) {
            av_em_log(avctx, AV_LOG_DEBUG, "Channel Count: %"PRIu16"\n", data);
            s->channel_cnt = data;
            if (data > 4) {
                av_em_log(avctx, AV_LOG_ERROR, "Channel Count of %"PRIu16" is unsupported\n", data);
                ret = AVERROR_PATCHWELCOME;
                break;
            }
        } else if (tag == 14) {
            av_em_log(avctx, AV_LOG_DEBUG, "Subband Count: %"PRIu16"\n", data);
            if (data != SUBBAND_COUNT) {
                av_em_log(avctx, AV_LOG_ERROR, "Subband Count of %"PRIu16" is unsupported\n", data);
                ret = AVERROR_PATCHWELCOME;
                break;
            }
        } else if (tag == 62) {
            s->channel_num = data;
            av_em_log(avctx, AV_LOG_DEBUG, "Channel number %"PRIu16"\n", data);
            if (s->channel_num >= planes) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid channel number\n");
                ret = AVERROR(EINVAL);
                break;
            }
            init_plane_defaults(s);
            ...
            if (s->subband_num != 0 && data == 1)  // hack
                s->level++;
            av_em_log(avctx, AV_LOG_DEBUG, "Subband number %"PRIu16"\n", data);
            s->subband_num = data;
            if (s->level >= DWT_LEVELS) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid level\n");
                ret = AVERROR(EINVAL);
                break;
            }
            if (s->subband_num > 3) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid subband number\n");
                ret = AVERROR(EINVAL);
                break;
            }
        } else if (tag == 51) {
            av_em_log(avctx, AV_LOG_DEBUG, "Subband number actual %"PRIu16"\n", data);
            s->subband_num_actual = data;
            if (s->subband_num_actual >= 10) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid subband number actual\n");
                ret = AVERROR(EINVAL);
                break;
            }
        } else if (tag == 35)
            av_em_log(avctx, AV_LOG_DEBUG, "Lowpass precision bits: %"PRIu16"\n", data);
        else if (tag == 53) {
            s->quantisation = data;
            av_em_log(avctx, AV_LOG_DEBUG, "Quantisation: %"PRIu16"\n", data);
        } else if (tag == 109) {
            s->prescale_shift[0] = (data >> 0) & 0x7;
            s->prescale_shift[1] = (data >> 3) & 0x7;
            s->prescale_shift[2] = (data >> 6) & 0x7;
            av_em_log(avctx, AV_LOG_DEBUG, "Prescale shift (VC-5): %x\n", data);
        } else if (tag == 27) {
            s->plane[s->channel_num].band[0][0].width  = data;
            s->plane[s->channel_num].band[0][0].stride = data;
            av_em_log(avctx, AV_LOG_DEBUG, "Lowpass width %"PRIu16"\n", data);
            if (data < 2 || data > s->plane[s->channel_num].band[0][0].a_width) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid lowpass width\n");
                ret = AVERROR(EINVAL);
                break;
            }
        } else if (tag == 28) {
            s->plane[s->channel_num].band[0][0].height = data;
            av_em_log(avctx, AV_LOG_DEBUG, "Lowpass height %"PRIu16"\n", data);
            if (data < 2 || data > s->plane[s->channel_num].band[0][0].height) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid lowpass height\n");
                ret = AVERROR(EINVAL);
                break;
            }
        } else if (tag == 1)
            av_em_log(avctx, AV_LOG_DEBUG, "Sample type? %"PRIu16"\n", data);
        else if (tag == 10) {
            if (data != 0) {
                avpriv_em_report_missing_feature(avctx, "Transform type of %"PRIu16, data);
                ret = AVERROR_PATCHWELCOME;
                break;
            }
            av_em_log(avctx, AV_LOG_DEBUG, "Transform-type? %"PRIu16"\n", data);
        } else if (abstag >= 0x4000 && abstag <= 0x40ff) {
            av_em_log(avctx, AV_LOG_DEBUG, "Small chunk length %d %s\n", data * 4, tag < 0 ? "optional" : "required");
            bytestream2_skipu(&gb, data * 4);
        } else if (tag == 23) {
            av_em_log(avctx, AV_LOG_DEBUG, "Skip frame\n");
            avpriv_em_report_missing_feature(avctx, "Skip frame");
            ret = AVERROR_PATCHWELCOME;
            break;
        } else if (tag == 2) {
            av_em_log(avctx, AV_LOG_DEBUG, "tag=2 header - skipping %i tag/value pairs\n", data);
            if (data > bytestream2_get_bytes_left(&gb) / 4) {
                av_em_log(avctx, AV_LOG_ERROR, "too many tag/value pairs (%d)\n", data);
            ...
            for (i = 0; i < data; i++) {
                ...
                av_em_log(avctx, AV_LOG_DEBUG, "Tag/Value = %x %x\n", tag2, val2);
            }
        } else if (tag == 41) {
            s->plane[s->channel_num].band[s->level][s->subband_num].width  = data;
            s->plane[s->channel_num].band[s->level][s->subband_num].stride = FFALIGN(data, 8);
            av_em_log(avctx, AV_LOG_DEBUG, "Highpass width %i channel %i level %i subband %i\n", data, s->channel_num, s->level, s->subband_num);
            if (data < 2) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid highpass width\n");
                ret = AVERROR(EINVAL);
                break;
            }
        } else if (tag == 42) {
            s->plane[s->channel_num].band[s->level][s->subband_num].height = data;
            av_em_log(avctx, AV_LOG_DEBUG, "Highpass height %i\n", data);
            if (data < 2) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid highpass height\n");
                ret = AVERROR(EINVAL);
                break;
            }
        } else if (tag == 49) {
            s->plane[s->channel_num].band[s->level][s->subband_num].width  = data;
            s->plane[s->channel_num].band[s->level][s->subband_num].stride = FFALIGN(data, 8);
            av_em_log(avctx, AV_LOG_DEBUG, "Highpass width2 %i\n", data);
            if (data < 2) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid highpass width2\n");
                ret = AVERROR(EINVAL);
                break;
            }
        } else if (tag == 50) {
            s->plane[s->channel_num].band[s->level][s->subband_num].height = data;
            av_em_log(avctx, AV_LOG_DEBUG, "Highpass height2 %i\n", data);
            if (data < 2) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid highpass height2\n");
                ret = AVERROR(EINVAL);
                break;
            }
        } else if (tag == 71) {
            s->codebook = data;
            av_em_log(avctx, AV_LOG_DEBUG, "Codebook %i\n", s->codebook);
        } else if (tag == 72) {
            s->codebook = data;
            av_em_log(avctx, AV_LOG_DEBUG, "Other codebook? %i\n", s->codebook);
        } else if (tag == 70) {
            av_em_log(avctx, AV_LOG_DEBUG, "Subsampling or bit-depth flag? %i\n", data);
            s->bpc = data;
            if (!(s->bpc == 10 || s->bpc == 12)) {
                av_em_log(avctx, AV_LOG_ERROR, "Invalid bits per channel\n");
                ret = AVERROR(EINVAL);
                break;
            }
        } else if (tag == 84) {
            av_em_log(avctx, AV_LOG_DEBUG, "Sample format? %i\n", data);
            if (data == 1)
                s->coded_format = AV_PIX_FMT_YUV422P10;
            else if (data == 3)
                ...
                avpriv_em_report_missing_feature(avctx, "Sample format of %"PRIu16, data);
                ret = AVERROR_PATCHWELCOME;
                break;
            }
            planes = av_em_pix_fmt_count_planes(s->coded_format);
        } else
            av_em_log(avctx, AV_LOG_DEBUG,  "Unknown tag %i data %x\n", tag, data);
        ...
        if (tag == 4 && data == 0x1a4a && s->coded_width && s->coded_height &&
            ...
            ret = em_set_dimensions(avctx, s->coded_width, s->coded_height);
            if (ret < 0)
                return ret;
            frame.f->width =
            frame.f->height = 0;

            if ((ret = em_thread_get_buffer(avctx, &frame, 0)) < 0)
                return ret;
        ...
        if (tag == 4 && data == 0xf0f && s->a_width && s->a_height) {
            ...
            if (!got_buffer) {
                av_em_log(avctx, AV_LOG_ERROR, "No end of header tag found\n");
                ret = AVERROR(EINVAL);
                goto end;
            }
            ...
            if (lowpass_height > lowpass_a_height || lowpass_width > lowpass_a_width ||
                ...
                av_em_log(avctx, AV_LOG_ERROR, "Too many lowpass coefficients\n");
                ret = AVERROR(EINVAL);
                goto end;
            }

            av_em_log(avctx, AV_LOG_DEBUG, "Start of lowpass coeffs component %d height:%d, width:%d\n", s->channel_num, lowpass_height, lowpass_width);
            ...
            av_em_log(avctx, AV_LOG_DEBUG, "Lowpass coefficients %d\n", lowpass_width * lowpass_height);
        }
        ...
        if (tag == 55 && s->subband_num_actual != 255 && s->a_width && s->a_height) {
            ...
            if (highpass_height > highpass_a_height || highpass_width > highpass_a_width || a_expected < expected) {
                av_em_log(avctx, AV_LOG_ERROR, "Too many highpass coefficents\n");
                ret = AVERROR(EINVAL);
                goto end;
            }

            av_em_log(avctx, AV_LOG_DEBUG, "Start subband coeffs plane %i level %i codebook %i expected %i\n", s->channel_num, s->level, s->codebook, expected);
            ...
            if (count > expected) {
                av_em_log(avctx, AV_LOG_ERROR, "Escape codeword not found, probably corrupt data\n");
                ret = AVERROR(EINVAL);
                goto end;
            }
            ...
            if (bytes > bytestream2_get_bytes_left(&gb)) {
                av_em_log(avctx, AV_LOG_ERROR, "Bitstream overread error\n");
                ret = AVERROR(EINVAL);
                goto end;
            } else
                bytestream2_seek(&gb, bytes, SEEK_CUR);

            av_em_log(avctx, AV_LOG_DEBUG, "End subband coeffs %i extra %i\n", count, count - expected);
            s->codebook = 0;
    ...
    if (!s->a_width || !s->a_height || s->a_format == AV_PIX_FMT_NONE ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "Invalid dimensions\n");
        ret = AVERROR(EINVAL);
        goto end;
    }
    ...
    for (plane = 0; plane < planes && !ret; plane++) {
        ...
        if (lowpass_height > s->plane[plane].band[0][0].a_height || lowpass_width > s->plane[plane].band[0][0].a_width ||
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Invalid plane dimensions\n");
            ret = AVERROR(EINVAL);
            goto end;
        }

        av_em_log(avctx, AV_LOG_DEBUG, "Decoding level 1 plane %i %i %i %i\n", plane, lowpass_height, lowpass_width, highpass_stride);
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Level 2 plane %i %i %i %i\n", plane, lowpass_height, lowpass_width, highpass_stride);
        ...
        av_em_log(avctx, AV_LOG_DEBUG, "Level 3 plane %i %i %i %i\n", plane, lowpass_height, lowpass_width, highpass_stride);
...
static av_cold int cfhd_close_decoder(AVEMCodecContext *avctx)
{
    CFHDContext *s = avctx->priv_data;
