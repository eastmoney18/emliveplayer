...
static const AVEMClass avdct_class = {
    .class_name              = "AVDCT",
    .option                  = avdct_options,
    .version                 = LIBAVUTIL_VERSION_INT,
};

const AVEMClass *avcodec_em_dct_get_class(void)
{
    return &avdct_class;
}

AVDCT *avcodec_em_dct_alloc(void)
{
    AVDCT *dsp = av_em_mallocz(sizeof(AVDCT));

    if (!dsp)
        return NULL;

    dsp->av_class = &avdct_class;
    av_em_opt_set_defaults(dsp);

    return dsp;
}

int avcodec_em_dct_init(AVDCT *dsp)
{
    AVEMCodecContext *avctx = avcodec_em_alloc_context3(NULL);
