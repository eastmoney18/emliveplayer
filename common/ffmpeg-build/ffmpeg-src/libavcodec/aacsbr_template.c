...
av_cold void AAC_RENAME(em_aac_sbr_init)(void)
{
    static const struct {
...
av_cold void AAC_RENAME(em_aac_sbr_ctx_init)(AACContext *ac, SpectralBandReplication *sbr)
{
    ...
    AAC_RENAME(em_ps_ctx_init)(&sbr->ps);
    AAC_RENAME(ff_sbrdsp_init)(&sbr->dsp);
    aacsbr_func_ptr_init(&sbr->c);
}

av_cold void AAC_RENAME(em_aac_sbr_ctx_close)(SpectralBandReplication *sbr)
{
...
static int check_n_master(AVEMCodecContext *avctx, int n_master, int bs_xover_band)
{
    // Requirements (14496-3 sp04 p205)
    if (n_master <= 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid n_master: %d\n", n_master);
        return -1;
    }
                             ...
                             SpectrumParameters *spectrum)
{
    ...
    if (!spectrum->bs_freq_scale) {
        ...
        if (num_bands_0 <= 0) { // Requirements (14496-3 sp04 p205)
            av_em_log(ac->avctx, AV_LOG_ERROR, "Invalid num_bands_0: %d\n", num_bands_0);
            return -1;
        }
        ...
        for (k = 1; k <= num_bands_0; k++) {
            if (vk0[k] <= 0) { // Requirements (14496-3 sp04 p205)
                av_em_log(ac->avctx, AV_LOG_ERROR, "Invalid vDk0[%d]: %d\n", k, vk0[k]);
                return -1;
            }
            vk0[k] += vk0[k-1];
        }
        ...
        if (two_regions) {
            ...
            for (k = 1; k <= num_bands_1; k++) {
                if (vk1[k] <= 0) { // Requirements (14496-3 sp04 p205)
                    av_em_log(ac->avctx, AV_LOG_ERROR, "Invalid vDk1[%d]: %d\n", k, vk1[k]);
                    return -1;
                }
                vk1[k] += vk1[k-1];
            }
...
static int sbr_hf_calc_npatches(AACContext *ac, SpectralBandReplication *sbr)
{
    ...
    do {
        ...
        if (k == last_k && msb == last_msb) {
            av_em_log(ac->avctx, AV_LOG_ERROR, "patch construction failed\n");
        ...
        if (sbr->num_patches > 5) {
            av_em_log(ac->avctx, AV_LOG_ERROR, "Too many patches: %d\n", sbr->num_patches);
            return -1;
        }
...
static int sbr_make_f_derived(AACContext *ac, SpectralBandReplication *sbr)
{
    ...
    if (sbr->kx[1] > 32) {
        av_em_log(ac->avctx, AV_LOG_ERROR, "Start frequency border too high: %d\n", sbr->kx[1]);
        return -1;
    }
    ...
    if (sbr->n_q > 5) {
        av_em_log(ac->avctx, AV_LOG_ERROR, "Too many noise floor scale factors: %d\n", sbr->n_q);
        return -1;
    }
                         ...
                         GetBitContext *gb, SBRData *ch_data)
{
    ...
    for (i = 1; i <= ch_data->bs_num_env; i++) {
        if (ch_data->t_env[i-1] >= ch_data->t_env[i]) {
            av_em_log(ac->avctx, AV_LOG_ERROR, "Not strictly monotone time borders\n");
            return -1;
        }
    }
            ...
            if (ch_data->bs_freq_res[i + 1] == ch_data->bs_freq_res[i]) {
                for (j = 0; j < sbr->n[ch_data->bs_freq_res[i + 1]]; j++) {
                    ...
                    if (ch_data->env_facs_q[i + 1][j] > 127U) {
                        av_em_log(ac->avctx, AV_LOG_ERROR, "env_facs_q %d is invalid\n", ch_data->env_facs_q[i + 1][j]);
                        return AVERROR_INVALIDDATA;
                    }
                }
        ...
        if (ch_data->bs_df_noise[i]) {
            for (j = 0; j < sbr->n_q; j++) {
                ...
                if (ch_data->noise_facs_q[i + 1][j] > 30U) {
                    av_em_log(ac->avctx, AV_LOG_ERROR, "noise_facs_q %d is invalid\n", ch_data->noise_facs_q[i + 1][j]);
                    return AVERROR_INVALIDDATA;
                }
            }
        } else {
                               ...
                               int bs_extension_id, int *num_bits_left)
{
    ...
    case EXTENSION_ID_PS:
        if (!ac->oc[1].m4ac.ps) {
            av_em_log(ac->avctx, AV_LOG_ERROR, "Parametric Stereo signaled to be not-present but was found in the bitstream.\n");
            skip_bits_long(gb, *num_bits_left); // bs_fill_bits
            *num_bits_left = 0;
        } else {
#if 1
            *num_bits_left -= AAC_RENAME(em_ps_read_data)(ac->avctx, gb, &sbr->ps, *num_bits_left);
            ac->avctx->profile = FF_PROFILE_AAC_HE_V2;
#else
            avpriv_em_report_missing_feature(ac->avctx, "Parametric Stereo");
            skip_bits_long(gb, *num_bits_left); // bs_fill_bits
            *num_bits_left = 0;
#endif
        }
    ...
    default:
        ...
        if (bs_extension_id || *num_bits_left > 16 || show_bits(gb, *num_bits_left))
            avpriv_em_request_sample(ac->avctx, "Reserved SBR extensions");
        skip_bits_long(gb, *num_bits_left); // bs_fill_bits
        *num_bits_left = 0;
        break;
    }
                                  ...
                                  GetBitContext *gb, int id_aac)
{
    ...
    if (get_bits1(gb)) { // bs_extended_data
        ...
        if (num_bits_left < 0) {
            av_em_log(ac->avctx, AV_LOG_ERROR, "SBR Extension over read.\n");
        }
        if (num_bits_left > 0)
                        ...
                        SBRData *ch_data, int e_a[2])
{
    ...
    for (e = 0; e < ch_data->bs_num_env; e++) {
        ...
        if (sbr->kx[1] != table[0]) {
            av_em_log(ac->avctx, AV_LOG_ERROR, "kx != f_table{high,low}[0]. "
                  ...
                  INTFLOAT* L, INTFLOAT* R)
{
    ...
    if (id_aac != sbr->id_aac) {
        av_em_log(ac->avctx, id_aac == TYPE_LFE ? AV_LOG_VERBOSE : AV_LOG_WARNING,
    ...
    if (ac->oc[1].m4ac.ps == 1) {
        if (sbr->ps.start) {
            AAC_RENAME(em_ps_apply)(ac->avctx, &sbr->ps, sbr->X[0], sbr->X[1], sbr->kx[1] + sbr->m[1]);
        } else {
