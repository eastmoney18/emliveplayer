...
static av_cold int libspeex_decode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->extradata && avctx->extradata_size >= 80) {
        ...
        if (!header)
            av_em_log(avctx, AV_LOG_WARNING, "Invalid Speex header\n");
    }
    if (avctx->codec_tag == MKTAG('S', 'P', 'X', 'N')) {
        int quality;
        if (!avctx->extradata || avctx->extradata && avctx->extradata_size < 47) {
            av_em_log(avctx, AV_LOG_ERROR, "Missing or invalid extradata.\n");
            return AVERROR_INVALIDDATA;
        }

        quality = avctx->extradata[37];
        if (quality > 10) {
            av_em_log(avctx, AV_LOG_ERROR, "Unsupported quality mode %d.\n", quality);
        ...
        default:
            ...
            av_em_log(avctx, AV_LOG_WARNING, "Invalid sample rate: %d\n"
    ...
    if (!mode) {
        av_em_log(avctx, AV_LOG_ERROR, "Unknown Speex mode %d", spx_mode);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (!s->dec_state) {
        av_em_log(avctx, AV_LOG_ERROR, "Error initializing libspeex decoder.\n");
        return -1;
    }
...
static int libspeex_decode_frame(AVEMCodecContext *avctx, void *data,
                                 int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (ret <= -2) {
        av_em_log(avctx, AV_LOG_ERROR, "Error decoding Speex frame.\n");
        return AVERROR_INVALIDDATA;
    }
    if (avctx->channels == 2)
...
static av_cold int libspeex_decode_close(AVEMCodecContext *avctx)
{
    LibSpeexContext *s = avctx->priv_data;
...
static av_cold void libspeex_decode_flush(AVEMCodecContext *avctx)
{
    LibSpeexContext *s = avctx->priv_data;
    speex_bits_reset(&s->bits);
}

AVEMCodec ff_libspeex_decoder = {
    .name           = "libspeex",
