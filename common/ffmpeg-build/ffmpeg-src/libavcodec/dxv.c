...
#define CHECKPOINT(x)                                                         \
    do {                                                                      \
        ...
        case 2:                                                               \
            ...
            if (idx > pos) {                                                  \
                av_em_log(avctx, AV_LOG_ERROR, "idx %d > %d\n", idx, pos);       \
...
static int dxv_decompress_dxt1(AVEMCodecContext *avctx)
{
    DXVContext *ctx = avctx->priv_data;
...
static int dxv_decompress_dxt5(AVEMCodecContext *avctx)
{
    DXVContext *ctx = avctx->priv_data;
    GetByteContext *gbc = &ctx->gbc;
    uint32_t value, op;
...
static int dxv_decompress_lzf(AVEMCodecContext *avctx)
{
    DXVContext *ctx = avctx->priv_data;
    return ff_lzf_uncompress(&ctx->gbc, &ctx->tex_data, &ctx->tex_size);
}

static int dxv_decompress_raw(AVEMCodecContext *avctx)
{
    DXVContext *ctx = avctx->priv_data;
    GetByteContext *gbc = &ctx->gbc;
...
static int dxv_decode(AVEMCodecContext *avctx, void *data,
                      int *got_frame, AVEMPacket *avpkt)
{
    DXVContext *ctx = avctx->priv_data;
    ThreadFrame tframe;
    GetByteContext *gbc = &ctx->gbc;
    int (*decompress_tex)(AVEMCodecContext *avctx);
    ...
    case MKBETAG('Y', 'G', '1', '0'):
        avpriv_em_report_missing_feature(avctx, "Tag 0x%08X", tag);
    ...
    default:
        ...
        if (old_type & 0x40) {
            ...
            av_em_log(avctx, AV_LOG_ERROR, "Unsupported header (0x%08X)\n.", tag);
            return AVERROR_INVALIDDATA;
        }
        ctx->tex_rat = 1;
        break;
    ...
    ret = av_em_reallocp(&ctx->tex_data, ctx->tex_size);
    if (ret < 0)
        return ret;
...
static int dxv_init(AVEMCodecContext *avctx)
{
    DXVContext *ctx = avctx->priv_data;
...
static int dxv_close(AVEMCodecContext *avctx)
{
    DXVContext *ctx = avctx->priv_data;

    av_em_freep(&ctx->tex_data);

    return 0;
}

AVEMCodec ff_dxv_decoder = {
    .name           = "dxv",
