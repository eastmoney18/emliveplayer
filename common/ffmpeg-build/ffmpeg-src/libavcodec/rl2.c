 ...
 */
static av_cold int rl2_decode_init(AVEMCodecContext *avctx)
{
    ...
    if (!avctx->extradata || avctx->extradata_size < EXTRADATA1_SIZE) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid extradata size\n");
    ...
    if (s->video_base >= avctx->width * avctx->height) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid video_base\n");
    ...
    if (back_size > 0) {
        uint8_t *back_frame = av_em_mallocz(avctx->width*avctx->height);
        if (!back_frame)
...
static int rl2_decode_frame(AVEMCodecContext *avctx,
 ...
 * @return 0 success, -1 on error
 */
static av_cold int rl2_decode_end(AVEMCodecContext *avctx)
{
    Rl2Context *s = avctx->priv_data;

    av_em_freep(&s->back_frame);

    return 0;
}


AVEMCodec ff_rl2_decoder = {
    .name           = "rl2",
