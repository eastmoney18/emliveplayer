...
static av_cold int libwebp_anim_encode_init(AVEMCodecContext *avctx)
{
    ...
    if (!ret) {
        ...
        enc_options.verbose = av_em_log_get_level() >= AV_LOG_VERBOSE;
...
static int libwebp_anim_encode_frame(AVEMCodecContext *avctx, AVEMPacket *pkt,
    ...
    if (!frame) {
        if (s->done) {  // Second flush: return empty package to denote finish.
            *got_packet = 0;
            ...
            if (ret) {
                ret = em_alloc_packet2(avctx, pkt, assembled_data.size, assembled_data.size);
                if (ret < 0)
                    return ret;
        ...
        *got_packet = 1;
        ...
        WebPPictureFree(pic);
        av_em_freep(&pic);
        av_em_frame_free(&alt_frame);
        return ret;
    }
}

static int libwebp_anim_encode_close(AVEMCodecContext *avctx)
{
    LibWebPAnimContext *s = avctx->priv_data;
    av_em_frame_free(&s->cc.ref);
...
AVEMCodec ff_libwebp_anim_encoder = {
    .name           = "libwebp_anim",
