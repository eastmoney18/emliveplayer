...
int ff_vaapi_context_init(AVEMCodecContext *avctx)
{
    FFVAContext * const vactx = ff_vaapi_get_context(avctx);
    const struct vaapi_context * const user_vactx = avctx->hwaccel_context;

    if (!user_vactx) {
        av_em_log(avctx, AV_LOG_ERROR, "Hardware acceleration context (hwaccel_context) does not exist.\n");
...
int ff_vaapi_context_fini(AVEMCodecContext *avctx)
{
    return 0;
}
...
int ff_vaapi_commit_slices(FFVAContext *vactx)
{
    ...
    slice_buf_ids =
        av_em_fast_realloc(vactx->slice_buf_ids,
...
VASliceParameterBufferBase *ff_vaapi_alloc_slice(FFVAContext *vactx, const uint8_t *buffer, uint32_t size)
{
    ...
    slice_params =
        av_em_fast_realloc(vactx->slice_params,
...
void ff_vaapi_common_end_frame(AVEMCodecContext *avctx)
{
    FFVAContext * const vactx = ff_vaapi_get_context(avctx);
    ...
    av_em_freep(&vactx->slice_buf_ids);
    av_em_freep(&vactx->slice_params);
...
int ff_vaapi_mpeg_end_frame(AVEMCodecContext *avctx)
{
    FFVAContext * const vactx = ff_vaapi_get_context(avctx);
