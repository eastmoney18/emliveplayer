...
static int decode_frame(AVEMCodecContext *avctx, void *data, int *data_size,
                        AVEMPacket *avpkt) {
    ...
    if (buf_size < 27 + 7 * 2 + 4 * (3 + has_alpha)) {
        av_em_log(avctx, AV_LOG_ERROR, "coded frame size %d too small\n", buf_size);
        return -1;
    }

    // read start and end time
    if (buf[0] != '[' || buf[13] != '-' || buf[26] != ']') {
        av_em_log(avctx, AV_LOG_ERROR, "invalid time code\n");
        return -1;
    }
    if (avpkt->pts != AV_NOPTS_VALUE)
        packet_time = av_em_rescale_q(avpkt->pts, AV_TIME_BASE_Q, (AVEMRational){1, 1000});
    ...
    if (av_em_image_check_size(w, h, 0, avctx) < 0)
        return -1;
    ...
    sub->rects =  av_em_mallocz(sizeof(*sub->rects));
    if (!sub->rects)
        return AVERROR(ENOMEM);

    sub->rects[0] = av_em_mallocz(sizeof(*sub->rects[0]));
    if (!sub->rects[0]) {
    ...
    sub->rects[0]->data[0] = av_em_alloc(w * h);
    sub->rects[0]->nb_colors = 4;
    sub->rects[0]->data[1] = av_em_mallocz(AVPALETTE_SIZE);
    if (!sub->rects[0]->data[0] || !sub->rects[0]->data[1]) {
        av_em_freep(&sub->rects[0]->data[1]);
        av_em_freep(&sub->rects[0]->data[0]);
        av_em_freep(&sub->rects[0]);
        av_em_freep(&sub->rects);
        return AVERROR(ENOMEM);
    }
    sub->num_rects = 1;
