...
static av_cold int yop_decode_close(AVEMCodecContext *avctx)
{
...
static av_cold int yop_decode_init(AVEMCodecContext *avctx)
{
    YopDecContext *s = avctx->priv_data;
    s->avctx = avctx;
    ...
    if (avctx->width & 1 || avctx->height & 1 ||
        ...
        av_em_log(avctx, AV_LOG_ERROR, "YOP has invalid dimensions\n");
        return AVERROR_INVALIDDATA;
    }

    if (avctx->extradata_size < 3) {
        av_em_log(avctx, AV_LOG_ERROR, "Missing or incomplete extradata.\n");
        return AVERROR_INVALIDDATA;
    }
...
static int yop_paint_block(YopDecContext *s, int linesize, int tag)
{
    if (s->src_end - s->srcptr < paint_lut[tag][3]) {
        av_em_log(s->avctx, AV_LOG_ERROR, "Packet too small.\n");
        return AVERROR_INVALIDDATA;
    }
...
static int yop_copy_previous_block(YopDecContext *s, int linesize, int copy_tag)
{
    ...
    if (bufptr < s->dstbuf) {
        av_em_log(s->avctx, AV_LOG_ERROR, "File probably corrupt\n");
        return AVERROR_INVALIDDATA;
    }
...
static int yop_decode_frame(AVEMCodecContext *avctx, void *data, int *got_frame,
                            AVEMPacket *avpkt)
{
    YopDecContext *s = avctx->priv_data;
    ...
    if(is_odd_frame>1){
        av_em_log(avctx, AV_LOG_ERROR, "frame is too odd %d\n", is_odd_frame);
        return AVERROR_INVALIDDATA;
    }
