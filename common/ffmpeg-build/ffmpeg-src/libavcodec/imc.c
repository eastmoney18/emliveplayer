...
static av_cold int imc_decode_init(AVEMCodecContext *avctx)
{
    int i, j, ret;
    ...
    if (avctx->channels > 2) {
        avpriv_em_request_sample(avctx, "Number of channels > 2");
        return AVERROR_PATCHWELCOME;
    }
    ...
    if ((ret = ff_fft_init(&q->fft, 7, 1))) {
        av_em_log(avctx, AV_LOG_INFO, "FFT init failed\n");
        return ret;
    }
    em_bswapdsp_init(&q->bdsp);
                          ...
                          int stream_format_code, int freebits, int flag)
{
    ...
    for (i = 0; i < BANDS - 1; i++) {
        if (chctx->flcoeffs5[i] <= 0) {
            av_em_log(NULL, AV_LOG_ERROR, "flcoeffs5 %f invalid\n", chctx->flcoeffs5[i]);
...
static void imc_get_coeffs(AVEMCodecContext *avctx,
...
static int imc_decode_block(AVEMCodecContext *avctx, IMCContext *q, int ch)
{
    int stream_format_code;
    int imc_hdr, i, j, ret;
    int flag;
    ...
    if (imc_hdr & 0x18) {
        av_em_log(avctx, AV_LOG_ERROR, "frame header check failed!\n");
        av_em_log(avctx, AV_LOG_ERROR, "got %X.\n", imc_hdr);
        return AVERROR_INVALIDDATA;
    }
    ...
    for(i=0; i<BANDS; i++) {
        if(chctx->flcoeffs1[i] > INT_MAX) {
            av_em_log(avctx, AV_LOG_ERROR, "scalefactor out of range\n");
            return AVERROR_INVALIDDATA;
        }
    }
    ...
    if ((ret = bit_allocation(q, chctx, stream_format_code,
                              ...
                              flag)) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Bit allocations failed\n");
        chctx->decoder_reset = 1;
        return ret;
    }
    ...
    if (inverse_quant_coeff(q, chctx, stream_format_code) < 0) {
        av_em_log(avctx, AV_LOG_ERROR, "Inverse quantization of coefficients failed\n");
        chctx->decoder_reset = 1;
        return AVERROR_INVALIDDATA;
    }
...
static int imc_decode_frame(AVEMCodecContext *avctx, void *data,
                            int *got_frame_ptr, AVEMPacket *avpkt)
{
    ...
    if (buf_size < IMC_BLOCK_SIZE * avctx->channels) {
        av_em_log(avctx, AV_LOG_ERROR, "frame too small!\n");
        return AVERROR_INVALIDDATA;
    }
...
static av_cold int imc_decode_close(AVEMCodecContext * avctx)
{
    IMCContext *q = avctx->priv_data;
