...
static int dxtory_decode_v1_rgb(AVEMCodecContext *avctx, AVFrame *pic,
...
static int dxtory_decode_v1_410(AVEMCodecContext *avctx, AVFrame *pic,
                                const uint8_t *src, int src_size)
{
    int h, w;
...
static int dxtory_decode_v1_420(AVEMCodecContext *avctx, AVFrame *pic,
                                const uint8_t *src, int src_size)
{
    int h, w;
    uint8_t *Y1, *Y2, *U, *V;
...
static int dxtory_decode_v1_444(AVEMCodecContext *avctx, AVFrame *pic,
                                const uint8_t *src, int src_size)
{
    int h, w;
    uint8_t *Y, *U, *V;
...
static int check_slice_size(AVEMCodecContext *avctx,
                            const uint8_t *src, int src_size,
                            int slice_size, int off)
{
    ...
    if (slice_size <= 16) {
        av_em_log(avctx, AV_LOG_ERROR, "invalid slice size %"PRIu32"\n",
               slice_size);
        return AVERROR_INVALIDDATA;
    }
...
static int load_buffer(AVEMCodecContext *avctx,
                       const uint8_t *src, int src_size,
                       GetByteContext *gb,
                       int *nslices, int *off)
{
    ...
    *off = FFALIGN(*nslices * 4 + 2, 16);
    if (src_size < *off) {
        av_em_log(avctx, AV_LOG_ERROR, "no slice data\n");
        return AVERROR_INVALIDDATA;
    }

    if (!*nslices) {
        avpriv_em_request_sample(avctx, "%d slices for %dx%d", *nslices,
...
static int dxtory_decode_v2(AVEMCodecContext *avctx, AVFrame *pic,
                            const uint8_t *src, int src_size,
...
static int dxtory_decode_v2_565(AVEMCodecContext *avctx, AVFrame *pic,
...
static int dxtory_decode_v2_rgb(AVEMCodecContext *avctx, AVFrame *pic,
                                const uint8_t *src, int src_size)
{
    return dxtory_decode_v2(avctx, pic, src, src_size,
...
static int dxtory_decode_v2_410(AVEMCodecContext *avctx, AVFrame *pic,
                                const uint8_t *src, int src_size)
{
    return dxtory_decode_v2(avctx, pic, src, src_size,
...
static int dxtory_decode_v2_420(AVEMCodecContext *avctx, AVFrame *pic,
                                const uint8_t *src, int src_size)
{
    return dxtory_decode_v2(avctx, pic, src, src_size,
...
static int dxtory_decode_v2_444(AVEMCodecContext *avctx, AVFrame *pic,
                                const uint8_t *src, int src_size)
{
    return dxtory_decode_v2(avctx, pic, src, src_size,
                        ...
                        AVEMPacket *avpkt)
{
    ...
    case 0x19000009:
        ...
        break;
    default:
        avpriv_em_request_sample(avctx, "Frame header %"PRIX32, AV_RB32(src));
        return AVERROR_PATCHWELCOME;
    }
