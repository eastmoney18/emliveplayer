...
static av_cold int wavpack_encode_init(AVEMCodecContext *avctx)
{
    ...
    if (avctx->channels > 255) {
        av_em_log(avctx, AV_LOG_ERROR, "Invalid channel count: %d\n", avctx->channels);
...
static int allocate_buffers2(WavPackEncodeContext *s, int nterms)
{
    ...
    for (i = 0; i < nterms + 2; i++) {
        av_em_fast_padded_malloc(&s->sampleptrs[i][0], &s->sampleptrs_size[i][0],
                              s->block_samples * 4);
        if (!s->sampleptrs[i][0])
            return AVERROR(ENOMEM);
        if (!(s->flags & WV_MONO_DATA)) {
            av_em_fast_padded_malloc(&s->sampleptrs[i][1], &s->sampleptrs_size[i][1],
                                  s->block_samples * 4);
            if (!s->sampleptrs[i][1])
                return AVERROR(ENOMEM);
        }
...
static int allocate_buffers(WavPackEncodeContext *s)
{
    ...
    for (i = 0; i < 2; i++) {
        av_em_fast_padded_malloc(&s->best_buffer[0], &s->best_buffer_size[0],
                              s->block_samples * 4);
        if (!s->best_buffer[0])
            return AVERROR(ENOMEM);

        av_em_fast_padded_malloc(&s->temp_buffer[i][0], &s->temp_buffer_size[i][0],
                              s->block_samples * 4);
        if (!s->temp_buffer[i][0])
            return AVERROR(ENOMEM);
        if (!(s->flags & WV_MONO_DATA)) {
            av_em_fast_padded_malloc(&s->best_buffer[1], &s->best_buffer_size[1],
                                  s->block_samples * 4);
            if (!s->best_buffer[1])
                return AVERROR(ENOMEM);

            av_em_fast_padded_malloc(&s->temp_buffer[i][1], &s->temp_buffer_size[i][1],
                                  s->block_samples * 4);
    ...
    for (pi = 0; pi < s->num_passes;) {
        ...
        while (1) {
            if (force_js || (wpds->joint_stereo && !force_ts)) {
                if (!got_js) {
                    av_em_fast_padded_malloc(&s->js_left,  &s->js_left_size,  buf_size);
                    av_em_fast_padded_malloc(&s->js_right, &s->js_right_size, buf_size);
                                ...
                                uint8_t *out, int out_size)
{
    ...
    if ((s->flags & WV_FLOAT_DATA) || (s->flags & MAG_MASK) >> MAG_LSB >= 24) {
        av_em_fast_padded_malloc(&s->orig_l, &s->orig_l_size, sizeof(int32_t) * nb_samples);
        memcpy(s->orig_l, samples_l, sizeof(int32_t) * nb_samples);
        if (!(s->flags & WV_MONO_DATA)) {
            av_em_fast_padded_malloc(&s->orig_r, &s->orig_r_size, sizeof(int32_t) * nb_samples);
...
static int wavpack_encode_frame(AVEMCodecContext *avctx, AVEMPacket *avpkt,
                                const AVFrame *frame, int *got_packet_ptr)
{
    ...
    av_em_fast_padded_malloc(&s->samples[0], &s->samples_size[0],
                          sizeof(int32_t) * s->block_samples);
    if (!s->samples[0])
        return AVERROR(ENOMEM);
    if (avctx->channels > 1) {
        av_em_fast_padded_malloc(&s->samples[1], &s->samples_size[1],
                              sizeof(int32_t) * s->block_samples);
        if (!s->samples[1])
            return AVERROR(ENOMEM);
    }
    ...
    if ((ret = em_alloc_packet2(avctx, avpkt, buf_size, 0)) < 0)
        return ret;
    buf = avpkt->data;
...
static av_cold int wavpack_encode_close(AVEMCodecContext *avctx)
{
    WavPackEncodeContext *s = avctx->priv_data;
    int i;

    for (i = 0; i < MAX_TERMS + 2; i++) {
        av_em_freep(&s->sampleptrs[i][0]);
        av_em_freep(&s->sampleptrs[i][1]);
        s->sampleptrs_size[i][0] = s->sampleptrs_size[i][1] = 0;
    }

    for (i = 0; i < 2; i++) {
        av_em_freep(&s->samples[i]);
        s->samples_size[i] = 0;

        av_em_freep(&s->best_buffer[i]);
        s->best_buffer_size[i] = 0;

        av_em_freep(&s->temp_buffer[i][0]);
        av_em_freep(&s->temp_buffer[i][1]);
        s->temp_buffer_size[i][0] = s->temp_buffer_size[i][1] = 0;
    }

    av_em_freep(&s->js_left);
    av_em_freep(&s->js_right);
    s->js_left_size = s->js_right_size = 0;

    av_em_freep(&s->orig_l);
    av_em_freep(&s->orig_r);
...
static const AVEMClass wavpack_encoder_class = {
