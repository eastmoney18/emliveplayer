...
static av_cold int tgq_decode_init(AVEMCodecContext *avctx)
{
...
static int tgq_decode_mb(TgqContext *s, AVFrame *frame, int mb_y, int mb_x)
{
    ...
    if (mode > 12) {
        ...
        if (mode == 3) {
            ...
            av_em_log(s->avctx, AV_LOG_ERROR, "unsupported mb mode %i\n", mode);
            return -1;
        }
...
static int tgq_decode_frame(AVEMCodecContext *avctx,
