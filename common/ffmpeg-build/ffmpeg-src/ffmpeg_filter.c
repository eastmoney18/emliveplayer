...
static const enum AVPixelFormat *get_compliance_unofficial_pix_fmts(enum AVEMCodecID codec_id, const enum AVPixelFormat default_formats[])
{
...
enum AVPixelFormat choose_pixel_fmt(AVEMStream *st, AVEMCodecContext *enc_ctx, AVEMCodec *codec, enum AVPixelFormat target)
{
...
{
    AVEMDictionaryEntry *strict_dict = av_dict_get(ost->encoder_opts, "strict", NULL, 0);
    if (strict_dict)
    ...
    if (ost->enc_ctx->pix_fmt != AV_PIX_FMT_NONE) {
        ...
        for (; *p != AV_PIX_FMT_NONE; p++) {
            ...
            avio_em_printf(s, "%s|", name);
        }
        len = avio_em_close_dyn_buf(s, &ret);
        ret[len - 1] = 0;
        return ret;
    } else
        return NULL;
...
static int configure_output_video_filter(FilterGraph *fg, OutputFilter *ofilter, AVFilterInOut *out)
{
    ...
    AVEMCodecContext *codec = ost->enc_ctx;
...
{                                                                  \
    ...
    if (avio_em_open_dyn_buf(&pb) < 0)                                \
        exit_program(1);                                           \
                                                                   \
    avio_em_printf(pb, "%s", ctx->filter->name);                      \
    if (nb_pads > 1)                                               \
        avio_em_printf(pb, ":%s", avfilter_pad_get_name(pads, inout->pad_idx));\
    avio_em_w8(pb, 0);                                                \
    avio_em_close_dyn_buf(pb, &f->name);                              \
}
...
{
    AVEMFormatContext *avf = input_files[ist->file_index]->ctx;
    int i, w, h;
                                        ...
                                        AVFilterInOut *in)
{
    ...
    AVEMRational tb = ist->framerate.num ? av_inv_q(ist->framerate) :
                                         ist->st->time_base;
    AVEMRational fr = ist->framerate;
    AVEMRational sar;
    AVEMBPrint args;
    char name[255];
    int ret, pad_idx = 0;
    ...
    if (!fr.num)
        fr = av_em_guess_frame_rate(input_files[ist->file_index]->ctx, ist->st, NULL);
