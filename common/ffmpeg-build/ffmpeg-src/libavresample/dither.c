                           ...
                           int channels, int nb_samples)
{
    ...
    for (ch = 0; ch < channels; ch++) {
        ...
        if (c->method == av_em_resample_DITHER_TRIANGULAR_NS) {
...
void ff_dither_free(DitherContext **cp)
{
    ...
    for (ch = 0; ch < c->channels; ch++)
        av_em_free(c->state[ch].noise_buf);
    av_em_free(c->state);
    av_em_freep(cp);
}
                                ...
                                enum AVResampleDitherMethod method)
{
    ...
    if (method == av_em_resample_DITHER_RECTANGULAR)
                               ...
                               int channels, int sample_rate, int apply_map)
{
    ...
    if (av_em_get_packed_sample_fmt(out_fmt) != AV_SAMPLE_FMT_S16 ||
        av_em_get_bytes_per_sample(in_fmt) <= 2) {
        av_em_log(avr, AV_LOG_ERROR, "dithering %s to %s is not supported\n",
    ...
    if (avr->dither_method == av_em_resample_DITHER_TRIANGULAR_NS &&
        sample_rate != 48000 && sample_rate != 44100) {
        av_em_log(avr, AV_LOG_WARNING, "sample rate must be 48000 or 44100 Hz "
               "for triangular_ns dither. using triangular_hp instead.\n");
        avr->dither_method = av_em_resample_DITHER_TRIANGULAR_HP;
    }
    ...
    c->state = av_em_mallocz(channels * sizeof(*c->state));
    if (!c->state)
        goto fail;
    c->channels = channels;
    ...
    av_em_lfg_init(&seed_gen, 0xC0FFEE);
    for (ch = 0; ch < channels; ch++) {
        DitherState *state = &c->state[ch];
