...
int avresample_open(AVAudioResampleContext *avr)
{
    ...
    if (avresample_is_open(avr)) {
        av_em_log(avr, AV_LOG_ERROR, "The resampling context is already open.\n");
        return AVERROR(EINVAL);
    }

    /* set channel mixing parameters */
    avr->in_channels = av_em_get_channel_layout_nb_channels(avr->in_channel_layout);
    if (avr->in_channels <= 0 || avr->in_channels > AVRESAMPLE_MAX_CHANNELS) {
        av_em_log(avr, AV_LOG_ERROR, "Invalid input channel layout: %"PRIu64"\n",
               avr->in_channel_layout);
        return AVERROR(EINVAL);
    }
    avr->out_channels = av_em_get_channel_layout_nb_channels(avr->out_channel_layout);
    if (avr->out_channels <= 0 || avr->out_channels > AVRESAMPLE_MAX_CHANNELS) {
        av_em_log(avr, AV_LOG_ERROR, "Invalid output channel layout: %"PRIu64"\n",
    ...
    if (avr->internal_sample_fmt == AV_SAMPLE_FMT_NONE &&
        ...
        enum AVSampleFormat  in_fmt = av_em_get_planar_sample_fmt(avr->in_sample_fmt);
        enum AVSampleFormat out_fmt = av_em_get_planar_sample_fmt(avr->out_sample_fmt);
        int max_bps = FFMAX(av_em_get_bytes_per_sample(in_fmt),
                            av_em_get_bytes_per_sample(out_fmt));
        if (max_bps <= 2) {
        ...
        av_em_log(avr, AV_LOG_DEBUG, "Using %s as internal sample format\n",
               av_em_get_sample_fmt_name(avr->internal_sample_fmt));
    }
    ...
    if (avr->use_channel_map && !avr->mixing_needed && !avr->resample_needed &&
        ...
        avr->internal_sample_fmt = av_em_get_planar_sample_fmt(avr->out_sample_fmt);
    }
    ...
    if (avr->use_channel_map) {
        if (avr->in_copy_needed) {
            ...
            av_em_log(avr, AV_LOG_TRACE, "remap channels during in_copy\n");
        } else if (avr->in_convert_needed) {
            avr->remap_point = REMAP_IN_CONVERT;
            av_em_log(avr, AV_LOG_TRACE, "remap channels during in_convert\n");
        } else if (avr->out_convert_needed) {
            avr->remap_point = REMAP_OUT_CONVERT;
            av_em_log(avr, AV_LOG_TRACE, "remap channels during out_convert\n");
        } else {
            avr->remap_point = REMAP_OUT_COPY;
            av_em_log(avr, AV_LOG_TRACE, "remap channels during out_copy\n");
        }

#ifdef DEBUG
        {
            int ch;
            av_em_log(avr, AV_LOG_TRACE, "output map: ");
            if (avr->ch_map_info.do_remap)
                for (ch = 0; ch < avr->in_channels; ch++)
                    av_em_log(avr, AV_LOG_TRACE, " % 2d", avr->ch_map_info.channel_map[ch]);
            else
                av_em_log(avr, AV_LOG_TRACE, "n/a");
            av_em_log(avr, AV_LOG_TRACE, "\n");
            av_em_log(avr, AV_LOG_TRACE, "copy map:   ");
            if (avr->ch_map_info.do_copy)
                for (ch = 0; ch < avr->in_channels; ch++)
                    av_em_log(avr, AV_LOG_TRACE, " % 2d", avr->ch_map_info.channel_copy[ch]);
            else
                av_em_log(avr, AV_LOG_TRACE, "n/a");
            av_em_log(avr, AV_LOG_TRACE, "\n");
            av_em_log(avr, AV_LOG_TRACE, "zero map:   ");
            if (avr->ch_map_info.do_zero)
                for (ch = 0; ch < avr->in_channels; ch++)
                    av_em_log(avr, AV_LOG_TRACE, " % 2d", avr->ch_map_info.channel_zero[ch]);
            else
                av_em_log(avr, AV_LOG_TRACE, "n/a");
            av_em_log(avr, AV_LOG_TRACE, "\n");
            av_em_log(avr, AV_LOG_TRACE, "input map:  ");
            for (ch = 0; ch < avr->in_channels; ch++)
                av_em_log(avr, AV_LOG_TRACE, " % 2d", avr->ch_map_info.input_map[ch]);
            av_em_log(avr, AV_LOG_TRACE, "\n");
        }
#endif
    } else
    ...
    avr->out_fifo = av_em_audio_fifo_alloc(avr->out_sample_fmt, avr->out_channels,
                                        1024);
    if (!avr->out_fifo) {
        ret = AVERROR(ENOMEM);
        goto error;
    ...
    if (avr->resample_needed) {
        avr->resample = em_audio_resample_init(avr);
        if (!avr->resample) {
            ret = AVERROR(ENOMEM);
            goto error;
        }
...
void avresample_close(AVAudioResampleContext *avr)
{
    ...
    av_em_audio_fifo_free(avr->out_fifo);
    avr->out_fifo = NULL;
    ff_audio_convert_free(&avr->ac_in);
    ff_audio_convert_free(&avr->ac_out);
    em_audio_resample_free(&avr->resample);
    ff_audio_mix_free(&avr->am);
    av_em_freep(&avr->mix_matrix);
                                  ...
                                  AudioData *output, AudioData *converted)
{
    ...
    if (!output || av_em_audio_fifo_size(avr->out_fifo) > 0 ||
        ...
        if (converted) {
            ...
            av_em_log(avr, AV_LOG_TRACE, "[FIFO] add %s to out_fifo\n", converted->name);
        ...
        if (output && output->allocated_samples > 0) {
            av_em_log(avr, AV_LOG_TRACE, "[FIFO] read from out_fifo to output\n");
            av_em_log(avr, AV_LOG_TRACE, "[end conversion]\n");
        ...
        av_em_log(avr, AV_LOG_TRACE, "[copy] %s to output\n", converted->name);
        output->nb_samples = 0;
                                           ...
                                           int in_plane_size, int in_samples)
{
    ...
    av_em_log(avr, AV_LOG_TRACE, "[start conversion]\n");

    /* initialize output_buffer with output data */
    direct_output = output && av_em_audio_fifo_size(avr->out_fifo) == 0;
    if (output) {
    ...
    if (input) {
        ...
        if (avr->upmix_needed && !avr->in_convert_needed && !avr->resample_needed &&
            ...
            av_em_log(avr, AV_LOG_TRACE, "[copy] %s to out_buffer\n", current_buffer->name);
            ...
            if (avr->in_convert_needed) {
                ...
                if (ret < 0)
                    return ret;
                av_em_log(avr, AV_LOG_TRACE, "[convert] %s to in_buffer\n", current_buffer->name);
                ret = ff_audio_convert(avr->ac_in, avr->in_buffer,
                                       current_buffer);
                if (ret < 0)
                    return ret;
            } else {
                av_em_log(avr, AV_LOG_TRACE, "[copy] %s to in_buffer\n", current_buffer->name);
            ...
            if (avr->downmix_needed) {
                av_em_log(avr, AV_LOG_TRACE, "[downmix] in_buffer\n");
    ...
    if (avr->resample_needed) {
        ...
        av_em_log(avr, AV_LOG_TRACE, "[resample] %s to %s\n",
                current_buffer ? current_buffer->name : "null",
                resample_out->name);
        ret = em_audio_resample(avr->resample, resample_out,
                                current_buffer);
        if (ret < 0)
            return ret;
    ...
    if (avr->upmix_needed) {
        av_em_log(avr, AV_LOG_TRACE, "[upmix] %s\n", current_buffer->name);
    ...
    if (avr->out_convert_needed) {
        if (direct_output && out_samples >= current_buffer->nb_samples) {
            ...
            av_em_log(avr, AV_LOG_TRACE, "[convert] %s to output\n", current_buffer->name);
            ...
            if (ret < 0)
                return ret;
            av_em_log(avr, AV_LOG_TRACE, "[convert] %s to out_buffer\n", current_buffer->name);
...
{
    int samples;
    int bytes_per_sample = av_em_get_bytes_per_sample(out->format);
    if (!bytes_per_sample)
        return AVERROR(EINVAL);

    samples = out->linesize[0] / bytes_per_sample;
    if (av_em_sample_fmt_is_planar(out->format)) {
        return samples;
    } else {
        int channels = av_em_get_channel_layout_nb_channels(out->channel_layout);
                             ...
                             AVFrame *out, AVFrame *in)
{
    ...
    if (out) {
        if (!out->linesize[0]) {
            ...
            if ((ret = av_em_frame_get_buffer(out, 0)) < 0) {
                if (setup)
                    avresample_close(avr);
                return ret;
            }
                          ...
                          int stride)
{
    ...
    in_channels  = av_em_get_channel_layout_nb_channels(avr->in_channel_layout);
    out_channels = av_em_get_channel_layout_nb_channels(avr->out_channel_layout);

    if ( in_channels <= 0 ||  in_channels > AVRESAMPLE_MAX_CHANNELS ||
        out_channels <= 0 || out_channels > AVRESAMPLE_MAX_CHANNELS) {
        av_em_log(avr, AV_LOG_ERROR, "Invalid channel layouts\n");
        return AVERROR(EINVAL);
    }

    if (!avr->mix_matrix) {
        av_em_log(avr, AV_LOG_ERROR, "matrix is not set\n");
        return AVERROR(EINVAL);
    }
                          ...
                          int stride)
{
    ...
    if (avr->mix_matrix)
        av_em_freep(&avr->mix_matrix);
    avr->mix_matrix = av_em_alloc(in_channels * out_channels *
                                   ...
                                   const int *channel_map)
{
    ...
    in_channels = av_em_get_channel_layout_nb_channels(avr->in_channel_layout);
    if (in_channels <= 0 ||  in_channels > AVRESAMPLE_MAX_CHANNELS) {
        av_em_log(avr, AV_LOG_ERROR, "Invalid input channel layout\n");
        return AVERROR(EINVAL);
    }
    ...
    for (ch = 0; ch < in_channels; ch++) {
        if (channel_map[ch] >= in_channels) {
            av_em_log(avr, AV_LOG_ERROR, "Invalid channel map\n");
            return AVERROR(EINVAL);
        }
        if (channel_map[ch] < 0) {
...
{
    return av_em_audio_fifo_size(avr->out_fifo);
}
...
int avresample_get_out_samples(AVAudioResampleContext *avr, int in_nb_samples)
{
    ...
    if (avr->resample_needed) {
        samples = av_em_rescale_rnd(samples,
                                 avr->out_sample_rate,
                                 avr->in_sample_rate,
                                 AV_ROUND_UP);
    }
...
{
    if (!output)
        return av_em_audio_fifo_drain(avr->out_fifo, nb_samples);
    return av_em_audio_fifo_read(avr->out_fifo, (void**)output, nb_samples);
}
