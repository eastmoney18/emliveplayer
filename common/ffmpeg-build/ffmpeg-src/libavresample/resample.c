...
static int build_filter(ResampleContext *c, double factor)
{
    ...
    tab = av_em_alloc(tap_count * sizeof(*tab));
    if (!tab)
    ...
    for (ph = 0; ph < phase_count; ph++) {
        ...
        for (i = 0; i < tap_count; i++) {
            ...
            case av_em_resample_FILTER_TYPE_CUBIC: {
            ...
            case av_em_resample_FILTER_TYPE_BLACKMAN_NUTTALL:
                w  = 2.0 * x / (factor * tap_count) + M_PI;
                y *= 0.3635819 - 0.4891775 * cos(    w) +
                                 0.1365995 * cos(2 * w) -
                                 0.0106411 * cos(3 * w);
                break;
            case av_em_resample_FILTER_TYPE_KAISER:
                w  = 2.0 * x / (factor * tap_count * M_PI);
...
ResampleContext *em_audio_resample_init(AVAudioResampleContext *avr)
{
    ResampleContext *c;
    ...
    if (avr->internal_sample_fmt != AV_SAMPLE_FMT_S16P &&
        ...
        av_em_log(avr, AV_LOG_ERROR, "Unsupported internal format for "
               "resampling: %s\n",
               av_em_get_sample_fmt_name(avr->internal_sample_fmt));
        return NULL;
    }
    c = av_em_mallocz(sizeof(*c));
    if (!c)
        return NULL;
    ...
    if (ARCH_AARCH64)
        em_audio_resample_init_aarch64(c, avr->internal_sample_fmt);
    if (ARCH_ARM)
        em_audio_resample_init_arm(c, avr->internal_sample_fmt);

    felem_size = av_em_get_bytes_per_sample(avr->internal_sample_fmt);
    c->filter_bank = av_em_mallocz(c->filter_length * (phase_count + 1) * felem_size);
    if (!c->filter_bank)
        goto error;
    ...
    if (!av_em_reduce(&c->src_incr, &c->dst_incr, out_rate,
    ...
    av_em_log(avr, AV_LOG_DEBUG, "resample: %s from %d Hz to %d Hz\n",
           av_em_get_sample_fmt_name(avr->internal_sample_fmt),
...
error:
    ...
    av_em_free(c->filter_bank);
    av_em_free(c);
    return NULL;
}

void em_audio_resample_free(ResampleContext **c)
{
    if (!*c)
        return;
    ff_audio_data_free(&(*c)->buffer);
                                ...
                                int compensation_distance)
{
    ...
    if (!avr->resample_needed) {
        av_em_log(avr, AV_LOG_ERROR, "Unable to set resampling compensation\n");
        return AVERROR(EINVAL);
    }
    c = avr->resample;
...
int em_audio_resample(ResampleContext *c, AudioData *dst, AudioData *src)
{
    ...
    if (!c->initial_padding_filled) {
        int bps = av_em_get_bytes_per_sample(c->avr->internal_sample_fmt);
        int i;
    ...
    if (!src && !c->final_padding_filled) {
        ...
        ret = ff_audio_data_realloc(c->buffer,
                                    ...
                                    c->padding_size);
        if (ret < 0) {
            av_em_log(c->avr, AV_LOG_ERROR, "Error reallocating resampling buffer\n");
            return AVERROR(ENOMEM);
        }
    ...
    if (!dst->read_only && dst->allow_realloc) {
        ...
        if (ret < 0) {
            av_em_log(c->avr, AV_LOG_ERROR, "error reallocating output\n");
            return ret;
        }
    }
    ...
    if (out_samples < 0) {
        av_em_log(c->avr, AV_LOG_ERROR, "error during resampling\n");
        return out_samples;
    }
    ...
    av_em_log(c->avr, AV_LOG_TRACE, "resampled %d in + %d leftover to %d out + %d leftover\n",
