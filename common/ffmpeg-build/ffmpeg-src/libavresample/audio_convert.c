                               ...
                               const char *descr, void *conv)
{
    ...
    case CONV_FUNC_TYPE_FLAT:
        if (av_em_get_packed_sample_fmt(ac->in_fmt)  == in_fmt &&
            av_em_get_packed_sample_fmt(ac->out_fmt) == out_fmt) {
            ac->conv_flat     = conv;
            ac->func_descr    = descr;
    ...
    if (found) {
        av_em_log(ac->avr, AV_LOG_DEBUG, "audio_convert: found function: %-4s "
               "to %-4s (%s)\n", av_em_get_sample_fmt_name(ac->in_fmt),
               av_em_get_sample_fmt_name(ac->out_fmt), descr);
    }
}
                                     ...
                                     int apply_map)
{
    ...
    if (avr->dither_method != av_em_resample_DITHER_NONE          &&
        av_em_get_packed_sample_fmt(out_fmt) == AV_SAMPLE_FMT_S16 &&
...
int ff_audio_convert(AudioConvert *ac, AudioData *out, AudioData *in)
{
    ...
    if (ac->dc) {
        /* dithered conversion */
        av_em_log(ac->avr, AV_LOG_TRACE, "%d samples - audio_convert: %s to %s (dithered)\n",
                len, av_em_get_sample_fmt_name(ac->in_fmt),
                av_em_get_sample_fmt_name(ac->out_fmt));
    ...
    av_em_log(ac->avr, AV_LOG_TRACE, "%d samples - audio_convert: %s to %s (%s)\n", len,
            av_em_get_sample_fmt_name(ac->in_fmt),
            av_em_get_sample_fmt_name(ac->out_fmt),
    ...
    if (ac->apply_map) {
        ...
        if (!ff_sample_fmt_is_planar(ac->out_fmt, ac->channels)) {
            av_em_log(ac->avr, AV_LOG_ERROR, "cannot remap packed format during conversion\n");
        ...
        if (map->do_copy || map->do_zero) {
            for (p = 0; p < ac->planes; p++) {
                if (map->channel_copy[p])
                    ...
                    av_em_samples_set_silence(&out->data[p], 0, len, 1, ac->out_fmt);
            }
        }
    } else {
        switch (ac->func_type) {
