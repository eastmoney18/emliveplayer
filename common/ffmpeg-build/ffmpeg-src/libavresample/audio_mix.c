                           ...
                           const char *descr, void *mix_func)
{
    if (fmt == am->fmt && coeff_type == am->coeff_type &&
        ...
        av_em_log(am->avr, AV_LOG_DEBUG, "audio_mix: found function: [fmt=%s] "
               "[c=%s] %s(%s)\n", av_em_get_sample_fmt_name(fmt),
...
static av_cold int mix_function_init(AudioMix *am)
{
    ...
    if (!am->mix) {
        av_em_log(am->avr, AV_LOG_ERROR, "audio_mix: NO FUNCTION FOUND: [fmt=%s] "
               "[c=%s] [%d to %d]\n", av_em_get_sample_fmt_name(am->fmt),
...
AudioMix *ff_audio_mix_alloc(AVAudioResampleContext *avr)
{
    ...
    if (avr->mix_matrix) {
        ...
        double *matrix_dbl = av_em_mallocz(avr->out_channels * avr->in_channels *
                                        sizeof(*matrix_dbl));
        if (!matrix_dbl)
            goto error;
        ...
        ret = avresample_build_matrix(avr->in_channel_layout,
                                      ...
                                      avr->matrix_encoding);
        if (ret < 0) {
            av_em_free(matrix_dbl);
            goto error;
        }

        ret = ff_audio_mix_set_matrix(am, matrix_dbl, avr->in_channels);
        if (ret < 0) {
            av_em_log(avr, AV_LOG_ERROR, "error setting mix matrix\n");
            av_em_free(matrix_dbl);
            goto error;
        }
...
int ff_audio_mix(AudioMix *am, AudioData *src)
{
    ...
    av_em_log(am->avr, AV_LOG_TRACE, "audio_mix: %d samples - %d to %d channels (%s)\n",
    ...
    if (am->out_matrix_channels < am->out_channels) {
        for (i = 0; i < am->out_channels; i++)
            if (am->output_zero[i])
                av_em_samples_set_silence(&src->data[i], 0, len, 1, am->fmt);
    }
...
int ff_audio_mix_get_matrix(AudioMix *am, double *matrix, int stride)
{
    ...
    if ( am->in_channels <= 0 ||  am->in_channels > AVRESAMPLE_MAX_CHANNELS ||
        ...
        av_em_log(am->avr, AV_LOG_ERROR, "Invalid channel counts\n");
        return AVERROR(EINVAL);
    }

#define GET_MATRIX_CONVERT(suffix, scale)                                   \
    if (!am->matrix_ ## suffix[0]) {                                        \
        av_em_log(am->avr, AV_LOG_ERROR, "matrix is not set\n");               \
        return AVERROR(EINVAL);                                             \
    }                                                                       \
    ...
    case AV_MIX_COEFF_TYPE_FLT:
        ...
        break;
    default:
        av_em_log(am->avr, AV_LOG_ERROR, "Invalid mix coeff type\n");
        return AVERROR(EINVAL);
    }
...
#define CONVERT_MATRIX(type, expr)                                          \
    am->matrix_## type[0] = av_em_mallocz(am->out_matrix_channels *            \
                                       am->in_matrix_channels  *            \
    ...
    av_em_get_channel_layout_string(in_layout_name, sizeof(in_layout_name),
                                 am->in_channels, am->in_layout);
    av_em_get_channel_layout_string(out_layout_name, sizeof(out_layout_name),
                                 am->out_channels, am->out_layout);
    av_em_log(am->avr, AV_LOG_DEBUG, "audio_mix: %s to %s\n",
           in_layout_name, out_layout_name);
    av_em_log(am->avr, AV_LOG_DEBUG, "matrix size: %d x %d\n",
           am->in_matrix_channels, am->out_matrix_channels);
    for (o = 0; o < am->out_channels; o++) {
        for (i = 0; i < am->in_channels; i++) {
            if (am->output_zero[o])
                av_em_log(am->avr, AV_LOG_DEBUG, "  (ZERO)");
            else if (am->input_skip[i] || am->output_zero[i] || am->output_skip[o])
                av_em_log(am->avr, AV_LOG_DEBUG, "  (SKIP)");
            else
                av_em_log(am->avr, AV_LOG_DEBUG, "  %0.3f ",
                       matrix[o * am->in_channels + i]);
        }
        av_em_log(am->avr, AV_LOG_DEBUG, "\n");
    }
