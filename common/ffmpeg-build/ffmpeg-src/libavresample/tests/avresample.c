...
{                                                                           \
    type v = expr;                                                          \
    type **out = (type **)data;                                             \
    if (av_em_sample_fmt_is_planar(sample_fmt))                                \
        out[ch][sample] = v;                                                \
    else                                                                    \
...
{
    switch (av_em_get_packed_sample_fmt(sample_fmt)) {
...
int main(int argc, char **argv)
{
    ...
    num_layouts = 2;
    if (argc > 1) {
        if (!av_em_strncasecmp(argv[1], "-h", 3)) {
            av_em_log(NULL, AV_LOG_INFO, "Usage: avresample-test [<num formats> "
    ...
    av_em_log_set_level(AV_LOG_DEBUG);

    av_em_lfg_init(&rnd, 0xC0FFEE);

    in_buf_size = av_em_samples_get_buffer_size(&in_linesize, 8, 48000 * 6,
                                             AV_SAMPLE_FMT_DBLP, 0);
    out_buf_size = in_buf_size;

    in_buf = av_em_alloc(in_buf_size);
    if (!in_buf)
        goto end;
    out_buf = av_em_alloc(out_buf_size);
    if (!out_buf)
        goto end;

    s = avresample_alloc_context();
    if (!s) {
        av_em_log(NULL, AV_LOG_ERROR, "Error allocating AVAudioResampleContext\n");
        ret = 1;
        goto end;
    }
    ...
    for (i = 0; i < num_formats; i++) {
        ...
        for (k = 0; k < num_layouts; k++) {
            ...
            in_channels  = av_em_get_channel_layout_nb_channels(in_ch_layout);
            for (m = 0; m < num_rates; m++) {
                in_rate = rates[m];

                ret = av_em_samples_fill_arrays(in_data, &in_linesize, in_buf,
                                             in_channels, in_rate * 6,
                                             in_fmt, 0);
                if (ret < 0) {
                    av_em_log(s, AV_LOG_ERROR, "failed in_data fill arrays\n");
                    goto end;
                }
                ...
                for (j = 0; j < num_formats; j++) {
                    ...
                    for (l = 0; l < num_layouts; l++) {
                        ...
                        out_channels  = av_em_get_channel_layout_nb_channels(out_ch_layout);
                        for (n = 0; n < num_rates; n++) {
                            out_rate = rates[n];

                            av_em_log(NULL, AV_LOG_INFO, "%s to %s, %d to %d channels, %d Hz to %d Hz\n",
                                   av_em_get_sample_fmt_name(in_fmt), av_em_get_sample_fmt_name(out_fmt),
                                   in_channels, out_channels, in_rate, out_rate);

                            ret = av_em_samples_fill_arrays(out_data, &out_linesize,
                                                         out_buf, out_channels,
                                                         out_rate * 6, out_fmt, 0);
                            if (ret < 0) {
                                av_em_log(s, AV_LOG_ERROR, "failed out_data fill arrays\n");
                                goto end;
                            }

                            av_em_opt_set_int(s, "in_channel_layout",  in_ch_layout,  0);
                            av_em_opt_set_int(s, "in_sample_fmt",      in_fmt,        0);
                            av_em_opt_set_int(s, "in_sample_rate",     in_rate,       0);
                            av_em_opt_set_int(s, "out_channel_layout", out_ch_layout, 0);
                            av_em_opt_set_int(s, "out_sample_fmt",     out_fmt,       0);
                            av_em_opt_set_int(s, "out_sample_rate",    out_rate,      0);

                            av_em_opt_set_int(s, "internal_sample_fmt", AV_SAMPLE_FMT_FLTP, 0);

                            ret = avresample_open(s);
                            if (ret < 0) {
                                av_em_log(s, AV_LOG_ERROR, "Error opening context\n");
                                goto end;
                            }
                            ...
                            if (ret < 0) {
                                ...
                                av_em_log(NULL, AV_LOG_ERROR, "%s\n", errbuf);
                                goto end;
                            }
                            av_em_log(NULL, AV_LOG_INFO, "Converted %d samples to %d samples\n",
                                   in_rate * 6, ret);
                            if (avresample_get_delay(s) > 0)
                                av_em_log(NULL, AV_LOG_INFO, "%d delay samples not converted\n",
                                       avresample_get_delay(s));
                            if (avresample_available(s) > 0)
                                av_em_log(NULL, AV_LOG_INFO, "%d samples available for output\n",
