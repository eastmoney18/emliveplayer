...
static const AVEMClass audio_data_class = {
...
int ff_sample_fmt_is_planar(enum AVSampleFormat sample_fmt, int channels)
{
    if (channels == 1)
        ...
        return av_em_sample_fmt_is_planar(sample_fmt);
}
                       ...
                       const char *name)
{
    ...
    if (channels < 1 || channels > AVRESAMPLE_MAX_CHANNELS) {
        av_em_log(a, AV_LOG_ERROR, "invalid channel count: %d\n", channels);
        return AVERROR(EINVAL);
    }

    a->sample_size = av_em_get_bytes_per_sample(sample_fmt);
    if (!a->sample_size) {
        av_em_log(a, AV_LOG_ERROR, "invalid sample format\n");
        return AVERROR(EINVAL);
    }
    ...
    for (p = 0; p < (a->is_planar ? channels : 1); p++) {
        if (!src[p]) {
            av_em_log(a, AV_LOG_ERROR, "invalid NULL pointer for src[%d]\n", p);
            return AVERROR(EINVAL);
        }
        a->data[p] = src[p];
    }
...
int ff_audio_data_realloc(AudioData *a, int nb_samples)
{
    ...
    new_buf_size = av_em_samples_get_buffer_size(&plane_size,
    ...
    if (a->nb_samples > 0 && a->is_planar) {
        ...
        ret = av_em_samples_alloc(new_data, &plane_size, a->allocated_channels,
        ...
        av_em_freep(&a->buffer);
        memcpy(a->data, new_data, sizeof(new_data));
        a->buffer = a->data[0];
    } else {
        av_em_freep(&a->buffer);
        a->buffer = av_em_alloc(new_buf_size);
        if (!a->buffer)
            return AVERROR(ENOMEM);
        ret = av_em_samples_fill_arrays(a->data, &plane_size, a->buffer,
                                     a->allocated_channels, nb_samples,
                                     a->sample_fmt, 0);
        if (ret < 0)
            return ret;
...
int ff_audio_data_copy(AudioData *dst, AudioData *src, ChannelMapInfo *map)
{
    ...
    if (map && !src->is_planar) {
        av_em_log(src, AV_LOG_ERROR, "cannot remap packed format during copy\n");
        return AVERROR(EINVAL);
    }
        ...
        if (map->do_copy || map->do_zero) {
            for (p = 0; p < src->planes; p++) {
                if (map->channel_copy[p])
                    ...
                    av_em_samples_set_silence(&dst->data[p], 0, src->nb_samples,
                                           1, dst->sample_fmt);
            }
        }
    } else {
                          ...
                          int src_offset, int nb_samples)
{
    ...
    /* validate input/output compatibility */
    if (dst->sample_fmt != src->sample_fmt || dst->channels != src->channels) {
        av_em_log(src, AV_LOG_ERROR, "sample format mismatch\n");
        return AVERROR(EINVAL);
    }
    ...
    if (dst_offset < 0 || dst_offset > dst->nb_samples ||
        ...
        av_em_log(src, AV_LOG_ERROR, "offset out-of-bounds: src=%d dst=%d\n",
    ...
    if (dst->read_only) {
        av_em_log(dst, AV_LOG_ERROR, "dst is read-only\n");
        return AVERROR(EINVAL);
    }
    ...
    if (ret < 0) {
        av_em_log(dst, AV_LOG_ERROR, "error reallocating dst\n");
        return ret;
    }
                              ...
                              int nb_samples)
{
    ...
    return av_em_audio_fifo_write(af, (void **)offset_data, nb_samples);
}
...
int ff_audio_data_read_from_fifo(AVAudioFifo *af, AudioData *a, int nb_samples)
{
    ...
    ret = av_em_audio_fifo_read(af, (void **)a->data, nb_samples);
    if (ret >= 0)
        a->nb_samples = ret;
    return ret;
}
