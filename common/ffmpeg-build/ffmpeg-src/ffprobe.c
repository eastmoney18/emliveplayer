...
typedef struct InputStream {
    ...
    AVEMCodecContext *dec_ctx;
} InputStream;

typedef struct InputFile {
    AVEMFormatContext *fmt_ctx;
...
#define SECTION_FLAG_HAS_VARIABLE_FIELDS 4 ///< the section may contain a variable number of fields with variable keys.
    ...
    AVEMDictionary *entries_to_show;
...
static AVEMInputFormat *iformat = NULL;
...
typedef struct Writer {
    const AVEMClass *priv_class;      ///< private class of the writer, if any
    ...
    void (*print_rational)      (WriterContext *wctx, AVEMRational *q, char *sep);
...
struct WriterContext {
    const AVEMClass *class;           ///< class of the writer
    ...
    AVEMBPrint section_pbuf[SECTION_MAX_NB_LEVELS]; ///< generic print buffer dedicated to each section,
...
static const AVEMClass writer_class = {
    .class_name = "Writer",
...
static void bprint_bytes(AVEMBPrint *bp, const uint8_t *ubuf, size_t ubuf_size)
{
    int i;
    av_bprintf(bp, "0X");
                       ...
                       const struct section *sections, int nb_sections)
{
    ...
    if (writer->priv_class) {
        ...
        *((const AVEMClass **)priv_ctx) = writer->priv_class;
...
static inline void writer_print_rational(WriterContext *wctx,
                                         const char *key, AVEMRational q, char sep)
{
    AVEMBPrint buf;
...
static void writer_print_time(WriterContext *wctx, const char *key,
                              int64_t ts, const AVEMRational *time_base, int is_duration)
{
    char buf[128];
...
static const AVEMClass name##_class = {               \
    .class_name = #name,                            \
 ...
 */
static const char *c_escape_str(AVEMBPrint *dst, const char *src, const char sep, void *log_ctx)
{
    const char *p;
 ...
 */
static const char *csv_escape_str(AVEMBPrint *dst, const char *src, const char sep, void *log_ctx)
{
    char meta_chars[] = { sep, '"', '\n', '\r', '\0' };
...
static const char *none_escape_str(AVEMBPrint *dst, const char *src, const char sep, void *log_ctx)
{
    return src;
}
...
typedef struct CompactContext {
    ...
    const char * (*escape_str)(AVEMBPrint *dst, const char *src, const char sep, void *log_ctx);
    int nested_section[SECTION_MAX_NB_LEVELS];
...
static const char *flat_escape_key_str(AVEMBPrint *dst, const char *src, const char sep)
{
    const char *p;
...
static const char *flat_escape_value_str(AVEMBPrint *dst, const char *src)
{
    const char *p;
...
{
    FlatContext *flat = wctx->priv;
    AVEMBPrint *buf = &wctx->section_pbuf[wctx->level];
    const struct section *section = wctx->section[wctx->level];
    const struct section *parent_section = wctx->level ?
        wctx->section[wctx->level-1] : NULL;
...
static char *ini_escape_str(AVEMBPrint *dst, const char *src)
{
    int i = 0;
    char c = 0;
...
static const char *json_escape_str(AVEMBPrint *dst, const char *src, void *log_ctx)
{
...
static const char *xml_escape_str(AVEMBPrint *dst, const char *src, void *log_ctx)
{
    const char *p;
...
static inline int show_tags(WriterContext *w, AVEMDictionary *tags, int section_id)
{
...
static void print_pkt_side_data(WriterContext *w,
                                const AVEMPacketSideData *side_data,
                                int nb_side_data,
                                ...
                                SectionID id_data)
{
    ...
    for (i = 0; i < nb_side_data; i++) {
        const AVEMPacketSideData *sd = &side_data[i];
...
static void show_packet(WriterContext *w, InputFile *ifile, AVEMPacket *pkt, int packet_idx)
{
    char val_str[128];
    AVEMStream *st = ifile->streams[pkt->stream_index].st;
    AVEMBPrint pbuf;
    const char *s;
...
static void show_subtitle(WriterContext *w, AVSubtitle *sub, AVEMStream *stream,
                          AVEMFormatContext *fmt_ctx)
{
    AVEMBPrint pbuf;
...
static void show_frame(WriterContext *w, AVFrame *frame, AVEMStream *stream,
                       AVEMFormatContext *fmt_ctx)
{
    AVEMBPrint pbuf;
    char val_str[128];
    ...
    print_ts  ("best_effort_timestamp", av_em_frame_get_best_effort_timestamp(frame));
    print_time("best_effort_timestamp_time", av_em_frame_get_best_effort_timestamp(frame), &stream->time_base);
    print_duration_ts  ("pkt_duration",      av_em_frame_get_pkt_duration(frame));
    print_duration_time("pkt_duration_time", av_em_frame_get_pkt_duration(frame), &stream->time_base);
    if (av_em_frame_get_pkt_pos (frame) != -1) print_fmt    ("pkt_pos", "%"PRId64, av_em_frame_get_pkt_pos(frame));
    else                      print_str_opt("pkt_pos", "N/A");
    if (av_em_frame_get_pkt_size(frame) != -1) print_val    ("pkt_size", av_em_frame_get_pkt_size(frame), unit_byte_str);
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        sar = av_em_guess_sample_aspect_ratio(fmt_ctx, stream, frame);
        if (sar.num) {
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        print_int("channels", av_em_frame_get_channels(frame));
        if (av_em_frame_get_channel_layout(frame)) {
            av_bprint_clear(&pbuf);
            av_bprint_channel_layout(&pbuf, av_em_frame_get_channels(frame),
                                     av_em_frame_get_channel_layout(frame));
    ...
    if (do_show_frame_tags)
        show_tags(w, av_em_frame_get_metadata(frame), SECTION_ID_FRAME_TAGS);
...
static av_always_inline int process_frame(WriterContext *w,
                                          InputFile *ifile,
                                          AVFrame *frame, AVEMPacket *pkt)
{
    AVEMFormatContext *fmt_ctx = ifile->fmt_ctx;
    AVEMCodecContext *dec_ctx = ifile->streams[pkt->stream_index].dec_ctx;
    AVEMCodecParameters *par = ifile->streams[pkt->stream_index].st->codecpar;
    AVSubtitle sub;
                                 ...
                                 const ReadInterval *interval, int64_t *cur_ts)
{
    ...
    if (interval->has_start) {
        ...
        if ((ret = avformat_em_seek_file(fmt_ctx, -1, -INT64_MAX, target, INT64_MAX, 0)) < 0) {
    ...
    while (!av_em_read_frame(fmt_ctx, &pkt)) {
        ...
        if (selected_streams[pkt.stream_index]) {
            AVEMRational tb = ifile->streams[pkt.stream_index].st->time_base;
...
static int show_stream(WriterContext *w, AVEMFormatContext *fmt_ctx, int stream_idx, InputStream *ist, int in_program)
{
    AVEMStream *stream = ist->st;
    AVEMCodecParameters *par;
    AVEMCodecContext *dec_ctx;
    char val_str[128];
    const char *s;
    AVEMRational sar, dar;
    AVEMBPrint pbuf;
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        sar = av_em_guess_sample_aspect_ratio(fmt_ctx, stream, NULL);
        if (sar.den) {
            print_q("sample_aspect_ratio", sar, ':');
...
static int show_program(WriterContext *w, InputFile *ifile, AVEMProgram *program)
{
    AVEMFormatContext *fmt_ctx = ifile->fmt_ctx;
    int i, ret = 0;
...
static int show_programs(WriterContext *w, InputFile *ifile)
{
    ...
    for (i = 0; i < fmt_ctx->nb_programs; i++) {
        AVEMProgram *program = fmt_ctx->programs[i];
        if (!program)
            continue;
...
static int show_chapters(WriterContext *w, InputFile *ifile)
{
    ...
    for (i = 0; i < fmt_ctx->nb_chapters; i++) {
        AVEMChapter *chapter = fmt_ctx->chapters[i];
...
static int show_format(WriterContext *w, InputFile *ifile)
{
    AVEMFormatContext *fmt_ctx = ifile->fmt_ctx;
    char val_str[128];
    int64_t size = fmt_ctx->pb ? avio_em_size(fmt_ctx->pb) : -1;
    int ret = 0;
    ...
    print_int("probe_score", av_em_format_get_probe_score(fmt_ctx));
...
static int open_input_file(InputFile *ifile, const char *filename)
{
    ...
    if ((err = avformat_em_open_input(&fmt_ctx, filename,
    ...
    err = avformat_em_find_stream_info(fmt_ctx, opts);
    ...
    av_em_dump_format(fmt_ctx, 0, filename, 0);
    ...
    for (i = 0; i < fmt_ctx->nb_streams; i++) {
        ...
        AVEMStream *stream = fmt_ctx->streams[i];
        AVEMCodec *codec;
        ...
        {
            AVEMDictionary *opts = filter_codec_opts(codec_opts, stream->codecpar->codec_id,
...
static void close_input_file(InputFile *ifile)
{
    ...
    avformat_em_close_input(&ifile->fmt_ctx);
}
...
#define CHECK_END if (ret < 0) goto end
    ...
    for (i = 0; i < ifile.fmt_ctx->nb_streams; i++) {
        if (stream_specifier) {
            ret = avformat_em_match_stream_specifier(ifile.fmt_ctx,
...
{
    iformat = av_em_find_input_format(arg);
    if (!iformat) {
...
static inline void mark_section_show_entries(SectionID section_id,
                                             int show_all_entries, AVEMDictionary *entries)
{
...
void show_help_default(const char *opt, const char *arg)
{
    ...
    show_help_children(avformat_em_get_class(), AV_OPT_FLAG_DECODING_PARAM);
}
