...
static av_cold int dv_sdp_parse_fmtp_config(AVEMFormatContext *s,
                                            AVEMStream *stream,
...
static av_cold int dv_parse_sdp_line(AVEMFormatContext *ctx, int st_index,
                                     PayloadContext *dv_data, const char *line)
{
    ...
    if (av_em_strstart(sdp_line_ptr, "fmtp:", &sdp_line_ptr)) {
...
static int dv_handle_packet(AVEMFormatContext *ctx, PayloadContext *rtp_dv_ctx,
                            ...
                            int flags)
{
    ...
    if (rtp_dv_ctx->buf && rtp_dv_ctx->timestamp != *timestamp) {
        emio_free_dyn_buf(&rtp_dv_ctx->buf);
    }

    /* sanity check for size of input packet: 1 byte payload at least */
    if (len < 1) {
        av_em_log(ctx, AV_LOG_ERROR, "Too short RTP/DV packet, got %d bytes\n", len);
        return AVERROR_INVALIDDATA;
    }

    /* start frame buffering with new dynamic buffer */
    if (!rtp_dv_ctx->buf) {
        res = avio_em_open_dyn_buf(&rtp_dv_ctx->buf);
        if (res < 0)
            return res;
    ...
    avio_em_write(rtp_dv_ctx->buf, buf, len);
