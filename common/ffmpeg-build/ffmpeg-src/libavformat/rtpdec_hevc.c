...
static av_cold int hevc_sdp_parse_fmtp_config(AVEMFormatContext *s,
                                              AVEMStream *stream,
                                              ...
                                              const char *attr, const char *value)
{
    ...
    if (!strcmp(attr, "profile-id")) {
        ...
        av_em_log(s, AV_LOG_TRACE, "SDP: found profile-id: %d\n", hevc_data->profile_id);
    }
    ...
    if (!strcmp(attr, "sprop-vps") || !strcmp(attr, "sprop-sps") ||
        ...
        em_h264_parse_sprop_parameter_sets(s, data_ptr,
                                           size_ptr, value);
    }
    ...
    */
    if (!strcmp(attr, "sprop-max-don-diff")) {
        ...
        av_em_log(s, AV_LOG_TRACE, "Found sprop-max-don-diff in SDP, DON field usage is: %d\n",
    ...
    if (!strcmp(attr, "sprop-depack-buf-nalus")) {
        if (atoi(value) > 0)
            hevc_data->using_donl_field = 1;
        av_em_log(s, AV_LOG_TRACE, "Found sprop-depack-buf-nalus in SDP, DON field usage is: %d\n",
                hevc_data->using_donl_field);
    }
...
static av_cold int hevc_parse_sdp_line(AVEMFormatContext *ctx, int st_index,
                                       PayloadContext *hevc_data, const char *line)
{
    AVEMStream *current_stream;
    ...
    if (av_em_strstart(sdp_line_ptr, "framesize:", &sdp_line_ptr)) {
        em_h264_parse_framesize(par, sdp_line_ptr);
    } else if (av_em_strstart(sdp_line_ptr, "fmtp:", &sdp_line_ptr)) {
        ...
        if (hevc_data->vps_size || hevc_data->sps_size ||
            ...
            par->extradata_size = hevc_data->vps_size + hevc_data->sps_size +
                                  hevc_data->pps_size + hevc_data->sei_size;
            par->extradata = av_em_alloc(par->extradata_size +
            ...
            av_em_freep(&hevc_data->vps);
            av_em_freep(&hevc_data->sps);
            av_em_freep(&hevc_data->pps);
            av_em_freep(&hevc_data->sei);
...
static int hevc_handle_packet(AVEMFormatContext *ctx, PayloadContext *rtp_hevc_ctx,
                              ...
                              int flags)
{
    ...
    if (len < RTP_HEVC_PAYLOAD_HEADER_SIZE + 1) {
        av_em_log(ctx, AV_LOG_ERROR, "Too short RTP/HEVC packet, got %d bytes\n", len);
    ...
    if (lid) {
        ...
        avpriv_em_report_missing_feature(ctx, "Multi-layer HEVC coding");
        return AVERROR_PATCHWELCOME;
    }

    /* sanity check for correct temporal ID */
    if (!tid) {
        av_em_log(ctx, AV_LOG_ERROR, "Illegal temporal ID in RTP/HEVC packet\n");
        return AVERROR_INVALIDDATA;
    }

    /* sanity check for correct NAL unit type */
    if (nal_type > 50) {
        av_em_log(ctx, AV_LOG_ERROR, "Unsupported (HEVC) NAL type (%d)\n", nal_type);
        return AVERROR_INVALIDDATA;
    }
    ...
    default:
        /* create A/V packet */
        if ((res = av_em_new_packet(pkt, sizeof(start_sequence) + len)) < 0)
            return res;
    ...
    case 48:
        ...
        res = em_h264_handle_aggregated_packet(ctx, rtp_hevc_ctx, pkt, buf, len,
    ...
    case 49:
        ...
        av_em_log(ctx, AV_LOG_TRACE, " FU type %d with %d bytes\n", fu_type, len);
        ...
        if (first_fragment && last_fragment) {
            av_em_log(ctx, AV_LOG_ERROR, "Illegal combination of S and E bit in RTP/HEVC packet\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        res = em_h264_handle_frag_packet(pkt, buf, len, first_fragment,
    ...
    case 50:
        ...
        avpriv_em_report_missing_feature(ctx, "PACI packets for RTP/HEVC");
...
RTPDynamicProtocolHandler em_hevc_dynamic_handler = {
    .enc_name         = "H265",
