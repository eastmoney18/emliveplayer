...
static const AVEMRational mxf_time_base[] = {
    { 1001, 24000 },
    { 1, 24},
    { 1001, 30000 },
    { 1001, 60000 },
...
const MXFSamplesPerFrame *ff_mxf_get_samples_per_frame(AVEMFormatContext *s,
                                                       AVEMRational time_base)
{
    int idx = av_em_find_nearest_q_idx(time_base, mxf_time_base);
    AVEMRational diff = av_em_sub_q(time_base, mxf_time_base[idx]);

    diff.num = abs(diff.num);

    if (av_em_cmp_q(diff, (AVEMRational){1, 1000}) >= 0)
        return NULL;

    if (av_em_cmp_q(time_base, mxf_time_base[idx]))
