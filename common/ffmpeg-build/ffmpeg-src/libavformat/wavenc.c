...
#if CONFIG_WAV_MUXER
static inline void bwf_write_bext_string(AVEMFormatContext *s, const char *key, int maxlen)
{
    AVEMDictionaryEntry *tag;
    size_t len = 0;

    if (tag = av_em_dict_get(s->metadata, key, NULL, 0)) {
        len = strlen(tag->value);
        len = FFMIN(len, maxlen);
        avio_em_write(s->pb, tag->value, len);
    }

    emio_fill(s->pb, 0, maxlen - len);
}

static void bwf_write_bext_chunk(AVEMFormatContext *s)
{
    AVEMDictionaryEntry *tmp_tag;
    ...
    if (tmp_tag = av_em_dict_get(s->metadata, "time_reference", NULL, 0))
        time_reference = strtoll(tmp_tag->value, NULL, 10);
    avio_em_wl64(s->pb, time_reference);
    avio_em_wl16(s->pb, 1);  // set version to 1

    if ((tmp_tag = av_em_dict_get(s->metadata, "umid", NULL, 0)) && strlen(tmp_tag->value) > 2) {
        ...
        for (i = 0; i < len/16; i++) {
            ...
            avio_em_wb64(s->pb, umidpart);
        }
        emio_fill(s->pb, 0, 64 - i*8);
    } else
        emio_fill(s->pb, 0, 64); // zero UMID

    emio_fill(s->pb, 0, 190); // Reserved

    if (tmp_tag = av_em_dict_get(s->metadata, "coding_history", NULL, 0))
        avio_em_put_str(s->pb, tmp_tag->value);

    ff_end_tag(s->pb, bext);
}

static av_cold void peak_free_buffers(AVEMFormatContext *s)
{
    WAVMuxContext *wav = s->priv_data;

    av_em_freep(&wav->peak_maxpos);
    av_em_freep(&wav->peak_maxneg);
    av_em_freep(&wav->peak_output);
}

static av_cold int peak_init_writer(AVEMFormatContext *s)
{
    WAVMuxContext *wav = s->priv_data;
    ...
    if (par->codec_id != AV_CODEC_ID_PCM_S8 &&
        ...
        AVEMCodec *codec = avcodec_em_find_decoder(s->streams[0]->codecpar->codec_id);
        av_em_log(s, AV_LOG_ERROR, "%s codec not supported for Peak Chunk\n",
               codec ? codec->name : "NONE");
        return -1;
    }

    wav->peak_bps = av_em_get_bits_per_sample(par->codec_id) / 8;
    ...
    wav->peak_maxpos = av_em_mallocz_array(par->channels, sizeof(*wav->peak_maxpos));
    wav->peak_maxneg = av_em_mallocz_array(par->channels, sizeof(*wav->peak_maxneg));
    wav->peak_output = av_em_alloc(PEAK_BUFFER_SIZE);
...
static void peak_write_frame(AVEMFormatContext *s)
{
    WAVMuxContext *wav = s->priv_data;
    AVEMCodecParameters *par = s->streams[0]->codecpar;
    int peak_of_peaks;
    ...
    for (c = 0; c < par->channels; c++) {
        ...
        if (wav->peak_outbuf_size - wav->peak_outbuf_bytes <
            ...
            wav->peak_outbuf_size += PEAK_BUFFER_SIZE;
            wav->peak_output = av_em_realloc(wav->peak_output,
                                          wav->peak_outbuf_size);
            if (!wav->peak_output) {
                av_em_log(s, AV_LOG_ERROR, "No memory for peak data\n");
                return;
            }
        }
...
static int peak_write_chunk(AVEMFormatContext *s)
{
    WAVMuxContext *wav = s->priv_data;
    ...
    if (!(s->flags & AVFMT_FLAG_BITEXACT)) {
        struct tm tmpbuf;
        av_em_log(s, AV_LOG_INFO, "Writing local time and date to Peak Envelope Chunk\n");
        now0 = av_em_gettime();
        now_secs = now0 / 1000000;
        if (strftime(timestamp, sizeof(timestamp), "%Y:%m:%d:%H:%M:%S:", localtime_r(&now_secs, &tmpbuf))) {
            av_em_strlcatf(timestamp, sizeof(timestamp), "%03d", (int)((now0 / 1000) % 1000));
        } else {
            av_em_log(s, AV_LOG_ERROR, "Failed to write timestamp\n");
            return -1;
        }
    }

    avio_em_wl32(pb, 1);                           /* version */
    avio_em_wl32(pb, wav->peak_format);            /* 8 or 16 bit */
    avio_em_wl32(pb, wav->peak_ppv);               /* positive and negative */
    avio_em_wl32(pb, wav->peak_block_size);        /* frames per value */
    avio_em_wl32(pb, par->channels);               /* number of channels */
    avio_em_wl32(pb, wav->peak_num_frames);        /* number of peak frames */
    avio_em_wl32(pb, wav->peak_pos_pop);           /* audio sample frame index */
    avio_em_wl32(pb, 128);                         /* equal to size of header */
    avio_em_write(pb, timestamp, 28);              /* ASCII time stamp */
    emio_fill(pb, 0, 60);

    avio_em_write(pb, wav->peak_output, wav->peak_outbuf_bytes);
...
static int wav_write_header(AVEMFormatContext *s)
{
    WAVMuxContext *wav = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int64_t fmt;
    ...
    if (s->nb_streams != 1) {
        av_em_log(s, AV_LOG_ERROR, "WAVE files have exactly one stream\n");
        return AVERROR(EINVAL);
    }

    if (wav->rf64 == RF64_ALWAYS) {
        emio_wfourcc(pb, "RF64");
        avio_em_wl32(pb, -1); /* RF64 chunk size: use size in ds64 */
    } else {
        emio_wfourcc(pb, "RIFF");
        avio_em_wl32(pb, -1); /* file length */
    }
    ...
    if (wav->rf64 != RF64_NEVER) {
        ...
        emio_wfourcc(pb, wav->rf64 == RF64_ALWAYS ? "ds64" : "JUNK");
        avio_em_wl32(pb, 28); /* chunk size */
        wav->ds64 = avio_em_tell(pb);
        emio_fill(pb, 0, 28);
    }
    ...
    if (wav->write_peak != 2) {
        ...
        if (ff_put_wav_header(s, pb, s->streams[0]->codecpar, 0) < 0) {
            const AVEMCodecDescriptor *desc = avcodec_em_descriptor_get(s->streams[0]->codecpar->codec_id);
            av_em_log(s, AV_LOG_ERROR, "%s codec not supported in WAVE format\n",
    ...
    avpriv_em_set_pts_info(s->streams[0], 64, 1, s->streams[0]->codecpar->sample_rate);
...
static int wav_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    AVEMIOContext *pb  = s->pb;
    WAVMuxContext    *wav = s->priv_data;
    ...
    if(pkt->pts != AV_NOPTS_VALUE) {
        ...
        av_em_log(s, AV_LOG_ERROR, "wav_write_packet: NOPTS\n");
    return 0;
}

static int wav_em_write_trailer(AVEMFormatContext *s)
{
    AVEMIOContext *pb  = s->pb;
    WAVMuxContext    *wav = s->priv_data;
    ...
    if (s->pb->seekable) {
        if (wav->write_peak != 2 && avio_em_tell(pb) - wav->data < UINT32_MAX) {
        ...
        if (wav->rf64 == RF64_ALWAYS || (wav->rf64 == RF64_AUTO && file_size - 8 > UINT32_MAX)) {
            ...
            avio_em_seek(pb, 4, SEEK_SET);
            avio_em_wl32(pb, (uint32_t)(file_size - 8));
        ...
        if (rf64) {
            ...
            avio_em_seek(pb, wav->ds64 - 8, SEEK_SET);
            emio_wfourcc(pb, "ds64");
            avio_em_wl32(pb, 28);                  /* ds64 chunk size */
            avio_em_wl64(pb, file_size - 8);       /* RF64 chunk size */
            avio_em_wl64(pb, data_size);           /* data chunk size */
            avio_em_wl64(pb, number_of_samples);   /* fact chunk number of samples */
            avio_em_wl32(pb, 0);                   /* number of table entries for non-'data' chunks */

            /* write -1 in data chunk size */
            avio_em_seek(pb, wav->data - 4, SEEK_SET);
            avio_em_wl32(pb, -1);
...
static const AVEMClass wav_muxer_class = {
...
AVEMOutputFormat ff_wav_muxer = {
    ...
    .write_trailer     = wav_em_write_trailer,
...
static void start_guid(AVEMIOContext *pb, const uint8_t *guid, int64_t *pos)
{
    *pos = avio_em_tell(pb);

    avio_em_write(pb, guid, 16);
    avio_em_wl64(pb, INT64_MAX);
}

static void end_guid(AVEMIOContext *pb, int64_t start)
{
    int64_t end, pos = avio_em_tell(pb);

    end = FFALIGN(pos, 8);
    emio_fill(pb, 0, end - pos);
    avio_em_seek(pb, start + 16, SEEK_SET);
    avio_em_wl64(pb, end - start);
    avio_em_seek(pb, end, SEEK_SET);
}

static int w64_write_header(AVEMFormatContext *s)
{
    WAVMuxContext *wav = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int64_t start;
    ...
    avio_em_write(pb, ff_w64_guid_riff, sizeof(ff_w64_guid_riff));
    avio_em_wl64(pb, -1);
    avio_em_write(pb, ff_w64_guid_wave, sizeof(ff_w64_guid_wave));
    start_guid(pb, ff_w64_guid_fmt, &start);
    if ((ret = ff_put_wav_header(s, pb, s->streams[0]->codecpar, 0)) < 0) {
        AVEMCodec *codec = avcodec_em_find_decoder(s->streams[0]->codecpar->codec_id);
        av_em_log(s, AV_LOG_ERROR, "%s codec not supported\n",
               codec ? codec->name : "NONE");
        return ret;
    }
    end_guid(pb, start);
...
static int w64_write_trailer(AVEMFormatContext *s)
{
    AVEMIOContext    *pb = s->pb;
    WAVMuxContext *wav = s->priv_data;
    int64_t file_size;
    ...
    if (pb->seekable) {
        ...
        if (s->streams[0]->codecpar->codec_tag != 0x01) {
            ...
            number_of_samples = av_em_rescale(wav->maxpts - wav->minpts + wav->last_duration,
                                           s->streams[0]->codecpar->sample_rate * (int64_t)s->streams[0]->time_base.num,
                                           s->streams[0]->time_base.den);
            avio_em_seek(pb, wav->fact_pos + 24, SEEK_SET);
            avio_em_wl64(pb, number_of_samples);
        }
