...
static int yop_probe(AVEMProbeData *probe_packet)
{
...
static int yop_read_header(AVEMFormatContext *s)
{
    YopDecContext *yop = s->priv_data;
    AVEMIOContext *pb  = s->pb;

    AVEMCodecParameters *audio_par, *video_par;
    AVEMStream *audio_stream, *video_stream;

    int frame_rate, ret;

    audio_stream = avformat_em_new_stream(s, NULL);
    video_stream = avformat_em_new_stream(s, NULL);
    if (!audio_stream || !video_stream)
        return AVERROR(ENOMEM);

    // Extra data that will be passed to the decoder
    if (em_alloc_extradata(video_stream->codecpar, 8))
        return AVERROR(ENOMEM);
    ...
    frame_rate              = avio_em_r8(pb);
    yop->frame_size         = avio_em_r8(pb) * 2048;
    video_par->width        = avio_em_rl16(pb);
    video_par->height       = avio_em_rl16(pb);

    video_stream->sample_aspect_ratio = (AVEMRational){1, 2};

    ret = avio_em_read(pb, video_par->extradata, 8);
    if (ret < 8)
    ...
    if (yop->audio_block_length < 920 ||
        ...
        av_em_log(s, AV_LOG_ERROR, "YOP has invalid header\n");
        return AVERROR_INVALIDDATA;
    }

    avio_em_seek(pb, 2048, SEEK_SET);

    avpriv_em_set_pts_info(video_stream, 32, 1, frame_rate);

    return 0;
}

static int yop_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    YopDecContext *yop = s->priv_data;
    AVEMIOContext *pb  = s->pb;
    ...
    ret = av_em_new_packet(&yop->video_packet,
                        yop->frame_size - yop->audio_block_length);
    if (ret < 0)
        return ret;

    yop->video_packet.pos = avio_em_tell(pb);

    ret = avio_em_read(pb, yop->video_packet.data, yop->palette_size);
    if (ret < 0) {
        goto err_out;
    ...
    ret = av_em_get_packet(pb, pkt, 920);
    if (ret < 0)
        goto err_out;
    ...
    avio_em_skip(pb, yop->audio_block_length - ret);
    ...
    if (ret < 0)
        ...
        av_em_shrink_packet(&yop->video_packet, yop->palette_size + ret);

    // Arbitrarily return the audio data first
    return yop->audio_block_length;

err_out:
    av_em_packet_unref(&yop->video_packet);
    return ret;
}

static int yop_read_close(AVEMFormatContext *s)
{
    YopDecContext *yop = s->priv_data;
    av_em_packet_unref(&yop->video_packet);
    return 0;
...
static int yop_read_seek(AVEMFormatContext *s, int stream_index,
                         int64_t timestamp, int flags)
{
    ...
    pos_max        = avio_em_size(s->pb) - yop->frame_size;
    ...
    if (avio_em_seek(s->pb, frame_pos, SEEK_SET) < 0)
        return -1;
...
AVEMInputFormat ff_yop_demuxer = {
    .name           = "yop",
