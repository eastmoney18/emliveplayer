...
static int read_header(AVEMFormatContext *s)
{
    ...
    avio_em_skip(pb, 4); /* magic number */
    if (avio_em_rl16(pb) != MAX_PAGES) {
        avpriv_em_request_sample(s, "max_pages != " AV_STRINGIFY(MAX_PAGES));
        return AVERROR_PATCHWELCOME;
    }

    anm->nb_pages   = avio_em_rl16(pb);
    anm->nb_records = avio_em_rl32(pb);
    avio_em_skip(pb, 2); /* max records per page */
    anm->page_table_offset = avio_em_rl16(pb);
    if (avio_em_rl32(pb) != ANIM_TAG)
        return AVERROR_INVALIDDATA;
    ...
    st->codecpar->width      = avio_em_rl16(pb);
    st->codecpar->height     = avio_em_rl16(pb);
    if (avio_em_r8(pb) != 0)
        goto invalid;
    avio_em_skip(pb, 1); /* frame rate multiplier info */

    /* ignore last delta record (used for looping) */
    if (avio_em_r8(pb))  /* has_last_delta */
        anm->nb_records = FFMAX(anm->nb_records - 1, 0);

    avio_em_skip(pb, 1); /* last_delta_valid */
    ...
    avio_em_skip(pb, 1); /* other recs per frame */

    if (avio_em_r8(pb) != 1)
        goto invalid;

    avio_em_skip(pb, 32); /* record_types */
    st->nb_frames = avio_em_rl32(pb);
    avpriv_em_set_pts_info(st, 64, 1, avio_em_rl16(pb));
    avio_em_skip(pb, 58);
    ...
    ret = avio_em_read(pb, st->codecpar->extradata, st->codecpar->extradata_size);
    if (ret < 0)
        return ret;

    /* read page table */
    ret = avio_em_seek(pb, anm->page_table_offset, SEEK_SET);
    if (ret < 0)
        return ret;

    for (i = 0; i < MAX_PAGES; i++) {
        Page *p = &anm->pt[i];
        p->base_record = avio_em_rl16(pb);
        p->nb_records  = avio_em_rl16(pb);
        p->size        = avio_em_rl16(pb);
    }
...
invalid:
    avpriv_em_request_sample(s, "Invalid header element");
    return AVERROR_PATCHWELCOME;
}
...
repeat:
    ...
    /* parse page header */
    if (anm->record < 0) {
        avio_em_seek(pb, anm->page_table_offset + MAX_PAGES*6 + (anm->page<<16), SEEK_SET);
        avio_em_skip(pb, 8 + 2*p->nb_records);
        anm->record = 0;
    }
    ...
    /* fetch record size */
    tmp = avio_em_tell(pb);
    avio_em_seek(pb, anm->page_table_offset + MAX_PAGES*6 + (anm->page<<16) +
              8 + anm->record * 2, SEEK_SET);
    record_size = avio_em_rl16(pb);
    avio_em_seek(pb, tmp, SEEK_SET);

    /* fetch record */
    pkt->size = av_em_get_packet(s->pb, pkt, record_size);
    if (pkt->size < 0)
        return pkt->size;
...
AVEMInputFormat ff_anm_demuxer = {
    .name           = "anm",
