...
static int subviewer1_probe(AVEMProbeData *p)
{
...
static int subviewer1_read_header(AVEMFormatContext *s)
{
    int delay = 0;
    AVEMPacket *sub = NULL;
    ...
    while (!avio_em_feof(s->pb)) {
        ...
        if (!strncmp(line, "[DELAY]", 7)) {
            em_get_line(s->pb, line, sizeof(line));
            sscanf(line, "%d", &delay);
        }
        ...
        if (sscanf(line, "[%d:%d:%d]", &hh, &mm, &ss) == 3) {
            ...
            len = em_get_line(s->pb, line, sizeof(line));
            line[strcspn(line, "\r\n")] = 0;
            if (!*line) {
                if (sub)
...
static int subviewer1_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    SubViewer1Context *subviewer1 = s->priv_data;
    return ff_subtitles_queue_read_packet(&subviewer1->q, pkt);
}

static int subviewer1_read_seek(AVEMFormatContext *s, int stream_index,
...
static int subviewer1_read_close(AVEMFormatContext *s)
{
    SubViewer1Context *subviewer1 = s->priv_data;
...
AVEMInputFormat ff_subviewer1_demuxer = {
    .name           = "subviewer1",
