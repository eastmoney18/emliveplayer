...
int ff_rtp_get_codec_info(AVEMCodecParameters *par, int payload_type)
{
    int i = 0;
...
int ff_rtp_get_payload_type(AVEMFormatContext *fmt,
                            AVEMCodecParameters *par, int idx)
{
    int i;
    AVEMOutputFormat *ofmt = fmt ? fmt->oformat : NULL;

    /* Was the payload type already specified for the RTP muxer? */
    if (ofmt && ofmt->priv_class && fmt->priv_data) {
        int64_t payload_type;
        if (av_em_opt_get_int(fmt->priv_data, "payload_type", 0, &payload_type) >= 0 &&
            payload_type >= 0)
...
enum AVEMCodecID ff_rtp_codec_id(const char *buf, enum AVEMMediaType codec_type)
{
    int i;

    for (i = 0; rtp_payload_types[i].pt >= 0; i++)
        if (!av_em_strcasecmp(buf, rtp_payload_types[i].enc_name) && (codec_type == rtp_payload_types[i].codec_type))
