...
typedef struct MXFDescriptor {
    ...
    UID codec_ul;
    AVEMRational sample_rate;
    AVEMRational aspect_ratio;
    int width;
...
typedef struct MXFIndexTable {
    ...
    AVEMIndexEntry *fake_index;   /* used for calling em_index_search_timestamp() */
...
typedef int MXFMetadataReadFunc(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset);
...
static int mxf_read_close(AVEMFormatContext *s);
...
static void mxf_free_metadataset(MXFMetadataSet **ctx, int freectx)
{
    ...
    switch ((*ctx)->type) {
    case Descriptor:
        av_em_freep(&((MXFDescriptor *)*ctx)->extradata);
        break;
    case MultipleDescriptor:
        av_em_freep(&((MXFDescriptor *)*ctx)->sub_descriptors_refs);
        break;
    case Sequence:
        av_em_freep(&((MXFSequence *)*ctx)->structural_components_refs);
        break;
    case EssenceGroup:
        av_em_freep(&((MXFEssenceGroup *)*ctx)->structural_components_refs);
        break;
    case SourcePackage:
    case MaterialPackage:
        av_em_freep(&((MXFPackage *)*ctx)->tracks_refs);
        av_em_freep(&((MXFPackage *)*ctx)->name);
        av_em_freep(&((MXFPackage *)*ctx)->comment_refs);
        break;
    case TaggedValue:
        av_em_freep(&((MXFTaggedValue *)*ctx)->name);
        av_em_freep(&((MXFTaggedValue *)*ctx)->value);
        break;
    case IndexTableSegment:
        seg = (MXFIndexTableSegment *)*ctx;
        av_em_freep(&seg->temporal_offset_entries);
        av_em_freep(&seg->flag_entries);
        av_em_freep(&seg->stream_offset_entries);
    default:
        break;
    }
    if (freectx)
...
static int64_t klv_decode_ber_length(AVEMIOContext *pb)
{
    uint64_t size = avio_em_r8(pb);
    if (size & 0x80) { /* long form */
        ...
        size = 0;
        while (bytes_num--)
            size = size << 8 | avio_em_r8(pb);
    }
    return size;
}

static int mxf_read_sync(AVEMIOContext *pb, const uint8_t *key, unsigned size)
{
    int i, b;
    for (i = 0; i < size && !avio_em_feof(pb); i++) {
        b = avio_em_r8(pb);
        if (b == key[0])
            i = 0;
        else if (b != key[i])
...
static int klv_read_packet(KLVPacket *klv, AVEMIOContext *pb)
{
    if (!mxf_read_sync(pb, mxf_klv_key, 4))
        return AVERROR_INVALIDDATA;
    klv->offset = avio_em_tell(pb) - 4;
    memcpy(klv->key, mxf_klv_key, 4);
    avio_em_read(pb, klv->key + 4, 12);
    klv->length = klv_decode_ber_length(pb);
    return klv->length == -1 ? -1 : 0;
}

static int mxf_get_stream_index(AVEMFormatContext *s, KLVPacket *klv)
{
    int i;
...
static int mxf_get_d10_aes3_packet(AVEMIOContext *pb, AVEMStream *st, AVEMPacket *pkt, int64_t length)
{
    ...
    if (length > 61444) /* worst case PAL 1920 samples 8 channels */
        return AVERROR_INVALIDDATA;
    length = av_em_get_packet(pb, pkt, length);
    if (length < 0)
        return length;
    data_ptr = pkt->data;
    ...
    av_em_shrink_packet(pkt, data_ptr - pkt->data);
    return 0;
}

static int mxf_decrypt_triplet(AVEMFormatContext *s, AVEMPacket *pkt, KLVPacket *klv)
{
    static const uint8_t checkv[16] = {0x43, 0x48, 0x55, 0x4b, 0x43, 0x48, 0x55, 0x4b, 0x43, 0x48, 0x55, 0x4b, 0x43, 0x48, 0x55, 0x4b};
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int64_t end = avio_em_tell(pb) + klv->length;
    int64_t size;
    uint64_t orig_size;
    ...
    if (!mxf->aesc && s->key && s->keylen == 16) {
        mxf->aesc = av_em_aes_alloc();
        if (!mxf->aesc)
            return AVERROR(ENOMEM);
        av_em_aes_init(mxf->aesc, s->key, 128, 1);
    }
    // crypto context
    avio_em_skip(pb, klv_decode_ber_length(pb));
    // plaintext offset
    klv_decode_ber_length(pb);
    plaintext_size = avio_em_rb64(pb);
    // source klv key
    klv_decode_ber_length(pb);
    avio_em_read(pb, klv->key, 16);
    ...
    if (size < 32 || size - 32 < orig_size)
        return AVERROR_INVALIDDATA;
    avio_em_read(pb, ivec, 16);
    avio_em_read(pb, tmpbuf, 16);
    if (mxf->aesc)
        av_em_aes_crypt(mxf->aesc, tmpbuf, tmpbuf, 1, ivec, 1);
    if (memcmp(tmpbuf, checkv, 16))
        av_em_log(s, AV_LOG_ERROR, "probably incorrect decryption key\n");
    size -= 32;
    size = av_em_get_packet(pb, pkt, size);
    if (size < 0)
        return size;
    ...
    size -= plaintext_size;
    if (mxf->aesc)
        av_em_aes_crypt(mxf->aesc, &pkt->data[plaintext_size],
                     &pkt->data[plaintext_size], size >> 4, ivec, 1);
    av_em_shrink_packet(pkt, orig_size);
    pkt->stream_index = index;
    avio_em_skip(pb, end - avio_em_tell(pb));
    return 0;
}

static int mxf_read_primer_pack(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    MXFContext *mxf = arg;
    int item_num = avio_em_rb32(pb);
    int item_len = avio_em_rb32(pb);

    if (item_len != 18) {
        avpriv_em_request_sample(pb, "Primer pack item length %d", item_len);
        return AVERROR_PATCHWELCOME;
    }
    if (item_num > 65536) {
        av_em_log(mxf->fc, AV_LOG_ERROR, "item_num %d is too large\n", item_num);
        return AVERROR_INVALIDDATA;
    }
    if (mxf->local_tags)
        av_em_log(mxf->fc, AV_LOG_VERBOSE, "Multiple primer packs\n");
    av_em_free(mxf->local_tags);
    mxf->local_tags_count = 0;
    mxf->local_tags = av_em_calloc(item_num, item_len);
    if (!mxf->local_tags)
        return AVERROR(ENOMEM);
    mxf->local_tags_count = item_num;
    avio_em_read(pb, mxf->local_tags, item_num*item_len);
    return 0;
}

static int mxf_read_partition_pack(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    MXFContext *mxf = arg;
    ...
    tmp_part = av_em_realloc_array(mxf->partitions, mxf->partitions_count + 1, sizeof(*mxf->partitions));
    if (!tmp_part)
        return AVERROR(ENOMEM);
    ...
    partition->pack_length = avio_em_tell(pb) - klv_offset + size;
    ...
    case 4:
        ...
        break;
    default:
        av_em_log(mxf->fc, AV_LOG_ERROR, "unknown partition type %i\n", uid[13]);
        return AVERROR_INVALIDDATA;
    }
    ...
    avio_em_skip(pb, 4);
    partition->kag_size = avio_em_rb32(pb);
    partition->this_partition = avio_em_rb64(pb);
    partition->previous_partition = avio_em_rb64(pb);
    footer_partition = avio_em_rb64(pb);
    partition->header_byte_count = avio_em_rb64(pb);
    partition->index_byte_count = avio_em_rb64(pb);
    partition->index_sid = avio_em_rb32(pb);
    avio_em_skip(pb, 8);
    partition->body_sid = avio_em_rb32(pb);
    if (avio_em_read(pb, op, sizeof(UID)) != sizeof(UID)) {
        av_em_log(mxf->fc, AV_LOG_ERROR, "Failed reading UID\n");
        return AVERROR_INVALIDDATA;
    }
    nb_essence_containers = avio_em_rb32(pb);

    if (partition->this_partition &&
        partition->previous_partition == partition->this_partition) {
        av_em_log(mxf->fc, AV_LOG_ERROR,
    ...
    if      (op[12] == 1 && op[13] == 1) mxf->op = OP1a;
        ...
        if (nb_essence_containers != 1) {
            ...
            /* only nag once */
            if (!mxf->op)
                av_em_log(mxf->fc, AV_LOG_WARNING,
        ...
        av_em_log(mxf->fc, AV_LOG_ERROR, "unknown operational pattern: %02xh %02xh - guessing OP1a\n", op[12], op[13]);
        mxf->op = OP1a;
    }

    if (partition->kag_size <= 0 || partition->kag_size > (1 << 20)) {
        av_em_log(mxf->fc, AV_LOG_WARNING, "invalid KAGSize %"PRId32" - guessing ",
               partition->kag_size);
        ...
        av_em_log(mxf->fc, AV_LOG_WARNING, "%"PRId32"\n", partition->kag_size);
    }
...
static int mxf_add_metadata_set(MXFContext *mxf, void *metadata_set)
{
    ...
    tmp = av_em_realloc_array(mxf->metadata_sets, mxf->metadata_sets_count + 1, sizeof(*mxf->metadata_sets));
    if (!tmp)
        return AVERROR(ENOMEM);
...
static int mxf_read_cryptographic_context(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    MXFCryptoContext *cryptocontext = arg;
    if (size != 16)
        return AVERROR_INVALIDDATA;
    if (IS_KLV_KEY(uid, mxf_crypto_source_container_ul))
        avio_em_read(pb, cryptocontext->source_container_ul, 16);
    return 0;
}

static int mxf_read_strong_ref_array(AVEMIOContext *pb, UID **refs, int *count)
{
    *count = avio_em_rb32(pb);
    *refs = av_em_calloc(*count, sizeof(UID));
    if (!*refs) {
        *count = 0;
        return AVERROR(ENOMEM);
    }
    avio_em_skip(pb, 4); /* useless size of objects, always 16 according to specs */
    avio_em_read(pb, (uint8_t *)*refs, *count * sizeof(UID));
    return 0;
}

static int mxf_read_content_storage(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    MXFContext *mxf = arg;
    switch (tag) {
    case 0x1901:
        if (mxf->packages_refs)
            av_em_log(mxf->fc, AV_LOG_VERBOSE, "Multiple packages_refs\n");
        av_em_free(mxf->packages_refs);
        return mxf_read_strong_ref_array(pb, &mxf->packages_refs, &mxf->packages_count);
    }
    return 0;
}

static int mxf_read_source_clip(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    MXFStructuralComponent *source_clip = arg;
    switch(tag) {
    case 0x0202:
        source_clip->duration = avio_em_rb64(pb);
        break;
    case 0x1201:
        source_clip->start_position = avio_em_rb64(pb);
        break;
    case 0x1101:
        /* UMID, only get last 16 bytes */
        avio_em_skip(pb, 16);
        avio_em_read(pb, source_clip->source_package_uid, 16);
        break;
    case 0x1102:
        source_clip->source_track_id = avio_em_rb32(pb);
        break;
    }
    return 0;
}

static int mxf_read_timecode_component(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    MXFTimecodeComponent *mxf_timecode = arg;
    switch(tag) {
    case 0x1501:
        mxf_timecode->start_frame = avio_em_rb64(pb);
        break;
    case 0x1502:
        mxf_timecode->rate = (AVEMRational){avio_em_rb16(pb), 1};
        break;
    case 0x1503:
        mxf_timecode->drop_frame = avio_em_r8(pb);
        break;
    }
    return 0;
}

static int mxf_read_pulldown_component(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    MXFPulldownComponent *mxf_pulldown = arg;
    switch(tag) {
    case 0x0d01:
        avio_em_read(pb, mxf_pulldown->input_segment_ref, 16);
        break;
    }
    return 0;
}

static int mxf_read_track(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    MXFTrack *track = arg;
    switch(tag) {
    case 0x4801:
        track->track_id = avio_em_rb32(pb);
        break;
    case 0x4804:
        avio_em_read(pb, track->track_number, 4);
        break;
    case 0x4b01:
        track->edit_rate.num = avio_em_rb32(pb);
        track->edit_rate.den = avio_em_rb32(pb);
        break;
    case 0x4803:
        avio_em_read(pb, track->sequence_ref, 16);
        break;
    }
    return 0;
}

static int mxf_read_sequence(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    MXFSequence *sequence = arg;
    switch(tag) {
    case 0x0202:
        sequence->duration = avio_em_rb64(pb);
        break;
    case 0x0201:
        avio_em_read(pb, sequence->data_definition_ul, 16);
        break;
        case 0x4b02:
        sequence->origin = avio_em_r8(pb);
        break;
    case 0x1001:
...
static int mxf_read_essence_group(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    MXFEssenceGroup *essence_group = arg;
    switch (tag) {
    case 0x0202:
        essence_group->duration = avio_em_rb64(pb);
        break;
    case 0x0501:
...
static inline int mxf_read_utf16_string(AVEMIOContext *pb, int size, char** str, int be)
{
    int ret;
    size_t buf_size;
    ...
    *str = av_em_alloc(buf_size);
    if (!*str)
        return AVERROR(ENOMEM);

    if (be)
        ret = avio_em_get_str16be(pb, size, *str, buf_size);
    else
        ret = avio_em_get_str16le(pb, size, *str, buf_size);
...
static int mxf_read_utf16 ## type ##_string(AVEMIOContext *pb, int size, char** str) \
{                                                                                  \
...
static int mxf_read_package(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    ...
    case 0x4401:
        /* UMID */
        avio_em_read(pb, package->package_ul, 16);
        avio_em_read(pb, package->package_uid, 16);
        break;
    case 0x4701:
        avio_em_read(pb, package->descriptor_ref, 16);
        break;
    case 0x4402:
...
static int mxf_read_index_entry_array(AVEMIOContext *pb, MXFIndexTableSegment *segment)
{
    int i, length;

    segment->nb_index_entries = avio_em_rb32(pb);

    length = avio_em_rb32(pb);

    if (!(segment->temporal_offset_entries=av_em_calloc(segment->nb_index_entries, sizeof(*segment->temporal_offset_entries))) ||
        !(segment->flag_entries          = av_em_calloc(segment->nb_index_entries, sizeof(*segment->flag_entries))) ||
        !(segment->stream_offset_entries = av_em_calloc(segment->nb_index_entries, sizeof(*segment->stream_offset_entries)))) {
        av_em_freep(&segment->temporal_offset_entries);
        av_em_freep(&segment->flag_entries);
        return AVERROR(ENOMEM);
    }

    for (i = 0; i < segment->nb_index_entries; i++) {
        segment->temporal_offset_entries[i] = avio_em_r8(pb);
        avio_em_r8(pb);                                        /* KeyFrameOffset */
        segment->flag_entries[i] = avio_em_r8(pb);
        segment->stream_offset_entries[i] = avio_em_rb64(pb);
        avio_em_skip(pb, length - 11);
    }
    return 0;
}

static int mxf_read_index_table_segment(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    MXFIndexTableSegment *segment = arg;
    switch(tag) {
    case 0x3F05:
        segment->edit_unit_byte_count = avio_em_rb32(pb);
        av_em_log(NULL, AV_LOG_TRACE, "EditUnitByteCount %d\n", segment->edit_unit_byte_count);
        break;
    case 0x3F06:
        segment->index_sid = avio_em_rb32(pb);
        av_em_log(NULL, AV_LOG_TRACE, "IndexSID %d\n", segment->index_sid);
        break;
    case 0x3F07:
        segment->body_sid = avio_em_rb32(pb);
        av_em_log(NULL, AV_LOG_TRACE, "BodySID %d\n", segment->body_sid);
        break;
    case 0x3F0A:
        av_em_log(NULL, AV_LOG_TRACE, "IndexEntryArray found\n");
        return mxf_read_index_entry_array(pb, segment);
    case 0x3F0B:
        segment->index_edit_rate.num = avio_em_rb32(pb);
        segment->index_edit_rate.den = avio_em_rb32(pb);
        av_em_log(NULL, AV_LOG_TRACE, "IndexEditRate %d/%d\n", segment->index_edit_rate.num,
                segment->index_edit_rate.den);
        break;
    case 0x3F0C:
        segment->index_start_position = avio_em_rb64(pb);
        av_em_log(NULL, AV_LOG_TRACE, "IndexStartPosition %"PRId64"\n", segment->index_start_position);
        break;
    case 0x3F0D:
        segment->index_duration = avio_em_rb64(pb);
        av_em_log(NULL, AV_LOG_TRACE, "IndexDuration %"PRId64"\n", segment->index_duration);
        break;
    }
    return 0;
}

static void mxf_read_pixel_layout(AVEMIOContext *pb, MXFDescriptor *descriptor)
{
    ...
    do {
        code = avio_em_r8(pb);
        value = avio_em_r8(pb);
        av_em_log(NULL, AV_LOG_TRACE, "pixel layout: code %#x\n", code);
...
static int mxf_read_generic_descriptor(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    ...
    case 0x3002: /* ContainerDuration */
        descriptor->duration = avio_em_rb64(pb);
        break;
    case 0x3004:
        avio_em_read(pb, descriptor->essence_container_ul, 16);
        break;
    case 0x3005:
        avio_em_read(pb, descriptor->codec_ul, 16);
        break;
    case 0x3006:
        descriptor->linked_track_id = avio_em_rb32(pb);
        break;
    case 0x3201: /* PictureEssenceCoding */
        avio_em_read(pb, descriptor->essence_codec_ul, 16);
        break;
    case 0x3203:
        descriptor->width = avio_em_rb32(pb);
        break;
    case 0x3202:
        descriptor->height = avio_em_rb32(pb);
        break;
    case 0x320C:
        descriptor->frame_layout = avio_em_r8(pb);
        break;
    case 0x320E:
        descriptor->aspect_ratio.num = avio_em_rb32(pb);
        descriptor->aspect_ratio.den = avio_em_rb32(pb);
        break;
    case 0x3212:
        descriptor->field_dominance = avio_em_r8(pb);
        break;
    case 0x3301:
        descriptor->component_depth = avio_em_rb32(pb);
        break;
    case 0x3302:
        descriptor->horiz_subsampling = avio_em_rb32(pb);
        break;
    case 0x3308:
        descriptor->vert_subsampling = avio_em_rb32(pb);
        break;
    case 0x3D03:
        descriptor->sample_rate.num = avio_em_rb32(pb);
        descriptor->sample_rate.den = avio_em_rb32(pb);
        break;
    case 0x3D06: /* SoundEssenceCompression */
        avio_em_read(pb, descriptor->essence_codec_ul, 16);
        break;
    case 0x3D07:
        descriptor->channels = avio_em_rb32(pb);
        break;
    case 0x3D01:
        descriptor->bits_per_sample = avio_em_rb32(pb);
        break;
    case 0x3401:
    ...
    default:
        ...
        if (IS_KLV_KEY(uid, mxf_sony_mpeg4_extradata)) {
            if (descriptor->extradata)
                av_em_log(NULL, AV_LOG_WARNING, "Duplicate sony_mpeg4_extradata\n");
            av_em_free(descriptor->extradata);
            descriptor->extradata_size = 0;
            descriptor->extradata = av_em_alloc(size);
            if (!descriptor->extradata)
                return AVERROR(ENOMEM);
            descriptor->extradata_size = size;
            avio_em_read(pb, descriptor->extradata, size);
        }
        if (IS_KLV_KEY(uid, mxf_jp2k_rsiz)) {
            uint32_t rsiz = avio_em_rb16(pb);
...
static int mxf_read_indirect_value(void *arg, AVEMIOContext *pb, int size)
{
...
static int mxf_read_tagged_value(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    MXFTaggedValue *tagged_value = arg;
    switch (tag){
    case 0x5001:
...
static int mxf_get_sorted_table_segments(MXFContext *mxf, int *nb_sorted_segments, MXFIndexTableSegment ***sorted_segments)
{
    ...
    if (!(unsorted_segments = av_em_calloc(nb_segments, sizeof(*unsorted_segments))) ||
        !(*sorted_segments  = av_em_calloc(nb_segments, sizeof(**sorted_segments)))) {
        av_em_freep(sorted_segments);
        av_em_free(unsorted_segments);
        return AVERROR(ENOMEM);
    }
...
static int mxf_edit_unit_absolute_offset(MXFContext *mxf, MXFIndexTable *index_table, int64_t edit_unit, int64_t *edit_unit_out, int64_t *offset_out, int nag)
{
        ...
        if (edit_unit < s->index_start_position + s->index_duration) {
            ...
            if (s->edit_unit_byte_count)
                ...
                if (index < 0 || index >= s->nb_index_entries) {
                    av_em_log(mxf->fc, AV_LOG_ERROR, "IndexSID %i segment at %"PRId64" IndexEntryArray too small\n",
                ...
                av_em_log(mxf->fc, AV_LOG_ERROR, "IndexSID %i segment at %"PRId64" missing EditUnitByteCount and IndexEntryArray\n",
                       index_table->index_sid, s->index_start_position);
                return AVERROR_INVALIDDATA;
            }
    ...
    if (nag)
        av_em_log(mxf->fc, AV_LOG_ERROR, "failed to map EditUnit %"PRId64" in IndexSID %i to an offset\n", edit_unit, index_table->index_sid);
...
static int mxf_compute_ptses_fake_index(MXFContext *mxf, MXFIndexTable *index_table)
{
    ...
    if (!(index_table->ptses      = av_em_calloc(index_table->nb_ptses, sizeof(int64_t))) ||
        !(index_table->fake_index = av_em_calloc(index_table->nb_ptses, sizeof(AVEMIndexEntry))) ||
        !(index_table->offsets    = av_em_calloc(index_table->nb_ptses, sizeof(int8_t))) ||
        !(flags                   = av_em_calloc(index_table->nb_ptses, sizeof(uint8_t)))) {
        av_em_freep(&index_table->ptses);
        av_em_freep(&index_table->fake_index);
        av_em_freep(&index_table->offsets);
        return AVERROR(ENOMEM);
    }
...
static int mxf_compute_index_tables(MXFContext *mxf)
{
    ...
    if ((ret = mxf_get_sorted_table_segments(mxf, &nb_sorted_segments, &sorted_segments)) ||
        nb_sorted_segments <= 0) {
        av_em_log(mxf->fc, AV_LOG_WARNING, "broken or empty index\n");
        return 0;
    }
    ...
    for (i = 0; i < nb_sorted_segments; i++) {
        if (i == 0 || sorted_segments[i-1]->index_sid != sorted_segments[i]->index_sid)
            ...
            av_em_log(mxf->fc, AV_LOG_ERROR, "found inconsistent BodySID\n");
            ret = AVERROR_INVALIDDATA;
            goto finish_decoding_index;
        }
    }

    mxf->index_tables = av_em_mallocz_array(mxf->nb_index_tables,
                                         sizeof(*mxf->index_tables));
    if (!mxf->index_tables) {
        av_em_log(mxf->fc, AV_LOG_ERROR, "failed to allocate index tables\n");
        ret = AVERROR(ENOMEM);
        goto finish_decoding_index;
    }
    ...
    for (i = j = 0; j < mxf->nb_index_tables; i += mxf->index_tables[j++].nb_segments) {
        ...
        t->segments = av_em_mallocz_array(t->nb_segments,
                                       sizeof(*t->segments));

        if (!t->segments) {
            av_em_log(mxf->fc, AV_LOG_ERROR, "failed to allocate IndexTableSegment"
                   " pointer array\n");
            ret = AVERROR(ENOMEM);
            goto finish_decoding_index;
        }
        ...
        if (sorted_segments[i]->index_start_position)
            av_em_log(mxf->fc, AV_LOG_WARNING, "IndexSID %i starts at EditUnit %"PRId64" - seeking may not work as expected\n",
        ...
        for (k = 0; k < t->nb_segments; k++) {
            ...
            if (t->nb_segments > 1)
                av_em_log(mxf->fc, AV_LOG_WARNING, "IndexSID %i segment %i has zero IndexDuration and there's more than one segment\n",
                       t->index_sid, k);

            if (mxf->fc->nb_streams <= 0) {
                av_em_log(mxf->fc, AV_LOG_WARNING, "no streams?\n");
                break;
            }
...
static int mxf_uid_to_str(UID uid, char **str)
{
    int i;
    char *p;
    p = *str = av_em_mallocz(sizeof(UID) * 2 + 4 + 1);
    if (!p)
        return AVERROR(ENOMEM);
...
static int mxf_umid_to_str(UID ul, UID uid, char **str)
{
    int i;
    char *p;
    p = *str = av_em_mallocz(sizeof(UID) * 4 + 2 + 1);
    if (!p)
        return AVERROR(ENOMEM);
    snprintf(p, 2 + 1, "0x");
    p += 2;
...
static int mxf_add_umid_metadata(AVEMDictionary **pm, const char *key, MXFPackage* package)
{
    char *str;
    int ret;
    if (!package)
    ...
    if ((ret = mxf_umid_to_str(package->package_ul, package->package_uid, &str)) < 0)
        return ret;
    av_em_dict_set(pm, key, str, AV_DICT_DONT_STRDUP_VAL);
    return 0;
}

static int mxf_add_timecode_metadata(AVEMDictionary **pm, const char *key, AVTimecode *tc)
{
    char buf[AV_TIMECODE_STR_SIZE];
    av_em_dict_set(pm, key, av_em_timecode_make_string(tc, buf, 0), 0);
...
static MXFDescriptor* mxf_resolve_multidescriptor(MXFContext *mxf, MXFDescriptor *descriptor, int track_id)
{
    ...
    if (descriptor->type == MultipleDescriptor) {
        for (i = 0; i < descriptor->sub_descriptors_count; i++) {
            ...
            if (!sub_descriptor) {
                av_em_log(mxf->fc, AV_LOG_ERROR, "could not resolve sub descriptor strong ref\n");
                continue;
            }
...
static int mxf_parse_package_comments(MXFContext *mxf, AVEMDictionary **pm, MXFPackage *package)
{
    MXFTaggedValue *tag;
    int size, i;
    char *key = NULL;
    ...
    for (i = 0; i < package->comment_count; i++) {
        ...
        key = av_em_mallocz(size);
        if (!key)
            return AVERROR(ENOMEM);

        snprintf(key, size, "comment_%s", tag->name);
        av_em_dict_set(pm, key, tag->value, AV_DICT_DONT_STRDUP_KEY);
    }
    return 0;
}

static int mxf_parse_physical_source_package(MXFContext *mxf, MXFTrack *source_track, AVEMStream *st)
{
    ...
    for (i = 0; i < source_track->sequence->structural_components_count; i++) {
        ...
        if (physical_package->name && physical_package->name[0])
            av_em_dict_set(&st->metadata, "reel_name", physical_package->name, 0);
        ...
        for (j = 0; j < physical_package->tracks_count; j++) {
            if (!(physical_track = mxf_resolve_strong_ref(mxf, &physical_package->tracks_refs[j], Track))) {
                av_em_log(mxf->fc, AV_LOG_ERROR, "could not resolve source track strong ref\n");
                continue;
            }

            if (!(physical_track->sequence = mxf_resolve_strong_ref(mxf, &physical_track->sequence_ref, Sequence))) {
                av_em_log(mxf->fc, AV_LOG_ERROR, "could not resolve source track sequence strong ref\n");
                continue;
            }
        ...
        if (physical_track->edit_rate.num <= 0 ||
            ...
            physical_track->edit_rate = (AVEMRational){25, 1};
        }
            ...
            for (k = 0; k < physical_track->sequence->structural_components_count; k++) {
                ...
                start_position = av_em_rescale_q(sourceclip->start_position,
                                              physical_track->edit_rate,
                                              source_track->edit_rate);

                if (av_em_timecode_init(&tc, mxf_tc->rate, flags, start_position + mxf_tc->start_frame, mxf->fc) == 0) {
...
static int mxf_parse_structural_metadata(MXFContext *mxf)
{
    ...
    av_em_log(mxf->fc, AV_LOG_TRACE, "metadata sets count %d\n", mxf->metadata_sets_count);
    ...
    if (!material_package) {
        av_em_log(mxf->fc, AV_LOG_ERROR, "no material package found\n");
        return AVERROR_INVALIDDATA;
    }

    mxf_add_umid_metadata(&mxf->fc->metadata, "material_package_umid", material_package);
    if (material_package->name && material_package->name[0])
        av_em_dict_set(&mxf->fc->metadata, "material_package_name", material_package->name, 0);
    ...
    for (i = 0; i < material_package->tracks_count; i++) {
        ...
        if (!(material_track = mxf_resolve_strong_ref(mxf, &material_package->tracks_refs[i], Track))) {
            av_em_log(mxf->fc, AV_LOG_ERROR, "could not resolve material track strong ref\n");
            continue;
        }
        ...
        if ((component = mxf_resolve_strong_ref(mxf, &material_track->sequence_ref, TimecodeComponent))) {
            ...
            flags = mxf_tc->drop_frame == 1 ? AV_TIMECODE_FLAG_DROPFRAME : 0;
            if (av_em_timecode_init(&tc, mxf_tc->rate, flags, mxf_tc->start_frame, mxf->fc) == 0) {
                mxf_add_timecode_metadata(&mxf->fc->metadata, "timecode", &tc);
            }
        }

        if (!(material_track->sequence = mxf_resolve_strong_ref(mxf, &material_track->sequence_ref, Sequence))) {
            av_em_log(mxf->fc, AV_LOG_ERROR, "could not resolve material track sequence strong ref\n");
            continue;
        }
        ...
        for (j = 0; j < material_track->sequence->structural_components_count; j++) {
            ...
            if (!source_package) {
                av_em_log(mxf->fc, AV_LOG_TRACE, "material track %d: no corresponding source package found\n", material_track->track_id);
                break;
            }
            ...
            if (!source_track) {
                av_em_log(mxf->fc, AV_LOG_ERROR, "material track %d: no corresponding source track found\n", material_track->track_id);
                break;
            }
        }
        ...
        if (memcmp(material_track->sequence->data_definition_ul, source_track->sequence->data_definition_ul, 16)) {
            av_em_log(mxf->fc, AV_LOG_ERROR, "material track %d: DataDefinition mismatch\n", material_track->track_id);
            continue;
        }

        st = avformat_em_new_stream(mxf->fc, NULL);
        if (!st) {
            av_em_log(mxf->fc, AV_LOG_ERROR, "could not allocate stream\n");
            ret = AVERROR(ENOMEM);
            goto fail_and_free;
        }
        ...
        if (material_track->edit_rate.num <= 0 ||
            ...
            material_track->edit_rate = (AVEMRational){25, 1};
        }
        avpriv_em_set_pts_info(st, 64, material_track->edit_rate.den, material_track->edit_rate.num);
        ...
        if (!descriptor) {
            av_em_log(mxf->fc, AV_LOG_INFO, "source track %d: stream %d, no descriptor found\n", source_track->track_id, st->index);
            continue;
        }
        ...
        if (IS_KLV_KEY(essence_container_ul, mxf_encrypted_essence_container)) {
            av_em_log(mxf->fc, AV_LOG_INFO, "broken encrypted mxf file\n");
        ...
        st->codecpar->codec_id = (enum AVEMCodecID)codec_ul->id;
        if (st->codecpar->codec_id == AV_CODEC_ID_NONE) {
            codec_ul = mxf_get_codec_ul(ff_mxf_codec_uls, &descriptor->codec_ul);
            st->codecpar->codec_id = (enum AVEMCodecID)codec_ul->id;
        }

        av_em_log(mxf->fc, AV_LOG_VERBOSE, "%s: Universal Label: ",
               avcodec_em_get_name(st->codecpar->codec_id));
        for (k = 0; k < 16; k++) {
            av_em_log(mxf->fc, AV_LOG_VERBOSE, "%.2x",
                   descriptor->essence_codec_ul[k]);
            if (!(k+1 & 19) || k == 5)
                av_em_log(mxf->fc, AV_LOG_VERBOSE, ".");
        }
        av_em_log(mxf->fc, AV_LOG_VERBOSE, "\n");

        mxf_add_umid_metadata(&st->metadata, "file_package_umid", source_package);
        if (source_package->name && source_package->name[0])
            av_em_dict_set(&st->metadata, "file_package_name", source_package->name, 0);
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            switch (descriptor->frame_layout) {
                ...
                case OneField:
                    ...
                    av_em_log(mxf->fc, AV_LOG_INFO, "OneField frame layout isn't currently supported\n");
                ...
                case SeparateFields:
                    ...
                    case MXF_BFF:
                        ...
                        break;
                    default:
                        avpriv_em_request_sample(mxf->fc,
                    ...
                    break;
                default:
                    av_em_log(mxf->fc, AV_LOG_INFO, "Unknown frame layout type: %d\n", descriptor->frame_layout);
            }
            ...
            if (material_track->sequence->origin) {
                av_em_dict_set_int(&st->metadata, "material_track_origin", material_track->sequence->origin, 0);
            }
            if (source_track->sequence->origin) {
                av_em_dict_set_int(&st->metadata, "source_track_origin", source_track->sequence->origin, 0);
            }
            ...
            if (st->codecpar->codec_id == AV_CODEC_ID_NONE || (st->codecpar->codec_id == AV_CODEC_ID_PCM_ALAW && (enum AVEMCodecID)container_ul->id != AV_CODEC_ID_NONE))
                st->codecpar->codec_id = (enum AVEMCodecID)container_ul->id;
            st->codecpar->channels = descriptor->channels;
            st->codecpar->bits_per_coded_sample = descriptor->bits_per_sample;

            if (descriptor->sample_rate.den > 0) {
                st->codecpar->sample_rate = descriptor->sample_rate.num / descriptor->sample_rate.den;
                avpriv_em_set_pts_info(st, 64, descriptor->sample_rate.den, descriptor->sample_rate.num);
            } else {
                av_em_log(mxf->fc, AV_LOG_WARNING, "invalid sample rate (%d/%d) "
                       "found for stream #%d, time base forced to 1/48000\n",
                       descriptor->sample_rate.num, descriptor->sample_rate.den,
                       st->index);
                avpriv_em_set_pts_info(st, 64, 1, 48000);
            }

            /* if duration is set, rescale it from EditRate to SampleRate */
            if (st->duration != AV_NOPTS_VALUE)
                st->duration = av_em_rescale_q(st->duration,
                                            av_em_inv_q(material_track->edit_rate),
                                            st->time_base);
            ...
            if (codec_id >= 0 &&
                ...
                av_em_dict_set(&st->metadata, "data_type",
                            mxf_data_essence_descriptor[codec_id], 0);
            }
        }
        if (descriptor->extradata) {
            if (!em_alloc_extradata(st->codecpar, descriptor->extradata_size)) {
...
#define SET_STR_METADATA(pb, name, str) do { \
    if ((ret = mxf_read_utf16be_string(pb, size, &str)) < 0) \
        return ret; \
    av_em_dict_set(&s->metadata, name, str, AV_DICT_DONT_STRDUP_VAL); \
} while (0)

#define SET_UID_METADATA(pb, name, var, str) do { \
    avio_em_read(pb, var, 16); \
...
static int mxf_read_identification_metadata(void *arg, AVEMIOContext *pb, int tag, int size, UID _uid, int64_t klv_offset)
{
    MXFContext *mxf = arg;
    AVEMFormatContext *s = mxf->fc;
    int ret;
    UID uid = { 0 };
    char *str = NULL;
    uint64_t ts;
...
static int mxf_read_preface_metadata(void *arg, AVEMIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    MXFContext *mxf = arg;
    AVEMFormatContext *s = mxf->fc;
    int ret;
...
{
    AVEMIOContext *pb = mxf->fc->pb;
    MXFMetadataSet *ctx = ctx_size ? av_em_mallocz(ctx_size) : mxf;
    uint64_t klv_end = avio_em_tell(pb) + klv->length;

    if (!ctx)
        return AVERROR(ENOMEM);
    mxf_metadataset_init(ctx, type);
    while (avio_em_tell(pb) + 4 < klv_end && !avio_em_feof(pb)) {
        int ret;
        int tag = avio_em_rb16(pb);
        int size = avio_em_rb16(pb); /* KLV specified by 0x53 */
        uint64_t next = avio_em_tell(pb) + size;
        UID uid = {0};

        av_em_log(mxf->fc, AV_LOG_TRACE, "local tag %#04x size %d\n", tag, size);
        if (!size) { /* ignore empty tag, needed for some files with empty UMID tag */
            av_em_log(mxf->fc, AV_LOG_ERROR, "local tag %#04x with 0 size\n", tag);
            continue;
        }
        if (tag > 0x7FFF) { /* dynamic tag */
            ...
            for (i = 0; i < mxf->local_tags_count; i++) {
                ...
                if (local_tag == tag) {
                    ...
                    av_em_log(mxf->fc, AV_LOG_TRACE, "local tag %#04x\n", local_tag);
                    PRINT_KEY(mxf->fc, "uid", uid);
                }
            }
        }
        if (ctx_size && tag == 0x3C0A) {
            avio_em_read(pb, ctx->uid, 16);
        ...
        if (avio_em_tell(pb) > klv_end) {
            ...
            av_em_log(mxf->fc, AV_LOG_ERROR,
                   ...
                   tag, klv->offset);
            return AVERROR_INVALIDDATA;
        } else if (avio_em_tell(pb) <= next)   /* only seek forward, else this can loop for a long time */
                                     ...
                                     int ctx_size, enum MXFMetadataSetType type)
{
    ...
    if (klv.key[5] == 0x53) {
        ...
        uint64_t next = avio_em_tell(s->pb) + klv.length;
        res = read(mxf, s->pb, 0, klv.length, klv.key, klv.offset);

        /* only seek forward, else this can loop for a long time */
        if (avio_em_tell(s->pb) > next) {
            av_em_log(s, AV_LOG_ERROR, "read past end of KLV @ %#"PRIx64"\n",
                   klv.offset);
            return AVERROR_INVALIDDATA;
        }

        avio_em_seek(s->pb, next, SEEK_SET);
    }
    if (res < 0) {
        av_em_log(s, AV_LOG_ERROR, "error reading header metadata\n");
        return res;
    }
    return 0;
}
...
static int mxf_seek_to_previous_partition(MXFContext *mxf)
{
    ...
    avio_em_seek(pb, mxf->run_in + mxf->current_partition->previous_partition, SEEK_SET);
    mxf->current_partition = NULL;

    av_em_log(mxf->fc, AV_LOG_TRACE, "seeking to previous partition\n");

    /* Make sure this is actually a PartitionPack, and if so parse it.
     * See deadlock2.mxf
     */
    if ((ret = klv_read_packet(&klv, pb)) < 0) {
        av_em_log(mxf->fc, AV_LOG_ERROR, "failed to read PartitionPack KLV\n");
        return ret;
    }

    if (!mxf_is_partition_pack_key(klv.key)) {
        av_em_log(mxf->fc, AV_LOG_ERROR, "PreviousPartition @ %" PRIx64 " isn't a PartitionPack\n", klv.offset);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (klv.offset >= current_partition_ofs) {
        av_em_log(mxf->fc, AV_LOG_ERROR, "PreviousPartition for PartitionPack @ %"
...
static int mxf_parse_handle_essence(MXFContext *mxf)
{
    ...
    if (mxf->parsing_backward) {
        ...
        if (!mxf->footer_partition) {
            av_em_log(mxf->fc, AV_LOG_TRACE, "no FooterPartition\n");
            return 0;
        }

        av_em_log(mxf->fc, AV_LOG_TRACE, "seeking to FooterPartition\n");

        /* remember where we were so we don't end up seeking further back than this */
        mxf->last_forward_tell = avio_em_tell(pb);

        if (!pb->seekable) {
            av_em_log(mxf->fc, AV_LOG_INFO, "file is not seekable - not parsing FooterPartition\n");
            return -1;
        }

        /* seek to FooterPartition and parse backward */
        if ((ret = avio_em_seek(pb, mxf->run_in + mxf->footer_partition, SEEK_SET)) < 0) {
            av_em_log(mxf->fc, AV_LOG_ERROR,
...
static int is_pcm(enum AVEMCodecID codec_id)
{
 ...
 */
static void mxf_handle_small_eubc(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
...
static int mxf_handle_missing_index_segment(MXFContext *mxf)
{
    ...
    if (!(segment = av_em_mallocz(sizeof(*segment))))
        return AVERROR(ENOMEM);
    ...
    segment->edit_unit_byte_count = (av_em_get_bits_per_sample(st->codecpar->codec_id) * st->codecpar->channels) >> 3;
...
static void mxf_read_random_index_pack(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    uint32_t length;
    ...
    avio_em_seek(s->pb, file_size - 4, SEEK_SET);
    length = avio_em_rb32(s->pb);

    if (length < min_rip_length || length > max_rip_length)
        goto end;
    avio_em_seek(s->pb, file_size - length, SEEK_SET);
    if (klv_read_packet(&klv, s->pb) < 0 ||
        !IS_KLV_KEY(klv.key, mxf_random_index_pack_key) ||
        klv.length != length - 20)
        goto end;

    avio_em_skip(s->pb, klv.length - 12);
    mxf->footer_partition = avio_em_rb64(s->pb);

    /* sanity check */
    if (mxf->run_in + mxf->footer_partition >= file_size) {
        av_em_log(s, AV_LOG_WARNING, "bad FooterPartition in RIP - ignoring\n");
        mxf->footer_partition = 0;
    }

end:
    avio_em_seek(s->pb, mxf->run_in, SEEK_SET);
}

static int mxf_read_header(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    KLVPacket klv;
    ...
    if (!mxf_read_sync(s->pb, mxf_header_partition_pack_key, 14)) {
        av_em_log(s, AV_LOG_ERROR, "could not find header partition pack key\n");
        return AVERROR_INVALIDDATA;
    }
    avio_em_seek(s->pb, -14, SEEK_CUR);
    mxf->fc = s;
    mxf->run_in = avio_em_tell(s->pb);
    ...
    while (!avio_em_feof(s->pb)) {
        ...
        av_em_log(s, AV_LOG_TRACE, "size %"PRIu64" offset %#"PRIx64"\n", klv.length, klv.offset);
        if (IS_KLV_KEY(klv.key, mxf_encrypted_triplet_key) ||
            ...
            if (!mxf->current_partition) {
                av_em_log(mxf->fc, AV_LOG_ERROR, "found essence prior to first PartitionPack\n");
                return AVERROR_INVALIDDATA;
            }
            ...
            if (!mxf->current_partition->essence_offset) {
                ...
                if (mxf->op == OPAtom) {
                    ...
                    */
                    mxf->current_partition->essence_offset = avio_em_tell(s->pb);
        ...
        if (!metadata->read) {
            av_em_log(s, AV_LOG_VERBOSE, "Dark key " PRIxUID "\n",
                            UID_ARG(klv.key));
            avio_em_skip(s->pb, klv.length);
        }
    }
    /* FIXME avoid seek */
    if (!essence_offset)  {
        av_em_log(s, AV_LOG_ERROR, "no essence\n");
        ret = AVERROR_INVALIDDATA;
        goto fail;
    }
    avio_em_seek(s->pb, essence_offset, SEEK_SET);
    ...
    if (mxf->nb_index_tables > 1) {
        ...
        av_em_log(mxf->fc, AV_LOG_INFO, "got %i index tables - only the first one (IndexSID %i) will be used\n",
               mxf->nb_index_tables, mxf->index_tables[0].index_sid);
    } else if (mxf->nb_index_tables == 0 && mxf->op == OPAtom) {
        av_em_log(mxf->fc, AV_LOG_ERROR, "cannot demux OPAtom without an index\n");
        ret = AVERROR_INVALIDDATA;
        goto fail;
    }
                                    ...
                                    uint64_t *sample_count)
{
    ...
    AVEMStream *st = mxf->fc->streams[stream_index];
    MXFTrack *track = st->priv_data;
    AVEMRational time_base = av_em_inv_q(track->edit_rate);
    AVEMRational sample_rate = av_em_inv_q(st->time_base);
    ...
    if (!spf) {
        ...
        *sample_count = av_em_q2d(av_em_mul_q((AVEMRational){mxf->current_edit_unit, 1},
                                        av_em_mul_q(sample_rate, time_base)));
        if (remainder)
            av_em_log(mxf->fc, AV_LOG_WARNING,
...
static int mxf_set_audio_pts(MXFContext *mxf, AVEMCodecParameters *par,
                             AVEMPacket *pkt)
{
    ...
    if (!bits_per_sample)
        bits_per_sample = av_em_get_bits_per_sample(par->codec_id);
...
static int mxf_read_packet_old(AVEMFormatContext *s, AVEMPacket *pkt)
{
    KLVPacket klv;
    MXFContext *mxf = s->priv_data;
    int ret;
    ...
    while ((ret = klv_read_packet(&klv, s->pb)) == 0) {
        ...
        if (IS_KLV_KEY(klv.key, mxf_encrypted_triplet_key)) {
            ...
            if (ret < 0) {
                av_em_log(s, AV_LOG_ERROR, "invalid encoded triplet\n");
                return ret;
            }
            return 0;
        }
        if (IS_KLV_KEY(klv.key, mxf_essence_element_key) ||
            ...
            next_klv = avio_em_tell(s->pb) + klv.length;
            ...
            if (next_ofs >= 0 && next_klv > next_ofs) {
                ...
                avpriv_em_request_sample(s,
                                      ...
                                      "next edit unit",
                                      mxf->current_edit_unit);
                klv.length = next_ofs - avio_em_tell(s->pb);
            }
            ...
            if (klv.key[12] == 0x06 && klv.key[13] == 0x01 && klv.key[14] == 0x10) {
                ret = mxf_get_d10_aes3_packet(s->pb, s->streams[index],
                                              pkt, klv.length);
                if (ret < 0) {
                    av_em_log(s, AV_LOG_ERROR, "error reading D-10 aes3 frame\n");
                    return ret;
                }
            } else {
                ret = av_em_get_packet(s->pb, pkt, klv.length);
                if (ret < 0)
                    return ret;
            }
            pkt->stream_index = index;
            ...
            avio_em_seek(s->pb, next_klv, SEEK_SET);
    ...
    return avio_em_feof(s->pb) ? AVERROR_EOF : ret;
}

static int mxf_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MXFContext *mxf = s->priv_data;
    int ret, size;
    ...
    if ((ret = mxf_edit_unit_absolute_offset(mxf, t, mxf->current_edit_unit + edit_units, NULL, &next_pos, 0)) < 0 &&
        ...
        av_em_log(s, AV_LOG_ERROR, "unable to compute the size of the last packet\n");
        return AVERROR_INVALIDDATA;
    }

    if ((size = next_pos - pos) <= 0) {
        av_em_log(s, AV_LOG_ERROR, "bad size: %i\n", size);
        return AVERROR_INVALIDDATA;
    }

    if ((ret64 = avio_em_seek(s->pb, pos, SEEK_SET)) < 0)
        return ret64;

    if ((size = av_em_get_packet(s->pb, pkt, size)) < 0)
        return size;
...
static int mxf_read_close(AVEMFormatContext *s)
{
    ...
    av_em_freep(&mxf->partitions);
    av_em_freep(&mxf->metadata_sets);
    av_em_freep(&mxf->aesc);
    av_em_freep(&mxf->local_tags);

    if (mxf->index_tables) {
        for (i = 0; i < mxf->nb_index_tables; i++) {
            av_em_freep(&mxf->index_tables[i].segments);
            av_em_freep(&mxf->index_tables[i].ptses);
            av_em_freep(&mxf->index_tables[i].fake_index);
            av_em_freep(&mxf->index_tables[i].offsets);
        }
    }
    av_em_freep(&mxf->index_tables);

    return 0;
}

static int mxf_probe(AVEMProbeData *p) {
...
/* XXX: use MXF Index */
static int mxf_read_seek(AVEMFormatContext *s, int stream_index, int64_t sample_time, int flags)
{
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO)
        sample_time = av_em_rescale_q(sample_time, st->time_base,
                                   av_em_inv_q(source_track->edit_rate));
    ...
    if (sample_time < 0)
        sample_time = 0;
    seconds = av_em_rescale(sample_time, st->time_base.num, st->time_base.den);

    seekpos = avio_em_seek(s->pb, (s->bit_rate * seconds) >> 3, SEEK_SET);
    if (seekpos < 0)
        return seekpos;

    em_update_cur_dts(s, st, sample_time);
    mxf->current_edit_unit = sample_time;
    } else {
        t = &mxf->index_tables[0];

        /* clamp above zero, else em_index_search_timestamp() returns negative
        ...
        if (t->fake_index) {
            ...
            if ((sample_time = em_index_search_timestamp(t->fake_index, t->nb_ptses, sample_time, flags)) < 0)
                return sample_time;
        ...
        em_update_cur_dts(s, st, sample_time);
        mxf->current_edit_unit = sample_time;
        avio_em_seek(s->pb, seekpos, SEEK_SET);
    }

    // Update all tracks sample count
    for (i = 0; i < s->nb_streams; i++) {
        AVEMStream *cur_st = s->streams[i];
...
AVEMInputFormat ff_mxf_demuxer = {
    .name           = "mxf",
