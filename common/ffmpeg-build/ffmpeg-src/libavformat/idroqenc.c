...
static int roq_write_header(struct AVEMFormatContext *s)
{
    uint8_t header[] = {
...
// set the actual fps
    for(n=0;n<s->nb_streams;n++) {
        if (s->streams[n]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (s->streams[n]->avg_frame_rate.den != 1) {
                av_em_log(s, AV_LOG_ERROR, "Frame rate must be integer\n");
                return AVERROR(EINVAL);
            }

            if ((fps=s->streams[n]->avg_frame_rate.num) > 255) {
                av_em_log(s, AV_LOG_ERROR, "Frame rate may not exceed 255fps\n");
                return AVERROR(EINVAL);
            }

            if (fps != 30) {
                av_em_log(s, AV_LOG_WARNING, "For vintage compatibility fps must be 30\n");
            }
    ...
    avio_em_write(s->pb, header, 8);
    avio_em_flush(s->pb);

    return 0;
}

AVEMOutputFormat ff_roq_muxer = {
    .name         = "roq",
