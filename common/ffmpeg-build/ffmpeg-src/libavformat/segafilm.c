...
static int film_probe(AVEMProbeData *p)
{
...
static int film_read_close(AVEMFormatContext *s)
{
    FilmDemuxContext *film = s->priv_data;

    av_em_freep(&film->sample_table);

    return 0;
}

static int film_read_header(AVEMFormatContext *s)
{
    FilmDemuxContext *film = s->priv_data;
    ...
    if (avio_em_read(pb, scratch, 16) != 16)
        return AVERROR(EIO);
    ...
    if (film->version == 0) {
        ...
        if (avio_em_read(pb, scratch, 20) != 20)
            return AVERROR(EIO);
        ...
        if (avio_em_read(pb, scratch, 32) != 32)
            return AVERROR(EIO);
    ...
    if (film->video_type) {
        ...
        if (film->video_type == AV_CODEC_ID_RAWVIDEO) {
            if (scratch[20] == 24) {
                ...
                av_em_log(s, AV_LOG_ERROR, "raw video is using unhandled %dbpp\n", scratch[20]);
                return -1;
            }
        }
    }
    ...
    if(film->sample_count >= UINT_MAX / sizeof(film_sample))
        return -1;
    film->sample_table = av_em_malloc_array(film->sample_count, sizeof(film_sample));
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
            avpriv_em_set_pts_info(st, 33, 1, film->base_clock);
        else
            avpriv_em_set_pts_info(st, 64, 1, film->audio_samplerate);
    }
    ...
    for (i = 0; i < film->sample_count; i++) {
        ...
        if (AV_RB32(&scratch[8]) == 0xFFFFFFFF) {
            ...
            video_frame_counter++;
            if (film->video_type)
                av_em_add_index_entry(s->streams[film->video_stream_index],
                                   film->sample_table[i].sample_offset,
...
static int film_read_packet(AVEMFormatContext *s,
                            AVEMPacket *pkt)
{
    FilmDemuxContext *film = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    avio_em_seek(pb, sample->sample_offset, SEEK_SET);

    ret = av_em_get_packet(pb, pkt, sample->sample_size);
...
static int film_read_seek(AVEMFormatContext *s, int stream_index, int64_t timestamp, int flags)
{
    FilmDemuxContext *film = s->priv_data;
    AVEMStream *st = s->streams[stream_index];
    int64_t pos;
    int ret = av_em_index_search_timestamp(st, timestamp, flags);
    if (ret < 0)
        return ret;

    pos = avio_em_seek(s->pb, st->index_entries[ret].pos, SEEK_SET);
    if (pos < 0)
        return pos;
...
AVEMInputFormat ff_segafilm_demuxer = {
    .name           = "film_cpk",
