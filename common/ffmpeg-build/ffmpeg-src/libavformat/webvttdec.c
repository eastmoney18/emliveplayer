...
static int webvtt_probe(AVEMProbeData *p)
{
...
static int webvtt_read_header(AVEMFormatContext *s)
{
    WebVTTContext *webvtt = s->priv_data;
    AVEMBPrint header, cue;
    int res = 0;
    ...
    av_em_bprint_init(&cue,    0, AV_BPRINT_SIZE_UNLIMITED);
...
#define SET_SIDE_DATA(name, type) do {                                  \
    if (name##_len) {                                                   \
        uint8_t *buf = av_em_packet_new_side_data(sub, type, name##_len);  \
        if (!buf) {                                                     \
            res = AVERROR(ENOMEM);                                      \
            goto end;                                                   \
        }                                                               \
    ...
    av_em_bprint_finalize(&cue,    NULL);
    av_em_bprint_finalize(&header, NULL);
    return res;
}

static int webvtt_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    WebVTTContext *webvtt = s->priv_data;
    return ff_subtitles_queue_read_packet(&webvtt->q, pkt);
}

static int webvtt_read_seek(AVEMFormatContext *s, int stream_index,
...
static int webvtt_read_close(AVEMFormatContext *s)
{
    WebVTTContext *webvtt = s->priv_data;
...
static const AVEMClass webvtt_demuxer_class = {
...
AVEMInputFormat ff_webvtt_demuxer = {
    .name           = "webvtt",
