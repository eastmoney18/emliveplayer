...
static const AVEMClass rtp_class = {
    .class_name = "rtp",
...
int ff_rtp_set_remote_url(EMURLContext *h, const char *uri)
{
    ...
    av_em_url_split(NULL, 0, NULL, 0, hostname, sizeof(hostname), &port,
                 path, sizeof(path), uri);
    rtcp_port = port + 1;

    p = strchr(uri, '?');
    if (p) {
        if (av_em_find_info_tag(buf, sizeof(buf), "rtcpport", p)) {
            rtcp_port = strtol(buf, NULL, 10);
        }
    }

    em_url_join(buf, sizeof(buf), "udp", NULL, hostname, port, "%s", path);
    em_udp_set_remote_url(s->rtp_hd, buf);

    em_url_join(buf, sizeof(buf), "udp", NULL, hostname, rtcp_port, "%s", path);
    em_udp_set_remote_url(s->rtcp_hd, buf);
    return 0;
}
                                         ...
                                         int type, int family, int flags)
{
    ...
    if ((error = getaddrinfo(hostname, service, &hints, &res))) {
        res = NULL;
        av_em_log(NULL, AV_LOG_ERROR, "rtp_resolve_host: %s\n", gai_strerror(error));
    }
...
static av_printf_format(3, 4) void url_add_option(char *buf, int buf_size, const char *fmt, ...)
{
    ...
    va_start(ap, fmt);
    if (strchr(buf, '?'))
        av_em_strlcat(buf, "&", buf_size);
    else
        av_em_strlcat(buf, "?", buf_size);
    vsnprintf(buf1, sizeof(buf1), fmt, ap);
    av_em_strlcat(buf, buf1, buf_size);
    va_end(ap);
}
...
{
    em_url_join(buf, buf_size, "udp", NULL, hostname, port, NULL);
    if (local_port >= 0)
...
static void rtp_parse_addr_list(EMURLContext *h, char *buf,
                                ...
                                int *address_list_size_ptr)
{
    ...
    while (p && p[0]) {
        ...
        if (ai) {
            source_addr = av_em_mallocz(sizeof(struct sockaddr_storage));
            if (!source_addr) {
                freeaddrinfo(ai);
                break;
            }
            ...
            av_em_log(h, AV_LOG_WARNING, "Unable to resolve %s\n", p);
        }
...
static int rtp_open(EMURLContext *h, const char *uri, int flags)
{
    RTPContext *s = h->priv_data;
    int rtp_port;
    ...
    av_em_url_split(NULL, 0, NULL, 0, hostname, sizeof(hostname), &rtp_port,
                 path, sizeof(path), uri);
    /* extract parameters */
    if (s->rtcp_port < 0)
    ...
    if (p) {
        ...
        if (av_em_find_info_tag(buf, sizeof(buf), "localrtpport", p)) {
            s->local_rtpport = strtol(buf, NULL, 10);
        }
        if (av_em_find_info_tag(buf, sizeof(buf), "localrtcpport", p)) {
        ...
        if (av_em_find_info_tag(buf, sizeof(buf), "write_to_source", p)) {
        ...
        if (av_em_find_info_tag(buf, sizeof(buf), "sources", p)) {
            av_em_strlcpy(include_sources, buf, sizeof(include_sources));
        ...
        if (av_em_find_info_tag(buf, sizeof(buf), "block", p)) {
            av_em_strlcpy(exclude_sources, buf, sizeof(exclude_sources));
    ...
    for (i = 0; i < max_retry_count; i++) {
        build_udp_url(s, buf, sizeof(buf),
                      ...
                      sources, block);
        if (ffurl_em_open_whitelist(&s->rtp_hd, buf, flags, &h->interrupt_callback,
                                 NULL, h->protocol_whitelist, h->protocol_blacklist, h) < 0)
            goto fail;
        s->local_rtpport = em_udp_get_local_port(s->rtp_hd);
        ...
        if (s->local_rtcpport < 0) {
            ...
            build_udp_url(s, buf, sizeof(buf),
                          ...
                          sources, block);
            if (ffurl_em_open_whitelist(&s->rtcp_hd, buf, rtcpflags,
        ...
        build_udp_url(s, buf, sizeof(buf),
                      hostname, s->rtcp_port, s->local_rtcpport,
                      sources, block);
        if (ffurl_em_open_whitelist(&s->rtcp_hd, buf, rtcpflags, &h->interrupt_callback,
                                 NULL, h->protocol_whitelist, h->protocol_blacklist, h) < 0)
            goto fail;
        break;
    }
    ...
    /* just to ease handle access. XXX: need to suppress direct handle
       access */
    s->rtp_fd = ffurl_em_get_file_handle(s->rtp_hd);
    s->rtcp_fd = ffurl_em_get_file_handle(s->rtcp_hd);
 ...
 fail:
    if (s->rtp_hd)
        ffurl_em_close(s->rtp_hd);
    if (s->rtcp_hd)
        ffurl_em_close(s->rtcp_hd);
    return AVERROR(EIO);
}

static int rtp_read(EMURLContext *h, uint8_t *buf, int size)
{
    RTPContext *s = h->priv_data;
    int len, n, i;
...
static int rtp_write(EMURLContext *h, const uint8_t *buf, int size)
{
    RTPContext *s = h->priv_data;
    int ret;
    EMURLContext *hd;
    ...
    if ((buf[0] & 0xc0) != (RTP_VERSION << 6))
        av_em_log(h, AV_LOG_WARNING, "Data doesn't look like RTP packets, "
    ...
    ret = ffurl_em_write(hd, buf, size);
    return ret;
}

static int rtp_close(EMURLContext *h)
{
    RTPContext *s = h->priv_data;
    int i;

    for (i = 0; i < s->nb_ssm_include_addrs; i++)
        av_em_freep(&s->ssm_include_addrs[i]);
    av_em_freep(&s->ssm_include_addrs);
    for (i = 0; i < s->nb_ssm_exclude_addrs; i++)
        av_em_freep(&s->ssm_exclude_addrs[i]);
    av_em_freep(&s->ssm_exclude_addrs);
...
int ff_rtp_get_local_rtp_port(EMURLContext *h)
{
    RTPContext *s = h->priv_data;
    return em_udp_get_local_port(s->rtp_hd);
}
...
int ff_rtp_get_local_rtcp_port(EMURLContext *h)
{
    RTPContext *s = h->priv_data;
    return em_udp_get_local_port(s->rtcp_hd);
}

static int rtp_get_file_handle(EMURLContext *h)
{
    RTPContext *s = h->priv_data;
    return s->rtp_fd;
}

static int rtp_get_multi_file_handle(EMURLContext *h, int **handles,
                                     int *numhandles)
{
    RTPContext *s = h->priv_data;
    int *hs       = *handles = av_em_alloc(sizeof(**handles) * 2);
    if (!hs)
...
const EMURLProtocol em_rtp_protocol = {
    .name                      = "rtp",
    .url_open                  = rtp_open,
    .url_read                  = rtp_read,
    .url_write                 = rtp_write,
