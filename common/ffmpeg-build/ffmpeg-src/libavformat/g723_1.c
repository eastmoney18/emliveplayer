...
static av_cold int g723_1_init(AVEMFormatContext *s)
{
    AVEMStream *st;
...
static int g723_1_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int size, byte, ret;
    ...
    ret = avio_em_read(s->pb, pkt->data + 1, size - 1);
    if (ret < size - 1) {
...
AVEMInputFormat ff_g723_1_demuxer = {
    .name        = "g723_1",
