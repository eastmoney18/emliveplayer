...
static int load_ipmovie_packet(IPMVEContext *s, AVEMIOContext *pb,
    AVEMPacket *pkt) {
    ...
    if (s->audio_chunk_offset && s->audio_channels && s->audio_bits) {
        if (s->audio_type == AV_CODEC_ID_NONE) {
            av_em_log(s->avf, AV_LOG_ERROR, "Can not read audio packet before"
        ...
        avio_em_seek(pb, s->audio_chunk_offset, SEEK_SET);
        s->audio_chunk_offset = 0;

        if (s->audio_chunk_size != av_em_get_packet(pb, pkt, s->audio_chunk_size))
            return CHUNK_EOF;
        ...
        av_em_log(s->avf, AV_LOG_TRACE, "sending audio frame with pts %"PRId64" (%d audio frames)\n",
                pkt->pts, s->audio_frame_count);
        ...
        if (av_em_new_packet(pkt, 2 + s->decode_map_chunk_size + s->video_chunk_size))
            return CHUNK_NOMEM;

        if (s->has_palette) {
            uint8_t *pal;

            pal = av_em_packet_new_side_data(pkt, AV_PKT_DATA_PALETTE,
                                          AVPALETTE_SIZE);
            if (pal) {
        ...
        if (s->changed) {
            em_add_param_change(pkt, 0, 0, 0, s->video_width, s->video_height);
            s->changed = 0;
        }
        pkt->pos= s->decode_map_chunk_offset;
        avio_em_seek(pb, s->decode_map_chunk_offset, SEEK_SET);
        s->decode_map_chunk_offset = 0;

        AV_WL16(pkt->data, s->decode_map_chunk_size);
        if (avio_em_read(pb, pkt->data + 2, s->decode_map_chunk_size) !=
            s->decode_map_chunk_size) {
            av_em_packet_unref(pkt);
            return CHUNK_EOF;
        }

        avio_em_seek(pb, s->video_chunk_offset, SEEK_SET);
        ...
        av_em_log(s->avf, AV_LOG_TRACE, "sending video frame with pts %"PRId64"\n", pkt->pts);
        ...
        avio_em_seek(pb, s->next_chunk_offset, SEEK_SET);
...
static int init_audio(AVEMFormatContext *s)
{
    IPMVEContext *ipmovie = s->priv_data;
    AVEMStream *st = avformat_em_new_stream(s, NULL);
    if (!st)
        return AVERROR(ENOMEM);
    avpriv_em_set_pts_info(st, 32, 1, ipmovie->audio_sample_rate);
...
static int process_ipmovie_chunk(IPMVEContext *s, AVEMIOContext *pb,
    AVEMPacket *pkt)
{
    ...
    if (avio_em_feof(pb))
        return CHUNK_EOF;
    if (avio_em_read(pb, chunk_preamble, CHUNK_PREAMBLE_SIZE) !=
        CHUNK_PREAMBLE_SIZE)
        return CHUNK_BAD;
    chunk_size = AV_RL16(&chunk_preamble[0]);
    chunk_type = AV_RL16(&chunk_preamble[2]);

    av_em_log(s->avf, AV_LOG_TRACE, "chunk type 0x%04X, 0x%04X bytes: ", chunk_type, chunk_size);

    switch (chunk_type) {

    case CHUNK_INIT_AUDIO:
        av_em_log(s->avf, AV_LOG_TRACE, "initialize audio\n");
        break;

    case CHUNK_AUDIO_ONLY:
        av_em_log(s->avf, AV_LOG_TRACE, "audio only\n");
        break;

    case CHUNK_INIT_VIDEO:
        av_em_log(s->avf, AV_LOG_TRACE, "initialize video\n");
        break;

    case CHUNK_VIDEO:
        av_em_log(s->avf, AV_LOG_TRACE, "video (and audio)\n");
        break;

    case CHUNK_SHUTDOWN:
        av_em_log(s->avf, AV_LOG_TRACE, "shutdown\n");
        break;

    case CHUNK_END:
        av_em_log(s->avf, AV_LOG_TRACE, "end\n");
        break;

    default:
        av_em_log(s->avf, AV_LOG_TRACE, "invalid chunk\n");
    ...
    while ((chunk_size > 0) && (chunk_type != CHUNK_BAD)) {
        ...
        if (avio_em_read(pb, opcode_preamble, CHUNK_PREAMBLE_SIZE) !=
            CHUNK_PREAMBLE_SIZE) {
            chunk_type = CHUNK_BAD;
            break;
        }
        ...
        if (chunk_size < 0) {
            av_em_log(s->avf, AV_LOG_TRACE, "chunk_size countdown just went negative\n");
            chunk_type = CHUNK_BAD;
            break;
        }

        av_em_log(s->avf, AV_LOG_TRACE, "  opcode type %02X, version %d, 0x%04X bytes: ",
                opcode_type, opcode_version, opcode_size);
        switch (opcode_type) {

        case OPCODE_END_OF_STREAM:
            av_em_log(s->avf, AV_LOG_TRACE, "end of stream\n");
            avio_em_skip(pb, opcode_size);
            break;

        case OPCODE_END_OF_CHUNK:
            av_em_log(s->avf, AV_LOG_TRACE, "end of chunk\n");
            avio_em_skip(pb, opcode_size);
            break;

        case OPCODE_CREATE_TIMER:
            av_em_log(s->avf, AV_LOG_TRACE, "create timer\n");
            if ((opcode_version > 0) || (opcode_size != 6)) {
                av_em_log(s->avf, AV_LOG_TRACE, "bad create_timer opcode\n");
                chunk_type = CHUNK_BAD;
                break;
            }
            if (avio_em_read(pb, scratch, opcode_size) !=
                opcode_size) {
                chunk_type = CHUNK_BAD;
                break;
            }
            s->frame_pts_inc = ((uint64_t)AV_RL32(&scratch[0])) * AV_RL16(&scratch[4]);
            av_em_log(s->avf, AV_LOG_TRACE, "  %.2f frames/second (timer div = %d, subdiv = %d)\n",
                    1000000.0 / s->frame_pts_inc, AV_RL32(&scratch[0]),
                    AV_RL16(&scratch[4]));
            break;

        case OPCODE_INIT_AUDIO_BUFFERS:
            av_em_log(s->avf, AV_LOG_TRACE, "initialize audio buffers\n");
            if (opcode_version > 1 || opcode_size > 10 || opcode_size < 6) {
                av_em_log(s->avf, AV_LOG_TRACE, "bad init_audio_buffers opcode\n");
                chunk_type = CHUNK_BAD;
                break;
            }
            if (avio_em_read(pb, scratch, opcode_size) !=
            ...
            av_em_log(s->avf, AV_LOG_TRACE, "audio: %d bits, %d Hz, %s, %s format\n",
        ...
        case OPCODE_START_STOP_AUDIO:
            av_em_log(s->avf, AV_LOG_TRACE, "start/stop audio\n");
            avio_em_skip(pb, opcode_size);
            break;

        case OPCODE_INIT_VIDEO_BUFFERS:
            av_em_log(s->avf, AV_LOG_TRACE, "initialize video buffers\n");
            if ((opcode_version > 2) || (opcode_size > 8) || opcode_size < 4
                || opcode_version == 2 && opcode_size < 8
            ) {
                av_em_log(s->avf, AV_LOG_TRACE, "bad init_video_buffers opcode\n");
                chunk_type = CHUNK_BAD;
                break;
            }
            if (avio_em_read(pb, scratch, opcode_size) !=
            ...
            av_em_log(s->avf, AV_LOG_TRACE, "video resolution: %d x %d\n",
        ...
        case OPCODE_UNKNOWN_15:
            av_em_log(s->avf, AV_LOG_TRACE, "unknown (but documented) opcode %02X\n", opcode_type);
            avio_em_skip(pb, opcode_size);
            break;

        case OPCODE_SEND_BUFFER:
            av_em_log(s->avf, AV_LOG_TRACE, "send buffer\n");
            avio_em_skip(pb, opcode_size);
            break;

        case OPCODE_AUDIO_FRAME:
            av_em_log(s->avf, AV_LOG_TRACE, "audio frame\n");

            /* log position and move on for now */
            s->audio_chunk_offset = avio_em_tell(pb);
            s->audio_chunk_size = opcode_size;
            avio_em_skip(pb, opcode_size);
            break;

        case OPCODE_SILENCE_FRAME:
            av_em_log(s->avf, AV_LOG_TRACE, "silence frame\n");
            avio_em_skip(pb, opcode_size);
            break;

        case OPCODE_INIT_VIDEO_MODE:
            av_em_log(s->avf, AV_LOG_TRACE, "initialize video mode\n");
            avio_em_skip(pb, opcode_size);
            break;

        case OPCODE_CREATE_GRADIENT:
            av_em_log(s->avf, AV_LOG_TRACE, "create gradient\n");
            avio_em_skip(pb, opcode_size);
            break;

        case OPCODE_SET_PALETTE:
            av_em_log(s->avf, AV_LOG_TRACE, "set palette\n");
            /* check for the logical maximum palette size
             * (3 * 256 + 4 bytes) */
            if (opcode_size > 0x304 || opcode_size < 4) {
                av_em_log(s->avf, AV_LOG_TRACE, "demux_ipmovie: set_palette opcode with invalid size\n");
                chunk_type = CHUNK_BAD;
                break;
            }
            if (avio_em_read(pb, scratch, opcode_size) != opcode_size) {
                chunk_type = CHUNK_BAD;
                break;
            }
            ...
            if (   (first_color > 0xFF) || (last_color > 0xFF)
                ...
                av_em_log(s->avf, AV_LOG_TRACE, "demux_ipmovie: set_palette indexes out of range (%d -> %d)\n",
        ...
        case OPCODE_SET_PALETTE_COMPRESSED:
            av_em_log(s->avf, AV_LOG_TRACE, "set palette compressed\n");
            avio_em_skip(pb, opcode_size);
            break;

        case OPCODE_SET_DECODING_MAP:
            av_em_log(s->avf, AV_LOG_TRACE, "set decoding map\n");

            /* log position and move on for now */
            s->decode_map_chunk_offset = avio_em_tell(pb);
            s->decode_map_chunk_size = opcode_size;
            avio_em_skip(pb, opcode_size);
            break;

        case OPCODE_VIDEO_DATA:
            av_em_log(s->avf, AV_LOG_TRACE, "set video data\n");

            /* log position and move on for now */
            s->video_chunk_offset = avio_em_tell(pb);
            s->video_chunk_size = opcode_size;
            avio_em_skip(pb, opcode_size);
            break;

        default:
            av_em_log(s->avf, AV_LOG_TRACE, "*** unknown opcode type\n");
            chunk_type = CHUNK_BAD;
            break;
    ...
    s->next_chunk_offset = avio_em_tell(pb);
...
static int ipmovie_probe(AVEMProbeData *p)
{
...
static int ipmovie_read_header(AVEMFormatContext *s)
{
    IPMVEContext *ipmovie = s->priv_data;
    ...
    avio_em_read(pb, signature_buffer, sizeof(signature_buffer));
    while (memcmp(signature_buffer, signature, sizeof(signature))) {
        memmove(signature_buffer, signature_buffer + 1, sizeof(signature_buffer) - 1);
        signature_buffer[sizeof(signature_buffer) - 1] = avio_em_r8(pb);
        if (avio_em_feof(pb))
            return AVERROR_EOF;
    }
    ...
    ipmovie->next_chunk_offset = avio_em_tell(pb) + 4;
    ...
    if (avio_em_read(pb, chunk_preamble, CHUNK_PREAMBLE_SIZE) !=
        ...
        return AVERROR(EIO);
    chunk_type = AV_RL16(&chunk_preamble[2]);
    avio_em_seek(pb, -CHUNK_PREAMBLE_SIZE, SEEK_CUR);
    ...
    if (!st)
        return AVERROR(ENOMEM);
    avpriv_em_set_pts_info(st, 63, 1, 1000000);
...
static int ipmovie_read_packet(AVEMFormatContext *s,
                               AVEMPacket *pkt)
{
    IPMVEContext *ipmovie = s->priv_data;
    AVEMIOContext *pb = s->pb;
...
AVEMInputFormat ff_ipmovie_demuxer = {
    .name           = "ipmovie",
