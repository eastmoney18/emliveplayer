...
static int yuv4_generate_header(AVEMFormatContext *s, char* buf)
{
    AVEMStream *st;
    int width, height;
    ...
    av_em_reduce(&raten, &rated, st->time_base.den,
...
static int yuv4_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if (*first_pkt) {
        *first_pkt = 0;
        if (yuv4_generate_header(s, buf2) < 0) {
            ...
            avio_em_write(pb, buf2, strlen(buf2));
        }
    }

    /* construct frame header */

    avio_em_printf(s->pb, "%s\n", Y4M_FRAME_MAGIC);
    ...
    case AV_PIX_FMT_YUV444P16:
        ...
        break;
    default:
        av_em_log(s, AV_LOG_ERROR, "The pixel format '%s' is not supported.\n",
               av_em_get_pix_fmt_name(st->codecpar->format));
        return AVERROR(EINVAL);
    }

    for (i = 0; i < height; i++) {
        avio_em_write(pb, ptr, width);
    ...
    if (st->codecpar->format != AV_PIX_FMT_GRAY8 &&
        ...
        av_em_pix_fmt_get_chroma_sub_sample(st->codecpar->format, &h_chroma_shift,
                                         &v_chroma_shift);
        ...
        for (i = 0; i < height; i++) {     /* Cb */
            avio_em_write(pb, ptr1, width);
            ptr1 += frame->linesize[1];
        }
        for (i = 0; i < height; i++) {     /* Cr */
            avio_em_write(pb, ptr2, width);
...
static int yuv4_write_header(AVEMFormatContext *s)
{
    int *first_pkt = s->priv_data;
    ...
    if (s->streams[0]->codecpar->codec_id != AV_CODEC_ID_WRAPPED_AVFRAME) {
        av_em_log(s, AV_LOG_ERROR, "ERROR: Codec not supported.\n");
        return AVERROR_INVALIDDATA;
    }

    switch (s->streams[0]->codecpar->format) {
    case AV_PIX_FMT_YUV411P:
        av_em_log(s, AV_LOG_WARNING, "Warning: generating rarely used 4:1:1 YUV "
    ...
    case AV_PIX_FMT_YUV444P16:
        if (s->strict_std_compliance >= FF_COMPLIANCE_NORMAL) {
            av_em_log(s, AV_LOG_ERROR, "'%s' is not an official yuv4mpegpipe pixel format. "
                   "Use '-strict -1' to encode to this pixel format.\n",
                   av_em_get_pix_fmt_name(s->streams[0]->codecpar->format));
            return AVERROR(EINVAL);
        }
        av_em_log(s, AV_LOG_WARNING, "Warning: generating non standard YUV stream. "
               "Mjpegtools will not work.\n");
        break;
    default:
        av_em_log(s, AV_LOG_ERROR, "ERROR: yuv4mpeg can only handle "
...
AVEMOutputFormat ff_yuv4mpegpipe_muxer = {
    .name              = "yuv4mpegpipe",
