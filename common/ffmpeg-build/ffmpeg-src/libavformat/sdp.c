                              ...
                              const char *dest_type, int ttl)
{
    if (dest_addr) {
        ...
        if (ttl > 0 && !strcmp(dest_type, "IP4")) {
             ...
             * not for IPv6. */
            av_em_strlcatf(buff, size, "c=IN %s %s/%d\r\n", dest_type, dest_addr, ttl);
        } else {
            av_em_strlcatf(buff, size, "c=IN %s %s\r\n", dest_type, dest_addr);
        }
    }
}
...
static void sdp_write_header(char *buff, int size, struct sdp_session_level *s)
{
    ...
    av_em_strlcatf(buff, size, "t=%d %d\r\n"
                               ...
                               int type_size)
{
    ...
    av_em_strlcpy(type, "IP4", type_size);
    if (!dest_addr[0])
        return 0;
    ...
    if (ai->ai_family == AF_INET6)
        av_em_strlcpy(type, "IP6", type_size);
#endif
    is_multicast = em_is_multicast_address(ai->ai_addr);
    freeaddrinfo(ai);
    return is_multicast;
}
#else
...
static int sdp_get_address(char *dest_addr, int size, int *ttl, const char *url)
{
    ...
    av_em_url_split(proto, sizeof(proto), NULL, 0, dest_addr, size, &port, NULL, 0, url);
    ...
    *ttl = 0;
    ...
    if (p) {
        ...
        if (av_em_find_info_tag(buff, sizeof(buff), "ttl", p)) {
...
static char *extradata2psets(AVEMFormatContext *s, AVEMCodecParameters *par)
{
    char *psets, *p;
    const uint8_t *r;
    ...
    if (par->extradata_size > MAX_EXTRADATA_SIZE) {
        av_em_log(s, AV_LOG_ERROR, "Too much extradata!\n");

        return NULL;
    }
    if (par->extradata[0] == 1) {
        if (em_avc_write_annexb_extradata(par->extradata, &extradata,
                                          &extradata_size))
            return NULL;
        tmpbuf = extradata;
    }

    psets = av_em_mallocz(MAX_PSET_SIZE);
    if (!psets) {
        av_em_log(s, AV_LOG_ERROR, "Cannot allocate memory for the parameter sets.\n");
        av_em_free(tmpbuf);
        return NULL;
    }
    memcpy(psets, pset_string, strlen(pset_string));
    p = psets + strlen(pset_string);
    r = em_avc_find_startcode(extradata, extradata + extradata_size);
    while (r < extradata + extradata_size) {
        ...
        while (!*(r++));
        nal_type = *r & 0x1f;
        r1 = em_avc_find_startcode(r, extradata + extradata_size);
        ...
        if (!av_em_base64_encode(p, MAX_PSET_SIZE - (p - psets), r, r1 - r)) {
            av_em_log(s, AV_LOG_ERROR, "Cannot Base64-encode %"PTRDIFF_SPECIFIER" %"PTRDIFF_SPECIFIER"!\n", MAX_PSET_SIZE - (p - psets), r1 - r);
            av_em_free(psets);
            av_em_free(tmpbuf);
...
static char *extradata2psets_hevc(AVEMCodecParameters *par)
{
    char *psets;
    uint8_t *extradata = par->extradata;
    int extradata_size = par->extradata_size;
    ...
    if (par->extradata[0] != 1) {
        ...
        if (ff_isom_write_hvcc(pb, par->extradata, par->extradata_size, 0) < 0) {
            avio_em_close_dyn_buf(pb, &tmpbuf);
            goto err;
        }
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (i > 0)
            av_em_strlcat(psets, "; ", MAX_PSET_SIZE);
        av_em_strlcatf(psets, MAX_PSET_SIZE, "sprop-%s=", ps_names[i]);
        ...
        for (j = 0; j < num_nalus; j++) {
            ...
            if (j > 0)
                av_em_strlcat(psets, ",", MAX_PSET_SIZE);
            strpos = strlen(psets);
            if (!av_em_base64_encode(psets + strpos, MAX_PSET_SIZE - strpos,
                                  &extradata[pos], len)) {
                av_em_free(psets);
                goto err;
            }
...
static char *extradata2config(AVEMFormatContext *s, AVEMCodecParameters *par)
{
    char *config;
    ...
    config = av_em_alloc(10 + par->extradata_size * 2);
    if (!config) {
        av_em_log(s, AV_LOG_ERROR, "Cannot allocate memory for the config info.\n");
        return NULL;
    }
    memcpy(config, "; config=", 9);
    em_data_to_hex(config + 9, par->extradata, par->extradata_size, 0);
    config[9 + par->extradata_size * 2] = 0;

    return config;
}

static char *xiph_extradata2config(AVEMFormatContext *s, AVEMCodecParameters *par)
{
    ...
    case AV_CODEC_ID_VORBIS:
        ...
        break;
    default:
        av_em_log(s, AV_LOG_ERROR, "Unsupported Xiph codec ID\n");
        return NULL;
    }
    ...
    config = av_em_alloc(config_len);
    if (!config)
        goto xiph_fail;

    encoded_config = av_em_alloc(AV_BASE64_SIZE(config_len));
    if (!encoded_config) {
        av_em_free(config);
        goto xiph_fail;
    }
    ...
    av_em_base64_encode(encoded_config, AV_BASE64_SIZE(config_len),
                     config, config_len);
    av_em_free(config);
...
static int latm_context2profilelevel(AVEMCodecParameters *par)
{
    /* MP4A-LATM
...
static char *latm_context2config(AVEMFormatContext *s, AVEMCodecParameters *par)
{
    /* MP4A-LATM
     * The RTP payload format specification is described in RFC 3016
     * The encoding specifications are provided in ISO/IEC 14496-3 */
    ...
    for (rate_index = 0; rate_index < 16; rate_index++)
        if (avpriv_mpeg4audio_sample_rates[rate_index] == par->sample_rate)
            break;
    if (rate_index == 16) {
        av_em_log(s, AV_LOG_ERROR, "Unsupported sample rate\n");
        return NULL;
    }
    ...
    config = av_em_alloc(6*2+1);
    if (!config) {
        av_em_log(s, AV_LOG_ERROR, "Cannot allocate memory for the config info.\n");
        return NULL;
    }
    em_data_to_hex(config, config_byte, 6, 1);
    config[12] = 0;

    return config;
}

static char *sdp_write_media_attributes(char *buff, int size, AVEMStream *st, int payload_type, AVEMFormatContext *fmt)
{
    char *config = NULL;
    AVEMCodecParameters *p = st->codecpar;

    switch (p->codec_id) {
        case AV_CODEC_ID_DIRAC:
            av_em_strlcatf(buff, size, "a=rtpmap:%d VC2/90000\r\n", payload_type);
            break;
        case AV_CODEC_ID_H264: {
            int mode = 1;
            if (fmt && fmt->oformat && fmt->oformat->priv_class &&
                av_em_opt_flag_is_set(fmt->priv_data, "rtpflags", "h264_mode0"))
                mode = 0;
            if (p->extradata_size) {
                config = extradata2psets(fmt, p);
            }
            av_em_strlcatf(buff, size, "a=rtpmap:%d H264/90000\r\n"
        ...
        case AV_CODEC_ID_H261:
        {
            ...
            if (payload_type >= RTP_PT_PRIVATE)
                av_em_strlcatf(buff, size, "a=rtpmap:%d H261/90000\r\n", payload_type);
            if (pic_fmt)
                av_em_strlcatf(buff, size, "a=fmtp:%d %s\r\n", payload_type, pic_fmt);
            break;
        }
        ...
        case AV_CODEC_ID_H263P:
            ...
            if (!fmt || !fmt->oformat->priv_class ||
                !av_em_opt_flag_is_set(fmt->priv_data, "rtpflags", "rfc2190") ||
                p->codec_id == AV_CODEC_ID_H263P)
            av_em_strlcatf(buff, size, "a=rtpmap:%d H263-2000/90000\r\n"
                                    "a=framesize:%d %d-%d\r\n",
                                    payload_type,
        ...
        case AV_CODEC_ID_HEVC:
            ...
            av_em_strlcatf(buff, size, "a=rtpmap:%d H265/90000\r\n", payload_type);
            if (config)
                av_em_strlcatf(buff, size, "a=fmtp:%d %s\r\n",
                                         payload_type, config);
            break;
        case AV_CODEC_ID_MPEG4:
            ...
            av_em_strlcatf(buff, size, "a=rtpmap:%d MP4V-ES/90000\r\n"
        ...
        case AV_CODEC_ID_AAC:
            if (fmt && fmt->oformat && fmt->oformat->priv_class &&
                av_em_opt_flag_is_set(fmt->priv_data, "rtpflags", "latm")) {
                config = latm_context2config(fmt, p);
                if (!config)
                    return NULL;
                av_em_strlcatf(buff, size, "a=rtpmap:%d MP4A-LATM/%d/%d\r\n"
                ...
                if (p->extradata_size) {
                     ...
                     *        codec parameters...
                     */
                    av_em_log(fmt, AV_LOG_ERROR, "AAC with no global headers is currently not supported.\n");
                    return NULL;
                }
                if (!config) {
                    return NULL;
                }
                av_em_strlcatf(buff, size, "a=rtpmap:%d MPEG4-GENERIC/%d/%d\r\n"
        ...
        case AV_CODEC_ID_PCM_S16BE:
            if (payload_type >= RTP_PT_PRIVATE)
                av_em_strlcatf(buff, size, "a=rtpmap:%d L16/%d/%d\r\n",
                                         payload_type,
                                         p->sample_rate, p->channels);
            break;
        case AV_CODEC_ID_PCM_MULAW:
            if (payload_type >= RTP_PT_PRIVATE)
                av_em_strlcatf(buff, size, "a=rtpmap:%d PCMU/%d/%d\r\n",
                                         payload_type,
                                         p->sample_rate, p->channels);
            break;
        case AV_CODEC_ID_PCM_ALAW:
            if (payload_type >= RTP_PT_PRIVATE)
                av_em_strlcatf(buff, size, "a=rtpmap:%d PCMA/%d/%d\r\n",
                                         payload_type,
                                         p->sample_rate, p->channels);
            break;
        case AV_CODEC_ID_AMR_NB:
            av_em_strlcatf(buff, size, "a=rtpmap:%d AMR/%d/%d\r\n"
                                    "a=fmtp:%d octet-align=1\r\n",
                                     payload_type, p->sample_rate, p->channels,
                                     payload_type);
            break;
        case AV_CODEC_ID_AMR_WB:
            av_em_strlcatf(buff, size, "a=rtpmap:%d AMR-WB/%d/%d\r\n"
                                    "a=fmtp:%d octet-align=1\r\n",
                                     payload_type, p->sample_rate, p->channels,
                                     payload_type);
            break;
        case AV_CODEC_ID_VORBIS:
            if (p->extradata_size)
                ...
                av_em_log(fmt, AV_LOG_ERROR, "Vorbis configuration info missing\n");
            if (!config)
                return NULL;

            av_em_strlcatf(buff, size, "a=rtpmap:%d vorbis/%d/%d\r\n"
        ...
        case AV_CODEC_ID_THEORA: {
            ...
            case AV_PIX_FMT_YUV444P:
                ...
                break;
            default:
                av_em_log(fmt, AV_LOG_ERROR, "Unsupported pixel format.\n");
                return NULL;
            }
            ...
            if (p->extradata_size)
                ...
                av_em_log(fmt, AV_LOG_ERROR, "Theora configuation info missing\n");
            if (!config)
                return NULL;

            av_em_strlcatf(buff, size, "a=rtpmap:%d theora/90000\r\n"
        ...
        case AV_CODEC_ID_VP8:
            av_em_strlcatf(buff, size, "a=rtpmap:%d VP8/90000\r\n",
                                     payload_type);
            break;
        case AV_CODEC_ID_VP9:
            av_em_strlcatf(buff, size, "a=rtpmap:%d VP9/90000\r\n",
                                     payload_type);
            break;
        case AV_CODEC_ID_MJPEG:
            if (payload_type >= RTP_PT_PRIVATE)
                av_em_strlcatf(buff, size, "a=rtpmap:%d JPEG/90000\r\n",
                                         payload_type);
            break;
        case AV_CODEC_ID_ADPCM_G722:
            if (payload_type >= RTP_PT_PRIVATE)
                av_em_strlcatf(buff, size, "a=rtpmap:%d G722/%d/%d\r\n",
                                         payload_type,
                                         8000, p->channels);
            break;
        case AV_CODEC_ID_ADPCM_G726: {
            if (payload_type >= RTP_PT_PRIVATE)
                av_em_strlcatf(buff, size, "a=rtpmap:%d G726-%d/%d\r\n",
                                         payload_type,
        ...
        case AV_CODEC_ID_ILBC:
            av_em_strlcatf(buff, size, "a=rtpmap:%d iLBC/%d\r\n"
                                    "a=fmtp:%d mode=%d\r\n",
                                     payload_type, p->sample_rate,
                                     payload_type, p->block_align == 38 ? 20 : 30);
            break;
        case AV_CODEC_ID_SPEEX:
            av_em_strlcatf(buff, size, "a=rtpmap:%d speex/%d\r\n",
                                     payload_type, p->sample_rate);
            if (st->codec) {
                const char *mode;
                uint64_t vad_option;
                ...
                if (st->codec->flags & AV_CODEC_FLAG_QSCALE)
                      mode = "on";
                else if (!av_em_opt_get_int(st->codec, "vad", AV_OPT_FLAG_ENCODING_PARAM, &vad_option) && vad_option)
                      mode = "vad";
                else
                      mode = "off";

                av_em_strlcatf(buff, size, "a=fmtp:%d vbr=%s\r\n",
                                        payload_type, mode);
            }
            break;
        case AV_CODEC_ID_OPUS:
            ...
            av_em_strlcatf(buff, size, "a=rtpmap:%d opus/48000/2\r\n",
                                     payload_type);
            if (p->channels == 2) {
                av_em_strlcatf(buff, size, "a=fmtp:%d sprop-stereo=1\r\n",
                                         payload_type);
            }
            break;
        default:
...
{
    AVEMDictionaryEntry *title = av_em_dict_get(ac[0]->metadata, "title", NULL, 0);
    ...
    for (i = 0; i < n_files; i++) {
        ...
        for (j = 0; j < ac[i]->nb_streams; j++) {
            em_sdp_write_media(buf, size, ac[i]->streams[j], index++,
                               dst[0] ? dst : NULL, dst_type,
                               (port > 0) ? port + j * 2 : 0,
                               ttl, ac[i]);
            if (port <= 0) {
                av_em_strlcatf(buf, size,
                                   "a=control:streamid=%d\r\n", i + j);
            }
            if (ac[i]->pb && ac[i]->pb->av_class) {
                uint8_t *crypto_suite = NULL, *crypto_params = NULL;
                av_em_opt_get(ac[i]->pb, "srtp_out_suite",  AV_OPT_SEARCH_CHILDREN,
                           &crypto_suite);
                av_em_opt_get(ac[i]->pb, "srtp_out_params", AV_OPT_SEARCH_CHILDREN,
                           &crypto_params);
                if (crypto_suite && crypto_suite[0])
                    av_em_strlcatf(buf, size,
                                "a=crypto:1 %s inline:%s\r\n",
                                crypto_suite, crypto_params);
                av_em_free(crypto_suite);
                av_em_free(crypto_params);
            }
        }
    }
