...
static int acm_probe(AVEMProbeData *p)
{
...
static int acm_read_header(AVEMFormatContext *s)
{
    AVEMStream *st;
    int ret;
    ...
    em_alloc_extradata(st->codecpar, 14);
    if (!st->codecpar->extradata)
        return AVERROR(ENOMEM);
    ret = avio_em_read(s->pb, st->codecpar->extradata, 14);
    if (ret < 10)
...
AVEMInputFormat ff_acm_demuxer = {
    .name           = "acm",
