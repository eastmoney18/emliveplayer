...
static int
flac_header (AVEMFormatContext * s, int idx)
{
    ...
    if (mdt == OGG_FLAC_METADATA_TYPE_STREAMINFO) {
        ...
        if (em_alloc_extradata(st->codecpar, FLAC_STREAMINFO_SIZE) < 0)
...
static int
old_flac_header (AVEMFormatContext * s, int idx)
{
    struct ogg *ogg = s->priv_data;
    AVEMStream *st = s->streams[idx];
    struct ogg_stream *os = ogg->streams + idx;
    AVEMCodecParserContext *parser = av_em_parser_init(AV_CODEC_ID_FLAC);
    ...
    av_em_parser_parse2(parser, avctx,
    ...
    if (avctx->sample_rate) {
        avpriv_em_set_pts_info(st, 64, 1, avctx->sample_rate);
