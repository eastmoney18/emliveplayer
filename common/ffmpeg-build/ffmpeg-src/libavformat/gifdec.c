...
static int gif_probe(AVEMProbeData *p)
{
    /* check magick */
...
static int resync(AVEMIOContext *pb)
{
    int i;
    for (i = 0; i < 6; i++) {
        int b = avio_em_r8(pb);
...
static int gif_read_header(AVEMFormatContext *s)
{
...
static int gif_skip_subblocks(AVEMIOContext *pb)
{
    int sb_size, ret = 0;

    while (0x00 != (sb_size = avio_em_r8(pb))) {
        if ((ret = avio_em_skip(pb, sb_size)) < 0)
            return ret;
    }
...
static int gif_read_ext(AVEMFormatContext *s)
{
    GIFDemuxContext *gdc = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int sb_size, ext_label = avio_em_r8(pb);
    int ret;

    if (ext_label == GIF_GCE_EXT_LABEL) {
        if ((sb_size = avio_em_r8(pb)) < 4) {
            av_em_log(s, AV_LOG_FATAL, "Graphic Control Extension block's size less than 4.\n");
            return AVERROR_INVALIDDATA;
        }

        /* skip packed fields */
        if ((ret = avio_em_skip(pb, 1)) < 0)
            return ret;

        gdc->delay = avio_em_rl16(pb);
        ...
        if ((ret = avio_em_skip(pb, sb_size - 3)) < 0 )
            return ret;
    } else if (ext_label == GIF_APP_EXT_LABEL) {
        uint8_t data[256];

        sb_size = avio_em_r8(pb);
        ret = avio_em_read(pb, data, sb_size);
        if (ret < 0 || !sb_size)
            return ret;
...
static int gif_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    GIFDemuxContext *gdc = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int packed_fields, block_label, ct_size,
        keyframe, frame_parsed = 0, ret;
    int64_t frame_start = avio_em_tell(pb), frame_end;
    unsigned char buf[6];

    if ((ret = avio_em_read(pb, buf, 6)) == 6) {
...
parse_keyframe:
        ...
        if ((ret = avio_em_skip(pb, 4)) < 0)
            return ret;

        packed_fields = avio_em_r8(pb);

        /* skip 1 byte of Background Color Index and 1 byte of Pixel Aspect Ratio */
        if ((ret = avio_em_skip(pb, 2)) < 0)
            return ret;
        ...
        if (packed_fields & 0x80) {
            ...
            if ((ret = avio_em_skip(pb, ct_size)) < 0)
                return ret;
        }
    } else {
        avio_em_seek(pb, -ret, SEEK_CUR);
        ret = AVERROR_EOF;
    }

    while (GIF_TRAILER != (block_label = avio_em_r8(pb)) && !avio_em_feof(pb)) {
        if (block_label == GIF_EXTENSION_INTRODUCER) {
            ...
            if (avio_em_r8(pb) < 1) {
                av_em_log(s, AV_LOG_ERROR, "lzw minimum code size must be >= 1\n");
                goto resync;
            }
            ...
            frame_end = avio_em_tell(pb);

            if (avio_em_seek(pb, frame_start, SEEK_SET) != frame_start)
                return AVERROR(EIO);

            ret = av_em_get_packet(pb, pkt, frame_end - frame_start);
            if (ret < 0)
                return ret;
            ...
            av_em_log(s, AV_LOG_ERROR, "invalid block label\n");
resync:
            if (!keyframe)
                avio_em_seek(pb, frame_start, SEEK_SET);
            if ((ret = resync(pb)) < 0)
                return ret;
            frame_start = avio_em_tell(pb) - 6;
            keyframe = 1;
            goto parse_keyframe;
        }
    }
    ...
    if ((ret >= 0 && !frame_parsed) || ret == AVERROR_EOF) {
        if (gdc->nb_frames == 1) {
            s->streams[0]->r_frame_rate = (AVEMRational) {100, gdc->last_duration};
        }
        /* This might happen when there is no image block
         * between extension blocks and GIF_TRAILER or EOF */
        if (!gdc->ignore_loop && (block_label == GIF_TRAILER || avio_em_feof(pb))
            && (gdc->total_iter < 0 || ++gdc->iter_count < gdc->total_iter))
            return avio_em_seek(pb, 0, SEEK_SET);
        return AVERROR_EOF;
    } else
        return ret;
}
...
AVEMInputFormat ff_gif_demuxer = {
    .name           = "gif",
