...
static int read_desc_chunk(AVEMFormatContext *s)
{
    ...
    flags = avio_em_rb32(pb);
    caf->bytes_per_packet  = avio_em_rb32(pb);
    st->codecpar->block_align = caf->bytes_per_packet;
    caf->frames_per_packet = avio_em_rb32(pb);
    ...
    if (st->codecpar->codec_tag == MKTAG('l','p','c','m'))
        st->codecpar->codec_id = em_mov_get_lpcm_codec_id(st->codecpar->bits_per_coded_sample, (flags ^ 0x2) | 0x4);
    else
        st->codecpar->codec_id = em_codec_get_id(em_codec_caf_tags, st->codecpar->codec_tag);
    return 0;
}

/** Read magic cookie chunk */
static int read_kuki_chunk(AVEMFormatContext *s, int64_t size)
{
    AVEMIOContext *pb = s->pb;
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_AAC) {
        ...
        strt = avio_em_tell(pb);
        em_mov_read_esds(s, pb);
        skip = size - (avio_em_tell(pb) - strt);
        if (skip < 0 || !st->codecpar->extradata ||
            st->codecpar->codec_id != AV_CODEC_ID_AAC) {
            av_em_log(s, AV_LOG_ERROR, "invalid AAC magic cookie\n");
...
#define ALAC_NEW_KUKI 24
        ...
        if (size < ALAC_NEW_KUKI) {
            av_em_log(s, AV_LOG_ERROR, "invalid ALAC magic cookie\n");
            avio_em_skip(pb, size);
            return AVERROR_INVALIDDATA;
        }
        if (avio_em_read(pb, preamble, ALAC_PREAMBLE) != ALAC_PREAMBLE) {
            av_em_log(s, AV_LOG_ERROR, "failed to read preamble\n");
            return AVERROR_INVALIDDATA;
        }

        av_em_freep(&st->codecpar->extradata);
        if (em_alloc_extradata(st->codecpar, ALAC_HEADER))
            return AVERROR(ENOMEM);
        ...
        if (!memcmp(&preamble[4], "frmaalac", 8)) {
            ...
            if (avio_em_read(pb, st->codecpar->extradata, ALAC_HEADER) != ALAC_HEADER) {
                av_em_log(s, AV_LOG_ERROR, "failed to read kuki header\n");
                av_em_freep(&st->codecpar->extradata);
                return AVERROR_INVALIDDATA;
            }
            avio_em_skip(pb, size - ALAC_PREAMBLE - ALAC_HEADER);
        } else {
            ...
            if (avio_em_read(pb, &st->codecpar->extradata[24], ALAC_NEW_KUKI - 12) != ALAC_NEW_KUKI - 12) {
                av_em_log(s, AV_LOG_ERROR, "failed to read new kuki header\n");
                av_em_freep(&st->codecpar->extradata);
                return AVERROR_INVALIDDATA;
            }
            avio_em_skip(pb, size - ALAC_NEW_KUKI);
        }
    } else {
        av_em_freep(&st->codecpar->extradata);
        if (em_get_extradata(s, st->codecpar, pb, size) < 0)
            return AVERROR(ENOMEM);
    }
...
static int read_pakt_chunk(AVEMFormatContext *s, int64_t size)
{
    AVEMIOContext *pb = s->pb;
    AVEMStream *st      = s->streams[0];
    CafContext *caf   = s->priv_data;
    ...
    ccount = avio_em_tell(pb);

    num_packets = avio_em_rb64(pb);
    if (num_packets < 0 || INT32_MAX / sizeof(AVEMIndexEntry) < num_packets)
        return AVERROR_INVALIDDATA;

    st->nb_frames  = avio_em_rb64(pb); /* valid frames */
    st->nb_frames += avio_em_rb32(pb); /* priming frames */
    st->nb_frames += avio_em_rb32(pb); /* remainder frames */

    st->duration = 0;
    for (i = 0; i < num_packets; i++) {
        av_em_add_index_entry(s->streams[0], pos, st->duration, 0, 0, AVINDEX_KEYFRAME);
        pos += caf->bytes_per_packet ? caf->bytes_per_packet : em_mp4_read_descr_len(pb);
        st->duration += caf->frames_per_packet ? caf->frames_per_packet : em_mp4_read_descr_len(pb);
    }

    if (avio_em_tell(pb) - ccount > size) {
        av_em_log(s, AV_LOG_ERROR, "error reading packet table\n");
        return AVERROR_INVALIDDATA;
    }
    avio_em_skip(pb, ccount + size - avio_em_tell(pb));
...
static void read_info_chunk(AVEMFormatContext *s, int64_t size)
{
    AVEMIOContext *pb = s->pb;
    unsigned int i;
    unsigned int nb_entries = avio_em_rb32(pb);
    for (i = 0; i < nb_entries && !avio_em_feof(pb); i++) {
        char key[32];
        char value[1024];
        avio_em_get_str(pb, INT_MAX, key, sizeof(key));
        avio_em_get_str(pb, INT_MAX, value, sizeof(value));
...
static int read_header(AVEMFormatContext *s)
{
    ...
    avio_em_skip(pb, 8); /* magic, version, file flags */

    /* audio description chunk */
    if (avio_em_rb32(pb) != MKBETAG('d','e','s','c')) {
        av_em_log(s, AV_LOG_ERROR, "desc chunk not present\n");
        return AVERROR_INVALIDDATA;
    }
    size = avio_em_rb64(pb);
    if (size != 32)
        return AVERROR_INVALIDDATA;
    ...
    while (!avio_em_feof(pb)) {
        ...
        case MKBETAG('d','a','t','a'):
            avio_em_skip(pb, 4); /* edit count */
            caf->data_start = avio_em_tell(pb);
            caf->data_size  = size < 0 ? -1 : size - 4;
            if (caf->data_size > 0 && pb->seekable)
                avio_em_skip(pb, caf->data_size);
            found_data = 1;
            break;

        case MKBETAG('c','h','a','n'):
            if ((ret = em_mov_read_chan(s, s->pb, st, size)) < 0)
                return ret;
            break;
...
#undef _
        ...
        if (size > 0) {
            if (pos > INT64_MAX - size)
                return AVERROR_INVALIDDATA;
            avio_em_skip(pb, FFMAX(0, pos + size - avio_em_tell(pb)));
        }
    }
    ...
    if (caf->bytes_per_packet > 0 && caf->frames_per_packet > 0) {
        ...
        av_em_log(s, AV_LOG_ERROR, "Missing packet table. It is required when "
    ...
    if (caf->data_size >= 0)
        avio_em_seek(pb, caf->data_start, SEEK_SET);
...
static int read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if (caf->data_size > 0) {
        left = (caf->data_start + caf->data_size) - avio_em_tell(pb);
        if (!left)
            return AVERROR_EOF;
        if (left < 0)
            return AVERROR(EIO);
    ...
    res = av_em_get_packet(pb, pkt, pkt_size);
    if (res < 0)
        return res;
                     ...
                     int64_t timestamp, int flags)
{
    ...
    if (caf->frames_per_packet > 0 && caf->bytes_per_packet > 0) {
        ...
        packet_cnt = av_em_index_search_timestamp(st, timestamp, flags);
    ...
    if (avio_em_seek(s->pb, pos + caf->data_start, SEEK_SET) < 0)
        return -1;
...
AVEMInputFormat ff_caf_demuxer = {
    .name           = "caf",
    ...
    .read_seek      = read_seek,
    .codec_tag      = (const AVEMCodecTag* const []){ em_codec_caf_tags, 0 },
};
