...
static int sami_probe(AVEMProbeData *p)
{
    char buf[6];
    FFTextReader tr;
...
static int sami_read_header(AVEMFormatContext *s)
{
    ...
    av_em_bprint_init(&hdr_buf, 0, AV_BPRINT_SIZE_UNLIMITED);
    ...
    while (!ff_text_eof(&tr)) {
        ...
        is_body = !av_em_strncasecmp(buf.str, "</BODY", 6);
        if (is_body) {
             av_em_bprint_clear(&buf);
             break;
        }

        is_sync = !av_em_strncasecmp(buf.str, "<SYNC", 5);
        if (is_sync)
            got_first_sync_point = 1;

        if (!got_first_sync_point) {
            av_em_bprintf(&hdr_buf, "%s", buf.str);
        } else {
    ...
    res = em_bprint_to_codecpar_extradata(st->codecpar, &hdr_buf);
    if (res < 0)
        goto end;
...
static int sami_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    SAMIContext *sami = s->priv_data;
    return ff_subtitles_queue_read_packet(&sami->q, pkt);
}

static int sami_read_seek(AVEMFormatContext *s, int stream_index,
...
static int sami_read_close(AVEMFormatContext *s)
{
    SAMIContext *sami = s->priv_data;
...
AVEMInputFormat ff_sami_demuxer = {
    .name           = "sami",
