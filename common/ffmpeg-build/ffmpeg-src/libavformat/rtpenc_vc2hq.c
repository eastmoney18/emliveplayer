...
static void send_packet(AVEMFormatContext *ctx, uint8_t parse_code, int info_hdr_size, const uint8_t *buf, int size, int i, int f, int rtp_m)
{
...
static void send_picture(AVEMFormatContext *ctx, const uint8_t *buf, int size, int interlaced)
{
    RTPMuxContext *rtp_ctx = ctx->priv_data;
    GetBitContext gc;
...
void ff_rtp_send_vc2hq(AVEMFormatContext *ctx, const uint8_t *frame_buf, int frame_size, int interlaced)
{
    ...
    while (unit < end) {
        ...
        case DIRAC_PCODE_PAD:
            break;
        default:
            avpriv_em_report_missing_feature(ctx, "VC-2 parse code %d", parse_code);
            break;
        }
        unit += unit_size;
    }
