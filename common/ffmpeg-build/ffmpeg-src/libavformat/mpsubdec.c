...
static int mpsub_probe(AVEMProbeData *p)
{
...
static int mpsub_read_header(AVEMFormatContext *s)
{
    MPSubContext *mpsub = s->priv_data;
    AVEMStream *st;
    AVEMBPrint buf;
    AVEMRational pts_info = (AVEMRational){ 100, 1 }; // ts based by default
    int res = 0;
    int multiplier = 100;
    ...
    while (!avio_em_feof(s->pb)) {
        ...
        int fps, len = em_get_line(s->pb, line, sizeof(line));
...
static int mpsub_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MPSubContext *mpsub = s->priv_data;
    return ff_subtitles_queue_read_packet(&mpsub->q, pkt);
}

static int mpsub_read_seek(AVEMFormatContext *s, int stream_index,
...
static int mpsub_read_close(AVEMFormatContext *s)
{
    MPSubContext *mpsub = s->priv_data;
...
AVEMInputFormat ff_mpsub_demuxer = {
    .name           = "mpsub",
