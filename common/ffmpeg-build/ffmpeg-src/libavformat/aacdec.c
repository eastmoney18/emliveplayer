...
static int adts_aac_probe(AVEMProbeData *p)
{
...
static int adts_aac_read_header(AVEMFormatContext *s)
{
    AVEMStream *st;
    ...
    ff_id3v1_read(s);
    if (s->pb->seekable &&
        !av_em_dict_get(s->metadata, "", NULL, AV_DICT_IGNORE_SUFFIX)) {
    ...
    avpriv_em_set_pts_info(st, 64, 1, 28224000);

    return 0;
}

AVEMInputFormat em_aac_demuxer = {
    .name         = "aac",
