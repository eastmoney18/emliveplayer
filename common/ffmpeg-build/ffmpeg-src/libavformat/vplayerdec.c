...
static int vplayer_probe(AVEMProbeData *p)
{
    char c;
...
static int vplayer_read_header(AVEMFormatContext *s)
{
...
static int vplayer_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    VPlayerContext *vplayer = s->priv_data;
    return ff_subtitles_queue_read_packet(&vplayer->q, pkt);
}

static int vplayer_read_seek(AVEMFormatContext *s, int stream_index,
...
static int vplayer_read_close(AVEMFormatContext *s)
{
    VPlayerContext *vplayer = s->priv_data;
...
AVEMInputFormat ff_vplayer_demuxer = {
    .name           = "vplayer",
