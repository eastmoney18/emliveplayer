...
static int return_stored_frame(AVEMFormatContext *ctx, PayloadContext *data,
                               AVEMStream *st, AVEMPacket *pkt, uint32_t *timestamp,
                               const uint8_t *buf, int len);

static int store_packet(AVEMFormatContext *ctx, PayloadContext *data,
                        AVEMStream *st, AVEMPacket *pkt, uint32_t *timestamp,
                        const uint8_t *buf, int len)
{
    ...
    if (interleave_size > 5) {
        av_em_log(ctx, AV_LOG_ERROR, "Invalid interleave size %d\n",
                                   interleave_size);
        return AVERROR_INVALIDDATA;
    }
    if (interleave_index > interleave_size) {
        av_em_log(ctx, AV_LOG_ERROR, "Invalid interleave index %d/%d\n",
    ...
    if ((ret = av_em_new_packet(pkt, frame_size)) < 0)
        return ret;
                               ...
                               const uint8_t *buf, int len)
{
    ...
    if (ip->size == 0) {
        ...
        if ((ret = av_em_new_packet(pkt, 1)) < 0)
            return ret;
...
static int qcelp_parse_packet(AVEMFormatContext *ctx, PayloadContext *data,
                              AVEMStream *st, AVEMPacket *pkt, uint32_t *timestamp,
