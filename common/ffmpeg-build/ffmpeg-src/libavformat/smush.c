...
static int smush_read_probe(AVEMProbeData *p)
{
...
static int smush_read_header(AVEMFormatContext *ctx)
{
    ...
    magic = avio_em_rb32(pb);
    avio_em_skip(pb, 4); // skip movie size

    if (magic == MKBETAG('A', 'N', 'I', 'M')) {
        if (avio_em_rb32(pb) != MKBETAG('A', 'H', 'D', 'R'))
        ...
        smush->version = 0;
        subversion     = avio_em_rl16(pb);
        nframes        = avio_em_rl16(pb);
        if (!nframes)
            return AVERROR_INVALIDDATA;

        avio_em_skip(pb, 2); // skip pad

        for (i = 0; i < 256; i++)
            palette[i] = avio_em_rb24(pb);

        avio_em_skip(pb, size - (3 * 256 + 6));
    } else if (magic == MKBETAG('S', 'A', 'N', 'M')) {
        if (avio_em_rb32(pb) != MKBETAG('S', 'H', 'D', 'R'))
            return AVERROR_INVALIDDATA;
        ...
        avio_em_skip(pb, 2); // skip pad
        avio_em_skip(pb, size - 14);

        if (avio_em_rb32(pb) != MKBETAG('F', 'L', 'H', 'D'))
            return AVERROR_INVALIDDATA;
        ...
        avio_em_skip(pb, size - read);
    } else {
        av_em_log(ctx, AV_LOG_ERROR, "Wrong magic\n");
        return AVERROR_INVALIDDATA;
    }

    vst = avformat_em_new_stream(ctx, 0);
    if (!vst)
    ...
    if (!smush->version) {
        if (em_alloc_extradata(vst->codecpar, 1024 + 2))
            return AVERROR(ENOMEM);
    ...
    if (got_audio) {
        ast = avformat_em_new_stream(ctx, 0);
        if (!ast)
            return AVERROR(ENOMEM);
...
static int smush_read_packet(AVEMFormatContext *ctx, AVEMPacket *pkt)
{
    SMUSHContext *smush = ctx->priv_data;
    AVEMIOContext *pb = ctx->pb;
    int done = 0;
    ...
    while (!done) {
        ...
        case MKBETAG('F', 'R', 'M', 'E'):
            if (smush->version)
                break;
            if ((ret = av_em_get_packet(pb, pkt, size)) < 0)
                return ret;
        ...
        case MKBETAG('W', 'a', 'v', 'e'):
            if (size < 13)
                return AVERROR_INVALIDDATA;
            if (av_em_get_packet(pb, pkt, size) < 13)
                return AVERROR(EIO);
...
AVEMInputFormat ff_smush_demuxer = {
    .name           = "smush",
