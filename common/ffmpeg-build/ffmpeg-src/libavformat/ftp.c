...
typedef struct {
    ...
    EMURLContext *conn_control;                    /**< Control connection */
    EMURLContext *conn_data;                       /**< Data connection, NULL when not connected */
...
static const AVEMClass ftp_context_class = {
    .class_name     = "ftp",
...
static int ftp_close(EMURLContext *h);

static int ftp_getc(FTPContext *s)
{
    int len;
    if (s->control_buf_ptr >= s->control_buf_end) {
        len = ffurl_em_read(s->conn_control, s->control_buffer, CONTROL_BUFFER_SIZE);
        if (len < 0) {
            return len;
        } else if (!len) {
            return -1;
...
static int ftp_status(FTPContext *s, char **line, const int response_codes[])
{
    ...
    AVEMBPrint line_buffer;

    if (line)
        av_em_bprint_init(&line_buffer, 0, AV_BPRINT_SIZE_AUTOMATIC);

    while (!code_found || dash) {
        if ((err = ftp_get_line(s, buf, sizeof(buf))) < 0) {
            if (line)
                av_em_bprint_finalize(&line_buffer, NULL);
            return err;
        }
    ...
    if (line)
        av_em_bprint_finalize(&line_buffer, line);
    return result;
}
                            ...
                            const int response_codes[], char **response)
{
    ...
    if ((err = ffurl_em_write(s->conn_control, command, strlen(command))) < 0)
        return err;
    if (!err)
        return -1;
...
{
    ffurl_em_closep(&s->conn_control);
...
static int ftp_passive_mode(FTPContext *s)
{
    ...
    *end  = '\0';
    /* skip ip */
    if (!av_em_strtok(start, ",", &end)) goto fail;
    if (!av_em_strtok(end, ",", &end)) goto fail;
    if (!av_em_strtok(end, ",", &end)) goto fail;
    if (!av_em_strtok(end, ",", &end)) goto fail;

    /* parse port number */
    start = av_em_strtok(end, ",", &end);
    if (!start) goto fail;
...
static int ftp_has_feature(FTPContext *s, const char *feature_name)
{
    ...
    return av_em_stristr(s->features, feature_name) != NULL;
}
...
static int ftp_connect_control_connection(EMURLContext *h)
{
    ...
    if (!s->conn_control) {
        em_url_join(buf, sizeof(buf), "tcp", NULL,
                    s->hostname, s->server_control_port, NULL);
        if (s->rw_timeout != -1) {
            av_em_dict_set_int(&opts, "timeout", s->rw_timeout, 0);
        } /* if option is not given, don't pass it and let tcp use its own default */
        err = ffurl_em_open_whitelist(&s->conn_control, buf, AVIO_FLAG_READ_WRITE,
                                   &h->interrupt_callback, &opts,
                                   h->protocol_whitelist, h->protocol_blacklist, h);
        av_em_dict_free(&opts);
        if (err < 0) {
            av_em_log(h, AV_LOG_ERROR, "Cannot open control connection\n");
            return err;
        }

        /* check if server is ready */
        if (ftp_status(s, ((h->flags & AVIO_FLAG_WRITE) ? &response : NULL), connect_codes) != 220) {
            av_em_log(h, AV_LOG_ERROR, "FTP server not ready for new users\n");
            return AVERROR(EACCES);
        }

        if ((h->flags & AVIO_FLAG_WRITE) && av_em_stristr(response, "pure-ftpd")) {
            av_em_log(h, AV_LOG_WARNING, "Pure-FTPd server is used as an output protocol. It is known issue this implementation may produce incorrect content and it cannot be fixed at this moment.");
        }
        av_em_free(response);

        if ((err = ftp_auth(s)) < 0) {
            av_em_log(h, AV_LOG_ERROR, "FTP authentication failed\n");
            return err;
        }

        if ((err = ftp_type(s)) < 0) {
            av_em_log(h, AV_LOG_ERROR, "Set content type failed\n");
            return err;
        }
...
static int ftp_connect_data_connection(EMURLContext *h)
{
    int err;
    char buf[CONTROL_BUFFER_SIZE];
    AVEMDictionary *opts = NULL;
    ...
    if (!s->conn_data) {
        ...
        em_url_join(buf, sizeof(buf), "tcp", NULL, s->hostname, s->server_data_port, NULL);
        if (s->rw_timeout != -1) {
            av_em_dict_set_int(&opts, "timeout", s->rw_timeout, 0);
        } /* if option is not given, don't pass it and let tcp use its own default */
        err = ffurl_em_open_whitelist(&s->conn_data, buf, h->flags,
                                   &h->interrupt_callback, &opts,
                                   h->protocol_whitelist, h->protocol_blacklist, h);
        av_em_dict_free(&opts);
        if (err < 0)
...
static int ftp_abort(EMURLContext *h)
{
    ...
    */
    ...
    if (ftp_send_command(s, command, NULL, NULL) < 0) {
        ...
        if ((err = ftp_connect_control_connection(h)) < 0) {
            av_em_log(h, AV_LOG_ERROR, "Reconnect failed.\n");
            return err;
        }
    } else {
        ftp_close_data_connection(s);
...
static int ftp_connect(EMURLContext *h, const char *url)
{
    ...
    av_em_url_split(proto, sizeof(proto),
    ...
    tok_user = av_em_strtok(credencials, ":", &end);
    tok_pass = av_em_strtok(end, ":", &end);
    if (!tok_user) {
        tok_user = "anonymous";
        tok_pass = av_em_x_if_null(s->anonymous_password, "nopassword");
    }
    s->user = av_em_strdup(tok_user);
    s->password = av_em_strdup(tok_pass);
    s->hostname = av_em_strdup(hostname);
    ...
    newpath = av_em_append_path_component(s->path, path);
    if (!newpath)
        return AVERROR(ENOMEM);
    av_em_free(s->path);
    s->path = newpath;
...
static int ftp_open(EMURLContext *h, const char *url, int flags)
{
    FTPContext *s = h->priv_data;
    int err;
  ...
  fail:
    av_em_log(h, AV_LOG_ERROR, "FTP open failed\n");
    ftp_close(h);
    return err;
}

static int64_t ftp_seek(EMURLContext *h, int64_t pos, int whence)
{
    FTPContext *s = h->priv_data;
    int err;
    ...
    if (new_pos < 0) {
        av_em_log(h, AV_LOG_ERROR, "Seeking to nagative position.\n");
        return AVERROR(EINVAL);
    }
...
static int ftp_read(EMURLContext *h, unsigned char *buf, int size)
{
    FTPContext *s = h->priv_data;
  ...
  retry:
    ...
    if (s->conn_data && s->state == DOWNLOADING) {
        read = ffurl_em_read(s->conn_data, buf, size);
        if (read >= 0) {
            s->position += read;
            if (s->position >= s->filesize) {
        ...
        if (read <= 0 && s->position < s->filesize && !h->is_streamed) {
            ...
            int64_t pos = s->position;
            av_em_log(h, AV_LOG_INFO, "Reconnect to FTP server.\n");
            if ((err = ftp_abort(h)) < 0)
                return err;
            if ((err = ftp_seek(h, pos, SEEK_SET)) < 0) {
                av_em_log(h, AV_LOG_ERROR, "Position cannot be restored.\n");
                return err;
            }
            if (!retry_done) {
                retry_done = 1;
    ...
    av_em_log(h, AV_LOG_DEBUG, "FTP read failed\n");
    return AVERROR(EIO);
}

static int ftp_write(EMURLContext *h, const unsigned char *buf, int size)
{
    int err;
    FTPContext *s = h->priv_data;
    int written;
    ...
    if (s->conn_data && s->state == UPLOADING) {
        written = ffurl_em_write(s->conn_data, buf, size);
        if (written > 0) {
            s->position += written;
    ...
    av_em_log(h, AV_LOG_ERROR, "FTP write failed\n");
    return AVERROR(EIO);
}
...
static int ftp_close(EMURLContext *h)
{
    ...
    ftp_close_both_connections(s);
    av_em_freep(&s->user);
    av_em_freep(&s->password);
    av_em_freep(&s->hostname);
    av_em_freep(&s->path);
    av_em_freep(&s->features);
...
static int ftp_get_file_handle(EMURLContext *h)
{
    FTPContext *s = h->priv_data;
    ...
    if (s->conn_data)
        return ffurl_em_get_file_handle(s->conn_data);

    return AVERROR(EIO);
}

static int ftp_shutdown(EMURLContext *h, int flags)
{
    FTPContext *s = h->priv_data;
    ...
    if (s->conn_data)
        return ffurl_em_shutdown(s->conn_data, flags);

    return AVERROR(EIO);
}

static int ftp_open_dir(EMURLContext *h)
{
    FTPContext *s = h->priv_data;
    int ret;
    ...
    if ((ret = ftp_list(s)) < 0)
        goto fail;
    s->dir_buffer = av_em_alloc(DIR_BUFFER_SIZE);
    if (!s->dir_buffer) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
...
static int64_t ftp_parse_date(const char *date)
{
    ...
    av_em_small_strptime(date, "%Y%m%d%H%M%S", &tv);
    return INT64_C(1000000) * av_em_timegm(&tv);
}

static int ftp_parse_entry_nlst(char *line, AVEMIODirEntry *next)
{
    next->name = av_em_strdup(line);
    return 0;
}

static int ftp_parse_entry_mlsd(char *mlsd, AVEMIODirEntry *next)
{
    char *fact, *value;
    ff_dlog(NULL, "%s\n", mlsd);
    while(fact = av_em_strtok(mlsd, ";", &mlsd)) {
        if (fact[0] == ' ') {
            next->name = av_em_strdup(&fact[1]);
            continue;
        }
        fact = av_em_strtok(fact, "=", &value);
        if (!av_em_strcasecmp(fact, "type")) {
            if (!av_em_strcasecmp(value, "cdir") || !av_em_strcasecmp(value, "pdir"))
                return 1;
            if (!av_em_strcasecmp(value, "dir"))
                next->type = AVIO_ENTRY_DIRECTORY;
            else if (!av_em_strcasecmp(value, "file"))
                next->type = AVIO_ENTRY_FILE;
            else if (!av_em_strcasecmp(value, "OS.unix=slink:"))
                next->type = AVIO_ENTRY_SYMBOLIC_LINK;
        } else if (!av_em_strcasecmp(fact, "modify")) {
            next->modification_timestamp = ftp_parse_date(value);
        } else if (!av_em_strcasecmp(fact, "UNIX.mode")) {
            next->filemode = strtoumax(value, NULL, 8);
        } else if (!av_em_strcasecmp(fact, "UNIX.uid") || !av_em_strcasecmp(fact, "UNIX.owner"))
            next->user_id = strtoumax(value, NULL, 10);
        else if (!av_em_strcasecmp(fact, "UNIX.gid") || !av_em_strcasecmp(fact, "UNIX.group"))
            next->group_id = strtoumax(value, NULL, 10);
        else if (!av_em_strcasecmp(fact, "size") || !av_em_strcasecmp(fact, "sizd"))
 ...
 */
static int ftp_parse_entry(EMURLContext *h, char *line, AVEMIODirEntry *next)
{
    FTPContext *s = h->priv_data;
...
static int ftp_read_dir(EMURLContext *h, AVEMIODirEntry **next)
{
    FTPContext *s = h->priv_data;
    char *start, *found;
    int ret, retried;
    ...
    do {
        ...
        while (!(found = strstr(start, "\n"))) {
            ...
            ret = ffurl_em_read(s->conn_data, s->dir_buffer + s->dir_buffer_size, DIR_BUFFER_SIZE - (s->dir_buffer_size + 1));
            if (ret < 0)
                return ret;
            if (!ret) {
                *next = NULL;
        ...
        *next = em_alloc_dir_entry();
        if (!*next)
            return AVERROR(ENOMEM);
        (*next)->utf8 = s->utf8;
        ret = ftp_parse_entry(h, start, *next);
        if (ret) {
            avio_em_free_directory_entry(next);
            if (ret < 0)
                return ret;
        }
    } while (ret > 0);
...
static int ftp_close_dir(EMURLContext *h)
{
    FTPContext *s = h->priv_data;
    av_em_freep(&s->dir_buffer);
    ffurl_em_closep(&s->conn_control);
    ffurl_em_closep(&s->conn_data);
    return 0;
}

static int ftp_delete(EMURLContext *h)
{
    FTPContext *s = h->priv_data;
    char command[MAX_URL_SIZE];
...
static int ftp_move(EMURLContext *h_src, EMURLContext *h_dst)
{
...
const EMURLProtocol em_ftp_protocol = {
    .name                = "ftp",
    .url_open            = ftp_open,
    .url_read            = ftp_read,
    .url_write           = ftp_write,
