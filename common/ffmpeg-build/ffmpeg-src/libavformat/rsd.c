...
static const AVEMCodecTag rsd_tags[] = {
...
static int rsd_probe(AVEMProbeData *p)
{
...
static int rsd_read_header(AVEMFormatContext *s)
{
    ...
    avio_em_skip(pb, 3); // "RSD"
    version = avio_em_r8(pb) - '0';

    par = st->codecpar;
    par->codec_type = AVMEDIA_TYPE_AUDIO;
    par->codec_tag  = avio_em_rl32(pb);
    par->codec_id   = em_codec_get_id(rsd_tags, par->codec_tag);
    if (!par->codec_id) {
        char tag_buf[32];

        av_em_get_codec_tag_string(tag_buf, sizeof(tag_buf), par->codec_tag);
        for (i=0; i < FF_ARRAY_ELEMS(rsd_unsupported_tags); i++) {
            if (par->codec_tag == rsd_unsupported_tags[i]) {
                avpriv_em_request_sample(s, "Codec tag: %s", tag_buf);
                return AVERROR_PATCHWELCOME;
            }
        }
        av_em_log(s, AV_LOG_ERROR, "Unknown codec tag: %s\n", tag_buf);
        return AVERROR_INVALIDDATA;
    }

    par->channels = avio_em_rl32(pb);
    if (!par->channels)
        return AVERROR_INVALIDDATA;

    avio_em_skip(pb, 4); // Bit depth
    par->sample_rate = avio_em_rl32(pb);
    if (!par->sample_rate)
        return AVERROR_INVALIDDATA;

    avio_em_skip(pb, 4); // Unknown

    switch (par->codec_id) {
    case AV_CODEC_ID_XMA2:
        par->block_align = 2048;
        em_alloc_extradata(par, 34);
        if (!par->extradata)
            return AVERROR(ENOMEM);
    ...
    case AV_CODEC_ID_ADPCM_PSX:
        ...
        if (pb->seekable)
            st->duration = av_em_get_audio_frame_duration2(par, avio_em_size(pb) - start);
        break;
    ...
    case AV_CODEC_ID_ADPCM_THP_LE:
        ...
        if ((ret = em_get_extradata(s, par, s->pb, 32)) < 0)
            return ret;
        if (pb->seekable)
            st->duration = av_em_get_audio_frame_duration2(par, avio_em_size(pb) - start);
        break;
    case AV_CODEC_ID_ADPCM_THP:
        ...
        avio_em_skip(s->pb, 0x1A4 - avio_em_tell(s->pb));

        if ((ret = em_alloc_extradata(st->codecpar, 32 * par->channels)) < 0)
            return ret;

        for (i = 0; i < par->channels; i++) {
            avio_em_read(s->pb, st->codecpar->extradata + 32 * i, 32);
            avio_em_skip(s->pb, 8);
        }
        if (pb->seekable)
            st->duration = (avio_em_size(pb) - start) / (8 * par->channels) * 14;
        break;
    ...
    case AV_CODEC_ID_PCM_S16BE:
        ...
        if (pb->seekable)
            st->duration = (avio_em_size(pb) - start) / 2 / par->channels;
        break;
    }

    avio_em_skip(pb, start - avio_em_tell(pb));
    if (par->codec_id == AV_CODEC_ID_XMA2) {
        avio_em_skip(pb, avio_em_rb32(pb) + avio_em_rb32(pb));
...
static int rsd_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if (par->codec_id == AV_CODEC_ID_ADPCM_IMA_RAD ||
        ...
        ret = av_em_get_packet(s->pb, pkt, par->block_align);
    } else if (par->codec_tag == MKTAG('W','A','D','P') &&
               par->channels > 1) {
        int i, ch;

        ret = av_em_new_packet(pkt, par->block_align);
        if (ret < 0)
            return ret;
        for (i = 0; i < 4; i++) {
            for (ch = 0; ch < par->channels; ch++) {
                pkt->data[ch * 8 + i * 2 + 0] = avio_em_r8(s->pb);
                pkt->data[ch * 8 + i * 2 + 1] = avio_em_r8(s->pb);
            }
        }
        ret = 0;
    } else {
...
AVEMInputFormat ff_rsd_demuxer = {
    .name           =   "rsd",
    ...
    .extensions     =   "rsd",
    .codec_tag      =   (const AVEMCodecTag* const []){rsd_tags, 0},
