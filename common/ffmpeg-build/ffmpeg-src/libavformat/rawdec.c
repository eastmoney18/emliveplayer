...
int ff_raw_read_partial_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    pkt->stream_index = 0;
    ret = emio_read_partial(s->pb, pkt->data, size);
    if (ret < 0) {
        av_em_packet_unref(pkt);
        return ret;
    }
    av_em_shrink_packet(pkt, ret);
    return ret;
}
...
int ff_raw_video_read_header(AVEMFormatContext *s)
{
    ...
    avpriv_em_set_pts_info(st, 64, 1, 1200000);
...
AVEMInputFormat em_data_demuxer = {
    .name           = "data",
...
static int mjpeg_probe(AVEMProbeData *p)
{
    int i;
    int state = -1;
    int nb_invalid = 0;
