...
static AVEMDictionary *map_to_opts(RTSPState *rt)
{
    AVEMDictionary *opts = NULL;
    char buf[256];

    snprintf(buf, sizeof(buf), "%d", rt->buffer_size);
    av_em_dict_set(&opts, "buffer_size", buf, 0);
...
static void rtsp_parse_range_npt(const char *p, int64_t *start, int64_t *end)
{
    ...
    *end = AV_NOPTS_VALUE;
    ...
    if (av_em_parse_time(start, buf, 1) < 0)
        return;
    if (*p == '-') {
        p++;
        get_word_sep(buf, sizeof(buf), "-", &p);
        if (av_em_parse_time(end, buf, 1) < 0)
            av_em_log(NULL, AV_LOG_DEBUG, "Failed to parse interval end specification '%s'\n", buf);
    }
}

static int get_sockaddr(AVEMFormatContext *s,
                        const char *buf, struct sockaddr_storage *sock)
{
    ...
    if ((ret = getaddrinfo(buf, NULL, &hints, &ai))) {
        av_em_log(s, AV_LOG_ERROR, "getaddrinfo(%s): %s\n",
               buf,
               gai_strerror(ret));
        return -1;
    }
...
static void init_rtp_handler(RTPDynamicProtocolHandler *handler,
                             RTSPStream *rtsp_st, AVEMStream *st)
{
    AVEMCodecParameters *par = st ? st->codecpar : NULL;
    if (!handler)
        return;
    if (par)
    ...
    if (handler->priv_data_size) {
        rtsp_st->dynamic_protocol_context = av_em_mallocz(handler->priv_data_size);
        if (!rtsp_st->dynamic_protocol_context)
            rtsp_st->dynamic_handler = NULL;
    }
}

static void finalize_rtp_handler_init(AVEMFormatContext *s, RTSPStream *rtsp_st,
                                      AVEMStream *st)
{
            ...
            if (rtsp_st->dynamic_protocol_context) {
                if (rtsp_st->dynamic_handler->close)
                    rtsp_st->dynamic_handler->close(
                        rtsp_st->dynamic_protocol_context);
                av_em_free(rtsp_st->dynamic_protocol_context);
            }
...
static int sdp_parse_rtpmap(AVEMFormatContext *s,
                            AVEMStream *st, RTSPStream *rtsp_st,
                            int payload_type, const char *p)
{
    AVEMCodecParameters *par = st->codecpar;
    char buf[256];
    int i;
    const AVEMCodecDescriptor *desc;
    const char *c_name;
    ...
    desc = avcodec_em_descriptor_get(par->codec_id);
    if (desc && desc->name)
        c_name = desc->name;
    else
        c_name = "(null)";
    ...
    case AVMEDIA_TYPE_AUDIO:
        av_em_log(s, AV_LOG_DEBUG, "audio codec set to: %s\n", c_name);
        par->sample_rate = RTSP_DEFAULT_AUDIO_SAMPLERATE;
        par->channels = RTSP_DEFAULT_NB_AUDIO_CHANNELS;
        if (i > 0) {
            par->sample_rate = i;
            avpriv_em_set_pts_info(st, 32, 1, par->sample_rate);
            get_word_sep(buf, sizeof(buf), "/", &p);
            i = atoi(buf);
            if (i > 0)
                par->channels = i;
        }
        av_em_log(s, AV_LOG_DEBUG, "audio samplerate set to: %i\n",
               par->sample_rate);
        av_em_log(s, AV_LOG_DEBUG, "audio channels set to: %i\n",
               par->channels);
        break;
    case AVMEDIA_TYPE_VIDEO:
        av_em_log(s, AV_LOG_DEBUG, "video codec set to: %s\n", c_name);
        if (i > 0)
            avpriv_em_set_pts_info(st, 32, 1, i);
        break;
    default:
        break;
    }
                                      ...
                                      struct RTSPSource ***dest, int *dest_count)
{
    ...
    for (i = 0; i < count; i++) {
        rtsp_src = addrs[i];
        rtsp_src2 = av_em_alloc(sizeof(*rtsp_src2));
        if (!rtsp_src2)
            continue;
...
static void parse_fmtp(AVEMFormatContext *s, RTSPState *rt,
...
static void sdp_parse_line(AVEMFormatContext *s, SDPParseState *s1,
                           int letter, const char *buf)
{
    ...
    av_em_log(s, AV_LOG_TRACE, "sdp: %c='%s'\n", letter, buf);
    ...
    case 'c':
        ...
        break;
    case 's':
        av_em_dict_set(&s->metadata, "title", p, 0);
        break;
    case 'i':
        if (s->nb_streams == 0) {
            av_em_dict_set(&s->metadata, "comment", p, 0);
            break;
        }
        break;
    case 'm':
        ...
        rtsp_st = av_em_mallocz(sizeof(RTSPStream));
        if (!rtsp_st)
            return;
        ...
        if (!strcmp(ff_rtp_enc_name(rtsp_st->sdp_payload_type), "MP2T")) {
            ...
            if (rtsp_st->sdp_payload_type < RTP_PT_PRIVATE) {
                ...
                if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO &&
                    ...
                    avpriv_em_set_pts_info(st, 32, 1, st->codecpar->sample_rate);
            ...
            if (rt->default_lang[0])
                av_em_dict_set(&st->metadata, "language", rt->default_lang, 0);
        }
        /* put a default control url */
        av_em_strlcpy(rtsp_st->control_url, rt->control_uri,
                   sizeof(rtsp_st->control_url));
        break;
    case 'a':
        if (av_em_strstart(p, "control:", &p)) {
            if (s->nb_streams == 0) {
                if (!strncmp(p, "rtsp://", 7))
                    av_em_strlcpy(rt->control_uri, p,
                ...
                av_em_url_split(proto, sizeof(proto), NULL, 0, NULL, 0,
                             NULL, NULL, 0, p);
                if (proto[0] == '\0') {
                    /* relative control URL */
                    if (rtsp_st->control_url[strlen(rtsp_st->control_url)-1]!='/')
                    av_em_strlcat(rtsp_st->control_url, "/",
                               sizeof(rtsp_st->control_url));
                    av_em_strlcat(rtsp_st->control_url, p,
                               sizeof(rtsp_st->control_url));
                } else
                    av_em_strlcpy(rtsp_st->control_url, p,
                               sizeof(rtsp_st->control_url));
            }
        } else if (av_em_strstart(p, "rtpmap:", &p) && s->nb_streams > 0) {
        ...
        } else if (av_em_strstart(p, "fmtp:", &p) ||
                   av_em_strstart(p, "framesize:", &p)) {
            ...
            if (s1->seen_rtpmap) {
                ...
                s1->seen_fmtp = 1;
                av_em_strlcpy(s1->delayed_fmtp, buf, sizeof(s1->delayed_fmtp));
            }
        } else if (av_em_strstart(p, "ssrc:", &p) && s->nb_streams > 0) {
            rtsp_st = rt->rtsp_streams[rt->nb_rtsp_streams - 1];
            get_word(buf1, sizeof(buf1), &p);
            rtsp_st->ssrc = strtoll(buf1, NULL, 10);
        } else if (av_em_strstart(p, "range:", &p)) {
            int64_t start, end;
        ...
        } else if (av_em_strstart(p, "lang:", &p)) {
            if (s->nb_streams > 0) {
                get_word(buf1, sizeof(buf1), &p);
                rtsp_st = rt->rtsp_streams[rt->nb_rtsp_streams - 1];
                if (rtsp_st->stream_index >= 0) {
                    st = s->streams[rtsp_st->stream_index];
                    av_em_dict_set(&st->metadata, "language", buf1, 0);
                }
            } else
                get_word(rt->default_lang, sizeof(rt->default_lang), &p);
        } else if (av_em_strstart(p, "IsRealDataType:integer;",&p)) {
            if (atoi(p) == 1)
                rt->transport = RTSP_TRANSPORT_RDT;
        } else if (av_em_strstart(p, "SampleRate:integer;", &p) &&
                   s->nb_streams > 0) {
            st = s->streams[s->nb_streams - 1];
            st->codecpar->sample_rate = atoi(p);
        } else if (av_em_strstart(p, "crypto:", &p) && s->nb_streams > 0) {
            // RFC 4568
            rtsp_st = rt->rtsp_streams[rt->nb_rtsp_streams - 1];
            get_word(buf1, sizeof(buf1), &p); // ignore tag
            get_word(rtsp_st->crypto_suite, sizeof(rtsp_st->crypto_suite), &p);
            p += strspn(p, SPACE_CHARS);
            if (av_em_strstart(p, "inline:", &p))
                get_word(rtsp_st->crypto_params, sizeof(rtsp_st->crypto_params), &p);
        } else if (av_em_strstart(p, "source-filter:", &p)) {
            int exclude = 0;
            get_word(buf1, sizeof(buf1), &p);
            ...
            while (*p != '\0') {
                rtsp_src = av_em_mallocz(sizeof(*rtsp_src));
                if (!rtsp_src)
                    return;
...
int ff_sdp_parse(AVEMFormatContext *s, const char *content)
{
    RTSPState *rt = s->priv_data;
    const char *p;
    int letter, i;
    ...
    for (i = 0; i < s1->nb_default_include_source_addrs; i++)
        av_em_freep(&s1->default_include_source_addrs[i]);
    av_em_freep(&s1->default_include_source_addrs);
    for (i = 0; i < s1->nb_default_exclude_source_addrs; i++)
        av_em_freep(&s1->default_exclude_source_addrs[i]);
    av_em_freep(&s1->default_exclude_source_addrs);

    rt->p = av_em_malloc_array(rt->nb_rtsp_streams + 1, sizeof(struct pollfd) * 2);
    if (!rt->p) return AVERROR(ENOMEM);
    return 0;
}
#endif /* CONFIG_RTPDEC */

void ff_rtsp_undo_setup(AVEMFormatContext *s, int send_packets)
{
    RTSPState *rt = s->priv_data;
    int i;
    ...
    for (i = 0; i < rt->nb_rtsp_streams; i++) {
        ...
        if (rtsp_st->transport_priv) {
            if (s->oformat) {
                AVEMFormatContext *rtpctx = rtsp_st->transport_priv;
                av_em_write_trailer(rtpctx);
        ...
        if (rtsp_st->rtp_handle)
            ffurl_em_close(rtsp_st->rtp_handle);
        rtsp_st->rtp_handle = NULL;
    }
}

/* close and free RTSP streams */
void ff_rtsp_close_streams(AVEMFormatContext *s)
{
    RTSPState *rt = s->priv_data;
    int i, j;
    RTSPStream *rtsp_st;
    ...
    for (i = 0; i < rt->nb_rtsp_streams; i++) {
        ...
        if (rtsp_st) {
            ...
            for (j = 0; j < rtsp_st->nb_include_source_addrs; j++)
                av_em_freep(&rtsp_st->include_source_addrs[j]);
            av_em_freep(&rtsp_st->include_source_addrs);
            for (j = 0; j < rtsp_st->nb_exclude_source_addrs; j++)
                av_em_freep(&rtsp_st->exclude_source_addrs[j]);
            av_em_freep(&rtsp_st->exclude_source_addrs);

            av_em_freep(&rtsp_st);
        }
    }
    av_em_freep(&rt->rtsp_streams);
    if (rt->asf_ctx) {
        avformat_em_close_input(&rt->asf_ctx);
    }
    if (CONFIG_RTPDEC && rt->ts)
        avpriv_mpegts_parse_close(rt->ts);
    av_em_freep(&rt->p);
    av_em_freep(&rt->recvbuf);
}

int ff_rtsp_open_transport_ctx(AVEMFormatContext *s, RTSPStream *rtsp_st)
{
    RTSPState *rt = s->priv_data;
    AVEMStream *st = NULL;
    ...
    if (CONFIG_RTSP_MUXER && s->oformat && st) {
        int ret = ff_rtp_chain_mux_open((AVEMFormatContext **)&rtsp_st->transport_priv,
        ...
        if (ret < 0)
            return ret;
        st->time_base = ((AVEMFormatContext*)rtsp_st->transport_priv)->streams[0]->time_base;
...
static void rtsp_parse_transport(AVEMFormatContext *s,
                                 RTSPMessageHeader *reply, const char *p)
{
    ...
    for (;;) {
        ...
        get_word_sep(transport_protocol, sizeof(transport_protocol),
                     "/", &p);
        if (!av_em_strcasecmp (transport_protocol, "rtp")) {
        ...
        } else if (!av_em_strcasecmp (transport_protocol, "x-pn-tng") ||
                   !av_em_strcasecmp (transport_protocol, "x-real-rdt")) {
            /* x-pn-tng/<protocol> */
            get_word_sep(lower_transport, sizeof(lower_transport), "/;,", &p);
            profile[0] = '\0';
            th->transport = RTSP_TRANSPORT_RDT;
        } else if (!av_em_strcasecmp(transport_protocol, "raw")) {
            get_word_sep(profile, sizeof(profile), "/;,", &p);
            lower_transport[0] = '\0';
            /* raw/raw/<protocol> */
            if (*p == '/') {
        ...
        if (!av_em_strcasecmp(lower_transport, "TCP"))
        ...
        while (*p != '\0' && *p != ',') {
            ...
            if (!strcmp(parameter, "port")) {
                ...
                if (*p == '=') {
                    p++;
                    get_word_sep(buf, sizeof(buf), ";,", &p);
                    av_em_strlcpy(th->source, buf, sizeof(th->source));
                }
...
static void rtsp_parse_rtp_info(RTSPState *rt, const char *p)
{
    ...
    for (;;) {
        ...
        read++;
        if (!strcmp(key, "url"))
            av_em_strlcpy(url, value, sizeof(url));
...
void ff_rtsp_parse_line(AVEMFormatContext *s,
                        ...
                        RTSPState *rt, const char *method)
{
    ...
    p = buf;
    if (av_em_stristart(p, "Session:", &p)) {
        int t;
        get_word_sep(reply->session_id, sizeof(reply->session_id), ";", &p);
        if (av_em_stristart(p, ";timeout=", &p) &&
            (t = strtol(p, NULL, 10)) > 0) {
            reply->timeout = t;
        }
    } else if (av_em_stristart(p, "Content-Length:", &p)) {
        reply->content_length = strtol(p, NULL, 10);
    } else if (av_em_stristart(p, "Transport:", &p)) {
        rtsp_parse_transport(s, reply, p);
    } else if (av_em_stristart(p, "CSeq:", &p)) {
        reply->seq = strtol(p, NULL, 10);
    } else if (av_em_stristart(p, "Range:", &p)) {
        rtsp_parse_range_npt(p, &reply->range_start, &reply->range_end);
    } else if (av_em_stristart(p, "RealChallenge1:", &p)) {
        p += strspn(p, SPACE_CHARS);
        av_em_strlcpy(reply->real_challenge, p, sizeof(reply->real_challenge));
    } else if (av_em_stristart(p, "Server:", &p)) {
        p += strspn(p, SPACE_CHARS);
        av_em_strlcpy(reply->server, p, sizeof(reply->server));
    } else if (av_em_stristart(p, "Notice:", &p) ||
               av_em_stristart(p, "X-Notice:", &p)) {
        reply->notice = strtol(p, NULL, 10);
    } else if (av_em_stristart(p, "Location:", &p)) {
        p += strspn(p, SPACE_CHARS);
        av_em_strlcpy(reply->location, p , sizeof(reply->location));
    } else if (av_em_stristart(p, "WWW-Authenticate:", &p) && rt) {
        p += strspn(p, SPACE_CHARS);
        em_http_auth_handle_header(&rt->auth_state, "WWW-Authenticate", p);
    } else if (av_em_stristart(p, "Authentication-Info:", &p) && rt) {
        p += strspn(p, SPACE_CHARS);
        em_http_auth_handle_header(&rt->auth_state, "Authentication-Info", p);
    } else if (av_em_stristart(p, "Content-Base:", &p) && rt) {
        p += strspn(p, SPACE_CHARS);
        if (method && !strcmp(method, "DESCRIBE"))
            av_em_strlcpy(rt->control_uri, p , sizeof(rt->control_uri));
    } else if (av_em_stristart(p, "RTP-Info:", &p) && rt) {
        p += strspn(p, SPACE_CHARS);
        if (method && !strcmp(method, "PLAY"))
            rtsp_parse_rtp_info(rt, p);
    } else if (av_em_stristart(p, "Public:", &p) && rt) {
        if (strstr(p, "GET_PARAMETER") &&
            method && !strcmp(method, "OPTIONS"))
            rt->get_parameter_supported = 1;
    } else if (av_em_stristart(p, "x-Accept-Dynamic-Rate:", &p) && rt) {
        p += strspn(p, SPACE_CHARS);
        rt->accept_dynamic_rate = atoi(p);
    } else if (av_em_stristart(p, "Content-Type:", &p)) {
        p += strspn(p, SPACE_CHARS);
        av_em_strlcpy(reply->content_type, p, sizeof(reply->content_type));
    }
}

/* skip a RTP/TCP interleaved packet */
void ff_rtsp_skip_packet(AVEMFormatContext *s)
{
    RTSPState *rt = s->priv_data;
    int ret, len, len1;
    uint8_t buf[1024];

    ret = ffurl_em_read_complete(rt->rtsp_hd, buf, 3);
    if (ret != 3)
        return;
    len = AV_RB16(buf + 1);

    av_em_log(s, AV_LOG_TRACE, "skipping RTP packet len=%d\n", len);
    ...
    while (len > 0) {
        ...
        if (len1 > sizeof(buf))
            len1 = sizeof(buf);
        ret = ffurl_em_read_complete(rt->rtsp_hd, buf, len1);
        if (ret != len1)
            return;
        len -= len1;
    }
...
int ff_rtsp_read_reply(AVEMFormatContext *s, RTSPMessageHeader *reply,
...
start:
    ...
    for (;;) {
        q = buf;
        for (;;) {
            ret = ffurl_em_read_complete(rt->rtsp_hd, &ch, 1);
            av_em_log(s, AV_LOG_TRACE, "ret=%d c=%02x [%c]\n", ret, ch, ch);
            if (ret != 1)
                return AVERROR_EOF;
            if (ch == '\n')
                break;
        ...
        *q = '\0';
        ...
        av_em_log(s, AV_LOG_TRACE, "line='%s'\n", buf);
        ...
        if (line_count == 0) {
            ...
            if (!strncmp(buf1, "RTSP/", 5)) {
                ...
                av_em_strlcpy(reply->reason, p, sizeof(reply->reason));
            } else {
                av_em_strlcpy(reply->reason, buf1, sizeof(reply->reason)); // method
                get_word(buf1, sizeof(buf1), &p); // object
                request = 1;
            }
        } else {
            ff_rtsp_parse_line(s, reply, p, rt, method);
            av_em_strlcat(rt->last_reply, p,    sizeof(rt->last_reply));
            av_em_strlcat(rt->last_reply, "\n", sizeof(rt->last_reply));
        }
        line_count++;
    }

    if (rt->session_id[0] == '\0' && reply->session_id[0] != '\0' && !request)
        av_em_strlcpy(rt->session_id, reply->session_id, sizeof(rt->session_id));

    content_length = reply->content_length;
    if (content_length > 0) {
        /* leave some room for a trailing '\0' (useful for simple parsing) */
        content = av_em_alloc(content_length + 1);
        if (!content)
            return AVERROR(ENOMEM);
        ffurl_em_read_complete(rt->rtsp_hd, content, content_length);
    ...
    if (request) {
        ...
        if (!strcmp(reply->reason, "OPTIONS")) {
            ...
            if (reply->seq)
                av_em_strlcatf(buf, sizeof(buf), "CSeq: %d\r\n", reply->seq);
            if (reply->session_id[0])
                av_em_strlcatf(buf, sizeof(buf), "Session: %s\r\n",
                                              reply->session_id);
        } else {
        ...
        if (rt->control_transport == RTSP_MODE_TUNNEL) {
            av_em_base64_encode(base64buf, sizeof(base64buf), buf, strlen(buf));
            ptr = base64buf;
        }
        ffurl_em_write(rt->rtsp_hd_out, ptr, strlen(ptr));

        rt->last_cmd_time = av_em_gettime_relative();
    ...
    if (rt->seq != reply->seq) {
        av_em_log(s, AV_LOG_WARNING, "CSeq %d expected, %d received.\n",
            rt->seq, reply->seq);
    }
 ...
 */
static int rtsp_send_cmd_with_content_async(AVEMFormatContext *s,
                                            ...
                                            int send_content_length)
{
    ...
    if (headers)
        av_em_strlcat(buf, headers, sizeof(buf));
    av_em_strlcatf(buf, sizeof(buf), "CSeq: %d\r\n", rt->seq);
    av_em_strlcatf(buf, sizeof(buf), "User-Agent: %s\r\n",  rt->user_agent);
    if (rt->session_id[0] != '\0' && (!headers ||
        !strstr(headers, "\nIf-Match:"))) {
        av_em_strlcatf(buf, sizeof(buf), "Session: %s\r\n", rt->session_id);
    }
    if (rt->auth[0]) {
        char *str = em_http_auth_create_response(&rt->auth_state,
                                                 rt->auth, url, method);
        if (str)
            av_em_strlcat(buf, str, sizeof(buf));
        av_em_free(str);
    }
    if (send_content_length > 0 && send_content)
        av_em_strlcatf(buf, sizeof(buf), "Content-Length: %d\r\n", send_content_length);
    av_em_strlcat(buf, "\r\n", sizeof(buf));
    ...
    av_em_log(s, AV_LOG_TRACE, "Sending:\n%s--\n", buf);

    ffurl_em_write(rt->rtsp_hd_out, out_buf, strlen(out_buf));
    if (send_content_length > 0 && send_content) {
        if (rt->control_transport == RTSP_MODE_TUNNEL) {
            av_em_log(s, AV_LOG_ERROR, "tunneling of RTSP requests "
                                    "with content data not supported\n");
            return AVERROR_PATCHWELCOME;
        }
        ffurl_em_write(rt->rtsp_hd_out, send_content, send_content_length);
    }
    rt->last_cmd_time = av_em_gettime_relative();
...
int ff_rtsp_send_cmd_async(AVEMFormatContext *s, const char *method,
                           const char *url, const char *headers)
{
    return rtsp_send_cmd_with_content_async(s, method, url, headers, NULL, 0);
}

int ff_rtsp_send_cmd(AVEMFormatContext *s, const char *method, const char *url,
...
int ff_rtsp_send_cmd_with_content(AVEMFormatContext *s,
                                  const char *method, const char *url,
                                  const char *header,
...
retry:
    ...
    if (reply->status_code > 400){
        av_em_log(s, AV_LOG_ERROR, "method %s failed: %d%s\n",
               method,
               reply->status_code,
               reply->reason);
...
int ff_rtsp_make_setup_request(AVEMFormatContext *s, const char *host, int port,
                              int lower_transport, const char *real_challenge)
{
    ...
    port_off = av_em_get_random_seed() % ((rt->rtp_port_max - rt->rtp_port_min)/2);
    /* even random offset */
    ...
    for (j = rt->rtp_port_min + port_off, i = 0; i < rt->nb_rtsp_streams; ++i) {
        ...
        if (lower_transport == RTSP_LOWER_TRANSPORT_UDP) {
            ...
            while (j <= rt->rtp_port_max) {
                AVEMDictionary *opts = map_to_opts(rt);

                em_url_join(buf, sizeof(buf), "rtp", NULL, host, -1,
                            "?localport=%d", j);
                /* we will use two ports per rtp stream (rtp and rtcp) */
                j += 2;
                err = ffurl_em_open_whitelist(&rtsp_st->rtp_handle, buf, AVIO_FLAG_READ_WRITE,
            ...
            av_em_log(s, AV_LOG_ERROR, "Unable to open an input RTP port\n");
            err = AVERROR(EIO);
            goto fail;
        ...
        have_port:
            snprintf(transport, sizeof(transport) - 1,
                     "%s/UDP;", trans_pref);
            if (rt->server_type != RTSP_SERVER_REAL)
                av_em_strlcat(transport, "unicast;", sizeof(transport));
            av_em_strlcatf(transport, sizeof(transport),
                     "client_port=%d", port);
            if (rt->transport == RTSP_TRANSPORT_RTP &&
                !(rt->server_type == RTSP_SERVER_WMS && i > 0))
                av_em_strlcatf(transport, sizeof(transport), "-%d", port + 1);
        }
        ...
        if (s->oformat) {
            av_em_strlcat(transport, ";mode=record", sizeof(transport));
        } else if (rt->server_type == RTSP_SERVER_REAL ||
                   rt->server_type == RTSP_SERVER_WMS)
            av_em_strlcat(transport, ";mode=play", sizeof(transport));
        snprintf(cmd, sizeof(cmd),
                 "Transport: %s\r\n",
                 transport);
        if (rt->accept_dynamic_rate)
            av_em_strlcat(cmd, "x-Dynamic-Rate: 0\r\n", sizeof(cmd));
        if (CONFIG_RTPDEC && i == 0 && rt->server_type == RTSP_SERVER_REAL) {
            char real_res[41], real_csum[9];
            ff_rdt_calc_response_and_checksum(real_res, real_csum,
                                              real_challenge);
            av_em_strlcatf(cmd, sizeof(cmd),
                        "If-Match: %s\r\n"
        ...
        if (reply->transports[0].lower_transport != lower_transport) {
            av_em_log(s, AV_LOG_ERROR, "Nonmatching transport in server reply\n");
            err = AVERROR_INVALIDDATA;
            goto fail;
        }
        ...
        case RTSP_LOWER_TRANSPORT_UDP: {
            ...
            if (rt->rtsp_flags & RTSP_FLAG_FILTER_SRC)
                av_em_strlcpy(options, "?connect=1", sizeof(options));
            /* Use source address if specified */
            if (reply->transports[0].source[0])
                peer = reply->transports[0].source;
            em_url_join(url, sizeof(url), "rtp", NULL, peer,
        ...
        case RTSP_LOWER_TRANSPORT_UDP_MULTICAST: {
            ...
            em_url_join(url, sizeof(url), "rtp", NULL, namebuf,
                        port, "%s", optbuf);
            if (ffurl_em_open_whitelist(&rtsp_st->rtp_handle, url, AVIO_FLAG_READ_WRITE,
...
void ff_rtsp_close_connections(AVEMFormatContext *s)
{
    RTSPState *rt = s->priv_data;
    if (rt->rtsp_hd_out != rt->rtsp_hd) ffurl_em_close(rt->rtsp_hd_out);
    ffurl_em_close(rt->rtsp_hd);
    rt->rtsp_hd = rt->rtsp_hd_out = NULL;
}

int ff_rtsp_connect(AVEMFormatContext *s)
{
    RTSPState *rt = s->priv_data;
    ...
    if (rt->rtp_port_max < rt->rtp_port_min) {
        av_em_log(s, AV_LOG_ERROR, "Invalid UDP port range, max port %d less "
                                "than min port %d\n", rt->rtp_port_max,
                                                      rt->rtp_port_min);
...
redirect:
    ...
    if (*auth) {
        av_em_strlcpy(rt->auth, auth, sizeof(rt->auth));
    }
    if (port < 0)
        port = default_port;
    ...
    if (s->oformat) {
        ...
        if (!lower_transport_mask || rt->control_transport == RTSP_MODE_TUNNEL) {
            av_em_log(s, AV_LOG_ERROR, "Unsupported lower transport method, "
     ...
     * stripped out. */
    em_url_join(rt->control_uri, sizeof(rt->control_uri), proto, NULL,
                host, port, "%s", path);
    ...
    if (rt->control_transport == RTSP_MODE_TUNNEL) {
        ...
        em_url_join(httpname, sizeof(httpname), "http", auth, host, port, "%s", path);
        snprintf(sessioncookie, sizeof(sessioncookie), "%08x%08x",
                 av_em_get_random_seed(), av_em_get_random_seed());

        /* GET requests */
        if (ffurl_em_alloc(&rt->rtsp_hd, httpname, AVIO_FLAG_READ,
        ...
        snprintf(headers, sizeof(headers),
                 ...
                 sessioncookie);
        av_em_opt_set(rt->rtsp_hd->priv_data, "headers", headers, 0);

        /* complete the connection */
        if (ffurl_em_connect(rt->rtsp_hd, NULL)) {
            err = AVERROR(EIO);
            goto fail;
        }

        /* POST requests */
        if (ffurl_em_alloc(&rt->rtsp_hd_out, httpname, AVIO_FLAG_WRITE,
                        &s->interrupt_callback) < 0 ) {
            err = AVERROR(EIO);
            goto fail;
        }
        ...
        snprintf(headers, sizeof(headers),
                 ...
                 sessioncookie);
        av_em_opt_set(rt->rtsp_hd_out->priv_data, "headers", headers, 0);
        av_em_opt_set(rt->rtsp_hd_out->priv_data, "chunked_post", "0", 0);
         ...
         */
        em_http_init_auth_state(rt->rtsp_hd_out, rt->rtsp_hd);

        /* complete the connection */
        if (ffurl_em_connect(rt->rtsp_hd_out, NULL)) {
            err = AVERROR(EIO);
            goto fail;
        }
    } else {
        ...
        em_url_join(tcpname, sizeof(tcpname), lower_rtsp_proto, NULL,
                    host, port,
                    "?timeout=%d", rt->stimeout);
        if ((ret = ffurl_em_open_whitelist(&rt->rtsp_hd, tcpname, AVIO_FLAG_READ_WRITE,
                       &s->interrupt_callback, NULL, s->protocol_whitelist, s->protocol_blacklist, NULL)) < 0) {
            err = ret;
            goto fail;
        }
    ...
    tcp_fd = ffurl_em_get_file_handle(rt->rtsp_hd);
    if (tcp_fd < 0) {
        err = tcp_fd;
        goto fail;
    }
    ...
    for (rt->server_type = RTSP_SERVER_RTP;;) {
        ...
        if (rt->server_type != RTSP_SERVER_REAL && reply->real_challenge[0]) {
            ...
            continue;
        } else if (!av_em_strncasecmp(reply->server, "WMServer/", 9)) {
            rt->server_type = RTSP_SERVER_WMS;
        } else if (rt->server_type == RTSP_SERVER_REAL)
    ...
    av_em_strlcpy(rt->real_challenge, real_challenge, sizeof(rt->real_challenge));
    rt->state = RTSP_STATE_IDLE;
 ...
 fail:
    ...
    if (reply->status_code >=300 && reply->status_code < 400 && s->iformat) {
        av_em_strlcpy(s->filename, reply->location, sizeof(s->filename));
        rt->session_id[0] = '\0';
        av_em_log(s, AV_LOG_INFO, "Status %d: Redirecting to %s\n",
               reply->status_code,
               s->filename);
        goto redirect;
    }
...
#if CONFIG_RTPDEC
static int udp_read_packet(AVEMFormatContext *s, RTSPStream **prtsp_st,
                           uint8_t *buf, int buf_size, int64_t wait_end)
{
    ...
    for (;;) {
        if (em_check_interrupt(&s->interrupt_callback))
            return AVERROR_EXIT;
        if (wait_end && wait_end - av_em_gettime_relative() < 0)
        ...
        for (i = 0; i < rt->nb_rtsp_streams; i++) {
            rtsp_st = rt->rtsp_streams[i];
            if (rtsp_st->rtp_handle) {
                if (ret = ffurl_em_get_multi_file_handle(rtsp_st->rtp_handle,
                                                      &fds, &fdsnum)) {
                    av_em_log(s, AV_LOG_ERROR, "Unable to recover rtp ports\n");
                    return ret;
                }
                if (fdsnum != 2) {
            ...
            for (i = 0; i < rt->nb_rtsp_streams; i++) {
                ...
                if (rtsp_st->rtp_handle) {
                    if (p[j].revents & POLLIN || p[j+1].revents & POLLIN) {
                        ret = ffurl_em_read(rtsp_st->rtp_handle, buf, buf_size);
                        if (ret > 0) {
                            *prtsp_st = rtsp_st;
                            return ret;
                        }
...
static int pick_stream(AVEMFormatContext *s, RTSPStream **rtsp_st,
                       const uint8_t *buf, int len)
{
    ...
    av_em_log(s, AV_LOG_WARNING, "Unable to pick stream for packet\n");
    return AVERROR(EAGAIN);
}

int ff_rtsp_fetch_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    RTSPState *rt = s->priv_data;
    int ret, len;
...
redo:
    ...
    if (!rt->recvbuf) {
        rt->recvbuf = av_em_alloc(RECVBUF_SIZE);
        if (!rt->recvbuf)
            return AVERROR(ENOMEM);
    }
    ...
    case RTSP_LOWER_TRANSPORT_CUSTOM:
        if (first_queue_st && rt->transport == RTSP_TRANSPORT_RTP &&
            wait_end && wait_end < av_em_gettime_relative())
            len = AVERROR(EAGAIN);
        else
            len = emio_read_partial(s->pb, rt->recvbuf, RECVBUF_SIZE);
    ...
    if (rt->transport == RTSP_TRANSPORT_RDT) {
        ...
        if (ret < 0) {
            ...
            if (rtpctx->first_rtcp_ntp_time != AV_NOPTS_VALUE) {
                ...
                for (i = 0; i < rt->nb_rtsp_streams; i++) {
                    ...
                    if (rtpctx2 && st && st2 &&
                        ...
                        rtpctx2->rtcp_ts_offset = av_em_rescale_q(
                            rtpctx->rtcp_ts_offset, st->time_base,
                            st2->time_base);
                    }
                }
                // Make real NTP start time available in AVEMFormatContext
                if (s->start_time_realtime == AV_NOPTS_VALUE) {
                    s->start_time_realtime = av_em_rescale (rtpctx->first_rtcp_ntp_time - (NTP_OFFSET << 32), 1000000, 1LL << 32);
                    if (rtpctx->st) {
                        s->start_time_realtime -=
                            av_em_rescale (rtpctx->rtcp_ts_offset,
            ...
            if (ret == -RTCP_BYE) {
                ...
                av_em_log(s, AV_LOG_DEBUG, "Received BYE for stream %d (%d/%d)\n",
...
static int sdp_probe(AVEMProbeData *p1)
{
                                ...
                                int count, struct RTSPSource **addrs)
{
    ...
    if (!count)
        return;
    av_em_strlcatf(buf, size, "&%s=%s", name, addrs[0]->addr);
    for (i = 1; i < count; i++)
        av_em_strlcatf(buf, size, ",%s", addrs[i]->addr);
}

static int sdp_read_header(AVEMFormatContext *s)
{
    RTSPState *rt = s->priv_data;
    RTSPStream *rtsp_st;
    int size, i, err;
    ...
    /* XXX: better loading */
    content = av_em_alloc(SDP_MAX_SIZE);
    if (!content)
        return AVERROR(ENOMEM);
    size = avio_em_read(s->pb, content, SDP_MAX_SIZE - 1);
    if (size <= 0) {
        av_em_free(content);
        return AVERROR_INVALIDDATA;
    }
    ...
    for (i = 0; i < rt->nb_rtsp_streams; i++) {
        ...
        if (!(rt->rtsp_flags & RTSP_FLAG_CUSTOM_IO)) {
            ...
            err = getnameinfo((struct sockaddr*) &rtsp_st->sdp_ip,
                              ...
                              namebuf, sizeof(namebuf), NULL, 0, NI_NUMERICHOST);
            if (err) {
                av_em_log(s, AV_LOG_ERROR, "getnameinfo: %s\n", gai_strerror(err));
                err = AVERROR(EIO);
                av_em_dict_free(&opts);
                goto fail;
            }
            em_url_join(url, sizeof(url), "rtp", NULL,
            ...
            append_source_addrs(url, sizeof(url), "block",
                                rtsp_st->nb_exclude_source_addrs,
                                rtsp_st->exclude_source_addrs);
            err = ffurl_em_open_whitelist(&rtsp_st->rtp_handle, url, AVIO_FLAG_READ,
                           &s->interrupt_callback, &opts, s->protocol_whitelist, s->protocol_blacklist, NULL);
...
static int sdp_read_close(AVEMFormatContext *s)
{
    ff_rtsp_close_streams(s);
    em_network_close();
    return 0;
...
static const AVEMClass sdp_demuxer_class = {
...
AVEMInputFormat ff_sdp_demuxer = {
    .name           = "sdp",
...
static int rtp_probe(AVEMProbeData *p)
{
    if (av_em_strstart(p->filename, "rtp:", NULL))
        return AVPROBE_SCORE_MAX;
    return 0;
}

static int rtp_read_header(AVEMFormatContext *s)
{
    uint8_t recvbuf[RTP_MAX_PACKET_LENGTH];
    char host[500], sdp[500];
    int ret, port;
    EMURLContext* in = NULL;
    int payload_type;
    AVEMCodecParameters *par = NULL;
    struct sockaddr_storage addr;
    AVEMIOContext pb;
    ...
    ret = ffurl_em_open_whitelist(&in, s->filename, AVIO_FLAG_READ,
                     &s->interrupt_callback, NULL, s->protocol_whitelist, s->protocol_blacklist, NULL);
    if (ret)
        goto fail;

    while (1) {
        ret = ffurl_em_read(in, recvbuf, sizeof(recvbuf));
        if (ret == AVERROR(EAGAIN))
            continue;
        if (ret < 0)
            goto fail;
        if (ret < 12) {
            av_em_log(s, AV_LOG_WARNING, "Received too short packet\n");
            continue;
        }

        if ((recvbuf[0] & 0xc0) != 0x80) {
            av_em_log(s, AV_LOG_WARNING, "Unsupported RTP version packet "
                                      "received\n");
            continue;
        }
    ...
    getsockname(ffurl_em_get_file_handle(in), (struct sockaddr*) &addr, &addrlen);
    ffurl_em_close(in);
    in = NULL;

    par = avcodec_em_parameters_alloc();
    if (!par) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    if (ff_rtp_get_codec_info(par, payload_type)) {
        av_em_log(s, AV_LOG_ERROR, "Unable to receive RTP payload type %d "
                                "without an SDP file describing it\n",
                                 payload_type);
        goto fail;
    }
    if (par->codec_type != AVMEDIA_TYPE_DATA) {
        av_em_log(s, AV_LOG_WARNING, "Guessing on RTP content - if not received "
                                  "properly you need an SDP file "
                                  "describing it\n");
    }

    av_em_url_split(NULL, 0, NULL, 0, host, sizeof(host), &port,
                 NULL, 0, s->filename);
    ...
    snprintf(sdp, sizeof(sdp),
             ...
             port, payload_type);
    av_em_log(s, AV_LOG_VERBOSE, "SDP:\n%s\n", sdp);
    avcodec_em_parameters_free(&par);

    emio_init_context(&pb, sdp, strlen(sdp), 0, NULL, NULL, NULL, NULL);
    s->pb = &pb;
...
static const AVEMClass rtp_demuxer_class = {
...
AVEMInputFormat ff_rtp_demuxer = {
    .name           = "rtp",
