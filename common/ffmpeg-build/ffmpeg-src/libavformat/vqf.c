...
static int vqf_probe(AVEMProbeData *probe_packet)
{
...
static void add_metadata(AVEMFormatContext *s, uint32_t tag,
                         unsigned int tag_len, unsigned int remaining)
{
    ...
    buf = av_em_alloc(len+1);
    if (!buf)
        return;
    avio_em_read(s->pb, buf, len);
    buf[len] = 0;
    AV_WL32(key, tag);
    av_em_dict_set(&s->metadata, key, buf, AV_DICT_DONT_STRDUP_VAL);
}
...
static int vqf_read_header(AVEMFormatContext *s)
{
    ...
    do {
        int len;
        chunk_tag = avio_em_rl32(s->pb);

        if (chunk_tag == MKTAG('D','A','T','A'))
            break;

        len = avio_em_rb32(s->pb);

        if ((unsigned) len > INT_MAX/2) {
            av_em_log(s, AV_LOG_ERROR, "Malformed header\n");
            return -1;
        }
        ...
        switch(chunk_tag){
        case MKTAG('C','O','M','M'):
            avio_em_read(s->pb, comm_chunk, 12);
            st->codecpar->channels = AV_RB32(comm_chunk    ) + 1;
            read_bitrate        = AV_RB32(comm_chunk + 4);
            rate_flag           = AV_RB32(comm_chunk + 8);
            avio_em_skip(s->pb, len-12);

            if (st->codecpar->channels <= 0) {
                av_em_log(s, AV_LOG_ERROR, "Invalid number of channels\n");
        ...
        {
            av_em_dict_set_int(&s->metadata, "size", avio_em_rb32(s->pb), 0);
        }
            break;
        ...
        case MKTAG('_','I','D','3'): // reserved for ID3 tags
            avio_em_skip(s->pb, FFMIN(len, header_size));
            break;
        default:
    ...
    } while (header_size >= 0 && !avio_em_feof(s->pb));

    switch (rate_flag) {
    case -1:
        av_em_log(s, AV_LOG_ERROR, "COMM tag not found!\n");
        return -1;
    case 44:
    ...
    default:
        if (rate_flag < 8 || rate_flag > 44) {
            av_em_log(s, AV_LOG_ERROR, "Invalid rate flag %d\n", rate_flag);
            return AVERROR_INVALIDDATA;
        }
    ...
    if (read_bitrate / st->codecpar->channels <  8 ||
        ...
        av_em_log(s, AV_LOG_ERROR, "Invalid bitrate per channel %d\n",
    ...
    case (44<<8) + 48:
        ...
        break;
    default:
        av_em_log(s, AV_LOG_ERROR, "Mode not supported: %d Hz, %"PRId64" kb/s.\n",
               st->codecpar->sample_rate, (int64_t)st->codecpar->bit_rate);
        return -1;
    }
    c->frame_bit_len = st->codecpar->bit_rate*size/st->codecpar->sample_rate;
    avpriv_em_set_pts_info(st, 64, size, st->codecpar->sample_rate);

    /* put first 12 bytes of COMM chunk in extradata */
    if (em_alloc_extradata(st->codecpar, 12))
        return AVERROR(ENOMEM);
...
static int vqf_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    VqfContext *c = s->priv_data;
    int ret;
    int size = (c->frame_bit_len - c->remaining_bits + 7)>>3;

    if (av_em_new_packet(pkt, size+2) < 0)
        return AVERROR(EIO);
    ...
    ret = avio_em_read(s->pb, pkt->data+2, size);
...
static int vqf_read_seek(AVEMFormatContext *s,
                         int stream_index, int64_t timestamp, int flags)
{
    ...
    pos = av_em_rescale_rnd(timestamp * st->codecpar->bit_rate,
                         st->time_base.num,
    ...
    st->cur_dts = av_em_rescale(pos, st->time_base.den,
                             st->codecpar->bit_rate * (int64_t)st->time_base.num);

    if ((ret = avio_em_seek(s->pb, ((pos-7) >> 3) + s->internal->data_offset, SEEK_SET)) < 0)
        return ret;
...
AVEMInputFormat ff_vqf_demuxer = {
    .name           = "vqf",
