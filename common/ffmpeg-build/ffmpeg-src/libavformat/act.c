...
static int read_header(AVEMFormatContext *s)
{
    ...
    */
    if (st->codecpar->sample_rate != 8000) {
        av_em_log(s, AV_LOG_ERROR, "Sample rate %d is not supported.\n", st->codecpar->sample_rate);
    ...
    avio_em_seek(pb, 257, SEEK_SET);
    msec=avio_em_rl16(pb);
    sec=avio_em_r8(pb);
    min=avio_em_rl32(pb);

    st->duration = av_em_rescale(1000*(min*60+sec)+msec, st->codecpar->sample_rate, 1000 * st->codecpar->frame_size);

    ctx->bytes_left_in_chunk=CHUNK_SIZE;

    avio_em_seek(pb, 512, SEEK_SET);
                          ...
                          AVEMPacket *pkt)
{
    ...
    if(s->streams[0]->codecpar->sample_rate==8000)
        ret=av_em_new_packet(pkt, 10);
    else
        ret=av_em_new_packet(pkt, 11);
    ...
    {
        ret = avio_em_read(pb, ctx->audio_buffer, frame_size);
    ...
    {
        avio_em_skip(pb, ctx->bytes_left_in_chunk);
        ctx->bytes_left_in_chunk=CHUNK_SIZE;
    }
...
AVEMInputFormat ff_act_demuxer = {
    .name           = "act",
