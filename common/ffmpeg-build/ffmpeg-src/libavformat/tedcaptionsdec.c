...
static const AVEMClass tedcaptions_demuxer_class = {
...
static void av_bprint_utf8(AVEMBPrint *bp, unsigned c)
{
    int bytes, i;

    if (c <= 0x7F) {
        av_em_bprint_chars(bp, c, 1);
        return;
    }
    bytes = (av_log2(c) - 2) / 5;
    av_em_bprint_chars(bp, (c >> (bytes * 6)) | ((0xFF80 >> bytes) & 0xFF), 1);
    for (i = bytes - 1; i >= 0; i--)
        av_em_bprint_chars(bp, ((c >> (i * 6)) & 0x3F) | 0x80, 1);
}

static void next_byte(AVEMIOContext *pb, int *cur_byte)
{
    uint8_t b;
    int ret = avio_em_read(pb, &b, 1);
    *cur_byte = ret > 0 ? b : ret == 0 ? AVERROR_EOF : ret;
}

static void skip_spaces(AVEMIOContext *pb, int *cur_byte)
{
...
static int expect_byte(AVEMIOContext *pb, int *cur_byte, uint8_t c)
{
...
static int parse_string(AVEMIOContext *pb, int *cur_byte, AVEMBPrint *bp, int full)
{
    int ret;

    av_em_bprint_init(bp, 0, full ? -1 : 1);
    ...
    while (*cur_byte > 0 && *cur_byte != '"') {
        if (*cur_byte == '\\') {
            ...
            if ((*cur_byte | 32) == 'u') {
                ...
                av_em_bprint_chars(bp, *cur_byte, 1);
            }
        } else {
            av_em_bprint_chars(bp, *cur_byte, 1);
        }
    ...
    if (ret < 0)
        goto fail;
    if (full && !av_em_bprint_is_complete(bp)) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    return 0;
...
static int parse_label(AVEMIOContext *pb, int *cur_byte, AVEMBPrint *bp)
{
    int ret;
...
static int parse_boolean(AVEMIOContext *pb, int *cur_byte, int *result)
{
...
static int parse_int(AVEMIOContext *pb, int *cur_byte, int64_t *result)
{
    int64_t val = 0;
...
static int parse_file(AVEMIOContext *pb, FFDemuxSubtitlesQueue *subs)
{
    int ret, cur_byte, start_of_par;
    AVEMBPrint label, content;
    ...
    while (1) {
        ...
        av_em_bprint_finalize(&content, NULL);
...
static av_cold int tedcaptions_read_header(AVEMFormatContext *avf)
{
    ...
    if (ret < 0) {
        if (ret == AVERROR_INVALIDDATA)
            av_em_log(avf, AV_LOG_ERROR, "Syntax error near offset %"PRId64".\n",
                   avio_em_tell(avf->pb));
...
static int tedcaptions_read_packet(AVEMFormatContext *avf, AVEMPacket *packet)
{
    TEDCaptionsDemuxer *tc = avf->priv_data;
...
static int tedcaptions_read_close(AVEMFormatContext *avf)
{
    TEDCaptionsDemuxer *tc = avf->priv_data;
...
static av_cold int tedcaptions_read_probe(AVEMProbeData *p)
{
...
static int tedcaptions_read_seek(AVEMFormatContext *avf, int stream_index,
...
AVEMInputFormat ff_tedcaptions_demuxer = {
    .name           = "tedcaptions",
