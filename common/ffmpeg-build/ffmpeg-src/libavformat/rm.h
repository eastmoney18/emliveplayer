...
extern const AVEMCodecTag ff_rm_codec_tags[];
...
extern AVEMInputFormat ff_rdt_demuxer;

/**
 * Read the MDPR chunk, which contains stream-specific codec initialization
 * parameters.
 *
 * @param s context containing RMContext and AVEMIOContext for stream reading
 ...
 */
int ff_rm_parse_packet (AVEMFormatContext *s, AVEMIOContext *pb,
                        AVEMStream *st, RMStream *rst, int len,
                        AVEMPacket *pkt, int *seq, int flags, int64_t ts);
 ...
 */
int ff_rm_retrieve_cache (AVEMFormatContext *s, AVEMIOContext *pb,
                          AVEMStream *st, RMStream *rst, AVEMPacket *pkt);
