...
static void add_serial_pair(WtvSyncEntry ** list, int * count, int64_t serial, int64_t value)
{
    ...
    WtvSyncEntry *new_list = av_em_realloc_array(*list, new_count, sizeof(WtvSyncEntry));
    if (!new_list)
        return;
...
typedef int WTVHeaderWriteFunc(AVEMIOContext *pb);
...
#define write_pad(pb, size) emio_fill(pb, 0, size)

/**
 * Write chunk header. If header chunk (0x80000000 set) then add to list of header chunks
 */
static void write_chunk_header(AVEMFormatContext *s, const ff_asf_guid *guid, int length, int stream_id)
{
    WtvContext *wctx = s->priv_data;
    AVEMIOContext *pb = s->pb;

    wctx->last_chunk_pos = avio_em_tell(pb) - wctx->timeline_start_pos;
    ff_put_guid(pb, guid);
    avio_em_wl32(pb, 32 + length);
    avio_em_wl32(pb, stream_id);
    avio_em_wl64(pb, wctx->serial);
...
static void write_chunk_header2(AVEMFormatContext *s, const ff_asf_guid *guid, int stream_id)
{
    WtvContext *wctx = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    avio_em_wl64(pb, last_chunk_pos);
}

static void finish_chunk_noindex(AVEMFormatContext *s)
{
    WtvContext *wctx = s->priv_data;
    AVEMIOContext *pb = s->pb;

    // update the chunk_len field and pad.
    int64_t chunk_len = avio_em_tell(pb) - (wctx->last_chunk_pos + wctx->timeline_start_pos);
    avio_em_seek(pb, -(chunk_len - 16), SEEK_CUR);
    avio_em_wl32(pb, chunk_len);
    avio_em_seek(pb, chunk_len - (16 + 4), SEEK_CUR);

    write_pad(pb, WTV_PAD8(chunk_len) - chunk_len);
    wctx->serial++;
}

static void write_index(AVEMFormatContext *s)
{
    AVEMIOContext *pb = s->pb;
    WtvContext *wctx = s->priv_data;
    int i;
    ...
    for (i = 0; i < wctx->nb_index; i++) {
        ...
        avio_em_wl64(pb, t->pos);
        avio_em_wl32(pb, t->stream_id);
        avio_em_wl32(pb, 0); // checksum?
        avio_em_wl64(pb, t->serial);
    }
...
static void finish_chunk(AVEMFormatContext *s)
{
    WtvContext *wctx = s->priv_data;
    finish_chunk_noindex(s);
...
static void put_videoinfoheader2(AVEMIOContext *pb, AVEMStream *st)
{
    AVEMRational dar = av_em_mul_q(st->sample_aspect_ratio, (AVEMRational){st->codecpar->width, st->codecpar->height});
    unsigned int num, den;
    av_em_reduce(&num, &den, dar.num, dar.den, 0xFFFFFFFF);

    /* VIDEOINFOHEADER2 */
    avio_em_wl32(pb, 0);
    avio_em_wl32(pb, 0);
    avio_em_wl32(pb, st->codecpar->width);
    avio_em_wl32(pb, st->codecpar->height);
    ...
    avio_em_wl32(pb, st->codecpar->bit_rate);
    avio_em_wl32(pb, 0);
    avio_em_wl64(pb, st->avg_frame_rate.num && st->avg_frame_rate.den ? INT64_C(10000000) / av_em_q2d(st->avg_frame_rate) : 0);
    avio_em_wl32(pb, 0);
    avio_em_wl32(pb, 0);
    ...
    ff_put_bmp_header(pb, st->codecpar, em_codec_bmp_tags, 0, 1);

    if (st->codecpar->codec_id == AV_CODEC_ID_MPEG2VIDEO) {
        int padding = (st->codecpar->extradata_size & 3) ? 4 - (st->codecpar->extradata_size & 3) : 0;
        /* MPEG2VIDEOINFO */
        avio_em_wl32(pb, 0);
        avio_em_wl32(pb, st->codecpar->extradata_size + padding);
        avio_em_wl32(pb, -1);
        avio_em_wl32(pb, -1);
        avio_em_wl32(pb, 0);
        avio_em_write(pb, st->codecpar->extradata, st->codecpar->extradata_size);
        emio_fill(pb, 0, padding);
    }
}

static int write_stream_codec_info(AVEMFormatContext *s, AVEMStream *st)
{
    const ff_asf_guid *g, *media_type, *format_type;
    const AVEMCodecTag *tags;
    AVEMIOContext *pb = s->pb;
    ...
    if (st->codecpar->codec_type  == AVMEDIA_TYPE_VIDEO) {
        ...
        tags = em_codec_bmp_tags;
    } else if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
        g = ff_get_codec_guid(st->codecpar->codec_id, em_codec_wav_guids);
        media_type = &ff_mediatype_audio;
        format_type = &ff_format_waveformatex;
        tags = em_codec_wav_tags;
    } else {
        av_em_log(s, AV_LOG_ERROR, "unknown codec_type (0x%x)\n", st->codecpar->codec_type);
        return -1;
    }
    ...
    avio_em_wl32(pb, 0); // size

    hdr_pos_start = avio_em_tell(pb);
    if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
    ...
    hdr_size = avio_em_tell(pb) - hdr_pos_start;

    // seek back write hdr_size
    avio_em_seek(pb, -(hdr_size + 4), SEEK_CUR);
    avio_em_wl32(pb, hdr_size + 32);
    avio_em_seek(pb, hdr_size, SEEK_CUR);
    if (g) {
        ff_put_guid(pb, g);           // actual_subtype
    } else {
        int tag = em_codec_get_tag(tags, st->codecpar->codec_id);
        if (!tag) {
            av_em_log(s, AV_LOG_ERROR, "unsupported codec_id (0x%x)\n", st->codecpar->codec_id);
            return -1;
        }
        avio_em_wl32(pb, tag);
        avio_em_write(pb, (const uint8_t[]){FF_MEDIASUBTYPE_BASE_GUID}, 12);
    }
...
static int write_stream_codec(AVEMFormatContext *s, AVEMStream * st)
{
    AVEMIOContext *pb = s->pb;
    int ret;
    write_chunk_header2(s, &ff_stream1_guid, 0x80000000 | 0x01);

    avio_em_wl32(pb,  0x01);
    write_pad(pb, 4);
    write_pad(pb, 4);

    ret = write_stream_codec_info(s, st);
    if (ret < 0) {
        av_em_log(s, AV_LOG_ERROR, "write stream codec info failed codec_type(0x%x)\n", st->codecpar->codec_type);
        return -1;
    }
...
static void write_sync(AVEMFormatContext *s)
{
    AVEMIOContext *pb = s->pb;
    WtvContext *wctx = s->priv_data;
    int64_t last_chunk_pos = wctx->last_chunk_pos;
    ...
    avio_em_wl64(pb, wctx->first_index_pos);
    avio_em_wl64(pb, wctx->last_timestamp_pos);
    avio_em_wl64(pb, 0);
...
static int write_stream_data(AVEMFormatContext *s, AVEMStream *st)
{
    AVEMIOContext *pb = s->pb;
    int ret;

    write_chunk_header2(s, &ff_SBE2_STREAM_DESC_EVENT, 0x80000000 | (st->index + INDEX_BASE));
    avio_em_wl32(pb, 0x00000001);
    avio_em_wl32(pb, st->index + INDEX_BASE); //stream_id
    avio_em_wl32(pb, 0x00000001);
    write_pad(pb, 8);
    ...
    avpriv_em_set_pts_info(st, 64, 1, 10000000);
...
static int write_header(AVEMFormatContext *s)
{
    ...
    avio_em_wl32(pb, 0x01);
    avio_em_wl32(pb, 0x02);
    avio_em_wl32(pb, 1 << WTV_SECTOR_BITS);
    avio_em_wl32(pb, 1 << WTV_BIGSECTOR_BITS);

    //write initial root fields
    avio_em_wl32(pb, 0); // root_size, update later
    write_pad(pb, 4);
    avio_em_wl32(pb, 0); // root_sector, update it later.

    write_pad(pb, 32);
    avio_em_wl32(pb, 0); // file ends pointer, update it later.

    pad = (1 << WTV_SECTOR_BITS) - avio_em_tell(pb);
    write_pad(pb, pad);

    wctx->timeline_start_pos = avio_em_tell(pb);
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (ret < 0) {
            av_em_log(s, AV_LOG_ERROR, "write stream codec failed codec_type(0x%x)\n", st->codecpar->codec_type);
            return -1;
        }
        if (!i)
            write_sync(s);
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (ret < 0) {
            av_em_log(s, AV_LOG_ERROR, "write stream data failed codec_type(0x%x)\n", st->codecpar->codec_type);
            return -1;
        }
    }
...
static void write_timestamp(AVEMFormatContext *s, AVEMPacket *pkt)
{
    AVEMIOContext *pb = s->pb;
    WtvContext  *wctx = s->priv_data;
    ...
    write_pad(pb, 8);
    avio_em_wl64(pb, pkt->pts == AV_NOPTS_VALUE ? -1 : pkt->pts);
    avio_em_wl64(pb, pkt->pts == AV_NOPTS_VALUE ? -1 : pkt->pts);
    avio_em_wl64(pb, pkt->pts == AV_NOPTS_VALUE ? -1 : pkt->pts);
    avio_em_wl64(pb, 0);
    avio_em_wl64(pb, par->codec_type == AVMEDIA_TYPE_VIDEO && (pkt->flags & AV_PKT_FLAG_KEY) ? 1 : 0);
    avio_em_wl64(pb, 0);
...
static int write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_MJPEG && !wctx->thumbnail.size) {
        av_em_copy_packet(&wctx->thumbnail, pkt);
        return 0;
...
static int write_table0_header_events(AVEMIOContext *pb)
{
    avio_em_wl32(pb, 0x10);
    write_pad(pb, 84);
    avio_em_wl64(pb, 0x32);
    return 96;
}

static int write_table0_header_legacy_attrib(AVEMIOContext *pb)
{
    int pad = 0;
    avio_em_wl32(pb, 0xFFFFFFFF);
    write_pad(pb, 12);
    avio_em_write(pb, legacy_attrib, sizeof(legacy_attrib));
...
static int write_table0_header_time(AVEMIOContext *pb)
{
    avio_em_wl32(pb, 0x10);
    write_pad(pb, 76);
    avio_em_wl64(pb, 0x40);
    return 88;
}
...
static int write_root_table(AVEMFormatContext *s, int64_t sector_pos)
{
    AVEMIOContext *pb = s->pb;
    WtvContext  *wctx = s->priv_data;
    int size, pad;
    ...
    for (i = 0; i < sizeof(wtv_root_entry_table)/sizeof(WTVRootEntryTable); i++, h++) {
        ...
        len_pos = avio_em_tell(pb);
        avio_em_wl16(pb, 40 + h->header_size + filename_padding + 8); // maybe updated later
        write_pad(pb, 6);
        avio_em_wl64(pb, write ? 0 : w->length);// maybe update later
        avio_em_wl32(pb, (h->header_size + filename_padding) >> 1);
        write_pad(pb, 4);

        avio_em_write(pb, h->header, h->header_size);
        write_pad(pb, filename_padding);

        if (write) {
            len = write(pb);
            // update length field
            avio_em_seek(pb, len_pos, SEEK_SET);
            avio_em_wl64(pb, 40 + h->header_size + filename_padding + len);
            avio_em_wl64(pb, len |(1ULL<<62) | (1ULL<<60));
            avio_em_seek(pb, 8 + h->header_size + filename_padding + len, SEEK_CUR);
        } else {
            avio_em_wl32(pb, w->first_sector);
            avio_em_wl32(pb, w->depth);
        }
    }

    // caculate root table size
    size = avio_em_tell(pb) - sector_pos;
...
static void write_fat(AVEMIOContext *pb, int start_sector, int nb_sectors, int shift)
{
    int i;
    for (i = 0; i < nb_sectors; i++) {
        avio_em_wl32(pb, start_sector + (i << shift));
    }
    // pad left sector pointer size
    write_pad(pb, WTV_SECTOR_SIZE - ((nb_sectors << 2) % WTV_SECTOR_SIZE));
}

static int64_t write_fat_sector(AVEMFormatContext *s, int64_t start_pos, int nb_sectors, int sector_bits, int depth)
{
    int64_t start_sector = start_pos >> WTV_SECTOR_BITS;
    int shift = sector_bits - WTV_SECTOR_BITS;

    int64_t fat = avio_em_tell(s->pb);
    ...
    if (depth == 2) {
        ...
        int64_t fat1 = avio_em_tell(s->pb);
...
static void write_table_entries_events(AVEMFormatContext *s)
{
    AVEMIOContext *pb = s->pb;
    WtvContext *wctx = s->priv_data;
    int i;
    for (i = 0; i < wctx->nb_sp_pairs; i++) {
        avio_em_wl64(pb, wctx->sp_pairs[i].serial);
        avio_em_wl64(pb, wctx->sp_pairs[i].value);
    }
}

static void write_table_entries_time(AVEMFormatContext *s)
{
    AVEMIOContext *pb = s->pb;
    WtvContext *wctx = s->priv_data;
    int i;
    for (i = 0; i < wctx->nb_st_pairs; i++) {
        avio_em_wl64(pb, wctx->st_pairs[i].value);
        avio_em_wl64(pb, wctx->st_pairs[i].serial);
    }
    avio_em_wl64(pb, wctx->last_pts);
    avio_em_wl64(pb, wctx->last_serial);
}

static void write_metadata_header(AVEMIOContext *pb, int type, const char *key, int value_size)
{
    ff_put_guid(pb, &ff_metadata_guid);
    avio_em_wl32(pb, type);
    avio_em_wl32(pb, value_size);
    avio_em_put_str16le(pb, key);
}
...
static void write_tag_int32(AVEMIOContext *pb, const char *key, int value)
{
    write_metadata_header(pb, 0, key, 4);
    avio_em_wl32(pb, value);
}

static void write_tag(AVEMIOContext *pb, const char *key, const char *value)
{
    write_metadata_header(pb, 1, key, strlen(value)*2 + 2);
    avio_em_put_str16le(pb, value);
}

static int attachment_value_size(const AVEMPacket *pkt, const AVEMDictionaryEntry *e)
{
    return strlen("image/jpeg")*2 + 2 + 1 + (e ? strlen(e->value)*2 : 0) + 2 + 4 + pkt->size;
}

static void write_table_entries_attrib(AVEMFormatContext *s)
{
    WtvContext *wctx = s->priv_data;
    AVEMIOContext *pb = s->pb;
    AVEMDictionaryEntry *tag = 0;
    ...
    if (wctx->thumbnail.size) {
        AVEMStream *st = s->streams[wctx->thumbnail.stream_index];
        tag = av_em_dict_get(st->metadata, "title", NULL, 0);
        write_metadata_header(pb, 2, "WM/Picture", attachment_value_size(&wctx->thumbnail, tag));

        avio_em_put_str16le(pb, "image/jpeg");
        avio_em_w8(pb, 0x10);
        avio_em_put_str16le(pb, tag ? tag->value : "");

        avio_em_wl32(pb, wctx->thumbnail.size);
        avio_em_write(pb, wctx->thumbnail.data, wctx->thumbnail.size);

        write_tag_int32(pb, "WM/MediaThumbType", 2);
    }
}

static void write_table_redirector_legacy_attrib(AVEMFormatContext *s)
{
    WtvContext *wctx = s->priv_data;
    AVEMIOContext *pb = s->pb;
    AVEMDictionaryEntry *tag = 0;
    ...
    if (wctx->thumbnail.size) {
        AVEMStream *st = s->streams[wctx->thumbnail.stream_index];
        avio_em_wl64(pb, pos);
        pos += metadata_header_size("WM/Picture") + attachment_value_size(&wctx->thumbnail, av_em_dict_get(st->metadata, "title", NULL, 0));
 ...
 * @return <0 on error
 */
static int finish_file(AVEMFormatContext *s, enum WtvFileIndex index, int64_t start_pos)
{
    WtvContext *wctx = s->priv_data;
    AVEMIOContext *pb = s->pb;
    WtvFile *w = &wctx->file[index];
    int64_t end_pos = avio_em_tell(pb);
    ...
    if (w->length <= WTV_SECTOR_SIZE) {
        ...
        av_em_log(s, AV_LOG_ERROR, "unsupported file allocation table depth (%"PRIi64" bytes)\n", w->length);
        return -1;
    }
...
static int write_trailer(AVEMFormatContext *s)
{
    ...
    // write root table
    sector_pos = avio_em_tell(pb);
    root_size = write_root_table(s, sector_pos);

    file_end_pos = avio_em_tell(pb);
    // update root value
    avio_em_seek(pb, 0x30, SEEK_SET);
    avio_em_wl32(pb, root_size);
    avio_em_seek(pb, 4, SEEK_CUR);
    avio_em_wl32(pb, sector_pos >> WTV_SECTOR_BITS);
    avio_em_seek(pb, 0x5c, SEEK_SET);
    avio_em_wl32(pb, file_end_pos >> WTV_SECTOR_BITS);

    avio_em_flush(pb);

    av_em_free(wctx->sp_pairs);
    av_em_free(wctx->st_pairs);
    av_em_packet_unref(&wctx->thumbnail);
    return 0;
}

AVEMOutputFormat ff_wtv_muxer = {
    .name           = "wtv",
    ...
    .codec_tag      = (const AVEMCodecTag* const []){ em_codec_bmp_tags,
                                                    em_codec_wav_tags, 0 },
};
