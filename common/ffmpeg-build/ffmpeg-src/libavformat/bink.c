...
static int read_header(AVEMFormatContext *s)
{
    ...
    if (vst->codecpar->codec_tag == AV_RL32("SMUS")) {
        do {
            ...
            avio_em_skip(pb, SMUSH_BLOCK_SIZE - 4);
            vst->codecpar->codec_tag = avio_em_rl32(pb);
        } while (!avio_em_feof(pb) && (vst->codecpar->codec_tag & 0xFFFFFF) != AV_RL32("BIK"));
        if (avio_em_feof(pb)) {
            av_em_log(s, AV_LOG_ERROR, "invalid SMUSH header: BIK not found\n");
            return AVERROR_INVALIDDATA;
        }
    }

    bink->file_size = avio_em_rl32(pb) + 8;
    vst->duration   = avio_em_rl32(pb);

    if (vst->duration > 1000000) {
        av_em_log(s, AV_LOG_ERROR, "invalid header: more than 1000000 frames\n");
        return AVERROR(EIO);
    }

    if (avio_em_rl32(pb) > bink->file_size) {
    ...
    fps_num = avio_em_rl32(pb);
    fps_den = avio_em_rl32(pb);
    ...
    avpriv_em_set_pts_info(vst, 64, fps_den, fps_num);
    ...
    if ((vst->codecpar->codec_tag & 0xFFFFFF) == MKTAG('K', 'B', '2', 0)) {
        av_em_log(s, AV_LOG_WARNING, "Bink 2 video is not implemented\n");
        vst->codecpar->codec_id = AV_CODEC_ID_NONE;
    }

    if (em_get_extradata(s, vst->codecpar, pb, 4) < 0)
        return AVERROR(ENOMEM);

    bink->num_audio_tracks = avio_em_rl32(pb);
    ...
    if (bink->num_audio_tracks) {
        avio_em_skip(pb, 4 * bink->num_audio_tracks);
        ...
        for (i = 0; i < bink->num_audio_tracks; i++) {
            ...
            ast->codecpar->sample_rate = avio_em_rl16(pb);
            avpriv_em_set_pts_info(ast, 64, 1, ast->codecpar->sample_rate);
            flags = avio_em_rl16(pb);
            ...
            if (em_alloc_extradata(ast->codecpar, 4))
                return AVERROR(ENOMEM);
            AV_WL32(ast->codecpar->extradata, vst->codecpar->codec_tag);
        }

        for (i = 0; i < bink->num_audio_tracks; i++)
            s->streams[i + 1]->id = avio_em_rl32(pb);
    }

    /* frame index table */
    next_pos = avio_em_rl32(pb);
    for (i = 0; i < vst->duration; i++) {
        ...
        if (next_pos <= pos) {
            av_em_log(s, AV_LOG_ERROR, "invalid frame index table\n");
            return AVERROR(EIO);
        }
        if ((ret = av_em_add_index_entry(vst, pos, i, next_pos - pos, 0,
                                      keyframe ? AVINDEX_KEYFRAME : 0)) < 0)
            return ret;
    }

    if (vst->index_entries)
        avio_em_seek(pb, vst->index_entries[0].pos + bink->smush_size, SEEK_SET);
    else
        avio_em_skip(pb, 4);
...
static int read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if (bink->current_track < 0) {
        int index_entry;
        AVEMStream *st = s->streams[0]; // stream 0 is video stream with index

        if (bink->video_pts >= st->duration)
            return AVERROR_EOF;

        index_entry = av_em_index_search_timestamp(st, bink->video_pts,
                                                AVSEEK_FLAG_ANY);
        if (index_entry < 0) {
            av_em_log(s, AV_LOG_ERROR,
    ...
    while (bink->current_track < bink->num_audio_tracks) {
        uint32_t audio_size = avio_em_rl32(pb);
        ...
        if (audio_size >= 4) {
            ...
            if ((ret = av_em_get_packet(pb, pkt, audio_size)) < 0)
                return ret;
            ...
            avio_em_skip(pb, audio_size);
        }
    }

    /* get video packet */
    if ((ret = av_em_get_packet(pb, pkt, bink->remain_packet_size)) < 0)
        return ret;
    pkt->stream_index = 0;
...
{
    BinkDemuxContext *bink = s->priv_data;
    AVEMStream *vst = s->streams[0];

    if (!s->pb->seekable)
        return -1;

    /* seek to the first frame */
    if (avio_em_seek(s->pb, vst->index_entries[0].pos + bink->smush_size, SEEK_SET) < 0)
        return -1;
...
AVEMInputFormat ff_bink_demuxer = {
    .name           = "bink",
