...
static void choose_qop(char *qop, int size)
{
    ...
    if (ptr && (!*end || av_isspace(*end) || *end == ',') &&
        ...
        av_em_strlcpy(qop, "auth", size);
    } else {
        qop[0] = 0;
    }
}

void em_http_auth_handle_header(HTTPAuthState *state, const char *key,
                                const char *value)
{
    if (!strcmp(key, "WWW-Authenticate") || !strcmp(key, "Proxy-Authenticate")) {
        const char *p;
        if (av_em_stristart(value, "Basic ", &p) &&
            state->auth_type <= HTTP_AUTH_BASIC) {
            state->auth_type = HTTP_AUTH_BASIC;
            state->realm[0] = 0;
            state->stale = 0;
            em_parse_key_value(p, (em_parse_key_val_cb) handle_basic_params,
                               state);
        } else if (av_em_stristart(value, "Digest ", &p) &&
                   state->auth_type <= HTTP_AUTH_DIGEST) {
            state->auth_type = HTTP_AUTH_DIGEST;
            memset(&state->digest_params, 0, sizeof(DigestParams));
            state->realm[0] = 0;
            state->stale = 0;
            em_parse_key_value(p, (em_parse_key_val_cb) handle_digest_params,
                               state);
            choose_qop(state->digest_params.qop,
                       sizeof(state->digest_params.qop));
            if (!av_em_strcasecmp(state->digest_params.stale, "true"))
                state->stale = 1;
        }
    } else if (!strcmp(key, "Authentication-Info")) {
        em_parse_key_value(value, (em_parse_key_val_cb) handle_digest_update,
                           state);
    }
}
...
static void update_md5_strings(struct AVMD5 *md5ctx, ...)
{
    ...
    while (1) {
        ...
        if (!str)
            break;
        av_em_md5_update(md5ctx, str, strlen(str));
    }
    va_end(vl);
}
                              ...
                              const char *method)
{
    ...
    for (i = 0; i < 2; i++)
        cnonce_buf[i] = av_em_get_random_seed();
    em_data_to_hex(cnonce, (const uint8_t*) cnonce_buf, sizeof(cnonce_buf), 1);
    cnonce[2*sizeof(cnonce_buf)] = 0;

    md5ctx = av_em_md5_alloc();
    if (!md5ctx)
        return NULL;

    av_em_md5_init(md5ctx);
    update_md5_strings(md5ctx, username, ":", state->realm, ":", password, NULL);
    av_em_md5_final(md5ctx, hash);
    em_data_to_hex(A1hash, hash, 16, 1);
    A1hash[32] = 0;
    ...
    av_em_md5_final(md5ctx, hash);
    em_data_to_hex(A2hash, hash, 16, 1);
    A2hash[32] = 0;
    ...
    av_em_md5_final(md5ctx, hash);
    em_data_to_hex(response, hash, 16, 1);
    response[32] = 0;
    ...
    authstr = av_em_alloc(len);
    if (!authstr)
        return NULL;
    snprintf(authstr, len, "Authorization: Digest ");

    /* TODO: Escape the quoted strings properly. */
    av_em_strlcatf(authstr, len, "username=\"%s\"",   username);
    av_em_strlcatf(authstr, len, ", realm=\"%s\"",     state->realm);
    av_em_strlcatf(authstr, len, ", nonce=\"%s\"",     digest->nonce);
    av_em_strlcatf(authstr, len, ", uri=\"%s\"",       uri);
    av_em_strlcatf(authstr, len, ", response=\"%s\"",  response);

    // we are violating the RFC and use "" because all others seem to do that too.
    if (digest->algorithm[0])
        av_em_strlcatf(authstr, len, ", algorithm=\"%s\"",  digest->algorithm);

    if (digest->opaque[0])
        av_em_strlcatf(authstr, len, ", opaque=\"%s\"", digest->opaque);
    if (digest->qop[0]) {
        av_em_strlcatf(authstr, len, ", qop=\"%s\"",    digest->qop);
        av_em_strlcatf(authstr, len, ", cnonce=\"%s\"", cnonce);
        av_em_strlcatf(authstr, len, ", nc=%s",         nc);
    }

    av_em_strlcatf(authstr, len, "\r\n");

    return authstr;
}

char *em_http_auth_create_response(HTTPAuthState *state, const char *auth,
                                   const char *path, const char *method)
{
    ...
    if (state->auth_type == HTTP_AUTH_BASIC) {
        ...
        authstr = av_em_alloc(len);
        if (!authstr) {
            av_em_free(decoded_auth);
            return NULL;
        }

        snprintf(authstr, len, "Authorization: Basic ");
        ptr = authstr + strlen(authstr);
        av_em_base64_encode(ptr, auth_b64_len, decoded_auth, strlen(decoded_auth));
        av_em_strlcat(ptr, "\r\n", len - (ptr - authstr));
        av_em_free(decoded_auth);
