...
static const AVEMClass sctp_class = {
    .class_name = "sctp",
...
static int sctp_open(EMURLContext *h, const char *uri, int flags)
{
    ...
    if (port <= 0 || port >= 65536) {
        av_em_log(s, AV_LOG_ERROR, "Port missing in uri\n");
        return AVERROR(EINVAL);
    }
    ...
    if (p) {
        if (av_em_find_info_tag(buf, sizeof(buf), "listen", p))
            s->listen = 1;
        if (av_em_find_info_tag(buf, sizeof(buf), "max_streams", p))
    ...
    if (ret) {
        av_em_log(h, AV_LOG_ERROR, "Failed to resolve hostname %s: %s\n",
...
restart:
    fd = em_socket(cur_ai->ai_family, SOCK_STREAM, IPPROTO_SCTP);
    if (fd < 0) {
        ret = ff_neterrno();
        goto fail;
    }
    ...
    if (s->listen) {
        if ((fd = em_listen_bind(fd, cur_ai->ai_addr, cur_ai->ai_addrlen,
...
static int sctp_read(EMURLContext *h, uint8_t *buf, int size)
{
    SCTPContext *s = h->priv_data;
    int ret;
...
static int sctp_write(EMURLContext *h, const uint8_t *buf, int size)
{
    SCTPContext *s = h->priv_data;
    int ret;
    ...
    if (s->max_streams) {
        ...
        if (info.sinfo_stream > s->max_streams) {
            av_em_log(h, AV_LOG_ERROR, "bad input data\n");
            return AVERROR_BUG;
        }
...
static int sctp_close(EMURLContext *h)
{
    SCTPContext *s = h->priv_data;
    closesocket(s->fd);
    return 0;
...
static int sctp_get_file_handle(EMURLContext *h)
{
    SCTPContext *s = h->priv_data;
    return s->fd;
}

const EMURLProtocol em_sctp_protocol = {
    .name                = "sctp",
    .url_open            = sctp_open,
    .url_read            = sctp_read,
