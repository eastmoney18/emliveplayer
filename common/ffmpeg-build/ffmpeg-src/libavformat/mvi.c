...
typedef struct MviDemuxContext {
    unsigned int (*get_int)(AVEMIOContext *);
...
static int read_header(AVEMFormatContext *s)
{
    ...
    if (em_alloc_extradata(vst->codecpar, 2))
        return AVERROR(ENOMEM);

    version                  = avio_em_r8(pb);
    vst->codecpar->extradata[0] = avio_em_r8(pb);
    vst->codecpar->extradata[1] = avio_em_r8(pb);
    frames_count             = avio_em_rl32(pb);
    msecs_per_frame          = avio_em_rl32(pb);
    vst->codecpar->width        = avio_em_rl16(pb);
    vst->codecpar->height       = avio_em_rl16(pb);
    avio_em_r8(pb);
    ast->codecpar->sample_rate  = avio_em_rl16(pb);
    mvi->audio_data_size     = avio_em_rl32(pb);
    avio_em_r8(pb);
    player_version           = avio_em_rl32(pb);
    avio_em_rl16(pb);
    avio_em_r8(pb);
    ...
    if (version != 7 || player_version > 213) {
        av_em_log(s, AV_LOG_ERROR, "unhandled version (%d,%d)\n", version, player_version);
        return AVERROR_INVALIDDATA;
    }
    ...
    avpriv_em_set_pts_info(vst, 64, msecs_per_frame, 1000000);
    vst->avg_frame_rate    = av_em_inv_q(vst->time_base);
    vst->codecpar->codec_type = AVMEDIA_TYPE_VIDEO;
    vst->codecpar->codec_id   = AV_CODEC_ID_MOTIONPIXELS;

    mvi->get_int = (vst->codecpar->width * vst->codecpar->height < (1 << 16)) ? avio_em_rl16 : avio_em_rl24;
...
static int read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if (mvi->video_frame_size == 0) {
        ...
        if ((ret = av_em_get_packet(pb, pkt, count)) < 0)
            return ret;
        ...
        if ((ret = av_em_get_packet(pb, pkt, mvi->video_frame_size)) < 0)
            return ret;
...
AVEMInputFormat ff_mvi_demuxer = {
    .name           = "mvi",
