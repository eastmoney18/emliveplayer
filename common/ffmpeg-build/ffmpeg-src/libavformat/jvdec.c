...
static int read_probe(AVEMProbeData *pd)
{
...
static int read_header(AVEMFormatContext *s)
{
    ...
    vst->duration           =
    vst->nb_frames          =
    ast->nb_index_entries   = avio_em_rl16(pb);
    avpriv_em_set_pts_info(vst, 64, avio_em_rl16(pb), 1000);
    ...
    ast->index_entries = av_em_alloc(ast->nb_index_entries *
                                   sizeof(*ast->index_entries));
    if (!ast->index_entries)
        return AVERROR(ENOMEM);

    jv->frames = av_em_alloc(ast->nb_index_entries * sizeof(JVFrame));
    if (!jv->frames)
        return AVERROR(ENOMEM);

    offset = 0x68 + ast->nb_index_entries * 16;
    for (i = 0; i < ast->nb_index_entries; i++) {
        AVEMIndexEntry *e   = ast->index_entries + i;
        JVFrame      *jvf = jv->frames + i;

        /* total frame size including audio, video, palette data and padding */
        e->size      = avio_em_rl32(pb);
        e->timestamp = i;
        e->pos       = offset;
        offset      += e->size;

        jvf->audio_size   = avio_em_rl32(pb);
        jvf->video_size   = avio_em_rl32(pb);
        jvf->palette_size = avio_em_r8(pb) ? 768 : 0;
        ...
        if (avio_em_r8(pb))
            av_em_log(s, AV_LOG_WARNING, "unsupported audio codec\n");

        jvf->video_type = avio_em_r8(pb);
        avio_em_skip(pb, 1);
...
static int read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    JVDemuxContext *jv = s->priv_data;
    AVEMIOContext *pb = s->pb;
    AVEMStream *ast = s->streams[0];

    while (!avio_em_feof(s->pb) && jv->pts < ast->nb_index_entries) {
        const AVEMIndexEntry *e   = ast->index_entries + jv->pts;
        ...
        case JV_AUDIO:
            jv->state++;
            if (jvf->audio_size) {
                if (av_em_get_packet(s->pb, pkt, jvf->audio_size) < 0)
                    return AVERROR(ENOMEM);
                pkt->stream_index = 0;
                pkt->pts          = e->timestamp;
        ...
        case JV_VIDEO:
            ...
            if (jvf->video_size || jvf->palette_size) {
                ...
                if (av_em_new_packet(pkt, size + JV_PREAMBLE_SIZE))
                    return AVERROR(ENOMEM);

                AV_WL32(pkt->data, jvf->video_size);
                pkt->data[4] = jvf->video_type;
                ret = avio_em_read(pb, pkt->data + JV_PREAMBLE_SIZE, size);
                if (ret < 0)
                    return ret;
                if (ret < size) {
        ...
        case JV_PADDING:
            avio_em_skip(pb, FFMAX(e->size - jvf->audio_size - jvf->video_size
                                        - jvf->palette_size, 0));
            jv->state = JV_AUDIO;
            jv->pts++;
        }
                     ...
                     int64_t ts, int flags)
{
    ...
    switch (stream_index) {
    case 0:
        i = av_em_index_search_timestamp(ast, ts, flags);
        break;
    case 1:
        i = ts;
        break;
    ...
    if (i < 0 || i >= ast->nb_index_entries)
        return 0;
    if (avio_em_seek(s->pb, ast->index_entries[i].pos, SEEK_SET) < 0)
        return -1;
