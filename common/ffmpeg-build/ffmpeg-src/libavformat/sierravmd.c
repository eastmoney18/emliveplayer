...
static int vmd_probe(AVEMProbeData *p)
{
    int w, h, sample_rate;
    if (p->buf_size < 806)
        return 0;
...
static int vmd_read_header(AVEMFormatContext *s)
{
    VmdDemuxContext *vmd = s->priv_data;
    AVEMIOContext *pb = s->pb;
    AVEMStream *st = NULL, *vst = NULL;
    ...
    if (avio_em_read(pb, vmd->vmd_header, VMD_HEADER_SIZE) != VMD_HEADER_SIZE)
        return AVERROR(EIO);
    ...
    if (width && height) {
        ...
        avpriv_em_set_pts_info(vst, 33, 1, 10);
        ...
        if (em_alloc_extradata(vst->codecpar, VMD_HEADER_SIZE))
            return AVERROR(ENOMEM);
    ...
    if (vmd->sample_rate) {
        ...
        av_em_reduce(&num, &den, num, den, (1UL<<31)-1);
        if (vst)
            avpriv_em_set_pts_info(vst, 33, num, den);
        avpriv_em_set_pts_info(st, 33, num, den);
    }
    ...
    avio_em_seek(pb, toc_offset, SEEK_SET);
    ...
    if(vmd->frame_count * vmd->frames_per_block >= UINT_MAX / sizeof(vmd_frame) - sound_buffers){
        av_em_log(s, AV_LOG_ERROR, "vmd->frame_count * vmd->frames_per_block too large\n");
        return -1;
    }
    raw_frame_table = av_em_alloc(raw_frame_table_size);
    vmd->frame_table = av_em_malloc_array(vmd->frame_count * vmd->frames_per_block + sound_buffers, sizeof(vmd_frame));
    if (!raw_frame_table || !vmd->frame_table) {
        ret = AVERROR(ENOMEM);
        goto error;
    }
    if (avio_em_read(pb, raw_frame_table, raw_frame_table_size) !=
    ...
    for (i = 0; i < vmd->frame_count; i++) {
        ...
        for (j = 0; j < vmd->frames_per_block; j++) {
            ...
            if ((ret = avio_em_read(pb, chunk, BYTES_PER_FRAME_RECORD)) != BYTES_PER_FRAME_RECORD) {
                av_em_log(s, AV_LOG_ERROR, "Failed to read frame record\n");
                if (ret >= 0)
            ...
            if (size > INT_MAX / 2) {
                av_em_log(s, AV_LOG_ERROR, "Invalid frame size\n");
                ret = AVERROR_INVALIDDATA;
                goto error;
            }
            if(!size && type != 1)
...
error:
    av_em_freep(&raw_frame_table);
    av_em_freep(&vmd->frame_table);
    return ret;
}

static int vmd_read_packet(AVEMFormatContext *s,
                           AVEMPacket *pkt)
{
    VmdDemuxContext *vmd = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    avio_em_seek(pb, frame->frame_offset, SEEK_SET);

    if(emio_limit(pb, frame->frame_size) != frame->frame_size)
        return AVERROR(EIO);
    if (av_em_new_packet(pkt, frame->frame_size + BYTES_PER_FRAME_RECORD))
        return AVERROR(ENOMEM);
    pkt->pos= avio_em_tell(pb);
    memcpy(pkt->data, frame->frame_record, BYTES_PER_FRAME_RECORD);
    if(vmd->is_indeo3 && frame->frame_record[0] == 0x02)
        ret = avio_em_read(pb, pkt->data, frame->frame_size);
    else
        ret = avio_em_read(pb, pkt->data + BYTES_PER_FRAME_RECORD,
            frame->frame_size);
    ...
    pkt->pts = frame->pts;
    av_em_log(s, AV_LOG_DEBUG, " dispatching %s frame with %d bytes and pts %"PRId64"\n",
...
static int vmd_read_close(AVEMFormatContext *s)
{
    VmdDemuxContext *vmd = s->priv_data;
...
AVEMInputFormat ff_vmd_demuxer = {
    .name           = "vmd",
