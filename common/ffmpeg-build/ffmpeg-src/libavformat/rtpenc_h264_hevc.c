...
static void flush_buffered(AVEMFormatContext *s1, int last)
{
    ...
    if (s->buf_ptr != s->buf) {
        ...
        if (s->buffered_nals == 1) {
            enum AVEMCodecID codec = s1->streams[0]->codecpar->codec_id;
...
static void nal_send(AVEMFormatContext *s1, const uint8_t *buf, int size, int last)
{
    RTPMuxContext *s = s1->priv_data;
    enum AVEMCodecID codec = s1->streams[0]->codecpar->codec_id;

    av_em_log(s1, AV_LOG_DEBUG, "Sending NAL %x of len %d M=%d\n", buf[0] & 0x1F, size, last);
    if (size <= s->max_payload_size) {
        ...
        av_em_log(s1, AV_LOG_DEBUG, "NAL size %d > %d\n", size, s->max_payload_size);
        if (codec == AV_CODEC_ID_H264) {
...
void ff_rtp_send_h264_hevc(AVEMFormatContext *s1, const uint8_t *buf1, int size)
{
    ...
    if (s->nal_length_size)
        r = em_avc_mp4_find_startcode(buf1, end, s->nal_length_size) ? buf1 : end;
    else
        r = em_avc_find_startcode(buf1, end);
    while (r < end) {
        const uint8_t *r1;

        if (s->nal_length_size) {
            r1 = em_avc_mp4_find_startcode(r, end, s->nal_length_size);
            if (!r1)
                r1 = end;
            r += s->nal_length_size;
        } else {
            while (!*(r++));
            r1 = em_avc_find_startcode(r, end);
        }
