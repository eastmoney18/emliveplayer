...
static int musx_probe(AVEMProbeData *p)
{
...
static int musx_read_header(AVEMFormatContext *s)
{
    unsigned type, version, coding, offset;
    AVEMStream *st;

    avio_em_skip(s->pb, 8);
    version = avio_em_rl32(s->pb);
    if (version != 10 &&
        version != 6 &&
        version != 5 &&
        version != 4 &&
        version != 201) {
        avpriv_em_request_sample(s, "Unsupported version: %d", version);
    ...
    if (version == 201) {
        ...
        case MKTAG('P', 'S', '3', '_'):
            ...
            avio_em_skip(s->pb, 44);
            coding = avio_em_rl32(s->pb);
        ...
        case MKTAG('W', 'I', 'I', '_'):
            ...
            if (coding != MKTAG('D', 'A', 'T', '4') &&
                coding != MKTAG('D', 'A', 'T', '8')) {
                avpriv_em_request_sample(s, "Unsupported coding: %X", coding);
                return AVERROR_PATCHWELCOME;
            }
            avio_em_skip(s->pb, 4);
            st->codecpar->codec_id   = AV_CODEC_ID_ADPCM_IMA_DAT4;
        ...
        case MKTAG('P', 'S', '2', '_'):
            ...
            break;
        default:
            avpriv_em_request_sample(s, "Unsupported type: %X", type);
            return AVERROR_PATCHWELCOME;
        }
...
static int musx_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
...
AVEMInputFormat ff_musx_demuxer = {
    .name           = "musx",
