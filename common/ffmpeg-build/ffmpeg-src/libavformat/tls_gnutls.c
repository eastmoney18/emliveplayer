...
static ssize_t gnutls_url_pull(gnutls_transport_ptr_t transport,
                               void *buf, size_t len)
{
    EMURLContext *h = (EMURLContext*) transport;
...
static int tls_open(EMURLContext *h, const char *uri, int flags, AVEMDictionary **options)
{
    ...
    if (c->cert_file && c->key_file) {
        ...
        av_em_log(h, AV_LOG_ERROR, "cert and key required\n");
    ...
    if (c->verify) {
        ...
        if ((ret = gnutls_certificate_verify_peers2(p->session, &status)) < 0) {
            av_em_log(h, AV_LOG_ERROR, "Unable to verify peer certificate: %s\n",
                                    gnutls_strerror(ret));
            ret = AVERROR(EIO);
            goto fail;
        }
        if (status & GNUTLS_CERT_INVALID) {
            av_em_log(h, AV_LOG_ERROR, "Peer certificate failed verification\n");
            ret = AVERROR(EIO);
            goto fail;
        }
        if (gnutls_certificate_type_get(p->session) != GNUTLS_CRT_X509) {
            av_em_log(h, AV_LOG_ERROR, "Unsupported certificate type\n");
            ret = AVERROR(EIO);
            goto fail;
        }
...
const EMURLProtocol em_tls_gnutls_protocol = {
    .name           = "tls",
    .url_open2      = tls_open,
    .url_read       = tls_read,
    .url_write      = tls_write,
