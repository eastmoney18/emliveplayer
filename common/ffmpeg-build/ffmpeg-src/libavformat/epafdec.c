...
static int epaf_probe(AVEMProbeData *p)
{
...
static int epaf_read_header(AVEMFormatContext *s)
{
    int le, sample_rate, codec, channels;
    AVEMStream *st;

    avio_em_skip(s->pb, 4);
    if (avio_em_rl32(s->pb))
        return AVERROR_INVALIDDATA;

    le = avio_em_rl32(s->pb);
    if (le && le != 1)
        return AVERROR_INVALIDDATA;

    if (le) {
        sample_rate = avio_em_rl32(s->pb);
        codec       = avio_em_rl32(s->pb);
        channels    = avio_em_rl32(s->pb);
    } else {
        sample_rate = avio_em_rb32(s->pb);
    ...
    case 2:
        ...
        break;
    case 1:
        avpriv_em_request_sample(s, "24-bit Paris PCM format");
    default:
        return AVERROR_INVALIDDATA;
    }
    ...
    if (avio_em_skip(s->pb, 2024) < 0)
        return AVERROR_INVALIDDATA;
    return 0;
}

AVEMInputFormat ff_epaf_demuxer = {
    .name           = "epaf",
