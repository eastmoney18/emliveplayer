                   ...
                   const struct addrinfo *hints, struct addrinfo **res)
{
    ...
    *res = NULL;
    sin  = av_em_mallocz(sizeof(struct sockaddr_in));
    if (!sin)
        return EAI_FAIL;
    sin->sin_family = AF_INET;

    if (node) {
        if (!em_inet_aton(node, &sin->sin_addr)) {
    ...
    ai = av_em_mallocz(sizeof(struct addrinfo));
    if (!ai) {
        av_em_free(sin);
        return EAI_FAIL;
    }
    ...
    *res            = ai;
    ...
    if (hints && (hints->ai_flags & AI_CANONNAME))
        ai->ai_canonname = h ? av_em_strdup(h->h_name) : NULL;
...
void em_freeaddrinfo(struct addrinfo *res)
{
    ...
    av_em_freep(&res->ai_canonname);
    av_em_freep(&res->ai_addr);
    av_em_freep(&res);
}
