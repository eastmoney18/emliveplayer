...
static const AVEMCodecTag lxf_tags[] = {
    { AV_CODEC_ID_MJPEG,       0 },
...
static int lxf_probe(AVEMProbeData *p)
{
 ...
 */
static int lxf_sync(AVEMFormatContext *s, uint8_t *header)
{
    uint8_t buf[LXF_IDENT_LENGTH];
    int ret;

    if ((ret = avio_em_read(s->pb, buf, LXF_IDENT_LENGTH)) != LXF_IDENT_LENGTH)
    ...
    while (memcmp(buf, LXF_IDENT, LXF_IDENT_LENGTH)) {
        ...
        buf[LXF_IDENT_LENGTH-1] = avio_em_r8(s->pb);
    }
 ...
 */
static int get_packet_header(AVEMFormatContext *s)
{
    ...
    ret = avio_em_read(pb, header + LXF_IDENT_LENGTH, 8);
    if (ret != 8)
        return ret < 0 ? ret : AVERROR_EOF;
    ...
    if (version > 1)
        avpriv_em_request_sample(s, "Format version %"PRIu32, version);

    if (header_size < (version ? 72 : 60) ||
        header_size > LXF_MAX_PACKET_HEADER_SIZE ||
        (header_size & 3)) {
        av_em_log(s, AV_LOG_ERROR, "Invalid header size 0x%"PRIx32"\n", header_size);
        return AVERROR_INVALIDDATA;
    }

    //read the rest of the packet header
    if ((ret = avio_em_read(pb, header + (p - header),
                          header_size - (p - header))) !=
                          header_size - (p - header))
        return ret < 0 ? ret : AVERROR_EOF;

    if (check_checksum(header, header_size))
        av_em_log(s, AV_LOG_ERROR, "checksum error\n");
    ...
    case 0:
        ...
        avio_em_skip(pb, (int64_t)(uint32_t)AV_RL32(p + 4) +
                      (int64_t)(uint32_t)AV_RL32(p + 12));
        break;
    case 1:
        //audio
        if (s->nb_streams < 2) {
            av_em_log(s, AV_LOG_INFO, "got audio packet, but no audio stream present\n");
            break;
        }
        ...
        if (st->codecpar->bits_per_coded_sample != (audio_format & 0x3F)) {
            av_em_log(s, AV_LOG_WARNING, "only tightly packed PCM currently supported\n");
        ...
        if (samples == LXF_SAMPLERATE * 5005 / 30000) {
            avpriv_em_set_pts_info(s->streams[0], 64, 1001, 30000);
        } else {
            ...
            avpriv_em_set_pts_info(s->streams[0], 64, 1, 25);
        }
...
static int lxf_read_header(AVEMFormatContext *s)
{
    LXFDemuxContext *lxf = s->priv_data;
    AVEMIOContext   *pb  = s->pb;
    ...
    if (ret != LXF_HEADER_DATA_SIZE) {
        av_em_log(s, AV_LOG_ERROR, "expected %d B size header, got %d\n",
               LXF_HEADER_DATA_SIZE, ret);
        return AVERROR_INVALIDDATA;
    }

    if ((ret = avio_em_read(pb, header_data, LXF_HEADER_DATA_SIZE)) != LXF_HEADER_DATA_SIZE)
        return ret < 0 ? ret : AVERROR_EOF;
    ...
    st->codecpar->codec_id   = em_codec_get_id(lxf_tags, st->codecpar->codec_tag);
    st->need_parsing         = AVSTREAM_PARSE_HEADERS;

    av_em_log(s, AV_LOG_DEBUG, "record: %x = %i-%02i-%02i\n",
           record_date, 1900 + (record_date & 0x7F), (record_date >> 7) & 0xF,
           (record_date >> 11) & 0x1F);

    av_em_log(s, AV_LOG_DEBUG, "expire: %x = %i-%02i-%02i\n",
           expiration_date, 1900 + (expiration_date & 0x7F), (expiration_date >> 7) & 0xF,
           (expiration_date >> 11) & 0x1F);

    if ((video_params >> 22) & 1)
        av_em_log(s, AV_LOG_WARNING, "VBI data not yet supported\n");
    ...
    avio_em_skip(s->pb, lxf->extended_size);

    return 0;
}

static int lxf_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    LXFDemuxContext *lxf = s->priv_data;
    AVEMIOContext   *pb  = s->pb;
    uint32_t stream;
    ...
    if (stream == 1 && s->nb_streams < 2) {
        av_em_log(s, AV_LOG_ERROR, "got audio packet without having an audio stream\n");
        return AVERROR_INVALIDDATA;
    }

    if ((ret2 = av_em_new_packet(pkt, ret)) < 0)
        return ret2;

    if ((ret2 = avio_em_read(pb, pkt->data, ret)) != ret) {
...
AVEMInputFormat ff_lxf_demuxer = {
    .name           = "lxf",
    ...
    .codec_tag      = (const AVEMCodecTag* const []){lxf_tags, 0},
};
