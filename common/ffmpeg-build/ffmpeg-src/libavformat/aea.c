...
static int aea_read_probe(AVEMProbeData *p)
{
...
static int aea_read_header(AVEMFormatContext *s)
{
    ...
    avio_em_skip(s->pb, 264);
    st->codecpar->channels = avio_em_r8(s->pb);
    avio_em_skip(s->pb, 1783);
    ...
    if (st->codecpar->channels != 1 && st->codecpar->channels != 2) {
        av_em_log(s, AV_LOG_ERROR, "Channels %d not supported!\n", st->codecpar->channels);
...
static int aea_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int ret = av_em_get_packet(s->pb, pkt, s->streams[0]->codecpar->block_align);
...
AVEMInputFormat ff_aea_demuxer = {
    .name           = "aea",
