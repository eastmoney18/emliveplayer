...
static int hash_write_header(struct AVEMFormatContext *s)
{
    struct HashContext *c = s->priv_data;
    int res = av_em_hash_alloc(&c->hash, c->hash_name);
    if (res < 0)
        return res;
    av_em_hash_init(c->hash);
    return 0;
}

static int hash_write_packet(struct AVEMFormatContext *s, AVEMPacket *pkt)
{
    struct HashContext *c = s->priv_data;
    av_em_hash_update(c->hash, pkt->data, pkt->size);
    return 0;
}

static int hash_write_trailer(struct AVEMFormatContext *s)
{
    struct HashContext *c = s->priv_data;
    char buf[AV_HASH_MAX_SIZE*2+128];
    snprintf(buf, sizeof(buf) - 200, "%s=", av_em_hash_get_name(c->hash));

    av_em_hash_final_hex(c->hash, buf + strlen(buf), sizeof(buf) - strlen(buf));
    av_em_strlcatf(buf, sizeof(buf), "\n");
    avio_em_write(s->pb, buf, strlen(buf));
    avio_em_flush(s->pb);

    av_em_hash_freep(&c->hash);
    return 0;
}
#endif

#if CONFIG_HASH_MUXER
static const AVEMClass hashenc_class = {
...
AVEMOutputFormat em_hash_muxer = {
    .name              = "hash",
...
#if CONFIG_MD5_MUXER
static const AVEMClass md5enc_class = {
...
static void framehash_print_extradata(struct AVEMFormatContext *s)
{
    int i;
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (par->extradata) {
            ...
            avio_em_printf(s->pb, "#extradata %d, %31d, ", i, par->extradata_size);
            av_em_hash_init(c->hash);
            av_em_hash_update(c->hash, par->extradata, par->extradata_size);
            av_em_hash_final_hex(c->hash, buf, sizeof(buf));
            avio_em_write(s->pb, buf, strlen(buf));
            avio_em_printf(s->pb, "\n");
        }
    }
...
static int framehash_write_header(struct AVEMFormatContext *s)
{
    struct HashContext *c = s->priv_data;
    int res = av_em_hash_alloc(&c->hash, c->hash_name);
    if (res < 0)
        return res;
    avio_em_printf(s->pb, "#format: frame checksums\n");
    avio_em_printf(s->pb, "#version: %d\n", c->format_version);
    avio_em_printf(s->pb, "#hash: %s\n", av_em_hash_get_name(c->hash));
    framehash_print_extradata(s);
    em_framehash_write_header(s);
    avio_em_printf(s->pb, "#stream#, dts,        pts, duration,     size, hash\n");
    return 0;
}

static int framehash_write_packet(struct AVEMFormatContext *s, AVEMPacket *pkt)
{
    struct HashContext *c = s->priv_data;
    char buf[AV_HASH_MAX_SIZE*2+128];
    int len;
    ...
    len = strlen(buf);
    av_em_hash_final_hex(c->hash, buf + len, sizeof(buf) - len);
    avio_em_write(s->pb, buf, strlen(buf));

    if (c->format_version > 1 && pkt->side_data_elems) {
        int i, j;
        avio_em_printf(s->pb, ", S=%d", pkt->side_data_elems);
        for (i = 0; i < pkt->side_data_elems; i++) {
            ...
            if (HAVE_BIGENDIAN && pkt->side_data[i].type == AV_PKT_DATA_PALETTE) {
                for (j = 0; j < pkt->side_data[i].size; j += sizeof(uint32_t)) {
                    ...
                    av_em_hash_update(c->hash, (uint8_t *)&data, sizeof(uint32_t));
                }
            } else
                av_em_hash_update(c->hash, pkt->side_data[i].data, pkt->side_data[i].size);
...
static int framehash_write_trailer(struct AVEMFormatContext *s)
{
    struct HashContext *c = s->priv_data;
    av_em_hash_freep(&c->hash);
    return 0;
...
static const AVEMClass framehash_class = {
...
AVEMOutputFormat ff_framehash_muxer = {
    .name              = "framehash",
...
static const AVEMClass framemd5_class = {
    .class_name = "frame hash encoder class",
    .item_name  = av_em_default_item_name,
    .option     = md5_options,
    .version    = LIBAVUTIL_VERSION_INT,
...
AVEMOutputFormat ff_framemd5_muxer = {
    .name              = "framemd5",
