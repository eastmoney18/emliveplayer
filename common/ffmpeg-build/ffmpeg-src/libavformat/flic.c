...
static int flic_probe(AVEMProbeData *p)
{
    int magic_number;
...
static int flic_read_header(AVEMFormatContext *s)
{
    ...
    if (avio_em_read(pb, header, FLIC_HEADER_SIZE) != FLIC_HEADER_SIZE)
        return AVERROR(EIO);
    ...
    if (em_alloc_extradata(st->codecpar, FLIC_HEADER_SIZE))
        return AVERROR(ENOMEM);
    memcpy(st->codecpar->extradata, header, FLIC_HEADER_SIZE);

    /* peek at the preamble to detect TFTD videos - they seem to always start with an audio chunk */
    if (avio_em_read(pb, preamble, FLIC_PREAMBLE_SIZE) != FLIC_PREAMBLE_SIZE) {
        av_em_log(s, AV_LOG_ERROR, "Failed to peek at preamble\n");
        return AVERROR(EIO);
    }

    avio_em_seek(pb, -FLIC_PREAMBLE_SIZE, SEEK_CUR);
    ...
    if (AV_RL16(&preamble[4]) == FLIC_TFTD_CHUNK_AUDIO) {
        ...
        avpriv_em_set_pts_info(st, 64, ast->codecpar->block_align, FLIC_TFTD_SAMPLE_RATE);
        avpriv_em_set_pts_info(ast, 64, 1, FLIC_TFTD_SAMPLE_RATE);
    } else if (AV_RL16(&header[0x10]) == FLIC_CHUNK_MAGIC_1) {
        avpriv_em_set_pts_info(st, 64, FLIC_MC_SPEED, 70);
        ...
        avpriv_em_set_pts_info(st, 64, speed, 70);
    } else if ((magic_number == FLIC_FILE_MAGIC_2) ||
               (magic_number == FLIC_FILE_MAGIC_3)) {
        avpriv_em_set_pts_info(st, 64, speed, 1000);
    } else {
        av_em_log(s, AV_LOG_ERROR, "Invalid or unsupported magic chunk in file\n");
...
static int flic_read_packet(AVEMFormatContext *s,
                            AVEMPacket *pkt)
{
    FlicDemuxContext *flic = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    while (!packet_read) {
        ...
        if ((ret = avio_em_read(pb, preamble, FLIC_PREAMBLE_SIZE)) !=
            FLIC_PREAMBLE_SIZE) {
            ret = AVERROR(EIO);
            break;
        }
        ...
        if (((magic == FLIC_CHUNK_MAGIC_1) || (magic == FLIC_CHUNK_MAGIC_2)) && size > FLIC_PREAMBLE_SIZE) {
            ...
            ret = avio_em_read(pb, pkt->data + FLIC_PREAMBLE_SIZE,
            ...
            pkt->pos = avio_em_tell(pb);
            ret = avio_em_read(pb, pkt->data, size);
...
AVEMInputFormat ff_flic_demuxer = {
    .name           = "flic",
