...
typedef struct APNGMuxContext {
    ...
    uint32_t plays;
    AVEMRational last_delay;

    uint64_t acTL_offset;
    uint32_t frame_number;

    AVEMPacket *prev_packet;
    AVEMRational prev_delay;
...
static void apng_write_chunk(AVEMIOContext *io_context, uint32_t tag,
                             uint8_t *buf, size_t length)
{
    ...
    avio_em_wb32(io_context, length);
    AV_WB32(tagbuf, tag);
    crc = av_em_crc(crc_table, crc, tagbuf, 4);
    avio_em_wb32(io_context, tag);
    if (length > 0) {
        crc = av_em_crc(crc_table, crc, buf, length);
        avio_em_write(io_context, buf, length);
    }
    avio_em_wb32(io_context, ~crc);
}

static int apng_write_header(AVEMFormatContext *format_context)
{
    ...
    avio_em_wb64(format_context->pb, PNGSIG);
    // Remaining headers are written when they are copied from the encoder

    return 0;
}

static void flush_packet(AVEMFormatContext *format_context, AVEMPacket *packet)
{
    APNGMuxContext *apng = format_context->priv_data;
    AVEMIOContext *io_context = format_context->pb;
    AVEMStream *codec_stream = format_context->streams[0];
    AVEMCodecParameters *codec_par = codec_stream->codecpar;
    ...
    if (apng->frame_number == 0 && !packet) {
        ...
        av_em_log(format_context, AV_LOG_INFO, "Only a single frame so saving as a normal PNG.\n");
        ...
        if (existing_acTL_chunk) {
            ...
            avio_em_write(io_context, codec_par->extradata, existing_acTL_chunk - codec_par->extradata);
            avio_em_write(io_context, chunk_after_acTL, codec_par->extradata + codec_par->extradata_size - chunk_after_acTL);
        } else {
            avio_em_write(io_context, codec_par->extradata, codec_par->extradata_size);
        }
        ...
        if (existing_fcTL_chunk) {
            ...
            avio_em_write(io_context, apng->prev_packet->data, existing_fcTL_chunk - apng->prev_packet->data);
            avio_em_write(io_context, chunk_after_fcTL, apng->prev_packet->data + apng->prev_packet->size - chunk_after_fcTL);
        } else {
            avio_em_write(io_context, apng->prev_packet->data, apng->prev_packet->size);
        }
    } else {
        uint8_t *existing_fcTL_chunk;
        ...
        if (apng->frame_number == 0) {
            ...
            if (!existing_acTL_chunk) {
                ...
                apng->acTL_offset = avio_em_tell(io_context);
        ...
        if (existing_fcTL_chunk) {
            ...
            if (delay.num == 0 && delay.den == 0) {
                if (packet) {
                    ...
                    if (!av_em_reduce(&delay.num, &delay.den, delay_num_raw, delay_den_raw, USHRT_MAX) &&
                ...
                AV_WB32(existing_fcTL_chunk + 26, ~av_em_crc(av_em_crc_get_table(AV_CRC_32_IEEE_LE), ~0U, existing_fcTL_chunk - 4, 26 + 4));
            }
    ...
    av_em_packet_unref(apng->prev_packet);
    if (packet)
        av_em_copy_packet(apng->prev_packet, packet);
}

static int apng_write_packet(AVEMFormatContext *format_context, AVEMPacket *packet)
{
    APNGMuxContext *apng = format_context->priv_data;

    if (!apng->prev_packet) {
        apng->prev_packet = av_em_alloc(sizeof(*apng->prev_packet));
        if (!apng->prev_packet)
...
static int apng_write_trailer(AVEMFormatContext *format_context)
{
    APNGMuxContext *apng = format_context->priv_data;
    AVEMIOContext *io_context = format_context->pb;
    uint8_t buf[8];
...
static const AVEMClass apng_muxer_class = {
...
AVEMOutputFormat ff_apng_muxer = {
    .name           = "apng",
