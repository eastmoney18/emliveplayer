...
static int seq_probe(AVEMProbeData *p)
{
    int i;
...
static int seq_init_frame_buffers(SeqDemuxContext *seq, AVEMIOContext *pb)
{
    int i, sz;
    TiertexSeqFrameBuffer *seq_buffer;

    avio_em_seek(pb, 256, SEEK_SET);
    ...
    for (i = 0; i < SEQ_NUM_FRAME_BUFFERS; i++) {
        ...
        if (sz == 0)
            ...
            seq_buffer->data = av_em_alloc(sz);
            if (!seq_buffer->data)
...
static int seq_fill_buffer(SeqDemuxContext *seq, AVEMIOContext *pb, int buffer_num, unsigned int data_offs, int data_size)
{
    TiertexSeqFrameBuffer *seq_buffer;
    ...
    avio_em_seek(pb, seq->current_frame_offs + data_offs, SEEK_SET);
    if (avio_em_read(pb, seq_buffer->data + seq_buffer->fill_size, data_size) != data_size)
        return AVERROR(EIO);
...
static int seq_parse_frame_data(SeqDemuxContext *seq, AVEMIOContext *pb)
{
    ...
    avio_em_seek(pb, seq->current_frame_offs, SEEK_SET);

    /* sound data */
    seq->current_audio_data_offs = avio_em_rl16(pb);
    ...
    /* palette data */
    seq->current_pal_data_offs = avio_em_rl16(pb);
    ...
    /* video data */
    for (i = 0; i < 4; i++)
        buffer_num[i] = avio_em_r8(pb);

    for (i = 0; i < 4; i++)
        offset_table[i] = avio_em_rl16(pb);
...
static int seq_read_header(AVEMFormatContext *s)
{
    int i, rc;
    ...
    avpriv_em_set_pts_info(st, 32, 1, SEQ_FRAME_RATE);
    ...
    st->start_time = 0;
    avpriv_em_set_pts_info(st, 32, 1, SEQ_SAMPLE_RATE);
...
static int seq_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int rc;
    SeqDemuxContext *seq = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    if (!seq->audio_buffer_full) {
        ...
        if (seq->current_pal_data_size + seq->current_video_data_size != 0) {
            if (av_em_new_packet(pkt, 1 + seq->current_pal_data_size + seq->current_video_data_size))
                return AVERROR(ENOMEM);

            pkt->data[0] = 0;
            if (seq->current_pal_data_size) {
                pkt->data[0] |= 1;
                avio_em_seek(pb, seq->current_frame_offs + seq->current_pal_data_offs, SEEK_SET);
                if (avio_em_read(pb, &pkt->data[1], seq->current_pal_data_size) != seq->current_pal_data_size)
                    return AVERROR(EIO);
            }
    ...
    avio_em_seek(pb, seq->current_frame_offs + seq->current_audio_data_offs, SEEK_SET);
    rc = av_em_get_packet(pb, pkt, seq->current_audio_data_size);
    if (rc < 0)
        return rc;
...
static int seq_read_close(AVEMFormatContext *s)
{
    int i;
    SeqDemuxContext *seq = s->priv_data;

    for (i = 0; i < SEQ_NUM_FRAME_BUFFERS; i++)
        av_em_freep(&seq->frame_buffers[i].data);

    return 0;
}

AVEMInputFormat ff_tiertexseq_demuxer = {
    .name           = "tiertexseq",
