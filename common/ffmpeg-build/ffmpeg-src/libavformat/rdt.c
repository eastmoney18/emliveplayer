...
struct RDTDemuxContext {
    AVEMFormatContext *ic; /**< the containing (RTSP) demux context */
     ...
     * codecs/bitrates). Each such stream is represented by one AVEMStream
     * in the AVEMFormatContext, and this variable points to the offset in
...
RDTDemuxContext *
ff_rdt_parse_open(AVEMFormatContext *ic, int first_stream_of_set_idx,
                  void *priv_data, RTPDynamicProtocolHandler *handler)
{
    RDTDemuxContext *s = av_em_mallocz(sizeof(RDTDemuxContext));
    if (!s)
        return NULL;
...
#define XOR_TABLE_SIZE 37
    ...
    av_em_md5_sum(zres, buf, 64);
    em_data_to_hex(response, zres, 16, 1);
...
static int
rdt_load_mdpr (PayloadContext *rdt, AVEMStream *st, int rule_nr)
{
    AVEMIOContext pb;
    unsigned int size;
    uint32_t tag;
    ...
    if (!rdt->mlti_data)
        return -1;
    emio_init_context(&pb, rdt->mlti_data, rdt->mlti_data_size, 0,
                  NULL, NULL, NULL, NULL);
    tag = avio_em_rl32(&pb);
    if (tag == MKTAG('M', 'L', 'T', 'I')) {
        ...
        if (rule_nr < 0 || rule_nr >= num)
            return -1;
        avio_em_skip(&pb, rule_nr * 2);
        chunk_nr = avio_em_rb16(&pb);
        avio_em_skip(&pb, (num - 1 - rule_nr) * 2);
        ...
        if (chunk_nr >= num)
            return -1;
        while (chunk_nr--)
            avio_em_skip(&pb, avio_em_rb32(&pb));
...
static int
rdt_parse_packet (AVEMFormatContext *ctx, PayloadContext *rdt, AVEMStream *st,
                  AVEMPacket *pkt, uint32_t *timestamp,
                  const uint8_t *buf, int len, uint16_t rtp_seq, int flags)
{
    ...
    if (rdt->audio_pkt_cnt == 0) {
        ...
        emio_init_context(&pb, (uint8_t *)buf, len, 0, NULL, NULL, NULL, NULL);
        ...
        if (res > 0) {
            if (st->codecpar->codec_id == AV_CODEC_ID_AAC) {
                ...
                rdt->rmctx->pb = avio_em_alloc_context (rdt->buffer, len - pos, 0,
                                                    NULL, NULL, NULL, NULL);
            }
            goto get_cache;
        }
...
int
ff_rdt_parse_packet(RDTDemuxContext *s, AVEMPacket *pkt,
...
{
    av_em_strlcatf(cmd, size, "stream=%d;rule=%d,stream=%d;rule=%d",
...
rdt_parse_b64buf (unsigned int *target_len, const char *p)
{
    ...
    *target_len = len * 3 / 4;
    target = av_em_mallocz(*target_len + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!target)
        return NULL;
    av_em_base64_decode(target, p, *target_len);
    return target;
}

static int
rdt_parse_sdp_line (AVEMFormatContext *s, int st_index,
                    PayloadContext *rdt, const char *line)
{
    AVEMStream *stream = s->streams[st_index];
    const char *p = line;

    if (av_em_strstart(p, "OpaqueData:buffer;", &p)) {
        rdt->mlti_data = rdt_parse_b64buf(&rdt->mlti_data_size, p);
    } else if (av_em_strstart(p, "StartTime:integer;", &p))
        stream->first_dts = atoi(p);
    else if (av_em_strstart(p, "ASMRuleBook:string;", &p)) {
        int n, first = -1;
        ...
        for (n = 0; n < s->nb_streams; n++)
            if (s->streams[n]->id == stream->id) {
                ...
                if (rdt->nb_rmst < count) {
                    if ((err = av_em_reallocp(&rdt->rmst,
...
static void
real_parse_asm_rule(AVEMStream *st, const char *p, const char *end)
{
    do {
...
static AVEMStream *
add_dstream(AVEMFormatContext *s, AVEMStream *orig_st)
{
    AVEMStream *st;
...
static void
real_parse_asm_rulebook(AVEMFormatContext *s, AVEMStream *orig_st,
                        const char *p)
{
    const char *end;
    int n_rules = 0, odd = 0;
...
void
ff_real_parse_sdp_a_line (AVEMFormatContext *s, int stream_index,
                          const char *line)
{
    const char *p = line;

    if (av_em_strstart(p, "ASMRuleBook:string;", &p))
        real_parse_asm_rulebook(s, s->streams[stream_index], p);
}



static av_cold int rdt_init(AVEMFormatContext *s, int st_index, PayloadContext *rdt)
{
    int ret;

    rdt->rmctx = avformat_em_alloc_context();
    if (!rdt->rmctx)
        return AVERROR(ENOMEM);

    if ((ret = em_copy_whiteblacklists(rdt->rmctx, s)) < 0)
        return ret;

    return avformat_em_open_input(&rdt->rmctx, "", &ff_rdt_demuxer, NULL);
}
...
rdt_close_context (PayloadContext *rdt)
{
    ...
    for (i = 0; i < rdt->nb_rmst; i++)
        if (rdt->rmst[i]) {
            ...
            av_em_freep(&rdt->rmst[i]);
        }
    if (rdt->rmctx)
        avformat_em_close_input(&rdt->rmctx);
    av_em_freep(&rdt->mlti_data);
    av_em_freep(&rdt->rmst);
}
