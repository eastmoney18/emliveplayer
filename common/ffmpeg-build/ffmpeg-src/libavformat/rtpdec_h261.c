...
static int h261_handle_packet(AVEMFormatContext *ctx, PayloadContext *rtp_h261_ctx,
                              ...
                              int flags)
{
    ...
    if (rtp_h261_ctx->buf && rtp_h261_ctx->timestamp != *timestamp) {
        emio_free_dyn_buf(&rtp_h261_ctx->buf);
        rtp_h261_ctx->endbyte_bits = 0;
    }

    /* sanity check for size of input packet: 1 byte payload at least */
    if (len < RTP_H261_PAYLOAD_HEADER_SIZE + 1) {
        av_em_log(ctx, AV_LOG_ERROR, "Too short RTP/H.261 packet, got %d bytes\n", len);
    ...
    if (!rtp_h261_ctx->buf) {
        ...
        if (!gobn && !sbit && !mbap && !quant) {
            res = avio_em_open_dyn_buf(&rtp_h261_ctx->buf);
            if (res < 0)
                return res;
    ...
    if (rtp_h261_ctx->endbyte_bits || sbit) {
        if (rtp_h261_ctx->endbyte_bits == sbit) {
            ...
            buf++;
            len--;
            avio_em_w8(rtp_h261_ctx->buf, rtp_h261_ctx->endbyte);
        } else {
            ...
            while (get_bits_left(&gb) >= 8)
                avio_em_w8(rtp_h261_ctx->buf, get_bits(&gb, 8));
    ...
    if (ebit) {
        if (len > 0)
            avio_em_write(rtp_h261_ctx->buf, buf, len - 1);
        rtp_h261_ctx->endbyte_bits = 8 - ebit;
        rtp_h261_ctx->endbyte      = buf[len - 1] & (0xff << ebit);
    } else {
        avio_em_write(rtp_h261_ctx->buf, buf, len);
    }
...
RTPDynamicProtocolHandler em_h261_dynamic_handler = {
    .enc_name          = "H261",
