...
static int64_t read_line(AVEMBPrint *buf, AVEMIOContext *pb)
{
...
static int lrc_probe(AVEMProbeData *p)
{
    int64_t offset = 0;
    int64_t mm;
    uint64_t ss, cs;
...
static int lrc_read_header(AVEMFormatContext *s)
{
                ...
                *right_bracket_offset = *comma_offset = '\0';
                if(strcmp(line.str + 1, "offset") ||
                   sscanf(comma_offset + 1, "%"SCNd64, &lrc->ts_offset) != 1) {
                    av_em_dict_set(&s->metadata, line.str + 1, comma_offset + 1, 0);
                }
                *comma_offset = ':';
...
static int lrc_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    LRCContext *lrc = s->priv_data;
    return ff_subtitles_queue_read_packet(&lrc->q, pkt);
}

static int lrc_read_seek(AVEMFormatContext *s, int stream_index,
...
static int lrc_read_close(AVEMFormatContext *s)
{
    LRCContext *lrc = s->priv_data;
...
AVEMInputFormat ff_lrc_demuxer = {
    .name           = "lrc",
