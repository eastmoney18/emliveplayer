...
static int dtshd_probe(AVEMProbeData *p)
{
...
static int dtshd_read_header(AVEMFormatContext *s)
{
    ...
    for (;;) {
        chunk_type = avio_em_rb64(pb);
        chunk_size = avio_em_rb64(pb);
        ...
        if (chunk_size > ((uint64_t)1 << 61)) {
            av_em_log(s, AV_LOG_ERROR, "chunk size too big\n");
            return AVERROR_INVALIDDATA;
        }

        switch (chunk_type) {
        case STRMDATA:
            data_start = avio_em_tell(pb);
        ...
        case AUPR_HDR:
            if (chunk_size < 21)
                return AVERROR_INVALIDDATA;
            avio_em_skip(pb, 3);
            st->codecpar->sample_rate = avio_em_rb24(pb);
            if (!st->codecpar->sample_rate)
                return AVERROR_INVALIDDATA;
            duration  = avio_em_rb32(pb); // num_frames
            duration *= avio_em_rb16(pb); // samples_per_frames
            st->duration = duration;
            avio_em_skip(pb, 5);
            st->codecpar->channels = ff_dca_count_chs_for_mask(avio_em_rb16(pb));
            st->codecpar->initial_padding = avio_em_rb16(pb);
            avio_em_skip(pb, chunk_size - 21);
            break;
        case FILEINFO:
            if (chunk_size > INT_MAX)
                goto skip;
            value = av_em_alloc(chunk_size);
            if (!value)
                goto skip;
            avio_em_read(pb, value, chunk_size);
            value[chunk_size - 1] = 0;
            av_em_dict_set(&s->metadata, "fileinfo", value,
                        AV_DICT_DONT_STRDUP_VAL);
            break;
        default:
skip:
            ret = avio_em_skip(pb, chunk_size);
            if (ret < 0)
                return ret;
        };
    }
    ...
    avio_em_seek(pb, data_start, SEEK_SET);
...
static int raw_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    left = dtshd->data_end - avio_em_tell(s->pb);
...
AVEMInputFormat ff_dtshd_demuxer = {
    .name           = "dtshd",
