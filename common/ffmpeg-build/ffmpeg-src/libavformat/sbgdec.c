                              ...
                              int *size, int *max_size)
{
    ...
    if (*size == *max_size) {
        ...
        if (*size >= m)
            return NULL;
        *array = av_em_realloc_f(*array, m, elsize);
        if (!*array)
            return NULL;
        *max_size = m;
    }
...
static inline int scale_double(void *log, double d, double m, int *r)
{
    ...
    if (m < INT_MIN || m >= INT_MAX) {
        if (log)
            av_em_log(log, AV_LOG_ERROR, "%g is too large\n", d);
        return AVERROR(EDOM);
    }
    *r = m;
    return 0;
    ...
    while (lex_char(p, '-') && lex_wsword(p, &ostr)) {
        for (; ostr.s < ostr.e; ostr.s++) {
            ...
            switch (opt) {
                ...
                case 'm':
                    FORWARD_ERROR(parse_optarg(p, opt, &oarg));
                    tptr = av_em_alloc(oarg.e - oarg.s + 1);
                    if (!tptr)
                        return AVERROR(ENOMEM);
                    memcpy(tptr, oarg.s, oarg.e - oarg.s);
                    tptr[oarg.e - oarg.s] = 0;
                    av_em_free(p->scs.opt_mix);
                    p->scs.opt_mix = tptr;
                    break;
                case 'q':
                    FORWARD_ERROR(parse_optarg(p, opt, &oarg));
...
static void free_script(struct sbg_script *s)
{
    ...
    av_em_freep(&s->synth);
    av_em_freep(&s->tseq);
    av_em_freep(&s->block_tseq);
    av_em_freep(&s->events);
    av_em_freep(&s->opt_mix);
}
...
fail:
    ...
    if (log && *sp.err_msg) {
        ...
        const char *ectx = av_em_x_if_null(memchr(ctx, '\n', sp.end - sp.cursor),
                                        sp.end);
        int lctx = ectx - ctx;
        const char *quote = "\"";
        ...
        av_em_log(log, AV_LOG_ERROR, "Error line %d: %s near %s%.*s%s.\n",
               sp.line_no, sp.err_msg, quote, lctx, ctx, quote);
    }
    return r;
}

static int read_whole_file(AVEMIOContext *io, int max_size, char **rbuf)
{
    char *buf = NULL;
    ...
    while (1) {
        if (bufsize - size < 1024) {
            ...
            buf = av_em_realloc_f(buf, bufsize, 1);
            if (!buf) {
                size = AVERROR(ENOMEM);
                goto fail;
            }
        }
        r = avio_em_read(io, buf, bufsize - size - 1);
        if (r == AVERROR_EOF)
            break;
        if (r < 0)
            goto fail;
...
#undef time
        ...
        now = tm ? tm->tm_hour * 3600 + tm->tm_min * 60 + tm->tm_sec :
                   now0 % DAY;
        av_em_log(log, AV_LOG_INFO, "Using %02d:%02d:%02d as NOW.\n",
                       ...
                       int64_t t0, struct sbg_script_tseq *tseq)
{
    ...
    if (tseq->lock++) {
        av_em_log(log, AV_LOG_ERROR, "Recursion loop on \"%.*s\"\n",
    ...
    if (i >= s->nb_def) {
        av_em_log(log, AV_LOG_ERROR, "Tone-set \"%.*s\" not defined\n",
               tseq->name_len, tseq->name);
        return AVERROR(EINVAL);
    }
    def = &s->def[i];
...
static int expand_script(void *log, struct sbg_script *s)
{
    ...
    if (!s->nb_events) {
        av_em_log(log, AV_LOG_ERROR, "No events in script\n");
        return AVERROR_INVALIDDATA;
    }
                             ...
                             int transition)
{
    ...
    switch (s1->type) {
        ...
        case SBG_TYPE_SPIN:
            av_em_log(log, AV_LOG_WARNING, "Spinning noise not implemented, "
                              ...
                              struct ws_intervals *inter)
{
    ...
    for (i = -1; i < s->nb_events; i++) {
        ...
        ev1->ts_int   = av_em_rescale(ev1->ts_int,   sample_rate, AV_TIME_BASE);
        ev1->ts_trans = av_em_rescale(ev1->ts_trans, sample_rate, AV_TIME_BASE);
        ev1->ts_next  = av_em_rescale(ev1->ts_next,  sample_rate, AV_TIME_BASE);
    }
    ...
    if (!inter->nb_inter)
        av_em_log(log, AV_LOG_WARNING, "Completely silent script.\n");
    return 0;
}

static int encode_intervals(struct sbg_script *s, AVEMCodecParameters *par,
                            struct ws_intervals *inter)
{
    int i, edata_size = 4;
    uint8_t *edata;
    ...
    if (em_alloc_extradata(par, edata_size))
        return AVERROR(ENOMEM);
    edata = par->extradata;
...
static av_cold int sbg_read_probe(AVEMProbeData *p)
{
    int r, score;
...
static av_cold int sbg_read_header(AVEMFormatContext *avf)
{
    ...
    if (script.opt_mix)
        av_em_log(avf, AV_LOG_WARNING, "Mix feature not implemented: "
    ...
    st = avformat_em_new_stream(avf, NULL);
    if (!st)
        return AVERROR(ENOMEM);
    ...
    st->probe_packets = 0;
    st->start_time    = av_em_rescale(script.start_ts,
                                   sbg->sample_rate, AV_TIME_BASE);
    st->duration      = script.end_ts == AV_NOPTS_VALUE ? AV_NOPTS_VALUE :
                        av_em_rescale(script.end_ts - script.start_ts,
                                   sbg->sample_rate, AV_TIME_BASE);
...
static int sbg_read_packet(AVEMFormatContext *avf, AVEMPacket *packet)
{
    int64_t ts, end_ts;
    ...
    if (end_ts <= ts)
        return AVERROR_EOF;
    if (av_em_new_packet(packet, 12) < 0)
        return AVERROR(ENOMEM);
...
static int sbg_read_seek2(AVEMFormatContext *avf, int stream_index,
                          int64_t min_ts, int64_t ts, int64_t max_ts, int flags)
{
    if (flags || stream_index > 0)
        return AVERROR(EINVAL);
    if (stream_index < 0)
        ts = av_em_rescale_q(ts, AV_TIME_BASE_Q, avf->streams[0]->time_base);
    avf->streams[0]->cur_dts = ts;
    return 0;
}

static int sbg_read_seek(AVEMFormatContext *avf, int stream_index,
...
static const AVEMClass sbg_demuxer_class = {
...
AVEMInputFormat ff_sbg_demuxer = {
    .name           = "sbg",
