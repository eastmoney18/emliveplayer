...
static int print_tls_error(EMURLContext *h, int ret)
{
    ...
    case errSSLXCertChainInvalid:
        av_em_log(h, AV_LOG_ERROR, "Invalid certificate chain\n");
        return AVERROR(EIO);
    case ioErr:
        return c->lastErr;
    default:
        av_em_log(h, AV_LOG_ERROR, "IO Error: %i\n", ret);
        return AVERROR(EIO);
    }
    return AVERROR(EIO);
}

static int import_pem(EMURLContext *h, char *path, CFArrayRef *array)
{
    ...
    if ((ret = emio_open_whitelist(&s, path, AVIO_FLAG_READ,
                                   &h->interrupt_callback, NULL,
                                   h->protocol_whitelist, h->protocol_blacklist)) < 0)
        goto end;

    if ((ret = avio_em_size(s)) < 0)
        goto end;
    ...
    if (!(buf = av_em_alloc(ret))) {
        ret = AVERROR(ENOMEM);
        goto end;
    }

    if ((ret = avio_em_read(s, buf, ret)) < 0)
        goto end;
...
static int load_ca(EMURLContext *h)
{
    TLSContext *c = h->priv_data;
    int ret = 0;
...
static int load_cert(EMURLContext *h)
{
    TLSContext *c = h->priv_data;
    int ret = 0;
...
{
    EMURLContext *h = (EMURLContext*)connection;
    TLSContext *c = h->priv_data;
    int read = ffurl_em_read_complete(c->tls_shared.tcp, data, *dataLength);
    if (read <= 0) {
        *dataLength = 0;
...
static OSStatus tls_write_cb(SSLConnectionRef connection, const void *data, size_t *dataLength)
{
    EMURLContext *h = (EMURLContext*)connection;
    TLSContext *c = h->priv_data;
    int written = ffurl_em_write(c->tls_shared.tcp, data, *dataLength);
    if (written <= 0) {
        *dataLength = 0;
...
#define CHECK_ERROR(func, ...) do {                                     \
        ...
        if (status != noErr) {                                          \
            ret = AVERROR_UNKNOWN;                                      \
            av_em_log(h, AV_LOG_ERROR, #func ": Error %i\n", (int)status); \
            goto fail;                                                  \
        }                                                               \
    } while (0)
...
static int tls_open(EMURLContext *h, const char *uri, int flags, AVEMDictionary **options)
{
    ...
    if (!c->ssl_context) {
        av_em_log(h, AV_LOG_ERROR, "Unable to create SSL context\n");
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    if (s->ca_file) {
    ...
    while (1) {
        ...
        av_em_log(h, AV_LOG_ERROR, "Unable to negotiate TLS/SSL session: %i\n", (int)status);
        ret = AVERROR(EIO);
        goto fail;
    }
...
static int tls_read(EMURLContext *h, uint8_t *buf, int size)
{
    TLSContext *c = h->priv_data;
    size_t processed = 0;
...
static int tls_write(EMURLContext *h, const uint8_t *buf, int size)
{
    TLSContext *c = h->priv_data;
    size_t processed = 0;
...
const EMURLProtocol em_tls_securetransport_protocol = {
    .name           = "tls",
    .url_open2      = tls_open,
    .url_read       = tls_read,
    .url_write      = tls_write,
