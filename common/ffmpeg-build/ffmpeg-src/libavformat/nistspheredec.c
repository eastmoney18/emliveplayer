...
static int nist_probe(AVEMProbeData *p)
{
...
static int nist_read_header(AVEMFormatContext *s)
{
    ...
    em_get_line(s->pb, buffer, sizeof(buffer));
    em_get_line(s->pb, buffer, sizeof(buffer));
    ...
    while (!avio_em_feof(s->pb)) {
        ...
        if (avio_em_tell(s->pb) >= header_size)
            return AVERROR_INVALIDDATA;
        ...
        if (!memcmp(buffer, "end_head", 8)) {
            ...
            if (!av_em_strcasecmp(coding, "pcm")) {
                if (st->codecpar->codec_id == AV_CODEC_ID_NONE)
                    st->codecpar->codec_id = em_get_pcm_codec_id(st->codecpar->bits_per_coded_sample,
                                                              0, be, 0xFFFF);
            } else if (!av_em_strcasecmp(coding, "alaw")) {
                st->codecpar->codec_id = AV_CODEC_ID_PCM_ALAW;
            } else if (!av_em_strcasecmp(coding, "ulaw") ||
                       !av_em_strcasecmp(coding, "mu-law")) {
                st->codecpar->codec_id = AV_CODEC_ID_PCM_MULAW;
            } else if (!av_em_strncasecmp(coding, "pcm,embedded-shorten", 20)) {
                st->codecpar->codec_id = AV_CODEC_ID_SHORTEN;
                if (em_alloc_extradata(st->codecpar, 1))
                    st->codecpar->extradata[0] = 1;
            } else {
                avpriv_em_request_sample(s, "coding %s", coding);
            }
            ...
            avio_em_skip(s->pb, header_size - avio_em_tell(s->pb));
            ...
            if (!av_em_strcasecmp(format, "01")) {
                be = 0;
            } else if (!av_em_strcasecmp(format, "10")) {
                be = 1;
            } else if (!av_em_strcasecmp(format, "mu-law")) {
                st->codecpar->codec_id = AV_CODEC_ID_PCM_MULAW;
            } else if (av_em_strcasecmp(format, "1")) {
                avpriv_em_request_sample(s, "sample byte format %s", format);
            ...
            if (sscanf(buffer, "%31s %*s %31s", key, value) == 2) {
                av_em_dict_set(&s->metadata, key, value, AV_DICT_APPEND);
            } else {
                av_em_log(s, AV_LOG_ERROR, "Failed to parse '%s' as metadata\n", buffer);
            }
        }
    }
...
AVEMInputFormat ff_nistsphere_demuxer = {
    .name           = "nistsphere",
