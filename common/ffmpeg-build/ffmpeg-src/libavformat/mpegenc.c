...
extern AVEMOutputFormat ff_mpeg1vcd_muxer;
extern AVEMOutputFormat ff_mpeg2dvd_muxer;
extern AVEMOutputFormat ff_mpeg2svcd_muxer;
extern AVEMOutputFormat ff_mpeg2vob_muxer;

static int put_pack_header(AVEMFormatContext *ctx, uint8_t *buf,
                           int64_t timestamp)
{
...
static int put_system_header(AVEMFormatContext *ctx, uint8_t *buf,
...
static int get_system_header_size(AVEMFormatContext *ctx)
{
...
static av_cold int mpeg_mux_init(AVEMFormatContext *ctx)
{
    MpegMuxContext *s = ctx->priv_data;
    ...
    if (ctx->packet_size) {
        if (ctx->packet_size < 20 || ctx->packet_size > (1 << 23) + 10) {
            av_em_log(ctx, AV_LOG_ERROR, "Invalid packet size %d\n",
                   ctx->packet_size);
            goto fail;
        }
    ...
    for (i = 0; i < ctx->nb_streams; i++) {
        ...
        st     = ctx->streams[i];
        stream = av_em_mallocz(sizeof(StreamInfo));
        if (!stream)
            goto fail;
        st->priv_data = stream;

        avpriv_em_set_pts_info(st, 64, 1, 90000);
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            props = (EMAVCPBProperties*)av_em_stream_get_side_data(st, AV_PKT_DATA_CPB_PROPERTIES, NULL);
            ...
            if (stream->max_buffer_size > 1024 * 8191) {
                av_em_log(ctx, AV_LOG_WARNING, "buffer size %d, too large\n", stream->max_buffer_size);
        ...
        case AVMEDIA_TYPE_SUBTITLE:
            ...
            break;
        default:
            av_em_log(ctx, AV_LOG_ERROR, "Invalid media type %s for output stream #%d\n",
                   av_em_get_media_type_string(st->codecpar->codec_type), i);
            return AVERROR(EINVAL);
        }
        stream->fifo = av_em_fifo_alloc(16);
        if (!stream->fifo)
            goto fail;
    }
    bitrate       = 0;
    ...
    if (s->user_mux_rate) {
        ...
        if (s->mux_rate >= (1<<22)) {
            av_em_log(ctx, AV_LOG_WARNING, "mux rate %d is too large\n", s->mux_rate);
            s->mux_rate = (1<<22) - 1;
        }
    }
...
fail:
    for (i = 0; i < ctx->nb_streams; i++)
        av_em_freep(&ctx->streams[i]->priv_data);
    return AVERROR(ENOMEM);
}

static inline void put_timestamp(AVEMIOContext *pb, int id, int64_t timestamp)
{
    avio_em_w8(pb, (id << 4) |  (((timestamp >> 30) & 0x07)   << 1) | 1);
    avio_em_wb16(pb, (uint16_t)((((timestamp >> 15) & 0x7fff) << 1) | 1));
    avio_em_wb16(pb, (uint16_t)((((timestamp)       & 0x7fff) << 1) | 1));
}

/* return the number of padding bytes that should be inserted into
 * the multiplexed stream. */
static int get_vcd_padding_size(AVEMFormatContext *ctx, int64_t pts)
{
    MpegMuxContext *s = ctx->priv_data;
    int pad_bytes = 0;
    ...
    if (s->vcd_padding_bitrate_num > 0 && pts != AV_NOPTS_VALUE) {
        ...
        // FIXME: this is wrong
        full_pad_bytes =
            av_em_rescale(s->vcd_padding_bitrate_num, pts, 90000LL * 8 * VCD_PADDING_BITRATE_DEN);
...
static void put_padding_packet(AVEMFormatContext *ctx, AVEMIOContext *pb,
                               int packet_bytes)
{
    MpegMuxContext *s = ctx->priv_data;
    int i;

    avio_em_wb32(pb, PADDING_STREAM);
    avio_em_wb16(pb, packet_bytes - 6);
    if (!s->is_mpeg2) {
        avio_em_w8(pb, 0x0f);
        packet_bytes -= 7;
    } else
...
static int get_nb_frames(AVEMFormatContext *ctx, StreamInfo *stream, int len)
{
    int nb_frames        = 0;
...
static int flush_packet(AVEMFormatContext *ctx, int stream_index,
                        int64_t pts, int64_t dts, int64_t scr, int trailer_size)
{
    ...
    av_em_log(ctx, AV_LOG_TRACE, "packet ID=%2x PTS=%0.3f\n", id, pts / 90000.0);
        ...
        if (s->is_vcd) {
            ...
            if (stream->align_iframe || s->packet_number == 0) {
                ...
                if (stream->bytes_to_iframe == 0 || s->packet_number == 0) {
                    ...
                    buf_ptr += size;
                    size     = buf_ptr - buffer;
                    avio_em_write(ctx->pb, buffer, size);

                    avio_em_wb32(ctx->pb, PRIVATE_STREAM_2);
                    avio_em_wb16(ctx->pb, 0x03d4);     // length
                    avio_em_w8(ctx->pb, 0x00);         // substream ID, 00=PCI
                    for (i = 0; i < 979; i++)
                        avio_em_w8(ctx->pb, 0x00);

                    avio_em_wb32(ctx->pb, PRIVATE_STREAM_2);
                    avio_em_wb16(ctx->pb, 0x03fa);     // length
                    avio_em_w8(ctx->pb, 0x01);         // substream ID, 01=DSI
                    for (i = 0; i < 1017; i++)
                        avio_em_w8(ctx->pb, 0x00);
    ...
    if (packet_size > 0) {
        ...
        stuffing_size = payload_size - av_em_fifo_size(stream->fifo);
        ...
        if (startcode == PRIVATE_STREAM_1 && id >= 0xa0) {
            if (payload_size < av_em_fifo_size(stream->fifo))
        ...
        avio_em_wb32(ctx->pb, startcode);

        avio_em_wb16(ctx->pb, packet_size);

        if (!s->is_mpeg2)
            for (i = 0; i < stuffing_size; i++)
                avio_em_w8(ctx->pb, 0xff);

        if (s->is_mpeg2) {
            avio_em_w8(ctx->pb, 0x80); /* mpeg2 id */
            ...
            avio_em_w8(ctx->pb, pes_flags); /* flags */
            avio_em_w8(ctx->pb, header_len - 3 + stuffing_size);
            ...
            if (pes_flags & 0x01) {  /* write pes extension */
                avio_em_w8(ctx->pb, 0x10); /* flags */

                /* P-STD buffer info */
                if ((id & 0xe0) == AUDIO_ID)
                    avio_em_wb16(ctx->pb, 0x4000 | stream->max_buffer_size / 128);
                else
                    avio_em_wb16(ctx->pb, 0x6000 | stream->max_buffer_size / 1024);
            }
        } else {
            if (pts != AV_NOPTS_VALUE) {
                if (dts != pts) {
        ...
        if (startcode == PRIVATE_STREAM_1) {
            avio_em_w8(ctx->pb, id);
            if (id >= 0xa0) {
                /* LPCM (XXX: check nb_frames) */
                avio_em_w8(ctx->pb, 7);
                avio_em_wb16(ctx->pb, 4); /* skip 3 header bytes */
                avio_em_w8(ctx->pb, stream->lpcm_header[0]);
                avio_em_w8(ctx->pb, stream->lpcm_header[1]);
                avio_em_w8(ctx->pb, stream->lpcm_header[2]);
            } else if (id >= 0x40) {
                /* AC-3 */
                avio_em_w8(ctx->pb, nb_frames);
                avio_em_wb16(ctx->pb, trailer_size + 1);
            }
        }

        /* output data */
        av_assert0(payload_size - stuffing_size <= av_em_fifo_size(stream->fifo));
        av_em_fifo_generic_read(stream->fifo, ctx->pb,
                             payload_size - stuffing_size,
                             (void (*)(void*, void*, int))avio_em_write);
...
static void put_vcd_padding_sector(AVEMFormatContext *ctx)
{
...
static int remove_decoded_packets(AVEMFormatContext *ctx, int64_t scr)
{
    int i;

    for (i = 0; i < ctx->nb_streams; i++) {
        AVEMStream *st = ctx->streams[i];
...
static int output_packet(AVEMFormatContext *ctx, int flush)
{
    MpegMuxContext *s = ctx->priv_data;
    AVEMStream *st;
    StreamInfo *stream;
    ...
    const int64_t max_delay = av_em_rescale(ctx->max_delay, 90000, AV_TIME_BASE);

retry:
    for (i = 0; i < ctx->nb_streams; i++) {
        AVEMStream *st = ctx->streams[i];
        StreamInfo *stream = st->priv_data;
        const int avail_data = av_em_fifo_size(stream->fifo);
    ...
    if (best_i < 0) {
        ...
        if (best_dts < INT64_MAX) {
            av_em_log(ctx, AV_LOG_TRACE, "bumping scr, scr:%f, dts:%f\n",
    ...
    av_assert0(av_em_fifo_size(stream->fifo) > 0);
    ...
    if (timestamp_packet) {
        av_em_log(ctx, AV_LOG_TRACE, "dts:%f pts:%f scr:%f stream:%d\n",
        ...
        av_assert0(av_em_fifo_size(stream->fifo) == trailer_size);
...
static int mpeg_mux_write_packet(AVEMFormatContext *ctx, AVEMPacket *pkt)
{
    int stream_index = pkt->stream_index;
    int size         = pkt->size;
    uint8_t *buf     = pkt->data;
    MpegMuxContext *s = ctx->priv_data;
    AVEMStream *st      = ctx->streams[stream_index];
    StreamInfo *stream = st->priv_data;
    int64_t pts, dts;
    PacketDesc *pkt_desc;
    int preload;
    ...
    preload = av_em_rescale(s->preload, 90000, AV_TIME_BASE);
    ...
    if (s->last_scr == AV_NOPTS_VALUE) {
        if (dts == AV_NOPTS_VALUE || (dts < preload && ctx->avoid_negative_ts) || s->is_dvd) {
            if (dts != AV_NOPTS_VALUE)
                s->preload += av_em_rescale(-dts, AV_TIME_BASE, 90000);
            s->last_scr = 0;
        } else {
        ...
        preload = av_em_rescale(s->preload, 90000, AV_TIME_BASE);
        av_em_log(ctx, AV_LOG_DEBUG, "First SCR: %"PRId64" First DTS: %"PRId64"\n", s->last_scr, dts + preload);
    }

    if (dts != AV_NOPTS_VALUE) dts += preload;
    if (pts != AV_NOPTS_VALUE) pts += preload;

    av_em_log(ctx, AV_LOG_TRACE, "dts:%f pts:%f flags:%d stream:%d nopts:%d\n",
            dts / 90000.0, pts / 90000.0, pkt->flags,
            pkt->stream_index, pts != AV_NOPTS_VALUE);
    if (!stream->premux_packet)
        stream->next_packet = &stream->premux_packet;
    *stream->next_packet     =
    pkt_desc                 = av_em_mallocz(sizeof(PacketDesc));
    pkt_desc->pts            = pts;
    pkt_desc->dts            = dts;
    ...
    if (av_em_fifo_realloc2(stream->fifo, av_em_fifo_size(stream->fifo) + size) < 0)
        return -1;
    ...
    if (s->is_dvd) {
        ...
        if (is_iframe &&
            ...
            stream->bytes_to_iframe = av_em_fifo_size(stream->fifo);
            stream->align_iframe    = 1;
            stream->vobu_start_pts  = pts;
        }
    }

    av_em_fifo_generic_write(stream->fifo, buf, size, NULL);
...
static int mpeg_mux_end(AVEMFormatContext *ctx)
{
    StreamInfo *stream;
    int i;
    ...
    // avio_em_wb32(ctx->pb, ISO_11172_END_CODE);
    // avio_em_flush(ctx->pb);
...
MPEGENC_CLASS(mpeg)
AVEMOutputFormat ff_mpeg1system_muxer = {
    .name              = "mpeg",
...
MPEGENC_CLASS(vcd)
AVEMOutputFormat ff_mpeg1vcd_muxer = {
    .name              = "vcd",
...
MPEGENC_CLASS(vob)
AVEMOutputFormat ff_mpeg2vob_muxer = {
    .name              = "vob",
...
MPEGENC_CLASS(svcd)
AVEMOutputFormat ff_mpeg2svcd_muxer = {
    .name              = "svcd",
...
MPEGENC_CLASS(dvd)
AVEMOutputFormat ff_mpeg2dvd_muxer = {
    .name              = "dvd",
