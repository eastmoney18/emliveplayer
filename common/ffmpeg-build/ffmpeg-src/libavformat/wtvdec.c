...
typedef struct WtvFile {
    AVEMIOContext *pb_filesystem;  /**< file system (AVEMFormatContext->pb) */
...
static int64_t seek_by_sector(AVEMIOContext *pb, int64_t sector, int64_t offset)
{
    return avio_em_seek(pb, (sector << WTV_SECTOR_BITS) + offset, SEEK_SET);
}
...
{
    WtvFile *wf = opaque;
    AVEMIOContext *pb = wf->pb_filesystem;
    int nread = 0;

    if (wf->error || pb->error)
        return -1;
    if (wf->position >= wf->length || avio_em_feof(pb))
        return 0;
    ...
    while(nread < buf_size) {
        ...
        n = avio_em_read(pb, buf, read_request);
        if (n <= 0)
            break;
        nread += n;
        buf += n;
 ...
 */
static int read_ints(AVEMIOContext *pb, uint32_t *data, int count)
{
    int i, total = 0;
    for (i = 0; i < count; i++) {
        if ((data[total] = avio_em_rl32(pb)))
           total++;
    }
    return total;
}
 ...
 */
static AVEMIOContext * wtvfile_open_sector(int first_sector, uint64_t length, int depth, AVEMFormatContext *s)
{
    AVEMIOContext *pb;
    WtvFile *wf;
    uint8_t *buffer;
    ...
    wf = av_em_mallocz(sizeof(WtvFile));
    if (!wf)
        return NULL;

    if (depth == 0) {
        wf->sectors = av_em_alloc(sizeof(uint32_t));
        if (!wf->sectors) {
            av_em_free(wf);
            return NULL;
        }
        ...
        wf->sectors = av_em_alloc(WTV_SECTOR_SIZE);
        if (!wf->sectors) {
            av_em_free(wf);
            return NULL;
        }
        ...
        wf->sectors = av_em_malloc_array(nb_sectors1, 1 << WTV_SECTOR_BITS);
        if (!wf->sectors) {
            av_em_free(wf);
            return NULL;
        }
        ...
        av_em_log(s, AV_LOG_ERROR, "unsupported file allocation table depth (0x%x)\n", depth);
        av_em_free(wf);
        return NULL;
    }
    ...
    if (!wf->nb_sectors) {
        av_em_freep(&wf->sectors);
        av_em_freep(&wf);
        return NULL;
    }

    size = avio_em_size(s->pb);
    if (size >= 0 && (int64_t)wf->sectors[wf->nb_sectors - 1] << WTV_SECTOR_BITS > size)
        av_em_log(s, AV_LOG_WARNING, "truncated file\n");

    /* check length */
    length &= 0xFFFFFFFFFFFF;
    if (length > ((int64_t)wf->nb_sectors << wf->sector_bits)) {
        av_em_log(s, AV_LOG_WARNING, "reported file length (0x%"PRIx64") exceeds number of available sectors (0x%"PRIx64")\n", length, (int64_t)wf->nb_sectors << wf->sector_bits);
        length = (int64_t)wf->nb_sectors <<  wf->sector_bits;
    }
    wf->length = length;
    ...
    buffer = av_em_alloc(1 << wf->sector_bits);
    if (!buffer) {
        av_em_freep(&wf->sectors);
        av_em_freep(&wf);
        return NULL;
    ...
    pb = avio_em_alloc_context(buffer, 1 << wf->sector_bits, 0, wf,
 ...
 * @return NULL on error
 */
static AVEMIOContext * wtvfile_open2(AVEMFormatContext *s, const uint8_t *buf, int buf_size, const uint8_t *filename, int filename_size)
{
    ...
    while(buf + 48 <= buf_end) {
        ...
        if (ff_guidcmp(buf, ff_dir_entry_guid)) {
            av_em_log(s, AV_LOG_ERROR, "unknown guid "FF_PRI_GUID", expected dir_entry_guid; "
        ...
        if (48 + (int64_t)name_size > buf_end - buf) {
            av_em_log(s, AV_LOG_ERROR, "filename exceeds buffer size; remaining directory entries ignored\n");
            break;
        }
 ...
 */
static void wtvfile_close(AVEMIOContext *pb)
{
    WtvFile *wf = pb->opaque;
    av_em_freep(&wf->sectors);
    av_em_freep(&pb->opaque);
...
typedef struct WtvContext {
    AVEMIOContext *pb;       /**< timeline file */
    int64_t epoch;
    int64_t pts;             /**< pts for next data chunk */
    int64_t last_valid_pts;  /**< latest valid pts, used for interactive seeking */

    /* maintain private seek index, as the AVEMIndexEntry->pos is relative to the
       start of the 'timeline' file, not the file system (AVEMFormatContext->pb) */
    AVEMIndexEntry *index_entries;
...
static void get_attachment(AVEMFormatContext *s, AVEMIOContext *pb, int length)
{
    char mime[1024];
    ...
    avio_em_get_str16le(pb, INT_MAX, mime, sizeof(mime));
    if (strcmp(mime, "image/jpeg"))
        goto done;

    avio_em_r8(pb);
    avio_em_get_str16le(pb, INT_MAX, description, sizeof(description));
    filesize = avio_em_rl32(pb);
    if (!filesize)
        goto done;
    ...
    if (!st)
        goto done;
    av_em_dict_set(&st->metadata, "title", description, 0);
    st->codecpar->codec_type = AVMEDIA_TYPE_VIDEO;
    st->codecpar->codec_id   = AV_CODEC_ID_MJPEG;
    st->id = -1;
    ret = av_em_get_packet(pb, &st->attached_pic, filesize);
    if (ret < 0)
        goto done;
    ...
    avio_em_seek(pb, pos + length, SEEK_SET);
}

static void get_tag(AVEMFormatContext *s, AVEMIOContext *pb, const char *key, int type, int length)
{
    int buf_size;
    char *buf;

    if (!strcmp(key, "WM/MediaThumbType")) {
        avio_em_skip(pb, length);
        return;
    }

    buf_size = FFMAX(2*length, LEN_PRETTY_GUID) + 1;
    buf = av_em_alloc(buf_size);
    if (!buf)
        return;

    if (type == 0 && length == 4) {
        snprintf(buf, buf_size, "%u", avio_em_rl32(pb));
    } else if (type == 1) {
        avio_em_get_str16le(pb, length, buf, buf_size);
        if (!strlen(buf)) {
           av_em_free(buf);
           return;
        }
    } else if (type == 3 && length == 4) {
        strcpy(buf, avio_em_rl32(pb) ? "true" : "false");
    } else if (type == 4 && length == 8) {
        int64_t num = avio_em_rl64(pb);
        ...
        snprintf(buf, buf_size, "%u", avio_em_rl16(pb));
    } else if (type == 6 && length == 16) {
        ff_asf_guid guid;
        avio_em_read(pb, guid, 16);
        ...
        av_em_freep(&buf);
        av_em_log(s, AV_LOG_WARNING, "unsupported metadata entry; key:%s, type:%d, length:0x%x\n", key, type, length);
        avio_em_skip(pb, length);
        return;
    }

    av_em_dict_set(&s->metadata, key, buf, 0);
    av_em_freep(&buf);
}
 ...
 */
static void parse_legacy_attrib(AVEMFormatContext *s, AVEMIOContext *pb)
{
    ff_asf_guid guid;
    int length, type;
    while(!avio_em_feof(pb)) {
        char key[1024];
        ff_get_guid(pb, &guid);
        type   = avio_em_rl32(pb);
        length = avio_em_rl32(pb);
        if (!length)
            break;
        if (ff_guidcmp(&guid, ff_metadata_guid)) {
            av_em_log(s, AV_LOG_WARNING, "unknown guid "FF_PRI_GUID", expected metadata_guid; "
                   "remaining metadata entries ignored\n", FF_ARG_GUID(guid));
            break;
        }
        avio_em_get_str16le(pb, INT_MAX, key, sizeof(key));
 ...
 */
static int parse_videoinfoheader2(AVEMFormatContext *s, AVEMStream *st)
{
    WtvContext *wtv = s->priv_data;
    AVEMIOContext *pb = wtv->pb;

    avio_em_skip(pb, 72);  // picture aspect ratio is unreliable
 ...
 */
static void parse_mpeg1waveformatex(AVEMStream *st)
{
    /* fwHeadLayer */
 ...
 * @return NULL on error
 */
static AVEMStream * new_stream(AVEMFormatContext *s, AVEMStream *st, int sid, int codec_type)
{
    if (st) {
        ...
        WtvStream *wst = av_em_mallocz(sizeof(WtvStream));
        if (!wst)
            return NULL;
        st = avformat_em_new_stream(s, NULL);
        if (!st) {
 ...
 * @return NULL on error
 */
static AVEMStream * parse_media_type(AVEMFormatContext *s, AVEMStream *st, int sid,
                                   ...
                                   ff_asf_guid formattype, uint64_t size)
{
    ...
    if (!ff_guidcmp(subtype, ff_mediasubtype_cpfilters_processed) &&
        ...
        if (size < 32) {
            av_em_log(s, AV_LOG_WARNING, "format buffer size underflow\n");
            avio_em_skip(pb, size);
            return NULL;
        }

        avio_em_skip(pb, size - 32);
        ff_get_guid(pb, &actual_subtype);
        ff_get_guid(pb, &actual_formattype);
        avio_em_seek(pb, -size, SEEK_CUR);
        ...
        if (!ff_guidcmp(formattype, ff_format_waveformatex)) {
            ...
            if (ff_guidcmp(formattype, ff_format_none))
                av_em_log(s, AV_LOG_WARNING, "unknown formattype:"FF_PRI_GUID"\n", FF_ARG_GUID(formattype));
            avio_em_skip(pb, size);
        }

        if (!memcmp(subtype + 4, (const uint8_t[]){FF_MEDIASUBTYPE_BASE_GUID}, 12)) {
            st->codecpar->codec_id = ff_wav_em_codec_get_id(AV_RL32(subtype), st->codecpar->bits_per_coded_sample);
            ...
            if (st->codecpar->extradata && st->codecpar->extradata_size >= 22)
                ...
                av_em_log(s, AV_LOG_WARNING, "MPEG1WAVEFORMATEX underflow\n");
        } else {
            st->codecpar->codec_id = em_codec_guid_get_id(em_codec_wav_guids, subtype);
            if (st->codecpar->codec_id == AV_CODEC_ID_NONE)
                av_em_log(s, AV_LOG_WARNING, "unknown subtype:"FF_PRI_GUID"\n", FF_ARG_GUID(subtype));
        }
        return st;
        ...
        if (!ff_guidcmp(formattype, ff_format_videoinfo2)) {
            ...
            avio_em_skip(pb, FFMAX(size - consumed, 0));
        ...
        if (!memcmp(subtype + 4, (const uint8_t[]){FF_MEDIASUBTYPE_BASE_GUID}, 12)) {
            st->codecpar->codec_id = em_codec_get_id(em_codec_bmp_tags, AV_RL32(subtype));
        } else {
            st->codecpar->codec_id = em_codec_guid_get_id(ff_video_guids, subtype);
        }
        if (st->codecpar->codec_id == AV_CODEC_ID_NONE)
            av_em_log(s, AV_LOG_WARNING, "unknown subtype:"FF_PRI_GUID"\n", FF_ARG_GUID(subtype));
        return st;
    ...
    av_em_log(s, AV_LOG_WARNING, "unknown media type, mediatype:"FF_PRI_GUID
...
static int recover(WtvContext *wtv, uint64_t broken_pos)
{
    ...
    for (i = 0; i < wtv->nb_index_entries; i++) {
        if (wtv->index_entries[i].pos > broken_pos) {
            int64_t ret = avio_em_seek(pb, wtv->index_entries[i].pos, SEEK_SET);
            if (ret < 0)
                return ret;
 ...
 */
static int parse_chunks(AVEMFormatContext *s, int mode, int64_t seekts, int *len_ptr)
{
    WtvContext *wtv = s->priv_data;
    AVEMIOContext *pb = wtv->pb;
    while (!avio_em_feof(pb)) {
        ...
        if (len < 32) {
            ...
            if (avio_em_feof(pb))
                return AVERROR_EOF;
            av_em_log(s, AV_LOG_WARNING, "encountered broken chunk\n");
            if ((ret = recover(wtv, avio_em_tell(pb) - 20)) < 0)
                return ret;
            continue;
        }
        sid = avio_em_rl32(pb) & 0x7FFF;
        avio_em_skip(pb, 8);
        consumed = 32;

        if (!ff_guidcmp(g, ff_SBE2_STREAM_DESC_EVENT)) {
            if (em_find_stream_index(s, sid) < 0) {
                ...
                size = avio_em_rl32(pb);
                parse_media_type(s, 0, sid, mediatype, subtype, formattype, size);
                consumed += 92 + size;
            }
        } else if (!ff_guidcmp(g, ff_stream2_guid)) {
            int stream_index = em_find_stream_index(s, sid);
            ...
            if (stream_index >= 0) {
                avio_em_skip(pb, 12);
                if (avio_em_rl32(pb))
                    av_em_log(s, AV_LOG_WARNING, "DVB scrambled stream detected (st:%d), decoding will likely fail\n", stream_index);
                consumed += 16;
            }
            ...
            if (stream_index >= 0) {
                ...
                if (mode == SEEK_TO_PTS && wtv->pts >= seekts) {
                    avio_em_skip(pb, WTV_PAD8(len) - consumed);
                    return 0;
                }
                }
            }
            ...
            int stream_index = em_find_stream_index(s, sid);
            if (stream_index >= 0)
                av_em_log(s, AV_LOG_WARNING, "encrypted stream detected (st:%d), decoding will likely fail\n", stream_index);
        } else if (
            ...
            av_em_log(s, AV_LOG_WARNING, "unsupported chunk:"FF_PRI_GUID"\n", FF_ARG_GUID(g));
...
static int read_header(AVEMFormatContext *s)
{
    ...
    avio_em_skip(s->pb, 0x30);
    root_size = avio_em_rl32(s->pb);
    if (root_size > sizeof(root)) {
        av_em_log(s, AV_LOG_ERROR, "root directory size exceeds sector size\n");
        return AVERROR_INVALIDDATA;
    }
    avio_em_skip(s->pb, 4);
    root_sector = avio_em_rl32(s->pb);

    ret = seek_by_sector(s->pb, root_sector, 0);
    if (ret < 0)
        return ret;
    root_size = avio_em_read(s->pb, root, root_size);
    if (root_size < 0)
        return AVERROR_INVALIDDATA;
    ...
    if (!wtv->pb) {
        av_em_log(s, AV_LOG_ERROR, "timeline data missing\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (ret < 0)
        return ret;
    avio_em_seek(wtv->pb, -32, SEEK_CUR);

    timeline_pos = avio_em_tell(s->pb); // save before opening another file
    ...
    if (s->nb_streams) {
        ...
        if (pb) {
            while(1) {
                uint64_t timestamp = avio_em_rl64(pb);
                uint64_t frame_nb  = avio_em_rl64(pb);
                if (avio_em_feof(pb))
                    break;
                em_add_index_entry(&wtv->index_entries, &wtv->nb_index_entries, &wtv->index_entries_allocated_size,
            ...
            if (wtv->nb_index_entries) {
                ...
                if (pb) {
                    AVEMIndexEntry *e = wtv->index_entries;
                    AVEMIndexEntry *e_end = wtv->index_entries + wtv->nb_index_entries - 1;
                    uint64_t last_position = 0;
                    while (1) {
                        uint64_t frame_nb = avio_em_rl64(pb);
                        uint64_t position = avio_em_rl64(pb);
    ...
    avio_em_seek(s->pb, timeline_pos, SEEK_SET);
    return 0;
}
...
static int read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    ret = av_em_get_packet(pb, pkt, len - 32);
    if (ret < 0)
        return ret;
    pkt->stream_index = stream_index;
    pkt->pts          = wtv->pts;
    avio_em_skip(pb, WTV_PAD8(len) - len);
    return 0;
}
                     ...
                     int64_t ts, int flags)
{
     ...
     * whereas AVEMIndexEntry->timestamp values are relative to epoch. */
    ts_relative = ts;
    if (wtv->epoch != AV_NOPTS_VALUE)
        ts_relative -= wtv->epoch;

    i = em_index_search_timestamp(wtv->index_entries, wtv->nb_index_entries, ts_relative, flags);
    if (i < 0) {
        if (wtv->last_valid_pts == AV_NOPTS_VALUE || ts < wtv->last_valid_pts) {
            if (avio_em_seek(pb, 0, SEEK_SET) < 0)
                return -1;
        } else if (st->duration != AV_NOPTS_VALUE && ts_relative > st->duration && wtv->nb_index_entries) {
            if (avio_em_seek(pb, wtv->index_entries[wtv->nb_index_entries - 1].pos, SEEK_SET) < 0)
                return -1;
        }
    ...
    if (avio_em_seek(pb, wtv->index_entries[i].pos, SEEK_SET) < 0)
        return -1;
    wtv->pts = wtv->index_entries[i].timestamp;
    if (wtv->epoch != AV_NOPTS_VALUE)
        wtv->pts += wtv->epoch;
...
static int read_close(AVEMFormatContext *s)
{
    WtvContext *wtv = s->priv_data;
    av_em_freep(&wtv->index_entries);
    wtvfile_close(wtv->pb);
    return 0;
}

AVEMInputFormat ff_wtv_demuxer = {
    .name           = "wtv",
