...
static int concat_probe(AVEMProbeData *probe)
{
...
static int add_file(AVEMFormatContext *avf, char *filename, ConcatFile **rfile,
                    unsigned *nb_files_alloc)
{
    ...
    if (cat->safe > 0 && !safe_filename(filename)) {
        av_em_log(avf, AV_LOG_ERROR, "Unsafe file name '%s'\n", filename);
        FAIL(AVERROR(EPERM));
    }

    proto = avio_em_find_protocol_name(filename);
    ...
    if (!memcmp(filename, proto, proto_len) &&
        ...
        if (!(url = av_em_alloc(url_len)))
            FAIL(AVERROR(ENOMEM));
        em_make_absolute_url(url, url_len, avf->filename, filename);
    ...
    if (cat->nb_files >= *nb_files_alloc) {
        ...
        if (n <= cat->nb_files || n > SIZE_MAX / sizeof(*cat->files) ||
            !(new_files = av_em_realloc(cat->files, n * sizeof(*cat->files))))
            FAIL(AVERROR(ENOMEM));
        cat->files = new_files;
        *nb_files_alloc = n;
    }
...
static int detect_stream_specific(AVEMFormatContext *avf, int idx)
{
    ConcatContext *cat = avf->priv_data;
    AVEMStream *st = cat->avf->streams[idx];
    ...
    if (cat->auto_convert && st->codecpar->codec_id == AV_CODEC_ID_H264 &&
        ...
        av_em_log(cat->avf, AV_LOG_INFO,
               "Auto-inserting h264_mp4toannexb bitstream filter\n");
        if (!(bsf = av_em_bitstream_filter_init("h264_mp4toannexb"))) {
            av_em_log(avf, AV_LOG_ERROR, "h264_mp4toannexb bitstream filter "
                   "required for H.264 streams\n");
            return AVERROR_BSF_NOT_FOUND;
        }
        cs->bsf = bsf;

        cs->avctx = avcodec_em_alloc_context3(NULL);
        if (!cs->avctx)
        ...
        ret = avcodec_em_parameters_to_context(cs->avctx, st->codecpar);
        if (ret < 0) {
            avcodec_em_free_context(&cs->avctx);
            return ret;
        }
...
static int match_streams_one_to_one(AVEMFormatContext *avf)
{
    ConcatContext *cat = avf->priv_data;
    AVEMStream *st;
    int i, ret;
    ...
    for (i = cat->cur_file->nb_streams; i < cat->avf->nb_streams; i++) {
        if (i < avf->nb_streams) {
            ...
            if (!(st = avformat_em_new_stream(avf, NULL)))
                return AVERROR(ENOMEM);
        }
...
static int match_streams_exact_id(AVEMFormatContext *avf)
{
    ConcatContext *cat = avf->priv_data;
    AVEMStream *st;
    int i, j, ret;
...
static int match_streams(AVEMFormatContext *avf)
{
    ConcatContext *cat = avf->priv_data;
    ConcatStream *map;
    int i, ret;
    ...
    if (cat->cur_file->nb_streams >= cat->avf->nb_streams)
        return 0;
    map = av_em_realloc(cat->cur_file->streams,
...
static int open_file(AVEMFormatContext *avf, unsigned fileno)
{
    ConcatContext *cat = avf->priv_data;
    ...
    if ((ret = em_copy_whiteblacklists(new_avf, avf)) < 0)
        return ret;

    if (cat->options)
        av_em_dict_copy(&tmp, cat->options, 0);

    ret = avformat_em_open_input(&new_avf, file->url, NULL, &tmp);
    av_em_dict_free(&tmp);
    if (ret < 0 ||
        (ret = avformat_em_find_stream_info(new_avf, NULL)) < 0) {
        av_em_log(avf, AV_LOG_ERROR, "Impossible to open '%s'\n", file->url);
    ...
    if (cat->avf)
        avformat_em_close_input(&cat->avf);
    ...
    if (cat->segment_time_metadata) {
        av_em_dict_set_int(&file->metadata, "lavf.concatdec.start_time", file->start_time, 0);
        if (file->duration != AV_NOPTS_VALUE)
            av_em_dict_set_int(&file->metadata, "lavf.concatdec.duration", file->duration, 0);
    }

    if ((ret = match_streams(avf)) < 0)
        return ret;
    if (file->inpoint != AV_NOPTS_VALUE) {
       if ((ret = avformat_em_seek_file(cat->avf, -1, INT64_MIN, file->inpoint, file->inpoint, 0)) < 0)
           return ret;
    }
    return 0;
}

static int concat_read_close(AVEMFormatContext *avf)
{
    ConcatContext *cat = avf->priv_data;
    unsigned i, j;

    for (i = 0; i < cat->nb_files; i++) {
        av_em_freep(&cat->files[i].url);
        for (j = 0; j < cat->files[i].nb_streams; j++) {
            if (cat->files[i].streams[j].avctx)
                avcodec_em_free_context(&cat->files[i].streams[j].avctx);
            if (cat->files[i].streams[j].bsf)
                av_em_bitstream_filter_close(cat->files[i].streams[j].bsf);
        }
        av_em_freep(&cat->files[i].streams);
        av_em_dict_free(&cat->files[i].metadata);
    }
    if (cat->avf)
        avformat_em_close_input(&cat->avf);
    av_em_dict_free(&cat->options);
    av_em_freep(&cat->files);
    return 0;
}

static int concat_read_header(AVEMFormatContext *avf, AVEMDictionary **options)
{
    ConcatContext *cat = avf->priv_data;
    uint8_t buf[4096];
    ...
    if (options && *options)
        av_em_dict_copy(&cat->options, *options, 0);

    while (1) {
        if ((ret = em_get_line(avf->pb, buf, sizeof(buf))) <= 0)
            break;
        line++;
        cursor = buf;
        ...
        if (!strcmp(keyword, "file")) {
            char *filename = av_em_get_token((const char **)&cursor, SPACE_CHARS);
            if (!filename) {
                av_em_log(avf, AV_LOG_ERROR, "Line %d: filename required\n", line);
            ...
            int64_t dur;
            if (!file) {
                av_em_log(avf, AV_LOG_ERROR, "Line %d: %s without file\n",
                       line, keyword);
                FAIL(AVERROR_INVALIDDATA);
            }
            if ((ret = av_em_parse_time(&dur, dur_str, 1)) < 0) {
                av_em_log(avf, AV_LOG_ERROR, "Line %d: invalid %s '%s'\n",
            ...
            metadata = av_em_get_token((const char **)&cursor, SPACE_CHARS);
            if (!metadata) {
                av_em_log(avf, AV_LOG_ERROR, "Line %d: packet metadata required\n", line);
                FAIL(AVERROR_INVALIDDATA);
            }
            if ((ret = av_em_dict_parse_string(&file->metadata, metadata, "=", "", 0)) < 0) {
                av_em_log(avf, AV_LOG_ERROR, "Line %d: failed to parse metadata string\n", line);
                av_em_freep(&metadata);
                FAIL(AVERROR_INVALIDDATA);
            }
            av_em_freep(&metadata);
        } else if (!strcmp(keyword, "stream")) {
            if (!avformat_em_new_stream(avf, NULL))
                FAIL(AVERROR(ENOMEM));
        } else if (!strcmp(keyword, "exact_stream_id")) {
            if (!avf->nb_streams) {
                av_em_log(avf, AV_LOG_ERROR, "Line %d: exact_stream_id without stream\n",
                       line);
                FAIL(AVERROR_INVALIDDATA);
            }
            ...
            if (strcmp(ver_kw, "version") || strcmp(ver_val, "1.0")) {
                av_em_log(avf, AV_LOG_ERROR, "Line %d: invalid version\n", line);
                FAIL(AVERROR_INVALIDDATA);
            }
            if (cat->safe < 0)
                cat->safe = 1;
        } else {
            av_em_log(avf, AV_LOG_ERROR, "Line %d: unknown keyword '%s'\n",
                   line, keyword);
            FAIL(AVERROR_INVALIDDATA);
        }
    }
...
static int open_next_file(AVEMFormatContext *avf)
{
    ConcatContext *cat = avf->priv_data;
...
static int filter_packet(AVEMFormatContext *avf, ConcatStream *cs, AVEMPacket *pkt)
{
    AVEMStream *st = avf->streams[cs->out_stream_index];
    AVBitStreamFilterContext *bsf;
    AVEMPacket pkt2;
    int ret;
    ...
    for (bsf = cs->bsf; bsf; bsf = bsf->next) {
        ...
        ret = av_em_bitstream_filter_filter(bsf, cs->avctx, NULL,
                                         &pkt2.data, &pkt2.size,
                                         pkt->data, pkt->size,
        ...
        if (cs->avctx->extradata_size > st->codecpar->extradata_size) {
            ...
            eret = em_alloc_extradata(st->codecpar, cs->avctx->extradata_size);
            if (eret < 0) {
                av_em_packet_unref(pkt);
                return AVERROR(ENOMEM);
            }
        ...
        if (ret == 0 && pkt2.data != pkt->data) {
            if ((ret = av_em_copy_packet(&pkt2, pkt)) < 0) {
                av_em_free(pkt2.data);
                return ret;
            }
            ret = 1;
        }
        if (ret > 0) {
            av_em_packet_unref(pkt);
            pkt2.buf = av_em_buffer_create(pkt2.data, pkt2.size,
...
static int packet_after_outpoint(ConcatContext *cat, AVEMPacket *pkt)
{
    if (cat->cur_file->outpoint != AV_NOPTS_VALUE && pkt->dts != AV_NOPTS_VALUE) {
        return av_em_compare_ts(pkt->dts, cat->avf->streams[pkt->stream_index]->time_base,
...
static int concat_read_packet(AVEMFormatContext *avf, AVEMPacket *pkt)
{
    ConcatContext *cat = avf->priv_data;
    int ret;
    int64_t delta;
    ...
    while (1) {
        ret = av_em_read_frame(cat->avf, pkt);
        if (ret == AVERROR_EOF) {
            is_new_st = 1;
...
open_fail:
        ...
        av_em_log(avf, AV_LOG_WARNING, "open_next_file() failed (%d)\n", try_counter);
    }
    if ((ret = filter_packet(avf, cs, pkt)))
        return ret;

    st = cat->avf->streams[pkt->stream_index];
    av_em_log(avf, AV_LOG_DEBUG, "file:%d stream:%d pts:%s pts_time:%s dts:%s dts_time:%s",
           (unsigned)(cat->cur_file - cat->files), pkt->stream_index,
           av_ts2str(pkt->pts), av_ts2timestr(pkt->pts, &st->time_base),
           av_ts2str(pkt->dts), av_ts2timestr(pkt->dts, &st->time_base));

    delta = av_em_rescale_q(cat->cur_file->start_time - cat->cur_file->file_inpoint,
                         AV_TIME_BASE_Q,
    ...
    if (pkt->dts != AV_NOPTS_VALUE)
        pkt->dts += delta;
    av_em_log(avf, AV_LOG_DEBUG, " -> pts:%s pts_time:%s dts:%s dts_time:%s\n",
           av_ts2str(pkt->pts), av_ts2timestr(pkt->pts, &st->time_base),
           av_ts2str(pkt->dts), av_ts2timestr(pkt->dts, &st->time_base));
    if (cat->cur_file->metadata) {
        uint8_t* metadata;
        int metadata_len;
        char* packed_metadata = av_em_packet_pack_dictionary(cat->cur_file->metadata, &metadata_len);
        if (!packed_metadata)
            return AVERROR(ENOMEM);
        if (!(metadata = av_em_packet_new_side_data(pkt, AV_PKT_DATA_STRINGS_METADATA, metadata_len))) {
            av_em_freep(&packed_metadata);
            return AVERROR(ENOMEM);
        }
...
static void rescale_interval(AVEMRational tb_in, AVEMRational tb_out,
                             int64_t *min_ts, int64_t *ts, int64_t *max_ts)
{
    *ts     = av_em_rescale_q    (*    ts, tb_in, tb_out);
    *min_ts = av_em_rescale_q_rnd(*min_ts, tb_in, tb_out,
                               AV_ROUND_UP   | AV_ROUND_PASS_MINMAX);
    *max_ts = av_em_rescale_q_rnd(*max_ts, tb_in, tb_out,
                               AV_ROUND_DOWN | AV_ROUND_PASS_MINMAX);
}

static int try_seek(AVEMFormatContext *avf, int stream,
                    int64_t min_ts, int64_t ts, int64_t max_ts, int flags)
{
    ...
    return avformat_em_seek_file(cat->avf, stream, min_ts, ts, max_ts, flags);
}

static int real_seek(AVEMFormatContext *avf, int stream,
                     int64_t min_ts, int64_t ts, int64_t max_ts, int flags)
{
    ConcatContext *cat = avf->priv_data;
    int ret, left, right;
...
static int concat_seek(AVEMFormatContext *avf, int stream,
                       int64_t min_ts, int64_t ts, int64_t max_ts, int flags)
{
    ConcatContext *cat = avf->priv_data;
    ConcatFile *cur_file_saved = cat->cur_file;
    AVEMFormatContext *cur_avf_saved = cat->avf;
    int ret;
    ...
    if ((ret = real_seek(avf, stream, min_ts, ts, max_ts, flags)) < 0) {
        ...
        avformat_em_close_input(&cur_avf_saved);
        cat->eof = 0;
    }
    return ret;
}
...
static const AVEMClass concat_class = {
...
AVEMInputFormat em_concat_demuxer = {
    .name           = "concat",
