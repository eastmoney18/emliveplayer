...
static int subviewer_probe(AVEMProbeData *p)
{
    char c;
...
static int subviewer_read_header(AVEMFormatContext *s)
{
...
static int subviewer_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    SubViewerContext *subviewer = s->priv_data;
    return ff_subtitles_queue_read_packet(&subviewer->q, pkt);
}

static int subviewer_read_seek(AVEMFormatContext *s, int stream_index,
...
static int subviewer_read_close(AVEMFormatContext *s)
{
    SubViewerContext *subviewer = s->priv_data;
...
AVEMInputFormat ff_subviewer_demuxer = {
    .name           = "subviewer",
