...
static int msnwc_tcp_probe(AVEMProbeData *p)
{
    int i;
...
static int msnwc_tcp_read_header(AVEMFormatContext *ctx)
{
    ...
    while(avio_em_r8(pb) != HEADER_SIZE && !avio_em_feof(pb)) ;

    if(avio_em_feof(pb)) {
        av_em_log(ctx, AV_LOG_ERROR, "Could not find valid start.\n");
...
static int msnwc_tcp_read_packet(AVEMFormatContext *ctx, AVEMPacket *pkt)
{
    AVEMIOContext *pb = ctx->pb;
    uint16_t keyframe;
    ...
    avio_em_skip(pb, 1); /* one byte has been read ahead */
    avio_em_skip(pb, 2);
    avio_em_skip(pb, 2);
    keyframe = avio_em_rl16(pb);
    size     = avio_em_rl32(pb);
    avio_em_skip(pb, 4);
    avio_em_skip(pb, 4);
    timestamp = avio_em_rl32(pb);
    ...
    avio_em_skip(pb, 1); /* Read ahead one byte of struct size like read_header */
...
AVEMInputFormat ff_msnwc_tcp_demuxer = {
    .name        = "msnwctcp",
