...
typedef struct RTMP_HTTPContext {
    ...
    EMURLContext   *stream;           ///< HTTP stream
...
static int rtmp_http_send_cmd(EMURLContext *h, const char *cmd)
{
    ...
    em_url_join(uri, sizeof(uri), "http", NULL, rt->host, rt->port,
                "/%s/%s/%d", cmd, rt->client_id, rt->seq++);

    av_em_opt_set_bin(rt->stream->priv_data, "post_data", rt->out_data,
                   rt->out_size, 0);

    /* send a new request to the server */
    if ((ret = em_http_do_new_request(rt->stream, uri)) < 0)
        return ret;
    /* re-init output buffer */
    rt->out_size = 0;

    /* read the first byte which contains the polling interval */
    if ((ret = ffurl_em_read(rt->stream, &c, 1)) < 0)
        return ret;
...
static int rtmp_http_write(EMURLContext *h, const uint8_t *buf, int size)
{
    RTMP_HTTPContext *rt = h->priv_data;
    ...
    if (rt->out_size + size > rt->out_capacity) {
        ...
        if ((err = av_em_reallocp(&rt->out_data, rt->out_capacity)) < 0) {
            rt->out_size = 0;
            rt->out_capacity = 0;
            return err;
        }
...
static int rtmp_http_read(EMURLContext *h, uint8_t *buf, int size)
{
    RTMP_HTTPContext *rt = h->priv_data;
    int ret, off = 0;
    ...
    do {
        ret = ffurl_em_read(rt->stream, buf + off, size);
...
static int rtmp_http_close(EMURLContext *h)
{
    RTMP_HTTPContext *rt = h->priv_data;
    ...
    av_em_freep(&rt->out_data);
    ffurl_em_close(rt->stream);

    return ret;
}

static int rtmp_http_open(EMURLContext *h, const char *uri, int flags)
{
    RTMP_HTTPContext *rt = h->priv_data;
    char headers[1024], url[1024];
    int ret, off = 0;

    av_em_url_split(NULL, 0, NULL, 0, rt->host, sizeof(rt->host), &rt->port,
                 NULL, 0, uri);
    ...
    if (rt->tls) {
        ...
        em_url_join(url, sizeof(url), "https", NULL, rt->host, rt->port, "/open/1");
    } else {
        if (rt->port < 0)
            rt->port = RTMPT_DEFAULT_PORT;
        em_url_join(url, sizeof(url), "http", NULL, rt->host, rt->port, "/open/1");
    }

    /* alloc the http context */
    if ((ret = ffurl_em_alloc(&rt->stream, url, AVIO_FLAG_READ_WRITE, NULL)) < 0)
        goto fail;
    ...
    av_em_opt_set(rt->stream->priv_data, "headers", headers, 0);
    av_em_opt_set(rt->stream->priv_data, "multiple_requests", "1", 0);
    av_em_opt_set_bin(rt->stream->priv_data, "post_data", "", 1, 0);

    /* open the http context */
    if ((ret = ffurl_em_connect(rt->stream, NULL)) < 0)
        goto fail;

    /* read the server reply which contains a unique ID */
    for (;;) {
        ret = ffurl_em_read(rt->stream, rt->client_id + off, sizeof(rt->client_id) - off);
        if (!ret || ret == AVERROR_EOF)
            break;
        if (ret < 0)
            goto fail;
...
static const AVEMClass ffrtmphttp_class = {
...
const EMURLProtocol em_ffrtmphttp_protocol = {
    .name           = "ffrtmphttp",
