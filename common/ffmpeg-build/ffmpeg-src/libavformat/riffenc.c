...
void ff_end_tag(AVEMIOContext *pb, int64_t start)
{
    ...
    if (pos & 1)
        avio_em_w8(pb, 0);
    avio_em_seek(pb, start - 4, SEEK_SET);
    avio_em_wl32(pb, (uint32_t)(pos - start));
    avio_em_seek(pb, FFALIGN(pos, 2), SEEK_SET);
}

/* WAVEFORMATEX header */
/* returns the size or -1 on error */
int ff_put_wav_header(AVEMFormatContext *s, AVEMIOContext *pb,
                      AVEMCodecParameters *par, int flags)
{
    int bps, blkalign, bytespersec, frame_size;
    int hdrsize;
    int64_t hdrstart = avio_em_tell(pb);
     ...
     * fall back on using AVEMCodecContext.frame_size, which is not as reliable
     * for indicating packet duration. */
    frame_size = av_em_get_audio_frame_duration2(par, par->block_align);

    waveformatextensible = (par->channels > 2 && par->channel_layout) ||
                           par->sample_rate > 48000 ||
                           par->codec_id == AV_CODEC_ID_EAC3 ||
                           av_em_get_bits_per_sample(par->codec_id) > 16;

    if (waveformatextensible)
        avio_em_wl16(pb, 0xfffe);
    else
        avio_em_wl16(pb, par->codec_tag);

    avio_em_wl16(pb, par->channels);
    avio_em_wl32(pb, par->sample_rate);
    if (par->codec_id == AV_CODEC_ID_ATRAC3 ||
        ...
        if (!(bps = av_em_get_bits_per_sample(par->codec_id))) {
    ...
    if (par->codec_id == AV_CODEC_ID_MP2) {
        ...
        blkalign = bps * par->channels / av_em_gcd(8, bps);
    ...
    avio_em_wl32(pb, bytespersec); /* bytes per second */
    avio_em_wl16(pb, blkalign);    /* block align */
    avio_em_wl16(pb, bps);         /* bits per sample */
    ...
    if (waveformatextensible) {
        ...
        avio_em_wl16(pb, riff_extradata - riff_extradata_start + 22);
        /* ValidBitsPerSample || SamplesPerBlock || Reserved */
        avio_em_wl16(pb, bps);
        /* dwChannelMask */
        avio_em_wl32(pb, write_channel_mask ? par->channel_layout : 0);
        /* GUID + next 3 */
        if (par->codec_id == AV_CODEC_ID_EAC3) {
            ff_put_guid(pb, ff_get_codec_guid(par->codec_id, em_codec_wav_guids));
        } else {
        avio_em_wl32(pb, par->codec_tag);
        avio_em_wl32(pb, 0x00100000);
        avio_em_wl32(pb, 0xAA000080);
        avio_em_wl32(pb, 0x719B3800);
        }
    } else if ((flags & FF_PUT_WAV_HEADER_FORCE_WAVEFORMATEX) ||
               par->codec_tag != 0x0001 /* PCM */ ||
               riff_extradata - riff_extradata_start) {
        /* WAVEFORMATEX */
        avio_em_wl16(pb, riff_extradata - riff_extradata_start); /* cbSize */
    } /* else PCMWAVEFORMAT */
    avio_em_write(pb, riff_extradata_start, riff_extradata - riff_extradata_start);
    hdrsize = avio_em_tell(pb) - hdrstart;
    if (hdrsize & 1) {
        hdrsize++;
        avio_em_w8(pb, 0);
    }
...
void ff_put_bmp_header(AVEMIOContext *pb, AVEMCodecParameters *par,
                       const AVEMCodecTag *tags, int for_asf, int ignore_extradata)
{
    ...
    avio_em_wl32(pb, 40 + (ignore_extradata || pal_avi ? 0 : extradata_size));
    avio_em_wl32(pb, par->width);
    //We always store RGB TopDown
    avio_em_wl32(pb, par->codec_tag || keep_height ? par->height : -par->height);
    /* planes */
    avio_em_wl16(pb, 1);
    /* depth */
    avio_em_wl16(pb, par->bits_per_coded_sample ? par->bits_per_coded_sample : 24);
    /* compression type */
    avio_em_wl32(pb, par->codec_tag);
    avio_em_wl32(pb, (par->width * par->height * (par->bits_per_coded_sample ? par->bits_per_coded_sample : 24)+7) / 8);
    avio_em_wl32(pb, 0);
    avio_em_wl32(pb, 0);
    ...
    avio_em_wl32(pb, pal_avi ? 1 << par->bits_per_coded_sample : 0);
    avio_em_wl32(pb, 0);

    if (!ignore_extradata) {
        if (par->extradata_size) {
            avio_em_write(pb, par->extradata, extradata_size);
...
void ff_parse_specific_params(AVEMStream *st, int *au_rate,
                              int *au_ssize, int *au_scale)
{
    ...
    audio_frame_size = av_em_get_audio_frame_duration2(par, 0);
    if (!audio_frame_size)
    ...
    *au_ssize = par->block_align;
    ...
    gcd        = av_em_gcd(*au_scale, *au_rate);
    *au_scale /= gcd;
    *au_rate  /= gcd;
}
...
static int riem_has_valid_tags(AVEMFormatContext *s)
{
    int i;

    for (i = 0; *riff_tags[i]; i++)
        if (av_em_dict_get(s->metadata, riff_tags[i], NULL, AV_DICT_MATCH_CASE))
            return 1;
...
void em_riff_write_info(AVEMFormatContext *s)
{
    ...
    ff_em_metadata_conv(&s->metadata, em_riff_info_conv, NULL);

    /* writing empty LIST is not nice and may cause problems */
    if (!riem_has_valid_tags(s))
        return;

    list_pos = ff_start_tag(pb, "LIST");
    emio_wfourcc(pb, "INFO");
    for (i = 0; *riff_tags[i]; i++)
        if ((t = av_em_dict_get(s->metadata, riff_tags[i],
                             NULL, AV_DICT_MATCH_CASE)))
            em_riff_write_info_tag(s->pb, t->key, t->value);
