 ...
 *         input buffer.
 */
static int qdm2_parse_config(PayloadContext *qdm, AVEMStream *st,
    ...
    while (end - p >= 2) {
        ...
        switch (config_item) {
            ...
            case 4: /* stream with extradata */
                ...
                if (em_alloc_extradata(st->codecpar, 26 + item_len)) {
 ...
 *         input buffer.
 */
static int qdm2_parse_subpacket(PayloadContext *qdm, AVEMStream *st,
                                const uint8_t *buf, const uint8_t *end)
{
    const uint8_t *p = buf;
 ...
 */
static int qdm2_restore_block(PayloadContext *qdm, AVEMStream *st, AVEMPacket *pkt)
{
    ...
    if ((res = av_em_new_packet(pkt, qdm->block_size)) < 0)
        return res;
...
static int qdm2_parse_packet(AVEMFormatContext *s, PayloadContext *qdm,
