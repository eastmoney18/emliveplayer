...
static int qcp_probe(AVEMProbeData *pd)
{
...
static int qcp_read_header(AVEMFormatContext *s)
{
    ...
    avio_em_rb32(pb);                    // "RIFF"
    avio_em_skip(pb, 4 + 8 + 4 + 1 + 1);    // filesize + "QLCMfmt " + chunk-size + major-version + minor-version

    st->codecpar->codec_type = AVMEDIA_TYPE_AUDIO;
    st->codecpar->channels   = 1;
    st->codecpar->channel_layout = AV_CH_LAYOUT_MONO;
    avio_em_read(pb, buf, 16);
    if (is_qcelp_13k_guid(buf)) {
        ...
        av_em_log(s, AV_LOG_ERROR, "Unknown codec GUID "FF_PRI_GUID".\n",
               FF_ARG_GUID(buf));
        return AVERROR_INVALIDDATA;
    }
    avio_em_skip(pb, 2 + 80); // codec-version + codec-name
    st->codecpar->bit_rate = avio_em_rl16(pb);

    s->packet_size = avio_em_rl16(pb);
    avio_em_skip(pb, 2); // block-size
    st->codecpar->sample_rate = avio_em_rl16(pb);
    avio_em_skip(pb, 2); // sample-size

    memset(c->rates_per_mode, -1, sizeof(c->rates_per_mode));
    nb_rates = avio_em_rl32(pb);
    nb_rates = FFMIN(nb_rates, 8);
    for (i=0; i<nb_rates; i++) {
        int size = avio_em_r8(pb);
        int mode = avio_em_r8(pb);
        if (mode > QCP_MAX_MODE) {
            av_em_log(s, AV_LOG_WARNING, "Unknown entry %d=>%d in rate-map-table.\n ", mode, size);
        } else
            c->rates_per_mode[mode] = size;
    }
    avio_em_skip(pb, 16 - 2*nb_rates + 20); // empty entries of rate-map-table + reserved

    return 0;
}

static int qcp_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    AVEMIOContext *pb = s->pb;
    QCPContext    *c  = s->priv_data;
    ...
    while(!avio_em_feof(pb)) {
        if (c->data_size) {
            int pkt_size, ret, mode = avio_em_r8(pb);
            ...
            if (c->data_size <= pkt_size) {
                av_em_log(s, AV_LOG_WARNING, "Data chunk is too small.\n");
                pkt_size = c->data_size - 1;
            }

            if ((ret = av_em_get_packet(pb, pkt, pkt_size)) >= 0) {
                if (pkt_size != ret)
                    av_em_log(s, AV_LOG_ERROR, "Packet size is too small.\n");
        ...
        if (avio_em_tell(pb) & 1 && avio_em_r8(pb))
            av_em_log(s, AV_LOG_WARNING, "Padding should be 0.\n");

        tag        = avio_em_rl32(pb);
        chunk_size = avio_em_rl32(pb);
        switch (tag) {
        case MKTAG('v', 'r', 'a', 't'):
            if (avio_em_rl32(pb)) // var-rate-flag
                s->packet_size = 0;
            avio_em_skip(pb, 4); // size-in-packets
            break;
        case MKTAG('d', 'a', 't', 'a'):
...
AVEMInputFormat ff_qcp_demuxer = {
    .name           = "qcp",
