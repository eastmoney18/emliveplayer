...
static int probe_bfstm(AVEMProbeData *p)
{
...
static av_always_inline unsigned int read16(AVEMFormatContext *s)
{
    BRSTMDemuxContext *b = s->priv_data;
    if (b->little_endian)
        return avio_em_rl16(s->pb);
    else
        return avio_em_rb16(s->pb);
}

static av_always_inline unsigned int read32(AVEMFormatContext *s)
{
    BRSTMDemuxContext *b = s->priv_data;
    if (b->little_endian)
        return avio_em_rl32(s->pb);
    else
        return avio_em_rb32(s->pb);
}
...
static int read_header(AVEMFormatContext *s)
{
    ...
    bom = avio_em_rb16(s->pb);
    if (bom != 0xFEFF && bom != 0xFFFE) {
        av_em_log(s, AV_LOG_ERROR, "invalid byte order: %X\n", bom);
    ...
    if (!bfstm) {
        major = avio_em_r8(s->pb);
        minor = avio_em_r8(s->pb);
        avio_em_skip(s->pb, 4); // size of file
        size = read16(s);
        if (size < 14)
            return AVERROR_INVALIDDATA;

        avio_em_skip(s->pb, size - 14);
        pos = avio_em_tell(s->pb);
        if (avio_em_rl32(s->pb) != MKTAG('H','E','A','D'))
            return AVERROR_INVALIDDATA;
    } else {
        ...
        avio_em_skip(s->pb, 4); // Unknown constant 0x00030000
        avio_em_skip(s->pb, 4); // size of file
        section_count = read16(s);
        avio_em_skip(s->pb, 2); // padding
        for (i = 0; avio_em_tell(s->pb) < header_size
                    && !(start && info_offset)
                    && i < section_count; i++) {
            ...
            case 0x4000:
                ...
                break;
            case 0x4001:
                avio_em_skip(s->pb, 4); // seek offset
                avio_em_skip(s->pb, 4); // seek size
                break;
            case 0x4002:
                start = read32(s) + 8;
                avio_em_skip(s->pb, 4); //data_size = read32(s);
                break;
            case 0x4003:
                avio_em_skip(s->pb, 4); // REGN offset
                avio_em_skip(s->pb, 4); // REGN size
                break;
            }
        }
        ...
        avio_em_skip(s->pb, info_offset - avio_em_tell(s->pb));
        pos = avio_em_tell(s->pb);
        if (avio_em_rl32(s->pb) != MKTAG('I','N','F','O'))
            return AVERROR_INVALIDDATA;
    }
    ...
    if (size < 40)
        return AVERROR_INVALIDDATA;
    avio_em_skip(s->pb, 4); // unknown
    h1offset = read32(s);
    if (h1offset > size)
        return AVERROR_INVALIDDATA;
    avio_em_skip(s->pb, 12);
    toffset = read32(s) + 16LL;
    if (toffset > size)
        return AVERROR_INVALIDDATA;

    avio_em_skip(s->pb, pos + h1offset + 8 - avio_em_tell(s->pb));
    codec = avio_em_r8(s->pb);
    ...
    loop = avio_em_r8(s->pb); // loop flag
    st->codecpar->codec_id = codec;
    st->codecpar->channels = avio_em_r8(s->pb);
    if (!st->codecpar->channels)
        return AVERROR_INVALIDDATA;

    avio_em_skip(s->pb, 1); // padding
    ...
    if (loop) {
        if (av_em_dict_set_int(&s->metadata, "loop_start",
                            av_em_rescale(read32(s), AV_TIME_BASE,
    ...
    if (b->block_count > UINT16_MAX) {
        av_em_log(s, AV_LOG_WARNING, "too many blocks: %u\n", b->block_count);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (codec == AV_CODEC_ID_ADPCM_THP || codec == AV_CODEC_ID_ADPCM_THP_LE) {
        ...
        avio_em_skip(s->pb, pos + toffset - avio_em_tell(s->pb));
        if (!bfstm)
            toffset = read32(s) + 16LL;
        else
        ...
        avio_em_skip(s->pb, pos + toffset - avio_em_tell(s->pb));
        b->table = av_em_mallocz(32 * st->codecpar->channels);
        if (!b->table)
            return AVERROR(ENOMEM);

        for (ch = 0; ch < st->codecpar->channels; ch++) {
            if (avio_em_read(s->pb, b->table + ch * 32, 32) != 32) {
                ret = AVERROR_INVALIDDATA;
                goto fail;
            }
            avio_em_skip(s->pb, bfstm ? 14 : 24);
        }
    }

    if (size < (avio_em_tell(s->pb) - pos)) {
        ret = AVERROR_INVALIDDATA;
        goto fail;
    }

    avio_em_skip(s->pb, size - (avio_em_tell(s->pb) - pos));
    ...
    while (!avio_em_feof(s->pb)) {
        ...
        case MKTAG('A','D','P','C'):
            ...
            if (b->adpc) {
                av_em_log(s, AV_LOG_WARNING, "skipping additional ADPC chunk\n");
                goto skip;
            } else {
                b->adpc = av_em_mallocz(asize);
                if (!b->adpc) {
                    ret = AVERROR(ENOMEM);
                    goto fail;
                }
                if (bfstm && codec != AV_CODEC_ID_ADPCM_THP_LE) {
                    ...
                    int i;
                    for (i = 0; i < asize; i += 2) {
                        b->adpc[i+1] = avio_em_r8(s->pb);
                        b->adpc[i]   = avio_em_r8(s->pb);
                    }
                } else {
                    avio_em_read(s->pb, b->adpc, asize);
                }
                avio_em_skip(s->pb, size - asize);
            }
            break;
        case MKTAG('D','A','T','A'):
            if ((start < avio_em_tell(s->pb)) ||
                (!b->adpc && (codec == AV_CODEC_ID_ADPCM_THP ||
                              codec == AV_CODEC_ID_ADPCM_THP_LE))) {
                ret = AVERROR_INVALIDDATA;
                goto fail;
            }
            avio_em_skip(s->pb, start - avio_em_tell(s->pb));

            if (bfstm && (codec == AV_CODEC_ID_ADPCM_THP ||
                          codec == AV_CODEC_ID_ADPCM_THP_LE))
                avio_em_skip(s->pb, 24);

            b->data_start = avio_em_tell(s->pb);

            if (!bfstm && (major != 1 || minor))
                avpriv_em_request_sample(s, "Version %d.%d", major, minor);

            return 0;
        default:
            av_em_log(s, AV_LOG_WARNING, "skipping unknown chunk: %X\n", chunk);
skip:
...
static int read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if (par->codec_id == AV_CODEC_ID_ADPCM_THP ||
        ...
        if (!b->adpc) {
            av_em_log(s, AV_LOG_ERROR, "adpcm_thp requires ADPC chunk, but none was found.\n");
            return AVERROR_INVALIDDATA;
        }
        if (!b->table) {
            b->table = av_em_mallocz(32 * par->channels);
            if (!b->table)
                return AVERROR(ENOMEM);
        }
        ...
        if (size > (INT_MAX - 32 - 4) ||
            ...
            return AVERROR_INVALIDDATA;
        if (av_em_new_packet(pkt, 8 + (32 + 4 + size) * par->channels) < 0)
            return AVERROR(ENOMEM);
        dst = pkt->data;
        ...
        for (i = 0; i < par->channels; i++) {
            ret = avio_em_read(s->pb, dst, size);
            dst += size;
                     ...
                     int64_t timestamp, int flags)
{
    ...
    ret = avio_em_seek(s->pb, b->data_start + timestamp * b->block_size *
                           st->codecpar->channels, SEEK_SET);
    if (ret < 0)
        return ret;

    b->current_block = timestamp;
    em_update_cur_dts(s, st, timestamp * b->samples_per_block);
    return 0;
}

AVEMInputFormat ff_brstm_demuxer = {
    .name           = "brstm",
...
AVEMInputFormat ff_bfstm_demuxer = {
    .name           = "bfstm",
