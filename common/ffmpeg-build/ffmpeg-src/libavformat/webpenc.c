...
static int webp_write_header(AVEMFormatContext *s)
{
    AVEMStream *st;

    if (s->nb_streams != 1) {
        av_em_log(s, AV_LOG_ERROR, "Only exactly 1 stream is supported\n");
        return AVERROR(EINVAL);
    }
    st = s->streams[0];
    if (st->codecpar->codec_id != AV_CODEC_ID_WEBP) {
        av_em_log(s, AV_LOG_ERROR, "Only WebP is supported\n");
        return AVERROR(EINVAL);
    }
    avpriv_em_set_pts_info(st, 24, 1, 1000);

    return 0;
}

static int is_animated_webp_packet(AVEMPacket *pkt)
{
    if (pkt->size) {
        int skip = 0;
        unsigned flags = 0;
...
static int flush(AVEMFormatContext *s, int trailer, int64_t pts)
{
    ...
    if (w->last_pkt.size) {
        ...
        if (!w->wrote_webp_header) {
            avio_em_write(s->pb, "RIFF\0\0\0\0WEBP", 12);
        ...
        if (w->frame_count == 1) {
            ...
            if (vp8x) {
                avio_em_write(s->pb, "VP8X", 4);
                avio_em_wl32(s->pb, 10);
                avio_em_w8(s->pb, flags);
                avio_em_wl24(s->pb, 0);
                avio_em_wl24(s->pb, st->codecpar->width - 1);
                avio_em_wl24(s->pb, st->codecpar->height - 1);
            }
            if (!trailer) {
                avio_em_write(s->pb, "ANIM", 4);
                avio_em_wl32(s->pb, 6);
                avio_em_wl32(s->pb, 0xFFFFFFFF);
                avio_em_wl16(s->pb, w->loop);
            }
        }

        if (w->frame_count > trailer) {
            avio_em_write(s->pb, "ANMF", 4);
            avio_em_wl32(s->pb, 16 + w->last_pkt.size - skip);
            avio_em_wl24(s->pb, 0);
            avio_em_wl24(s->pb, 0);
            avio_em_wl24(s->pb, st->codecpar->width - 1);
            avio_em_wl24(s->pb, st->codecpar->height - 1);
            if (w->last_pkt.pts != AV_NOPTS_VALUE && pts != AV_NOPTS_VALUE) {
                avio_em_wl24(s->pb, pts - w->last_pkt.pts);
            } else
                avio_em_wl24(s->pb, w->last_pkt.duration);
            avio_em_w8(s->pb, 0);
        }
        avio_em_write(s->pb, w->last_pkt.data + skip, w->last_pkt.size - skip);
        av_em_packet_unref(&w->last_pkt);
    }

    return 0;
}

static int webp_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    WebpContext *w = s->priv_data;
    ...
    if (w->using_webp_anim_encoder) {
        ...
        if ((ret = flush(s, 0, pkt->pts)) < 0)
            return ret;
        av_em_copy_packet(&w->last_pkt, pkt);
    }
    ++w->frame_count;
...
static int webp_write_trailer(AVEMFormatContext *s)
{
    unsigned filesize;
    WebpContext *w = s->priv_data;
    ...
    if (w->using_webp_anim_encoder) {
        ...
        filesize = avio_em_tell(s->pb);
        avio_em_seek(s->pb, 4, SEEK_SET);
        avio_em_wl32(s->pb, filesize - 8);
        // Note: without the following, avio only writes 8 bytes to the file.
        avio_em_seek(s->pb, filesize, SEEK_SET);
    }
...
static const AVEMClass webp_muxer_class = {
    .class_name = "WebP muxer",
    .item_name  = av_em_default_item_name,
    .version    = LIBAVUTIL_VERSION_INT,
    .option     = options,
};
AVEMOutputFormat ff_webp_muxer = {
    .name           = "webp",
