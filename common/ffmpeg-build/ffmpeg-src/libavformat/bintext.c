...
typedef struct {
    ...
    AVEMRational framerate; /**< frames per second (private option) */
    uint64_t fsize;  /**< file size less metadata buffer */
} BinDemuxContext;

static AVEMStream * init_stream(AVEMFormatContext *s)
{
    ...
    avpriv_em_set_pts_info(st, 60, bin->framerate.den, bin->framerate.num);

    /* simulate tty display speed */
    bin->chars_per_frame = av_clip(av_em_q2d(st->time_base) * bin->chars_per_frame, 1, INT_MAX);
 ...
 */
static void calculate_height(AVEMCodecParameters *par, uint64_t fsize)
{
...
static int next_tag_read(AVEMFormatContext *avctx, uint64_t *fsize)
{
    AVEMIOContext *pb = avctx->pb;
    char buf[36];
    int len;
    uint64_t start_pos = avio_em_size(pb) - 256;

    avio_em_seek(pb, start_pos, SEEK_SET);
    if (avio_em_read(pb, buf, sizeof(next_magic)) != sizeof(next_magic))
        return -1;
    if (memcmp(buf, next_magic, sizeof(next_magic)))
        return -1;
    if (avio_em_r8(pb) != 0x01)
        return -1;
...
#define GET_EFI2_META(name,size) \
    ...
    if (len < 1 || len > size) \
        return -1; \
    if (avio_em_read(pb, buf, size) == size && *buf) { \
        buf[len] = 0; \
        av_em_dict_set(&avctx->metadata, name, buf, 0); \
    }
...
static void predict_width(AVEMCodecParameters *par, uint64_t fsize, int got_width)
{
...
static int bintext_read_header(AVEMFormatContext *s)
{
    BinDemuxContext *bin = s->priv_data;
    AVEMIOContext *pb = s->pb;

    AVEMStream *st = init_stream(s);
    if (!st)
        return AVERROR(ENOMEM);
    st->codecpar->codec_id    = AV_CODEC_ID_BINTEXT;

    if (em_alloc_extradata(st->codecpar, 2))
        return AVERROR(ENOMEM);
...
static int xbin_probe(AVEMProbeData *p)
{
...
static int xbin_read_header(AVEMFormatContext *s)
{
    BinDemuxContext *bin = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    avio_em_skip(pb, 5);
    st->codecpar->width   = avio_em_rl16(pb)<<3;
    st->codecpar->height  = avio_em_rl16(pb);
    fontheight         = avio_em_r8(pb);
    ...
    if (avio_em_read(pb, st->codecpar->extradata + 2, st->codecpar->extradata_size - 2) < 0)
        return AVERROR(EIO);

    if (pb->seekable) {
        bin->fsize = avio_em_size(pb) - 9 - st->codecpar->extradata_size;
        ff_sauce_read(s, &bin->fsize, NULL, 0);
        avio_em_seek(pb, 9 + st->codecpar->extradata_size, SEEK_SET);
    }
...
static int adf_read_header(AVEMFormatContext *s)
{
    BinDemuxContext *bin = s->priv_data;
    AVEMIOContext *pb = s->pb;
    AVEMStream *st;
    ...
    if (em_alloc_extradata(st->codecpar, 2 + 48 + 4096))
        return AVERROR(ENOMEM);
    st->codecpar->extradata[0] = 16;
    st->codecpar->extradata[1] = BINTEXT_PALETTE|BINTEXT_FONT;

    if (avio_em_read(pb, st->codecpar->extradata + 2, 24) < 0)
        return AVERROR(EIO);
    avio_em_skip(pb, 144);
    if (avio_em_read(pb, st->codecpar->extradata + 2 + 24, 24) < 0)
        return AVERROR(EIO);
    if (avio_em_read(pb, st->codecpar->extradata + 2 + 48, 4096) < 0)
        return AVERROR(EIO);

    if (pb->seekable) {
        int got_width = 0;
        bin->fsize = avio_em_size(pb) - 1 - 192 - 4096;
        st->codecpar->width = 80<<3;
        ff_sauce_read(s, &bin->fsize, &got_width, 0);
        if (!bin->width)
            calculate_height(st->codecpar, bin->fsize);
        avio_em_seek(pb, 1 + 192 + 4096, SEEK_SET);
    }
    return 0;
}
...
static int idf_probe(AVEMProbeData *p)
{
...
static int idf_read_header(AVEMFormatContext *s)
{
    BinDemuxContext *bin = s->priv_data;
    AVEMIOContext *pb = s->pb;
    AVEMStream *st;
    ...
    avio_em_seek(pb, avio_em_size(pb) - 4096 - 48, SEEK_SET);

    if (avio_em_read(pb, st->codecpar->extradata + 2 + 48, 4096) < 0)
        return AVERROR(EIO);
    if (avio_em_read(pb, st->codecpar->extradata + 2, 48) < 0)
        return AVERROR(EIO);

    bin->fsize = avio_em_size(pb) - 12 - 4096 - 48;
    ff_sauce_read(s, &bin->fsize, &got_width, 0);
    if (!bin->width)
        calculate_height(st->codecpar, bin->fsize);
                           ...
                           AVEMPacket *pkt)
{
    ...
    if (bin->fsize > 0) {
        if (av_em_get_packet(s->pb, pkt, bin->fsize) < 0)
            return AVERROR(EIO);
        bin->fsize = -1; /* done */
    } else if (!bin->fsize) {
        if (avio_em_feof(s->pb))
            return AVERROR(EIO);
        if (av_em_get_packet(s->pb, pkt, bin->chars_per_frame) < 0)
            return AVERROR(EIO);
    } else {
        return AVERROR(EIO);
    }
...
#if CONFIG_BINTEXT_DEMUXER
AVEMInputFormat ff_bintext_demuxer = {
    .name           = "bin",
...
#if CONFIG_XBIN_DEMUXER
AVEMInputFormat ff_xbin_demuxer = {
    .name           = "xbin",
...
#if CONFIG_ADF_DEMUXER
AVEMInputFormat ff_adf_demuxer = {
    .name           = "adf",
...
#if CONFIG_IDF_DEMUXER
AVEMInputFormat ff_idf_demuxer = {
    .name           = "idf",
