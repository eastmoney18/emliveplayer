...
static int vp8_header(AVEMFormatContext *s, int idx)
{
    ...
    case 0x01:
        if (os->psize < VP8_HEADER_SIZE) {
            av_em_log(s, AV_LOG_ERROR, "Invalid OggVP8 header packet");
    ...
    case 0x02:
        ...
        break;
    default:
        av_em_log(s, AV_LOG_ERROR, "Unknown VP8 header type 0x%02X\n", p[5]);
        return AVERROR_INVALIDDATA;
    }
...
static uint64_t vp8_gptopts(AVEMFormatContext *s, int idx,
...
static int vp8_packet(AVEMFormatContext *s, int idx)
{
    struct ogg *ogg = s->priv_data;
    struct ogg_stream *os = ogg->streams + idx;
    uint8_t *p = os->buf + os->pstart;
