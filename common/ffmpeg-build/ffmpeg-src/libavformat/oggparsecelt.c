...
static int celt_header(AVEMFormatContext *s, int idx)
{
    ...
    if (os->psize == 60 &&
        ...
        priv = av_em_alloc(sizeof(struct oggcelt_private));
        if (!priv)
            return AVERROR(ENOMEM);
        if (em_alloc_extradata(st->codecpar, 2 * sizeof(uint32_t)) < 0) {
            av_em_free(priv);
            return AVERROR(ENOMEM);
        }
