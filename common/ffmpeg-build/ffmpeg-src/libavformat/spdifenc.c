...
#define SPDIF_FLAG_BIGENDIAN    0x01
    ...
    int (*header_info) (AVEMFormatContext *s, AVEMPacket *pkt);
} IEC61937Context;
...
static const AVEMClass spdif_class = {
    .class_name     = "spdif",
...
static int spdif_header_ac3(AVEMFormatContext *s, AVEMPacket *pkt)
{
...
static int spdif_header_eac3(AVEMFormatContext *s, AVEMPacket *pkt)
{
    IEC61937Context *ctx = s->priv_data;
    ...
    ctx->hd_buf = av_em_fast_realloc(ctx->hd_buf, &ctx->hd_buf_size, ctx->hd_buf_filled + pkt->size);
    if (!ctx->hd_buf)
...
static int spdif_header_dts4(AVEMFormatContext *s, AVEMPacket *pkt, int core_size,
                             int sample_rate, int blocks)
{
    ...
    if (!core_size) {
        av_em_log(s, AV_LOG_ERROR, "HD mode not supported for this format\n");
        return AVERROR(EINVAL);
    }

    if (!sample_rate) {
        av_em_log(s, AV_LOG_ERROR, "Unknown DTS sample rate for HD\n");
    ...
    if (subtype < 0) {
        av_em_log(s, AV_LOG_ERROR, "Specified HD rate of %d Hz would require an "
    ...
    if (sizeof(dtshd_start_code) + 2 + pkt_size
        ...
        if (!ctx->dtshd_skip)
            av_em_log(s, AV_LOG_WARNING, "DTS-HD bitrate too high, "
    ...
    av_em_fast_malloc(&ctx->hd_buf, &ctx->hd_buf_size, ctx->out_bytes);
    if (!ctx->hd_buf)
        return AVERROR(ENOMEM);
...
static int spdif_header_dts(AVEMFormatContext *s, AVEMPacket *pkt)
{
    IEC61937Context *ctx = s->priv_data;
    ...
    case DCA_SYNCWORD_SUBSTREAM:
        ...
        av_em_log(s, AV_LOG_ERROR, "stray DTS-HD frame\n");
        return AVERROR_INVALIDDATA;
    default:
        av_em_log(s, AV_LOG_ERROR, "bad DTS syncword 0x%"PRIx32"\n", syncword_dts);
        return AVERROR_INVALIDDATA;
    }
    blocks++;
    ...
    default:
        av_em_log(s, AV_LOG_ERROR, "%i samples in DTS frame not supported\n",
               blocks << 5);
    ...
    if (ctx->out_bytes == ctx->pkt_offset) {
        ...
        avpriv_em_request_sample(s, "Unrecognized large DTS frame");
...
static int spdif_header_mpeg(AVEMFormatContext *s, AVEMPacket *pkt)
{
    IEC61937Context *ctx = s->priv_data;
    ...
    if (layer == 3 || version == 1) {
        av_em_log(s, AV_LOG_ERROR, "Wrong MPEG file format\n");
        return AVERROR_INVALIDDATA;
    }
    av_em_log(s, AV_LOG_DEBUG, "version: %i layer: %i extension: %i\n", version, layer, extension);
...
static int spdif_header_aac(AVEMFormatContext *s, AVEMPacket *pkt)
{
    IEC61937Context *ctx = s->priv_data;
    ...
    ret = avpriv_em_aac_parse_header(&gbc, &hdr);
    if (ret < 0) {
        av_em_log(s, AV_LOG_ERROR, "Wrong AAC file format\n");
        return AVERROR_INVALIDDATA;
    }
...
static int spdif_header_truehd(AVEMFormatContext *s, AVEMPacket *pkt)
{
    IEC61937Context *ctx = s->priv_data;
    ...
    if (pkt->size > TRUEHD_FRAME_OFFSET - mat_code_length) {
        ...
        avpriv_em_request_sample(s, "Too large TrueHD frame of %d bytes",
                              pkt->size);
...
static int spdif_write_header(AVEMFormatContext *s)
{
    IEC61937Context *ctx = s->priv_data;
    ...
    case AV_CODEC_ID_MLP:
        ...
        ctx->hd_buf = av_em_alloc(MAT_FRAME_SIZE);
        if (!ctx->hd_buf)
            return AVERROR(ENOMEM);
        break;
    default:
        av_em_log(s, AV_LOG_ERROR, "codec not supported\n");
        return AVERROR_PATCHWELCOME;
    }
    return 0;
}

static int spdif_write_trailer(AVEMFormatContext *s)
{
    IEC61937Context *ctx = s->priv_data;
    av_em_freep(&ctx->buffer);
    av_em_freep(&ctx->hd_buf);
    return 0;
}

static av_always_inline void spdif_put_16(IEC61937Context *ctx,
                                          AVEMIOContext *pb, unsigned int val)
{
...
static int spdif_write_packet(struct AVEMFormatContext *s, AVEMPacket *pkt)
{
    IEC61937Context *ctx = s->priv_data;
    int ret, padding;
    ...
    if (padding < 0) {
        av_em_log(s, AV_LOG_ERROR, "bitrate is too high\n");
        return AVERROR(EINVAL);
    }
    ...
    if (ctx->extra_bswap ^ (ctx->spdif_flags & SPDIF_FLAG_BIGENDIAN)) {
        avio_em_write(s->pb, ctx->out_buf, ctx->out_bytes & ~1);
    } else {
        av_em_fast_malloc(&ctx->buffer, &ctx->buffer_size, ctx->out_bytes + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!ctx->buffer)
            return AVERROR(ENOMEM);
        ff_spdif_bswap_buf16((uint16_t *)ctx->buffer, (uint16_t *)ctx->out_buf, ctx->out_bytes >> 1);
        avio_em_write(s->pb, ctx->buffer, ctx->out_bytes & ~1);
    }
    ...
    emio_fill(s->pb, 0, padding);

    av_em_log(s, AV_LOG_DEBUG, "type=%x len=%i pkt_offset=%i\n",
           ctx->data_type, ctx->out_bytes, ctx->pkt_offset);

    return 0;
}

AVEMOutputFormat ff_spdif_muxer = {
    .name              = "spdif",
