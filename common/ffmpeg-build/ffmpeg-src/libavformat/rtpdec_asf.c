...
/**
 * The following code is basically a buffered AVEMIOContext,
...
static void init_packetizer(AVEMIOContext *pb, uint8_t *buf, int len)
{
    emio_init_context(pb, buf, len, 0, NULL, packetizer_read, NULL, NULL);
...
int ff_wms_parse_sdp_a_line(AVEMFormatContext *s, const char *p)
{
    int ret = 0;
    if (av_em_strstart(p, "pgmpu:data:application/vnd.ms.wms-hdr.asfv1;base64,", &p)) {
        AVEMIOContext pb;
        RTSPState *rt = s->priv_data;
        AVEMDictionary *opts = NULL;
        int len = strlen(p) * 6 / 8;
        char *buf = av_em_mallocz(len);
        AVEMInputFormat *iformat;

        if (!buf)
            return AVERROR(ENOMEM);
        av_em_base64_decode(buf, p, len);
        ...
        if (!(iformat = av_em_find_input_format("asf")))
            return AVERROR_DEMUXER_NOT_FOUND;

        rt->asf_ctx = avformat_em_alloc_context();
        if (!rt->asf_ctx) {
            av_em_free(buf);
            return AVERROR(ENOMEM);
        }
        rt->asf_ctx->pb      = &pb;
        av_em_dict_set(&opts, "no_resync_search", "1", 0);

        if ((ret = em_copy_whiteblacklists(rt->asf_ctx, s)) < 0) {
            av_em_dict_free(&opts);
            return ret;
        }

        ret = avformat_em_open_input(&rt->asf_ctx, "", iformat, &opts);
        av_em_dict_free(&opts);
        if (ret < 0) {
            av_em_free(buf);
            return ret;
        }
        av_em_dict_copy(&s->metadata, rt->asf_ctx->metadata, 0);
        rt->asf_pb_pos = avio_em_tell(&pb);
        av_em_free(buf);
        rt->asf_ctx->pb = NULL;
    }
    return ret;
...
static int asfrtp_parse_sdp_line(AVEMFormatContext *s, int stream_index,
                                 PayloadContext *asf, const char *line)
{
    if (stream_index < 0)
        return 0;
    if (av_em_strstart(line, "stream:", &line)) {
        RTSPState *rt = s->priv_data;
        ...
        if (rt->asf_ctx) {
            ...
            for (i = 0; i < rt->asf_ctx->nb_streams; i++) {
                if (s->streams[stream_index]->id == rt->asf_ctx->streams[i]->id) {
                    avcodec_em_parameters_copy(s->streams[stream_index]->codecpar,
                                            rt->asf_ctx->streams[i]->codecpar);
                    s->streams[stream_index]->need_parsing =
                        rt->asf_ctx->streams[i]->need_parsing;
                    avpriv_em_set_pts_info(s->streams[stream_index], 32, 1, 1000);
                }
           }
        }
    }
 ...
 */
static int asfrtp_parse_packet(AVEMFormatContext *s, PayloadContext *asf,
                               AVEMStream *st, AVEMPacket *pkt,
                               uint32_t *timestamp,
                               ...
                               int flags)
{
    ...
    if (len > 0) {
        ...
        while (avio_em_tell(pb) + 4 < len) {
            int start_off = avio_em_tell(pb);

            mflags = avio_em_r8(pb);
            len_off = avio_em_rb24(pb);
            ...
            if (!(mflags & 0x40)) {
                 ...
                 */
                if (asf->pktbuf && len_off != avio_em_tell(asf->pktbuf)) {
                    emio_free_dyn_buf(&asf->pktbuf);
                }
                if (!len_off && !asf->pktbuf &&
                    (res = avio_em_open_dyn_buf(&asf->pktbuf)) < 0)
                    return res;
                if (!asf->pktbuf)
                    return AVERROR(EIO);

                avio_em_write(asf->pktbuf, buf + off, len - off);
                avio_em_skip(pb, len - off);
                if (!(flags & RTP_FLAG_MARKER))
                    return -1;
                out_len     = avio_em_close_dyn_buf(asf->pktbuf, &asf->buf);
                asf->pktbuf = NULL;
            } else {
                /**
                ...
                if (FFMIN(cur_len, len - off) < 0)
                    return -1;
                if ((res = av_em_reallocp(&asf->buf, out_len)) < 0)
                    return res;
                memcpy(asf->buf + prev_len, buf + off,
                       FFMIN(cur_len, len - off));
                avio_em_skip(pb, cur_len);
            }
        }
    ...
    for (;;) {
        ...
        res = em_read_packet(rt->asf_ctx, pkt);
        rt->asf_pb_pos = avio_em_tell(pb);
        if (res != 0)
            break;
