...
static int yuv4_read_header(AVEMFormatContext *s)
{
    ...
    for (tokstart = &header[strlen(Y4M_MAGIC) + 1];
        ...
        case 'C': // Color space
            if (strncmp("420jpeg", tokstart, 7) == 0) {
                ...
                av_em_log(s, AV_LOG_ERROR, "Cannot handle 4:4:4:4 "
                       "YUV4MPEG stream.\n");
                return -1;
                ...
                av_em_log(s, AV_LOG_ERROR, "YUV4MPEG stream contains an unknown "
                       "pixel format.\n");
                return -1;
            }
        ...
        case 'I': // Interlace type
            ...
            case 'b':
                ...
                break;
            case 'm':
                av_em_log(s, AV_LOG_ERROR, "YUV4MPEG stream contains mixed "
                       "interlaced and non-interlaced frames.\n");
            default:
                av_em_log(s, AV_LOG_ERROR, "YUV4MPEG has invalid header.\n");
    ...
    av_em_reduce(&raten, &rated, raten, rated, (1UL << 31) - 1);
    avpriv_em_set_pts_info(st, 64, rated, raten);
    st->avg_frame_rate                = av_em_inv_q(st->time_base);
    st->codecpar->format              = pix_fmt;
    st->codecpar->codec_type          = AVMEDIA_TYPE_VIDEO;
    st->codecpar->codec_id            = AV_CODEC_ID_RAWVIDEO;
    st->sample_aspect_ratio           = (AVEMRational){ aspectn, aspectd };
    st->codecpar->chroma_location     = chroma_sample_location;
    st->codecpar->field_order         = field_order;
    s->packet_size = av_em_image_get_buffer_size(st->codecpar->format, width, height, 1) + Y4M_FRAME_MAGIC_LEN;
    if ((int) s->packet_size < 0)
        return s->packet_size;
    s->internal->data_offset = avio_em_tell(pb);

    st->duration = (avio_em_size(pb) - avio_em_tell(pb)) / s->packet_size;

    return 0;
}

static int yuv4_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int i;
    ...
    ret = av_em_get_packet(s->pb, pkt, s->packet_size - Y4M_FRAME_MAGIC_LEN);
    if (ret < 0)
        return ret;
...
static int yuv4_read_seek(AVEMFormatContext *s, int stream_index,
                          int64_t pts, int flags)
{
    if (avio_em_seek(s->pb, pts * s->packet_size + s->internal->data_offset, SEEK_SET) < 0)
        return -1;
    return 0;
}

static int yuv4_probe(AVEMProbeData *pd)
{
    /* check file header */
...
AVEMInputFormat ff_yuv4mpegpipe_demuxer = {
    .name           = "yuv4mpegpipe",
