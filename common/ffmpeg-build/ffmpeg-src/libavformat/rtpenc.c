...
static const AVEMClass rtp_muxer_class = {
...
static int is_supported(enum AVEMCodecID id)
{
    switch(id) {
...
static int rtp_write_header(AVEMFormatContext *s1)
{
    ...
    if (s1->nb_streams != 1) {
        av_em_log(s1, AV_LOG_ERROR, "Only one stream supported in the RTP muxer\n");
        return AVERROR(EINVAL);
    }
    st = s1->streams[0];
    if (!is_supported(st->codecpar->codec_id)) {
        av_em_log(s1, AV_LOG_ERROR, "Unsupported codec %s\n", avcodec_em_get_name(st->codecpar->codec_id));
    ...
    s->base_timestamp = av_em_get_random_seed();
    s->timestamp = s->base_timestamp;
    s->cur_timestamp = 0;
    if (!s->ssrc)
        s->ssrc = av_em_get_random_seed();
    s->first_packet = 1;
    s->first_rtcp_ntp_time = em_ntp_time();
    ...
    if (s->seq < 0) {
        if (s1->flags & AVFMT_FLAG_BITEXACT) {
            ...
            s->seq = av_em_get_random_seed() & 0x0fff;
    } else
    ...
    if (s1->packet_size <= 12) {
        av_em_log(s1, AV_LOG_ERROR, "Max packet size %d too low\n", s1->packet_size);
        return AVERROR(EIO);
    }
    s->buf = av_em_alloc(s1->packet_size);
    if (!s->buf) {
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
        ...
        avpriv_em_set_pts_info(st, 32, 1, 90000);
    }
    s->buf_ptr = s->buf;
    ...
    case AV_CODEC_ID_ADPCM_G722:
        ...
        avpriv_em_set_pts_info(st, 32, 1, 8000);
        break;
    case AV_CODEC_ID_OPUS:
        if (st->codecpar->channels > 2) {
            av_em_log(s1, AV_LOG_ERROR, "Multistream opus not supported in RTP\n");
            goto fail;
        }
        ...
        avpriv_em_set_pts_info(st, 32, 1, 48000);
        break;
    case AV_CODEC_ID_ILBC:
        if (st->codecpar->block_align != 38 && st->codecpar->block_align != 50) {
            av_em_log(s1, AV_LOG_ERROR, "Incorrect iLBC block size specified\n");
            goto fail;
        }
    ...
    case AV_CODEC_ID_AMR_WB:
        ...
        if (1 + s->max_frames_per_packet + n > s->max_payload_size) {
            av_em_log(s1, AV_LOG_ERROR, "RTP max payload size too small for AMR\n");
            goto fail;
        }
        if (st->codecpar->channels != 1) {
            av_em_log(s1, AV_LOG_ERROR, "Only mono is supported\n");
            goto fail;
        }
        break;
    case AV_CODEC_ID_AAC:
...
static void rtcp_send_sr(AVEMFormatContext *s1, int64_t ntp_time, int bye)
{
    RTPMuxContext *s = s1->priv_data;
    uint32_t rtp_ts;

    av_em_log(s1, AV_LOG_TRACE, "RTCP: %02x %"PRIx64" %x\n", s->payload_type, ntp_time, s->timestamp);

    s->last_rtcp_ntp_time = ntp_time;
    rtp_ts = av_em_rescale_q(ntp_time - s->first_rtcp_ntp_time, (AVEMRational){1, 1000000},
                          s1->streams[0]->time_base) + s->base_timestamp;
    avio_em_w8(s1->pb, RTP_VERSION << 6);
    avio_em_w8(s1->pb, RTCP_SR);
    avio_em_wb16(s1->pb, 6); /* length in words - 1 */
    avio_em_wb32(s1->pb, s->ssrc);
    avio_em_wb32(s1->pb, ntp_time / 1000000);
    avio_em_wb32(s1->pb, ((ntp_time % 1000000) << 32) / 1000000);
    avio_em_wb32(s1->pb, rtp_ts);
    avio_em_wb32(s1->pb, s->packet_count);
    avio_em_wb32(s1->pb, s->octet_count);

    if (s->cname) {
        int len = FFMIN(strlen(s->cname), 255);
        avio_em_w8(s1->pb, (RTP_VERSION << 6) + 1);
        avio_em_w8(s1->pb, RTCP_SDES);
        avio_em_wb16(s1->pb, (7 + len + 3) / 4); /* length in words - 1 */

        avio_em_wb32(s1->pb, s->ssrc);
        avio_em_w8(s1->pb, 0x01); /* CNAME */
        avio_em_w8(s1->pb, len);
        avio_em_write(s1->pb, s->cname, len);
        avio_em_w8(s1->pb, 0); /* END */
    ...
    if (bye) {
        avio_em_w8(s1->pb, (RTP_VERSION << 6) | 1);
        avio_em_w8(s1->pb, RTCP_BYE);
        avio_em_wb16(s1->pb, 1); /* length in words - 1 */
        avio_em_wb32(s1->pb, s->ssrc);
    }
...
void ff_rtp_send_data(AVEMFormatContext *s1, const uint8_t *buf1, int len, int m)
{
    RTPMuxContext *s = s1->priv_data;

    av_em_log(s1, AV_LOG_TRACE, "rtp_send_data size=%d\n", len);

    /* build the RTP header */
    avio_em_w8(s1->pb, RTP_VERSION << 6);
    avio_em_w8(s1->pb, (s->payload_type & 0x7f) | ((m & 0x01) << 7));
    avio_em_wb16(s1->pb, s->seq);
    avio_em_wb32(s1->pb, s->timestamp);
    avio_em_wb32(s1->pb, s->ssrc);

    avio_em_write(s1->pb, buf1, len);
    avio_em_flush(s1->pb);
...
static int rtp_send_samples(AVEMFormatContext *s1,
                            const uint8_t *buf1, int size, int sample_size_bits)
{
    ...
    int aligned_samples_size = sample_size_bits/av_em_gcd(sample_size_bits, 8);
...
static void rtp_send_mpegaudio(AVEMFormatContext *s1,
...
static void rtp_send_raw(AVEMFormatContext *s1,
                         const uint8_t *buf1, int size)
{
    RTPMuxContext *s = s1->priv_data;
...
static void rtp_send_mpegts_raw(AVEMFormatContext *s1,
                                const uint8_t *buf1, int size)
{
    RTPMuxContext *s = s1->priv_data;
    int len, out_len;
...
static int rtp_send_ilbc(AVEMFormatContext *s1, const uint8_t *buf, int size)
{
    RTPMuxContext *s = s1->priv_data;
    AVEMStream *st = s1->streams[0];
    int frame_duration = av_em_get_audio_frame_duration2(st->codecpar, 0);
    int frame_size = st->codecpar->block_align;
    int frames = size / frame_size;

    while (frames > 0) {
        if (s->num_frames > 0 &&
            av_em_compare_ts(s->cur_timestamp - s->timestamp, st->time_base,
...
static int rtp_write_packet(AVEMFormatContext *s1, AVEMPacket *pkt)
{
    RTPMuxContext *s = s1->priv_data;
    AVEMStream *st = s1->streams[0];
    int rtcp_bytes;
    ...
    av_em_log(s1, AV_LOG_TRACE, "%d: write len=%d\n", pkt->stream_index, size);

    rtcp_bytes = ((s->octet_count - s->last_octet_count) * RTCP_TX_RATIO_NUM) /
        RTCP_TX_RATIO_DEN;
    if ((s->first_packet || ((rtcp_bytes >= RTCP_SR_SIZE) &&
                            (em_ntp_time() - s->last_rtcp_ntp_time > 5000000))) &&
        !(s->flags & FF_RTP_FLAG_SKIP_RTCP)) {
        rtcp_send_sr(s1, em_ntp_time(), 0);
    ...
    case AV_CODEC_ID_H263:
        if (s->flags & FF_RTP_FLAG_RFC2190) {
            ...
            const uint8_t *mb_info =
                av_em_packet_get_side_data(pkt, AV_PKT_DATA_H263_MB_INFO,
                                        &mb_info_size);
...
static int rtp_write_trailer(AVEMFormatContext *s1)
{
    RTPMuxContext *s = s1->priv_data;
    ...
    if (s1->pb && (s->flags & FF_RTP_FLAG_SEND_BYE))
        rtcp_send_sr(s1, em_ntp_time(), 1);
    av_em_freep(&s->buf);

    return 0;
}

AVEMOutputFormat ff_rtp_muxer = {
    .name              = "rtp",
