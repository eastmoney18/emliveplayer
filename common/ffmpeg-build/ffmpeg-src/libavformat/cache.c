...
static int cache_open(EMURLContext *h, const char *arg, int flags, AVEMDictionary **options)
{
    char *buffername;
    Context *c= h->priv_data;

    av_em_strstart(arg, "cache:", &arg);

    c->fd = avpriv_em_tempfile("ffcache", &buffername, 0, h);
    if (c->fd < 0){
        av_em_log(h, AV_LOG_ERROR, "Failed to create tempfile\n");
        return c->fd;
    }

    unlink(buffername);
    av_em_freep(&buffername);

    return ffurl_em_open_whitelist(&c->inner, arg, flags, &h->interrupt_callback,
                                options, h->protocol_whitelist, h->protocol_blacklist, h);
}

static int add_entry(EMURLContext *h, const unsigned char *buf, int size)
{
    Context *c= h->priv_data;
    int64_t pos = -1;
    int ret;
    ...
    if (pos < 0) {
        ret = AVERROR(errno);
        av_em_log(h, AV_LOG_ERROR, "seek in cache failed\n");
        goto fail;
    }
    c->cache_pos = pos;
    ...
    if (ret < 0) {
        ret = AVERROR(errno);
        av_em_log(h, AV_LOG_ERROR, "write in cache failed\n");
        goto fail;
    }
    c->cache_pos += ret;

    entry = av_em_tree_find(c->root, &c->logical_pos, cmp, (void**)next);
    ...
    if (!entry ||
        ...
        entry = av_em_alloc(sizeof(*entry));
        node = av_em_tree_node_alloc();
        if (!entry || !node) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        ...
        entry_ret = av_em_tree_insert(&c->root, entry, cmp, &node);
        if (entry_ret && entry_ret != entry) {
            ret = -1;
            av_em_log(h, AV_LOG_ERROR, "av_em_tree_insert failed\n");
            goto fail;
        }
    } else
        entry->size += ret;
...
static int cache_read(EMURLContext *h, unsigned char *buf, int size)
{
    Context *c= h->priv_data;
    ...
    if (c->logical_pos != c->inner_pos) {
        r = ffurl_em_seek(c->inner, c->logical_pos, SEEK_SET);
        if (r<0) {
            av_em_log(h, AV_LOG_ERROR, "Failed to perform internal seek\n");
            return r;
        }
        c->inner_pos = r;
    }

    r = ffurl_em_read(c->inner, buf, size);
    if (r == 0 && size>0) {
        c->is_true_eof = 1;
...
static int64_t cache_seek(EMURLContext *h, int64_t pos, int whence)
{
    Context *c= h->priv_data;
    int64_t ret;

    if (whence == AVSEEK_SIZE) {
        pos= ffurl_em_seek(c->inner, pos, whence);
        if(pos <= 0){
            pos= ffurl_em_seek(c->inner, -1, SEEK_END);
            if (ffurl_em_seek(c->inner, c->inner_pos, SEEK_SET) < 0)
                av_em_log(h, AV_LOG_ERROR, "Inner protocol failed to seekback end : %"PRId64"\n", pos);
        }
        if (pos > 0)
            c->is_true_eof = 1;
...
resolve_eof:
    ...
    //cache miss
    ret= ffurl_em_seek(c->inner, pos, whence);
...
static int cache_close(EMURLContext *h)
{
    Context *c= h->priv_data;

    av_em_log(h, AV_LOG_INFO, "Statistics, cache hits:%"PRId64" cache misses:%"PRId64"\n",
           c->cache_hit, c->cache_miss);

    close(c->fd);
    ffurl_em_close(c->inner);
    av_em_tree_enumerate(c->root, NULL, NULL, enu_free);
    av_em_tree_destroy(c->root);
...
static const AVEMClass cache_context_class = {
    .class_name = "Cache",
...
const EMURLProtocol em_cache_protocol = {
    .name                = "cache",
