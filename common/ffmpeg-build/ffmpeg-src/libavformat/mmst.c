...
static int send_command_packet(MMSTContext *mmst)
{
    ...
    // write it out.
    write_result= ffurl_em_write(mms->mms_hd, mms->out_buffer, exact_length);
...
static int mms_put_utf16(MMSContext *mms, const uint8_t *src)
{
    ...
    int len;
    emio_init_context(&bic, mms->write_out_ptr,
            sizeof(mms->out_buffer) - size, 1, NULL, NULL, NULL, NULL);

    len = avio_em_put_str16le(&bic, src);
    if (len < 0)
        return len;
...
{
    MMSContext *mms = &mmst->mms;
    av_em_log(NULL, AV_LOG_TRACE, "Stream changing!\n");

    // 40 is the packet header size, 7 is the prefix size.
    mmst->header_packet_id= AV_RL32(mms->in_buffer + 40 + 7);
    av_em_log(NULL, AV_LOG_TRACE, "Changed header prefix to 0x%x", mmst->header_packet_id);
}
...
static MMSSCPacketType get_tcp_server_response(MMSTContext *mmst)
{
    ...
    MMSContext *mms = &mmst->mms;
    for(;;) {
        read_result = ffurl_em_read_complete(mms->mms_hd, mms->in_buffer, 8);
        if (read_result != 8) {
            if(read_result < 0) {
                av_em_log(NULL, AV_LOG_ERROR,
        ...
        if(AV_RL32(mms->in_buffer + 4)==0xb00bface) {
            ...
            read_result= ffurl_em_read_complete(mms->mms_hd, mms->in_buffer+8, 4);
            if(read_result != 4) {
                av_em_log(NULL, AV_LOG_ERROR,
            ...
            av_em_log(NULL, AV_LOG_TRACE, "Length remaining is %d\n", length_remaining);
            ...
            read_result = ffurl_em_read_complete(mms->mms_hd, mms->in_buffer + 12,
                                            length_remaining) ;
            ...
            read_result= ffurl_em_read_complete(mms->mms_hd, mms->in_buffer, length_remaining);
            if(read_result != length_remaining) {
                av_em_log(NULL, AV_LOG_ERROR,
            ...
            if(packet_id_type == mmst->header_packet_id) {
                ...
                if(!mms->header_parsed) {
                    if ((err = av_em_reallocp(&mms->asf_header,
                                           mms->asf_header_size +
                ...
                av_em_log(NULL, AV_LOG_TRACE, "packet id type %d is old.", packet_id_type);
                continue;
            }
        }
                              ...
                              const MMSSCPacketType expect_type)
{
    ...
    if(send_fun) {
        ...
        if (ret < 0) {
            av_em_log(NULL, AV_LOG_TRACE, "Send Packet error before expecting recv packet %d\n", expect_type);
            return ret;
        }
    }
...
/** Send MMST stream selection command based on the AVEMStream->discard values. */
...
static int mms_close(EMURLContext *h)
{
...
static int mms_open(EMURLContext *h, const char *uri, int flags)
{
    ...
    em_url_join(tcpname, sizeof(tcpname), "tcp", NULL, mmst->host, port, NULL);
    err = ffurl_em_open_whitelist(&mms->mms_hd, tcpname, AVIO_FLAG_READ_WRITE,
    ...
    if (err) {
        av_em_log(NULL, AV_LOG_TRACE, "asf header parsed failed!\n");
        goto fail;
    }
    mms->header_parsed = 1;
    ...
    av_em_log(NULL, AV_LOG_TRACE, "Leaving open (success)\n");
    return 0;
fail:
    mms_close(h);
    av_em_log(NULL, AV_LOG_TRACE, "Leaving open (failure: %d)\n", err);
    return err;
}

/** Read ASF data through the protocol. */
static int mms_read(EMURLContext *h, uint8_t *buf, int size)
{
        ...
        if(mms->asf_header_read_size < mms->asf_header_size) {
            ...
            if (err == 0) {
                if(mms->remaining_in_len>mms->asf_packet_len) {
                    ...
                    result = ff_mms_read_data(mms, buf, size);
                    if (result == 0) {
                        av_em_log(NULL, AV_LOG_TRACE, "Read ASF media packet size is zero!\n");
                        break;
                    }
                }
            } else {
                av_em_log(NULL, AV_LOG_TRACE, "read packet error!\n");
                break;
            }
        }
...
const EMURLProtocol em_mmst_protocol = {
    .name           = "mmst",
    .url_open       = mms_open,
    .url_read       = mms_read,
    .url_close      = mms_close,
