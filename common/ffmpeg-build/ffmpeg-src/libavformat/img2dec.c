 ...
 */
static int find_image_range(AVEMIOContext *pb, int *pfirst_index, int *plast_index,
                            const char *path, int start_index, int start_index_range)
{
    ...
    for (first_index = start_index; first_index < start_index + start_index_range; first_index++) {
        if (av_em_get_frame_filename(buf, sizeof(buf), path, first_index) < 0) {
            *pfirst_index =
            *plast_index  = 1;
            if (pb || avio_em_check(buf, AVIO_FLAG_READ) > 0)
                return 0;
            return -1;
        }
        if (avio_em_check(buf, AVIO_FLAG_READ) > 0)
            break;
    }
    ...
    for (;;) {
        ...
        for (;;) {
            if (!range)
                ...
                range1 = 2 * range;
            if (av_em_get_frame_filename(buf, sizeof(buf), path,
                                      last_index + range1) < 0)
                goto fail;
            if (avio_em_check(buf, AVIO_FLAG_READ) <= 0)
                break;
...
static int img_read_probe(AVEMProbeData *p)
{
    if (p->filename && em_guess_image2_codec(p->filename)) {
        if (av_em_filename_number_test(p->filename))
            ...
            return 0;
        else if (av_em_match_ext(p->filename, "raw") || av_em_match_ext(p->filename, "gif"))
            return 5;
        else
...
int ff_img_read_header(AVEMFormatContext *s1)
{
    ...
    st = avformat_em_new_stream(s1, NULL);
    if (!st) {
        return AVERROR(ENOMEM);
    }

    if (s->pixel_format &&
        (pix_fmt = av_em_get_pix_fmt(s->pixel_format)) == AV_PIX_FMT_NONE) {
        av_em_log(s1, AV_LOG_ERROR, "No such pixel format: %s.\n",
               s->pixel_format);
        return AVERROR(EINVAL);
    }

    av_em_strlcpy(s->path, s1->filename, sizeof(s->path));
    s->img_number = 0;
    s->img_count  = 0;
    ...
    if (s->ts_from_file == 2) {
        ...
        av_em_log(s1, AV_LOG_ERROR, "POSIX.1-2008 not supported, nanosecond file timestamps unavailable\n");
        return AVERROR(ENOSYS);
#endif
        avpriv_em_set_pts_info(st, 64, 1, 1000000000);
    } else if (s->ts_from_file)
        avpriv_em_set_pts_info(st, 64, 1, 1);
    else
        avpriv_em_set_pts_info(st, 64, s->framerate.den, s->framerate.num);
    ...
    if (!s->is_pipe) {
        ...
        if (s->use_glob) {
            ...
            av_em_log(s1, AV_LOG_WARNING, "Pattern type 'glob_sequence' is deprecated: "
                   "use pattern_type 'glob' instead\n");
#if HAVE_GLOB
            dup = q = av_em_strdup(p);
            while (*q) {
    ...
    if (s1->video_codec_id) {
        ...
        s->split_planes       = str && !av_em_strcasecmp(str + 1, "y");
        st->codecpar->codec_type = AVMEDIA_TYPE_VIDEO;
        if (s1->pb) {
            int probe_buffer_size = 2048;
            uint8_t *probe_buffer = av_em_realloc(NULL, probe_buffer_size + AVPROBE_PADDING_SIZE);
            ...
            probe_buffer_size = avio_em_read(s1->pb, probe_buffer, probe_buffer_size);
            if (probe_buffer_size < 0) {
                av_em_free(probe_buffer);
            ...
            if (s1->flags & AVFMT_FLAG_CUSTOM_IO) {
                avio_em_seek(s1->pb, 0, SEEK_SET);
            } else
                emio_rewind_with_probe_data(s1->pb, &probe_buffer, probe_buffer_size);
        }
        if (st->codecpar->codec_id == AV_CODEC_ID_NONE)
            st->codecpar->codec_id = em_guess_image2_codec(s->path);
...
int ff_img_read_packet(AVEMFormatContext *s1, AVEMPacket *pkt)
{
    VideoDemuxData *s = s1->priv_data;
    ...
    AVEMIOContext *f[3]     = { NULL };
    AVEMCodecParameters *par = s1->streams[0]->codecpar;
    ...
    if (!s->is_pipe) {
        ...
        if (s->pattern_type == PT_NONE) {
            av_em_strlcpy(filename_bytes, s->path, sizeof(filename_bytes));
        } else if (s->use_glob) {
#if HAVE_GLOB
            filename = s->globstate.gl_pathv[s->img_number];
#endif
        } else {
        if (av_em_get_frame_filename(filename_bytes, sizeof(filename_bytes),
                                  s->path,
        ...
        for (i = 0; i < 3; i++) {
            if (s1->pb &&
                ...
                if (i >= 1)
                    break;
                av_em_log(s1, AV_LOG_ERROR, "Could not open file : %s\n",
                       filename);
                return AVERROR(EIO);
            }
            size[i] = avio_em_size(f[i]);
        ...
        if (par->codec_id == AV_CODEC_ID_NONE) {
            AVEMProbeData pd = { 0 };
            AVEMInputFormat *ifmt;
            uint8_t header[PROBE_BUF_MIN + AVPROBE_PADDING_SIZE];
            int ret;
            int score = 0;

            ret = avio_em_read(f[0], header, PROBE_BUF_MIN);
            if (ret < 0)
                return ret;
            memset(header + ret, 0, sizeof(header) - ret);
            avio_em_skip(f[0], -ret);
            pd.buf = header;
            pd.buf_size = ret;
            pd.filename = filename;

            ifmt = av_em_probe_input_format3(&pd, 1, &score);
        ...
        f[0] = s1->pb;
        if (avio_em_feof(f[0]) && s->loop && s->is_pipe)
            avio_em_seek(f[0], 0, SEEK_SET);
        if (avio_em_feof(f[0]))
            return AVERROR_EOF;
        if (s->frame_size > 0) {
            size[0] = s->frame_size;
        } else if (!s1->streams[0]->parser) {
            size[0] = avio_em_size(s1->pb);
        } else {
            size[0] = 4096;
        }
    }

    res = av_em_new_packet(pkt, size[0] + size[1] + size[2]);
    if (res < 0) {
        goto fail;
    }
    pkt->stream_index = 0;
...
#endif
        av_em_add_index_entry(s1->streams[0], s->img_number, pkt->pts, 0, 0, AVINDEX_KEYFRAME);
    } else if (!s->is_pipe) {
        pkt->pts      = s->pts;
    }

    if (s->is_pipe)
        pkt->pos = avio_em_tell(f[0]);

    pkt->size = 0;
    for (i = 0; i < 3; i++) {
        if (f[i]) {
            ret[i] = avio_em_read(f[i], pkt->data + pkt->size, size[i]);
            if (s->loop && s->is_pipe && ret[i] == AVERROR_EOF) {
                if (avio_em_seek(f[i], 0, SEEK_SET) >= 0) {
                    pkt->pos = 0;
                    ret[i] = avio_em_read(f[i], pkt->data + pkt->size, size[i]);
                }
            }
            if (!s->is_pipe && f[i] != s1->pb)
                em_format_io_close(s1, &f[i]);
            if (ret[i] > 0)
                pkt->size += ret[i];
        }
    }
...
static int img_read_close(struct AVEMFormatContext* s1)
{
#if HAVE_GLOB
    VideoDemuxData *s = s1->priv_data;
    if (s->use_glob) {
...
static int img_read_seek(AVEMFormatContext *s, int stream_index, int64_t timestamp, int flags)
{
    VideoDemuxData *s1 = s->priv_data;
    AVEMStream *st = s->streams[0];

    if (s1->ts_from_file) {
        int index = av_em_index_search_timestamp(st, timestamp, flags);
        if(index < 0)
            return -1;
...
static const AVEMClass img2_class = {
    .class_name = "image2 demuxer",
    .item_name  = av_em_default_item_name,
    .option     = ff_img_options,
    .version    = LIBAVUTIL_VERSION_INT,
};
AVEMInputFormat ff_image2_demuxer = {
    .name           = "image2",
...
static const AVEMClass img2pipe_class = {
    .class_name = "image2pipe demuxer",
    .item_name  = av_em_default_item_name,
    .option     = ff_img_options,
    .version    = LIBAVUTIL_VERSION_INT,
};
AVEMInputFormat ff_image2pipe_demuxer = {
    .name           = "image2pipe",
...
static int bmp_probe(AVEMProbeData *p)
{
...
static int dds_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
...
static int dpx_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
    int w, h;
...
static int exr_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
...
static int j2k_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
...
static int jpeg_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
    int i, state = SOI;
...
static int jpegls_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
...
static int pcx_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
...
static int qdraw_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
...
static int pictor_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
...
static int png_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
...
static int sgi_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
...
static int sunrast_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
...
static int tiff_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
...
static int webp_probe(AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
...
static int pnm_magic_check(const AVEMProbeData *p, int magic)
{
    const uint8_t *b = p->buf;
...
static inline int pnm_probe(const AVEMProbeData *p)
{
    const uint8_t *b = p->buf;
...
static int pbm_probe(AVEMProbeData *p)
{
    return pnm_magic_check(p, 1) || pnm_magic_check(p, 4) ? pnm_probe(p) : 0;
}

static inline int pgmx_probe(AVEMProbeData *p)
{
    return pnm_magic_check(p, 2) || pnm_magic_check(p, 5) ? pnm_probe(p) : 0;
}

static int pgm_probe(AVEMProbeData *p)
{
    int ret = pgmx_probe(p);
    return ret && !av_em_match_ext(p->filename, "pgmyuv") ? ret : 0;
}

static int pgmyuv_probe(AVEMProbeData *p) // custom FFmpeg format recognized by file extension
{
    int ret = pgmx_probe(p);
    return ret && av_em_match_ext(p->filename, "pgmyuv") ? ret : 0;
}

static int ppm_probe(AVEMProbeData *p)
{
    return pnm_magic_check(p, 3) || pnm_magic_check(p, 6) ? pnm_probe(p) : 0;
}

static int pam_probe(AVEMProbeData *p)
{
    return pnm_magic_check(p, 7) ? pnm_probe(p) : 0;
}

#define IMAGEAUTO_DEMUXER(imgname, codecid)\
static const AVEMClass imgname ## _class = {\
    .class_name = AV_STRINGIFY(imgname) " demuxer",\
    .item_name  = av_em_default_item_name,\
    .option     = ff_img_options,\
    .version    = LIBAVUTIL_VERSION_INT,\
};\
AVEMInputFormat ff_image_ ## imgname ## _pipe_demuxer = {\
