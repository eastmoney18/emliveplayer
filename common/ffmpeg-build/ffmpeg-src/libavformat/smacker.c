...
static int smacker_probe(AVEMProbeData *p)
{
...
static int smacker_read_header(AVEMFormatContext *s)
{
    ...
    smk->magic = avio_em_rl32(pb);
    if (smk->magic != MKTAG('S', 'M', 'K', '2') && smk->magic != MKTAG('S', 'M', 'K', '4'))
        return AVERROR_INVALIDDATA;
    smk->width = avio_em_rl32(pb);
    smk->height = avio_em_rl32(pb);
    smk->frames = avio_em_rl32(pb);
    smk->pts_inc = (int32_t)avio_em_rl32(pb);
    if (smk->pts_inc > INT_MAX / 100) {
        av_em_log(s, AV_LOG_ERROR, "pts_inc %d is too large\n", smk->pts_inc);
        return AVERROR_INVALIDDATA;
    }

    smk->flags = avio_em_rl32(pb);
    if(smk->flags & SMACKER_FLAG_RING_FRAME)
        smk->frames++;
    for(i = 0; i < 7; i++)
        smk->audio[i] = avio_em_rl32(pb);
    smk->treesize = avio_em_rl32(pb);

    if(smk->treesize >= UINT_MAX/4){ // smk->treesize + 16 must not overflow (this check is probably redundant)
        av_em_log(s, AV_LOG_ERROR, "treesize too large\n");
        return AVERROR_INVALIDDATA;
    }

//FIXME remove extradata "rebuilding"
    smk->mmap_size = avio_em_rl32(pb);
    smk->mclr_size = avio_em_rl32(pb);
    smk->full_size = avio_em_rl32(pb);
    smk->type_size = avio_em_rl32(pb);
    for(i = 0; i < 7; i++) {
        smk->rates[i]  = avio_em_rl24(pb);
        smk->aflags[i] = avio_em_r8(pb);
    }
    smk->pad = avio_em_rl32(pb);
    /* setup data */
    if(smk->frames > 0xFFFFFF) {
        av_em_log(s, AV_LOG_ERROR, "Too many frames: %"PRIu32"\n", smk->frames);
        return AVERROR_INVALIDDATA;
    }
    smk->frm_size = av_em_malloc_array(smk->frames, sizeof(*smk->frm_size));
    smk->frm_flags = av_em_alloc(smk->frames);
    if (!smk->frm_size || !smk->frm_flags) {
        av_em_freep(&smk->frm_size);
        av_em_freep(&smk->frm_flags);
    ...
    for(i = 0; i < smk->frames; i++) {
        smk->frm_size[i] = avio_em_rl32(pb);
    }
    for(i = 0; i < smk->frames; i++) {
        smk->frm_flags[i] = avio_em_r8(pb);
    }
    ...
    if(smk->pts_inc < 0)
        ...
        smk->pts_inc *= 100;
    tbase = 100000;
    av_em_reduce(&tbase, &smk->pts_inc, tbase, smk->pts_inc, (1UL<<31)-1);
    avpriv_em_set_pts_info(st, 33, smk->pts_inc, tbase);
    st->duration = smk->frames;
    /* handle possible audio streams */
    for(i = 0; i < 7; i++) {
        smk->indexes[i] = -1;
        if (smk->rates[i]) {
            ast[i] = avformat_em_new_stream(s, NULL);
            if (!ast[i])
                return AVERROR(ENOMEM);
            ...
            avpriv_em_set_pts_info(ast[i], 64, 1, ast[i]->codecpar->sample_rate
    ...
    if(em_alloc_extradata(st->codecpar, smk->treesize + 16)){
    ...
    ret = avio_em_read(pb, st->codecpar->extradata + 16, st->codecpar->extradata_size - 16);
    ...
    smk->curstream = -1;
    smk->nextpos = avio_em_tell(pb);

    return 0;
}


static int smacker_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    SmackerContext *smk = s->priv_data;
    int flags;
    int ret;
    ...
    if (avio_em_feof(s->pb) || smk->cur_frame >= smk->frames)
        return AVERROR_EOF;

    /* if we demuxed all streams, pass another frame */
    if(smk->curstream < 0) {
        avio_em_seek(s->pb, smk->nextpos, 0);
        ...
        if(flags & SMACKER_PAL){
            ...
            while(sz < 256){
                ...
                if(t & 0x80){ /* skip palette entries */
                    ...
                    *pal++ = smk_pal[t];
                    *pal++ = smk_pal[avio_em_r8(s->pb) & 0x3F];
                    *pal++ = smk_pal[avio_em_r8(s->pb) & 0x3F];
                    sz++;
                }
            }
            avio_em_seek(s->pb, pos, 0);
            palchange |= 1;
        }
        flags >>= 1;
        smk->curstream = -1;
        ...
        for(i = 0; i < 7; i++) {
            if(flags & 1) {
                ...
                size = avio_em_rl32(s->pb) - 4;
                if (!size || size + 4LL > frame_size) {
                    av_em_log(s, AV_LOG_ERROR, "Invalid audio part size\n");
                    return AVERROR_INVALIDDATA;
                }
                frame_size -= size;
                frame_size -= 4;
                smk->curstream++;
                if ((err = av_em_reallocp(&smk->bufs[smk->curstream], size)) < 0) {
                    smk->buf_sizes[smk->curstream] = 0;
                    return err;
                }
                smk->buf_sizes[smk->curstream] = size;
                ret = avio_em_read(s->pb, smk->bufs[smk->curstream], size);
                if(ret != size)
                    return AVERROR(EIO);
        ...
        if (frame_size < 0 || frame_size >= INT_MAX/2)
            return AVERROR_INVALIDDATA;
        if (av_em_new_packet(pkt, frame_size + 769))
            return AVERROR(ENOMEM);
        if(smk->frm_size[smk->cur_frame] & 1)
            palchange |= 2;
        pkt->data[0] = palchange;
        memcpy(pkt->data + 1, smk->pal, 768);
        ret = avio_em_read(s->pb, pkt->data + 769, frame_size);
        if(ret != frame_size)
            return AVERROR(EIO);
        ...
        pkt->size = ret + 769;
        smk->cur_frame++;
        smk->nextpos = avio_em_tell(s->pb);
    } else {
        if (smk->stream_id[smk->curstream] < 0 || !smk->bufs[smk->curstream])
            return AVERROR_INVALIDDATA;
        if (av_em_new_packet(pkt, smk->buf_sizes[smk->curstream]))
            return AVERROR(ENOMEM);
...
static int smacker_read_close(AVEMFormatContext *s)
{
    SmackerContext *smk = s->priv_data;
    int i;

    for(i = 0; i < 7; i++)
        av_em_freep(&smk->bufs[i]);
    av_em_freep(&smk->frm_size);
    av_em_freep(&smk->frm_flags);
...
AVEMInputFormat ff_smacker_demuxer = {
    .name           = "smk",
