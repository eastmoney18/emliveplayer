...
static int rpl_probe(AVEMProbeData *p)
{
...
static int read_line(AVEMIOContext * pb, char* line, int bufsize)
{
    int i;
...
static int32_t read_line_and_int(AVEMIOContext * pb, int* error)
{
...
/** Parsing for fps, which can be a fraction. Unfortunately,
  ...
  */
static AVEMRational read_fps(const char* line, int* error)
{
    int64_t num, den = 1;
    AVEMRational result;
    ...
    if (!num)
        *error = -1;
    av_em_reduce(&result.num, &result.den, num, den, 0x7FFFFFFF);
    return result;
}

static int rpl_read_header(AVEMFormatContext *s)
{
    ...
    av_em_dict_set(&s->metadata, "title"    , line, 0);
    error |= read_line(pb, line, sizeof(line));      // date/copyright
    av_em_dict_set(&s->metadata, "copyright", line, 0);
    error |= read_line(pb, line, sizeof(line));      // author and other
    av_em_dict_set(&s->metadata, "author"   , line, 0);
    ...
    avpriv_em_set_pts_info(vst, 32, fps.den, fps.num);
    ...
    switch (vst->codecpar->codec_tag) {
        ...
        case 130:
            ...
            break;
        default:
            avpriv_em_report_missing_feature(s, "Video format %i",
    ...
    if (audio_format) {
        ...
        if (ast->codecpar->codec_id == AV_CODEC_ID_NONE)
            avpriv_em_request_sample(s, "Audio format %"PRId32,
                                  audio_format);
        avpriv_em_set_pts_info(ast, 32, 1, ast->codecpar->bit_rate);
    } else {
        for (i = 0; i < 3; i++)
            error |= read_line(pb, line, sizeof(line));
    }
    ...
    // Read the index
    avio_em_seek(pb, chunk_catalog_offset, SEEK_SET);
    total_audio_size = 0;
    for (i = 0; !error && i < number_of_chunks; i++) {
        ...
        av_em_add_index_entry(vst, offset, i * rpl->frames_per_chunk,
                           video_size, rpl->frames_per_chunk, 0);
        if (ast)
            av_em_add_index_entry(ast, offset + video_size, total_audio_size,
...
static int rpl_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    RPLContext *rpl = s->priv_data;
    AVEMIOContext *pb = s->pb;
    AVEMStream* stream;
    AVEMIndexEntry* index_entry;
    int ret;
    ...
    if (rpl->frame_in_part == 0)
        if (avio_em_seek(pb, index_entry->pos, SEEK_SET) < 0)
            return AVERROR(EIO);
    ...
    if (stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO &&
        ...
        frame_size = avio_em_rl32(pb);
        if (avio_em_seek(pb, -8, SEEK_CUR) < 0)
            return AVERROR(EIO);

        ret = av_em_get_packet(pb, pkt, frame_size);
        if (ret < 0)
            return ret;
        if (ret != frame_size) {
        ...
        ret = av_em_get_packet(pb, pkt, index_entry->size);
        if (ret < 0)
            return ret;
...
AVEMInputFormat ff_rpl_demuxer = {
    .name           = "rpl",
