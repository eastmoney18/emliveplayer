...
#if CONFIG_AMR_MUXER
static int amr_write_header(AVEMFormatContext *s)
{
    ...
    if (par->codec_id == AV_CODEC_ID_AMR_NB) {
        avio_em_write(pb, AMR_header,   sizeof(AMR_header)   - 1); /* magic number */
    } else if (par->codec_id == AV_CODEC_ID_AMR_WB) {
        avio_em_write(pb, AMRWB_header, sizeof(AMRWB_header) - 1); /* magic number */
    } else {
        return -1;
    }
    avio_em_flush(pb);
...
static int amr_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
...
static int amr_probe(AVEMProbeData *p)
{
...
/* amr input */
static int amr_read_header(AVEMFormatContext *s)
{
    AVEMIOContext *pb = s->pb;
    AVEMStream *st;
    uint8_t header[9];

    avio_em_read(pb, header, 6);

    st = avformat_em_new_stream(s, NULL);
    if (!st)
        return AVERROR(ENOMEM);
    if (memcmp(header, AMR_header, 6)) {
        avio_em_read(pb, header + 6, 3);
...
static int amr_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    AVEMCodecParameters *par = s->streams[0]->codecpar;
    ...
    if (!size || av_em_new_packet(pkt, size))
        return AVERROR(EIO);
    ...
    read              = avio_em_read(s->pb, pkt->data + 1, size - 1);
...
AVEMInputFormat ff_amr_demuxer = {
    .name           = "amr",
