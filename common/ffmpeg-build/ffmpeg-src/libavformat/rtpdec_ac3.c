...
static int ac3_handle_packet(AVEMFormatContext *ctx, PayloadContext *data,
                             ...
                             int flags)
{
    ...
    case 0: /* One or more complete frames */
        if (!nr_frames) {
            av_em_log(ctx, AV_LOG_ERROR, "Invalid AC3 packet data\n");
            return AVERROR_INVALIDDATA;
        }
        if (av_em_new_packet(pkt, len)) {
    ...
    if (data->last_frame != data->nr_frames) {
        emio_free_dyn_buf(&data->fragment);
        av_em_log(ctx, AV_LOG_ERROR, "Missed %d packets\n",
