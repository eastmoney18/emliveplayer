...
static int gsm_probe(AVEMProbeData *p)
{
...
static int gsm_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int ret, size;
...
static int gsm_read_header(AVEMFormatContext *s)
{
    ...
    avpriv_em_set_pts_info(st, 64, GSM_BLOCK_SAMPLES, GSM_SAMPLE_RATE);
...
static const AVEMClass gsm_class = {
...
AVEMInputFormat ff_gsm_demuxer = {
    .name           = "gsm",
