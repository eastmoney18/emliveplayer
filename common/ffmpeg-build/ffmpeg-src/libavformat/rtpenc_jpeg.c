...
void ff_rtp_send_jpeg(AVEMFormatContext *s1, const uint8_t *buf, int size)
{
    ...
    if (s1->streams[0]->codecpar->format == AV_PIX_FMT_YUVJ422P ||
        ...
        av_em_log(s1, AV_LOG_ERROR, "Unsupported pixel format\n");
        return;
    }
    ...
    for (i = 0; i < size; i++) {
        ...
        if (buf[i + 1] == DQT) {
            int tables, j;
            if (buf[i + 4] & 0xF0)
                av_em_log(s1, AV_LOG_WARNING,
                       "Only 8-bit precision is supported.\n");

            /* a quantization table is 64 bytes long */
            tables = AV_RB16(&buf[i + 2]) / 65;
            if (i + 5 + tables * 65 > size) {
                av_em_log(s1, AV_LOG_ERROR, "Too short JPEG header. Aborted!\n");
                return;
            }
            if (nb_qtables + tables > 4) {
                av_em_log(s1, AV_LOG_ERROR, "Invalid number of quantisation tables\n");
                return;
            }
