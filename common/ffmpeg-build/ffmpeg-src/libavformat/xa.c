...
static int xa_probe(AVEMProbeData *p)
{
...
static int xa_read_header(AVEMFormatContext *s)
{
    ...
    avio_em_skip(pb, 4);       /* Skip the XA ID */
    xa->out_size            =  avio_em_rl32(pb);
    avio_em_skip(pb, 2);       /* Skip the tag */
    st->codecpar->channels     = avio_em_rl16(pb);
    st->codecpar->sample_rate  = avio_em_rl32(pb);
    avio_em_skip(pb, 4);       /* Skip average byte rate */
    avio_em_skip(pb, 2);       /* Skip block align */
    avio_em_skip(pb, 2);       /* Skip bits-per-sample */
...
static int xa_read_packet(AVEMFormatContext *s,
                          AVEMPacket *pkt)
{
    MaxisXADemuxContext *xa = s->priv_data;
    ...
    ret = av_em_get_packet(pb, pkt, packet_size);
    if(ret < 0)
        return ret;
