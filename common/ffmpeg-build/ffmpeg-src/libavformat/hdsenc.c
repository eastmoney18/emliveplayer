...
static int parse_header(OutputStream *os, const uint8_t *buf, int buf_size)
{
    ...
    while (buf_size >= 11 + 4) {
        ...
        if (type == 8 || type == 9) {
            ...
            os->extra_packets[os->nb_extra_packets] = av_em_alloc(size);
            ...
            os->metadata      = av_em_alloc(os->metadata_size);
            if (!os->metadata)
                return AVERROR(ENOMEM);
...
static void hds_free(AVEMFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        av_em_freep(&os->metadata);
        for (j = 0; j < os->nb_extra_packets; j++)
            av_em_freep(&os->extra_packets[j]);
...
static int write_manifest(AVEMFormatContext *s, int final)
{
    ...
    if (c->nb_streams > 0)
        duration = c->streams[0].last_ts * av_em_q2d(s->streams[0]->time_base);
    ...
    avio_em_printf(out, "<manifest xmlns=\"http://ns.adobe.com/f4m/1.0\">\n");
    avio_em_printf(out, "\t<id>%s</id>\n", av_em_basename(s->filename));
    avio_em_printf(out, "\t<streamType>%s</streamType>\n",
                     final ? "recorded" : "live");
    avio_em_printf(out, "\t<deliveryType>streaming</deliveryType>\n");
    if (final)
        avio_em_printf(out, "\t<duration>%f</duration>\n", duration);
    for (i = 0; i < c->nb_streams; i++) {
        OutputStream *os = &c->streams[i];
        int b64_size = AV_BASE64_SIZE(os->metadata_size);
        char *base64 = av_em_alloc(b64_size);
        if (!base64) {
            em_format_io_close(s, &out);
            return AVERROR(ENOMEM);
        }
        av_em_base64_encode(base64, b64_size, os->metadata, os->metadata_size);

        avio_em_printf(out, "\t<bootstrapInfo profile=\"named\" url=\"stream%d.abst\" id=\"bootstrap%d\" />\n", i, i);
        avio_em_printf(out, "\t<media bitrate=\"%d\" url=\"stream%d\" bootstrapInfoId=\"bootstrap%d\">\n", os->bitrate/1000, i, i);
        avio_em_printf(out, "\t\t<metadata>%s</metadata>\n", base64);
        avio_em_printf(out, "\t</media>\n");
        av_em_free(base64);
    }
    avio_em_printf(out, "</manifest>\n");
    avio_em_flush(out);
    em_format_io_close(s, &out);
    return em_rename(temp_filename, filename, s);
}

static void update_size(AVEMIOContext *out, int64_t pos)
{
    int64_t end = avio_em_tell(out);
    avio_em_seek(out, pos, SEEK_SET);
    avio_em_wb32(out, end - pos);
    avio_em_seek(out, end, SEEK_SET);
}
...
static int write_abst(AVEMFormatContext *s, OutputStream *os, int final)
{
    HDSContext *c = s->priv_data;
    AVEMIOContext *out;
    char filename[1024], temp_filename[1024];
    ...
    avio_em_wb32(out, 0); // abst size
    avio_em_wl32(out, MKTAG('a','b','s','t'));
    avio_em_wb32(out, 0); // version + flags
    avio_em_wb32(out, os->fragment_index - 1); // BootstrapinfoVersion
    avio_em_w8(out, final ? 0 : 0x20); // profile, live, update
    avio_em_wb32(out, 1000); // timescale
    avio_em_wb64(out, cur_media_time);
    avio_em_wb64(out, 0); // SmpteTimeCodeOffset
    avio_em_w8(out, 0); // MovieIdentifer (null string)
    avio_em_w8(out, 0); // ServerEntryCount
    avio_em_w8(out, 0); // QualityEntryCount
    avio_em_w8(out, 0); // DrmData (null string)
    avio_em_w8(out, 0); // MetaData (null string)
    avio_em_w8(out, 1); // SegmentRunTableCount
    asrt_pos = avio_em_tell(out);
    avio_em_wb32(out, 0); // asrt size
    avio_em_wl32(out, MKTAG('a','s','r','t'));
    avio_em_wb32(out, 0); // version + flags
    avio_em_w8(out, 0); // QualityEntryCount
    avio_em_wb32(out, 1); // SegmentRunEntryCount
    avio_em_wb32(out, 1); // FirstSegment
    avio_em_wb32(out, final ? (os->fragment_index - 1) : 0xffffffff); // FragmentsPerSegment
    update_size(out, asrt_pos);
    avio_em_w8(out, 1); // FragmentRunTableCount
    afrt_pos = avio_em_tell(out);
    avio_em_wb32(out, 0); // afrt size
    avio_em_wl32(out, MKTAG('a','f','r','t'));
    avio_em_wb32(out, 0); // version + flags
    avio_em_wb32(out, 1000); // timescale
    avio_em_w8(out, 0); // QualityEntryCount
    avio_em_wb32(out, fragments); // FragmentRunEntryCount
    for (i = start; i < os->nb_fragments; i++) {
        avio_em_wb32(out, os->fragments[i]->n);
        avio_em_wb64(out, os->fragments[i]->start_time);
        avio_em_wb32(out, os->fragments[i]->duration);
    }
...
static int init_file(AVEMFormatContext *s, OutputStream *os, int64_t start_ts)
{
    int ret, i;
    ret = s->io_open(s, &os->out, os->temp_filename, AVIO_FLAG_WRITE, NULL);
    if (ret < 0)
        return ret;
    avio_em_wb32(os->out, 0);
    avio_em_wl32(os->out, MKTAG('m','d','a','t'));
    for (i = 0; i < os->nb_extra_packets; i++) {
        AV_WB24(os->extra_packets[i] + 4, start_ts);
        os->extra_packets[i][7] = (start_ts >> 24) & 0x7f;
        avio_em_write(os->out, os->extra_packets[i], os->extra_packet_sizes[i]);
    }
    return 0;
}

static void close_file(AVEMFormatContext *s, OutputStream *os)
{
    int64_t pos = avio_em_tell(os->out);
    avio_em_seek(os->out, 0, SEEK_SET);
    avio_em_wb32(os->out, pos);
    avio_em_flush(os->out);
    em_format_io_close(s, &os->out);
}

static int hds_write_header(AVEMFormatContext *s)
{
    HDSContext *c = s->priv_data;
    int ret = 0, i;
    ...
    if (mkdir(s->filename, 0777) == -1 && errno != EEXIST) {
        ret = AVERROR(errno);
        av_em_log(s, AV_LOG_ERROR , "Failed to create directory %s\n", s->filename);
        goto fail;
    }

    oformat = av_em_guess_format("flv", NULL, NULL);
    if (!oformat) {
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            av_em_log(s, AV_LOG_ERROR, "Unsupported stream type in stream %d\n", i);
            ret = AVERROR(EINVAL);
            goto fail;
        }
        ...
        if (!os->ctx) {
            ...
            ctx->pb = avio_em_alloc_context(os->iobuf, sizeof(os->iobuf),
                                         AVIO_FLAG_WRITE, os,
                                         NULL, hds_write, NULL);
            if (!ctx->pb) {
                ret = AVERROR(ENOMEM);
    ...
    for (i = 0; i < c->nb_streams; i++) {
        OutputStream *os = &c->streams[i];
        int j;
        if ((ret = avformat_em_write_header(os->ctx, NULL)) < 0) {
             goto fail;
        }
        os->ctx_inited = 1;
                        ...
                        int64_t start_time, int64_t duration)
{
    ...
    if (os->nb_fragments >= os->fragments_size) {
        ...
        if ((ret = av_em_reallocp_array(&os->fragments, os->fragments_size,
...
static int hds_flush(AVEMFormatContext *s, OutputStream *os, int final,
                     int64_t end_ts)
{
    HDSContext *c = s->priv_data;
    int i, ret = 0;
    ...
    ret = em_rename(os->temp_filename, target_filename, s);
    if (ret < 0)
        return ret;
...
static int hds_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    HDSContext *c = s->priv_data;
    ...
    os->packets_written++;
    return em_write_chained(os->ctx, pkt->stream_index - os->first_stream, pkt, s, 0);
}

static int hds_write_trailer(AVEMFormatContext *s)
{
    HDSContext *c = s->priv_data;
    int i;
...
static const AVEMClass hds_class = {
