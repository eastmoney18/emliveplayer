...
const AVEMCodecTag em_mp4_obj_type[] = {
...
const AVEMCodecTag em_codec_movvideo_tags[] = {
...
const AVEMCodecTag em_codec_movaudio_tags[] = {
...
const AVEMCodecTag em_codec_movsubtitle_tags[] = {
...
int em_mov_iso639_to_lang(const char lang[4], int mp4)
{
    int i, code = 0;
...
int em_mov_lang_to_iso639(unsigned code, char to[4])
{
    int i;
    memset(to, 0, 4);
...
int em_mp4_read_descr_len(AVEMIOContext *pb)
{
    int len = 0;
    int count = 4;
    while (count--) {
...
int em_mp4_read_descr(AVEMFormatContext *fc, AVEMIOContext *pb, int *tag)
{
    int len;
    *tag = avio_em_r8(pb);
    len = em_mp4_read_descr_len(pb);
    av_em_log(fc, AV_LOG_TRACE, "MPEG-4 description: tag=0x%02x len=%d\n", *tag, len);
    return len;
}

void em_mp4_parse_es_descr(AVEMIOContext *pb, int *es_id)
{
     int flags;
     if (es_id) *es_id = avio_em_rb16(pb);
     else                avio_em_rb16(pb);
     flags = avio_em_r8(pb);
     if (flags & 0x80) //streamDependenceFlag
         avio_em_rb16(pb);
     if (flags & 0x40) { //URL_Flag
         int len = avio_em_r8(pb);
         avio_em_skip(pb, len);
     }
...
static const AVEMCodecTag mp4_audio_types[] = {
...
int em_mp4_read_dec_config_descr(AVEMFormatContext *fc, AVEMStream *st, AVEMIOContext *pb)
{
    enum AVEMCodecID codec_id;
    unsigned v;
    int len, tag;
    int ret;
    int object_type_id = avio_em_r8(pb);
    avio_em_r8(pb); /* stream type */
    avio_em_rb24(pb); /* buffer size db */
    ...
    st->codecpar->bit_rate = avio_em_rb32(pb); /* avg bitrate */

    codec_id= em_codec_get_id(em_mp4_obj_type, object_type_id);
    if (codec_id)
        st->codecpar->codec_id = codec_id;
    av_em_log(fc, AV_LOG_TRACE, "esds object type id 0x%02x\n", object_type_id);
    len = em_mp4_read_descr(fc, pb, &tag);
    if (tag == MP4DecSpecificDescrTag) {
        av_em_log(fc, AV_LOG_TRACE, "Specific MPEG-4 header len=%d\n", len);
        if (!len || (uint64_t)len > (1<<30))
            return -1;
        av_em_free(st->codecpar->extradata);
        if ((ret = em_get_extradata(fc, st->codecpar, pb, len)) < 0)
            return ret;
        if (st->codecpar->codec_id == AV_CODEC_ID_AAC) {
            ...
            av_em_log(fc, AV_LOG_TRACE, "mp4a config channels %d obj %d ext obj %d "
                    "sample rate %d ext sample rate %d\n", st->codecpar->channels,
                    cfg.object_type, cfg.ext_object_type,
                    cfg.sample_rate, cfg.ext_sample_rate);
            if (!(st->codecpar->codec_id = em_codec_get_id(mp4_audio_types,
                                                        cfg.object_type)))
...
#if 0
int em_mov_read_chan(AVEMFormatContext *s, AVEMStream *st, int64_t size)
{
    ...
    if (!codec->channel_layout)
        av_em_log(s, AV_LOG_WARNING, "Unknown container channel layout.\n");
    avio_em_skip(pb, size);
...
void em_mov_write_chan(AVEMIOContext *pb, int64_t channel_layout)
{
    ...
    if (layout_tag) {
        ...
        avio_em_wb32(pb, 0);          // mChannelBitmap
    } else {
        avio_em_wb32(pb, 0x10000);    // kCAFChannelLayoutTag_UseChannelBitmap
        avio_em_wb32(pb, channel_layout);
    }
