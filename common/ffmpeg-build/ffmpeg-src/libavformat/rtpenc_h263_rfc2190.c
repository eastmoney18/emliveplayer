...
static void send_mode_a(AVEMFormatContext *s1, const struct H263Info *info,
...
static void send_mode_b(AVEMFormatContext *s1, const struct H263Info *info,
...
void ff_rtp_send_h263_rfc2190(AVEMFormatContext *s1, const uint8_t *buf, int size,
                              const uint8_t *mb_info, int mb_info_size)
{
    ...
    while (size > 0) {
        ...
        if (len < size) {
            const uint8_t *end = em_h263_find_resync_marker_reverse(buf,
                                                                    buf + len);
            len = end - buf;
            if (len == s->max_payload_size - 8) {
                ...
                if (mb_info_pos < mb_info_count) {
                    ...
                    av_em_log(s1, AV_LOG_ERROR, "Unable to split H.263 packet, "
