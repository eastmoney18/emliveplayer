...
static int srt_write_header(AVEMFormatContext *avf)
{
    ...
    if (avf->streams[0]->codecpar->codec_id != AV_CODEC_ID_TEXT &&
        ...
        av_em_log(avf, AV_LOG_ERROR,
               ...
               avcodec_em_get_name(avf->streams[0]->codecpar->codec_id));
        return AVERROR(EINVAL);
    }
    avpriv_em_set_pts_info(avf->streams[0], 64, 1, 1000);
    srt->index = 1;
    return 0;
}

static int srt_write_packet(AVEMFormatContext *avf, AVEMPacket *pkt)
{
    SRTContext *srt = avf->priv_data;
    ...
    p = av_em_packet_get_side_data(pkt, AV_PKT_DATA_SUBTITLE_POSITION, &size);
    if (p && size == 16) {
        x1 = AV_RL32(p     );
        y1 = AV_RL32(p +  4);
        x2 = AV_RL32(p +  8);
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (p)
        avio_em_printf(avf->pb, "  X1:%03d X2:%03d Y1:%03d Y2:%03d",
                    x1, x2, y1, y2);
    avio_em_printf(avf->pb, "\n");
