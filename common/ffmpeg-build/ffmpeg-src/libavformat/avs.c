...
static int avs_probe(AVEMProbeData * p)
{
    const uint8_t *d;
...
static int avs_read_header(AVEMFormatContext * s)
{
    ...
    avio_em_skip(s->pb, 4);
    avs->width = avio_em_rl16(s->pb);
    avs->height = avio_em_rl16(s->pb);
    avs->bits_per_sample = avio_em_rl16(s->pb);
    avs->fps = avio_em_rl16(s->pb);
    avs->nb_frames = avio_em_rl32(s->pb);
    ...
    if (avs->width != 318 || avs->height != 198)
        av_em_log(s, AV_LOG_ERROR, "This avs pretend to be %dx%d "
...
static int
avs_read_video_packet(AVEMFormatContext * s, AVEMPacket * pkt,
                      ...
                      uint8_t * palette, int palette_size)
{
    ...
    ret = av_em_new_packet(pkt, size + palette_size);
    if (ret < 0)
        return ret;
    ...
    ret = avio_em_read(s->pb, pkt->data + palette_size + 4, size - 4) + 4;
    if (ret < size) {
...
static int avs_read_audio_packet(AVEMFormatContext * s, AVEMPacket * pkt)
{
    AvsFormat *avs = s->priv_data;
    int ret, size;

    size = avio_em_tell(s->pb);
    ret = ff_voc_get_packet(s, pkt, avs->st_audio, avs->remaining_audio_size);
    size = avio_em_tell(s->pb) - size;
...
static int avs_read_packet(AVEMFormatContext * s, AVEMPacket * pkt)
{
    AvsFormat *avs = s->priv_data;
    ...
    while (1) {
        if (avs->remaining_frame_size <= 0) {
            if (!avio_em_rl16(s->pb))    /* found EOF */
                return AVERROR(EIO);
            avs->remaining_frame_size = avio_em_rl16(s->pb) - 4;
        }

        while (avs->remaining_frame_size > 0) {
            sub_type = avio_em_r8(s->pb);
            type = avio_em_r8(s->pb);
            size = avio_em_rl16(s->pb);
            if (size < 4)
            ...
            case AVS_PALETTE:
                if (size - 4 > sizeof(palette))
                    return AVERROR_INVALIDDATA;
                ret = avio_em_read(s->pb, palette, size - 4);
                if (ret < size - 4)
                    return AVERROR(EIO);
                palette_size = size;
                break;
            ...
            case AVS_VIDEO:
                if (!avs->st_video) {
                    avs->st_video = avformat_em_new_stream(s, NULL);
                    if (!avs->st_video)
...
#endif
                    avs->st_video->avg_frame_rate = (AVEMRational){avs->fps, 1};
                }
            ...
            case AVS_AUDIO:
                if (!avs->st_audio) {
                    avs->st_audio = avformat_em_new_stream(s, NULL);
                    if (!avs->st_audio)
                        return AVERROR(ENOMEM);
            ...
            default:
                avio_em_skip(s->pb, size - 4);
            }
        }
    }
}

static int avs_read_close(AVEMFormatContext * s)
{
    return 0;
}

AVEMInputFormat ff_avs_demuxer = {
    .name           = "avs",
