...
static av_cold int vp9_init(AVEMFormatContext *ctx, int st_index,
                            PayloadContext *data)
{
...
static int vp9_handle_packet(AVEMFormatContext *ctx, PayloadContext *rtp_vp9_ctx,
                             ...
                             int flags)
{
    ...
    if (rtp_vp9_ctx->buf && rtp_vp9_ctx->timestamp != *timestamp)
        emio_free_dyn_buf(&rtp_vp9_ctx->buf);

    /* sanity check for size of input packet: 1 byte payload at least */
    if (len < RTP_VP9_DESC_REQUIRED_SIZE + 1) {
        av_em_log(ctx, AV_LOG_ERROR, "Too short RTP/VP9 packet, got %d bytes\n", len);
    ...
    if (last_fragment != rtp_m) {
        av_em_log(ctx, AV_LOG_ERROR, "Invalid combination of B and M marker (%d != %d)\n", last_fragment, rtp_m);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (has_pic_id) {
        ...
        if (buf[0] & 0x80) {
            if (len < 2) {
                av_em_log(ctx, AV_LOG_ERROR, "Too short RTP/VP9 packet\n");
                return AVERROR_INVALIDDATA;
            }
    ...
    if (has_ss_data) {
        ...
        len--;
        if (n_s > 0) {
            avpriv_em_report_missing_feature(ctx, "VP9 scalability structure with multiple layers");
    ...
    if (!rtp_vp9_ctx->buf) {
        ...
        if (first_fragment) {
            res = avio_em_open_dyn_buf(&rtp_vp9_ctx->buf);
            if (res < 0)
                return res;
    ...
    avio_em_write(rtp_vp9_ctx->buf, buf, len);
