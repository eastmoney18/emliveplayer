...
uint64_t em_mov_get_channel_layout(uint32_t tag, uint32_t bitmap)
{
    int i, channels;
    const struct MovChannelLayoutMap *layout_map;

    /* use em_mov_get_channel_label() to build a layout instead */
...
uint32_t em_mov_get_channel_layout_tag(enum AVEMCodecID codec_id,
                                       ...
                                       uint32_t *bitmap)
{
    ...
    if (layouts) {
        ...
        channels = av_em_get_channel_layout_nb_channels(channel_layout);
        if (channels > 9)
            channels = 0;
        layout_map = mov_ch_layout_map[channels];
...
int em_mov_read_chan(AVEMFormatContext *s, AVEMIOContext *pb, AVEMStream *st,
                     int64_t size)
{
    ...
    bitmap     = avio_em_rb32(pb);
    num_descr  = avio_em_rb32(pb);

    av_em_log(s, AV_LOG_TRACE, "chan: layout=%u bitmap=%u num_descr=%u\n",
    ...
    for (i = 0; i < num_descr; i++) {
        ...
        label     = avio_em_rb32(pb);          // mChannelLabel
        avio_em_rb32(pb);                      // mChannelFlags
        avio_em_rl32(pb);                      // mCoordinates[0]
        avio_em_rl32(pb);                      // mCoordinates[1]
        avio_em_rl32(pb);                      // mCoordinates[2]
        size -= 20;
        if (layout_tag == 0) {
    ...
    if (layout_tag == 0) {
        ...
        st->codecpar->channel_layout = em_mov_get_channel_layout(layout_tag, bitmap);
    avio_em_skip(pb, size - 12);
