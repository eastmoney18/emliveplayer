...
/* If you add a tag here that is not in em_codec_bmp_tags[]
   or em_codec_wav_tags[], add it also to additional_audio_tags[]
...
const CodecEMMime ff_mkv_image_mime_tags[] = {
...
const CodecEMMime ff_mkv_mime_tags[] = {
...
int ff_mkv_stereo3d_conv(AVEMStream *st, MatroskaVideoStereoModeType stereo_mode)
{
    AVEMPacketSideData *sd, *tmp;
    AVStereo3D *stereo;

    stereo = av_em_stereo3d_alloc();
    if (!stereo)
