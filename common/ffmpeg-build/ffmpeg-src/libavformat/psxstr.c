...
static int str_probe(AVEMProbeData *p)
{
...
static int str_read_header(AVEMFormatContext *s)
{
    ...
    if (avio_em_read(pb, sector, RIFF_HEADER_SIZE) != RIFF_HEADER_SIZE)
        return AVERROR(EIO);
...
static int str_read_packet(AVEMFormatContext *s,
                           AVEMPacket *ret_pkt)
{
    AVEMIOContext *pb = s->pb;
    StrDemuxContext *str = s->priv_data;
    ...
    while (1) {
        ...
        if (avio_em_read(pb, sector, RAW_CD_SECTOR_SIZE) != RAW_CD_SECTOR_SIZE)
            return AVERROR(EIO);
        ...
        case CDXA_TYPE_VIDEO:
            {
                ...
                if(!(   frame_size>=0
                     && current_sector < sector_count
                     && sector_count*VIDEO_DATA_CHUNK_SIZE >=frame_size)){
                    av_em_log(s, AV_LOG_ERROR, "Invalid parameters %d %d %d\n", current_sector, sector_count, frame_size);
                    break;
                }
                ...
                if(str->channels[channel].video_stream_index < 0){
                    ...
                    avpriv_em_set_pts_info(st, 64, 1, 15);
                ...
                if(pkt->size != sector_count*VIDEO_DATA_CHUNK_SIZE){
                    if(pkt->data)
                        av_em_log(s, AV_LOG_ERROR, "missmatching sector_count\n");
                    av_em_packet_unref(pkt);
                    if (av_em_new_packet(pkt, sector_count*VIDEO_DATA_CHUNK_SIZE))
                        return AVERROR(EIO);
                    memset(pkt->data, 0, sector_count*VIDEO_DATA_CHUNK_SIZE);

                    pkt->pos= avio_em_tell(pb) - RAW_CD_SECTOR_SIZE;
                    pkt->stream_index =
        ...
        case CDXA_TYPE_AUDIO:
            ...
            //    st->codecpar->bit_rate = 0; //FIXME;
                ...
                avpriv_em_set_pts_info(st, 64, 18 * 224 / st->codecpar->channels,
                                    st->codecpar->sample_rate);
                st->start_time = 0;
            }
            pkt = ret_pkt;
            if (av_em_new_packet(pkt, 2304))
                return AVERROR(EIO);
            ...
            return 0;
        default:
            av_em_log(s, AV_LOG_WARNING, "Unknown sector type %02X\n", sector[0x12]);
...
static int str_read_close(AVEMFormatContext *s)
{
    StrDemuxContext *str = s->priv_data;
    int i;
    for(i=0; i<32; i++){
        if(str->channels[i].tmp_pkt.data)
            av_em_packet_unref(&str->channels[i].tmp_pkt);
    }

    return 0;
}

AVEMInputFormat ff_str_demuxer = {
    .name           = "psxstr",
