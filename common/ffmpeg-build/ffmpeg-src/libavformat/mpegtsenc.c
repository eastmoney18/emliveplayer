...
static void mpegts_write_section(MpegTSSection *s, uint8_t *buf, int len)
{
    ...
    crc = av_bswap32(av_em_crc(av_em_crc_get_table(AV_CRC_32_IEEE),
                            -1, buf, len - 4));
...
typedef struct MpegTSWriteStream {
    ...
    int payload_flags;
    uint8_t *payload;
    AVEMFormatContext *amux;
    AVEMRational user_tb;
...
static void mpegts_write_pat(AVEMFormatContext *s)
{
...
static int mpegts_write_pmt(AVEMFormatContext *s, MpegTSService *service)
{
    MpegTSWrite *ts = s->priv_data;
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (s->nb_programs) {
            int k, found = 0;
            AVEMProgram *program = service->program;
                ...
                *q++ = 0x80;
                ...
                if (st->codecpar->extradata && st->codecpar->extradata_size >= 19) {
                        ...
                        *q++ = st->codecpar->channels;
                        ...
                        if (st->codecpar->extradata[19] == st->codecpar->channels - coupled_stream_counts[st->codecpar->channels] &&
                            ...
                            *q++ = st->codecpar->channels | 0x80;
                            ...
                            av_em_log(s, AV_LOG_ERROR, "Unsupported Opus Vorbis-style channel mapping");
                            *q++ = 0xff;
                        }
                    } else {
                        /* Unsupported */
                        av_em_log(s, AV_LOG_ERROR, "Unsupported Opus channel mapping for family %d", st->codecpar->extradata[18]);
                        *q++ = 0xff;
                    }
                    ...
                    *q++ = st->codecpar->channels;
                    ...
                    /* Unsupported */
                    av_em_log(s, AV_LOG_ERROR, "Unsupported Opus channel mapping");
                    *q++ = 0xff;
                }
            }
...
static void mpegts_write_sdt(AVEMFormatContext *s)
{
    MpegTSWrite *ts = s->priv_data;
    MpegTSService *service;
                                         ...
                                         const char *name)
{
    ...
    service = av_em_mallocz(sizeof(MpegTSService));
    if (!service)
        return NULL;
    service->pmt.pid       = ts->pmt_start_pid + ts->nb_services;
    service->sid           = sid;
    service->pcr_pid       = 0x1fff;
    service->provider_name = av_em_strdup(provider_name);
    service->name          = av_em_strdup(name);
    if (!service->provider_name || !service->name)
        goto fail;
    if (av_em_dynarray_add_nofree(&ts->services, &ts->nb_services, service) < 0)
        goto fail;

    return service;
fail:
    av_em_freep(&service->provider_name);
    av_em_freep(&service->name);
    av_em_free(service);
    return NULL;
}

static int64_t get_pcr(const MpegTSWrite *ts, AVEMIOContext *pb)
{
    return av_em_rescale(avio_em_tell(pb) + 11, 8 * PCR_TIME_BASE, ts->mux_rate) +
           ts->first_pcr;
}

static void mpegts_prefix_m2ts_header(AVEMFormatContext *s)
{
    MpegTSWrite *ts = s->priv_data;
    if (ts->m2ts_mode) {
        ...
        avio_em_write(s->pb, (unsigned char *) &tp_extra_header,
...
{
    AVEMFormatContext *ctx = s->opaque;
    mpegts_prefix_m2ts_header(ctx);
    avio_em_write(ctx->pb, packet, TS_PACKET_SIZE);
}

static int mpegts_init(AVEMFormatContext *s)
{
    MpegTSWrite *ts = s->priv_data;
    MpegTSWriteStream *ts_st;
    MpegTSService *service;
    AVEMStream *st, *pcr_st = NULL;
    AVEMDictionaryEntry *title, *provider;
    int i, j;
    ...
    if (!s->nb_programs) {
        ...
        title = av_em_dict_get(s->metadata, "service_name", NULL, 0);
        if (!title)
            title = av_em_dict_get(s->metadata, "title", NULL, 0);
        service_name  = title ? title->value : DEFAULT_SERVICE_NAME;
        provider      = av_em_dict_get(s->metadata, "service_provider", NULL, 0);
        ...
        for (i = 0; i < s->nb_programs; i++) {
            AVEMProgram *program = s->programs[i];
            title = av_em_dict_get(program->metadata, "service_name", NULL, 0);
            if (!title)
                title = av_em_dict_get(program->metadata, "title", NULL, 0);
            service_name  = title ? title->value : DEFAULT_SERVICE_NAME;
            provider      = av_em_dict_get(program->metadata, "service_provider", NULL, 0);
            provider_name = provider ? provider->value : DEFAULT_PROVIDER_NAME;
    ...
    pids = av_em_malloc_array(s->nb_streams, sizeof(*pids));
    if (!pids) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        ts_st = av_em_mallocz(sizeof(MpegTSWriteStream));
        if (!ts_st) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        ...
        ts_st->payload = av_em_mallocz(ts->pes_payload_size);
        if (!ts_st->payload) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }

        program = av_em_find_program_from_stream(s, NULL, i);
        if (program) {
        ...
        if (ts_st->pid == service->pmt.pid) {
            av_em_log(s, AV_LOG_ERROR, "Duplicate stream id %d\n", ts_st->pid);
            ret = AVERROR(EINVAL);
            goto fail;
        }
        for (j = 0; j < i; j++) {
        ...
        if (st->codecpar->codec_id == AV_CODEC_ID_AAC &&
            ...
            AVEMStream *ast;
            ts_st->amux = avformat_em_alloc_context();
            if (!ts_st->amux) {
                ret = AVERROR(ENOMEM);
                goto fail;
            }
            ts_st->amux->oformat =
                av_em_guess_format((ts->flags & MPEGTS_FLAG_AAC_LATM) ? "latm" : "adts",
                                NULL, NULL);
            if (!ts_st->amux->oformat) {
                ret = AVERROR(EINVAL);
                goto fail;
            }
            if (!(ast = avformat_em_new_stream(ts_st->amux, NULL))) {
                ret = AVERROR(ENOMEM);
                goto fail;
            }
            ret = avcodec_em_parameters_copy(ast->codecpar, st->codecpar);
            if (ret != 0)
                goto fail;
            ast->time_base = st->time_base;
            ret = avformat_em_write_header(ts_st->amux, NULL);
            if (ret < 0)
                goto fail;
        }
        if (st->codecpar->codec_id == AV_CODEC_ID_OPUS) {
    ...
    if (ts->mux_rate > 1) {
        ...
        if (ts->copyts < 1)
            ts->first_pcr = av_em_rescale(s->max_delay, PCR_TIME_BASE, AV_TIME_BASE);
    } else {
        /* Arbitrary values, PAT/PMT will also be written on video key frames */
        ts->sdt_packet_period = 200;
        ts->pat_packet_period = 40;
        if (pcr_st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
            int frame_size = av_em_get_audio_frame_duration2(pcr_st->codecpar, 0);
            if (!frame_size) {
                av_em_log(s, AV_LOG_WARNING, "frame size not set\n");
    ...
    if (ts->mux_rate == 1)
        av_em_log(s, AV_LOG_VERBOSE, "muxrate VBR, ");
    else
        av_em_log(s, AV_LOG_VERBOSE, "muxrate %d, ", ts->mux_rate);
    av_em_log(s, AV_LOG_VERBOSE,
           "pcr every %d pkts, sdt every %d, pat/pmt every %d pkts\n",
           service->pcr_packet_period,
           ts->sdt_packet_period, ts->pat_packet_period);

    if (ts->m2ts_mode == -1) {
        if (av_em_match_ext(s->filename, "m2ts")) {
            ts->m2ts_mode = 1;
        } else {
            ts->m2ts_mode = 0;
        }
...
static void retransmit_si_info(AVEMFormatContext *s, int force_pat, int64_t dts)
{
    MpegTSWrite *ts = s->priv_data;
    int i;
...
static void mpegts_insert_null_packet(AVEMFormatContext *s)
{
    uint8_t *q;
    ...
    *q++ = 0x10;
    ...
    avio_em_write(s->pb, buf, TS_PACKET_SIZE);
}

/* Write a single transport stream packet with a PCR and no payload */
static void mpegts_insert_pcr_only(AVEMFormatContext *s, AVEMStream *st)
{
    MpegTSWrite *ts = s->priv_data;
    MpegTSWriteStream *ts_st = st->priv_data;
    uint8_t *q;
...
static void mpegts_write_pes(AVEMFormatContext *s, AVEMStream *st,
                             ...
                             int64_t pts, int64_t dts, int key, int stream_id)
{
    ...
    int64_t delay = av_em_rescale(s->max_delay, 90000, AV_TIME_BASE);
        ...
        *q++      = 0x10 | ts_st->cc; // payload indicator + CC
        ...
        if (write_pcr) {
            ...
            if (dts != AV_NOPTS_VALUE && dts < pcr / 300)
                av_em_log(s, AV_LOG_WARNING, "dts < pcr, TS is invalid\n");
...
int ff_check_h264_startcode(AVEMFormatContext *s, const AVEMStream *st, const AVEMPacket *pkt)
{
    if (pkt->size < 5 || AV_RB32(pkt->data) != 0x0000001 && AV_RB24(pkt->data) != 0x000001) {
        if (!st->nb_frames) {
            av_em_log(s, AV_LOG_ERROR, "H.264 bitstream malformed, "
                   "no startcode found, use the video bitstream filter 'h264_mp4toannexb' to fix it "
                   "('-bsf:v h264_mp4toannexb' option with ffmpeg)\n");
            return AVERROR_INVALIDDATA;
        }
        av_em_log(s, AV_LOG_WARNING, "H.264 bitstream error, startcode missing, size %d", pkt->size);
        if (pkt->size) av_em_log(s, AV_LOG_WARNING, " data %08X", AV_RB32(pkt->data));
        av_em_log(s, AV_LOG_WARNING, "\n");
    }
    return 0;
}

static int check_hevc_startcode(AVEMFormatContext *s, const AVEMStream *st, const AVEMPacket *pkt)
{
    if (pkt->size < 5 || AV_RB32(pkt->data) != 0x0000001 && AV_RB24(pkt->data) != 0x000001) {
        if (!st->nb_frames) {
            av_em_log(s, AV_LOG_ERROR, "HEVC bitstream malformed, no startcode found\n");
            return AVERROR_PATCHWELCOME;
        }
        av_em_log(s, AV_LOG_WARNING, "HEVC bitstream error, startcode missing, size %d", pkt->size);
        if (pkt->size) av_em_log(s, AV_LOG_WARNING, " data %08X", AV_RB32(pkt->data));
        av_em_log(s, AV_LOG_WARNING, "\n");
    }
    return 0;
 ...
 */
static int opus_get_packet_samples(AVEMFormatContext *s, AVEMPacket *pkt)
{
...
static int mpegts_write_packet_internal(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    MpegTSWrite *ts = s->priv_data;
    MpegTSWriteStream *ts_st = st->priv_data;
    const int64_t delay = av_em_rescale(s->max_delay, 90000, AV_TIME_BASE) * 2;
    ...
    if (ts_st->first_pts_check && pts == AV_NOPTS_VALUE) {
        av_em_log(s, AV_LOG_ERROR, "first pts value must be set\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_H264) {
        ...
        do {
            p = avpriv_em_find_start_code(p, buf_end, &state);
            av_em_log(s, AV_LOG_TRACE, "nal %d\n", state & 0x1f);
            if ((state & 0x1f) == 7)
                extradd = 0;
        ...
        if ((state & 0x1f) != 9) { // AUD NAL
            data = av_em_alloc(pkt->size + 6 + extradd);
            if (!data)
                return AVERROR(ENOMEM);
        ...
        if (pkt->size < 2) {
            av_em_log(s, AV_LOG_ERROR, "AAC packet too short\n");
            return AVERROR_INVALIDDATA;
        }
        if ((AV_RB16(pkt->data) & 0xfff0) != 0xfff0) {
            ...
            if (!ts_st->amux) {
                av_em_log(s, AV_LOG_ERROR, "AAC bitstream not in ADTS format "
                                        "and extradata missing\n");
            } else {
            av_em_init_packet(&pkt2);
            pkt2.data = pkt->data;
            pkt2.size = pkt->size;
            av_assert0(pkt->dts != AV_NOPTS_VALUE);
            pkt2.dts = av_em_rescale_q(pkt->dts, st->time_base, ts_st->amux->streams[0]->time_base);

            ret = avio_em_open_dyn_buf(&ts_st->amux->pb);
            if (ret < 0)
                return AVERROR(ENOMEM);

            ret = av_em_write_frame(ts_st->amux, &pkt2);
            if (ret < 0) {
                emio_free_dyn_buf(&ts_st->amux->pb);
                return ret;
            }
            size            = avio_em_close_dyn_buf(ts_st->amux->pb, &data);
            ts_st->amux->pb = NULL;
            buf             = data;
            }
        }
        ...
        if (pkt->size < 2) {
            av_em_log(s, AV_LOG_ERROR, "Opus packet too short\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        if ((AV_RB16(pkt->data) >> 5) != 0x3ff) {
            ...
            data = av_em_alloc(ctrl_header_size);
            if (!data)
                return AVERROR(ENOMEM);
            ...
            av_em_log(s, AV_LOG_WARNING, "Got MPEG-TS formatted Opus data, unhandled");
        }
    }
    ...
    if (ts_st->payload_size && (ts_st->payload_size + size > ts->pes_payload_size ||
        (dts != AV_NOPTS_VALUE && ts_st->payload_dts != AV_NOPTS_VALUE &&
         av_em_compare_ts(dts - ts_st->payload_dts, st->time_base,
...
static void mpegts_write_flush(AVEMFormatContext *s)
{
    int i;
...
static int mpegts_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    if (!pkt) {
...
static int mpegts_write_end(AVEMFormatContext *s)
{
    if (s->pb)
        mpegts_write_flush(s);
...
static void mpegts_deinit(AVEMFormatContext *s)
{
    MpegTSWrite *ts = s->priv_data;
    MpegTSService *service;
    int i;
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        MpegTSWriteStream *ts_st = st->priv_data;
        if (ts_st) {
            av_em_freep(&ts_st->payload);
            if (ts_st->amux) {
                avformat_em_free_context(ts_st->amux);
                ts_st->amux = NULL;
            }
        }
    }
    ...
    av_em_freep(&ts->services);
}

static int mpegts_check_bitstream(struct AVEMFormatContext *s, const AVEMPacket *pkt)
{
    int ret = 1;
    AVEMStream *st = s->streams[pkt->stream_index];
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_H264) {
        if (pkt->size >= 5 && AV_RB32(pkt->data) != 0x0000001 &&
            ...
            ret = em_stream_add_bitstream_filter(st, "h264_mp4toannexb", NULL);
    } else if (st->codecpar->codec_id == AV_CODEC_ID_HEVC) {
        if (pkt->size >= 5 && AV_RB32(pkt->data) != 0x0000001 &&
                              AV_RB24(pkt->data) != 0x000001)
            ret = em_stream_add_bitstream_filter(st, "hevc_mp4toannexb", NULL);
    }
...
static const AVEMClass mpegts_muxer_class = {
...
AVEMOutputFormat ff_mpegts_muxer = {
    .name           = "mpegts",
