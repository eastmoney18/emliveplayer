...
static int spdif_get_offset_and_codec(AVEMFormatContext *s,
                                      ...
                                      enum AVEMCodecID *codec)
{
    ...
    case IEC61937_MPEG2_AAC:
        ...
        if (avpriv_em_aac_parse_header(&gbc, &aac_hdr) < 0) {
            if (s) /* be silent during a probe */
                av_em_log(s, AV_LOG_ERROR, "Invalid AAC packet in IEC 61937\n");
    ...
    default:
        if (s) { /* be silent during a probe */
            avpriv_em_request_sample(s, "Data type 0x%04x in IEC 61937",
                                  data_type);
        }
...
static int spdif_probe(AVEMProbeData *p)
{
    enum AVEMCodecID codec;
    return ff_spdif_probe (p->buf, p->buf_size, &codec);
}

int ff_spdif_probe(const uint8_t *p_buf, int buf_size, enum AVEMCodecID *codec)
{
...
static int spdif_read_header(AVEMFormatContext *s)
{
    s->ctx_flags |= AVFMTCTX_NOHEADER;
    return 0;
}

int ff_spdif_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    data_type = avio_em_rl16(pb);
    pkt_size_bits = avio_em_rl16(pb);

    if (pkt_size_bits % 16)
        avpriv_em_request_sample(s, "Packet not ending at a 16-bit boundary");

    ret = av_em_new_packet(pkt, FFALIGN(pkt_size_bits, 16) >> 3);
    if (ret)
        return ret;

    pkt->pos = avio_em_tell(pb) - BURST_HEADER_SIZE;

    if (avio_em_read(pb, pkt->data, pkt->size) < pkt->size) {
    ...
    avio_em_skip(pb, offset - pkt->size - BURST_HEADER_SIZE);
    ...
    if (!s->nb_streams) {
        ...
        avpriv_em_report_missing_feature(s, "Codec change in IEC 61937");
        return AVERROR_PATCHWELCOME;
    }
...
AVEMInputFormat ff_spdif_demuxer = {
    .name           = "spdif",
