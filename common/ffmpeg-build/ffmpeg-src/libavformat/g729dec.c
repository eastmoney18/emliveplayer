...
static int g729_read_header(AVEMFormatContext *s)
{
    AVEMStream* st;
    ...
    case 8000:
        ...
        break;
    default:
        av_em_log(s, AV_LOG_ERROR, "Invalid bit_rate value %"PRId64". "
               "Only 6400 and 8000 b/s are supported.", s->bit_rate);
        return AVERROR(EINVAL);
    }

    avpriv_em_set_pts_info(st, st->codecpar->block_align << 3, 1,
                        st->codecpar->sample_rate);

    return 0;
}

static int g729_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    AVEMStream *st = s->streams[0];
    int ret = av_em_get_packet(s->pb, pkt, st->codecpar->block_align);
    if (ret < 0)
        return ret;
...
static const AVEMClass g729_demuxer_class = {
...
AVEMInputFormat ff_g729_demuxer = {
    .name           = "g729",
