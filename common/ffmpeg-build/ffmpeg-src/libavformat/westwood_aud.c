...
static int wsaud_probe(AVEMProbeData *p)
{
    int field;
...
static int wsaud_read_header(AVEMFormatContext *s)
{
    ...
    if (avio_em_read(pb, header, AUD_HEADER_SIZE) != AUD_HEADER_SIZE)
        return AVERROR(EIO);
    ...
    case  1:
        if (channels != 1) {
            avpriv_em_request_sample(s, "Stereo WS-SND1");
    ...
    case 99:
        ...
        break;
    default:
        avpriv_em_request_sample(s, "Unknown codec: %d", codec);
        return AVERROR_PATCHWELCOME;
    }
    avpriv_em_set_pts_info(st, 64, 1, sample_rate);
...
static int wsaud_read_packet(AVEMFormatContext *s,
                             AVEMPacket *pkt)
{
    AVEMIOContext *pb = s->pb;
    ...
    if (avio_em_read(pb, preamble, AUD_CHUNK_PREAMBLE_SIZE) !=
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_WESTWOOD_SND1) {
        ...
        if ((ret = av_em_new_packet(pkt, chunk_size + 4)) < 0)
            return ret;
        if ((ret = avio_em_read(pb, &pkt->data[4], chunk_size)) != chunk_size)
...
AVEMInputFormat ff_wsaud_demuxer = {
    .name           = "wsaud",
