...
static int get_line(AVEMIOContext *pb, char *line, int line_size)
{
    em_get_line(pb, line, line_size);
...
static int parse_multipart_header(AVEMIOContext *pb,
                                    int* size,
                                    const char* expected_boundary,
                                    void *log_ctx);

static int mpjpeg_read_close(AVEMFormatContext *s)
{
    MPJPEGDemuxContext *mpjpeg = s->priv_data;
    av_em_freep(&mpjpeg->boundary);
    av_em_freep(&mpjpeg->searchstr);
    return 0;
}

static int mpjpeg_read_probe(AVEMProbeData *p)
{
    AVEMIOContext *pb;
    int ret = 0;
    int size = 0;
    ...
    pb = avio_em_alloc_context(p->buf, p->buf_size, 0, NULL, NULL, NULL, NULL);
    if (!pb)
        return 0;
...
static int mpjpeg_read_header(AVEMFormatContext *s)
{
    AVEMStream *st;
    ...
    avpriv_em_set_pts_info(st, 60, 1, 25);
                            ...
                            void *log_ctx)
{
    ...
    *size = -1;
    ...
    if (!av_em_strstart(line, expected_boundary, NULL)) {
        if (log_ctx)
        av_em_log(log_ctx,
            AV_LOG_ERROR,
    ...
    while (!pb->eof_reached) {
        ...
        if (!av_em_strcasecmp(tag, "Content-type")) {
            if (av_em_strcasecmp(value, "image/jpeg")) {
                if (log_ctx)
        ...
        } else if (!av_em_strcasecmp(tag, "Content-Length")) {
...
static char* mpjpeg_get_boundary(AVEMIOContext* pb)
{
    ...
    av_em_opt_get(pb, "mime_type", AV_OPT_SEARCH_CHILDREN, &mime_type);
    start = mime_type;
    while (start != NULL && *start != '\0') {
        ...
        if (!av_em_stristart(start, "boundary=", &start)) {
            end = strchr(start, ';');
            if (end)
                len = end - start - 1;
            else
...
static int mpjpeg_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int size;
    int ret;
    ...
    if (mpjpeg->boundary == NULL) {
        ...
        if (boundary != NULL) {
            ...
            mpjpeg->searchstr = av_em_asprintf( "\r\n%s\r\n", boundary );
        } else {
            mpjpeg->boundary = av_em_strdup("--");
            mpjpeg->searchstr = av_em_strdup("\r\n--");
        }
    ...
    if (size > 0) {
        ...
        emio_ensure_seekback(s->pb, read_chunk);

        while ((ret = av_em_append_packet(s->pb, pkt, read_chunk - remaining)) >= 0) {
            /* scan the new data */
            char *start;
            ...
            do {
                if (!memcmp(start, mpjpeg->searchstr, mpjpeg->searchstr_len)) {
                    ...
                    avio_em_seek(s->pb, -len, SEEK_CUR);
                    pkt->size -= len;
                    return pkt->size;
                }
                len--;
...
static const AVEMClass mpjpeg_demuxer_class = {
...
AVEMInputFormat ff_mpjpeg_demuxer = {
    .name              = "mpjpeg",
