...
static int ingenient_probe(AVEMProbeData *p)
{
...
static int ingenient_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int ret, size, w, h, unk1, unk2;

    if (avio_em_rl32(s->pb) != MKTAG('M', 'J', 'P', 'G'))
    ...
    avio_em_skip(s->pb, 8); // zero + size (padded?)
    avio_em_skip(s->pb, 2);
    unk1 = avio_em_rl16(s->pb);
    unk2 = avio_em_rl16(s->pb);
    avio_em_skip(s->pb, 22); // ASCII timestamp

    av_em_log(s, AV_LOG_DEBUG, "Ingenient packet: size=%d, width=%d, height=%d, unk1=%d unk2=%d\n",
        size, w, h, unk1, unk2);
...
AVEMInputFormat ff_ingenient_demuxer = {
    .name           = "ingenient",
