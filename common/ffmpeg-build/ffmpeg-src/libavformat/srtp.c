                            ...
                            int outlen)
{
    ...
    for (i = 0, outpos = 0; outpos < outlen; i++) {
        ...
        AV_WB16(&iv[14], i);
        av_em_aes_crypt(aes, keystream, iv, 1, NULL, 0);
                       ...
                       const char *params)
{
    ...
    if (!strcmp(suite, "AES_CM_128_HMAC_SHA1_80") ||
        ...
        av_em_log(NULL, AV_LOG_WARNING, "SRTP Crypto suite %s not supported\n",
                                     suite);
        return AVERROR(EINVAL);
    }
    if (av_em_base64_decode(buf, params, sizeof(buf)) != sizeof(buf)) {
        av_em_log(NULL, AV_LOG_WARNING, "Incorrect amount of SRTP params\n");
        return AVERROR(EINVAL);
    }
    // MKI and lifetime not handled yet
    s->aes  = av_em_aes_alloc();
    s->hmac = av_em_hmac_alloc(AV_HMAC_SHA1);
    if (!s->aes || !s->hmac)
    ...
    // RFC 3711
    av_em_aes_init(s->aes, s->master_key, 128, 0);
...
int ff_srtp_decrypt(struct SRTPContext *s, uint8_t *buf, int *lenptr)
{
    ...
    // Authentication HMAC
    av_em_hmac_init(s->hmac, rtcp ? s->rtcp_auth : s->rtp_auth, sizeof(s->rtp_auth));
    // If MKI is used, this should exclude the MKI as well
    av_em_hmac_update(s->hmac, buf, len - hmac_size);
    ...
    if (!rtcp) {
        ...
        AV_WB32(rocbuf, roc);
        av_em_hmac_update(s->hmac, rocbuf, 4);
    }

    av_em_hmac_final(s->hmac, hmac, sizeof(hmac));
    if (memcmp(hmac, buf + len - hmac_size, hmac_size)) {
        av_em_log(NULL, AV_LOG_WARNING, "HMAC mismatch\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    *lenptr = len;
    ...
    av_em_aes_init(s->aes, rtcp ? s->rtcp_key : s->rtp_key, 128, 0);
                    ...
                    uint8_t *out, int outlen)
{
    ...
    av_em_hmac_init(s->hmac, rtcp ? s->rtcp_auth : s->rtp_auth, sizeof(s->rtp_auth));
    av_em_hmac_update(s->hmac, out, buf + len - out);
    if (!rtcp) {
        uint8_t rocbuf[4];
