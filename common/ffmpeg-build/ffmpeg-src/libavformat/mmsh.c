...
static int mmsh_close(EMURLContext *h)
{
    MMSHContext *mmsh = (MMSHContext *)h->priv_data;
    MMSContext *mms   = &mmsh->mms;
    if (mms->mms_hd)
        ffurl_em_closep(&mms->mms_hd);
    av_em_freep(&mms->streams);
    av_em_freep(&mms->asf_header);
    return 0;
}
...
static ChunkType get_chunk_header(MMSHContext *mmsh, int *len)
{
    ...
    res = ffurl_em_read_complete(mms->mms_hd, chunk_header, CHUNK_HEADER_LENGTH);
    if (res != CHUNK_HEADER_LENGTH) {
        av_em_log(NULL, AV_LOG_ERROR, "Read data packet header failed!\n");
        return AVERROR(EIO);
    }
    ...
    case CHUNK_TYPE_DATA:
        ...
        break;
    default:
        av_em_log(NULL, AV_LOG_ERROR, "Strange chunk type %d\n", chunk_type);
        return AVERROR_INVALIDDATA;
    }

    res = ffurl_em_read_complete(mms->mms_hd, ext_header, ext_header_len);
    if (res != ext_header_len) {
        av_em_log(NULL, AV_LOG_ERROR, "Read ext header failed!\n");
        return AVERROR(EIO);
    }
...
static int read_data_packet(MMSHContext *mmsh, const int len)
{
    ...
    res = ffurl_em_read_complete(mms->mms_hd, mms->in_buffer, len);
    av_em_log(NULL, AV_LOG_TRACE, "Data packet len = %d\n", len);
    if (res != len) {
        av_em_log(NULL, AV_LOG_ERROR, "Read data packet failed!\n");
        return AVERROR(EIO);
    }
...
static int get_http_header_data(MMSHContext *mmsh)
{
    ...
    for (;;) {
        ...
        if (res < 0) {
            ...
            if (!mms->header_parsed) {
                if (mms->asf_header) {
                    if (len != mms->asf_header_size) {
                        ...
                        av_em_log(NULL, AV_LOG_TRACE, "Header len changed from %d to %d\n",
                                mms->asf_header_size, len);
                        av_em_freep(&mms->asf_header);
                    }
                }
                mms->asf_header = av_em_mallocz(len);
                if (!mms->asf_header) {
            ...
            res = ffurl_em_read_complete(mms->mms_hd, mms->asf_header, len);
            if (res != len) {
                av_em_log(NULL, AV_LOG_ERROR,
            ...
            if (len) {
                ...
                res = ffurl_em_read_complete(mms->mms_hd, mms->in_buffer, len);
                if (res != len) {
                    av_em_log(NULL, AV_LOG_ERROR, "Read other chunk type data failed!\n");
                    return AVERROR(EIO);
                } else {
                    av_em_log(NULL, AV_LOG_TRACE, "Skip chunk type %d \n", chunk_type);
                    continue;
                }
            }
        }
...
static int mmsh_open_internal(EMURLContext *h, const char *uri, int flags, int timestamp, int64_t pos)
{
    int i, port, err;
    ...
    mms = &mmsh->mms;
    av_em_strlcpy(mmsh->location, uri, sizeof(mmsh->location));

    av_em_url_split(NULL, 0, NULL, 0,
        host, sizeof(host), &port, path, sizeof(path), mmsh->location);
    if (port<0)
        port = 80; // default mmsh protocol port
    em_url_join(httpname, sizeof(httpname), "http", NULL, host, port, "%s", path);

    if (ffurl_em_alloc(&mms->mms_hd, httpname, AVIO_FLAG_READ,
    ...
    av_em_opt_set(mms->mms_hd->priv_data, "headers", headers, 0);

    err = ffurl_em_connect(mms->mms_hd, NULL);
    if (err) {
        goto fail;
    }
    err = get_http_header_data(mmsh);
    if (err) {
        av_em_log(NULL, AV_LOG_ERROR, "Get http header data failed!\n");
        goto fail;
    }

    // close the socket and then reopen it for sending the second play request.
    ffurl_em_close(mms->mms_hd);
    memset(headers, 0, sizeof(headers));
    if ((err = ffurl_em_alloc(&mms->mms_hd, httpname, AVIO_FLAG_READ,
                           &h->interrupt_callback)) < 0) {
        goto fail;
    }
    stream_selection = av_em_mallocz(mms->stream_num * 19 + 1);
    if (!stream_selection)
        return AVERROR(ENOMEM);
    for (i = 0; i < mms->stream_num; i++) {
        ...
        if (err < 0)
            goto fail;
        av_em_strlcat(stream_selection, tmp, mms->stream_num * 19 + 1);
    }
    // send play request
    ...
    av_em_freep(&stream_selection);
    if (err < 0) {
        av_em_log(NULL, AV_LOG_ERROR, "Build play request failed!\n");
        goto fail;
    }
    av_em_log(NULL, AV_LOG_TRACE, "out_buffer is %s", headers);
    av_em_opt_set(mms->mms_hd->priv_data, "headers", headers, 0);
    ...
    av_em_log(NULL, AV_LOG_TRACE, "Connection successfully open\n");
    return 0;
fail:
    av_em_freep(&stream_selection);
    mmsh_close(h);
    av_em_log(NULL, AV_LOG_TRACE, "Connection failed with error %d\n", err);
    return err;
}

static int mmsh_open(EMURLContext *h, const char *uri, int flags)
{
...
static int handle_chunk_type(MMSHContext *mmsh)
{
    ...
    case CHUNK_TYPE_END:
        mmsh->chunk_seq = 0;
        av_em_log(NULL, AV_LOG_ERROR, "Stream ended!\n");
        return AVERROR(EIO);
    case CHUNK_TYPE_STREAM_CHANGE:
        mms->header_parsed = 0;
        if (res = get_http_header_data(mmsh)) {
            av_em_log(NULL, AV_LOG_ERROR,"Stream changed! Failed to get new header!\n");
            return res;
        }
        break;
    case CHUNK_TYPE_DATA:
        return read_data_packet(mmsh, len);
    default:
        av_em_log(NULL, AV_LOG_ERROR, "Recv other type packet %d\n", chunk_type);
        return AVERROR_INVALIDDATA;
    }
    return 0;
}

static int mmsh_read(EMURLContext *h, uint8_t *buf, int size)
{
    int res = 0;
    MMSHContext *mmsh = h->priv_data;
    MMSContext *mms   = &mmsh->mms;
...
static int64_t mmsh_read_seek(EMURLContext *h, int stream_index,
                        int64_t timestamp, int flags)
{
    MMSHContext *mmsh_old = h->priv_data;
    MMSHContext *mmsh     = av_em_mallocz(sizeof(*mmsh));
    int ret;
...
static int64_t mmsh_seek(EMURLContext *h, int64_t pos, int whence)
{
    MMSHContext *mmsh = h->priv_data;
    MMSContext *mms   = &mmsh->mms;
...
const EMURLProtocol em_mmsh_protocol = {
    .name           = "mmsh",
    .url_open       = mmsh_open,
    .url_read       = mmsh_read,
    .url_seek       = mmsh_seek,
