...
typedef struct OutputStream {
    ...
    char dirname[1024];
    uint8_t iobuf[32768];
    EMURLContext *out;  // Current output stream where all output is written
    EMURLContext *out2; // Auxiliary output stream where all output is also written
    EMURLContext *tail_out; // The actual main output stream, if we're currently seeked back to write elsewhere
...
static int ism_write(void *opaque, uint8_t *buf, int buf_size)
{
    ...
    if (os->out)
        ffurl_em_write(os->out, buf, buf_size);
    if (os->out2)
        ffurl_em_write(os->out2, buf, buf_size);
    os->cur_pos += buf_size;
...
static int64_t ism_seek(void *opaque, int64_t offset, int whence)
{
    ...
    if (os->tail_out) {
        if (os->out) {
            ffurl_em_close(os->out);
        }
        if (os->out2) {
            ffurl_em_close(os->out2);
        }
        os->out = os->tail_out;
        os->out2 = NULL;
        os->tail_out = NULL;
    ...
    if (offset >= os->cur_start_pos) {
        if (os->out)
            ffurl_em_seek(os->out, offset - os->cur_start_pos, SEEK_SET);
        os->cur_pos = offset;
        return offset;
    }
    for (i = os->nb_fragments - 1; i >= 0; i--) {
        ...
        if (offset >= frag->start_pos && offset < frag->start_pos + frag->size) {
            ...
            os->tail_out = os->out;
            av_em_dict_set(&opts, "truncate", "0", 0);
            ret = ffurl_em_open_whitelist(&os->out, frag->file, AVIO_FLAG_WRITE,
            ...
            av_em_dict_set(&opts, "truncate", "0", 0);
            ffurl_em_open_whitelist(&os->out2, frag->infofile, AVIO_FLAG_WRITE,
                                 &os->ctx->interrupt_callback, &opts, os->ctx->protocol_whitelist, os->ctx->protocol_blacklist, NULL);
            av_em_dict_free(&opts);
            ffurl_em_seek(os->out, offset - frag->start_pos, SEEK_SET);
            if (os->out2)
                ffurl_em_seek(os->out2, offset - frag->start_pos, SEEK_SET);
            os->cur_pos = offset;
            return offset;
        }
    }
...
{
    AVEMCodecParameters *par = os->ctx->streams[0]->codecpar;
    uint8_t *ptr = par->extradata;
    int size = par->extradata_size;
    int i;
    if (par->codec_id == AV_CODEC_ID_H264) {
        em_avc_write_annexb_extradata(ptr, &ptr, &size);
        if (!ptr)
            ptr = par->extradata;
    }
    if (!ptr)
        return;
    os->private_str = av_em_mallocz(2*size + 1);
    if (!os->private_str)
        goto fail;
    for (i = 0; i < size; i++)
...
static void ism_free(AVEMFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        ffurl_em_close(os->out);
        ffurl_em_close(os->out2);
        ffurl_em_close(os->tail_out);
        os->out = os->out2 = os->tail_out = NULL;
        if (os->ctx && os->ctx_inited)
            av_em_write_trailer(os->ctx);
        if (os->ctx && os->ctx->pb)
            av_em_freep(&os->ctx->pb);
        if (os->ctx)
            avformat_em_free_context(os->ctx);
        av_em_freep(&os->private_str);
        for (j = 0; j < os->nb_fragments; j++)
            av_em_freep(&os->fragments[j]);
        av_em_freep(&os->fragments);
    }
    av_em_freep(&c->streams);
}

static void output_chunk_list(OutputStream *os, AVEMIOContext *out, int final, int skip, int window_size)
{
    ...
    for (i = start; i < os->nb_fragments - skip; i++) {
        Fragment *frag = os->fragments[i];
        if (!final || removed)
            avio_em_printf(out, "<c t=\"%"PRIu64"\" d=\"%"PRIu64"\" />\n", frag->start_time, frag->duration);
        else
            avio_em_printf(out, "<c n=\"%d\" d=\"%"PRIu64"\" />\n", frag->n, frag->duration);
    }
}
...
static int write_manifest(AVEMFormatContext *s, int final)
{
    ...
    avio_em_printf(out, "<SmoothStreamingMedia MajorVersion=\"2\" MinorVersion=\"0\" Duration=\"%"PRIu64"\"", duration);
    if (!final)
        avio_em_printf(out, " IsLive=\"true\" LookAheadFragmentCount=\"%d\" DVRWindowLength=\"0\"", c->lookahead_count);
    avio_em_printf(out, ">\n");
    if (c->has_video) {
        int last = -1, index = 0;
        avio_em_printf(out, "<StreamIndex Type=\"video\" QualityLevels=\"%d\" Chunks=\"%d\" Url=\"QualityLevels({bitrate})/Fragments(video={start time})\">\n", video_streams, video_chunks);
        for (i = 0; i < s->nb_streams; i++) {
            OutputStream *os = &c->streams[i];
            if (s->streams[i]->codecpar->codec_type != AVMEDIA_TYPE_VIDEO)
                continue;
            last = i;
            avio_em_printf(out, "<QualityLevel Index=\"%d\" Bitrate=\"%"PRId64"\" FourCC=\"%s\" MaxWidth=\"%d\" MaxHeight=\"%d\" CodecPrivateData=\"%s\" />\n", index, (int64_t)s->streams[i]->codecpar->bit_rate, os->fourcc, s->streams[i]->codecpar->width, s->streams[i]->codecpar->height, os->private_str);
            index++;
        }
        output_chunk_list(&c->streams[last], out, final, c->lookahead_count, c->window_size);
        avio_em_printf(out, "</StreamIndex>\n");
    }
    if (c->has_audio) {
        int last = -1, index = 0;
        avio_em_printf(out, "<StreamIndex Type=\"audio\" QualityLevels=\"%d\" Chunks=\"%d\" Url=\"QualityLevels({bitrate})/Fragments(audio={start time})\">\n", audio_streams, audio_chunks);
        for (i = 0; i < s->nb_streams; i++) {
            OutputStream *os = &c->streams[i];
            if (s->streams[i]->codecpar->codec_type != AVMEDIA_TYPE_AUDIO)
                continue;
            last = i;
            avio_em_printf(out, "<QualityLevel Index=\"%d\" Bitrate=\"%"PRId64"\" FourCC=\"%s\" SamplingRate=\"%d\" Channels=\"%d\" BitsPerSample=\"16\" PacketSize=\"%d\" AudioTag=\"%d\" CodecPrivateData=\"%s\" />\n", index, (int64_t)s->streams[i]->codecpar->bit_rate, os->fourcc, s->streams[i]->codecpar->sample_rate, s->streams[i]->codecpar->channels, os->packet_size, os->audio_tag, os->private_str);
            index++;
        }
        output_chunk_list(&c->streams[last], out, final, c->lookahead_count, c->window_size);
        avio_em_printf(out, "</StreamIndex>\n");
    }
    avio_em_printf(out, "</SmoothStreamingMedia>\n");
    avio_em_flush(out);
    em_format_io_close(s, &out);
    return em_rename(temp_filename, filename, s);
}

static int ism_write_header(AVEMFormatContext *s)
{
    SmoothStreamingContext *c = s->priv_data;
    int ret = 0, i;
    ...
    if (mkdir(s->filename, 0777) == -1 && errno != EEXIST) {
        ret = AVERROR(errno);
        av_em_log(s, AV_LOG_ERROR, "mkdir failed\n");
        goto fail;
    }

    oformat = av_em_guess_format("ismv", NULL, NULL);
    if (!oformat) {
        ret = AVERROR_MUXER_NOT_FOUND;
        goto fail;
    }

    c->streams = av_em_mallocz_array(s->nb_streams, sizeof(*c->streams));
    if (!c->streams) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (!s->streams[i]->codecpar->bit_rate) {
            av_em_log(s, AV_LOG_ERROR, "No bit rate set for stream %d\n", i);
            ret = AVERROR(EINVAL);
            goto fail;
        }
        ...
        if (!ctx || em_copy_whiteblacklists(ctx, s) < 0) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        os->ctx = ctx;
        ...
        ctx->pb = avio_em_alloc_context(os->iobuf, sizeof(os->iobuf), AVIO_FLAG_WRITE, os, NULL, ism_write, ism_seek);
        if (!ctx->pb) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }

        av_em_dict_set_int(&opts, "ism_lookahead", c->lookahead_count, 0);
        av_em_dict_set(&opts, "movflags", "frag_custom", 0);
    ...
    if (!c->has_video && c->min_frag_duration <= 0) {
        av_em_log(s, AV_LOG_WARNING, "no video stream and no min frag duration set\n");
        ret = AVERROR(EINVAL);
        goto fail;
    }
...
static int parse_fragment(AVEMFormatContext *s, const char *filename, int64_t *start_ts, int64_t *duration, int64_t *moof_size, int64_t size)
{
    AVEMIOContext *in;
    int ret;
    uint32_t len;
    if ((ret = s->io_open(s, &in, filename, AVIO_FLAG_READ, NULL)) < 0)
        return ret;
    ret = AVERROR(EIO);
    *moof_size = avio_em_rb32(in);
    if (*moof_size < 8 || *moof_size > size)
        goto fail;
    if (avio_em_rl32(in) != MKTAG('m','o','o','f'))
        goto fail;
    len = avio_em_rb32(in);
    if (len > *moof_size)
        goto fail;
    if (avio_em_rl32(in) != MKTAG('m','f','h','d'))
        goto fail;
    avio_em_seek(in, len - 8, SEEK_CUR);
    avio_em_rb32(in); /* traf size */
    if (avio_em_rl32(in) != MKTAG('t','r','a','f'))
        goto fail;
    while (avio_em_tell(in) < *moof_size) {
        uint32_t len = avio_em_rb32(in);
        uint32_t tag = avio_em_rl32(in);
        int64_t end = avio_em_tell(in) + len - 8;
        ...
        if (tag == MKTAG('u','u','i','d')) {
            ...
            uint8_t uuid[16];
            avio_em_read(in, uuid, 16);
            if (!memcmp(uuid, tfxd, 16) && len >= 8 + 16 + 4 + 16) {
                avio_em_seek(in, 4, SEEK_CUR);
                *start_ts = avio_em_rb64(in);
                *duration = avio_em_rb64(in);
                ret = 0;
                break;
            }
        }
        avio_em_seek(in, end, SEEK_SET);
    }
fail:
    em_format_io_close(s, &in);
    return ret;
}
...
static int add_fragment(OutputStream *os, const char *file, const char *infofile, int64_t start_time, int64_t duration, int64_t start_pos, int64_t size)
{
    ...
    if (os->nb_fragments >= os->fragments_size) {
        ...
        if ((err = av_em_reallocp(&os->fragments, sizeof(*os->fragments) *
                               os->fragments_size)) < 0) {
            os->fragments_size = 0;
            os->nb_fragments = 0;
            return err;
    ...
    frag = av_em_mallocz(sizeof(*frag));
    if (!frag)
        return AVERROR(ENOMEM);
    av_em_strlcpy(frag->file, file, sizeof(frag->file));
    av_em_strlcpy(frag->infofile, infofile, sizeof(frag->infofile));
...
static int copy_moof(AVEMFormatContext *s, const char* infile, const char *outfile, int64_t size)
{
    AVEMIOContext *in, *out;
    int ret = 0;
    ...
    while (size > 0) {
        ...
        n = avio_em_read(in, buf, n);
        if (n <= 0) {
            ret = AVERROR(EIO);
            break;
        }
        avio_em_write(out, buf, n);
        size -= n;
    }
    avio_em_flush(out);
    em_format_io_close(s, &out);
...
static int ism_flush(AVEMFormatContext *s, int final)
{
    SmoothStreamingContext *c = s->priv_data;
    int i, ret = 0;
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        ret = ffurl_em_open_whitelist(&os->out, filename, AVIO_FLAG_WRITE, &s->interrupt_callback, NULL, s->protocol_whitelist, s->protocol_blacklist, NULL);
        if (ret < 0)
            break;
        ...
        ret = em_rename(filename, target_filename, s);
        if (ret < 0)
            break;
...
static int ism_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    SmoothStreamingContext *c = s->priv_data;
    ...
    if ((!c->has_video || st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) &&
        av_em_compare_ts(pkt->dts - st->first_dts, st->time_base,
...
static int ism_write_trailer(AVEMFormatContext *s)
{
    SmoothStreamingContext *c = s->priv_data;
    ism_flush(s, 1);
...
static const AVEMClass ism_class = {
...
AVEMOutputFormat ff_smoothstreaming_muxer = {
