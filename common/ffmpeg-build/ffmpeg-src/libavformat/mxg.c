...
static int mxg_read_header(AVEMFormatContext *s)
{
    ...
    avpriv_em_set_pts_info(audio_st, 64, 1, 1000000);
...
static int mxg_update_cache(AVEMFormatContext *s, unsigned int cache_size)
{
    MXGContext *mxg = s->priv_data;
    ...
    buffer = av_em_fast_realloc(mxg->buffer, &mxg->buffer_size,
    ...
    /* get data */
    ret = avio_em_read(s->pb, mxg->buffer_ptr + mxg->cache_size,
...
static int mxg_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int ret;
    unsigned int size;
    ...
    while (!avio_em_feof(s->pb) && !s->pb->error){
        ...
        if (startmarker_ptr != search_end) { /* start marker found */
            ...
            if (marker == SOI) {
                ...
                if (!mxg->soi_ptr) {
                    av_em_log(s, AV_LOG_WARNING, "Found EOI before SOI, skipping\n");
                    continue;
                }
...
static int mxg_close(struct AVEMFormatContext *s)
{
    MXGContext *mxg = s->priv_data;
    av_em_freep(&mxg->buffer);
    return 0;
}

AVEMInputFormat ff_mxg_demuxer = {
    .name           = "mxg",
