...
static int lrc_write_header(AVEMFormatContext *s)
{
    const AVEMDictionaryEntry *metadata_item;
    ...
    if(s->streams[0]->codecpar->codec_id != AV_CODEC_ID_SUBRIP &&
       s->streams[0]->codecpar->codec_id != AV_CODEC_ID_TEXT) {
        av_em_log(s, AV_LOG_ERROR, "Unsupported subtitle codec: %s\n",
               avcodec_em_get_name(s->streams[0]->codecpar->codec_id));
        return AVERROR(EINVAL);
    }
    ...
    if(!(s->flags & AVFMT_FLAG_BITEXACT)) { // avoid breaking regression tests
         ...
         * to it.
         */
        av_em_dict_set(&s->metadata, "ve", AV_STRINGIFY(LIBAVFORMAT_VERSION), 0);
    } else {
        av_em_dict_set(&s->metadata, "ve", NULL, 0);
    }
    for(metadata_item = NULL;
       (metadata_item = av_em_dict_get(s->metadata, "", metadata_item,
...
static int lrc_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    if(pkt->pts != AV_NOPTS_VALUE) {
        char *data = av_em_alloc(pkt->size + 1);
        char *line;
        char *delim;
        ...
        while(line) {
            ...
            if(pkt->pts >= 0) {
                avio_em_printf(s->pb, "[%02"PRId64":%02"PRId64".%02"PRId64"]",
                            (pkt->pts / 6000),
                            ((pkt->pts / 100) % 60),
                            (pkt->pts % 100));
            } else {
