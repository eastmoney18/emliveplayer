...
struct PayloadContext {
    AVEMIOContext *frame;         ///< current frame buffer
...
static int jpeg_parse_packet(AVEMFormatContext *ctx, PayloadContext *jpeg,
                             ...
                             int flags)
{
    ...
    if (len < 8) {
        av_em_log(ctx, AV_LOG_ERROR, "Too short RTP/JPEG packet.\n");
    ...
    if (type > 1) {
        av_em_log(ctx, AV_LOG_ERROR, "Unimplemented RTP/JPEG type %d\n", type);
    ...
    if (off == 0) {
        ...
        if (q > 127) {
            ...
            if (precision)
                av_em_log(ctx, AV_LOG_WARNING, "Only 8-bit precision is supported.\n");
            ...
            if (q == 0 || q > 99) {
                av_em_log(ctx, AV_LOG_ERROR, "Reserved q value %d\n", q);
                return AVERROR_INVALIDDATA;
            }
        ...
        if ((ret = avio_em_open_dyn_buf(&jpeg->frame)) < 0)
            return ret;
        ...
        avio_em_write(jpeg->frame, hdr, jpeg->hdr_size);
    }
    ...
    if (off != avio_em_tell(jpeg->frame) - jpeg->hdr_size) {
        av_em_log(ctx, AV_LOG_ERROR,
    ...
    avio_em_write(jpeg->frame, buf, len);
    ...
    if (flags & RTP_FLAG_MARKER) {
        ...
        /* Put EOI marker. */
        avio_em_write(jpeg->frame, buf, sizeof(buf));
