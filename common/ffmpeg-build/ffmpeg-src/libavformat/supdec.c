...
static int sup_read_header(AVEMFormatContext *s)
{
...
static int sup_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int64_t pts, dts, pos;
    int ret;
    ...
    if (avio_em_rb16(s->pb) != SUP_PGS_MAGIC)
        return avio_em_feof(s->pb) ? AVERROR_EOF : AVERROR_INVALIDDATA;

    pts = avio_em_rb32(s->pb);
    dts = avio_em_rb32(s->pb);

    if ((ret = av_em_get_packet(s->pb, pkt, 3)) < 0)
        return ret;
    ...
    if (pkt->size >= 3) {
        ...
        if ((ret = av_em_append_packet(s->pb, pkt, len)) < 0)
            return ret;
    }
...
static int sup_probe(AVEMProbeData *p)
{
...
AVEMInputFormat ff_sup_demuxer = {
    .name           = "sup",
