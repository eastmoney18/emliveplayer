...
typedef struct SegmentContext {
    ...
    AVEMIOContext *list_pb;  ///< list file put-byte context
...
static void print_csv_escaped_str(AVEMIOContext *ctx, const char *str)
{
...
static int segment_mux_init(AVEMFormatContext *s)
{
    ...
    ret = avformat_em_alloc_output_context2(&seg->avf, seg->oformat, NULL, NULL);
    if (ret < 0)
        return ret;
    oc = seg->avf;
    ...
    for (i = 0; i < s->nb_streams; i++) {
        AVEMStream *st;
        AVEMCodecParameters *ipar, *opar;

        if (!(st = avformat_em_new_stream(oc, NULL)))
            return AVERROR(ENOMEM);
        ipar = s->streams[i]->codecpar;
        opar = st->codecpar;
        avcodec_em_parameters_copy(opar, ipar);
        if (!oc->oformat->codec_tag ||
            av_em_codec_get_id (oc->oformat->codec_tag, ipar->codec_tag) == opar->codec_id ||
            av_em_codec_get_tag(oc->oformat->codec_tag, ipar->codec_id) <= 0) {
        ...
        av_em_dict_copy(&st->metadata, s->streams[i]->metadata, 0);
    }

    return 0;
}

static int set_segment_filename(AVEMFormatContext *s)
{
    SegmentContext *seg = s->priv_data;
    AVEMFormatContext *oc = seg->avf;
    size_t size;
    int ret;
    ...
    if (seg->use_strftime) {
        ...
        if (!strftime(oc->filename, sizeof(oc->filename), s->filename, tm)) {
            av_em_log(oc, AV_LOG_ERROR, "Could not get segment filename with strftime\n");
            return AVERROR(EINVAL);
        }
    } else if (av_em_get_frame_filename(oc->filename, sizeof(oc->filename),
                                     s->filename, seg->segment_idx) < 0) {
        av_em_log(oc, AV_LOG_ERROR, "Invalid segment filename template '%s'\n", s->filename);
        return AVERROR(EINVAL);
    }

    /* copy modified name in list entry */
    size = strlen(av_em_basename(oc->filename)) + 1;
    if (seg->entry_prefix)
        size += strlen(seg->entry_prefix);

    if ((ret = av_em_reallocp(&seg->cur_entry.filename, size)) < 0)
        return ret;
    snprintf(seg->cur_entry.filename, size, "%s%s",
             seg->entry_prefix ? seg->entry_prefix : "",
             av_em_basename(oc->filename));

    return 0;
}

static int segment_start(AVEMFormatContext *s, int write_header)
{
    SegmentContext *seg = s->priv_data;
    AVEMFormatContext *oc = seg->avf;
    int err = 0;
    ...
    if ((err = s->io_open(s, &oc->pb, oc->filename, AVIO_FLAG_WRITE, NULL)) < 0) {
        av_em_log(s, AV_LOG_ERROR, "Failed to open segment '%s'\n", oc->filename);
        return err;
    }
    ...
    if (write_header) {
        ...
        av_em_dict_copy(&options, seg->format_options, 0);
        err = avformat_em_write_header(oc, &options);
...
static int segment_list_open(AVEMFormatContext *s)
{
    SegmentContext *seg = s->priv_data;
    int ret;
    ...
    if (ret < 0) {
        av_em_log(s, AV_LOG_ERROR, "Failed to open segment list '%s'\n", seg->list);
        return ret;
    }
    ...
    if (seg->list_type == LIST_TYPE_M3U8 && seg->segment_list_entries) {
        ...
        avio_em_printf(seg->list_pb, "#EXTM3U\n");
        avio_em_printf(seg->list_pb, "#EXT-X-VERSION:3\n");
        avio_em_printf(seg->list_pb, "#EXT-X-MEDIA-SEQUENCE:%d\n", seg->segment_list_entries->index);
        avio_em_printf(seg->list_pb, "#EXT-X-ALLOW-CACHE:%s\n",
                    seg->list_flags & SEGMENT_LIST_FLAG_CACHE ? "YES" : "NO");

        av_em_log(s, AV_LOG_VERBOSE, "EXT-X-MEDIA-SEQUENCE:%d\n",
               seg->segment_list_entries->index);

        for (entry = seg->segment_list_entries; entry; entry = entry->next)
            max_duration = FFMAX(max_duration, entry->end_time - entry->start_time);
        avio_em_printf(seg->list_pb, "#EXT-X-TARGETDURATION:%"PRId64"\n", (int64_t)ceil(max_duration));
    } else if (seg->list_type == LIST_TYPE_FFCONCAT) {
        avio_em_printf(seg->list_pb, "ffconcat version 1.0\n");
    }

    return ret;
}

static void segment_list_print_entry(AVEMIOContext      *list_ioctx,
                                     ListType          list_type,
                                     ...
                                     void *log_ctx)
{
    switch (list_type) {
    case LIST_TYPE_FLAT:
        avio_em_printf(list_ioctx, "%s\n", list_entry->filename);
        break;
    case LIST_TYPE_CSV:
    case LIST_TYPE_EXT:
        print_csv_escaped_str(list_ioctx, list_entry->filename);
        avio_em_printf(list_ioctx, ",%f,%f\n", list_entry->start_time, list_entry->end_time);
        break;
    case LIST_TYPE_M3U8:
        avio_em_printf(list_ioctx, "#EXTINF:%f,\n%s\n",
                    list_entry->end_time - list_entry->start_time, list_entry->filename);
        break;
    case LIST_TYPE_FFCONCAT:
    {
        char *buf;
        if (av_em_escape(&buf, list_entry->filename, NULL, av_em_escape_MODE_AUTO, av_em_escape_FLAG_WHITESPACE) < 0) {
            av_em_log(log_ctx, AV_LOG_WARNING,
                   "Error writing list entry '%s' in list file\n", list_entry->filename);
            return;
        }
        avio_em_printf(list_ioctx, "file %s\n", buf);
        av_em_free(buf);
        break;
    }
    default:
...
static int segment_end(AVEMFormatContext *s, int write_trailer, int is_last)
{
    SegmentContext *seg = s->priv_data;
    AVEMFormatContext *oc = seg->avf;
    int ret = 0;
    ...
    av_em_write_frame(oc, NULL); /* Flush any buffered data (fragmented mp4) */
    if (write_trailer)
        ret = av_em_write_trailer(oc);

    if (ret < 0)
        av_em_log(s, AV_LOG_ERROR, "Failure occurred when ending segment '%s'\n",
               oc->filename);

    if (seg->list) {
        if (seg->list_size || seg->list_type == LIST_TYPE_M3U8) {
            SegmentListEntry *entry = av_em_mallocz(sizeof(*entry));
            if (!entry) {
                ret = AVERROR(ENOMEM);
                goto end;
            }
            ...
            entry->filename = av_em_strdup(entry->filename);
            ...
            if (seg->list_type == LIST_TYPE_M3U8 && is_last)
                avio_em_printf(seg->list_pb, "#EXT-X-ENDLIST\n");
            em_format_io_close(s, &seg->list_pb);
            if (seg->use_rename)
                em_rename(seg->temp_list_filename, seg->list, s);
        } else {
            segment_list_print_entry(seg->list_pb, seg->list_type, &seg->cur_entry, s);
            avio_em_flush(seg->list_pb);
        }
    }

    av_em_log(s, AV_LOG_VERBOSE, "segment:'%s' count:%d ended\n",
           seg->avf->filename, seg->segment_count);
    seg->segment_count++;

    if (seg->increment_tc) {
        tcr = av_em_dict_get(s->metadata, "timecode", NULL, 0);
        if (tcr) {
            /* search the first video stream */
            for (i = 0; i < s->nb_streams; i++) {
                if (s->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
                    rate = s->streams[i]->avg_frame_rate;/* Get fps from the video stream */
                    err = av_em_timecode_init_from_string(&tc, rate, tcr->value, s);
                    if (err < 0) {
                        av_em_log(s, AV_LOG_WARNING, "Could not increment timecode, error occurred during timecode creation.");
                        break;
                    }
                    tc.start += (int)((seg->cur_entry.end_time - seg->cur_entry.start_time) * av_em_q2d(rate));/* increment timecode */
                    av_em_dict_set(&s->metadata, "timecode",
                                av_em_timecode_make_string(&tc, buf, 0), 0);
                    break;
                }
            }
        } else {
            av_em_log(s, AV_LOG_WARNING, "Could not increment timecode, no timecode metadata found");
        }
    }

end:
    em_format_io_close(oc, &oc->pb);
                       ...
                       const char *times_str)
{
    char *p;
    int i, ret = 0;
    char *times_str1 = av_em_strdup(times_str);
    char *saveptr = NULL;
...
#define FAIL(err) ret = err; goto end
    ...
    *times = av_em_malloc_array(*nb_times, sizeof(**times));
    if (!*times) {
        av_em_log(log_ctx, AV_LOG_ERROR, "Could not allocate forced times array\n");
        FAIL(AVERROR(ENOMEM));
    }
    ...
    for (i = 0; i < *nb_times; i++) {
        int64_t t;
        char *tstr = av_em_strtok(p, ",", &saveptr);
        p = NULL;

        if (!tstr || !tstr[0]) {
            av_em_log(log_ctx, AV_LOG_ERROR, "Empty time specification in times list %s\n",
                   times_str);
            FAIL(AVERROR(EINVAL));
        }

        ret = av_em_parse_time(&t, tstr, 1);
        if (ret < 0) {
                        ...
                        const char *frames_str)
{
    char *p;
    int i, ret = 0;
    char *frames_str1 = av_em_strdup(frames_str);
    char *saveptr = NULL;
...
#define FAIL(err) ret = err; goto end
    ...
    *frames = av_em_malloc_array(*nb_frames, sizeof(**frames));
    if (!*frames) {
        av_em_log(log_ctx, AV_LOG_ERROR, "Could not allocate forced frames array\n");
        FAIL(AVERROR(ENOMEM));
    }
    ...
    for (i = 0; i < *nb_frames; i++) {
        long int f;
        char *tailptr;
        char *fstr = av_em_strtok(p, ",", &saveptr);

        p = NULL;
        if (!fstr) {
            av_em_log(log_ctx, AV_LOG_ERROR, "Empty frame specification in frame list %s\n",
                   frames_str);
            FAIL(AVERROR(EINVAL));
        }
    ...
    av_em_free(frames_str1);
    return ret;
}

static int open_null_ctx(AVEMIOContext **ctx)
{
    int buf_size = 32768;
    uint8_t *buf = av_em_alloc(buf_size);
    if (!buf)
        return AVERROR(ENOMEM);
    *ctx = avio_em_alloc_context(buf, buf_size, AVIO_FLAG_WRITE, NULL, NULL, NULL, NULL);
    if (!*ctx) {
        av_em_free(buf);
        return AVERROR(ENOMEM);
    }
...
static void close_null_ctxp(AVEMIOContext **pb)
{
    av_em_freep(&(*pb)->buffer);
    av_em_freep(pb);
}

static int select_reference_stream(AVEMFormatContext *s)
{
    SegmentContext *seg = s->priv_data;
    int ret, i;
    ...
    if (!strcmp(seg->reference_stream_specifier, "auto")) {
        ...
        static const enum AVEMMediaType type_priority_list[] = {
            AVMEDIA_TYPE_VIDEO,
            AVMEDIA_TYPE_AUDIO,
        ...
        for (i = 0; i < s->nb_streams; i++) {
            ret = avformat_em_match_stream_specifier(s, s->streams[i],
    ...
    if (seg->reference_stream_index < 0) {
        av_em_log(s, AV_LOG_ERROR, "Could not select stream matching identifier '%s'\n",
               seg->reference_stream_specifier);
        return AVERROR(EINVAL);
    }
...
{
    em_format_io_close(seg->avf, &seg->list_pb);
    avformat_em_free_context(seg->avf);
    seg->avf = NULL;
}

static int seg_init(AVEMFormatContext *s)
{
    SegmentContext *seg = s->priv_data;
    AVEMFormatContext *oc = seg->avf;
    AVEMDictionary *options = NULL;
    ...
    if (seg->times_str) {
        ...
        if (!seg->time_str)
            seg->time_str = av_em_strdup("2");
        if ((ret = av_em_parse_time(&seg->time, seg->time_str, 1)) < 0) {
            av_em_log(s, AV_LOG_ERROR,
        ...
        if (seg->use_clocktime) {
            if (seg->time <= 0) {
                av_em_log(s, AV_LOG_ERROR, "Invalid negative segment_time with segment_atclocktime option set\n");
                return AVERROR(EINVAL);
            }
    ...
    if (seg->format_options_str) {
        ret = av_em_dict_parse_string(&seg->format_options, seg->format_options_str, "=", ":", 0);
        if (ret < 0) {
    ...
    if (seg->list) {
        if (seg->list_type == LIST_TYPE_UNDEFINED) {
            if      (av_em_match_ext(seg->list, "csv" )) seg->list_type = LIST_TYPE_CSV;
            else if (av_em_match_ext(seg->list, "ext" )) seg->list_type = LIST_TYPE_EXT;
            else if (av_em_match_ext(seg->list, "m3u8")) seg->list_type = LIST_TYPE_M3U8;
            else if (av_em_match_ext(seg->list, "ffcat,ffconcat")) seg->list_type = LIST_TYPE_FFCONCAT;
    ...
    if (seg->list_type == LIST_TYPE_EXT)
        av_em_log(s, AV_LOG_WARNING, "'ext' list type option is deprecated in favor of 'csv'\n");

    if ((ret = select_reference_stream(s)) < 0)
        goto fail;
    av_em_log(s, AV_LOG_VERBOSE, "Selected stream id:%d type:%s\n",
           seg->reference_stream_index,
           av_em_get_media_type_string(s->streams[seg->reference_stream_index]->codecpar->codec_type));

    seg->oformat = av_em_guess_format(seg->format, s->filename, NULL);
    ...
    if (seg->oformat->flags & AVFMT_NOFILE) {
        av_em_log(s, AV_LOG_ERROR, "format %s not supported.\n",
               seg->oformat->name);
        ret = AVERROR(EINVAL);
        goto fail;
    }
    ...
    av_em_dict_copy(&options, seg->format_options, 0);
    ret = avformat_em_write_header(oc, &options);
    ...
    for (i = 0; i < s->nb_streams; i++) {
        AVEMStream *inner_st  = oc->streams[i];
...
static int seg_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    SegmentContext *seg = s->priv_data;
...
calc_times:
    ...
    if (pkt->stream_index == seg->reference_stream_index &&
        ...
        seg->cur_entry.start_time = (double)pkt->pts * av_em_q2d(st->time_base);
        seg->cur_entry.start_pts = av_em_rescale_q(pkt->pts, st->time_base, AV_TIME_BASE_Q);
    ...
    if (pkt->stream_index == seg->reference_stream_index) {
        if (pkt->pts != AV_NOPTS_VALUE)
            seg->cur_entry.end_time =
                FFMAX(seg->cur_entry.end_time, (double)(pkt->pts + pkt->duration) * av_em_q2d(st->time_base));
        seg->cur_entry.last_duration = pkt->duration;
    }

    if (seg->segment_frame_count == 0) {
        av_em_log(s, AV_LOG_VERBOSE, "segment:'%s' starts with packet stream:%d pts:%s pts_time:%s frame:%d\n",
               seg->avf->filename, pkt->stream_index,
               av_ts2str(pkt->pts), av_ts2timestr(pkt->pts, &st->time_base), seg->frame_count);
    }

    av_em_log(s, AV_LOG_DEBUG, "stream:%d start_pts_time:%s pts:%s pts_time:%s dts:%s dts_time:%s",
           pkt->stream_index,
    ...
    offset = av_em_rescale_q(seg->initial_offset - (seg->reset_timestamps ? seg->cur_entry.start_pts : 0),
    ...
    av_em_log(s, AV_LOG_DEBUG, " -> pts:%s pts_time:%s dts:%s dts_time:%s\n",
           av_ts2str(pkt->pts), av_ts2timestr(pkt->pts, &st->time_base),
           av_ts2str(pkt->dts), av_ts2timestr(pkt->dts, &st->time_base));

    ret = em_write_chained(seg->avf, pkt->stream_index, pkt, s, seg->initial_offset || seg->reset_timestamps);
...
static int seg_write_trailer(struct AVEMFormatContext *s)
{
    SegmentContext *seg = s->priv_data;
    AVEMFormatContext *oc = seg->avf;
...
fail:
    ...
    av_em_dict_free(&seg->format_options);
    av_em_opt_free(seg);
    av_em_freep(&seg->times);
    av_em_freep(&seg->frames);
    av_em_freep(&seg->cur_entry.filename);

    cur = seg->segment_list_entries;
    while (cur) {
        next = cur->next;
        av_em_freep(&cur->filename);
        av_em_free(cur);
        cur = next;
    }
...
static int seg_check_bitstream(struct AVEMFormatContext *s, const AVEMPacket *pkt)
{
    SegmentContext *seg = s->priv_data;
    AVEMFormatContext *oc = seg->avf;
    if (oc->oformat->check_bitstream) {
        ...
        if (ret == 1) {
            AVEMStream *st = s->streams[pkt->stream_index];
            AVEMStream *ost = oc->streams[pkt->stream_index];
...
static const AVEMClass seg_class = {
...
AVEMOutputFormat ff_segment_muxer = {
    .name           = "segment",
...
static const AVEMClass sseg_class = {
...
AVEMOutputFormat ff_stream_segment_muxer = {
