...
static int tmv_probe(AVEMProbeData *p)
{
...
static int tmv_read_header(AVEMFormatContext *s)
{
    ...
    if (avio_em_rl32(pb) != TMV_TAG)
        return -1;

    if (!(vst = avformat_em_new_stream(s, NULL)))
        return AVERROR(ENOMEM);

    if (!(ast = avformat_em_new_stream(s, NULL)))
        return AVERROR(ENOMEM);
    ...
    tmv->audio_chunk_size   = avio_em_rl16(pb);
    if (!tmv->audio_chunk_size) {
        av_em_log(s, AV_LOG_ERROR, "invalid audio chunk size\n");
        return -1;
    }

    comp_method             = avio_em_r8(pb);
    if (comp_method) {
        av_em_log(s, AV_LOG_ERROR, "unsupported compression method %d\n",
               comp_method);
        return -1;
    }

    char_cols = avio_em_r8(pb);
    char_rows = avio_em_r8(pb);
    tmv->video_chunk_size = char_cols * char_rows * 2;

    features  = avio_em_r8(pb);
    if (features & ~(TMV_PADDING | TMV_STEREO)) {
        av_em_log(s, AV_LOG_ERROR, "unsupported features 0x%02x\n",
    ...
    avpriv_em_set_pts_info(ast, 32, 1, ast->codecpar->sample_rate);

    fps.num = ast->codecpar->sample_rate * ast->codecpar->channels;
    fps.den = tmv->audio_chunk_size;
    av_em_reduce(&fps.num, &fps.den, fps.num, fps.den, 0xFFFFFFFFLL);
...
static int tmv_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    TMVContext *tmv   = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    ret = av_em_get_packet(pb, pkt, pkt_size);

    if (tmv->stream_index)
        avio_em_skip(pb, tmv->padding);
...
static int tmv_read_seek(AVEMFormatContext *s, int stream_index,
                         int64_t timestamp, int flags)
{
    ...
    if (avio_em_seek(s->pb, pos + TMV_HEADER_SIZE, SEEK_SET) < 0)
        return -1;
    tmv->stream_index = 0;
    return 0;
}

AVEMInputFormat ff_tmv_demuxer = {
    .name           = "tmv",
