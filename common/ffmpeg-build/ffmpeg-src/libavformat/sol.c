...
static int sol_probe(AVEMProbeData *p)
{
    /* check file header */
...
static enum AVEMCodecID sol_codec_id(int magic, int type)
{
    if (type & SOL_DPCM)
...
static int sol_read_header(AVEMFormatContext *s)
{
    ...
    /* check ".snd" header */
    magic = avio_em_rl16(pb);
    tag = avio_em_rl32(pb);
    if (tag != MKTAG('S', 'O', 'L', 0))
        return -1;
    rate = avio_em_rl16(pb);
    type = avio_em_r8(pb);
    avio_em_skip(pb, 4); /* size */
    if (magic != 0x0B8D)
        avio_em_r8(pb); /* newer SOLs contain padding byte */
...
static int sol_read_packet(AVEMFormatContext *s,
                          AVEMPacket *pkt)
{
    int ret;
    ...
    if (avio_em_feof(s->pb))
        return AVERROR(EIO);
    ret= av_em_get_packet(s->pb, pkt, MAX_SIZE);
    if (ret < 0)
        return ret;
...
AVEMInputFormat ff_sol_demuxer = {
    .name           = "sol",
