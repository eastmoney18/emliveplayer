...
static int read_header(AVEMFormatContext *s)
{
    ...
    /* read header */
    avio_em_rl16(pb);   /* total number of chunks */
    frame_rate = avio_em_rl16(pb);
    avio_em_rl16(pb);   /* ibm-pc video bios mode */
    width = avio_em_rl16(pb);
    height = avio_em_rl16(pb);
    avio_em_skip(pb, length - 10);  /* unknown data */
    ...
    avpriv_em_set_pts_info(st, 64, 1, frame_rate);
    ...
    if (length == MM_HEADER_LEN_AV) {
        ...
        avpriv_em_set_pts_info(st, 64, 1, 8000); /* 8000 hz */
    }
                           ...
                           AVEMPacket *pkt)
{
    ...
    while(1) {
        ...
        if (avio_em_read(pb, preamble, MM_PREAMBLE_SIZE) != MM_PREAMBLE_SIZE) {
            return AVERROR(EIO);
        }
        ...
        case MM_TYPE_INTER_HHV :
            ...
            if (av_em_new_packet(pkt, length + MM_PREAMBLE_SIZE))
                return AVERROR(ENOMEM);
            memcpy(pkt->data, preamble, MM_PREAMBLE_SIZE);
            if (avio_em_read(pb, pkt->data + MM_PREAMBLE_SIZE, length) != length)
                return AVERROR(EIO);
        ...
        case MM_TYPE_AUDIO :
            if (av_em_get_packet(s->pb, pkt, length)<0)
                return AVERROR(ENOMEM);
            pkt->stream_index = 1;
        ...
        default :
            av_em_log(s, AV_LOG_INFO, "unknown chunk type 0x%x\n", type);
            avio_em_skip(pb, length);
        }
    }
}

AVEMInputFormat ff_mm_demuxer = {
    .name           = "mm",
