...
static int read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    while (!frame_complete) {
        ...
        type  = avio_em_rb16(s->pb); // 257 or 258
        size  = avio_em_rb16(s->pb);
        flags = avio_em_rb16(s->pb); //some flags, 0x80 indicates end of frame
                avio_em_rb16(s->pb); //packet number
        pts   = avio_em_rb32(s->pb);
                avio_em_rb32(s->pb); //6A 13 E3 88
        ...
        if (!first_pkt) {
            ...
            ret = av_em_append_packet(s->pb, pkt, size);
            if (ret < 0) {
                av_em_log(s, AV_LOG_ERROR, "failed to grow packet\n");
                av_em_packet_unref(pkt);
                return ret;
            }
        }
        if (ret < size) {
            av_em_log(s, AV_LOG_ERROR, "Truncated packet! Read %d of %d bytes\n",
                   ret, size);
...
AVEMInputFormat ff_iv8_demuxer = {
    .name           = "iv8",
