...
static const AVEMClass latm_muxer_class = {
...
static int latm_decode_extradata(LATMContext *ctx, uint8_t *buf, int size)
{
    ...
    if (size > MAX_EXTRADATA_SIZE) {
        av_em_log(ctx, AV_LOG_ERROR, "Extradata is larger than currently supported.\n");
    ...
    if (ctx->object_type == AOT_ALS && (ctx->off & 7)) {
        ...
        av_em_log(ctx, AV_LOG_ERROR, "BUG: ALS offset is not byte-aligned\n");
        return AVERROR_INVALIDDATA;
    }
    /* FIXME: are any formats not allowed in LATM? */

    if (m4ac.object_type > AOT_SBR && m4ac.object_type != AOT_ALS) {
        av_em_log(ctx, AV_LOG_ERROR, "Muxing MPEG-4 AOT %d in LATM is not supported\n", m4ac.object_type);
        return AVERROR_INVALIDDATA;
    }
...
static int latm_write_header(AVEMFormatContext *s)
{
...
static void latm_write_frame_header(AVEMFormatContext *s, PutBitContext *bs)
{
    LATMContext *ctx = s->priv_data;
    AVEMCodecParameters *par = s->streams[0]->codecpar;
    int header_size;
...
static int latm_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    LATMContext *ctx = s->priv_data;
    ...
    avio_em_write(pb, loas_header, 3);
    avio_em_write(pb, ctx->buffer, len);

    return 0;

too_large:
    av_em_log(s, AV_LOG_ERROR, "LATM packet size larger than maximum size 0x1fff\n");
    return AVERROR_INVALIDDATA;
}

static int latm_check_bitstream(struct AVEMFormatContext *s, const AVEMPacket *pkt)
{
    int ret = 1;
...
AVEMOutputFormat ff_latm_muxer = {
    .name           = "latm",
