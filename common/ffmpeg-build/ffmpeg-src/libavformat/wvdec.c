...
static int wv_probe(AVEMProbeData *p)
{
    /* check file header */
    if (p->buf_size <= 32)
        return 0;
...
static int wv_read_block_header(AVEMFormatContext *ctx, AVEMIOContext *pb)
{
    ...
    wc->pos = avio_em_tell(pb);

    /* don't return bogus packets with the ape tag data */
    if (wc->apetag_start && wc->pos >= wc->apetag_start)
        return AVERROR_EOF;

    ret = avio_em_read(pb, wc->block_header, WV_HEADER_SIZE);
    if (ret != WV_HEADER_SIZE)
        return (ret < 0) ? ret : AVERROR_EOF;

    ret = ff_wv_parse_header(&wc->header, wc->block_header);
    if (ret < 0) {
        av_em_log(ctx, AV_LOG_ERROR, "Invalid block header.\n");
        return ret;
    }

    if (wc->header.version < 0x402 || wc->header.version > 0x410) {
        av_em_log(ctx, AV_LOG_ERROR, "Unsupported version %03X\n", wc->header.version);
    ...
    if ((rate == -1 || !chan) && !wc->block_parsed) {
        int64_t block_end = avio_em_tell(pb) + wc->header.blocksize;
        if (!pb->seekable) {
            av_em_log(ctx, AV_LOG_ERROR,
                   "Cannot determine additional parameters\n");
            return AVERROR_INVALIDDATA;
        }
        while (avio_em_tell(pb) < block_end && !avio_em_feof(pb)) {
            int id, size;
            id   = avio_em_r8(pb);
            size = (id & 0x80) ? avio_em_rl24(pb) : avio_em_r8(pb);
            size <<= 1;
            if (id & 0x40)
                size--;
            switch (id & 0x3F) {
            case 0xD:
                ...
                switch (size - 2) {
                case 0:
                    chmask = avio_em_r8(pb);
                    break;
                case 1:
                    chmask = avio_em_rl16(pb);
                    break;
                case 2:
                    chmask = avio_em_rl24(pb);
                    break;
                case 3:
                    chmask = avio_em_rl32(pb);
                    break;
                case 5:
                    avio_em_skip(pb, 1);
                    chan  |= (avio_em_r8(pb) & 0xF) << 8;
                    chmask = avio_em_rl24(pb);
                    break;
                default:
                    av_em_log(ctx, AV_LOG_ERROR,
        ...
        avio_em_seek(pb, block_end - wc->header.blocksize, SEEK_SET);
    }
    if (!wc->bpp)
        wc->bpp    = bpp;
    if (!wc->chan)
...
static int wv_read_header(AVEMFormatContext *s)
{
    ...
    for (;;) {
        if ((ret = wv_read_block_header(s, pb)) < 0)
            return ret;
        if (!wc->header.samples)
            avio_em_skip(pb, wc->header.blocksize);
        else
            break;
    }
...
static int wv_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    pos = wc->pos;
    if (av_em_new_packet(pkt, wc->header.blocksize + WV_HEADER_SIZE) < 0)
        return AVERROR(ENOMEM);
    memcpy(pkt->data, wc->block_header, WV_HEADER_SIZE);
    ret = avio_em_read(s->pb, pkt->data + WV_HEADER_SIZE, wc->header.blocksize);
    ...
    while (!(wc->header.flags & WV_FLAG_FINAL_BLOCK)) {
        ...
        off = pkt->size;
        if ((ret = av_em_grow_packet(pkt, WV_HEADER_SIZE + wc->header.blocksize)) < 0) {
            av_em_packet_unref(pkt);
            return ret;
        }
        memcpy(pkt->data + off, wc->block_header, WV_HEADER_SIZE);

        ret = avio_em_read(s->pb, pkt->data + off + WV_HEADER_SIZE, wc->header.blocksize);
        if (ret != wc->header.blocksize) {
            av_em_packet_unref(pkt);
            return (ret < 0) ? ret : AVERROR_EOF;
        }
