...
static int aqt_probe(AVEMProbeData *p)
{
    int frame;
...
static int aqt_read_header(AVEMFormatContext *s)
{
    ...
    avpriv_em_set_pts_info(st, 64, aqt->frame_rate.den, aqt->frame_rate.num);
...
static int aqt_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    AQTitleContext *aqt = s->priv_data;
    return ff_subtitles_queue_read_packet(&aqt->q, pkt);
}

static int aqt_read_seek(AVEMFormatContext *s, int stream_index,
...
static int aqt_read_close(AVEMFormatContext *s)
{
    AQTitleContext *aqt = s->priv_data;
...
static const AVEMClass aqt_class = {
    .class_name = "aqtdec",
...
AVEMInputFormat ff_aqtitle_demuxer = {
    .name           = "aqtitle",
