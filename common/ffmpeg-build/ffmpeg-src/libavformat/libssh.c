...
static av_cold int libssh_create_ssh_session(LIBSSHContext *libssh, const char* hostname, unsigned int port)
{
    ...
    if (!(libssh->session = ssh_new())) {
        av_em_log(libssh, AV_LOG_ERROR, "SSH session creation failed: %s\n", ssh_get_error(libssh->session));
    ...
    if (ssh_options_parse_config(libssh->session, NULL) < 0) {
        av_em_log(libssh, AV_LOG_WARNING, "Could not parse the config file.\n");
    }

    if (ssh_connect(libssh->session) != SSH_OK) {
        av_em_log(libssh, AV_LOG_ERROR, "Connection failed: %s\n", ssh_get_error(libssh->session));
        return AVERROR(EIO);
    }
...
static av_cold int libssh_authentication(LIBSSHContext *libssh, const char *user, const char *password)
{
        ...
        if (libssh->priv_key) {
            ...
            if (!ssh_try_publickey_from_file(libssh->session, libssh->priv_key, &pub_key, &type)) {
                ...
                if (ssh_userauth_pubkey(libssh->session, NULL, pub_key, priv_key) == SSH_AUTH_SUCCESS) {
                    av_em_log(libssh, AV_LOG_DEBUG, "Authentication successful with selected private key.\n");
                    authorized = 1;
                }
            } else {
                av_em_log(libssh, AV_LOG_DEBUG, "Invalid key is provided.\n");
                return AVERROR(EACCES);
            }
        } else if (ssh_userauth_autopubkey(libssh->session, password) == SSH_AUTH_SUCCESS) {
            av_em_log(libssh, AV_LOG_DEBUG, "Authentication successful with auto selected key.\n");
            authorized = 1;
        }
    }
    ...
    if (!authorized && (auth_methods & SSH_AUTH_METHOD_PASSWORD)) {
        if (ssh_userauth_password(libssh->session, NULL, password) == SSH_AUTH_SUCCESS) {
            av_em_log(libssh, AV_LOG_DEBUG, "Authentication successful with password.\n");
            authorized = 1;
        }
    }

    if (!authorized) {
        av_em_log(libssh, AV_LOG_ERROR, "Authentication failed.\n");
        return AVERROR(EACCES);
    }
...
static av_cold int libssh_create_sftp_session(LIBSSHContext *libssh)
{
    if (!(libssh->sftp = sftp_new(libssh->session))) {
        av_em_log(libssh, AV_LOG_ERROR, "SFTP session creation failed: %s\n", ssh_get_error(libssh->session));
        return AVERROR(ENOMEM);
    }

    if (sftp_init(libssh->sftp) != SSH_OK) {
        av_em_log(libssh, AV_LOG_ERROR, "Error initializing sftp session: %s\n", ssh_get_error(libssh->session));
        return AVERROR(EIO);
    }
...
static av_cold int libssh_open_file(LIBSSHContext *libssh, int flags, const char *file)
{
    ...
    if (!(libssh->file = sftp_open(libssh->sftp, file, access, 0666))) {
        av_em_log(libssh, AV_LOG_ERROR, "Error opening sftp file: %s\n", ssh_get_error(libssh->session));
        return AVERROR(EIO);
    }
...
static av_cold void libssh_stat_file(LIBSSHContext *libssh)
{
    ...
    if (!(stat = sftp_fstat(libssh->file))) {
        av_em_log(libssh, AV_LOG_WARNING, "Cannot stat remote file.\n");
        libssh->filesize = -1;
    } else {
...
static av_cold int libssh_close(EMURLContext *h)
{
...
static av_cold int libssh_connect(EMURLContext *h, const char *url, char *path, size_t path_size)
{
    LIBSSHContext *libssh = h->priv_data;
    ...
    user = av_em_strtok(credencials, ":", &end);
    pass = av_em_strtok(end, ":", &end);
...
static av_cold int libssh_open(EMURLContext *h, const char *url, int flags)
{
    int ret;
    LIBSSHContext *libssh = h->priv_data;
...
static int64_t libssh_seek(EMURLContext *h, int64_t pos, int whence)
{
    LIBSSHContext *libssh = h->priv_data;
    int64_t newpos;

    if (libssh->filesize == -1 && (whence == AVSEEK_SIZE || whence == SEEK_END)) {
        av_em_log(h, AV_LOG_ERROR, "Error during seeking.\n");
        return AVERROR(EIO);
    }
...
static int libssh_read(EMURLContext *h, unsigned char *buf, int size)
{
    LIBSSHContext *libssh = h->priv_data;
    int bytes_read;

    if ((bytes_read = sftp_read(libssh->file, buf, size)) < 0) {
        av_em_log(libssh, AV_LOG_ERROR, "Read error.\n");
        return AVERROR(EIO);
    }
    return bytes_read;
}

static int libssh_write(EMURLContext *h, const unsigned char *buf, int size)
{
    LIBSSHContext *libssh = h->priv_data;
    int bytes_written;

    if ((bytes_written = sftp_write(libssh->file, buf, size)) < 0) {
        av_em_log(libssh, AV_LOG_ERROR, "Write error.\n");
        return AVERROR(EIO);
    }
    return bytes_written;
}

static int libssh_open_dir(EMURLContext *h)
{
    LIBSSHContext *libssh = h->priv_data;
    int ret;
    char path[MAX_URL_SIZE];
    ...
    if (!(libssh->dir = sftp_opendir(libssh->sftp, path))) {
        av_em_log(libssh, AV_LOG_ERROR, "Error opening sftp dir: %s\n", ssh_get_error(libssh->session));
        ret = AVERROR(EIO);
        goto fail;
    }
...
static int libssh_read_dir(EMURLContext *h, AVEMIODirEntry **next)
{
    LIBSSHContext *libssh = h->priv_data;
    sftp_attributes attr = NULL;
    AVEMIODirEntry *entry;

    *next = entry = em_alloc_dir_entry();
    if (!entry)
        return AVERROR(ENOMEM);
    ...
    entry->name = av_em_strdup(attr->name);
...
static int libssh_close_dir(EMURLContext *h)
{
    LIBSSHContext *libssh = h->priv_data;
    if (libssh->dir)
...
static int libssh_delete(EMURLContext *h)
{
    int ret;
    LIBSSHContext *libssh = h->priv_data;
    sftp_attributes attr = NULL;
...
static int libssh_move(EMURLContext *h_src, EMURLContext *h_dst)
{
    int ret;
...
static const AVEMClass libssh_context_class = {
    .class_name     = "libssh",
...
const EMURLProtocol em_libssh_protocol = {
    .name                = "sftp",
