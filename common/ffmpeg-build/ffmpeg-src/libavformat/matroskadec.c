...
static int matroska_read_close(AVEMFormatContext *s);

static int matroska_resync(MatroskaDemuxContext *matroska, int64_t last_pos)
{
    AVEMIOContext *pb = matroska->ctx->pb;
    uint32_t id;
    matroska->current_id = 0;
    matroska->num_levels = 0;

    /* seek to next position to resync from */
    if (avio_em_seek(pb, last_pos + 1, SEEK_SET) < 0)
        goto eof;
 ...
 */
static int ebml_read_num(MatroskaDemuxContext *matroska, AVEMIOContext *pb,
                         int max_size, uint64_t *number)
{
    int read = 1, n = 1;
    uint64_t total = 0;

    /* The first byte tells us the length in bytes - avio_em_r8() can normally
     * return 0, but since that's not a valid first ebmlID byte, we can
     * use it safely here to catch EOS. */
    if (!(total = avio_em_r8(pb))) {
        /* we might encounter EOS here */
        if (!avio_em_feof(pb)) {
            int64_t pos = avio_em_tell(pb);
            av_em_log(matroska->ctx, AV_LOG_ERROR,
 ...
 * encodings.
 */
static int ebml_read_length(MatroskaDemuxContext *matroska, AVEMIOContext *pb,
                            uint64_t *number)
{
 ...
 */
static int ebml_read_uint(AVEMIOContext *pb, int size, uint64_t *num)
{
    int n = 0;
 ...
 * 0 is success, < 0 is failure.
 */
static int ebml_read_sint(AVEMIOContext *pb, int size, int64_t *num)
{
    int n = 1;
    ...
    if (size == 0) {
        ...
        *num = sign_extend(avio_em_r8(pb), 8);

        /* big-endian ordering; build up number */
        while (n++ < size)
            *num = ((uint64_t)*num << 8) | avio_em_r8(pb);
    }
 ...
 * 0 is success, < 0 is failure.
 */
static int ebml_read_float(AVEMIOContext *pb, int size, double *num)
{
    if (size == 0)
        *num = 0;
    else if (size == 4)
        *num = av_int2float(avio_em_rb32(pb));
    else if (size == 8)
        *num = av_int2double(avio_em_rb64(pb));
    else
        return AVERROR_INVALIDDATA;
 ...
 * 0 is success, < 0 is failure.
 */
static int ebml_read_ascii(AVEMIOContext *pb, int size, char **str)
{
    char *res;

    /* EBML strings are usually not 0-terminated, so we allocate one
     * byte more, read the string and NULL-terminate it ourselves. */
    if (!(res = av_em_alloc(size + 1)))
        return AVERROR(ENOMEM);
    if (avio_em_read(pb, (uint8_t *) res, size) != size) {
        av_em_free(res);
        return AVERROR(EIO);
    }
    (res)[size] = '\0';
 ...
 * 0 is success, < 0 is failure.
 */
static int ebml_read_binary(AVEMIOContext *pb, int length, EbmlBin *bin)
{
    av_em_fast_padded_malloc(&bin->data, &bin->size, length);
    if (!bin->data)
        return AVERROR(ENOMEM);

    bin->size = length;
    bin->pos  = avio_em_tell(pb);
    if (avio_em_read(pb, bin->data, length) != length) {
        av_em_freep(&bin->data);
        bin->size = 0;
        return AVERROR(EIO);
    }
...
{
    AVEMIOContext pb;
    emio_init_context(&pb, data, size, 0, NULL, NULL, NULL, NULL);
                         ...
                         uint32_t id, void *data)
{
    ...
    if (!syntax[i].id && id != EBML_ID_VOID && id != EBML_ID_CRC32) {
        av_em_log(matroska->ctx, AV_LOG_DEBUG, "Unknown entry 0x%"PRIX32"\n", id);
    }
    ...
    for (i = 0; syntax[i].id; i++)
        ...
        case EBML_UTF8:
            ...
            if (syntax[i].def.s) {
                ...
                *dst = av_em_strdup(syntax[i].def.s);
                if (!*dst)
                    return AVERROR(ENOMEM);
            }
            break;
                                                        ...
                                                        uint32_t id)
{
    ...
    if (matroska->num_level1_elems >= FF_ARRAY_ELEMS(matroska->level1_elems)) {
        av_em_log(matroska->ctx, AV_LOG_ERROR, "Too many level1 elements or circular seekheads.\n");
        return NULL;
    }
                           ...
                           EbmlSyntax *syntax, void *data)
{
    ...
    if (syntax->list_elem_size) {
        EbmlList *list = data;
        newelem = av_em_realloc_array(list->elem, list->nb_elem + 1, syntax->list_elem_size);
        if (!newelem)
            return AVERROR(ENOMEM);
        list->elem = newelem;
    ...
    case EBML_STOP:
        return 1;
    default:
        if (emio_limit(pb, length) != length)
            return AVERROR(EIO);
        return avio_em_skip(pb, length) < 0 ? AVERROR(EIO) : 0;
    }
    if (res == AVERROR_INVALIDDATA)
        av_em_log(matroska->ctx, AV_LOG_ERROR, "Invalid element\n");
    else if (res == AVERROR(EIO))
        av_em_log(matroska->ctx, AV_LOG_ERROR, "Read error\n");
    return res;
}
...
static void ebml_free(EbmlSyntax *syntax, void *data)
{
    ...
    for (i = 0; syntax[i].id; i++) {
        ...
        case EBML_STR:
        case EBML_UTF8:
            av_em_freep(data_off);
            break;
        case EBML_BIN:
            av_em_freep(&((EbmlBin *) data_off)->data);
            break;
        case EBML_LEVEL1:
        case EBML_NEST:
 ...
 * Autodetecting...
 */
static int matroska_probe(AVEMProbeData *p)
{
    uint64_t total = 0;
                                                 ...
                                                 int num)
{
    ...
    av_em_log(matroska->ctx, AV_LOG_ERROR, "Invalid track number %d\n", num);
    return NULL;
}
                                  ...
                                  MatroskaTrack *track)
{
    ...
    case MATROSKA_TRACK_ENCODING_COMP_HEADERSTRIP:
    {
        ...
        if (header_size && !header) {
            av_em_log(NULL, AV_LOG_ERROR, "Compression size but no data in headerstrip\n");
            return -1;
        }
    ...
    case MATROSKA_TRACK_ENCODING_COMP_LZO:
        do {
            olen       = pkt_size *= 3;
            newpktdata = av_em_realloc(pkt_data, pkt_size + AV_LZO_OUTPUT_PADDING);
            if (!newpktdata) {
                result = AVERROR(ENOMEM);
                goto failed;
            }
            pkt_data = newpktdata;
            result   = av_em_lzo1x_decode(pkt_data, &olen, data, &isize);
    ...
    case MATROSKA_TRACK_ENCODING_COMP_ZLIB:
    {
        ...
        do {
            pkt_size  *= 3;
            newpktdata = av_em_realloc(pkt_data, pkt_size);
            if (!newpktdata) {
                inflateEnd(&zstream);
                result = AVERROR(ENOMEM);
                goto failed;
...
static void matroska_convert_tag(AVEMFormatContext *s, EbmlList *list,
                                 AVEMDictionary **metadata, char *prefix)
{
    ...
    for (i = 0; i < list->nb_elem; i++) {
        ...
        if (!tags[i].name) {
            av_em_log(s, AV_LOG_WARNING, "Skipping invalid tag with no TagName.\n");
            continue;
        }
        if (prefix)
            snprintf(key, sizeof(key), "%s/%s", prefix, tags[i].name);
        else
            av_em_strlcpy(key, tags[i].name, sizeof(key));
        if (tags[i].def || !lang) {
            av_em_dict_set(metadata, key, tags[i].string, 0);
            if (tags[i].sub.nb_elem)
                matroska_convert_tag(s, &tags[i].sub, metadata, key);
        }
        if (lang) {
            av_em_strlcat(key, "-", sizeof(key));
            av_em_strlcat(key, lang, sizeof(key));
            av_em_dict_set(metadata, key, tags[i].string, 0);
            if (tags[i].sub.nb_elem)
                matroska_convert_tag(s, &tags[i].sub, metadata, key);
        }
    }
    ff_em_metadata_conv(metadata, NULL, ff_mkv_metadata_conv);
}

static void matroska_convert_tags(AVEMFormatContext *s)
{
                                         ...
                                         uint64_t pos)
{
    ...
    int64_t before_pos = avio_em_tell(matroska->ctx->pb);
    MatroskaLevel level;
    int64_t offset;
    int ret = 0;
    ...
    if (avio_em_seek(matroska->ctx->pb, offset, SEEK_SET) == offset) {
        /* We don't want to lose our seekhead level, so we add
         * a dummy. This is a crude hack. */
        if (matroska->num_levels == EBML_MAX_DEPTH) {
            av_em_log(matroska->ctx, AV_LOG_INFO,
    ...
    /* seek back */
    avio_em_seek(matroska->ctx->pb, before_pos, SEEK_SET);
...
static void matroska_add_index_entries(MatroskaDemuxContext *matroska)
{
    ...
    if (index[1].time > 1E14 / matroska->time_scale) {
        av_em_log(matroska->ctx, AV_LOG_WARNING, "Dropping apparently-broken index.\n");
        return;
    }
    for (i = 0; i < index_list->nb_elem; i++) {
        ...
        for (j = 0; j < pos_list->nb_elem; j++) {
            ...
            if (track && track->stream)
                av_em_add_index_entry(track->stream,
...
static void matroska_metadata_creation_time(AVEMDictionary **metadata, int64_t date_utc)
{
    char buffer[32];
...
static int matroska_parse_flac(AVEMFormatContext *s,
                               MatroskaTrack *track,
                               int *offset)
{
    AVEMStream *st = track->stream;
    uint8_t *p = track->codec_priv.data;
    int size   = track->codec_priv.size;

    if (size < 8 + FLAC_STREAMINFO_SIZE || p[4] & 0x7f) {
        av_em_log(s, AV_LOG_WARNING, "Invalid FLAC private data\n");
    ...
    *offset = 8;
    ...
    while (size >= 4) {
        ...
        if (block_type == FLAC_METADATA_TYPE_VORBIS_COMMENT) {
            ...
            AVEMDictionaryEntry *chmask;

            ff_vorbis_comment(s, &dict, p, block_size, 0);
            chmask = av_em_dict_get(dict, "WAVEFORMATEXTENSIBLE_CHANNEL_MASK", NULL, 0);
            if (chmask) {
...
static int mkv_parse_video_color(AVEMStream *st, const MatroskaTrack *track) {
    ...
    if (has_mastering_primaries || has_mastering_luminance) {
        ...
        AVMasteringDisplayMetadata *metadata =
            (AVMasteringDisplayMetadata*) av_em_stream_new_side_data(
...
static int get_qt_codec(MatroskaTrack *track, uint32_t *fourcc, enum AVEMCodecID *codec_id)
{
    const AVEMCodecTag *codec_tags;

    codec_tags = track->type == MATROSKA_TRACK_TYPE_VIDEO ?
            em_codec_movvideo_tags : em_codec_movaudio_tags;
     ...
     * size at the start. */
    if (em_codec_get_id(codec_tags, AV_RL32(track->codec_priv.data))) {
        uint8_t *p = av_em_realloc(track->codec_priv.data,
    ...
    *codec_id = em_codec_get_id(codec_tags, *fourcc);

    return 0;
}

static int matroska_parse_tracks(AVEMFormatContext *s)
{
    MatroskaDemuxContext *matroska = s->priv_data;
    MatroskaTrack *tracks = matroska->tracks.elem;
    AVEMStream *st;
    ...
    for (i = 0; i < matroska->tracks.nb_elem; i++) {
        ...
        if (track->audio.samplerate < 0 || track->audio.samplerate > INT_MAX ||
            ...
            av_em_log(matroska->ctx, AV_LOG_WARNING,
        ...
        if (encodings_list->nb_elem > 1) {
            ...
            if (encodings[0].type) {
                if (encodings[0].encryption.key_id.size > 0) {
                    ...
                    key_id_base64 = av_em_alloc(b64_size);
                    if (key_id_base64 == NULL)
                        return AVERROR(ENOMEM);

                    av_em_base64_encode(key_id_base64, b64_size,
        ...
        st = track->stream = avformat_em_new_stream(s, NULL);
        if (!st) {
            av_em_free(key_id_base64);
            return AVERROR(ENOMEM);
        }

        if (key_id_base64) {
            /* export encryption key id as base64 metadata tag */
            av_em_dict_set(&st->metadata, "enc_key_id", key_id_base64, 0);
            av_em_freep(&key_id_base64);
        }
        ...
        if (!strcmp(track->codec_id, "V_MS/VFW/FOURCC") &&
            ...
            codec_id            = em_codec_get_id(em_codec_bmp_tags,
                                                  fourcc);
            if (!codec_id)
                codec_id        = em_codec_get_id(em_codec_movvideo_tags,
                                                  fourcc);
            extradata_offset    = 40;
                   ...
                   track->codec_priv.data) {
            int ret;
            emio_init_context(&b, track->codec_priv.data,
                              track->codec_priv.size,
            ...
            if (fourcc == 0) {
                if (sample_size == 8) {
                    fourcc = MKTAG('r','a','w',' ');
                    codec_id = em_codec_get_id(em_codec_movaudio_tags, fourcc);
            ...
            if (codec_id == AV_CODEC_ID_NONE && AV_RL32(track->codec_priv.data+4) == AV_RL32("SMI ")) {
                ...
                codec_id = em_codec_get_id(em_codec_movvideo_tags, fourcc);
            }
            if (codec_id == AV_CODEC_ID_NONE) {
                char buf[32];
                av_em_get_codec_tag_string(buf, sizeof(buf), fourcc);
                av_em_log(matroska->ctx, AV_LOG_ERROR,
            ...
            if (track->codec_priv.size >= 86) {
                ...
                emio_init_context(&b, track->codec_priv.data,
                                  track->codec_priv.size,
                                  0, NULL, NULL, NULL, NULL);
                if (ff_em_get_qtpalette(codec_id, &b, track->palette)) {
                    bit_depth &= 0x1F;
                    track->has_palette = 1;
                }
            }
            ...
            extradata   = av_em_mallocz(5 + AV_INPUT_BUFFER_PADDING_SIZE);
            if (!extradata)
                return AVERROR(ENOMEM);
            ...
            extradata      = av_em_mallocz(extradata_size +
            ...
            extradata_size = 30;
            extradata      = av_em_mallocz(extradata_size + AV_INPUT_BUFFER_PADDING_SIZE);
            if (!extradata)
                return AVERROR(ENOMEM);
            emio_init_context(&b, extradata, extradata_size, 1,
                              NULL, NULL, NULL, NULL);
            avio_em_write(&b, "TTA1", 4);
            avio_em_wl16(&b, 1);
            if (track->audio.channels > UINT16_MAX ||
                ...
                av_em_freep(&extradata);
                if (matroska->ctx->error_recognition & AV_EF_EXPLODE)
                    return AVERROR_INVALIDDATA;
                else
                    continue;
            }
            avio_em_wl16(&b, track->audio.channels);
            avio_em_wl16(&b, track->audio.bitdepth);
            if (track->audio.out_samplerate < 0 || track->audio.out_samplerate > INT_MAX)
                return AVERROR_INVALIDDATA;
            avio_em_wl32(&b, track->audio.out_samplerate);
            avio_em_wl32(&b, av_em_rescale((matroska->duration * matroska->time_scale),
            ...
            emio_init_context(&b, track->codec_priv.data,
                              ...
                              0, NULL, NULL, NULL, NULL);
            avio_em_skip(&b, 22);
            flavor                       = avio_em_rb16(&b);
            track->audio.coded_framesize = avio_em_rb32(&b);
            avio_em_skip(&b, 12);
            track->audio.sub_packet_h    = avio_em_rb16(&b);
            track->audio.frame_size      = avio_em_rb16(&b);
            track->audio.sub_packet_size = avio_em_rb16(&b);
            if (flavor                        < 0 ||
                ...
                return AVERROR_INVALIDDATA;
            track->audio.buf = av_em_malloc_array(track->audio.sub_packet_h,
        ...
        if (codec_id == AV_CODEC_ID_NONE)
            av_em_log(matroska->ctx, AV_LOG_INFO,
                   "Unknown/unsupported AVEMCodecID %s.\n", track->codec_id);

        if (track->time_scale < 0.01)
            track->time_scale = 1.0;
        avpriv_em_set_pts_info(st, 64, matroska->time_scale * track->time_scale,
                            1000 * 1000 * 1000);    /* 64 bit pts in ns */

        /* convert the delay from ns to the track timebase */
        track->codec_delay_in_track_tb = av_em_rescale_q(track->codec_delay,
        ...
        if (strcmp(track->language, "und"))
            av_em_dict_set(&st->metadata, "language", track->language, 0);
        av_em_dict_set(&st->metadata, "title", track->name, 0);
        ...
        if (!st->codecpar->extradata) {
            if (extradata) {
                ...
                if (em_alloc_extradata(st->codecpar, track->codec_priv.size))
                    return AVERROR(ENOMEM);
        ...
        if (track->type == MATROSKA_TRACK_TYPE_VIDEO) {
            ...
            av_em_reduce(&st->sample_aspect_ratio.num,
            ...
            if (track->video.stereo_mode && track->video.stereo_mode < MATROSKA_VIDEO_STEREOMODE_TYPE_NB)
                av_em_dict_set(&st->metadata, "stereo_mode", ff_matroska_video_stereo_mode[track->video.stereo_mode], 0);

            /* export alpha mode flag as metadata tag  */
            if (track->video.alpha_mode)
                av_em_dict_set(&st->metadata, "alpha_mode", "1", 0);
            ...
            for (j=0; j < track->operation.combine_planes.nb_elem; j++) {
                ...
                for (k=0; k < matroska->tracks.nb_elem; k++)
                    if (planes[j].uid == tracks[k].uid && tracks[k].stream) {
                        av_em_dict_set(&tracks[k].stream->metadata,
                                    "stereo_mode", buf, 0);
                        break;
                    }
            }
            ...
            if (track->codec_delay > 0) {
                st->codecpar->initial_padding = av_em_rescale_q(track->codec_delay,
                                                             (AVEMRational){1, 1000000000},
                                                             (AVEMRational){1, st->codecpar->codec_id == AV_CODEC_ID_OPUS ?
                                                                             48000 : st->codecpar->sample_rate});
            }
            if (track->seek_preroll > 0) {
                st->codecpar->seek_preroll = av_em_rescale_q(track->seek_preroll,
                                                          (AVEMRational){1, 1000000000},
                                                          (AVEMRational){1, st->codecpar->sample_rate});
            }
...
static int matroska_read_header(AVEMFormatContext *s)
{
    MatroskaDemuxContext *matroska = s->priv_data;
    ...
    if (ebml_parse(matroska, ebml_syntax, &ebml) || !ebml.doctype) {
        av_em_log(matroska->ctx, AV_LOG_ERROR, "EBML header parsing failed\n");
    ...
    if (i >= FF_ARRAY_ELEMS(matroska_doctypes)) {
        av_em_log(s, AV_LOG_WARNING, "Unknown EBML doctype '%s'\n", ebml.doctype);
        if (matroska->ctx->error_recognition & AV_EF_EXPLODE) {
            ebml_free(ebml_syntax, &ebml);
            return AVERROR_INVALIDDATA;
        }
    ...
    pos = avio_em_tell(matroska->ctx->pb);
    ...
    if (matroska->duration)
        matroska->ctx->duration = matroska->duration * matroska->time_scale *
                                  1000 / AV_TIME_BASE;
    av_em_dict_set(&s->metadata, "title", matroska->title, 0);
    av_em_dict_set(&s->metadata, "encoder", matroska->muxingapp, 0);
    ...
    for (j = 0; j < attachments_list->nb_elem; j++) {
        if (!(attachments[j].filename && attachments[j].mime &&
            ...
            av_em_log(matroska->ctx, AV_LOG_ERROR, "incomplete attachment\n");
        } else {
            AVEMStream *st = avformat_em_new_stream(s, NULL);
            if (!st)
                break;
            av_em_dict_set(&st->metadata, "filename", attachments[j].filename, 0);
            av_em_dict_set(&st->metadata, "mimetype", attachments[j].mime, 0);
            ...
            if (st->codecpar->codec_id != AV_CODEC_ID_NONE) {
                ...
                if ((res = av_em_new_packet(&st->attached_pic, attachments[j].bin.size)) < 0)
                    return res;
                ...
                if (em_alloc_extradata(st->codecpar, attachments[j].bin.size))
                    break;
    ...
    for (i = 0; i < chapters_list->nb_elem; i++)
        if (chapters[i].start != AV_NOPTS_VALUE && chapters[i].uid &&
            ...
            chapters[i].chapter =
                avpriv_em_new_chapter(s, chapters[i].uid,
                                   (AVEMRational) { 1, 1000000000 },
                                   chapters[i].start, chapters[i].end,
                                   chapters[i].title);
            if (chapters[i].chapter) {
                av_em_dict_set(&chapters[i].chapter->metadata,
...
/*
 * Put one packet in an application-supplied AVEMPacket struct.
                                   ...
                                   AVEMPacket *pkt)
{
    if (matroska->num_packets > 0) {
        MatroskaTrack *tracks = matroska->tracks.elem;
        MatroskaTrack *track;
        memcpy(pkt, matroska->packets[0], sizeof(AVEMPacket));
        av_em_freep(&matroska->packets[0]);
        track = &tracks[pkt->stream_index];
        if (track->has_palette) {
            uint8_t *pal = av_em_packet_new_side_data(pkt, AV_PKT_DATA_PALETTE, AVPALETTE_SIZE);
            if (!pal) {
                av_em_log(matroska->ctx, AV_LOG_ERROR, "Cannot append palette to packet\n");
            } else {
        ...
        if (matroska->num_packets > 1) {
            ...
            memmove(&matroska->packets[0], &matroska->packets[1],
                    (matroska->num_packets - 1) * sizeof(AVEMPacket *));
            newpackets = av_em_realloc(matroska->packets,
                                    (matroska->num_packets - 1) *
                                    sizeof(AVEMPacket *));
            if (newpackets)
                matroska->packets = newpackets;
        } else {
            av_em_freep(&matroska->packets);
...
static void matroska_clear_queue(MatroskaDemuxContext *matroska)
{
    ...
    if (matroska->packets) {
        ...
        for (n = 0; n < matroska->num_packets; n++) {
            av_em_packet_unref(matroska->packets[n]);
            av_em_freep(&matroska->packets[n]);
        }
        av_em_freep(&matroska->packets);
                                ...
                                uint32_t **lace_buf, int *laces)
{
    ...
    if (!type) {
        *laces    = 1;
        *lace_buf = av_em_mallocz(sizeof(int));
        if (!*lace_buf)
            return AVERROR(ENOMEM);
    ...
    *laces    = *data + 1;
    data     += 1;
    size     -= 1;
    lace_size = av_em_mallocz(*laces * sizeof(int));
    if (!lace_size)
        return AVERROR(ENOMEM);
...
static int matroska_parse_rm_audio(MatroskaDemuxContext *matroska,
                                   MatroskaTrack *track, AVEMStream *st,
                                  ...
                                  uint8_t **pdst, int *size)
{
    ...
    while (srclen >= 8) {
        ...
        tmp = av_em_realloc(dst, dstlen + blocksize + 32);
        if (!tmp) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
                                 ...
                                 int64_t pos)
{
    ...
    pkt = av_em_mallocz(sizeof(*pkt));
    if (!pkt)
        return AVERROR(ENOMEM);
    err = av_em_new_packet(pkt, text_len);
    if (err < 0) {
        av_em_free(pkt);
        return AVERROR(err);
    }
    ...
    if (id_len > 0) {
        buf = av_em_packet_new_side_data(pkt,
                                ...
                                int64_t discard_padding)
{
    ...
    pkt = av_em_mallocz(sizeof(AVEMPacket));
    if (!pkt) {
        if (pkt_data != data)
            av_em_freep(&pkt_data);
        return AVERROR(ENOMEM);
    ...
    if (av_em_new_packet(pkt, pkt_size + offset) < 0) {
        av_em_free(pkt);
        res = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    if (additional_size > 0) {
        uint8_t *side_data = av_em_packet_new_side_data(pkt,
    ...
    if (discard_padding) {
        ...
        AV_WL32(side_data, 0);
        AV_WL32(side_data + 4, av_em_rescale_q(discard_padding,
                                            (AVEMRational){1, 1000000000},
                                            (AVEMRational){1, st->codecpar->sample_rate}));
    }
                                ...
                                int64_t cluster_pos, int64_t discard_padding)
{
    ...
    if ((n = matroska_ebmlnum_uint(matroska, data, size, &num)) < 0) {
        av_em_log(matroska->ctx, AV_LOG_ERROR, "EBML block data error\n");
        return n;
    }
    data += n;
    size -= n;
    ...
    if (cluster_time != (uint64_t) -1 &&
        ...
        if (is_keyframe)
            av_em_add_index_entry(st, cluster_pos, timecode, 0, 0,
                               AVINDEX_KEYFRAME);
    }
    ...
    if (matroska->skip_to_keyframe &&
        ...
        if (is_keyframe)
            ...
            av_em_log(matroska->ctx, AV_LOG_ERROR, "File is broken, keyframes not correctly marked!\n");
            matroska->skip_to_keyframe = 0;
        }
    }
    ...
    for (n = 0; n < laces; n++) {
        ...
        if (lace_size[n] > size) {
            av_em_log(matroska->ctx, AV_LOG_ERROR, "Invalid packet size\n");
            break;
        }
...
static int matroska_parse_cluster_incremental(MatroskaDemuxContext *matroska)
{
    ...
    if (res == 1) {
        ...
        matroska->current_cluster_pos        = avio_em_tell(matroska->ctx->pb);
        matroska->prev_pkt                   = NULL;
...
static int matroska_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MatroskaDemuxContext *matroska = s->priv_data;

    while (matroska_deliver_packet(matroska, pkt)) {
        int64_t pos = avio_em_tell(matroska->ctx->pb);
        if (matroska->done)
            return AVERROR_EOF;
...
static int matroska_read_seek(AVEMFormatContext *s, int stream_index,
                              int64_t timestamp, int flags)
{
    ...
    if ((index = av_em_index_search_timestamp(st, timestamp, flags)) < 0 || index == st->nb_index_entries - 1) {
        avio_em_seek(s->pb, st->index_entries[st->nb_index_entries - 1].pos,
                  SEEK_SET);
        matroska->current_id = 0;
        while ((index = av_em_index_search_timestamp(st, timestamp, flags)) < 0 || index == st->nb_index_entries - 1) {
    ...
    for (i = 0; i < matroska->tracks.nb_elem; i++) {
        ...
        if (tracks[i].type == MATROSKA_TRACK_TYPE_SUBTITLE &&
            ...
            index_sub = av_em_index_search_timestamp(
    ...
    avio_em_seek(s->pb, st->index_entries[index_min].pos, SEEK_SET);
    matroska->current_id       = 0;
    ...
    matroska->done             = 0;
    matroska->num_levels       = 0;
    em_update_cur_dts(s, st, st->index_entries[index].timestamp);
    return 0;
err:
...
static int matroska_read_close(AVEMFormatContext *s)
{
    ...
    for (n = 0; n < matroska->tracks.nb_elem; n++)
        if (tracks[n].type == MATROSKA_TRACK_TYPE_AUDIO)
            av_em_freep(&tracks[n].audio.buf);
    ebml_free(matroska_cluster, &matroska->current_cluster);
 ...
 */
static CueDesc get_cue_desc(AVEMFormatContext *s, int64_t ts, int64_t cues_start) {
    MatroskaDemuxContext *matroska = s->priv_data;
    CueDesc cue_desc;
    int i;
    int nb_index_entries = s->streams[0]->nb_index_entries;
    AVEMIndexEntry *index_entries = s->streams[0]->index_entries;
...
static int webm_clusters_start_with_keyframe(AVEMFormatContext *s)
{
    MatroskaDemuxContext *matroska = s->priv_data;
    ...
    index = av_em_index_search_timestamp(s->streams[0], 0, 0);
    if (index < 0)  return 0;
    cluster_pos = s->streams[0]->index_entries[index].pos;
    before_pos = avio_em_tell(s->pb);
    while (1) {
        int64_t cluster_id = 0, cluster_length = 0;
        AVEMPacket *pkt;
        avio_em_seek(s->pb, cluster_pos, SEEK_SET);
    ...
    avio_em_seek(s->pb, before_pos, SEEK_SET);
    return rv;
}
...
static int buffer_size_after_time_downloaded(int64_t time_ns, double search_sec, int64_t bps,
                                             ...
                                             double* sec_to_download, AVEMFormatContext *s,
                                             int64_t cues_start)
{
 ...
 * */
static int64_t webm_dash_manifest_compute_bandwidth(AVEMFormatContext *s, int64_t cues_start)
{
    MatroskaDemuxContext *matroska = s->priv_data;
...
static int webm_dash_manifest_cues(AVEMFormatContext *s)
{
    MatroskaDemuxContext *matroska = s->priv_data;
    EbmlList *seekhead_list = &matroska->seekhead;
    ...
    before_pos = avio_em_tell(matroska->ctx->pb);
    cues_start = seekhead[i].pos + matroska->segment_start;
    if (avio_em_seek(matroska->ctx->pb, cues_start, SEEK_SET) == cues_start) {
    ...
    // cues start
    av_em_dict_set_int(&s->streams[0]->metadata, CUES_START, cues_start, 0);

    // cues end
    av_em_dict_set_int(&s->streams[0]->metadata, CUES_END, cues_end, 0);

    // bandwidth
    bandwidth = webm_dash_manifest_compute_bandwidth(s, cues_start);
    if (bandwidth < 0) return -1;
    av_em_dict_set_int(&s->streams[0]->metadata, BANDWIDTH, bandwidth, 0);

    // check if all clusters start with key frames
    av_em_dict_set_int(&s->streams[0]->metadata, CLUSTER_KEYFRAME, webm_clusters_start_with_keyframe(s), 0);

    // store cue point timestamps as a comma separated list for checking subsegment alignment in
    // the muxer. assumes that each timestamp cannot be more than 20 characters long.
    buf = av_em_malloc_array(s->streams[0]->nb_index_entries, 20 * sizeof(char));
    if (!buf) return -1;
    strcpy(buf, "");
    ...
    av_em_dict_set(&s->streams[0]->metadata, CUE_TIMESTAMPS, buf, 0);
    av_em_free(buf);

    return 0;
}

static int webm_dash_manifest_read_header(AVEMFormatContext *s)
{
    char *buf;
    ...
    MatroskaDemuxContext *matroska = s->priv_data;
    if (ret) {
        av_em_log(s, AV_LOG_ERROR, "Failed to read file headers\n");
        return -1;
    }

    if (!matroska->is_live) {
        buf = av_em_asprintf("%g", matroska->duration);
        if (!buf) return AVERROR(ENOMEM);
        av_em_dict_set(&s->streams[0]->metadata, DURATION, buf, 0);
        av_em_free(buf);

        // initialization range
        // 5 is the offset of Cluster ID.
        av_em_dict_set_int(&s->streams[0]->metadata, INITIALIZATION_RANGE, avio_em_tell(s->pb) - 5, 0);
    }

    // basename of the file
    buf = strrchr(s->filename, '/');
    av_em_dict_set(&s->streams[0]->metadata, FILENAME, buf ? ++buf : s->filename, 0);

    // track number
    tracks = matroska->tracks.elem;
    av_em_dict_set_int(&s->streams[0]->metadata, TRACK_NUMBER, tracks[0].num, 0);

    // parse the cues and populate Cue related fields
    return matroska->is_live ? 0 : webm_dash_manifest_cues(s);
}

static int webm_dash_manifest_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    return AVERROR_EOF;
}
...
static const AVEMClass webm_dash_class = {
...
AVEMInputFormat ff_matroska_demuxer = {
    .name           = "matroska,webm",
...
AVEMInputFormat ff_webm_dash_manifest_demuxer = {
