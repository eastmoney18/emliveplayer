...
static int read_header(AVEMFormatContext *s)
{
    ...
    avpriv_em_set_pts_info(vst, 32, 1, 300);

    ret = avio_em_size(s->pb);
    if (ret < 0) {
        av_em_log(s, AV_LOG_WARNING, "Cannot calculate duration as file size cannot be determined\n");
    } else
...
static int read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    while (1) {
        ret = av_em_get_packet(s->pb, pkt, CDG_PACKET_SIZE);
...
AVEMInputFormat ff_cdg_demuxer = {
    .name           = "cdg",
