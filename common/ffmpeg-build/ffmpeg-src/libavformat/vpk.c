...
static int vpk_probe(AVEMProbeData *p)
{
...
static int vpk_read_header(AVEMFormatContext *s)
{
    ...
    avio_em_skip(s->pb, 4);
    st->duration           = avio_em_rl32(s->pb) * 28 / 16;
    offset                 = avio_em_rl32(s->pb);
    st->codecpar->codec_type  = AVMEDIA_TYPE_AUDIO;
    st->codecpar->codec_id    = AV_CODEC_ID_ADPCM_PSX;
    st->codecpar->block_align = avio_em_rl32(s->pb);
...
static int vpk_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if (vpk->current_block == vpk->block_count) {
        ...
        ret = av_em_new_packet(pkt, vpk->last_block_size);
        if (ret < 0)
            return ret;
        for (i = 0; i < par->channels; i++) {
            ret = avio_em_read(s->pb, pkt->data + i * size, size);
...
AVEMInputFormat ff_vpk_demuxer = {
    .name           = "vpk",
