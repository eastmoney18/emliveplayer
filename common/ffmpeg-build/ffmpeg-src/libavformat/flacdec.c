...
static void reset_index_position(int64_t metadata_head_size, AVEMStream *st)
{
...
static int flac_read_header(AVEMFormatContext *s)
{
    ...
    if (avio_em_rl32(s->pb) != MKTAG('f','L','a','C')) {
        avio_em_seek(s->pb, -4, SEEK_CUR);
        return 0;
    }

    /* process metadata blocks */
    while (!avio_em_feof(s->pb) && !metadata_last) {
        avio_em_read(s->pb, header, 4);
        ...
        case FLAC_METADATA_TYPE_SEEKTABLE:
            buffer = av_em_mallocz(metadata_size + AV_INPUT_BUFFER_PADDING_SIZE);
            if (!buffer) {
                return AVERROR(ENOMEM);
            }
            if (avio_em_read(s->pb, buffer, metadata_size) != metadata_size) {
                RETURN_ERROR(AVERROR(EIO));
            }
            break;
        /* skip metadata block for unsupported types */
        default:
            ret = avio_em_skip(s->pb, metadata_size);
            if (ret < 0)
                return ret;
        }
        ...
        if (metadata_type == FLAC_METADATA_TYPE_STREAMINFO) {
            ...
            for (i = 0; i < chapters; i++) {
                ...
                offset += ti * 12;
                avpriv_em_new_chapter(s, track, st->time_base, start, AV_NOPTS_VALUE, isrc);
            }
            av_em_freep(&buffer);
            ...
            av_em_freep(&buffer);
            if (ret < 0) {
                av_em_log(s, AV_LOG_ERROR, "Error parsing attached picture.\n");
                return ret;
            }
            ...
            if (metadata_type == FLAC_METADATA_TYPE_VORBIS_COMMENT) {
                ...
                if (ret < 0) {
                    av_em_log(s, AV_LOG_WARNING, "error parsing VorbisComment metadata\n");
                } else if (ret > 0) {
                    s->event_flags |= AVFMT_EVENT_FLAG_METADATA_UPDATED;
                }

                /* parse the channels mask if present */
                chmask = av_em_dict_get(s->metadata, "WAVEFORMATEXTENSIBLE_CHANNEL_MASK", NULL, 0);
                if (chmask) {
                    ...
                    if (!mask || mask & ~0x3ffffULL) {
                        ...
                        av_em_dict_set(&s->metadata, "WAVEFORMATEXTENSIBLE_CHANNEL_MASK", NULL, 0);
                    }
                }
            }
            av_em_freep(&buffer);
    ...
    reset_index_position(avio_em_tell(s->pb), st);
    return 0;
...
static int raw_flac_probe(AVEMProbeData *p)
{
...
static int flac_probe(AVEMProbeData *p)
{
...
static av_unused int64_t flac_read_timestamp(AVEMFormatContext *s, int stream_index,
                                             int64_t *ppos, int64_t pos_limit)
{
    AVEMPacket pkt, out_pkt;
    AVEMStream *st = s->streams[stream_index];
    AVEMCodecParserContext *parser;
    int ret;
    int64_t pts = AV_NOPTS_VALUE;

    if (avio_em_seek(s->pb, *ppos, SEEK_SET) < 0)
        return AV_NOPTS_VALUE;

    av_em_init_packet(&pkt);
    parser = av_em_parser_init(st->codecpar->codec_id);
    if (!parser){
        return AV_NOPTS_VALUE;
    }
    ...
    for (;;){
        ...
        av_em_parser_parse2(parser, st->internal->avctx,
    ...
    av_em_parser_close(parser);
    return pts;
}

static int flac_seek(AVEMFormatContext *s, int stream_index, int64_t timestamp, int flags) {
    int index;
    int64_t pos;
    AVEMIndexEntry e;
    FLACDecContext *flac = s->priv_data;
    ...
    index = av_em_index_search_timestamp(s->streams[0], timestamp, flags);
    if(index<0 || index >= s->streams[0]->nb_index_entries)
        return -1;

    e = s->streams[0]->index_entries[index];
    pos = avio_em_seek(s->pb, e.pos, SEEK_SET);
    if (pos >= 0) {
        return 0;
    }
    return -1;
...
AVEMInputFormat ff_flac_demuxer = {
    .name           = "flac",
