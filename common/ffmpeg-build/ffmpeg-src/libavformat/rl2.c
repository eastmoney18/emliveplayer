 ...
 */
static int rl2_probe(AVEMProbeData *p)
{
 ...
 */
static av_cold int rl2_read_header(AVEMFormatContext *s)
{
    ...
    avio_em_skip(pb,4);          /* skip FORM tag */
    back_size = avio_em_rl32(pb); /**< get size of the background frame */
    signature = avio_em_rb32(pb);
    avio_em_skip(pb, 4);         /* data size */
    frame_count = avio_em_rl32(pb);

    /* disallow back_sizes and frame_counts that may lead to overflows later */
    if(back_size > INT_MAX/2  || frame_count > INT_MAX / sizeof(uint32_t))
        return AVERROR_INVALIDDATA;

    avio_em_skip(pb, 2);         /* encoding method */
    sound_rate = avio_em_rl16(pb);
    rate = avio_em_rl16(pb);
    channels = avio_em_rl16(pb);
    def_sound_size = avio_em_rl16(pb);
    ...
    if(em_get_extradata(s, st->codecpar, pb, st->codecpar->extradata_size) < 0)
        return AVERROR(ENOMEM);
    ...
    if(sound_rate){
        ...
        st->codecpar->block_align = st->codecpar->channels *
            st->codecpar->bits_per_coded_sample / 8;
        avpriv_em_set_pts_info(st,32,1,rate);
    }

    avpriv_em_set_pts_info(s->streams[0], 32, pts_num, pts_den);

    chunk_size =   av_em_alloc(frame_count * sizeof(uint32_t));
    audio_size =   av_em_alloc(frame_count * sizeof(uint32_t));
    chunk_offset = av_em_alloc(frame_count * sizeof(uint32_t));

    if(!chunk_size || !audio_size || !chunk_offset){
        av_em_free(chunk_size);
        av_em_free(audio_size);
        av_em_free(chunk_offset);
        return AVERROR(ENOMEM);
    }

    /** read offset and size tables */
    for(i=0; i < frame_count;i++)
        chunk_size[i] = avio_em_rl32(pb);
    for(i=0; i < frame_count;i++)
        chunk_offset[i] = avio_em_rl32(pb);
    for(i=0; i < frame_count;i++)
        audio_size[i] = avio_em_rl32(pb) & 0xFFFF;
    ...
    for(i=0;i<frame_count;i++){
        ...
        if(sound_rate && audio_size[i]){
            av_em_add_index_entry(s->streams[1], chunk_offset[i],
                audio_frame_counter,audio_size[i], 0, AVINDEX_KEYFRAME);
            audio_frame_counter += audio_size[i] / channels;
        }
        av_em_add_index_entry(s->streams[0], chunk_offset[i] + audio_size[i],
 ...
 * @return 0 on success, AVERROR otherwise
 */
static int rl2_read_packet(AVEMFormatContext *s,
                            AVEMPacket *pkt)
{
    ...
    avio_em_seek(pb, sample->pos, SEEK_SET);

    /** fill the packet */
    ret = av_em_get_packet(pb, pkt, sample->size);
    if(ret != sample->size){
 ...
 */
static int rl2_read_seek(AVEMFormatContext *s, int stream_index, int64_t timestamp, int flags)
{
    ...
    for(i=0; i < s->nb_streams; i++){
        ...
        index = av_em_index_search_timestamp(st2,
                    av_em_rescale_q(timestamp, st->time_base, st2->time_base),
...
AVEMInputFormat ff_rl2_demuxer = {
    .name           = "rl2",
