...
static int ass_probe(AVEMProbeData *p)
{
    char buf[13];
    FFTextReader tr;
...
static int ass_read_close(AVEMFormatContext *s)
{
...
static int read_dialogue(ASSContext *ass, AVEMBPrint *dst, const uint8_t *p,
                         int64_t *start, int *duration)
{
    ...
    if (sscanf(p, "Dialogue: %*[^,],%d:%d:%d%*c%d,%d:%d:%d%*c%d,%n",
        ...
        *duration = end - *start;
        ...
        av_em_bprintf(dst, "%u,%d,%s", ass->readorder++, layer, p + pos);
...
static int64_t get_line(AVEMBPrint *buf, FFTextReader *tr)
{
...
static int ass_read_header(AVEMFormatContext *s)
{
    ASSContext *ass = s->priv_data;
    AVEMBPrint header, line, rline;
    int res = 0;
    AVEMStream *st;
    FFTextReader tr;
    ...
    av_em_bprint_init(&line,   0, AV_BPRINT_SIZE_UNLIMITED);
    av_em_bprint_init(&rline,  0, AV_BPRINT_SIZE_UNLIMITED);
    ...
    for (;;) {
        ...
        if (read_dialogue(ass, &rline, line.str, &ts_start, &duration) < 0) {
            av_em_bprintf(&header, "%s", line.str);
            continue;
        }
    ...
    res = em_bprint_to_codecpar_extradata(st->codecpar, &header);
    if (res < 0)
        goto end;
    ...
    av_em_bprint_finalize(&line,   NULL);
    av_em_bprint_finalize(&rline,  NULL);
    return res;
}

static int ass_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ASSContext *ass = s->priv_data;
    return ff_subtitles_queue_read_packet(&ass->q, pkt);
}

static int ass_read_seek(AVEMFormatContext *s, int stream_index,
...
AVEMInputFormat ff_ass_demuxer = {
    .name           = "ass",
