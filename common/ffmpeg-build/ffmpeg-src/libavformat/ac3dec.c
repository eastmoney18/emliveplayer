...
static int ac3_eac3_probe(AVEMProbeData *p, enum AVEMCodecID expected_codec_id)
{
    int max_frames, first_frames = 0, frames;
    const uint8_t *buf, *buf2, *end;
    AC3HeaderInfo *phdr = NULL;
    GetBitContext gbc;
    enum AVEMCodecID codec_id = AV_CODEC_ID_AC3;
    ...
    for(; buf < end; buf++) {
        ...
        for(frames = 0; buf2 < end; frames++) {
            ...
            if(av_em_crc(av_em_crc_get_table(AV_CRC_16_ANSI), 0, gbc.buffer + 2, phdr->frame_size - 2))
                break;
...
static int ac3_probe(AVEMProbeData *p)
{
    return ac3_eac3_probe(p, AV_CODEC_ID_AC3);
}

AVEMInputFormat ff_ac3_demuxer = {
    .name           = "ac3",
...
static int eac3_probe(AVEMProbeData *p)
{
    return ac3_eac3_probe(p, AV_CODEC_ID_EAC3);
}

AVEMInputFormat ff_eac3_demuxer = {
    .name           = "eac3",
