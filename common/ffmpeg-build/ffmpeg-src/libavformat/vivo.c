...
static int vivo_probe(AVEMProbeData *p)
{
...
static int vivo_get_packet_header(AVEMFormatContext *s)
{
    ...
    default:
        av_em_log(s, AV_LOG_ERROR, "unknown packet type %d\n", vivo->type);
    ...
    if (get_length) {
        ...
        if (c & 0x80) {
            ...
            if (c & 0x80) {
                av_em_log(s, AV_LOG_ERROR, "coded length is more than two bytes\n");
                return AVERROR_INVALIDDATA;
            }
        }
    }
...
static int vivo_read_header(AVEMFormatContext *s)
{
    VivoContext *vivo = s->priv_data;
    AVEMRational fps = { 1, 25};
    AVEMStream *ast, *vst;
    ...
    while (1) {
        ...
        if (vivo->length <= 1024) {
            avio_em_read(s->pb, vivo->text, vivo->length);
            vivo->text[vivo->length] = 0;
        } else {
            av_em_log(s, AV_LOG_WARNING, "too big header, skipping\n");
            avio_em_skip(s->pb, vivo->length);
            continue;
        }
        ...
        while (*line) {
            ...
            *line_end = 0;
            ...
            value = strchr(key, ':');
            if (!value) {
                av_em_log(s, AV_LOG_WARNING, "missing colon in key:value pair '%s'\n",
                       value);
                continue;
            }
            ...
            *value++ = 0;
            ...
            av_em_log(s, AV_LOG_DEBUG, "header: '%s' = '%s'\n", key, value);

            value_int = strtol(value, &end_value, 10);
            value_used = 0;
            if (*end_value == 0) { // valid integer
                av_em_log(s, AV_LOG_DEBUG, "got a valid integer (%ld)\n", value_int);
                value_used = 1;
            ...
            if (!strcmp(key, "Version")) {
                ...
                value_used = 1;
                if (!av_em_parse_ratio(&tmp, value, 10000, AV_LOG_WARNING, s))
                    fps = av_em_inv_q(tmp);
            }
    ...
    avpriv_em_set_pts_info(vst, 64, fps.num, fps.den);
    if (duration)
        s->duration = av_em_rescale(duration, 1000, 1);
...
static int vivo_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    VivoContext *vivo = s->priv_data;
    AVEMIOContext *pb = s->pb;
...
restart:
    ...
    if ((ret = av_em_get_packet(pb, pkt, vivo->length)) < 0)
        goto fail;
    ...
    while (vivo->sequence == old_sequence &&
        ...
        if ((ret = av_em_append_packet(pb, pkt, vivo->length)) < 0)
            break;
...
AVEMInputFormat ff_vivo_demuxer = {
    .name           = "vivo",
