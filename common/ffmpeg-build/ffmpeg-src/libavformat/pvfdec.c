...
static int pvf_probe(AVEMProbeData *p)
{
...
static int pvf_read_header(AVEMFormatContext *s)
{
    char buffer[32];
    AVEMStream *st;
    ...
    st->codecpar->codec_id    = em_get_pcm_codec_id(bps, 0, 1, 0xFFFF);
...
AVEMInputFormat ff_pvf_demuxer = {
    .name           = "pvf",
