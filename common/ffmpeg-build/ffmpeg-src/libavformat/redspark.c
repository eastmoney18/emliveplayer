...
static int redspark_probe(AVEMProbeData *p)
{
    uint32_t key, data;
    uint8_t header[8];
...
static int redspark_read_header(AVEMFormatContext *s)
{
    ...
    if (par->sample_rate <= 0 || par->sample_rate > 96000) {
        av_em_log(s, AV_LOG_ERROR, "Invalid sample rate: %d\n", par->sample_rate);
    ...
    if (em_alloc_extradata(par, 32 * par->channels)) {
        return AVERROR_INVALIDDATA;
    }
...
static int redspark_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if (avio_em_feof(s->pb) || redspark->samples_count == s->streams[0]->duration)
        return AVERROR_EOF;
...
AVEMInputFormat ff_redspark_demuxer = {
    .name           =   "redspark",
