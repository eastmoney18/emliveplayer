...
static const AVEMClass srtp_context_class = {
    .class_name     = "srtp",
...
static int srtp_close(EMURLContext *h)
{
...
static int srtp_open(EMURLContext *h, const char *uri, int flags)
{
    SRTPProtoContext *s = h->priv_data;
    ...
    em_url_join(buf, sizeof(buf), "rtp", NULL, hostname, rtp_port, "%s", path);
    if ((ret = ffurl_em_open_whitelist(&s->rtp_hd, buf, flags, &h->interrupt_callback,
...
static int srtp_read(EMURLContext *h, uint8_t *buf, int size)
{
    SRTPProtoContext *s = h->priv_data;
    int ret;
start:
    ret = ffurl_em_read(s->rtp_hd, buf, size);
...
static int srtp_write(EMURLContext *h, const uint8_t *buf, int size)
{
    SRTPProtoContext *s = h->priv_data;
    if (!s->srtp_out.aes)
        return ffurl_em_write(s->rtp_hd, buf, size);
    size = ff_srtp_encrypt(&s->srtp_out, buf, size, s->encryptbuf,
                           sizeof(s->encryptbuf));
    if (size < 0)
        return size;
    return ffurl_em_write(s->rtp_hd, s->encryptbuf, size);
}

static int srtp_get_file_handle(EMURLContext *h)
{
    SRTPProtoContext *s = h->priv_data;
    return ffurl_em_get_file_handle(s->rtp_hd);
}

static int srtp_get_multi_file_handle(EMURLContext *h, int **handles,
                                      int *numhandles)
{
    SRTPProtoContext *s = h->priv_data;
    return ffurl_em_get_multi_file_handle(s->rtp_hd, handles, numhandles);
}

const EMURLProtocol em_srtp_protocol = {
    .name                      = "srtp",
    .url_open                  = srtp_open,
    .url_read                  = srtp_read,
