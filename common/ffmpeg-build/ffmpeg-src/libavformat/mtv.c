...
static int mtv_probe(AVEMProbeData *p)
{
...
static int mtv_read_header(AVEMFormatContext *s)
{
    ...
    avio_em_skip(pb, 3);
    mtv->file_size         = avio_em_rl32(pb);
    mtv->segments          = avio_em_rl32(pb);
    avio_em_skip(pb, 32);
    mtv->audio_identifier  = avio_em_rl24(pb);
    mtv->audio_br          = avio_em_rl16(pb);
    mtv->img_colorfmt      = avio_em_rl24(pb);
    mtv->img_bpp           = avio_em_r8(pb);
    mtv->img_width         = avio_em_rl16(pb);
    mtv->img_height        = avio_em_rl16(pb);
    mtv->img_segment_size  = avio_em_rl16(pb);
    ...
    if(!mtv->img_height || !mtv->img_width || !mtv->img_segment_size){
        av_em_log(s, AV_LOG_ERROR, "width or height or segment_size is invalid and I cannot calculate them from other information\n");
        return AVERROR_INVALIDDATA;
    }

    avio_em_skip(pb, 4);
    audio_subsegments = avio_em_rl16(pb);

    if (audio_subsegments == 0) {
        avpriv_em_request_sample(s, "MTV files without audio");
    ...
    avpriv_em_set_pts_info(st, 64, 1, mtv->video_fps);
    ...
    avpriv_em_set_pts_info(st, 64, 1, MTV_AUDIO_SAMPLING_RATE);
    ...
    if(avio_em_seek(pb, MTV_HEADER_SIZE, SEEK_SET) != MTV_HEADER_SIZE)
        return AVERROR(EIO);
...
static int mtv_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MTVDemuxContext *mtv = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int ret;

    if((avio_em_tell(pb) - s->internal->data_offset + mtv->img_segment_size) % mtv->full_segment_size)
    {
        avio_em_skip(pb, MTV_AUDIO_PADDING_SIZE);

        ret = av_em_get_packet(pb, pkt, MTV_ASUBCHUNK_DATA_SIZE);
        if(ret < 0)
            return ret;
    ...
    {
        ret = av_em_get_packet(pb, pkt, mtv->img_segment_size);
        if(ret < 0)
            return ret;
...
AVEMInputFormat ff_mtv_demuxer = {
    .name           = "mtv",
