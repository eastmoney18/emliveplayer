...
static int jacosub_probe(AVEMProbeData *p)
{
...
static int jacosub_read_close(AVEMFormatContext *s)
{
...
static int jacosub_read_header(AVEMFormatContext *s)
{
    AVEMBPrint header;
    ...
    av_em_bprint_init(&header, 1024+AV_INPUT_BUFFER_PADDING_SIZE, 4096);

    while (!avio_em_feof(pb)) {
        int cmd_len;
        const char *p = line;
        int64_t pos = avio_em_tell(pb);
        int len = em_get_line(pb, line, sizeof(line));
        ...
        if (av_em_strncasecmp(p, cmds[i], cmd_len) == 0)
            p += cmd_len;
        else
            p++;
        p = jss_skip_whitespace(p);
        ...
        case 'T': // ...but must be placed after TIMERES
            ...
            if (!jacosub->timeres)
                ...
                av_em_bprintf(&header, "#T %s", p);
            break;
        }
    }

    /* general/essential directives in the extradata */
    ret = em_bprint_to_codecpar_extradata(st->codecpar, &header);
    if (ret < 0)
        goto fail;
    ...
    for (i = 0; i < jacosub->q.nb_subs; i++) {
        AVEMPacket *sub = &jacosub->q.subs[i];
...
static int jacosub_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    JACOsubContext *jacosub = s->priv_data;
    return ff_subtitles_queue_read_packet(&jacosub->q, pkt);
}

static int jacosub_read_seek(AVEMFormatContext *s, int stream_index,
...
AVEMInputFormat ff_jacosub_demuxer = {
    .name           = "jacosub",
