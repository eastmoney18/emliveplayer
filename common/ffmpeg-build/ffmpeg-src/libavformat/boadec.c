...
static int read_header(AVEMFormatContext *s)
{
    ...
    s->internal->data_offset = avio_em_rl32(s->pb);
    avio_em_r8(s->pb);
    st->codecpar->block_align = st->codecpar->channels * avio_em_rl32(s->pb);
...
static int read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    return av_em_get_packet(s->pb, pkt, st->codecpar->block_align);
}

AVEMInputFormat ff_boa_demuxer = {
    .name           = "boa",
