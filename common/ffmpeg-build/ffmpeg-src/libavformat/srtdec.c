...
static int srt_probe(AVEMProbeData *p)
{
    int v;
    char buf[64], *pbuf;
    FFTextReader tr;
...
static int add_event(FFDemuxSubtitlesQueue *q, AVEMBPrint *buf, char *line_cache,
                     const struct event_info *ei, int append_cache)
{
    ...
    if (buf->len) {
        AVEMPacket *sub = ff_subtitles_queue_insert(q, buf->str, buf->len, 0);
        if (!sub)
        ...
        if (ei->x1 != -1) {
            uint8_t *p = av_em_packet_new_side_data(sub, AV_PKT_DATA_SUBTITLE_POSITION, 16);
            if (p) {
                AV_WL32(p,      ei->x1);
                AV_WL32(p +  4, ei->y1);
                AV_WL32(p +  8, ei->x2);
...
static int srt_read_header(AVEMFormatContext *s)
{
...
static int srt_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    SRTContext *srt = s->priv_data;
    return ff_subtitles_queue_read_packet(&srt->q, pkt);
}

static int srt_read_seek(AVEMFormatContext *s, int stream_index,
...
static int srt_read_close(AVEMFormatContext *s)
{
    SRTContext *srt = s->priv_data;
...
AVEMInputFormat ff_srt_demuxer = {
    .name        = "srt",
