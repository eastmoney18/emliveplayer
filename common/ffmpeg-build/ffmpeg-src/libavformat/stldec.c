...
static int stl_probe(AVEMProbeData *p)
{
    char c;
...
static int stl_read_header(AVEMFormatContext *s)
{
...
static int stl_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    STLContext *stl = s->priv_data;
    return ff_subtitles_queue_read_packet(&stl->q, pkt);
}

static int stl_read_seek(AVEMFormatContext *s, int stream_index,
...
static int stl_read_close(AVEMFormatContext *s)
{
    STLContext *stl = s->priv_data;
...
AVEMInputFormat ff_stl_demuxer = {
    .name           = "stl",
