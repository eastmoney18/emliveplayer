...
static int bmv_read_header(AVEMFormatContext *s)
{
    AVEMStream *st, *ast;
    ...
    avpriv_em_set_pts_info(ast, 16, 1, 22050);
...
static int bmv_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    BMVContext *c = s->priv_data;
    int type, err;
    ...
    while (c->get_next) {
        ...
        if (type == BMV_END)
            return AVERROR_EOF;
        c->size = avio_em_rl24(s->pb);
        if (!c->size)
            return AVERROR_INVALIDDATA;
        if ((err = av_em_reallocp(&c->packet, c->size + 1)) < 0)
            return err;
        c->packet[0] = type;
        if (avio_em_read(s->pb, c->packet + 1, c->size) != c->size)
            return AVERROR(EIO);
        if (type & BMV_AUDIO) {
            int audio_size = c->packet[1] * 65 + 1;
            if (audio_size >= c->size) {
                av_em_log(s, AV_LOG_ERROR, "Reported audio size %d is bigger than packet size (%d)\n",
                       audio_size, c->size);
                return AVERROR_INVALIDDATA;
            }
            if (av_em_new_packet(pkt, audio_size) < 0)
                return AVERROR(ENOMEM);
    ...
    if (av_em_new_packet(pkt, c->size + 1) < 0)
        return AVERROR(ENOMEM);
    pkt->stream_index = 0;
    c->get_next = 1;
...
static int bmv_read_close(AVEMFormatContext *s)
{
    BMVContext *c = s->priv_data;

    av_em_freep(&c->packet);

    return 0;
}

AVEMInputFormat ff_bmv_demuxer = {
    .name           = "bmv",
