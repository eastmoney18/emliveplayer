...
static int cdxl_read_probe(AVEMProbeData *p)
{
...
static int cdxl_read_header(AVEMFormatContext *s)
{
    CDXLDemuxContext *cdxl = s->priv_data;
    int ret;

    if (cdxl->framerate && (ret = av_em_parse_video_rate(&cdxl->fps, cdxl->framerate)) < 0) {
    ...
    cdxl->filesize = avio_em_size(s->pb);
...
static int cdxl_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    CDXLDemuxContext *cdxl = s->priv_data;
    ...
    pos = avio_em_tell(pb);
    if (!cdxl->read_chunk &&
        avio_em_read(pb, cdxl->header, CDXL_HEADER_SIZE) != CDXL_HEADER_SIZE)
        return AVERROR_EOF;
    if (cdxl->header[0] != 1) {
        av_em_log(s, AV_LOG_ERROR, "non-standard cdxl file\n");
    ...
    if (cdxl->read_chunk && audio_size) {
        if (cdxl->audio_stream_index == -1) {
            ...
            avpriv_em_set_pts_info(st, 64, 1, cdxl->sample_rate);
        }
        ...
        if (cdxl->video_stream_index == -1) {
            ...
            if (cdxl->framerate)
                avpriv_em_set_pts_info(st, 64, cdxl->fps.den, cdxl->fps.num);
            else
                avpriv_em_set_pts_info(st, 64, 1, cdxl->sample_rate);
        }

        if (av_em_new_packet(pkt, video_size + CDXL_HEADER_SIZE) < 0)
            return AVERROR(ENOMEM);
        memcpy(pkt->data, cdxl->header, CDXL_HEADER_SIZE);
        ret = avio_em_read(pb, pkt->data + CDXL_HEADER_SIZE, video_size);
        if (ret < 0) {
            av_em_packet_unref(pkt);
            return ret;
        }
        av_em_shrink_packet(pkt, CDXL_HEADER_SIZE + ret);
    ...
    if (!cdxl->read_chunk)
        avio_em_skip(pb, current_size - audio_size - video_size - CDXL_HEADER_SIZE);
    return ret;
}
...
static const AVEMClass cdxl_demuxer_class = {
...
AVEMInputFormat ff_cdxl_demuxer = {
    .name           = "cdxl",
