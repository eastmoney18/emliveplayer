...
struct PayloadContext {
    ...
    AVEMIOContext *fragment;
};

static void mpa_robust_close_context(PayloadContext *data)
{
    emio_free_dyn_buf(&data->fragment);
    av_em_free(data->split_buf);
}

static int mpa_robust_parse_rtp_header(AVEMFormatContext *ctx,
                                       ...
                                       unsigned *adu_size, unsigned *cont)
{
    ...
    if (len < 2) {
        av_em_log(ctx, AV_LOG_ERROR, "Invalid %d bytes packet\n", len);
...
static int mpa_robust_parse_packet(AVEMFormatContext *ctx, PayloadContext *data,
                                   ...
                                   int len, uint16_t seq, int flags)
{
    ...
    if (!buf) {
        ...
        if (continuation || adu_size > len) {
            av_em_freep(&data->split_buf);
            av_em_log(ctx, AV_LOG_ERROR, "Invalid frame\n");
            return AVERROR_INVALIDDATA;
        }

        if (av_em_new_packet(pkt, adu_size)) {
    ...
    if (!continuation && adu_size <= len) {
        ...
        err = avio_em_open_dyn_buf(&data->fragment);
        if (err < 0)
            return err;

        avio_em_write(data->fragment, buf, len);
