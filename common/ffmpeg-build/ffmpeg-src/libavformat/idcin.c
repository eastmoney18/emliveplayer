...
static int idcin_probe(AVEMProbeData *p)
{
...
static int idcin_read_header(AVEMFormatContext *s)
{
    ...
    sample_rate = avio_em_rl32(pb);
    bytes_per_sample = avio_em_rl32(pb);
    channels = avio_em_rl32(pb);

    if (s->pb->eof_reached) {
        av_em_log(s, AV_LOG_ERROR, "incomplete header\n");
    ...
    if (sample_rate > 0) {
        ...
        if (bytes_per_sample < 1 || bytes_per_sample > 2) {
            av_em_log(s, AV_LOG_ERROR, "invalid bytes per sample: %u\n",
                   bytes_per_sample);
            return AVERROR_INVALIDDATA;
        }
        if (channels < 1 || channels > 2) {
            av_em_log(s, AV_LOG_ERROR, "invalid channels: %u\n", channels);
            return AVERROR_INVALIDDATA;
        }
    ...
    avpriv_em_set_pts_info(st, 33, 1, IDCIN_FPS);
    st->start_time = 0;
    ...
    if ((ret = em_get_extradata(s, st->codecpar, pb, HUFFMAN_TABLE_SIZE)) < 0)
        return ret;
    ...
    if (idcin->audio_present) {
        ...
        if (!st)
            return AVERROR(ENOMEM);
        avpriv_em_set_pts_info(st, 63, 1, sample_rate);
        st->start_time = 0;
    ...
    idcin->first_pkt_pos = avio_em_tell(s->pb);

    return 0;
}

static int idcin_read_packet(AVEMFormatContext *s,
                             AVEMPacket *pkt)
{
    int ret;
    ...
    if (idcin->next_chunk_is_video) {
        command = avio_em_rl32(pb);
        if (command == 2) {
            return AVERROR(EIO);
        } else if (command == 1) {
            /* trigger a palette change */
            ret = avio_em_read(pb, palette_buffer, 768);
            if (ret < 0) {
                return ret;
            } else if (ret != 768) {
                av_em_log(s, AV_LOG_ERROR, "incomplete packet\n");
                return AVERROR(EIO);
            }
        ...
        if (chunk_size < 4 || chunk_size > INT_MAX - 4) {
            av_em_log(s, AV_LOG_ERROR, "invalid chunk size: %u\n", chunk_size);
            return AVERROR_INVALIDDATA;
        }
...
static int idcin_read_seek(AVEMFormatContext *s, int stream_index,
                           int64_t timestamp, int flags)
{
    IdcinDemuxContext *idcin = s->priv_data;

    if (idcin->first_pkt_pos > 0) {
        int64_t ret = avio_em_seek(s->pb, idcin->first_pkt_pos, SEEK_SET);
        if (ret < 0)
            return ret;
        em_update_cur_dts(s, s->streams[idcin->video_stream_index], 0);
        idcin->next_chunk_is_video = 1;
        idcin->current_audio_chunk = 0;
        return 0;
    }
...
AVEMInputFormat ff_idcin_demuxer = {
    .name           = "idcin",
