...
typedef struct DASHContext {
    const AVEMClass *class;  /* Class for private options. */
    int window_size;
    ...
    AVEMRational min_frame_rate, max_frame_rate;
...
static int dash_write(void *opaque, uint8_t *buf, int buf_size)
{
    ...
    if (os->out)
        avio_em_write(os->out, buf, buf_size);
    return buf_size;
}

// RFC 6381
static void set_codec_str(AVEMFormatContext *s, AVEMCodecParameters *par,
                          char *str, int size)
{
    const AVEMCodecTag *tags[2] = { NULL, NULL };
    uint32_t tag;
    if (par->codec_type == AVMEDIA_TYPE_VIDEO)
        tags[0] = em_codec_movvideo_tags;
    else if (par->codec_type == AVMEDIA_TYPE_AUDIO)
        tags[0] = em_codec_movaudio_tags;
    else
        return;

    tag = av_em_codec_get_tag(tags, par->codec_id);
    if (!tag)
        return;
    if (size < 5)
        return;
    ...
    if (!strcmp(str, "mp4a") || !strcmp(str, "mp4v")) {
        uint32_t oti;
        tags[0] = em_mp4_obj_type;
        oti = av_em_codec_get_tag(tags, par->codec_id);
        if (oti)
            av_em_strlcatf(str, size, ".%02x", oti);
        else
            return;
        ...
        if (tag == MKTAG('m', 'p', '4', 'a')) {
            if (par->extradata_size >= 2) {
                ...
                av_em_strlcatf(str, size, ".%d", aot);
            }
        } else if (tag == MKTAG('m', 'p', '4', 'v')) {
            // Unimplemented, should output ProfileLevelIndication as a decimal number
            av_em_log(s, AV_LOG_WARNING, "Incomplete RFC 6381 codec string for mp4v\n");
        }
        ...
        if (extradata[0] != 1) {
            ...
            if (avio_em_open_dyn_buf(&pb) < 0)
                return;
            if (em_isom_write_avcc(pb, extradata, extradata_size) < 0) {
                emio_free_dyn_buf(&pb);
                return;
            }
            extradata_size = avio_em_close_dyn_buf(pb, &extradata);
            tmpbuf = extradata;
        }

        if (extradata_size >= 4)
            av_em_strlcatf(str, size, ".%02x%02x%02x",
                        extradata[1], extradata[2], extradata[3]);
        av_em_free(tmpbuf);
    }
}

static void dash_free(AVEMFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (os->ctx && os->ctx_inited)
            av_em_write_trailer(os->ctx);
        if (os->ctx && os->ctx->pb)
            av_em_free(os->ctx->pb);
        em_format_io_close(s, &os->out);
        if (os->ctx)
            avformat_em_free_context(os->ctx);
        for (j = 0; j < os->nb_segments; j++)
            av_em_free(os->segments[j]);
        av_em_free(os->segments);
    }
    av_em_freep(&c->streams);
}

static void output_segment_list(OutputStream *os, AVEMIOContext *out, DASHContext *c)
{
    ...
    if (c->use_template) {
        ...
        avio_em_printf(out, "\t\t\t\t<SegmentTemplate timescale=\"%d\" ", timescale);
        if (!c->use_timeline)
            avio_em_printf(out, "duration=\"%"PRId64"\" ", c->last_duration);
        avio_em_printf(out, "initialization=\"%s\" media=\"%s\" startNumber=\"%d\">\n", c->init_seg_name, c->media_seg_name, c->use_timeline ? start_number : 1);
        if (c->use_timeline) {
            int64_t cur_time = 0;
            avio_em_printf(out, "\t\t\t\t\t<SegmentTimeline>\n");
            for (i = start_index; i < os->nb_segments; ) {
                Segment *seg = os->segments[i];
                int repeat = 0;
                avio_em_printf(out, "\t\t\t\t\t\t<S ");
                if (i == start_index || seg->time != cur_time) {
                    cur_time = seg->time;
                    avio_em_printf(out, "t=\"%"PRId64"\" ", seg->time);
                }
                avio_em_printf(out, "d=\"%d\" ", seg->duration);
        ...
        avio_em_printf(out, "\t\t\t\t</SegmentTemplate>\n");
    } else if (c->single_file) {
        avio_em_printf(out, "\t\t\t\t<BaseURL>%s</BaseURL>\n", os->initfile);
        avio_em_printf(out, "\t\t\t\t<SegmentList timescale=\"%d\" duration=\"%"PRId64"\" startNumber=\"%d\">\n", AV_TIME_BASE, c->last_duration, start_number);
        avio_em_printf(out, "\t\t\t\t\t<Initialization range=\"%"PRId64"-%"PRId64"\" />\n", os->init_start_pos, os->init_start_pos + os->init_range_length - 1);
        for (i = start_index; i < os->nb_segments; i++) {
            Segment *seg = os->segments[i];
            avio_em_printf(out, "\t\t\t\t\t<SegmentURL mediaRange=\"%"PRId64"-%"PRId64"\" ", seg->start_pos, seg->start_pos + seg->range_length - 1);
            if (seg->index_length)
                avio_em_printf(out, "indexRange=\"%"PRId64"-%"PRId64"\" ", seg->start_pos, seg->start_pos + seg->index_length - 1);
            avio_em_printf(out, "/>\n");
        }
        avio_em_printf(out, "\t\t\t\t</SegmentList>\n");
    } else {
        avio_em_printf(out, "\t\t\t\t<SegmentList timescale=\"%d\" duration=\"%"PRId64"\" startNumber=\"%d\">\n", AV_TIME_BASE, c->last_duration, start_number);
        avio_em_printf(out, "\t\t\t\t\t<Initialization sourceURL=\"%s\" />\n", os->initfile);
        for (i = start_index; i < os->nb_segments; i++) {
            Segment *seg = os->segments[i];
            avio_em_printf(out, "\t\t\t\t\t<SegmentURL media=\"%s\" />\n", seg->file);
        }
        avio_em_printf(out, "\t\t\t\t</SegmentList>\n");
    }
}
...
static DASHTmplId dash_read_tmpl_id(const char *identifier, char *format_tag,
    ...
    if (av_em_strstart(identifier, "$$", &next_ptr)) {
        id_type = DASH_TMPL_ID_ESCAPE;
        *ptr = next_ptr;
    } else if (av_em_strstart(identifier, "$RepresentationID$", &next_ptr)) {
        ...
        av_em_strlcpy(format_tag, "%d", format_tag_size);
        *ptr = next_ptr;
    } else { // the following identifiers may have an explicit format_tag
        if (av_em_strstart(identifier, "$Number", &next_ptr))
            id_type = DASH_TMPL_ID_NUMBER;
        else if (av_em_strstart(identifier, "$Bandwidth", &next_ptr))
            id_type = DASH_TMPL_ID_BANDWIDTH;
        else if (av_em_strstart(identifier, "$Time", &next_ptr))
        ...
        if (id_type != DASH_TMPL_ID_UNDEFINED) {
            ...
            if (next_ptr[0] == '$') { // no dash format-tag
                ...
                *ptr = &next_ptr[1];
                ...
                if (av_em_strstart(next_ptr, "%0", &width_ptr) &&
                    av_isdigit(width_ptr[0]) &&
                    av_em_strstart(&width_ptr[1], "d$", &next_ptr)) {
                    // yes, we're using a format tag to build format_tag.
                    snprintf(format_tag, format_tag_size, "%s%c%s", "%0", width_ptr[0], number_format);
                    *ptr = next_ptr;
                } else {
                    av_em_log(NULL, AV_LOG_WARNING, "Failed to parse format-tag beginning with %s. Expected either a "
...
static void dash_fill_tmpl_params(char *dst, size_t buffer_size,
    ...
    while (dst_pos < buffer_size - 1 && *t_cur) {
        ...
        if (t_next) {
            ...
            av_em_strlcpy(&dst[dst_pos], t_cur, num_copy_bytes + 1);
            // advance
            dst_pos += num_copy_bytes;
            t_cur = t_next;
        } else { // no more DASH identifiers to substitute - just copy the rest over and break
            av_em_strlcpy(&dst[dst_pos], t_cur, buffer_size - dst_pos);
            break;
        }
        ...
        case DASH_TMPL_ID_UNDEFINED:
            ...
            av_em_strlcpy(&dst[dst_pos], t_cur, 2);
            n = 1;
            t_next = &t_cur[1];
            break;
        }
...
static char *xmlescape(const char *str) {
    ...
    char *out = av_em_realloc(NULL, outlen + 1);
    int pos = 0;
    if (!out)
        return NULL;
    for (; *str; str++) {
        if (pos + 6 > outlen) {
            char *tmp;
            outlen = 2 * outlen + 6;
            tmp = av_em_realloc(out, outlen + 1);
            if (!tmp) {
                av_em_free(out);
                return NULL;
            }
...
static void write_time(AVEMIOContext *out, int64_t time)
{
    ...
    seconds %= 60;
    minutes %= 60;
    avio_em_printf(out, "PT");
    if (hours)
        avio_em_printf(out, "%dH", hours);
    if (hours || minutes)
        avio_em_printf(out, "%dM", minutes);
    avio_em_printf(out, "%d.%dS", seconds, fractions / (AV_TIME_BASE / 10));
}
...
static int write_manifest(AVEMFormatContext *s, int final)
{
    DASHContext *c = s->priv_data;
    AVEMIOContext *out;
    ...
    if (ret < 0) {
        av_em_log(s, AV_LOG_ERROR, "Unable to open %s for writing\n", temp_filename);
        return ret;
    }
    avio_em_printf(out, "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n");
    avio_em_printf(out, "<MPD xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"\n"
    ...
    if (final) {
        avio_em_printf(out, "\tmediaPresentationDuration=\"");
        ...
        if (c->use_template && !c->use_timeline)
            update_period = 500;
        avio_em_printf(out, "\tminimumUpdatePeriod=\"PT%"PRId64"S\"\n", update_period);
        avio_em_printf(out, "\tsuggestedPresentationDelay=\"PT%"PRId64"S\"\n", c->last_duration / AV_TIME_BASE);
        ...
        if (c->availability_start_time[0])
            avio_em_printf(out, "\tavailabilityStartTime=\"%s\"\n", c->availability_start_time);
        format_date_now(now_str, sizeof(now_str));
        if (now_str[0])
            avio_em_printf(out, "\tpublishTime=\"%s\"\n", now_str);
        if (c->window_size && c->use_template) {
            avio_em_printf(out, "\ttimeShiftBufferDepth=\"");
            write_time(out, c->last_duration * c->window_size);
            avio_em_printf(out, "\"\n");
        }
    }
    avio_em_printf(out, "\tminBufferTime=\"");
    write_time(out, c->last_duration);
    avio_em_printf(out, "\">\n");
    avio_em_printf(out, "\t<ProgramInformation>\n");
    if (title) {
        char *escaped = xmlescape(title->value);
        avio_em_printf(out, "\t\t<Title>%s</Title>\n", escaped);
        av_em_free(escaped);
    }
    avio_em_printf(out, "\t</ProgramInformation>\n");
    if (c->window_size && s->nb_streams > 0 && c->streams[0].nb_segments > 0 && !c->use_template) {
        ...
        int64_t start_time = av_em_rescale_q(os->segments[start_index]->time, s->streams[0]->time_base, AV_TIME_BASE_Q);
        avio_em_printf(out, "\t<Period start=\"");
        write_time(out, start_time);
        avio_em_printf(out, "\">\n");
    } else {
        avio_em_printf(out, "\t<Period start=\"PT0.0S\">\n");
    }

    if (c->has_video) {
        avio_em_printf(out, "\t\t<AdaptationSet contentType=\"video\" segmentAlignment=\"true\" bitstreamSwitching=\"true\"");
        if (c->max_frame_rate.num && !c->ambiguous_frame_rate)
            avio_em_printf(out, " %s=\"%d/%d\"", (av_em_cmp_q(c->min_frame_rate, c->max_frame_rate) < 0) ? "maxFrameRate" : "frameRate", c->max_frame_rate.num, c->max_frame_rate.den);
        avio_em_printf(out, ">\n");
        ...
        for (i = 0; i < s->nb_streams; i++) {
            ...
            avio_em_printf(out, "\t\t\t<Representation id=\"%d\" mimeType=\"video/mp4\" codecs=\"%s\"%s width=\"%d\" height=\"%d\"", i, os->codec_str, os->bandwidth_str, st->codecpar->width, st->codecpar->height);
            if (st->avg_frame_rate.num)
                avio_em_printf(out, " frameRate=\"%d/%d\"", st->avg_frame_rate.num, st->avg_frame_rate.den);
            avio_em_printf(out, ">\n");

            output_segment_list(&c->streams[i], out, c);
            avio_em_printf(out, "\t\t\t</Representation>\n");
        }
        avio_em_printf(out, "\t\t</AdaptationSet>\n");
    }
    if (c->has_audio) {
        avio_em_printf(out, "\t\t<AdaptationSet contentType=\"audio\" segmentAlignment=\"true\" bitstreamSwitching=\"true\">\n");
        for (i = 0; i < s->nb_streams; i++) {
            AVEMStream *st = s->streams[i];
            OutputStream *os = &c->streams[i];
            ...
            avio_em_printf(out, "\t\t\t<Representation id=\"%d\" mimeType=\"audio/mp4\" codecs=\"%s\"%s audioSamplingRate=\"%d\">\n", i, os->codec_str, os->bandwidth_str, st->codecpar->sample_rate);
            avio_em_printf(out, "\t\t\t\t<AudioChannelConfiguration schemeIdUri=\"urn:mpeg:dash:23003:3:audio_channel_configuration:2011\" value=\"%d\" />\n", st->codecpar->channels);
            output_segment_list(&c->streams[i], out, c);
            avio_em_printf(out, "\t\t\t</Representation>\n");
        }
        avio_em_printf(out, "\t\t</AdaptationSet>\n");
    }
    avio_em_printf(out, "\t</Period>\n");
    avio_em_printf(out, "</MPD>\n");
    avio_em_flush(out);
    em_format_io_close(s, &out);
    return avpriv_em_io_move(temp_filename, s->filename);
}

static int dash_write_header(AVEMFormatContext *s)
{
    DASHContext *c = s->priv_data;
    int ret = 0, i;
    AVEMOutputFormat *oformat;
    char *ptr;
    char basename[1024];
    ...
    av_em_strlcpy(c->dirname, s->filename, sizeof(c->dirname));
    ptr = strrchr(c->dirname, '/');
    if (ptr) {
        av_em_strlcpy(basename, &ptr[1], sizeof(basename));
        ptr[1] = '\0';
    } else {
        c->dirname[0] = '\0';
        av_em_strlcpy(basename, s->filename, sizeof(basename));
    }
    ...
    oformat = av_em_guess_format("mp4", NULL, NULL);
    if (!oformat) {
        ret = AVERROR_MUXER_NOT_FOUND;
        goto fail;
    }

    c->streams = av_em_mallocz(sizeof(*c->streams) * s->nb_streams);
    if (!c->streams) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (os->bit_rate) {
            ...
            av_em_log(s, level, "No bit rate set for stream %d\n", i);
        ...
        if (!(st = avformat_em_new_stream(ctx, NULL))) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        ...
        ctx->pb = avio_em_alloc_context(os->iobuf, sizeof(os->iobuf), AVIO_FLAG_WRITE, os, NULL, dash_write, NULL);
        if (!ctx->pb) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        ...
        av_em_dict_set(&opts, "movflags", "frag_custom+dash+delay_moov", 0);
        if ((ret = avformat_em_write_header(ctx, &opts)) < 0) {
             goto fail;
        }
        os->ctx_inited = 1;
        avio_em_flush(ctx->pb);
        av_em_dict_free(&opts);

        av_em_log(s, AV_LOG_VERBOSE, "Representation %d init segment will be written to: %s\n", i, filename);
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            AVEMRational avg_frame_rate = s->streams[i]->avg_frame_rate;
            if (avg_frame_rate.num > 0) {
                if (av_em_cmp_q(avg_frame_rate, c->min_frame_rate) < 0)
                    c->min_frame_rate = avg_frame_rate;
                if (av_em_cmp_q(c->max_frame_rate, avg_frame_rate) < 0)
    ...
    if (!c->has_video && c->min_seg_duration <= 0) {
        av_em_log(s, AV_LOG_WARNING, "no video stream and no min seg duration set\n");
        ret = AVERROR(EINVAL);
    }
    ret = write_manifest(s, 0);
    if (!ret)
        av_em_log(s, AV_LOG_VERBOSE, "Manifest written to: %s\n", s->filename);
                       ...
                       int64_t index_length)
{
    ...
    if (os->nb_segments >= os->segments_size) {
        ...
        if ((err = av_em_reallocp(&os->segments, sizeof(*os->segments) *
                               os->segments_size)) < 0) {
            os->segments_size = 0;
            os->nb_segments = 0;
            return err;
    ...
    seg = av_em_mallocz(sizeof(*seg));
    if (!seg)
        return AVERROR(ENOMEM);
    av_em_strlcpy(seg->file, file, sizeof(seg->file));
    seg->time = time;
...
static void write_styp(AVEMIOContext *pb)
{
    avio_em_wb32(pb, 24);
    emio_wfourcc(pb, "styp");
    emio_wfourcc(pb, "msdh");
    avio_em_wb32(pb, 0); /* minor */
    emio_wfourcc(pb, "msdh");
    emio_wfourcc(pb, "msix");
}

static void find_index_range(AVEMFormatContext *s, const char *full_path,
                             int64_t pos, int *index_length)
{
    ...
    if (ret < 0)
        return;
    if (avio_em_seek(pb, pos, SEEK_SET) != pos) {
        em_format_io_close(s, &pb);
        return;
    }
    ret = avio_em_read(pb, buf, 8);
    em_format_io_close(s, &pb);
    if (ret < 8)
        return;
...
static int update_stream_extradata(AVEMFormatContext *s, OutputStream *os,
                                   AVEMCodecParameters *par)
{
    ...
    extradata = av_em_alloc(par->extradata_size);
...
static int dash_flush(AVEMFormatContext *s, int final, int stream)
{
    DASHContext *c = s->priv_data;
    int i, ret = 0;
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (!os->init_range_length) {
            av_em_write_frame(os->ctx, NULL);
            os->init_range_length = avio_em_tell(os->ctx->pb);
            if (!c->single_file)
                em_format_io_close(s, &os->out);
        }

        start_pos = avio_em_tell(os->ctx->pb);
        ...
        av_em_write_frame(os->ctx, NULL);
        avio_em_flush(os->ctx->pb);
        os->packets_written = 0;

        range_length = avio_em_tell(os->ctx->pb) - start_pos;
        if (c->single_file) {
            find_index_range(s, full_path, start_pos, &index_length);
        } else {
            em_format_io_close(s, &os->out);
            ret = avpriv_em_io_move(temp_path, full_path);
            if (ret < 0)
                break;
        }
        add_segment(os, filename, os->start_pts, os->max_pts - os->start_pts, start_pos, range_length, index_length);
        av_em_log(s, AV_LOG_VERBOSE, "Representation %d media segment %d written to: %s\n", i, os->segment_index, full_path);
    }
...
static int dash_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    DASHContext *c = s->priv_data;
    ...
    if ((!c->has_video || st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) &&
        ...
        av_em_compare_ts(pkt->pts - os->first_pts, st->time_base,
                      seg_end_duration, AV_TIME_BASE_Q) >= 0) {
        int64_t prev_duration = c->last_duration;

        c->last_duration = av_em_rescale_q(pkt->pts - os->start_pts,
                                        st->time_base,
                                        AV_TIME_BASE_Q);
        c->total_duration = av_em_rescale_q(pkt->pts - os->first_pts,
                                         st->time_base,
                                         AV_TIME_BASE_Q);
    ...
    os->packets_written++;
    return em_write_chained(os->ctx, 0, pkt, s, 0);
}

static int dash_write_trailer(AVEMFormatContext *s)
{
    DASHContext *c = s->priv_data;
    ...
    if (s->nb_streams > 0) {
        ...
        if (!c->last_duration)
            c->last_duration = av_em_rescale_q(os->max_pts - os->start_pts,
                                            s->streams[0]->time_base,
                                            AV_TIME_BASE_Q);
        c->total_duration = av_em_rescale_q(os->max_pts - os->first_pts,
                                         s->streams[0]->time_base,
                                         AV_TIME_BASE_Q);
    }
    dash_flush(s, 1, -1);
...
static const AVEMClass dash_class = {
...
AVEMOutputFormat ff_dash_muxer = {
    .name           = "dash",
