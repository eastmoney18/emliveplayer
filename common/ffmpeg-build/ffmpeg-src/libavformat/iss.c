...
static void get_token(AVEMIOContext *s, char *buf, int maxlen)
{
    int i = 0;
    char c;
...
static int iss_probe(AVEMProbeData *p)
{
...
static av_cold int iss_read_header(AVEMFormatContext *s)
{
    ...
    if (sscanf(token, "%d", &iss->packet_size) != 1) {
        av_em_log(s, AV_LOG_ERROR, "Failed parsing packet size\n");
    ...
    if (iss->packet_size <= 0) {
        av_em_log(s, AV_LOG_ERROR, "packet_size %d is invalid\n", iss->packet_size);
        return AVERROR_INVALIDDATA;
    }

    iss->sample_start_pos = avio_em_tell(pb);
...
static int iss_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    IssDemuxContext *iss = s->priv_data;
    int ret = av_em_get_packet(s->pb, pkt, iss->packet_size);

    if(ret != iss->packet_size)
        return AVERROR(EIO);

    pkt->stream_index = 0;
    pkt->pts = avio_em_tell(s->pb) - iss->sample_start_pos;
    if(s->streams[0]->codecpar->channels > 0)
        pkt->pts /= s->streams[0]->codecpar->channels*2;
    return 0;
}

AVEMInputFormat ff_iss_demuxer = {
    .name           = "iss",
