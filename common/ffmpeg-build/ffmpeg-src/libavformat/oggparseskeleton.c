...
static int skeleton_header(AVEMFormatContext *s, int idx)
{
    ...
    if (!strncmp(buf, "fishead", 8)) {
        ...
        if (version_major != 3 && version_major != 4) {
            av_em_log(s, AV_LOG_WARNING, "Unknown skeleton version %d.%d\n",
        ...
        if (start_den > 0 && start_num > 0) {
            int base_den;
            av_em_reduce(&start_time, &base_den, start_num, start_den, INT_MAX);
            avpriv_em_set_pts_info(st, 64, 1, base_den);
            os->lastpts =
        ...
        if (target_idx < 0) {
            av_em_log(s, AV_LOG_WARNING, "Serial number in fisbone doesn't match any stream\n");
            return 1;
        }
        os = ogg->streams + target_idx;
        if (os->start_granule != OGG_NOGRANULE_VALUE) {
            av_em_log(s, AV_LOG_WARNING, "Multiple fisbone for the same stream\n");
            return 1;
        }
