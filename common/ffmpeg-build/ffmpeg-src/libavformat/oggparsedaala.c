...
static int daala_header(AVEMFormatContext *s, int idx)
{
    int i, err;
    uint8_t *cdp;
    GetByteContext gb;
    AVEMRational timebase;
    ...
    case 0x80:
        ...
        if (timebase.num < 0 && timebase.den < 0) {
            av_em_log(s, AV_LOG_WARNING, "Invalid timebase, assuming 30 FPS\n");
            timebase.num = 1;
            timebase.den = 30;
        }
        avpriv_em_set_pts_info(st, 64, timebase.den, timebase.num);

        hdr->frame_duration = bytestream2_get_ne32(&gb);
        hdr->gpshift = bytestream2_get_byte(&gb);
        if (hdr->gpshift >= 32) {
            av_em_log(s, AV_LOG_ERROR, "Too large gpshift %d (>= 32).\n",
                   hdr->gpshift);
        ...
        if ((st->codecpar->format = daala_match_pix_fmt(&hdr->format)) < 0)
            av_em_log(s, AV_LOG_ERROR, "Unsupported pixel format - %i %i\n",
    ...
    case 0x82:
        ...
        break;
    default:
        av_em_log(s, AV_LOG_ERROR, "Unknown header type %X\n", os->buf[os->pstart]);
        return AVERROR_INVALIDDATA;
        break;
    }

    if ((err = av_em_reallocp(&st->codecpar->extradata,
...
static uint64_t daala_gptopts(AVEMFormatContext *ctx, int idx, uint64_t gp,
                              int64_t *dts)
{
...
static int daala_packet(AVEMFormatContext *s, int idx)
{
    int seg, duration = 1;
    struct ogg *ogg = s->priv_data;
    struct ogg_stream *os = ogg->streams + idx;
