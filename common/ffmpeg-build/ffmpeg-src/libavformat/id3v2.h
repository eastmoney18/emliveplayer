 ...
 */
void em_id3v2_read_dict(AVEMIOContext *pb, AVEMDictionary **metadata, const char *magic, ID3v2ExtraMeta **extra_meta);

/**
 * Read an ID3v2 tag, including supported extra metadata and chapters.
 *
 * Data is read from and stored to AVEMFormatContext.
 *
 * @param extra_meta If not NULL, extra metadata is parsed into a list of
 * ID3v2ExtraMeta structs and *extra_meta points to the head of the list
 * @param[opt] max_search_search restrict ID3 magic number search (bytes from start)
 */
void em_id3v2_read(AVEMFormatContext *s, const char *magic, ID3v2ExtraMeta **extra_meta,
 ...
 */
void ff_id3v2_finish(ID3v2EncContext *id3, AVEMIOContext *pb, int padding_bytes);
 ...
 * If in doubt, use ID3v2_DEFAULT_MAGIC.
 */
int ff_id3v2_write_simple(struct AVEMFormatContext *s, int id3v2_version, const char *magic);
...
extern const CodecEMMime ff_id3v2_mime_tags[];
