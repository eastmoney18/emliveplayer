...
/**
 * This number of bytes if added at the beginning of each AVEMPacket
...
static int get_metadata(AVEMFormatContext *s,
                        const char *const tag,
                        const unsigned data_size)
{
    uint8_t *buf = ((data_size + 1) == 0) ? NULL : av_em_alloc(data_size + 1);

    if (!buf)
        return AVERROR(ENOMEM);

    if (avio_em_read(s->pb, buf, data_size) != data_size) {
        av_em_free(buf);
        return AVERROR(EIO);
    }
    buf[data_size] = 0;
...
static int iff_probe(AVEMProbeData *p)
{
...
static const AVEMCodecTag dsd_codec_tags[] = {
...
static int parse_dsd_diin(AVEMFormatContext *s, AVEMStream *st, uint64_t eof)
{
    AVEMIOContext *pb = s->pb;

    while (avio_em_tell(pb) + 12 <= eof && !avio_em_feof(pb)) {
        uint32_t tag      = avio_em_rl32(pb);
        uint64_t size     = avio_em_rb64(pb);
        uint64_t orig_pos = avio_em_tell(pb);
        ...
        if (metadata_tag && size > 4) {
            unsigned int tag_size = avio_em_rb32(pb);
            int ret = get_metadata(s, metadata_tag, FFMIN(tag_size, size - 4));
            if (ret < 0) {
                av_em_log(s, AV_LOG_ERROR, "cannot allocate metadata tag %s!\n", metadata_tag);
                return ret;
            }
        }

        avio_em_skip(pb, size - (avio_em_tell(pb) - orig_pos) + (size & 1));
    }

    return 0;
}

static int parse_dsd_prop(AVEMFormatContext *s, AVEMStream *st, uint64_t eof)
{
    AVEMIOContext *pb = s->pb;
    char abss[24];
    ...
    while (avio_em_tell(pb) + 12 <= eof && !avio_em_feof(pb)) {
        ...
        case MKTAG('A','B','S','S'):
            ...
            hour = avio_em_rb16(pb);
            min  = avio_em_r8(pb);
            sec  = avio_em_r8(pb);
            snprintf(abss, sizeof(abss), "%02dh:%02dm:%02ds:%d", hour, min, sec, avio_em_rb32(pb));
            av_em_dict_set(&st->metadata, "absolute_start_time", abss, 0);
            break;
        ...
        case MKTAG('C','H','N','L'):
            ...
            if (st->codecpar->channels > FF_ARRAY_ELEMS(dsd_layout)) {
                avpriv_em_request_sample(s, "channel layout");
                break;
            }
            for (i = 0; i < st->codecpar->channels; i++)
                dsd_layout[i] = avio_em_rl32(pb);
            for (i = 0; i < FF_ARRAY_ELEMS(dsd_channel_layout); i++) {
                const DSDLayoutDesc * d = &dsd_channel_layout[i];
                if (av_em_get_channel_layout_nb_channels(d->layout) == st->codecpar->channels &&
        ...
        case MKTAG('C','M','P','R'):
            if (size < 4)
                return AVERROR_INVALIDDATA;
            st->codecpar->codec_tag = tag = avio_em_rl32(pb);
            st->codecpar->codec_id = em_codec_get_id(dsd_codec_tags, tag);
            if (!st->codecpar->codec_id) {
                av_em_log(s, AV_LOG_ERROR, "'%c%c%c%c' compression is not supported\n",
        ...
        case MKTAG('I','D','3',' '):
            ...
            em_id3v2_read(s, ID3v2_DEFAULT_MAGIC, &id3v2_extra_meta, size);
            if (id3v2_extra_meta) {
            ...
            if (size < avio_em_tell(pb) - orig_pos) {
                av_em_log(s, AV_LOG_ERROR, "id3 exceeds chunk size\n");
                return AVERROR_INVALIDDATA;
            }
            break;
        ...
        case MKTAG('L','S','C','O'):
            ...
            if (config != 0xFFFF) {
                ...
                if (!st->codecpar->channel_layout)
                    avpriv_em_request_sample(s, "loudspeaker configuration %d", config);
            }
            break;
        }
...
static int read_dst_frame(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    while (!avio_em_feof(pb)) {
        chunk_pos = avio_em_tell(pb);
        if (chunk_pos >= iff->body_end)
            return AVERROR_EOF;

        chunk_id = avio_em_rl32(pb);
        data_size = iff->is_64bit ? avio_em_rb64(pb) : avio_em_rb32(pb);
        data_pos = avio_em_tell(pb);
        ...
        case ID_DSTF:
            if (!pkt) {
                iff->body_pos  = avio_em_tell(pb) - (iff->is_64bit ? 12 : 8);
                iff->body_size = iff->body_end - iff->body_pos;
                return 0;
            }
            ret = av_em_get_packet(pb, pkt, data_size);
            if (ret < 0)
                return ret;
            if (data_size & 1)
                avio_em_skip(pb, 1);
            ...
            avio_em_seek(pb, chunk_pos, SEEK_SET);
            return 0;

        case ID_FRTE:
            if (data_size < 4)
                return AVERROR_INVALIDDATA;
            s->streams[0]->duration = avio_em_rb32(pb) * 588LL * s->streams[0]->codecpar->sample_rate / 44100;
            break;
        }

        avio_em_skip(pb, data_size - (avio_em_tell(pb) - data_pos) + (data_size & 1));
    }
...
static int iff_read_header(AVEMFormatContext *s)
{
    IffDemuxContext *iff = s->priv_data;
    AVEMIOContext *pb = s->pb;
    AVEMStream *st;
    ...
    iff->is_64bit = avio_em_rl32(pb) == ID_FRM8;
    avio_em_skip(pb, iff->is_64bit ? 8 : 4);
    ...
    while(!avio_em_feof(pb)) {
        ...
        case ID_VHDR:
            ...
            if (data_size >= 16) {
                avio_em_skip(pb, 1);
                iff->svx8_compression = avio_em_r8(pb);
            }
            break;
        ...
        case ID_MHDR:
            ...
            st->codecpar->channels = avio_em_rb16(pb);
            iff->maud_compression = avio_em_rb16(pb);
        ...
        case ID_CAMG:
            if (data_size < 4)
                return AVERROR_INVALIDDATA;
            screenmode                = avio_em_rb32(pb);
            break;

        case ID_CMAP:
            if (data_size < 3 || data_size > 768 || data_size % 3) {
                 av_em_log(s, AV_LOG_ERROR, "Invalid CMAP chunk size %"PRIu64"\n",
                        data_size);
                 return AVERROR_INVALIDDATA;
            }
            st->codecpar->extradata_size = data_size + IFF_EXTRA_VIDEO_SIZE;
            st->codecpar->extradata      = av_em_alloc(data_size + IFF_EXTRA_VIDEO_SIZE + AV_INPUT_BUFFER_PADDING_SIZE);
            if (!st->codecpar->extradata)
                return AVERROR(ENOMEM);
            if (avio_em_read(pb, st->codecpar->extradata + IFF_EXTRA_VIDEO_SIZE, data_size) < 0)
                return AVERROR(EIO);
            break;
        ...
        case ID_BMHD:
            ...
            avio_em_skip(pb, 4); // x, y offset
            st->codecpar->bits_per_coded_sample = avio_em_r8(pb);
            if (data_size >= 10)
                masking                      = avio_em_r8(pb);
            if (data_size >= 11)
                iff->bitmap_compression      = avio_em_r8(pb);
            if (data_size >= 14) {
                avio_em_skip(pb, 1); // padding
                transparency                 = avio_em_rb16(pb);
            }
            if (data_size >= 16) {
                st->sample_aspect_ratio.num  = avio_em_r8(pb);
                st->sample_aspect_ratio.den  = avio_em_r8(pb);
            }
            break;
        ...
        case ID_DPAN:
            avio_em_skip(pb, 2);
            st->duration = avio_em_rb16(pb);
            break;

        case ID_DPEL:
            if (data_size < 4 || (data_size & 3))
                return AVERROR_INVALIDDATA;
            if ((fmt_size = avio_em_read(pb, fmt, sizeof(fmt))) < 0)
                return fmt_size;
            if (fmt_size == sizeof(deep_rgb24) && !memcmp(fmt, deep_rgb24, sizeof(deep_rgb24)))
                ...
                avpriv_em_request_sample(s, "color format %.16s", fmt);
                return AVERROR_PATCHWELCOME;
            }
            break;
        ...
        case ID_DGBL:
            ...
            st->codecpar->width              = avio_em_rb16(pb);
            st->codecpar->height             = avio_em_rb16(pb);
            iff->bitmap_compression          = avio_em_rb16(pb);
            st->sample_aspect_ratio.num      = avio_em_r8(pb);
            st->sample_aspect_ratio.den      = avio_em_r8(pb);
        ...
        case ID_TVDC:
            if (data_size < sizeof(iff->tvdc))
                return AVERROR_INVALIDDATA;
            res = avio_em_read(pb, iff->tvdc, sizeof(iff->tvdc));
            if (res < 0)
                return res;
            break;
        ...
        case MKTAG('F','V','E','R'):
            if (data_size < 4)
                return AVERROR_INVALIDDATA;
            version = avio_em_rb32(pb);
            av_em_log(s, AV_LOG_DEBUG, "DSIFF v%d.%d.%d.%d\n",version >> 24, (version >> 16) & 0xFF, (version >> 8) & 0xFF, version & 0xFF);
            st->codecpar->codec_type = AVMEDIA_TYPE_AUDIO;
            break;
        ...
        case MKTAG('P','R','O','P'):
            if (data_size < 4)
                return AVERROR_INVALIDDATA;
            if (avio_em_rl32(pb) != MKTAG('S','N','D',' ')) {
                avpriv_em_request_sample(s, "unknown property type");
                break;
            }
        ...
        case MKTAG('C','O','M','T'):
            ...
            for (i = 0; i < nb_comments; i++) {
                ...
                year = avio_em_rb16(pb);
                mon  = avio_em_r8(pb);
                day  = avio_em_r8(pb);
                hour = avio_em_r8(pb);
                min  = avio_em_r8(pb);
                snprintf(tmp, sizeof(tmp), "%04d-%02d-%02d %02d:%02d", year, mon, day, hour, min);
                av_em_dict_set(&st->metadata, "comment_time", tmp, 0);
        ...
        avio_em_skip(pb, data_size - (avio_em_tell(pb) - orig_pos) + (data_size & 1));
    }

    if (st->codecpar->codec_tag == ID_ANIM)
        avio_em_seek(pb, 12, SEEK_SET);
    else
        avio_em_seek(pb, iff->body_pos, SEEK_SET);
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        if (st->codecpar->codec_tag == ID_16SV)
            ...
            if (iff->maud_bits == 8 && !iff->maud_compression) {
                ...
                avpriv_em_request_sample(s, "compression %d and bit depth %d", iff->maud_compression, iff->maud_bits);
                return AVERROR_PATCHWELCOME;
            }
        ...
        st->codecpar->bits_per_coded_sample = av_em_get_bits_per_sample(st->codecpar->codec_id);
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (st->codecpar->codec_tag == ID_ANIM)
            avpriv_em_set_pts_info(st, 32, 1, 60);
        ...
        if (!st->codecpar->extradata) {
            ...
            st->codecpar->extradata      = av_em_alloc(IFF_EXTRA_VIDEO_SIZE + AV_INPUT_BUFFER_PADDING_SIZE);
            if (!st->codecpar->extradata)
                return AVERROR(ENOMEM);
        }
...
static int iff_read_packet(AVEMFormatContext *s,
                           AVEMPacket *pkt)
{
    IffDemuxContext *iff = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
        if (st->codecpar->codec_tag == ID_DSD || st->codecpar->codec_tag == ID_MAUD) {
            ret = av_em_get_packet(pb, pkt, FFMIN(iff->body_end - pos, 1024 * st->codecpar->block_align));
        } else if (st->codecpar->codec_tag == ID_DST) {
            return read_dst_frame(s, pkt);
        } else {
            if (iff->body_size > INT_MAX)
                return AVERROR_INVALIDDATA;
            ret = av_em_get_packet(pb, pkt, iff->body_size);
        }
        ...
        while (!avio_em_feof(pb)) {
            ...
            orig_pos  = avio_em_tell(pb);
            chunk_id  = avio_em_rl32(pb);
            data_size = avio_em_rb32(pb);
            chunk_id2 = avio_em_rl32(pb);
...
AVEMInputFormat ff_iff_demuxer = {
    .name           = "iff",
