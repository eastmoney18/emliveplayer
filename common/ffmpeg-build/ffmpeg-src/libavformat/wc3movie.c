...
static int wc3_probe(AVEMProbeData *p)
{
    if (p->buf_size < 12)
        return 0;
...
static int wc3_read_header(AVEMFormatContext *s)
{
    ...
    do {
        ...
        case BNAM_TAG:
            ...
            if ((ret = avio_em_read(pb, buffer, size)) != size)
                return AVERROR(EIO);
            buffer[size] = 0;
            av_em_dict_set(&s->metadata, "title", buffer,
                                   AV_DICT_DONT_STRDUP_VAL);
            break;

        case SIZE_TAG:
            /* video resolution override */
            wc3->width  = avio_em_rl32(pb);
            wc3->height = avio_em_rl32(pb);
            break;

        case PALT_TAG:
            /* one of several palettes */
            avio_em_seek(pb, -8, SEEK_CUR);
            av_em_append_packet(pb, &wc3->vpkt, 8 + PALETTE_SIZE);
            break;

        default:
            av_em_log(s, AV_LOG_ERROR, "  unrecognized WC3 chunk: %c%c%c%c (0x%02X%02X%02X%02X)\n",
    ...
    if (!st)
        return AVERROR(ENOMEM);
    avpriv_em_set_pts_info(st, 33, 1, WC3_FRAME_FPS);
...
static int wc3_read_packet(AVEMFormatContext *s,
                           AVEMPacket *pkt)
{
    Wc3DemuxContext *wc3 = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    while (!packet_read) {
        ...
        case SHOT_TAG:
            /* load up new palette */
            avio_em_seek(pb, -8, SEEK_CUR);
            av_em_append_packet(pb, &wc3->vpkt, 8 + 4);
            break;

        case VGA__TAG:
            /* send out video chunk */
            avio_em_seek(pb, -8, SEEK_CUR);
            ret= av_em_append_packet(pb, &wc3->vpkt, 8 + size);
        ...
        case TEXT_TAG:
            ...
            avio_em_skip(pb, size);
#else
            if ((unsigned)size > sizeof(text) || (ret = avio_em_read(pb, text, size)) != size)
                ret = AVERROR(EIO);
            else {
                int i = 0;
                av_log (s, AV_LOG_DEBUG, "Subtitle time!\n");
                if (i >= size || av_em_strnlen(&text[i + 1], size - i - 1) >= size - i - 1)
                    return AVERROR_INVALIDDATA;
...
static int wc3_read_close(AVEMFormatContext *s)
{
    Wc3DemuxContext *wc3 = s->priv_data;

    if (wc3->vpkt.size > 0)
        av_em_packet_unref(&wc3->vpkt);

    return 0;
}

AVEMInputFormat ff_wc3_demuxer = {
    .name           = "wc3movie",
