...
static const AVEMClass crypto_class = {
    .class_name     = "crypto",
                       ...
                       const char *desc)
{
    if (!*buf_len) {
        if (!default_buf_len) {
            av_em_log(c, AV_LOG_ERROR, "%s not set\n", desc);
        ...
        *buf = av_em_memdup(default_buf, default_buf_len);
        if (!*buf)
...
static int crypto_open2(EMURLContext *h, const char *uri, int flags, AVEMDictionary **options)
{
    const char *nested_url;
    int ret = 0;
    CryptoContext *c = h->priv_data;

    if (!av_em_strstart(uri, "crypto+", &nested_url) &&
        !av_em_strstart(uri, "crypto:", &nested_url)) {
    ...
    if ((ret = ffurl_em_open_whitelist(&c->hd, nested_url, flags,
                                    &h->interrupt_callback, options,
                                    h->protocol_whitelist, h->protocol_blacklist, h)) < 0) {
        av_em_log(h, AV_LOG_ERROR, "Unable to open resource: %s\n", nested_url);
        goto err;
    }
    ...
    if (flags & AVIO_FLAG_READ) {
        ...
        ret = av_em_aes_init(c->aes_decrypt, c->decrypt_key, BLOCKSIZE*8, 1);
        if (ret < 0)
            goto err;
    }

    if (flags & AVIO_FLAG_WRITE) {
        c->aes_encrypt = av_em_aes_alloc();
        if (!c->aes_encrypt) {
            ret = AVERROR(ENOMEM);
            goto err;
        }
        ret = av_em_aes_init(c->aes_encrypt, c->encrypt_key, BLOCKSIZE*8, 0);
        if (ret < 0)
            goto err;
    }
...
static int crypto_read(EMURLContext *h, uint8_t *buf, int size)
{
    CryptoContext *c = h->priv_data;
    int blocks;
retry:
    ...
    while (c->indata - c->indata_used < 2*BLOCKSIZE) {
        int n = ffurl_em_read(c->hd, c->inbuffer + c->indata,
    ...
    if (!c->eof)
        blocks--;
    av_em_aes_crypt(c->aes_decrypt, c->outbuffer, c->inbuffer + c->indata_used,
...
static int crypto_write(EMURLContext *h, const unsigned char *buf, int size)
{
    CryptoContext *c = h->priv_data;
    ...
    if (out_size) {
        out_buf = av_em_alloc(out_size);
        if (!out_buf)
            return AVERROR(ENOMEM);

        if (c->pad_len) {
            memcpy(&c->pad[c->pad_len], buf, BLOCKSIZE - c->pad_len);
            av_em_aes_crypt(c->aes_encrypt, out_buf, c->pad, 1, c->encrypt_iv, 0);
            blocks--;
        }

        av_em_aes_crypt(c->aes_encrypt, &out_buf[c->pad_len ? BLOCKSIZE : 0],
                             &buf[c->pad_len ? BLOCKSIZE - c->pad_len: 0],
                             blocks, c->encrypt_iv, 0);

        ret = ffurl_em_write(c->hd, out_buf, out_size);
        av_em_free(out_buf);
        if (ret < 0)
            return ret;
...
static int crypto_close(EMURLContext *h)
{
    CryptoContext *c = h->priv_data;
    ...
    if (c->aes_encrypt) {
        ...
        av_em_aes_crypt(c->aes_encrypt, out_buf, c->pad, 1, c->encrypt_iv, 0);
        if ((ret =  ffurl_em_write(c->hd, out_buf, BLOCKSIZE)) < 0)
            return ret;
    }

    if (c->hd)
        ffurl_em_close(c->hd);
    av_em_freep(&c->aes_decrypt);
    av_em_freep(&c->aes_encrypt);
    return 0;
}

const EMURLProtocol em_crypto_protocol = {
    .name            = "crypto",
