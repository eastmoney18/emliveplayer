...
static int write_header(AVEMFormatContext *s)
{
    ...
    if (s->nb_streams != 1 || par->codec_id != AV_CODEC_ID_ASS) {
        av_em_log(s, AV_LOG_ERROR, "Exactly one ASS/SSA stream is needed.\n");
...
static void purge_dialogues(AVEMFormatContext *s, int force)
{
    int n = 0;
    ASSContext *ass = s->priv_data;
    ...
    while (dialogue && (dialogue->readorder == ass->expected_readorder || force)) {
        ...
        if (dialogue->readorder != ass->expected_readorder) {
            av_em_log(s, AV_LOG_WARNING, "ReadOrder gap found between %d and %d\n",
                   ass->expected_readorder, dialogue->readorder);
            ass->expected_readorder = dialogue->readorder;
        }
        avio_em_printf(s->pb, "Dialogue: %s\r\n", dialogue->line);
        if (dialogue == ass->last_added_dialogue)
            ass->last_added_dialogue = next;
        av_em_freep(&dialogue->line);
        av_em_free(dialogue);
        if (next)
            next->prev = NULL;
    ...
    ass->cache_size -= n;
    if (n > 1)
        av_em_log(s, AV_LOG_DEBUG, "wrote %d ASS lines, cached dialogues: %d, waiting for event id %d\n",
...
static int write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    int hh1, mm1, ss1, ms1;
    int hh2, mm2, ss2, ms2;
    DialogueLine *dialogue = av_em_mallocz(sizeof(*dialogue));
    ...
    if (dialogue->readorder < ass->expected_readorder)
        av_em_log(s, AV_LOG_WARNING, "Unexpected ReadOrder %d\n",
               dialogue->readorder);
    if (*p == ',')
        p++;
    ...
    dialogue->line = av_em_asprintf("%s%ld,%d:%02d:%02d.%02d,%d:%02d:%02d.%02d,%s",
                                 ass->ssa_mode ? "Marked=" : "",
...
static int write_trailer(AVEMFormatContext *s)
{
    ...
    if (ass->trailer) {
        avio_em_write(s->pb, ass->trailer, ass->trailer_size);
    }
...
static const AVEMClass ass_class = {
