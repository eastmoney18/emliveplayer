    ...
    if (n_## entry_name ##s) {                                                 \
        ...
        for (i = 0; i < n_## entry_name ##s; i++) {                            \
            ...
            if (!*item_name)                                                   \
                continue;                                                      \
            if (n)                                                             \
                av_em_dict_set(&s->metadata, #entry_name, "\n", AV_DICT_APPEND);  \
            av_em_dict_set(&s->metadata, #entry_name, item_name, AV_DICT_APPEND); \
            n++;                                                               \
        }                                                                      \
                                                                               \
        extra = av_em_asprintf(", %u/%u " #entry_name "%s",                       \
                            n, n_## entry_name ##s, n > 1 ? "s" : "");         \
        if (!extra)                                                            \
            return AVERROR(ENOMEM);                                            \
        av_em_dict_set(&s->metadata, "extra info", extra, AV_DICT_APPEND);        \
        av_em_free(extra);                                                        \
    }                                                                          \
} while (0)

static int modplug_load_metadata(AVEMFormatContext *s)
{
    ...
    if (name && *name) av_em_dict_set(&s->metadata, "name",    name, 0);
    if (msg  && *msg)  av_em_dict_set(&s->metadata, "message", msg,  0);

    extra = av_em_asprintf("%u pattern%s, %u channel%s",
                        n_patterns, n_patterns > 1 ? "s" : "",
                        n_channels, n_channels > 1 ? "s" : "");
    if (!extra)
        return AVERROR(ENOMEM);
    av_em_dict_set(&s->metadata, "extra info", extra, AV_DICT_DONT_STRDUP_VAL);
...
static int modplug_read_header(AVEMFormatContext *s)
{
    AVEMStream *st;
    AVEMIOContext *pb = s->pb;
    ModPlug_Settings settings;
    ModPlugContext *modplug = s->priv_data;
    int64_t sz = avio_em_size(pb);

    if (sz < 0) {
        av_em_log(s, AV_LOG_WARNING, "Could not determine file size\n");
        sz = modplug->max_size;
    } else if (modplug->max_size && sz > modplug->max_size) {
        sz = modplug->max_size;
        av_em_log(s, AV_LOG_WARNING, "Max file size reach%s, allocating %"PRIi64"B "
               "but demuxing is likely to fail due to incomplete buffer\n",
               sz == FF_MODPLUG_DEF_FILE_SIZE ? " (see -max_size)" : "", sz);
    }

    if (modplug->color_eval) {
        int r = av_em_expr_parse(&modplug->expr, modplug->color_eval, var_names,
                              NULL, NULL, NULL, NULL, 0, s);
        if (r < 0)
            return r;
    }

    modplug->buf = av_em_alloc(modplug->max_size);
    if (!modplug->buf)
        return AVERROR(ENOMEM);
    sz = avio_em_read(pb, modplug->buf, sz);
    ...
    if (modplug->video_stream) {
        ...
        if (!vst)
            return AVERROR(ENOMEM);
        avpriv_em_set_pts_info(vst, 64, 1, 1000);
...
static int modplug_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ModPlugContext *modplug = s->priv_data;
    ...
    if (modplug->video_stream) {
        ...
        if (modplug->video_switch) {
            ...
            if (av_em_new_packet(pkt, modplug->fsize) < 0)
                return AVERROR(ENOMEM);
            pkt->stream_index = 1;
            ...
            if (modplug->expr) {
                ...
                for (y = 0; y < modplug->h; y++) {
                    for (x = 0; x < modplug->w; x++) {
                        ...
                        var_values[VAR_X] = x;
                        var_values[VAR_Y] = y;
                        color = av_em_expr_eval(modplug->expr, var_values, NULL);
    ...
    if (av_em_new_packet(pkt, AUDIO_PKT_SIZE) < 0)
        return AVERROR(ENOMEM);
...
static int modplug_read_close(AVEMFormatContext *s)
{
    ModPlugContext *modplug = s->priv_data;
    ModPlug_Unload(modplug->f);
    av_em_freep(&modplug->buf);
    return 0;
}

static int modplug_read_seek(AVEMFormatContext *s, int stream_idx, int64_t ts, int flags)
{
    ModPlugContext *modplug = s->priv_data;
...
static int modplug_probe(AVEMProbeData *p)
{
    if (av_em_match_ext(p->filename, modplug_extensions)) {
        if (p->buf_size < 16384)
...
static const AVEMClass modplug_class = {
...
AVEMInputFormat ff_libmodplug_demuxer = {
    .name           = "libmodplug",
