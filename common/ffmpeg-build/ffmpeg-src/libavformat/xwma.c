...
static int xwma_probe(AVEMProbeData *p)
{
...
static int xwma_read_header(AVEMFormatContext *s)
{
    int64_t size;
    int ret = 0;
    ...
    if (st->codecpar->codec_id != AV_CODEC_ID_WMAV2 &&
        ...
        avpriv_em_request_sample(s, "Unexpected codec (tag 0x04%x; id %d)",
        ...
        if (st->codecpar->extradata_size != 0) {
             ...
             */
            avpriv_em_request_sample(s, "Unexpected extradata (%d bytes)",
                                  st->codecpar->extradata_size);
        } else if (st->codecpar->codec_id == AV_CODEC_ID_WMAPRO) {
            if (em_alloc_extradata(st->codecpar, 18))
                return AVERROR(ENOMEM);
            ...
            if (em_alloc_extradata(st->codecpar, 6))
                return AVERROR(ENOMEM);
    ...
    if (!st->codecpar->channels) {
        av_em_log(s, AV_LOG_WARNING, "Invalid channel count: %d\n",
               st->codecpar->channels);
        return AVERROR_INVALIDDATA;
    }
    if (!st->codecpar->bits_per_coded_sample) {
        av_em_log(s, AV_LOG_WARNING, "Invalid bits_per_coded_sample: %d\n",
    ...
    for (;;) {
        ...
        if (tag == MKTAG('d', 'a', 't', 'a')) {
            ...
            if (dpds_table) {
                av_em_log(s, AV_LOG_ERROR, "two dpds chunks present\n");
             ...
             */
            dpds_table = av_em_malloc_array(dpds_table_size, sizeof(uint32_t));
            if (!dpds_table) {
                return AVERROR(ENOMEM);
            }

            for (i = 0; i < dpds_table_size; ++i) {
                dpds_table[i] = avio_em_rl32(pb);
                size -= 4;
            }
        }
    ...
    if (!size) {
        ...
        xwma->data_end = avio_em_tell(pb) + size;
...
fail:
    av_em_free(dpds_table);

    return ret;
}

static int xwma_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int ret, size;
    int64_t left;
    AVEMStream *st;
    ...
    left = xwma->data_end - avio_em_tell(s->pb);
    if (left <= 0) {
        return AVERROR_EOF;
    }
...
AVEMInputFormat ff_xwma_demuxer = {
    .name           = "xwma",
