...
struct PayloadContext {
    ...
    uint32_t timestamp;
    AVEMIOContext* fragment;    ///< buffer for split payloads
    uint8_t *split_buf;
...
{
    emio_free_dyn_buf(&data->fragment);
    av_em_freep(&data->split_buf);
}


static int xiph_handle_packet(AVEMFormatContext *ctx, PayloadContext *data,
                              ...
                              int flags)
{
    ...
    if (!buf) {
        if (!data->split_buf || data->split_pos + 2 > data->split_buf_len ||
            data->split_pkts <= 0) {
            av_em_log(ctx, AV_LOG_ERROR, "No more data to return\n");
            return AVERROR_INVALIDDATA;
        }
        pkt_len = AV_RB16(data->split_buf + data->split_pos);
        data->split_pos += 2;
        if (pkt_len > data->split_buf_len - data->split_pos) {
            av_em_log(ctx, AV_LOG_ERROR, "Not enough data to return\n");
            return AVERROR_INVALIDDATA;
        }
        if (av_em_new_packet(pkt, pkt_len)) {
            av_em_log(ctx, AV_LOG_ERROR, "Out of memory.\n");
    ...
    if (len < 6 || len > INT_MAX/2) {
        av_em_log(ctx, AV_LOG_ERROR, "Invalid %d byte packet\n", len);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (fragmented == 0) {
        ...
        if (num_pkts > 0) {
            if (len > data->split_buf_size || !data->split_buf) {
                ...
                data->split_buf = av_em_alloc(data->split_buf_size);
                if (!data->split_buf) {
                    av_em_log(ctx, AV_LOG_ERROR, "Out of memory.\n");
        ...
        if((res = avio_em_open_dyn_buf(&data->fragment)) < 0)
            return res;

        avio_em_write(data->fragment, buf, pkt_len);
        ...
        if (data->timestamp != *timestamp) {
            ...
            emio_free_dyn_buf(&data->fragment);
            av_em_log(ctx, AV_LOG_ERROR, "RTP timestamps don't match!\n");
            return AVERROR_INVALIDDATA;
        }
        if (!data->fragment) {
 ...
 */
static int
parse_packed_headers(AVEMFormatContext *s,
                     const uint8_t * packed_headers,
                     const uint8_t * packed_headers_end,
                     AVEMCodecParameters *par, PayloadContext * xiph_data)
{
    ...
    if (em_alloc_extradata(par, extradata_alloc)) {
        av_em_log(s, AV_LOG_ERROR, "Out of memory\n");
        return AVERROR(ENOMEM);
    }
    ptr = par->extradata;
    *ptr++ = 2;
    ptr += av_em_xiphlacing(ptr, length1);
    ptr += av_em_xiphlacing(ptr, length2);
...
static int xiph_parse_fmtp_pair(AVEMFormatContext *s,
                                AVEMStream* stream,
                                ...
                                const char *attr, const char *value)
{
    ...
    if (!strcmp(attr, "sampling")) {
        ...
        if (decoded_alloc <= INT_MAX) {
            decoded_packet = av_em_alloc(decoded_alloc);
            if (decoded_packet) {
                packet_size =
                    av_em_base64_decode(decoded_packet, value, decoded_alloc);
            ...
            av_em_log(s, AV_LOG_ERROR, "Packet too large\n");
            result = AVERROR_INVALIDDATA;
        }
        av_em_free(decoded_packet);
    }
    return result;
}

static int xiph_parse_sdp_line(AVEMFormatContext *s, int st_index,
