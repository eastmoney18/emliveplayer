...
static int rtsp_read_close(AVEMFormatContext *s)
{
    ...
    em_network_close();
    rt->real_setup = NULL;
    av_em_freep(&rt->real_setup_cache);
    return 0;
}

static inline int read_line(AVEMFormatContext *s, char *rbuf, const int rbufsize,
                            int *rbuflen)
{
    RTSPState *rt = s->priv_data;
    int idx       = 0;
    ...
    *rbuflen      = 0;
    ...
    do {
        ret = ffurl_em_read_complete(rt->rtsp_hd, rbuf + idx, 1);
        if (ret <= 0)
    ...
    av_em_log(s, AV_LOG_ERROR, "Message too long\n");
    return AVERROR(EIO);
}

static int rtsp_send_reply(AVEMFormatContext *s, enum RTSPStatusCode code,
                           const char *extracontent, uint16_t seq)
{
    ...
    av_em_strlcatf(message, sizeof(message), "CSeq: %d\r\n", seq);
    av_em_strlcatf(message, sizeof(message), "Server: %s\r\n", LIBAVFORMAT_IDENT);
    if (extracontent)
        av_em_strlcat(message, extracontent, sizeof(message));
    av_em_strlcat(message, "\r\n", sizeof(message));
    av_em_log(s, AV_LOG_TRACE, "Sending response:\n%s", message);
    ffurl_em_write(rt->rtsp_hd_out, message, strlen(message));

    return 0;
}

static inline int check_sessionid(AVEMFormatContext *s,
                                  RTSPMessageHeader *request)
{
    RTSPState *rt = s->priv_data;
    unsigned char *session_id = rt->session_id;
    if (!session_id[0]) {
        av_em_log(s, AV_LOG_WARNING, "There is no session-id at the moment\n");
        return 0;
    }
    if (strcmp(session_id, request->session_id)) {
        av_em_log(s, AV_LOG_ERROR, "Unexpected session-id %s\n",
               request->session_id);
...
static inline int rtsp_read_request(AVEMFormatContext *s,
                                    RTSPMessageHeader *request,
                                    const char *method)
{
    RTSPState *rt = s->priv_data;
    ...
    do {
        ...
        if (ret)
            return ret;
        if (rbuflen > 1) {
            av_em_log(s, AV_LOG_TRACE, "Parsing[%d]: %s\n", rbuflen, rbuf);
            ff_rtsp_parse_line(s, request, rbuf, rt, method);
        }
    } while (rbuflen > 0);
    if (request->seq != rt->seq + 1) {
        av_em_log(s, AV_LOG_ERROR, "Unexpected Sequence number %d\n",
               request->seq);
        return AVERROR(EINVAL);
    }
...
static int rtsp_read_announce(AVEMFormatContext *s)
{
    RTSPState *rt             = s->priv_data;
    ...
    if (strcmp(request.content_type, "application/sdp")) {
        av_em_log(s, AV_LOG_ERROR, "Unexpected content type %s\n",
    ...
    if (request.content_length && request.content_length < sizeof(sdp) - 1) {
        /* Read SDP */
        if (ffurl_em_read_complete(rt->rtsp_hd, sdp, request.content_length)
        ...
        av_em_log(s, AV_LOG_VERBOSE, "SDP: %s\n", sdp);
...
static int rtsp_read_options(AVEMFormatContext *s)
{
    RTSPState *rt             = s->priv_data;
    RTSPMessageHeader request = { 0 };
    int ret                   = 0;
...
static int rtsp_read_setup(AVEMFormatContext *s, char* host, char *controlurl)
{
    RTSPState *rt             = s->priv_data;
    RTSPMessageHeader request = { 0 };
    int ret                   = 0;
    ...
    if (!request.nb_transports) {
        av_em_log(s, AV_LOG_ERROR, "No transport defined in SETUP\n");
    ...
    for (transportidx = 0; transportidx < request.nb_transports;
        ...
        if (!request.transports[transportidx].mode_record ||
            ...
            av_em_log(s, AV_LOG_ERROR, "mode=record/receive not set or transport"
                   " protocol not supported (yet)\n");
            return AVERROR_INVALIDDATA;
        }
    }
    if (request.nb_transports > 1)
        av_em_log(s, AV_LOG_WARNING, "More than one transport not supported, "
               "using first of all\n");
    ...
    if (streamid == rt->nb_rtsp_streams) {
        av_em_log(s, AV_LOG_ERROR, "Unable to find requested track\n");
        return AVERROR_STREAM_NOT_FOUND;
    }
    ...
    if (request.transports[0].lower_transport == RTSP_LOWER_TRANSPORT_TCP) {
        ...
        do {
            ...
            em_url_join(url, sizeof(url), "rtp", NULL, host, localport, NULL);
            av_em_log(s, AV_LOG_TRACE, "Opening: %s", url);
            ret = ffurl_em_open_whitelist(&rtsp_st->rtp_handle, url, AVIO_FLAG_READ_WRITE,
        ...
        av_em_log(s, AV_LOG_TRACE, "Listening on: %d",
    ...
    while (strlen(rt->session_id) < 8)
        av_em_strlcatf(rt->session_id, 512, "%u", av_em_get_random_seed());

    av_em_strlcatf(responseheaders, sizeof(responseheaders), "Session: %s\r\n",
                rt->session_id);
    /* Send Reply */
...
static int rtsp_read_record(AVEMFormatContext *s)
{
    RTSPState *rt             = s->priv_data;
    RTSPMessageHeader request = { 0 };
    int ret                   = 0;
...
static inline int parse_command_line(AVEMFormatContext *s, const char *line,
                                     ...
                                     enum RTSPMethod *methodcode)
{
    ...
    if (!linept) {
        av_em_log(s, AV_LOG_ERROR, "Error parsing method string\n");
        return AVERROR_INVALIDDATA;
    }

    if (linept - line > methodsize - 1) {
        av_em_log(s, AV_LOG_ERROR, "Method string too long\n");
        return AVERROR(EIO);
    }
    ...
    if (rt->state == RTSP_STATE_IDLE) {
        if ((*methodcode != ANNOUNCE) && (*methodcode != OPTIONS)) {
            av_em_log(s, AV_LOG_ERROR, "Unexpected command in Idle State %s\n",
                   line);
        ...
        if ((*methodcode != OPTIONS) && (*methodcode != RECORD)
            && (*methodcode != SETUP)) {
            av_em_log(s, AV_LOG_ERROR, "Unexpected command in Paused State %s\n",
                   line);
            return AVERROR_PROTOCOL_NOT_FOUND;
        }
        ...
        if ((*methodcode != PAUSE) && (*methodcode != OPTIONS)
            && (*methodcode != TEARDOWN)) {
            av_em_log(s, AV_LOG_ERROR, "Unexpected command in Streaming State"
                   " %s\n", line);
            return AVERROR_PROTOCOL_NOT_FOUND;
        }
    } else {
        av_em_log(s, AV_LOG_ERROR, "Unexpected State [%d]\n", rt->state);
        return AVERROR_BUG;
    }

    searchlinept = strchr(linept, ' ');
    if (!searchlinept) {
        av_em_log(s, AV_LOG_ERROR, "Error parsing message URI\n");
        return AVERROR_INVALIDDATA;
    }
    if (searchlinept - linept > urisize - 1) {
        av_em_log(s, AV_LOG_ERROR, "uri string length exceeded buffer size\n");
        return AVERROR(EIO);
    }
    ...
    if (strcmp(rt->control_uri, uri)) {
        ...
        int ctl_port;
        av_em_url_split(NULL, 0, auth, sizeof(auth), host, sizeof(host), &port,
                     path, sizeof(path), uri);
        av_em_url_split(NULL, 0, ctl_auth, sizeof(ctl_auth), ctl_host,
                     sizeof(ctl_host), &ctl_port, ctl_path, sizeof(ctl_path),
                     rt->control_uri);
        if (strcmp(host, ctl_host))
            av_em_log(s, AV_LOG_INFO, "Host %s differs from expected %s\n",
                   host, ctl_host);
        if (strcmp(path, ctl_path) && *methodcode != SETUP)
            av_em_log(s, AV_LOG_WARNING, "WARNING: Path %s differs from expected"
                   " %s\n", path, ctl_path);
        if (*methodcode == ANNOUNCE) {
            av_em_log(s, AV_LOG_INFO,
                   "Updating control URI to %s\n", uri);
            av_em_strlcpy(rt->control_uri, uri, sizeof(rt->control_uri));
        }
    }

    linept = searchlinept + 1;
    if (!av_em_strstart(linept, "RTSP/1.0", NULL)) {
        av_em_log(s, AV_LOG_ERROR, "Error parsing protocol or version\n");
        return AVERROR_PROTOCOL_NOT_FOUND;
    }
    return 0;
}

int ff_rtsp_parse_streaming_commands(AVEMFormatContext *s)
{
    RTSPState *rt = s->priv_data;
    ...
    if (ret) {
        av_em_log(s, AV_LOG_ERROR, "RTSP: Unexpected Command\n");
        return ret;
    }
...
static int rtsp_read_play(AVEMFormatContext *s)
{
    RTSPState *rt = s->priv_data;
    ...
    av_em_log(s, AV_LOG_DEBUG, "hello state=%d\n", rt->state);
    rt->nb_byes = 0;
        ...
        if (rt->transport == RTSP_TRANSPORT_RTP &&
            ...
            for (i = 0; i < rt->nb_rtsp_streams; i++) {
                ...
                rtpctx->range_start_offset =
                    av_em_rescale_q(reply->range_start, AV_TIME_BASE_Q,
                                 st->time_base);
            }
        }
    }
...
/* pause the stream */
static int rtsp_read_pause(AVEMFormatContext *s)
{
    RTSPState *rt = s->priv_data;
    RTSPMessageHeader reply1, *reply = &reply1;
...
int ff_rtsp_setup_input_streams(AVEMFormatContext *s, RTSPMessageHeader *reply)
{
    RTSPState *rt = s->priv_data;
    char cmd[1024];
...
static int rtsp_listen(AVEMFormatContext *s)
{
    RTSPState *rt = s->priv_data;
    ...
    av_em_url_split(proto, sizeof(proto), auth, sizeof(auth), host, sizeof(host),
                 &port, path, sizeof(path), s->filename);

    /* em_url_join. No authorization by now (NULL) */
    em_url_join(rt->control_uri, sizeof(rt->control_uri), proto, NULL, host,
                port, "%s", path);
    ...
    em_url_join(tcpname, sizeof(tcpname), lower_proto, NULL, host, port,
                "?listen&listen_timeout=%d", rt->initial_timeout * 1000);

    if (ret = ffurl_em_open_whitelist(&rt->rtsp_hd, tcpname, AVIO_FLAG_READ_WRITE,
                                   &s->interrupt_callback, NULL,
                                   s->protocol_whitelist, s->protocol_blacklist, NULL)) {
        av_em_log(s, AV_LOG_ERROR, "Unable to open RTSP for listening\n");
        return ret;
    }
    rt->state       = RTSP_STATE_IDLE;
...
static int rtsp_probe(AVEMProbeData *p)
{
    if (
#if CONFIG_TLS_PROTOCOL
        av_em_strstart(p->filename, "rtsps:", NULL) ||
#endif
        av_em_strstart(p->filename, "rtsp:", NULL))
        return AVPROBE_SCORE_MAX;
    return 0;
}

static int rtsp_read_header(AVEMFormatContext *s)
{
    RTSPState *rt = s->priv_data;
    int ret;
    ...
    if (rt->rtsp_flags & RTSP_FLAG_LISTEN) {
        ...
        rt->real_setup_cache = !s->nb_streams ? NULL :
            av_em_mallocz_array(s->nb_streams, 2 * sizeof(*rt->real_setup_cache));
...
int ff_rtsp_tcp_read_packet(AVEMFormatContext *s, RTSPStream **prtsp_st,
                            uint8_t *buf, int buf_size)
{
    ...
    av_em_log(s, AV_LOG_TRACE, "tcp_read_packet:\n");
redo:
    for (;;) {
    ...
    id  = buf[0];
    len = AV_RB16(buf + 1);
    av_em_log(s, AV_LOG_TRACE, "id=%d len=%d\n", id, len);
    if (len > buf_size || len < 8)
        goto redo;
    /* get the data */
    ret = ffurl_em_read_complete(rt->rtsp_hd, buf, len);
    if (ret != len)
        return -1;
...
static int resetup_tcp(AVEMFormatContext *s)
{
    RTSPState *rt = s->priv_data;
    char host[1024];
    int port;

    av_em_url_split(NULL, 0, NULL, 0, host, sizeof(host), &port, NULL, 0,
                 s->filename);
...
static int rtsp_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    RTSPState *rt = s->priv_data;
    int ret;
    RTSPMessageHeader reply1, *reply = &reply1;
...
retry:
    if (rt->server_type == RTSP_SERVER_REAL) {
        ...
        if (!rt->need_subscription) {
            if (memcmp (rt->real_setup, rt->real_setup_cache,
                        sizeof(enum AVEMDiscard) * s->nb_streams)) {
                snprintf(cmd, sizeof(cmd),
                         "Unsubscribe: %s\r\n",
        ...
        if (rt->need_subscription) {
                ...
                for (r = 0; r < s->nb_streams; r++) {
                    if (s->streams[r]->id == i) {
                        if (s->streams[r]->discard != AVDISCARD_ALL) {
                            if (!first)
                                av_em_strlcat(rt->last_subscription, ",",
            ...
            av_em_strlcatf(cmd, sizeof(cmd), "%s\r\n", rt->last_subscription);
            ff_rtsp_send_cmd(s, "SET_PARAMETER", rt->control_uri,
                             cmd, reply, NULL);
            if (reply->status_code != RTSP_STATUS_OK)
                return ff_rtsp_averror(reply->status_code, AVERROR_INVALIDDATA);
    ...
    if (ret < 0) {
        if (ret == AVERROR(ETIMEDOUT) && !rt->packets) {
            if (rt->lower_transport == RTSP_LOWER_TRANSPORT_UDP &&
                ...
                RTSPMessageHeader reply1, *reply = &reply1;
                av_em_log(s, AV_LOG_WARNING, "UDP timeout, retrying with TCP\n");
    ...
    if (!(rt->rtsp_flags & RTSP_FLAG_LISTEN)) {
        ...
        if ((av_em_gettime_relative() - rt->last_cmd_time) / 1000000 >= rt->timeout / 2 ||
            rt->auth_state.stale) {
...
static int rtsp_read_seek(AVEMFormatContext *s, int stream_index,
                          int64_t timestamp, int flags)
{
    RTSPState *rt = s->priv_data;
    int ret;

    rt->seek_timestamp = av_em_rescale_q(timestamp,
...
static const AVEMClass rtsp_demuxer_class = {
...
AVEMInputFormat ff_rtsp_demuxer = {
    .name           = "rtsp",
