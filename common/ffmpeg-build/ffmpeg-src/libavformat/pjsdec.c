...
static int pjs_probe(AVEMProbeData *p)
{
    char c;
    int64_t start, end;
...
static int pjs_read_header(AVEMFormatContext *s)
{
...
static int pjs_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    PJSContext *pjs = s->priv_data;
    return ff_subtitles_queue_read_packet(&pjs->q, pkt);
}

static int pjs_read_seek(AVEMFormatContext *s, int stream_index,
...
static int pjs_read_close(AVEMFormatContext *s)
{
    PJSContext *pjs = s->priv_data;
...
AVEMInputFormat ff_pjs_demuxer = {
    .name           = "pjs",
