...
static int find_expected_header(AVEMCodecParameters *p, int size, int key_frame,
                                uint8_t out[64])
{
...
static int find_header_idx(AVEMFormatContext *s, AVEMCodecParameters *p, int size,
                           int frame_type)
{
...
static void build_elision_headers(AVEMFormatContext *s)
{
    NUTContext *nut = s->priv_data;
    int i;
    //FIXME this is lame
...
static void build_frame_code(AVEMFormatContext *s)
{
    NUTContext *nut = s->priv_data;
    ...
    for (stream_id = 0; stream_id < s->nb_streams; stream_id++) {
        ...
        AVEMCodecParameters *par        = s->streams[stream_id]->codecpar;
        ...
        if (par->codec_type == AVMEDIA_TYPE_AUDIO) {
            frame_size = av_em_get_audio_frame_duration2(par, 0);
            if (par->codec_id == AV_CODEC_ID_VORBIS && !frame_size)
                frame_size = 64;
        } else {
            AVEMRational f = av_em_div_q(av_em_inv_q(s->streams[stream_id]->avg_frame_rate), *nut->stream[stream_id].time_base);
            if (f.den == 1 && f.num>0)
                frame_size = f.num;
        }
        if (!frame_size)
        ...
        if (is_audio) {
            ...
            if (par->block_align > 0) {
                ...
                int frame_size = av_em_get_audio_frame_duration2(par, 0);
...
static void put_tt(NUTContext *nut, AVEMRational *time_base, AVEMIOContext *bc, uint64_t val)
{
 ...
 */
static void put_str(AVEMIOContext *bc, const char *string)
{
    size_t len = strlen(string);

    em_put_v(bc, len);
    avio_em_write(bc, string, len);
}

static void put_s(AVEMIOContext *bc, int64_t val)
{
    em_put_v(bc, 2 * FFABS(val) - (val > 0));
}

#ifdef TRACE
static inline void ff_put_v_trace(AVEMIOContext *bc, uint64_t v, const char *file,
                                  const char *func, int line)
{
    av_em_log(NULL, AV_LOG_DEBUG, "ff_put_v %5"PRId64" / %"PRIX64" in %s %s:%d\n", v, v, file, func, line);

    em_put_v(bc, v);
}

static inline void put_s_trace(AVEMIOContext *bc, int64_t v, const char *file, const char *func, int line)
{
    av_em_log(NULL, AV_LOG_DEBUG, "put_s %5"PRId64" / %"PRIX64" in %s %s:%d\n", v, v, file, func, line);

    put_s(bc, v);
}
#define em_put_v(bc, v)  ff_put_v_trace(bc, v, __FILE__, __PRETTY_FUNCTION__, __LINE__)
#define put_s(bc, v)  put_s_trace(bc, v, __FILE__, __PRETTY_FUNCTION__, __LINE__)
#endif

//FIXME remove calculate_checksum
static void put_packet(NUTContext *nut, AVEMIOContext *bc, AVEMIOContext *dyn_bc,
                       int calculate_checksum, uint64_t startcode)
{
    uint8_t *dyn_buf = NULL;
    int dyn_size     = avio_em_close_dyn_buf(dyn_bc, &dyn_buf);
    int forw_ptr     = dyn_size + 4 * calculate_checksum;

    if (forw_ptr > 4096)
        emio_init_checksum(bc, em_crc04C11DB7_update, 0);
    avio_em_wb64(bc, startcode);
    em_put_v(bc, forw_ptr);
    if (forw_ptr > 4096)
        avio_em_wl32(bc, emio_get_checksum(bc));

    if (calculate_checksum)
        emio_init_checksum(bc, em_crc04C11DB7_update, 0);
    avio_em_write(bc, dyn_buf, dyn_size);
    if (calculate_checksum)
        avio_em_wl32(bc, emio_get_checksum(bc));
...
static void write_mainheader(NUTContext *nut, AVEMIOContext *bc)
{
    int i, j, tmp_pts, tmp_flags, tmp_stream, tmp_mul, tmp_size, tmp_fields,
        tmp_head_idx;
    int64_t tmp_match;

    em_put_v(bc, nut->version);
    if (nut->version > 3)
        em_put_v(bc, nut->minor_version = 1);
    em_put_v(bc, nut->avf->nb_streams);
    em_put_v(bc, nut->max_distance);
    em_put_v(bc, nut->time_base_count);

    for (i = 0; i < nut->time_base_count; i++) {
        em_put_v(bc, nut->time_base[i].num);
        em_put_v(bc, nut->time_base[i].den);
    }
...
//              nut->frame_code[i].res        != tmp_res      ||
        ...
        em_put_v(bc, tmp_flags);
        em_put_v(bc, tmp_fields);
        if (tmp_fields > 0) put_s(bc, tmp_pts);
        if (tmp_fields > 1) em_put_v(bc, tmp_mul);
        if (tmp_fields > 2) em_put_v(bc, tmp_stream);
        if (tmp_fields > 3) em_put_v(bc, tmp_size);
        if (tmp_fields > 4) em_put_v(bc, 0 /*tmp_res*/);
        if (tmp_fields > 5) em_put_v(bc, j);
        if (tmp_fields > 6) em_put_v(bc, tmp_match);
        if (tmp_fields > 7) em_put_v(bc, tmp_head_idx);
    }
    em_put_v(bc, nut->header_count - 1);
    for (i = 1; i < nut->header_count; i++) {
        em_put_v(bc, nut->header_len[i]);
        avio_em_write(bc, nut->header[i], nut->header_len[i]);
    }
    // flags had been effectively introduced in version 4
    if (nut->version > 3)
        em_put_v(bc, nut->flags);
}

static int write_streamheader(AVEMFormatContext *avctx, AVEMIOContext *bc,
                              AVEMStream *st, int i)
{
    ...
    case AVMEDIA_TYPE_VIDEO:    em_put_v(bc, 0); break;
    case AVMEDIA_TYPE_AUDIO:    em_put_v(bc, 1); break;
    case AVMEDIA_TYPE_SUBTITLE: em_put_v(bc, 2); break;
    default:                    em_put_v(bc, 3); break;
    }
    em_put_v(bc, 4);

    if (par->codec_tag) {
        avio_em_wl32(bc, par->codec_tag);
    } else {
        av_em_log(avctx, AV_LOG_ERROR, "No codec tag defined for stream %d\n", i);
        return AVERROR(EINVAL);
    }

    em_put_v(bc, nut->stream[i].time_base - nut->time_base);
    em_put_v(bc, nut->stream[i].msb_pts_shift);
    em_put_v(bc, nut->stream[i].max_pts_distance);
    em_put_v(bc, par->video_delay);
    avio_em_w8(bc, 0); /* flags: 0x1 - fixed_fps, 0x2 - index_present */

    em_put_v(bc, par->extradata_size);
    avio_em_write(bc, par->extradata, par->extradata_size);

    switch (par->codec_type) {
    case AVMEDIA_TYPE_AUDIO:
        em_put_v(bc, par->sample_rate);
        em_put_v(bc, 1);
        em_put_v(bc, par->channels);
        break;
    case AVMEDIA_TYPE_VIDEO:
        em_put_v(bc, par->width);
        em_put_v(bc, par->height);
        ...
        if (st->sample_aspect_ratio.num <= 0 ||
            ...
            em_put_v(bc, st->sample_aspect_ratio.num);
            em_put_v(bc, st->sample_aspect_ratio.den);
        }
        em_put_v(bc, 0); /* csp type -- unknown */
        break;
    default:
        break;
    }
...
static int add_info(AVEMIOContext *bc, const char *type, const char *value)
{
    put_str(bc, type);
    put_s(bc, -1);
    put_str(bc, value);
...
static int write_globalinfo(NUTContext *nut, AVEMIOContext *bc)
{
    ...
    int ret          = avio_em_open_dyn_buf(&dyn_bc);
    if (ret < 0)
        return ret;
    ...
    em_put_v(bc, 0); //stream_if_plus1
    em_put_v(bc, 0); //chapter_id
    em_put_v(bc, 0); //timestamp_start
    em_put_v(bc, 0); //length

    em_put_v(bc, count);

    dyn_size = avio_em_close_dyn_buf(dyn_bc, &dyn_buf);
    avio_em_write(bc, dyn_buf, dyn_size);
    av_em_free(dyn_buf);
    return 0;
}

static int write_streaminfo(NUTContext *nut, AVEMIOContext *bc, int stream_id) {
    AVEMFormatContext *s= nut->avf;
    AVEMStream* st = s->streams[stream_id];
    AVEMDictionaryEntry *t = NULL;
    AVEMIOContext *dyn_bc;
    uint8_t *dyn_buf=NULL;
    ...
    while ((t = av_em_dict_get(st->metadata, "", t, AV_DICT_IGNORE_SUFFIX)))
        count += add_info(dyn_bc, t->key, t->value);
    ...
    if (count) {
        em_put_v(bc, stream_id + 1); //stream_id_plus1
        em_put_v(bc, 0); //chapter_id
        em_put_v(bc, 0); //timestamp_start
        em_put_v(bc, 0); //length
...
static int write_chapter(NUTContext *nut, AVEMIOContext *bc, int id)
{
    AVEMIOContext *dyn_bc;
    uint8_t *dyn_buf     = NULL;
    AVEMDictionaryEntry *t = NULL;
    AVEMChapter *ch        = nut->avf->chapters[id];
    int ret, dyn_size, count = 0;

    ret = avio_em_open_dyn_buf(&dyn_bc);
    if (ret < 0)
        return ret;

    em_put_v(bc, 0);                                        // stream_id_plus1
    put_s(bc, id + 1);                                      // chapter_id
    put_tt(nut, nut->chapter[id].time_base, bc, ch->start); // chapter_start
    em_put_v(bc, ch->end - ch->start);                      // chapter_len

    while ((t = av_em_dict_get(ch->metadata, "", t, AV_DICT_IGNORE_SUFFIX)))
        count += add_info(dyn_bc, t->key, t->value);
...
static int write_index(NUTContext *nut, AVEMIOContext *bc) {
    int i;
    Syncpoint dummy= { .pos= 0 };
    Syncpoint *next_node[2] = { NULL };
    int64_t startpos = avio_em_tell(bc);
    int64_t payload_size;

    put_tt(nut, nut->max_pts_tb, bc, nut->max_pts);

    em_put_v(bc, nut->sp_count);

    for (i=0; i<nut->sp_count; i++) {
        av_em_tree_find(nut->syncpoints, &dummy, ff_nut_sp_pos_cmp, (void**)next_node);
        em_put_v(bc, (next_node[1]->pos >> 4) - (dummy.pos>>4));
    ...
    for (i=0; i<nut->avf->nb_streams; i++) {
        ...
        for (j=0; j<nut->sp_count; j++) {
            ...
            if (j && nus->keyframe_pts[j] == nus->keyframe_pts[j-1]) {
                av_em_log(nut->avf, AV_LOG_WARNING, "Multiple keyframes with same PTS\n");
            ...
            em_put_v(bc, 1 + 2*flag + 4*n);
            for (k= j - n; k<=j && k<nut->sp_count; k++) {
                if (nus->keyframe_pts[k] == AV_NOPTS_VALUE)
                    continue;
                av_assert0(nus->keyframe_pts[k] > last_pts);
                em_put_v(bc, nus->keyframe_pts[k] - last_pts);
                last_pts = nus->keyframe_pts[k];
            }
        }
    }

    payload_size = avio_em_tell(bc) - startpos + 8 + 4;

    avio_em_wb64(bc, 8 + payload_size + av_log2(payload_size) / 7 + 1 + 4*(payload_size > 4096));

    return 0;
}

static int write_headers(AVEMFormatContext *avctx, AVEMIOContext *bc)
{
    NUTContext *nut = avctx->priv_data;
    AVEMIOContext *dyn_bc;
    int i, ret;
...
static int nut_write_header(AVEMFormatContext *s)
{
    NUTContext *nut = s->priv_data;
    ...
    nut->stream   = av_em_calloc(s->nb_streams,  sizeof(*nut->stream ));
    nut->chapter  = av_em_calloc(s->nb_chapters, sizeof(*nut->chapter));
    nut->time_base= av_em_calloc(s->nb_streams +
                              s->nb_chapters, sizeof(*nut->time_base));
    if (!nut->stream || !nut->chapter || !nut->time_base) {
        av_em_freep(&nut->stream);
        av_em_freep(&nut->chapter);
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO && st->codecpar->sample_rate) {
            time_base = (AVEMRational) {1, st->codecpar->sample_rate};
        } else {
            time_base = em_choose_timebase(s, st, 48000);
        }

        avpriv_em_set_pts_info(st, 64, time_base.num, time_base.den);

        for (j = 0; j < nut->time_base_count; j++)
            if (!memcmp(&time_base, &nut->time_base[j], sizeof(AVEMRational))) {
                break;
            }
    ...
    for (i = 0; i < s->nb_chapters; i++) {
        ...
        for (j = 0; j < nut->time_base_count; j++)
            if (!memcmp(&ch->time_base, &nut->time_base[j], sizeof(AVEMRational)))
                break;
    ...
    avio_em_write(bc, ID_STRING, strlen(ID_STRING));
    avio_em_w8(bc, 0);
...
static int find_best_header_idx(NUTContext *nut, AVEMPacket *pkt)
{
    int i;
    int best_i   = 0;
    int best_len = 0;
...
static int write_sm_data(AVEMFormatContext *s, AVEMIOContext *bc, AVEMPacket *pkt, int is_meta)
{
    int ret, i, dyn_size;
    unsigned flags;
    AVEMIOContext *dyn_bc;
    ...
    for (i = 0; i<pkt->side_data_elems; i++) {
        ...
        if (is_meta) {
            ...
            default:
                ...
                put_s(dyn_bc, -2);
                put_str(dyn_bc, "bin");
                em_put_v(dyn_bc, pkt->side_data[i].size);
                avio_em_write(dyn_bc, data, pkt->side_data[i].size);
                sm_data_count++;
                break;
            case AV_PKT_DATA_PARAM_CHANGE:
                ...
                if (flags & AV_SIDE_DATA_PARAM_CHANGE_CHANNEL_LAYOUT) {
                    ...
                    put_str(dyn_bc, "u64");
                    em_put_v(bc, 8);
                    avio_em_write(dyn_bc, data, 8); data+=8;
                    sm_data_count++;
                }
...
static int nut_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    NUTContext *nut    = s->priv_data;
    StreamContext *nus = &nut->stream[pkt->stream_index];
    AVEMIOContext *bc    = s->pb, *dyn_bc, *sm_bc = NULL;
    FrameCode *fc;
    int64_t coded_pts;
    ...
    if (pkt->pts < 0) {
        ...
        if (pkt->pts == AV_NOPTS_VALUE)
            av_em_log(s, AV_LOG_ERROR, "Try to enable the genpts flag\n");
        return AVERROR(EINVAL);
    }

    if (pkt->side_data_elems && nut->version > 3) {
        ret = avio_em_open_dyn_buf(&sm_bc);
        if (ret < 0)
            return ret;
        ret = write_sm_data(s, sm_bc, pkt, 0);
        if (ret >= 0)
            ret = write_sm_data(s, sm_bc, pkt, 1);
        sm_size = avio_em_close_dyn_buf(sm_bc, &sm_buf);
        if (ret < 0)
            goto fail;
        data_size += sm_size;
    }

    if (1LL << (20 + 3 * nut->header_count) <= avio_em_tell(bc))
        write_headers(s, bc);

    if (key_frame && !(nus->last_flags & FLAG_KEY))
        store_sp = 1;

    if (data_size + 30 /*FIXME check*/ + avio_em_tell(bc) >= nut->last_syncpoint_pos + nut->max_distance)
        store_sp = 1;
...
//FIXME: Ensure store_sp is 1 in the first place.
    ...
    if (store_sp &&
        ...
        nut->last_syncpoint_pos = avio_em_tell(bc);
        ret                     = avio_em_open_dyn_buf(&dyn_bc);
        if (ret < 0)
            goto fail;
        put_tt(nut, nus->time_base, dyn_bc, pkt->dts);
        em_put_v(dyn_bc, sp_pos != INT64_MAX ? (nut->last_syncpoint_pos - sp_pos) >> 4 : 0);

        if (nut->flags & NUT_BROADCAST) {
            put_tt(nut, nus->time_base, dyn_bc,
                   av_em_rescale_q(av_em_gettime(), AV_TIME_BASE_Q, *nus->time_base));
        }
        ...
        if ((1ll<<60) % nut->sp_count == 0)
            for (i=0; i<s->nb_streams; i++) {
                int j;
                StreamContext *nus = &nut->stream[i];
                av_em_reallocp_array(&nus->keyframe_pts, 2*nut->sp_count, sizeof(*nus->keyframe_pts));
                if (!nus->keyframe_pts) {
                    ret = AVERROR(ENOMEM);
                    goto fail;
                }
    ...
    for (i = 0; i < 256; i++) {
        ...
        if (flags & FLAG_STREAM_ID)
            length += em_get_v_length(pkt->stream_index);

        if (data_size % fc->size_mul != fc->size_lsb)
            continue;
        if (flags & FLAG_SIZE_MSB)
            length += em_get_v_length(data_size / fc->size_mul);

        if (flags & FLAG_CHECKSUM)
            length += 4;

        if (flags & FLAG_CODED_PTS)
            length += em_get_v_length(coded_pts);
    ...
    emio_init_checksum(bc, em_crc04C11DB7_update, 0);
    avio_em_w8(bc, frame_code);
    if (flags & FLAG_CODED) {
        em_put_v(bc, (flags ^ needed_flags) & ~(FLAG_CODED));
        flags = needed_flags;
    }
    if (flags & FLAG_STREAM_ID)  em_put_v(bc, pkt->stream_index);
    if (flags & FLAG_CODED_PTS)  em_put_v(bc, coded_pts);
    if (flags & FLAG_SIZE_MSB )  em_put_v(bc, data_size / fc->size_mul);
    if (flags & FLAG_HEADER_IDX) em_put_v(bc, header_idx = best_header_idx);

    if (flags & FLAG_CHECKSUM)   avio_em_wl32(bc, emio_get_checksum(bc));
    else                         emio_get_checksum(bc);

    if (flags & FLAG_SM_DATA) {
        avio_em_write(bc, sm_buf, sm_size);
    }
    avio_em_write(bc, pkt->data + nut->header_len[header_idx], pkt->size - nut->header_len[header_idx]);
    ...
    if (!nut->max_pts_tb || av_em_compare_ts(nut->max_pts, *nut->max_pts_tb, pkt->pts, *nus->time_base) < 0) {
        nut->max_pts = pkt->pts;
...
static int nut_write_trailer(AVEMFormatContext *s)
{
    NUTContext *nut = s->priv_data;
    AVEMIOContext *bc = s->pb, *dyn_bc;
    int ret;
...
static void nut_write_deinit(AVEMFormatContext *s)
{
    NUTContext *nut = s->priv_data;
    int i;
    ...
    if (nut->stream)
        for (i=0; i<s->nb_streams; i++)
            av_em_freep(&nut->stream[i].keyframe_pts);
