...
typedef struct {
    ...
    unsigned char buf[8];
    int buf_pos, buf_len;
    AVEMIOContext buf_pb;
} FFTextReader;

/**
 * Initialize the FFTextReader from the given AVEMIOContext. This function will
...
typedef struct {
    AVEMPacket *subs;         ///< array of subtitles packets
 ...
 * matches AVEMInputFormat->read_seek2 prototypes.
 */
...
/**
 * @brief Same as ff_subtitles_read_text_chunk(), but read from an AVEMIOContext.
 */
