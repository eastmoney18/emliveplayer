...
static int roq_probe(AVEMProbeData *p)
{
...
static int roq_read_header(AVEMFormatContext *s)
{
    ...
    /* get the main header */
    if (avio_em_read(pb, preamble, RoQ_CHUNK_PREAMBLE_SIZE) !=
...
static int roq_read_packet(AVEMFormatContext *s,
                           AVEMPacket *pkt)
{
    RoqDemuxContext *roq = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    while (!packet_read) {
        ...
        if ((ret = avio_em_read(pb, preamble, RoQ_CHUNK_PREAMBLE_SIZE)) !=
            RoQ_CHUNK_PREAMBLE_SIZE)
            return AVERROR(EIO);
        ...
        chunk_size = emio_limit(pb, chunk_size);
        ...
        case RoQ_INFO:
            if (roq->video_stream_index == -1) {
                ...
                avpriv_em_set_pts_info(st, 63, 1, roq->frame_rate);
                roq->video_stream_index = st->index;
                st->codecpar->codec_type   = AVMEDIA_TYPE_VIDEO;
                st->codecpar->codec_id     = AV_CODEC_ID_ROQ;
                st->codecpar->codec_tag    = 0;  /* no fourcc */

                if (avio_em_read(pb, preamble, RoQ_CHUNK_PREAMBLE_SIZE) != RoQ_CHUNK_PREAMBLE_SIZE)
                    return AVERROR(EIO);
            ...
            avio_em_skip(pb, RoQ_CHUNK_PREAMBLE_SIZE);
            break;
        ...
        case RoQ_QUAD_CODEBOOK:
            ...
            codebook_offset = avio_em_tell(pb) - RoQ_CHUNK_PREAMBLE_SIZE;
            codebook_size = chunk_size;
            avio_em_skip(pb, codebook_size);
            if (avio_em_read(pb, preamble, RoQ_CHUNK_PREAMBLE_SIZE) !=
                RoQ_CHUNK_PREAMBLE_SIZE)
                return AVERROR(EIO);
            ...
            /* rewind */
            avio_em_seek(pb, codebook_offset, SEEK_SET);
        ...
        case RoQ_SOUND_STEREO:
            if (roq->audio_stream_index == -1) {
                ...
                if (!st)
                    return AVERROR(ENOMEM);
                avpriv_em_set_pts_info(st, 32, 1, RoQ_AUDIO_SAMPLE_RATE);
        ...
        case RoQ_QUAD_VQ:
            ...
            /* load up the packet */
            if (av_em_new_packet(pkt, chunk_size + RoQ_CHUNK_PREAMBLE_SIZE))
                return AVERROR(EIO);
            /* copy over preamble */
            ...
            ret = avio_em_read(pb, pkt->data + RoQ_CHUNK_PREAMBLE_SIZE,
                chunk_size);
            if (ret != chunk_size)
                ret = AVERROR(EIO);
        ...
        default:
            av_em_log(s, AV_LOG_ERROR, "  unknown RoQ chunk (%04X)\n", chunk_type);
            return AVERROR_INVALIDDATA;
        }
    }
...
AVEMInputFormat ff_roq_demuxer = {
    .name           = "roq",
