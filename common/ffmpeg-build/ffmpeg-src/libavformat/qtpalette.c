...
int ff_em_get_qtpalette(int codec_id, AVEMIOContext *pb, uint32_t *palette)
{
    int tmp, bit_depth, color_table_id, greyscale, i;

    avio_em_seek(pb, 82, SEEK_CUR);
    ...
    color_table_id = avio_em_rb16(pb);
    ...
    if ((bit_depth == 1 || bit_depth == 2 || bit_depth == 4 || bit_depth == 8)) {
        ...
        if (greyscale && bit_depth > 1 && color_table_id) {
             ...
             * description */
            color_start = avio_em_rb32(pb);
            avio_em_rb16(pb); /* color table flags */
            color_end = avio_em_rb16(pb);
