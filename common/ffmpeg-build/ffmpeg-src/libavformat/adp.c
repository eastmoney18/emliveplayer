...
static int adp_probe(AVEMProbeData *p)
{
    int i, changes = 0;
    char last = 0;
...
static int adp_read_header(AVEMFormatContext *s)
{
    AVEMStream *st;
    ...
    st->start_time            = 0;
    if (s->pb->seekable)
        st->duration          = av_em_get_audio_frame_duration2(st->codecpar, avio_em_size(s->pb));
...
static int adp_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int ret, size = 1024;
...
AVEMInputFormat ff_adp_demuxer = {
    .name           = "adp",
