...
static const AVEMClass unix_class = {
    .class_name = "unix",
...
static int unix_open(EMURLContext *h, const char *filename, int flags)
{
    UnixContext *s = h->priv_data;
    int fd, ret;

    av_em_strstart(filename, "unix:", &filename);
    s->addr.sun_family = AF_UNIX;
    av_em_strlcpy(s->addr.sun_path, filename, sizeof(s->addr.sun_path));

    if ((fd = em_socket(AF_UNIX, s->type, 0)) < 0)
        return ff_neterrno();
    ...
    if (s->listen) {
        ret = em_listen_bind(fd, (struct sockaddr *)&s->addr,
                             sizeof(s->addr), s->timeout, h);
        if (ret < 0)
            goto fail;
        fd = ret;
    } else {
        ret = em_listen_connect(fd, (struct sockaddr *)&s->addr,
...
static int unix_read(EMURLContext *h, uint8_t *buf, int size)
{
    UnixContext *s = h->priv_data;
    int ret;

    if (!(h->flags & AVIO_FLAG_NONBLOCK)) {
        ret = em_network_wait_fd(s->fd, 0);
        if (ret < 0)
            return ret;
    }
...
static int unix_write(EMURLContext *h, const uint8_t *buf, int size)
{
    UnixContext *s = h->priv_data;
    int ret;

    if (!(h->flags & AVIO_FLAG_NONBLOCK)) {
        ret = em_network_wait_fd(s->fd, 1);
        if (ret < 0)
            return ret;
    }
...
static int unix_close(EMURLContext *h)
{
    UnixContext *s = h->priv_data;
    if (s->listen)
        unlink(s->addr.sun_path);
...
static int unix_get_file_handle(EMURLContext *h)
{
    UnixContext *s = h->priv_data;
    return s->fd;
}

const EMURLProtocol em_unix_protocol = {
    .name                = "unix",
    .url_open            = unix_open,
    .url_read            = unix_read,
