...
static int wsvqa_probe(AVEMProbeData *p)
{
...
static int wsvqa_read_header(AVEMFormatContext *s)
{
    ...
    avio_em_seek(pb, 20, SEEK_SET);

    /* the VQA header needs to go to the decoder */
    if (em_get_extradata(s, st->codecpar, pb, VQA_HEADER_SIZE) < 0)
        return AVERROR(ENOMEM);
    ...
    if (fps < 1 || fps > 30) {
        av_em_log(s, AV_LOG_ERROR, "invalid fps: %d\n", fps);
        return AVERROR_INVALIDDATA;
    }
    avpriv_em_set_pts_info(st, 64, 1, fps);
    ...
    do {
        if (avio_em_read(pb, scratch, VQA_PREAMBLE_SIZE) != VQA_PREAMBLE_SIZE)
            return AVERROR(EIO);
...
static int wsvqa_read_packet(AVEMFormatContext *s,
                             AVEMPacket *pkt)
{
    WsVqaDemuxContext *wsvqa = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    while (avio_em_read(pb, preamble, VQA_PREAMBLE_SIZE) == VQA_PREAMBLE_SIZE) {
        ...
        if ((chunk_type == SND0_TAG) || (chunk_type == SND1_TAG) ||
            ...
            case CMDS_TAG:
                break;
            default:
                av_em_log(s, AV_LOG_INFO, "Skipping unknown chunk 0x%08X\n", chunk_type);
            }
            avio_em_skip(pb, chunk_size + skip_byte);
        }
    }
...
AVEMInputFormat ff_wsvqa_demuxer = {
    .name           = "wsvqa",
