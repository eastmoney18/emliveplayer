...
static int genh_probe(AVEMProbeData *p)
{
...
static int genh_read_header(AVEMFormatContext *s)
{
    ...
    align                  =
    c->interleave_size     = avio_em_rl32(s->pb);
    ...
    case  7: ret = em_alloc_extradata(st->codecpar, 2);
             if (ret < 0)
                 return ret;
    ...
    start_offset = avio_em_rl32(s->pb);
    header_size  = avio_em_rl32(s->pb);
    ...
    coef[0]          = avio_em_rl32(s->pb);
    coef[1]          = avio_em_rl32(s->pb);
    c->dsp_int_type  = avio_em_rl32(s->pb);
    coef_type        = avio_em_rl32(s->pb);
    coef_splitted[0] = avio_em_rl32(s->pb);
    coef_splitted[1] = avio_em_rl32(s->pb);

    if (st->codecpar->codec_id == AV_CODEC_ID_ADPCM_THP) {
        if (st->codecpar->channels > 2) {
            avpriv_em_request_sample(s, "channels %d>2", st->codecpar->channels);
            return AVERROR_PATCHWELCOME;
        }

        em_alloc_extradata(st->codecpar, 32 * st->codecpar->channels);
        for (ch = 0; ch < st->codecpar->channels; ch++) {
            if (coef_type & 1) {
                avpriv_em_request_sample(s, "coef_type & 1");
                return AVERROR_PATCHWELCOME;
            } else {
                avio_em_seek(s->pb, coef[ch], SEEK_SET);
                avio_em_read(s->pb, st->codecpar->extradata + 32 * ch, 32);
            }
        }
    ...
    avio_em_skip(s->pb, start_offset - avio_em_tell(s->pb));
...
static int genh_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if (c->dsp_int_type == 1 && par->codec_id == AV_CODEC_ID_ADPCM_THP &&
        ...
        if (avio_em_feof(s->pb))
            return AVERROR_EOF;
        ret = av_em_new_packet(pkt, 8 * par->channels);
        if (ret < 0)
            return ret;
        for (i = 0; i < 8 / c->interleave_size; i++) {
            for (ch = 0; ch < par->channels; ch++) {
                pkt->data[ch * 8 + i*c->interleave_size+0] = avio_em_r8(s->pb);
                pkt->data[ch * 8 + i*c->interleave_size+1] = avio_em_r8(s->pb);
            }
        }
        ret = 0;
    } else if (par->codec_id == AV_CODEC_ID_SDX2_DPCM) {
        ret = av_em_get_packet(s->pb, pkt, par->block_align * 1024);

    } else {
        ret = av_em_get_packet(s->pb, pkt, par->block_align ? par->block_align : 1024 * par->channels);
    }
...
AVEMInputFormat ff_genh_demuxer = {
    .name           = "genh",
