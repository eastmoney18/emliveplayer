...
static int tls_shutdown_client(EMURLContext *h)
{
    ...
    if (c->connected) {
        ...
        if (sspi_ret != SEC_E_OK)
            av_em_log(h, AV_LOG_ERROR, "ApplyControlToken failed\n");
        ...
        if (sspi_ret == SEC_E_OK || sspi_ret == SEC_I_CONTEXT_EXPIRED) {
            ret = ffurl_em_write(s->tcp, outbuf.pvBuffer, outbuf.cbBuffer);
            FreeContextBuffer(outbuf.pvBuffer);
            if (ret < 0 || ret != outbuf.cbBuffer)
                av_em_log(h, AV_LOG_ERROR, "Failed to send close message\n");
        }
...
static int tls_close(EMURLContext *h)
{
    TLSContext *c = h->priv_data;
    ...
    av_em_freep(&c->enc_buf);
    c->enc_buf_size = c->enc_buf_offset = 0;

    av_em_freep(&c->dec_buf);
    c->dec_buf_size = c->dec_buf_offset = 0;

    if (c->tls_shared.tcp)
        ffurl_em_close(c->tls_shared.tcp);
    return 0;
}

static int tls_client_handshake_loop(EMURLContext *h, int initial)
{
    TLSContext *c = h->priv_data;
    TLSShared *s = &c->tls_shared;
    SECURITY_STATUS sspi_ret;
    ...
    if (c->enc_buf == NULL) {
        c->enc_buf_offset = 0;
        ret = av_em_reallocp(&c->enc_buf, SCHANNEL_INITIAL_BUFFER_SIZE);
        if (ret < 0)
            goto fail;
    ...
    if (c->dec_buf == NULL) {
        c->dec_buf_offset = 0;
        ret = av_em_reallocp(&c->dec_buf, SCHANNEL_INITIAL_BUFFER_SIZE);
        if (ret < 0)
            goto fail;
    ...
    while (1) {
        if (c->enc_buf_size - c->enc_buf_offset < SCHANNEL_FREE_BUFFER_SIZE) {
            ...
            ret = av_em_reallocp(&c->enc_buf, c->enc_buf_size);
            if (ret < 0) {
                c->enc_buf_size = c->enc_buf_offset = 0;
                goto fail;
            }
        ...
        if (read_data) {
            ret = ffurl_em_read(c->tls_shared.tcp, c->enc_buf + c->enc_buf_offset,
                             c->enc_buf_size - c->enc_buf_offset);
            if (ret < 0) {
                av_em_log(h, AV_LOG_ERROR, "Failed to read handshake response\n");
                goto fail;
            }
        ...
        /* input buffers */
        init_sec_buffer(&inbuf[0], SECBUFFER_TOKEN, av_em_alloc(c->enc_buf_offset), c->enc_buf_offset);
        init_sec_buffer(&inbuf[1], SECBUFFER_EMPTY, NULL, 0);
        init_sec_buffer_desc(&inbuf_desc, inbuf, 2);

        if (inbuf[0].pvBuffer == NULL) {
            av_em_log(h, AV_LOG_ERROR, "Failed to allocate input buffer\n");
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        ...
        sspi_ret = InitializeSecurityContext(&c->cred_handle, &c->ctxt_handle, s->host, c->request_flags,
                                             ...
                                             &c->ctxt_timestamp);
        av_em_freep(&inbuf[0].pvBuffer);

        if (sspi_ret == SEC_E_INCOMPLETE_MESSAGE) {
            av_em_log(h, AV_LOG_DEBUG, "Received incomplete handshake, need more data\n");
            read_data = 1;
            continue;
        }
        ...
        if (sspi_ret == SEC_I_INCOMPLETE_CREDENTIALS &&
            ...
            av_em_log(h, AV_LOG_VERBOSE, "Client certificate has been requested, ignoring\n");
            c->request_flags |= ISC_REQ_USE_SUPPLIED_CREDS;
            read_data = 0;
            continue;
        }
        ...
        if (sspi_ret == SEC_I_CONTINUE_NEEDED || sspi_ret == SEC_E_OK) {
            for (i = 0; i < 3; i++) {
                if (outbuf[i].BufferType == SECBUFFER_TOKEN && outbuf[i].cbBuffer > 0) {
                    ret = ffurl_em_write(c->tls_shared.tcp, outbuf[i].pvBuffer, outbuf[i].cbBuffer);
                    if (ret < 0 || ret != outbuf[i].cbBuffer) {
                        av_em_log(h, AV_LOG_VERBOSE, "Failed to send handshake data\n");
                        ret = AVERROR(EIO);
                        goto fail;
                    }
                }
            ...
            if (sspi_ret == SEC_E_WRONG_PRINCIPAL)
                av_em_log(h, AV_LOG_ERROR, "SNI or certificate check failed\n");
            else
                av_em_log(h, AV_LOG_ERROR, "Creating security context failed (0x%lx)\n", sspi_ret);
            ret = AVERROR_UNKNOWN;
            goto fail;
        }
...
static int tls_client_handshake(EMURLContext *h)
{
    TLSContext *c = h->priv_data;
    TLSShared *s = &c->tls_shared;
    SecBuffer outbuf;
    ...
    sspi_ret = InitializeSecurityContext(&c->cred_handle, NULL, s->host, c->request_flags, 0, 0,
                                         ...
                                         &c->ctxt_timestamp);
    if (sspi_ret != SEC_I_CONTINUE_NEEDED) {
        av_em_log(h, AV_LOG_ERROR, "Unable to create initial security context (0x%lx)\n", sspi_ret);
        ret = AVERROR_UNKNOWN;
        goto fail;
    }
    ...
    FreeContextBuffer(outbuf.pvBuffer);
    if (ret < 0 || ret != outbuf.cbBuffer) {
        av_em_log(h, AV_LOG_ERROR, "Failed to send initial handshake data\n");
        ret = AVERROR(EIO);
        goto fail;
    }
...
static int tls_open(EMURLContext *h, const char *uri, int flags, AVEMDictionary **options)
{
    TLSContext *c = h->priv_data;
    TLSShared *s = &c->tls_shared;
    SECURITY_STATUS sspi_ret;
    ...
    if ((ret = em_tls_open_underlying(s, h, uri, options)) < 0)
        goto fail;

    if (s->listen) {
        av_em_log(h, AV_LOG_ERROR, "TLS Listen Sockets with SChannel is not implemented.\n");
        ret = AVERROR(EINVAL);
        goto fail;
    }
    ...
    sspi_ret = AcquireCredentialsHandle(NULL, (TCHAR *)UNISP_NAME, SECPKG_CRED_OUTBOUND,
                                        ...
                                        &c->cred_timestamp);
    if (sspi_ret != SEC_E_OK) {
        av_em_log(h, AV_LOG_ERROR, "Unable to acquire security credentials (0x%lx)\n", sspi_ret);
        ret = AVERROR_UNKNOWN;
        goto fail;
    }
...
static int tls_read(EMURLContext *h, uint8_t *buf, int len)
{
    TLSContext *c = h->priv_data;
    TLSShared *s = &c->tls_shared;
    ...
    if (!c->connection_closed) {
        ...
        ret = ffurl_em_read(s->tcp, c->enc_buf + c->enc_buf_offset,
                         c->enc_buf_size - c->enc_buf_offset);
        if (ret < 0) {
            av_em_log(h, AV_LOG_ERROR, "Unable to read from socket\n");
            return ret;
        } else if (ret == 0)
    ...
    while (c->enc_buf_offset > 0 && sspi_ret == SEC_E_OK && c->dec_buf_offset < len) {
        ...
        if (sspi_ret == SEC_E_OK || sspi_ret == SEC_I_RENEGOTIATE ||
            ...
            if (inbuf[1].BufferType == SECBUFFER_DATA) {
                ...
                if (c->dec_buf_size - c->dec_buf_offset < size || c->dec_buf_size < len)  {
                    ...
                    if (c->dec_buf_size < len)
                        c->dec_buf_size = len;
                    ret = av_em_reallocp(&c->dec_buf, c->dec_buf_size);
                    if (ret < 0) {
                        c->dec_buf_size = c->dec_buf_offset = 0;
                        return ret;
                    }
            ...
            if (sspi_ret == SEC_I_RENEGOTIATE) {
                if (c->enc_buf_offset) {
                    av_em_log(h, AV_LOG_ERROR, "Cannot renegotiate, encrypted data buffer not empty\n");
                    ret = AVERROR_UNKNOWN;
                    goto cleanup;
                }

                av_em_log(h, AV_LOG_VERBOSE, "Re-negotiating security context\n");
                ...
                if (!c->connection_closed) {
                    c->connection_closed = 1;
                    av_em_log(h, AV_LOG_VERBOSE, "Server closed the connection\n");
                }
                ret = 0;
                goto cleanup;
            }
            ...
            av_em_log(h, AV_LOG_ERROR, "Unable to decrypt message\n");
            ret = AVERROR(EIO);
            goto cleanup;
        }
    }
...
static int tls_write(EMURLContext *h, const uint8_t *buf, int len)
{
    TLSContext *c = h->priv_data;
    TLSShared *s = &c->tls_shared;
    SECURITY_STATUS sspi_ret;
    ...
    if (sspi_ret == SEC_E_OK)  {
        ...
        ret = ffurl_em_write(s->tcp, data, len);
        if (ret < 0 || ret != len) {
            ret = AVERROR(EIO);
            av_em_log(h, AV_LOG_ERROR, "Writing encrypted data to socket failed\n");
            goto done;
        }
    } else {
        av_em_log(h, AV_LOG_ERROR, "Encrypting data failed\n");
...
static const AVEMClass tls_class = {
    .class_name = "tls",
...
const EMURLProtocol em_tls_schannel_protocol = {
    .name           = "tls",
    .url_open2      = tls_open,
    .url_read       = tls_read,
    .url_write      = tls_write,
