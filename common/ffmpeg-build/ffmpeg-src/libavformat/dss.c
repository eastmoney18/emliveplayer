...
static int dss_probe(AVEMProbeData *p)
{
...
static int dss_read_metadata_date(AVEMFormatContext *s, unsigned int offset,
                                  const char *key)
{
    ...
    ret = avio_em_read(s->pb, string, DSS_TIME_SIZE);
    ...
    return av_em_dict_set(&s->metadata, key, datetime, 0);
}

static int dss_read_metadata_string(AVEMFormatContext *s, unsigned int offset,
                                    unsigned int size, const char *key)
{
    ...
    value = av_em_mallocz(size + 1);
    if (!value)
        return AVERROR(ENOMEM);

    ret = avio_em_read(s->pb, value, size);
    if (ret < size) {
        ret = ret < 0 ? ret : AVERROR_EOF;
        goto exit;
    }

    ret = av_em_dict_set(&s->metadata, key, value, 0);
...
static int dss_read_header(AVEMFormatContext *s)
{
    ...
    avio_em_seek(pb, DSS_HEAD_OFFSET_ACODEC, SEEK_SET);
    ctx->audio_codec = avio_em_r8(pb);
    ...
    if (ctx->audio_codec == DSS_ACODEC_DSS_SP) {
        ...
        avpriv_em_request_sample(s, "Support for codec %x in DSS",
                              ctx->audio_codec);
    ...
    if (avio_em_seek(pb, DSS_HEADER_SIZE, SEEK_SET) != DSS_HEADER_SIZE)
        return AVERROR(EIO);

    ctx->counter = 0;
    ctx->swap    = 0;

    ctx->dss_sp_buf = av_em_alloc(DSS_FRAME_SIZE + 1);
    if (!ctx->dss_sp_buf)
        return AVERROR(ENOMEM);
...
static void dss_skip_audio_header(AVEMFormatContext *s, AVEMPacket *pkt)
{
    DSSDemuxContext *ctx = s->priv_data;
    AVEMIOContext *pb = s->pb;

    avio_em_skip(pb, DSS_AUDIO_BLOCK_HEADER_SIZE);
...
static int dss_sp_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    DSSDemuxContext *ctx = s->priv_data;
    ...
    ret = av_em_new_packet(pkt, DSS_FRAME_SIZE);
    if (ret < 0)
        return ret;
    ...
    if (ctx->counter < 0) {
        ...
        ret = avio_em_read(s->pb, ctx->dss_sp_buf + offset + buff_offset,
                        size2 - offset);
        if (ret < size2 - offset)
            goto error_eof;
...
static int dss_723_1_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    DSSDemuxContext *ctx = s->priv_data;
    AVEMStream *st = s->streams[0];
    ...
    if (ctx->counter < 0) {
        ...
        ret = avio_em_read(s->pb, pkt->data + offset,
                        size2 - offset);
        if (ret < size2 - offset) {
            av_em_packet_unref(pkt);
            return ret < 0 ? ret : AVERROR_EOF;
    ...
    ret = avio_em_read(s->pb, pkt->data + offset, size - offset);
    if (ret < size - offset) {
        av_em_packet_unref(pkt);
        return ret < 0 ? ret : AVERROR_EOF;
    }
...
static int dss_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    DSSDemuxContext *ctx = s->priv_data;
...
static int dss_read_close(AVEMFormatContext *s)
{
    DSSDemuxContext *ctx = s->priv_data;

    av_em_freep(&ctx->dss_sp_buf);

    return 0;
}

static int dss_read_seek(AVEMFormatContext *s, int stream_index,
                         int64_t timestamp, int flags)
{
    ...
    ret = avio_em_seek(s->pb, seekto, SEEK_SET);
    if (ret < 0)
        return ret;

    avio_em_read(s->pb, header, DSS_AUDIO_BLOCK_HEADER_SIZE);
    ...
    if (offset == DSS_AUDIO_BLOCK_HEADER_SIZE) {
        ctx->counter = 0;
        offset = avio_em_skip(s->pb, -DSS_AUDIO_BLOCK_HEADER_SIZE);
    } else {
        ctx->counter = DSS_BLOCK_SIZE - offset;
        offset = avio_em_skip(s->pb, offset - DSS_AUDIO_BLOCK_HEADER_SIZE);
    }
...
AVEMInputFormat ff_dss_demuxer = {
    .name           = "dss",
