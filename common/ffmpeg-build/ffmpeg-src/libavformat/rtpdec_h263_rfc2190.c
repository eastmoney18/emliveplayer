...
{
    emio_free_dyn_buf(&data->buf);
}

static int h263_handle_packet(AVEMFormatContext *ctx, PayloadContext *data,
                              ...
                              int flags)
{
    ...
    if (data->newformat)
        return em_h263_handle_packet(ctx, data, st, pkt, timestamp, buf, len,
                                     seq, flags);
    ...
    if (len < 4) {
        av_em_log(ctx, AV_LOG_ERROR, "Too short H.263 RTP packet: %d\n", len);
    ...
    if (!(buf[0] & 0xf8)) { /* Reserved bits in RFC 2429/4629 are zero */
        if ((src == 0 || src >= 6) && r) {
            ...
            data->newformat = 1;
            return em_h263_handle_packet(ctx, data, st, pkt, timestamp, buf,
                                         len, seq, flags);
        }
    }
    ...
    if (!data->buf) {
        ...
        if (len > 4 && AV_RB32(buf) >> 10 == 0x20) {
            ret = avio_em_open_dyn_buf(&data->buf);
            if (ret < 0)
                return ret;
    ...
    if (data->endbyte_bits || sbit) {
        if (data->endbyte_bits == sbit) {
            ...
            buf++;
            len--;
            avio_em_w8(data->buf, data->endbyte);
        } else {
            ...
            while (get_bits_left(&gb) >= 8)
                avio_em_w8(data->buf, get_bits(&gb, 8));
    ...
    if (ebit) {
        if (len > 0)
            avio_em_write(data->buf, buf, len - 1);
        data->endbyte_bits = 8 - ebit;
        data->endbyte = buf[len - 1] & (0xff << ebit);
    } else {
        avio_em_write(data->buf, buf, len);
    }
...
RTPDynamicProtocolHandler em_h263_rfc2190_dynamic_handler = {
