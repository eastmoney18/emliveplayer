...
static int write_header(AVEMFormatContext *s)
{
    ...
    if (!cpr->ctx) {
        av_em_log(s, AV_LOG_ERROR, "Failed to create chromaprint context.\n");
    ...
    if (cpr->silence_threshold != -1) {
        ...
        if (!chromaprint_set_option(cpr->ctx, "silence_threshold", cpr->silence_threshold)) {
            av_em_log(s, AV_LOG_ERROR, "Failed to set silence threshold.\n");
            goto fail;
        }
#else
        av_em_log(s, AV_LOG_ERROR, "Setting the silence threshold requires Chromaprint "
    ...
    if (st->codecpar->channels > 2) {
        av_em_log(s, AV_LOG_ERROR, "Only up to 2 channels are supported\n");
        goto fail;
    }

    if (st->codecpar->sample_rate < 1000) {
        av_em_log(s, AV_LOG_ERROR, "Sampling rate must be at least 1000\n");
        goto fail;
    }

    if (!chromaprint_start(cpr->ctx, st->codecpar->sample_rate, st->codecpar->channels)) {
        av_em_log(s, AV_LOG_ERROR, "Failed to start chromaprint\n");
        goto fail;
    }
...
static int write_trailer(AVEMFormatContext *s)
{
    ...
    if (!chromaprint_finish(cpr->ctx)) {
        av_em_log(s, AV_LOG_ERROR, "Failed to generate fingerprint\n");
        goto fail;
    }

    if (!chromaprint_get_raw_fingerprint(cpr->ctx, &fp, &size)) {
        av_em_log(s, AV_LOG_ERROR, "Failed to retrieve fingerprint\n");
        goto fail;
    }

    switch (cpr->fp_format) {
    case FINGERPRINT_RAW:
        avio_em_write(pb, fp, size);
        break;
    ...
    case FINGERPRINT_BASE64:
        if (!chromaprint_encode_fingerprint(fp, size, cpr->algorithm, &enc_fp, &enc_size,
            ...
            av_em_log(s, AV_LOG_ERROR, "Failed to encode fingerprint\n");
            goto fail;
        }
        avio_em_write(pb, enc_fp, enc_size);
        break;
    }
...
static const AVEMClass chromaprint_class = {
...
AVEMOutputFormat ff_chromaprint_muxer = {
    .name              = "chromaprint",
