...
static const AVEMClass bluray_context_class = {
    .class_name     = "bluray",
...
static int check_disc_info(EMURLContext *h)
{
    ...
    if (!disc_info) {
        av_em_log(h, AV_LOG_ERROR, "bd_get_disc_info() failed\n");
        return -1;
    }

    if (!disc_info->bluray_detected) {
        av_em_log(h, AV_LOG_ERROR, "BluRay disc not detected\n");
        return -1;
    }
    ...
    if (disc_info->aacs_detected && !disc_info->aacs_handled) {
        if (!disc_info->libaacs_detected) {
            ...
            av_em_log(h, AV_LOG_ERROR, "Your libaacs can't decrypt this media\n");
        }
        return -1;
    }
    ...
    if (disc_info->bdplus_detected && !disc_info->bdplus_handled) {
        ...
        */
            av_em_log(h, AV_LOG_ERROR, "Unable to decrypt BD+ encrypted media\n");
        /*}*/
        return -1;
    }
...
static int bluray_close(EMURLContext *h)
{
    BlurayContext *bd = h->priv_data;
    if (bd->bd) {
        bd_close(bd->bd);
...
static int bluray_open(EMURLContext *h, const char *path, int flags)
{
    BlurayContext *bd = h->priv_data;
    int num_title_idx;
    const char *diskname = path;

    av_em_strstart(path, BLURAY_PROTO_PREFIX, &diskname);

    bd->bd = bd_open(diskname, NULL);
    if (!bd->bd) {
        av_em_log(h, AV_LOG_ERROR, "bd_open() failed\n");
        return AVERROR(EIO);
    }
    ...
    if (bd->region > 0 && bd->region < 5) {
        av_em_log(h, AV_LOG_INFO, "setting region code to %d (%c)\n", bd->region, 'A' + (bd->region - 1));
    ...
    */
    ...
    av_em_log(h, AV_LOG_INFO, "%d usable playlists:\n", num_title_idx);
    if (num_title_idx < 1) {
        return AVERROR(EIO);
    }
    ...
    if (bd->playlist < 0) {
        ...
        for (i = 0; i < num_title_idx; i++) {
            ...
            av_em_log(h, AV_LOG_INFO, "playlist %05d.mpls (%d:%02d:%02d)\n",
                   info->playlist,
        ...
        av_em_log(h, AV_LOG_INFO, "selected %05d.mpls\n", bd->playlist);
    }

    /* select playlist */
    if (bd_select_playlist(bd->bd, bd->playlist) <= 0) {
        av_em_log(h, AV_LOG_ERROR, "bd_select_playlist(%05d.mpls) failed\n", bd->playlist);
        return AVERROR(EIO);
    }
...
static int bluray_read(EMURLContext *h, unsigned char *buf, int size)
{
    BlurayContext *bd = h->priv_data;
    int len;
...
static int64_t bluray_seek(EMURLContext *h, int64_t pos, int whence)
{
    BlurayContext *bd = h->priv_data;
    ...
    av_em_log(h, AV_LOG_ERROR, "Unsupported whence operation %d\n", whence);
    return AVERROR(EINVAL);
}


const EMURLProtocol em_bluray_protocol = {
    .name            = "bluray",
