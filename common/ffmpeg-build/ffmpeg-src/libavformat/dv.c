...
struct DVDemuxContext {
    ...
    AVEMFormatContext*  fctx;
    AVEMStream*         vst;
    AVEMStream*         ast[4];
    AVEMPacket          audio_pkt[4];
                            ...
                            const AVDVProfile *sys)
{
    ...
    if (ipcm + sys->n_difchan > (quant == 1 ? 2 : 4)) {
        av_em_log(NULL, AV_LOG_ERROR, "too many dv pcm frames\n");
        return AVERROR_INVALIDDATA;
    }
...
static int dv_extract_audio_info(DVDemuxContext *c, const uint8_t *frame)
{
    ...
    if (freq >= FF_ARRAY_ELEMS(dv_audio_frequency)) {
        av_em_log(c->fctx, AV_LOG_ERROR,
               "Unrecognized audio sample rate index (%d)\n", freq);
        return 0;
    }

    if (stype > 3) {
        av_em_log(c->fctx, AV_LOG_ERROR, "stype %d is invalid\n", stype);
        c->ach = 0;
        return 0;
    }
    ...
    for (i = 0; i < ach; i++) {
        if (!c->ast[i]) {
            c->ast[i] = avformat_em_new_stream(c->fctx, NULL);
            if (!c->ast[i])
                break;
            avpriv_em_set_pts_info(c->ast[i], 64, 1, 30000);
            c->ast[i]->codecpar->codec_type = AVMEDIA_TYPE_AUDIO;
            c->ast[i]->codecpar->codec_id   = AV_CODEC_ID_PCM_S16LE;

            av_em_init_packet(&c->audio_pkt[i]);
...
static int dv_extract_video_info(DVDemuxContext *c, const uint8_t *frame)
{
    ...
    avpriv_em_set_pts_info(c->vst, 64, c->sys->time_base.num,
                        c->sys->time_base.den);
    c->vst->avg_frame_rate = av_em_inv_q(c->vst->time_base);
    ...
    par->bit_rate = av_em_rescale_q(c->sys->frame_size,
                                   (AVEMRational) { 8, 1 },
                                   c->sys->time_base);
...
static int dv_extract_timecode(DVDemuxContext* c, const uint8_t* frame, char *tc)
{
    ...
    if (!tc_pack)
        return 0;
    av_em_timecode_make_smpte_tc_string(tc, AV_RB32(tc_pack + 1), prevent_df);
    return 1;
}

/* The following 3 functions constitute our interface to the world */

DVDemuxContext *avpriv_dv_init_demux(AVEMFormatContext *s)
{
    DVDemuxContext *c;

    c = av_em_mallocz(sizeof(DVDemuxContext));
    if (!c)
        return NULL;

    c->vst = avformat_em_new_stream(s, NULL);
    if (!c->vst) {
        av_em_free(c);
        return NULL;
    }
...
int avpriv_dv_get_packet(DVDemuxContext *c, AVEMPacket *pkt)
{
    int size = -1;
    int i;
...
int avpriv_dv_produce_packet(DVDemuxContext *c, AVEMPacket *pkt,
                             uint8_t *buf, int buf_size, int64_t pos)
{
    ...
    if (buf_size < DV_PROFILE_BYTES ||
        !(c->sys = av_em_dv_frame_profile(c->sys, buf, buf_size)) ||
...
static int64_t dv_frame_offset(AVEMFormatContext *s, DVDemuxContext *c,
                               int64_t timestamp, int flags)
{
    // FIXME: sys may be wrong if last dv_read_packet() failed (buffer is junk)
    const int frame_size = c->sys->frame_size;
    int64_t offset;
    int64_t size       = avio_em_size(s->pb) - s->internal->data_offset;
...
void ff_dv_offset_reset(DVDemuxContext *c, int64_t frame_offset)
{
    ...
    if (c->ach) {
        if (c->sys) {
        c->abytes = av_em_rescale_q(c->frames, c->sys->time_base,
                                 (AVEMRational) { 8, c->ast[0]->codecpar->bit_rate });
        } else
            av_em_log(c->fctx, AV_LOG_ERROR, "cannot adjust audio bytes\n");
    }
    c->audio_pkt[0].size = c->audio_pkt[1].size = 0;
    c->audio_pkt[2].size = c->audio_pkt[3].size = 0;
}
...
static int dv_read_timecode(AVEMFormatContext *s) {
    int ret;
    ...
    uint8_t *partial_frame = av_em_mallocz(sizeof(*partial_frame) *
                                        partial_frame_size);
    ...
    ret = avio_em_read(s->pb, partial_frame, partial_frame_size);
    if (ret < 0)
        goto finish;
    ...
    if (ret)
        av_em_dict_set(&s->metadata, "timecode", timecode, 0);
    else
        av_em_log(s, AV_LOG_ERROR, "Detected timecode is invalid\n");

finish:
    av_em_free(partial_frame);
    avio_em_seek(s->pb, pos, SEEK_SET);
    return ret;
}

static int dv_read_header(AVEMFormatContext *s)
{
    ...
    state = avio_em_rb32(s->pb);
    while ((state & 0xffffff7f) != 0x1f07003f) {
        if (avio_em_feof(s->pb)) {
            av_em_log(s, AV_LOG_ERROR, "Cannot find DV header.\n");
            return -1;
        }
        if (state == 0x003f0700 || state == 0xff3f0700)
            marker_pos = avio_em_tell(s->pb);
        if (state == 0xff3f0701 && avio_em_tell(s->pb) - marker_pos == 80) {
            avio_em_seek(s->pb, -163, SEEK_CUR);
            state = avio_em_rb32(s->pb);
            break;
        }
    ...
    if (avio_em_read(s->pb, c->buf + 4, DV_PROFILE_BYTES - 4) != DV_PROFILE_BYTES - 4 ||
        avio_em_seek(s->pb, -DV_PROFILE_BYTES, SEEK_CUR) < 0)
        return AVERROR(EIO);

    c->dv_demux->sys = av_em_dv_frame_profile(c->dv_demux->sys,
                                           c->buf,
                                           DV_PROFILE_BYTES);
    if (!c->dv_demux->sys) {
    ...
    s->bit_rate = av_em_rescale_q(c->dv_demux->sys->frame_size,
                               (AVEMRational) { 8, 1 },
...
static int dv_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int size;
    RawDVContext *c = s->priv_data;
    ...
    if (size < 0) {
        ...
        ret = avio_em_read(s->pb, c->buf, size);
        if (ret < 0) {
            return ret;
        } else if (ret == 0) {
            return AVERROR(EIO);
...
static int dv_read_seek(AVEMFormatContext *s, int stream_index,
                        int64_t timestamp, int flags)
{
    ...
    if (avio_em_seek(s->pb, offset, SEEK_SET) < 0)
        return -1;
...
static int dv_read_close(AVEMFormatContext *s)
{
    RawDVContext *c = s->priv_data;
    av_em_freep(&c->dv_demux);
    return 0;
}

static int dv_probe(AVEMProbeData *p)
{
