...
static int ilbc_parse_fmtp(AVEMFormatContext *s,
                           ...
                           const char *attr, const char *value)
{
    if (!strcmp(attr, "mode")) {
        ...
        case 30:
            ...
            break;
        default:
            av_em_log(s, AV_LOG_ERROR, "Unsupported iLBC mode %d\n", mode);
...
static int ilbc_parse_sdp_line(AVEMFormatContext *s, int st_index,
                               PayloadContext *data, const char *line)
{
    ...
    if (av_em_strstart(line, "fmtp:", &p)) {
        ...
        if (!st->codecpar->block_align) {
            av_em_log(s, AV_LOG_ERROR, "No iLBC mode set\n");
            return AVERROR(EINVAL);
        }
    }
    return 0;
