...
static int vc2hq_handle_sequence_header(PayloadContext *pl_ctx, AVEMStream *st, AVEMPacket *pkt,
                                        const uint8_t *buf, int len)
{
    int res;
    uint32_t size = DIRAC_DATA_UNIT_HEADER_SIZE + len;

    if ((res = av_em_new_packet(pkt, DIRAC_DATA_UNIT_HEADER_SIZE + len)) < 0)
        return res;
...
static int vc2hq_mark_end_of_sequence(PayloadContext *pl_ctx, AVEMStream *st, AVEMPacket *pkt)
{
    int res;
    uint32_t size = 0;

    /* create A/V packet */
    if ((res = av_em_new_packet(pkt, DIRAC_DATA_UNIT_HEADER_SIZE)) < 0)
        return res;
...
static int vc2hq_handle_frame_fragment(AVEMFormatContext *ctx, PayloadContext *pl_ctx, AVEMStream *st,
                                       AVEMPacket *pkt, uint32_t *timestamp, const uint8_t *buf, int len,
                                       int flags)
{
    int res;
    uint32_t pic_nr;
    ...
    if (len < 16) {
        av_em_log(ctx, AV_LOG_ERROR, "Too short RTP/VC2hq packet, got %d bytes\n", len);
    ...
    if (pl_ctx->buf && pl_ctx->frame_nr != pic_nr) {
        av_em_log(ctx, AV_LOG_WARNING, "Dropping buffered RTP/VC2hq packet fragments - non-continuous picture numbers\n");
        emio_free_dyn_buf(&pl_ctx->buf);
    }
    ...
    if (no_slices == 0) {
        ...
        if (!pl_ctx->buf) {
            ...
            res = avio_em_open_dyn_buf(&pl_ctx->buf);
            if (res < 0)
                return res;

            /* reserve memory for frame header */
            res = avio_em_seek(pl_ctx->buf, DIRAC_DATA_UNIT_HEADER_SIZE + DIRAC_PIC_NR_SIZE, SEEK_SET);
            if (res < 0)
                return res;
        ...
        avio_em_write(pl_ctx->buf, buf + 16 /* skip pl header */, frag_len);
        ...
        avio_em_write(pl_ctx->buf, buf + 20 /* skip pl header */, frag_len);
        pl_ctx->frame_size += frag_len;
...
static int vc2hq_handle_packet(AVEMFormatContext *ctx, PayloadContext *pl_ctx,
                               ...
                               int flags)
{
    ...
    if (pl_ctx->buf && pl_ctx->timestamp != *timestamp) {
        av_em_log(ctx, AV_LOG_WARNING, "Dropping buffered RTP/VC2hq packet fragments - non-continuous timestamps\n");
        emio_free_dyn_buf(&pl_ctx->buf);
        pl_ctx->frame_size = 0;
    }
