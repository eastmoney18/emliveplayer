...
typedef struct NSVContext {
    ...
    AVEMPacket ahead[2]; /* [v, a] if .data is !NULL there is something */
    /* cached */
    int64_t duration;
    uint32_t vtag, atag;
...
static const AVEMCodecTag nsv_codec_video_tags[] = {
...
static const AVEMCodecTag nsv_codec_audio_tags[] = {
...
//static int nsv_load_index(AVEMFormatContext *s);
static int nsv_read_chunk(AVEMFormatContext *s, int fill_header);

#define print_tag(str, tag, size)       \
    av_em_log(NULL, AV_LOG_TRACE, "%s: tag=%c%c%c%c\n", \
            str, tag & 0xff,            \
            (tag >> 8) & 0xff,          \
            (tag >> 16) & 0xff,         \
            (tag >> 24) & 0xff);
...
static int nsv_resync(AVEMFormatContext *s)
{
    ...
    av_em_log(s, AV_LOG_TRACE, "%s(), offset = %"PRId64", state = %d\n", __FUNCTION__, avio_em_tell(pb), nsv->state);

    //nsv->state = NSV_UNSYNC;

    for (i = 0; i < NSV_MAX_RESYNC; i++) {
        if (avio_em_feof(pb)) {
            av_em_log(s, AV_LOG_TRACE, "NSV EOF\n");
            nsv->state = NSV_UNSYNC;
            return -1;
        }
        v <<= 8;
        v |= avio_em_r8(pb);
        if (i < 8) {
            av_em_log(s, AV_LOG_TRACE, "NSV resync: [%d] = %02x\n", i, v & 0x0FF);
        }

        if ((v & 0x0000ffff) == 0xefbe) { /* BEEF */
            av_em_log(s, AV_LOG_TRACE, "NSV resynced on BEEF after %d bytes\n", i+1);
            nsv->state = NSV_FOUND_BEEF;
            return 0;
        }
        /* we read as big-endian, thus the MK*BE* */
        if (v == TB_NSVF) { /* NSVf */
            av_em_log(s, AV_LOG_TRACE, "NSV resynced on NSVf after %d bytes\n", i+1);
            nsv->state = NSV_FOUND_NSVF;
            return 0;
        }
        if (v == MKBETAG('N', 'S', 'V', 's')) { /* NSVs */
            av_em_log(s, AV_LOG_TRACE, "NSV resynced on NSVs after %d bytes\n", i+1);
            nsv->state = NSV_FOUND_NSVS;
            return 0;
        }

    }
    av_em_log(s, AV_LOG_TRACE, "NSV sync lost\n");
    return -1;
}

static int nsv_parse_NSVf_header(AVEMFormatContext *s)
{
    NSVContext *nsv = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    //s->file_size = (uint32_t)avio_em_rl32(pb);
    file_size = (uint32_t)avio_em_rl32(pb);
    av_em_log(s, AV_LOG_TRACE, "NSV NSVf chunk_size %u\n", size);
    av_em_log(s, AV_LOG_TRACE, "NSV NSVf file_size %u\n", file_size);

    nsv->duration = duration = avio_em_rl32(pb); /* in ms */
    av_em_log(s, AV_LOG_TRACE, "NSV NSVf duration %"PRId64" ms\n", duration);
    // XXX: store it in AVEMStreams

    strings_size = avio_em_rl32(pb);
    table_entries = avio_em_rl32(pb);
    table_entries_used = avio_em_rl32(pb);
    av_em_log(s, AV_LOG_TRACE, "NSV NSVf info-strings size: %d, table entries: %d, bis %d\n",
            strings_size, table_entries, table_entries_used);
    if (avio_em_feof(pb))
        return -1;

    av_em_log(s, AV_LOG_TRACE, "NSV got header; filepos %"PRId64"\n", avio_em_tell(pb));
    ...
    if (strings_size > 0) {
        ...
        p = strings = av_em_mallocz((size_t)strings_size + 1);
        if (!p)
            return AVERROR(ENOMEM);
        endp = strings + strings_size;
        avio_em_read(pb, strings, strings_size);
        while (p < endp) {
            while (*p == ' ')
                p++; /* strip out spaces */
            if (p >= endp-2)
            ...
            *p++ = '\0';
            ...
            if (!p || p >= endp)
                break;
            *p++ = '\0';
            av_em_log(s, AV_LOG_TRACE, "NSV NSVf INFO: %s='%s'\n", token, value);
            av_em_dict_set(&s->metadata, token, value, 0);
        }
        av_em_free(strings);
    }
    if (avio_em_feof(pb))
    ...
    av_em_log(s, AV_LOG_TRACE, "NSV got infos; filepos %"PRId64"\n", avio_em_tell(pb));
    ...
    if (table_entries_used > 0) {
        ...
        if((unsigned)table_entries_used >= UINT_MAX / sizeof(uint32_t))
            return -1;
        nsv->nsvs_file_offset = av_em_malloc_array((unsigned)table_entries_used, sizeof(uint32_t));
        if (!nsv->nsvs_file_offset)
            return AVERROR(ENOMEM);

        for(i=0;i<table_entries_used;i++)
            nsv->nsvs_file_offset[i] = avio_em_rl32(pb) + size;

        if(table_entries > table_entries_used &&
           avio_em_rl32(pb) == MKTAG('T','O','C','2')) {
            nsv->nsvs_timestamps = av_em_malloc_array((unsigned)table_entries_used, sizeof(uint32_t));
            if (!nsv->nsvs_timestamps)
                return AVERROR(ENOMEM);
            for(i=0;i<table_entries_used;i++) {
                nsv->nsvs_timestamps[i] = avio_em_rl32(pb);
            }
        }
    }

    av_em_log(s, AV_LOG_TRACE, "NSV got index; filepos %"PRId64"\n", avio_em_tell(pb));

    avio_em_seek(pb, nsv->base_offset + size, SEEK_SET); /* required for dumbdriving-271.nsv (2 extra bytes) */
...
static int nsv_parse_NSVs_header(AVEMFormatContext *s)
{
    NSVContext *nsv = s->priv_data;
    AVEMIOContext *pb = s->pb;
    uint32_t vtag, atag;
    ...
    vtag = avio_em_rl32(pb);
    atag = avio_em_rl32(pb);
    vwidth = avio_em_rl16(pb);
    vheight = avio_em_rl16(pb);
    i = avio_em_r8(pb);

    av_em_log(s, AV_LOG_TRACE, "NSV NSVs framerate code %2x\n", i);
    if(i&0x80) { /* odd way of giving native framerates from docs */
        int t=(i & 0x7F)>>2;
        if(t<16) framerate = (AVEMRational){1, t+1};
        else     framerate = (AVEMRational){t-15, 1};
        ...
        framerate= (AVEMRational){i, 1};

    nsv->avsync = avio_em_rl16(pb);
    nsv->framerate = framerate;

    print_tag("NSV NSVs vtag", vtag, 0);
    print_tag("NSV NSVs atag", atag, 0);
    av_em_log(s, AV_LOG_TRACE, "NSV NSVs vsize %dx%d\n", vwidth, vheight);
    ...
    if (s->nb_streams == 0) { /* streams not yet published, let's do that */
        ...
        if (vtag != T_NONE) {
            ...
            st->id = NSV_ST_VIDEO;
            nst = av_em_mallocz(sizeof(NSVStream));
            if (!nst)
                goto fail;
            st->priv_data = nst;
            st->codecpar->codec_type = AVMEDIA_TYPE_VIDEO;
            st->codecpar->codec_tag = vtag;
            st->codecpar->codec_id = em_codec_get_id(nsv_codec_video_tags, vtag);
            st->codecpar->width = vwidth;
            st->codecpar->height = vheight;
            st->codecpar->bits_per_coded_sample = 24; /* depth XXX */

            avpriv_em_set_pts_info(st, 64, framerate.den, framerate.num);
            st->start_time = 0;
            st->duration = av_em_rescale(nsv->duration, framerate.num, 1000*framerate.den);

            for(i=0;i<nsv->index_entries;i++) {
                if(nsv->nsvs_timestamps) {
                    av_em_add_index_entry(st, nsv->nsvs_file_offset[i], nsv->nsvs_timestamps[i],
                                       0, 0, AVINDEX_KEYFRAME);
                } else {
                    int64_t ts = av_em_rescale(i*nsv->duration/nsv->index_entries, framerate.num, 1000*framerate.den);
                    av_em_add_index_entry(st, nsv->nsvs_file_offset[i], ts, 0, 0, AVINDEX_KEYFRAME);
                }
            }
        }
        if (atag != T_NONE) {
            ...
            st->codecpar->codec_id = em_codec_get_id(nsv_codec_audio_tags, atag);
            ...
            avpriv_em_set_pts_info(st, 64, 1, framerate.num*1000);
            st->start_time = 0;
            st->duration = (int64_t)nsv->duration * framerate.num;
        }
    } else {
        if (nsv->vtag != vtag || nsv->atag != atag || nsv->vwidth != vwidth || nsv->vheight != vwidth) {
            av_em_log(s, AV_LOG_TRACE, "NSV NSVs header values differ from the first one!!!\n");
            //return -1;
        }
    }
...
static int nsv_read_header(AVEMFormatContext *s)
{
    NSVContext *nsv = s->priv_data;
    int i, err;

    av_em_log(s, AV_LOG_TRACE, "%s()\n", __FUNCTION__);
    av_em_log(s, AV_LOG_TRACE, "filename '%s'\n", s->filename);
    ...
    av_em_log(s, AV_LOG_TRACE, "parsed header\n");
    return err;
}
...
null_chunk_retry:
    ...
    auxcount = avio_em_r8(pb);
    vsize = avio_em_rl16(pb);
    asize = avio_em_rl16(pb);
    vsize = (vsize << 4) | (auxcount >> 4);
    auxcount &= 0x0f;
    av_em_log(s, AV_LOG_TRACE, "NSV CHUNK %d aux, %u bytes video, %d bytes audio\n", auxcount, vsize, asize);
    /* skip aux stuff */
    for (i = 0; i < auxcount; i++) {
        uint32_t av_unused auxtag;
        auxsize = avio_em_rl16(pb);
        auxtag = avio_em_rl32(pb);
        av_em_log(s, AV_LOG_TRACE, "NSV aux data: '%c%c%c%c', %d bytes\n",
              (auxtag & 0x0ff),
              ((auxtag >> 8) & 0x0ff),
              ((auxtag >> 16) & 0x0ff),
              ((auxtag >> 24) & 0x0ff),
              auxsize);
        avio_em_skip(pb, auxsize);
    ...
    if (vsize && st[NSV_ST_VIDEO]) {
        ...
        if ((ret = av_em_get_packet(pb, pkt, vsize)) < 0)
            return ret;
        pkt->stream_index = st[NSV_ST_VIDEO]->index;//NSV_ST_VIDEO;
        pkt->dts = nst->frame_offset;
        pkt->flags |= nsv->state == NSV_HAS_READ_NSVS ? AV_PKT_FLAG_KEY : 0; /* keyframe only likely on a sync frame */
        for (i = 0; i < FFMIN(8, vsize); i++)
            av_em_log(s, AV_LOG_TRACE, "NSV video: [%d] = %02x\n", i, pkt->data[i]);
    }
    if(st[NSV_ST_VIDEO])
    ...
    if (asize && st[NSV_ST_AUDIO]) {
        ...
        if (asize && st[NSV_ST_AUDIO]->codecpar->codec_tag == MKTAG('P', 'C', 'M', ' ')/* && fill_header*/) {
            ...
            samplerate = avio_em_rl16(pb);
            if (!channels || !samplerate)
                return AVERROR_INVALIDDATA;
            asize-=4;
            av_em_log(s, AV_LOG_TRACE, "NSV RAWAUDIO: bps %d, nchan %d, srate %d\n", bps, channels, samplerate);
            if (fill_header) {
                st[NSV_ST_AUDIO]->need_parsing = AVSTREAM_PARSE_NONE; /* we know everything */
                if (bps != 16) {
                    av_em_log(s, AV_LOG_TRACE, "NSV AUDIO bit/sample != 16 (%d)!!!\n", bps);
                }
                bps /= channels; // ???
                if (bps == 8)
        ...
        if ((ret = av_em_get_packet(pb, pkt, asize)) < 0)
            return ret;
        ...
        if( nsv->state == NSV_HAS_READ_NSVS && st[NSV_ST_VIDEO] ) {
            ...
            av_em_log(s, AV_LOG_TRACE, "NSV AUDIO: sync:%d, dts:%"PRId64, nsv->avsync, pkt->dts);
        }
        nst->frame_offset++;
    }
...
static int nsv_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    NSVContext *nsv = s->priv_data;
    int i, err = 0;
    ...
    for (i = 0; i < 2; i++) {
        if (nsv->ahead[i].data) {
            av_em_log(s, AV_LOG_TRACE, "%s: using cached packet[%d]\n", __FUNCTION__, i);
            /* avoid the cost of new_packet + memcpy(->data) */
            memcpy(pkt, &nsv->ahead[i], sizeof(AVEMPacket));
...
static int nsv_read_seek(AVEMFormatContext *s, int stream_index, int64_t timestamp, int flags)
{
    NSVContext *nsv = s->priv_data;
    ...
    if (avio_em_seek(s->pb, st->index_entries[index].pos, SEEK_SET) < 0)
        return -1;
...
static int nsv_read_close(AVEMFormatContext *s)
{
    NSVContext *nsv = s->priv_data;

    av_em_freep(&nsv->nsvs_file_offset);
    av_em_freep(&nsv->nsvs_timestamps);
    if (nsv->ahead[0].data)
        av_em_packet_unref(&nsv->ahead[0]);
    if (nsv->ahead[1].data)
        av_em_packet_unref(&nsv->ahead[1]);
    return 0;
}

static int nsv_probe(AVEMProbeData *p)
{
    int i, score = 0;
    ...
    if (av_em_match_ext(p->filename, "nsv"))
        return AVPROBE_SCORE_EXTENSION;
    /* FIXME: add mime-type check */
    return score;
}

AVEMInputFormat ff_nsv_demuxer = {
    .name           = "nsv",
