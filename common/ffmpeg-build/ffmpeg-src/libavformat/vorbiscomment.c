...
int64_t ff_vorbiscomment_length(AVEMDictionary *m, const char *vendor_string)
{
    int64_t len = 8;
...
int ff_vorbiscomment_write(uint8_t **p, AVEMDictionary **m,
                           const char *vendor_string)
{
    bytestream_put_le32(p, strlen(vendor_string));
    bytestream_put_buffer(p, vendor_string, strlen(vendor_string));
    if (*m) {
        int count = av_em_dict_count(*m);
        AVEMDictionaryEntry *tag = NULL;
