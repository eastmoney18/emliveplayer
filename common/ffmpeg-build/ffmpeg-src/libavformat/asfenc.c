...
static const AVEMCodecTag codec_asf_bmp_tags[] = {
...
static void put_str16(AVEMIOContext *s, const char *tag)
{
    int len;
    uint8_t *pb;
    AVEMIOContext *dyn_buf;
    if (avio_em_open_dyn_buf(&dyn_buf) < 0)
        return;

    avio_em_put_str16le(dyn_buf, tag);
    len = avio_em_close_dyn_buf(dyn_buf, &pb);
    avio_em_wl16(s, len);
    avio_em_write(s, pb, len);
    av_em_freep(&pb);
}

static int64_t put_header(AVEMIOContext *pb, const ff_asf_guid *g)
{
    int64_t pos;
...
/* update header size */
static void end_header(AVEMIOContext *pb, int64_t pos)
{
    int64_t pos1;

    pos1 = avio_em_tell(pb);
    avio_em_seek(pb, pos + 16, SEEK_SET);
    avio_em_wl64(pb, pos1 - pos);
    avio_em_seek(pb, pos1, SEEK_SET);
}

/* write an asf chunk (only used in streaming case) */
static void put_chunk(AVEMFormatContext *s, int type,
                      int payload_length, int flags)
{
    ...
    avio_em_wl16(pb, type);
    avio_em_wl16(pb, length);      // size
    avio_em_wl32(pb, asf->seqno);  // sequence number
    avio_em_wl16(pb, flags);       // unknown bytes
    avio_em_wl16(pb, length);      // size_confirm
    asf->seqno++;
}
...
static int asf_write_markers(AVEMFormatContext *s)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int i;
    AVEMRational scale = {1, 10000000};
    int64_t hpos = put_header(pb, &ff_asf_marker_header);

    ff_put_guid(pb, &ff_asf_reserved_4);// ASF spec mandates this reserved value
    avio_em_wl32(pb, s->nb_chapters);     // markers count
    avio_em_wl16(pb, 0);                  // ASF spec mandates 0 for this
    avio_em_wl16(pb, 0);                  // name length 0, no name given

    for (i = 0; i < s->nb_chapters; i++) {
        AVEMChapter *c = s->chapters[i];
        AVEMDictionaryEntry *t = av_em_dict_get(c->metadata, "title", NULL, 0);
        int64_t pres_time = av_em_rescale_q(c->start, c->time_base, scale);
        uint64_t offset;
        ...
        if (t) {
            ...
            avio_em_put_str16le(dyn_buf, t->value);
            len = avio_em_close_dyn_buf(dyn_buf, &buf);
        }
        avio_em_wl64(pb, offset);            // offset of the packet with send_time
        avio_em_wl64(pb, pres_time + PREROLL_TIME * 10000); // presentation time
        avio_em_wl16(pb, 12 + len);          // entry length
        avio_em_wl32(pb, send_time);         // send time
        avio_em_wl32(pb, 0);                 // flags, should be 0
        avio_em_wl32(pb, len / 2);           // marker desc length in WCHARS!
        if (t) {
            avio_em_write(pb, buf, len);     // marker desc
            av_em_freep(&buf);
        }
    }
    end_header(pb, hpos);
...
static int asf_write_header1(AVEMFormatContext *s, int64_t file_size,
                             int64_t data_chunk_size)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    AVEMDictionaryEntry *tags[5];
    ...
    ff_em_metadata_conv(&s->metadata, ff_asf_metadata_conv, NULL);

    tags[0] = av_em_dict_get(s->metadata, "title", NULL, 0);
    tags[1] = av_em_dict_get(s->metadata, "author", NULL, 0);
    tags[2] = av_em_dict_get(s->metadata, "copyright", NULL, 0);
    tags[3] = av_em_dict_get(s->metadata, "comment", NULL, 0);
    tags[4] = av_em_dict_get(s->metadata, "rating", NULL, 0);

    duration       = asf->duration + PREROLL_TIME * 10000;
    has_title      = tags[0] || tags[1] || tags[2] || tags[3] || tags[4];

    if (!file_size) {
        if (em_parse_creation_time_metadata(s, &asf->creation_time, 0) != 0)
            av_em_dict_set(&s->metadata, "creation_time", NULL, 0);
    }

    metadata_count = av_em_dict_count(s->metadata);
    ...
    for (n = 0; n < s->nb_streams; n++) {
        ...
        avpriv_em_set_pts_info(s->streams[n], 32, 1, 1000); /* 32 bit pts in ms */
        ...
        entry = av_em_dict_get(s->streams[n]->metadata, "language", NULL, 0);
        if (entry) {
    ...
    avio_em_wl64(pb, -1); /* header length, will be patched after */
    avio_em_wl32(pb, 3 + has_title + !!metadata_count + s->nb_streams); /* number of chunks in header */
    avio_em_w8(pb, 1); /* ??? */
    avio_em_w8(pb, 2); /* ??? */

    /* file header */
    header_offset = avio_em_tell(pb);
    hpos          = put_header(pb, &ff_asf_file_header);
    ff_put_guid(pb, &ff_asf_my_guid);
    avio_em_wl64(pb, file_size);
    avio_em_wl64(pb, unix_to_file_time(asf->creation_time));
    avio_em_wl64(pb, asf->nb_packets); /* number of packets */
    avio_em_wl64(pb, duration); /* end time stamp (in 100ns units) */
    avio_em_wl64(pb, asf->duration); /* duration (in 100ns units) */
    avio_em_wl64(pb, PREROLL_TIME); /* start time stamp */
    avio_em_wl32(pb, (asf->is_streamed || !pb->seekable) ? 3 : 2);  /* ??? */
    avio_em_wl32(pb, s->packet_size); /* packet size */
    avio_em_wl32(pb, s->packet_size); /* packet size */
    avio_em_wl32(pb, bit_rate ? bit_rate : -1); /* Maximum data rate in bps */
    end_header(pb, hpos);
    ...
    avio_em_wl16(pb, 6);
    avio_em_wl32(pb, 0); /* length, to be filled later */
    if (asf->nb_languages) {
        int64_t hpos2;
        int i;
        ...
        avio_em_wl16(pb, asf->nb_languages);
        for (i = 0; i < asf->nb_languages; i++) {
            avio_em_w8(pb, 6);
            avio_em_put_str16le(pb, asf->languages[i]);
        }
        end_header(pb, hpos2);
        ...
        if (nb_audio_languages > 1) {
            ...
            avio_em_wl16(pb, nb_audio_languages);
            for (i = 0; i < asf->nb_languages; i++) {
                if (audio_language_counts[i]) {
                    avio_em_wl16(pb, audio_language_counts[i]);
                    for (n = 0; n < s->nb_streams; n++)
        ...
        for (n = 0; n < s->nb_streams; n++) {
            ...
            avio_em_wl64(pb, 0); /* start time */
            avio_em_wl64(pb, 0); /* end time */
            avio_em_wl32(pb, s->streams[n]->codecpar->bit_rate); /* data bitrate bps */
            avio_em_wl32(pb, 5000); /* buffer size ms */
            avio_em_wl32(pb, 0); /* initial buffer fullness */
            avio_em_wl32(pb, s->streams[n]->codecpar->bit_rate); /* peak data bitrate */
            avio_em_wl32(pb, 5000); /* maximum buffer size ms */
            avio_em_wl32(pb, 0); /* max initial buffer fullness */
            avio_em_wl32(pb, 0); /* max object size */
            avio_em_wl32(pb, (!asf->is_streamed && pb->seekable) << 1); /* flags - seekable */
            avio_em_wl16(pb, n + 1); /* stream number */
            avio_em_wl16(pb, asf->streams[n].stream_language_index); /* language id index */
            avio_em_wl64(pb, 0); /* avg time per frame */
            avio_em_wl16(pb, 0); /* stream name count */
            avio_em_wl16(pb, 0); /* payload extension system count */
    ...
    if (has_aspect_ratio) {
        ...
        avio_em_wl16(pb, 2 * has_aspect_ratio);
        for (n = 0; n < s->nb_streams; n++) {
            par = s->streams[n]->codecpar;
            if (   par->codec_type == AVMEDIA_TYPE_VIDEO
                && par->sample_aspect_ratio.num > 0
                && par->sample_aspect_ratio.den > 0) {
                AVEMRational sar = par->sample_aspect_ratio;
                avio_em_wl16(pb, 0);
                // the stream number is set like this below
                avio_em_wl16(pb, n + 1);
                avio_em_wl16(pb, 26); // name_len
                avio_em_wl16(pb,  3); // value_type
                avio_em_wl32(pb,  4); // value_len
                avio_em_put_str16le(pb, "AspectRatioX");
                avio_em_wl32(pb, sar.num);
                avio_em_wl16(pb, 0);
                // the stream number is set like this below
                avio_em_wl16(pb, n + 1);
                avio_em_wl16(pb, 26); // name_len
                avio_em_wl16(pb,  3); // value_type
                avio_em_wl32(pb,  4); // value_len
                avio_em_put_str16le(pb, "AspectRatioY");
                avio_em_wl32(pb, sar.den);
            }
        }
        end_header(pb, hpos2);
    }
    {
        int64_t pos1;
        pos1 = avio_em_tell(pb);
        avio_em_seek(pb, hpos + 42, SEEK_SET);
        avio_em_wl32(pb, pos1 - hpos - 46);
        avio_em_seek(pb, pos1, SEEK_SET);
    }
    end_header(pb, hpos);
    ...
    if (has_title) {
        ...
        for (n = 0; n < FF_ARRAY_ELEMS(tags); n++) {
            len = tags[n] ? avio_em_put_str16le(dyn_buf, tags[n]->value) : 0;
            avio_em_wl16(pb, len);
        }
        len = avio_em_close_dyn_buf(dyn_buf, &buf);
    ...
    if (metadata_count) {
        ...
        avio_em_wl16(pb, metadata_count);
        while ((tag = av_em_dict_get(s->metadata, "", tag, AV_DICT_IGNORE_SUFFIX))) {
    ...
    for (n = 0; n < s->nb_streams; n++) {
        ...
        avio_em_wl64(pb, 0); /* ??? */
        es_pos = avio_em_tell(pb);
        avio_em_wl32(pb, extra_size); /* wav header len */
        avio_em_wl32(pb, extra_size2); /* additional data len */
        avio_em_wl16(pb, n + 1); /* stream number */
        avio_em_wl32(pb, 0); /* ??? */
        ...
        if (par->codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            if (wavsize != extra_size) {
                cur_pos = avio_em_tell(pb);
                avio_em_seek(pb, es_pos, SEEK_SET);
                avio_em_wl32(pb, wavsize); /* wav header len */
                avio_em_seek(pb, cur_pos, SEEK_SET);
            }
            /* ERROR Correction */
            avio_em_w8(pb, 0x01);
            if (par->codec_id == AV_CODEC_ID_ADPCM_G726 || !par->block_align) {
                avio_em_wl16(pb, 0x0190);
                avio_em_wl16(pb, 0x0190);
            } else {
                avio_em_wl16(pb, par->block_align);
                avio_em_wl16(pb, par->block_align);
            }
            avio_em_wl16(pb, 0x01);
            avio_em_w8(pb, 0x00);
        } else {
            avio_em_wl32(pb, par->width);
            avio_em_wl32(pb, par->height);
            avio_em_w8(pb, 2); /* ??? */
            avio_em_wl16(pb, 40 + par->extradata_size); /* size */

            /* BITMAPINFOHEADER header */
            ff_put_bmp_header(pb, par, em_codec_bmp_tags, 1, 0);
        }
        end_header(pb, hpos);
    }
    ...
    avio_em_wl32(pb, s->nb_streams);
    for (n = 0; n < s->nb_streams; n++) {
        const AVEMCodecDescriptor *codec_desc;
        const char *desc;

        par  = s->streams[n]->codecpar;
        codec_desc = avcodec_em_descriptor_get(par->codec_id);
        ...
        if (desc) {
            ...
            avio_em_put_str16le(dyn_buf, desc);
            len = avio_em_close_dyn_buf(dyn_buf, &buf);
            avio_em_wl16(pb, len / 2); // "number of characters" = length in bytes / 2
        ...
        avio_em_wl16(pb, 0); /* no parameters */

        /* id */
        if (par->codec_type == AVMEDIA_TYPE_AUDIO) {
            avio_em_wl16(pb, 2);
            avio_em_wl16(pb, par->codec_tag);
        } else {
            avio_em_wl16(pb, 4);
            avio_em_wl32(pb, par->codec_tag);
        }
        if (!par->codec_tag)
            return -1;
    }
    ...
    if (asf->is_streamed) {
        ...
        avio_em_seek(pb, header_offset - 10 - 30, SEEK_SET);
        avio_em_wl16(pb, header_size);
        avio_em_seek(pb, header_offset - 2 - 30, SEEK_SET);
        avio_em_wl16(pb, header_size);

        header_size -= 8 + 30 + DATA_HEADER_SIZE;
    }
    header_size += 24 + 6;
    avio_em_seek(pb, header_offset - 14, SEEK_SET);
    avio_em_wl64(pb, header_size);
    avio_em_seek(pb, cur_pos, SEEK_SET);

    /* movie chunk, followed by packets of packet_size */
    asf->data_offset = cur_pos;
    ff_put_guid(pb, &ff_asf_data_header);
    avio_em_wl64(pb, data_chunk_size);
    ff_put_guid(pb, &ff_asf_my_guid);
    avio_em_wl64(pb, asf->nb_packets); /* nb packets */
    avio_em_w8(pb, 1); /* ??? */
    avio_em_w8(pb, 1); /* ??? */
    return 0;
}

static int asf_write_header(AVEMFormatContext *s)
{
    ASFContext *asf = s->priv_data;
    ...
    if (s->nb_streams > 127) {
        av_em_log(s, AV_LOG_ERROR, "ASF can only handle 127 streams\n");
        return AVERROR(EINVAL);
    }

    asf->index_ptr             = av_em_alloc(sizeof(ASFIndex) * ASF_INDEX_BLOCK);
    if (!asf->index_ptr)
        return AVERROR(ENOMEM);
    ...
    emio_init_context(&asf->pb, asf->packet_buf, s->packet_size, 1,
                      NULL, NULL, NULL, NULL);
...
static int asf_write_stream_header(AVEMFormatContext *s)
{
    ASFContext *asf = s->priv_data;
...
static int put_payload_parsing_info(AVEMFormatContext *s,
                                    ...
                                    int nb_payloads, int padsize)
{
    ...
    avio_em_w8(pb, ASF_PACKET_ERROR_CORRECTION_FLAGS);
    ...
    avio_em_w8(pb, iLengthTypeFlags);

    avio_em_w8(pb, ASF_PPI_PROPERTY_FLAGS);

    if (iLengthTypeFlags & ASF_PPI_FLAG_PADDING_LENGTH_FIELD_IS_WORD)
        avio_em_wl16(pb, padsize - 2);
    if (iLengthTypeFlags & ASF_PPI_FLAG_PADDING_LENGTH_FIELD_IS_BYTE)
        avio_em_w8(pb, padsize - 1);

    avio_em_wl32(pb, sendtime);
    avio_em_wl16(pb, duration);
    if (asf->multi_payloads_present)
        avio_em_w8(pb, nb_payloads | ASF_PAYLOAD_FLAGS);

    ppi_size = avio_em_tell(pb) - start;

    return ppi_size;
}

static void flush_packet(AVEMFormatContext *s)
{
    ASFContext *asf = s->priv_data;
    ...
    avio_em_write(s->pb, asf->packet_buf, s->packet_size - packet_hdr_size);
...
static void put_payload_header(AVEMFormatContext *s, ASFStream *stream,
                               int64_t presentation_time, int m_obj_size,
                               int m_obj_offset, int payload_len, int flags)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = &asf->pb;
    int val;
    ...
    avio_em_w8(pb, stream->seq);     // Media object number
    avio_em_wl32(pb, m_obj_offset);  // Offset Into Media Object
    ...
    avio_em_w8(pb, ASF_PAYLOAD_REPLICATED_DATA_LENGTH);

    avio_em_wl32(pb, m_obj_size);        // Replicated Data - Media Object Size
    avio_em_wl32(pb, (uint32_t) presentation_time); // Replicated Data - Presentation Time

    if (asf->multi_payloads_present) {
        avio_em_wl16(pb, payload_len);   // payload length
    }
}

static void put_frame(AVEMFormatContext *s, ASFStream *stream, AVEMStream *avst,
                      ...
                      int m_obj_size, int flags)
{
    ...
    while (m_obj_offset < m_obj_size) {
        ...
        if (frag_len1 > 0) {
            ...
            avio_em_write(&asf->pb, buf, payload_len);
...
static int update_index(AVEMFormatContext *s, int start_sec,
                         ...
                         uint64_t packet_offset)
{
    ...
    if (start_sec > asf->next_start_sec) {
        ...
        if (start_sec > asf->nb_index_memory_alloc) {
            ...
            if ((err = av_em_reallocp_array(&asf->index_ptr,
...
static int asf_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ASFStream *stream;
    ...
    int flags = pkt->flags;
    int ret;
    uint64_t offset = avio_em_tell(pb);
    ...
    if (   pts < - PREROLL_TIME
        ...
        av_em_log(s, AV_LOG_ERROR, "input pts %"PRId64" is invalid\n", pts);
        return AVERROR(EINVAL);
    }
    pts *= 10000;
...
static int asf_write_index(AVEMFormatContext *s, const ASFIndex *index,
                           uint16_t max, uint32_t count)
{
    ...
    avio_em_wl64(pb, 24 + 16 + 8 + 4 + 4 + (4 + 2) * count);
    ff_put_guid(pb, &ff_asf_my_guid);
    avio_em_wl64(pb, ASF_INDEXED_INTERVAL);
    avio_em_wl32(pb, max);
    avio_em_wl32(pb, count);
    for (i = 0; i < count; i++) {
        avio_em_wl32(pb, index[i].packet_number);
        avio_em_wl16(pb, index[i].packet_count);
    }

    return 0;
}

static int asf_write_trailer(AVEMFormatContext *s)
{
    ASFContext *asf = s->priv_data;
...
#if CONFIG_ASF_MUXER
static const AVEMClass asf_muxer_class = {
...
AVEMOutputFormat ff_asf_muxer = {
    ...
    .codec_tag      = (const AVEMCodecTag * const []) {
        codec_asf_bmp_tags, em_codec_bmp_tags, em_codec_wav_tags, 0
    },
...
static const AVEMClass asf_stream_muxer_class = {
...
AVEMOutputFormat ff_asf_stream_muxer = {
    .name           = "asf_stream",
    .long_name      = NULL_IF_CONFIG_SMALL("ASF (Advanced / Active Streaming Format)"),
    .mime_type      = "video/x-ms-asf",
    .extensions     = "asf,wmv,wma",
