...
static int realtext_probe(AVEMProbeData *p)
{
    char buf[7];
    FFTextReader tr;
    ...
    return !av_em_strncasecmp(buf, "<window", 7) ? AVPROBE_SCORE_EXTENSION : 0;
}
...
static int realtext_read_header(AVEMFormatContext *s)
{
    ...
    while (!ff_text_eof(&tr)) {
        ...
        if (!av_em_strncasecmp(buf.str, "<window", 7)) {
            /* save header to extradata */
            const char *p = ff_smil_get_attr_ptr(buf.str, "duration");

            if (p)
                duration = read_ts(p);
            st->codecpar->extradata = av_em_strdup(buf.str);
            ...
            int merge = !av_em_strncasecmp(buf.str, "<time", 5) ? 0 : 1;
...
static int realtext_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    RealTextContext *rt = s->priv_data;
    return ff_subtitles_queue_read_packet(&rt->q, pkt);
}

static int realtext_read_seek(AVEMFormatContext *s, int stream_index,
...
static int realtext_read_close(AVEMFormatContext *s)
{
    RealTextContext *rt = s->priv_data;
...
AVEMInputFormat ff_realtext_demuxer = {
    .name           = "realtext",
