...
static void put_ebml_id(AVEMIOContext *pb, unsigned int id)
{
    int i = ebml_id_size(id);
    while (i--)
        avio_em_w8(pb, (uint8_t)(id >> (i * 8)));
}
 ...
 */
static void put_ebml_size_unknown(AVEMIOContext *pb, int bytes)
{
    av_assert0(bytes <= 8);
    avio_em_w8(pb, 0x1ff >> bytes);
    emio_fill(pb, 0xff, bytes - 1);
}
 ...
 */
static void put_ebml_num(AVEMIOContext *pb, uint64_t num, int bytes)
{
    ...
    num |= 1ULL << bytes * 7;
    for (i = bytes - 1; i >= 0; i--)
        avio_em_w8(pb, (uint8_t)(num >> i * 8));
}

static void put_ebml_uint(AVEMIOContext *pb, unsigned int elementid, uint64_t val)
{
    int i, bytes = 1;
    uint64_t tmp = val;
    while (tmp >>= 8)
    ...
    for (i = bytes - 1; i >= 0; i--)
        avio_em_w8(pb, (uint8_t)(val >> i * 8));
}

static void put_ebml_sint(AVEMIOContext *pb, unsigned int elementid, int64_t val)
{
    int i, bytes = 1;
...
static void put_ebml_float(AVEMIOContext *pb, unsigned int elementid, double val)
{
    put_ebml_id(pb, elementid);
    put_ebml_num(pb, 8, 0);
    avio_em_wb64(pb, av_double2int(val));
}

static void put_ebml_binary(AVEMIOContext *pb, unsigned int elementid,
...
static void put_ebml_string(AVEMIOContext *pb, unsigned int elementid,
                            const char *str)
{
 ...
 */
static void put_ebml_void(AVEMIOContext *pb, uint64_t size)
{
    int64_t currentpos = avio_em_tell(pb);
    ...
    emio_fill(pb, 0, currentpos + size - avio_em_tell(pb));
}

static ebml_master start_ebml_master(AVEMIOContext *pb, unsigned int elementid,
                                     uint64_t expectedsize)
{
    int bytes = expectedsize ? ebml_num_size(expectedsize) : 8;
    put_ebml_id(pb, elementid);
    put_ebml_size_unknown(pb, bytes);
    return (ebml_master) {avio_em_tell(pb), bytes };
}

static void end_ebml_master(AVEMIOContext *pb, ebml_master master)
{
    int64_t pos = avio_em_tell(pb);

    if (avio_em_seek(pb, master.pos - master.sizebytes, SEEK_SET) < 0)
        return;
    put_ebml_num(pb, pos - master.pos, master.sizebytes);
    avio_em_seek(pb, pos, SEEK_SET);
}

static void put_xiph_size(AVEMIOContext *pb, int size)
{
    emio_fill(pb, 255, size / 255);
    avio_em_w8(pb, size % 255);
}
...
static void mkv_free(MatroskaMuxContext *mkv) {
    if (mkv->main_seekhead) {
        av_em_freep(&mkv->main_seekhead->entries);
        av_em_freep(&mkv->main_seekhead);
    }
    if (mkv->cues) {
        av_em_freep(&mkv->cues->entries);
        av_em_freep(&mkv->cues);
    }
    av_em_freep(&mkv->tracks);
    av_em_freep(&mkv->stream_durations);
    av_em_freep(&mkv->stream_duration_offsets);
}
 ...
 */
static mkv_seekhead *mkv_start_seekhead(AVEMIOContext *pb, int64_t segment_offset,
                                        int numelements)
{
    mkv_seekhead *new_seekhead = av_em_mallocz(sizeof(mkv_seekhead));
    if (!new_seekhead)
        return NULL;
    ...
    if (numelements > 0) {
        new_seekhead->filepos = avio_em_tell(pb);
...
static int mkv_add_seekhead_entry(mkv_seekhead *seekhead, unsigned int elementid, uint64_t filepos)
{
    ...
    entries = av_em_realloc_array(entries, seekhead->num_entries + 1, sizeof(mkv_seekhead_entry));
    if (!entries)
 ...
 */
static int64_t mkv_write_seekhead(AVEMIOContext *pb, MatroskaMuxContext *mkv)
{
    ...
    if (seekhead->reserved_size > 0) {
        if (avio_em_seek(pb, seekhead->filepos, SEEK_SET) < 0) {
            currentpos = -1;
            goto fail;
        }
    }
    ...
    if (seekhead->reserved_size > 0) {
        uint64_t remaining = seekhead->filepos + seekhead->reserved_size - avio_em_tell(pb);
...
{
    mkv_cues *cues = av_em_mallocz(sizeof(mkv_cues));
    if (!cues)
        return NULL;
                            ...
                            int64_t cluster_pos, int64_t relative_pos, int64_t duration)
{
    ...
    entries = av_em_realloc_array(entries, cues->num_entries + 1, sizeof(mkv_cuepoint));
    if (!entries)
        return AVERROR(ENOMEM);
...
static int64_t mkv_write_cues(AVEMFormatContext *s, mkv_cues *cues, mkv_track *tracks, int num_tracks)
{
...
static int put_xiph_codecpriv(AVEMFormatContext *s, AVEMIOContext *pb, AVEMCodecParameters *par)
{
    ...
    if (avpriv_split_xiph_headers(par->extradata, par->extradata_size,
        ...
        av_em_log(s, AV_LOG_ERROR, "Extradata corrupt.\n");
        return -1;
    }

    avio_em_w8(pb, 2);                    // number packets - 1
    for (j = 0; j < 2; j++) {
        put_xiph_size(pb, header_len[j]);
    }
    for (j = 0; j < 3; j++)
        avio_em_write(pb, header_start[j], header_len[j]);

    return 0;
}

static int put_wv_codecpriv(AVEMIOContext *pb, AVEMCodecParameters *par)
{
    if (par->extradata && par->extradata_size == 2)
        avio_em_write(pb, par->extradata, 2);
    else
        avio_em_wl16(pb, 0x403); // fallback to the version mentioned in matroska specs
    return 0;
}

static int put_flac_codecpriv(AVEMFormatContext *s,
                              AVEMIOContext *pb, AVEMCodecParameters *par)
{
    ...
    if (write_comment) {
        ...
        av_em_dict_set(&dict, "WAVEFORMATEXTENSIBLE_CHANNEL_MASK", buf, 0);

        len = ff_vorbiscomment_length(dict, vendor);
        if (len >= ((1<<24) - 4))
            return AVERROR(EINVAL);

        data = av_em_alloc(len + 4);
        if (!data) {
...
static int get_aac_sample_rates(AVEMFormatContext *s, AVEMCodecParameters *par,
...
static int mkv_write_native_codecprivate(AVEMFormatContext *s,
                                         AVEMCodecParameters *par,
                                         AVEMIOContext *dyn_cp)
{
    switch (par->codec_id) {
    ...
    case AV_CODEC_ID_H264:
        return em_isom_write_avcc(dyn_cp, par->extradata,
                                  par->extradata_size);
    ...
    case AV_CODEC_ID_ALAC:
        if (par->extradata_size < 36) {
            ...
            avio_em_write(dyn_cp, par->extradata + 12,
                       par->extradata_size - 12);
        break;
    default:
        if (par->codec_id == AV_CODEC_ID_PRORES &&
            em_codec_get_id(em_codec_movvideo_tags, par->codec_tag) == AV_CODEC_ID_PRORES) {
            avio_em_wl32(dyn_cp, par->codec_tag);
        } else if (par->extradata_size && par->codec_id != AV_CODEC_ID_TTA)
            avio_em_write(dyn_cp, par->extradata, par->extradata_size);
    }

    return 0;
}

static int mkv_write_codecprivate(AVEMFormatContext *s, AVEMIOContext *pb,
                                  AVEMCodecParameters *par,
                                  int native_id, int qt_id)
{
    ...
    ret = avio_em_open_dyn_buf(&dyn_cp);
    if (ret < 0)
        return ret;
    ...
    if (native_id) {
        ...
        if (qt_id) {
            if (!par->codec_tag)
                par->codec_tag = em_codec_get_tag(em_codec_movvideo_tags,
                                                    par->codec_id);
            if (par->extradata_size) {
                if (   em_codec_get_id(em_codec_movvideo_tags, par->codec_tag) == par->codec_id
                    && em_codec_get_id(em_codec_movvideo_tags, AV_RL32(par->extradata + 4)) != par->codec_id
                ) {
                    int i;
                    avio_em_wb32(dyn_cp, 0x5a + par->extradata_size);
                    avio_em_wl32(dyn_cp, par->codec_tag);
                    for(i = 0; i < 0x5a - 8; i++)
                        avio_em_w8(dyn_cp, 0);
                }
            ...
            if (!em_codec_get_tag(em_codec_bmp_tags, par->codec_id))
                av_em_log(s, AV_LOG_WARNING, "codec %s is not supported by this format\n",
                       avcodec_em_get_name(par->codec_id));

            if (!par->codec_tag)
                par->codec_tag = em_codec_get_tag(em_codec_bmp_tags,
                                                  par->codec_id);
            if (!par->codec_tag && par->codec_id != AV_CODEC_ID_RAWVIDEO) {
                av_em_log(s, AV_LOG_ERROR, "No bmp codec tag found for codec %s\n",
                       avcodec_em_get_name(par->codec_id));
                ret = AVERROR(EINVAL);
            }

            ff_put_bmp_header(dyn_cp, par, em_codec_bmp_tags, 0, 0);
        }
    } else if (par->codec_type == AVMEDIA_TYPE_AUDIO) {
        unsigned int tag;
        tag = em_codec_get_tag(em_codec_wav_tags, par->codec_id);
        if (!tag) {
            av_em_log(s, AV_LOG_ERROR, "No wav codec tag found for codec %s\n",
                   avcodec_em_get_name(par->codec_id));
            ret = AVERROR(EINVAL);
        }
        if (!par->codec_tag)
    ...
    codecpriv_size = avio_em_close_dyn_buf(dyn_cp, &codecpriv);
    if (codecpriv_size)
        put_ebml_binary(pb, MATROSKA_ID_CODECPRIVATE, codecpriv,
                        codecpriv_size);
    av_em_free(codecpriv);
    return ret;
}

static int mkv_write_video_color(AVEMIOContext *pb, AVEMCodecParameters *par, AVEMStream *st) {
    int side_data_size = 0;
    const uint8_t *side_data = av_em_stream_get_side_data(
...
static void mkv_write_field_order(AVEMIOContext *pb,
...
static int mkv_write_stereo_mode(AVEMFormatContext *s, AVEMIOContext *pb,
                                 AVEMStream *st, int mode, int *h_width, int *h_height)
{
    int i;
    int ret = 0;
    ...
    *h_height = 1;
    ...
    if ((tag = av_em_dict_get(st->metadata, "stereo_mode", NULL, 0)) ||
        (tag = av_em_dict_get( s->metadata, "stereo_mode", NULL, 0))) {
    ...
    for (i = 0; i < st->nb_side_data; i++) {
        AVEMPacketSideData sd = st->side_data[i];
...
static int mkv_write_track(AVEMFormatContext *s, MatroskaMuxContext *mkv,
                           int i, AVEMIOContext *pb, int default_stream_exists)
{
    ...
    int native_id = 0;
    int qt_id = 0;
    int bit_depth = av_em_get_bits_per_sample(par->codec_id);
    ...
    if (par->codec_type == AVMEDIA_TYPE_AUDIO) {
        if (!bit_depth && par->codec_id != AV_CODEC_ID_ADPCM_G726) {
            if (par->bits_per_raw_sample)
                ...
                bit_depth = av_em_get_bytes_per_sample(par->format) << 3;
        }
        if (!bit_depth)
    ...
    if ((tag = av_em_dict_get(st->metadata, "title", NULL, 0)))
        put_ebml_string(pb, MATROSKA_ID_TRACKNAME, tag->value);
    tag = av_em_dict_get(st->metadata, "language", NULL, 0);
    ...
    if (mkv->mode == MODE_WEBM && par->codec_id == AV_CODEC_ID_WEBVTT) {
        ...
        if (par->codec_id == AV_CODEC_ID_RAWVIDEO && !par->codec_tag) {
            if (mkv->allow_raw_vfw) {
                ...
                av_em_log(s, AV_LOG_ERROR, "Raw RGB is not supported Natively in Matroska, you can use AVI or NUT or\n"
    ...
    if (par->codec_type == AVMEDIA_TYPE_AUDIO && par->initial_padding && par->codec_id == AV_CODEC_ID_OPUS) {
        int64_t codecdelay = av_em_rescale_q(par->initial_padding,
                                          (AVEMRational){ 1, 48000 },
                                          (AVEMRational){ 1, 1000000000 });
        if (codecdelay < 0) {
            av_em_log(s, AV_LOG_ERROR, "Initial padding is invalid\n");
            return AVERROR(EINVAL);
        }
//         mkv->tracks[i].ts_offset = av_em_rescale_q(par->initial_padding,
//                                                 (AVEMRational){ 1, par->sample_rate },
//                                                 st->time_base);
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if(   st->avg_frame_rate.num > 0 && st->avg_frame_rate.den > 0
           && av_em_cmp_q(av_em_inv_q(st->avg_frame_rate), st->time_base) > 0)
        ...
        if (!native_id &&
            em_codec_get_tag(em_codec_movvideo_tags, par->codec_id) &&
            ((!em_codec_get_tag(em_codec_bmp_tags,   par->codec_id) && par->codec_id != AV_CODEC_ID_RAWVIDEO) ||
        ...
        if (((tag = av_em_dict_get(st->metadata, "alpha_mode", NULL, 0)) && atoi(tag->value)) ||
            ((tag = av_em_dict_get( s->metadata, "alpha_mode", NULL, 0)) && atoi(tag->value)) ||
        ...
        if (st->sample_aspect_ratio.num) {
            int64_t d_width = av_em_rescale(par->width, st->sample_aspect_ratio.num, st->sample_aspect_ratio.den);
            if (d_width > INT_MAX) {
                av_em_log(s, AV_LOG_ERROR, "Overflow in display width\n");
                return AVERROR(EINVAL);
            }
    ...
    case AVMEDIA_TYPE_SUBTITLE:
        if (!native_id) {
            av_em_log(s, AV_LOG_ERROR, "Subtitle codec %d is not supported.\n", par->codec_id);
        ...
        break;
    default:
        av_em_log(s, AV_LOG_ERROR, "Only audio, video, and subtitles are supported for Matroska.\n");
        return AVERROR(EINVAL);
    }
...
static int mkv_write_tracks(AVEMFormatContext *s)
{
    ...
    ret = mkv_add_seekhead_entry(mkv->main_seekhead, MATROSKA_ID_TRACKS, avio_em_tell(pb));
    if (ret < 0)
        return ret;
...
static int mkv_write_chapters(AVEMFormatContext *s)
{
    MatroskaMuxContext *mkv = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ebml_master chapters, editionentry;
    AVEMRational scale = {1, 1E9};
    int i, ret;

    if (!s->nb_chapters || mkv->wrote_chapters)
        return 0;

    ret = mkv_add_seekhead_entry(mkv->main_seekhead, MATROSKA_ID_CHAPTERS, avio_em_tell(pb));
    if (ret < 0) return ret;
    ...
    for (i = 0; i < s->nb_chapters; i++) {
        ...
        int64_t chapterstart = av_em_rescale_q(c->start, c->time_base, scale);
        int64_t chapterend   = av_em_rescale_q(c->end,   c->time_base, scale);
...
static int mkv_write_simpletag(AVEMIOContext *pb, AVEMDictionaryEntry *t)
{
    uint8_t *key = av_em_strdup(t->key);
    uint8_t *p   = key;
...
static int mkv_write_tag_targets(AVEMFormatContext *s,
                                 ...
                                 ebml_master *tags, ebml_master* tag)
{
    ...
    if (!tags->pos) {
        ret = mkv_add_seekhead_entry(mkv->main_seekhead, MATROSKA_ID_TAGS, avio_em_tell(s->pb));
        if (ret < 0) return ret;
...
static int mkv_write_tag(AVEMFormatContext *s, AVEMDictionary *m, unsigned int elementid,
                         unsigned int uid, ebml_master *tags)
{
    ...
    while ((t = av_em_dict_get(m, "", t, AV_DICT_IGNORE_SUFFIX))) {
        if (av_em_strcasecmp(t->key, "title") &&
            av_em_strcasecmp(t->key, "stereo_mode") &&
            av_em_strcasecmp(t->key, "creation_time") &&
            av_em_strcasecmp(t->key, "encoding_tool") &&
            (elementid != MATROSKA_ID_TAGTARGETS_TRACKUID ||
             av_em_strcasecmp(t->key, "language"))) {
...
static int mkv_check_tag(AVEMDictionary *m)
{
    AVEMDictionaryEntry *t = NULL;

    while ((t = av_em_dict_get(m, "", t, AV_DICT_IGNORE_SUFFIX)))
        if (av_em_strcasecmp(t->key, "title") && av_em_strcasecmp(t->key, "stereo_mode"))
            return 1;

    return 0;
}

static int mkv_write_tags(AVEMFormatContext *s)
{
    MatroskaMuxContext *mkv = s->priv_data;
    ebml_master tags = {0};
    int i, ret;
    ...
    if (!mkv->is_live) {
        for (i = 0; i < s->nb_streams; i++) {
            ...
            mkv->stream_duration_offsets[i] = avio_em_tell(s->pb);
...
static int mkv_write_attachments(AVEMFormatContext *s)
{
    MatroskaMuxContext *mkv = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    av_em_lfg_init(&c, av_em_get_random_seed());

    ret = mkv_add_seekhead_entry(mkv->main_seekhead, MATROSKA_ID_ATTACHMENTS, avio_em_tell(pb));
    if (ret < 0) return ret;
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (t = av_em_dict_get(st->metadata, "title", NULL, 0))
            put_ebml_string(pb, MATROSKA_ID_FILEDESC, t->value);
        if (!(t = av_em_dict_get(st->metadata, "filename", NULL, 0))) {
            av_em_log(s, AV_LOG_ERROR, "Attachment stream %d has no filename tag.\n", i);
            return AVERROR(EINVAL);
        }
        put_ebml_string(pb, MATROSKA_ID_FILENAME, t->value);
        if (t = av_em_dict_get(st->metadata, "mimetype", NULL, 0))
            mimetype = t->value;
        ...
        if (!mimetype) {
            av_em_log(s, AV_LOG_ERROR, "Attachment stream %d has no mimetype tag and "
                                    "it cannot be deduced from the codec id.\n", i);
            return AVERROR(EINVAL);
        }

        if (s->flags & AVFMT_FLAG_BITEXACT) {
            struct AVSHA *sha = av_em_sha_alloc();
            uint8_t digest[20];
            if (!sha)
                return AVERROR(ENOMEM);
            av_em_sha_init(sha, 160);
            av_em_sha_update(sha, st->codecpar->extradata, st->codecpar->extradata_size);
            av_em_sha_final(sha, digest);
            av_em_free(sha);
            fileuid = AV_RL64(digest);
        } else {
            fileuid = av_lfg_get(&c);
        }
        av_em_log(s, AV_LOG_VERBOSE, "Using %.16"PRIx64" for attachment %d\n",
               fileuid, i);
...
static int mkv_write_header(AVEMFormatContext *s)
{
    MatroskaMuxContext *mkv = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    if (mkv->mode != MODE_WEBM ||
        av_em_dict_get(s->metadata, "stereo_mode", NULL, 0) ||
        av_em_dict_get(s->metadata, "alpha_mode", NULL, 0))
        version = 4;
    ...
    for (i = 0; i < s->nb_streams; i++) {
        if (s->streams[i]->codecpar->codec_id == AV_CODEC_ID_ATRAC3 ||
            ...
            av_em_log(s, AV_LOG_ERROR,
                   ...
                   avcodec_em_get_name(s->streams[i]->codecpar->codec_id));
            return AVERROR_PATCHWELCOME;
        }
        if (s->streams[i]->codecpar->codec_id == AV_CODEC_ID_OPUS ||
            av_em_dict_get(s->streams[i]->metadata, "stereo_mode", NULL, 0) ||
            av_em_dict_get(s->streams[i]->metadata, "alpha_mode", NULL, 0))
            version = 4;
    }

    mkv->tracks = av_em_mallocz_array(s->nb_streams, sizeof(*mkv->tracks));
    if (!mkv->tracks) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    mkv->segment_offset = avio_em_tell(pb);
    ...
    ret = mkv_add_seekhead_entry(mkv->main_seekhead, MATROSKA_ID_INFO, avio_em_tell(pb));
    if (ret < 0) goto fail;

    segment_info = start_ebml_master(pb, MATROSKA_ID_INFO, 0);
    put_ebml_uint(pb, MATROSKA_ID_TIMECODESCALE, 1000000);
    if ((tag = av_em_dict_get(s->metadata, "title", NULL, 0)))
        put_ebml_string(pb, MATROSKA_ID_TITLE, tag->value);
    if (!(s->flags & AVFMT_FLAG_BITEXACT)) {
        uint32_t segment_uid[4];
        AVLFG lfg;

        av_em_lfg_init(&lfg, av_em_get_random_seed());

        for (i = 0; i < 4; i++)
            segment_uid[i] = av_lfg_get(&lfg);

        put_ebml_string(pb, MATROSKA_ID_MUXINGAPP, LIBAVFORMAT_IDENT);
        if ((tag = av_em_dict_get(s->metadata, "encoding_tool", NULL, 0)))
    ...
    if (em_parse_creation_time_metadata(s, &creation_time, 0) > 0) {
    ...
    mkv->duration = 0;
    mkv->duration_offset = avio_em_tell(pb);
    if (!mkv->is_live) {
    ...
    mkv->stream_durations = av_em_mallocz(s->nb_streams * sizeof(int64_t));
    mkv->stream_duration_offsets = av_em_mallocz(s->nb_streams * sizeof(int64_t));
    ...
    if (pb->seekable && mkv->reserve_cues_space) {
        mkv->cues_pos = avio_em_tell(pb);
        put_ebml_void(pb, mkv->reserve_cues_space);
    }

    av_em_init_packet(&mkv->cur_audio_pkt);
...
static void mkv_write_block(AVEMFormatContext *s, AVEMIOContext *pb,
                            unsigned int blockid, AVEMPacket *pkt, int keyframe)
{
    MatroskaMuxContext *mkv = s->priv_data;
    AVEMCodecParameters *par = s->streams[pkt->stream_index]->codecpar;
    ...
    av_em_log(s, AV_LOG_DEBUG, "Writing block at offset %" PRIu64 ", size %d, "
           "pts %" PRId64 ", dts %" PRId64 ", duration %" PRId64 ", keyframe %d\n",
           avio_em_tell(pb), pkt->size, pkt->pts, pkt->dts, pkt->duration,
           keyframe != 0);
    if (par->codec_id == AV_CODEC_ID_H264 && par->extradata_size > 0 &&
        (AV_RB24(par->extradata) == 1 || AV_RB32(par->extradata) == 1))
        em_avc_parse_nal_units_buf(pkt->data, &data, &size);
    else if (par->codec_id == AV_CODEC_ID_HEVC && par->extradata_size > 6 &&
             (AV_RB24(par->extradata) == 1 || AV_RB32(par->extradata) == 1))
        /* extradata is Annex B, assume the bitstream is too and convert it */
        em_hevc_annexb2mp4_buf(pkt->data, &data, &size, 0, NULL);
    else if (par->codec_id == AV_CODEC_ID_WAVPACK) {
        int ret = mkv_strip_wavpack(pkt->data, &data, &size);
        if (ret < 0) {
            av_em_log(s, AV_LOG_ERROR, "Error stripping a WavPack packet.\n");
            return;
        }
    } else
        data = pkt->data;
    ...
    side_data = av_em_packet_get_side_data(pkt,
                                        AV_PKT_DATA_SKIP_SAMPLES,
                                        &side_data_size);

    if (side_data && side_data_size >= 10) {
        discard_padding = av_em_rescale_q(AV_RL32(side_data + 4),
                                       (AVEMRational){1, par->sample_rate},
                                       (AVEMRational){1, 1000000000});
    }
    ...
    avio_em_w8(pb, 0x80 | track_number);
    avio_em_wb16(pb, ts - mkv->cluster_pts);
    avio_em_w8(pb, (blockid == MATROSKA_ID_SIMPLEBLOCK && keyframe) ? (1 << 7) : 0);
    avio_em_write(pb, data + offset, size);
    if (data != pkt->data)
        av_em_free(data);
    ...
    if (side_data_size && additional_id == 1) {
        ...
        avio_em_write(pb, side_data, side_data_size);
...
static int mkv_write_vtt_blocks(AVEMFormatContext *s, AVEMIOContext *pb, AVEMPacket *pkt)
{
    MatroskaMuxContext *mkv = s->priv_data;
    ...
    av_em_log(s, AV_LOG_DEBUG, "Writing block at offset %" PRIu64 ", size %d, "
           ...
           avio_em_tell(pb), size, pkt->pts, pkt->dts, pkt->duration, flags);

    blockgroup = start_ebml_master(pb, MATROSKA_ID_BLOCKGROUP, mkv_blockgroup_size(size));

    put_ebml_id(pb, MATROSKA_ID_BLOCK);
    put_ebml_num(pb, size + 4, 0);
    avio_em_w8(pb, 0x80 | (pkt->stream_index + 1));     // this assumes stream_index is less than 126
    avio_em_wb16(pb, ts - mkv->cluster_pts);
    avio_em_w8(pb, flags);
...
static void mkv_flush_dynbuf(AVEMFormatContext *s)
{
    MatroskaMuxContext *mkv = s->priv_data;
    int bufsize;
    uint8_t *dyn_buf;
    ...
    bufsize = avio_em_close_dyn_buf(mkv->dyn_bc, &dyn_buf);
    avio_em_write(s->pb, dyn_buf, bufsize);
    av_em_free(dyn_buf);
    mkv->dyn_bc = NULL;
}

static void mkv_start_new_cluster(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MatroskaMuxContext *mkv = s->priv_data;
    AVEMIOContext *pb;
    ...
    av_em_log(s, AV_LOG_DEBUG,
            ...
            avio_em_tell(pb), pkt->pts, pkt->dts);
...
static int mkv_write_packet_internal(AVEMFormatContext *s, AVEMPacket *pkt, int add_cue)
{
    MatroskaMuxContext *mkv = s->priv_data;
    AVEMIOContext *pb         = s->pb;
    AVEMCodecParameters *par  = s->streams[pkt->stream_index]->codecpar;
    ...
    if (ts == AV_NOPTS_VALUE) {
        av_em_log(s, AV_LOG_ERROR, "Can't write packet with unknown timestamp\n");
        return AVERROR(EINVAL);
    }
    ...
    if (mkv->cluster_pos != -1) {
        ...
        if ((int16_t)cluster_time != cluster_time) {
            av_em_log(s, AV_LOG_WARNING, "Starting new cluster due to timestamp\n");
    ...
    if (!s->pb->seekable) {
        if (!mkv->dyn_bc) {
            ret = avio_em_open_dyn_buf(&mkv->dyn_bc);
            if (ret < 0) {
                av_em_log(s, AV_LOG_ERROR, "Failed to open dynamic buffer\n");
                return ret;
            }
        }
        pb = mkv->dyn_bc;
    ...
    if (mkv->cluster_pos == -1) {
        mkv->cluster_pos = avio_em_tell(s->pb);
        mkv->cluster     = start_ebml_master(pb, MATROSKA_ID_CLUSTER, 0);
        put_ebml_uint(pb, MATROSKA_ID_CLUSTERTIMECODE, FFMAX(0, ts));
        mkv->cluster_pts = FFMAX(0, ts);
    }

    relative_packet_pos = avio_em_tell(s->pb) - mkv->cluster.pos;
...
static int mkv_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MatroskaMuxContext *mkv = s->priv_data;
    ...
    // after 4k and on a keyframe
    if (s->pb->seekable) {
        cluster_size = avio_em_tell(s->pb) - mkv->cluster_pos;
    } else {
        cluster_size = avio_em_tell(mkv->dyn_bc);
    }
    ...
    if (!mkv->cluster_pos)
        avio_em_write_marker(s->pb,
                          av_em_rescale_q(pkt->dts, s->streams[pkt->stream_index]->time_base, AV_TIME_BASE_Q),
    ...
    if (mkv->cur_audio_pkt.size > 0) {
        ...
        av_em_packet_unref(&mkv->cur_audio_pkt);
        if (ret < 0) {
            av_em_log(s, AV_LOG_ERROR,
    ...
    if (codec_type == AVMEDIA_TYPE_AUDIO) {
        ret = av_em_packet_ref(&mkv->cur_audio_pkt, pkt);
    } else
        ret = mkv_write_packet_internal(s, pkt, 0);
    return ret;
}

static int mkv_write_flush_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MatroskaMuxContext *mkv = s->priv_data;
    AVEMIOContext *pb;
    if (s->pb->seekable)
...
static int mkv_write_trailer(AVEMFormatContext *s)
{
    MatroskaMuxContext *mkv = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    if (pb->seekable) {
        if (mkv->cues->num_entries) {
            if (mkv->reserve_cues_space) {
                ...
                currentpos = avio_em_tell(pb);
                avio_em_seek(pb, mkv->cues_pos, SEEK_SET);

                cuespos  = mkv_write_cues(s, mkv->cues, mkv->tracks, s->nb_streams);
                cues_end = avio_em_tell(pb);
        ...
        // update the duration
        av_em_log(s, AV_LOG_DEBUG, "end duration = %" PRIu64 "\n", mkv->duration);
        currentpos = avio_em_tell(pb);
        avio_em_seek(pb, mkv->duration_offset, SEEK_SET);
        ...
        if (mkv->stream_durations) {
            ...
            for (i = 0; i < s->nb_streams; ++i) {
                AVEMStream *st = s->streams[i];
                double duration_sec = mkv->stream_durations[i] * av_em_q2d(st->time_base);
                char duration_string[20] = "";

                av_em_log(s, AV_LOG_DEBUG, "stream %d end duration = %" PRIu64 "\n", i,
                       mkv->stream_durations[i]);

                if (!mkv->is_live && mkv->stream_duration_offsets[i] > 0) {
                    avio_em_seek(pb, mkv->stream_duration_offsets[i], SEEK_SET);
...
static int mkv_query_codec(enum AVEMCodecID codec_id, int std_compliance)
{
    int i;
    ...
    if (std_compliance < FF_COMPLIANCE_NORMAL) {
        enum AVEMMediaType type = avcodec_em_get_type(codec_id);
...
static int mkv_init(struct AVEMFormatContext *s)
{
    int i;
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        avpriv_em_set_pts_info(s->streams[i], 64, 1, 1000);
    }

    return 0;
}

static int mkv_check_bitstream(struct AVEMFormatContext *s, const AVEMPacket *pkt)
{
    int ret = 1;
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_AAC) {
        if (pkt->size > 2 && (AV_RB16(pkt->data) & 0xfff0) == 0xfff0)
            ret = em_stream_add_bitstream_filter(st, "aac_adtstoasc", NULL);
    } else if (st->codecpar->codec_id == AV_CODEC_ID_VP9) {
        ret = em_stream_add_bitstream_filter(st, "vp9_superframe", NULL);
    }

    return ret;
}

static const AVEMCodecTag additional_audio_tags[] = {
...
static const AVEMCodecTag additional_video_tags[] = {
...
static const AVEMCodecTag additional_subtitle_tags[] = {
...
static const AVEMClass matroska_class = {
...
AVEMOutputFormat ff_matroska_muxer = {
    .name              = "matroska",
    ...
    .codec_tag         = (const AVEMCodecTag* const []){
         em_codec_bmp_tags, em_codec_wav_tags,
...
static const AVEMClass webm_class = {
...
AVEMOutputFormat ff_webm_muxer = {
    .name              = "webm",
...
static const AVEMClass mka_class = {
    .class_name = "matroska audio muxer",
    .item_name  = av_em_default_item_name,
    .option     = options,
    .version    = LIBAVUTIL_VERSION_INT,
};
AVEMOutputFormat ff_matroska_audio_muxer = {
    .name              = "matroska",
    ...
    .flags             = AVFMT_GLOBALHEADER | AVFMT_TS_NONSTRICT |
                         AVFMT_ALLOW_FLUSH,
    .codec_tag         = (const AVEMCodecTag* const []){
        em_codec_wav_tags, additional_audio_tags, 0
    },
