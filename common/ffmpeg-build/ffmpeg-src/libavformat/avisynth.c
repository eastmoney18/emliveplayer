 ...
 * by libavformat. */
static av_cold int avisynth_context_create(AVEMFormatContext *s)
{
...
/* Create AVEMStream from audio and video data. */
static int avisynth_create_stream_video(AVEMFormatContext *s, AVEMStream *st)
{
    AviSynthContext *avs = s->priv_data;
    ...
    st->avg_frame_rate    = (AVEMRational) { avs->vi->fps_numerator,
                                           avs->vi->fps_denominator };
    st->start_time        = 0;
    st->duration          = avs->vi->num_frames;
    st->nb_frames         = avs->vi->num_frames;
    avpriv_em_set_pts_info(st, 32, avs->vi->fps_denominator, avs->vi->fps_numerator);
...
static int avisynth_create_stream_audio(AVEMFormatContext *s, AVEMStream *st)
{
    AviSynthContext *avs = s->priv_data;
    ...
    avpriv_em_set_pts_info(st, 64, 1, avs->vi->audio_samples_per_second);
...
static int avisynth_create_stream(AVEMFormatContext *s)
{
    AviSynthContext *avs = s->priv_data;
    AVEMStream *st;
    int ret;
...
static int avisynth_open_file(AVEMFormatContext *s)
{
    AviSynthContext *avs = s->priv_data;
    AVS_Value arg, val;
    int ret;
    ...
    if (!avs_is_clip(val)) {
        av_em_log(s, AV_LOG_ERROR, "AviSynth script did not return a clip\n");
        ret = AVERROR_UNKNOWN;
        goto fail;
    }
...
static void avisynth_next_stream(AVEMFormatContext *s, AVEMStream **st,
                                 AVEMPacket *pkt, int *discard)
{
    AviSynthContext *avs = s->priv_data;
...
/* Copy AviSynth clip data into an AVEMPacket. */
static int avisynth_read_packet_video(AVEMFormatContext *s, AVEMPacket *pkt,
                                      int discard)
{
    AviSynthContext *avs = s->priv_data;
    ...
    if (av_em_new_packet(pkt, pkt->size) < 0)
...
static int avisynth_read_packet_audio(AVEMFormatContext *s, AVEMPacket *pkt,
                                      int discard)
{
    AviSynthContext *avs = s->priv_data;
    AVEMRational fps, samplerate;
    int samples;
    int64_t n;
    const char *error;
    ...
    if (avs_has_video(avs->vi)) {
        if (avs->curr_frame < avs->vi->num_frames)
            samples = av_em_rescale_q(avs->curr_frame, samplerate, fps) -
                      avs->curr_sample;
        else
            samples = av_em_rescale_q(1, samplerate, fps);
    } else {
        samples = 1000;
    }
...
static av_cold int avisynth_read_header(AVEMFormatContext *s)
{
    int ret;

    // Calling library must implement a lock for thread-safe opens.
    if (ret = avpriv_em_lock_avformat())
        return ret;

    if (ret = avisynth_open_file(s)) {
        avpriv_em_unlock_avformat();
        return ret;
    }
...
static int avisynth_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    AviSynthContext *avs = s->priv_data;
    AVEMStream *st;
    int discard = 0;
...
static av_cold int avisynth_read_close(AVEMFormatContext *s)
{
    if (avpriv_em_lock_avformat())
        return AVERROR_UNKNOWN;
...
static int avisynth_read_seek(AVEMFormatContext *s, int stream_index,
                              int64_t timestamp, int flags)
{
    ...
    fps        = (AVEMRational) { avs->vi->fps_numerator,
                                avs->vi->fps_denominator };
    samplerate = (AVEMRational) { avs->vi->audio_samples_per_second, 1 };
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
        ...
        if (avs_has_audio(avs->vi))
            avs->curr_sample = av_em_rescale_q(timestamp, samplerate, fps);
    } else {
        if ((timestamp >= avs->vi->num_audio_samples) || (timestamp < 0))
            return AVERROR_EOF;
        /* Force frame granularity for seeking. */
        if (avs_has_video(avs->vi)) {
            avs->curr_frame  = av_em_rescale_q(timestamp, fps, samplerate);
            avs->curr_sample = av_em_rescale_q(avs->curr_frame, samplerate, fps);
        } else {
...
AVEMInputFormat ff_avisynth_demuxer = {
    .name           = "avisynth",
