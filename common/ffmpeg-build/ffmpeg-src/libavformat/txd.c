...
static int txd_probe(AVEMProbeData * pd) {
...
static int txd_read_header(AVEMFormatContext *s) {
    AVEMStream *st;
    ...
    avpriv_em_set_pts_info(st, 64, 1, 5);
...
static int txd_read_packet(AVEMFormatContext *s, AVEMPacket *pkt) {
...
next_chunk:
    ...
    marker     = avio_em_rl32(pb);

    if (avio_em_feof(s->pb))
        return AVERROR_EOF;
    if (marker != TXD_MARKER && marker != TXD_MARKER2) {
        av_em_log(s, AV_LOG_ERROR, "marker does not match\n");
    ...
    case TXD_INFO:
        if (chunk_size > 100)
            break;
    case TXD_EXTRA:
        avio_em_skip(s->pb, chunk_size);
    case TXD_FILE:
    case TXD_TEXTURE:
        goto next_chunk;
    default:
        av_em_log(s, AV_LOG_ERROR, "unknown chunk id %i\n", id);
        return AVERROR_INVALIDDATA;
    }

    ret = av_em_get_packet(s->pb, pkt, chunk_size);
    if (ret < 0)
        return ret;
    pkt->stream_index = 0;
...
AVEMInputFormat ff_txd_demuxer = {
    .name        = "txd",
