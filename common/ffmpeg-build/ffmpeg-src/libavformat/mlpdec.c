...
static int av_always_inline mlp_thd_probe(AVEMProbeData *p, uint32_t sync)
{
...
static int mlp_probe(AVEMProbeData *p)
{
    return mlp_thd_probe(p, 0xf8726fbb);
}

AVEMInputFormat ff_mlp_demuxer = {
    .name           = "mlp",
...
static int thd_probe(AVEMProbeData *p)
{
    return mlp_thd_probe(p, 0xf8726fba);
}

AVEMInputFormat ff_truehd_demuxer = {
    .name           = "truehd",
