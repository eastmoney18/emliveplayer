...
static int fourxm_probe(AVEMProbeData *p)
{
...
static int parse_vtrk(AVEMFormatContext *s,
                      ...
                      int left)
{
    ...
    /* allocate a new AVEMStream */
    st = avformat_em_new_stream(s, NULL);
    if (!st)
        return AVERROR(ENOMEM);

    avpriv_em_set_pts_info(st, 60, fourxm->fps.den, fourxm->fps.num);
    ...
    st->codecpar->extradata      = av_em_mallocz(4 + AV_INPUT_BUFFER_PADDING_SIZE);
...
static int parse_strk(AVEMFormatContext *s,
                      FourxmDemuxContext *fourxm, uint8_t *buf, int size,
                      int left)
{
    AVEMStream *st;
    ...
    if ((unsigned)track >= UINT_MAX / sizeof(AudioTrack) - 1) {
        av_em_log(s, AV_LOG_ERROR, "current_track too large\n");
        return AVERROR_INVALIDDATA;
    }

    if (track + 1 > fourxm->track_count) {
        if (av_em_reallocp_array(&fourxm->tracks, track + 1, sizeof(AudioTrack)))
            return AVERROR(ENOMEM);
    ...
    if (fourxm->tracks[track].channels    <= 0 ||
        ...
        av_em_log(s, AV_LOG_ERROR, "audio header invalid\n");
        return AVERROR_INVALIDDATA;
    }
    if (!fourxm->tracks[track].adpcm && fourxm->tracks[track].bits<8) {
        av_em_log(s, AV_LOG_ERROR, "bits unspecified for non ADPCM\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    st->id = track;
    avpriv_em_set_pts_info(st, 60, 1, fourxm->tracks[track].sample_rate);
...
static int fourxm_read_header(AVEMFormatContext *s)
{
    ...
    header = av_em_alloc(header_size);
    if (!header)
        return AVERROR(ENOMEM);
    if (avio_em_read(pb, header, header_size) != header_size) {
        av_em_free(header);
        return AVERROR(EIO);
    }
    ...
    for (i = 0; i < header_size - 8; i++) {
        ...
        if (size > header_size - i - 8 && (fourcc_tag == vtrk_TAG || fourcc_tag == strk_TAG)) {
            av_em_log(s, AV_LOG_ERROR, "chunk larger than array %d>%d\n", size, header_size - i - 8);
            return AVERROR_INVALIDDATA;
        }

        if (fourcc_tag == std__TAG) {
            if (header_size - i < 16) {
                av_em_log(s, AV_LOG_ERROR, "std TAG truncated\n");
                ret = AVERROR_INVALIDDATA;
                goto fail;
            }
            fourxm->fps = av_em_d2q(av_int2float(AV_RL32(&header[i + 12])), 10000);
    ...
    return 0;
fail:
    av_em_freep(&fourxm->tracks);
    av_em_free(header);
    return ret;
}

static int fourxm_read_packet(AVEMFormatContext *s,
                              AVEMPacket *pkt)
{
    FourxmDemuxContext *fourxm = s->priv_data;
    AVEMIOContext *pb            = s->pb;
    ...
    while (!packet_read) {
        if ((ret = avio_em_read(s->pb, header, 8)) < 0)
            return ret;
        ...
        case cfr2_TAG:
             ...
             * and size */
            if (size + 8 < size || av_em_new_packet(pkt, size + 8))
                return AVERROR(EIO);
            pkt->stream_index = fourxm->video_stream_index;
            pkt->pts          = fourxm->video_pts;
            pkt->pos          = avio_em_tell(s->pb);
            memcpy(pkt->data, header, 8);
            ret = avio_em_read(s->pb, &pkt->data[8], size);

            if (ret < 0) {
                av_em_packet_unref(pkt);
            } else {
                packet_read = 1;
                av_em_shrink_packet(pkt, ret + 8);
            }
            break;

        case snd__TAG:
            track_number = avio_em_rl32(pb);
            avio_em_skip(pb, 4);
            size -= 8;
...
static int fourxm_read_close(AVEMFormatContext *s)
{
    FourxmDemuxContext *fourxm = s->priv_data;
...
AVEMInputFormat ff_fourxm_demuxer = {
    .name           = "4xm",
