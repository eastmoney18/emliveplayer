...
void ff_asfcrypt_dec(const uint8_t key[20], uint8_t *data, int len)
{
    ...
    av_em_rc4_init(rc4, key, 12 * 8, 1);
    av_em_rc4_crypt(rc4, (uint8_t *)rc4buff, NULL, sizeof(rc4buff), NULL, 1);
    multiswap_init((uint8_t *)rc4buff, ms_keys);

    packetkey  = AV_RN64(&qwords[num_qwords * 8 - 8]);
    packetkey ^= rc4buff[7];
    av_em_des_init(des, key + 12, 64, 1);
    av_em_des_crypt(des, (uint8_t *)&packetkey, (uint8_t *)&packetkey, 1, NULL, 1);
    packetkey ^= rc4buff[6];

    av_em_rc4_init(rc4, (uint8_t *)&packetkey, 64, 1);
    av_em_rc4_crypt(rc4, data, data, len, NULL, 1);
