...
{
    av_em_log_set_callback(count_warnings);
    num_warnings = 0;
}

static void reset_count_warnings(void)
{
    av_em_log_set_callback(av_em_log_default_callback);
}

static int io_write(void *opaque, uint8_t *buf, int size)
{
    out_size += size;
    av_em_md5_update(md5, buf, size);
    if (out)
...
static void init_fps(int bf, int audio_preroll, int fps)
{
    ...
    if (!ctx)
        exit(1);
    ctx->oformat = av_em_guess_format(format, NULL, NULL);
    if (!ctx->oformat)
        exit(1);
    ctx->pb = avio_em_alloc_context(iobuf, iobuf_size, AVIO_FLAG_WRITE, NULL, NULL, io_write, NULL);
    if (!ctx->pb)
        exit(1);
    ...
    st->codecpar->extradata = av_em_mallocz(st->codecpar->extradata_size + AV_INPUT_BUFFER_PADDING_SIZE);
    ...
    if (avformat_em_write_header(ctx, &opts) < 0)
        exit(1);
...
static void mux_frames(int n)
{
    ...
    while (1) {
        ...
        if (av_em_compare_ts(audio_dts, audio_st->time_base, video_dts, video_st->time_base) < 0) {
        ...
        if (skip_write_audio && pkt.stream_index == 1)
            continue;
        if (do_interleave)
            av_em_interleaved_write_frame(ctx, &pkt);
        else
            av_em_write_frame(ctx, &pkt);
    }
}
...
int main(int argc, char **argv)
{
    ...
    av_em_dict_set(&opts, "movflags", "frag_keyframe", 0);
    init(0, 0);
    mux_gops(2);
    finish();
    close_out();
    ...
    av_em_dict_set(&opts, "movflags", "frag_keyframe", 0);
    av_em_dict_set(&opts, "use_editlist", "1", 0);
    init(1, 1);
    mux_gops(2);
    finish();
    close_out();
    ...
    av_em_dict_set(&opts, "movflags", "frag_keyframe", 0);
    av_em_dict_set(&opts, "use_editlist", "0", 0);
    init(1, 0);
    mux_gops(2);
    finish();
    close_out();
    ...
    // moof+mdat pairs.
    init_out("empty-moov");
    av_em_dict_set(&opts, "movflags", "frag_keyframe+empty_moov", 0);
    av_em_dict_set(&opts, "use_editlist", "0", 0);
    init(0, 0);
    mux_gops(2);
    finish();
    ...
    av_em_dict_set(&opts, "movflags", "frag_keyframe+empty_moov", 0);
    av_em_dict_set(&opts, "avoid_negative_ts", "0", 0);
    init(1, 0);
    mux_gops(2);
    finish();
    close_out();
    ...
    // simple input
    init_out("delay-moov");
    av_em_dict_set(&opts, "movflags", "frag_keyframe+delay_moov", 0);
    av_em_dict_set(&opts, "use_editlist", "0", 0);
    init(0, 0);
    mux_gops(2);
    finish();
    ...
    av_em_dict_set(&opts, "movflags", "frag_custom+delay_moov", 0);
    init(0, 0);
    mux_gops(1);
    av_em_write_frame(ctx, NULL); // Force writing the moov
    check(out_size > 0, "No moov written");
    av_em_write_frame(ctx, NULL);
    mux_gops(1);
    av_em_write_frame(ctx, NULL);
    finish();
    close_out();
    ...
    av_em_write_frame(ctx, NULL); // Flush the first fragment
    ...
    av_em_write_frame(ctx, NULL); // Flush the second fragment
    close_out();
    ...
    av_em_dict_set(&opts, "movflags", "frag_custom+empty_moov+dash", 0);
    init(0, 0);
    mux_gops(1);
    av_em_write_frame(ctx, NULL); // Output the first fragment
    init_out("empty-moov-second-frag");
    mux_gops(1);
    av_em_write_frame(ctx, NULL); // Output the second fragment
    close_out();
    memcpy(content, hash, HASH_SIZE);
    finish();
    ...
    // one before.
    av_em_dict_set(&opts, "movflags", "frag_custom+empty_moov+dash+frag_discont", 0);
    av_em_dict_set(&opts, "fragment_index", "2", 0);
    av_em_dict_set(&opts, "avoid_negative_ts", "0", 0);
    av_em_dict_set(&opts, "use_editlist", "0", 0);
    init(0, 0);
    skip_gops(1);
    ...
    av_em_dict_set(&opts, "movflags", "frag_custom+delay_moov+dash+frag_discont", 0);
    av_em_dict_set(&opts, "fragment_index", "2", 0);
    init(0, 0);
    skip_gops(1);
    mux_gops(1);
    av_em_write_frame(ctx, NULL); // Output the moov
    ...
    av_em_dict_set(&opts, "movflags", "frag_custom+delay_moov+dash", 0);
    init(1, 0);
    mux_gops(1);
    ...
    // by the edit list.
    init_out("vfr");
    av_em_dict_set(&opts, "movflags", "frag_keyframe+delay_moov+dash", 0);
    init_fps(1, 1, 3);
    ...
    // Here, using av_em_interleaved_write_frame allows the muxer to get the
    ...
    av_em_dict_set(&opts, "movflags", "frag_keyframe+delay_moov", 0);
    av_em_dict_set(&opts, "frag_duration", "650000", 0);
    init_fps(1, 1, 30);
    mux_frames(gop_size/2);
