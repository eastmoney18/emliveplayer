...
static void ts_str(char buffer[60], int64_t ts, AVEMRational base)
{
    if (ts == AV_NOPTS_VALUE) {
        strcpy(buffer, " NOPTS   ");
        return;
    }
    ts= av_em_rescale_q(ts, base, (AVEMRational){1, 1000000});
...
int main(int argc, char **argv)
{
    ...
    for(i=2; i<argc; i+=2){
        if       (!strcmp(argv[i], "-seekforw")){
            ...
            av_em_dict_set(&format_opts, argv[i] + 1, argv[i+1], 0);
        } else {
            argc = 1;
        }
    }

    av_em_dict_set(&format_opts, "channels", "1", 0);
    av_em_dict_set(&format_opts, "sample_rate", "22050", 0);
    ...
    ret = avformat_em_open_input(&ic, filename, NULL, &format_opts);
    ...
    ret = avformat_em_find_stream_info(ic, NULL);
    if (ret < 0) {
    ...
    if(seekfirst != AV_NOPTS_VALUE){
        if(firstback)   avformat_em_seek_file(ic, -1, INT64_MIN, seekfirst, seekfirst, 0);
        else            avformat_em_seek_file(ic, -1, seekfirst, seekfirst, INT64_MAX, 0);
    }
    for(i=0; ; i++){
        AVEMPacket pkt = { 0 };
        AVEMStream *av_uninit(st);
        char ts_buf[60];
        ...
        if(stream_id>=0){
            ...
            timestamp= av_em_rescale_q(timestamp, AV_TIME_BASE_Q, st->time_base);
        }
        //FIXME fully test the new seek API
        if(i&1) ret = avformat_em_seek_file(ic, stream_id, INT64_MIN, timestamp, timestamp, 0);
        else    ret = avformat_em_seek_file(ic, stream_id, timestamp, timestamp, INT64_MAX, 0);
