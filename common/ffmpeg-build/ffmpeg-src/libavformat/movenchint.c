...
int em_mov_init_hinting(AVEMFormatContext *s, int index, int src_index)
{
    ...
    /* Copy the RTP AVEMStream timebase back to the hint AVEMStream */
...
fail:
    ...
    avcodec_em_parameters_free(&track->par);
    /* Set a default timescale, to avoid crashes in av_em_dump_format */
...
static void sample_queue_pop(HintSampleQueue *queue)
{
    ...
    if (queue->samples[0].own_data)
        av_em_freep(&queue->samples[0].data);
    queue->len--;
...
static void sample_queue_free(HintSampleQueue *queue)
{
    ...
    for (i = 0; i < queue->len; i++)
        if (queue->samples[i].own_data)
            av_em_freep(&queue->samples[i].data);
    av_em_freep(&queue->samples);
    queue->len  = 0;
    queue->size = 0;
}
                              ...
                              int sample)
{
    ...
    if (!queue->samples || queue->len >= queue->size) {
        HintSample *samples;
        samples = av_em_realloc_array(queue->samples, queue->size + 10, sizeof(HintSample));
        if (!samples)
            return;
        queue->size += 10;
...
static void sample_queue_retain(HintSampleQueue *queue)
{
    ...
    for (i = 0; i < queue->len; ) {
        ...
        if (!sample->own_data) {
            uint8_t *ptr = av_em_alloc(sample->size);
            if (!ptr) {
...
static void output_immediate(const uint8_t *data, int size,
                             AVEMIOContext *out, int *entries)
{
    while (size > 0) {
        int len = size;
        if (len > 14)
            len = 14;
        avio_em_w8(out, 1); /* immediate constructor */
        avio_em_w8(out, len); /* amount of valid data */
        avio_em_write(out, data, len);
        data += len;
        size -= len;
...
static void output_match(AVEMIOContext *out, int match_sample,
                         int match_offset, int match_len, int *entries)
{
    avio_em_w8(out, 2); /* sample constructor */
    avio_em_w8(out, 0); /* track reference */
    avio_em_wb16(out, match_len);
    avio_em_wb32(out, match_sample);
    avio_em_wb32(out, match_offset);
    avio_em_wb16(out, 1); /* bytes per block */
    avio_em_wb16(out, 1); /* samples per block */
    (*entries)++;
}
 ...
 */
static int write_hint_packets(AVEMIOContext *out, const uint8_t *data,
                              int size, MOVTrack *trk, int64_t *dts)
{
    ...
    count_pos = avio_em_tell(out);
    /* RTPsample header */
    avio_em_wb16(out, 0); /* packet count */
    avio_em_wb16(out, 0); /* reserved */
    ...
    while (size > 4) {
        ...
        count++;
        /* RTPpacket header */
        avio_em_wb32(out, 0); /* relative_time */
        avio_em_write(out, data, 2); /* RTP header */
        avio_em_wb16(out, seq); /* RTPsequenceseed */
        avio_em_wb16(out, ts_diff ? 4 : 0); /* reserved + flags (extra_flag) */
        entries_pos = avio_em_tell(out);
        avio_em_wb16(out, 0); /* entry count */
        if (ts_diff) { /* if extra_flag is set */
            avio_em_wb32(out, 16); /* extra_information_length */
            avio_em_wb32(out, 12); /* rtpoffsetTLV box */
            avio_em_write(out, "rtpo", 4);
            avio_em_wb32(out, ts_diff);
        }
        ...
        curpos = avio_em_tell(out);
        avio_em_seek(out, entries_pos, SEEK_SET);
        avio_em_wb16(out, entries);
        avio_em_seek(out, curpos, SEEK_SET);
    }

    curpos = avio_em_tell(out);
    avio_em_seek(out, count_pos, SEEK_SET);
    avio_em_wb16(out, count);
    avio_em_seek(out, curpos, SEEK_SET);
    return count;
}

int em_mov_add_hinted_packet(AVEMFormatContext *s, AVEMPacket *pkt,
                             int track_index, int sample,
                             uint8_t *sample_data, int sample_size)
{
    MOVMuxContext *mov = s->priv_data;
    MOVTrack *trk = &mov->tracks[track_index];
    AVEMFormatContext *rtp_ctx = trk->rtp_ctx;
    uint8_t *buf = NULL;
    int size;
    AVEMIOContext *hintbuf = NULL;
    AVEMPacket hint_pkt;
    int ret = 0, count;
    ...
    em_write_chained(rtp_ctx, 0, pkt, s, 0);

    /* Fetch the output from the RTP muxer, open a new output buffer
     * for next time. */
    size = avio_em_close_dyn_buf(rtp_ctx->pb, &buf);
    if ((ret = emio_open_dyn_packet_buf(&rtp_ctx->pb,
    ...
    if ((ret = avio_em_open_dyn_buf(&hintbuf)) < 0)
        goto done;
    av_em_init_packet(&hint_pkt);
    count = write_hint_packets(hintbuf, buf, size, trk, &hint_pkt.dts);
    av_em_freep(&buf);

    /* Write the hint data into the hint track */
    hint_pkt.size = size = avio_em_close_dyn_buf(hintbuf, &buf);
    hint_pkt.data = buf;
    ...
    if (count > 0)
        em_mov_write_packet(s, &hint_pkt);
done:
    av_em_free(buf);
...
void em_mov_close_hinting(MOVTrack *track)
{
    AVEMFormatContext *rtp_ctx = track->rtp_ctx;
    ...
    if (!rtp_ctx)
        return;
    if (rtp_ctx->pb) {
        av_em_write_trailer(rtp_ctx);
        emio_free_dyn_buf(&rtp_ctx->pb);
    }
