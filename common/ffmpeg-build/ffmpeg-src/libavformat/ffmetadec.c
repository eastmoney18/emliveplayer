...
static void get_line(AVEMIOContext *s, uint8_t *buf, int size)
{
    do {
        uint8_t c;
        int i = 0;

        while ((c = avio_em_r8(s))) {
            if (c == '\\') {
                if (i < size - 1)
                    buf[i++] = c;
                c = avio_em_r8(s);
        ...
        buf[i] = 0;
    } while (!avio_em_feof(s) && (buf[0] == ';' || buf[0] == '#' || buf[0] == 0));
}

static AVEMChapter *read_chapter(AVEMFormatContext *s)
{
    uint8_t line[256];
    int64_t start, end;
    AVEMRational tb = {1, 1e9};
    ...
    if (!sscanf(line, "START=%"SCNd64, &start)) {
        av_em_log(s, AV_LOG_ERROR, "Expected chapter start timestamp, found %s.\n", line);
    ...
    if (!sscanf(line, "END=%"SCNd64, &end)) {
        av_em_log(s, AV_LOG_ERROR, "Expected chapter end timestamp, found %s.\n", line);
        end = AV_NOPTS_VALUE;
    }

    return avpriv_em_new_chapter(s, s->nb_chapters, tb, start, end, NULL);
}

static uint8_t *unescape(const uint8_t *buf, int size)
{
    uint8_t *ret = av_em_alloc(size + 1);
    uint8_t *p1  = ret;
...
static int read_tag(const uint8_t *line, AVEMDictionary **m)
{
    uint8_t *key, *value;
    ...
    av_em_dict_set(m, key, value, AV_DICT_DONT_STRDUP_KEY | AV_DICT_DONT_STRDUP_VAL);
    return 0;
}
...
static int read_header(AVEMFormatContext *s)
{
    ...
    s->start_time = 0;
    if (s->nb_chapters)
        s->duration = av_em_rescale_q(s->chapters[s->nb_chapters - 1]->end,
...
AVEMInputFormat ff_ffmetadata_demuxer = {
    .name        = "ffmetadata",
