...
static const AVEMClass mpegts_class = {
...
static const AVEMClass mpegtsraw_class = {
...
typedef struct PESContext {
    ...
    AVEMFormatContext *stream;
    AVEMStream *st;
    AVEMStream *sub_st; /**< stream for the embedded AC3 stream in HDMV TrueHD */
    enum MpegTSState state;
    /* used to get the format */
    int data_index;
    int flags; /**< copied to the AVEMPacket flags */
    int total_size;
    int pes_header_size;
...
extern AVEMInputFormat em_mpegts_demuxer;
...
{
    struct Program *p;
    if (av_em_reallocp_array(&ts->prg, ts->nb_prg + 1, sizeof(*ts->prg)) < 0) {
        ts->nb_prg = 0;
        return;
    }
    p = &ts->prg[ts->nb_prg];
...
static void set_pcr_pid(AVEMFormatContext *s, unsigned int programid, unsigned int pid)
{
    int i;
                               ...
                               const uint8_t *buf, int buf_size, int is_start)
{
    ...
    if (tss->section_h_size != -1 &&
        ...
        if (tss->check_crc) {
            crc_valid = !av_em_crc(av_em_crc_get_table(AV_CRC_32_IEEE), -1, tss->section_buf, tss->section_h_size);
                                        ...
                                        enum MpegTSFilterType type)
{
    ...
    av_em_log(ts->stream, AV_LOG_TRACE, "Filter: pid=0x%x type=%d\n", pid, type);

    if (pid >= NB_PID_MAX || ts->pids[pid])
        return NULL;
    filter = av_em_mallocz(sizeof(MpegTSFilter));
    if (!filter)
        return NULL;
    ts->pids[pid] = filter;
                                                ...
                                                int check_crc)
{
    ...
    sec->opaque      = opaque;
    sec->section_buf = av_em_alloc(MAX_SECTION_SIZE);
...
static void mpegts_close_filter(MpegTSContext *ts, MpegTSFilter *filter)
{
    ...
    if (filter->type == MPEGTS_SECTION)
        av_em_freep(&filter->u.section_filter.section_buf);
    else if (filter->type == MPEGTS_PES) {
        PESContext *pes = filter->u.pes_filter.opaque;
        av_em_buffer_unref(&pes->buffer);
        /* referenced private data will be freed later in
         * avformat_em_close_input */
        if (!((PESContext *)filter->u.pes_filter.opaque)->st) {
            av_em_freep(&filter->u.pes_filter.opaque);
        }
    }
...
static int get_packet_size(const uint8_t *buf, int size)
{
    ...
    av_em_log(NULL, AV_LOG_TRACE, "score: %d, dvhs_score: %d, fec_score: %d \n",
...
static void mpegts_find_stream_type(AVEMStream *st,
                                    uint32_t stream_type,
...
static int mpegts_set_stream_info(AVEMStream *st, PESContext *pes,
                                  uint32_t stream_type, uint32_t prog_reg_desc)
{
    ...
    if (avcodec_em_is_open(st->internal->avctx)) {
        av_em_log(pes->stream, AV_LOG_DEBUG, "cannot set stream info, internal codec is open\n");
        return 0;
    }
    ...
    av_em_log(pes->stream, AV_LOG_DEBUG,
    ...
    if ((prog_reg_desc == AV_RL32("HDMV") ||
        ...
        if (pes->stream_type == 0x83) {
            ...
            PESContext *sub_pes = av_em_alloc(sizeof(*sub_pes));
            if (!sub_pes)
                return AVERROR(ENOMEM);
            memcpy(sub_pes, pes, sizeof(*sub_pes));

            sub_st = avformat_em_new_stream(pes->stream, NULL);
            if (!sub_st) {
                av_em_free(sub_pes);
                return AVERROR(ENOMEM);
            }
            ...
            sub_st->id = pes->pid;
            avpriv_em_set_pts_info(sub_st, 33, 1, 90000);
...
static int new_pes_packet(PESContext *pes, AVEMPacket *pkt)
{
    char *sd;
    ...
    if (pes->total_size != MAX_PES_PAYLOAD &&
        ...
        PES_START_SIZE) {
        av_em_log(pes->stream, AV_LOG_WARNING, "PES packet size mismatch\n");
    ...
    sd = av_em_packet_new_side_data(pkt, AV_PKT_DATA_MPEGTS_STREAM_ID, 1);
    if (!sd)
        return AVERROR(ENOMEM);
    *sd = pes->stream_id;
                          ...
                          const uint8_t *buf, int buf_size)
{
    ...
    if (sl->timestamp_len && sl->timestamp_res)
        avpriv_em_set_pts_info(pes->st, sl->timestamp_len, 1, sl->timestamp_res);
        ...
        case MPEGTS_HEADER:
            ...
            if (pes->data_index == PES_START_SIZE) {
                ...
                if (pes->header[0] == 0x00 && pes->header[1] == 0x00 &&
                    ...
                    av_em_log(pes->stream, AV_LOG_TRACE, "pid=%x pes_code=%#x\n", pes->pid,
                            code);
                    ...
                    if (!pes->st) {
                        ...
                        pes->st = avformat_em_new_stream(ts->stream, NULL);
                        if (!pes->st)
                            return AVERROR(ENOMEM);
                        pes->st->id = pes->pid;
                    ...
                    /* allocate pes buffer */
                    pes->buffer = av_em_buffer_alloc(pes->total_size +
                    ...
                    if (code != 0x1bc && code != 0x1bf && /* program_stream_map, private_stream_2 */
                        ...
                        if (pes->st->codecpar->codec_id == AV_CODEC_ID_NONE && !pes->st->request_probe) {
                            av_em_log(pes->stream, AV_LOG_TRACE,
        ...
        case MPEGTS_PESHEADER_FILL:
            ...
            if (pes->data_index == pes->pes_header_size) {
                ...
                if (   pes->ts->fix_teletext_pts
                    ...
                    AVEMProgram *p = NULL;
                    while ((p = av_em_find_program_from_stream(pes->stream, p, pes->st->index))) {
                            ...
                            if (f) {
                                ...
                                if (f->type == MPEGTS_PES) {
                                    ...
                                    for (i = 0; i < p->nb_stream_indexes; i++) {
                                        AVEMStream *pst = pes->stream->streams[p->stream_index[i]];
...
static PESContext *add_pes_stream(MpegTSContext *ts, int pid, int pcr_pid)
{
    ...
    pes = av_em_mallocz(sizeof(PESContext));
    if (!pes)
        return 0;
    pes->ts      = ts;
...
static int init_MP4DescrParseContext(MP4DescrParseContext *d, AVEMFormatContext *s,
                                     ...
                                     Mp4Descr *descr, int max_descr_count)
{
    ...
    if ((ret = emio_init_context(&d->pb, (unsigned char *)buf, size, 0,
...
static void update_offsets(AVEMIOContext *pb, int64_t *off, int *len)
{
    int64_t new_off = avio_em_tell(pb);
    (*len) -= new_off - *off;
    *off    = new_off;
}
...
static int parse_MP4ESDescrTag(MP4DescrParseContext *d, int64_t off, int len)
{
    ...
    if (d->descr_count >= d->max_descr_count)
        return AVERROR_INVALIDDATA;
    em_mp4_parse_es_descr(&d->pb, &es_id);
                                      ...
                                      int len)
{
    ...
    if (!descr)
        return AVERROR_INVALIDDATA;
    d->active_descr->dec_config_descr = av_em_alloc(len);
    if (!descr->dec_config_descr)
        return AVERROR(ENOMEM);
    descr->dec_config_descr_len = len;
    avio_em_read(&d->pb, descr->dec_config_descr, len);
    return 0;
}
...
#define R8_CHECK_CLIP_MAX(dst, maxv) do {                       \
    ...
    predefined = avio_em_r8(&d->pb);
    if (!predefined) {
        int lengths;
        int flags = avio_em_r8(&d->pb);
        ...
        descr->sl.timestamp_res   = avio_em_rb32(&d->pb);
        avio_em_rb32(&d->pb);
        R8_CHECK_CLIP_MAX(timestamp_len, 63);
        R8_CHECK_CLIP_MAX(ocr_len,       63);
        R8_CHECK_CLIP_MAX(au_len,        31);
        descr->sl.inst_bitrate_len   = avio_em_r8(&d->pb);
        lengths                      = avio_em_rb16(&d->pb);
        descr->sl.degr_prior_len     = lengths >> 12;
        descr->sl.au_seq_num_len     = (lengths >> 7) & 0x1f;
        descr->sl.packet_seq_num_len = (lengths >> 2) & 0x1f;
    } else if (!d->predefined_SLConfigDescriptor_seen){
        avpriv_em_report_missing_feature(d->s, "Predefined SLConfigDescriptor");
...
{
    int tag;
    int len1 = em_mp4_read_descr(d->s, &d->pb, &tag);
    int ret = 0;

    update_offsets(&d->pb, &off, &len);
    if (len < 0 || len1 > len || len1 <= 0) {
        av_em_log(d->s, AV_LOG_ERROR,
    ...
    if (d->level++ >= MAX_LEVEL) {
        av_em_log(d->s, AV_LOG_ERROR, "Maximum MP4 descriptor level exceeded\n");
        ret = AVERROR_INVALIDDATA;
        goto done;
    }

    if (target_tag && tag != target_tag) {
        av_em_log(d->s, AV_LOG_ERROR, "Found tag %x expected %x\n", tag,
               target_tag);
        ret = AVERROR_INVALIDDATA;
        goto done;
    }
    ...
    d->level--;
    avio_em_seek(&d->pb, off + len1, SEEK_SET);
    return ret;
}

static int mp4_read_iods(AVEMFormatContext *s, const uint8_t *buf, unsigned size,
                         Mp4Descr *descr, int *descr_count, int max_descr_count)
{
    ...
    ret = parse_mp4_descr(&d, avio_em_tell(&d.pb), size, MP4IODescrTag);

    *descr_count = d.descr_count;
    return ret;
}

static int mp4_read_od(AVEMFormatContext *s, const uint8_t *buf, unsigned size,
                       Mp4Descr *descr, int *descr_count, int max_descr_count)
{
    MP4DescrParseContext d;
    int ret;
    ...
    ret = parse_mp4_descr_arr(&d, avio_em_tell(&d.pb), size);
    ...
    for (pid = 0; pid < NB_PID_MAX; pid++) {
        ...
        for (i = 0; i < mp4_descr_count; i++) {
            ...
            if (ts->pids[pid]->type != MPEGTS_PES) {
                av_em_log(s, AV_LOG_ERROR, "pid %x is not PES\n", pid);
                continue;
            }
            ...
            emio_init_context(&pb, mp4_descr[i].dec_config_descr,
                              mp4_descr[i].dec_config_descr_len, 0,
                              NULL, NULL, NULL, NULL);
            em_mp4_read_dec_config_descr(s, st, &pb);
            ...
            st->codecpar->codec_type = avcodec_em_get_type(st->codecpar->codec_id);
            st->internal->need_context_update = 1;
        }
    }
    for (i = 0; i < mp4_descr_count; i++)
        av_em_free(mp4_descr[i].dec_config_descr);
}
...
int ff_parse_mpeg2_descriptor(AVEMFormatContext *fc, AVEMStream *st, int stream_type,
                              ...
                              MpegTSContext *ts)
{
    ...
    av_em_log(fc, AV_LOG_TRACE, "tag: 0x%02x len=%d\n", desc_tag, desc_len);
    ...
    case 0x1F: /* FMC descriptor */
        ...
        if (mp4_descr_count > 0 &&
            ...
            AVEMIOContext pb;
            emio_init_context(&pb, mp4_descr->dec_config_descr,
    ...
    case 0x56: /* DVB teletext descriptor */
        {
            ...
            if (language_count > 0) {
                ...
                if (st->codecpar->extradata == NULL) {
                    if (em_alloc_extradata(st->codecpar, language_count * 2)) {
                        return AVERROR(ENOMEM);
                    }
                }
    ...
    case 0x59: /* subtitling descriptor */
        {
            ...
            if (language_count > 1) {
                avpriv_em_request_sample(fc, "DVB subtitles with multiple languages");
            }
            ...
            if (language_count > 0) {
                ...
                if (st->codecpar->extradata == NULL) {
                    if (em_alloc_extradata(st->codecpar, language_count * 5)) {
                        return AVERROR(ENOMEM);
                    }
                }
    ...
    case 0x05: /* registration descriptor */
        ...
        av_em_log(fc, AV_LOG_TRACE, "reg_desc=%.4s\n", (char *)&st->codecpar->codec_tag);
        if (st->codecpar->codec_id == AV_CODEC_ID_NONE || st->request_probe > 0) {
    ...
    case 0x7f: /* DVB extension descriptor */
        ...
        if (st->codecpar->codec_id == AV_CODEC_ID_OPUS &&
            ...
            if (!st->codecpar->extradata) {
                st->codecpar->extradata = av_em_mallocz(sizeof(opus_default_extradata) +
                                                     AV_INPUT_BUFFER_PADDING_SIZE);
                if (!st->codecpar->extradata)
                    return AVERROR(ENOMEM);
                ...
                if (channel_config_code <= 0x8) {
                    ...
                    avpriv_em_request_sample(fc, "Opus in MPEG-TS - channel_config_code > 0x8");
                }
                st->need_parsing = AVSTREAM_PARSE_FULL;
                st->internal->need_context_update = 1;
            }
...
static void pmt_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    av_em_log(ts->stream, AV_LOG_TRACE, "PMT: len %i\n", section_len);
    ...
    av_em_log(ts->stream, AV_LOG_TRACE, "sid=0x%x sec_num=%d/%d version=%d tid=%d\n",
    ...
    av_em_log(ts->stream, AV_LOG_TRACE, "pcr_pid=0x%x\n", pcr_pid);
    ...
    while (program_info_length >= 2) {
        ...
        av_em_log(ts->stream, AV_LOG_TRACE, "program tag: 0x%02x len=%d\n", tag, len);
    ...
    for (;;) {
        ...
        if (ts->pids[pid] && ts->pids[pid]->type == MPEGTS_PES) {
            pes = ts->pids[pid]->u.pes_filter.opaque;
            if (!pes->st) {
                pes->st     = avformat_em_new_stream(pes->stream, NULL);
                if (!pes->st)
                    goto out;
                pes->st->id = pes->pid;
            }
            ...
            if (pes) {
                st = avformat_em_new_stream(pes->stream, NULL);
                if (!st)
                    goto out;
                st->id = pes->pid;
            }
        } else {
            int idx = em_find_stream_index(ts->stream, pid);
            if (idx >= 0) {
                st = ts->stream->streams[idx];
            } else {
                st = avformat_em_new_stream(ts->stream, NULL);
                if (!st)
                    goto out;
                st->id = pid;
                st->codecpar->codec_type = AVMEDIA_TYPE_DATA;
        ...
        av_em_program_add_stream_index(ts->stream, h->id, st->index);
        ...
        for (;;) {
            ...
            if (pes && prog_reg_desc == AV_RL32("HDMV") &&
                ...
                av_em_program_add_stream_index(ts->stream, h->id,
                                            pes->sub_st->index);
...
static void pat_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    av_em_log(ts->stream, AV_LOG_TRACE, "PAT:\n");
    hex_dump_debug(ts->stream, section, section_len);
    ...
    for (;;) {
        ...
        av_em_log(ts->stream, AV_LOG_TRACE, "sid=0x%x pid=0x%x\n", sid, pmt_pid);

        if (sid == 0x0000) {
            /* NIT info */
        } else {
            MpegTSFilter *fil = ts->pids[pmt_pid];
            program = av_em_new_program(ts->stream, sid);
            if (program) {
...
static void sdt_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    av_em_log(ts->stream, AV_LOG_TRACE, "SDT:\n");
    hex_dump_debug(ts->stream, section, section_len);
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            av_em_log(ts->stream, AV_LOG_TRACE, "tag: 0x%02x len=%d\n",
                    desc_tag, desc_len);
            ...
            case 0x48:
                ...
                if (name) {
                    AVEMProgram *program = av_em_new_program(ts->stream, sid);
                    if (program) {
                        av_em_dict_set(&program->metadata, "service_name", name, 0);
                        av_em_dict_set(&program->metadata, "service_provider",
                                    provider_name, 0);
                    }
                }
                av_em_free(name);
                av_em_free(provider_name);
                break;
            default:
                break;
            }
...
static int handle_packet(MpegTSContext *ts, const uint8_t *packet)
{
    ...
    tss->last_cc = cc;
    if (!cc_ok) {
        av_em_log(ts->stream, AV_LOG_DEBUG,
        ...
        if (ts->stream->ctx_flags & AVFMTCTX_NOHEADER && ts->scan_all_pmts <= 0) {
            ...
            if (i == ts->nb_prg && ts->nb_prg > 0) {
                ...
                for (i = 0; i < ts->stream->nb_streams; i++) {
                    AVEMStream *st = ts->stream->streams[i];
                    types |= 1<<st->codecpar->codec_type;
                }
                if ((types & (1<<AVMEDIA_TYPE_AUDIO) && types & (1<<AVMEDIA_TYPE_VIDEO)) || pos > 100000) {
                    av_em_log(ts->stream, AV_LOG_DEBUG, "All programs have pmt, headers found\n");
...
static void reanalyze(MpegTSContext *ts) {
    ...
    if (ts->size_stat_count > SIZE_STAT_THRESHOLD) {
        ...
        if (newsize && newsize != ts->raw_packet_size) {
            av_em_log(ts->stream, AV_LOG_WARNING, "changing packet size to %d\n", newsize);
 ...
 * get_packet_size() ?) */
static int mpegts_resync(AVEMFormatContext *s, int seekback, const uint8_t *current_packet)
{
    ...
    avio_em_seek(pb, -FFMIN(seekback, pos), SEEK_CUR);

    //Special case for files like 01c56b0dc1.ts
    if (current_packet[0] == 0x80 && current_packet[12] == 0x47) {
        avio_em_seek(pb, 12, SEEK_CUR);
        return 0;
    }
...
static int read_packet(AVEMFormatContext *s, uint8_t *buf, int raw_packet_size,
                       const uint8_t **data)
{
    AVEMIOContext *pb = s->pb;
    int len;
    ...
    for (;;) {
        len = emio_read_indirect(pb, buf, TS_PACKET_SIZE, data);
...
static void finished_reading_packet(AVEMFormatContext *s, int raw_packet_size)
{
    AVEMIOContext *pb = s->pb;
...
static int handle_packets(MpegTSContext *ts, int64_t nb_packets)
{
    ...
    if (avio_em_tell(s->pb) != ts->last_pos) {
        int i;
        av_em_log(ts->stream, AV_LOG_TRACE, "Skipping after seek\n");
    ...
    ts->last_pos = avio_em_tell(s->pb);
    return ret;
}

static int mpegts_probe(AVEMProbeData *p)
{
...
static void seek_back(AVEMFormatContext *s, AVEMIOContext *pb, int64_t pos) {

    /* NOTE: We attempt to seek on non-seekable files as well, as the
     * probe buffer usually is big enough. Only warn if the seek failed
     * on files where the seek should work. */
    if (avio_em_seek(pb, pos, SEEK_SET) < 0)
        av_em_log(s, pb->seekable ? AV_LOG_ERROR : AV_LOG_INFO, "Unable to seek back to the start\n");
}

static int mpegts_read_header(AVEMFormatContext *s)
{
    MpegTSContext *ts = s->priv_data;
    AVEMIOContext *pb   = s->pb;
    ...
    if (emio_ensure_seekback(pb, probesize) < 0)
        av_em_log(s, AV_LOG_WARNING, "Failed to allocate buffers for seekback\n");

    /* read the first 8192 bytes to get packet size */
    pos = avio_em_tell(pb);
    len = avio_em_read(pb, buf, sizeof(buf));
    ts->raw_packet_size = get_packet_size(buf, len);
    if (ts->raw_packet_size <= 0) {
        av_em_log(s, AV_LOG_WARNING, "Could not detect TS packet size, defaulting to non-FEC/DVHS\n");
        ts->raw_packet_size = TS_PACKET_SIZE;
    }
    ts->stream     = s;
    ts->auto_guess = 0;

    if (s->iformat == &em_mpegts_demuxer) {
        /* normal demux */
        ...
        av_em_log(ts->stream, AV_LOG_TRACE, "tuning done\n");
        ...
        if (!st)
            return AVERROR(ENOMEM);
        avpriv_em_set_pts_info(st, 60, 1, 27000000);
        st->codecpar->codec_type = AVMEDIA_TYPE_DATA;
        ...
        av_em_log(ts->stream, AV_LOG_TRACE, "start=%0.3f pcr=%0.3f incr=%d\n",
...
static int mpegts_raw_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MpegTSContext *ts = s->priv_data;
    int ret, i;
    ...
    if (av_em_new_packet(pkt, TS_PACKET_SIZE) < 0)
        return AVERROR(ENOMEM);
    ...
    if (ts->mpeg2ts_compute_pcr) {
        ...
        if (parse_pcr(&pcr_h, &pcr_l, pkt->data) == 0) {
            ...
            for (i = 0; i < MAX_PACKET_READAHEAD; i++) {
                avio_em_seek(s->pb, pos + i * ts->raw_packet_size, SEEK_SET);
                avio_em_read(s->pb, pcr_buf, 12);
...
static int mpegts_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MpegTSContext *ts = s->priv_data;
    int ret, i;
...
static int mpegts_read_close(AVEMFormatContext *s)
{
    MpegTSContext *ts = s->priv_data;
    mpegts_free(ts);
    return 0;
...
static av_unused int64_t mpegts_get_pcr(AVEMFormatContext *s, int stream_index,
                              int64_t *ppos, int64_t pos_limit)
{
    ...
    while(pos < pos_limit) {
        if (avio_em_seek(s->pb, pos, SEEK_SET) < 0)
            return AV_NOPTS_VALUE;
        if (avio_em_read(s->pb, buf, TS_PACKET_SIZE) != TS_PACKET_SIZE)
            return AV_NOPTS_VALUE;
        if (buf[0] != 0x47) {
...
static int64_t mpegts_get_dts(AVEMFormatContext *s, int stream_index,
                              int64_t *ppos, int64_t pos_limit)
{
    MpegTSContext *ts = s->priv_data;
    int64_t pos;
    ...
    while(pos < pos_limit) {
        ...
        AVEMPacket pkt;
        av_em_init_packet(&pkt);
        ret = av_em_read_frame(s, &pkt);
        if (ret < 0)
            return AV_NOPTS_VALUE;
        if (pkt.dts != AV_NOPTS_VALUE && pkt.pos >= 0) {
            em_reduce_index(s, pkt.stream_index);
            av_em_add_index_entry(s->streams[pkt.stream_index], pkt.pos, pkt.dts, 0, 0, AVINDEX_KEYFRAME /* FIXME keyframe? */);
...
MpegTSContext *avpriv_mpegts_parse_open(AVEMFormatContext *s)
{
    MpegTSContext *ts;

    ts = av_em_mallocz(sizeof(MpegTSContext));
    if (!ts)
        return NULL;
 ...
 * packet is output */
int avpriv_mpegts_parse_packet(MpegTSContext *ts, AVEMPacket *pkt,
...
AVEMInputFormat em_mpegts_demuxer = {
    .name           = "mpegts",
...
AVEMInputFormat ff_mpegtsraw_demuxer = {
    .name           = "mpegtsraw",
