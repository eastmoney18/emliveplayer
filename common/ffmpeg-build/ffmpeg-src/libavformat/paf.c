...
static int read_close(AVEMFormatContext *s)
{
    ...
    av_em_freep(&p->blocks_count_table);
    av_em_freep(&p->frames_offset_table);
    av_em_freep(&p->blocks_offset_table);
    av_em_freep(&p->video_frame);
    av_em_freep(&p->audio_frame);
    av_em_freep(&p->temp_audio_frame);

    return 0;
}

static void read_table(AVEMFormatContext *s, uint32_t *table, uint32_t count)
{
    int i;

    for (i = 0; i < count; i++)
        table[i] = avio_em_rl32(s->pb);

    avio_em_skip(s->pb, 4 * (FFALIGN(count, 512) - count));
}
...
static int read_header(AVEMFormatContext *s)
{
    ...
    avpriv_em_set_pts_info(vst, 64, 1, 10);
    ...
    avpriv_em_set_pts_info(ast, 64, 1, 22050);

    p->buffer_size    = avio_em_rl32(pb);
    p->preload_count  = avio_em_rl32(pb);
    p->frame_blks     = avio_em_rl32(pb);
    p->start_offset   = avio_em_rl32(pb);
    p->max_video_blks = avio_em_rl32(pb);
    p->max_audio_blks = avio_em_rl32(pb);
    ...
    p->blocks_count_table  = av_em_mallocz(p->nb_frames *
                                        sizeof(*p->blocks_count_table));
    p->frames_offset_table = av_em_mallocz(p->nb_frames *
                                        sizeof(*p->frames_offset_table));
    p->blocks_offset_table = av_em_mallocz(p->frame_blks *
                                        sizeof(*p->blocks_offset_table));

    p->video_size  = p->max_video_blks * p->buffer_size;
    p->video_frame = av_em_mallocz(p->video_size);

    p->audio_size       = p->max_audio_blks * p->buffer_size;
    p->audio_frame      = av_em_mallocz(p->audio_size);
    p->temp_audio_frame = av_em_mallocz(p->audio_size);
    ...
    avio_em_seek(pb, p->buffer_size, SEEK_SET);
    ...
    avio_em_seek(pb, p->start_offset, SEEK_SET);
...
static int read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if (p->got_audio) {
        if (av_em_new_packet(pkt, p->audio_size) < 0)
            return AVERROR(ENOMEM);
    ...
    for (i = 0; i < count; i++) {
        ...
        if (p->blocks_offset_table[p->current_frame_block] & (1U << 31)) {
            ...
            avio_em_read(pb, p->audio_frame + offset, p->buffer_size);
            ...
            avio_em_read(pb, p->video_frame + offset, p->buffer_size);
        }
...
AVEMInputFormat ff_paf_demuxer = {
    .name           = "paf",
