...
static int get_vpx_color_space(AVEMFormatContext *s,
                               enum AVColorSpace color_space)
{
    ...
    default:
        av_em_log(s, AV_LOG_ERROR, "Unsupported color space (%d)\n", color_space);
        return -1;
    }
}
...
static int get_vpx_chroma_subsampling(AVEMFormatContext *s,
                                      enum AVPixelFormat pixel_format,
                                      enum AVChromaLocation chroma_location)
{
    int chroma_w, chroma_h;
    if (av_em_pix_fmt_get_chroma_sub_sample(pixel_format, &chroma_w, &chroma_h) == 0) {
    ...
    av_em_log(s, AV_LOG_ERROR, "Unsupported pixel format (%d)\n", pixel_format);
    return -1;
}

static int get_bit_depth(AVEMFormatContext *s, enum AVPixelFormat pixel_format)
{
    const AVPixFmtDescriptor *desc = av_em_pix_fmt_desc_get(pixel_format);
    if (desc == NULL) {
        av_em_log(s, AV_LOG_ERROR, "Unsupported pixel format (%d)\n",
               pixel_format);
        return -1;
...
int ff_isom_write_vpcc(AVEMFormatContext *s, AVEMIOContext *pb,
                       AVEMCodecParameters *par)
{
    ...
    avio_em_w8(pb, profile);
    avio_em_w8(pb, level);
    avio_em_w8(pb, (bit_depth << 4) | vpx_color_space);
    avio_em_w8(pb, (vpx_chroma_subsampling << 4) | (vpx_transfer_function << 1) |
