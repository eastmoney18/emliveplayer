...
static double get_duration(AVEMFormatContext *s)
{
    int i = 0;
    double max = 0.0;
    for (i = 0; i < s->nb_streams; i++) {
        AVEMDictionaryEntry *duration = av_em_dict_get(s->streams[i]->metadata,
                                                  DURATION, NULL, 0);
...
static int write_header(AVEMFormatContext *s)
{
    ...
    avio_em_printf(s->pb, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
    avio_em_printf(s->pb, "<MPD\n");
    avio_em_printf(s->pb, "  xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"\n");
    avio_em_printf(s->pb, "  xmlns=\"urn:mpeg:DASH:schema:MPD:2011\"\n");
    avio_em_printf(s->pb, "  xsi:schemaLocation=\"urn:mpeg:DASH:schema:MPD:2011\"\n");
    avio_em_printf(s->pb, "  type=\"%s\"\n", w->is_live ? "dynamic" : "static");
    if (!w->is_live) {
        avio_em_printf(s->pb, "  mediaPresentationDuration=\"PT%gS\"\n",
                    get_duration(s));
    }
    avio_em_printf(s->pb, "  minBufferTime=\"PT%gS\"\n", min_buffer_time);
    avio_em_printf(s->pb, "  profiles=\"%s\"%s",
    ...
    if (w->is_live) {
        ...
        if (w->debug_mode) {
            av_em_strlcpy(gmt_iso, "", 1);
        }
        avio_em_printf(s->pb, "  availabilityStartTime=\"%s\"\n", gmt_iso);
        avio_em_printf(s->pb, "  timeShiftBufferDepth=\"PT%gS\"\n", w->time_shift_buffer_depth);
        avio_em_printf(s->pb, "  minimumUpdatePeriod=\"PT%dS\"", w->minimum_update_period);
        avio_em_printf(s->pb, ">\n");
        if (w->utc_timing_url) {
            avio_em_printf(s->pb, "<UTCTiming\n");
            avio_em_printf(s->pb, "  schemeIdUri=\"urn:mpeg:dash:utc:http-iso:2014\"\n");
            avio_em_printf(s->pb, "  value=\"%s\"/>\n", w->utc_timing_url);
        }
    }
    return 0;
}

static void write_footer(AVEMFormatContext *s)
{
    avio_em_printf(s->pb, "</MPD>\n");
}

static int subsegment_alignment(AVEMFormatContext *s, AdaptationSet *as) {
    int i;
    AVEMDictionaryEntry *gold = av_em_dict_get(s->streams[as->streams[0]]->metadata,
                                          CUE_TIMESTAMPS, NULL, 0);
    if (!gold) return 0;
    for (i = 1; i < as->nb_streams; i++) {
        AVEMDictionaryEntry *ts = av_em_dict_get(s->streams[as->streams[i]]->metadata,
                                            CUE_TIMESTAMPS, NULL, 0);
...
static int bitstream_switching(AVEMFormatContext *s, AdaptationSet *as) {
    int i;
    AVEMDictionaryEntry *gold_track_num = av_em_dict_get(s->streams[as->streams[0]]->metadata,
                                                    TRACK_NUMBER, NULL, 0);
    AVEMCodecParameters *gold_par = s->streams[as->streams[0]]->codecpar;
    if (!gold_track_num) return 0;
    for (i = 1; i < as->nb_streams; i++) {
        AVEMDictionaryEntry *track_num = av_em_dict_get(s->streams[as->streams[i]]->metadata,
                                                   TRACK_NUMBER, NULL, 0);
        AVEMCodecParameters *par = s->streams[as->streams[i]]->codecpar;
        if (!track_num ||
 ...
 * < 0 on failure.
 */
static int write_representation(AVEMFormatContext *s, AVEMStream *stream, char *id,
                                int output_width, int output_height,
                                int output_sample_rate) {
    WebMDashMuxContext *w = s->priv_data;
    AVEMDictionaryEntry *irange = av_em_dict_get(stream->metadata, INITIALIZATION_RANGE, NULL, 0);
    AVEMDictionaryEntry *cues_start = av_em_dict_get(stream->metadata, CUES_START, NULL, 0);
    AVEMDictionaryEntry *cues_end = av_em_dict_get(stream->metadata, CUES_END, NULL, 0);
    AVEMDictionaryEntry *filename = av_em_dict_get(stream->metadata, FILENAME, NULL, 0);
    AVEMDictionaryEntry *bandwidth = av_em_dict_get(stream->metadata, BANDWIDTH, NULL, 0);
    if ((w->is_live && (!filename)) ||
        (!w->is_live && (!irange || !cues_start || !cues_end || !filename || !bandwidth))) {
        return AVERROR_INVALIDDATA;
    }
    avio_em_printf(s->pb, "<Representation id=\"%s\"", id);
    // FIXME: For live, This should be obtained from the input file or as an AVOption.
    avio_em_printf(s->pb, " bandwidth=\"%s\"",
    ...
    if (stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO && output_width)
        avio_em_printf(s->pb, " width=\"%d\"", stream->codecpar->width);
    if (stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO && output_height)
        avio_em_printf(s->pb, " height=\"%d\"", stream->codecpar->height);
    if (stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO && output_sample_rate)
        avio_em_printf(s->pb, " audioSamplingRate=\"%d\"", stream->codecpar->sample_rate);
    if (w->is_live) {
        // For live streams, Codec and Mime Type always go in the Representation tag.
        avio_em_printf(s->pb, " codecs=\"%s\"", get_codec_name(stream->codecpar->codec_id));
        avio_em_printf(s->pb, " mimeType=\"%s/webm\"",
        ...
        // is always 1.
        avio_em_printf(s->pb, " startsWithSAP=\"1\"");
        avio_em_printf(s->pb, ">");
    } else {
        avio_em_printf(s->pb, ">\n");
        avio_em_printf(s->pb, "<BaseURL>%s</BaseURL>\n", filename->value);
        avio_em_printf(s->pb, "<SegmentBase\n");
        avio_em_printf(s->pb, "  indexRange=\"%s-%s\">\n", cues_start->value, cues_end->value);
        avio_em_printf(s->pb, "<Initialization\n");
        avio_em_printf(s->pb, "  range=\"0-%s\" />\n", irange->value);
        avio_em_printf(s->pb, "</SegmentBase>\n");
    }
    avio_em_printf(s->pb, "</Representation>\n");
    return 0;
}
 ...
 */
static int check_matching_width(AVEMFormatContext *s, AdaptationSet *as) {
    int first_width, i;
 ...
 */
static int check_matching_height(AVEMFormatContext *s, AdaptationSet *as) {
    int first_height, i;
    if (as->nb_streams < 2) return 1;
 ...
 */
static int check_matching_sample_rate(AVEMFormatContext *s, AdaptationSet *as) {
...
static void free_adaptation_sets(AVEMFormatContext *s) {
    WebMDashMuxContext *w = s->priv_data;
    int i;
    for (i = 0; i < w->nb_as; i++) {
        av_em_freep(&w->as[i].streams);
    }
    av_em_freep(&w->as);
    w->nb_as = 0;
}
...
static int parse_filename(char *filename, char **representation_id,
    ...
    char *temp_pos = NULL;
    char *filename_str = av_em_strdup(filename);
    if (!filename_str) return AVERROR(ENOMEM);
    temp_pos = av_em_stristr(filename_str, "_");
    while (temp_pos) {
        underscore_pos = temp_pos + 1;
        temp_pos = av_em_stristr(temp_pos + 1, "_");
    }
    if (!underscore_pos) return AVERROR_INVALIDDATA;
    period_pos = av_em_stristr(underscore_pos, ".");
    if (!period_pos) return AVERROR_INVALIDDATA;
    *(underscore_pos - 1) = 0;
    if (representation_id) {
        *representation_id = av_em_alloc(period_pos - underscore_pos + 1);
        if (!(*representation_id)) return AVERROR(ENOMEM);
        av_em_strlcpy(*representation_id, underscore_pos, period_pos - underscore_pos + 1);
    }
    if (initialization_pattern) {
        *initialization_pattern = av_em_asprintf("%s_$RepresentationID$.hdr",
                                              filename_str);
        if (!(*initialization_pattern)) return AVERROR(ENOMEM);
    }
    if (media_pattern) {
        *media_pattern = av_em_asprintf("%s_$RepresentationID$_$Number$.chk",
                                     filename_str);
        if (!(*media_pattern)) return AVERROR(ENOMEM);
    }
    av_em_free(filename_str);
    return 0;
}
 ...
 */
static int write_adaptation_set(AVEMFormatContext *s, int as_index)
{
    WebMDashMuxContext *w = s->priv_data;
    AdaptationSet *as = &w->as[as_index];
    AVEMCodecParameters *par = s->streams[as->streams[0]]->codecpar;
    AVEMDictionaryEntry *lang;
    int i;
    ...
    avio_em_printf(s->pb, "<AdaptationSet id=\"%s\"", as->id);
    avio_em_printf(s->pb, " mimeType=\"%s/webm\"",
                par->codec_type == AVMEDIA_TYPE_VIDEO ? "video" : "audio");
    avio_em_printf(s->pb, " codecs=\"%s\"", get_codec_name(par->codec_id));

    lang = av_em_dict_get(s->streams[as->streams[0]]->metadata, "language", NULL, 0);
    if (lang) avio_em_printf(s->pb, " lang=\"%s\"", lang->value);

    if (par->codec_type == AVMEDIA_TYPE_VIDEO && width_in_as)
        avio_em_printf(s->pb, " width=\"%d\"", par->width);
    if (par->codec_type == AVMEDIA_TYPE_VIDEO && height_in_as)
        avio_em_printf(s->pb, " height=\"%d\"", par->height);
    if (par->codec_type == AVMEDIA_TYPE_AUDIO && sample_rate_in_as)
        avio_em_printf(s->pb, " audioSamplingRate=\"%d\"", par->sample_rate);

    avio_em_printf(s->pb, " bitstreamSwitching=\"%s\"",
                boolean[bitstream_switching(s, as)]);
    avio_em_printf(s->pb, " subsegmentAlignment=\"%s\"",
                boolean[w->is_live || subsegment_alignment(s, as)]);

    for (i = 0; i < as->nb_streams; i++) {
        AVEMDictionaryEntry *kf = av_em_dict_get(s->streams[as->streams[i]]->metadata,
                                            CLUSTER_KEYFRAME, NULL, 0);
        if (!w->is_live && (!kf || !strncmp(kf->value, "0", 1))) subsegmentStartsWithSAP = 0;
    }
    avio_em_printf(s->pb, " subsegmentStartsWithSAP=\"%d\"", subsegmentStartsWithSAP);
    avio_em_printf(s->pb, ">\n");

    if (w->is_live) {
        AVEMDictionaryEntry *filename =
            av_em_dict_get(s->streams[as->streams[0]]->metadata, FILENAME, NULL, 0);
        char *initialization_pattern = NULL;
        char *media_pattern = NULL;
        int ret = parse_filename(filename->value, NULL, &initialization_pattern,
                                 &media_pattern);
        if (ret) return ret;
        avio_em_printf(s->pb, "<ContentComponent id=\"1\" type=\"%s\"/>\n",
                    par->codec_type == AVMEDIA_TYPE_VIDEO ? "video" : "audio");
        avio_em_printf(s->pb, "<SegmentTemplate");
        avio_em_printf(s->pb, " timescale=\"1000\"");
        avio_em_printf(s->pb, " duration=\"%d\"", w->chunk_duration);
        avio_em_printf(s->pb, " media=\"%s\"", media_pattern);
        avio_em_printf(s->pb, " startNumber=\"%d\"", w->chunk_start_index);
        avio_em_printf(s->pb, " initialization=\"%s\"", initialization_pattern);
        avio_em_printf(s->pb, "/>\n");
        av_em_free(initialization_pattern);
        av_em_free(media_pattern);
    }
    ...
    for (i = 0; i < as->nb_streams; i++) {
        ...
        if (w->is_live) {
            AVEMDictionaryEntry *filename =
                av_em_dict_get(s->streams[as->streams[i]]->metadata, FILENAME, NULL, 0);
            if (!filename)
                return AVERROR(EINVAL);
            if (ret = parse_filename(filename->value, &representation_id, NULL, NULL))
                return ret;
        } else {
            representation_id = av_em_asprintf("%d", w->representation_id++);
            if (!representation_id) return AVERROR(ENOMEM);
        }
        ret = write_representation(s, s->streams[as->streams[i]],
                                   representation_id, !width_in_as,
                                   !height_in_as, !sample_rate_in_as);
        av_em_free(representation_id);
        if (ret) return ret;
    }
    avio_em_printf(s->pb, "</AdaptationSet>\n");
    return 0;
}
...
{
    int ret;
    char *q = av_em_alloc(sizeof(char) * len);
    if (!q)
        return AVERROR(ENOMEM);
    av_em_strlcpy(q, p, len);
    ret = atoi(q);
    av_em_free(q);
    return ret;
}

static int parse_adaptation_sets(AVEMFormatContext *s)
{
    WebMDashMuxContext *w = s->priv_data;
    ...
    while (p < w->adaptation_sets + strlen(w->adaptation_sets)) {
        if (*p == ' ')
            ...
            void *mem = av_em_realloc(w->as, sizeof(*w->as) * (w->nb_as + 1));
            if (mem == NULL)
                return AVERROR(ENOMEM);
            w->as = mem;
            ++w->nb_as;
            ...
            *q = 0;
            ...
            q = p;
            while (*q != '\0' && *q != ',' && *q != ' ') q++;
            as->streams = av_em_realloc(as->streams, sizeof(*as->streams) * ++as->nb_streams);
            if (as->streams == NULL)
                return AVERROR(ENOMEM);
...
static int webm_dash_manifest_write_header(AVEMFormatContext *s)
{
    int i;
    double start = 0.0;
    int ret;
    ...
    avio_em_printf(s->pb, "<Period id=\"0\"");
    avio_em_printf(s->pb, " start=\"PT%gS\"", start);
    if (!w->is_live) {
        avio_em_printf(s->pb, " duration=\"PT%gS\"", get_duration(s));
    }
    avio_em_printf(s->pb, " >\n");
...
static int webm_dash_manifest_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    return AVERROR_EOF;
}

static int webm_dash_manifest_write_trailer(AVEMFormatContext *s)
{
    free_adaptation_sets(s);
    return 0;
}
...
AVEMOutputFormat ff_webm_dash_manifest_muxer = {
