 ...
 * by calling av_em_read_play/pause().
 */
 ...
 *
 * @todo Use AVEMIOContext instead of EMURLContext
 */
typedef struct RTSPState {
    const AVEMClass *class;             /**< Class for private options. */
    EMURLContext *rtsp_hd; /* RTSP TCP connection handle */
    ...
    /** the seek value requested when calling av_em_seek_frame(). This value
     ...
     * other cases, this is a copy of AVEMFormatContext->filename. */
...
#define RTSP_FLAG_CUSTOM_IO   0x4    /**< Do all IO via the AVEMIOContext. */
 ...
 * AVEMStreams. In this case, each AVEMStream in this set has similar content
 * (but different codec/bitrate).
 */
typedef struct RTSPStream {
    EMURLContext *rtp_handle;   /**< RTP stream handle (if UDP) */
    void *transport_priv; /**< RTP/RDT parse context if input, RTP AVEMFormatContext if output */
 ...
 */
int ff_rtsp_send_cmd(AVEMFormatContext *s, const char *method,
                     const char *url, const char *headers,
 ...
 */
int ff_rtsp_setup_output_streams(AVEMFormatContext *s, const char *addr);
 ...
 * within the AVEMFormatContext; also allocate the RTP streams and the
 ...
 */
int ff_rtsp_tcp_write_packet(AVEMFormatContext *s, RTSPStream *rtsp_st);

/**
 * Receive one packet from the RTSPStreams set up in the AVEMFormatContext
