...
static av_cold int amr_init(AVEMFormatContext *s, int st_index, PayloadContext *data)
{
    data->channels = 1;
    return 0;
}

static int amr_handle_packet(AVEMFormatContext *ctx, PayloadContext *data,
                             ...
                             int flags)
{
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_AMR_NB) {
        ...
        av_em_log(ctx, AV_LOG_ERROR, "Bad codec ID\n");
        return AVERROR_INVALIDDATA;
    }

    if (st->codecpar->channels != 1) {
        av_em_log(ctx, AV_LOG_ERROR, "Only mono AMR is supported\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (1 + frames >= len) {
        ...
        av_em_log(ctx, AV_LOG_ERROR, "No speech data found\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    for (i = 0; i < frames; i++) {
        ...
        if (speech_data + frame_size > buf + len) {
            ...
            av_em_log(ctx, AV_LOG_WARNING, "Too little speech data in the RTP packet\n");
    ...
    if (speech_data < buf + len) {
        av_em_log(ctx, AV_LOG_WARNING, "Too much speech data in the RTP packet?\n");
        /* Set the unwritten part of the packet to zero. */
        memset(ptr, 0, pkt->data + pkt->size - ptr);
        pkt->size = ptr - pkt->data;
    }
...
static int amr_parse_fmtp(AVEMFormatContext *s,
                          ...
                          const char *attr, const char *value)
{
     ...
     */
    if (!strcmp(value, "")) {
        av_em_log(s, AV_LOG_WARNING, "AMR fmtp attribute %s had "
...
static int amr_parse_sdp_line(AVEMFormatContext *s, int st_index,
                              PayloadContext *data, const char *line)
{
    ...
    if (av_em_strstart(line, "fmtp:", &p)) {
        ...
        if (!data->octet_align || data->crc ||
            ...
            av_em_log(s, AV_LOG_ERROR, "Unsupported RTP/AMR configuration!\n");
            return -1;
        }
        return ret;
    }
