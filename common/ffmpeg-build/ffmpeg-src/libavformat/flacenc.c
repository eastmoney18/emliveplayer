...
static int flac_write_block_padding(AVEMIOContext *pb, unsigned int n_padding_bytes,
                                    int last_block)
{
    avio_em_w8(pb, last_block ? 0x81 : 0x01);
    avio_em_wb24(pb, n_padding_bytes);
    emio_fill(pb, 0, n_padding_bytes);
    return 0;
}

static int flac_write_block_comment(AVEMIOContext *pb, AVEMDictionary **m,
...
static int flac_write_header(struct AVEMFormatContext *s)
{
    int ret;
    ...
    if (par->channel_layout &&
        ...
        AVEMDictionaryEntry *chmask = av_em_dict_get(s->metadata, "WAVEFORMATEXTENSIBLE_CHANNEL_MASK",
                                                NULL, 0);

        if (chmask) {
            av_em_log(s, AV_LOG_WARNING, "A WAVEFORMATEXTENSIBLE_CHANNEL_MASK is "
                   "already present, this muxer will not overwrite it.\n");
        } else {
            uint8_t buf[32];
            snprintf(buf, sizeof(buf), "0x%"PRIx64, par->channel_layout);
            av_em_dict_set(&s->metadata, "WAVEFORMATEXTENSIBLE_CHANNEL_MASK", buf, 0);
        }
    }
...
static int flac_write_trailer(struct AVEMFormatContext *s)
{
    ...
    if (pb->seekable) {
        ...
        avio_em_write(pb, streaminfo, FLAC_STREAMINFO_SIZE);
        avio_em_seek(pb, file_size, SEEK_SET);
        avio_em_flush(pb);
    } else {
        av_em_log(s, AV_LOG_WARNING, "unable to rewrite FLAC header.\n");
    }

    av_em_freep(&c->streaminfo);

    return 0;
}

static int flac_write_packet(struct AVEMFormatContext *s, AVEMPacket *pkt)
{
    FlacMuxerContext *c = s->priv_data;
    uint8_t *streaminfo;
    int streaminfo_size;
    ...
    streaminfo = av_em_packet_get_side_data(pkt, AV_PKT_DATA_NEW_EXTRADATA,
                                         &streaminfo_size);
    if (streaminfo && streaminfo_size == FLAC_STREAMINFO_SIZE) {
        av_em_freep(&c->streaminfo);

        c->streaminfo = av_em_alloc(FLAC_STREAMINFO_SIZE);
        if (!c->streaminfo)
            return AVERROR(ENOMEM);
...
static const AVEMClass flac_muxer_class = {
...
AVEMOutputFormat ff_flac_muxer = {
    .name              = "flac",
