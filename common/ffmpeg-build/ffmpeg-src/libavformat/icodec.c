...
static int read_header(AVEMFormatContext *s)
{
    ...
    avio_em_skip(pb, 4);
    ico->nb_images = avio_em_rl16(pb);

    ico->images = av_em_malloc_array(ico->nb_images, sizeof(IcoImage));
    if (!ico->images)
    ...
    for (i = 0; i < ico->nb_images; i++) {
        ...
        if (avio_em_seek(pb, 6 + i * 16, SEEK_SET) < 0)
            break;
        ...
        st->codecpar->width      = avio_em_r8(pb);
        st->codecpar->height     = avio_em_r8(pb);
        ico->images[i].nb_pal = avio_em_r8(pb);
        if (ico->images[i].nb_pal == 255)
            ico->images[i].nb_pal = 0;

        avio_em_skip(pb, 5);

        ico->images[i].size   = avio_em_rl32(pb);
        ico->images[i].offset = avio_em_rl32(pb);

        if (avio_em_seek(pb, ico->images[i].offset, SEEK_SET) < 0)
            break;

        codec = avio_em_rl32(pb);
        switch (codec) {
...
static int read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if ((ret = avio_em_seek(pb, image->offset, SEEK_SET)) < 0)
        return ret;

    if (s->streams[ico->current_image]->codecpar->codec_id == AV_CODEC_ID_PNG) {
        if ((ret = av_em_get_packet(pb, pkt, image->size)) < 0)
            return ret;
    } else {
        uint8_t *buf;
        if ((ret = av_em_new_packet(pkt, 14 + image->size)) < 0)
            return ret;
        buf = pkt->data;
        ...
        if ((ret = avio_em_read(pb, buf, image->size)) < 0)
            return ret;
...
AVEMInputFormat ff_ico_demuxer = {
    .name           = "ico",
