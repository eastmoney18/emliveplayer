...
static av_cold int wv_write_header(AVEMFormatContext *ctx)
{
    if (ctx->nb_streams > 1 ||
        ctx->streams[0]->codecpar->codec_id != AV_CODEC_ID_WAVPACK) {
        av_em_log(ctx, AV_LOG_ERROR, "This muxer only supports a single WavPack stream.\n");
...
static int wv_write_packet(AVEMFormatContext *ctx, AVEMPacket *pkt)
{
    ...
    if (pkt->size < WV_HEADER_SIZE ||
        ...
        av_em_log(ctx, AV_LOG_ERROR, "Invalid WavPack packet.\n");
        return AVERROR(EINVAL);
    }
    s->samples += header.samples;

    avio_em_write(ctx->pb, pkt->data, pkt->size);

    return 0;
}

static av_cold int wv_write_trailer(AVEMFormatContext *ctx)
{
    WvMuxContext *s = ctx->priv_data;
    ...
    if (ctx->pb->seekable && s->samples &&
        ...
        int64_t pos = avio_em_tell(ctx->pb);
        avio_em_seek(ctx->pb, 12, SEEK_SET);
        avio_em_wl32(ctx->pb, s->samples);
        avio_em_seek(ctx->pb, pos, SEEK_SET);
    }
