...
static void rtmp_log(int level, const char *fmt, va_list args)
{
    ...
    av_em_vlog(NULL, level, fmt, args);
    av_em_log(NULL, level, "\n");
}

static int rtmp_close(EMURLContext *s)
{
    LibRTMPContext *ctx = s->priv_data;
    RTMP *r = &ctx->rtmp;

    RTMP_Close(r);
    av_em_freep(&ctx->temp_filename);
    return 0;
}
...
static int rtmp_open(EMURLContext *s, const char *uri, int flags)
{
    ...
    if (!(ctx->temp_filename = filename = av_em_alloc(len)))
        return AVERROR(ENOMEM);

    av_em_strlcpy(filename, s->filename, len);
    if (ctx->app) {
        av_em_strlcat(filename, " app=", len);
        av_em_strlcat(filename, ctx->app, len);
    }
    if (ctx->tcurl) {
        av_em_strlcat(filename, " tcUrl=", len);
        av_em_strlcat(filename, ctx->tcurl, len);
    }
    if (ctx->pageurl) {
        av_em_strlcat(filename, " pageUrl=", len);
        av_em_strlcat(filename, ctx->pageurl, len);
    }
    if (ctx->swfurl) {
        av_em_strlcat(filename, " swfUrl=", len);
        av_em_strlcat(filename, ctx->swfurl, len);
    }
    if (ctx->flashver) {
        av_em_strlcat(filename, " flashVer=", len);
        av_em_strlcat(filename, ctx->flashver, len);
    }
    if (ctx->conn) {
        char *sep, *p = ctx->conn;
        while (p) {
            av_em_strlcat(filename, " conn=", len);
            p += strspn(p, " ");
            if (!*p)
                break;
            sep = strchr(p, ' ');
    ...
    if (ctx->playpath) {
        av_em_strlcat(filename, " playpath=", len);
        av_em_strlcat(filename, ctx->playpath, len);
    }
    if (ctx->live)
        av_em_strlcat(filename, " live=1", len);
    if (ctx->subscribe) {
        av_em_strlcat(filename, " subscribe=", len);
        av_em_strlcat(filename, ctx->subscribe, len);
    }
    if (ctx->client_buffer_time) {
        av_em_strlcat(filename, " buffer=", len);
        av_em_strlcat(filename, ctx->client_buffer_time, len);
    }
    if (ctx->swfurl || ctx->swfverify) {
        av_em_strlcat(filename, " swfUrl=", len);

        if (ctx->swfverify) {
            av_em_strlcat(filename, ctx->swfverify, len);
            av_em_strlcat(filename, " swfVfy=1", len);
        } else {
            av_em_strlcat(filename, ctx->swfurl, len);
        }
    }
...
static int rtmp_read_pause(EMURLContext *s, int pause)
{
    LibRTMPContext *ctx = s->priv_data;
    RTMP *r = &ctx->rtmp;
...
static int64_t rtmp_read_seek(EMURLContext *s, int stream_index,
                              int64_t timestamp, int flags)
{
    ...
    if (stream_index < 0)
        timestamp = av_em_rescale_rnd(timestamp, 1000, AV_TIME_BASE,
...
static int rtmp_get_file_handle(EMURLContext *s)
{
    LibRTMPContext *ctx = s->priv_data;
    RTMP *r = &ctx->rtmp;
...
static const AVEMClass lib ## flavor ## _class = {\
...
RTMP_CLASS(rtmp)
const EMURLProtocol em_librtmp_protocol = {
    .name                = "rtmp",
    .url_open            = rtmp_open,
    .url_read            = rtmp_read,
...
RTMP_CLASS(rtmpt)
const EMURLProtocol em_librtmpt_protocol = {
    .name                = "rtmpt",
    .url_open            = rtmp_open,
    .url_read            = rtmp_read,
    .url_write           = rtmp_write,
...
RTMP_CLASS(rtmpe)
const EMURLProtocol em_librtmpe_protocol = {
    .name                = "rtmpe",
    .url_open            = rtmp_open,
    .url_read            = rtmp_read,
    .url_write           = rtmp_write,
...
RTMP_CLASS(rtmpte)
const EMURLProtocol em_librtmpte_protocol = {
    .name                = "rtmpte",
    .url_open            = rtmp_open,
    .url_read            = rtmp_read,
    .url_write           = rtmp_write,
...
RTMP_CLASS(rtmps)
const EMURLProtocol em_librtmps_protocol = {
    .name                = "rtmps",
    .url_open            = rtmp_open,
    .url_read            = rtmp_read,
    .url_write           = rtmp_write,
