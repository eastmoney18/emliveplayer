...
void ff_text_init_avio(void *s, FFTextReader *r, AVEMIOContext *pb)
{
    int i;
    r->pb = pb;
    ...
    r->type = FF_UTF_8;
    for (i = 0; i < 2; i++)
        r->buf[r->buf_len++] = avio_em_r8(r->pb);
...
void ff_text_init_buf(FFTextReader *r, void *buf, size_t size)
{
    ...
    emio_init_context(&r->buf_pb, buf, size, 0, NULL, NULL, NULL, NULL);
    ff_text_init_avio(NULL, r, &r->buf_pb);
}

int64_t ff_text_pos(FFTextReader *r)
{
    return avio_em_tell(r->pb) - r->buf_len + r->buf_pos;
}
...
int ff_text_r8(FFTextReader *r)
{
    ...
    if (r->type == FF_UTF16LE) {
        GET_UTF16(val, avio_em_rl16(r->pb), return 0;)
    } else if (r->type == FF_UTF16BE) {
        GET_UTF16(val, avio_em_rb16(r->pb), return 0;)
    } else {
        return avio_em_r8(r->pb);
    }
    if (!val)
        return 0;
    r->buf_pos = 0;
...
{
    return r->buf_pos >= r->buf_len && avio_em_feof(r->pb);
}
...
AVEMPacket *ff_subtitles_queue_insert(FFDemuxSubtitlesQueue *q,
                                    const uint8_t *event, size_t len, int merge)
{
    ...
    if (merge && q->nb_subs > 0) {
        ...
        old_len = sub->size;
        if (av_em_grow_packet(sub, len) < 0)
            return NULL;
        ...
        if (q->nb_subs >= INT_MAX/sizeof(*q->subs) - 1)
            return NULL;
        subs = av_em_fast_realloc(q->subs, &q->allocated_size,
                               (q->nb_subs + 1) * sizeof(*q->subs));
        if (!subs)
            return NULL;
        q->subs = subs;
        sub = &subs[q->nb_subs++];
        if (av_em_new_packet(sub, len) < 0)
            return NULL;
...
{
    const AVEMPacket *s1 = a;
    const AVEMPacket *s2 = b;
    if (s1->pts == s2->pts)
...
static void drop_dups(void *log_ctx, FFDemuxSubtitlesQueue *q)
{
    ...
    for (i = 1; i < q->nb_subs; i++) {
        ...
        const AVEMPacket *last = &q->subs[last_id];
    ...
    if (drop) {
        q->nb_subs -= drop;
        av_em_log(log_ctx, AV_LOG_WARNING, "Dropping %d duplicated subtitle events\n", drop);
    }
}
...
int ff_subtitles_queue_read_packet(FFDemuxSubtitlesQueue *q, AVEMPacket *pkt)
{
    AVEMPacket *sub = q->subs + q->current_sub_idx;

    if (q->current_sub_idx == q->nb_subs)
        return AVERROR_EOF;
    if (av_em_copy_packet(pkt, sub) < 0) {
...
int ff_subtitles_queue_seek(FFDemuxSubtitlesQueue *q, AVEMFormatContext *s, int stream_index,
...
int ff_smil_extract_next_text_chunk(FFTextReader *tr, AVEMBPrint *buf, char *c)
{
    int i = 0;
    char end_chr;
    ...
    end_chr = *c == '<' ? '>' : '<';
    do {
        av_em_bprint_chars(buf, *c, 1);
        *c = ff_text_r8(tr);
        i++;
    } while (*c != end_chr && *c);
    if (end_chr == '>') {
        av_em_bprint_chars(buf, '>', 1);
        *c = 0;
    }
    return i;
}
...
const char *ff_smil_get_attr_ptr(const char *s, const char *attr)
{
    ...
    while (*s) {
        ...
        while (av_isspace(*s))
            s++;
        if (!av_em_strncasecmp(s, attr, len) && s[len] == '=')
...
void ff_subtitles_read_text_chunk(FFTextReader *tr, AVEMBPrint *buf)
{
...
void ff_subtitles_read_chunk(AVEMIOContext *pb, AVEMBPrint *buf)
{
    FFTextReader tr;
