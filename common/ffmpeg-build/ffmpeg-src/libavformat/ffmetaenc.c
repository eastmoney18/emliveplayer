...
static void write_escape_str(AVEMIOContext *s, const uint8_t *str)
{
    const uint8_t *p = str;
...
static void write_tags(AVEMIOContext *s, AVEMDictionary *m)
{
...
{
    avio_em_write(s->pb, ID_STRING, sizeof(ID_STRING) - 1);
    avio_em_w8(s->pb, '1');          // version
    avio_em_w8(s->pb, '\n');
    avio_em_flush(s->pb);
    return 0;
}
...
static int write_trailer(AVEMFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        avio_em_write(s->pb, ID_STREAM, sizeof(ID_STREAM) - 1);
        avio_em_w8(s->pb, '\n');
    ...
    for (i = 0; i < s->nb_chapters; i++) {
        ...
        avio_em_write(s->pb, ID_CHAPTER, sizeof(ID_CHAPTER) - 1);
        avio_em_w8(s->pb, '\n');
        avio_em_printf(s->pb, "TIMEBASE=%d/%d\n", ch->time_base.num, ch->time_base.den);
        avio_em_printf(s->pb, "START=%"PRId64"\n", ch->start);
        avio_em_printf(s->pb, "END=%"PRId64"\n",   ch->end);
...
AVEMOutputFormat ff_ffmetadata_muxer = {
    .name          = "ffmetadata",
