...
static int mtaf_probe(AVEMProbeData *p)
{
    if (p->buf_size < 0x44)
        return 0;
...
static int mtaf_read_header(AVEMFormatContext *s)
{
    int stream_count;
    AVEMStream *st;
    ...
    avio_em_skip(s->pb, 0x5c);
    st->duration = avio_em_rl32(s->pb);
    avio_em_skip(s->pb, 1);
    stream_count = avio_em_r8(s->pb);
    if (!stream_count)
    ...
    avio_em_seek(s->pb, 0x800, SEEK_SET);

    return 0;
}

static int mtaf_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
...
AVEMInputFormat ff_mtaf_demuxer = {
    .name           = "mtaf",
