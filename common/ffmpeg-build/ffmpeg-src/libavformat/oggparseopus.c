...
static int opus_header(AVEMFormatContext *avf, int idx)
{
    struct ogg *ogg              = avf->priv_data;
    struct ogg_stream *os        = &ogg->streams[idx];
    AVEMStream *st                 = avf->streams[idx];
    struct oggopus_private *priv = os->private;
    uint8_t *packet              = os->buf + os->pstart;

    if (!priv) {
        priv = os->private = av_em_mallocz(sizeof(*priv));
        if (!priv)
    ...
    if (os->flags & OGG_FLAG_BOS) {
        ...
        if (em_alloc_extradata(st->codecpar, os->psize))
            return AVERROR(ENOMEM);

        memcpy(st->codecpar->extradata, packet, os->psize);

        st->codecpar->sample_rate = 48000;
        st->codecpar->seek_preroll = av_em_rescale(OPUS_SEEK_PREROLL_MS,
...
static int opus_packet(AVEMFormatContext *avf, int idx)
{
    struct ogg *ogg              = avf->priv_data;
    struct ogg_stream *os        = &ogg->streams[idx];
    AVEMStream *st                 = avf->streams[idx];
    ...
    if (os->granule > (1LL << 62)) {
        av_em_log(avf, AV_LOG_ERROR, "Unsupported huge granule pos %"PRId64 "\n", os->granule);
        return AVERROR_INVALIDDATA;
    }
