...
static int mpjpeg_write_header(AVEMFormatContext *s)
{
...
static int mpjpeg_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MPJPEGContext *mpj = s->priv_data;
    avio_em_printf(s->pb, "Content-type: image/jpeg\r\n");
    avio_em_printf(s->pb, "Content-length: %d\r\n\r\n",
                pkt->size);
...
static int mpjpeg_write_trailer(AVEMFormatContext *s)
{
    return 0;
}
...
static const AVEMClass mpjpeg_muxer_class = {
...
AVEMOutputFormat ff_mpjpeg_muxer = {
    .name              = "mpjpeg",
