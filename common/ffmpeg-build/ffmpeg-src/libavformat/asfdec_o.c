...
typedef struct GUIDParseTable {
    const char *name;
    ff_asf_guid guid;
    int (*read_object)(AVEMFormatContext *, const struct GUIDParseTable *);
    int is_subobject;
} GUIDParseTable;
...
typedef struct ASFStream {
    ...
    int index;  // stream index in AVEMFormatContext, set in asf_read_stream_properties
    int type;
...
typedef struct ASFStreamData{
    char langs[32];
    AVEMDictionary *asf_met; // for storing per-stream metadata
...
static int detect_unknown_subobject(AVEMFormatContext *s, int64_t offset, int64_t size);
static const GUIDParseTable *find_guid(ff_asf_guid guid);

static int asf_probe(AVEMProbeData *pd)
{
    /* check file header */
...
static void align_position(AVEMIOContext *pb,  int64_t offset, uint64_t size)
{
    if (size < INT64_MAX - offset && avio_em_tell(pb) != offset + size)
        avio_em_seek(pb, offset + size, SEEK_SET);
}

static int asf_read_unknown(AVEMFormatContext *s, const GUIDParseTable *g)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    uint64_t size   = avio_em_rl64(pb);
    int ret;
    ...
    if (!g->is_subobject) {
        if (!(ret = strcmp(g->name, "Header Extension")))
            avio_em_skip(pb, 22); // skip reserved fields and Data Size
        if ((ret = detect_unknown_subobject(s, asf->unknown_offset,
                                            asf->unknown_size)) < 0)
            return ret;
    } else {
        if (size < 24) {
            av_em_log(s, AV_LOG_ERROR, "Too small size %"PRIu64" (< 24).\n", size);
            return AVERROR_INVALIDDATA;
        }
        avio_em_skip(pb, size - 24);
    }

    return 0;
}

static int get_asf_string(AVEMIOContext *pb, int maxlen, char *buf, int buflen)
{
    char *q = buf;
    int ret = 0;
    if (buflen <= 0)
    ...
    while (ret + 1 < maxlen) {
        uint8_t tmp;
        uint32_t ch;
        GET_UTF16(ch, (ret += 2) <= maxlen ? avio_em_rl16(pb) : 0, break;);
...
static int asf_read_marker(AVEMFormatContext *s, const GUIDParseTable *g)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    uint64_t size   = avio_em_rl64(pb);
    ...
    avio_em_skip(pb, 8);
    avio_em_skip(pb, 8); // skip reserved GUID
    nb_markers = avio_em_rl32(pb);
    avio_em_skip(pb, 2); // skip reserved field
    len = avio_em_rl16(pb);
    for (i = 0; i < len; i++)
        avio_em_skip(pb, 1);
    ...
    for (i = 0; i < nb_markers; i++) {
        ...
        avio_em_skip(pb, 2); // entry length
        avio_em_skip(pb, 4); // send time
        avio_em_skip(pb, 4); // flags
        len = avio_em_rl32(pb);

        if ((ret = avio_em_get_str16le(pb, len, name,
                                    sizeof(name))) < len)
            avio_em_skip(pb, len - ret);
        avpriv_em_new_chapter(s, i, (AVEMRational) { 1, 10000000 }, pts,
                           AV_NOPTS_VALUE, name);
    }
...
static int asf_read_metadata(AVEMFormatContext *s, const char *title, uint16_t len,
                             unsigned char *ch, uint16_t buflen)
{
    AVEMIOContext *pb = s->pb;

    avio_em_get_str16le(pb, len, ch, buflen);
    if (ch[0]) {
        if (av_em_dict_set(&s->metadata, title, ch, 0) < 0)
            av_em_log(s, AV_LOG_WARNING, "av_em_dict_set failed.\n");
    }

    return 0;
}

static int asf_read_value(AVEMFormatContext *s, const uint8_t *name,
                          uint16_t val_len, int type, AVEMDictionary **met)
{
    int ret;
    uint8_t *value;
    ...
    if (type == ASF_UNICODE) {
        ...
        if ((ret = get_asf_string(pb, val_len, value, buflen)) < 0)
            goto failed;
        if (av_em_dict_set(met, name, value, 0) < 0)
            av_em_log(s, AV_LOG_WARNING, "av_em_dict_set failed.\n");
    } else {
        char buf[256];
        ...
        if ((ret = avio_em_read(pb, value, val_len)) < 0)
            goto failed;
        if (ret < 2 * val_len)
            value[ret] = '\0';
        else
        ...
        if (av_em_dict_set(met, name, buf, 0) < 0)
            av_em_log(s, AV_LOG_WARNING, "av_em_dict_set failed.\n");
    }
    av_em_freep(&value);
...
static int asf_read_generic_value(AVEMIOContext *pb, int type, uint64_t *value)
{

    switch (type) {
    case ASF_BOOL:
        *value = avio_em_rl16(pb);
        break;
    case ASF_DWORD:
        *value = avio_em_rl32(pb);
        break;
    case ASF_QWORD:
        *value = avio_em_rl64(pb);
        break;
    case ASF_WORD:
        *value = avio_em_rl16(pb);
        break;
...
static int asf_set_metadata(AVEMFormatContext *s, const uint8_t *name,
                            int type, AVEMDictionary **met)
{
    AVEMIOContext *pb = s->pb;
    uint64_t value;
    char buf[32];
...
static int asf_read_picture(AVEMFormatContext *s, int len)
{
    ASFContext *asf       = s->priv_data;
    AVEMPacket pkt          = { 0 };
    ...
    /* picture type */
    type = avio_em_r8(s->pb);
    len--;
    if (type >= FF_ARRAY_ELEMS(ff_id3v2_picture_types) || type < 0) {
        av_em_log(s, AV_LOG_WARNING, "Unknown attached picture type: %d.\n", type);
        type = 0;
    }

    /* picture data size */
    picsize = avio_em_rl32(s->pb);
    len    -= 4;

    /* picture MIME type */
    len -= avio_em_get_str16le(s->pb, len, mimetype, sizeof(mimetype));
    ...
    if (picsize >= len) {
        av_em_log(s, AV_LOG_ERROR, "Invalid attached picture data size: %d >= %d.\n",
               picsize, len);
        return AVERROR_INVALIDDATA;
    }
    ...
    desc     = av_em_alloc(desc_len);
    if (!desc)
        return AVERROR(ENOMEM);
    len -= avio_em_get_str16le(s->pb, len - picsize, desc, desc_len);

    ret = av_em_get_packet(s->pb, &pkt, picsize);
    if (ret < 0)
        goto fail;
    ...
    asf->asf_st[asf->nb_streams] = av_em_mallocz(sizeof(*asf_st));
    ...
    if (*desc) {
        if (av_em_dict_set(&st->metadata, "title", desc, AV_DICT_DONT_STRDUP_VAL) < 0)
            av_em_log(s, AV_LOG_WARNING, "av_em_dict_set failed.\n");
    } else
        av_em_freep(&desc);

    if (av_em_dict_set(&st->metadata, "comment", ff_id3v2_picture_types[type], 0) < 0)
        av_em_log(s, AV_LOG_WARNING, "av_em_dict_set failed.\n");
...
static void get_id3_tag(AVEMFormatContext *s, int len)
{
    ID3v2ExtraMeta *id3v2_extra_meta = NULL;

    em_id3v2_read(s, ID3v2_DEFAULT_MAGIC, &id3v2_extra_meta, len);
    if (id3v2_extra_meta)
        ff_id3v2_parse_apic(s, &id3v2_extra_meta);
    ff_id3v2_free_extra_meta(&id3v2_extra_meta);
}

static int process_metadata(AVEMFormatContext *s, const uint8_t *name, uint16_t name_len,
                            uint16_t val_len, uint16_t type, AVEMDictionary **met)
{
    int ret;
    ff_asf_guid guid;
...
static int asf_read_ext_content(AVEMFormatContext *s, const GUIDParseTable *g)
{
    ASFContext *asf  = s->priv_data;
    AVEMIOContext *pb  = s->pb;
    uint64_t size    = avio_em_rl64(pb);
    uint16_t nb_desc = avio_em_rl16(pb);
    int i, ret;
    ...
    for (i = 0; i < nb_desc; i++) {
        ...
        name_len = avio_em_rl16(pb);
        if (!name_len)
            return AVERROR_INVALIDDATA;
        name = av_em_alloc(name_len);
        if (!name)
            return AVERROR(ENOMEM);
        avio_em_get_str16le(pb, name_len, name,
                         name_len);
        type    = avio_em_rl16(pb);
        // BOOL values are 16 bits long in the Metadata Object
        // but 32 bits long in the Extended Content Description Object
        if (type == ASF_BOOL)
            type = ASF_DWORD;
        val_len = avio_em_rl16(pb);
...
static AVEMStream *find_stream(AVEMFormatContext *s, uint16_t st_num)
{
    AVEMStream *st = NULL;
    ASFContext *asf = s->priv_data;
    int i;
...
static int asf_store_aspect_ratio(AVEMFormatContext *s, uint8_t st_num, uint8_t *name, int type)
{
    ASFContext *asf   = s->priv_data;
    AVEMIOContext *pb   = s->pb;
    uint64_t value = 0;
...
static int asf_read_metadata_obj(AVEMFormatContext *s, const GUIDParseTable *g)
{
    ASFContext *asf   = s->priv_data;
    AVEMIOContext *pb   = s->pb;
    uint64_t size     = avio_em_rl64(pb);
    uint16_t nb_recs  = avio_em_rl16(pb); // number of records in the Description Records list
    int i, ret;
    ...
    for (i = 0; i < nb_recs; i++) {
        ...
        avio_em_skip(pb, 2); // skip reserved field
        st_num   = avio_em_rl16(pb);
        name_len = avio_em_rl16(pb);
        buflen   = 2 * name_len + 1;
        if (!name_len)
            break;
        type     = avio_em_rl16(pb);
        val_len  = avio_em_rl32(pb);
        name     = av_em_alloc(buflen);
        if (!name)
            return AVERROR(ENOMEM);
        avio_em_get_str16le(pb, name_len, name,
                         buflen);
...
static int asf_read_content_desc(AVEMFormatContext *s, const GUIDParseTable *g)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int i;
...
static int asf_read_properties(AVEMFormatContext *s, const GUIDParseTable *g)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    time_t creation_time;

    avio_em_rl64(pb); // read object size
    avio_em_skip(pb, 16); // skip File ID
    avio_em_skip(pb, 8);  // skip File size
    creation_time = avio_em_rl64(pb);
    if (!(asf->b_flags & ASF_FLAG_BROADCAST)) {
        ...
        if (tm) {
            ...
            buf[0] = '\0';
        if (buf[0]) {
            if (av_em_dict_set(&s->metadata, "creation_time", buf, 0) < 0)
                av_em_log(s, AV_LOG_WARNING, "av_em_dict_set failed.\n");
        }
    }
    asf->nb_packets  = avio_em_rl64(pb);
    asf->duration    = avio_em_rl64(pb) / 10000; // stream duration
    avio_em_skip(pb, 8); // skip send duration
    asf->preroll     = avio_em_rl64(pb);
    asf->duration   -= asf->preroll;
    asf->b_flags     = avio_em_rl32(pb);
    avio_em_skip(pb, 4); // skip minimal packet size
    asf->packet_size  = avio_em_rl32(pb);
    avio_em_skip(pb, 4); // skip max_bitrate

    return 0;
}

static int parse_video_info(AVEMIOContext *pb, AVEMStream *st)
{
    uint16_t size;
    unsigned int tag;

    st->codecpar->width  = avio_em_rl32(pb);
    st->codecpar->height = avio_em_rl32(pb);
    avio_em_skip(pb, 1); // skip reserved flags
    size = avio_em_rl16(pb); // size of the Format Data
    tag  = ff_get_bmp_header(pb, st, NULL);
    st->codecpar->codec_tag = tag;
    st->codecpar->codec_id  = em_codec_get_id(em_codec_bmp_tags, tag);

    if (size > BMP_HEADER_SIZE) {
        int ret;
        st->codecpar->extradata_size  = size - BMP_HEADER_SIZE;
        if (!(st->codecpar->extradata = av_em_alloc(st->codecpar->extradata_size +
        ...
        memset(st->codecpar->extradata + st->codecpar->extradata_size , 0,
               AV_INPUT_BUFFER_PADDING_SIZE);
        if ((ret = avio_em_read(pb, st->codecpar->extradata,
                             st->codecpar->extradata_size)) < 0)
            return ret;
    }
    return 0;
...
static int asf_read_stream_properties(AVEMFormatContext *s, const GUIDParseTable *g)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    uint64_t size;
    ...
    avio_em_skip(pb, 8); // skip the time offset
    ts_data_len      = avio_em_rl32(pb);
    err_data_len     = avio_em_rl32(pb);
    flags            = avio_em_rl16(pb); // bit 15 - Encrypted Content
    ...
    if (!st)
        return AVERROR(ENOMEM);
    avpriv_em_set_pts_info(st, 32, 1, 1000); // pts should be dword, in milliseconds
    ...
    av_em_init_packet(&asf_st->pkt.avpkt);
    asf_st->pkt.data_size        = 0;
    avio_em_skip(pb, 4); // skip reserved field
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        break;
    default:
        avio_em_skip(pb, ts_data_len);
        break;
    }

    if (err_data_len) {
        if (type == AVMEDIA_TYPE_AUDIO) {
            uint8_t span = avio_em_r8(pb);
            if (span > 1) {
                asf_st->span              = span;
                asf_st->virtual_pkt_len   = avio_em_rl16(pb);
                asf_st->virtual_chunk_len = avio_em_rl16(pb);
                if (!asf_st->virtual_chunk_len || !asf_st->virtual_pkt_len)
                    return AVERROR_INVALIDDATA;
                avio_em_skip(pb, err_data_len - 5);
            } else
                avio_em_skip(pb, err_data_len - 1);
        } else
            avio_em_skip(pb, err_data_len);
    }
...
static void set_language(AVEMFormatContext *s, const char *rfc1766, AVEMDictionary **met)
{
    ...
    if (rfc1766 && strlen(rfc1766) > 1) {
        ...
        if (iso6392)
            if (av_em_dict_set(met, "language", iso6392, 0) < 0)
                av_em_log(s, AV_LOG_WARNING, "av_em_dict_set failed.\n");
    }
}

static int asf_read_ext_stream_properties(AVEMFormatContext *s, const GUIDParseTable *g)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    AVEMStream *st    = NULL;
    ...
    start_time = avio_em_rl64(pb);
    end_time   = avio_em_rl64(pb);
    bitrate    = avio_em_rl32(pb);
    avio_em_skip(pb, 28); // skip some unused values
    st_num     = avio_em_rl16(pb);
    st_num    &= ASF_STREAM_NUM;
    lang_idx   = avio_em_rl16(pb); // Stream Language ID Index
    for (i = 0; i < asf->nb_streams; i++) {
    ...
    time_per_frame = avio_em_rl64(pb); // average time per frame
    if (st) {
    ...
    nb_st_name = avio_em_rl16(pb);
    nb_pay_exts   = avio_em_rl16(pb);
    for (i = 0; i < nb_st_name; i++) {
        uint16_t len;

        avio_em_rl16(pb); // Language ID Index
        len = avio_em_rl16(pb);
        avio_em_skip(pb, len);
    }
    ...
    for (i = 0; i < nb_pay_exts; i++) {
        uint32_t len;
        avio_em_skip(pb, 16); // Extension System ID
        avio_em_skip(pb, 2);  // Extension Data Size
        len = avio_em_rl32(pb);
        avio_em_skip(pb, len);
    }
...
static int asf_read_language_list(AVEMFormatContext *s, const GUIDParseTable *g)
{
    ASFContext *asf   = s->priv_data;
    AVEMIOContext *pb   = s->pb;
    int i, ret;
    uint64_t size     = avio_em_rl64(pb);
    uint16_t nb_langs = avio_em_rl16(pb);
...
static int asf_read_data(AVEMFormatContext *s, const GUIDParseTable *g)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    uint64_t size   = asf->data_size = avio_em_rl64(pb);
    int i;
    ...
    avio_em_skip(pb, 16); // skip File ID
    size = avio_em_rl64(pb); // Total Data Packets
    if (size != asf->nb_packets)
        av_em_log(s, AV_LOG_WARNING,
               ...
               size, asf->nb_packets);
    avio_em_skip(pb, 2); // skip reserved field
    asf->first_packet_offset = avio_em_tell(pb);
...
static int asf_read_simple_index(AVEMFormatContext *s, const GUIDParseTable *g)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    AVEMStream *st    = NULL;
    ...
    if (!st) {
        avio_em_skip(pb, size - 24); // if there's no video stream, skip index object
        return 0;
    }
    avio_em_skip(pb, 16); // skip File ID
    interval = avio_em_rl64(pb);
    avio_em_skip(pb, 4);
    nb_entries = avio_em_rl32(pb);
    for (i = 0; i < nb_entries; i++) {
        pkt_num = avio_em_rl32(pb);
        ret = avio_em_skip(pb, 2);
        if (ret < 0) {
            av_em_log(s, AV_LOG_ERROR, "Skipping failed in asf_read_simple_index.\n");
            return ret;
        }
        if (prev_pkt_num != pkt_num) {
            av_em_add_index_entry(st, asf->first_packet_offset + asf->packet_size *
                               pkt_num, av_em_rescale(interval, i, 10000),
...
static int asf_read_subpayload(AVEMFormatContext *s, AVEMPacket *pkt, int is_header)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    uint8_t sub_len;
    ...
    if (is_header) {
        asf->dts_delta = avio_em_r8(pb);
        if (asf->nb_mult_left) {
            asf->mult_sub_len = avio_em_rl16(pb); // total
        }
        asf->sub_header_offset = avio_em_tell(pb);
        asf->nb_sub = 0;
        asf->sub_left = 1;
    }
    sub_len = avio_em_r8(pb);
    if ((ret = av_em_get_packet(pb, pkt, sub_len)) < 0) // each subpayload is entire frame
        return ret;
    for (i = 0; i < asf->nb_streams; i++) {
    ...
    if (asf->nb_mult_left && (avio_em_tell(pb) >=
                              (asf->sub_header_offset + asf->mult_sub_len))) {
        asf->sub_left = 0;
        asf->nb_mult_left--;
    }
    if (avio_em_tell(pb) >= asf->packet_offset + asf->packet_size - asf->pad_len) {
        asf->sub_left = 0;
        if (!asf->nb_mult_left) {
            avio_em_skip(pb, asf->pad_len);
            if (avio_em_tell(pb) != asf->packet_offset + asf->packet_size) {
                if (!asf->packet_size)
                    return AVERROR_INVALIDDATA;
                av_em_log(s, AV_LOG_WARNING,
                       "Position %"PRId64" wrong, should be %"PRId64"\n",
                       avio_em_tell(pb), asf->packet_offset + asf->packet_size);
                avio_em_seek(pb, asf->packet_offset + asf->packet_size, SEEK_SET);
            }
        }
    }
...
static void reset_packet(ASFPacket *asf_pkt)
{
    ...
    asf_pkt->dts       = 0;
    asf_pkt->duration  = 0;
    av_em_packet_unref(&asf_pkt->avpkt);
    av_em_init_packet(&asf_pkt->avpkt);
}

static int asf_read_replicated_data(AVEMFormatContext *s, ASFPacket *asf_pkt)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int ret, data_size;
    ...
    if (!asf_pkt->data_size) {
        data_size = avio_em_rl32(pb); // read media object size
        if (data_size <= 0)
            return AVERROR_INVALIDDATA;
        if ((ret = av_em_new_packet(&asf_pkt->avpkt, data_size)) < 0)
            return ret;
        asf_pkt->data_size = asf_pkt->size_left = data_size;
    } else
        avio_em_skip(pb, 4); // reading of media object size is already done
    asf_pkt->dts = avio_em_rl32(pb); // read presentation time
    if (asf->rep_data_len && (asf->rep_data_len >= 8))
        avio_em_skip(pb, asf->rep_data_len - 8); // skip replicated data

    return 0;
}

static int asf_read_multiple_payload(AVEMFormatContext *s, AVEMPacket *pkt,
                                 ASFPacket *asf_pkt)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    if (asf->rep_data_len == 1) {
        ...
        if (asf->rep_data_len)
            if ((ret = asf_read_replicated_data(s, asf_pkt)) < 0)
                return ret;
        pay_len = avio_em_rl16(pb); // payload length should be WORD
        if (pay_len > asf->packet_size) {
            av_em_log(s, AV_LOG_ERROR,
                   ...
                   pay_len, asf->packet_size, avio_em_tell(pb));
            return AVERROR_INVALIDDATA;
        }
        ...
        if (asf_pkt->size_left <= 0)
            return AVERROR_INVALIDDATA;
        if ((ret = avio_em_read(pb, p, pay_len)) < 0)
            return ret;
        if (s->key && s->keylen == 20)
            ff_asfcrypt_dec(s->key, p, ret);
        avio_em_skip(pb, skip);
...
static int asf_read_single_payload(AVEMFormatContext *s, ASFPacket *asf_pkt)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int64_t  offset;
    ...
    if (!asf_pkt->data_size) {
        ...
        avio_em_skip(pb, 4); // skip media object size
    asf_pkt->dts = avio_em_rl32(pb); // read presentation time
    if (asf->rep_data_len >= 8)
        avio_em_skip(pb, asf->rep_data_len - 8); // skip replicated data
    offset = avio_em_tell(pb);
    ...
    if (asf_pkt->size_left > size)
        ...
        asf_pkt->size_left = 0;
    if ((ret = avio_em_read(pb, p, size)) < 0)
        return ret;
    if (s->key && s->keylen == 20)
            ff_asfcrypt_dec(s->key, p, ret);
    if (asf->packet_size_internal)
        avio_em_skip(pb, asf->packet_size - asf->packet_size_internal);
    avio_em_skip(pb, asf->pad_len); // skip padding

    return 0;
}

static int asf_read_payload(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int ret, i;
    ...
    if (!asf->sub_left) {
        ...
        if (!asf_pkt) {
            if (asf->packet_offset + asf->packet_size <= asf->data_offset + asf->data_size) {
                if (!asf->packet_size) {
                    av_em_log(s, AV_LOG_ERROR, "Invalid packet size 0.\n");
                    return AVERROR_INVALIDDATA;
                }
                avio_em_seek(pb, asf->packet_offset + asf->packet_size, SEEK_SET);
                av_em_log(s, AV_LOG_WARNING, "Skipping the stream with the invalid stream index %d.\n",
                       asf->stream_index);
        ...
        if (asf_pkt->size_left && (asf_pkt->frame_num != media_len)) {
            av_em_log(s, AV_LOG_WARNING, "Unfinished frame will be ignored\n");
...
static int asf_read_packet_header(AVEMFormatContext *s)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    uint64_t size;
    ...
    asf->packet_offset = avio_em_tell(pb);
    error_flags = avio_em_r8(pb); // read Error Correction Flags
    if (error_flags & ASF_PACKET_FLAG_ERROR_CORRECTION_PRESENT) {
        if (!(error_flags & ASF_ERROR_CORRECTION_LENGTH_TYPE)) {
            size = error_flags & ASF_PACKET_ERROR_CORRECTION_DATA_SIZE;
            avio_em_skip(pb, size);
        }
        len_flags       = avio_em_r8(pb);
    } else
        len_flags = error_flags;
    asf->prop_flags = avio_em_r8(pb);
    ...
    asf->send_time = avio_em_rl32(pb); // send time
    avio_em_skip(pb, 2); // skip duration
    if (len_flags & ASF_PPI_FLAG_MULTIPLE_PAYLOADS_PRESENT) { // Multiple Payloads present
        pay_flags = avio_em_r8(pb);
...
static int asf_deinterleave(AVEMFormatContext *s, ASFPacket *asf_pkt, int st_num)
{
    ASFContext *asf    = s->priv_data;
    ...
    if ((ret = av_em_new_packet(&pkt, asf_pkt->data_size)) < 0)
        return ret;
...
static int asf_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ASFContext *asf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int ret, i;

    if ((avio_em_tell(pb) >= asf->data_offset + asf->data_size) &&
        ...
        case READ_MULTI_SUB:
            if (!asf->sub_left && !asf->nb_mult_left) {
                ...
                if (!asf->return_subpayload &&
                    (avio_em_tell(pb) <= asf->packet_offset +
                     asf->packet_size - asf->pad_len))
                    avio_em_skip(pb, asf->pad_len); // skip padding
                if (asf->packet_offset + asf->packet_size > avio_em_tell(pb))
                    avio_em_seek(pb, asf->packet_offset + asf->packet_size, SEEK_SET);
            ...
            if (asf_pkt && !asf_pkt->size_left && asf_pkt->data_size) {
                if (asf->asf_st[i]->span > 1 &&
                    ...
                    if ((ret = asf_deinterleave(s, asf_pkt, i)) < 0)
                        return ret;
                av_em_packet_move_ref(pkt, &asf_pkt->avpkt);
                pkt->stream_index  = asf->asf_st[i]->index;
                pkt->flags         = asf_pkt->flags;
...
static int asf_read_close(AVEMFormatContext *s)
{
    ASFContext *asf = s->priv_data;
    int i;

    for (i = 0; i < ASF_MAX_STREAMS; i++) {
        av_em_dict_free(&asf->asf_sd[i].asf_met);
        if (i < asf->nb_streams) {
            av_em_packet_unref(&asf->asf_st[i]->pkt.avpkt);
            av_em_freep(&asf->asf_st[i]);
        }
    }
...
static void reset_packet_state(AVEMFormatContext *s)
{
    ASFContext *asf        = s->priv_data;
    int i;
    ...
    for (i = 0; i < asf->nb_streams; i++) {
        ...
        pkt->dts       = 0;
        pkt->duration  = 0;
        av_em_packet_unref(&pkt->avpkt);
        av_em_init_packet(&pkt->avpkt);
    }
}
 ...
 */
static int64_t asf_read_timestamp(AVEMFormatContext *s, int stream_index,
                                  int64_t *pos, int64_t pos_limit)
{
    ...
    avio_em_seek(s->pb, pkt_pos, SEEK_SET);
    pkt_offset = pkt_pos;

    reset_packet_state(s);
    while (avio_em_tell(s->pb) < data_end) {

        int i, ret, st_found;

        av_em_init_packet(&pkt);
        pkt_offset = avio_em_tell(s->pb);
        ...
        for (i = 0; i < asf->nb_streams; i++) {
            ...
            if (pkt.flags & AV_PKT_FLAG_KEY) {
                dts = pkt.dts;
                if (dts) {
                    av_em_add_index_entry(s->streams[pkt.stream_index], pkt_pos,
...
static int asf_read_seek(AVEMFormatContext *s, int stream_index,
                         int64_t timestamp, int flags)
{
    ...
    if (s->streams[stream_index]->nb_index_entries && asf->is_simple_index) {
        idx = av_em_index_search_timestamp(s->streams[stream_index], timestamp, flags);
        if (idx < 0 || idx >= s->streams[stream_index]->nb_index_entries)
            return AVERROR_INVALIDDATA;
        avio_em_seek(s->pb, s->streams[stream_index]->index_entries[idx].pos, SEEK_SET);
    } else {
        if ((ret = em_seek_frame_binary(s, stream_index, timestamp, flags)) < 0)
            return ret;
    }
...
static int detect_unknown_subobject(AVEMFormatContext *s, int64_t offset, int64_t size)
{
    ...
    while (avio_em_tell(pb) <= offset + size) {
        if (avio_em_tell(pb) == asf->offset)
            break;
        asf->offset = avio_em_tell(pb);
        if ((ret = ff_get_guid(pb, &guid)) < 0)
            return ret;
        g = find_guid(guid);
        if (g) {
...
static int asf_read_header(AVEMFormatContext *s)
{
    ASFContext *asf         = s->priv_data;
    AVEMIOContext *pb         = s->pb;
    const GUIDParseTable *g = NULL;
    ...
    if (ff_guidcmp(&guid, &ff_asf_header))
        return AVERROR_INVALIDDATA;
    avio_em_skip(pb, 8); // skip header object size
    avio_em_skip(pb, 6); // skip number of header objects and 2 reserved bytes
    asf->data_reached = 0;
    ...
    if (!asf->data_reached) {
        av_em_log(s, AV_LOG_ERROR, "Data Object was not found.\n");
        ret = AVERROR_INVALIDDATA;
        goto failed;
    }
    if (pb->seekable)
        avio_em_seek(pb, asf->first_packet_offset, SEEK_SET);

    for (i = 0; i < asf->nb_streams; i++) {
        const char *rfc1766 = asf->asf_sd[asf->asf_st[i]->lang_idx].langs;
        AVEMStream *st        = s->streams[asf->asf_st[i]->index];
    ...
    for (i = 0; i < ASF_MAX_STREAMS; i++) {
        ...
        st = find_stream(s, i);
        if (st) {
            av_em_dict_copy(&st->metadata, asf->asf_sd[i].asf_met, AV_DICT_IGNORE_SUFFIX);
...
AVEMInputFormat ff_asf_o_demuxer = {
    .name           = "asf_o",
