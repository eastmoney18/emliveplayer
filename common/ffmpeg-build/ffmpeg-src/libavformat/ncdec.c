...
static int nc_probe(AVEMProbeData *probe_packet)
{
    int size;
...
static int nc_read_header(AVEMFormatContext *s)
{
...
static int nc_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int size;
    int ret;
    ...
    if (size == 0) {
        av_em_log(s, AV_LOG_DEBUG, "Next packet size is zero\n");
        return AVERROR(EAGAIN);
    }

    ret = av_em_get_packet(s->pb, pkt, size);
    if (ret != size) {
        if (ret > 0) av_em_packet_unref(pkt);
        return AVERROR(EIO);
    }
