...
static void ogg_update_checksum(AVEMFormatContext *s, AVEMIOContext *pb, int64_t crc_offset)
{
    int64_t pos = avio_em_tell(pb);
    uint32_t checksum = emio_get_checksum(pb);
    avio_em_seek(pb, crc_offset, SEEK_SET);
    avio_em_wb32(pb, checksum);
    avio_em_seek(pb, pos, SEEK_SET);
}

static int ogg_write_page(AVEMFormatContext *s, OGGPage *page, int extra_flags)
{
    ...
    ret = avio_em_open_dyn_buf(&pb);
    if (ret < 0)
        return ret;
    emio_init_checksum(pb, em_crc04C11DB7_update, 0);
    emio_wfourcc(pb, "OggS");
    avio_em_w8(pb, 0);
    avio_em_w8(pb, page->flags | extra_flags);
    avio_em_wl64(pb, page->granule);
    avio_em_wl32(pb, oggstream->serial_num);
    avio_em_wl32(pb, oggstream->page_counter++);
    crc_offset = avio_em_tell(pb);
    avio_em_wl32(pb, 0); // crc
    avio_em_w8(pb, page->segments_count);
    avio_em_write(pb, page->segments, page->segments_count);
    avio_em_write(pb, page->data, page->size);

    ogg_update_checksum(s, pb, crc_offset);
    avio_em_flush(pb);

    size = avio_em_close_dyn_buf(pb, &buf);
    if (size < 0)
        return size;

    avio_em_write(s->pb, buf, size);
    avio_em_flush(s->pb);
    av_em_free(buf);
...
static int ogg_compare_granule(AVEMFormatContext *s, OGGPage *next, OGGPage *page)
{
    AVEMStream *st2 = s->streams[next->stream_index];
    AVEMStream *st  = s->streams[page->stream_index];
    int64_t next_granule, cur_granule;

    if (next->granule == -1 || page->granule == -1)
        return 0;

    next_granule = av_em_rescale_q(ogg_granule_to_timestamp(st2->priv_data, next->granule),
                                st2->time_base, AV_TIME_BASE_Q);
    cur_granule  = av_em_rescale_q(ogg_granule_to_timestamp(st->priv_data, page->granule),
...
static int ogg_buffer_page(AVEMFormatContext *s, OGGStreamContext *oggstream)
{
    OGGContext *ogg = s->priv_data;
    OGGPageList **p = &ogg->page_list;
    OGGPageList *l = av_em_mallocz(sizeof(*l));
...
static int ogg_buffer_data(AVEMFormatContext *s, AVEMStream *st,
                           ...
                           int header)
{
    ...
    for (i = 0; i < total_segments; ) {
...

        {
            ...
            int64_t start = av_em_rescale_q(page->start_granule, st->time_base,
                                         AV_TIME_BASE_Q);
            int64_t next  = av_em_rescale_q(page->granule, st->time_base,
                                         AV_TIME_BASE_Q);
...
static uint8_t *ogg_write_vorbiscomment(int64_t offset, int bitexact,
                                        int *header_len, AVEMDictionary **m, int framing_bit)
{
    const char *vendor = bitexact ? "ffmpeg" : LIBAVFORMAT_IDENT;
    int64_t size;
    uint8_t *p, *p0;

    ff_em_metadata_conv(m, ff_vorbiscomment_metadata_conv, NULL);

    size = offset + ff_vorbiscomment_length(*m, vendor) + framing_bit;
    if (size > INT_MAX)
        return NULL;
    p = av_em_mallocz(size);
    if (!p)
        return NULL;
    p0 = p;
...
static int ogg_build_flac_headers(AVEMCodecParameters *par,
                                  ...
                                  AVEMDictionary **m)
{
    ...
    oggstream->header[0] = av_em_mallocz(51); // per ogg flac specs
...
static int ogg_build_speex_headers(AVEMCodecParameters *par,
                                   OGGStreamContext *oggstream, int bitexact,
                                   AVEMDictionary **m)
{
    uint8_t *p;
    ...
    p = av_em_mallocz(SPEEX_HEADER_SIZE);
    if (!p)
        return AVERROR(ENOMEM);
...
static int ogg_build_opus_headers(AVEMCodecParameters *par,
                                  OGGStreamContext *oggstream, int bitexact,
                                  AVEMDictionary **m)
{
    uint8_t *p;
    ...
    p = av_em_mallocz(par->extradata_size);
    if (!p)
        return AVERROR(ENOMEM);
    oggstream->header[0] = p;
...
static void ogg_write_pages(AVEMFormatContext *s, int flush)
{
    OGGContext *ogg = s->priv_data;
    OGGPageList *next, *p;
...
static int ogg_write_header(AVEMFormatContext *s)
{
    OGGContext *ogg = s->priv_data;
    ...
    if (ogg->pref_size)
        av_em_log(s, AV_LOG_WARNING, "The pagesize option is deprecated\n");
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (st->codecpar->codec_id != AV_CODEC_ID_VORBIS &&
            ...
            av_em_log(s, AV_LOG_ERROR, "Unsupported codec id in stream %d\n", i);
            return AVERROR(EINVAL);
        }

        if (!st->codecpar->extradata || !st->codecpar->extradata_size) {
            av_em_log(s, AV_LOG_ERROR, "No extradata present\n");
            return AVERROR_INVALIDDATA;
        }
        oggstream = av_em_mallocz(sizeof(*oggstream));
        if (!oggstream)
            return AVERROR(ENOMEM);
        ...
        if (!(s->flags & AVFMT_FLAG_BITEXACT))
            do {
                serial_num = av_em_get_random_seed();
                for (j = 0; j < i; j++) {
        ...
        av_em_dict_copy(&st->metadata, s->metadata, AV_DICT_DONT_OVERWRITE);
        ...
        if (st->codecpar->codec_id == AV_CODEC_ID_FLAC) {
            int err = ogg_build_flac_headers(st->codecpar, oggstream,
                                             ...
                                             &st->metadata);
            if (err) {
                av_em_log(s, AV_LOG_ERROR, "Error writing FLAC headers\n");
            ...
            int err = ogg_build_speex_headers(st->codecpar, oggstream,
                                              ...
                                              &st->metadata);
            if (err) {
                av_em_log(s, AV_LOG_ERROR, "Error writing Speex headers\n");
                av_em_freep(&st->priv_data);
                return err;
            }
            ...
            int err = ogg_build_opus_headers(st->codecpar, oggstream,
                                             ...
                                             &st->metadata);
            if (err) {
                av_em_log(s, AV_LOG_ERROR, "Error writing Opus headers\n");
                av_em_freep(&st->priv_data);
                return err;
            }
        } else {
            ...
            if (avpriv_split_xiph_headers(st->codecpar->extradata, st->codecpar->extradata_size,
                ...
                av_em_log(s, AV_LOG_ERROR, "Extradata corrupted\n");
                av_em_freep(&st->priv_data);
                return AVERROR_INVALIDDATA;
            }
            ...
            if (st->codecpar->codec_id == AV_CODEC_ID_THEORA) {
                ...
                av_em_log(s, AV_LOG_DEBUG, "theora kfgshift %d, vrev %d\n",
...
static int ogg_write_packet_internal(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_THEORA) {
        ...
        granule = pkt->pts + pkt->duration +
                  av_em_rescale_q(st->codecpar->initial_padding,
...
static int ogg_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    int i;
...
static int ogg_write_trailer(AVEMFormatContext *s)
{
    int i;
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (st->codecpar->codec_id == AV_CODEC_ID_FLAC ||
            st->codecpar->codec_id == AV_CODEC_ID_SPEEX ||
            st->codecpar->codec_id == AV_CODEC_ID_OPUS) {
            av_em_freep(&oggstream->header[0]);
        }
        av_em_freep(&oggstream->header[1]);
        av_em_freep(&st->priv_data);
    }
    return 0;
}
...
AVEMOutputFormat ff_opus_muxer = {
    .name              = "opus",
