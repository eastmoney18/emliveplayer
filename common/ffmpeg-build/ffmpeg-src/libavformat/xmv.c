...
typedef struct XMVAudioPacket {
    ...
    enum AVEMCodecID codec_id; ///< The codec ID of the compression scheme.
...
static int xmv_probe(AVEMProbeData *p)
{
...
static int xmv_read_close(AVEMFormatContext *s)
{
    XMVDemuxContext *xmv = s->priv_data;

    av_em_freep(&xmv->audio);

    return 0;
}

static int xmv_read_header(AVEMFormatContext *s)
{
    XMVDemuxContext *xmv = s->priv_data;
    ...
    avio_em_skip(pb, 4); /* Next packet size */

    this_packet_size = avio_em_rl32(pb);

    avio_em_skip(pb, 4); /* Max packet size */
    avio_em_skip(pb, 4); /* "xobX" */

    file_version = avio_em_rl32(pb);
    if ((file_version != 4) && (file_version != 2))
        avpriv_em_request_sample(s, "Uncommon version %"PRIu32"", file_version);

    /* Video tracks */

    xmv->video_width    = avio_em_rl32(pb);
    xmv->video_height   = avio_em_rl32(pb);
    xmv->video_duration = avio_em_rl32(pb);

    /* Audio tracks */

    xmv->audio_track_count = avio_em_rl16(pb);

    avio_em_skip(pb, 2); /* Unknown (padding?) */

    xmv->audio = av_em_mallocz_array(xmv->audio_track_count, sizeof(XMVAudioPacket));
    if (!xmv->audio) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    for (audio_track = 0; audio_track < xmv->audio_track_count; audio_track++) {
        ...
        packet->compression     = avio_em_rl16(pb);
        packet->channels        = avio_em_rl16(pb);
        packet->sample_rate     = avio_em_rl32(pb);
        packet->bits_per_sample = avio_em_rl16(pb);
        packet->flags           = avio_em_rl16(pb);
        ...
        packet->codec_id      = ff_wav_em_codec_get_id(packet->compression,
        ...
        if (packet->flags & XMV_AUDIO_ADPCM51)
            av_em_log(s, AV_LOG_WARNING, "Unsupported 5.1 ADPCM audio stream "
                                      "(0x%04X)\n", packet->flags);

        if (!packet->channels || !packet->sample_rate ||
             packet->channels >= UINT16_MAX / XMV_BLOCK_ALIGN_SIZE) {
            av_em_log(s, AV_LOG_ERROR, "Invalid parameters for audio track %"PRIu16".\n",
                   audio_track);
    ...
    xmv->next_packet_offset = avio_em_tell(pb);
...
static void xmv_read_extradata(uint8_t *extradata, AVEMIOContext *pb)
{
    /* Read the XMV extradata */

    uint32_t data = avio_em_rl32(pb);
...
static int xmv_process_packet_header(AVEMFormatContext *s)
{
    XMVDemuxContext *xmv = s->priv_data;
    AVEMIOContext     *pb  = s->pb;
    int ret;
    ...
    /* Next packet size */
    xmv->next_packet_size = avio_em_rl32(pb);

    /* Packet video header */

    if (avio_em_read(pb, data, 8) != 8)
        return AVERROR(EIO);
    ...
    for (audio_track = 0; audio_track < xmv->audio_track_count; audio_track++) {
        ...
        if (!packet->created) {
            ...
            avpriv_em_set_pts_info(ast, 32, packet->block_samples, packet->sample_rate);
    ...
    if (xmv->video.data_size > 0) {
        if (xmv->video.has_extradata) {
            ...
            if (xmv->video.stream_index >= 0) {
                AVEMStream *vst = s->streams[xmv->video.stream_index];
                ...
                if (vst->codecpar->extradata_size < 4) {
                    ...
                    if ((ret = em_alloc_extradata(vst->codecpar, 4)) < 0)
                        return ret;
                }
...
static int xmv_fetch_new_packet(AVEMFormatContext *s)
{
    XMVDemuxContext *xmv = s->priv_data;
    AVEMIOContext     *pb  = s->pb;
    int result;
    ...
    if (avio_em_seek(pb, xmv->this_packet_offset, SEEK_SET) != xmv->this_packet_offset)
        return AVERROR(EIO);
...
static int xmv_fetch_audio_packet(AVEMFormatContext *s,
                                  AVEMPacket *pkt, uint32_t stream)
{
    XMVDemuxContext *xmv   = s->priv_data;
    AVEMIOContext     *pb    = s->pb;
    ...
    /* Seek to it */
    if (avio_em_seek(pb, audio->data_offset, SEEK_SET) != audio->data_offset)
        return AVERROR(EIO);
    ...
    /* Read the packet */
    result = av_em_get_packet(pb, pkt, data_size);
    if (result <= 0)
        return result;
...
static int xmv_fetch_video_packet(AVEMFormatContext *s,
                                  AVEMPacket *pkt)
{
    XMVDemuxContext *xmv   = s->priv_data;
    AVEMIOContext     *pb    = s->pb;
    ...
    /* Seek to it */
    if (avio_em_seek(pb, video->data_offset, SEEK_SET) != video->data_offset)
        return AVERROR(EIO);

    /* Read the frame header */
    frame_header = avio_em_rl32(pb);
    ...
    /* Get the packet data */
    result = av_em_get_packet(pb, pkt, frame_size);
...
static int xmv_read_packet(AVEMFormatContext *s,
                           AVEMPacket *pkt)
{
    XMVDemuxContext *xmv = s->priv_data;
    int result;
...
AVEMInputFormat ff_xmv_demuxer = {
    .name           = "xmv",
