...
{
    av_em_freep(&data->au_headers);
    av_em_freep(&data->mode);
}

static int parse_fmtp_config(AVEMCodecParameters *par, char *value)
{
    /* decode the hexa encoded parameter */
    int len = em_hex_to_data(NULL, value);
    av_em_freep(&par->extradata);
    if (em_alloc_extradata(par, len))
        return AVERROR(ENOMEM);
    em_hex_to_data(par->extradata, value);
    return 0;
}
...
static int rtp_parse_mp4_au(PayloadContext *data, const uint8_t *buf, int len)
{
    ...
    if (!data->au_headers || data->au_headers_allocated < data->nb_au_headers) {
        av_em_free(data->au_headers);
        data->au_headers = av_em_alloc(sizeof(struct AUHeaders) * data->nb_au_headers);
        if (!data->au_headers)
            return AVERROR(ENOMEM);
...
/* Follows RFC 3640 */
static int aac_parse_packet(AVEMFormatContext *ctx, PayloadContext *data,
                            ...
                            int flags)
{
    ...
    if (!buf) {
        if (data->cur_au_index > data->nb_au_headers) {
            av_em_log(ctx, AV_LOG_ERROR, "Invalid parser state\n");
            return AVERROR_INVALIDDATA;
        }
        if (data->buf_size - data->buf_pos < data->au_headers[data->cur_au_index].size) {
            av_em_log(ctx, AV_LOG_ERROR, "Invalid AU size\n");
            return AVERROR_INVALIDDATA;
        }
        if ((ret = av_em_new_packet(pkt, data->au_headers[data->cur_au_index].size)) < 0) {
    ...
    if (rtp_parse_mp4_au(data, buf, len)) {
        av_em_log(ctx, AV_LOG_ERROR, "Error parsing AU headers\n");
        return -1;
    }
    ...
    if (data->nb_au_headers == 1 && len < data->au_headers[0].size) {
        ...
        if (data->timestamp != *timestamp ||
            ...
            data->buf_pos = 0;
            data->buf_size = 0;
            av_em_log(ctx, AV_LOG_ERROR, "Invalid packet received\n");
            return AVERROR_INVALIDDATA;
        }
        ...
        if (data->buf_pos != data->buf_size) {
            data->buf_pos = 0;
            av_em_log(ctx, AV_LOG_ERROR, "Missed some packets, discarding frame\n");
            return AVERROR_INVALIDDATA;
        }

        data->buf_pos = 0;
        ret = av_em_new_packet(pkt, data->buf_size);
        if (ret < 0) {
            av_em_log(ctx, AV_LOG_ERROR, "Out of memory\n");
            return ret;
        }
    ...
    if (len < data->au_headers[0].size) {
        av_em_log(ctx, AV_LOG_ERROR, "First AU larger than packet size\n");
        return AVERROR_INVALIDDATA;
    }
    if ((ret = av_em_new_packet(pkt, data->au_headers[0].size)) < 0) {
        av_em_log(ctx, AV_LOG_ERROR, "Out of memory\n");
        return ret;
    }
                      ...
                      const char *attr, const char *value)
{
    ...
    if (par->codec_id == AV_CODEC_ID_AAC) {
        ...
        for (i = 0; attr_names[i].str; ++i) {
            if (!av_em_strcasecmp(attr, attr_names[i].str)) {
                if (attr_names[i].type == ATTR_NAME_TYPE_INT) {
                    *(int *)((char *)data+
                        attr_names[i].offset) = atoi(value);
                } else if (attr_names[i].type == ATTR_NAME_TYPE_STR)
                    *(char **)((char *)data+
                        attr_names[i].offset) = av_em_strdup(value);
            }
        }
    }
    return 0;
...
static int parse_sdp_line(AVEMFormatContext *s, int st_index,
...
RTPDynamicProtocolHandler em_mpeg4_generic_dynamic_handler = {
