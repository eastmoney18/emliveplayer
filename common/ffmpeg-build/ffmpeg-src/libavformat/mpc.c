...
static int mpc_probe(AVEMProbeData *p)
{
...
static int mpc_read_header(AVEMFormatContext *s)
{
    MPCContext *c = s->priv_data;
    AVEMStream *st;

    if(avio_em_rl24(s->pb) != MKTAG('M', 'P', '+', 0)){
        av_em_log(s, AV_LOG_ERROR, "Not a Musepack file\n");
        return AVERROR_INVALIDDATA;
    }
    c->ver = avio_em_r8(s->pb);
    if(c->ver != 0x07 && c->ver != 0x17){
        av_em_log(s, AV_LOG_ERROR, "Can demux Musepack SV7, got version %02X\n", c->ver);
        return AVERROR_INVALIDDATA;
    }
    c->fcount = avio_em_rl32(s->pb);
    if((int64_t)c->fcount * sizeof(MPCFrame) >= UINT_MAX){
        av_em_log(s, AV_LOG_ERROR, "Too many frames, seeking is not possible\n");
        return AVERROR_INVALIDDATA;
    }
    if(c->fcount){
        c->frames = av_em_alloc(c->fcount * sizeof(MPCFrame));
        if(!c->frames){
            av_em_log(s, AV_LOG_ERROR, "Cannot allocate seektable\n");
            return AVERROR(ENOMEM);
        }
    }else{
        av_em_log(s, AV_LOG_WARNING, "Container reports no frames\n");
    }
    c->curframe = 0;
    c->lastframe = -1;
    c->curbits = 8;
    ...
    if (em_get_extradata(s, st->codecpar, s->pb, 16) < 0)
        return AVERROR(ENOMEM);
    st->codecpar->sample_rate = mpc_rate[st->codecpar->extradata[2] & 3];
    avpriv_em_set_pts_info(st, 32, MPC_FRAMESIZE, st->codecpar->sample_rate);
    /* scan for seekpoints */
    st->start_time = 0;
...
static int mpc_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MPCContext *c = s->priv_data;
    ...
    if(c->curframe != c->lastframe + 1){
        avio_em_seek(s->pb, c->frames[c->curframe].pos, SEEK_SET);
    ...
    tmp = avio_em_rl32(s->pb);
    if(curbits <= 12){
        size2 = (tmp >> (12 - curbits)) & 0xFFFFF;
    }else{
        size2 = (tmp << (curbits - 12) | avio_em_rl32(s->pb) >> (44 - curbits)) & 0xFFFFF;
    }
    curbits += 20;
    avio_em_seek(s->pb, pos, SEEK_SET);
    ...
    if(cur == c->frames_noted && c->fcount){
        ...
        av_em_add_index_entry(s->streams[0], cur, cur, size, 0, AVINDEX_KEYFRAME);
        c->frames_noted++;
    }
    c->curbits = (curbits + size2) & 0x1F;

    if ((ret = av_em_new_packet(pkt, size + 4)) < 0)
        return ret;
    ...
    pkt->stream_index = 0;
    pkt->pts = cur;
    ret = avio_em_read(s->pb, pkt->data + 4, size);
    if(c->curbits)
...
static int mpc_read_close(AVEMFormatContext *s)
{
    MPCContext *c = s->priv_data;

    av_em_freep(&c->frames);
    return 0;
}
 ...
 */
static int mpc_read_seek(AVEMFormatContext *s, int stream_index, int64_t timestamp, int flags)
{
    AVEMStream *st = s->streams[stream_index];
    MPCContext *c = s->priv_data;
    AVEMPacket pkt1, *pkt = &pkt1;
    int ret;
    int index = av_em_index_search_timestamp(st, FFMAX(timestamp - DELAY_FRAMES, 0), flags);
    uint32_t lastframe;
...
AVEMInputFormat ff_mpc_demuxer = {
    .name           = "mpc",
