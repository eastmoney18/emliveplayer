...
static int ilbc_write_header(AVEMFormatContext *s)
{
    AVEMIOContext *pb = s->pb;
    AVEMCodecParameters *par;

    if (s->nb_streams != 1) {
        av_em_log(s, AV_LOG_ERROR, "Unsupported number of streams\n");
    ...
    if (par->block_align == 50) {
        avio_em_write(pb, mode30_header, sizeof(mode30_header) - 1);
    } else if (par->block_align == 38) {
        avio_em_write(pb, mode20_header, sizeof(mode20_header) - 1);
    } else {
        av_em_log(s, AV_LOG_ERROR, "Unsupported mode\n");
        return AVERROR(EINVAL);
    }
    avio_em_flush(pb);
    return 0;
...
static int ilbc_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    avio_em_write(s->pb, pkt->data, pkt->size);
    return 0;
}

static int ilbc_probe(AVEMProbeData *p)
{
...
static int ilbc_read_header(AVEMFormatContext *s)
{
    AVEMIOContext *pb = s->pb;
    AVEMStream *st;
    uint8_t header[9];

    avio_em_read(pb, header, 9);
    ...
    if (!memcmp(header, mode20_header, sizeof(mode20_header) - 1)) {
        ...
        av_em_log(s, AV_LOG_ERROR, "Unrecognized iLBC file header\n");
...
static int ilbc_read_packet(AVEMFormatContext *s,
                          AVEMPacket *pkt)
{
    AVEMCodecParameters *par = s->streams[0]->codecpar;
    int ret;

    if ((ret = av_em_new_packet(pkt, par->block_align)) < 0)
        return ret;

    pkt->stream_index = 0;
    pkt->pos = avio_em_tell(s->pb);
    pkt->duration = par->block_align == 38 ? 160 : 240;
    if ((ret = avio_em_read(s->pb, pkt->data, par->block_align)) != par->block_align) {
...
AVEMInputFormat ff_ilbc_demuxer = {
    .name         = "ilbc",
...
AVEMOutputFormat ff_ilbc_muxer = {
    .name         = "ilbc",
    .long_name    = NULL_IF_CONFIG_SMALL("iLBC storage"),
