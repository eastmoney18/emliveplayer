...
static int id3v1_set_string(AVEMFormatContext *s, const char *key,
                            uint8_t *buf, int buf_size)
{
    AVEMDictionaryEntry *tag;
    if ((tag = av_em_dict_get(s->metadata, key, NULL, 0)))
        av_em_strlcpy(buf, tag->value, buf_size);
    return !!tag;
}

static int id3v1_create_tag(AVEMFormatContext *s, uint8_t *buf)
{
    AVEMDictionaryEntry *tag;
    int i, count = 0;
    ...
    buf[1] = 'A';
    buf[2] = 'G';
    /* we knowingly overspecify each tag length by one byte to compensate for the mandatory null byte added by av_em_strlcpy */
    ...
    if ((tag = av_em_dict_get(s->metadata, "TRCK", NULL, 0))) { //track
        buf[125] = 0;
        buf[126] = atoi(tag->value);
        count++;
    }
    buf[127] = 0xFF; /* default to unknown genre */
    if ((tag = av_em_dict_get(s->metadata, "TCON", NULL, 0))) { //genre
        for(i = 0; i <= ID3v1_GENRE_MAX; i++) {
            if (!av_em_strcasecmp(tag->value, ff_id3v1_genre_str[i])) {
                buf[127] = i;
                count++;
                break;
            }
...
typedef struct MP3Context {
    ...
    AVEMPacketList *queue, *queue_end;
} MP3Context;
 ...
 */
static int mp3_write_xing(AVEMFormatContext *s)
{
    MP3Context       *mp3 = s->priv_data;
    AVEMCodecParameters *par = s->streams[mp3->audio_stream_idx]->codecpar;
    AVEMDictionaryEntry *enc = av_em_dict_get(s->streams[mp3->audio_stream_idx]->metadata, "encoder", NULL, 0);
    AVEMIOContext *dyn_ctx;
    int32_t        header;
    ...
    if (i == FF_ARRAY_ELEMS(avpriv_mpa_freq_tab)) {
        av_em_log(s, AV_LOG_WARNING, "Unsupported sample rate, not writing Xing header.\n");
        return -1;
    }
    ...
    default: av_em_log(s, AV_LOG_WARNING, "Unsupported number of channels, "
    ...
    ret = avio_em_open_dyn_buf(&dyn_ctx);
    if (ret < 0)
        return ret;

    avio_em_wb32(dyn_ctx, header);

    emio_fill(dyn_ctx, 0, mp3->xing_offset - 4);
    emio_wfourcc(dyn_ctx, "Xing");
    avio_em_wb32(dyn_ctx, 0x01 | 0x02 | 0x04 | 0x08);  // frames / size / TOC / vbr scale
    ...
    avio_em_wb32(dyn_ctx, 0);  // frames
    avio_em_wb32(dyn_ctx, 0);  // size

    // TOC
    for (i = 0; i < XING_TOC_SIZE; i++)
        avio_em_w8(dyn_ctx, (uint8_t)(255 * i / XING_TOC_SIZE));

    // vbr quality
    // we write it, because some (broken) tools always expect it to be present
    avio_em_wb32(dyn_ctx, 0);
    ...
    if (enc) {
        ...
        avio_em_write(dyn_ctx, encoder_str, sizeof(encoder_str));
    } else
        avio_em_write(dyn_ctx, "Lavf\0\0\0\0\0", 9);

    avio_em_w8(dyn_ctx, 0);      // tag revision 0 / unknown vbr method
    avio_em_w8(dyn_ctx, 0);      // unknown lowpass filter value
    emio_fill(dyn_ctx, 0, 8); // empty replaygain fields
    avio_em_w8(dyn_ctx, 0);      // unknown encoding flags
    avio_em_w8(dyn_ctx, 0);      // unknown abr/minimal bitrate

    // encoder delay
    if (par->initial_padding - 528 - 1 >= 1 << 12) {
        av_em_log(s, AV_LOG_WARNING, "Too many samples of initial padding.\n");
    }
    avio_em_wb24(dyn_ctx, FFMAX(par->initial_padding - 528 - 1, 0)<<12);

    avio_em_w8(dyn_ctx,   0); // misc
    avio_em_w8(dyn_ctx,   0); // mp3gain
    avio_em_wb16(dyn_ctx, 0); // preset

    // audio length and CRCs (will be updated later)
    avio_em_wb32(dyn_ctx, 0); // music length
    avio_em_wb16(dyn_ctx, 0); // music crc
    avio_em_wb16(dyn_ctx, 0); // tag crc

    emio_fill(dyn_ctx, 0, mpah.frame_size - bytes_needed);

    mp3->xing_frame_size   = avio_em_close_dyn_buf(dyn_ctx, &mp3->xing_frame);
    mp3->xing_frame_offset = avio_em_tell(s->pb);
    avio_em_write(s->pb, mp3->xing_frame, mp3->xing_frame_size);
 ...
 */
static void mp3_xing_add_frame(MP3Context *mp3, AVEMPacket *pkt)
{
    int i;
...
static int mp3_write_audio_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MP3Context  *mp3 = s->priv_data;
    ...
    if (pkt->data && pkt->size >= 4) {
        ...
        if (ret >= 0) {
            ...
            av_em_log(s, AV_LOG_WARNING, "Audio packet of size %d (starting with %08X...) "
        ...
        if (mp3->xing_offset) {
            ...
            mp3->audio_crc   = av_em_crc(av_em_crc_get_table(AV_CRC_16_ANSI_LE),
...
static int mp3_queue_flush(AVEMFormatContext *s)
{
    MP3Context *mp3 = s->priv_data;
    AVEMPacketList *pktl;
    int ret = 0, write = 1;
...
static void mp3_update_xing(AVEMFormatContext *s)
{
    MP3Context  *mp3 = s->priv_data;
    AVReplayGain *rg;
    uint16_t tag_crc;
    ...
    /* write replaygain */
    rg = (AVReplayGain*)av_em_stream_get_side_data(s->streams[0], AV_PKT_DATA_REPLAYGAIN,
                                                &rg_size);
    if (rg && rg_size >= sizeof(*rg)) {
        uint16_t val;

        AV_WB32(mp3->xing_frame + mp3->xing_offset + 131,
                av_em_rescale(rg->track_peak, 1 << 23, 100000));
    ...
    tag_crc = av_em_crc(av_em_crc_get_table(AV_CRC_16_ANSI_LE), 0, mp3->xing_frame, 190);
    AV_WB16(mp3->xing_frame + mp3->xing_offset + XING_SIZE - 2, tag_crc);

    avio_em_seek(s->pb,  mp3->xing_frame_offset, SEEK_SET);
    avio_em_write(s->pb, mp3->xing_frame, mp3->xing_frame_size);
    avio_em_seek(s->pb, 0, SEEK_END);
}

static int mp3_write_trailer(struct AVEMFormatContext *s)
{
    uint8_t buf[ID3v1_TAG_SIZE];
    MP3Context *mp3 = s->priv_data;

    if (mp3->pics_to_write) {
        av_em_log(s, AV_LOG_WARNING, "No packets were sent for some of the "
               "attached pictures.\n");
        mp3_queue_flush(s);
    }
    ...
    if (mp3->write_id3v1 && id3v1_create_tag(s, buf) > 0) {
        avio_em_write(s->pb, buf, ID3v1_TAG_SIZE);
    }

    if (mp3->xing_offset)
        mp3_update_xing(s);

    av_em_freep(&mp3->xing_frame);

    return 0;
}

static int query_codec(enum AVEMCodecID id, int std_compliance)
{
    const CodecEMMime *cm= ff_id3v2_mime_tags;
...
static const AVEMClass mp3_muxer_class = {
...
static int mp3_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MP3Context *mp3 = s->priv_data;
    ...
    if (pkt->stream_index == mp3->audio_stream_idx) {
        if (mp3->pics_to_write) {
            ...
            AVEMPacketList *pktl = av_em_mallocz(sizeof(*pktl));
            int ret;
            if (!pktl) {
                av_em_log(s, AV_LOG_WARNING, "Not enough memory to buffer audio. Skipping picture streams\n");
                mp3->pics_to_write = 0;
                mp3_queue_flush(s);
                return mp3_write_audio_packet(s, pkt);
            }

            ret = av_em_copy_packet(&pktl->pkt, pkt);
            if (ret < 0) {
                av_em_freep(&pktl);
                return ret;
            }
        ...
        if (s->streams[pkt->stream_index]->nb_frames == 1) {
            av_em_log(s, AV_LOG_WARNING, "Got more than one picture in stream %d,"
                   " ignoring.\n", pkt->stream_index);
        }
...
static int mp3_write_header(struct AVEMFormatContext *s)
{
    MP3Context  *mp3 = s->priv_data;
    int ret, i;
    ...
    if (mp3->id3v2_version      &&
        mp3->id3v2_version != 3 &&
        mp3->id3v2_version != 4) {
        av_em_log(s, AV_LOG_ERROR, "Invalid ID3v2 version requested: %d. Only "
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
            if (mp3->audio_stream_idx >= 0 || st->codecpar->codec_id != AV_CODEC_ID_MP3) {
                av_em_log(s, AV_LOG_ERROR, "Invalid audio stream. Exactly one MP3 "
                       "audio stream is required.\n");
                return AVERROR(EINVAL);
            }
            mp3->audio_stream_idx = i;
        } else if (st->codecpar->codec_type != AVMEDIA_TYPE_VIDEO) {
            av_em_log(s, AV_LOG_ERROR, "Only audio streams and pictures are allowed in MP3.\n");
            return AVERROR(EINVAL);
        }
    }
    if (mp3->audio_stream_idx < 0) {
        av_em_log(s, AV_LOG_ERROR, "No audio stream present.\n");
        return AVERROR(EINVAL);
    }
    mp3->pics_to_write = s->nb_streams - 1;

    if (mp3->pics_to_write && !mp3->id3v2_version) {
        av_em_log(s, AV_LOG_ERROR, "Attached pictures were requested, but the "
