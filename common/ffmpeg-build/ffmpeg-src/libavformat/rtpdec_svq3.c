...
struct PayloadContext {
    AVEMIOContext *pktbuf;
    int64_t        timestamp;
};

/** return 0 on packet, <0 on partial packet or error... */
static int svq3_parse_packet (AVEMFormatContext *s, PayloadContext *sv,
                              ...
                              int flags)
{
    ...
    if (config_packet) {
        ...
        if (len < 2 || em_alloc_extradata(st->codecpar, len + 8))
            return AVERROR_INVALIDDATA;
    ...
    if (start_packet) {
        ...
        emio_free_dyn_buf(&sv->pktbuf);
        if ((res = avio_em_open_dyn_buf(&sv->pktbuf)) < 0)
            return res;
    ...
    avio_em_write(sv->pktbuf, buf, len);
