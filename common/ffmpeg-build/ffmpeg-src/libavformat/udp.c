...
static const AVEMClass udp_class = {
    .class_name = "udp",
...
static const AVEMClass udplite_context_class = {
    .class_name     = "udplite",
    .item_name      = av_em_default_item_name,
    .option         = options,
    .version        = LIBAVUTIL_VERSION_INT,
...
{
    char errbuf[100];
    av_em_strerror(ff_neterrno(), errbuf, sizeof(errbuf));
    av_em_log(ctx, level, "%s: %s\n", prefix, errbuf);
}
...
static struct addrinfo *udp_resolve_host(EMURLContext *h,
                                         ...
                                         int type, int family, int flags)
{
    ...
    if ((error = getaddrinfo(node, service, &hints, &res))) {
        res = NULL;
        av_em_log(h, AV_LOG_ERROR, "getaddrinfo(%s, %s): %s\n",
               node ? node : "unknown",
               service,
               gai_strerror(error));
    }
...
static int udp_set_multicast_sources(EMURLContext *h,
                                     ...
                                     int nb_sources, int include)
{
    ...
    for (i = 0; i < nb_sources; i++) {
        ...
        if (sourceaddr->ai_addr->sa_family != AF_INET) {
            freeaddrinfo(sourceaddr);
            av_em_log(NULL, AV_LOG_ERROR, "%s is of incorrect protocol family\n",
                   sources[i]);
            return AVERROR(EINVAL);
        }
...
static int udp_set_url(EMURLContext *h,
...
static int udp_socket_create(EMURLContext *h, struct sockaddr_storage *addr,
                             socklen_t *addr_len, const char *localaddr)
{
    ...
    for (res = res0; res; res=res->ai_next) {
        if (s->udplite_coverage)
            udp_fd = em_socket(res->ai_family, SOCK_DGRAM, IPPROTO_UDPLITE);
        else
            udp_fd = em_socket(res->ai_family, SOCK_DGRAM, 0);
        if (udp_fd != -1) break;
...
static int udp_port(struct sockaddr_storage *addr, int addr_len)
{
    ...
    if ((error = getnameinfo((struct sockaddr *)addr, addr_len, NULL, 0,  sbuf, sizeof(sbuf), NI_NUMERICSERV)) != 0) {
        av_em_log(NULL, AV_LOG_ERROR, "getnameinfo: %s\n", gai_strerror(error));
        return -1;
    }
...
int em_udp_set_remote_url(EMURLContext *h, const char *uri)
{
    ...
    av_em_url_split(NULL, 0, NULL, 0, hostname, sizeof(hostname), &port, NULL, 0, uri);
    ...
    s->is_multicast = em_is_multicast_address((struct sockaddr*) &s->dest_addr);
    p = strchr(uri, '?');
    if (p) {
        if (av_em_find_info_tag(buf, sizeof(buf), "connect", p)) {
 ...
 * @param h media file context
 */
static int udp_get_file_handle(EMURLContext *h)
{
    UDPContext *s = h->priv_data;
    return s->udp_fd;
}
...
#if HAVE_PTHREAD_CANCEL
static void *circular_buffer_task_rx( void *_EMURLContext)
{
    EMURLContext *h = _EMURLContext;
    UDPContext *s = h->priv_data;
    int old_cancelstate;

    pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, &old_cancelstate);
    pthread_mutex_lock(&s->mutex);
    if (em_socket_nonblock(s->udp_fd, 0) < 0) {
        av_em_log(h, AV_LOG_ERROR, "Failed to set blocking mode");
    ...
    while(1) {
        ...
        if(av_em_fifo_space(s->fifo) < len + 4) {
            /* No Space left */
            if (s->overrun_nonfatal) {
                av_em_log(h, AV_LOG_WARNING, "Circular buffer overrun. "
                        "Surviving due to overrun_nonfatal option\n");
                continue;
            } else {
                av_em_log(h, AV_LOG_ERROR, "Circular buffer overrun. "
        ...
        av_em_fifo_generic_write(s->fifo, s->tmp, len+4, NULL);
...
static void *circular_buffer_task_tx( void *_EMURLContext)
{
    EMURLContext *h = _EMURLContext;
    UDPContext *s = h->priv_data;
    int old_cancelstate;
    int64_t target_timestamp = av_em_gettime_relative();
    int64_t start_timestamp = av_em_gettime_relative();
    int64_t sent_bits = 0;
    ...
    for(;;) {
        ...
        av_em_fifo_generic_read(s->fifo, tmp, 4, NULL);
        len=AV_RL32(tmp);

        av_assert0(len >= 0);
        av_assert0(len <= sizeof(s->tmp));

        av_em_fifo_generic_read(s->fifo, s->tmp, len, NULL);

        pthread_mutex_unlock(&s->mutex);
        pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, &old_cancelstate);

        if (s->bitrate) {
            timestamp = av_em_gettime_relative();
                             ...
                             int max_sources)
{
    ...
    while (1) {
        ...
        if (next)
            *next = '\0';
        sources[*num_sources] = av_em_strdup(source_start);
...
static int udp_open(EMURLContext *h, const char *uri, int flags)
{
    ...
    p = strchr(uri, '?');
    if (p) {
        if (av_em_find_info_tag(buf, sizeof(buf), "reuse", p)) {
            char *endptr = NULL;
        ...
        if (av_em_find_info_tag(buf, sizeof(buf), "overrun_nonfatal", p)) {
            char *endptr = NULL;
        ...
        if (av_em_find_info_tag(buf, sizeof(buf), "ttl", p)) {
            s->ttl = strtol(buf, NULL, 10);
        }
        if (av_em_find_info_tag(buf, sizeof(buf), "udplite_coverage", p)) {
            s->udplite_coverage = strtol(buf, NULL, 10);
        }
        if (av_em_find_info_tag(buf, sizeof(buf), "localport", p)) {
            s->local_port = strtol(buf, NULL, 10);
        }
        if (av_em_find_info_tag(buf, sizeof(buf), "pkt_size", p)) {
            s->pkt_size = strtol(buf, NULL, 10);
        }
        if (av_em_find_info_tag(buf, sizeof(buf), "buffer_size", p)) {
            s->buffer_size = strtol(buf, NULL, 10);
        }
        if (av_em_find_info_tag(buf, sizeof(buf), "connect", p)) {
            s->is_connected = strtol(buf, NULL, 10);
        }
        if (av_em_find_info_tag(buf, sizeof(buf), "dscp", p)) {
            dscp = strtol(buf, NULL, 10);
        }
        if (av_em_find_info_tag(buf, sizeof(buf), "fifo_size", p)) {
        ...
        if (av_em_find_info_tag(buf, sizeof(buf), "bitrate", p)) {
        ...
        if (av_em_find_info_tag(buf, sizeof(buf), "burst_bits", p)) {
            s->burst_bits = strtoll(buf, NULL, 10);
        }
        if (av_em_find_info_tag(buf, sizeof(buf), "localaddr", p)) {
            av_em_strlcpy(localaddr, buf, sizeof(localaddr));
        }
        if (av_em_find_info_tag(buf, sizeof(buf), "sources", p)) {
            if (parse_source_list(buf, include_sources, &num_include_sources,
                                  FF_ARRAY_ELEMS(include_sources)))
                goto fail;
        }
        if (av_em_find_info_tag(buf, sizeof(buf), "block", p)) {
            if (parse_source_list(buf, exclude_sources, &num_exclude_sources,
                                  FF_ARRAY_ELEMS(exclude_sources)))
                goto fail;
        }
        if (!is_output && av_em_find_info_tag(buf, sizeof(buf), "timeout", p))
            s->timeout = strtol(buf, NULL, 10);
        if (is_output && av_em_find_info_tag(buf, sizeof(buf), "broadcast", p))
    ...
    /* XXX: fix av_em_url_split */
    if (hostname[0] == '\0' || hostname[0] == '?') {
        /* only accepts null hostname if input */
        if (!(flags & AVIO_FLAG_READ))
            goto fail;
    } else {
        if (em_udp_set_remote_url(h, uri) < 0)
            goto fail;
    }
    ...
    if (s->udplite_coverage) {
        if (setsockopt (udp_fd, IPPROTO_UDPLITE, UDPLITE_SEND_CSCOV, &(s->udplite_coverage), sizeof(s->udplite_coverage)) != 0)
            av_em_log(h, AV_LOG_WARNING, "socket option UDPLITE_SEND_CSCOV not available");

        if (setsockopt (udp_fd, IPPROTO_UDPLITE, UDPLITE_RECV_CSCOV, &(s->udplite_coverage), sizeof(s->udplite_coverage)) != 0)
            av_em_log(h, AV_LOG_WARNING, "socket option UDPLITE_RECV_CSCOV not available");
    }
    ...
    if (s->is_multicast) {
        ...
        if (h->flags & AVIO_FLAG_READ) {
            ...
            if (num_include_sources && num_exclude_sources) {
                av_em_log(h, AV_LOG_ERROR, "Simultaneously including and excluding multicast sources is not supported\n");
                goto fail;
            }
    ...
    if (is_output) {
        ...
        if (getsockopt(udp_fd, SOL_SOCKET, SO_RCVBUF, &tmp, &len) < 0) {
            ...
            av_em_log(h, AV_LOG_DEBUG, "end receive buffer size reported is %d\n", tmp);
            if(tmp < s->buffer_size)
                av_em_log(h, AV_LOG_WARNING, "attempted to set receive buffer to size %d but it only ended up set as %d", s->buffer_size, tmp);
        }

        /* make the socket non-blocking */
        em_socket_nonblock(udp_fd, 1);
    }
    if (s->is_connected) {
    ...
    for (i = 0; i < num_include_sources; i++)
        av_em_freep(&include_sources[i]);
    for (i = 0; i < num_exclude_sources; i++)
        av_em_freep(&exclude_sources[i]);
    ...
    */
    ...
    if (is_output && s->bitrate && !s->circular_buffer_size) {
        ...
        av_em_log(h, AV_LOG_WARNING,"'bitrate' option was set but 'circular_buffer_size' is not, but required\n");
    }
    ...
    if ((!is_output && s->circular_buffer_size) || (is_output && s->bitrate && s->circular_buffer_size)) {
        ...
        s->fifo = av_em_fifo_alloc(s->circular_buffer_size);
        ret = pthread_mutex_init(&s->mutex, NULL);
        if (ret != 0) {
            av_em_log(h, AV_LOG_ERROR, "pthread_mutex_init failed : %s\n", strerror(ret));
            goto fail;
        }
        ret = pthread_cond_init(&s->cond, NULL);
        if (ret != 0) {
            av_em_log(h, AV_LOG_ERROR, "pthread_cond_init failed : %s\n", strerror(ret));
            goto cond_fail;
        }
        ret = pthread_create(&s->circular_buffer_thread, NULL, is_output?circular_buffer_task_tx:circular_buffer_task_rx, h);
        if (ret != 0) {
            av_em_log(h, AV_LOG_ERROR, "pthread_create failed : %s\n", strerror(ret));
            goto thread_fail;
        }
        s->thread_started = 1;
    }
...
static int udplite_open(EMURLContext *h, const char *uri, int flags)
{
    UDPContext *s = h->priv_data;
...
static int udp_read(EMURLContext *h, uint8_t *buf, int size)
{
    UDPContext *s = h->priv_data;
    int ret;
#if HAVE_PTHREAD_CANCEL
    ...
    if (s->fifo) {
        ...
        do {
            ...
            if (avail) { // >=size) {
                ...
                avail= AV_RL32(tmp);
                if(avail > size){
                    av_em_log(h, AV_LOG_WARNING, "Part of datagram lost due to insufficient buffer size\n");
                    avail= size;
                }

                av_em_fifo_generic_read(s->fifo, buf, avail, NULL);
                av_em_fifo_drain(s->fifo, AV_RL32(tmp) - avail);
                pthread_mutex_unlock(&s->mutex);
                return avail;
                ...
                int64_t t = av_em_gettime() + 100000;
    ...
    if (!(h->flags & AVIO_FLAG_NONBLOCK)) {
        ret = em_network_wait_fd(s->udp_fd, 0);
        if (ret < 0)
            return ret;
    }
...
static int udp_write(EMURLContext *h, const uint8_t *buf, int size)
{
    UDPContext *s = h->priv_data;
    int ret;
    ...
    if (s->fifo) {
        ...
        */
        ...
        if(av_em_fifo_space(s->fifo) < size + 4) {
            /* What about a partial packet tx ? */
            pthread_mutex_unlock(&s->mutex);
            return AVERROR(ENOMEM);
        }
        AV_WL32(tmp, size);
        av_em_fifo_generic_write(s->fifo, tmp, 4, NULL); /* size of packet */
        av_em_fifo_generic_write(s->fifo, (uint8_t *)buf, size, NULL); /* the data */
        pthread_cond_signal(&s->cond);
        pthread_mutex_unlock(&s->mutex);
        return size;
    }
#endif
    if (!(h->flags & AVIO_FLAG_NONBLOCK)) {
        ret = em_network_wait_fd(s->udp_fd, 1);
        if (ret < 0)
            return ret;
    }
...
static int udp_close(EMURLContext *h)
{
    UDPContext *s = h->priv_data;
    ...
    if (s->thread_started) {
        ...
        if (ret != 0)
            av_em_log(h, AV_LOG_ERROR, "pthread_join(): %s\n", strerror(ret));
        pthread_mutex_destroy(&s->mutex);
        pthread_cond_destroy(&s->cond);
    }
#endif
...
const EMURLProtocol em_udp_protocol = {
    .name                = "udp",
    .url_open            = udp_open,
    .url_read            = udp_read,
    .url_write           = udp_write,
...
const EMURLProtocol em_udplite_protocol = {
    .name                = "udplite",
