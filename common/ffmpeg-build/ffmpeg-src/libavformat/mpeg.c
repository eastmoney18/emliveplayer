...
static int mpegps_probe(AVEMProbeData *p)
{
    uint32_t code = -1;
    int i;
...
//     av_em_log(NULL, AV_LOG_ERROR, "vid:%d aud:%d sys:%d pspack:%d invalid:%d size:%d \n",
...
typedef struct MpegDemuxContext {
    ...
    int imkh_cctv;
#if CONFIG_VOBSUB_DEMUXER
    AVEMFormatContext *sub_ctx;
    FFDemuxSubtitlesQueue q[32];
    char *sub_name;
#endif
} MpegDemuxContext;

static int mpegps_read_header(AVEMFormatContext *s)
{
    MpegDemuxContext *m = s->priv_data;
    char buffer[7];
    int64_t last_pos = avio_em_tell(s->pb);

    m->header_state = 0xff;
    s->ctx_flags   |= AVFMTCTX_NOHEADER;

    avio_em_get_str(s->pb, 6, buffer, sizeof(buffer));
    if (!memcmp("IMKH", buffer, 4)) {
        m->imkh_cctv = 1;
    } else if (!memcmp("Sofdec", buffer, 6)) {
        m->sofdec = 1;
    } else
       avio_em_seek(s->pb, last_pos, SEEK_SET);

    /* no need to do more */
    return 0;
}

static int64_t get_pts(AVEMIOContext *pb, int c)
{
    uint8_t buf[5];

    buf[0] = c < 0 ? avio_em_r8(pb) : c;
    avio_em_read(pb, buf + 1, 4);

    return ff_parse_pes_pts(buf);
}

static int find_next_start_code(AVEMIOContext *pb, int *size_ptr,
                                int32_t *header_state)
{
 ...
 */
static long mpegps_psm_parse(MpegDemuxContext *m, AVEMIOContext *pb)
{
    int psm_length, ps_info_length, es_map_length;

    psm_length = avio_em_rb16(pb);
    avio_em_r8(pb);
    avio_em_r8(pb);
    ps_info_length = avio_em_rb16(pb);

    /* skip program_stream_info */
    avio_em_skip(pb, ps_info_length);
    /*es_map_length = */avio_em_rb16(pb);
    /* Ignore es_map_length, trust psm_length */
    es_map_length = psm_length - ps_info_length - 10;

    /* at least one es available? */
    while (es_map_length >= 4) {
        unsigned char type      = avio_em_r8(pb);
        unsigned char es_id     = avio_em_r8(pb);
        uint16_t es_info_length = avio_em_rb16(pb);

        /* remember mapping from stream id to stream type */
        m->psm_es_type[es_id] = type;
        /* skip program_stream_info */
        avio_em_skip(pb, es_info_length);
        es_map_length -= 4 + es_info_length;
    }
    avio_em_rb32(pb); /* crc32 */
    return 2 + psm_length;
}
 ...
 */
static int mpegps_read_pes_header(AVEMFormatContext *s,
                                  ...
                                  int64_t *ppts, int64_t *pdts)
{
    ...
    int64_t pts, dts;
    int64_t last_sync = avio_em_tell(s->pb);

error_redo:
    avio_em_seek(s->pb, last_sync, SEEK_SET);
redo:
    /* next start code (should be immediately after) */
    m->header_state = 0xff;
    size      = MAX_SYNC_SIZE;
    startcode = find_next_start_code(s->pb, &size, &m->header_state);
    last_sync = avio_em_tell(s->pb);
    if (startcode < 0) {
    ...
    if (startcode == PADDING_STREAM) {
        avio_em_skip(s->pb, avio_em_rb16(s->pb));
        goto redo;
    }
    if (startcode == PRIVATE_STREAM_2) {
        if (!m->sofdec) {
            /* Need to detect whether this from a DVD or a 'Sofdec' stream */
            int len = avio_em_rb16(s->pb);
            int bytesread = 0;
            uint8_t *ps2buf = av_em_alloc(len);

            if (ps2buf) {
                bytesread = avio_em_read(s->pb, ps2buf, len);

                if (bytesread != len) {
                    avio_em_skip(s->pb, len - bytesread);
                } else {
                    uint8_t *p = 0;
                    if (len >= 6)
                ...
                if (!m->dvd || avio_em_skip(s->pb, -(len + 2)) < 0) {
                    /* Skip back failed.
    ...
    if (!((startcode >= 0x1c0 && startcode <= 0x1df) ||
        ...
        goto redo;
    if (ppos) {
        *ppos = avio_em_tell(s->pb) - 4;
    }
    len = avio_em_rb16(s->pb);
    pts =
    dts = AV_NOPTS_VALUE;
    if (startcode != PRIVATE_STREAM_2)
    {
    ...
    if ((c & 0xe0) == 0x20) {
        ...
        if (flags & 0x3f && header_len == 0) {
            flags &= 0xC0;
            av_em_log(s, AV_LOG_WARNING, "Further flags set but no bytes left\n");
        }
        if (flags & 0x01) { /* PES extension */
            pes_ext = avio_em_r8(s->pb);
            header_len--;
            ...
            if (pes_ext & 0x40 || skip > header_len) {
                av_em_log(s, AV_LOG_WARNING, "pes_ext %X is invalid\n", pes_ext);
                pes_ext = skip = 0;
            }
            avio_em_skip(s->pb, skip);
            header_len -= skip;

            if (pes_ext & 0x01) { /* PES extension 2 */
                ext2_len = avio_em_r8(s->pb);
                header_len--;
                if ((ext2_len & 0x7f) > 0) {
                    id_ext = avio_em_r8(s->pb);
                    if ((id_ext & 0x80) == 0)
        ...
        if (header_len < 0)
            goto error_redo;
        avio_em_skip(s->pb, header_len);
    } else if (c != 0xf)
        goto redo;
    }

    if (startcode == PRIVATE_STREAM_1) {
        startcode = avio_em_r8(s->pb);
        len--;
    }
    if (len < 0)
        goto error_redo;
    if (dts != AV_NOPTS_VALUE && ppos) {
        ...
        for (i = 0; i < s->nb_streams; i++) {
            if (startcode == s->streams[i]->id &&
                ...
                em_reduce_index(s, i);
                av_em_add_index_entry(s->streams[i], *ppos, dts, 0, 0,
...
static int mpegps_read_packet(AVEMFormatContext *s,
                              AVEMPacket *pkt)
{
    MpegDemuxContext *m = s->priv_data;
    AVEMStream *st;
...
redo:
    ...
    if (startcode >= 0x80 && startcode <= 0xcf) {
        ...
        /* audio: skip header */
        avio_em_r8(s->pb);
        lpcm_header_len = avio_em_rb16(s->pb);
        len -= 3;
    ...
    es_type = m->psm_es_type[startcode & 0xff];
        ...
        avio_em_read(s->pb, buf, 8);
        avio_em_seek(s->pb, -8, SEEK_CUR);
...
found:
    ...
    ret = av_em_get_packet(s->pb, pkt, len);
    ...
    if (s->debug & FF_FDEBUG_TS)
        av_em_log(s, AV_LOG_TRACE, "%d: pts=%0.3f dts=%0.3f size=%d\n",
...
static int64_t mpegps_read_dts(AVEMFormatContext *s, int stream_index,
                               int64_t *ppos, int64_t pos_limit)
{
    ...
    for (;;) {
        ...
        if (len < 0) {
            if (s->debug & FF_FDEBUG_TS)
                av_em_log(s, AV_LOG_TRACE, "none (ret=%d)\n", len);
            return AV_NOPTS_VALUE;
        }
    ...
    if (s->debug & FF_FDEBUG_TS)
        av_em_log(s, AV_LOG_TRACE, "pos=0x%"PRIx64" dts=0x%"PRIx64" %0.3f\n",
            pos, dts, dts / 90000.0);
    *ppos = pos;
    return dts;
}

AVEMInputFormat ff_mpegps_demuxer = {
    .name           = "mpeg",
...
static int vobsub_probe(AVEMProbeData *p)
{
...
static int vobsub_read_header(AVEMFormatContext *s)
{
    ...
    AVEMInputFormat *iformat;

    if (!vobsub->sub_name) {
        char *ext;
        vobsub->sub_name = av_em_strdup(s->filename);
        if (!vobsub->sub_name) {
            ret = AVERROR(ENOMEM);
            goto end;
        }
        ...
        if (fname_len < 4 || *(ext - 1) != '.') {
            av_em_log(s, AV_LOG_ERROR, "The input index filename is too short "
                   "to guess the associated .SUB file\n");
            ret = AVERROR_INVALIDDATA;
            goto end;
        }
        memcpy(ext, !strncmp(ext, "IDX", 3) ? "SUB" : "sub", 3);
        av_em_log(s, AV_LOG_VERBOSE, "IDX/SUB: %s -> %s\n", s->filename, vobsub->sub_name);
    }

    if (!(iformat = av_em_find_input_format("mpeg"))) {
        ret = AVERROR_DEMUXER_NOT_FOUND;
        goto end;
    }

    vobsub->sub_ctx = avformat_em_alloc_context();
    if (!vobsub->sub_ctx) {
        ret = AVERROR(ENOMEM);
        goto end;
    }

    if ((ret = em_copy_whiteblacklists(vobsub->sub_ctx, s)) < 0)
        goto end;

    ret = avformat_em_open_input(&vobsub->sub_ctx, vobsub->sub_name, iformat, NULL);
    if (ret < 0) {
        av_em_log(s, AV_LOG_ERROR, "Unable to open %s as MPEG subtitles\n", vobsub->sub_name);
        goto end;
    }

    av_em_bprint_init(&header, 0, AV_BPRINT_SIZE_UNLIMITED);
    while (!avio_em_feof(s->pb)) {
        ...
        if (!strncmp(line, "id:", 3)) {
            if (sscanf(line, "id: %63[^,], index: %u", id, &stream_id) != 2) {
                av_em_log(s, AV_LOG_WARNING, "Unable to parse index line '%s', "
            ...
            if (stream_id >= FF_ARRAY_ELEMS(vobsub->q)) {
                av_em_log(s, AV_LOG_ERROR, "Maximum number of subtitles streams reached\n");
                ret = AVERROR(EINVAL);
                goto end;
            }
            ...
            av_em_log(s, AV_LOG_DEBUG, "IDX stream[%d] id=%s\n", stream_id, id);
            ...
            if (stream_id == -1) {
                av_em_log(s, AV_LOG_ERROR, "Timestamp declared before any stream\n");
                ret = AVERROR_INVALIDDATA;
                goto end;
            }
            ...
            if (!st || st->id != stream_id) {
                ...
                av_em_dict_set(&st->metadata, "language", id, 0);
                if (alt[0])
                    av_em_dict_set(&st->metadata, "title", alt, 0);
            }

            if (sscanf(p, "%02d:%02d:%02d:%03d, filepos: %"SCNx64,
                       &hh, &mm, &ss, &ms, &pos) != 5) {
                av_em_log(s, AV_LOG_ERROR, "Unable to parse timestamp line '%s', "
                       "abort parsing\n", line);
                ret = AVERROR_INVALIDDATA;
                goto end;
            }
            timestamp = (hh*3600LL + mm*60LL + ss) * 1000LL + ms + delay;
            timestamp = av_em_rescale_q(timestamp, av_em_make_q(1, 1000), st->time_base);
            ...
            while (*p == ' ')
                p++;
            av_em_log(s, AV_LOG_DEBUG, "IDX stream[%d] name=%s\n", stream_id, p);
            av_em_strlcpy(alt, p, sizeof(alt));
            header_parsed = 1;
            ...
            if (sscanf(p, "%d", &langidx) != 1)
                av_em_log(s, AV_LOG_ERROR, "Invalid langidx specified\n");

        } else if (!header_parsed) {
            if (line[0] && line[0] != '#')
                av_em_bprintf(&header, "%s\n", line);
        }
    }
    ...
    if (!av_em_bprint_is_complete(&header)) {
        av_em_bprint_finalize(&header, NULL);
        ret = AVERROR(ENOMEM);
        goto end;
    }
    av_em_bprint_finalize(&header, &header_str);
    for (i = 0; i < s->nb_streams; i++) {
        AVEMStream *sub_st = s->streams[i];
        sub_st->codecpar->extradata      = av_em_strdup(header_str);
        sub_st->codecpar->extradata_size = header.len;
    }
    av_em_free(header_str);

end:
    return ret;
}

static int vobsub_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MpegDemuxContext *vobsub = s->priv_data;
    FFDemuxSubtitlesQueue *q;
    AVEMIOContext *pb = vobsub->sub_ctx->pb;
    ...
    avio_em_seek(pb, idx_pkt.pos, SEEK_SET);
    ...
    do {
        ...
        int64_t pts, dts;
        int64_t old_pos = avio_em_tell(pb), new_pos;
        int pkt_size;
        ...
        ret = av_em_grow_packet(pkt, to_read);
        if (ret < 0)
            goto fail;

        n = avio_em_read(pb, pkt->data + (pkt->size - to_read), to_read);
        if (n < to_read)
            pkt->size -= to_read - n;
...
static int vobsub_read_seek(AVEMFormatContext *s, int stream_index,
...
static int vobsub_read_close(AVEMFormatContext *s)
{
    int i;
    MpegDemuxContext *vobsub = s->priv_data;
    ...
    if (vobsub->sub_ctx)
        avformat_em_close_input(&vobsub->sub_ctx);
    return 0;
}
...
static const AVEMClass vobsub_demuxer_class = {
    .class_name = "vobsub",
...
AVEMInputFormat ff_vobsub_demuxer = {
    .name           = "vobsub",
