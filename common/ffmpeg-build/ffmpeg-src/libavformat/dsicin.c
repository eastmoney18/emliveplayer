...
static int cin_probe(AVEMProbeData *p)
{
...
static int cin_read_file_header(CinDemuxContext *cin, AVEMIOContext *pb) {
    CinFileHeader *hdr = &cin->file_header;

    if (avio_em_rl32(pb) != 0x55AA0000)
        return AVERROR_INVALIDDATA;

    hdr->video_frame_size   = avio_em_rl32(pb);
    hdr->video_frame_width  = avio_em_rl16(pb);
    hdr->video_frame_height = avio_em_rl16(pb);
    hdr->audio_frequency    = avio_em_rl32(pb);
    hdr->audio_bits         = avio_em_r8(pb);
    hdr->audio_stereo       = avio_em_r8(pb);
    hdr->audio_frame_size   = avio_em_rl16(pb);
...
static int cin_read_header(AVEMFormatContext *s)
{
    int rc;
    ...
    avpriv_em_set_pts_info(st, 32, 1, 12);
    ...
    avpriv_em_set_pts_info(st, 32, 1, 22050);
...
static int cin_read_frame_header(CinDemuxContext *cin, AVEMIOContext *pb) {
    CinFrameHeader *hdr = &cin->frame_header;

    hdr->video_frame_type = avio_em_r8(pb);
    hdr->audio_frame_type = avio_em_r8(pb);
    hdr->pal_colors_count = avio_em_rl16(pb);
    hdr->video_frame_size = avio_em_rl32(pb);
    hdr->audio_frame_size = avio_em_rl32(pb);

    if (avio_em_feof(pb) || pb->error)
        return AVERROR(EIO);

    if (avio_em_rl32(pb) != 0xAA55AA55)
        return AVERROR_INVALIDDATA;
...
static int cin_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    CinDemuxContext *cin = s->priv_data;
    AVEMIOContext *pb = s->pb;
    CinFrameHeader *hdr = &cin->frame_header;
    ...
    if (cin->audio_buffer_size == 0) {
        ...
        pkt_size = emio_limit(pb, pkt_size);

        ret = av_em_new_packet(pkt, 4 + pkt_size);
        if (ret < 0)
            return ret;
        ...
        ret = avio_em_read(pb, &pkt->data[4], pkt_size);
        if (ret < 0) {
            av_em_packet_unref(pkt);
            return ret;
        }
        if (ret < pkt_size)
            av_em_shrink_packet(pkt, 4 + ret);
    ...
    /* audio packet */
    ret = av_em_get_packet(pb, pkt, cin->audio_buffer_size);
    if (ret < 0)
        return ret;
...
AVEMInputFormat ff_dsicin_demuxer = {
    .name           = "dsicin",
