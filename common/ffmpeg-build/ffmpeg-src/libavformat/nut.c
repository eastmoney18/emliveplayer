...
const AVEMCodecTag ff_nut_subtitle_tags[] = {
...
const AVEMCodecTag ff_nut_data_tags[] = {
    { AV_CODEC_ID_TEXT,             MKTAG('U', 'T', 'F', '8') },
    { AV_CODEC_ID_NONE,             0 }
};

const AVEMCodecTag ff_nut_video_tags[] = {
...
const AVEMCodecTag ff_nut_audio_extra_tags[] = {
...
const AVEMCodecTag ff_nut_audio_tags[] = {
...
const AVEMCodecTag * const ff_nut_codec_tags[] = {
    ff_nut_video_tags, ff_nut_audio_tags, ff_nut_subtitle_tags,
    em_codec_bmp_tags, em_codec_wav_tags, ff_nut_audio_extra_tags, ff_nut_data_tags, 0
};

void ff_nut_reset_ts(NUTContext *nut, AVEMRational time_base, int64_t val)
{
    int i;
    for (i = 0; i < nut->avf->nb_streams; i++)
        nut->stream[i].last_pts =
            av_em_rescale_rnd(val,
...
{
    Syncpoint *sp           = av_em_mallocz(sizeof(Syncpoint));
    struct AVTreeNode *node = av_em_tree_node_alloc();
    ...
    sp->ts       = ts;
    av_em_tree_insert(&nut->syncpoints, sp, ff_nut_sp_pos_cmp, &node);
    if (node) {
        av_em_free(sp);
        av_em_free(node);
    }
...
{
    if (nut->syncpoints) {
        av_em_tree_enumerate(nut->syncpoints, NULL, NULL, enu_free);
        av_em_tree_destroy(nut->syncpoints);
    }
}
