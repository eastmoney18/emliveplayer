...
struct DVMuxContext {
    ...
    AVEMStream         *ast[2];        /* stereo audio streams */
...
static int dv_write_pack(enum dv_pack_type pack_id, DVMuxContext *c, uint8_t* buf, ...)
{
    ...
    switch (pack_id) {
    case dv_timecode:
        timecode  = av_em_timecode_get_smpte_from_framenum(&c->tc, c->frames);
    ...
    case dv_video_recdate:  /* VAUX recording date */
        ct = c->start_time + av_em_rescale_rnd(c->frames, c->sys->time_base.num,
...
static int dv_assemble_frame(AVEMFormatContext *s,
                             DVMuxContext *c, AVEMStream* st,
                             uint8_t* data, int data_size, uint8_t** frame)
{
    ...
    *frame = &c->frame_buf[0];
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (c->has_video)
            av_em_log(s, AV_LOG_ERROR, "Can't process DV frame #%d. Insufficient audio data or severe sync problem.\n", c->frames);
        if (data_size != c->sys->frame_size) {
            av_em_log(s, AV_LOG_ERROR, "Unexpected frame size, %d != %d\n",
    ...
    case AVMEDIA_TYPE_AUDIO:
        for (i = 0; i < c->n_ast && st != c->ast[i]; i++);
          ...
          /* FIXME: we have to have more sensible approach than this one */
        if (av_em_fifo_size(c->audio_data[i]) + data_size >= 100*MAX_AUDIO_FRAME_SIZE)
            av_em_log(s, AV_LOG_ERROR, "Can't process DV frame #%d. Insufficient video data or severe sync problem.\n", c->frames);
        av_em_fifo_generic_write(c->audio_data[i], data, data_size, NULL);
        ...
        c->has_audio |= ((reqasize <= av_em_fifo_size(c->audio_data[i])) << i);
    ...
    if (c->has_video == 1 && c->has_audio + 1 == 1 << c->n_ast) {
        ...
        for (i=0; i < c->n_ast; i++) {
            ...
            av_em_fifo_drain(c->audio_data[i], reqasize);
            c->has_audio |= ((reqasize <= av_em_fifo_size(c->audio_data[i])) << i);
        }
...
static DVMuxContext* dv_init_mux(AVEMFormatContext* s)
{
    ...
    c->sys = av_em_dv_codec_profile2(vst->codecpar->width, vst->codecpar->height,
    ...
    c->has_audio  = 0;
    c->has_video  = 0;
    em_parse_creation_time_metadata(s, &c->start_time, 1);

    for (i=0; i < c->n_ast; i++) {
        if (c->ast[i] && !(c->audio_data[i]=av_em_fifo_alloc_array(100, MAX_AUDIO_FRAME_SIZE))) {
            while (i > 0) {
                i--;
                av_em_fifo_freep(&c->audio_data[i]);
            }
            goto bail_out;
        }
    }
...
static int dv_write_header(AVEMFormatContext *s)
{
    AVEMRational rate;
    ...
    if (!dv_init_mux(s)) {
        av_em_log(s, AV_LOG_ERROR, "Can't initialize DV format!\n"
    ...
    if (!tcr) { // no global timecode, look into the streams
        int i;
        for (i = 0; i < s->nb_streams; i++) {
            tcr = av_em_dict_get(s->streams[i]->metadata, "timecode", NULL, 0);
            if (tcr)
                break;
        }
    }
    if (tcr && av_em_timecode_init_from_string(&dvc->tc, rate, tcr->value, s) >= 0)
        return 0;
    return av_em_timecode_init(&dvc->tc, rate, 0, 0, s);
}

static int dv_write_packet(struct AVEMFormatContext *s, AVEMPacket *pkt)
{
    uint8_t* frame;
    int fsize;
    ...
    if (fsize > 0) {
        avio_em_write(s->pb, frame, fsize);
    }
    return 0;
}
 ...
 */
static int dv_write_trailer(struct AVEMFormatContext *s)
{
    dv_delete_mux(s->priv_data);
    return 0;
}

AVEMOutputFormat ff_dv_muxer = {
    .name              = "dv",
