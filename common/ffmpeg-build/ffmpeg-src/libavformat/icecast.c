...
static void cat_header(AVEMBPrint *bp, const char key[], const char value[])
{
    if (NOT_EMPTY(value))
        av_em_bprintf(bp, "%s: %s\r\n", key, value);
}

static int icecast_close(EMURLContext *h)
{
...
static int icecast_open(EMURLContext *h, const char *uri, int flags)
{
    IcecastContext *s = h->priv_data;

    // Dict to set options that we pass to the HTTP protocol
    AVEMDictionary *opt_dict = NULL;
    ...
    if (!av_em_bprint_is_complete(&bp)) {
        ret = AVERROR(ENOMEM);
        goto cleanup;
    }
    av_em_bprint_finalize(&bp, &headers);

    // Set options
    av_em_dict_set(&opt_dict, "method", s->legacy_icecast ? "SOURCE" : "PUT", 0);
    av_em_dict_set(&opt_dict, "auth_type", "basic", 0);
    av_em_dict_set(&opt_dict, "headers", headers, 0);
    av_em_dict_set(&opt_dict, "chunked_post", "0", 0);
    av_em_dict_set(&opt_dict, "send_expect_100", s->legacy_icecast ? "0" : "1", 0);
    if (NOT_EMPTY(s->content_type))
        av_em_dict_set(&opt_dict, "content_type", s->content_type, 0);
    else
        av_em_dict_set(&opt_dict, "content_type", "audio/mpeg", 0);
    if (NOT_EMPTY(s->user_agent))
        av_em_dict_set(&opt_dict, "user_agent", s->user_agent, 0);

    // Parse URI
    av_em_url_split(NULL, 0, auth, sizeof(auth), host, sizeof(host),
    ...
    if (auth[0]) {
        ...
        if (sep) {
            *sep = 0;
            ...
            if (s->pass) {
                av_em_free(s->pass);
                av_em_log(h, AV_LOG_WARNING, "Overwriting -password <pass> with URI password!\n");
            }
            if (!(s->pass = av_em_strdup(sep))) {
                ret = AVERROR(ENOMEM);
                goto cleanup;
            }
        }
        if (!(user = av_em_strdup(auth))) {
            ret = AVERROR(ENOMEM);
            goto cleanup;
        }
    }
    ...
    if (!path[0] || strcmp(path, "/") == 0) {
        av_em_log(h, AV_LOG_ERROR, "No mountpoint (path) specified!\n");
        ret = AVERROR(EIO);
        goto cleanup;
    }

    // Build new URI for passing to http protocol
    em_url_join(h_url, sizeof(h_url), "http", auth, host, port, "%s", path);
    // Finally open http proto handler
    ret = ffurl_em_open_whitelist(&s->hd, h_url, AVIO_FLAG_READ_WRITE, NULL,
                               &opt_dict, h->protocol_whitelist, h->protocol_blacklist, h);

cleanup:
    av_em_freep(&user);
    av_em_freep(&headers);
    av_em_dict_free(&opt_dict);

    return ret;
}

static int icecast_write(EMURLContext *h, const uint8_t *buf, int size)
{
    IcecastContext *s = h->priv_data;
    if (!s->send_started) {
        s->send_started = 1;
        if (!s->content_type && size >= 8) {
            ...
            if (memcmp(buf, oggs, sizeof(oggs)) == 0) {
                av_em_log(h, AV_LOG_WARNING, "Streaming Ogg but appropriate content type NOT set!\n");
                av_em_log(h, AV_LOG_WARNING, "Set it with -content_type application/ogg\n");
            } else if (memcmp(buf, opus, sizeof(opus)) == 0) {
                av_em_log(h, AV_LOG_WARNING, "Streaming Opus but appropriate content type NOT set!\n");
                av_em_log(h, AV_LOG_WARNING, "Set it with -content_type audio/ogg\n");
            } else if (memcmp(buf, webm, sizeof(webm)) == 0) {
                av_em_log(h, AV_LOG_WARNING, "Streaming WebM but appropriate content type NOT set!\n");
                av_em_log(h, AV_LOG_WARNING, "Set it with -content_type video/webm\n");
            } else {
                av_em_log(h, AV_LOG_WARNING, "It seems you are streaming an unsupported format.\n");
                av_em_log(h, AV_LOG_WARNING, "It might work, but is not officially supported in Icecast!\n");
            }
        }
    }
...
static const AVEMClass icecast_context_class = {
    .class_name     = "icecast",
...
const EMURLProtocol em_icecast_protocol = {
    .name            = "icecast",
