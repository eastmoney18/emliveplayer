...
static int cdata_probe(AVEMProbeData *p)
{
...
static int cdata_read_header(AVEMFormatContext *s)
{
    ...
    switch (header) {
        ...
        default:
            av_em_log(s, AV_LOG_INFO, "unknown header 0x%04x\n", header);
            return -1;
    };

    sample_rate = avio_em_rb16(pb);
    avio_em_skip(pb, (avio_em_r8(pb) & 0x20) ? 15 : 11);
...
static int cdata_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    CdataDemuxContext *cdata = s->priv_data;
...
AVEMInputFormat ff_ea_cdata_demuxer = {
    .name           = "ea_cdata",
