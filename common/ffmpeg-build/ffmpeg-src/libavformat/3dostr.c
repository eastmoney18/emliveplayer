...
static int threedostr_probe(AVEMProbeData *p)
{
...
static int threedostr_read_header(AVEMFormatContext *s)
{
    unsigned chunk, codec = 0, size, ctrl_size = -1, found_shdr = 0;
    AVEMStream *st;

    while (!avio_em_feof(s->pb) && !found_shdr) {
        ...
        case MKTAG('S','N','D','S'):
            if (size < 56)
                return AVERROR_INVALIDDATA;
            avio_em_skip(s->pb, 8);
            if (avio_em_rl32(s->pb) != MKTAG('S','H','D','R'))
                return AVERROR_INVALIDDATA;
            ...
            if (ctrl_size == 20 || ctrl_size == 3 || ctrl_size == -1)
                st->duration       = (avio_em_rb32(s->pb) - 1) / st->codecpar->channels;
            else
                st->duration       = avio_em_rb32(s->pb) * 16 / st->codecpar->channels;
            size -= 56;
            found_shdr = 1;
            break;
        case MKTAG('S','H','D','R'):
            if (size >  0x78) {
                avio_em_skip(s->pb, 0x74);
                size -= 0x78;
                if (avio_em_rl32(s->pb) == MKTAG('C','T','R','L') && size > 4) {
                    ctrl_size = avio_em_rb32(s->pb);
                    size -= 4;
                }
            }
            break;
        default:
            av_em_log(s, AV_LOG_DEBUG, "skipping unknown chunk: %X\n", chunk);
            break;
        }
...
static int threedostr_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    while (!found_ssmp) {
        ...
        case MKTAG('S','N','D','S'):
            if (size <= 16)
                return AVERROR_INVALIDDATA;
            avio_em_skip(s->pb, 8);
            if (avio_em_rl32(s->pb) != MKTAG('S','S','M','P'))
                return AVERROR_INVALIDDATA;
            avio_em_skip(s->pb, 4);
            size -= 16;
...
AVEMInputFormat ff_threedostr_demuxer = {
    .name           = "3dostr",
