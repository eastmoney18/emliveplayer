...
extern AVEMOutputFormat ff_mxf_d10_muxer;
extern AVEMOutputFormat ff_mxf_opatom_muxer;
...
typedef struct MXFStreamContext {
    ...
    AVEMRational aspect_ratio; ///< display aspect ratio
...
typedef struct MXFContainerEssenceEntry {
    ...
    UID element_ul;
    UID codec_ul;
    void (*write_desc)(AVEMFormatContext *, AVEMStream *);
...
static void mxf_write_wav_desc(AVEMFormatContext *s, AVEMStream *st);
static void mxf_write_aes3_desc(AVEMFormatContext *s, AVEMStream *st);
static void mxf_write_mpegvideo_desc(AVEMFormatContext *s, AVEMStream *st);
static void mxf_write_cdci_desc(AVEMFormatContext *s, AVEMStream *st);
static void mxf_write_generic_sound_desc(AVEMFormatContext *s, AVEMStream *st);
...
static void mxf_write_uuid(AVEMIOContext *pb, enum MXFMetadataSetType type, int value)
{
    avio_em_write(pb, uuid_base, 12);
    avio_em_wb16(pb, type);
    avio_em_wb16(pb, value);
}

static void mxf_write_umid(AVEMFormatContext *s, int type)
{
    MXFContext *mxf = s->priv_data;
    avio_em_write(s->pb, umid_ul, 13);
    avio_em_wb24(s->pb, mxf->instance_number);
    avio_em_write(s->pb, mxf->umid, 15);
    avio_em_w8(s->pb, type);
}

static void mxf_write_refs_count(AVEMIOContext *pb, int ref_count)
{
    avio_em_wb32(pb, ref_count);
    avio_em_wb32(pb, 16);
}
...
static int klv_encode_ber_length(AVEMIOContext *pb, uint64_t len)
{
    ...
    // long form
    avio_em_w8(pb, 0x80 + size);
    while(size) {
        size--;
        avio_em_w8(pb, len >> 8 * size & 0xff);
    }
    return 0;
}

static void klv_encode_ber4_length(AVEMIOContext *pb, int len)
{
    avio_em_w8(pb, 0x80 + 3);
    avio_em_wb24(pb, len);
}

static void klv_encode_ber9_length(AVEMIOContext *pb, uint64_t len)
{
    avio_em_w8(pb, 0x80 + 8);
    avio_em_wb64(pb, len);
}
 ...
 */
static int mxf_get_essence_container_ul_index(enum AVEMCodecID id)
{
    int i;
...
static void mxf_write_primer_pack(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    ...
    avio_em_write(pb, primer_pack_key, 16);
    klv_encode_ber_length(pb, local_tag_number * 18 + 8);

    avio_em_wb32(pb, local_tag_number); // local_tag num
    avio_em_wb32(pb, 18); // item size, always 18 according to the specs

    for (i = 0; i < FF_ARRAY_ELEMS(mxf_local_tag_batch); i++) {
        avio_em_wb16(pb, mxf_local_tag_batch[i].local_tag);
        avio_em_write(pb, mxf_local_tag_batch[i].uid, 16);
    }
    if (mxf->store_user_comments)
        for (i = 0; i < FF_ARRAY_ELEMS(mxf_user_comments_local_tag); i++) {
            avio_em_wb16(pb, mxf_user_comments_local_tag[i].local_tag);
            avio_em_write(pb, mxf_user_comments_local_tag[i].uid, 16);
        }
}

static void mxf_write_local_tag(AVEMIOContext *pb, int size, int tag)
{
    avio_em_wb16(pb, tag);
    avio_em_wb16(pb, size);
}

static void mxf_write_metadata_key(AVEMIOContext *pb, unsigned int value)
{
    avio_em_write(pb, header_metadata_key, 13);
    avio_em_wb24(pb, value);
}

static void mxf_free(AVEMFormatContext *s)
{
    int i;
...
static void mxf_write_essence_container_refs(AVEMFormatContext *s)
{
    ...
    av_em_log(s,AV_LOG_DEBUG, "essence container count:%d\n", c->essence_container_count);
    for (i = 0; i < c->essence_container_count; i++) {
        MXFStreamContext *sc = s->streams[i]->priv_data;
        avio_em_write(pb, mxf_essence_container_uls[sc->index].container_ul, 16);
    }

    if (c->essence_container_count > 1)
        avio_em_write(pb, multiple_desc_ul, 16);
}

static void mxf_write_preface(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    avio_em_wb64(pb, mxf->timestamp);

    // write version
    mxf_write_local_tag(pb, 2, 0x3B05);
    avio_em_wb16(pb, 258); // v1.2
    ...
    if (s->oformat == &ff_mxf_opatom_muxer)
        avio_em_write(pb, opatom_ul, 16);
    else
        avio_em_write(pb, op1a_ul, 16);
...
static uint64_t mxf_utf16len(const char *utf8_str)
{
    ...
    while (*q) {
        ...
        continue;
invalid:
        av_em_log(NULL, AV_LOG_ERROR, "Invaid UTF8 sequence in mxf_utf16len\n\n");
    }
    size += 1;
    return size;
}
...
static int mxf_utf16_local_tag_length(const char *utf8_str)
{
    ...
    if (size >= UINT16_MAX/2) {
        av_em_log(NULL, AV_LOG_ERROR, "utf16 local tag size %"PRIx64" invalid (too large), ignoring\n", size);
        return 0;
    }
 ...
 */
static void mxf_write_local_tag_utf16(AVEMIOContext *pb, int tag, const char *value)
{
    ...
    avio_em_put_str16be(pb, value);
}

static void mxf_write_identification(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
...
static void mxf_write_content_storage(AVEMFormatContext *s)
{
    AVEMIOContext *pb = s->pb;
...
static void mxf_write_track(AVEMFormatContext *s, AVEMStream *st, enum MXFMetadataSetType type)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    avio_em_wb32(pb, st->index+2);

    // write track number
    mxf_write_local_tag(pb, 4, 0x4804);
    if (type == MaterialPackage)
        avio_em_wb32(pb, 0); // track number of material package is 0
    else
        avio_em_write(pb, sc->track_essence_element_key + 12, 4);

    mxf_write_local_tag(pb, 8, 0x4B01);

    if (st == mxf->timecode_track && s->oformat == &ff_mxf_opatom_muxer){
        avio_em_wb32(pb, mxf->tc.rate.num);
        avio_em_wb32(pb, mxf->tc.rate.den);
    } else {
        avio_em_wb32(pb, mxf->time_base.den);
        avio_em_wb32(pb, mxf->time_base.num);
    }
...
static void mxf_write_common_fields(AVEMFormatContext *s, AVEMStream *st)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    if (st == mxf->timecode_track)
        avio_em_write(pb, smpte_12m_timecode_track_data_ul, 16);
    else {
        const MXFCodecUL *data_def_ul = mxf_get_data_definition_ul(st->codecpar->codec_type);
        avio_em_write(pb, data_def_ul->uid, 16);
    }
    ...
    if (st != mxf->timecode_track && s->oformat == &ff_mxf_opatom_muxer && st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO){
        avio_em_wb64(pb, mxf->body_offset / mxf->edit_unit_byte_count);
    } else {
        avio_em_wb64(pb, mxf->duration);
    }
}

static void mxf_write_sequence(AVEMFormatContext *s, AVEMStream *st, enum MXFMetadataSetType type)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
...
static void mxf_write_timecode_component(AVEMFormatContext *s, AVEMStream *st, enum MXFMetadataSetType type)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    avio_em_wb64(pb, mxf->tc.start);

    // Rounded Time Code Base
    mxf_write_local_tag(pb, 2, 0x1502);
    avio_em_wb16(pb, mxf->timecode_base);

    // Drop Frame
    mxf_write_local_tag(pb, 1, 0x1503);
    avio_em_w8(pb, !!(mxf->tc.flags & AV_TIMECODE_FLAG_DROPFRAME));
}

static void mxf_write_structural_component(AVEMFormatContext *s, AVEMStream *st, enum MXFMetadataSetType type)
{
    AVEMIOContext *pb = s->pb;
    int i;
...
static void mxf_write_multi_descriptor(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    const uint8_t *ul;
...
static void mxf_write_generic_desc(AVEMFormatContext *s, AVEMStream *st, const UID key, unsigned size)
{
    MXFContext *mxf = s->priv_data;
    MXFStreamContext *sc = st->priv_data;
    AVEMIOContext *pb = s->pb;

    avio_em_write(pb, key, 16);
...
static void mxf_write_cdci_common(AVEMFormatContext *s, AVEMStream *st, const UID key, unsigned size)
{
    MXFStreamContext *sc = st->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    avio_em_wb32(pb, st->codecpar->width);

    mxf_write_local_tag(pb, 4, 0x3202);
    avio_em_wb32(pb, stored_height>>sc->interlaced);
    ...
    avio_em_wb32(pb, display_height>>sc->interlaced);

    // presentation Y offset
    mxf_write_local_tag(pb, 4, 0x320B);
    avio_em_wb32(pb, (st->codecpar->height - display_height)>>sc->interlaced);

    // component depth
    mxf_write_local_tag(pb, 4, 0x3301);
    avio_em_wb32(pb, sc->component_depth);

    // horizontal subsampling
    mxf_write_local_tag(pb, 4, 0x3302);
    avio_em_wb32(pb, sc->h_chroma_sub_sample);

    // color siting
    mxf_write_local_tag(pb, 1, 0x3303);
    avio_em_w8(pb, sc->color_siting);

    if (sc->signal_standard) {
        mxf_write_local_tag(pb, 1, 0x3215);
        avio_em_w8(pb, sc->signal_standard);
    }

    // frame layout
    mxf_write_local_tag(pb, 1, 0x320C);
    avio_em_w8(pb, sc->interlaced);
    ...
    avio_em_wb32(pb, sc->aspect_ratio.num);
    avio_em_wb32(pb, sc->aspect_ratio.den);

    mxf_write_local_tag(pb, 16, 0x3201);
    avio_em_write(pb, *sc->codec_ul, 16);

    if (sc->interlaced && sc->field_dominance) {
        mxf_write_local_tag(pb, 1, 0x3212);
        avio_em_w8(pb, sc->field_dominance);
    }
...
static void mxf_write_mpegvideo_desc(AVEMFormatContext *s, AVEMStream *st)
{
    ...
    if (st->codecpar->codec_id != AV_CODEC_ID_H264) {
        ...
        avio_em_wb32(pb, sc->video_bit_rate);

        // profile and level
        mxf_write_local_tag(pb, 1, 0x8007);
        if (!st->codecpar->profile)
            profile_and_level |= 0x80; // escape bit
        avio_em_w8(pb, profile_and_level);
    } else {
        mxf_write_cdci_common(s, st, mxf_mpegvideo_descriptor_key, 0);
    }
}

static void mxf_write_generic_sound_common(AVEMFormatContext *s, AVEMStream *st, const UID key, unsigned size)
{
    AVEMIOContext *pb = s->pb;
    MXFContext *mxf = s->priv_data;
    ...
    avio_em_wb32(pb, st->codecpar->sample_rate);
    avio_em_wb32(pb, 1);

    mxf_write_local_tag(pb, 4, 0x3D07);
    if (mxf->channel_count == -1) {
        if (show_warnings && (s->oformat == &ff_mxf_d10_muxer) && (st->codecpar->channels != 4) && (st->codecpar->channels != 8))
            av_em_log(s, AV_LOG_WARNING, "the number of audio channels shall be 4 or 8 : the output will not comply to MXF D-10 specs, use -d10_channelcount to fix this\n");
        avio_em_wb32(pb, st->codecpar->channels);
    } else if (s->oformat == &ff_mxf_d10_muxer) {
        if (show_warnings && (mxf->channel_count < st->codecpar->channels))
            av_em_log(s, AV_LOG_WARNING, "d10_channelcount < actual number of audio channels : some channels will be discarded\n");
        if (show_warnings && (mxf->channel_count != 4) && (mxf->channel_count != 8))
            av_em_log(s, AV_LOG_WARNING, "d10_channelcount shall be set to 4 or 8 : the output will not comply to MXF D-10 specs\n");
        avio_em_wb32(pb, mxf->channel_count);
    } else {
        if (show_warnings && mxf->channel_count != -1 && s->oformat != &ff_mxf_opatom_muxer)
            av_em_log(s, AV_LOG_ERROR, "-d10_channelcount requires MXF D-10 and will be ignored\n");
        avio_em_wb32(pb, st->codecpar->channels);
    }

    mxf_write_local_tag(pb, 4, 0x3D01);
    avio_em_wb32(pb, av_em_get_bits_per_sample(st->codecpar->codec_id));
}

static void mxf_write_wav_common(AVEMFormatContext *s, AVEMStream *st, const UID key, unsigned size)
{
    AVEMIOContext *pb = s->pb;
    ...
    avio_em_wb16(pb, st->codecpar->block_align);

    // avg bytes per sec
    mxf_write_local_tag(pb, 4, 0x3D09);
    avio_em_wb32(pb, st->codecpar->block_align*st->codecpar->sample_rate);
}
...
static int mxf_write_tagged_value(AVEMFormatContext *s, const char* name, const char* value)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    avio_em_write(pb, mxf_indirect_value_utf16le, 17);
    avio_em_put_str16le(pb, value);

    mxf->tagged_value_count++;
    return 0;
}

static int mxf_write_user_comments(AVEMFormatContext *s, const AVEMDictionary *m)
{
    MXFContext *mxf = s->priv_data;
    AVEMDictionaryEntry *t = NULL;
    int count = 0;

    while ((t = av_em_dict_get(m, "comment_", t, AV_DICT_IGNORE_SUFFIX))) {
        if (mxf->tagged_value_count >= UINT16_MAX) {
            av_em_log(s, AV_LOG_ERROR, "too many tagged values, ignoring remaining\n");
            return count;
        }
...
static void mxf_write_package(AVEMFormatContext *s, enum MXFMetadataSetType type, const char *package_name)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    av_em_log(s,AV_LOG_DEBUG, "package type:%d\n", type);
...
static int mxf_write_essence_container_data(AVEMFormatContext *s)
{
    AVEMIOContext *pb = s->pb;
...
static int mxf_write_header_metadata_sets(AVEMFormatContext *s)
{
    ...
    if (entry = av_em_dict_get(s->metadata, "material_package_name", NULL, 0))
       material_package_name = entry->value;

    if (entry = av_em_dict_get(s->metadata, "file_package_name", NULL, 0)) {
        file_package_name = entry->value;
    } else {
        /* check if any of the streams contain a file_package_name */
        for (i = 0; i < s->nb_streams; i++) {
            st = s->streams[i];
            if (entry = av_em_dict_get(st->metadata, "file_package_name", NULL, 0)) {
                file_package_name = entry->value;
                break;
            }
        }
...
static void mxf_write_index_table_segment(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    av_em_log(s, AV_LOG_DEBUG, "edit units count %d\n", mxf->edit_units_count);

    if (!mxf->edit_units_count && !mxf->edit_unit_byte_count)
        return;

    avio_em_write(pb, index_table_segment_key, 16);
    ...
    avio_em_wb64(pb, mxf->last_indexed_edit_unit);

    // index duration
    mxf_write_local_tag(pb, 8, 0x3F0D);
    if (mxf->edit_unit_byte_count)
        avio_em_wb64(pb, 0); // index table covers whole container
    else
        avio_em_wb64(pb, mxf->edit_units_count);

    // edit unit byte count
    mxf_write_local_tag(pb, 4, 0x3F05);
    avio_em_wb32(pb, mxf->edit_unit_byte_count);
    ...
    if (!mxf->edit_unit_byte_count) {
        ...
        avio_em_w8(pb, mxf->slice_count);

        // delta entry array
        mxf_write_local_tag(pb, 8 + (s->nb_streams+1)*6, 0x3F09);
        avio_em_wb32(pb, s->nb_streams+1); // num of entries
        avio_em_wb32(pb, 6);               // size of one entry
        // write system item delta entry
        avio_em_w8(pb, 0);
        avio_em_w8(pb, 0); // slice entry
        avio_em_wb32(pb, 0); // element delta
        for (i = 0; i < s->nb_streams; i++) {
            AVEMStream *st = s->streams[i];
            MXFStreamContext *sc = st->priv_data;
            avio_em_w8(pb, sc->temporal_reordering);
            if (sc->temporal_reordering)
                temporal_reordering = 1;
            if (i == 0) { // video track
                avio_em_w8(pb, 0); // slice number
                avio_em_wb32(pb, KAG_SIZE); // system item size including klv fill
            } else { // audio track
                unsigned audio_frame_size = sc->aic.samples[0]*sc->aic.sample_size;
                audio_frame_size += klv_fill_size(audio_frame_size);
                avio_em_w8(pb, 1);
                avio_em_wb32(pb, (i-1)*audio_frame_size); // element delta
            }
        }

        mxf_write_local_tag(pb, 8 + mxf->edit_units_count*(11+mxf->slice_count*4), 0x3F0A);
        avio_em_wb32(pb, mxf->edit_units_count);  // num of entries
        avio_em_wb32(pb, 11+mxf->slice_count*4);  // size of one entry
        ...
        for (i = 0; i < mxf->edit_units_count; i++) {
            ...
            if (temporal_reordering) {
                ...
                if (pic_num_in_gop != mxf->index_entries[i].temporal_ref) {
                    ...
                    if (j == mxf->edit_units_count)
                        av_em_log(s, AV_LOG_WARNING, "missing frames\n");
                    temporal_offset = j - key_index - pic_num_in_gop;
                }
            }
            avio_em_w8(pb, temporal_offset);

            if ((mxf->index_entries[i].flags & 0x30) == 0x30) { // back and forward prediction
                avio_em_w8(pb, mxf->last_key_index - i);
            } else {
                avio_em_w8(pb, key_index - i); // key frame offset
            ...
            avio_em_w8(pb, mxf->index_entries[i].flags);
            // stream offset
            avio_em_wb64(pb, mxf->index_entries[i].offset);
            if (s->nb_streams > 1)
                avio_em_wb32(pb, mxf->index_entries[i].slice_offset);
        }
...
static void mxf_write_klv_fill(AVEMFormatContext *s)
{
    unsigned pad = klv_fill_size(avio_em_tell(s->pb));
    if (pad) {
        avio_em_write(s->pb, klv_fill_key, 16);
        pad -= 16 + 4;
        klv_encode_ber4_length(s->pb, pad);
        emio_fill(s->pb, 0, pad);
        av_assert1(!(avio_em_tell(s->pb) & (KAG_SIZE-1)));
    }
}

static int mxf_write_partition(AVEMFormatContext *s, int bodysid,
                                int indexsid,
                                const uint8_t *key, int write_metadata)
{
    ...
    uint64_t partition_offset = avio_em_tell(pb);
    int err;
    ...
    if (key && !memcmp(key, body_partition_key, 16)) {
        if ((err = av_em_reallocp_array(&mxf->body_partition_offset, mxf->body_partitions_count + 1,
    ...
    if (key)
        ...
        avio_em_write(pb, body_partition_key, 16);

    klv_encode_ber_length(pb, 88 + 16LL * DESCRIPTOR_COUNT(mxf->essence_container_count));

    // write partition value
    avio_em_wb16(pb, 1); // majorVersion
    avio_em_wb16(pb, 2); // minorVersion
    avio_em_wb32(pb, KAG_SIZE); // KAGSize

    avio_em_wb64(pb, partition_offset); // ThisPartition

    if (key && !memcmp(key, body_partition_key, 16) && mxf->body_partitions_count > 1)
        avio_em_wb64(pb, mxf->body_partition_offset[mxf->body_partitions_count-2]); // PreviousPartition
    else if (key && !memcmp(key, footer_partition_key, 16) && mxf->body_partitions_count)
        avio_em_wb64(pb, mxf->body_partition_offset[mxf->body_partitions_count-1]); // PreviousPartition
    else
        avio_em_wb64(pb, 0);

    avio_em_wb64(pb, mxf->footer_partition_offset); // footerPartition

    // set offset
    header_byte_count_offset = avio_em_tell(pb);
    avio_em_wb64(pb, 0); // headerByteCount, update later

    // indexTable
    avio_em_wb64(pb, index_byte_count); // indexByteCount
    avio_em_wb32(pb, index_byte_count ? indexsid : 0); // indexSID

    // BodyOffset
    if (bodysid && mxf->edit_units_count && mxf->body_partitions_count && s->oformat != &ff_mxf_opatom_muxer)
        avio_em_wb64(pb, mxf->body_offset);
    else
        avio_em_wb64(pb, 0);

    avio_em_wb32(pb, bodysid); // bodySID
    ...
    if (write_metadata) {
        ...
        avio_em_seek(pb, header_byte_count_offset, SEEK_SET);
        avio_em_wb64(pb, header_byte_count);
...
static int mxf_parse_dnxhd_frame(AVEMFormatContext *s, AVEMStream *st,
AVEMPacket *pkt)
{
    MXFContext *mxf = s->priv_data;
    MXFStreamContext *sc = st->priv_data;
    ...
    sc->aspect_ratio = (AVEMRational){ 16, 9 };
...
static int mxf_parse_dv_frame(AVEMFormatContext *s, AVEMStream *st, AVEMPacket *pkt)
{
    MXFContext *mxf = s->priv_data;
    MXFStreamContext *sc = st->priv_data;
    ...
    if ((vs_pack[2] & 0x07) == 0x02)
        ...
        sc->aspect_ratio = (AVEMRational){ 4, 3 };
    ...
    // av_em_log(s, AV_LOG_DEBUG, "DV vsc pack ff/ss = %x\n", vsc_pack[2] >> 6);
    ...
    case 0x18: // DV100 720p
        ...
        if (sc->interlaced) {
            av_em_log(s, AV_LOG_ERROR, "source marked as interlaced but codec profile is progressive\n");
            sc->interlaced = 0;
        }
        break;
...
static int mxf_parse_h264_frame(AVEMFormatContext *s, AVEMStream *st,
                                AVEMPacket *pkt, MXFIndexEntry *e)
{
    MXFContext *mxf = s->priv_data;
    MXFStreamContext *sc = st->priv_data;
    ...
    for (;;) {
        buf = avpriv_em_find_start_code(buf, buf_end, &state);
        if (buf >= buf_end)
            break;
        --buf;
        switch (state & 0x1f) {
    ...
    sc->aspect_ratio = (AVEMRational){ 16, 9 }; // 16:9 is mandatory for broadcast HD
    ...
    if (!uid_found) {
        av_em_log(s, AV_LOG_ERROR, "AVC Intra 50/100 supported only\n");
        return 0;
    }
...
static const UID *mxf_get_mpeg2_codec_ul(AVEMCodecParameters *par)
{
    int long_gop = 1;
...
static int mxf_parse_mpeg2_frame(AVEMFormatContext *s, AVEMStream *st,
                                 AVEMPacket *pkt, MXFIndexEntry *e)
{
    MXFStreamContext *sc = st->priv_data;
    uint32_t c = -1;
    ...
    for(i = 0; i < pkt->size - 4; i++) {
        ...
        if (c == 0x1b5) {
            ...
            case 2:  sc->aspect_ratio = (AVEMRational){  4,  3}; break;
            case 3:  sc->aspect_ratio = (AVEMRational){ 16,  9}; break;
            case 4:  sc->aspect_ratio = (AVEMRational){221,100}; break;
            default:
                av_em_reduce(&sc->aspect_ratio.num, &sc->aspect_ratio.den,
            ...
            if (pict_type == 2) { // P-frame
                ...
                av_em_log(s, AV_LOG_ERROR, "error parsing mpeg2 frame\n");
                return 0;
            }
        }
    }
...
static void mxf_gen_umid(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    uint32_t seed = av_em_get_random_seed();
...
static int mxf_init_timecode(AVEMFormatContext *s, AVEMStream *st, AVEMRational rate)
{
    MXFContext *mxf = s->priv_data;
    AVEMDictionaryEntry *tcr = av_em_dict_get(s->metadata, "timecode", NULL, 0);
    if (!tcr)
        tcr = av_em_dict_get(st->metadata, "timecode", NULL, 0);

    if (tcr)
        return av_em_timecode_init_from_string(&mxf->tc, rate, tcr->value, s);
    else
        return av_em_timecode_init(&mxf->tc, rate, 0, 0, s);
}

static int mxf_write_header(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    int i, ret;
    ...
    if (s->oformat == &ff_mxf_opatom_muxer && s->nb_streams !=1){
        av_em_log(s, AV_LOG_ERROR, "there must be exactly one stream for mxf opatom\n");
        return -1;
    }

    if (!av_em_dict_get(s->metadata, "comment_", NULL, AV_DICT_IGNORE_SUFFIX))
        mxf->store_user_comments = 0;

    for (i = 0; i < s->nb_streams; i++) {
        AVEMStream *st = s->streams[i];
        MXFStreamContext *sc = av_em_mallocz(sizeof(*sc));
        if (!sc)
            return AVERROR(ENOMEM);
        st->priv_data = sc;

        if (((i == 0) ^ (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)) && s->oformat != &ff_mxf_opatom_muxer) {
            av_em_log(s, AV_LOG_ERROR, "there must be exactly one video stream and it must be the first one\n");
            return -1;
        }

        if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            const AVPixFmtDescriptor *pix_desc = av_em_pix_fmt_desc_get(st->codecpar->format);
            // TODO: should be avg_frame_rate
            AVEMRational rate, tbc = st->time_base;
            ...
            switch (em_choose_chroma_location(s, st)) {
            ...
            if (!spf) {
                av_em_log(s, AV_LOG_ERROR, "Unsupported video frame rate %d/%d\n",
                       tbc.den, tbc.num);
                return AVERROR(EINVAL);
            }
            mxf->time_base = spf->time_base;
            rate = av_em_inv_q(mxf->time_base);
            avpriv_em_set_pts_info(st, 64, mxf->time_base.num, mxf->time_base.den);
            ...
            if (s->oformat == &ff_mxf_d10_muxer) {
                if ((sc->video_bit_rate == 50000000) && (mxf->time_base.den == 25)) {
                    ...
                    av_em_log(s, AV_LOG_ERROR, "error MXF D-10 only support 30/40/50 mbit/s\n");
                    return -1;
                }
            ...
            if (st->codecpar->sample_rate != 48000) {
                av_em_log(s, AV_LOG_ERROR, "only 48khz is implemented\n");
                return -1;
            }
            avpriv_em_set_pts_info(st, 64, 1, st->codecpar->sample_rate);
            if (s->oformat == &ff_mxf_d10_muxer) {
                if (st->index != 1) {
                    av_em_log(s, AV_LOG_ERROR, "MXF D-10 only support one audio track\n");
                    return -1;
                }
                if (st->codecpar->codec_id != AV_CODEC_ID_PCM_S16LE &&
                    st->codecpar->codec_id != AV_CODEC_ID_PCM_S24LE) {
                    av_em_log(s, AV_LOG_ERROR, "MXF D-10 only support 16 or 24 bits le audio\n");
                }
                sc->index = ((MXFStreamContext*)s->streams[0]->priv_data)->index + 1;
            } else if (s->oformat == &ff_mxf_opatom_muxer) {
                AVEMRational tbc = av_em_inv_q(mxf->audio_edit_rate);

                if (st->codecpar->codec_id != AV_CODEC_ID_PCM_S16LE &&
                    st->codecpar->codec_id != AV_CODEC_ID_PCM_S24LE) {
                    av_em_log(s, AV_LOG_ERROR, "Only pcm_s16le and pcm_s24le audio codecs are implemented\n");
                    return AVERROR_PATCHWELCOME;
                }
                if (st->codecpar->channels != 1) {
                    av_em_log(s, AV_LOG_ERROR, "MXF OPAtom only supports single channel audio\n");
                    return AVERROR(EINVAL);
                }

                spf = ff_mxf_get_samples_per_frame(s, tbc);
                if (!spf){
                    av_em_log(s, AV_LOG_ERROR, "Unsupported timecode frame rate %d/%d\n", tbc.den, tbc.num);
                    return AVERROR(EINVAL);
                }

                mxf->time_base = st->time_base;
                if((ret = mxf_init_timecode(s, st, av_em_inv_q(spf->time_base))) < 0)
                    return ret;

                mxf->timecode_base = (tbc.den + tbc.num/2) / tbc.num;
                mxf->edit_unit_byte_count = (av_em_get_bits_per_sample(st->codecpar->codec_id) * st->codecpar->channels) >> 3;
                sc->index = 2;
            } else {
                mxf->slice_count = 1;
            }
        ...
        if (!sc->index) {
            ...
            if (sc->index == -1) {
                av_em_log(s, AV_LOG_ERROR, "track %d: could not find essence container ul, "
    ...
    if (em_parse_creation_time_metadata(s, &timestamp, 1) > 0)
        mxf->timestamp = mxf_parse_timestamp(timestamp);
    mxf->duration = -1;

    mxf->timecode_track = av_em_mallocz(sizeof(*mxf->timecode_track));
    if (!mxf->timecode_track)
        return AVERROR(ENOMEM);
    mxf->timecode_track->priv_data = av_em_mallocz(sizeof(MXFStreamContext));
    if (!mxf->timecode_track->priv_data)
        return AVERROR(ENOMEM);
    mxf->timecode_track->index = -1;

    if (!spf)
        spf = ff_mxf_get_samples_per_frame(s, (AVEMRational){ 1, 25 });
...
static void mxf_write_system_item(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    unsigned frame;
    ...
    avio_em_write(pb, system_metadata_pack_key, 16);
    klv_encode_ber4_length(pb, 57);
    avio_em_w8(pb, 0x5c); // UL, user date/time stamp, picture and sound item present
    avio_em_w8(pb, 0x04); // content package rate
    avio_em_w8(pb, 0x00); // content package type
    avio_em_wb16(pb, 0x00); // channel handle
    avio_em_wb16(pb, (mxf->tc.start + frame) & 0xFFFF); // continuity count, supposed to overflow
    if (mxf->essence_container_count > 1)
        avio_em_write(pb, multiple_desc_ul, 16);
    else {
        MXFStreamContext *sc = s->streams[0]->priv_data;
    ...
    avio_em_w8(pb, 0);
    avio_em_wb64(pb, 0);
    avio_em_wb64(pb, 0); // creation date/time stamp

    avio_em_w8(pb, 0x81); // SMPTE 12M time code
    time_code = av_em_timecode_get_smpte_from_framenum(&mxf->tc, frame);
    avio_em_wb32(pb, time_code);
    avio_em_wb32(pb, 0); // binary group data
    avio_em_wb64(pb, 0);

    // write system metadata package set
    avio_em_write(pb, system_metadata_package_set_key, 16);
    klv_encode_ber4_length(pb, 35);
    avio_em_w8(pb, 0x83); // UMID
    avio_em_wb16(pb, 0x20);
    mxf_write_umid(s, 1);
}

static void mxf_write_d10_video_packet(AVEMFormatContext *s, AVEMStream *st, AVEMPacket *pkt)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    MXFStreamContext *sc = st->priv_data;
    ...
    if (pad > 20) {
        ...
        av_em_log(s, AV_LOG_WARNING, "cannot fill d-10 video packet\n");
        emio_fill(s->pb, 0, pad);
    }
}

static void mxf_write_d10_audio_packet(AVEMFormatContext *s, AVEMStream *st, AVEMPacket *pkt)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    avio_em_w8(pb, (frame_size == 1920 ? 0 : (mxf->edit_units_count-1) % 5 + 1));
    avio_em_wl16(pb, frame_size);
    avio_em_w8(pb, (1<<st->codecpar->channels)-1);
...
static int mxf_write_opatom_body_partition(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    if ((err = mxf_write_partition(s, 1, 0, key, 0)) < 0)
        return err;
    mxf_write_klv_fill(s);
    avio_em_write(pb, sc->track_essence_element_key, 16);
    klv_encode_ber9_length(pb, mxf->body_offset);
    return 0;
}

static int mxf_write_opatom_packet(AVEMFormatContext *s, AVEMPacket *pkt, MXFIndexEntry *ie)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
...
static int mxf_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    ...
    if (!mxf->edit_unit_byte_count && !(mxf->edit_units_count % EDIT_UNITS_PER_BODY)) {
        if ((err = av_em_reallocp_array(&mxf->index_entries, mxf->edit_units_count
                                     + EDIT_UNITS_PER_BODY, sizeof(*mxf->index_entries))) < 0) {
            mxf->edit_units_count = 0;
            av_em_log(s, AV_LOG_ERROR, "could not allocate index entries\n");
            return err;
        }
    }
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_MPEG2VIDEO) {
        if (!mxf_parse_mpeg2_frame(s, st, pkt, &ie)) {
            av_em_log(s, AV_LOG_ERROR, "could not get mpeg2 profile and level\n");
            return -1;
        }
    } else if (st->codecpar->codec_id == AV_CODEC_ID_DNXHD) {
        if (!mxf_parse_dnxhd_frame(s, st, pkt)) {
            av_em_log(s, AV_LOG_ERROR, "could not get dnxhd profile\n");
            return -1;
        }
    } else if (st->codecpar->codec_id == AV_CODEC_ID_DVVIDEO) {
        if (!mxf_parse_dv_frame(s, st, pkt)) {
            av_em_log(s, AV_LOG_ERROR, "could not get dv profile\n");
            return -1;
        }
    } else if (st->codecpar->codec_id == AV_CODEC_ID_H264) {
        if (!mxf_parse_h264_frame(s, st, pkt, &ie)) {
            av_em_log(s, AV_LOG_ERROR, "could not get h264 profile\n");
            return -1;
        }
    }
    ...
    if (st->index == 0) {
        ...
        if (!mxf->edit_units_count) {
            av_em_log(s, AV_LOG_ERROR, "No packets in first stream\n");
            return AVERROR_PATCHWELCOME;
        }
    ...
    mxf_write_klv_fill(s);
    avio_em_write(pb, sc->track_essence_element_key, 16); // write key
    if (s->oformat == &ff_mxf_d10_muxer) {
        if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
...
static void mxf_write_random_index_pack(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    uint64_t pos = avio_em_tell(pb);
    int i;

    avio_em_write(pb, random_index_pack_key, 16);
    klv_encode_ber_length(pb, 28 + 12LL*mxf->body_partitions_count);

    if (mxf->edit_unit_byte_count && s->oformat != &ff_mxf_opatom_muxer)
        avio_em_wb32(pb, 1); // BodySID of header partition
    else
        avio_em_wb32(pb, 0);
    avio_em_wb64(pb, 0); // offset of header partition

    for (i = 0; i < mxf->body_partitions_count; i++) {
        avio_em_wb32(pb, 1); // BodySID
        avio_em_wb64(pb, mxf->body_partition_offset[i]);
    }

    avio_em_wb32(pb, 0); // BodySID of footer partition
    avio_em_wb64(pb, mxf->footer_partition_offset);

    avio_em_wb32(pb, avio_em_tell(pb) - pos + 4);
}

static int mxf_write_footer(AVEMFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int err = 0;
    ...
    mxf_write_klv_fill(s);
    mxf->footer_partition_offset = avio_em_tell(pb);
    ...
    if (s->pb->seekable) {
        if (s->oformat == &ff_mxf_opatom_muxer){
            ...
            avio_em_seek(pb, mxf->body_partition_offset[0], SEEK_SET);
            if ((err = mxf_write_opatom_body_partition(s)) < 0)
                goto end;
        }
    ...
    av_em_freep(&mxf->index_entries);
    av_em_freep(&mxf->body_partition_offset);
    av_em_freep(&mxf->timecode_track->priv_data);
    av_em_freep(&mxf->timecode_track);
...
static int mxf_interleave_get_packet(AVEMFormatContext *s, AVEMPacket *out, AVEMPacket *pkt, int flush)
{
    ...
    if (stream_count && (s->nb_streams == stream_count || flush)) {
        ...
        *out = pktl->pkt;
        av_em_log(s, AV_LOG_TRACE, "out st:%d dts:%"PRId64"\n", (*out).stream_index, (*out).dts);
    ...
    out:
        av_em_init_packet(out);
        return 0;
    }
}

static int mxf_compare_timestamps(AVEMFormatContext *s, AVEMPacket *next, AVEMPacket *pkt)
{
...
static int mxf_interleave(AVEMFormatContext *s, AVEMPacket *out, AVEMPacket *pkt, int flush)
{
...
static const AVEMClass mxf_muxer_class = {
...
static const AVEMClass mxf_d10_muxer_class = {
...
static const AVEMClass mxf_opatom_muxer_class = {
...
AVEMOutputFormat ff_mxf_d10_muxer = {
    .name              = "mxf_d10",
...
AVEMOutputFormat ff_mxf_opatom_muxer = {
    .name              = "mxf_opatom",
