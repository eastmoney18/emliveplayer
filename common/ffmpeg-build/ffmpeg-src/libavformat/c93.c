...
static int read_header(AVEMFormatContext *s)
{
    ...
    for (i = 0; i < 512; i++) {
        c93->block_records[i].index = avio_em_rl16(pb);
        c93->block_records[i].length = avio_em_r8(pb);
        c93->block_records[i].frames = avio_em_r8(pb);
        if (c93->block_records[i].frames > 32) {
            av_em_log(s, AV_LOG_ERROR, "too many frames in block\n");
    ...
    video = avformat_em_new_stream(s, NULL);
    if (!video)
    ...
    video->sample_aspect_ratio = (AVEMRational) { 5, 6 };
    avpriv_em_set_pts_info(video, 64, 2, 25);
...
static int read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    ...
    if (c93->next_pkt_is_audio) {
        c93->current_frame++;
        c93->next_pkt_is_audio = 0;
        datasize = avio_em_rl16(pb);
        if (datasize > 42) {
            if (!c93->audio) {
                c93->audio = avformat_em_new_stream(s, NULL);
                if (!c93->audio)
                    return AVERROR(ENOMEM);
                c93->audio->codecpar->codec_type = AVMEDIA_TYPE_AUDIO;
            }
            avio_em_skip(pb, 26); /* VOC header */
    ...
    if (c93->current_frame == 0) {
        avio_em_seek(pb, br->index * 2048, SEEK_SET);
        for (i = 0; i < 32; i++) {
            c93->frame_offsets[i] = avio_em_rl32(pb);
        }
    }

    avio_em_seek(pb,br->index * 2048 +
            c93->frame_offsets[c93->current_frame], SEEK_SET);
    datasize = avio_em_rl16(pb); /* video frame size */

    ret = av_em_new_packet(pkt, datasize + 768 + 1);
    if (ret < 0)
        return ret;
    pkt->data[0] = 0;
    pkt->size = datasize + 1;

    ret = avio_em_read(pb, pkt->data + 1, datasize);
    if (ret < datasize) {
        ret = AVERROR(EIO);
        goto fail;
    }

    datasize = avio_em_rl16(pb); /* palette size */
    if (datasize) {
        if (datasize != 768) {
            av_em_log(s, AV_LOG_ERROR, "invalid palette size %u\n", datasize);
            ret = AVERROR_INVALIDDATA;
            goto fail;
        }
        pkt->data[0] |= C93_HAS_PALETTE;
        ret = avio_em_read(pb, pkt->data + pkt->size, datasize);
        if (ret < datasize) {
            ret = AVERROR(EIO);
            goto fail;
        }
...
AVEMInputFormat ff_c93_demuxer = {
    .name           = "c93",
