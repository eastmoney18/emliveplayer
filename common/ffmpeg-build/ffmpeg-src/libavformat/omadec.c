...
static void hex_log(AVEMFormatContext *s, int level,
                    const char *name, const uint8_t *value, int len)
{
    char buf[33];
    len = FFMIN(len, 16);
    if (av_em_log_get_level() < level)
        return;
    em_data_to_hex(buf, value, len, 1);
    buf[len << 1] = '\0';
    av_em_log(s, level, "%s: %s\n", name, buf);
}

static int kset(AVEMFormatContext *s, const uint8_t *r_val, const uint8_t *n_val,
                int len)
{
...
static int rprobe(AVEMFormatContext *s, uint8_t *enc_header, unsigned size,
                  const uint8_t *r_val)
{
    OMAContext *oc = s->priv_data;
    unsigned int pos;
    ...
    av_des = av_em_des_alloc();
    if (!av_des)
        return AVERROR(ENOMEM);

    /* m_val */
    av_em_des_init(av_des, r_val, 192, 1);
    av_em_des_crypt(av_des, oc->m_val, &enc_header[48], 1, NULL, 1);

    /* s_val */
    av_em_des_init(av_des, oc->m_val, 64, 0);
    av_em_des_crypt(av_des, oc->s_val, NULL, 1, NULL, 0);

    /* sm_val */
    pos = OMA_ENC_HEADER_SIZE + oc->k_size + oc->e_size;
    av_em_des_init(av_des, oc->s_val, 64, 0);
    av_em_des_mac(av_des, oc->sm_val, &enc_header[pos], (oc->i_size >> 3));
...
static int nprobe(AVEMFormatContext *s, uint8_t *enc_header, unsigned size,
                  const uint8_t *n_val)
{
    OMAContext *oc = s->priv_data;
    uint64_t pos;
    ...
    if (AV_RB32(&enc_header[pos]) != oc->rid)
        av_em_log(s, AV_LOG_DEBUG, "Mismatching RID\n");
    ...
    av_em_des_init(av_des, n_val, 192, 1);
    while (datalen-- > 0) {
        av_em_des_crypt(av_des, oc->r_val, &enc_header[pos], 2, NULL, 1);
...
static int decrypt_init(AVEMFormatContext *s, ID3v2ExtraMeta *em, uint8_t *header)
{
    OMAContext *oc = s->priv_data;
    ...
    oc->encrypted = 1;
    av_em_log(s, AV_LOG_INFO, "File is encrypted\n");
    ...
    if (!em) {
        av_em_log(s, AV_LOG_ERROR, "No encryption header found\n");
    ...
    if (AV_RB16(gdata) != 1)
        av_em_log(s, AV_LOG_WARNING, "Unknown version in encryption header\n");
    ...
    if (memcmp(&gdata[OMA_ENC_HEADER_SIZE], "KEYRING     ", 12)) {
        av_em_log(s, AV_LOG_ERROR, "Invalid encryption header\n");
        return AVERROR_INVALIDDATA;
    }
    if (OMA_ENC_HEADER_SIZE + oc->k_size + oc->e_size + oc->i_size + 8 > geob->datasize ||
        OMA_ENC_HEADER_SIZE + 48 > geob->datasize) {
        av_em_log(s, AV_LOG_ERROR, "Too little GEOB data\n");
        return AVERROR_INVALIDDATA;
    }
    oc->rid = AV_RB32(&gdata[OMA_ENC_HEADER_SIZE + 28]);
    av_em_log(s, AV_LOG_DEBUG, "RID: %.8"PRIx32"\n", oc->rid);
    ...
    if (!memcmp(oc->r_val, (const uint8_t[8]){0}, 8) ||
        ...
        if (i >= FF_ARRAY_ELEMS(leaf_table)) {
            av_em_log(s, AV_LOG_ERROR, "Invalid key\n");
            return AVERROR_INVALIDDATA;
        }
    }

    oc->av_des = av_em_des_alloc();
    if (!oc->av_des)
        return AVERROR(ENOMEM);

    /* e_val */
    av_em_des_init(oc->av_des, oc->m_val, 64, 0);
    av_em_des_crypt(oc->av_des, oc->e_val,
                 &gdata[OMA_ENC_HEADER_SIZE + 40], 1, NULL, 0);
    hex_log(s, AV_LOG_DEBUG, "EK", oc->e_val, 8);

    /* init e_val */
    av_em_des_init(oc->av_des, oc->e_val, 64, 1);

    return 0;
}

static int oma_read_header(AVEMFormatContext *s)
{
    ...
    em_id3v2_read(s, ID3v2_EA3_MAGIC, &extra_meta, 0);
    ret = avio_em_read(s->pb, buf, EA3_HEADER_SIZE);
    if (ret < EA3_HEADER_SIZE)
        return -1;

    if (memcmp(buf, ((const uint8_t[]){'E', 'A', '3'}), 3) ||
        buf[4] != 0 || buf[5] != EA3_HEADER_SIZE) {
        av_em_log(s, AV_LOG_ERROR, "Couldn't find the EA3 header !\n");
        return AVERROR_INVALIDDATA;
    }

    oc->content_start = avio_em_tell(s->pb);
    ...
    st->codecpar->codec_id   = em_codec_get_id(ff_oma_codec_tags,
    ...
    case OMA_CODECID_ATRAC3:
        ...
        if (samplerate != 44100)
            avpriv_em_request_sample(s, "Sample rate %d", samplerate);
        ...
        if (em_alloc_extradata(st->codecpar, 14))
            return AVERROR(ENOMEM);
    ...
    case OMA_CODECID_ATRAC3P:
        ...
        avpriv_em_set_pts_info(st, 64, 1, samplerate);
        break;
    ...
    case OMA_CODECID_LPCM:
        ...
        break;
    default:
        av_em_log(s, AV_LOG_ERROR, "Unsupported codec %d!\n", buf[32]);
...
static int oma_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    OMAContext *oc  = s->priv_data;
    ...
    int ret         = av_em_get_packet(s->pb, pkt, packet_size);
    ...
    if (pos >= oc->content_start && byte_rate > 0) {
        pkt->pts =
        pkt->dts = av_em_rescale(pos - oc->content_start, st->time_base.den,
    ...
    if (oc->encrypted) {
        ...
        if (ret == packet_size)
            av_em_des_crypt(oc->av_des, pkt->data, pkt->data,
...
static int oma_read_probe(AVEMProbeData *p)
{
...
static int oma_read_seek(struct AVEMFormatContext *s,
                         int stream_index, int64_t timestamp, int flags)
{
    ...
    /* readjust IV for CBC */
    if (err || avio_em_tell(s->pb) < oc->content_start)
        goto wipe;
    if ((err = avio_em_seek(s->pb, -8, SEEK_CUR)) < 0)
        goto wipe;
    if ((err = avio_em_read(s->pb, oc->iv, 8)) < 8) {
        if (err >= 0)
            err = AVERROR_EOF;
        goto wipe;
    }
...
static int oma_read_close(AVEMFormatContext *s)
{
    OMAContext *oc = s->priv_data;
    av_em_free(oc->av_des);
    return 0;
}

AVEMInputFormat ff_oma_demuxer = {
    .name           = "oma",
    ...
    .codec_tag      = (const AVEMCodecTag* const []){ff_oma_codec_tags, 0},
};
