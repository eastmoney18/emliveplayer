...
static int lmlm4_probe(AVEMProbeData *pd)
{
...
static int lmlm4_read_header(AVEMFormatContext *s)
{
    AVEMStream *st;
    ...
    avpriv_em_set_pts_info(st, 64, 1001, 30000);
...
static int lmlm4_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    AVEMIOContext *pb = s->pb;
    int ret;
    unsigned int frame_type, packet_size, padding, frame_size;

    avio_em_rb16(pb);                       /* channel number */
    frame_type  = avio_em_rb16(pb);
    packet_size = avio_em_rb32(pb);
    padding     = -packet_size & 511;
    frame_size  = packet_size - 8;

    if (frame_type > LMLM4_MPEG1L2 || frame_type == LMLM4_INVALID) {
        av_em_log(s, AV_LOG_ERROR, "invalid or unsupported frame_type\n");
        return AVERROR(EIO);
    }
    if (packet_size > LMLM4_MAX_PACKET_SIZE || packet_size<=8) {
        av_em_log(s, AV_LOG_ERROR, "packet size %d is invalid\n", packet_size);
        return AVERROR(EIO);
    }

    if ((ret = av_em_get_packet(pb, pkt, frame_size)) <= 0)
        return AVERROR(EIO);

    avio_em_skip(pb, padding);
...
AVEMInputFormat ff_lmlm4_demuxer = {
    .name           = "lmlm4",
