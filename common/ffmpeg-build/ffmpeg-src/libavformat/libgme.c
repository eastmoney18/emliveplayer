...
static void add_meta(AVEMFormatContext *s, const char *name, const char *value)
{
    if (value && value[0])
        av_em_dict_set(&s->metadata, name, value, 0);
}

static int load_metadata(AVEMFormatContext *s)
{
...
static int read_header_gme(AVEMFormatContext *s)
{
    AVEMStream *st;
    ...
    sz = avio_em_read(pb, buf, sz);

    // Data left means our buffer (the max_size option) is too small
    if (avio_em_read(pb, &dummy, 1) == 1) {
        av_em_log(s, AV_LOG_ERROR, "File size is larger than max_size option "
...
static int read_packet_gme(AVEMFormatContext *s, AVEMPacket *pkt)
{
    GMEContext *gme = s->priv_data;
    ...
    if ((ret = av_em_new_packet(pkt, AUDIO_PKT_SIZE)) < 0)
        return ret;
...
static int read_close_gme(AVEMFormatContext *s)
{
    GMEContext *gme = s->priv_data;
...
static int read_seek_gme(AVEMFormatContext *s, int stream_idx, int64_t ts, int flags)
{
    GMEContext *gme = s->priv_data;
...
static int probe_gme(AVEMProbeData *p)
{
...
static const AVEMClass class_gme = {
...
AVEMInputFormat ff_libgme_demuxer = {
    .name           = "libgme",
