...
static int chunk_mux_init(AVEMFormatContext *s)
{
    WebMChunkContext *wc = s->priv_data;
    AVEMFormatContext *oc;
    int ret;

    ret = avformat_em_alloc_output_context2(&wc->avf, wc->oformat, NULL, NULL);
    if (ret < 0)
        return ret;
    oc = wc->avf;
    ...
    *(const AVEMClass**)oc->priv_data = oc->oformat->priv_class;
    av_em_opt_set_defaults(oc->priv_data);
    av_em_opt_set_int(oc->priv_data, "dash", 1, 0);
    av_em_opt_set_int(oc->priv_data, "cluster_time_limit", wc->chunk_duration, 0);
    av_em_opt_set_int(oc->priv_data, "live", 1, 0);
...
static int get_chunk_filename(AVEMFormatContext *s, int is_header, char *filename)
{
    WebMChunkContext *wc = s->priv_data;
    AVEMFormatContext *oc = wc->avf;
    if (!filename) {
    ...
    if (is_header) {
        ...
        av_em_strlcpy(filename, wc->header_filename, strlen(wc->header_filename) + 1);
    } else {
        if (av_em_get_frame_filename(filename, MAX_FILENAME_SIZE,
                                  s->filename, wc->chunk_index - 1) < 0) {
            av_em_log(oc, AV_LOG_ERROR, "Invalid chunk filename template '%s'\n", s->filename);
            return AVERROR(EINVAL);
        }
    }
    return 0;
...
static int webm_chunk_write_header(AVEMFormatContext *s)
{
    WebMChunkContext *wc = s->priv_data;
    ...
    wc->oformat = av_em_guess_format("webm", s->filename, "video/webm");
    if (!wc->oformat)
...
static int chunk_start(AVEMFormatContext *s)
{
    WebMChunkContext *wc = s->priv_data;
    AVEMFormatContext *oc = wc->avf;
    int ret;

    ret = avio_em_open_dyn_buf(&oc->pb);
    if (ret < 0)
        return ret;
    wc->chunk_index++;
    return 0;
...
static int chunk_end(AVEMFormatContext *s)
{
    WebMChunkContext *wc = s->priv_data;
    AVEMFormatContext *oc = wc->avf;
    int ret;
    ...
    buffer_size = avio_em_close_dyn_buf(oc->pb, &buffer);
    ...
    if (ret < 0)
        goto fail;
    avio_em_write(pb, buffer, buffer_size);
...
static int webm_chunk_write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    WebMChunkContext *wc = s->priv_data;
    AVEMFormatContext *oc = wc->avf;
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
        wc->duration_written += av_em_rescale_q(pkt->pts - wc->prev_pts,
                                             st->time_base,
                                             (AVEMRational) {1, 1000});
        wc->prev_pts = pkt->pts;
    }
...
static int webm_chunk_write_trailer(AVEMFormatContext *s)
{
    WebMChunkContext *wc = s->priv_data;
    AVEMFormatContext *oc = wc->avf;
...
static const AVEMClass webm_chunk_class = {
...
AVEMOutputFormat ff_webm_chunk_muxer = {
    .name           = "webm_chunk",
