...
static const AVEMCodecTag swf_audio_codec_tags[] = {
...
static int get_swf_tag(AVEMIOContext *pb, int *len_ptr)
{
    int tag, len;
...
static int swf_probe(AVEMProbeData *p)
{
    GetBitContext gb;
...
retry:
    if (!z->avail_in) {
        int n = avio_em_read(s->pb, swf->zbuf_in, ZBUF_SIZE);
        if (n < 0)
            return n;
...
static int swf_read_header(AVEMFormatContext *s)
{
    SWFContext *swf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int nbits, len, tag;

    tag = avio_em_rb32(pb) & 0xffffff00;
    avio_em_rl32(pb);

    if (tag == MKBETAG('C', 'W', 'S', 0)) {
        av_em_log(s, AV_LOG_INFO, "SWF compressed file detected\n");
#if CONFIG_ZLIB
        swf->zbuf_in  = av_em_alloc(ZBUF_SIZE);
        swf->zbuf_out = av_em_alloc(ZBUF_SIZE);
        swf->zpb = avio_em_alloc_context(swf->zbuf_out, ZBUF_SIZE, 0, s,
                                      zlib_refill, NULL, NULL);
        if (!swf->zbuf_in || !swf->zbuf_out || !swf->zpb)
            return AVERROR(ENOMEM);
        swf->zpb->seekable = 0;
        if (inflateInit(&swf->zstream) != Z_OK) {
            av_em_log(s, AV_LOG_ERROR, "Unable to init zlib context\n");
            return AVERROR(EINVAL);
        }
        pb = swf->zpb;
#else
        av_em_log(s, AV_LOG_ERROR, "zlib support is required to read SWF compressed files\n");
        return AVERROR(EIO);
#endif
    } else if (tag != MKBETAG('F', 'W', 'S', 0))
        return AVERROR(EIO);
    /* skip rectangle size */
    nbits = avio_em_r8(pb) >> 3;
    len = (4 * nbits - 3 + 7) / 8;
    avio_em_skip(pb, len);
    swf->frame_rate = avio_em_rl16(pb); /* 8.8 fixed */
    avio_em_rl16(pb); /* frame count */
...
static AVEMStream *create_new_audio_stream(AVEMFormatContext *s, int id, int info)
{
    ...
    ast->codecpar->codec_id   = em_codec_get_id(swf_audio_codec_tags, info>>4 & 15);
    ...
    avpriv_em_set_pts_info(ast, 64, 1, ast->codecpar->sample_rate);
    return ast;
}

static int swf_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    SWFContext *swf = s->priv_data;
    AVEMIOContext *pb = s->pb;
    AVEMStream *vst = NULL, *ast = NULL, *st = 0;
    ...
    for(;;) {
        ...
        if (tag < 0)
            return tag;
        if (len < 0) {
            av_em_log(s, AV_LOG_ERROR, "invalid tag length: %d\n", len);
            return AVERROR_INVALIDDATA;
        }
        if (tag == TAG_VIDEOSTREAM) {
            int ch_id = avio_em_rl16(pb);
            len -= 2;
            ...
            vst->codecpar->codec_id = em_codec_get_id(ff_swf_codec_tags, avio_em_r8(pb));
            avpriv_em_set_pts_info(vst, 16, 256, swf->frame_rate);
            len -= 8;
            ...
            avio_em_r8(pb);
            v = avio_em_r8(pb);
            swf->samples_per_frame = avio_em_rl16(pb);
            ...
            if (!ast)
                return AVERROR(ENOMEM);
            ast->duration = avio_em_rl32(pb); // number of samples
            if (((v>>4) & 15) == 2) { // MP3 sound data record
                ast->skip_samples = avio_em_rl16(pb);
                len -= 2;
            }
            len -= 7;
            if ((res = av_em_get_packet(pb, pkt, len)) < 0)
                return res;
            pkt->pos = pos;
            ...
            const int ch_id   = avio_em_rl16(pb);
            const int bmp_fmt = avio_em_r8(pb);
            const int width   = avio_em_rl16(pb);
            const int height  = avio_em_rl16(pb);
            int pix_fmt;
            ...
            case 5: // RGB24 (0RGB)
                ...
                break;
            default:
                av_em_log(s, AV_LOG_ERROR, "invalid bitmap format %d, skipped\n", bmp_fmt);
                goto bitmap_end_skip;
            }

            linesize = FFALIGN(linesize, 4);

            if (av_em_image_check_size(width, height, 0, s) < 0 ||
                linesize >= INT_MAX / height ||
                linesize * height >= INT_MAX - colormapsize * colormapbpp) {
                av_em_log(s, AV_LOG_ERROR, "invalid frame size %dx%d\n", width, height);
                goto bitmap_end_skip;
            }
            ...
            zbuf = av_em_alloc(len);
            buf  = av_em_alloc(out_len);
            if (!zbuf || !buf) {
                res = AVERROR(ENOMEM);
                goto bitmap_end;
            }

            len = avio_em_read(pb, zbuf, len);
            if (len < 0 || (res = uncompress(buf, &out_len, zbuf, len)) != Z_OK) {
                av_em_log(s, AV_LOG_WARNING, "Failed to uncompress one bitmap\n");
                goto bitmap_end_skip;
            }
            ...
            if (i == s->nb_streams) {
                ...
                avpriv_em_set_pts_info(vst, 64, 256, swf->frame_rate);
                st = vst;
            }

            if ((res = av_em_new_packet(pkt, out_len - colormapsize * colormapbpp)) < 0)
                goto bitmap_end;
            if (!st->codecpar->width && !st->codecpar->height) {
                st->codecpar->width  = width;
                st->codecpar->height = height;
            } else {
                em_add_param_change(pkt, 0, 0, 0, width, height);
            }
            pkt->pos = pos;
            pkt->stream_index = st->index;
            ...
            if (st->codecpar->format != AV_PIX_FMT_NONE && st->codecpar->format != pix_fmt) {
                av_em_log(s, AV_LOG_ERROR, "pixel format change unsupported\n");
            } else
...
bitmap_end_skip:
            ...
            av_em_freep(&buf);
#else
            av_em_log(s, AV_LOG_ERROR, "this file requires zlib support compiled in\n");
#endif
            ...
            avio_em_rl16(pb); /* BITMAP_ID */
            len -= 2;
            if (len < 4)
                goto skip;
            if ((res = av_em_new_packet(pkt, len)) < 0)
                return res;
            if (avio_em_read(pb, pkt->data, 4) != 4) {
            ...
            if (AV_RB32(pkt->data) == 0xffd8ffd9 ||
                ...
                res = avio_em_read(pb, pkt->data, pkt->size);
            } else {
                res = avio_em_read(pb, pkt->data + 4, pkt->size - 4);
                if (res >= 0)
                    res += 4;
            }
            if (res != pkt->size) {
            ...
            av_em_log(s, AV_LOG_DEBUG, "Unknown tag: %d\n", tag);
        }
    skip:
        if(len<0)
            av_em_log(s, AV_LOG_WARNING, "Cliping len %d\n", len);
        len = FFMAX(0, len);
        avio_em_skip(pb, len);
    }
}
...
#if CONFIG_ZLIB
static av_cold int swf_read_close(AVEMFormatContext *avctx)
{
    SWFContext *s = avctx->priv_data;
    inflateEnd(&s->zstream);
    av_em_freep(&s->zbuf_in);
    av_em_freep(&s->zbuf_out);
    av_em_freep(&s->zpb);
    return 0;
}
#endif

AVEMInputFormat ff_swf_demuxer = {
    .name           = "swf",
