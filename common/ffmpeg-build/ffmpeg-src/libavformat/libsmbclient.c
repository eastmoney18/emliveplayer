...
static av_cold int libsmbc_connect(EMURLContext *h)
{
    ...
    if (!libsmbc->ctx) {
        ...
        av_em_log(h, AV_LOG_ERROR, "Cannot create context: %s.\n", strerror(errno));
        return ret;
    }
    if (!smbc_init_context(libsmbc->ctx)) {
        int ret = AVERROR(errno);
        av_em_log(h, AV_LOG_ERROR, "Cannot initialize context: %s.\n", strerror(errno));
        return ret;
    }
    ...
    if (smbc_init(NULL, 0) < 0) {
        int ret = AVERROR(errno);
        av_em_log(h, AV_LOG_ERROR, "Initialization failed: %s\n", strerror(errno));
        return ret;
    }
    return 0;
}

static av_cold int libsmbc_close(EMURLContext *h)
{
    LIBSMBContext *libsmbc = h->priv_data;
    if (libsmbc->fd >= 0) {
...
static av_cold int libsmbc_open(EMURLContext *h, const char *url, int flags)
{
    LIBSMBContext *libsmbc = h->priv_data;
    int access, ret;
    struct stat st;
    ...
    if ((libsmbc->fd = smbc_open(url, access, 0666)) < 0) {
        ret = AVERROR(errno);
        av_em_log(h, AV_LOG_ERROR, "File open failed: %s\n", strerror(errno));
        goto fail;
    }

    if (smbc_fstat(libsmbc->fd, &st) < 0)
        av_em_log(h, AV_LOG_WARNING, "Cannot stat file: %s\n", strerror(errno));
    else
...
static int64_t libsmbc_seek(EMURLContext *h, int64_t pos, int whence)
{
    LIBSMBContext *libsmbc = h->priv_data;
    int64_t newpos;
    ...
    if (whence == AVSEEK_SIZE) {
        if (libsmbc->filesize == -1) {
            av_em_log(h, AV_LOG_ERROR, "Error during seeking: filesize is unknown.\n");
            return AVERROR(EIO);
        } else
    ...
    if ((newpos = smbc_lseek(libsmbc->fd, pos, whence)) < 0) {
        int err = errno;
        av_em_log(h, AV_LOG_ERROR, "Error during seeking: %s\n", strerror(err));
        return AVERROR(err);
    }
...
static int libsmbc_read(EMURLContext *h, unsigned char *buf, int size)
{
    LIBSMBContext *libsmbc = h->priv_data;
    int bytes_read;
    ...
    if ((bytes_read = smbc_read(libsmbc->fd, buf, size)) < 0) {
        int ret = AVERROR(errno);
        av_em_log(h, AV_LOG_ERROR, "Read error: %s\n", strerror(errno));
        return ret;
    }
...
static int libsmbc_write(EMURLContext *h, const unsigned char *buf, int size)
{
    LIBSMBContext *libsmbc = h->priv_data;
    int bytes_written;
    ...
    if ((bytes_written = smbc_write(libsmbc->fd, buf, size)) < 0) {
        int ret = AVERROR(errno);
        av_em_log(h, AV_LOG_ERROR, "Write error: %s\n", strerror(errno));
        return ret;
    }
...
static int libsmbc_open_dir(EMURLContext *h)
{
    LIBSMBContext *libsmbc = h->priv_data;
    int ret;
    ...
    if ((libsmbc->dh = smbc_opendir(h->filename)) < 0) {
        ret = AVERROR(errno);
        av_em_log(h, AV_LOG_ERROR, "Error opening dir: %s\n", strerror(errno));
        goto fail;
    }
...
static int libsmbc_read_dir(EMURLContext *h, AVEMIODirEntry **next)
{
    LIBSMBContext *libsmbc = h->priv_data;
    ...
    *next = entry = em_alloc_dir_entry();
    ...
    entry->name = av_em_strdup(dirent->name);
    if (!entry->name) {
        av_em_freep(next);
        return AVERROR(ENOMEM);
    }

    url = av_em_append_path_component(h->filename, dirent->name);
    if (url) {
        struct stat st;
        if (!smbc_stat(url, &st)) {
...
static int libsmbc_close_dir(EMURLContext *h)
{
    LIBSMBContext *libsmbc = h->priv_data;
    if (libsmbc->dh >= 0) {
...
static int libsmbc_delete(EMURLContext *h)
{
    LIBSMBContext *libsmbc = h->priv_data;
    int ret;
    struct stat st;
...
static int libsmbc_move(EMURLContext *h_src, EMURLContext *h_dst)
{
...
static const AVEMClass libsmbclient_context_class = {
    .class_name     = "libsmbc",
...
const EMURLProtocol em_libsmbclient_protocol = {
    .name                = "smb",
