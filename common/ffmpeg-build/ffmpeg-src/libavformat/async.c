...
static int ring_init(RingBuffer *ring, unsigned int capacity, int read_back_capacity)
{
    ...
    ring->fifo = av_em_fifo_alloc(capacity + read_back_capacity);
    if (!ring->fifo)
...
static int async_open(EMURLContext *h, const char *arg, int flags, AVEMDictionary **options)
{
    ...
    if (ret != 0) {
        av_em_log(h, AV_LOG_ERROR, "ffurl_em_open failed : %s, %s\n", av_err2str(ret), arg);
        goto url_fail;
    }
...
EMURLProtocol em_async_protocol = {
    .name                = "async",
...
const EMURLProtocol ff_async_test_protocol = {
    .name                = "async-test",
...
int main(void)
{
    ...
    ffurl_register_protocol(&em_async_protocol);
    ffurl_register_protocol(&ff_async_test_protocol);

    /*
     * test normal read
     */
    ret = ffurl_em_open(&h, "async:async-test:", AVIO_FLAG_READ, NULL, NULL);
    printf("open: %d\n", ret);

    size = ffurl_em_size(h);
    printf("size: %"PRId64"\n", size);

    pos = ffurl_em_seek(h, 0, SEEK_CUR);
    read_len = 0;
    while (1) {
        ret = ffurl_em_read(h, buf, sizeof(buf));
        if (ret == AVERROR_EOF) {
            printf("read-error: AVERROR_EOF at %"PRId64"\n", ffurl_em_seek(h, 0, SEEK_CUR));
            break;
        }
        else if (ret == 0)
            break;
        else if (ret < 0) {
            printf("read-error: %d at %"PRId64"\n", ret, ffurl_em_seek(h, 0, SEEK_CUR));
            goto fail;
        } else {
            for (i = 0; i < ret; ++i) {
                if (buf[i] != (pos & 0xFF)) {
     ...
     * test normal seek
     */
    ret = ffurl_em_read(h, buf, 1);
    printf("read: %d\n", ret);

    pos = ffurl_em_seek(h, TEST_SEEK_POS, SEEK_SET);
    printf("seek: %"PRId64"\n", pos);
    ...
    ret = ffurl_em_open(&h, "async:async-test:", AVIO_FLAG_READ, NULL, &opts);
    printf("open: %d\n", ret);
