                              ...
                              int channel)
{
    ...
    nb_alloc = channel + 16;
    // This can't use the av_em_reallocp family of functions, since we
    // would need to free each element in the array before the array
    // itself is freed.
    ptr = av_em_realloc_array(*prev_pkt, nb_alloc, sizeof(**prev_pkt));
    if (!ptr)
...
int ff_rtmp_packet_read(EMURLContext *h, RTMPPacket *p,
                        int chunk_size, RTMPPacket **prev_pkt, int *nb_prev_pkt)
{
    uint8_t hdr;

    if (ffurl_em_read(h, &hdr, 1) != 1)
        return AVERROR(EIO);
...
static int rtmp_packet_read_one_chunk(EMURLContext *h, RTMPPacket *p,
                                      ...
                                      int *nb_prev_pkt, uint8_t hdr)
{
    ...
    if (channel_id < 2) { //special case for channel number >= 64
        buf[1] = 0;
        if (ffurl_em_read_complete(h, buf, channel_id + 1) != channel_id + 1)
            return AVERROR(EIO);
    ...
    if (hdr == RTMP_PS_ONEBYTE) {
        ...
        if (ffurl_em_read_complete(h, buf, 3) != 3)
            return AVERROR(EIO);
        written += 3;
        ...
        if (hdr != RTMP_PS_FOURBYTES) {
            ...
            written += 3;
            size = AV_RB24(buf);
            if (ffurl_em_read_complete(h, buf, 1) != 1)
                return AVERROR(EIO);
            written++;
            type = buf[0];
            if (hdr == RTMP_PS_TWELVEBYTES) {
                if (ffurl_em_read_complete(h, buf, 4) != 4)
                    return AVERROR(EIO);
                written += 4;
                extra = AV_RL32(buf);
            }
    ...
    if (!prev_pkt[channel_id].read) {
        if ((ret = ff_em_rtmp_packet_create(p, channel_id, type, timestamp,
                                         size)) < 0)
            return ret;
        p->read = written;
        p->offset = 0;
    ...
    if (ffurl_em_read_complete(h, p->data + p->offset, toread) != toread) {
        ff_em_rtmp_packet_destroy(p);
        return AVERROR(EIO);
    }
    size      -= toread;
    p->read   += toread;
...
int ff_rtmp_packet_read_internal(EMURLContext *h, RTMPPacket *p, int chunk_size,
...
int ff_rtmp_packet_write(EMURLContext *h, RTMPPacket *pkt,
                         int chunk_size, RTMPPacket **prev_pkt_ptr,
                         int *nb_prev_pkt)
{
    ...
    if ((ret = ffurl_em_write(h, pkt_hdr, p - pkt_hdr)) < 0)
        return ret;
    written = p - pkt_hdr + pkt->size;
    while (off < pkt->size) {
        int towrite = FFMIN(chunk_size, pkt->size - off);
        if ((ret = ffurl_em_write(h, pkt->data + off, towrite)) < 0)
            return ret;
        off += towrite;
        if (off < pkt->size) {
            uint8_t marker = 0xC0 | pkt->channel_id;
            if ((ret = ffurl_em_write(h, &marker, 1)) < 0)
                return ret;
            written++;
            if (pkt->ts_field == 0xFFFFFF) {
                uint8_t ts_header[4];
                AV_WB32(ts_header, timestamp);
                if ((ret = ffurl_em_write(h, ts_header, 4)) < 0)
                    return ret;
                written += 4;
            }
        }
...
int ff_em_rtmp_packet_create(RTMPPacket *pkt, int channel_id, RTMPPacketType type,
                          int timestamp, int size)
{
    if (size) {
        pkt->data = av_em_realloc(NULL, size);
        if (!pkt->data)
            return AVERROR(ENOMEM);
    }
    pkt->size       = size;
...
void ff_em_rtmp_packet_destroy(RTMPPacket *pkt)
{
    if (!pkt)
        return;
    ...
    for (;;) {
        ...
        if (size == namelen && !memcmp(data-size, name, namelen)) {
            ...
            case AMF_DATA_TYPE_STRING:
                ...
                av_em_strlcpy(dst, data, FFMIN(len+1, dst_size));
                break;
            default:
                return -1;
            }
                             ...
                             const uint8_t *data_end)
{
    ...
    switch ((type = *data++)) {
    case AMF_DATA_TYPE_NUMBER:
        av_em_log(ctx, AV_LOG_DEBUG, " number %g\n", av_int2double(AV_RB64(data)));
        return;
    case AMF_DATA_TYPE_BOOL:
        av_em_log(ctx, AV_LOG_DEBUG, " bool %d\n", *data);
        return;
    case AMF_DATA_TYPE_STRING:
    case AMF_DATA_TYPE_LONG_STRING:
        ...
        buf[size] = 0;
        av_em_log(ctx, AV_LOG_DEBUG, " string '%s'\n", buf);
        return;
    case AMF_DATA_TYPE_NULL:
        av_em_log(ctx, AV_LOG_DEBUG, " NULL\n");
        return;
    case AMF_DATA_TYPE_ARRAY:
        parse_key = 0;
    case AMF_DATA_TYPE_MIXEDARRAY:
        ...
        while (nb-- > 0 || type != AMF_DATA_TYPE_ARRAY) {
            ...
            if (parse_key) {
                ...
                if (size >= data_end - data)
                    return;
                data += size;
                av_em_log(ctx, AV_LOG_DEBUG, "  %s: ", buf);
            }
...
{
    av_em_log(ctx, AV_LOG_DEBUG, "RTMP packet type '%s'(%d) for channel %d, timestamp %d, extra field %d size %d\n",
    ...
    if (p->type == RTMP_PT_INVOKE || p->type == RTMP_PT_NOTIFY) {
        ...
        av_em_log(ctx, AV_LOG_DEBUG, "Server BW = %d\n", AV_RB32(p->data));
    } else if (p->type == RTMP_PT_CLIENT_BW){
        av_em_log(ctx, AV_LOG_DEBUG, "Client BW = %d\n", AV_RB32(p->data));
    } else if (p->type != RTMP_PT_AUDIO && p->type != RTMP_PT_VIDEO && p->type != RTMP_PT_METADATA) {
        int i;
        for (i = 0; i < p->size; i++)
            av_em_log(ctx, AV_LOG_DEBUG, " %02X", p->data[i]);
        av_em_log(ctx, AV_LOG_DEBUG, "\n");
    }
}
