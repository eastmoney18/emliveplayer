...
static int write_header(AVEMFormatContext *s)
{
    ...
    avio_em_write(s->pb, header, strlen(header));
    avpriv_em_set_pts_info(s->streams[0], 64, 1, 1000);
    return 0;
}
...
static int write_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    char buf[256];
    if (pkt->stream_index)
        av_em_log(s, AV_LOG_WARNING, "More than one stream unsupported\n");
    snprintf(buf, sizeof(buf), "%" PRId64 "\n", pkt->dts);
    avio_em_write(s->pb, buf, strlen(buf));
    return 0;
}

AVEMOutputFormat ff_mkvtimestamp_v2_muxer = {
