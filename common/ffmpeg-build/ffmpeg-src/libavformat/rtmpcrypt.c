...
typedef struct RTMPEContext {
    ...
    EMURLContext   *stream;            ///< TCP stream
                                ...
                                const uint8_t *clientdata, int type)
{
    ...
    if ((ret = ff_rtmp_calc_digest(serverdata + server_pos, 128, 0, secret_key,
                                   128, digest)) < 0)
        return ret;
    av_em_rc4_init(&rt->key_out, digest, 16 * 8, 1);

    /* set input key */
    if ((ret = ff_rtmp_calc_digest(clientdata + client_pos, 128, 0, secret_key,
                                   128, digest)) < 0)
        return ret;
    av_em_rc4_init(&rt->key_in, digest, 16 * 8, 1);
...
static void rtmpe8_sig(const uint8_t *in, uint8_t *out, int key_id)
{
    ...
    av_em_xtea_le_init(&ctx, rtmpe8_keys[key_id]);
    av_em_xtea_le_crypt(&ctx, out, in, 1, NULL, 0);
}
...
static void rtmpe9_sig(const uint8_t *in, uint8_t *out, int key_id)
{
    ...
    av_em_blowfish_init(&ctx, rtmpe9_keys[key_id], 24);
    av_em_blowfish_crypt_ecb(&ctx, &xl, &xr, 0);

    AV_WL32(out, xl);
    AV_WL32(out + 4, xr);
}

void ff_rtmpe_encrypt_sig(EMURLContext *h, uint8_t *sig, const uint8_t *digest,
                          int type)
{
    int i;
...
int ff_rtmpe_update_keystream(EMURLContext *h)
{
    ...
    av_em_rc4_crypt(&rt->key_in, buf, NULL, sizeof(buf), NULL, 1);
    av_em_rc4_crypt(&rt->key_out, buf, NULL, sizeof(buf), NULL, 1);
...
static int rtmpe_close(EMURLContext *h)
{
    RTMPEContext *rt = h->priv_data;
...
static int rtmpe_open(EMURLContext *h, const char *uri, int flags)
{
    RTMPEContext *rt = h->priv_data;
    char host[256], url[1024];
    int ret, port;

    av_em_url_split(NULL, 0, NULL, 0, host, sizeof(host), &port, NULL, 0, uri);

    if (rt->tunneling) {
        if (port < 0)
            port = 80;
        em_url_join(url, sizeof(url), "ffrtmphttp", NULL, host, port, NULL);
    } else {
        if (port < 0)
            port = 1935;
        em_url_join(url, sizeof(url), "tcp", NULL, host, port, NULL);
    }

    /* open the tcp or ffrtmphttp connection */
    if ((ret = ffurl_em_open_whitelist(&rt->stream, url, AVIO_FLAG_READ_WRITE,
...
static int rtmpe_read(EMURLContext *h, uint8_t *buf, int size)
{
    RTMPEContext *rt = h->priv_data;
    int ret;

    rt->stream->flags |= h->flags & AVIO_FLAG_NONBLOCK;
    ret = ffurl_em_read(rt->stream, buf, size);
    ...
    if (rt->handshaked && ret > 0) {
        ...
        av_em_rc4_crypt(&rt->key_in, buf, buf, ret, NULL, 1);
    }

    return ret;
}

static int rtmpe_write(EMURLContext *h, const uint8_t *buf, int size)
{
    RTMPEContext *rt = h->priv_data;
    int ret;
    ...
    if (rt->handshaked) {
        ...
        av_em_rc4_crypt(&rt->key_out, buf, buf, size, NULL, 1);
    }

    if ((ret = ffurl_em_write(rt->stream, buf, size)) < 0)
        return ret;
...
static const AVEMClass ffrtmpcrypt_class = {
...
const EMURLProtocol em_ffrtmpcrypt_protocol = {
    .name            = "ffrtmpcrypt",
    .url_open        = rtmpe_open,
    .url_read        = rtmpe_read,
