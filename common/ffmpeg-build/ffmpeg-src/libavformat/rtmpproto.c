...
typedef struct EMRTMPContext {
    const AVEMClass *class;
    EMURLContext*   stream;                     ///< TCP stream used in interactions with RTMP server
...
static int handle_chunk_size(EMURLContext *s, RTMPPacket *pkt);

static int add_tracked_method(EMRTMPContext *rt, const char *name, int id)
{
    int err;

    if (rt->nb_tracked_methods + 1 > rt->tracked_methods_size) {
        rt->tracked_methods_size = (rt->nb_tracked_methods + 1) * 2;
        if ((err = av_em_reallocp(&rt->tracked_methods, rt->tracked_methods_size *
    ...
    rt->tracked_methods[rt->nb_tracked_methods].name = av_em_strdup(name);
...
static void del_tracked_method(EMRTMPContext *rt, int index)
{
...
static int find_tracked_method(EMURLContext *s, RTMPPacket *pkt, int offset,
                               char **tracked_method)
{
    EMRTMPContext *rt = s->priv_data;
    GetByteContext gbc;
    double pkt_id;
    int ret;
    int i;
...
static void free_tracked_methods(EMRTMPContext *rt)
{
    int i;

    for (i = 0; i < rt->nb_tracked_methods; i ++)
        av_em_freep(&rt->tracked_methods[i].name);
    av_em_freep(&rt->tracked_methods);
    rt->tracked_methods_size = 0;
    rt->nb_tracked_methods   = 0;
}

static int rtmp_send_packet(EMRTMPContext *rt, RTMPPacket *pkt, int track)
{
    int ret;
...
fail:
    ff_em_rtmp_packet_destroy(pkt);
    return ret;
}

static int rtmp_write_amf_data(EMURLContext *s, char *param, uint8_t **p)
{
    char *field, *value;
    char type;
...
fail:
    av_em_log(s, AV_LOG_ERROR, "Invalid AMF parameter: %s\n", param);
 ...
 */
static int gen_connect(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;

    if ((ret = ff_em_rtmp_packet_create(&pkt, RTMP_SYSTEM_CHANNEL, RTMP_PT_INVOKE,
...
static int read_connect(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt = { 0 };
    uint8_t *p;
    const uint8_t *cp;
    ...
    if (ff_amf_read_string(&gbc, command, sizeof(command), &stringlen)) {
        av_em_log(s, AV_LOG_ERROR, "Unable to read command string\n");
        ff_em_rtmp_packet_destroy(&pkt);
        return AVERROR_INVALIDDATA;
    }
    if (strcmp(command, "connect")) {
        av_em_log(s, AV_LOG_ERROR, "Expecting connect, got %s\n", command);
        ff_em_rtmp_packet_destroy(&pkt);
        return AVERROR_INVALIDDATA;
    }
    ret = ff_amf_read_number(&gbc, &seqnum);
    if (ret)
        av_em_log(s, AV_LOG_WARNING, "SeqNum not found\n");
    ...
    if (ret)
        av_em_log(s, AV_LOG_WARNING, "App field not found in connect\n");
    if (!ret && strcmp(tmpstr, rt->app))
        av_em_log(s, AV_LOG_WARNING, "App field don't match up: %s <-> %s\n",
               tmpstr, rt->app);
    ff_em_rtmp_packet_destroy(&pkt);

    // Send Window Acknowledgement Size (as defined in specification)
    if ((ret = ff_em_rtmp_packet_create(&pkt, RTMP_NETWORK_CHANNEL,
    ...
    // Chunk size
    if ((ret = ff_em_rtmp_packet_create(&pkt, RTMP_SYSTEM_CHANNEL,
 ...
 */
static int gen_release_stream(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;
    ...
    av_em_log(s, AV_LOG_DEBUG, "Releasing stream...\n");
    p = pkt.data;
 ...
 */
static int gen_fcpublish_stream(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;
    ...
    av_em_log(s, AV_LOG_DEBUG, "FCPublish stream...\n");
    p = pkt.data;
 ...
 */
static int gen_fcunpublish_stream(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;
    ...
    av_em_log(s, AV_LOG_DEBUG, "UnPublishing stream...\n");
    p = pkt.data;
 ...
 */
static int gen_create_stream(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;

    av_em_log(s, AV_LOG_DEBUG, "Creating stream...\n");
 ...
 */
static int gen_delete_stream(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;

    av_em_log(s, AV_LOG_DEBUG, "Deleting stream...\n");
 ...
 * in seconds.
 */
static int gen_get_stream_length(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;

    if ((ret = ff_em_rtmp_packet_create(&pkt, RTMP_SOURCE_CHANNEL, RTMP_PT_INVOKE,
 ...
 */
static int gen_buffer_time(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;

    if ((ret = ff_em_rtmp_packet_create(&pkt, RTMP_NETWORK_CHANNEL, RTMP_PT_PING,
                                     1, 10)) < 0)
        return ret;
 ...
 */
static int gen_play(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;

    av_em_log(s, AV_LOG_DEBUG, "Sending play command for '%s'\n", rt->playpath);
...
static int gen_seek(EMURLContext *s, EMRTMPContext *rt, int64_t timestamp)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;

    av_em_log(s, AV_LOG_DEBUG, "Sending seek command for timestamp %"PRId64"\n",
           timestamp);

    if ((ret = ff_em_rtmp_packet_create(&pkt, 3, RTMP_PT_INVOKE, 0, 26)) < 0)
        return ret;
 ...
 */
static int gen_pause(EMURLContext *s, EMRTMPContext *rt, int pause, uint32_t timestamp)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;

    av_em_log(s, AV_LOG_DEBUG, "Sending pause command for timestamp %d\n",
           timestamp);

    if ((ret = ff_em_rtmp_packet_create(&pkt, 3, RTMP_PT_INVOKE, 0, 29)) < 0)
        return ret;
 ...
 */
static int gen_publish(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;

    av_em_log(s, AV_LOG_DEBUG, "Sending publish command for '%s'\n", rt->playpath);
 ...
 */
static int gen_pong(EMURLContext *s, EMRTMPContext *rt, RTMPPacket *ppkt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;
    ...
    if (ppkt->size < 6) {
        av_em_log(s, AV_LOG_ERROR, "Too short ping packet (%d)\n",
               ppkt->size);
        return AVERROR_INVALIDDATA;
    }
 ...
 */
static int gen_swf_verification(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;

    av_em_log(s, AV_LOG_DEBUG, "Sending SWF verification...\n");
    if ((ret = ff_em_rtmp_packet_create(&pkt, RTMP_NETWORK_CHANNEL, RTMP_PT_PING,
                                     0, 44)) < 0)
        return ret;
 ...
 */
static int gen_server_bw(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;

    if ((ret = ff_em_rtmp_packet_create(&pkt, RTMP_NETWORK_CHANNEL, RTMP_PT_SERVER_BW,
                                     0, 4)) < 0)
        return ret;
 ...
 */
static int gen_check_bw(EMURLContext *s, EMRTMPContext *rt)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;
 ...
 */
static int gen_bytes_read(EMURLContext *s, EMRTMPContext *rt, uint32_t ts)
{
    RTMPPacket pkt;
    uint8_t *p;
    int ret;

    if ((ret = ff_em_rtmp_packet_create(&pkt, RTMP_NETWORK_CHANNEL, RTMP_PT_BYTES_READ,
                                     ts, 4)) < 0)
        return ret;
...
static int gen_fcsubscribe_stream(EMURLContext *s, EMRTMPContext *rt,
                        ...
                        const uint8_t *key, int keylen, uint8_t *dst)
{
    ...
    hmac = av_em_hmac_alloc(AV_HMAC_SHA256);
    if (!hmac)
        return AVERROR(ENOMEM);

    av_em_hmac_init(hmac, key, keylen);
    if (gap <= 0) {
        av_em_hmac_update(hmac, src, len);
    } else { //skip 32 bytes used for storing digest
        av_em_hmac_update(hmac, src, gap);
        av_em_hmac_update(hmac, src + gap + 32, len - gap - 32);
    }
    av_em_hmac_final(hmac, dst, 32);

    av_em_hmac_free(hmac);
...
static int rtmp_calc_swf_verification(EMURLContext *s, EMRTMPContext *rt,
                                      uint8_t *buf)
{
    uint8_t *p;
    int ret;
                                     ...
                                     uint8_t **out_data, int64_t *out_size)
{
    ...
    do {
        ...
        if (!(ptr = av_em_realloc(*out_data, *out_size + size))) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        *out_data = ptr;
...
static int rtmp_calc_swfhash(EMURLContext *s)
{
    EMRTMPContext *rt = s->priv_data;
    ...
    if ((ret = ffurl_em_open_whitelist(&stream, rt->swfverify, AVIO_FLAG_READ,
                                    &s->interrupt_callback, NULL,
                                    s->protocol_whitelist, s->protocol_blacklist, s)) < 0) {
        av_em_log(s, AV_LOG_ERROR, "Cannot open connection %s.\n", rt->swfverify);
        goto fail;
    }

    if ((in_size = ffurl_em_seek(stream, 0, AVSEEK_SIZE)) < 0) {
        ret = AVERROR(EIO);
        goto fail;
    }

    if (!(in_data = av_em_alloc(in_size))) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }

    if ((ret = ffurl_em_read_complete(stream, in_data, in_size)) < 0)
        goto fail;
    ...
    av_em_opt_set_bin(rt, "rtmp_swfhash", swfhash, 32, 0);
    rt->swfsize = swfsize;

fail:
    av_em_freep(&in_data);
    av_em_freep(&out_data);
    ffurl_em_close(stream);
    return ret;
}
 ...
 */
static int rtmp_handshake(EMURLContext *s, EMRTMPContext *rt)
{
    AVLFG rnd;
    ...
    av_em_log(s, AV_LOG_DEBUG, "Handshaking...\n");

    av_em_lfg_init(&rnd, 0xDEADC0DE);
    ...
    if ((ret = ffurl_em_write(rt->stream, tosend,
                           RTMP_HANDSHAKE_PACKET_SIZE + 1)) < 0) {
        av_em_log(s, AV_LOG_ERROR, "Cannot write RTMP handshake request\n");
        return ret;
    }

    if ((ret = ffurl_em_read_complete(rt->stream, serverdata,
                                   RTMP_HANDSHAKE_PACKET_SIZE + 1)) < 0) {
        av_em_log(s, AV_LOG_ERROR, "Cannot read RTMP handshake response\n");
        return ret;
    }

    if ((ret = ffurl_em_read_complete(rt->stream, clientdata,
                                   RTMP_HANDSHAKE_PACKET_SIZE)) < 0) {
        av_em_log(s, AV_LOG_ERROR, "Cannot read RTMP handshake response\n");
        return ret;
    }

    av_em_log(s, AV_LOG_DEBUG, "Type answer %d\n", serverdata[0]);
    av_em_log(s, AV_LOG_DEBUG, "Server version %d.%d.%d.%d\n",
    ...
    if (rt->is_input && serverdata[5] >= 3) {
        ...
        if (!server_pos) {
            ...
            if (!server_pos) {
                av_em_log(s, AV_LOG_ERROR, "Server response validating failed\n");
                return AVERROR(EIO);
            }
        }
        ...
        if (memcmp(signature, clientdata + RTMP_HANDSHAKE_PACKET_SIZE - 32, 32)) {
            av_em_log(s, AV_LOG_ERROR, "Signature mismatch\n");
            return AVERROR(EIO);
        }
        ...
        if ((ret = ffurl_em_write(rt->stream, serverdata + 1,
                               RTMP_HANDSHAKE_PACKET_SIZE)) < 0)
            return ret;
...
static int rtmp_receive_hs_packet(EMRTMPContext* rt, uint32_t *first_int,
                                  uint32_t *second_int, char *arraydata,
                                  int size)
{
    int inoutsize;

    inoutsize = ffurl_em_read_complete(rt->stream, arraydata,
                                    RTMP_HANDSHAKE_PACKET_SIZE);
    if (inoutsize <= 0)
        return AVERROR(EIO);
    if (inoutsize != RTMP_HANDSHAKE_PACKET_SIZE) {
        av_em_log(rt, AV_LOG_ERROR, "Erroneous Message size %d"
...
static int rtmp_send_hs_packet(EMRTMPContext* rt, uint32_t first_int,
                               uint32_t second_int, char *arraydata, int size)
{
    ...
    inoutsize = ffurl_em_write(rt->stream, arraydata,
                            RTMP_HANDSHAKE_PACKET_SIZE);
    if (inoutsize != RTMP_HANDSHAKE_PACKET_SIZE) {
        av_em_log(rt, AV_LOG_ERROR, "Unable to write answer\n");
        return AVERROR(EIO);
    }
 ...
 */
static int rtmp_server_handshake(EMURLContext *s, EMRTMPContext *rt)
{
    ...
    inoutsize = ffurl_em_read_complete(rt->stream, buffer, 1);       // Receive C0
    if (inoutsize <= 0) {
        av_em_log(s, AV_LOG_ERROR, "Unable to read handshake\n");
        return AVERROR(EIO);
    }
    // Check Version
    if (buffer[0] != 3) {
        av_em_log(s, AV_LOG_ERROR, "RTMP protocol version mismatch\n");
        return AVERROR(EIO);
    }
    if (ffurl_em_write(rt->stream, buffer, 1) <= 0) {                 // Send S0
        av_em_log(s, AV_LOG_ERROR,
    ...
    ret = rtmp_receive_hs_packet(rt, &hs_epoch, &zeroes, hs_c1,
                                 RTMP_HANDSHAKE_PACKET_SIZE);
    if (ret) {
        av_em_log(s, AV_LOG_ERROR, "RTMP Handshake C1 Error\n");
        return ret;
    }
    /* Send S1 */
    ...
    for (randomidx = 8; randomidx < (RTMP_HANDSHAKE_PACKET_SIZE);
         randomidx += 4)
        AV_WB32(hs_s1 + randomidx, av_em_get_random_seed());

    ret = rtmp_send_hs_packet(rt, hs_my_epoch, 0, hs_s1,
                              RTMP_HANDSHAKE_PACKET_SIZE);
    if (ret) {
        av_em_log(s, AV_LOG_ERROR, "RTMP Handshake S1 Error\n");
        return ret;
    }
    /* Send S2 */
    ret = rtmp_send_hs_packet(rt, hs_epoch, 0, hs_c1,
                              RTMP_HANDSHAKE_PACKET_SIZE);
    if (ret) {
        av_em_log(s, AV_LOG_ERROR, "RTMP Handshake S2 Error\n");
        return ret;
    }
    /* Receive C2 */
    ret = rtmp_receive_hs_packet(rt, &temp, &zeroes, buffer,
                                 RTMP_HANDSHAKE_PACKET_SIZE);
    if (ret) {
        av_em_log(s, AV_LOG_ERROR, "RTMP Handshake C2 Error\n");
        return ret;
    }
    if (temp != hs_my_epoch)
...
static int handle_chunk_size(EMURLContext *s, RTMPPacket *pkt)
{
    ...
    if (rt->in_chunk_size <= 0) {
        av_em_log(s, AV_LOG_ERROR, "Incorrect chunk size %d\n",
               rt->in_chunk_size);
        return AVERROR_INVALIDDATA;
    }
    av_em_log(s, AV_LOG_DEBUG, "New incoming chunk size = %d\n",
           rt->in_chunk_size);

    return 0;
}

static int handle_ping(EMURLContext *s, RTMPPacket *pkt)
{
    EMRTMPContext *rt = s->priv_data;
    int t, ret;
    ...
    if (t == 6) {
        ...
        if (rt->swfsize) {
            ...
            av_em_log(s, AV_LOG_WARNING, "Ignoring SWFVerification request.\n");
        }
    }
...
static int handle_client_bw(EMURLContext *s, RTMPPacket *pkt)
{
    EMRTMPContext *rt = s->priv_data;
    ...
    if (rt->client_report_size <= 0) {
        av_em_log(s, AV_LOG_ERROR, "Incorrect client bandwidth %d\n",
                rt->client_report_size);
        return AVERROR_INVALIDDATA;

    }
    av_em_log(s, AV_LOG_DEBUG, "Client bandwidth = %d\n", rt->client_report_size);
    rt->client_report_size >>= 1;

    return 0;
}

static int handle_server_bw(EMURLContext *s, RTMPPacket *pkt)
{
    EMRTMPContext *rt = s->priv_data;
    ...
    if (rt->server_bw <= 0) {
        av_em_log(s, AV_LOG_ERROR, "Incorrect server bandwidth %d\n",
               rt->server_bw);
        return AVERROR_INVALIDDATA;
    }
    av_em_log(s, AV_LOG_DEBUG, "Server bandwidth = %d\n", rt->server_bw);

    return 0;
}

static int do_adobe_auth(EMRTMPContext *rt, const char *user, const char *salt,
                         const char *opaque, const char *challenge)
{
    uint8_t hash[16];
    char hashstr[AV_BASE64_SIZE(sizeof(hash))], challenge2[10];
    struct AVMD5 *md5 = av_em_md5_alloc();
    if (!md5)
        return AVERROR(ENOMEM);

    snprintf(challenge2, sizeof(challenge2), "%08x", av_em_get_random_seed());

    av_em_md5_init(md5);
    av_em_md5_update(md5, user, strlen(user));
    av_em_md5_update(md5, salt, strlen(salt));
    av_em_md5_update(md5, rt->password, strlen(rt->password));
    av_em_md5_final(md5, hash);
    av_em_base64_encode(hashstr, sizeof(hashstr), hash,
                     sizeof(hash));
    av_em_md5_init(md5);
    av_em_md5_update(md5, hashstr, strlen(hashstr));
    if (opaque)
        av_em_md5_update(md5, opaque, strlen(opaque));
    else if (challenge)
        av_em_md5_update(md5, challenge, strlen(challenge));
    av_em_md5_update(md5, challenge2, strlen(challenge2));
    av_em_md5_final(md5, hash);
    av_em_base64_encode(hashstr, sizeof(hashstr), hash,
                     sizeof(hash));
    ...
    if (opaque)
        av_em_strlcatf(rt->auth_params, sizeof(rt->auth_params),
                    "&opaque=%s", opaque);
...
static int do_llnw_auth(EMRTMPContext *rt, const char *user, const char *nonce)
{
    uint8_t hash[16];
    ...
    snprintf(cnonce, sizeof(cnonce), "%08x", av_em_get_random_seed());

    av_em_md5_init(md5);
    av_em_md5_update(md5, user, strlen(user));
    av_em_md5_update(md5, ":", 1);
    av_em_md5_update(md5, realm, strlen(realm));
    av_em_md5_update(md5, ":", 1);
    av_em_md5_update(md5, rt->password, strlen(rt->password));
    av_em_md5_final(md5, hash);
    em_data_to_hex(hashstr1, hash, 16, 1);
    hashstr1[32] = '\0';

    av_em_md5_init(md5);
    av_em_md5_update(md5, method, strlen(method));
    av_em_md5_update(md5, ":/", 2);
    av_em_md5_update(md5, rt->app, strlen(rt->app));
    if (!strchr(rt->app, '/'))
        av_em_md5_update(md5, "/_definst_", strlen("/_definst_"));
    av_em_md5_final(md5, hash);
    em_data_to_hex(hashstr2, hash, 16, 1);
    hashstr2[32] = '\0';

    av_em_md5_init(md5);
    av_em_md5_update(md5, hashstr1, strlen(hashstr1));
    av_em_md5_update(md5, ":", 1);
    if (nonce)
        av_em_md5_update(md5, nonce, strlen(nonce));
    av_em_md5_update(md5, ":", 1);
    av_em_md5_update(md5, nc, strlen(nc));
    av_em_md5_update(md5, ":", 1);
    av_em_md5_update(md5, cnonce, strlen(cnonce));
    av_em_md5_update(md5, ":", 1);
    av_em_md5_update(md5, qop, strlen(qop));
    av_em_md5_update(md5, ":", 1);
    av_em_md5_update(md5, hashstr2, strlen(hashstr2));
    av_em_md5_final(md5, hash);
    em_data_to_hex(hashstr1, hash, 16, 1);
...
static int handle_connect_error(EMURLContext *s, const char *desc)
{
    EMRTMPContext *rt = s->priv_data;
    ...
    if (!rt->username[0] || !rt->password[0]) {
        av_em_log(s, AV_LOG_ERROR, "No credentials set\n");
        return AVERROR_UNKNOWN;
    }

    if (strstr(desc, "?reason=authfailed")) {
        av_em_log(s, AV_LOG_ERROR, "Incorrect username/password\n");
        return AVERROR_UNKNOWN;
    } else if (strstr(desc, "?reason=nosuchuser")) {
        av_em_log(s, AV_LOG_ERROR, "Incorrect username\n");
        return AVERROR_UNKNOWN;
    }

    if (rt->auth_tried) {
        av_em_log(s, AV_LOG_ERROR, "Authentication failed\n");
        return AVERROR_UNKNOWN;
    }
    ...
    if (!(cptr = strstr(desc, "?reason=needauth"))) {
        av_em_log(s, AV_LOG_ERROR, "No auth parameters found\n");
        return AVERROR_UNKNOWN;
    }

    av_em_strlcpy(buf, cptr + 1, sizeof(buf));
    ptr = buf;
        ...
        if (value) {
            *value++ = '\0';
            if (!strcmp(ptr, "user")) {
                ...
                av_em_log(s, AV_LOG_INFO, "Ignoring unsupported var %s\n", ptr);
            }
        } else {
            av_em_log(s, AV_LOG_WARNING, "Variable %s has NULL value\n", ptr);
        }
        ptr = next;
    }
...
static int handle_invoke_error(EMURLContext *s, RTMPPacket *pkt)
{
    EMRTMPContext *rt = s->priv_data;
    ...
    if (!ff_amf_get_field_value(pkt->data + 9, data_end,
        ...
        if (tracked_method && (!strcmp(tracked_method, "_checkbw")      ||
            ...
            ret = AVERROR_UNKNOWN;
        av_em_log(s, level, "Server error: %s\n", tmpstr);
    }

    av_em_free(tracked_method);
    return ret;
}

static int write_begin(EMURLContext *s)
{
    EMRTMPContext *rt = s->priv_data;
    PutByteContext pbc;
    RTMPPacket spkt = { 0 };
    ...
    // Send Stream Begin 1
    if ((ret = ff_em_rtmp_packet_create(&spkt, RTMP_NETWORK_CHANNEL,
                                     RTMP_PT_PING, 0, 6)) < 0) {
        av_em_log(s, AV_LOG_ERROR, "Unable to create response packet\n");
        return ret;
    }
    ...
    ff_em_rtmp_packet_destroy(&spkt);

    return ret;
}

static int write_status(EMURLContext *s, RTMPPacket *pkt,
                        const char *status, const char *filename)
{
    ...
    if ((ret = ff_em_rtmp_packet_create(&spkt, RTMP_SYSTEM_CHANNEL,
                                     RTMP_PT_INVOKE, 0,
                                     RTMP_PKTDATA_DEFAULT_SIZE)) < 0) {
        av_em_log(s, AV_LOG_ERROR, "Unable to create response packet\n");
        return ret;
...
static int send_invoke_response(EMURLContext *s, RTMPPacket *pkt)
{
    EMRTMPContext *rt = s->priv_data;
    double seqnum;
    char filename[64];
    ...
    if (ff_amf_read_string(&gbc, command, sizeof(command),
                           &stringlen)) {
        av_em_log(s, AV_LOG_ERROR, "Error in PT_INVOKE\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (!strcmp(command, "FCPublish") ||
        ...
        if (s->filename) {
            ...
            if (strcmp(pchar, filename))
                av_em_log(s, AV_LOG_WARNING, "Unexpected stream %s, expecting"
                       " %s\n", filename, pchar);
        }
...
static int handle_invoke_result(EMURLContext *s, RTMPPacket *pkt)
{
    EMRTMPContext *rt = s->priv_data;
    char *tracked_method = NULL;
    int ret = 0;
    ...
    if (!strcmp(tracked_method, "connect")) {
        ...
        if (read_number_result(pkt, &stream_id)) {
            av_em_log(s, AV_LOG_WARNING, "Unexpected reply on connect()\n");
        } else {
        ...
        if (read_number_result(pkt, &rt->duration)) {
            av_em_log(s, AV_LOG_WARNING, "Unexpected reply on getStreamLength()\n");
        }
    }
...
static int handle_invoke_status(EMURLContext *s, RTMPPacket *pkt)
{
    EMRTMPContext *rt = s->priv_data;
    const uint8_t *data_end = pkt->data + pkt->size;
    ...
    if (!t && !strcmp(tmpstr, "error")) {
        ...
        if (!t)
            av_em_log(s, AV_LOG_ERROR, "Server error: %s\n", tmpstr);
        return -1;
    }
...
static int handle_invoke(EMURLContext *s, RTMPPacket *pkt)
{
    EMRTMPContext *rt = s->priv_data;
    int ret = 0;
...
static int update_offset(EMRTMPContext *rt, int size)
{
    int old_flv_size;
...
static int append_flv_data(EMRTMPContext *rt, RTMPPacket *pkt, int skip)
{
    ...
    if ((ret = av_em_reallocp(&rt->flv_data, rt->flv_size)) < 0) {
...
static int handle_notify(EMURLContext *s, RTMPPacket *pkt)
{
    EMRTMPContext *rt  = s->priv_data;
 ...
 */
static int rtmp_parse_result(EMURLContext *s, EMRTMPContext *rt, RTMPPacket *pkt)
{
    int ret;
    ...
    case RTMP_PT_BYTES_READ:
        av_em_log(s, AV_LOG_TRACE, "received bytes read report\n");
        break;
    ...
    case RTMP_PT_NOTIFY:
        ...
        break;
    default:
        av_em_log(s, AV_LOG_VERBOSE, "Unknown packet type received 0x%02X\n", pkt->type);
        break;
    }
    return 0;
}

static int handle_metadata(EMRTMPContext *rt, RTMPPacket *pkt)
{
    ...
    if (p != rt->flv_data + rt->flv_size) {
        av_em_log(NULL, AV_LOG_WARNING, "Incomplete flv packets in "
 ...
 */
static int get_packet(EMURLContext *s, int for_header)
{
    EMRTMPContext *rt = s->priv_data;
    int ret;
    ...
    for (;;) {
        ...
        if (rt->bytes_read - rt->last_bytes_read > rt->client_report_size) {
            av_em_log(s, AV_LOG_DEBUG, "Sending bytes read report\n");
        ...
        if (rt->state == STATE_SEEKING) {
            ff_em_rtmp_packet_destroy(&rpkt);
...
static int rtmp_close(EMURLContext *h)
{
    EMRTMPContext *rt = h->priv_data;
    int ret = 0, i, j;

    if (!rt->is_input) {
        rt->flv_data = NULL;
        if (rt->out_pkt.size)
            ff_em_rtmp_packet_destroy(&rt->out_pkt);
    ...
    for (i = 0; i < 2; i++) {
        for (j = 0; j < rt->nb_prev_pkt[i]; j++)
            ff_em_rtmp_packet_destroy(&rt->prev_pkt[i][j]);
        av_em_freep(&rt->prev_pkt[i]);
    }

    free_tracked_methods(rt);
    av_em_freep(&rt->flv_data);
    ffurl_em_close(rt->stream);
    return ret;
}
 ...
 * @return 0 for successful operation, negative value in case of error
 */
static int inject_fake_duration_metadata(EMRTMPContext *rt)
{
    ...
    if (!(rt->flv_data = av_em_alloc(rt->flv_size + 55))) {
 ...
 */
static int rtmp_open(EMURLContext *s, const char *uri, int flags)
{
    EMRTMPContext *rt = s->priv_data;
    ...
    if (rt->listen_timeout > 0)
        rt->listen = 1;
    //add by chenhd, eastmoney
//    rt->app_ctx = (AVApplicationContext *)(intptr_t)rt->app_ctx_intptr;
    rt->app_ctx = (AVApplicationContext *)av_em_dict_strtoptr(rt->app_ctx_intptr);
    /////////////////////////
    rt->is_input = !(flags & AVIO_FLAG_WRITE);

    av_em_url_split(proto, sizeof(proto), auth, sizeof(auth),
    ...
    if (auth[0]) {
        ...
        if (ptr) {
            *ptr = '\0';
            av_em_strlcpy(rt->username, auth, sizeof(rt->username));
            av_em_strlcpy(rt->password, ptr + 1, sizeof(rt->password));
        }
    }

    if (rt->listen && strcmp(proto, "rtmp")) {
        av_em_log(s, AV_LOG_ERROR, "rtmp_listen not available for %s\n",
               proto);
        return AVERROR(EINVAL);
    }
    if (!strcmp(proto, "rtmpt") || !strcmp(proto, "rtmpts")) {
        if (!strcmp(proto, "rtmpts"))
            av_em_dict_set(&opts, "ffrtmphttp_tls", "1", 1);

        /* open the http tunneling connection */
        em_url_join(buf, sizeof(buf), "ffrtmphttp", NULL, hostname, port, NULL);
    } else if (!strcmp(proto, "rtmps")) {
        /* open the tls connection */
        if (port < 0)
            port = RTMPS_DEFAULT_PORT;
        em_url_join(buf, sizeof(buf), "tls", NULL, hostname, port, NULL);
    } else if (!strcmp(proto, "rtmpe") || (!strcmp(proto, "rtmpte"))) {
        if (!strcmp(proto, "rtmpte"))
            av_em_dict_set(&opts, "ffrtmpcrypt_tunneling", "1", 1);

        /* open the encrypted connection */
        em_url_join(buf, sizeof(buf), "ffrtmpcrypt", NULL, hostname, port, NULL);
        rt->encrypted = 1;
    } else {
        ...
        if (rt->listen)
            em_url_join(buf, sizeof(buf), "tcp", NULL, hostname, port,
                        "?listen&listen_timeout=%d",
                        rt->listen_timeout * 1000);
        else if (rt->use_ijktcphook){ 
            av_em_log(s, AV_LOG_INFO, "use ijktcphook protocol,hostname:%s, port:%d\n", hostname, port);
            em_url_join(buf, sizeof(buf), "ijktcphook", NULL, hostname, port, NULL);
//            av_em_dict_set_int(&opts, "ijkapplication", (int64_t)(intptr_t)rt->app_ctx, 0);
            av_em_dict_set_intptr(&opts, "ijkapplication", (uintptr_t)rt->app_ctx, 0);
        }
        else{
            printf("use tcp protocol,hostname:%s, port:%d\n", hostname, port);
            av_em_log(s, AV_LOG_INFO, "use tcp protocol,hostname:%s, port:%d\n", hostname, port);
            em_url_join(buf, sizeof(buf), "tcp", NULL, hostname, port, NULL);
        }
    }

reconnect:
    if ((ret = ffurl_em_open_whitelist(&rt->stream, buf, AVIO_FLAG_READ_WRITE,
    ...
    rt->app = av_em_alloc(APP_MAX_LENGTH);
    if (!rt->app) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    if (qmark && strstr(qmark, "slist=")) {
        ...
        av_em_strlcpy(rt->app, path + 1, APP_MAX_LENGTH);
        fname = strstr(path, "slist=") + 6;
        // Strip any further query parameters from fname
        amp = strchr(fname, '&');
        if (amp) {
            av_em_strlcpy(fname_buffer, fname, FFMIN(amp - fname + 1,
                                                  sizeof(fname_buffer)));
            fname = fname_buffer;
        }
        ...
        if (!p) {
            if (old_app) {
                ...
                fname = NULL;
                av_em_strlcpy(rt->app, next, APP_MAX_LENGTH);
            }
        } else {
            ...
            if (!fname || (c && c < fname)) {
                fname = p + 1;
                av_em_strlcpy(rt->app, path + 1, FFMIN(p - path, APP_MAX_LENGTH));
            } else {
                fname++;
                av_em_strlcpy(rt->app, path + 1, FFMIN(fname - path - 1, APP_MAX_LENGTH));
            }
        }
    }
    ...
    if (!rt->playpath) {
        rt->playpath = av_em_alloc(PLAYPATH_MAX_LENGTH);
        if (!rt->playpath) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        ...
        if (fname) {
            ...
            av_em_strlcat(rt->playpath, fname, PLAYPATH_MAX_LENGTH);
        } else {
            rt->playpath[0] = '\0';
        }
    }
    ...
    if (!rt->tcurl) {
        rt->tcurl = av_em_alloc(TCURL_MAX_LENGTH);
        if (!rt->tcurl) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        em_url_join(rt->tcurl, TCURL_MAX_LENGTH, proto, NULL, hostname,
                    port, "/%s", rt->app);
    }

    if (!rt->flashver) {
        rt->flashver = av_em_alloc(FLASHVER_MAX_LENGTH);
        if (!rt->flashver) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
    ...
    av_em_log(s, AV_LOG_DEBUG, "Proto = %s, path = %s, app = %s, fname = %s\n",
...
static int rtmp_read(EMURLContext *s, uint8_t *buf, int size)
{
    EMRTMPContext *rt = s->priv_data;
    int orig_size = size;
    int ret;
...
static int64_t rtmp_seek(EMURLContext *s, int stream_index, int64_t timestamp,
                         int flags)
{
    EMRTMPContext *rt = s->priv_data;
    int ret;
...
static int rtmp_pause(EMURLContext *s, int pause)
{
    EMRTMPContext *rt = s->priv_data;
    int ret;
    av_em_log(s, AV_LOG_DEBUG, "Pause at timestamp %d\n",
           rt->last_timestamp);
    if ((ret = gen_pause(s, rt, pause, rt->last_timestamp)) < 0) {
        av_em_log(s, AV_LOG_ERROR, "Unable to send pause command at timestamp %d\n",
               rt->last_timestamp);
        return ret;
    }
    return 0;
...
static int rtmp_write(EMURLContext *s, const uint8_t *buf, int size)
{
    EMRTMPContext *rt = s->priv_data;
    int size_temp = size;
    ...
    do {
        ...
        if (rt->flv_header_bytes < RTMP_HEADER) {
            ...
            if ((ret = ff_em_rtmp_packet_create(&rt->out_pkt, channel,
            ...
            if (rt->out_pkt.type == RTMP_PT_NOTIFY) {
                ...
                if (!ff_amf_read_string(&gbc, commandbuffer, sizeof(commandbuffer),
                    ...
                    if (!strcmp(commandbuffer, "onMetaData") ||
                        ...
                        uint8_t *ptr;
                        if ((ret = av_em_reallocp(&rt->out_pkt.data, rt->out_pkt.size + 16)) < 0) {
    ...
    ret = ffurl_em_read(rt->stream, &c, 1);
...
#define OFFSET(x) offsetof(EMRTMPContext, x)
...
static const AVOption rtmp_options[] = {
    ...
    {"use_ijktcphook", "use ijk tcp hook read rtmp packets", OFFSET(use_ijktcphook), AV_OPT_TYPE_INT, {.i64 = 0}, 0, 1, DEC},
    { "ijkapplication", "AVApplicationContext", OFFSET(app_ctx_intptr), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, .flags = DEC },
	{ NULL },
};

#define RTMP_PROTOCOL(flavor)                    \
static const AVEMClass flavor##_class = {          \
    .class_name = #flavor,                       \
    ...
    .version    = LIBAVUTIL_VERSION_INT,         \
                                                 ...
                                                 \
const EMURLProtocol em_##flavor##_protocol = {     \
    .name           = #flavor,                   \
    .url_open       = rtmp_open,                 \
    .url_read       = rtmp_read,                 \
    ...
    .priv_data_size = sizeof(EMRTMPContext),       \
