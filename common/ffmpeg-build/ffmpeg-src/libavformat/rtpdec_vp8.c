...
static int vp8_broken_sequence(AVEMFormatContext *ctx, PayloadContext *vp8,
                               const char *msg)
{
    vp8->sequence_ok = 0;
    av_em_log(ctx, AV_LOG_WARNING, "%s", msg);
    emio_free_dyn_buf(&vp8->data);
    return AVERROR(EAGAIN);
}

static int vp8_handle_packet(AVEMFormatContext *ctx, PayloadContext *vp8,
                             ...
                             int flags)
{
    ...
    if (start_partition && part_id == 0 && len >= 3) {
        ...
        if (!non_key) {
            ...
            int can_continue = vp8->data && !vp8->is_keyframe &&
                               avio_em_tell(vp8->data) >= vp8->first_part_size;
            if (!vp8->sequence_ok)
                return AVERROR(EAGAIN);
            if (!vp8->got_keyframe)
            ...
            if (vp8->data) {
                ...
                if (avio_em_tell(vp8->data) >= vp8->first_part_size) {
                    int ret = ff_rtp_finalize_packet(pkt, &vp8->data, st->index);
                    if (ret < 0)
                        return ret;
                    pkt->flags |= AV_PKT_FLAG_CORRUPT;
        ...
        if ((res = avio_em_open_dyn_buf(&vp8->data)) < 0)
            return res;
        ...
        if (seq != expected_seq) {
            ...
            } else if (vp8->data && avio_em_tell(vp8->data) >= vp8->first_part_size) {
                vp8->broken_frame = 1;
                vp8->sequence_dirty = 1;
            } else {
                return vp8_broken_sequence(ctx, vp8,
    ...
    vp8->prev_seq = seq;
    if (!vp8->broken_frame)
        avio_em_write(vp8->data, buf, len);
...
static av_cold int vp8_init(AVEMFormatContext *s, int st_index, PayloadContext *vp8)
{
    vp8->sequence_ok = 1;
    return 0;
}
