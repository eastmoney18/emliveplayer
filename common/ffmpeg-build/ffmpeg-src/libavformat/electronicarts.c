...
static uint32_t read_arbitrary(AVEMIOContext *pb)
{
    uint8_t size, byte;
    int i;
    uint32_t word;
...
static int process_audio_header_elements(AVEMFormatContext *s)
{
    ...
    while (!avio_em_feof(pb) && in_header) {
        int in_subheader;
        uint8_t byte;
        byte = avio_em_r8(pb);
        ...
        switch (byte) {
        case 0xFD:
            av_em_log(s, AV_LOG_DEBUG, "entered audio subheader\n");
            in_subheader = 1;
            while (!avio_em_feof(pb) && in_subheader) {
                uint8_t subbyte;
                subbyte = avio_em_r8(pb);
                ...
                case 0x8A:
                    ...
                    av_em_log(s, AV_LOG_DEBUG, "exited audio subheader\n");
                    in_subheader = 0;
                    break;
                case 0xA0:
            ...
            break;
        case 0xFF:
            av_em_log(s, AV_LOG_DEBUG, "end of header block reached\n");
            in_header = 0;
            break;
        default:
            av_em_log(s, AV_LOG_DEBUG,
    ...
    case -1:
        ...
        case -1:
            break;
        default:
            avpriv_em_request_sample(s, "stream type; revision=%i", revision);
            return 0;
        }
        switch (revision2) {
        case  8:
        ...
        case 10:
            ...
            default:
                avpriv_em_request_sample(s, "stream type; revision=%i, revision2=%i", revision, revision2);
                return 0;
            }
            break;
        case 15:
        ...
        default:
            ...
            avpriv_em_request_sample(s, "stream type; revision2=%i", revision2);
            return 0;
        }
        break;
    default:
...
static void process_audio_header_eacs(AVEMFormatContext *s)
{
    EaDemuxContext *ea = s->priv_data;
    AVEMIOContext *pb    = s->pb;
    int compression_type;

    ea->sample_rate  = ea->big_endian ? avio_em_rb32(pb) : avio_em_rl32(pb);
    ea->bytes        = avio_em_r8(pb);   /* 1=8-bit, 2=16-bit */
    ea->num_channels = avio_em_r8(pb);
    compression_type = avio_em_r8(pb);
    avio_em_skip(pb, 13);
...
static void process_audio_header_sead(AVEMFormatContext *s)
{
    EaDemuxContext *ea = s->priv_data;
    AVEMIOContext *pb    = s->pb;

    ea->sample_rate  = avio_em_rl32(pb);
    ea->bytes        = avio_em_rl32(pb);  /* 1=8-bit, 2=16-bit */
    ea->num_channels = avio_em_rl32(pb);
    ea->audio_codec  = AV_CODEC_ID_ADPCM_IMA_EA_SEAD;
}

static void process_video_header_mdec(AVEMFormatContext *s, VideoProperties *video)
{
    AVEMIOContext *pb    = s->pb;
    avio_em_skip(pb, 4);
    video->width       = avio_em_rl16(pb);
    video->height      = avio_em_rl16(pb);
    video->time_base   = (AVEMRational) { 1, 15 };
    video->codec = AV_CODEC_ID_MDEC;
}

static int process_video_header_vp6(AVEMFormatContext *s, VideoProperties *video)
{
    AVEMIOContext *pb = s->pb;

    avio_em_skip(pb, 8);
    video->nb_frames = avio_em_rl32(pb);
    avio_em_skip(pb, 4);
    video->time_base.den = avio_em_rl32(pb);
    video->time_base.num = avio_em_rl32(pb);
    if (video->time_base.den <= 0 || video->time_base.num <= 0) {
        av_em_log(s, AV_LOG_ERROR, "Timebase is invalid\n");
...
static void process_video_header_cmv(AVEMFormatContext *s, VideoProperties *video)
{
    int fps;

    avio_em_skip(s->pb, 10);
    fps = avio_em_rl16(s->pb);
    if (fps)
        video->time_base = (AVEMRational) { 1, fps };
    video->codec = AV_CODEC_ID_CMV;
}

/* Process EA file header.
 * Return 1 if the EA file is valid and successfully opened, 0 otherwise. */
static int process_ea_header(AVEMFormatContext *s)
{
    ...
    for (i = 0; i < 5 && (!ea->audio_codec || !ea->video.codec); i++) {
        uint64_t startpos     = avio_em_tell(pb);
        int err               = 0;

        blockid = avio_em_rl32(pb);
        ...
        if (size < 8) {
            av_em_log(s, AV_LOG_ERROR, "chunk size too small\n");
            return AVERROR_INVALIDDATA;
        }

        switch (blockid) {
        case ISNh_TAG:
            if (avio_em_rl32(pb) != EACS_TAG) {
                avpriv_em_request_sample(s, "unknown 1SNh headerid");
                return 0;
            }
        ...
        case TGQs_TAG:
            ...
            ea->video.time_base   = (AVEMRational) { 1, 15 };
            break;
        ...
        case MADk_TAG:
            ...
            avio_em_skip(pb, 6);
            ea->video.time_base = (AVEMRational) { avio_em_rl16(pb), 1000 };
            break;
        ...
        if (err < 0) {
            av_em_log(s, AV_LOG_ERROR, "error parsing header: %i\n", err);
            return err;
        }

        avio_em_seek(pb, startpos + size, SEEK_SET);
    }
...
static int ea_probe(AVEMProbeData *p)
{
...
static int init_video_stream(AVEMFormatContext *s, VideoProperties *video)
{
    AVEMStream *st;
    ...
    if (video->time_base.num)
        avpriv_em_set_pts_info(st, 64, video->time_base.num, video->time_base.den);
    st->r_frame_rate       =
    st->avg_frame_rate     = av_em_inv_q(video->time_base);
    return 0;
}

static int ea_read_header(AVEMFormatContext *s)
{
    EaDemuxContext *ea = s->priv_data;
    AVEMStream *st;
    ...
    if (ea->audio_codec) {
        ...
        if (!st)
            return AVERROR(ENOMEM);
        avpriv_em_set_pts_info(st, 33, 1, ea->sample_rate);
...
static int ea_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    EaDemuxContext *ea = s->priv_data;
    AVEMIOContext *pb    = s->pb;
    ...
    while (!packet_read || partial_packet) {
        chunk_type = avio_em_rl32(pb);
        chunk_size = ea->big_endian ? avio_em_rb32(pb) : avio_em_rl32(pb);
        if (chunk_size < 8)
            return AVERROR_INVALIDDATA;
        chunk_size -= 8;
        ...
        case SDEN_TAG:
            if (!ea->audio_codec) {
                avio_em_skip(pb, chunk_size);
                break;
            } else if (ea->audio_codec == AV_CODEC_ID_PCM_S16LE_PLANAR ||
                       ea->audio_codec == AV_CODEC_ID_MP3) {
                num_samples = avio_em_rl32(pb);
                avio_em_skip(pb, 8);
                chunk_size -= 12;
            ...
            if (partial_packet) {
                avpriv_em_request_sample(s, "video header followed by audio packet");
            ...
            ret = av_em_get_packet(pb, pkt, chunk_size);
            if (ret < 0)
                return ret;
            ...
            case AV_CODEC_ID_ADPCM_EA_R3:
                if (pkt->size < 4) {
                    av_em_log(s, AV_LOG_ERROR, "Packet is too short\n");
                    av_em_packet_unref(pkt);
                    return AVERROR_INVALIDDATA;
                }
        ...
        case MADm_TAG:
        case MADe_TAG:
            avio_em_seek(pb, -8, SEEK_CUR);    // include chunk preamble
            chunk_size += 8;
        ...
        case mTCD_TAG:
            ...
            avio_em_skip(pb, 8);               // skip ea DCT header
            chunk_size -= 8;
            goto get_video_packet;
...
get_video_packet:
            ...
            if (partial_packet) {
                ret = av_em_append_packet(pb, pkt, chunk_size);
            } else
                ret = av_em_get_packet(pb, pkt, chunk_size);
            if (ret < 0) {
                packet_read = 1;
