...
static int hnm_probe(AVEMProbeData *p)
{
    if (p->buf_size < 4)
        return 0;
...
static int hnm_read_header(AVEMFormatContext *s)
{
    ...
    avio_em_skip(pb, 8);
    hnm->width     = avio_em_rl16(pb);
    hnm->height    = avio_em_rl16(pb);
    hnm->filesize  = avio_em_rl32(pb);
    hnm->frames    = avio_em_rl32(pb);
    hnm->taboffset = avio_em_rl32(pb);
    hnm->bits      = avio_em_rl16(pb);
    hnm->channels  = avio_em_rl16(pb);
    hnm->framesize = avio_em_rl32(pb);
    avio_em_skip(pb, 32);
    ...
    vst->codecpar->extradata  = av_em_mallocz(1);
    ...
    avpriv_em_set_pts_info(vst, 33, 1, HNM4_FRAME_FPS);

    return 0;
}

static int hnm_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    Hnm4DemuxContext *hnm = s->priv_data;
    AVEMIOContext *pb = s->pb;
    int ret = 0;
    ...
    if (hnm->superchunk_remaining == 0) {
        ...
        superchunk_size = avio_em_rl24(pb);
        avio_em_skip(pb, 1);

        hnm->superchunk_remaining = superchunk_size - 4;
    }

    chunk_size = avio_em_rl24(pb);
    avio_em_skip(pb, 1);
    chunk_id = avio_em_rl16(pb);
    avio_em_skip(pb, 2);

    if (chunk_size > hnm->superchunk_remaining || !chunk_size) {
        av_em_log(s, AV_LOG_ERROR,
               "invalid chunk size: %"PRIu32", offset: %"PRId64"\n",
               chunk_size, avio_em_tell(pb));
        avio_em_skip(pb, hnm->superchunk_remaining - 8);
        hnm->superchunk_remaining = 0;
    }
    ...
    default:
        av_em_log(s, AV_LOG_WARNING, "unknown chunk found: %"PRIu16", offset: %"PRId64"\n",
               chunk_id, avio_em_tell(pb));
        avio_em_skip(pb, chunk_size - 8);
        hnm->superchunk_remaining -= chunk_size;
        break;
    }
...
static int hnm_read_close(AVEMFormatContext *s)
{
    Hnm4DemuxContext *hnm = s->priv_data;

    if (hnm->vpkt.size > 0)
        av_em_packet_unref(&hnm->vpkt);

    return 0;
}

AVEMInputFormat em_hnm_demuxer = {
    .name           = "hnm",
