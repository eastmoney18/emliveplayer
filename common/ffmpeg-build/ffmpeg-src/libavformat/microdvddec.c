...
static int microdvd_probe(AVEMProbeData *p)
{
    unsigned char c;
...
static int microdvd_read_header(AVEMFormatContext *s)
{
    AVEMRational pts_info = (AVEMRational){ 2997, 125 };  /* default: 23.976 fps */
    ...
    while (!avio_em_feof(s->pb)) {
        ...
        int len = em_get_line(s->pb, line_buf, sizeof(line_buf));
        char *line = line_buf;
        ...
        if (i++ < 3) {
            ...
            if ((sscanf(line, "{%d}{}%6lf",    &frame, &fps) == 2 ||
                ...
                pts_info = av_em_d2q(fps, 100000);
                has_real_fps = 1;
                continue;
            }
            if (!st->codecpar->extradata && sscanf(line, "{DEFAULT}{}%c", &c) == 1) {
                st->codecpar->extradata = av_em_strdup(line + 11);
...
#define SKIP_FRAME_ID                                       \
    p = strchr(p, '}');                                     \
    if (!p) {                                               \
        av_em_log(s, AV_LOG_WARNING, "Invalid event \"%s\""    \
               " at line %d\n", line, i);                   \
        continue;                                           \
    }                                                       \
    p++
    ...
    avpriv_em_set_pts_info(st, 64, pts_info.den, pts_info.num);
    st->codecpar->codec_type = AVMEDIA_TYPE_SUBTITLE;
    st->codecpar->codec_id   = AV_CODEC_ID_MICRODVD;
    return 0;
}

static int microdvd_read_packet(AVEMFormatContext *s, AVEMPacket *pkt)
{
    MicroDVDContext *microdvd = s->priv_data;
    return ff_subtitles_queue_read_packet(&microdvd->q, pkt);
}

static int microdvd_read_seek(AVEMFormatContext *s, int stream_index,
...
static int microdvd_read_close(AVEMFormatContext *s)
{
    MicroDVDContext *microdvd = s->priv_data;
...
static const AVEMClass microdvd_class = {
...
AVEMInputFormat ff_microdvd_demuxer = {
    .name           = "microdvd",
