...
static int qt_rtp_parse_packet(AVEMFormatContext *s, PayloadContext *qt,
                               ...
                               int len, uint16_t seq, int flags)
{
    ...
    if (qt->remaining) {
        ...
        if (av_em_new_packet(pkt, qt->bytes_per_frame))
    ...
    if (has_payload_desc) {
        ...
        if (!is_start || !is_finish) {
            avpriv_em_request_sample(s, "RTP-X-QT with payload description "
        ...
        if ((st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO &&
            ...
            (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO &&
                 tag != MKTAG('s','o','u','n')))
            return AVERROR_INVALIDDATA;
        avpriv_em_set_pts_info(st, 32, 1, avio_em_rb32(&pb));

        if (pos + data_len > len)
            return AVERROR_INVALIDDATA;
        /* TLVs */
        while (avio_em_tell(&pb) + 4 < pos + data_len) {
            int tlv_len = avio_em_rb16(&pb);
            tag = avio_em_rl16(&pb);
            if (avio_em_tell(&pb) + tlv_len > pos + data_len)
                return AVERROR_INVALIDDATA;
...
#define MKTAG16(a,b) MKTAG(a,b,0,0)
            ...
            case MKTAG16('s','d'): {
                ...
                MOVContext *mc = av_em_mallocz(sizeof(*mc));
                if (!mc)
                    return AVERROR(ENOMEM);
                mc->fc = s;
                st->priv_data = msc = av_em_mallocz(sizeof(MOVStreamContext));
                if (!msc) {
                    av_em_free(mc);
                    st->priv_data = priv_data;
                    return AVERROR(ENOMEM);
                }
                /* em_mov_read_stsd_entries updates stream s->nb_streams-1,
                 * so set it temporarily to indicate which stream to update. */
                s->nb_streams = st->index + 1;
                em_mov_read_stsd_entries(mc, &pb, 1);
            ...
            default:
                avio_em_skip(&pb, tlv_len);
                break;
            }
        }

        /* 32-bit alignment */
        avio_em_skip(&pb, ((avio_em_tell(&pb) + 3) & ~3) - avio_em_tell(&pb));
    } else
        avio_em_seek(&pb, 4, SEEK_SET);

    if (has_packet_info) {
        avpriv_em_request_sample(s, "RTP-X-QT with packet-specific info");
        return AVERROR_PATCHWELCOME;
    }

    alen = len - avio_em_tell(&pb);
    if (alen <= 0)
        return AVERROR_INVALIDDATA;
    ...
    case 3: /* one data packet spread over 1 or multiple RTP packets */
        if (qt->pkt.size > 0 && qt->timestamp == *timestamp) {
            int err;
            if ((err = av_em_reallocp(&qt->pkt.data, qt->pkt.size + alen +
            ...
            av_em_freep(&qt->pkt.data);
            av_em_init_packet(&qt->pkt);
            qt->pkt.data = av_em_realloc(NULL, alen + AV_INPUT_BUFFER_PADDING_SIZE);
            if (!qt->pkt.data)
                return AVERROR(ENOMEM);
            qt->pkt.size = 0;
            qt->timestamp = *timestamp;
        }
        memcpy(qt->pkt.data + qt->pkt.size, buf + avio_em_tell(&pb), alen);
        qt->pkt.size += alen;
        if (has_marker_bit) {
            int ret = av_em_packet_from_data(pkt, qt->pkt.data, qt->pkt.size);
            if (ret < 0)
                return ret;
    ...
    case 1: /* constant packet size, multiple packets per RTP packet */
        ...
        if (av_em_new_packet(pkt, qt->bytes_per_frame))
            return AVERROR(ENOMEM);
        memcpy(pkt->data, buf + avio_em_tell(&pb), qt->bytes_per_frame);
        pkt->flags = keyframe ? AV_PKT_FLAG_KEY : 0;
        pkt->stream_index = st->index;
        if (qt->remaining > 0) {
            av_em_freep(&qt->pkt.data);
            qt->pkt.data = av_em_realloc(NULL, qt->remaining * qt->bytes_per_frame);
            if (!qt->pkt.data) {
            ...
            memcpy(qt->pkt.data,
                   buf + avio_em_tell(&pb) + qt->bytes_per_frame,
    ...
    default:  /* unimplemented */
        avpriv_em_request_sample(NULL, "RTP-X-QT with packing scheme 2");
        return AVERROR_PATCHWELCOME;
    }
}
