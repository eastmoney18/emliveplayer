...
typedef struct MyAVEMPacketList {
    AVEMPacket pkt;
    struct MyAVEMPacketList *next;
    int serial;
} MyAVEMPacketList;

typedef struct PacketQueue {
    MyAVEMPacketList *first_pkt, *last_pkt;
    int nb_packets;
    int size;
    int64_t duration;
    int abort_request;
...
static AVEMInputFormat *file_iformat;
...
{
    MyAVEMPacketList *pkt1;

    if (q->abort_request)
       return -1;

    pkt1 = av_malloc(sizeof(MyAVEMPacketList));
    if (!pkt1)
        return -1;
    pkt1->pkt = *pkt;
    pkt1->next = NULL;
    ...
    do {
        ...
        switch (d->avctx->codec_type) {
            ...
            case AVMEDIA_TYPE_AUDIO:
                ...
                if (got_frame) {
                    AVEMRational tb = (AVEMRational){1, frame->sample_rate};
                    if (frame->pts != AV_NOPTS_VALUE)
                        frame->pts = av_rescale_q(frame->pts, d->avctx->time_base, tb);
                    else if (frame->pkt_pts != AV_NOPTS_VALUE)
                        frame->pts = av_rescale_q(frame->pkt_pts, av_em_codec_get_pkt_timebase(d->avctx), tb);
...
static void calculate_display_rect(SDL_Rect *rect,
                                   ...
                                   int pic_width, int pic_height, AVEMRational pic_sar)
{
    float aspect_ratio;
...
static void set_default_window_size(int width, int height, AVEMRational sar)
{
    SDL_Rect rect;
...
#else
        {
            AVEMDictionaryEntry *e = av_dict_get(sws_dict, "sws_flags", NULL, 0);
            if (e) {
                const AVEMClass *class = sws_get_class();
        ...
        if (got_frame) {
                ...
                if (reconfigure) {
                    ...
                    av_log(NULL, AV_LOG_DEBUG,
                           ...
                           frame->sample_rate, av_em_frame_get_channels(frame), av_get_sample_fmt_name(frame->format), buf2, is->auddec.pkt_serial);
            ...
            while ((ret = av_buffersink_get_frame_flags(is->out_audio_filter, frame, 0)) >= 0) {
                ...
                af->duration = av_q2d((AVEMRational){frame->nb_samples, frame->sample_rate});
...
#endif
            duration = (frame_rate.num && frame_rate.den ? av_q2d((AVEMRational){frame_rate.den, frame_rate.num}) : 0);
            pts = (frame->pts == AV_NOPTS_VALUE) ? NAN : frame->pts * av_q2d(tb);
            ret = queue_picture(is, frame, pts, duration, av_em_frame_get_pkt_pos(frame), is->viddec.pkt_serial);
            av_frame_unref(frame);
#if CONFIG_AVFILTER
        }
#endif
...
static int audio_decode_frame(VideoState *is)
{
    ...
    data_size = av_samples_get_buffer_size(NULL, av_em_frame_get_channels(af->frame),
                                           af->frame->nb_samples,
                                           af->frame->format, 1);

    dec_channel_layout =
        (af->frame->channel_layout && av_em_frame_get_channels(af->frame) == av_get_channel_layout_nb_channels(af->frame->channel_layout)) ?
        af->frame->channel_layout : av_get_default_channel_layout(av_em_frame_get_channels(af->frame));
        ...
        if (!is->swr_ctx || swr_init(is->swr_ctx) < 0) {
            av_log(NULL, AV_LOG_ERROR,
                   "Cannot create sample rate converter for conversion of %d Hz %s %d channels to %d Hz %s %d channels!\n",
                    af->frame->sample_rate, av_get_sample_fmt_name(af->frame->format), av_em_frame_get_channels(af->frame),
...
static int stream_has_enough_packets(AVEMStream *st, int stream_id, PacketQueue *queue) {
    return stream_id < 0 ||
           queue->abort_request ||
...
static int read_thread(void *arg)
{
    ...
    err = avformat_em_open_input(&ic, is->filename, is->iformat, &format_opts);
    if (err < 0) {
    ...
    err = avformat_em_find_stream_info(ic, opts);
    ...
    if (ic->pb)
        ic->pb->eof_reached = 0; // FIXME hack, ffplay maybe should not use avio_em_feof() to test for the end
    ...
    if (start_time != AV_NOPTS_VALUE) {
        ...
        ret = avformat_em_seek_file(ic, -1, INT64_MIN, timestamp, INT64_MAX, 0);
        if (ret < 0) {
    ...
    if (show_status)
        av_em_dump_format(ic, 0, is->filename, 0);
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        if (wanted_stream_spec[type] && st_index[type] == -1)
            if (avformat_em_match_stream_specifier(ic, st, wanted_stream_spec[type]) > 0)
                st_index[type] = i;
    }
    ...
    if (!video_disable && !subtitle_disable)
        st_index[AVMEDIA_TYPE_SUBTITLE] =
            av_em_find_best_stream(ic, AVMEDIA_TYPE_SUBTITLE,
                                st_index[AVMEDIA_TYPE_SUBTITLE],
    ...
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
        AVEMStream *st = ic->streams[st_index[AVMEDIA_TYPE_VIDEO]];
        AVEMCodecParameters *codecpar = st->codecpar;
        AVEMRational sar = av_em_guess_sample_aspect_ratio(ic, st, NULL);
        if (codecpar->width)
    ...
    for (;;) {
        ...
        if (is->paused != is->last_paused) {
            ...
            if (is->paused)
                is->read_pause_return = av_em_read_pause(ic);
            else
                av_em_read_play(ic);
        }
...
static VideoState *stream_open(const char *filename, AVEMInputFormat *iformat)
{
    VideoState *is;
...
static void stream_cycle_channel(VideoState *is, int codec_type)
{
    ...
    if (codec_type != AVMEDIA_TYPE_VIDEO && is->video_stream != -1) {
        p = av_em_find_program_from_stream(ic, NULL, is->video_stream);
        if (p) {
...
static void seek_chapter(VideoState *is, int incr)
{
    ...
    for (i = 0; i < is->ic->nb_chapters; i++) {
        AVEMChapter *ch = is->ic->chapters[i];
        ...
        case SDL_KEYDOWN:
            ...
            do_seek:
                    if (seek_by_bytes) {
                        ...
                        if (pos < 0)
                            pos = avio_em_tell(cur_stream->ic->pb);
        ...
        case SDL_MOUSEMOTION:
            ...
            if (event.type == SDL_MOUSEBUTTONDOWN) {
                ...
                if (seek_by_bytes || cur_stream->ic->duration <= 0) {
                    uint64_t size =  avio_em_size(cur_stream->ic->pb);
...
{
    file_iformat = av_em_find_input_format(arg);
    if (!file_iformat) {
