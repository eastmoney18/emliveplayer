...
# armv7, armv7s, arm64(only support)
...
echo "CFG Flag:$FFMPEG_CFG_FLAGS_ARM"
...
echo "===================="
FF_TOOLS_ROOT="$FF_BUILD_ROOT/tools"
echo fftools_root:$FF_TOOLS_ROOT
export PATH="$FF_TOOLS_ROOT/:$PATH"
echo PATH:$PATH
echo "gasp: $FF_TOOLS_ROOT/gas-preprocessor.pl"
...
#--------------------
# with openssl
echo ${FFMPEG_DEP_OPENSSL_LIB}/libssl.a
...
if [ -f "./config.h" ]; then
	...
	sed -ig 's/HAVE_CLOCK_GETTIME\ 1/HAVE_CLOCK_GETTIME\ 0/g' ./config.h
    make clean
fi
