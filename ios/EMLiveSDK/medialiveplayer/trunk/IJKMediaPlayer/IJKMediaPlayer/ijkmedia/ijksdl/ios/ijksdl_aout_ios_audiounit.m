...
static void aout_present_audio_pcm(SDL_Aout *aout, uint8_t *buff, int size, int sr, int channels, int ptsms)
{
    if (!aout)
        return;
    EMMediaPlayer *opaque = (EMMediaPlayer *)aout->caller_opaque;
    if(opaque){
        emmp_present_audio_pcm(opaque, buff, size, sr, channels, ptsms);
    }
}
...
SDL_Aout *SDL_AoutIos_CreateForAudioUnit()
{
    ...
    aout->func_present_audio_pcm = aout_present_audio_pcm;
