...
- (BOOL)setupAudioSpec:(const SDL_AudioSpec *)aSpec
{
    if (aSpec == NULL) {
        return NO;
    }
...
- (id)initWithAudioSpec:(const SDL_AudioSpec *)aSpec
{
    ...
    if (self) {
        ...
        if (![self setupAudioSpec:aSpec]) {
            NSLog(@"setup audiospec failed.");
            self = nil;
            return nil;
        }
...
- (void)play
{
    ...
    @synchronized(_lock) {
        ...
        if (status != noErr) {
            NSLog(@"AudioQueue: AudioQueueStart failed (%d)\n", (int)status);
            [self close];
            if (![self setupAudioSpec:&_spec]) {
                NSLog(@"reset audio queue is also failed....");
                return;
            }
        }
