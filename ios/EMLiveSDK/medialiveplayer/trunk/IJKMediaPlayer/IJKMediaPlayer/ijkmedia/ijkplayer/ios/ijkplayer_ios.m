...
EMMediaPlayer *emmp_ios_create(int (*msg_loop)(void*))
{
...
void emmp_ios_set_glview_l(EMMediaPlayer *mp, IJKSDLGLView *glView)
{
    assert(mp);
    assert(mp->ffplayer);
...
void emmp_ios_set_glview(EMMediaPlayer *mp, IJKSDLGLView *glView)
{
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    emmp_ios_set_glview_l(mp, glView);
    pthread_mutex_unlock(&mp->mutex);
}

bool emmp_ios_is_videotoolbox_open_l(EMMediaPlayer *mp)
{
    assert(mp);
    assert(mp->ffplayer);
...
bool emmp_ios_is_videotoolbox_open(EMMediaPlayer *mp)
{
    assert(mp);
    MPTRACE("%s()\n", __func__);
    pthread_mutex_lock(&mp->mutex);
    bool ret = emmp_ios_is_videotoolbox_open_l(mp);
    pthread_mutex_unlock(&mp->mutex);
    MPTRACE("%s()=%d\n", __func__, ret ? 1 : 0);
    return ret;
}
