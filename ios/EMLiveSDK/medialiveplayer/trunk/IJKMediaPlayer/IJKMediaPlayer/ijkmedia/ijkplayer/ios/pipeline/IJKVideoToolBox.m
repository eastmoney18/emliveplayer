...
struct VideoToolBoxContext {
    ...
    bool                        dealloced;
    int                         m_buffer_deep;
    AVEMPacket                    m_buffer_packet[MAX_PKT_QUEUE_DEEP];
...
static int  vtbformat_init(VTBFormatDesc *fmt_desc, AVEMCodecParameters *codecpar);
...
void QueuePicture(VideoToolBoxContext* ctx) {
    ...
    if (true == GetVTBPicture(ctx, &picture)) {
        AVEMRational tb = ctx->ffp->is->video_st->time_base;
        AVEMRational frame_rate = av_em_guess_frame_rate(ctx->ffp->is->ic, ctx->ffp->is->video_st, NULL);
        double duration = (frame_rate.num && frame_rate.den ? av_em_q2d((AVEMRational) {frame_rate.den, frame_rate.num}) : 0);
        double pts = (picture.pts == AV_NOPTS_VALUE) ? NAN : picture.pts * av_em_q2d(tb);
                       ...
                       CMTime presentationDuration)
{
    @autoreleasepool {
        ...
        if (sample_info && !sample_info->is_decoding) {
        ...
        if (status != 0) {
          ...
          //  ffp_notify_msg1(ffp, FFP_MSG_OPEN_HW_DECODER_FAILED);
            goto failed;
        }
...
#ifdef ENABLE_SORT
        if ((kVTDecodeInfo_FrameDropped & infoFlags) || newFrame->pic.pts > UINT32_MAX) {
            ALOGI("droped pts is %lld\n", newFrame->pic.pts);
            goto failed;
        }
#else
        ...
        // FIXME: duplicated code
        {
            ...
            if (newFrame->pic.pts != AV_NOPTS_VALUE)
                dpts = av_em_q2d(is->video_st->time_base) * newFrame->pic.pts;
...
VTDecompressionSessionRef vtbsession_create(VideoToolBoxContext* context)
{
    ...
    if (ret)
        return NULL;
    assert(context->fmt_desc.fmt_desc);
...
static int find_nalu_prefix(uint8_t *pData)
{
    uint8_t prefix[4] = {0, 0, 0, 1};
    for (int i = 0; i < 4; i++) {
        if (pData[i]  !=  prefix[i]) {
            return i + 1;
        }
...
static int decode_video_internal(VideoToolBoxContext* context, AVEMCodecContext *avctx, const AVEMPacket *avpkt, int* got_picture_ptr)
{
    FFPlayer *ffp                   = context->ffp;
    OSStatus status                 = 0;
    ...
    if (pData)
        ffp->cur_stream_unix_time = check_tx_stream_unix_time(pData);
    if (context->fmt_desc.convert_bytestream) {
        // ALOGI("the buffer should m_convert_byte\n");
        if(avio_em_open_dyn_buf(&pb) < 0) {
            goto failed;
        }
        em_avc_parse_nal_units(pb, pData, iSize);
        demux_size = avio_em_close_dyn_buf(pb, &demux_buff);
        // ALOGI("demux_size:%d\n", demux_size);
        if (demux_size == 0) {
            goto failed;
        }
        ...
        while (nal_start < end) {
            ...
            avio_em_wb32(pb, nal_size);
            nal_start += 3;
            avio_em_write(pb, nal_start, nal_size);
            nal_start += nal_size;
        }
        demux_size = avio_em_close_dyn_buf(pb, &demux_buff);
        sample_buff = CreateSampleBufferFrom(context->fmt_desc.fmt_desc, demux_buff, demux_size);
        ...
        int last_prefix = 0;
        int find_ret = 0;
        if ((pData[4] & 0x1F) != 7 && (pData[4] & 0x1F) != 8) {
            while (i + 4 < iSize) {
                find_ret = find_nalu_prefix(pData + i);
                if (!find_ret) {
                    int newSize = i - last_prefix - 4;
                    pData[last_prefix] = ((newSize >> 24) & 0xFF);
                    pData[last_prefix + 1] = ((newSize >> 16) & 0xFF);
                    pData[last_prefix + 2] = ((newSize >> 8) & 0xFF);
                    pData[last_prefix + 3] = (newSize & 0xFF);
                    newSize = iSize - i - 4;
                    pData[i] = ((newSize >> 24) & 0xFF);
                    pData[i+1] = ((newSize >> 16) & 0xFF);
                    pData[i+2] = ((newSize >> 8) & 0xFF);
                    pData[i+3] = (newSize & 0xFF);
                    last_prefix = i;
                    i++;
                }else {
                    i += find_ret;
    ...
    SDL_SpeedSampler3Add(&ffp->stat.video_bitrate_sampler, dts * av_em_q2d(ffp->is->video_st->time_base) * 1000, iSize);
    *got_picture_ptr = 1;
    return 0;
failed:
    if (sample_buff) {
...
static inline void ResetPktBuffer(VideoToolBoxContext* context) {
    for (int i = 0 ; i < context->m_buffer_deep; i++) {
        av_em_packet_unref(&context->m_buffer_packet[i]);
    }
    context->m_buffer_deep = 0;
    memset(context->m_buffer_packet, 0, sizeof(context->m_buffer_packet));
}

static inline void DuplicatePkt(VideoToolBoxContext* context, const AVEMPacket* pkt) {
    if (context->m_buffer_deep >= MAX_PKT_QUEUE_DEEP) {
        context->idr_based_identified = false;
        ResetPktBuffer(context);
    }
    AVEMPacket* avpkt = &context->m_buffer_packet[context->m_buffer_deep];
    av_em_copy_packet(avpkt, pkt);
    context->m_buffer_deep++;
}



static int decode_video(VideoToolBoxContext* context, AVEMCodecContext *avctx, AVEMPacket *avpkt, int* got_picture_ptr)
{
    int      ret            = 0;
    ...
    if (context->ffp->vtb_handle_resolution_change &&
        ...
        size_data = av_em_packet_get_side_data(avpkt, AV_PKT_DATA_NEW_EXTRADATA, &size_data_size);
        // minimum avcC(sps,pps) = 7
        if (size_data && size_data_size > 7) {
            int             got_picture = 0;
            AVFrame        *frame      = av_em_frame_alloc();
            AVEMDictionary   *codec_opts = NULL;
            AVEMCodecContext *new_avctx  = avcodec_em_alloc_context3(avctx->codec);
            if (!new_avctx)
                return AVERROR(ENOMEM);

            avcodec_em_parameters_to_context(new_avctx, context->codecpar);
            av_em_freep(&new_avctx->extradata);
            new_avctx->extradata = av_em_mallocz(size_data_size + AV_INPUT_BUFFER_PADDING_SIZE);
            if (!new_avctx->extradata)
                return AVERROR(ENOMEM);
            memcpy(new_avctx->extradata, size_data, size_data_size);
            new_avctx->extradata_size = size_data_size;

            av_em_dict_set(&codec_opts, "threads", "1", 0);
            ret = avcodec_em_open2(new_avctx, avctx->codec, &codec_opts);
            av_em_dict_free(&codec_opts);
            if (ret < 0) {
                avcodec_em_free_context(&new_avctx);
                return ret;
            }

            ret = avcodec_em_decode_video2(new_avctx, frame, &got_picture, avpkt);
            if (ret < 0) {
                avcodec_em_free_context(&new_avctx);
                return ret;
            }
            ...
            if (got_picture) {
                if (context->codecpar->width  != new_avctx->width &&
                    ...
                    avcodec_em_parameters_from_context(context->codecpar, new_avctx);
    ...
    if (context->refresh_session) {
        ...
        if ((context->m_buffer_deep > 0) &&
            ...
            for (int i = 0; i < context->m_buffer_deep; i++) {
                AVEMPacket* pkt = &context->m_buffer_packet[i];
...
void videotoolbox_free(VideoToolBoxContext* context)
{
    ...
    avcodec_em_parameters_free(&context->codecpar);
}
...
int videotoolbox_decode_frame(VideoToolBoxContext* context)
{
    ...
    do {
        ...
        if (!d->packet_pending || d->queue->serial != d->pkt_serial) {
            ...
            do {
                ...
                if (ffp_is_flush_packet(&pkt)) {
                    ...
                    if (is->viddec.clear_picture_flushed) {
                        ffp_frame_queue_empty(&is->pictq);
                        is->viddec.clear_picture_flushed = 0;
                    }
                }
            } while (ffp_is_flush_packet(&pkt) || d->queue->serial != d->pkt_serial);

            av_em_packet_split_side_data(&pkt);
        ...
        //printf("decode one video frame, ret:%d, got_frame:%d.\n", ret, got_frame);
        if (ret < 0) {
            d->packet_pending = 0;
        } else {
            d->pkt_temp.dts =
        ...
        case AV_CODEC_ID_H264:
            ...
            if (extradata[0] == 1) {
                ...
                if ((extradata[0] == 0 && extradata[1] == 0 && extradata[2] == 0 && extradata[3] == 1) ||
                    ...
                    em_isom_write_avcc(pb, extradata, extrasize);
                    extradata = NULL;

                    extrasize = avio_em_close_dyn_buf(pb, &extradata);

                    if (!validate_avcC_spc(extradata, extrasize, &fmt_desc->max_ref_frames, &sps_level, &sps_profile)) {
                        av_em_free(extradata);
                        goto fail;
                    }
...
VideoToolBoxContext* videotoolbox_create(FFPlayer* ffp, AVEMCodecContext* avctx)
{
    int ret = 0;
    ...
    context_vtb->codecpar = avcodec_em_parameters_alloc();
    if (!context_vtb->codecpar)
        goto fail;

    ret = avcodec_em_parameters_from_context(context_vtb->codecpar, avctx);
    if (ret)
        goto fail;

    context_vtb->ffp = ffp;
   // context_vtb->idr_based_identified = true; //00 00 00 01 开头的H264数据，现在算法无法正常判断，不能默认true

//    ret = vtbformat_init(&context_vtb->fmt_desc, context_vtb->codecpar); //vtbsession_create 中包含init方法，重复调用存在内存泄漏
//    if (ret)
//        goto fail;
